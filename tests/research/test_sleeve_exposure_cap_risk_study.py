...
def test_job_implementation_closes_current_callable_source() -> None:
    ...
    assert any(path.name == "sleeve_exposure_cap_risk_study.py" for path in paths)
    assert not any(
    ...
    assert any(
        row["path"].endswith("sleeve_exposure_cap_risk_runtime.py")
...
def test_job_plan_accepts_one_verified_family_definition(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "axiom_rift.research.sleeve_exposure_cap_risk_study."
        "build_sleeve_exposure_cap_risk_protocol_definition",
        current_definition,
    )
    plan = build_sleeve_exposure_cap_risk_job_plan(
        repository_root=".",
...
def test_prospective_pair_plan_and_atomic_proofs_validate_end_to_end(
    ...
    scoped = SleeveExposureCapRiskJobPlan(
        mission_id=MISSION_ID,
        study_id="STU-PROSPECTIVE-PAIR",
        executable_id=SUBJECT_ID,
        definition=definition,
