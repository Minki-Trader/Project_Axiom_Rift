...
class StructuralBreakTests(unittest.TestCase):
    ...
    def test_feature_is_prefix_invariant(self)->None:
        rows=1800;time=pd.date_range("2025-01-01",periods=rows,freq="5min");close=20_000+np.cumsum(np.sin(np.arange(rows)/17)+0.1);frame=pd.DataFrame({"time":time,"close":close});full=compute_structural_break_score(frame,"variance_shift_cusum_576");prefix=compute_structural_break_score(frame.iloc[:1600].copy(),"variance_shift_cusum_576")
