...
from axiom_rift.research.volume_price_study import (
    CALLABLE_IDENTITY,
    EVIDENCE_MODES,
    PLANNED_CLAIMS,
...
class VolumePriceStudyPlanTests(unittest.TestCase):
    def setUp(self) -> None:
        executable_ids = sorted(volume_price_executable_configuration_map())
        self.assertEqual(len(executable_ids), 12)
        self.executable_id = executable_ids[0]
        self.plan = build_volume_price_validation_plan(self.executable_id)
    ...
    def test_plan_is_canonical_and_covers_all_claims_and_modes(self) -> None:
        ...
        self.assertEqual(SELECTION_TOTAL_EXPOSURES, 78)
    ...
    def test_production_runner_binds_plan_environment_and_artifact_chain(self) -> None:
        ...
        with TemporaryDirectory() as temporary, patch(
            "axiom_rift.research.volume_price_study.StateWriter.verify_running_job_execution",
            return_value=binding,
        ), patch(
            "axiom_rift.research.volume_price_study._compute_registered_volume_price_surface",
            return_value={"schema": "fixture_surface"},
        ), patch(
            "axiom_rift.research.volume_price_study.project_volume_price_evaluation",
            return_value=evaluation(),
        ):
            packet = execute_volume_price_job(
                repository_root=temporary,
                execution=execution,
            )
            consumer_cases = []
            for index, consumer_executable_id in enumerate(
                sorted(volume_price_executable_configuration_map())[1:],
                start=1,
            ):
                consumer_plan = build_volume_price_validation_plan(
            ...
            with patch(
                ...
                "axiom_rift.research.volume_price_study.StateWriter.verify_reproducible_cache_producer",
                return_value=None,
