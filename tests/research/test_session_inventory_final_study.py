...
from axiom_rift.research.session_inventory_final_study import (
    CALLABLE_IDENTITY,
    EVIDENCE_MODES,
    PLANNED_CLAIMS,
...
class SessionInventoryFinalStudyPlanTests(unittest.TestCase):
    def setUp(self) -> None:
        configuration_map = session_inventory_executable_configuration_map()
        ...
        self.plan = build_session_inventory_validation_plan(self.executable_id)
    ...
    def test_plan_is_canonical_and_covers_all_claims_and_modes(self) -> None:
        ...
        self.assertNotIn("daily_entries_p114_milli", evaluation()["metrics"])
        self.assertEqual(
            CALLABLE_IDENTITY,
            "axiom_rift.research.session_inventory_final_study."
            "execute_session_inventory_job.v1",
        )
        self.assertTrue(
            all(
                name.startswith("scientific/STU-0008/")
                for name in output_names(self.executable_id).values()
            )
        )
        self.assertTrue(
            surface_cache_output_name().startswith("local/cache/STU-0008/")
        )
        self.assertTrue(
            surface_manifest_output_name().startswith("scientific/STU-0008/")
        )
    ...
    def test_production_runner_binds_plan_environment_and_artifact_chain(self) -> None:
        ...
        with TemporaryDirectory() as temporary, patch(
            "axiom_rift.research.session_inventory_final_study.StateWriter.verify_running_job_execution",
            return_value=binding,
        ), patch(
            "axiom_rift.research.session_inventory_final_study._compute_registered_session_inventory_surface",
            return_value={"schema": "fixture_surface"},
        ), patch(
            "axiom_rift.research.session_inventory_final_study.project_session_inventory_evaluation",
            ...
            for index, consumer_executable_id in enumerate(
                sorted(session_inventory_executable_configuration_map())[1:],
                start=1,
            ):
                consumer_plan = build_session_inventory_validation_plan(
            ...
            with patch(
                ...
                "axiom_rift.research.session_inventory_final_study.StateWriter.verify_reproducible_cache_producer",
                return_value=None,
