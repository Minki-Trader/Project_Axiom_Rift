...
from axiom_rift.research.session_inventory_followup_terminal_study import (
    CALLABLE_IDENTITY,
    EVALUATION_SCHEMA,
    EVIDENCE_MODES,
    PLANNED_CLAIMS,
...
class SessionInventoryFollowupTerminalStudyPlanTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_plan_is_canonical_and_covers_all_claims_and_modes(self) -> None:
        ...
        self.assertEqual(SELECTION_TOTAL_EXPOSURES, 162)
        self.assertEqual(SURFACE_SCHEMA, "session_inventory_followup_surface.v1")
        self.assertEqual(
            EVALUATION_SCHEMA,
        ...
        self.assertEqual(
            CALLABLE_IDENTITY,
            "axiom_rift.research.session_inventory_followup_terminal_study."
            "execute_session_inventory_followup_job.v1",
        )
        self.assertTrue(
            all(
                name.startswith("scientific/STU-0012/")
                for name in output_names(self.executable_id).values()
            )
        )
        self.assertTrue(
            surface_cache_output_name().startswith("local/cache/STU-0012/")
        )
        self.assertTrue(
            surface_manifest_output_name().startswith("scientific/STU-0012/")
        )
    ...
    def test_production_runner_binds_plan_environment_and_artifact_chain(self) -> None:
        ...
        with TemporaryDirectory() as temporary, patch(
            "axiom_rift.research.session_inventory_followup_terminal_study.StateWriter.verify_running_job_execution",
            return_value=binding,
        ), patch(
            "axiom_rift.research.session_inventory_followup_terminal_study._compute_registered_session_inventory_followup_surface",
            return_value={"schema": SURFACE_SCHEMA},
        ), patch(
            "axiom_rift.research.session_inventory_followup_terminal_study.project_session_inventory_followup_evaluation",
            ...
            for index, consumer_executable_id in enumerate(
                sorted(session_inventory_followup_executable_configuration_map())[1:],
                start=1,
            ):
                consumer_plan = build_session_inventory_followup_validation_plan(
            ...
            with patch(
                ...
                "axiom_rift.research.session_inventory_followup_terminal_study.StateWriter.verify_reproducible_cache_producer",
                return_value=None,
