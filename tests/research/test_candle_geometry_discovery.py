...
class CandleGeometryTests(unittest.TestCase):
    ...
    def test_geometry_is_prefix_invariant(self)->None:
        rows=160;time=pd.date_range("2025-01-01",periods=rows,freq="5min");base=20_000+np.cumsum(np.sin(np.arange(rows)/7));frame=pd.DataFrame({"time":time,"open":base-0.2,"high":base+1.2,"low":base-1.0,"close":base+0.1});full=compute_candle_score(frame,"wick_balance_12");prefix=compute_candle_score(frame.iloc[:120].copy(),"wick_balance_12")
