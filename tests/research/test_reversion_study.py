...
from axiom_rift.research.reversion_discovery import (
    ...
    reversion_configurations,
    reversion_executable,
    trend_dependency_sha256,
)
from axiom_rift.research.reversion_study import (
    CALLABLE_IDENTITY,
    EVIDENCE_MODES,
    PLANNED_CLAIMS,
    build_measurement,
...
class ReversionStudyPlanTests(unittest.TestCase):
    def setUp(self) -> None:
        self.executable_id = reversion_executable(reversion_configurations()[0]).identity
        self.plan = build_reversion_validation_plan(self.executable_id)
    ...
    def test_plan_is_canonical_and_covers_all_claims_and_modes(self) -> None:
        ...
        self.assertEqual(SELECTION_TOTAL_EXPOSURES, 54)
    ...
    def test_production_runner_binds_plan_environment_and_artifact_chain(self) -> None:
        ...
        with TemporaryDirectory() as temporary, patch(
            "axiom_rift.research.reversion_study.StateWriter.verify_running_job_execution",
            return_value=binding,
        ), patch(
            "axiom_rift.research.reversion_study._compute_registered_reversion_surface",
            return_value={"schema": "fixture_surface"},
        ), patch(
            "axiom_rift.research.reversion_study.project_reversion_evaluation",
            return_value=evaluation(),
        ):
            packet = execute_reversion_job(
            ...
            with patch(
                ...
                "axiom_rift.research.reversion_study.StateWriter.verify_reproducible_cache_producer",
                return_value=None,
            ), patch(
                "axiom_rift.research.reversion_study._compute_registered_reversion_surface",
                side_effect=AssertionError("consumer recomputed the surface"),
            ):
                consumer_packet = execute_reversion_job(
                    repository_root=temporary,
