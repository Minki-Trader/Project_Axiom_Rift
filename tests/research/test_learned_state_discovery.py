...
class LearnedStateTests(unittest.TestCase):
    ...
    def test_fold_model_is_prefix_invariant(self)->None:
        frame=load_observed_development(Path(__file__).resolve().parents[2]).frame;time=pd.to_datetime(frame["time"]);start=time.iloc[500];end=time.iloc[5000];full=fit_fold_model(frame.iloc[:8000].copy(),"ridge_price_control",start,end);prefix=fit_fold_model(frame.iloc[:7000].copy(),"ridge_price_control",start,end)
        import numpy as np
