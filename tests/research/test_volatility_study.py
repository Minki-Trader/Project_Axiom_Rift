...
from axiom_rift.research.volatility_study import (
    CALLABLE_IDENTITY,
    EVIDENCE_MODES,
    PLANNED_CLAIMS,
...
class VolatilityStudyPlanTests(unittest.TestCase):
    def setUp(self) -> None:
        executable_ids = sorted(volatility_executable_configuration_map())
        self.assertEqual(len(executable_ids), 12)
        self.executable_id = executable_ids[0]
        self.plan = build_volatility_validation_plan(self.executable_id)
    ...
    def test_plan_is_canonical_and_covers_all_claims_and_modes(self) -> None:
        ...
        self.assertEqual(SELECTION_TOTAL_EXPOSURES, 66)
    ...
    def test_production_runner_binds_plan_environment_and_artifact_chain(self) -> None:
        ...
        with TemporaryDirectory() as temporary, patch(
            "axiom_rift.research.volatility_study.StateWriter.verify_running_job_execution",
            return_value=binding,
        ), patch(
            "axiom_rift.research.volatility_study._compute_registered_volatility_surface",
            return_value={"schema": "fixture_surface"},
        ), patch(
            "axiom_rift.research.volatility_study.project_volatility_evaluation",
            return_value=evaluation(),
        ):
            packet = execute_volatility_job(
                repository_root=temporary,
                execution=execution,
            )
            consumer_cases = []
            for index, consumer_executable_id in enumerate(
                sorted(volatility_executable_configuration_map())[1:],
                start=1,
            ):
                consumer_plan = build_volatility_validation_plan(
            ...
            with patch(
                ...
                "axiom_rift.research.volatility_study.StateWriter.verify_reproducible_cache_producer",
                return_value=None,
