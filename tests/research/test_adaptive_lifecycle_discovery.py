...
class AdaptiveLifecycleTests(unittest.TestCase):
    ...
    def test_shared_entry_feature_is_prefix_invariant(self)->None:
        rows=1000;time=pd.date_range("2025-01-01",periods=rows,freq="5min");close=20_000+np.cumsum(np.sin(np.arange(rows)/17)+0.1);frame=pd.DataFrame({"time":time,"close":close});full=compute_entry_score(frame,"volatility_barrier_48");control=compute_entry_score(frame,"fixed_hold_control_48");prefix=compute_entry_score(frame.iloc[:900].copy(),"volatility_barrier_48")
