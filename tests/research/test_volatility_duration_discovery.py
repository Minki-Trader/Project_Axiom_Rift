...
class VolatilityDurationTests(unittest.TestCase):
    ...
    def test_features_are_prefix_invariant(self)->None:
        rows=1500;time=pd.date_range("2025-01-01",periods=rows,freq="5min");close=20_000+np.cumsum(np.sin(np.arange(rows)/13)+np.sin(np.arange(rows)/71));frame=pd.DataFrame({"time":time,"close":close});full=compute_volatility_duration_score(frame,"mature_state_age_24_47");prefix=compute_volatility_duration_score(frame.iloc[:1400].copy(),"mature_state_age_24_47")
        for left,right in zip(full,prefix,strict=True):np.testing.assert_allclose(left[:1400],right,rtol=0,atol=0,equal_nan=True)
