...
from axiom_rift.research import cross_asset_relative_strength_discovery as discovery
...
def validator_surface(repository_root: Path) -> dict[str, object]:
    ...
    configuration_map = cross_asset_relative_strength_executable_configuration_map(
        repository_root
    )
...
class CrossAssetRelativeStrengthStudyPlanTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.plan = build_cross_asset_relative_strength_validation_plan(self.executable_id)
    ...
    def test_plan_is_canonical_and_covers_all_claims_and_modes(self) -> None:
        ...
        self.assertEqual(SELECTION_TOTAL_EXPOSURES, 234)
        self.assertEqual(SURFACE_SCHEMA, "cross_asset_relative_strength_surface.v1")
        self.assertEqual(
            EVALUATION_SCHEMA,
            "cross_asset_relative_strength_evaluation.v1",
        )
        self.assertIn("daily_entries_p90_milli", evaluation()["metrics"])
        self.assertNotIn("daily_entries_p234_milli", evaluation()["metrics"])
        self.assertEqual(
            CALLABLE_IDENTITY,
            "axiom_rift.research.cross_asset_relative_strength_study."
            "execute_cross_asset_relative_strength_job.v1",
        )
        self.assertTrue(
            all(
                name.startswith("scientific/STU-0020/")
                for name in output_names(self.executable_id).values()
            )
        )
        self.assertTrue(
            surface_cache_output_name(self.root).startswith(
                "local/cache/STU-0020/"
            )
        )
        self.assertTrue(
            surface_manifest_output_name().startswith("scientific/STU-0020/")
        )
    ...
    def test_production_runner_binds_plan_environment_and_artifact_chain(self) -> None:
        ...
        with patch(
            "axiom_rift.research.cross_asset_relative_strength_study.StateWriter.verify_running_job_execution",
            return_value=binding,
        ), patch(
            "axiom_rift.research.cross_asset_relative_strength_study._compute_registered_cross_asset_relative_strength_surface",
            return_value={"schema": SURFACE_SCHEMA},
        ), patch(
            "axiom_rift.research.cross_asset_relative_strength_study.project_cross_asset_relative_strength_evaluation",
            return_value=evaluation(),
        ):
            packet = execute_cross_asset_relative_strength_job(
            ...
            for index, consumer_executable_id in enumerate(
                ...
                consumer_plan = build_cross_asset_relative_strength_validation_plan(
            ...
            with patch(
                ...
                "axiom_rift.research.cross_asset_relative_strength_study.StateWriter.verify_reproducible_cache_producer",
                return_value=None,
