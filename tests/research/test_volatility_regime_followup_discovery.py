...
from axiom_rift.research import volatility_regime_followup_discovery as followup
...
class VolatilityRegimeFollowupDiscoveryTests(unittest.TestCase):
    def test_surface_has_exact_registered_twelve_and_bound_dependencies(self) -> None:
        configurations = followup.volatility_regime_followup_configurations()
        executables = [followup.volatility_regime_followup_executable(item) for item in configurations]
        ...
        self.assertEqual(
            ...
            [
                "rv24_120_unfiltered-fade-h12",
                "rv24_120_unfiltered-fade-h48",
                "rv24_120_unfiltered-breakout-h12",
                "rv24_120_unfiltered-breakout-h48",
            ],
        )
        for executable in executables:
            self.assertIn(followup.volatility_regime_followup_implementation_sha256(), executable.engine_contract)
            ...
            self.assertIn("bonferroni_198", executable.engine_contract)

    def test_feature_is_prefix_invariant_and_rewarms_after_gap(self) -> None:
        frame = synthetic_frame()
        full, _, _ = followup.compute_volatility_residual_score(frame)
        prefix, _, _ = followup.compute_volatility_residual_score(frame.iloc[:400])
        np.testing.assert_allclose(full[:400], prefix, rtol=0.0, atol=0.0, equal_nan=True)

        gapped = frame.copy()
        gapped.loc[250:, "time"] += pd.Timedelta(hours=2)
        score, _, run = followup.compute_volatility_residual_score(gapped)
        self.assertEqual(run[250], 1)
        self.assertTrue(np.isnan(score[393]))
        self.assertTrue(np.isfinite(score[394]))
    ...
    def test_middle_tertile_and_broker_clock_gates_are_exact(self) -> None:
        ...
        middle = followup.apply_profile_gate(
            ...
            volatility,
            time,
            profile="rv24_120_middle_volatility_train_tertile",
    ...
    def test_registered_control_metrics_are_pairwise_noncompensatory(self) -> None:
        ...
        profile_net = {
            "rv24_120_unfiltered": 100,
            "rv24_120_middle_volatility_train_tertile": 80,
            "rv24_120_broker15_22": 60,
        }
        results = []
        for configuration in followup.volatility_regime_followup_configurations():
            net = profile_net[configuration.profile] + 10 * configuration.signal_sign
            results.append(
                followup._ConfigurationResult(
                    configuration=configuration,
                    executable_id=followup.volatility_regime_followup_executable(configuration).identity,
    ...
    def test_canonical_surface_projects_one_bound_subject(self) -> None:
        ...
        context = []
        for configuration in followup.volatility_regime_followup_configurations():
            identity = followup.volatility_regime_followup_executable(configuration).identity
            evaluations.append(
                {
        ...
        surface = {
            ...
            "discovery_implementation_sha256": followup.volatility_regime_followup_implementation_sha256(),
            ...
            "schema": "volatility_regime_followup_surface.v1", "selection_context": context,
        ...
        projected = followup.project_volatility_regime_followup_evaluation(
            surface,
        ...
        self.assertEqual(projected["schema"], "volatility_regime_followup_evaluation.v1")
