...
from axiom_rift.research.volume_price_followup_study import (
    CALLABLE_IDENTITY,
    EVALUATION_SCHEMA,
    EVIDENCE_MODES,
    PLANNED_CLAIMS,
...
class VolumePriceFollowupStudyPlanTests(unittest.TestCase):
    def setUp(self) -> None:
        configuration_map = volume_price_followup_executable_configuration_map()
        ...
        self.plan = build_volume_price_followup_validation_plan(self.executable_id)
    ...
    def test_plan_is_canonical_and_covers_all_claims_and_modes(self) -> None:
        ...
        self.assertEqual(SELECTION_TOTAL_EXPOSURES, 126)
        self.assertEqual(SURFACE_SCHEMA, "volume_price_followup_surface.v1")
        self.assertEqual(
            EVALUATION_SCHEMA,
            "volume_price_followup_evaluation.v1",
        )
        self.assertIn("daily_entries_p90_milli", evaluation()["metrics"])
        self.assertNotIn("daily_entries_p126_milli", evaluation()["metrics"])
        self.assertEqual(
            CALLABLE_IDENTITY,
            "axiom_rift.research.volume_price_followup_study."
            "execute_volume_price_followup_job.v1",
        )
        self.assertTrue(
            all(
                name.startswith("scientific/STU-0009/")
                for name in output_names(self.executable_id).values()
            )
        )
        self.assertTrue(
            surface_cache_output_name().startswith("local/cache/STU-0009/")
        )
        self.assertTrue(
            surface_manifest_output_name().startswith("scientific/STU-0009/")
        )
    ...
    def test_production_runner_binds_plan_environment_and_artifact_chain(self) -> None:
        ...
        with TemporaryDirectory() as temporary, patch(
            "axiom_rift.research.volume_price_followup_study.StateWriter.verify_running_job_execution",
            return_value=binding,
        ), patch(
            "axiom_rift.research.volume_price_followup_study._compute_registered_volume_price_followup_surface",
            return_value={"schema": SURFACE_SCHEMA},
        ), patch(
            "axiom_rift.research.volume_price_followup_study.project_volume_price_followup_evaluation",
            return_value=evaluation(),
        ):
            packet = execute_volume_price_followup_job(
                repository_root=temporary,
                execution=execution,
            )
            consumer_cases = []
            for index, consumer_executable_id in enumerate(
                sorted(volume_price_followup_executable_configuration_map())[1:],
                start=1,
            ):
                consumer_plan = build_volume_price_followup_validation_plan(
            ...
            with patch(
                ...
                "axiom_rift.research.volume_price_followup_study.StateWriter.verify_reproducible_cache_producer",
                return_value=None,
