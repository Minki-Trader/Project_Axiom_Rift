...
class AnalogStateTests(unittest.TestCase):
    ...
    def test_fold_analog_is_prefix_invariant(self)->None:
        frame=load_observed_development(Path(__file__).resolve().parents[2]).frame;time=pd.to_datetime(frame["time"]);start=time.iloc[500];end=time.iloc[10000];full=fit_fold_analog(frame.iloc[:15000].copy(),"knn_return_control_25",start,end);prefix=fit_fold_analog(frame.iloc[:14000].copy(),"knn_return_control_25",start,end)
