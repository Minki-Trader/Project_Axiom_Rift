...
from axiom_rift.research.usdjpy_source import source_validation_plan_hash
from axiom_rift.research.usdjpy_source_study import (
...
class USDJPYSourceStudyTests(unittest.TestCase):
    def test_two_jobs_have_distinct_callables_outputs_and_plans(self) -> None:
        ...
        self.assertTrue(
            all(name.startswith("source/usdjpy/") for name in historical.values())
        )
        self.assertTrue(
            all(name.startswith("source/usdjpy/") for name in runtime.values())
        )
        self.assertNotEqual(
