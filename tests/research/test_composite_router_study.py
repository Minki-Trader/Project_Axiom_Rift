...
from axiom_rift.research.composite_router_study import (
    CALLABLE_IDENTITY,
    EVALUATION_SCHEMA,
    EVIDENCE_MODES,
    PLANNED_CLAIMS,
...
class CompositeRouterStudyPlanTests(unittest.TestCase):
    def setUp(self) -> None:
        configuration_map = composite_router_executable_configuration_map()
        ...
        self.plan = build_composite_router_validation_plan(self.executable_id)
    ...
    def test_plan_is_canonical_and_covers_all_claims_and_modes(self) -> None:
        ...
        self.assertEqual(SELECTION_TOTAL_EXPOSURES, 210)
        self.assertEqual(SURFACE_SCHEMA, "composite_router_surface.v1")
        self.assertEqual(
            EVALUATION_SCHEMA,
            "composite_router_evaluation.v1",
        )
        self.assertIn("daily_entries_p90_milli", evaluation()["metrics"])
        self.assertNotIn("daily_entries_p210_milli", evaluation()["metrics"])
        self.assertEqual(
            CALLABLE_IDENTITY,
            "axiom_rift.research.composite_router_study."
            "execute_composite_router_job.v1",
        )
        self.assertTrue(
            all(
                name.startswith("scientific/STU-0016/")
                for name in output_names(self.executable_id).values()
            )
        )
        self.assertTrue(
            surface_cache_output_name().startswith("local/cache/STU-0016/")
        )
        self.assertTrue(
            surface_manifest_output_name().startswith("scientific/STU-0016/")
        )
    ...
    def test_production_runner_binds_plan_environment_and_artifact_chain(self) -> None:
        ...
        with TemporaryDirectory() as temporary, patch(
            "axiom_rift.research.composite_router_study.StateWriter.verify_running_job_execution",
            return_value=binding,
        ), patch(
            "axiom_rift.research.composite_router_study._compute_registered_composite_router_surface",
            return_value={"schema": SURFACE_SCHEMA},
        ), patch(
            "axiom_rift.research.composite_router_study.project_composite_router_evaluation",
            return_value=evaluation(),
        ):
            packet = execute_composite_router_job(
                repository_root=temporary,
                execution=execution,
            )
            consumer_cases = []
            for index, consumer_executable_id in enumerate(
                sorted(composite_router_executable_configuration_map())[1:],
                start=1,
            ):
                consumer_plan = build_composite_router_validation_plan(
            ...
            with patch(
                ...
                "axiom_rift.research.composite_router_study.StateWriter.verify_reproducible_cache_producer",
                return_value=None,
