...
from axiom_rift.research.us30_source import source_validation_plan_hash
...
class US30SourceStudyTests(unittest.TestCase):
    def test_two_jobs_have_distinct_callables_outputs_and_plans(self) -> None:
        ...
        self.assertTrue(all(name.startswith("source/us30/") for name in historical.values()))
        self.assertTrue(all(name.startswith("source/us30/") for name in runtime.values()))
        self.assertNotEqual(
