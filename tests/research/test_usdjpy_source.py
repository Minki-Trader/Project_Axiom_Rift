...
def runtime_probe() -> dict[str, object]:
    value = {
        ...
        "market_clock_offset_seconds": 0,
        "market_clock_coherent": True,
        "latest_closed_bar_utc": "2026-07-13T00:00:00Z",
        "retrieval_latency_ms": 5,
        "dtype_fields": list(USDJPY_COLUMNS),
    }
...
class USDJPYSourceTests(unittest.TestCase):
    def test_completed_bars_use_the_mt5_market_clock_coordinate(self) -> None:
        ...
        completed = _completed_rate_epochs(epochs, market_epoch_seconds=12001)
        self.assertEqual(completed.tolist(), [10800, 11100, 11400, 11700])

    def test_contract_binds_exact_spot_fx_surface_and_plans(self) -> None:
        contract = usdjpy_source_contract()
        self.assertEqual(contract.runtime_identifier, "USDJPY")
        self.assertEqual(contract.mapping()["runtime_symbol"], "USDJPY")
        self.assertEqual(contract.instrument()["asset_type"], "spot_fx")
        self.assertEqual(contract.instrument()["roll"], "spot_fx_no_contract_roll")
        self.assertEqual(contract.schema()["columns"], list(USDJPY_COLUMNS))
        self.assertNotEqual(
    ...
    def test_small_historical_fixture_is_audited_but_not_eligible(self) -> None:
        raw = (
            ",".join(USDJPY_COLUMNS)
            + "\n2018.05.07 01:00:00,109.100,109.110,109.090,109.105,10,1,0"
            + "\n2026.06.26 23:50:00,159.100,159.110,159.090,159.105,12,1,0\n"
        ).encode("ascii")
        measurement = audit_usdjpy_historical_bytes(
            raw, observed_at_utc="2026-07-13T00:00:00Z"
        )
    ...
    def test_runtime_validator_derives_every_fact_and_reads_every_artifact(self) -> None:
        ...
        with TemporaryDirectory() as temporary:
            ...
            request = EvidenceValidationRequest(
                ...
                mission_id="MIS-0006",
                evidence_subject={"kind": "Study", "id": "STU-0102"},
                binding={
