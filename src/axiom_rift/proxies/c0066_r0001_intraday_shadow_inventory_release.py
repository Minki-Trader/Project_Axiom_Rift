"""C0066 R0001 proxy evidence for intraday shadow inventory release."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0066_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0066_r0001_shadow_inventory_summary.json"
...
FEATURE_NAMES = (
    "favorable_shadow_pressure",
    "adverse_shadow_pressure",
    "shadow_imbalance_pressure",
    "shadow_cluster_density",
    "current_shadow_rejection",
    "body_follow_through",
    "close_location_after_shadow",
    "range_extreme_release",
    "volume_shadow_confirmation",
    "prior_day_shadow_contrast",
    "session_maturity_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_shadow_inventory_release_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_shadow_inventory_release_state"
SELECTION_RULE = "top_fold_local_shadow_inventory_release_scores_per_active_day"
...
MIN_DAY_BARS = 30
SHADOW_INVENTORY_LOOKBACK_BARS = 14
SHADOW_INVENTORY_SHORT_LOOKBACK_BARS = 6
MIN_SHADOW_INVENTORY_ACTIVITY = 0.10
...
class LinearShadowInventoryReleaseModel:
    fold_id: str
    ...
    shadow_inventory_release_failure_rate: float
...
def run_c0066_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_shadow_inventory_release_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_shadow_inventory_release_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        SHADOW_INVENTORY_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        shadow["favorable_shadow_pressure"],
        shadow["adverse_shadow_pressure"],
        shadow["shadow_imbalance_pressure"],
        shadow["shadow_cluster_density"],
        shadow["current_shadow_rejection"],
        shadow["body_follow_through"],
        shadow["close_location_after_shadow"],
        shadow["range_extreme_release"],
        shadow["volume_shadow_confirmation"],
        shadow["prior_day_shadow_contrast"],
        shadow["session_maturity_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        shadow_pressure_bucket(shadow["favorable_shadow_pressure"], "favorable"),
        shadow_pressure_bucket(shadow["adverse_shadow_pressure"], "adverse"),
        shadow_imbalance_bucket(shadow["shadow_imbalance_pressure"]),
        shadow_cluster_bucket(shadow["shadow_cluster_density"]),
        shadow_rejection_bucket(shadow["current_shadow_rejection"]),
        body_follow_bucket(shadow["body_follow_through"]),
        shadow_settlement_bucket(shadow["close_location_after_shadow"]),
        range_extreme_release_bucket(shadow["range_extreme_release"]),
        volume_confirmation_bucket(shadow["volume_shadow_confirmation"]),
        prior_shadow_contrast_bucket(shadow["prior_day_shadow_contrast"]),
        session_pressure_bucket(shadow["session_maturity_pressure"]),
...
def shadow_inventory_release_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    window_start = index - SHADOW_INVENTORY_LOOKBACK_BARS + 1
    short_start = index - SHADOW_INVENTORY_SHORT_LOOKBACK_BARS + 1
    ...
    window_width = max(window_high - window_low, 1e-9)
    favorable_values: list[float] = []
    adverse_values: list[float] = []
    cluster_hits = 0
    for item in window:
        upper_shadow, lower_shadow = wick_lengths(item)
        item_range = max(item.high - item.low, 1e-9)
        favorable_shadow = lower_shadow if direction > 0 else upper_shadow
        adverse_shadow = upper_shadow if direction > 0 else lower_shadow
        favorable_ratio = favorable_shadow / item_range
        adverse_ratio = adverse_shadow / item_range
        favorable_values.append(favorable_ratio)
        adverse_values.append(adverse_ratio)
        if favorable_ratio > 0.34 and favorable_ratio > adverse_ratio:
            cluster_hits += 1
    favorable_shadow_pressure = base.mean(favorable_values) or 0.0
    adverse_shadow_pressure = base.mean(adverse_values) or 0.0
    shadow_imbalance_pressure = favorable_shadow_pressure - adverse_shadow_pressure
    shadow_cluster_density = cluster_hits / len(window)
    current_shadow_rejection = directional_wick_rejection(signal_bar, direction)
    short_body = sum(direction * (item.close - item.open) for item in short_window)
    body_follow_through = short_body / max(average_range * len(short_window), 1e-9)
    close_location_after_shadow = directional_bar_settlement(signal_bar, direction)
    if direction > 0:
        range_extreme_release = (signal_bar.close - window_low) / window_width
    else:
        range_extreme_release = (window_high - signal_bar.close) / window_width
    ...
    for item, volume in zip(short_window, volume_window):
        ...
        favorable_shadow = lower_shadow if direction > 0 else upper_shadow
        adverse_shadow = upper_shadow if direction > 0 else lower_shadow
        shadow_bias = (favorable_shadow - adverse_shadow) / item_range
        if shadow_bias > 0.08:
            directional_volume += volume
        elif shadow_bias < -0.08:
            opposing_volume += volume
    expected_recent_volume = prior_volume_per_bar * len(short_window)
    volume_shadow_confirmation = (directional_volume - opposing_volume) / max(expected_recent_volume, 1e-9)
    if context.prior_day_shadow_balance is None:
        prior_day_shadow_contrast = 0.0
    else:
        prior_day_shadow_contrast = context.prior_day_shadow_balance if direction > 0 else -context.prior_day_shadow_balance
    session_maturity_pressure = bounded_session_progress(signal_bar.time) * max(
        favorable_shadow_pressure,
        max(shadow_imbalance_pressure, 0.0),
        shadow_cluster_density,
        max(current_shadow_rejection, 0.0),
        close_location_after_shadow,
        max(volume_shadow_confirmation, 0.0),
    )
    activity = max(
        favorable_shadow_pressure,
        adverse_shadow_pressure,
        abs(shadow_imbalance_pressure),
        shadow_cluster_density,
        abs(current_shadow_rejection),
        abs(body_follow_through),
        close_location_after_shadow,
        range_extreme_release,
        abs(volume_shadow_confirmation),
        abs(prior_day_shadow_contrast),
        session_maturity_pressure,
    )
    if activity < MIN_SHADOW_INVENTORY_ACTIVITY:
        return {"event_count": 0.0}
    if (
        shadow_cluster_density < 0.12
        and shadow_imbalance_pressure < 0.06
        and current_shadow_rejection < 0.08
        and favorable_shadow_pressure < 0.22
    ):
        return {"event_count": 0.0}
    return {
        "favorable_shadow_pressure": bounded(favorable_shadow_pressure, 0.0, 1.0),
        "adverse_shadow_pressure": bounded(adverse_shadow_pressure, 0.0, 1.0),
        "shadow_imbalance_pressure": bounded(shadow_imbalance_pressure, -1.0, 1.0),
        "shadow_cluster_density": bounded(shadow_cluster_density, 0.0, 1.0),
        "current_shadow_rejection": bounded(current_shadow_rejection, -1.0, 1.0),
        "body_follow_through": bounded(body_follow_through, -4.0, 4.0),
        "close_location_after_shadow": bounded(close_location_after_shadow, 0.0, 1.0),
        "range_extreme_release": bounded(range_extreme_release, 0.0, 1.0),
        "volume_shadow_confirmation": bounded(volume_shadow_confirmation, -4.0, 4.0),
        "prior_day_shadow_contrast": bounded(prior_day_shadow_contrast, -2.0, 2.0),
...
def candidate_label(
    ...
    metrics = shadow_inventory_release_metrics(
        bars,
        volumes,
    ...
    if metrics["event_count"] <= 0:
        return None
    shadow_reward = 0.22 if metrics["shadow_imbalance_pressure"] > 0.08 and terminal > 0 else 0.0
    cluster_reward = 0.14 if metrics["shadow_cluster_density"] > 0.18 and terminal > 0 else 0.0
    rejection_reward = 0.18 if metrics["current_shadow_rejection"] > 0.14 and terminal > 0 else 0.0
    body_reward = 0.12 if metrics["body_follow_through"] > 0.12 and terminal > 0 else 0.0
    settlement_reward = 0.12 if metrics["close_location_after_shadow"] > 0.62 and terminal > 0 else 0.0
    release_reward = 0.10 if metrics["range_extreme_release"] > 0.58 and terminal > 0 else 0.0
    volume_reward = 0.10 if metrics["volume_shadow_confirmation"] > 0.18 and terminal > 0 else 0.0
    prior_contrast_reward = 0.06 if metrics["prior_day_shadow_contrast"] > 0.18 and terminal > 0 else 0.0
    failed_shadow_penalty = 0.24 if metrics["shadow_imbalance_pressure"] < 0.02 and terminal <= 0 else 0.0
    adverse_shadow_penalty = (
        0.20
        if metrics["adverse_shadow_pressure"] > metrics["favorable_shadow_pressure"] and terminal <= 0
        else 0.0
    )
    weak_rejection_penalty = 0.18 if metrics["current_shadow_rejection"] < -0.08 and terminal <= 0 else 0.0
    failed_settlement_penalty = 0.18 if metrics["close_location_after_shadow"] < 0.42 and terminal <= 0 else 0.0
    chase_penalty = 0.14 if metrics["range_extreme_release"] > 0.92 and terminal <= 0 else 0.0
    ...
    shadow_path_quality = (0.44 * terminal + 0.36 * mfe - 0.72 * mae) / average_range
...
def fit_linear_shadow_inventory_release_model(candidates: list[base.Candidate], fold_id: str) -> LinearShadowInventoryReleaseModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearShadowInventoryReleaseModel(
            fold_id,
    ...
    shadow_inventory_release_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearShadowInventoryReleaseModel(
        fold_id=fold_id,
        ...
        shadow_inventory_release_failure_rate=shadow_inventory_release_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearShadowInventoryReleaseModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_shadow_inventory_release_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0066_r0001_intraday_shadow_inventory_release",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0066_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0066_r0001_shadow_inventory_summary.json",
        ],
        ...
        "conditional_profiles": {
            "shadow_inventory_release_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0066 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0066_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0066 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "shadow_inventory_release_lookback_bars": SHADOW_INVENTORY_LOOKBACK_BARS,
        "shadow_inventory_release_short_lookback_bars": SHADOW_INVENTORY_SHORT_LOOKBACK_BARS,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_shadow_inventory_release_model_summary(model: LinearShadowInventoryReleaseModel) -> dict[str, object]:
    return {
        ...
        "shadow_inventory_release_failure_rate": rounded(model.shadow_inventory_release_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearShadowInventoryReleaseModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_shadow_inventory_release_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "shadow_inventory_release_profile": profiles["shadow_inventory_release_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0066_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0066_r0001_proxy_trades.csv"
    evidence["shadow_inventory_release_summary"] = "artifacts/c0066_r0001_shadow_inventory_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0066_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0066_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0066 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0066_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0066_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0066_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0066_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0066_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0066_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0066_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0066_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0066_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0066 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0066 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260706_produce_c0066_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0066_r0001_proxy_evidence
  refines:
  - dec_20260706_open_c0066_r0001_intraday_shadow_inventory_release_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0066_r0001_proxy_records_shadow_inventory_release_reference_surface
  ...
  - next_work_is_produce_c0066_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260706_produce_c0066_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0066-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0066_r0001_intraday_shadow_inventory_release",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0066-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0066-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0066-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0066_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0066_r0001_intraday_shadow_inventory_release",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0066-R0001-shadow-inventory-release-SUMMARY",
            "artifact_role": "shadow_inventory_release_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0066_r0001_shadow_inventory_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0066_r0001_intraday_shadow_inventory_release",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0066_r0001_mt5_logic_parity_evidence",
        }
    ]
