"""C0113 R0001 proxy evidence for intraday session percentile transition."""
...
CAMPAIGN_ID = "C0113"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0113_intraday_session_percentile_transition_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0113_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0113_r0001_session_percentile_transition_summary.json"
...
FEATURE_NAMES = (
    "position_percentile",
    "percentile_velocity",
    "percentile_acceleration",
    "transition_smoothness",
    "dwell_balance",
    "range_growth_efficiency",
    "volume_confirmation",
    "close_location_confirm",
    "vwap_side_confirm",
    "prior_day_position_context",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_session_percentile_transition_rank"
LABEL_SHAPE = "target_first_quality_after_session_percentile_transition"
SELECTION_RULE = "top_fold_local_session_percentile_transition_scores_per_active_day"
...
MIN_DAY_BARS = 18
SESSION_PERCENTILE_TRANSITION_LOOKBACK_BARS = 42
SESSION_PERCENTILE_RECENT_BARS = 8
SESSION_PERCENTILE_BASE_BARS = 14
MIN_PERCENTILE_VELOCITY = 0.08
MIN_SESSION_PERCENTILE_TRANSITION_ACTIVITY = 0.045
...
class LinearSessionPercentileTransitionModel:
    fold_id: str
    ...
    session_percentile_transition_failure_rate: float
...
def run_c0113_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_session_percentile_transition_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_session_percentile_transition_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        SESSION_PERCENTILE_TRANSITION_LOOKBACK_BARS,
        SESSION_PERCENTILE_RECENT_BARS + SESSION_PERCENTILE_BASE_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = session_percentile_transition_metrics(
    ...
    features = (
        metrics["position_percentile"],
        metrics["percentile_velocity"],
        metrics["percentile_acceleration"],
        metrics["transition_smoothness"],
        metrics["dwell_balance"],
        metrics["range_growth_efficiency"],
        metrics["volume_confirmation"],
        metrics["close_location_confirm"],
        metrics["vwap_side_confirm"],
        metrics["prior_day_position_context"],
        metrics["session_maturity"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        position_percentile_bucket(metrics["position_percentile"]),
        percentile_velocity_bucket(metrics["percentile_velocity"]),
        percentile_acceleration_bucket(metrics["percentile_acceleration"]),
        transition_smoothness_bucket(metrics["transition_smoothness"]),
        dwell_balance_bucket(metrics["dwell_balance"]),
        range_growth_efficiency_bucket(metrics["range_growth_efficiency"]),
        volume_confirmation_bucket(metrics["volume_confirmation"]),
        close_location_confirm_bucket(metrics["close_location_confirm"]),
        vwap_side_confirm_bucket(metrics["vwap_side_confirm"]),
        prior_day_position_context_bucket(metrics["prior_day_position_context"]),
...
def session_percentile_transition_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    recent_start = max(index - SESSION_PERCENTILE_RECENT_BARS + 1, context.day_start_index)
    base_start = max(recent_start - SESSION_PERCENTILE_BASE_BARS, context.day_start_index)
    lookback_start = max(index - SESSION_PERCENTILE_TRANSITION_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    range_before_recent = historical_day_width(bars, context, index, SESSION_PERCENTILE_RECENT_BARS)
    day_low = min(bar.low for bar in day_window)
    day_high = max(bar.high for bar in day_window)
    range_width = max(day_high - day_low, average_range * 0.25)
    base_positions = [
        directional_session_percentile_position(bar.close, day_low, day_high, direction) for bar in base_window
    ]
    recent_positions = [
        directional_session_percentile_position(bar.close, day_low, day_high, direction) for bar in recent_window
    ]
    if not base_positions or not recent_positions:
        return {"event_count": 0.0}
    current_position = directional_session_percentile_position(signal_bar.close, day_low, day_high, direction)
    base_position = base.mean(base_positions) or current_position
    recent_position = base.mean(recent_positions[:-1] or recent_positions) or current_position
    percentile_velocity = current_position - base_position
    percentile_acceleration = current_position - recent_position
    if percentile_velocity < MIN_PERCENTILE_VELOCITY:
        return {"event_count": 0.0}
    ...
    directional_net = direction * (close_path[-1] - close_path[0])
    transition_smoothness = directional_net / max(close_travel, average_range * 0.10)
    if transition_smoothness < -0.10:
        return {"event_count": 0.0}

    dwell_threshold = 0.58
    dwell_balance = sum(1 for value in recent_positions if value >= dwell_threshold) / max(len(recent_positions), 1)
    range_growth = (day_range_so_far - (range_before_recent or day_range_so_far)) / max(average_range, 1e-9)
    range_growth_efficiency = range_growth / max(recent_range_mean / max(average_range, 1e-9), 0.25)
    volume_confirmation = recent_volume_mean / max(base_volume_mean, average_volume * 0.25) - 1.0
    close_location_confirm = directional_bar_settlement(signal_bar, direction)
    if close_location_confirm < 0.34:
        return {"event_count": 0.0}
    vwap_side_confirm = direction * (
        signal_bar.close - session_vwap_until(bars, volumes, context.day_start_index, index)
    ) / average_range
    prior_day_position_context = 0.0
    if context.prior_day_close is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_day_position_context = direction * (signal_bar.close - context.prior_day_close) / context.prior_day_range
        prior_day_position_context += direction * (context.prior_day_shadow_balance or 0.0) * 0.08
    ...
    activity = max(
        max(percentile_velocity, 0.0) * 0.70,
        max(percentile_acceleration, 0.0) * 0.42,
        max(transition_smoothness, 0.0) * 0.35,
        max(dwell_balance - 0.25, 0.0) * 0.32,
        max(range_growth_efficiency, 0.0) * 0.20,
        max(volume_confirmation, 0.0) * 0.15,
        max(close_location_confirm - 0.34, 0.0) * 0.34,
        max(vwap_side_confirm, 0.0) * 0.18,
    )
    if activity < MIN_SESSION_PERCENTILE_TRANSITION_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "position_percentile": bounded(current_position, 0.0, 1.0),
        "percentile_velocity": bounded(percentile_velocity, -1.0, 1.0),
        "percentile_acceleration": bounded(percentile_acceleration, -1.0, 1.0),
        "transition_smoothness": bounded(transition_smoothness, -1.0, 1.0),
        "dwell_balance": bounded(dwell_balance, 0.0, 1.0),
        "range_growth_efficiency": bounded(range_growth_efficiency, -4.0, 4.0),
        "volume_confirmation": bounded(volume_confirmation, -4.0, 6.0),
        "close_location_confirm": bounded(close_location_confirm, 0.0, 1.0),
        "vwap_side_confirm": bounded(vwap_side_confirm, -4.0, 4.0),
        "prior_day_position_context": bounded(prior_day_position_context, -4.0, 4.0),
        ...
        "session_percentile_transition_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0113_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0113_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0113_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0113_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0113_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0113_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0113_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0113_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0113_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0113_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0113_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0113_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0113_boundary_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0113_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0113_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0113_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0113_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0113_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0113_prior_boundary_bucket(value: float) -> str:
    if value < -0.12:
...
def c0113_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    position_reward = 0.10 if metrics["position_percentile"] > 0.62 and terminal > 0 else 0.0
    velocity_reward = 0.12 if metrics["percentile_velocity"] > 0.18 and terminal > 0 else 0.0
    acceleration_reward = 0.08 if metrics["percentile_acceleration"] > 0.06 and terminal > 0 else 0.0
    smoothness_reward = 0.07 if metrics["transition_smoothness"] > 0.24 and terminal > 0 else 0.0
    dwell_reward = 0.06 if metrics["dwell_balance"] > 0.45 and terminal > 0 else 0.0
    range_growth_reward = 0.05 if metrics["range_growth_efficiency"] > 0.15 and terminal > 0 else 0.0
    volume_reward = 0.04 if metrics["volume_confirmation"] > 0.10 and terminal > 0 else 0.0
    close_reward = 0.08 if metrics["close_location_confirm"] > 0.62 and terminal > 0 else 0.0
    vwap_reward = 0.06 if metrics["vwap_side_confirm"] > 0.02 and terminal > 0 else 0.0
    prior_context_reward = 0.04 if metrics["prior_day_position_context"] > 0.15 and terminal > 0 else 0.0
    weak_velocity_penalty = 0.10 if metrics["percentile_velocity"] < 0.10 and terminal <= 0 else 0.0
    acceleration_penalty = 0.08 if metrics["percentile_acceleration"] < -0.04 and terminal <= 0 else 0.0
    smoothness_penalty = 0.08 if metrics["transition_smoothness"] < 0.05 and terminal <= 0 else 0.0
    position_penalty = 0.07 if metrics["position_percentile"] < 0.48 and terminal <= 0 else 0.0
    dwell_penalty = 0.06 if metrics["dwell_balance"] < 0.25 and terminal <= 0 else 0.0
    close_penalty = 0.07 if metrics["close_location_confirm"] < 0.50 and terminal <= 0 else 0.0
    vwap_penalty = 0.07 if metrics["vwap_side_confirm"] < -0.10 and terminal <= 0 else 0.0
    ...
    session_percentile_transition_path_quality = (0.62 * terminal + 0.28 * mfe - 0.65 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + session_percentile_transition_path_quality
        + position_reward
        + velocity_reward
        + acceleration_reward
        + smoothness_reward
        ...
        - max(0.0, 0.12 - metrics["percentile_velocity"]) * 0.05
    )


def fit_linear_session_percentile_transition_model(candidates: list[base.Candidate], fold_id: str) -> LinearSessionPercentileTransitionModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearSessionPercentileTransitionModel(
            fold_id,
    ...
    session_percentile_transition_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearSessionPercentileTransitionModel(
        fold_id=fold_id,
        ...
        session_percentile_transition_failure_rate=session_percentile_transition_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearSessionPercentileTransitionModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0113-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_session_percentile_transition_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0113_r0001_intraday_session_percentile_transition",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0113_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0113_r0001_session_percentile_transition_summary.json",
        ],
        ...
        "conditional_profiles": {
            "session_percentile_transition_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0113 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0113_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0113 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "session_percentile_transition_lookback_bars": SESSION_PERCENTILE_TRANSITION_LOOKBACK_BARS,
        "session_percentile_transition_recent_bars": SESSION_PERCENTILE_RECENT_BARS,
        "session_percentile_transition_prior_bars": SESSION_PERCENTILE_BASE_BARS,
        "min_percentile_velocity": MIN_PERCENTILE_VELOCITY,
        "min_session_percentile_transition_activity": MIN_SESSION_PERCENTILE_TRANSITION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_session_percentile_transition_model_summary(model: LinearSessionPercentileTransitionModel) -> dict[str, object]:
    return {
        ...
        "session_percentile_transition_failure_rate": rounded(model.session_percentile_transition_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearSessionPercentileTransitionModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_session_percentile_transition_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "session_percentile_transition_profile": profiles["session_percentile_transition_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0113_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0113_r0001_proxy_trades.csv"
    evidence["session_percentile_transition_summary"] = "artifacts/c0113_r0001_session_percentile_transition_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0113_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0113_r0001_proxy_trades.csv",
        "artifacts/c0113_r0001_session_percentile_transition_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0113_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0113 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0113_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0113_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0113_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0113_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0113_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0113_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0113_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0113_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0113_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0113 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0113 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0113_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0113_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0113_r0001_intraday_session_percentile_transition_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0113_r0001_proxy_records_session_percentile_transition_reference_surface
  ...
  - next_work_is_produce_c0113_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0113_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0113-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0113_r0001_intraday_session_percentile_transition",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0113-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0113-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0113-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0113_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0113_r0001_intraday_session_percentile_transition",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0113-R0001-SESSION-PERCENTILE-TRANSITION-SUMMARY",
            "artifact_role": "session_percentile_transition_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0113_r0001_session_percentile_transition_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0113_r0001_intraday_session_percentile_transition",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0113_r0001_mt5_logic_parity_evidence",
        }
    ]
