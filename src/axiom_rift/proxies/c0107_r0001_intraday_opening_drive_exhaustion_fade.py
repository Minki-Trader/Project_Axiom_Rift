"""C0107 R0001 proxy evidence for intraday opening drive exhaustion fade."""
...
CAMPAIGN_ID = "C0107"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0107_intraday_opening_drive_exhaustion_fade_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0107_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0107_r0001_opening_drive_exhaustion_fade_summary.json"
...
FEATURE_NAMES = (
    "opening_drive_extension",
    "opening_drive_efficiency_loss",
    "opening_drive_volume_decay",
    "opening_extreme_position",
    "vwap_reversion_pressure",
    "counter_body_turn",
    "adverse_wick_absorption",
    "reclaim_against_drive_strength",
    "prior_day_contrarian_context",
    "fade_alignment",
    "session_phase_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_opening_drive_exhaustion_fade_rank"
LABEL_SHAPE = "target_first_quality_after_opening_drive_exhaustion_fade"
SELECTION_RULE = "top_fold_local_opening_drive_exhaustion_fade_scores_per_active_day"
...
MIN_DAY_BARS = 30
OPENING_DRIVE_EXHAUSTION_FADE_LOOKBACK_BARS = 54
OPENING_DRIVE_EXHAUSTION_FADE_RECENT_BARS = 10
OPENING_DRIVE_EXHAUSTION_FADE_PRIOR_BARS = 30
MIN_OPENING_DRIVE_EXHAUSTION_FADE_ACTIVITY = 0.05
...
class LinearOpeningDriveExhaustionFadeModel:
    fold_id: str
    ...
    opening_drive_exhaustion_fade_failure_rate: float
...
def run_c0107_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_opening_drive_exhaustion_fade_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_opening_drive_exhaustion_fade_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        OPENING_DRIVE_EXHAUSTION_FADE_LOOKBACK_BARS,
        OPENING_DRIVE_EXHAUSTION_FADE_RECENT_BARS + OPENING_DRIVE_EXHAUSTION_FADE_PRIOR_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = opening_drive_exhaustion_fade_metrics(
    ...
    features = (
        metrics["opening_drive_extension"],
        metrics["opening_drive_efficiency_loss"],
        metrics["opening_drive_volume_decay"],
        metrics["opening_extreme_position"],
        metrics["vwap_reversion_pressure"],
        metrics["counter_body_turn"],
        metrics["adverse_wick_absorption"],
        metrics["reclaim_against_drive_strength"],
        metrics["prior_day_contrarian_context"],
        metrics["fade_alignment"],
        metrics["session_phase_maturity"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        opening_drive_extension_bucket(metrics["opening_drive_extension"]),
        path_efficiency_loss_bucket(metrics["opening_drive_efficiency_loss"]),
        participation_decay_bucket(metrics["opening_drive_volume_decay"]),
        opening_extreme_position_bucket(metrics["opening_extreme_position"]),
        vwap_reversion_depth_bucket(metrics["vwap_reversion_pressure"]),
        counter_body_turn_bucket(metrics["counter_body_turn"]),
        adverse_absorption_bucket(metrics["adverse_wick_absorption"]),
        reclaim_against_drive_bucket(metrics["reclaim_against_drive_strength"]),
        c0107_prior_boundary_bucket(metrics["prior_day_contrarian_context"]),
        c0107_time_reversal_bucket(metrics["fade_alignment"]),
...
def opening_drive_exhaustion_fade_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    recent_start = max(index - OPENING_DRIVE_EXHAUSTION_FADE_RECENT_BARS + 1, context.day_start_index)
    prior_start = max(recent_start - OPENING_DRIVE_EXHAUSTION_FADE_PRIOR_BARS, context.day_start_index)
    lookback_start = max(index - OPENING_DRIVE_EXHAUSTION_FADE_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    if session_phase_maturity < 0.10 or session_phase_maturity > 0.78:
        return {"event_count": 0.0}
    ...
    recent_travel = sum(abs(bars[pos].close - bars[pos - 1].close) for pos in range(recent_start + 1, index + 1))
    day_travel = sum(
        abs(bars[pos].close - bars[pos - 1].close) for pos in range(context.day_start_index + 1, index + 1)
    )
    if day_travel <= 0:
        return {"event_count": 0.0}
    adverse_net = -direction * (signal_bar.close - context.day_open)
    opening_drive_extension = adverse_net / average_range
    if opening_drive_extension < 0.16:
        return {"event_count": 0.0}
    day_efficiency = adverse_net / max(day_travel, average_range * 0.10)
    recent_adverse_net = -direction * (signal_bar.close - bars[recent_start].open)
    recent_efficiency = recent_adverse_net / max(recent_travel, average_range * 0.10)
    opening_drive_efficiency_loss = day_efficiency - recent_efficiency
    opening_drive_volume_decay = prior_volume_mean / max(average_volume, 1e-9) - recent_volume_mean / max(
        average_volume,
        1e-9,
    )
    long_position = (signal_bar.close - context.low_so_far) / day_range_so_far
    if direction > 0:
        opening_extreme_position = 1.0 - long_position
        vwap_reversion_pressure = (session_vwap - signal_bar.close) / average_range
        reclaim_against_drive_strength = (signal_bar.close - recent_low) / average_range
    else:
        opening_extreme_position = long_position
        vwap_reversion_pressure = (signal_bar.close - session_vwap) / average_range
        reclaim_against_drive_strength = (recent_high - signal_bar.close) / average_range
    recent_body_turn = base.mean([directional_body_score(bar, direction) for bar in recent_window[-3:]]) or 0.0
    counter_body_turn = 0.65 * directional_body_score(signal_bar, direction) + 0.35 * recent_body_turn
    adverse_wick = base.mean([adverse_wick_absorption(bar, direction) for bar in recent_window[-4:]]) or 0.0
    prior_day_contrarian_context = 0.0
    ...
    fade_alignment = session_phase_maturity * (
        0.22 * opening_drive_extension
        + 0.20 * max(opening_drive_efficiency_loss, 0.0)
        + 0.17 * max(vwap_reversion_pressure, 0.0)
        + 0.14 * max(counter_body_turn, 0.0)
        + 0.11 * max(opening_drive_volume_decay, 0.0)
        + 0.10 * max(reclaim_against_drive_strength, 0.0)
        + 0.06 * max(adverse_wick, 0.0)
    )
    activity = max(
        opening_drive_extension,
        max(opening_drive_efficiency_loss, 0.0) * 0.70,
        max(vwap_reversion_pressure, 0.0) * 0.60,
        max(counter_body_turn, 0.0) * 0.45,
        max(opening_drive_volume_decay, 0.0) * 0.30,
        max(fade_alignment, 0.0) * 0.25,
    )
    if activity < MIN_OPENING_DRIVE_EXHAUSTION_FADE_ACTIVITY:
        return {"event_count": 0.0}
    if opening_drive_efficiency_loss < -0.32 or vwap_reversion_pressure < -0.20:
        return {"event_count": 0.0}
    return {
        "opening_drive_extension": bounded(opening_drive_extension, 0.0, 6.0),
        "opening_drive_efficiency_loss": bounded(opening_drive_efficiency_loss, -2.0, 2.0),
        "opening_drive_volume_decay": bounded(opening_drive_volume_decay, -4.0, 4.0),
        "opening_extreme_position": bounded(opening_extreme_position, 0.0, 1.0),
        "vwap_reversion_pressure": bounded(vwap_reversion_pressure, -4.0, 4.0),
        "counter_body_turn": bounded(counter_body_turn, -1.0, 1.0),
        "adverse_wick_absorption": bounded(adverse_wick, 0.0, 1.0),
        "reclaim_against_drive_strength": bounded(reclaim_against_drive_strength, -4.0, 4.0),
        "prior_day_contrarian_context": bounded(prior_day_contrarian_context, -2.0, 2.0),
        "fade_alignment": bounded(fade_alignment, -4.0, 4.0),
        ...
        "opening_drive_exhaustion_fade_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0107_boundary_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0107_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0107_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0107_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0107_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0107_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0107_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0107_opening_reclaim_bucket(value: float) -> str:
    if value < -0.08:
...
def c0107_prior_boundary_bucket(value: float) -> str:
    if value < -0.12:
...
def c0107_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    extension_reward = 0.13 if metrics["opening_drive_extension"] > 0.35 and terminal > 0 else 0.0
    efficiency_reward = 0.14 if metrics["opening_drive_efficiency_loss"] > 0.10 and terminal > 0 else 0.0
    vwap_reward = 0.12 if metrics["vwap_reversion_pressure"] > 0.18 and terminal > 0 else 0.0
    counter_body_reward = 0.10 if metrics["counter_body_turn"] > 0.08 and terminal > 0 else 0.0
    volume_reward = 0.08 if metrics["opening_drive_volume_decay"] > 0.10 and terminal > 0 else 0.0
    reclaim_reward = 0.08 if metrics["reclaim_against_drive_strength"] > 0.18 and terminal > 0 else 0.0
    adverse_wick_reward = 0.06 if metrics["adverse_wick_absorption"] > 0.25 and terminal > 0 else 0.0
    prior_reward = 0.05 if metrics["prior_day_contrarian_context"] > 0.08 and terminal > 0 else 0.0
    time_reward = 0.06 if metrics["fade_alignment"] > 0.08 and terminal > 0 else 0.0
    weak_extension_penalty = 0.08 if metrics["opening_drive_extension"] < 0.24 and terminal <= 0 else 0.0
    continued_efficiency_penalty = 0.14 if metrics["opening_drive_efficiency_loss"] < -0.10 and terminal <= 0 else 0.0
    vwap_against_penalty = 0.12 if metrics["vwap_reversion_pressure"] < -0.10 and terminal <= 0 else 0.0
    body_against_penalty = 0.09 if metrics["counter_body_turn"] < -0.08 and terminal <= 0 else 0.0
    participation_continuation_penalty = 0.08 if metrics["opening_drive_volume_decay"] < -0.12 and terminal <= 0 else 0.0
    ...
    opening_drive_exhaustion_fade_path_quality = (0.54 * terminal + 0.30 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + opening_drive_exhaustion_fade_path_quality
        + extension_reward
        + efficiency_reward
        + vwap_reward
        + counter_body_reward
        ...
        - continued_efficiency_penalty
        - vwap_against_penalty
        - body_against_penalty
        - participation_continuation_penalty
        - stall_penalty
...
def fit_linear_opening_drive_exhaustion_fade_model(candidates: list[base.Candidate], fold_id: str) -> LinearOpeningDriveExhaustionFadeModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearOpeningDriveExhaustionFadeModel(
            fold_id,
    ...
    opening_drive_exhaustion_fade_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearOpeningDriveExhaustionFadeModel(
        fold_id=fold_id,
        ...
        opening_drive_exhaustion_fade_failure_rate=opening_drive_exhaustion_fade_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearOpeningDriveExhaustionFadeModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0107-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_opening_drive_exhaustion_fade_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0107_r0001_intraday_opening_drive_exhaustion_fade",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0107_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0107_r0001_opening_drive_exhaustion_fade_summary.json",
        ],
        ...
        "conditional_profiles": {
            "opening_drive_exhaustion_fade_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0107 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0107_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0107 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "opening_drive_exhaustion_fade_lookback_bars": OPENING_DRIVE_EXHAUSTION_FADE_LOOKBACK_BARS,
        "opening_drive_exhaustion_fade_recent_bars": OPENING_DRIVE_EXHAUSTION_FADE_RECENT_BARS,
        "opening_drive_exhaustion_fade_prior_bars": OPENING_DRIVE_EXHAUSTION_FADE_PRIOR_BARS,
        "min_opening_drive_exhaustion_fade_activity": MIN_OPENING_DRIVE_EXHAUSTION_FADE_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_opening_drive_exhaustion_fade_model_summary(model: LinearOpeningDriveExhaustionFadeModel) -> dict[str, object]:
    return {
        ...
        "opening_drive_exhaustion_fade_failure_rate": rounded(model.opening_drive_exhaustion_fade_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearOpeningDriveExhaustionFadeModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_opening_drive_exhaustion_fade_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "opening_drive_exhaustion_fade_profile": profiles["opening_drive_exhaustion_fade_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0107_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0107_r0001_proxy_trades.csv"
    evidence["opening_drive_exhaustion_fade_summary"] = "artifacts/c0107_r0001_opening_drive_exhaustion_fade_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0107_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0107_r0001_proxy_trades.csv",
        "artifacts/c0107_r0001_opening_drive_exhaustion_fade_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0107_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0107 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0107_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0107_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0107_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0107_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0107_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0107_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0107_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0107_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0107_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0107 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0107 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0107_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0107_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0107_r0001_intraday_opening_drive_exhaustion_fade_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0107_r0001_proxy_records_opening_drive_exhaustion_fade_reference_surface
  ...
  - next_work_is_produce_c0107_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0107_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0107-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0107_r0001_intraday_opening_drive_exhaustion_fade",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0107-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0107-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0107-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0107_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0107_r0001_intraday_opening_drive_exhaustion_fade",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0107-R0001-opening-drive-exhaustion-fade-SUMMARY",
            "artifact_role": "opening_drive_exhaustion_fade_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0107_r0001_opening_drive_exhaustion_fade_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0107_r0001_intraday_opening_drive_exhaustion_fade",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0107_r0001_mt5_logic_parity_evidence",
        }
    ]
