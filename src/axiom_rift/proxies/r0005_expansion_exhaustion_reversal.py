"""R0005 proxy evidence for expansion-exhaustion reversal."""
...
def run_r0005_proxy(write: bool = True) -> dict[str, object]:
        ...
        while index <= end_index:
            if index > start_index:
                ...
                if active is None and has_evaluable_next_bar(bars, index - 1, end_index):
                    ...
                    if next_direction != 0:
                        pending_direction = reversal_direction(next_direction)
...
def build_proxy_payload(trades: list[Trade], windows: list[Window]) -> dict[str, object]:
    ...
    return {
        ...
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.r0005_expansion_exhaustion_reversal",
        "proxy_config_path": "campaigns/C0001_regime_response_discovery/runs/R0005/run_manifest.json",
        ...
        "deferred_with_reason": [
            {
                ...
                "reason": "R0005 proxy currently measures raw US100 index points only; USD conversion requires MT5 symbol contract, tick value, and fixed-lot discovery sizing",
                "blocking_condition": "record the matching MT5 symbol specification and fixed-lot sizing for the 500 USD account",
                "revisit_when": "during R0005 MT5 probe evidence production",
            ...
            },
            {
                ...
                "revisit_when": "before R0005 closeout",
                "claim_boundary": {"claim_authority": False},
            }
        ],
        "claim_boundary": {
...
def proxy_config() -> dict[str, object]:
    return {
        "bar_basis": "closed_m5",
        "entry_basis": "next_bar_open_after_expansion_close",
        ...
        "response_mode": "expansion_exhaustion_reversal",
        "trade_direction_policy": "reverse_expansion_direction_immediately_after_expansion_close",
    }
...
def update_artifact_lineage(proxy_hash: str) -> None:
    ...
    for record in data.get("artifact_records", []):
        if record.get("artifact_id") == "A0001":
            ...
            record["produced_by"] = "axiom_rift.proxies.r0005_expansion_exhaustion_reversal"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_matching_mt5_probe_for_R0005"
    data["deferred_with_reason"] = [
        {
            "field": "decision",
            "reason": "proxy evidence is recorded but MT5 and proxy-vs-MT5 evidence are still missing",
            "blocking_condition": "R0005 cannot close until the mandatory MT5 probe and proxy-vs-MT5 comparison are recorded",
            "revisit_when": "after R0005 MT5 and proxy-vs-MT5 evidence files contain measured results",
        }
    ]
