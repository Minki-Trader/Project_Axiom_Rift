"""C0122 R0001 proxy evidence for intraday compression release failure fade."""
...
CAMPAIGN_ID = "C0122"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0122_intraday_compression_release_failure_fade_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0122_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0122_r0001_compression_release_failure_fade_summary.json"
...
FEATURE_NAMES = (
    ...
    "compression_volume_dryness",
    "release_attempt_extension",
    "failed_reentry_pressure",
    "rejection_wick_pressure",
    "vwap_snapback_alignment",
    "session_range_reversion_position",
    "prior_session_range_pressure",
    "false_breakout_volume_disagreement",
    "range_reentry_efficiency",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_compression_release_failure_fade_rank"
LABEL_SHAPE = "target_first_quality_after_compression_release_failure_fade"
SELECTION_RULE = "top_fold_local_compression_release_failure_fade_scores_per_active_day"
...
MIN_DAY_BARS = 18
COMPRESSION_RELEASE_FAILURE_FADE_LOOKBACK_BARS = 48
COMPRESSION_RELEASE_FAILURE_FADE_COMPRESSION_BARS = 18
COMPRESSION_RELEASE_FAILURE_FADE_RELEASE_BARS = 8
MIN_COMPRESSION_RANGE_CONTRACTION = -0.20
MIN_COMPRESSION_RELEASE_FAILURE_FADE_ACTIVITY = 0.055
...
class LinearCompressionReleaseFailureFadeModel:
    fold_id: str
    ...
    compression_release_failure_fade_failure_rate: float
...
def run_c0122_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_compression_release_failure_fade_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_compression_release_failure_fade_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        COMPRESSION_RELEASE_FAILURE_FADE_LOOKBACK_BARS,
        COMPRESSION_RELEASE_FAILURE_FADE_COMPRESSION_BARS + COMPRESSION_RELEASE_FAILURE_FADE_RELEASE_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = compression_release_failure_fade_metrics(
    ...
    features = (
        metrics["compression_range_contraction"],
        metrics["compression_volume_dryness"],
        metrics["release_attempt_extension"],
        metrics["failed_reentry_pressure"],
        metrics["rejection_wick_pressure"],
        metrics["vwap_snapback_alignment"],
        metrics["session_range_reversion_position"],
        metrics["prior_session_range_pressure"],
        metrics["false_breakout_volume_disagreement"],
        metrics["range_reentry_efficiency"],
        metrics["session_maturity"],
        metrics["spread_stress_inverse"],
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        range_moderation_bucket(metrics["compression_range_contraction"]),
        volume_stability_bucket(metrics["compression_volume_dryness"]),
        breakout_pressure_bucket(metrics["release_attempt_extension"]),
        breakout_pressure_bucket(metrics["failed_reentry_pressure"]),
        absorption_bucket(metrics["rejection_wick_pressure"]),
        vwap_alignment_bucket(metrics["vwap_snapback_alignment"]),
        range_position_pressure_bucket(metrics["session_range_reversion_position"]),
        prior_day_position_context_bucket(metrics["prior_session_range_pressure"]),
        volume_confirmation_bucket(metrics["false_breakout_volume_disagreement"]),
        drift_efficiency_bucket(metrics["range_reentry_efficiency"]),
...
def compression_release_failure_fade_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    release_start = max(index - COMPRESSION_RELEASE_FAILURE_FADE_RELEASE_BARS + 1, context.day_start_index)
    compression_start = max(release_start - COMPRESSION_RELEASE_FAILURE_FADE_COMPRESSION_BARS, context.day_start_index)
    lookback_start = max(index - COMPRESSION_RELEASE_FAILURE_FADE_LOOKBACK_BARS + 1, context.day_start_index)
    if compression_start < 0 or release_start < 0 or lookback_start < 0:
        return {"event_count": 0.0}
    compression_window = bars[compression_start:release_start]
    release_window = bars[release_start : index + 1]
    lookback_window = bars[lookback_start : index + 1]
    day_window = bars[context.day_start_index : index + 1]
    if len(compression_window) < 10 or len(release_window) < 4 or len(lookback_window) < 24 or len(day_window) < MIN_DAY_BARS:
        return {"event_count": 0.0}
    average_volume = volume_average[index]
    if average_volume is None or average_volume <= 0:
        return {"event_count": 0.0}

    compression_ranges = [bar.high - bar.low for bar in compression_window]
    release_ranges = [bar.high - bar.low for bar in release_window]
    lookback_ranges = [bar.high - bar.low for bar in lookback_window]
    compression_range_mean = base.mean(compression_ranges) or average_range
    release_range_mean = base.mean(release_ranges) or average_range
    lookback_range_mean = base.mean(lookback_ranges) or average_range
    compression_volume_mean = base.mean(volumes[compression_start:release_start]) or average_volume
    release_volume_mean = base.mean(volumes[release_start : index + 1]) or average_volume
    lookback_volume_mean = base.mean(volumes[lookback_start : index + 1]) or average_volume
    if compression_range_mean <= 0 or release_range_mean <= 0 or lookback_range_mean <= 0:
        return {"event_count": 0.0}

    session_phase_maturity = bounded_session_progress(signal_bar.time)
    if session_phase_maturity < 0.10 or session_phase_maturity > 0.90:
        return {"event_count": 0.0}

    compression_high = max(bar.high for bar in compression_window)
    compression_low = min(bar.low for bar in compression_window)
    compression_mid = (compression_high + compression_low) / 2.0
    compression_width = max(compression_high - compression_low, 1e-9)
    normalized_expected_width = max(lookback_range_mean * math.sqrt(len(compression_window)) * 0.80, average_range)
    compression_range_contraction = 1.0 - compression_width / normalized_expected_width
    if compression_range_contraction < MIN_COMPRESSION_RANGE_CONTRACTION:
        return {"event_count": 0.0}
    compression_volume_dryness = 1.0 - compression_volume_mean / max(lookback_volume_mean, average_volume, 1e-9)

    release_extreme = min(bar.low for bar in release_window) if direction > 0 else max(bar.high for bar in release_window)
    release_attempt_extension = -direction * (release_extreme - compression_mid) / max(average_range, 1e-9)
    if release_attempt_extension < 0.04 or release_attempt_extension > 3.20:
        return {"event_count": 0.0}

    release_settlement_balance = base.mean([directional_bar_settlement(bar, direction) for bar in release_window]) or 0.0
    ...
    rejection_wick_pressure = base.mean([rejection_wick_share(bar) for bar in release_window]) or 0.0
    rejection_wick_pressure = 0.55 * max(release_settlement_balance, 0.0) + 0.45 * rejection_wick_pressure
    failed_reentry_pressure = direction * (signal_bar.close - compression_mid) / max(average_range, 1e-9)
    failed_reentry_pressure += directional_bar_settlement(signal_bar, direction) * 0.30
    if max(failed_reentry_pressure, rejection_wick_pressure, compression_range_contraction) < 0.01:
        return {"event_count": 0.0}

    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    vwap_snapback_alignment = direction * (signal_bar.close - session_vwap) / max(average_range * 3.0, 1e-9)
    prior_session_range_pressure = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_session_range_pressure = -direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
    if context.prior_day_shadow_balance is not None:
        prior_session_range_pressure += -direction * context.prior_day_shadow_balance * 0.08
    directional_position = directional_session_percentile_position(signal_bar.close, context.low_so_far, context.high_so_far, direction)
    session_range_reversion_position = 1.0 - directional_position
    false_breakout_volume_disagreement = release_volume_mean / max(compression_volume_mean, average_volume * 0.50, 1e-9)
    false_breakout_volume_disagreement -= release_attempt_extension * 0.15
    release_path = sum(abs(release_window[pos].close - release_window[pos - 1].close) for pos in range(1, len(release_window)))
    range_reentry_efficiency = direction * (signal_bar.close - release_extreme) / max(release_path, average_range, 1e-9)
    ...
    activity = max(
        max(compression_range_contraction, 0.0) * 0.18,
        max(compression_volume_dryness, 0.0) * 0.08,
        release_attempt_extension * 0.16,
        max(failed_reentry_pressure, 0.0) * 0.24,
        rejection_wick_pressure * 0.18,
        max(vwap_snapback_alignment, 0.0) * 0.10,
        max(false_breakout_volume_disagreement, 0.0) * 0.08,
        max(range_reentry_efficiency, 0.0) * 0.16,
    )
    if activity < MIN_COMPRESSION_RELEASE_FAILURE_FADE_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "compression_range_contraction": bounded(compression_range_contraction, -2.0, 1.0),
        "compression_volume_dryness": bounded(compression_volume_dryness, -4.0, 1.0),
        "release_attempt_extension": bounded(release_attempt_extension, -1.0, 6.0),
        "failed_reentry_pressure": bounded(failed_reentry_pressure, -4.0, 8.0),
        "rejection_wick_pressure": bounded(rejection_wick_pressure, 0.0, 1.0),
        "vwap_snapback_alignment": bounded(vwap_snapback_alignment, -4.0, 4.0),
        "session_range_reversion_position": bounded(session_range_reversion_position, 0.0, 1.0),
        "prior_session_range_pressure": bounded(prior_session_range_pressure, -3.0, 3.0),
        "false_breakout_volume_disagreement": bounded(false_breakout_volume_disagreement, -4.0, 8.0),
        "range_reentry_efficiency": bounded(range_reentry_efficiency, -2.0, 2.0),
        "session_maturity": bounded(session_phase_maturity, 0.0, 1.0),
        "spread_stress_inverse": bounded(spread_stress_inverse, 0.0, 1.0),
        "recent_range_ratio": bounded(release_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        "base_range_ratio": bounded(compression_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        "lookback_range_ratio": bounded(lookback_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        "recent_volume_ratio": bounded(release_volume_mean / max(average_volume, 1e-9), 0.0, 8.0),
        "base_volume_ratio": bounded(compression_volume_mean / max(average_volume, 1e-9), 0.0, 8.0),
        "lookback_volume_ratio": bounded(lookback_volume_mean / max(average_volume, 1e-9), 0.0, 8.0),
        "compression_mid": rounded(compression_mid) or 0.0,
        "release_extreme": rounded(release_extreme) or 0.0,
        "compression_release_failure_fade_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0122_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0122_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0122_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0122_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0122_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0122_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0122_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0122_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0122_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0122_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0122_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0122_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0122_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0122_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0122_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0122_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0122_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0122_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0122_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def c0122_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    compression_reward = 0.08 if metrics["compression_range_contraction"] > 0.10 and terminal > 0 else 0.0
    dryness_reward = 0.05 if metrics["compression_volume_dryness"] > -0.15 and terminal > 0 else 0.0
    extension_reward = 0.09 if 0.08 <= metrics["release_attempt_extension"] <= 1.60 and terminal > 0 else 0.0
    reentry_reward = 0.13 if metrics["failed_reentry_pressure"] > 0.10 and terminal > 0 else 0.0
    rejection_reward = 0.10 if metrics["rejection_wick_pressure"] > 0.30 and terminal > 0 else 0.0
    vwap_reward = 0.06 if metrics["vwap_snapback_alignment"] > -0.05 and terminal > 0 else 0.0
    position_reward = 0.05 if metrics["session_range_reversion_position"] > 0.30 and terminal > 0 else 0.0
    volume_disagreement_reward = 0.06 if metrics["false_breakout_volume_disagreement"] > 0.20 and terminal > 0 else 0.0
    efficiency_reward = 0.08 if metrics["range_reentry_efficiency"] > 0.20 and terminal > 0 else 0.0
    weak_compression_penalty = 0.08 if metrics["compression_range_contraction"] < -0.05 and terminal <= 0 else 0.0
    overextended_penalty = 0.08 if metrics["release_attempt_extension"] > 1.80 and terminal <= 0 else 0.0
    weak_reentry_penalty = 0.10 if metrics["failed_reentry_pressure"] < 0.00 and terminal <= 0 else 0.0
    weak_rejection_penalty = 0.08 if metrics["rejection_wick_pressure"] < 0.18 and terminal <= 0 else 0.0
    thin_reentry_penalty = 0.06 if metrics["range_reentry_efficiency"] < 0.02 and terminal <= 0 else 0.0
    ...
    return (
        ...
        + vwap_reward
        + position_reward
        + volume_disagreement_reward
        + efficiency_reward
        - weak_compression_penalty
        - overextended_penalty
        - weak_reentry_penalty
        - weak_rejection_penalty
        - thin_reentry_penalty
        - stall_penalty
        - unfavorable_path_penalty
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["recent_range_ratio"] - 2.80) * 0.04
        - max(0.0, metrics["recent_volume_ratio"] - 4.20) * 0.015
        - max(0.0, 0.06 - metrics["release_attempt_extension"]) * 0.12
    )


def fit_linear_compression_release_failure_fade_model(candidates: list[base.Candidate], fold_id: str) -> LinearCompressionReleaseFailureFadeModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearCompressionReleaseFailureFadeModel(
            fold_id,
    ...
    compression_release_failure_fade_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearCompressionReleaseFailureFadeModel(
        fold_id=fold_id,
        ...
        compression_release_failure_fade_failure_rate=compression_release_failure_fade_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearCompressionReleaseFailureFadeModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0122-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_compression_release_failure_fade_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0122_r0001_intraday_compression_release_failure_fade",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0122_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0122_r0001_compression_release_failure_fade_summary.json",
        ],
        ...
        "conditional_profiles": {
            "compression_release_failure_fade_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0122 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0122_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0122 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "compression_release_failure_fade_lookback_bars": COMPRESSION_RELEASE_FAILURE_FADE_LOOKBACK_BARS,
        "compression_release_failure_fade_compression_bars": COMPRESSION_RELEASE_FAILURE_FADE_COMPRESSION_BARS,
        "compression_release_failure_fade_release_bars": COMPRESSION_RELEASE_FAILURE_FADE_RELEASE_BARS,
        "min_compression_range_contraction": MIN_COMPRESSION_RANGE_CONTRACTION,
        "min_compression_release_failure_fade_activity": MIN_COMPRESSION_RELEASE_FAILURE_FADE_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_compression_release_failure_fade_model_summary(model: LinearCompressionReleaseFailureFadeModel) -> dict[str, object]:
    return {
        ...
        "compression_release_failure_fade_failure_rate": rounded(model.compression_release_failure_fade_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearCompressionReleaseFailureFadeModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_compression_release_failure_fade_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "compression_release_failure_fade_profile": profiles["compression_release_failure_fade_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0122_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0122_r0001_proxy_trades.csv"
    evidence["compression_release_failure_fade_summary"] = "artifacts/c0122_r0001_compression_release_failure_fade_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0122_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0122_r0001_proxy_trades.csv",
        "artifacts/c0122_r0001_compression_release_failure_fade_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0122_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0122 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0122_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0122_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0122_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0122_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0122_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0122_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0122_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0122_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0122_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0122 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0122 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260708_produce_c0122_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0122_r0001_proxy_evidence
  refines:
  - dec_20260708_open_c0122_r0001_intraday_compression_release_failure_fade_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0122_r0001_proxy_records_compression_release_failure_fade_reference_surface
  ...
  - next_work_is_produce_c0122_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260708_produce_c0122_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0122-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0122_r0001_intraday_compression_release_failure_fade",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0122-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0122-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0122-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0122_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0122_r0001_intraday_compression_release_failure_fade",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0122-R0001-COMPRESSION-RELEASE-FAILURE-FADE-SUMMARY",
            "artifact_role": "compression_release_failure_fade_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0122_r0001_compression_release_failure_fade_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0122_r0001_intraday_compression_release_failure_fade",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0122_r0001_mt5_logic_parity_evidence",
        }
    ]
