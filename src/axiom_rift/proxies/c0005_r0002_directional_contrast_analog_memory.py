"""C0005 R0002 proxy evidence for directional contrast analog-memory entries."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0005_continuous_analog_memory_discovery" / "runs" / "R0002"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0005_r0002_proxy_trades.csv"
ANALOG_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0005_r0002_analog_memory_summary.json"
...
MODEL_FAMILY = "fold_local_directional_contrast_analog_memory"
LABEL_SHAPE = "directional_favorable_minus_adverse_excursion_contrast"
ANALOG_NEIGHBOR_COUNT = 64
MAX_ANALOG_DISTANCE = 5.35
MIN_CONTRAST_LIFT = -0.02
MAX_ADVERSE_RATE_LIFT = 0.08
...
def run_c0005_r0002_proxy(write: bool = True) -> dict[str, object]:
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            ...
            if features is None:
                continue
            label = directional_contrast_label(bars, range_average, index, direction) if include_labels else None
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|directional_contrast_analog_memory",
                    features=features,
                    label=label,
                )
            )
...
def directional_contrast_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    index: int,
    direction: int,
    ...
    return 0.90 * first_hit + 0.50 * favorable - 0.85 * adverse + 0.35 * terminal_norm - spread_norm
...
def fit_analog_model(candidates: list[base.Candidate], fold_id: str) -> AnalogModel:
    ...
    if not labeled:
        ...
        return AnalogModel(fold_id, zeros, ones, np.array([], dtype=float), None, 0, 0, 0.0, 0.0, 0.0)
    ...
    return AnalogModel(
        ...
        global_adverse_rate=float(np.mean(labels < -0.35)),
    )
...
def score_candidates(candidates: list[base.Candidate], model: AnalogModel) -> list[base.Candidate]:
    ...
    local_adverse_rate = (neighbor_labels < -0.35).mean(axis=1)
    local_upper_tail = np.maximum(neighbor_labels, 0.0).mean(axis=1)
    mean_distance = distances.mean(axis=1)
    scores = (
        local_mean
        + 0.32 * (local_positive_rate - model.global_positive_rate)
        - 0.42 * (local_adverse_rate - model.global_adverse_rate)
    ...
    for index, candidate in enumerate(candidates):
        if mean_distance[index] > MAX_ANALOG_DISTANCE:
            score = None
        elif local_mean[index] - model.global_mean < MIN_CONTRAST_LIFT:
            score = None
...
def build_proxy_payload(
    ...
    payload["proxy_id"] = "PX-C0005-R0002"
    payload["proxy_engine"] = "axiom_rift.proxies.c0005_r0002_directional_contrast_analog_memory"
    payload["proxy_config_path"] = "campaigns/C0005_continuous_analog_memory_discovery/runs/R0002/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_directional_contrast_analog_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0005_continuous_analog_memory_discovery/runs/R0002/kpi/proxy.json",
        "campaigns/C0005_continuous_analog_memory_discovery/runs/R0002/artifacts/c0005_r0002_proxy_trades.csv",
        "campaigns/C0005_continuous_analog_memory_discovery/runs/R0002/artifacts/c0005_r0002_analog_memory_summary.json",
    ]
    ...
    profiles["analog_memory_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "min_contrast_lift": MIN_CONTRAST_LIFT,
            "max_adverse_rate_lift": MAX_ADVERSE_RATE_LIFT,
            "fold_models": fold_models,
            "state_distributions": state_distributions,
            "candidates_by_fold": candidates_by_fold,
            "selection_rule": "top_fold_local_directional_contrast_analog_scores_per_active_day",
    ...
    config.update(
        {
            ...
            "min_contrast_lift": MIN_CONTRAST_LIFT,
            "max_adverse_rate_lift": MAX_ADVERSE_RATE_LIFT,
            "score_interpretation": "higher_score_means_favorable_minus_adverse_local_analog_contrast",
            "variant_boundary": "directional_contrast_analog_memory_not_r0001_parameter_nudge_or_state_archetype_retry",
        }
    )
    return config
...
def write_analog_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_directional_contrast_analog_memory_summary_v1",
        "template": False,
        "work_unit_id": "C0005",
        "campaign_id": "C0005",
        "run_id": "R0002",
        ...
        [
            artifact_record(
                ...
                [
                    ...
                    "campaigns/C0005_continuous_analog_memory_discovery/runs/R0002/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0005-R0002-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0005-R0002-ANALOG-MEMORY-SUMMARY",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0005_r0002_mt5_logic_parity_evidence",
        }
    ]
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0005_r0002_directional_contrast_analog_memory",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0005_r0002_proxy_trades.csv"
    evidence["analog_memory_summary"] = "artifacts/c0005_r0002_analog_memory_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0005_r0002_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0005_r0002_proxy_trades.csv",
        "artifacts/c0005_r0002_analog_memory_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0005 R0002 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0005_r0002_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0005_r0002_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    if "produce_c0005_r0002_proxy_evidence" not in completed:
        completed.append("produce_c0005_r0002_proxy_evidence")
    next_action = "produce_c0005_r0002_mt5_logic_parity_evidence"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0005_r0002_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0005_continuous_analog_memory_discovery/runs/R0002/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0005_r0002_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            ...
            "R0004": "R0002",
            "R0001": "R0002",
            "c0004_r0001_fold_local_state_archetype": "c0005_r0002_directional_contrast_analog_memory",
            "c0004_r0001": "c0005_r0002",
