"""C0126 R0001 proxy evidence for intraday volatility budget exhaustion reversal."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0126_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0126_r0001_volatility_budget_exhaustion_reversal_summary.json"
...
FEATURE_NAMES = (
    "range_budget_consumption",
    "impulse_extension_pressure",
    "participation_exhaustion",
    "effort_decay_pressure",
    "adverse_wick_reversal_pressure",
    "vwap_tether_reversion_room",
    "opening_range_overextension",
    "prior_session_exhaustion_context",
    "micro_reversal_efficiency",
    "timebox_exhaustion_quality",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_volatility_budget_exhaustion_reversal_rank"
LABEL_SHAPE = "target_first_quality_after_volatility_budget_exhaustion_reversal"
SELECTION_RULE = "top_fold_local_volatility_budget_exhaustion_reversal_scores_per_active_day"
...
MIN_DAY_BARS = 18
VOLATILITY_BUDGET_EXHAUSTION_REVERSAL_LOOKBACK_BARS = 48
VOLATILITY_BUDGET_EXHAUSTION_REVERSAL_MICRO_BARS = 12
MIN_RANGE_BUDGET_CONSUMPTION = 0.62
MIN_IMPULSE_EXTENSION_PRESSURE = 0.025
MIN_VOLATILITY_BUDGET_EXHAUSTION_REVERSAL_ACTIVITY = 0.05
...
class LinearVolatilityBudgetExhaustionReversalModel:
    fold_id: str
    ...
    volatility_budget_exhaustion_reversal_failure_rate: float
...
def run_c0126_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_volatility_budget_exhaustion_reversal_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_volatility_budget_exhaustion_reversal_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        VOLATILITY_BUDGET_EXHAUSTION_REVERSAL_LOOKBACK_BARS,
        VOLATILITY_BUDGET_EXHAUSTION_REVERSAL_MICRO_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = volatility_budget_exhaustion_reversal_metrics(
    ...
    features = (
        metrics["range_budget_consumption"],
        metrics["impulse_extension_pressure"],
        metrics["participation_exhaustion"],
        metrics["effort_decay_pressure"],
        metrics["adverse_wick_reversal_pressure"],
        metrics["vwap_tether_reversion_room"],
        metrics["opening_range_overextension"],
        metrics["prior_session_exhaustion_context"],
        metrics["micro_reversal_efficiency"],
        metrics["timebox_exhaustion_quality"],
        metrics["session_maturity"],
        metrics["spread_stress_inverse"],
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        breakout_pressure_bucket(metrics["range_budget_consumption"]),
        range_position_pressure_bucket(metrics["impulse_extension_pressure"]),
        participation_decay_bucket(metrics["participation_exhaustion"]),
        range_reversion_efficiency_bucket(metrics["effort_decay_pressure"]),
        adverse_wick_control_bucket(metrics["adverse_wick_reversal_pressure"]),
        vwap_alignment_bucket(metrics["vwap_tether_reversion_room"]),
        prior_day_position_context_bucket(metrics["opening_range_overextension"]),
        prior_tailwind_bucket(metrics["prior_session_exhaustion_context"]),
        pullback_containment_bucket(metrics["micro_reversal_efficiency"]),
        session_maturity_bucket(metrics["timebox_exhaustion_quality"]),
...
def volatility_budget_exhaustion_reversal_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    micro_start = max(index - VOLATILITY_BUDGET_EXHAUSTION_REVERSAL_MICRO_BARS + 1, context.day_start_index)
    lookback_start = max(index - VOLATILITY_BUDGET_EXHAUSTION_REVERSAL_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    range_budget_consumption = session_range / max(
        lookback_range_mean * math.sqrt(min(context.bars_so_far, 96)) / 3.0,
        average_range,
        1e-9,
    )
    if range_budget_consumption < MIN_RANGE_BUDGET_CONSUMPTION:
        return {"event_count": 0.0}

    impulse_extension_pressure = direction * (session_mid - signal_bar.close) / max(
        average_range * 3.0,
        1e-9,
    )
    if impulse_extension_pressure < MIN_IMPULSE_EXTENSION_PRESSURE:
        return {"event_count": 0.0}

    earlier_start = max(lookback_start, micro_start - VOLATILITY_BUDGET_EXHAUSTION_REVERSAL_MICRO_BARS)
    earlier_window = bars[earlier_start:micro_start] or lookback_window[: len(micro_window)]
    earlier_range_mean = base.mean([bar.high - bar.low for bar in earlier_window]) or lookback_range_mean
    earlier_volume_mean = base.mean(volumes[earlier_start:micro_start]) or lookback_volume_mean
    participation_exhaustion = 1.0 - micro_volume_mean / max(lookback_volume_mean, average_volume, 1e-9)
    effort_decay_pressure = (
        earlier_range_mean / max(earlier_volume_mean, 1e-9)
        - micro_range_mean / max(micro_volume_mean, 1e-9)
    ) * max(average_volume / max(average_range, 1e-9), 1e-9)
    adverse_wick_reversal_pressure = base.mean(
    ...
    vwap_tether_reversion_room = direction * (session_vwap - signal_bar.close) / max(
        average_range * 3.0,
        1e-9,
    )
    opening_mid = (context.opening_high + context.opening_low) / 2.0
    opening_width = max(context.opening_high - context.opening_low, average_range, 1e-9)
    opening_range_overextension = direction * (opening_mid - signal_bar.close) / max(
        opening_width,
        average_range * 2.0,
        1e-9,
    )
    prior_session_exhaustion_context = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_session_exhaustion_context = -direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
    if context.prior_day_shadow_balance is not None:
        prior_session_exhaustion_context += direction * context.prior_day_shadow_balance * 0.08
    path_travel = sum(abs(micro_window[pos].close - micro_window[pos - 1].close) for pos in range(1, len(micro_window)))
    micro_reversal_efficiency = direction * (
        signal_bar.close - micro_window[0].close
    ) / max(path_travel, average_range, 1e-9)
    timebox_exhaustion_quality = bounded(1.0 - abs(session_phase_maturity - 0.64) / 0.38, 0.0, 1.0)
    ...
    activity = (
        max(range_budget_consumption, 0.0) * 0.16
        + max(impulse_extension_pressure, 0.0) * 0.17
        + max(participation_exhaustion, 0.0) * 0.12
        + max(effort_decay_pressure, 0.0) * 0.10
        + adverse_wick_reversal_pressure * 0.11
        + max(vwap_tether_reversion_room, 0.0) * 0.12
        + max(opening_range_overextension, 0.0) * 0.08
        + max(prior_session_exhaustion_context, 0.0) * 0.05
        + max(micro_reversal_efficiency, 0.0) * 0.06
        + timebox_exhaustion_quality * 0.03
    )
    if activity < MIN_VOLATILITY_BUDGET_EXHAUSTION_REVERSAL_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "range_budget_consumption": bounded(range_budget_consumption, 0.0, 8.0),
        "impulse_extension_pressure": bounded(impulse_extension_pressure, -4.0, 4.0),
        "participation_exhaustion": bounded(participation_exhaustion, -4.0, 4.0),
        "effort_decay_pressure": bounded(effort_decay_pressure, -4.0, 4.0),
        "adverse_wick_reversal_pressure": bounded(adverse_wick_reversal_pressure, 0.0, 1.0),
        "vwap_tether_reversion_room": bounded(vwap_tether_reversion_room, -4.0, 4.0),
        "opening_range_overextension": bounded(opening_range_overextension, -4.0, 4.0),
        "prior_session_exhaustion_context": bounded(prior_session_exhaustion_context, -3.0, 3.0),
        "micro_reversal_efficiency": bounded(micro_reversal_efficiency, -2.0, 2.0),
        "timebox_exhaustion_quality": bounded(timebox_exhaustion_quality, 0.0, 1.0),
        "range_stretch_pressure": bounded(range_budget_consumption, 0.0, 8.0),
        ...
        "opening_mid": rounded(opening_mid) or 0.0,
        "volatility_budget_exhaustion_reversal_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0126_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0126_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0126_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0126_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0126_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0126_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0126_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0126_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0126_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0126_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0126_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0126_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0126_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0126_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0126_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0126_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0126_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0126_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0126_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def c0126_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    budget_reward = 0.11 if metrics["range_budget_consumption"] > 0.85 and terminal > 0 else 0.0
    extension_reward = 0.13 if metrics["impulse_extension_pressure"] > 0.08 and terminal > 0 else 0.0
    exhaustion_reward = 0.10 if metrics["participation_exhaustion"] > 0.04 and terminal > 0 else 0.0
    effort_decay_reward = 0.08 if metrics["effort_decay_pressure"] > 0.00 and terminal > 0 else 0.0
    wick_reward = 0.08 if metrics["adverse_wick_reversal_pressure"] > 0.22 and terminal > 0 else 0.0
    vwap_reward = 0.10 if metrics["vwap_tether_reversion_room"] > -0.02 and terminal > 0 else 0.0
    opening_reward = 0.06 if metrics["opening_range_overextension"] > 0.03 and terminal > 0 else 0.0
    prior_exhaustion_reward = 0.04 if metrics["prior_session_exhaustion_context"] > -0.20 and terminal > 0 else 0.0
    path_reversal_reward = 0.12 if metrics["micro_reversal_efficiency"] > 0.02 and terminal > 0 else 0.0
    timing_reward = 0.04 if metrics["timebox_exhaustion_quality"] > 0.20 and terminal > 0 else 0.0
    weak_budget_penalty = 0.10 if metrics["range_budget_consumption"] < 0.74 and terminal <= 0 else 0.0
    weak_extension_penalty = 0.11 if metrics["impulse_extension_pressure"] < 0.04 and terminal <= 0 else 0.0
    weak_exhaustion_penalty = 0.08 if metrics["participation_exhaustion"] < -0.20 and terminal <= 0 else 0.0
    weak_vwap_penalty = 0.08 if metrics["vwap_tether_reversion_room"] < -0.18 and terminal <= 0 else 0.0
    weak_reversal_penalty = 0.10 if metrics["micro_reversal_efficiency"] < -0.08 and terminal <= 0 else 0.0
    ...
    return (
        ...
        + vwap_reward
        + opening_reward
        + prior_exhaustion_reward
        + path_reversal_reward
        + timing_reward
        - weak_budget_penalty
        - weak_extension_penalty
        - weak_exhaustion_penalty
        - weak_vwap_penalty
        - weak_reversal_penalty
        ...
        - max(0.0, MIN_RANGE_BUDGET_CONSUMPTION - metrics["range_budget_consumption"]) * 0.12
        - max(0.0, MIN_IMPULSE_EXTENSION_PRESSURE - metrics["impulse_extension_pressure"]) * 0.18
    )


def fit_linear_volatility_budget_exhaustion_reversal_model(candidates: list[base.Candidate], fold_id: str) -> LinearVolatilityBudgetExhaustionReversalModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearVolatilityBudgetExhaustionReversalModel(
            fold_id,
    ...
    volatility_budget_exhaustion_reversal_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearVolatilityBudgetExhaustionReversalModel(
        fold_id=fold_id,
        ...
        volatility_budget_exhaustion_reversal_failure_rate=volatility_budget_exhaustion_reversal_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearVolatilityBudgetExhaustionReversalModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_volatility_budget_exhaustion_reversal_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0126_r0001_intraday_volatility_budget_exhaustion_reversal",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0126_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0126_r0001_volatility_budget_exhaustion_reversal_summary.json",
        ],
        ...
        "conditional_profiles": {
            "volatility_budget_exhaustion_reversal_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0126 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0126_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0126 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "volatility_budget_exhaustion_reversal_lookback_bars": VOLATILITY_BUDGET_EXHAUSTION_REVERSAL_LOOKBACK_BARS,
        "volatility_budget_exhaustion_reversal_micro_bars": VOLATILITY_BUDGET_EXHAUSTION_REVERSAL_MICRO_BARS,
        "min_range_budget_consumption": MIN_RANGE_BUDGET_CONSUMPTION,
        "min_impulse_extension_pressure": MIN_IMPULSE_EXTENSION_PRESSURE,
        "min_volatility_budget_exhaustion_reversal_activity": MIN_VOLATILITY_BUDGET_EXHAUSTION_REVERSAL_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_volatility_budget_exhaustion_reversal_model_summary(model: LinearVolatilityBudgetExhaustionReversalModel) -> dict[str, object]:
    return {
        ...
        "volatility_budget_exhaustion_reversal_failure_rate": rounded(model.volatility_budget_exhaustion_reversal_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearVolatilityBudgetExhaustionReversalModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_volatility_budget_exhaustion_reversal_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "volatility_budget_exhaustion_reversal_profile": profiles["volatility_budget_exhaustion_reversal_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0126_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0126_r0001_proxy_trades.csv"
    evidence["volatility_budget_exhaustion_reversal_summary"] = "artifacts/c0126_r0001_volatility_budget_exhaustion_reversal_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0126_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0126_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0126 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0126_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0126_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0126_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0126_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0126_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0126_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0126_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0126_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0126_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0126 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0126 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260708_produce_c0126_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0126_r0001_proxy_evidence
  refines:
  - dec_20260708_open_c0126_r0001_intraday_volatility_budget_exhaustion_reversal_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0126_r0001_proxy_records_volatility_budget_exhaustion_reversal_reference_surface
  ...
  - next_work_is_produce_c0126_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260708_produce_c0126_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0126-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0126_r0001_intraday_volatility_budget_exhaustion_reversal",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0126-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0126-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0126-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0126_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0126_r0001_intraday_volatility_budget_exhaustion_reversal",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0126-R0001-VOLATILITY-BUDGET-EXHAUSTION-REVERSAL-SUMMARY",
            "artifact_role": "volatility_budget_exhaustion_reversal_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0126_r0001_volatility_budget_exhaustion_reversal_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0126_r0001_intraday_volatility_budget_exhaustion_reversal",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0126_r0001_mt5_logic_parity_evidence",
        }
    ]
