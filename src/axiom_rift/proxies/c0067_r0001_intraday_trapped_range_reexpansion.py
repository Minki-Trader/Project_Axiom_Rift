"""C0067 R0001 proxy evidence for intraday trapped range reexpansion."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0067_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0067_r0001_trapped_range_summary.json"
...
FEATURE_NAMES = (
    "trap_compression_ratio",
    "inside_close_density",
    "boundary_pressure",
    "trapped_breakout_attempt",
    "false_break_pressure",
    "expansion_readiness",
    "release_body_impulse",
    "volume_release_confirmation",
    "prior_day_range_trap_context",
    "session_maturity_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_trapped_range_reexpansion_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_trapped_range_reexpansion_state"
SELECTION_RULE = "top_fold_local_trapped_range_reexpansion_scores_per_active_day"
...
MIN_DAY_BARS = 30
TRAPPED_RANGE_LOOKBACK_BARS = 18
TRAPPED_RANGE_SHORT_LOOKBACK_BARS = 6
MIN_TRAPPED_RANGE_ACTIVITY = 0.10
...
class LinearTrappedRangeReexpansionModel:
    fold_id: str
    ...
    trapped_range_reexpansion_failure_rate: float
...
def run_c0067_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_trapped_range_reexpansion_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_trapped_range_reexpansion_model_summary(model))
...
def candidate_state_and_features(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        trap["trap_compression_ratio"],
        trap["inside_close_density"],
        trap["boundary_pressure"],
        trap["trapped_breakout_attempt"],
        trap["false_break_pressure"],
        trap["expansion_readiness"],
        trap["release_body_impulse"],
        trap["volume_release_confirmation"],
        trap["prior_day_range_trap_context"],
        trap["session_maturity_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        trap_compression_bucket(trap["trap_compression_ratio"]),
        inside_close_bucket(trap["inside_close_density"]),
        boundary_pressure_bucket(trap["boundary_pressure"]),
        breakout_attempt_bucket(trap["trapped_breakout_attempt"]),
        false_break_bucket(trap["false_break_pressure"]),
        expansion_readiness_bucket(trap["expansion_readiness"]),
        release_body_bucket(trap["release_body_impulse"]),
        volume_release_bucket(trap["volume_release_confirmation"]),
        prior_range_trap_bucket(trap["prior_day_range_trap_context"]),
        session_pressure_bucket(trap["session_maturity_pressure"]),
...
def trapped_range_reexpansion_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    window_start = index - TRAPPED_RANGE_LOOKBACK_BARS + 1
    short_start = index - TRAPPED_RANGE_SHORT_LOOKBACK_BARS + 1
    ...
    if len(window) < 8 or len(short_window) < 3 or len(prior_window) < 6:
        return {"event_count": 0.0}
    ...
    typical_window_width = max(average_range * 5.5, 1e-9)
    trap_compression_ratio = 1.0 - (prior_width / typical_window_width)
    ...
    for item in prior_window:
        ...
        if 0.20 <= location <= 0.80:
            inside_hits += 1
    inside_close_density = inside_hits / len(prior_window)

    if direction > 0:
        boundary_pressure = (signal_bar.close - prior_low) / prior_width
        trapped_breakout_attempt = max(0.0, signal_bar.high - prior_high) / average_range
        failed_settlement = max(0.0, prior_high - signal_bar.close) / average_range
        opposite_pierce = max(0.0, prior_low - signal_bar.low) / average_range
    else:
        boundary_pressure = (prior_high - signal_bar.close) / prior_width
        trapped_breakout_attempt = max(0.0, prior_low - signal_bar.low) / average_range
        failed_settlement = max(0.0, signal_bar.close - prior_low) / average_range
        opposite_pierce = max(0.0, signal_bar.high - prior_high) / average_range
    false_break_pressure = max(trapped_breakout_attempt * min(1.0, failed_settlement), opposite_pierce)
    mean_short_range = base.mean([item.high - item.low for item in short_window]) or average_range
    short_range_intensity = mean_short_range / max(average_range, 1e-9)
    directional_close_hits = sum(1 for item in short_window if direction * (item.close - item.open) > 0)
    directional_close_density = directional_close_hits / len(short_window)
    expansion_readiness = (
        0.45 * max(trap_compression_ratio, 0.0)
        + 0.25 * inside_close_density
        + 0.15 * min(short_range_intensity, 2.0)
        + 0.15 * directional_close_density
    )
    release_body_impulse = direction * (signal_bar.close - signal_bar.open) / average_range
    ...
    volume_release_confirmation = (directional_volume - opposing_volume) / max(expected_recent_volume, 1e-9)
    if context.prior_day_range is None or context.prior_day_range <= 0:
        prior_day_range_trap_context = 0.0
    else:
        prior_day_range_trap_context = (context.prior_day_range - prior_width) / context.prior_day_range
    session_maturity_pressure = bounded_session_progress(signal_bar.time) * max(
        max(trap_compression_ratio, 0.0),
        inside_close_density,
        boundary_pressure,
        trapped_breakout_attempt,
        max(release_body_impulse, 0.0),
        max(volume_release_confirmation, 0.0),
    )
    activity = max(
        max(trap_compression_ratio, 0.0),
        max(inside_close_density - 0.35, 0.0),
        max(boundary_pressure - 0.50, 0.0),
        trapped_breakout_attempt,
        max(expansion_readiness - 0.20, 0.0),
        abs(release_body_impulse),
        abs(volume_release_confirmation),
        max(prior_day_range_trap_context, 0.0),
        session_maturity_pressure,
    )
    if activity < MIN_TRAPPED_RANGE_ACTIVITY:
        return {"event_count": 0.0}
    if (
        trap_compression_ratio < 0.03
        and inside_close_density < 0.42
        and trapped_breakout_attempt < 0.04
        and release_body_impulse < 0.06
    ):
        return {"event_count": 0.0}
    return {
        "trap_compression_ratio": bounded(trap_compression_ratio, -1.0, 1.0),
        "inside_close_density": bounded(inside_close_density, 0.0, 1.0),
        "boundary_pressure": bounded(boundary_pressure, -0.50, 1.50),
        "trapped_breakout_attempt": bounded(trapped_breakout_attempt, 0.0, 3.0),
        "false_break_pressure": bounded(false_break_pressure, 0.0, 3.0),
        "expansion_readiness": bounded(expansion_readiness, 0.0, 2.0),
        "release_body_impulse": bounded(release_body_impulse, -4.0, 4.0),
        "volume_release_confirmation": bounded(volume_release_confirmation, -4.0, 4.0),
        "prior_day_range_trap_context": bounded(prior_day_range_trap_context, -1.0, 1.0),
...
def trap_compression_bucket(value: float) -> str:
    if value < 0.10:
        return "trap_loose"
    if value < 0.32:
        return "trap_compressed"
    return "trap_tight"


def inside_close_bucket(value: float) -> str:
    if value < 0.45:
        return "inside_close_sparse"
    if value < 0.68:
        return "inside_close_mixed"
    return "inside_close_dense"


def boundary_pressure_bucket(value: float) -> str:
    if value < 0.55:
        return "boundary_pressure_low"
    if value < 0.82:
        return "boundary_pressure_mid"
    return "boundary_pressure_high"


def breakout_attempt_bucket(value: float) -> str:
    if value < 0.04:
        return "breakout_attempt_absent"
    if value < 0.28:
        return "breakout_attempt_probe"
    return "breakout_attempt_forceful"


def false_break_bucket(value: float) -> str:
    if value < 0.04:
        return "false_break_low"
    if value < 0.24:
        return "false_break_mid"
    return "false_break_high"


def expansion_readiness_bucket(value: float) -> str:
    if value < 0.32:
        return "expansion_readiness_low"
    if value < 0.62:
        return "expansion_readiness_mid"
    return "expansion_readiness_high"
...
def release_body_bucket(value: float) -> str:
    ...
    if value < 0.16:
        return "release_body_neutral"
    return "release_body_with"


def volume_release_bucket(value: float) -> str:
    if value < -0.18:
        return "volume_release_against"
    if value < 0.22:
        return "volume_release_mixed"
    return "volume_release_with"


def prior_range_trap_bucket(value: float) -> str:
    if value < 0.20:
        return "prior_range_trap_low"
    if value < 0.55:
        return "prior_range_trap_mid"
    return "prior_range_trap_high"
...
def candidate_label(
    ...
    metrics = trapped_range_reexpansion_metrics(
        bars,
        volumes,
    ...
    if metrics["event_count"] <= 0:
        return None
    compression_reward = 0.16 if metrics["trap_compression_ratio"] > 0.12 and terminal > 0 else 0.0
    inside_reward = 0.12 if metrics["inside_close_density"] > 0.50 and terminal > 0 else 0.0
    boundary_reward = 0.18 if metrics["boundary_pressure"] > 0.62 and terminal > 0 else 0.0
    breakout_reward = 0.20 if metrics["trapped_breakout_attempt"] > 0.08 and terminal > 0 else 0.0
    readiness_reward = 0.14 if metrics["expansion_readiness"] > 0.35 and terminal > 0 else 0.0
    body_reward = 0.12 if metrics["release_body_impulse"] > 0.12 and terminal > 0 else 0.0
    volume_reward = 0.10 if metrics["volume_release_confirmation"] > 0.15 and terminal > 0 else 0.0
    prior_trap_reward = 0.06 if metrics["prior_day_range_trap_context"] > 0.22 and terminal > 0 else 0.0
    false_break_penalty = 0.22 if metrics["false_break_pressure"] > 0.18 and terminal <= 0 else 0.0
    weak_release_penalty = 0.20 if metrics["trapped_breakout_attempt"] < 0.02 and terminal <= 0 else 0.0
    failed_boundary_penalty = 0.18 if metrics["boundary_pressure"] < 0.42 and terminal <= 0 else 0.0
    failed_compression_penalty = 0.14 if metrics["trap_compression_ratio"] < 0.0 and terminal <= 0 else 0.0
    ...
    return (
        ...
        - failed_boundary_penalty
        - failed_compression_penalty
        - stall_penalty
...
def fit_linear_trapped_range_reexpansion_model(candidates: list[base.Candidate], fold_id: str) -> LinearTrappedRangeReexpansionModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearTrappedRangeReexpansionModel(
            fold_id,
    ...
    trapped_range_reexpansion_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearTrappedRangeReexpansionModel(
        fold_id=fold_id,
        ...
        trapped_range_reexpansion_failure_rate=trapped_range_reexpansion_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearTrappedRangeReexpansionModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_trapped_range_reexpansion_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0067_r0001_intraday_trapped_range_reexpansion",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0067_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0067_r0001_trapped_range_summary.json",
        ],
        ...
        "conditional_profiles": {
            "trapped_range_reexpansion_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0067 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0067_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0067 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "trapped_range_reexpansion_lookback_bars": TRAPPED_RANGE_LOOKBACK_BARS,
        "trapped_range_reexpansion_short_lookback_bars": TRAPPED_RANGE_SHORT_LOOKBACK_BARS,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_trapped_range_reexpansion_model_summary(model: LinearTrappedRangeReexpansionModel) -> dict[str, object]:
    return {
        ...
        "trapped_range_reexpansion_failure_rate": rounded(model.trapped_range_reexpansion_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearTrappedRangeReexpansionModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_trapped_range_reexpansion_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "trapped_range_reexpansion_profile": profiles["trapped_range_reexpansion_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0067_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0067_r0001_proxy_trades.csv"
    evidence["trapped_range_reexpansion_summary"] = "artifacts/c0067_r0001_trapped_range_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0067_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0067_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0067 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0067_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0067_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0067_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0067_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0067_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0067_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0067_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0067_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0067_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0067 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0067 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260706_produce_c0067_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0067_r0001_proxy_evidence
  refines:
  - dec_20260706_open_c0067_r0001_intraday_trapped_range_reexpansion_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0067_r0001_proxy_records_trapped_range_reexpansion_reference_surface
  ...
  - next_work_is_produce_c0067_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260706_produce_c0067_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0067-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0067_r0001_intraday_trapped_range_reexpansion",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0067-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0067-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0067-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0067_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0067_r0001_intraday_trapped_range_reexpansion",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0067-R0001-trapped-range-reexpansion-SUMMARY",
            "artifact_role": "trapped_range_reexpansion_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0067_r0001_trapped_range_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0067_r0001_intraday_trapped_range_reexpansion",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0067_r0001_mt5_logic_parity_evidence",
        }
    ]
