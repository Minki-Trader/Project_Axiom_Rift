"""C0075 R0001 proxy evidence for intraday conviction decay."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0075_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0075_r0001_cd_summary.json"
...
FEATURE_NAMES = (
    "opposing_run_stretch",
    "body_decay_ratio",
    "close_location_erosion",
    "counter_wick_pressure",
    "participation_decay",
    "path_efficiency_loss",
    "range_extension_pressure",
    "failed_followthrough_pressure",
    "prior_day_directional_context",
    "session_maturity_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_conviction_decay_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_conviction_decay_state"
SELECTION_RULE = "top_fold_local_conviction_decay_scores_per_active_day"
...
MIN_DAY_BARS = 30
CONVICTION_DECAY_LOOKBACK_BARS = 24
CONVICTION_DECAY_SHORT_LOOKBACK_BARS = 6
MIN_CONVICTION_DECAY_ACTIVITY = 0.08
...
class LinearConvictionDecayModel:
    fold_id: str
    ...
    conviction_decay_failure_rate: float
...
def run_c0075_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_conviction_decay_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_conviction_decay_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        CONVICTION_DECAY_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    release = conviction_decay_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        release["opposing_run_stretch"],
        release["body_decay_ratio"],
        release["close_location_erosion"],
        release["counter_wick_pressure"],
        release["participation_decay"],
        release["path_efficiency_loss"],
        release["range_extension_pressure"],
        release["failed_followthrough_pressure"],
        release["prior_day_directional_context"],
        release["session_maturity_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        opposing_run_bucket(release["opposing_run_stretch"]),
        body_decay_bucket(release["body_decay_ratio"]),
        close_erosion_bucket(release["close_location_erosion"]),
        counter_wick_decay_bucket(release["counter_wick_pressure"]),
        participation_decay_bucket(release["participation_decay"]),
        path_efficiency_loss_bucket(release["path_efficiency_loss"]),
        conviction_range_extension_bucket(release["range_extension_pressure"]),
        failed_followthrough_bucket(release["failed_followthrough_pressure"]),
        prior_direction_context_bucket(release["prior_day_directional_context"]),
...
def conviction_decay_metrics(
    ...
    window_start = index - CONVICTION_DECAY_LOOKBACK_BARS + 1
    short_start = index - CONVICTION_DECAY_SHORT_LOOKBACK_BARS + 1
    previous_short_start = index - (2 * CONVICTION_DECAY_SHORT_LOOKBACK_BARS) + 1
    ...
    opposing_run_stretch = max(
        0.0,
        opposing_direction * (signal_bar.close - medium_window[0].open) / max(average_range, 1e-9),
    )
    previous_body_pressure = base.mean(
        [
            max(0.0, opposing_direction * (bar.close - bar.open) / max(bar.high - bar.low, 1e-9))
            for bar in previous_short_window
        ]
    ) or 0.0
    short_body_pressure = base.mean(
        [
            max(0.0, opposing_direction * (bar.close - bar.open) / max(bar.high - bar.low, 1e-9))
            for bar in short_window
        ]
    ) or 0.0
    body_decay_ratio = previous_body_pressure / max(short_body_pressure, 0.05)
    previous_close_pressure = base.mean(
        [directional_bar_settlement(bar, opposing_direction) for bar in previous_short_window]
    ) or 0.0
    short_close_pressure = base.mean(
        [directional_bar_settlement(bar, opposing_direction) for bar in short_window]
    ) or 0.0
    close_location_erosion = previous_close_pressure - short_close_pressure
    counter_wick_pressure = max(
        adverse_wick_absorption(signal_bar, direction),
        base.mean([adverse_wick_absorption(bar, direction) for bar in short_window]) or 0.0,
    )
    ...
    participation_decay = previous_volume_ratio - short_volume_ratio
    path_travel = sum(
        abs(medium_window[pos].close - medium_window[pos - 1].close)
        for pos in range(1, len(medium_window))
    )
    path_net = opposing_direction * (signal_bar.close - medium_window[0].close)
    opposing_path_efficiency = max(0.0, path_net / max(path_travel, 1e-9))
    path_efficiency_loss = opposing_run_stretch * (1.0 - min(opposing_path_efficiency, 1.0))
    range_extension_pressure = day_range / max(prior_day_range, average_range, 1e-9)
    failed_followthrough_pressure = (
        max(0.0, body_decay_ratio - 1.0)
        + max(0.0, close_location_erosion)
        + counter_wick_pressure
        + max(0.0, participation_decay)
        + path_efficiency_loss
    )
    prior_day_directional_context = opposing_direction * prior_day_return / max(prior_day_range, 1e-9)
    decay_pressure = max(
        opposing_run_stretch,
        max(0.0, body_decay_ratio - 1.0),
        max(0.0, close_location_erosion),
        ...
        max(0.0, participation_decay),
        path_efficiency_loss,
        abs(range_extension_pressure - 1.0),
        failed_followthrough_pressure,
        max(0.0, prior_day_directional_context),
    )
    session_maturity_pressure = bounded_session_progress(signal_bar.time) * decay_pressure
    activity = max(decay_pressure, session_maturity_pressure)
    if activity < MIN_CONVICTION_DECAY_ACTIVITY:
        return {"event_count": 0.0}
    if (
        opposing_run_stretch < 0.30
        and body_decay_ratio < 1.08
        and close_location_erosion < 0.05
        and failed_followthrough_pressure < 0.25
    ):
        return {"event_count": 0.0}
    if counter_wick_pressure < 0.06 and close_location_erosion < 0.02 and participation_decay < 0.02:
        return {"event_count": 0.0}
    return {
        "opposing_run_stretch": bounded(opposing_run_stretch, 0.0, 8.0),
        "body_decay_ratio": bounded(body_decay_ratio, 0.0, 8.0),
        "close_location_erosion": bounded(close_location_erosion, -1.0, 1.0),
        "counter_wick_pressure": bounded(counter_wick_pressure, 0.0, 1.0),
        "participation_decay": bounded(participation_decay, -4.0, 4.0),
        "path_efficiency_loss": bounded(path_efficiency_loss, 0.0, 8.0),
        "range_extension_pressure": bounded(range_extension_pressure, 0.0, 6.0),
        "failed_followthrough_pressure": bounded(failed_followthrough_pressure, 0.0, 10.0),
        "prior_day_directional_context": bounded(prior_day_directional_context, -2.0, 2.0),
...
def candidate_label(
    ...
    metrics = conviction_decay_metrics(
        bars,
        volumes,
    ...
    if metrics["event_count"] <= 0:
        return None
    opposing_run_reward = (
        0.14 if 0.55 <= metrics["opposing_run_stretch"] <= 3.20 and terminal > 0 else 0.0
    )
    body_decay_reward = (
        0.16 if metrics["body_decay_ratio"] > 1.18 and terminal > 0 else 0.0
    )
    close_erosion_reward = (
        0.14 if metrics["close_location_erosion"] > 0.08 and terminal > 0 else 0.0
    )
    counter_wick_reward = (
        0.12 if metrics["counter_wick_pressure"] > 0.18 and terminal > 0 else 0.0
    )
    participation_decay_reward = (
        0.10 if metrics["participation_decay"] > 0.05 and terminal > 0 else 0.0
    )
    path_loss_reward = (
        0.12 if metrics["path_efficiency_loss"] > 0.25 and terminal > 0 else 0.0
    )
    failed_followthrough_reward = (
        0.12 if metrics["failed_followthrough_pressure"] > 0.55 and terminal > 0 else 0.0
    )
    prior_context_reward = (
        0.06 if metrics["prior_day_directional_context"] > -0.20 and terminal > 0 else 0.0
    )
    thin_run_penalty = (
        0.16 if metrics["opposing_run_stretch"] < 0.28 and terminal <= 0 else 0.0
    )
    no_decay_penalty = (
        0.16 if metrics["body_decay_ratio"] < 0.92 and terminal <= 0 else 0.0
    )
    continuation_close_penalty = (
        0.14 if metrics["close_location_erosion"] < -0.04 and terminal <= 0 else 0.0
    )
    weak_counter_wick_penalty = (
        0.12 if metrics["counter_wick_pressure"] < 0.10 and terminal <= 0 else 0.0
    )
    participation_expansion_penalty = (
        0.10 if metrics["participation_decay"] < -0.08 and terminal <= 0 else 0.0
    )
    efficient_opposing_path_penalty = (
        0.12 if metrics["path_efficiency_loss"] < 0.12 and terminal <= 0 else 0.0
    )
    ...
    conviction_decay_path_quality = (0.44 * terminal + 0.36 * mfe - 0.72 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + conviction_decay_path_quality
        + opposing_run_reward
        + body_decay_reward
        + close_erosion_reward
        + counter_wick_reward
        ...
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["range_extension_pressure"] - 3.0) * 0.06
    )


def fit_linear_conviction_decay_model(candidates: list[base.Candidate], fold_id: str) -> LinearConvictionDecayModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearConvictionDecayModel(
            fold_id,
    ...
    conviction_decay_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearConvictionDecayModel(
        fold_id=fold_id,
        ...
        conviction_decay_failure_rate=conviction_decay_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearConvictionDecayModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_conviction_decay_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0075_r0001_intraday_conviction_decay",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0075_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0075_r0001_cd_summary.json",
        ],
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0075 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0075_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0075 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "conviction_decay_lookback_bars": CONVICTION_DECAY_LOOKBACK_BARS,
        "conviction_decay_short_lookback_bars": CONVICTION_DECAY_SHORT_LOOKBACK_BARS,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_conviction_decay_model_summary(model: LinearConvictionDecayModel) -> dict[str, object]:
    return {
        ...
        "conviction_decay_failure_rate": rounded(model.conviction_decay_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearConvictionDecayModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_conviction_decay_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "conviction_decay_profile": profiles["conviction_decay_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0075_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0075_r0001_proxy_trades.csv"
    evidence["conviction_decay_summary"] = "artifacts/c0075_r0001_cd_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0075_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0075_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0075 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0075_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0075_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0075_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0075_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0075_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0075_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0075_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0075_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0075_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0075 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0075 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0075_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0075_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0075_r0001_intraday_conviction_decay_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0075_r0001_proxy_records_conviction_decay_reference_surface
  ...
  - next_work_is_produce_c0075_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0075_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0075-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0075_r0001_intraday_conviction_decay",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0075-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0075-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0075-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0075_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0075_r0001_intraday_conviction_decay",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0075-R0001-conviction-decay-SUMMARY",
            "artifact_role": "conviction_decay_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0075_r0001_cd_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0075_r0001_intraday_conviction_decay",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0075_r0001_mt5_logic_parity_evidence",
        }
    ]
