"""C0073 R0001 proxy evidence for intraday spread shock relief."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0073_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0073_r0001_ssr_summary.json"
...
FEATURE_NAMES = (
    "short_spread_shock_ratio",
    "medium_spread_shock_ratio",
    "spread_relief_slope",
    "friction_range_compression",
    "price_recovery_alignment",
    "post_shock_settlement",
    "shock_exhaustion_pressure",
    "volume_confirmation_after_relief",
    "prior_day_range_context",
    "session_maturity_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_spread_shock_relief_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_spread_shock_relief_state"
SELECTION_RULE = "top_fold_local_spread_shock_relief_scores_per_active_day"
...
MIN_DAY_BARS = 30
SPREAD_SHOCK_RELIEF_LOOKBACK_BARS = 20
SPREAD_SHOCK_RELIEF_SHORT_LOOKBACK_BARS = 6
MIN_SPREAD_SHOCK_RELIEF_ACTIVITY = 0.06
...
class LinearSpreadShockReliefModel:
    fold_id: str
    ...
    spread_shock_relief_failure_rate: float
...
def run_c0073_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_spread_shock_relief_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_spread_shock_relief_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        SPREAD_SHOCK_RELIEF_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    release = spread_shock_relief_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        release["short_spread_shock_ratio"],
        release["medium_spread_shock_ratio"],
        release["spread_relief_slope"],
        release["friction_range_compression"],
        release["price_recovery_alignment"],
        release["post_shock_settlement"],
        release["shock_exhaustion_pressure"],
        release["volume_confirmation_after_relief"],
        release["prior_day_range_context"],
        release["session_maturity_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        short_spread_shock_bucket(release["short_spread_shock_ratio"]),
        medium_spread_shock_bucket(release["medium_spread_shock_ratio"]),
        spread_relief_slope_bucket(release["spread_relief_slope"]),
        friction_range_bucket(release["friction_range_compression"]),
        price_recovery_bucket(release["price_recovery_alignment"]),
        post_shock_settlement_bucket(release["post_shock_settlement"]),
        shock_exhaustion_bucket(release["shock_exhaustion_pressure"]),
        relief_volume_bucket(release["volume_confirmation_after_relief"]),
        prior_day_range_context_bucket(release["prior_day_range_context"]),
...
def spread_shock_relief_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    if prior_volume_per_bar is None or prior_volume_per_bar <= 0 or prior_day_range is None or prior_day_range <= 0:
        return {"event_count": 0.0}
    window_start = index - SPREAD_SHOCK_RELIEF_LOOKBACK_BARS + 1
    short_start = index - SPREAD_SHOCK_RELIEF_SHORT_LOOKBACK_BARS + 1
    previous_short_start = index - (2 * SPREAD_SHOCK_RELIEF_SHORT_LOOKBACK_BARS) + 1
    ...
    previous_volume_window = volumes[previous_short_start:previous_short_end]
    short_spreads = [bar.spread_points for bar in short_window]
    medium_spreads = [bar.spread_points for bar in medium_window]
    previous_spreads = [bar.spread_points for bar in previous_short_window]
    short_ranges = [max(bar.high - bar.low, 1e-9) for bar in short_window]
    previous_ranges = [max(bar.high - bar.low, 1e-9) for bar in previous_short_window]
    short_spread = base.mean(short_spreads) or signal_bar.spread_points
    previous_spread = base.mean(previous_spreads) or short_spread
    medium_spread = base.mean(medium_spreads) or short_spread
    max_medium_spread = max(medium_spreads)
    baseline_spread = max(medium_spread, previous_spread, average_range * 0.01, 1e-9)
    short_range_mean = base.mean(short_ranges) or average_range
    previous_range_mean = base.mean(previous_ranges) or average_range

    short_spread_shock_ratio = short_spread / baseline_spread
    medium_spread_shock_ratio = max_medium_spread / baseline_spread
    spread_relief_slope = (previous_spread - short_spread) / baseline_spread
    friction_range_compression = previous_spread / max(short_range_mean, 1e-9)
    signal_body_efficiency = direction * (signal_bar.close - signal_bar.open) / signal_range
    price_recovery_alignment = direction * (signal_bar.close - short_window[0].open) / max(average_range, 1e-9)
    post_shock_settlement = directional_bar_settlement(signal_bar, direction)
    shock_exhaustion_pressure = max(0.0, medium_spread_shock_ratio - short_spread_shock_ratio) * max(
        0.0,
        post_shock_settlement - 0.45,
    )
    ...
    volume_confirmation_after_relief = (short_volume_ratio - previous_volume_ratio) * max(
        spread_relief_slope,
        0.0,
    )
    prior_day_range_context = prior_day_range / max(average_range, 1e-9)
    relief_pressure = max(
        abs(short_spread_shock_ratio - 1.0),
        abs(medium_spread_shock_ratio - 1.0),
        abs(spread_relief_slope),
        friction_range_compression,
        abs(price_recovery_alignment),
        abs(post_shock_settlement - 0.5),
        shock_exhaustion_pressure,
        abs(volume_confirmation_after_relief),
        abs(prior_day_range_context - 1.0),
    )
    session_maturity_pressure = bounded_session_progress(signal_bar.time) * max(
        relief_pressure,
        max(spread_relief_slope, 0.0),
    ...
    if activity < MIN_SPREAD_SHOCK_RELIEF_ACTIVITY:
        return {"event_count": 0.0}
    if (
        medium_spread_shock_ratio < 1.03
        and abs(spread_relief_slope) < 0.04
        and shock_exhaustion_pressure < 0.03
        and abs(price_recovery_alignment) < 0.06
        and abs(volume_confirmation_after_relief) < 0.04
    ):
        return {"event_count": 0.0}
    return {
        "short_spread_shock_ratio": bounded(short_spread_shock_ratio, 0.0, 6.0),
        "medium_spread_shock_ratio": bounded(medium_spread_shock_ratio, 0.0, 6.0),
        "spread_relief_slope": bounded(spread_relief_slope, -4.0, 4.0),
        "friction_range_compression": bounded(friction_range_compression, 0.0, 2.0),
        "price_recovery_alignment": bounded(price_recovery_alignment, -4.0, 4.0),
        "post_shock_settlement": bounded(post_shock_settlement, 0.0, 1.0),
        "shock_exhaustion_pressure": bounded(shock_exhaustion_pressure, 0.0, 4.0),
        "volume_confirmation_after_relief": bounded(volume_confirmation_after_relief, -4.0, 4.0),
        "prior_day_range_context": bounded(prior_day_range_context, 0.0, 4.0),
...
def short_spread_shock_bucket(value: float) -> str:
    if value < 0.70:
        return "short_spread_relief_low"
    if value < 1.15:
        return "short_spread_normal"
    return "short_spread_shocked"


def medium_spread_shock_bucket(value: float) -> str:
    if value < 1.10:
        return "medium_spread_calm"
    if value < 1.85:
        return "medium_spread_elevated"
    return "medium_spread_shock"


def spread_relief_slope_bucket(value: float) -> str:
    if value < -0.08:
        return "spread_rebuilding"
    if value < 0.08:
        return "spread_flat"
    return "spread_relieving"


def friction_range_bucket(value: float) -> str:
    if value < 0.035:
        return "friction_range_low"
    if value < 0.080:
        return "friction_range_mid"
    return "friction_range_high"


def price_recovery_bucket(value: float) -> str:
    if value < -0.20:
        return "price_recovery_adverse"
    if value < 0.20:
        return "price_recovery_flat"
    return "price_recovery_aligned"


def post_shock_settlement_bucket(value: float) -> str:
    if value < 0.42:
        return "post_shock_settlement_weak"
    if value < 0.65:
        return "post_shock_settlement_mixed"
    return "post_shock_settlement_strong"


def shock_exhaustion_bucket(value: float) -> str:
    if value < 0.06:
        return "shock_exhaustion_low"
    if value < 0.25:
        return "shock_exhaustion_mid"
    return "shock_exhaustion_high"


def relief_volume_bucket(value: float) -> str:
    if value < -0.08:
        return "relief_volume_fading"
    if value < 0.10:
        return "relief_volume_flat"
    return "relief_volume_confirmed"
...
def candidate_label(
    ...
    metrics = spread_shock_relief_metrics(
        bars,
        volumes,
    ...
    if metrics["event_count"] <= 0:
        return None
    spread_relief_reward = (
        0.18 if metrics["spread_relief_slope"] > 0.08 and terminal > 0 else 0.0
    )
    shock_exhaustion_reward = (
        0.16 if metrics["shock_exhaustion_pressure"] > 0.08 and terminal > 0 else 0.0
    )
    price_recovery_reward = (
        0.14 if metrics["price_recovery_alignment"] > 0.10 and terminal > 0 else 0.0
    )
    settlement_reward = (
        0.12 if metrics["post_shock_settlement"] > 0.62 and terminal > 0 else 0.0
    )
    friction_compression_reward = (
        0.08 if metrics["friction_range_compression"] < 0.07 and terminal > 0 else 0.0
    )
    volume_relief_reward = (
        0.08 if metrics["volume_confirmation_after_relief"] > 0.06 and terminal > 0 else 0.0
    )
    prior_range_reward = (
        0.06 if 0.70 <= metrics["prior_day_range_context"] <= 2.20 and terminal > 0 else 0.0
    )
    spread_rebuild_penalty = (
        0.18 if metrics["spread_relief_slope"] < -0.06 and terminal <= 0 else 0.0
    )
    persistent_shock_penalty = (
        0.16 if metrics["short_spread_shock_ratio"] > 1.20 and terminal <= 0 else 0.0
    )
    adverse_price_recovery_penalty = (
        0.16 if metrics["price_recovery_alignment"] < -0.08 and terminal <= 0 else 0.0
    )
    weak_settlement_penalty = (
        0.14 if metrics["post_shock_settlement"] < 0.40 and terminal <= 0 else 0.0
    )
    weak_exhaustion_penalty = (
        0.12 if metrics["shock_exhaustion_pressure"] < 0.03 and terminal <= 0 else 0.0
    )
    volume_fade_penalty = (
        0.10 if metrics["volume_confirmation_after_relief"] < -0.06 and terminal <= 0 else 0.0
    )
    ...
    spread_shock_relief_path_quality = (0.44 * terminal + 0.36 * mfe - 0.72 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + spread_shock_relief_path_quality
        + spread_relief_reward
        + shock_exhaustion_reward
        + price_recovery_reward
        + settlement_reward
        ...
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["short_spread_shock_ratio"] - 1.5) * 0.05
    )


def fit_linear_spread_shock_relief_model(candidates: list[base.Candidate], fold_id: str) -> LinearSpreadShockReliefModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearSpreadShockReliefModel(
            fold_id,
    ...
    spread_shock_relief_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearSpreadShockReliefModel(
        fold_id=fold_id,
        ...
        spread_shock_relief_failure_rate=spread_shock_relief_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearSpreadShockReliefModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_spread_shock_relief_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0073_r0001_intraday_spread_shock_relief",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0073_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0073_r0001_ssr_summary.json",
        ],
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0073 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0073_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0073 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "spread_shock_relief_lookback_bars": SPREAD_SHOCK_RELIEF_LOOKBACK_BARS,
        "spread_shock_relief_short_lookback_bars": SPREAD_SHOCK_RELIEF_SHORT_LOOKBACK_BARS,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_spread_shock_relief_model_summary(model: LinearSpreadShockReliefModel) -> dict[str, object]:
    return {
        ...
        "spread_shock_relief_failure_rate": rounded(model.spread_shock_relief_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearSpreadShockReliefModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_spread_shock_relief_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "spread_shock_relief_profile": profiles["spread_shock_relief_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0073_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0073_r0001_proxy_trades.csv"
    evidence["spread_shock_relief_summary"] = "artifacts/c0073_r0001_ssr_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0073_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0073_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0073 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0073_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0073_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0073_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0073_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0073_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0073_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0073_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0073_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0073_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0073 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0073 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260706_produce_c0073_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0073_r0001_proxy_evidence
  refines:
  - dec_20260706_open_c0073_r0001_intraday_spread_shock_relief_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0073_r0001_proxy_records_spread_shock_relief_reference_surface
  ...
  - next_work_is_produce_c0073_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260706_produce_c0073_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0073-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0073_r0001_intraday_spread_shock_relief",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0073-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0073-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0073-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0073_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0073_r0001_intraday_spread_shock_relief",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0073-R0001-spread-shock-relief-SUMMARY",
            "artifact_role": "spread_shock_relief_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0073_r0001_ssr_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0073_r0001_intraday_spread_shock_relief",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0073_r0001_mt5_logic_parity_evidence",
        }
    ]
