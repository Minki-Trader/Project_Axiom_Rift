"""C0117 R0001 proxy evidence for intraday overnight inventory unwind."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0117_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0117_r0001_overnight_inventory_unwind_summary.json"
...
FEATURE_NAMES = (
    "overnight_position_pressure",
    "overnight_inventory_skew",
    "overnight_extension_pressure",
    "core_reentry_depth",
    "session_unwind_progress",
    "range_reversion_efficiency",
    "volume_unwind_ratio",
    "vwap_unwind_alignment",
    "day_open_tension",
    "prior_day_range_context",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_overnight_inventory_unwind_rank"
LABEL_SHAPE = "target_first_quality_after_overnight_inventory_unwind"
SELECTION_RULE = "top_fold_local_overnight_inventory_unwind_scores_per_active_day"
...
MIN_DAY_BARS = 18
OVERNIGHT_INVENTORY_UNWIND_LOOKBACK_BARS = 42
OVERNIGHT_INVENTORY_UNWIND_RECENT_BARS = 10
OVERNIGHT_INVENTORY_UNWIND_BASE_BARS = 14
MIN_OVERNIGHT_RANGE_MULTIPLE = 0.35
MIN_OVERNIGHT_INVENTORY_UNWIND_ACTIVITY = 0.060
...
class LinearOvernightInventoryUnwindModel:
    fold_id: str
    ...
    overnight_inventory_unwind_failure_rate: float
...
def run_c0117_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_overnight_inventory_unwind_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_overnight_inventory_unwind_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        OVERNIGHT_INVENTORY_UNWIND_LOOKBACK_BARS,
        OVERNIGHT_INVENTORY_UNWIND_RECENT_BARS + OVERNIGHT_INVENTORY_UNWIND_BASE_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = overnight_inventory_unwind_metrics(
    ...
    features = (
        metrics["overnight_position_pressure"],
        metrics["overnight_inventory_skew"],
        metrics["overnight_extension_pressure"],
        metrics["core_reentry_depth"],
        metrics["session_unwind_progress"],
        metrics["range_reversion_efficiency"],
        metrics["volume_unwind_ratio"],
        metrics["vwap_unwind_alignment"],
        metrics["day_open_tension"],
        metrics["prior_day_range_context"],
        metrics["session_maturity"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        overnight_position_pressure_bucket(metrics["overnight_position_pressure"]),
        overnight_inventory_skew_bucket(metrics["overnight_inventory_skew"]),
        overnight_extension_pressure_bucket(metrics["overnight_extension_pressure"]),
        core_reentry_depth_bucket(metrics["core_reentry_depth"]),
        session_unwind_progress_bucket(metrics["session_unwind_progress"]),
        range_reversion_efficiency_bucket(metrics["range_reversion_efficiency"]),
        volume_unwind_ratio_bucket(metrics["volume_unwind_ratio"]),
        vwap_unwind_alignment_bucket(metrics["vwap_unwind_alignment"]),
        day_open_tension_bucket(metrics["day_open_tension"]),
        prior_day_range_context_bucket(metrics["prior_day_range_context"]),
...
def overnight_inventory_unwind_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    if (
        context.overnight_high is None
        or context.overnight_low is None
        or context.overnight_close is None
        or context.overnight_range is None
        or context.overnight_return is None
    ):
        return {"event_count": 0.0}
    if context.overnight_range <= max(average_range * MIN_OVERNIGHT_RANGE_MULTIPLE, 1e-9):
        return {"event_count": 0.0}
    recent_start = max(index - OVERNIGHT_INVENTORY_UNWIND_RECENT_BARS + 1, context.day_start_index)
    base_start = max(recent_start - OVERNIGHT_INVENTORY_UNWIND_BASE_BARS, context.day_start_index)
    lookback_start = max(index - OVERNIGHT_INVENTORY_UNWIND_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    overnight_high = context.overnight_high
    overnight_low = context.overnight_low
    overnight_mid = (overnight_high + overnight_low) / 2.0
    overnight_range = context.overnight_range
    overnight_inventory_skew = -direction * context.overnight_return / max(overnight_range, 1e-9)
    if overnight_inventory_skew < -0.35:
        return {"event_count": 0.0}

    overnight_close_pressure = -direction * (context.overnight_close - overnight_mid) / average_range
    overnight_position_pressure = direction * (signal_bar.close - overnight_mid) / average_range
    overnight_extension_pressure = max(
        overnight_close_pressure,
        direction * (signal_bar.close - overnight_mid) / average_range,
    )
    if direction > 0:
        core_reentry_depth = (signal_bar.close - overnight_low) / average_range
        overnight_retest_room = (overnight_high - signal_bar.close) / average_range
    else:
        core_reentry_depth = (overnight_high - signal_bar.close) / average_range
        overnight_retest_room = (signal_bar.close - overnight_low) / average_range
    if core_reentry_depth < 0.10:
        return {"event_count": 0.0}

    session_unwind_progress = direction * (signal_bar.close - context.day_open) / average_range
    ...
    range_reversion_efficiency = recent_net / max(close_travel, average_range * 0.12)
    volume_unwind_ratio = recent_volume_mean / max(average_volume, 1e-9)
    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    vwap_unwind_alignment = 1.0 - abs(session_vwap - overnight_mid) / max(average_range, 1e-9)
    day_open_tension = direction * (signal_bar.close - context.day_open) / average_range
    prior_day_range_context = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_day_range_context = direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
    if context.prior_day_shadow_balance is not None:
        prior_day_range_context += direction * context.prior_day_shadow_balance * 0.08
    ...
    activity = max(
        max(overnight_inventory_skew + 0.20, 0.0) * 0.32,
        max(overnight_extension_pressure, 0.0) * 0.24,
        max(core_reentry_depth - 0.10, 0.0) * 0.28,
        max(session_unwind_progress, 0.0) * 0.24,
        max(range_reversion_efficiency, 0.0) * 0.22,
        max(volume_unwind_ratio - 0.78, 0.0) * 0.08,
        max(vwap_unwind_alignment - 0.10, 0.0) * 0.12,
        max(day_open_tension, 0.0) * 0.12,
        max(overnight_retest_room, 0.0) * 0.08,
    )
    if activity < MIN_OVERNIGHT_INVENTORY_UNWIND_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "overnight_position_pressure": bounded(overnight_position_pressure, -4.0, 8.0),
        "overnight_inventory_skew": bounded(overnight_inventory_skew, -3.0, 3.0),
        "overnight_extension_pressure": bounded(overnight_extension_pressure, -4.0, 8.0),
        "core_reentry_depth": bounded(core_reentry_depth, -2.0, 8.0),
        "session_unwind_progress": bounded(session_unwind_progress, -4.0, 8.0),
        "range_reversion_efficiency": bounded(range_reversion_efficiency, -2.0, 2.0),
        "volume_unwind_ratio": bounded(volume_unwind_ratio, 0.0, 8.0),
        "vwap_unwind_alignment": bounded(vwap_unwind_alignment, -4.0, 1.0),
        "day_open_tension": bounded(day_open_tension, -4.0, 8.0),
        "prior_day_range_context": bounded(prior_day_range_context, -3.0, 3.0),
        ...
        "overnight_range_ratio": bounded(overnight_range / max(average_range, 1e-9), 0.0, 12.0),
        "overnight_retest_room": bounded(overnight_retest_room, -4.0, 8.0),
        "day_range_so_far": day_range_so_far,
        "overnight_inventory_unwind_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0117_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0117_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0117_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0117_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0117_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0117_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0117_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0117_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0117_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0117_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0117_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0117_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0117_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0117_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0117_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0117_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0117_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0117_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0117_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def c0117_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    inventory_reward = 0.12 if metrics["overnight_inventory_skew"] > 0.25 and terminal > 0 else 0.0
    extension_reward = 0.12 if metrics["overnight_extension_pressure"] > 0.28 and terminal > 0 else 0.0
    reentry_reward = 0.10 if metrics["core_reentry_depth"] > 0.35 and terminal > 0 else 0.0
    progress_reward = 0.10 if metrics["session_unwind_progress"] > 0.18 and terminal > 0 else 0.0
    efficiency_reward = 0.08 if metrics["range_reversion_efficiency"] > 0.18 and terminal > 0 else 0.0
    volume_reward = 0.06 if 0.85 <= metrics["volume_unwind_ratio"] <= 1.90 and terminal > 0 else 0.0
    vwap_reward = 0.06 if metrics["vwap_unwind_alignment"] > 0.15 and terminal > 0 else 0.0
    day_open_reward = 0.06 if metrics["day_open_tension"] > 0.10 and terminal > 0 else 0.0
    weak_inventory_penalty = 0.10 if metrics["overnight_inventory_skew"] < -0.10 and terminal <= 0 else 0.0
    weak_extension_penalty = 0.10 if metrics["overnight_extension_pressure"] < 0.05 and terminal <= 0 else 0.0
    weak_reentry_penalty = 0.08 if metrics["core_reentry_depth"] < 0.18 and terminal <= 0 else 0.0
    weak_progress_penalty = 0.08 if metrics["session_unwind_progress"] < -0.05 and terminal <= 0 else 0.0
    weak_efficiency_penalty = 0.07 if metrics["range_reversion_efficiency"] < -0.05 and terminal <= 0 else 0.0
    overheated_volume_penalty = 0.06 if metrics["volume_unwind_ratio"] > 2.60 and terminal <= 0 else 0.0
    ...
    overnight_inventory_unwind_path_quality = (0.62 * terminal + 0.28 * mfe - 0.65 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + overnight_inventory_unwind_path_quality
        + inventory_reward
        + extension_reward
        + reentry_reward
        + progress_reward
        ...
        - max(0.0, 0.18 - metrics["core_reentry_depth"]) * 0.12
    )


def fit_linear_overnight_inventory_unwind_model(candidates: list[base.Candidate], fold_id: str) -> LinearOvernightInventoryUnwindModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearOvernightInventoryUnwindModel(
            fold_id,
    ...
    overnight_inventory_unwind_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearOvernightInventoryUnwindModel(
        fold_id=fold_id,
        ...
        overnight_inventory_unwind_failure_rate=overnight_inventory_unwind_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearOvernightInventoryUnwindModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_overnight_inventory_unwind_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0117_r0001_intraday_overnight_inventory_unwind",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0117_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0117_r0001_overnight_inventory_unwind_summary.json",
        ],
        ...
        "conditional_profiles": {
            "overnight_inventory_unwind_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0117 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0117_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0117 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "overnight_inventory_unwind_lookback_bars": OVERNIGHT_INVENTORY_UNWIND_LOOKBACK_BARS,
        "overnight_inventory_unwind_recent_bars": OVERNIGHT_INVENTORY_UNWIND_RECENT_BARS,
        "overnight_inventory_unwind_base_bars": OVERNIGHT_INVENTORY_UNWIND_BASE_BARS,
        "min_overnight_range_multiple": MIN_OVERNIGHT_RANGE_MULTIPLE,
        "min_overnight_inventory_unwind_activity": MIN_OVERNIGHT_INVENTORY_UNWIND_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_overnight_inventory_unwind_model_summary(model: LinearOvernightInventoryUnwindModel) -> dict[str, object]:
    return {
        ...
        "overnight_inventory_unwind_failure_rate": rounded(model.overnight_inventory_unwind_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearOvernightInventoryUnwindModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_overnight_inventory_unwind_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "overnight_inventory_unwind_profile": profiles["overnight_inventory_unwind_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0117_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0117_r0001_proxy_trades.csv"
    evidence["overnight_inventory_unwind_summary"] = "artifacts/c0117_r0001_overnight_inventory_unwind_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0117_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0117_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0117 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0117_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0117_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0117_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0117_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0117_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0117_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0117_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0117_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0117_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0117 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0117 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0117_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0117_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0117_r0001_intraday_overnight_inventory_unwind_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0117_r0001_proxy_records_overnight_inventory_unwind_reference_surface
  ...
  - next_work_is_produce_c0117_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0117_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0117-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0117_r0001_intraday_overnight_inventory_unwind",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0117-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0117-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0117-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0117_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0117_r0001_intraday_overnight_inventory_unwind",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0117-R0001-OVERNIGHT-INVENTORY-UNWIND-SUMMARY",
            "artifact_role": "overnight_inventory_unwind_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0117_r0001_overnight_inventory_unwind_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0117_r0001_intraday_overnight_inventory_unwind",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0117_r0001_mt5_logic_parity_evidence",
        }
    ]
