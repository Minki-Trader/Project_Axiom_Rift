"""R0001 proxy evidence for volatility-expansion continuation."""
...
def run_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_payload(trades: list[Trade], windows: list[Window]) -> dict[str, object]:
    ...
    return {
        ...
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.r0001_volatility_expansion",
        "proxy_config_path": "campaigns/C0001_regime_response_discovery/runs/R0001/run_manifest.json",
        ...
        "deferred_with_reason": [
            {
                ...
                "reason": "R0001 proxy currently measures raw US100 index points only; USD conversion requires MT5 symbol contract, tick value, and fixed-lot discovery sizing",
                "blocking_condition": "record the matching MT5 symbol specification and fixed-lot sizing for the 500 USD account",
                "revisit_when": "during R0001 MT5 probe evidence production",
            ...
            },
            {
                ...
                "revisit_when": "before R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            }
        ],
        "claim_boundary": {
...
def update_artifact_lineage(proxy_hash: str) -> None:
    ...
    for record in data.get("artifact_records", []):
        if record.get("artifact_id") == "A0001":
            ...
            record["produced_by"] = "axiom_rift.proxies.r0001_volatility_expansion"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_matching_mt5_probe_for_r0001"
    data["deferred_with_reason"] = [
        {
            "field": "decision",
            "reason": "proxy evidence is recorded but MT5 and proxy-vs-MT5 evidence are still missing",
            "blocking_condition": "R0001 cannot close until the mandatory MT5 probe and proxy-vs-MT5 comparison are recorded",
            "revisit_when": "after R0001 MT5 and proxy-vs-MT5 evidence files contain measured results",
        }
    ]
