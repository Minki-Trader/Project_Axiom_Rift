"""C0035 R0001 proxy evidence for intraday cycle-phase resonance discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0035_intraday_cycle_phase_resonance_discovery" / "runs" / "R0001"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0035_intraday_cycle_phase_resonance_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0035_r0001_proxy_trades.csv"
CYCLE_PHASE_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0035_r0001_cycle_phase_summary.json"
...
FEATURE_NAMES = (
    ...
    "directional_oscillator_12",
    "directional_oscillator_24",
    "directional_oscillator_48",
    "directional_oscillator_96",
    "directional_oscillator_slope_6",
    "directional_oscillator_slope_12",
    "directional_oscillator_curvature_6",
    "phase_turn_pressure_12",
    "phase_turn_pressure_24",
    "phase_continuation_pressure_12",
    "phase_continuation_pressure_24",
    "amplitude_ratio_12",
    "amplitude_ratio_24",
    "amplitude_ratio_48",
    "autocorr_quality_12",
    "autocorr_quality_24",
    "return_alternation_12",
    "zero_cross_recency_24",
    "phase_energy_shift_12_vs_48",
    "reversal_inflection_score_6",
    "trend_resonance_score_12",
...
MODEL_FAMILY = "fold_local_intraday_cycle_phase_resonance_rank"
LABEL_SHAPE = "directional_intraday_cycle_phase_resonance_survival_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_intraday_cycle_phase_resonance_scores_per_active_day"
...
def run_c0035_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    cycle_context = build_cycle_context(bars, range_average, short_range_average, day_context)
    trades: list[base.Trade] = []
...
def build_candidates(
    ...
    cycle_context: list[dict[str, float] | None],
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = intraday_cycle_phase_resonance_features(
                bars,
                range_average,
                short_range_average,
                day_context,
            ...
            if features is None:
                continue
            label = (
                intraday_cycle_phase_resonance_label(bars, range_average, cycle_context, index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|intraday_cycle_phase_resonance",
                    features=features,
                    label=label,
                )
            )
...
def build_cycle_context(
    ...
    for index, bar in enumerate(bars):
        ...
        if index < 120 or average_range is None or average_range <= 0 or short_average_range is None or day is None:
            continue
        osc12 = oscillator_position(bars, index, 12, average_range)
        osc24 = oscillator_position(bars, index, 24, average_range)
        osc48 = oscillator_position(bars, index, 48, average_range)
        osc96 = oscillator_position(bars, index, 96, average_range)
        osc12_3 = oscillator_position(bars, index - 3, 12, average_range)
        osc12_6 = oscillator_position(bars, index - 6, 12, average_range)
        osc24_12 = oscillator_position(bars, index - 12, 24, average_range)
        range_3 = mean_bar_range(bars, index, 3)
        range_24 = mean_bar_range(bars, index, 24)
        range_48 = mean_bar_range(bars, index, 48)
        amp12 = close_amplitude_ratio(bars, index, 12, average_range)
        amp24 = close_amplitude_ratio(bars, index, 24, average_range)
        amp48 = close_amplitude_ratio(bars, index, 48, average_range)
        contexts[index] = {
            "oscillator_12": osc12,
            "oscillator_24": osc24,
            "oscillator_48": osc48,
            "oscillator_96": osc96,
            "oscillator_slope_6": osc12 - osc12_6,
            "oscillator_slope_12": osc24 - osc24_12,
            "oscillator_curvature_6": (osc12 - osc12_3) - (osc12_3 - osc12_6),
            "amplitude_ratio_12": amp12,
            "amplitude_ratio_24": amp24,
            "amplitude_ratio_48": amp48,
            "autocorr_quality_12": return_autocorr(bars, index, 12, 3),
            "autocorr_quality_24": return_autocorr(bars, index, 24, 6),
            "return_alternation_12": return_alternation(bars, index, 12),
            "zero_cross_recency_24": zero_cross_recency(bars, index, 24, average_range),
            "phase_energy_shift_12_vs_48": amp12 / max(amp48, 1e-9) - 1.0,
            "range_expansion_3_vs_24": range_3 / max(range_24, 1e-9),
            "range_compression_6_vs_48": mean_bar_range(bars, index, 6) / max(range_48, 1e-9),
...
def oscillator_position(bars: list[base.Bar], index: int, lookback: int, average_range: float) -> float:
    return (bars[index].close - mean_close(bars, index, lookback)) / max(average_range, 1e-9)


def mean_close(bars: list[base.Bar], index: int, lookback: int) -> float:
    return sum(bars[offset].close for offset in range(index - lookback + 1, index + 1)) / lookback


def close_amplitude_ratio(bars: list[base.Bar], index: int, lookback: int, average_range: float) -> float:
    closes = [bars[offset].close for offset in range(index - lookback + 1, index + 1)]
    return (max(closes) - min(closes)) / max(average_range, 1e-9)


def return_autocorr(bars: list[base.Bar], index: int, lookback: int, lag: int) -> float:
    returns = [bars[offset].close - bars[offset - 1].close for offset in range(index - lookback + 1, index + 1)]
    if len(returns) <= lag:
        return 0.0
    left = returns[lag:]
    right = returns[:-lag]
    ...
    if denom <= 1e-12:
        return 0.0
    return bounded(numerator / denom, -1.0, 1.0)


def return_alternation(bars: list[base.Bar], index: int, lookback: int) -> float:
...
def zero_cross_recency(bars: list[base.Bar], index: int, lookback: int, average_range: float) -> float:
    previous = oscillator_position(bars, index - lookback, 12, average_range)
    last_cross_offset: int | None = None
    for offset in range(index - lookback + 1, index + 1):
        current = oscillator_position(bars, offset, 12, average_range)
        if previous == 0.0 or current == 0.0 or (previous < 0.0 < current) or (previous > 0.0 > current):
            last_cross_offset = offset
        previous = current
    if last_cross_offset is None:
        return 0.0
    return 1.0 - min(index - last_cross_offset, lookback) / lookback


def intraday_cycle_phase_resonance_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    if average_range is None or average_range <= 0 or short_average_range is None or short_average_range <= 0:
        return None
    day = day_context[index]
    cycle = cycle_context[index]
    if day is None or cycle is None:
        return None
    spread_over_range = bars[index].spread_points / average_range
    if spread_over_range > 0.65:
        return None
    osc12 = cycle["oscillator_12"]
    osc24 = cycle["oscillator_24"]
    slope6 = cycle["oscillator_slope_6"]
    slope12 = cycle["oscillator_slope_12"]
    curvature6 = cycle["oscillator_curvature_6"]
    turn12 = max(0.0, -direction * osc12) * max(0.0, direction * slope6)
    turn24 = max(0.0, -direction * osc24) * max(0.0, direction * slope12)
    continuation12 = max(0.0, direction * osc12) * max(0.0, direction * slope6)
    continuation24 = max(0.0, direction * osc24) * max(0.0, direction * slope12)
    reversal_inflection = max(0.0, -direction * osc12) * max(0.0, direction * curvature6)
    trend_resonance = max(0.0, direction * osc24) * max(0.0, direction * slope12) * max(
        0.0, cycle["autocorr_quality_24"]
    )
    features = (
        float(direction),
        ...
        direction * osc12,
        direction * osc24,
        direction * cycle["oscillator_48"],
        direction * cycle["oscillator_96"],
        direction * slope6,
        direction * slope12,
        direction * curvature6,
        turn12,
        turn24,
        continuation12,
        continuation24,
        cycle["amplitude_ratio_12"],
        cycle["amplitude_ratio_24"],
        cycle["amplitude_ratio_48"],
        cycle["autocorr_quality_12"],
        cycle["autocorr_quality_24"],
        cycle["return_alternation_12"],
        cycle["zero_cross_recency_24"],
        cycle["phase_energy_shift_12_vs_48"],
        reversal_inflection,
        trend_resonance,
        directional_close_location(bars[index], direction),
        directional_close_location_mean(bars, index, direction, 3),
        directional_close_streak(bars, index, direction, 6),
        ...
        cycle["range_expansion_3_vs_24"],
        cycle["range_compression_6_vs_48"],
        day_range_position_directional(bars, day, index, direction),
        cycle["session_progress"],
        spread_over_range,
    )
    if len(features) != len(FEATURE_NAMES):
        raise RuntimeError(f"C0035 feature length mismatch: {len(features)} != {len(FEATURE_NAMES)}")
    return tuple(float(value) for value in features)


def intraday_cycle_phase_resonance_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    cycle_context: list[dict[str, float] | None],
    index: int,
    ...
    future_phase_acceptance = future_directional_phase_acceptance(
        bars, cycle_context, entry_index, exit_index, direction, average_range
    )
    future_phase_retention = future_cycle_phase_retention(cycle_context, entry_index, exit_index, direction)
    ...
    return (
        ...
        + 0.30 * future_phase_acceptance
        + 0.20 * future_phase_retention
...
def future_cycle_phase_retention(
    cycle_context: list[dict[str, float] | None],
    entry_index: int,
    exit_index: int,
    direction: int,
    ...
    for offset in range(entry_index, exit_index + 1):
        context = cycle_context[offset]
        if context is None:
            continue
        values.append(
            0.28 * direction * context["oscillator_12"]
            + 0.24 * direction * context["oscillator_24"]
            + 0.22 * direction * context["oscillator_slope_6"]
            + 0.18 * direction * context["oscillator_slope_12"]
            + 0.14 * max(0.0, context["autocorr_quality_24"])
            - 0.16 * context["return_alternation_12"]
            - 0.12 * context["zero_cross_recency_24"]
        )
    return sum(values) / max(len(values), 1)


def future_directional_phase_acceptance(
    bars: list[base.Bar],
    cycle_context: list[dict[str, float] | None],
    entry_index: int,
    exit_index: int,
    ...
    for offset in range(entry_index, exit_index + 1):
        ...
        values.append(
            move
            + 0.22 * direction * context["oscillator_slope_6"]
            + 0.18 * direction * context["oscillator_12"]
            + 0.12 * max(0.0, context["autocorr_quality_12"])
            - 0.14 * context["return_alternation_12"]
            - 0.10 * context["spread_over_range"]
        )
    if not values:
        return 0.0
...
def linear_model_summary(model: LinearEdgeModel) -> dict[str, object]:
    return {
        ...
        "score_interpretation": "higher_score_means_direct_fold_local_intraday_cycle_phase_resonance_rank",
    }
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0035"
    payload["work_unit_id"] = "C0035"
    payload["run_id"] = "R0001"
    payload["proxy_id"] = "PX-C0035-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0035_r0001_intraday_cycle_phase_resonance"
    payload["proxy_config_path"] = "campaigns/C0035_intraday_cycle_phase_resonance_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_intraday_cycle_phase_resonance_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0035_intraday_cycle_phase_resonance_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0035_intraday_cycle_phase_resonance_discovery/runs/R0001/artifacts/c0035_r0001_proxy_trades.csv",
        "campaigns/C0035_intraday_cycle_phase_resonance_discovery/runs/R0001/artifacts/c0035_r0001_cycle_phase_summary.json",
    ]
    ...
    profiles["intraday_cycle_phase_resonance_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
            "model_family": MODEL_FAMILY,
            "feature_count": len(FEATURE_NAMES),
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0035_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "score_interpretation": "higher_score_means_direct_fold_local_intraday_cycle_phase_resonance_rank",
            "variant_boundary": (
                "intraday_cycle_phase_resonance_rank_not_effort_absorption_seasonal_residual_tail_risk_skew_entropy_"
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_cycle_phase_summary_artifact(payload, CYCLE_PHASE_SUMMARY_PATH)
    write_text_local(PROXY_PATH, json.dumps(payload, indent=2, sort_keys=True) + "\n")
    trade_hash = sha256_file_local(TRADE_ARTIFACT_PATH)
    summary_hash = sha256_file_local(CYCLE_PHASE_SUMMARY_PATH)
...
def write_cycle_phase_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_intraday_cycle_phase_resonance_summary_v1",
        "template": False,
        "work_unit_id": "C0035",
        "campaign_id": "C0035",
        "run_id": "R0001",
        ...
        "intraday_cycle_phase_resonance_profile": profiles["intraday_cycle_phase_resonance_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "intraday_cycle_phase_resonance_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0035-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0035_intraday_cycle_phase_resonance_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0035_intraday_cycle_phase_resonance_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0035-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0035-R0001-CYCLE-PHASE-SUMMARY",
                "intraday_cycle_phase_resonance_summary_artifact",
                "json",
                "campaigns/C0035_intraday_cycle_phase_resonance_discovery/runs/R0001/artifacts/c0035_r0001_cycle_phase_summary.json",
                summary_hash,
                ["campaigns/C0035_intraday_cycle_phase_resonance_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0035_r0001_intraday_cycle_phase_resonance",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0035_r0001_proxy_trades.csv"
    evidence["intraday_cycle_phase_resonance_summary"] = "artifacts/c0035_r0001_cycle_phase_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0035_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0035_r0001_proxy_trades.csv",
        "artifacts/c0035_r0001_cycle_phase_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0035 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0035_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0035_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0035_intraday_cycle_phase_resonance_discovery"
    data["project"]["active_run"] = "campaigns/C0035_intraday_cycle_phase_resonance_discovery/runs/R0001"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0035_intraday_cycle_phase_resonance_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0035_intraday_cycle_phase_resonance_discovery",
        "open_c0035_r0001_fold_local_intraday_cycle_phase_resonance_rank_run",
        "produce_c0035_r0001_proxy_evidence",
    ):
    ...
    data["active_campaign"] = "campaigns/C0035_intraday_cycle_phase_resonance_discovery"
    data["active_run"] = "campaigns/C0035_intraday_cycle_phase_resonance_discovery/runs/R0001"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0035_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0035_intraday_cycle_phase_resonance_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0035_r0001_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["canonical_source"] = "campaigns/C0035_intraday_cycle_phase_resonance_discovery/runs/R0001/kpi/proxy.json"
    data["canonical_status"] = "proxy_recorded_pending_mt5"
    data["current_decision"] = "produce_c0035_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = "campaigns/C0035_intraday_cycle_phase_resonance_discovery"
    data["active_run"] = "campaigns/C0035_intraday_cycle_phase_resonance_discovery/runs/R0001"
    data["next_required_action"] = "produce_c0035_r0001_mt5_logic_parity_evidence"
    data["current_evidence_summary"] = {
        "source_campaign": "campaigns/C0035_intraday_cycle_phase_resonance_discovery",
        "current_task": "produce_c0035_r0001_mt5_logic_parity_evidence",
        "active_run": "campaigns/C0035_intraday_cycle_phase_resonance_discovery/runs/R0001",
        "active_run_status": "proxy_recorded_pending_mt5",
        "evidence_status": "proxy_recorded_pending_mt5",
        "campaign_family": "intraday_cycle_phase_resonance",
        "entries_per_active_day": summary.get("entries_per_active_day") if isinstance(summary, dict) else None,
        "proxy_trade_count": required.get("proxy_trade_count") if isinstance(required, dict) else None,
        "proxy_net_pnl_points": required.get("proxy_net_pnl_points") if isinstance(required, dict) else None,
        "proxy_profit_factor": required.get("proxy_profit_factor") if isinstance(required, dict) else None,
        "next_required_action": "produce_c0035_r0001_mt5_logic_parity_evidence",
        "note": "C0035 R0001 proxy evidence recorded; MT5 paired validation remains mandatory regardless of proxy result.",
        "source_campaign_manifest": "campaigns/C0035_intraday_cycle_phase_resonance_discovery/campaign.yaml",
    }
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0035",
            "c0004_r0001_fold_local_state_archetype": "c0035_r0001_intraday_cycle_phase_resonance",
            "c0004_r0001": "c0035_r0001",
            "fold_local_state_archetype_discovery": "intraday_cycle_phase_resonance_discovery",
            ...
            "state_archetype_summary": "intraday_cycle_phase_resonance_summary",
        }
