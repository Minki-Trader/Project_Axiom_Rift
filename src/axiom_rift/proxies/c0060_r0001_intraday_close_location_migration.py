"""C0060 R0001 proxy evidence for intraday close location migration."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0060_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0060_r0001_close_location_migration_summary.json"
...
FEATURE_NAMES = (
    "close_location_level",
    "close_location_migration_18",
    "recent_close_location_shift_6",
    "close_location_persistence",
    "close_location_rotation_pressure",
    "close_location_variance_inverse",
    "range_position_migration",
    "directional_settlement_efficiency",
    "volume_confirmed_close_shift",
    "day_range_completion_ratio",
    "session_progress_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_close_location_migration_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_close_location_migration_state"
SELECTION_RULE = "top_fold_local_close_location_migration_scores_per_active_day"
...
MIN_DAY_BARS = 36
CLOSE_LOCATION_MIGRATION_LOOKBACK_BARS = 18
CLOSE_LOCATION_MIGRATION_SHORT_LOOKBACK_BARS = 6
MIN_CLOSE_LOCATION_MIGRATION_ACTIVITY = 0.16
...
class LinearCloseLocationMigrationModel:
    fold_id: str
    ...
    close_location_failure_rate: float
...
def run_c0060_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_close_location_migration_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_close_location_migration_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        CLOSE_LOCATION_MIGRATION_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    migration = close_location_migration_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        migration["close_location_level"],
        migration["close_location_migration_18"],
        migration["recent_close_location_shift_6"],
        migration["close_location_persistence"],
        migration["close_location_rotation_pressure"],
        migration["close_location_variance_inverse"],
        migration["range_position_migration"],
        migration["directional_settlement_efficiency"],
        migration["volume_confirmed_close_shift"],
    ...
    dimensions = (
        close_location_level_bucket(migration["close_location_level"]),
        close_location_migration_bucket(migration["close_location_migration_18"]),
        recent_close_shift_bucket(migration["recent_close_location_shift_6"]),
        close_location_persistence_bucket(migration["close_location_persistence"]),
        close_location_rotation_bucket(migration["close_location_rotation_pressure"]),
        close_location_variance_bucket(migration["close_location_variance_inverse"]),
        range_position_migration_bucket(migration["range_position_migration"]),
        settlement_efficiency_bucket(migration["directional_settlement_efficiency"]),
...
def close_location_migration_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    if prior_volume_per_bar is None or prior_volume_per_bar <= 0:
        return {"event_count": 0.0}
    window = bars[index - CLOSE_LOCATION_MIGRATION_LOOKBACK_BARS + 1 : index + 1]
    short_window = bars[index - CLOSE_LOCATION_MIGRATION_SHORT_LOOKBACK_BARS + 1 : index + 1]
    close_locations = [directional_close_location(item, direction) for item in window]
    short_locations = [directional_close_location(item, direction) for item in short_window]
    close_location_level = close_locations[-1]
    close_location_migration = close_locations[-1] - close_locations[0]
    recent_close_location_shift = short_locations[-1] - short_locations[0]
    location_steps = [close_locations[pos] - close_locations[pos - 1] for pos in range(1, len(close_locations))]
    if not location_steps:
        return {"event_count": 0.0}
    positive_steps = sum(1 for step in location_steps if step > 0.03)
    negative_steps = sum(1 for step in location_steps if step < -0.03)
    close_location_persistence = (positive_steps - negative_steps) / len(location_steps)
    mean_location = sum(close_locations) / len(close_locations)
    variance = sum((value - mean_location) ** 2 for value in close_locations) / len(close_locations)
    close_location_variance_inverse = 1.0 / (1.0 + 8.0 * math.sqrt(variance))
    close_location_rotation_pressure = max(0.0, -close_location_migration) * max(0.0, recent_close_location_shift)
    early_range_position = directional_range_position(
        bars,
        index - CLOSE_LOCATION_MIGRATION_SHORT_LOOKBACK_BARS,
        direction,
        CLOSE_LOCATION_MIGRATION_LOOKBACK_BARS,
    )
    current_range_position = directional_range_position(bars, index, direction, CLOSE_LOCATION_MIGRATION_LOOKBACK_BARS)
    if early_range_position is None or current_range_position is None:
        return {"event_count": 0.0}
    range_position_migration = current_range_position - early_range_position
    travel = sum(max(item.high - item.low, 0.0) for item in short_window)
    directional_settlement_efficiency = direction * (
        close - bars[index - CLOSE_LOCATION_MIGRATION_SHORT_LOOKBACK_BARS + 1].open
    ) / max(travel, 1e-9)
    range_completion = day_width / average_range
    recent_volume = sum(volumes[max(0, index - CLOSE_LOCATION_MIGRATION_SHORT_LOOKBACK_BARS + 1) : index + 1])
    expected_recent_volume = prior_volume_per_bar * CLOSE_LOCATION_MIGRATION_SHORT_LOOKBACK_BARS
    recent_intensity = recent_volume / max(expected_recent_volume, 1e-9)
    volume_confirmed_close_shift = recent_close_location_shift * max(recent_intensity, 0.0)
    session_pressure = bounded_session_progress(bars[index].time) * max(
        abs(close_location_migration),
        abs(recent_close_location_shift),
        close_location_rotation_pressure,
        abs(range_position_migration),
        abs(volume_confirmed_close_shift),
    )
    activity = max(
        abs(close_location_level - 0.5),
        abs(close_location_migration),
        abs(recent_close_location_shift),
        abs(close_location_persistence),
        close_location_rotation_pressure,
        abs(range_position_migration),
        abs(volume_confirmed_close_shift),
        session_pressure,
    )
    if activity < MIN_CLOSE_LOCATION_MIGRATION_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "close_location_level": bounded(close_location_level, 0.0, 1.0),
        "close_location_migration_18": bounded(close_location_migration, -1.0, 1.0),
        "recent_close_location_shift_6": bounded(recent_close_location_shift, -1.0, 1.0),
        "close_location_persistence": bounded(close_location_persistence, -1.0, 1.0),
        "close_location_rotation_pressure": bounded(close_location_rotation_pressure, 0.0, 1.0),
        "close_location_variance_inverse": bounded(close_location_variance_inverse, 0.0, 1.0),
        "range_position_migration": bounded(range_position_migration, -1.0, 1.0),
        "directional_settlement_efficiency": bounded(directional_settlement_efficiency, -2.0, 2.0),
        "volume_confirmed_close_shift": bounded(volume_confirmed_close_shift, -3.0, 3.0),
...
def close_location_level_bucket(value: float) -> str:
    if value < 0.35:
        return "close_settlement_weak"
    if value < 0.65:
        return "close_settlement_mid"
    return "close_settlement_strong"


def close_location_migration_bucket(value: float) -> str:
    if value < -0.18:
        return "close_location_migrating_against"
    if value < 0.18:
        return "close_location_migration_flat"
    return "close_location_migrating_with"


def recent_close_shift_bucket(value: float) -> str:
    if value < -0.16:
        return "recent_close_shift_against"
    if value < 0.16:
        return "recent_close_shift_flat"
    return "recent_close_shift_with"


def close_location_persistence_bucket(value: float) -> str:
    if value < -0.20:
        return "close_location_persistence_against"
    if value < 0.25:
        return "close_location_persistence_mixed"
    return "close_location_persistence_with"


def close_location_rotation_bucket(value: float) -> str:
    if value < 0.08:
        return "close_rotation_absent"
    if value < 0.55:
        return "close_rotation_partial"
    return "close_rotation_strong"


def close_location_variance_bucket(value: float) -> str:
    if value < 0.45:
        return "close_location_choppy"
    if value < 0.72:
        return "close_location_mixed_variance"
    return "close_location_stable"


def range_position_migration_bucket(value: float) -> str:
    if value < -0.25:
        return "range_position_migrating_against"
    if value < 0.35:
        return "range_position_migration_flat"
    return "range_position_migrating_with"


def settlement_efficiency_bucket(value: float) -> str:
    if value < -0.20:
        return "settlement_efficiency_against"
    if value < 0.25:
        return "settlement_efficiency_neutral"
    return "settlement_efficiency_with"
...
def candidate_label(
    ...
    close = bars[index].close
    close_location = directional_close_location(bars[index], direction)
    prior_close_location = directional_close_location(bars[index - CLOSE_LOCATION_MIGRATION_LOOKBACK_BARS], direction)
    recent_origin_location = directional_close_location(
        bars[index - CLOSE_LOCATION_MIGRATION_SHORT_LOOKBACK_BARS],
        direction,
    )
    close_location_migration = close_location - prior_close_location
    recent_close_shift = close_location - recent_origin_location
    continuation_reward = 0.18 if close_location > 0.62 and recent_close_shift > 0.12 and terminal > 0 else 0.0
    reversal_reward = (
        0.22
        if close_location_migration < -0.18 and recent_close_shift > 0.14 and terminal > 0
        else 0.0
    )
    failed_settlement_penalty = 0.24 if close_location > 0.64 and terminal <= 0 and mae > mfe else 0.0
    false_migration_penalty = 0.16 if recent_close_shift > 0.16 and terminal <= 0 else 0.0
...
def fit_linear_close_location_migration_model(candidates: list[base.Candidate], fold_id: str) -> LinearCloseLocationMigrationModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearCloseLocationMigrationModel(
            fold_id,
    ...
    close_location_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearCloseLocationMigrationModel(
        fold_id=fold_id,
        ...
        close_location_failure_rate=close_location_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearCloseLocationMigrationModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_close_location_migration_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0060_r0001_intraday_close_location_migration",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0060_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0060_r0001_close_location_migration_summary.json",
        ],
        ...
        "conditional_profiles": {
            "close_location_migration_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0060 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0060_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0060 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "close_location_migration_lookback_bars": CLOSE_LOCATION_MIGRATION_LOOKBACK_BARS,
        "close_location_migration_short_lookback_bars": CLOSE_LOCATION_MIGRATION_SHORT_LOOKBACK_BARS,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_close_location_migration_model_summary(model: LinearCloseLocationMigrationModel) -> dict[str, object]:
    return {
        ...
        "close_location_failure_rate": rounded(model.close_location_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearCloseLocationMigrationModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_close_location_migration_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "close_location_migration_profile": profiles["close_location_migration_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0060_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_trade_artifact"] = "artifacts/c0060_r0001_proxy_trades.csv"
    evidence["close_location_migration_summary"] = "artifacts/c0060_r0001_close_location_migration_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0060_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0060_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0060 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0060_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0060_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0060_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0060_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0060_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0060_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0060_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0060_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0060_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0060 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0060 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260706_produce_c0060_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0060_r0001_proxy_evidence
  refines:
  - dec_20260706_open_c0060_r0001_intraday_close_location_migration_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0060_r0001_proxy_records_close_location_migration_reference_surface
  ...
  - next_work_is_produce_c0060_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260706_produce_c0060_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0060-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0060_r0001_intraday_close_location_migration",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0060-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0060-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0060-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0060_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0060_r0001_intraday_close_location_migration",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0060-R0001-close-location-migration-SUMMARY",
            "artifact_role": "close_location_migration_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0060_r0001_close_location_migration_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0060_r0001_intraday_close_location_migration",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0060_r0001_mt5_logic_parity_evidence",
        }
    ]
