"""C0118 R0001 proxy evidence for intraday liquidity shelf decay break."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0118_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0118_r0001_liquidity_shelf_decay_break_summary.json"
...
FEATURE_NAMES = (
    "shelf_touch_density",
    "shelf_dwell_compression",
    "shelf_break_impulse",
    "shelf_rejection_balance",
    "false_touch_absorption",
    "range_energy_release",
    "volume_escape_ratio",
    "vwap_shelf_distance",
    "prior_session_shelf_context",
    "shelf_position_pressure",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_liquidity_shelf_decay_break_rank"
LABEL_SHAPE = "target_first_quality_after_liquidity_shelf_decay_break"
SELECTION_RULE = "top_fold_local_liquidity_shelf_decay_break_scores_per_active_day"
...
MIN_DAY_BARS = 18
LIQUIDITY_SHELF_DECAY_BREAK_LOOKBACK_BARS = 54
LIQUIDITY_SHELF_DECAY_BREAK_RECENT_BARS = 10
LIQUIDITY_SHELF_DECAY_BREAK_BASE_BARS = 18
MIN_SHELF_TOUCH_DENSITY = 0.14
MIN_LIQUIDITY_SHELF_DECAY_BREAK_ACTIVITY = 0.055
...
class LinearLiquidityShelfDecayBreakModel:
    fold_id: str
    ...
    liquidity_shelf_decay_break_failure_rate: float
...
def run_c0118_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_liquidity_shelf_decay_break_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_liquidity_shelf_decay_break_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        LIQUIDITY_SHELF_DECAY_BREAK_LOOKBACK_BARS,
        LIQUIDITY_SHELF_DECAY_BREAK_RECENT_BARS + LIQUIDITY_SHELF_DECAY_BREAK_BASE_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = liquidity_shelf_decay_break_metrics(
    ...
    features = (
        metrics["shelf_touch_density"],
        metrics["shelf_dwell_compression"],
        metrics["shelf_break_impulse"],
        metrics["shelf_rejection_balance"],
        metrics["false_touch_absorption"],
        metrics["range_energy_release"],
        metrics["volume_escape_ratio"],
        metrics["vwap_shelf_distance"],
        metrics["prior_session_shelf_context"],
        metrics["shelf_position_pressure"],
        metrics["session_maturity"],
        metrics["spread_stress_inverse"],
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        touch_density_bucket(metrics["shelf_touch_density"]),
        compression_bucket(metrics["shelf_dwell_compression"]),
        breakout_pressure_bucket(metrics["shelf_break_impulse"]),
        absorption_balance_bucket(metrics["shelf_rejection_balance"]),
        absorption_bucket(metrics["false_touch_absorption"]),
        range_expansion_bucket(metrics["range_energy_release"]),
        volume_confirmation_bucket(metrics["volume_escape_ratio"]),
        vwap_alignment_bucket(metrics["vwap_shelf_distance"]),
        prior_day_position_context_bucket(metrics["prior_session_shelf_context"]),
        range_position_pressure_bucket(metrics["shelf_position_pressure"]),
...
def liquidity_shelf_decay_break_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    recent_start = max(index - LIQUIDITY_SHELF_DECAY_BREAK_RECENT_BARS + 1, context.day_start_index)
    base_start = max(recent_start - LIQUIDITY_SHELF_DECAY_BREAK_BASE_BARS, context.day_start_index)
    lookback_start = max(index - LIQUIDITY_SHELF_DECAY_BREAK_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    shelf_source_prices = [bar.low if direction > 0 else bar.high for bar in lookback_window]
    shelf_price = base.percentile(shelf_source_prices, 0.35 if direction > 0 else 0.65)
    if shelf_price is None:
        return {"event_count": 0.0}
    shelf_tolerance = max(average_range * 0.18, lookback_range_mean * 0.10)

    def touches_shelf(bar: base.Bar) -> bool:
        reference = bar.low if direction > 0 else bar.high
        return abs(reference - shelf_price) <= shelf_tolerance

    lookback_touch_count = sum(1 for bar in lookback_window if touches_shelf(bar))
    recent_touches = [bar for bar in recent_window if touches_shelf(bar)]
    shelf_touch_density = lookback_touch_count / max(len(lookback_window), 1)
    if shelf_touch_density < MIN_SHELF_TOUCH_DENSITY:
        return {"event_count": 0.0}
    ...
    shelf_dwell_compression = 1.0 - min(1.0, base_width / max(lookback_width, average_range * 1.5))
    directional_close_from_shelf = direction * (signal_bar.close - shelf_price) / max(average_range, 1e-9)
    recent_directional_net = direction * (recent_window[-1].close - recent_window[0].close)
    shelf_break_impulse = directional_close_from_shelf + recent_directional_net / max(recent_range_mean * len(recent_window), average_range)
    shelf_rejection_balance = base.mean([directional_bar_settlement(bar, direction) for bar in recent_window]) or 0.0
    absorbed_touch_count = sum(1 for bar in recent_touches if direction * (bar.close - shelf_price) > 0)
    false_touch_absorption = absorbed_touch_count / max(len(recent_touches), 1)
    if shelf_break_impulse < -0.25 and false_touch_absorption < 0.30:
        return {"event_count": 0.0}

    range_energy_release = recent_range_mean / max(base_range_mean, 1e-9)
    volume_escape_ratio = recent_volume_mean / max(average_volume, 1e-9)
    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    vwap_shelf_distance = 1.0 - abs(session_vwap - shelf_price) / max(average_range * 3.0, 1e-9)
    prior_session_shelf_context = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_session_shelf_context = direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
    if context.prior_day_shadow_balance is not None:
        prior_session_shelf_context += direction * context.prior_day_shadow_balance * 0.08
    shelf_position_pressure = directional_session_percentile_position(
        signal_bar.close,
        context.low_so_far,
        context.high_so_far,
        direction,
    ...
    activity = max(
        shelf_touch_density * 0.45,
        max(shelf_dwell_compression, 0.0) * 0.30,
        max(shelf_break_impulse, 0.0) * 0.24,
        max(shelf_rejection_balance, 0.0) * 0.22,
        false_touch_absorption * 0.20,
        max(range_energy_release - 0.80, 0.0) * 0.12,
        max(volume_escape_ratio - 0.70, 0.0) * 0.08,
        max(vwap_shelf_distance, 0.0) * 0.06,
    )
    if activity < MIN_LIQUIDITY_SHELF_DECAY_BREAK_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "shelf_touch_density": bounded(shelf_touch_density, 0.0, 1.0),
        "shelf_dwell_compression": bounded(shelf_dwell_compression, -1.0, 1.0),
        "shelf_break_impulse": bounded(shelf_break_impulse, -4.0, 8.0),
        "shelf_rejection_balance": bounded(shelf_rejection_balance, -1.0, 1.0),
        "false_touch_absorption": bounded(false_touch_absorption, 0.0, 1.0),
        "range_energy_release": bounded(range_energy_release, 0.0, 8.0),
        "volume_escape_ratio": bounded(volume_escape_ratio, 0.0, 8.0),
        "vwap_shelf_distance": bounded(vwap_shelf_distance, -4.0, 1.0),
        "prior_session_shelf_context": bounded(prior_session_shelf_context, -3.0, 3.0),
        "shelf_position_pressure": bounded(shelf_position_pressure, 0.0, 1.0),
        ...
        "shelf_price": rounded(shelf_price) or 0.0,
        "shelf_tolerance": bounded(shelf_tolerance, 0.0, 8.0),
        "liquidity_shelf_decay_break_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0118_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0118_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0118_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0118_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0118_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0118_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0118_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0118_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0118_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0118_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0118_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0118_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0118_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0118_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0118_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0118_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0118_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0118_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0118_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def c0118_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    touch_reward = 0.10 if metrics["shelf_touch_density"] > 0.24 and terminal > 0 else 0.0
    compression_reward = 0.10 if metrics["shelf_dwell_compression"] > 0.18 and terminal > 0 else 0.0
    break_reward = 0.12 if metrics["shelf_break_impulse"] > 0.22 and terminal > 0 else 0.0
    rejection_reward = 0.09 if metrics["shelf_rejection_balance"] > 0.15 and terminal > 0 else 0.0
    absorption_reward = 0.08 if metrics["false_touch_absorption"] > 0.42 and terminal > 0 else 0.0
    energy_reward = 0.07 if metrics["range_energy_release"] > 0.92 and terminal > 0 else 0.0
    volume_reward = 0.06 if 0.80 <= metrics["volume_escape_ratio"] <= 2.20 and terminal > 0 else 0.0
    vwap_reward = 0.06 if metrics["vwap_shelf_distance"] > 0.10 and terminal > 0 else 0.0
    position_reward = 0.06 if metrics["shelf_position_pressure"] > 0.48 and terminal > 0 else 0.0
    weak_touch_penalty = 0.08 if metrics["shelf_touch_density"] < 0.18 and terminal <= 0 else 0.0
    weak_compression_penalty = 0.08 if metrics["shelf_dwell_compression"] < -0.05 and terminal <= 0 else 0.0
    weak_break_penalty = 0.10 if metrics["shelf_break_impulse"] < 0.00 and terminal <= 0 else 0.0
    weak_rejection_penalty = 0.08 if metrics["shelf_rejection_balance"] < -0.05 and terminal <= 0 else 0.0
    overheated_volume_penalty = 0.06 if metrics["volume_escape_ratio"] > 2.80 and terminal <= 0 else 0.0
    ...
    liquidity_shelf_decay_break_path_quality = (0.62 * terminal + 0.28 * mfe - 0.65 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + liquidity_shelf_decay_break_path_quality
        + touch_reward
        + compression_reward
        + break_reward
        + rejection_reward
        ...
        - max(0.0, 0.16 - metrics["shelf_touch_density"]) * 0.12
    )


def fit_linear_liquidity_shelf_decay_break_model(candidates: list[base.Candidate], fold_id: str) -> LinearLiquidityShelfDecayBreakModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearLiquidityShelfDecayBreakModel(
            fold_id,
    ...
    liquidity_shelf_decay_break_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearLiquidityShelfDecayBreakModel(
        fold_id=fold_id,
        ...
        liquidity_shelf_decay_break_failure_rate=liquidity_shelf_decay_break_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearLiquidityShelfDecayBreakModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_liquidity_shelf_decay_break_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0118_r0001_intraday_liquidity_shelf_decay_break",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0118_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0118_r0001_liquidity_shelf_decay_break_summary.json",
        ],
        ...
        "conditional_profiles": {
            "liquidity_shelf_decay_break_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0118 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0118_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0118 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "liquidity_shelf_decay_break_lookback_bars": LIQUIDITY_SHELF_DECAY_BREAK_LOOKBACK_BARS,
        "liquidity_shelf_decay_break_recent_bars": LIQUIDITY_SHELF_DECAY_BREAK_RECENT_BARS,
        "liquidity_shelf_decay_break_base_bars": LIQUIDITY_SHELF_DECAY_BREAK_BASE_BARS,
        "min_shelf_touch_density": MIN_SHELF_TOUCH_DENSITY,
        "min_liquidity_shelf_decay_break_activity": MIN_LIQUIDITY_SHELF_DECAY_BREAK_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_liquidity_shelf_decay_break_model_summary(model: LinearLiquidityShelfDecayBreakModel) -> dict[str, object]:
    return {
        ...
        "liquidity_shelf_decay_break_failure_rate": rounded(model.liquidity_shelf_decay_break_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearLiquidityShelfDecayBreakModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_liquidity_shelf_decay_break_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "liquidity_shelf_decay_break_profile": profiles["liquidity_shelf_decay_break_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0118_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0118_r0001_proxy_trades.csv"
    evidence["liquidity_shelf_decay_break_summary"] = "artifacts/c0118_r0001_liquidity_shelf_decay_break_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0118_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0118_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0118 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0118_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0118_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0118_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0118_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0118_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0118_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0118_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0118_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0118_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0118 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0118 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0118_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0118_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0118_r0001_intraday_liquidity_shelf_decay_break_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0118_r0001_proxy_records_liquidity_shelf_decay_break_reference_surface
  ...
  - next_work_is_produce_c0118_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0118_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0118-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0118_r0001_intraday_liquidity_shelf_decay_break",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0118-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0118-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0118-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0118_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0118_r0001_intraday_liquidity_shelf_decay_break",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0118-R0001-LIQUIDITY-SHELF-DECAY-BREAK-SUMMARY",
            "artifact_role": "liquidity_shelf_decay_break_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0118_r0001_liquidity_shelf_decay_break_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0118_r0001_intraday_liquidity_shelf_decay_break",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0118_r0001_mt5_logic_parity_evidence",
        }
    ]
