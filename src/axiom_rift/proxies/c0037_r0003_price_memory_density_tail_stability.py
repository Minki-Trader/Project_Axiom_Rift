"""C0037 R0003 proxy evidence for price memory density tail stability."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0037_intraday_price_memory_density_discovery" / "runs" / "R0003"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0037_r0003_proxy_trades.csv"
PRICE_MEMORY_DENSITY_TAIL_STABILITY_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0037_r0003_price_memory_density_tail_stability_summary.json"
...
MODEL_FAMILY = "fold_local_price_memory_density_tail_stability_stress_resilience_conditioner"
LABEL_SHAPE = "directional_price_memory_density_tail_stability_stress_resilience_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
CONDITIONER_WEIGHT = 0.52
SELECTION_RULE = "top_fold_local_tail_stability_stress_resilience_scores_per_active_day"
SOURCE_ROBUSTNESS_AUDIT_PATH = (
    "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0002/kpi/candidate_robustness_audit.json"
)
...
def run_c0037_r0003_proxy(write: bool = True) -> dict[str, object]:
        ...
        for direction in (1, -1):
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|intraday_price_memory_density_tail_stability",
                    features=features,
                    label=label,
                )
            )
...
def robustness_adjusted_label(candidate: base.Candidate) -> float:
    ...
    values = tail_stability_values(candidate)
    return (
        label
        + 0.18 * base_score
        + 0.30 * values["stable_escape_quality"]
        + 0.18 * values["stress_resilience_margin"]
        + 0.08 * values["density_balance_stability"]
        - 0.72 * values["tail_risk_score"]
        - 0.20 * values["adverse_cost_pressure"]
        - 0.12 * values["month_turn_pressure"]
    )
...
def conditioner_features(candidate: base.Candidate) -> tuple[float, ...]:
    ...
    return (
        ...
        values["month_sin"],
        values["month_cos"],
        values["month_turn_pressure"],
        values["tail_risk_score"],
    )


def tail_stability_values(candidate: base.Candidate) -> dict[str, float]:
    base_score = float(candidate.score or 0.0)
    ...
    month_turn_pressure = 1.0 if day <= 3 or day >= 27 else 0.0
    stable_escape_quality = (
    ...
    return {
        ...
        "month_sin": month_sin,
        "month_cos": month_cos,
        "month_turn_pressure": month_turn_pressure,
...
def score_candidates(candidates: list[base.Candidate], conditioner: RobustnessConditioner) -> list[base.Candidate]:
    ...
    for index, candidate in enumerate(base_scored):
        ...
        score = (
            ...
            - 0.10 * raw_features[1]
            - 0.08 * raw_features[13]
            + 0.08 * raw_features[4]
            + 0.08 * (conditioner.robust_label_rate - conditioner.fragile_label_rate)
        )
...
def conditioner_summary(conditioner: RobustnessConditioner) -> dict[str, object]:
    return {
        ...
        "score_interpretation": "higher_score_means_closer_to_train_tail_stable_stress_resilient_state",
    }
...
def build_proxy_payload(
    ...
    payload["proxy_id"] = "PX-C0037-R0003"
    payload["proxy_engine"] = "axiom_rift.proxies.c0037_r0003_price_memory_density_tail_stability"
    payload["proxy_config_path"] = "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003/run_manifest.json"
    payload["split_policy"] = (
        "rolling_window_test_oos_only_with_train_is_price_memory_density_tail_stability_stress_resilience_conditioner_fit"
    )
    payload["proxy_artifact_paths"] = [
        "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003/kpi/proxy.json",
        "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003/artifacts/c0037_r0003_proxy_trades.csv",
        "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003/artifacts/c0037_r0003_price_memory_density_tail_stability_summary.json",
    ]
    ...
    profiles["price_memory_density_tail_stability_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
            "model_family": MODEL_FAMILY,
            "base_feature_count": len(FEATURE_NAMES),
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0037_r0003_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "feature_weight_floor": FEATURE_WEIGHT_FLOOR,
            "feature_weight_ceiling": FEATURE_WEIGHT_CEILING,
            "score_interpretation": "higher_score_means_train_only_tail_stability_stress_resilience_conditioned_rank",
            "variant_boundary": (
                "train_only_price_memory_density_tail_stability_stress_resilience_conditioner_informed_by_"
                "c0037_r0002_weakest_fold_cost_buffer_adverse_cost_survival_and_monthly_loss_audit_not_threshold_"
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_price_memory_density_summary_artifact(payload, PRICE_MEMORY_DENSITY_TAIL_STABILITY_SUMMARY_PATH)
    write_text_local(PROXY_PATH, json.dumps(payload, indent=2, sort_keys=True) + "\n")
    trade_hash = sha256_file_local(TRADE_ARTIFACT_PATH)
    summary_hash = sha256_file_local(PRICE_MEMORY_DENSITY_TAIL_STABILITY_SUMMARY_PATH)
...
def write_price_memory_density_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_price_memory_density_tail_stability_summary_v1",
        "template": False,
        "work_unit_id": "C0037",
        "campaign_id": "C0037",
        "run_id": "R0003",
        ...
        "price_memory_density_tail_stability_profile": profiles["price_memory_density_tail_stability_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "price_memory_density_tail_stability_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0037-R0003-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0037-R0003-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0037-R0003-PRICE-MEMORY-DENSITY-SUMMARY",
                "price_memory_density_tail_stability_summary_artifact",
                "json",
                "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003/artifacts/c0037_r0003_price_memory_density_tail_stability_summary.json",
                summary_hash,
                ["campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0037_r0003_price_memory_density_tail_stability",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0037_r0003_proxy_trades.csv"
    evidence["price_memory_density_tail_stability_summary"] = "artifacts/c0037_r0003_price_memory_density_tail_stability_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0037_r0003_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0037_r0003_proxy_trades.csv",
        "artifacts/c0037_r0003_price_memory_density_tail_stability_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0037 R0003 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0037_r0003_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0037_r0003_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_run"] = "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003"
    ...
    for item in (
        ...
        "open_c0037_r0003_price_memory_density_tail_stability_stress_resilience_run",
        "produce_c0037_r0003_proxy_evidence",
    ):
    ...
    data["active_run"] = "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0037_r0003_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0037_r0003_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["canonical_source"] = "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003/kpi/proxy.json"
    data["canonical_status"] = "proxy_recorded_pending_mt5"
    data["current_decision"] = "produce_c0037_r0003_mt5_logic_parity_evidence"
    data["active_campaign"] = "campaigns/C0037_intraday_price_memory_density_discovery"
    data["active_run"] = "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003"
    data["next_required_action"] = "produce_c0037_r0003_mt5_logic_parity_evidence"
    data["current_evidence_summary"] = {
        "source_campaign": "campaigns/C0037_intraday_price_memory_density_discovery",
        "current_task": "produce_c0037_r0003_mt5_logic_parity_evidence",
        "active_run": "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003",
        ...
        "proxy_profit_factor": required.get("proxy_profit_factor") if isinstance(required, dict) else None,
        "next_required_action": "produce_c0037_r0003_mt5_logic_parity_evidence",
        "note": "C0037 R0003 proxy evidence recorded; MT5 paired validation remains mandatory regardless of proxy result.",
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0037",
            "c0004_r0003_fold_local_state_archetype": "c0037_r0003_price_memory_density_tail_stability",
            "c0004_r0003": "c0037_r0003",
            ...
            "state_archetype_summary": "price_memory_density_tail_stability_summary",
        }
