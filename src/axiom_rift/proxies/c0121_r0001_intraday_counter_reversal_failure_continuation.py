"""C0121 R0001 proxy evidence for intraday counter reversal failure continuation."""
...
CAMPAIGN_ID = "C0121"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0121_intraday_counter_reversal_failure_continuation_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0121_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0121_r0001_counter_reversal_failure_continuation_summary.json"
...
FEATURE_NAMES = (
    "impulse_context_strength",
    "counter_reversal_attempt_depth",
    "failed_reclaim_distance",
    "continuation_resumption_pressure",
    "opposite_wick_rejection",
    "counter_volume_exhaustion",
    "trend_vwap_alignment",
    "session_directional_position",
    "prior_session_continuation_context",
    "range_expansion_efficiency",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_counter_reversal_failure_continuation_rank"
LABEL_SHAPE = "target_first_quality_after_counter_reversal_failure_continuation"
SELECTION_RULE = "top_fold_local_counter_reversal_failure_continuation_scores_per_active_day"
...
MIN_DAY_BARS = 18
COUNTER_REVERSAL_FAILURE_CONTINUATION_LOOKBACK_BARS = 48
COUNTER_REVERSAL_FAILURE_CONTINUATION_IMPULSE_BARS = 12
COUNTER_REVERSAL_FAILURE_CONTINUATION_COUNTER_BARS = 10
MIN_IMPULSE_CONTEXT_STRENGTH = 0.30
MIN_COUNTER_REVERSAL_FAILURE_CONTINUATION_ACTIVITY = 0.055
...
class LinearCounterReversalFailureContinuationModel:
    fold_id: str
    ...
    counter_reversal_failure_continuation_failure_rate: float
...
def run_c0121_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_counter_reversal_failure_continuation_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_counter_reversal_failure_continuation_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        COUNTER_REVERSAL_FAILURE_CONTINUATION_LOOKBACK_BARS,
        COUNTER_REVERSAL_FAILURE_CONTINUATION_COUNTER_BARS + COUNTER_REVERSAL_FAILURE_CONTINUATION_IMPULSE_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = counter_reversal_failure_continuation_metrics(
    ...
    features = (
        metrics["impulse_context_strength"],
        metrics["counter_reversal_attempt_depth"],
        metrics["failed_reclaim_distance"],
        metrics["continuation_resumption_pressure"],
        metrics["opposite_wick_rejection"],
        metrics["counter_volume_exhaustion"],
        metrics["trend_vwap_alignment"],
        metrics["session_directional_position"],
        metrics["prior_session_continuation_context"],
    ...
    dimensions = (
        breakout_pressure_bucket(metrics["impulse_context_strength"]),
        opposing_run_bucket(metrics["counter_reversal_attempt_depth"]),
        breakout_pressure_bucket(metrics["failed_reclaim_distance"]),
        breakout_pressure_bucket(metrics["continuation_resumption_pressure"]),
        absorption_bucket(metrics["opposite_wick_rejection"]),
        volume_confirmation_bucket(metrics["counter_volume_exhaustion"]),
        vwap_alignment_bucket(metrics["trend_vwap_alignment"]),
        range_position_pressure_bucket(metrics["session_directional_position"]),
        prior_day_position_context_bucket(metrics["prior_session_continuation_context"]),
        drift_efficiency_bucket(metrics["range_expansion_efficiency"]),
...
def counter_reversal_failure_continuation_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    counter_start = max(index - COUNTER_REVERSAL_FAILURE_CONTINUATION_COUNTER_BARS + 1, context.day_start_index)
    impulse_start = max(counter_start - COUNTER_REVERSAL_FAILURE_CONTINUATION_IMPULSE_BARS, context.day_start_index)
    lookback_start = max(index - COUNTER_REVERSAL_FAILURE_CONTINUATION_LOOKBACK_BARS + 1, context.day_start_index)
    if impulse_start < 0 or counter_start < 0 or lookback_start < 0:
        return {"event_count": 0.0}
    impulse_window = bars[impulse_start:counter_start]
    counter_window = bars[counter_start : index + 1]
    lookback_window = bars[lookback_start : index + 1]
    day_window = bars[context.day_start_index : index + 1]
    if len(impulse_window) < 6 or len(counter_window) < 5 or len(lookback_window) < 24 or len(day_window) < MIN_DAY_BARS:
        return {"event_count": 0.0}
    average_volume = volume_average[index]
    if average_volume is None or average_volume <= 0:
        return {"event_count": 0.0}

    counter_ranges = [bar.high - bar.low for bar in counter_window]
    impulse_ranges = [bar.high - bar.low for bar in impulse_window]
    lookback_ranges = [bar.high - bar.low for bar in lookback_window]
    counter_range_mean = base.mean(counter_ranges) or average_range
    ...
    impulse_volume_mean = base.mean(volumes[impulse_start:counter_start]) or average_volume
    counter_volume_mean = base.mean(volumes[counter_start : index + 1]) or average_volume
    lookback_volume_mean = base.mean(volumes[lookback_start : index + 1]) or average_volume
    if counter_range_mean <= 0 or impulse_range_mean <= 0 or lookback_range_mean <= 0:
        return {"event_count": 0.0}
    ...
    impulse_move = direction * (impulse_window[-1].close - impulse_window[0].close)
    impulse_context_strength = impulse_move / max(impulse_range_mean * math.sqrt(len(impulse_window)), average_range)
    if impulse_context_strength < MIN_IMPULSE_CONTEXT_STRENGTH:
        return {"event_count": 0.0}

    counter_extreme = min(bar.low for bar in counter_window) if direction > 0 else max(bar.high for bar in counter_window)
    counter_start_close = counter_window[0].close
    counter_attempt_move = -direction * (counter_extreme - counter_start_close)
    counter_reversal_attempt_depth = counter_attempt_move / max(abs(impulse_move), average_range)
    if counter_reversal_attempt_depth < 0.04 or counter_reversal_attempt_depth > 1.35:
        return {"event_count": 0.0}

    counter_mid = (min(bar.low for bar in counter_window) + max(bar.high for bar in counter_window)) / 2.0
    counter_settlement_balance = base.mean([directional_bar_settlement(bar, direction) for bar in counter_window]) or 0.0

    def rejection_wick_share(bar: base.Bar) -> float:
        bar_range = max(bar.high - bar.low, 1e-9)
        if direction > 0:
            wick = min(bar.open, bar.close) - bar.low
        else:
            wick = bar.high - max(bar.open, bar.close)
        return bounded(wick / bar_range, 0.0, 1.0)

    opposite_wick_rejection = base.mean([rejection_wick_share(bar) for bar in counter_window]) or 0.0
    opposite_wick_rejection = 0.55 * max(counter_settlement_balance, 0.0) + 0.45 * opposite_wick_rejection
    failed_reclaim_distance = direction * (signal_bar.close - counter_mid) / max(average_range, 1e-9)
    failed_reclaim_distance += directional_bar_settlement(signal_bar, direction) * 0.35
    continuation_resumption_pressure = direction * (signal_bar.close - counter_start_close) / max(
        average_range,
        1e-9,
    )
    continuation_resumption_pressure += direction * (signal_bar.close - counter_extreme) / max(
        counter_range_mean * len(counter_window),
        average_range,
    )
    if max(failed_reclaim_distance, continuation_resumption_pressure, opposite_wick_rejection) < 0.02:
        return {"event_count": 0.0}

    counter_volume_exhaustion = 1.0 - counter_volume_mean / max(impulse_volume_mean * 1.10, average_volume, 1e-9)
    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    trend_vwap_alignment = direction * (signal_bar.close - session_vwap) / max(average_range * 3.0, 1e-9)
    prior_session_continuation_context = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_session_continuation_context = direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
    if context.prior_day_shadow_balance is not None:
        prior_session_continuation_context += direction * context.prior_day_shadow_balance * 0.08
    session_directional_position = directional_session_percentile_position(
        signal_bar.close,
        context.low_so_far,
        context.high_so_far,
        direction,
    )
    impulse_path = sum(abs(impulse_window[pos].close - impulse_window[pos - 1].close) for pos in range(1, len(impulse_window)))
    range_expansion_efficiency = abs(impulse_move) / max(impulse_path, average_range, 1e-9)
    ...
    activity = max(
        impulse_context_strength * 0.22,
        counter_reversal_attempt_depth * 0.14,
        max(failed_reclaim_distance, 0.0) * 0.24,
        max(continuation_resumption_pressure, 0.0) * 0.22,
        opposite_wick_rejection * 0.18,
        max(counter_volume_exhaustion, 0.0) * 0.08,
        max(trend_vwap_alignment, 0.0) * 0.10,
        range_expansion_efficiency * 0.08,
    )
    if activity < MIN_COUNTER_REVERSAL_FAILURE_CONTINUATION_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "impulse_context_strength": bounded(impulse_context_strength, -4.0, 8.0),
        "counter_reversal_attempt_depth": bounded(counter_reversal_attempt_depth, -1.0, 2.0),
        "failed_reclaim_distance": bounded(failed_reclaim_distance, -4.0, 8.0),
        "continuation_resumption_pressure": bounded(continuation_resumption_pressure, -4.0, 8.0),
        "opposite_wick_rejection": bounded(opposite_wick_rejection, 0.0, 1.0),
        "counter_volume_exhaustion": bounded(counter_volume_exhaustion, -4.0, 1.0),
        "trend_vwap_alignment": bounded(trend_vwap_alignment, -4.0, 4.0),
        "session_directional_position": bounded(session_directional_position, 0.0, 1.0),
        "prior_session_continuation_context": bounded(prior_session_continuation_context, -3.0, 3.0),
        "range_expansion_efficiency": bounded(range_expansion_efficiency, 0.0, 1.0),
        "session_maturity": bounded(session_phase_maturity, 0.0, 1.0),
        "spread_stress_inverse": bounded(spread_stress_inverse, 0.0, 1.0),
        "recent_range_ratio": bounded(counter_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        "base_range_ratio": bounded(impulse_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        "lookback_range_ratio": bounded(lookback_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        "recent_volume_ratio": bounded(counter_volume_mean / max(average_volume, 1e-9), 0.0, 8.0),
        "base_volume_ratio": bounded(impulse_volume_mean / max(average_volume, 1e-9), 0.0, 8.0),
        "lookback_volume_ratio": bounded(lookback_volume_mean / max(average_volume, 1e-9), 0.0, 8.0),
        "counter_mid": rounded(counter_mid) or 0.0,
        "counter_extreme": rounded(counter_extreme) or 0.0,
        "counter_reversal_failure_continuation_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0121_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0121_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0121_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0121_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0121_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0121_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0121_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0121_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0121_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0121_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0121_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0121_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0121_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0121_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0121_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0121_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0121_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0121_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0121_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def c0121_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    impulse_reward = 0.10 if metrics["impulse_context_strength"] > 0.55 and terminal > 0 else 0.0
    counter_depth_reward = 0.08 if 0.08 <= metrics["counter_reversal_attempt_depth"] <= 0.95 and terminal > 0 else 0.0
    failed_reclaim_reward = 0.13 if metrics["failed_reclaim_distance"] > 0.18 and terminal > 0 else 0.0
    resumption_reward = 0.10 if metrics["continuation_resumption_pressure"] > 0.14 and terminal > 0 else 0.0
    rejection_reward = 0.09 if metrics["opposite_wick_rejection"] > 0.34 and terminal > 0 else 0.0
    volume_reward = 0.06 if metrics["counter_volume_exhaustion"] > -0.10 and terminal > 0 else 0.0
    vwap_reward = 0.06 if metrics["trend_vwap_alignment"] > 0.05 and terminal > 0 else 0.0
    position_reward = 0.05 if metrics["session_directional_position"] > 0.45 and terminal > 0 else 0.0
    expansion_reward = 0.05 if metrics["range_expansion_efficiency"] > 0.28 and terminal > 0 else 0.0
    weak_impulse_penalty = 0.08 if metrics["impulse_context_strength"] < 0.42 and terminal <= 0 else 0.0
    deep_counter_penalty = 0.08 if metrics["counter_reversal_attempt_depth"] > 1.05 and terminal <= 0 else 0.0
    weak_reclaim_penalty = 0.10 if metrics["failed_reclaim_distance"] < 0.00 and terminal <= 0 else 0.0
    weak_rejection_penalty = 0.08 if metrics["opposite_wick_rejection"] < 0.20 and terminal <= 0 else 0.0
    overheated_volume_penalty = 0.06 if metrics["counter_volume_exhaustion"] < -1.20 and terminal <= 0 else 0.0
    ...
    counter_reversal_failure_continuation_path_quality = (0.62 * terminal + 0.28 * mfe - 0.65 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + counter_reversal_failure_continuation_path_quality
        + impulse_reward
        + counter_depth_reward
        + failed_reclaim_reward
        + resumption_reward
        + rejection_reward
        + volume_reward
        + vwap_reward
        ...
        - deep_counter_penalty
        - weak_reclaim_penalty
        ...
        - max(0.0, 0.30 - metrics["impulse_context_strength"]) * 0.12
    )


def fit_linear_counter_reversal_failure_continuation_model(candidates: list[base.Candidate], fold_id: str) -> LinearCounterReversalFailureContinuationModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearCounterReversalFailureContinuationModel(
            fold_id,
    ...
    counter_reversal_failure_continuation_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearCounterReversalFailureContinuationModel(
        fold_id=fold_id,
        ...
        counter_reversal_failure_continuation_failure_rate=counter_reversal_failure_continuation_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearCounterReversalFailureContinuationModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0121-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_counter_reversal_failure_continuation_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0121_r0001_intraday_counter_reversal_failure_continuation",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0121_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0121_r0001_counter_reversal_failure_continuation_summary.json",
        ],
        ...
        "conditional_profiles": {
            "counter_reversal_failure_continuation_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0121 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0121_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0121 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "counter_reversal_failure_continuation_lookback_bars": COUNTER_REVERSAL_FAILURE_CONTINUATION_LOOKBACK_BARS,
        "counter_reversal_failure_continuation_impulse_bars": COUNTER_REVERSAL_FAILURE_CONTINUATION_IMPULSE_BARS,
        "counter_reversal_failure_continuation_counter_bars": COUNTER_REVERSAL_FAILURE_CONTINUATION_COUNTER_BARS,
        "min_impulse_context_strength": MIN_IMPULSE_CONTEXT_STRENGTH,
        "min_counter_reversal_failure_continuation_activity": MIN_COUNTER_REVERSAL_FAILURE_CONTINUATION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_counter_reversal_failure_continuation_model_summary(model: LinearCounterReversalFailureContinuationModel) -> dict[str, object]:
    return {
        ...
        "counter_reversal_failure_continuation_failure_rate": rounded(model.counter_reversal_failure_continuation_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearCounterReversalFailureContinuationModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_counter_reversal_failure_continuation_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "counter_reversal_failure_continuation_profile": profiles["counter_reversal_failure_continuation_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0121_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0121_r0001_proxy_trades.csv"
    evidence["counter_reversal_failure_continuation_summary"] = "artifacts/c0121_r0001_counter_reversal_failure_continuation_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0121_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0121_r0001_proxy_trades.csv",
        "artifacts/c0121_r0001_counter_reversal_failure_continuation_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0121_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0121 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0121_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0121_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0121_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0121_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0121_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0121_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0121_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0121_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0121_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0121 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0121 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0121_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0121_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0121_r0001_intraday_counter_reversal_failure_continuation_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0121_r0001_proxy_records_counter_reversal_failure_continuation_reference_surface
  ...
  - next_work_is_produce_c0121_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0121_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0121-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0121_r0001_intraday_counter_reversal_failure_continuation",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0121-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0121-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0121-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0121_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0121_r0001_intraday_counter_reversal_failure_continuation",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0121-R0001-COUNTER-REVERSAL-FAILURE-CONTINUATION-SUMMARY",
            "artifact_role": "counter_reversal_failure_continuation_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0121_r0001_counter_reversal_failure_continuation_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0121_r0001_intraday_counter_reversal_failure_continuation",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0121_r0001_mt5_logic_parity_evidence",
        }
    ]
