"""C0079 R0001 proxy evidence for intraday boundary recoil elasticity."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0079_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0079_r0001_bre_summary.json"
...
FEATURE_NAMES = (
    "boundary_extreme_score",
    "boundary_wick_rejection",
    "close_reclaim_from_boundary",
    "stretch_vs_average_range",
    "opening_range_dislocation",
    "volume_climax_fade",
    "short_term_counterpressure",
    "prior_day_extension_context",
    "session_elasticity_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_boundary_recoil_elasticity_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_boundary_recoil_elasticity_state"
SELECTION_RULE = "top_fold_local_boundary_recoil_elasticity_scores_per_active_day"
...
MIN_DAY_BARS = 30
BOUNDARY_RECOIL_ELASTICITY_LOOKBACK_BARS = 18
BOUNDARY_RECOIL_ELASTICITY_SHORT_LOOKBACK_BARS = 6
MIN_BOUNDARY_RECOIL_ELASTICITY_ACTIVITY = 0.18
...
class LinearBoundaryRecoilElasticityModel:
    fold_id: str
    ...
    boundary_recoil_elasticity_failure_rate: float
...
def run_c0079_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_boundary_recoil_elasticity_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_boundary_recoil_elasticity_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        BOUNDARY_RECOIL_ELASTICITY_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    release = boundary_recoil_elasticity_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        release["boundary_extreme_score"],
        release["boundary_wick_rejection"],
        release["close_reclaim_from_boundary"],
        release["stretch_vs_average_range"],
        release["opening_range_dislocation"],
        release["volume_climax_fade"],
        release["short_term_counterpressure"],
        release["prior_day_extension_context"],
        release["session_elasticity_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        boundary_extreme_bucket(release["boundary_extreme_score"]),
        boundary_wick_bucket(release["boundary_wick_rejection"]),
        close_reclaim_bucket(release["close_reclaim_from_boundary"]),
        stretch_bucket(release["stretch_vs_average_range"]),
        opening_dislocation_bucket(release["opening_range_dislocation"]),
        volume_climax_fade_bucket(release["volume_climax_fade"]),
        counterpressure_bucket(release["short_term_counterpressure"]),
        prior_extension_context_bucket(release["prior_day_extension_context"]),
        session_pressure_bucket(release["session_elasticity_pressure"]),
...
def boundary_recoil_elasticity_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    short_start = index - BOUNDARY_RECOIL_ELASTICITY_SHORT_LOOKBACK_BARS + 1
    prior_start = index - BOUNDARY_RECOIL_ELASTICITY_LOOKBACK_BARS + 1
    ...
    if day_width <= 0 or day_width < 0.45 * average_range:
        return {"event_count": 0.0}
    close_position = (signal_bar.close - context.low_so_far) / max(day_width, 1e-9)
    boundary_extreme_score = 1.0 - close_position if direction > 0 else close_position
    boundary_wick_rejection = directional_wick_rejection(signal_bar, direction)
    close_reclaim_from_boundary = directional_bar_settlement(signal_bar, direction)
    stretch_vs_average_range = (
        (context.day_open - signal_bar.close) / average_range
        if direction > 0
        else (signal_bar.close - context.day_open) / average_range
    )
    opening_range_dislocation = (
        (context.opening_low - signal_bar.close) / average_range
        if direction > 0
        else (signal_bar.close - context.opening_high) / average_range
    )
    ...
    short_volume_ratio = short_volume_mean / max(average_volume, 1e-9)
    current_adverse_body = max(-directional_body_score(signal_bar, direction), 0.0)
    volume_climax_fade = max(current_volume_ratio - short_volume_ratio, 0.0) * (1.0 + current_adverse_body)
    short_term_counterpressure = base.mean([directional_body_score(bar, direction) for bar in short_window]) or 0.0
    prior_day_extension_context = -direction * prior_day_return / max(prior_day_range, 1e-9)
    recoil_pressure = max(
        boundary_extreme_score,
        max(boundary_wick_rejection, 0.0),
        max(close_reclaim_from_boundary - 0.42, 0.0),
        min(max(stretch_vs_average_range, 0.0) / 3.0, 1.0),
        min(max(opening_range_dislocation, 0.0) / 2.0, 1.0),
        min(max(volume_climax_fade, 0.0) / 2.0, 1.0),
        max(short_term_counterpressure, 0.0),
    )
    session_elasticity_pressure = bounded_session_progress(signal_bar.time) * recoil_pressure
    activity = max(recoil_pressure, session_elasticity_pressure)
    if activity < MIN_BOUNDARY_RECOIL_ELASTICITY_ACTIVITY:
        return {"event_count": 0.0}
    if boundary_extreme_score < 0.16 and opening_range_dislocation < 0.05:
        return {"event_count": 0.0}
    if close_reclaim_from_boundary < 0.22 and boundary_wick_rejection < -0.08:
        return {"event_count": 0.0}
    return {
        "boundary_extreme_score": bounded(boundary_extreme_score, 0.0, 1.0),
        "boundary_wick_rejection": bounded(boundary_wick_rejection, -1.0, 1.0),
        "close_reclaim_from_boundary": bounded(close_reclaim_from_boundary, 0.0, 1.0),
        "stretch_vs_average_range": bounded(stretch_vs_average_range, -3.0, 8.0),
        "opening_range_dislocation": bounded(opening_range_dislocation, -3.0, 8.0),
        "volume_climax_fade": bounded(volume_climax_fade, 0.0, 6.0),
        "short_term_counterpressure": bounded(short_term_counterpressure, -1.0, 1.0),
        "prior_day_extension_context": bounded(prior_day_extension_context, -2.0, 2.0),
        "session_elasticity_pressure": bounded(session_elasticity_pressure, 0.0, 4.0),
        "event_count": 1.0,
    }
...
def candidate_label(
    ...
    metrics = boundary_recoil_elasticity_metrics(
        bars,
        volumes,
    ...
    if metrics["event_count"] <= 0:
        return None
    boundary_reward = 0.16 if metrics["boundary_extreme_score"] > 0.32 and terminal > 0 else 0.0
    wick_rejection_reward = 0.13 if metrics["boundary_wick_rejection"] > 0.10 and terminal > 0 else 0.0
    reclaim_reward = 0.14 if metrics["close_reclaim_from_boundary"] > 0.52 and terminal > 0 else 0.0
    stretch_reward = 0.10 if 0.35 <= metrics["stretch_vs_average_range"] <= 2.60 and terminal > 0 else 0.0
    opening_dislocation_reward = 0.10 if metrics["opening_range_dislocation"] > 0.12 and terminal > 0 else 0.0
    climax_reward = 0.08 if metrics["volume_climax_fade"] > 0.10 and terminal > 0 else 0.0
    counterpressure_reward = 0.11 if metrics["short_term_counterpressure"] > 0.03 and terminal > 0 else 0.0
    prior_context_reward = 0.06 if metrics["prior_day_extension_context"] > -0.20 and terminal > 0 else 0.0
    weak_boundary_penalty = 0.13 if metrics["boundary_extreme_score"] < 0.18 and terminal <= 0 else 0.0
    wick_against_penalty = 0.12 if metrics["boundary_wick_rejection"] < -0.08 and terminal <= 0 else 0.0
    weak_reclaim_penalty = 0.13 if metrics["close_reclaim_from_boundary"] < 0.26 and terminal <= 0 else 0.0
    thin_stretch_penalty = 0.08 if metrics["stretch_vs_average_range"] < 0.20 and terminal <= 0 else 0.0
    failed_dislocation_penalty = 0.09 if metrics["opening_range_dislocation"] < 0.02 and terminal <= 0 else 0.0
    no_climax_penalty = 0.08 if metrics["volume_climax_fade"] < 0.05 and terminal <= 0 else 0.0
    counterpressure_penalty = 0.12 if metrics["short_term_counterpressure"] < -0.08 and terminal <= 0 else 0.0
    ...
    boundary_recoil_elasticity_path_quality = (0.44 * terminal + 0.36 * mfe - 0.72 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + boundary_recoil_elasticity_path_quality
        + boundary_reward
        + wick_rejection_reward
        + reclaim_reward
        + stretch_reward
        + opening_dislocation_reward
        + climax_reward
        + counterpressure_reward
        + prior_context_reward
        - weak_boundary_penalty
        - wick_against_penalty
        - weak_reclaim_penalty
        - thin_stretch_penalty
        - failed_dislocation_penalty
        - no_climax_penalty
        - counterpressure_penalty
        - stall_penalty
        - unfavorable_path_penalty
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["stretch_vs_average_range"] - 2.60) * 0.06
    )


def fit_linear_boundary_recoil_elasticity_model(candidates: list[base.Candidate], fold_id: str) -> LinearBoundaryRecoilElasticityModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearBoundaryRecoilElasticityModel(
            fold_id,
    ...
    boundary_recoil_elasticity_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearBoundaryRecoilElasticityModel(
        fold_id=fold_id,
        ...
        boundary_recoil_elasticity_failure_rate=boundary_recoil_elasticity_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearBoundaryRecoilElasticityModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_boundary_recoil_elasticity_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0079_r0001_intraday_boundary_recoil_elasticity",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0079_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0079_r0001_bre_summary.json",
        ],
        ...
        "conditional_profiles": {
            "boundary_recoil_elasticity_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0079 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0079_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0079 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "boundary_recoil_elasticity_lookback_bars": BOUNDARY_RECOIL_ELASTICITY_LOOKBACK_BARS,
        "boundary_recoil_elasticity_short_lookback_bars": BOUNDARY_RECOIL_ELASTICITY_SHORT_LOOKBACK_BARS,
        "min_boundary_recoil_elasticity_activity": MIN_BOUNDARY_RECOIL_ELASTICITY_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_boundary_recoil_elasticity_model_summary(model: LinearBoundaryRecoilElasticityModel) -> dict[str, object]:
    return {
        ...
        "boundary_recoil_elasticity_failure_rate": rounded(model.boundary_recoil_elasticity_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearBoundaryRecoilElasticityModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_boundary_recoil_elasticity_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "boundary_recoil_elasticity_profile": profiles["boundary_recoil_elasticity_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0079_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0079_r0001_proxy_trades.csv"
    evidence["boundary_recoil_elasticity_summary"] = "artifacts/c0079_r0001_bre_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0079_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0079_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0079 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0079_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0079_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0079_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0079_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0079_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0079_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0079_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0079_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0079_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0079 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0079 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0079_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0079_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0079_r0001_intraday_boundary_recoil_elasticity_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0079_r0001_proxy_records_boundary_recoil_elasticity_reference_surface
  ...
  - next_work_is_produce_c0079_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0079_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0079-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0079_r0001_intraday_boundary_recoil_elasticity",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0079-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0079-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0079-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0079_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0079_r0001_intraday_boundary_recoil_elasticity",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0079-R0001-boundary-recoil-elasticity-SUMMARY",
            "artifact_role": "boundary_recoil_elasticity_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0079_r0001_bre_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0079_r0001_intraday_boundary_recoil_elasticity",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0079_r0001_mt5_logic_parity_evidence",
        }
    ]
