"""C0103 R0001 proxy evidence for intraday range walk entropy transition."""
...
CAMPAIGN_ID = "C0103"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0103_intraday_range_walk_entropy_transition_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0103_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0103_r0001_range_walk_entropy_transition_summary.json"
...
FEATURE_NAMES = (
    "range_walk_entropy",
    "directional_run_persistence",
    "net_path_efficiency",
    "range_expansion_decay",
    "settlement_persistence",
    "wick_opposition_pressure",
    "session_position_extreme",
    "prior_day_range_walk_context",
    "time_decay_alignment",
    "session_phase_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_range_walk_entropy_transition_rank"
LABEL_SHAPE = "target_first_quality_after_range_walk_entropy_transition"
SELECTION_RULE = "top_fold_local_range_walk_entropy_transition_scores_per_active_day"
...
MIN_DAY_BARS = 30
RANGE_WALK_ENTROPY_TRANSITION_LOOKBACK_BARS = 72
RANGE_WALK_ENTROPY_TRANSITION_RECENT_BARS = 14
RANGE_WALK_ENTROPY_TRANSITION_PRIOR_BARS = 28
MIN_RANGE_WALK_ENTROPY_TRANSITION_ACTIVITY = 0.07
...
class LinearRangeWalkEntropyTransitionModel:
    fold_id: str
    ...
    range_walk_entropy_transition_failure_rate: float
...
def run_c0103_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_range_walk_entropy_transition_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_range_walk_entropy_transition_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        RANGE_WALK_ENTROPY_TRANSITION_LOOKBACK_BARS,
        RANGE_WALK_ENTROPY_TRANSITION_RECENT_BARS + RANGE_WALK_ENTROPY_TRANSITION_PRIOR_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = range_walk_entropy_transition_metrics(
    ...
    features = (
        metrics["range_walk_entropy"],
        metrics["directional_run_persistence"],
        metrics["net_path_efficiency"],
        metrics["range_expansion_decay"],
        metrics["settlement_persistence"],
        metrics["wick_opposition_pressure"],
        metrics["session_position_extreme"],
        metrics["prior_day_range_walk_context"],
        metrics["time_decay_alignment"],
        metrics["session_phase_maturity"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        c0103_entropy_bucket(metrics["range_walk_entropy"]),
        c0103_run_persistence_bucket(metrics["directional_run_persistence"]),
        c0103_path_efficiency_bucket(metrics["net_path_efficiency"]),
        c0103_range_decay_bucket(metrics["range_expansion_decay"]),
        c0103_settlement_bucket(metrics["settlement_persistence"]),
        c0103_wick_pressure_bucket(metrics["wick_opposition_pressure"]),
        c0103_session_position_bucket(metrics["session_position_extreme"]),
        c0103_prior_context_bucket(metrics["prior_day_range_walk_context"]),
        c0103_time_decay_bucket(metrics["time_decay_alignment"]),
...
def range_walk_entropy_transition_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    recent_start = max(index - RANGE_WALK_ENTROPY_TRANSITION_RECENT_BARS + 1, context.day_start_index)
    prior_start = max(recent_start - RANGE_WALK_ENTROPY_TRANSITION_PRIOR_BARS, context.day_start_index)
    lookback_start = max(index - RANGE_WALK_ENTROPY_TRANSITION_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    if len(prior_window) < 10 or len(recent_window) < 7 or len(lookback_window) < 24:
        return {"event_count": 0.0}
    ...
    if recent_range_mean <= 0 or prior_range_mean <= 0:
        return {"event_count": 0.0}

    session_phase_maturity = bounded_session_progress(signal_bar.time)
    if session_phase_maturity < 0.12:
        return {"event_count": 0.0}
    prior_volume_ratio = prior_volume_mean / max(average_volume, 1e-9)
    recent_range_ratio = recent_range_mean / max(average_range, 1e-9)
    prior_range_ratio = prior_range_mean / max(average_range, 1e-9)
    recent_returns = [recent_window[pos].close - recent_window[pos - 1].close for pos in range(1, len(recent_window))]
    prior_returns = [prior_window[pos].close - prior_window[pos - 1].close for pos in range(1, len(prior_window))]
    if not recent_returns or not prior_returns:
        return {"event_count": 0.0}

    directional_steps = sum(1 for value in recent_returns if direction * value > 0)
    opposing_steps = sum(1 for value in recent_returns if direction * value < 0)

    def binary_entropy(first: int, second: int) -> float:
        total = first + second
        if total <= 0:
            return 1.0
        entropy = 0.0
    ...
    entropy_norm = binary_entropy(directional_steps, opposing_steps)
    step_count = max(directional_steps + opposing_steps, 1)
    step_bias = (directional_steps - opposing_steps) / step_count
    range_walk_entropy = step_bias * (1.0 - entropy_norm)
    ...
    for value in recent_returns:
        directional_value = direction * value
        if directional_value > 0:
            ...
            current_with = 0
            current_against = 0
        max_with_run = max(max_with_run, current_with)
        max_against_run = max(max_against_run, current_against)
    directional_run_persistence = (max_with_run - max_against_run) / max(len(recent_returns), 1)

    recent_move = signal_bar.close - recent_window[0].open
    recent_travel = sum(abs(value) for value in recent_returns)
    net_path_efficiency = direction * recent_move / max(recent_travel, average_range * 0.25)
    range_expansion_decay = prior_range_mean / max(recent_range_mean, average_range * 0.25) - 1.0
    settlement_persistence = base.mean([directional_bar_settlement(bar, direction) for bar in recent_window]) or 0.0
    wick_opposition_pressure = -(base.mean([directional_wick_rejection(bar, direction) for bar in recent_window]) or 0.0)
    session_position = directional_session_position(signal_bar.close, context, direction)
    session_position_extreme = 0.0 if session_position is None else 2.0 * session_position - 1.0
    prior_day_range_walk_context = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        ...
        prior_day_range_walk_context = (
            direction * context.prior_day_return / max(context.prior_day_range, 1e-9) * max(0.0, 1.0 - shadow_balance)
        )
    time_decay_alignment = session_phase_maturity * (
        0.45 * range_walk_entropy
        + 0.30 * net_path_efficiency
        + 0.18 * directional_run_persistence
        + 0.10 * max(range_expansion_decay, 0.0)
        - 0.08 * max(wick_opposition_pressure, 0.0)
        - 0.10 * entropy_norm
    )
    activity = max(
        abs(range_walk_entropy),
        abs(net_path_efficiency) * 0.25,
        abs(directional_run_persistence) * 0.35,
        max(settlement_persistence - 0.50, 0.0) * 0.25,
        abs(session_position_extreme) * 0.12,
        max(range_expansion_decay, 0.0) * 0.12,
    )
    if activity < MIN_RANGE_WALK_ENTROPY_TRANSITION_ACTIVITY:
        return {"event_count": 0.0}
    if abs(range_walk_entropy) < 0.02 and abs(net_path_efficiency) < 0.08 and abs(directional_run_persistence) < 0.08:
        return {"event_count": 0.0}
    return {
        "range_walk_entropy": bounded(range_walk_entropy, -1.0, 1.0),
        "directional_run_persistence": bounded(directional_run_persistence, -1.0, 1.0),
        "net_path_efficiency": bounded(net_path_efficiency, -2.0, 2.0),
        "range_expansion_decay": bounded(range_expansion_decay, -4.0, 4.0),
        "settlement_persistence": bounded(settlement_persistence, 0.0, 1.0),
        "wick_opposition_pressure": bounded(wick_opposition_pressure, -1.0, 1.0),
        "session_position_extreme": bounded(session_position_extreme, -1.0, 1.0),
        "prior_day_range_walk_context": bounded(prior_day_range_walk_context, -2.0, 2.0),
        "time_decay_alignment": bounded(time_decay_alignment, -4.0, 4.0),
        ...
        "prior_volume_ratio": bounded(prior_volume_ratio, 0.0, 8.0),
        "entropy_norm": bounded(entropy_norm, 0.0, 1.0),
        "range_walk_entropy_transition_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0103_entropy_bucket(value: float) -> str:
    if value < -0.08:
        return "entropy_against"
    if value < 0.10:
        return "entropy_mixed"
    return "entropy_aligned"


def c0103_run_persistence_bucket(value: float) -> str:
    if value < -0.16:
        return "run_persistence_against"
    if value < 0.20:
        return "run_persistence_mixed"
    return "run_persistence_aligned"


def c0103_path_efficiency_bucket(value: float) -> str:
    if value < -0.08:
        return "path_efficiency_against"
    if value < 0.18:
        return "path_efficiency_noisy"
    return "path_efficiency_clean"


def c0103_range_decay_bucket(value: float) -> str:
    if value < -0.20:
        return "range_reexpanding"
    if value < 0.25:
        return "range_decay_neutral"
    return "range_decay_present"


def c0103_settlement_bucket(value: float) -> str:
    if value < 0.42:
        return "settlement_against"
    if value < 0.62:
        return "settlement_balanced"
    return "settlement_persistent"


def c0103_wick_pressure_bucket(value: float) -> str:
    if value < -0.08:
        return "wick_pressure_supportive"
    if value < 0.14:
        return "wick_pressure_mixed"
    return "wick_pressure_opposing"


def c0103_session_position_bucket(value: float) -> str:
    if value < -0.30:
        return "session_position_adverse"
    if value < 0.30:
        return "session_position_middle"
    return "session_position_extreme"


def c0103_prior_context_bucket(value: float) -> str:
    if value < -0.18:
        return "prior_range_walk_against"
    if value < 0.18:
        return "prior_range_walk_neutral"
    return "prior_range_walk_aligned"


def c0103_time_decay_bucket(value: float) -> str:
    if value < -0.08:
        return "time_decay_fading"
    if value < 0.16:
        return "time_decay_mixed"
    return "time_decay_aligned"
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    entropy_reward = 0.14 if metrics["range_walk_entropy"] > 0.10 and terminal > 0 else 0.0
    run_reward = 0.11 if metrics["directional_run_persistence"] > 0.18 and terminal > 0 else 0.0
    efficiency_reward = 0.13 if metrics["net_path_efficiency"] > 0.16 and terminal > 0 else 0.0
    decay_reward = 0.08 if metrics["range_expansion_decay"] > 0.20 and terminal > 0 else 0.0
    settlement_reward = 0.10 if metrics["settlement_persistence"] > 0.62 and terminal > 0 else 0.0
    wick_relief_reward = 0.07 if metrics["wick_opposition_pressure"] < 0.05 and terminal > 0 else 0.0
    extreme_reward = 0.06 if metrics["session_position_extreme"] > 0.20 and terminal > 0 else 0.0
    prior_reward = 0.05 if metrics["prior_day_range_walk_context"] > 0.12 and terminal > 0 else 0.0
    time_decay_reward = 0.06 if metrics["time_decay_alignment"] > 0.08 and terminal > 0 else 0.0
    entropy_penalty = 0.12 if metrics["range_walk_entropy"] < -0.08 and terminal <= 0 else 0.0
    run_break_penalty = 0.10 if metrics["directional_run_persistence"] < -0.10 and terminal <= 0 else 0.0
    inefficient_path_penalty = 0.12 if metrics["net_path_efficiency"] < -0.08 and terminal <= 0 else 0.0
    adverse_wick_penalty = 0.10 if metrics["wick_opposition_pressure"] > 0.18 and terminal <= 0 else 0.0
    anti_context_penalty = 0.06 if metrics["prior_day_range_walk_context"] < -0.18 and terminal <= 0 else 0.0
    ...
    range_walk_entropy_transition_path_quality = (0.54 * terminal + 0.30 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + range_walk_entropy_transition_path_quality
        + entropy_reward
        + run_reward
        + efficiency_reward
        + decay_reward
        ...
        - entropy_penalty
        - run_break_penalty
        - inefficient_path_penalty
        - adverse_wick_penalty
        - anti_context_penalty
        - stall_penalty
        - unfavorable_path_penalty
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["recent_range_ratio"] - 2.80) * 0.05
        - max(0.0, metrics["prior_range_ratio"] - 3.50) * 0.02
    )


def fit_linear_range_walk_entropy_transition_model(candidates: list[base.Candidate], fold_id: str) -> LinearRangeWalkEntropyTransitionModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearRangeWalkEntropyTransitionModel(
            fold_id,
    ...
    range_walk_entropy_transition_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearRangeWalkEntropyTransitionModel(
        fold_id=fold_id,
        ...
        range_walk_entropy_transition_failure_rate=range_walk_entropy_transition_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearRangeWalkEntropyTransitionModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0103-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_range_walk_entropy_transition_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0103_r0001_intraday_range_walk_entropy_transition",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0103_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0103_r0001_range_walk_entropy_transition_summary.json",
        ],
        ...
        "conditional_profiles": {
            "range_walk_entropy_transition_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0103 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0103_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0103 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "range_walk_entropy_transition_lookback_bars": RANGE_WALK_ENTROPY_TRANSITION_LOOKBACK_BARS,
        "range_walk_entropy_transition_recent_bars": RANGE_WALK_ENTROPY_TRANSITION_RECENT_BARS,
        "range_walk_entropy_transition_prior_bars": RANGE_WALK_ENTROPY_TRANSITION_PRIOR_BARS,
        "min_range_walk_entropy_transition_activity": MIN_RANGE_WALK_ENTROPY_TRANSITION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_range_walk_entropy_transition_model_summary(model: LinearRangeWalkEntropyTransitionModel) -> dict[str, object]:
    return {
        ...
        "range_walk_entropy_transition_failure_rate": rounded(model.range_walk_entropy_transition_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearRangeWalkEntropyTransitionModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_range_walk_entropy_transition_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "range_walk_entropy_transition_profile": profiles["range_walk_entropy_transition_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0103_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0103_r0001_proxy_trades.csv"
    evidence["range_walk_entropy_transition_summary"] = "artifacts/c0103_r0001_range_walk_entropy_transition_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0103_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0103_r0001_proxy_trades.csv",
        "artifacts/c0103_r0001_range_walk_entropy_transition_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0103_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0103 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0103_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0103_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0103_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0103_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0103_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0103_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0103_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0103_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0103_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0103 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0103 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0103_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0103_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0103_r0001_intraday_range_walk_entropy_transition_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0103_r0001_proxy_records_range_walk_entropy_transition_reference_surface
  ...
  - next_work_is_produce_c0103_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0103_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0103-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0103_r0001_intraday_range_walk_entropy_transition",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0103-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0103-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0103-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0103_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0103_r0001_intraday_range_walk_entropy_transition",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0103-R0001-range-walk-entropy-transition-SUMMARY",
            "artifact_role": "range_walk_entropy_transition_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0103_r0001_range_walk_entropy_transition_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0103_r0001_intraday_range_walk_entropy_transition",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0103_r0001_mt5_logic_parity_evidence",
        }
    ]
