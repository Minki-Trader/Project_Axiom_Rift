"""C0006 R0001 proxy evidence for liquidity sweep and reclaim events."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0006_liquidity_sweep_reclaim_event_discovery" / "runs" / "R0001"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0006_r0001_proxy_trades.csv"
EVENT_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0006_r0001_liquidity_sweep_reclaim_summary.json"
...
FEATURE_NAMES = (
    ...
    "reclaim_strength_over_range",
    "rejection_wick_fraction",
    "close_back_inside_fraction",
...
MODEL_FAMILY = "fold_local_train_only_liquidity_sweep_reclaim_event_utility"
LABEL_SHAPE = "event_reclaim_directional_target_before_stop_plus_path_quality"
...
MIN_RECLAIM_OVER_BAR_RANGE = 0.03
...
def run_c0006_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            event = sweep_reclaim_features(bars, range_average, short_range_average, index, direction)
            if event is None:
                continue
            scope, features = event
            label = liquidity_sweep_reclaim_label(bars, range_average, index, direction) if include_labels else None
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|{scope}|liquidity_sweep_reclaim",
                    features=features,
                    label=label,
                )
            )
...
def sweep_reclaim_features(
    ...
    if direction > 0:
        candidates.extend(
            [
                ("local", local_low, max(local_low - bar.low, 0.0), max(bar.close - local_low, 0.0), local_high - local_low),
                ("wide", wide_low, max(wide_low - bar.low, 0.0), max(bar.close - wide_low, 0.0), wide_high - wide_low),
                ("session", session_extreme, max(session_extreme - bar.low, 0.0), max(bar.close - session_extreme, 0.0), wide_high - wide_low),
            ]
        )
    else:
        candidates.extend(
            [
                ("local", local_high, max(bar.high - local_high, 0.0), max(local_high - bar.close, 0.0), local_high - local_low),
                ("wide", wide_high, max(bar.high - wide_high, 0.0), max(wide_high - bar.close, 0.0), wide_high - wide_low),
                ("session", session_extreme, max(bar.high - session_extreme, 0.0), max(session_extreme - bar.close, 0.0), wide_high - wide_low),
            ]
        )
    valid = [
        row
        for row in candidates
        if row[2] / average_range >= MIN_SWEEP_DEPTH_OVER_RANGE and row[3] / bar_range >= MIN_RECLAIM_OVER_BAR_RANGE
    ]
    if not valid:
        return None
    scope, level, sweep_depth, reclaim_strength, event_range_width = max(
        valid,
    ...
    close_back_inside = reclaim_strength / bar_range
    ...
    return scope, (
        ...
        reclaim_strength / average_range,
...
def liquidity_sweep_reclaim_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    index: int,
    direction: int,
...
def score_event_candidates(candidates: list[base.Candidate], model: EventUtilityModel) -> list[base.Candidate]:
    ...
    reclaim_strength = features[:, FEATURE_NAMES.index("reclaim_strength_over_range")]
    rejection_wick = features[:, FEATURE_NAMES.index("rejection_wick_fraction")]
    close_inside = features[:, FEATURE_NAMES.index("close_back_inside_fraction")]
    ...
    scores = (
        0.62 * signed_utility
        + 0.22 * np.tanh(2.2 * reclaim_strength)
...
def build_proxy_payload(
    ...
    payload["proxy_id"] = "PX-C0006-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0006_r0001_liquidity_sweep_reclaim"
    payload["proxy_config_path"] = "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_liquidity_sweep_reclaim_event_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0001/artifacts/c0006_r0001_proxy_trades.csv",
        "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0001/artifacts/c0006_r0001_liquidity_sweep_reclaim_summary.json",
    ]
    ...
    profiles["liquidity_sweep_reclaim_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
            ...
            "min_reclaim_over_bar_range": MIN_RECLAIM_OVER_BAR_RANGE,
            "score_component_names": list(SCORE_COMPONENT_NAMES),
            "fold_models": fold_models,
            "state_distributions": state_distributions,
            "candidates_by_fold": candidates_by_fold,
            "selection_rule": "top_fold_local_liquidity_sweep_reclaim_scores_per_active_day",
    ...
    config.update(
        {
            ...
            "min_reclaim_over_bar_range": MIN_RECLAIM_OVER_BAR_RANGE,
            "score_component_names": list(SCORE_COMPONENT_NAMES),
            "score_interpretation": "higher_score_means_train_fold_event_utility_supports_reclaim_follow_through",
            "variant_boundary": "new_liquidity_sweep_reclaim_event_grammar_not_analog_memory_state_bucket_score_cell_exit_shape_or_parameter_nudge",
        }
    )
    return config
...
def write_event_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_liquidity_sweep_reclaim_summary_v1",
        "template": False,
        "work_unit_id": "C0006",
        "campaign_id": "C0006",
        "run_id": "R0001",
        ...
        "liquidity_sweep_reclaim_profile": profiles["liquidity_sweep_reclaim_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "liquidity_sweep_reclaim_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0006-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0006-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0006-R0001-EVENT-UTILITY-SUMMARY",
                "liquidity_sweep_reclaim_summary_artifact",
                "json",
                "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0001/artifacts/c0006_r0001_liquidity_sweep_reclaim_summary.json",
                summary_hash,
                ["campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0001/kpi/proxy.json"],
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0006_r0001_mt5_logic_parity_evidence",
        }
    ]
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0006_r0001_liquidity_sweep_reclaim",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0006_r0001_proxy_trades.csv"
    evidence["liquidity_sweep_reclaim_summary"] = "artifacts/c0006_r0001_liquidity_sweep_reclaim_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0006_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0006_r0001_proxy_trades.csv",
        "artifacts/c0006_r0001_liquidity_sweep_reclaim_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0006 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0006_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0006_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    if "produce_c0006_r0001_proxy_evidence" not in completed:
        completed.append("produce_c0006_r0001_proxy_evidence")
    next_action = "produce_c0006_r0001_mt5_logic_parity_evidence"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0006_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0006_r0001_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            ...
            "R0001": "R0001",
            "R0001": "R0001",
            "c0004_r0001_fold_local_state_archetype": "c0006_r0001_liquidity_sweep_reclaim",
            "c0004_r0001": "c0006_r0001",
