"""C0080 R0001 proxy evidence for intraday micro pullback failure."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0080_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0080_r0001_mpf_summary.json"
...
FEATURE_NAMES = (
    "trend_impulse_pressure",
    "micro_pullback_depth",
    "pullback_rejection_wick",
    "close_rejoin_strength",
    "failed_counter_body_ratio",
    "range_ladder_retention",
    "volume_dry_up_then_reignite",
    "prior_day_trend_context",
    "session_continuation_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_micro_pullback_failure_rank"
LABEL_SHAPE = "target_first_continuation_quality_conditioned_on_failed_micro_pullback_state"
SELECTION_RULE = "top_fold_local_micro_pullback_failure_scores_per_active_day"
...
MIN_DAY_BARS = 30
MICRO_PULLBACK_FAILURE_LOOKBACK_BARS = 18
MICRO_PULLBACK_FAILURE_SHORT_LOOKBACK_BARS = 6
MIN_MICRO_PULLBACK_FAILURE_ACTIVITY = 0.18
...
class LinearMicroPullbackFailureModel:
    fold_id: str
    ...
    micro_pullback_failure_failure_rate: float
...
def run_c0080_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_micro_pullback_failure_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_micro_pullback_failure_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        MICRO_PULLBACK_FAILURE_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    release = micro_pullback_failure_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        release["trend_impulse_pressure"],
        release["micro_pullback_depth"],
        release["pullback_rejection_wick"],
        release["close_rejoin_strength"],
        release["failed_counter_body_ratio"],
        release["range_ladder_retention"],
        release["volume_dry_up_then_reignite"],
        release["prior_day_trend_context"],
        release["session_continuation_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        impulse_range_bucket(release["trend_impulse_pressure"]),
        shallow_pullback_bucket(release["micro_pullback_depth"]),
        wick_bucket(release["pullback_rejection_wick"]),
        close_commitment_bucket(release["close_rejoin_strength"]),
        body_bucket(release["failed_counter_body_ratio"]),
        retention_bucket(release["range_ladder_retention"]),
        volume_confirmation_bucket(release["volume_dry_up_then_reignite"]),
        prior_direction_context_bucket(release["prior_day_trend_context"]),
        session_pressure_bucket(release["session_continuation_pressure"]),
...
def micro_pullback_failure_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    short_start = index - MICRO_PULLBACK_FAILURE_SHORT_LOOKBACK_BARS + 1
    prior_start = index - MICRO_PULLBACK_FAILURE_LOOKBACK_BARS + 1
    ...
    trend_impulse_pressure = direction * (signal_bar.close - prior_window[0].open) / average_range
    if direction > 0:
        pre_pullback_extreme = max(bar.high for bar in prior_window[:-2])
        pullback_extreme = min(bar.low for bar in short_window)
        range_ladder_retention = (signal_bar.close - min(bar.low for bar in prior_window)) / prior_width
    else:
        pre_pullback_extreme = min(bar.low for bar in prior_window[:-2])
        pullback_extreme = max(bar.high for bar in short_window)
        range_ladder_retention = (max(bar.high for bar in prior_window) - signal_bar.close) / prior_width
    micro_pullback_depth = direction * (pre_pullback_extreme - pullback_extreme) / average_range
    pullback_rejection_wick = directional_wick_rejection(signal_bar, direction)
    close_rejoin_strength = directional_bar_settlement(signal_bar, direction)
    ...
    prior_counter_body = base.mean([max(-directional_body_score(bar, direction), 0.0) for bar in short_window[:-1]]) or 0.0
    current_directional_body = max(directional_body_score(signal_bar, direction), 0.0)
    failed_counter_body_ratio = prior_counter_body * (1.0 + current_directional_body)
    volume_dry_up_then_reignite = max(current_volume_ratio - early_volume_ratio, 0.0)
    prior_day_trend_context = direction * prior_day_return / max(prior_day_range, 1e-9)
    continuation_pressure = max(
        max(trend_impulse_pressure, 0.0) / 3.0,
        min(max(micro_pullback_depth, 0.0) / 1.8, 1.0),
        max(pullback_rejection_wick, 0.0),
        max(close_rejoin_strength - 0.45, 0.0),
        max(failed_counter_body_ratio, 0.0),
        min(max(volume_dry_up_then_reignite, 0.0) / 2.0, 1.0),
    )
    session_continuation_pressure = bounded_session_progress(signal_bar.time) * continuation_pressure
    activity = max(continuation_pressure, session_continuation_pressure)
    if activity < MIN_MICRO_PULLBACK_FAILURE_ACTIVITY:
        return {"event_count": 0.0}
    if trend_impulse_pressure < 0.10 or micro_pullback_depth < 0.04:
        return {"event_count": 0.0}
    if close_rejoin_strength < 0.28 and pullback_rejection_wick < -0.05:
        return {"event_count": 0.0}
    return {
        "trend_impulse_pressure": bounded(trend_impulse_pressure, -3.0, 8.0),
        "micro_pullback_depth": bounded(micro_pullback_depth, -2.0, 5.0),
        "pullback_rejection_wick": bounded(pullback_rejection_wick, -1.0, 1.0),
        "close_rejoin_strength": bounded(close_rejoin_strength, 0.0, 1.0),
        "failed_counter_body_ratio": bounded(failed_counter_body_ratio, 0.0, 3.0),
        "range_ladder_retention": bounded(range_ladder_retention, 0.0, 1.0),
        "volume_dry_up_then_reignite": bounded(volume_dry_up_then_reignite, 0.0, 6.0),
        "prior_day_trend_context": bounded(prior_day_trend_context, -2.0, 2.0),
        "session_continuation_pressure": bounded(session_continuation_pressure, 0.0, 4.0),
        "event_count": 1.0,
    }
...
def candidate_label(
    ...
    metrics = micro_pullback_failure_metrics(
        bars,
        volumes,
    ...
    if metrics["event_count"] <= 0:
        return None
    impulse_reward = 0.16 if metrics["trend_impulse_pressure"] > 0.35 and terminal > 0 else 0.0
    pullback_reward = 0.13 if 0.08 <= metrics["micro_pullback_depth"] <= 1.65 and terminal > 0 else 0.0
    rejection_reward = 0.12 if metrics["pullback_rejection_wick"] > 0.08 and terminal > 0 else 0.0
    rejoin_reward = 0.14 if metrics["close_rejoin_strength"] > 0.55 and terminal > 0 else 0.0
    failed_counter_reward = 0.12 if metrics["failed_counter_body_ratio"] > 0.10 and terminal > 0 else 0.0
    retention_reward = 0.10 if metrics["range_ladder_retention"] > 0.52 and terminal > 0 else 0.0
    volume_reward = 0.08 if metrics["volume_dry_up_then_reignite"] > 0.08 and terminal > 0 else 0.0
    prior_context_reward = 0.06 if metrics["prior_day_trend_context"] > -0.20 and terminal > 0 else 0.0
    weak_impulse_penalty = 0.13 if metrics["trend_impulse_pressure"] < 0.18 and terminal <= 0 else 0.0
    deep_pullback_penalty = 0.12 if metrics["micro_pullback_depth"] > 1.85 and terminal <= 0 else 0.0
    wick_against_penalty = 0.12 if metrics["pullback_rejection_wick"] < -0.08 and terminal <= 0 else 0.0
    weak_rejoin_penalty = 0.13 if metrics["close_rejoin_strength"] < 0.30 and terminal <= 0 else 0.0
    weak_counter_fail_penalty = 0.10 if metrics["failed_counter_body_ratio"] < 0.04 and terminal <= 0 else 0.0
    low_retention_penalty = 0.10 if metrics["range_ladder_retention"] < 0.42 and terminal <= 0 else 0.0
    no_volume_reignite_penalty = 0.08 if metrics["volume_dry_up_then_reignite"] < 0.03 and terminal <= 0 else 0.0
    ...
    micro_pullback_failure_path_quality = (0.44 * terminal + 0.36 * mfe - 0.72 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + micro_pullback_failure_path_quality
        + impulse_reward
        + pullback_reward
        + rejection_reward
        + rejoin_reward
        + failed_counter_reward
        + retention_reward
        + volume_reward
        ...
        - weak_rejoin_penalty
        - weak_counter_fail_penalty
        - low_retention_penalty
        - no_volume_reignite_penalty
        - stall_penalty
        - unfavorable_path_penalty
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["micro_pullback_depth"] - 2.20) * 0.06
    )


def fit_linear_micro_pullback_failure_model(candidates: list[base.Candidate], fold_id: str) -> LinearMicroPullbackFailureModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearMicroPullbackFailureModel(
            fold_id,
    ...
    micro_pullback_failure_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearMicroPullbackFailureModel(
        fold_id=fold_id,
        ...
        micro_pullback_failure_failure_rate=micro_pullback_failure_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearMicroPullbackFailureModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_micro_pullback_failure_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0080_r0001_intraday_micro_pullback_failure",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0080_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0080_r0001_mpf_summary.json",
        ],
        ...
        "conditional_profiles": {
            "micro_pullback_failure_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0080 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0080_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0080 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "micro_pullback_failure_lookback_bars": MICRO_PULLBACK_FAILURE_LOOKBACK_BARS,
        "micro_pullback_failure_short_lookback_bars": MICRO_PULLBACK_FAILURE_SHORT_LOOKBACK_BARS,
        "min_micro_pullback_failure_activity": MIN_MICRO_PULLBACK_FAILURE_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_micro_pullback_failure_model_summary(model: LinearMicroPullbackFailureModel) -> dict[str, object]:
    return {
        ...
        "micro_pullback_failure_failure_rate": rounded(model.micro_pullback_failure_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearMicroPullbackFailureModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_micro_pullback_failure_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "micro_pullback_failure_profile": profiles["micro_pullback_failure_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0080_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0080_r0001_proxy_trades.csv"
    evidence["micro_pullback_failure_summary"] = "artifacts/c0080_r0001_mpf_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0080_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0080_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0080 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0080_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0080_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0080_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0080_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0080_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0080_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0080_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0080_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0080_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0080 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0080 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0080_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0080_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0080_r0001_intraday_micro_pullback_failure_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0080_r0001_proxy_records_micro_pullback_failure_reference_surface
  ...
  - next_work_is_produce_c0080_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0080_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0080-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0080_r0001_intraday_micro_pullback_failure",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0080-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0080-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0080-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0080_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0080_r0001_intraday_micro_pullback_failure",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0080-R0001-micro-pullback-failure-SUMMARY",
            "artifact_role": "micro_pullback_failure_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0080_r0001_mpf_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0080_r0001_intraday_micro_pullback_failure",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0080_r0001_mt5_logic_parity_evidence",
        }
    ]
