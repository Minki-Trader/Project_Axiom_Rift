"""C0043 R0001 proxy evidence for intraday range shock digest discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0043_intraday_range_shock_digest_discovery" / "runs" / "R0001"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0043_intraday_range_shock_digest_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0043_r0001_proxy_trades.csv"
RANGE_SHOCK_DIGEST_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0043_r0001_range_shock_digest_summary.json"
...
FEATURE_NAMES = (
    ...
    "range_shock_24",
    "range_shock_48",
    "directional_shock_close_efficiency",
    "shock_cluster_density_6",
    "shock_cluster_density_12",
    "shock_cluster_density_24",
    "post_shock_compression_6",
    "post_shock_compression_12",
    "post_shock_expansion_6",
    "post_shock_expansion_12",
    "directional_post_shock_return_3",
    "directional_post_shock_return_6",
    "shock_follow_through_3",
    "shock_follow_through_6",
    "shock_exhaustion_3",
    "shock_exhaustion_6",
    "shock_reversal_pressure_6",
    "shock_continuation_pressure_6",
    "shock_range_position",
    "directional_day_range_depletion",
    "day_range_depletion",
    "shock_to_day_range_ratio",
    "range_acceleration_3_12",
    "range_acceleration_6_24",
    "body_to_shock_range",
    "directional_body_to_shock_range",
    "upper_wick_shock_pressure",
    "lower_wick_shock_pressure",
    "directional_wick_shock_pressure",
    "shock_churn_ratio_6",
    "shock_churn_ratio_12",
    "range_volatility_of_volatility_12",
    "range_volatility_of_volatility_24",
    "shock_session_progress",
    "spread_over_range",
    "spread_shock_stress_12",
    "directional_close_streak_12",
    "path_efficiency_12",
    "path_churn_ratio_12",
    "normalized_close_return_3",
    "shock_regime_contrast_12_48",
)
MODEL_FAMILY = "fold_local_intraday_range_shock_digest_rank"
LABEL_SHAPE = "directional_intraday_range_shock_digest_survival_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_intraday_range_shock_digest_scores_per_active_day"
...
def run_c0043_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    price_context = build_range_shock_digest_context(bars, range_average, short_range_average, day_context)
    trades: list[base.Trade] = []
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = intraday_range_shock_digest_features(
                bars,
                range_average,
                short_range_average,
                day_context,
            ...
            if features is None:
                continue
            label = (
                intraday_range_shock_digest_label(bars, range_average, price_context, index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|intraday_range_shock_digest",
                    features=features,
                    label=label,
                )
            )
...
def build_range_shock_digest_context(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
...
def intraday_range_shock_digest_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    if len(features) != len(FEATURE_NAMES):
        raise RuntimeError(f"C0043 feature length mismatch: {len(features)} != {len(FEATURE_NAMES)}")
    return tuple(float(value) for value in features)


def intraday_range_shock_digest_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    ribbon_context: list[dict[str, float] | None],
    index: int,
    ...
    future_release = future_range_shock_digest_retention(ribbon_context, entry_index, exit_index, direction)
...
def future_range_shock_digest_retention(
    ribbon_context: list[dict[str, float] | None],
    entry_index: int,
    exit_index: int,
    direction: int,
...
def linear_model_summary(model: LinearEdgeModel) -> dict[str, object]:
    return {
        ...
        "score_interpretation": "higher_score_means_direct_fold_local_intraday_range_shock_digest_rank",
    }
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0043"
    payload["work_unit_id"] = "C0043"
    payload["run_id"] = "R0001"
    payload["proxy_id"] = "PX-C0043-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0043_r0001_intraday_range_shock_digest"
    payload["proxy_config_path"] = "campaigns/C0043_intraday_range_shock_digest_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_intraday_range_shock_digest_rank_fit"
    payload["claim_boundary"] = dict(CLAIM_BOUNDARY)
    payload["proxy_artifact_paths"] = [
        "campaigns/C0043_intraday_range_shock_digest_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0043_intraday_range_shock_digest_discovery/runs/R0001/artifacts/c0043_r0001_proxy_trades.csv",
        "campaigns/C0043_intraday_range_shock_digest_discovery/runs/R0001/artifacts/c0043_r0001_range_shock_digest_summary.json",
    ]
    ...
    profiles["intraday_range_shock_digest_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
            "model_family": MODEL_FAMILY,
            "feature_count": len(FEATURE_NAMES),
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0043_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "score_interpretation": "higher_score_means_direct_fold_local_intraday_range_shock_digest_rank",
            "variant_boundary": (
                "intraday_range_shock_digest_rank_not_effort_absorption_seasonal_residual_tail_risk_skew_entropy_"
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_range_shock_digest_summary_artifact(payload, RANGE_SHOCK_DIGEST_SUMMARY_PATH)
    write_text_local(PROXY_PATH, json.dumps(payload, indent=2, sort_keys=True) + "\n")
    trade_hash = sha256_file_local(TRADE_ARTIFACT_PATH)
    summary_hash = sha256_file_local(RANGE_SHOCK_DIGEST_SUMMARY_PATH)
...
def write_range_shock_digest_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_intraday_range_shock_digest_summary_v1",
        "template": False,
        "work_unit_id": "C0043",
        "campaign_id": "C0043",
        "run_id": "R0001",
        ...
        "intraday_range_shock_digest_profile": profiles["intraday_range_shock_digest_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "intraday_range_shock_digest_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0043-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0043_intraday_range_shock_digest_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0043_intraday_range_shock_digest_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0043-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0043-R0001-RANGE-SHOCK-DIGEST-SUMMARY",
                "intraday_range_shock_digest_summary_artifact",
                "json",
                "campaigns/C0043_intraday_range_shock_digest_discovery/runs/R0001/artifacts/c0043_r0001_range_shock_digest_summary.json",
                summary_hash,
                ["campaigns/C0043_intraday_range_shock_digest_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0043_r0001_intraday_range_shock_digest",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0043_r0001_proxy_trades.csv"
    evidence["intraday_range_shock_digest_summary"] = "artifacts/c0043_r0001_range_shock_digest_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0043_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0043_r0001_proxy_trades.csv",
        "artifacts/c0043_r0001_range_shock_digest_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0043 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0043_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0043_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0043_intraday_range_shock_digest_discovery"
    data["project"]["active_run"] = "campaigns/C0043_intraday_range_shock_digest_discovery/runs/R0001"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0043_intraday_range_shock_digest_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0043_intraday_range_shock_digest_discovery",
        "open_c0043_r0001_fold_local_intraday_range_shock_digest_rank_run",
        "produce_c0043_r0001_proxy_evidence",
    ):
    ...
    data["active_campaign"] = "campaigns/C0043_intraday_range_shock_digest_discovery"
    data["active_run"] = "campaigns/C0043_intraday_range_shock_digest_discovery/runs/R0001"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0043_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0043_intraday_range_shock_digest_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0043_r0001_mt5_logic_parity_evidence",
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["canonical_source"] = "campaigns/C0043_intraday_range_shock_digest_discovery/runs/R0001/kpi/proxy.json"
    data["canonical_status"] = "proxy_recorded_pending_mt5"
    data["current_decision"] = "produce_c0043_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = "campaigns/C0043_intraday_range_shock_digest_discovery"
    data["active_run"] = "campaigns/C0043_intraday_range_shock_digest_discovery/runs/R0001"
    data["next_required_action"] = "produce_c0043_r0001_mt5_logic_parity_evidence"
    data["current_evidence_summary"] = {
        "source_campaign": "campaigns/C0043_intraday_range_shock_digest_discovery",
        "current_task": "produce_c0043_r0001_mt5_logic_parity_evidence",
        "active_run": "campaigns/C0043_intraday_range_shock_digest_discovery/runs/R0001",
        "active_run_status": "proxy_recorded_pending_mt5",
        "evidence_status": "proxy_recorded_pending_mt5",
        "campaign_family": "intraday_range_shock_digest",
        "entries_per_active_day": summary.get("entries_per_active_day") if isinstance(summary, dict) else None,
        "proxy_trade_count": required.get("proxy_trade_count") if isinstance(required, dict) else None,
        "proxy_net_pnl_points": required.get("proxy_net_pnl_points") if isinstance(required, dict) else None,
        "proxy_profit_factor": required.get("proxy_profit_factor") if isinstance(required, dict) else None,
        "next_required_action": "produce_c0043_r0001_mt5_logic_parity_evidence",
        "note": "C0043 R0001 proxy evidence recorded; MT5 paired validation remains mandatory regardless of proxy result.",
        "source_campaign_manifest": "campaigns/C0043_intraday_range_shock_digest_discovery/campaign.yaml",
    }
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0043",
            "c0004_r0001_fold_local_state_archetype": "c0043_r0001_intraday_range_shock_digest",
            "c0004_r0001": "c0043_r0001",
            "fold_local_state_archetype_discovery": "intraday_range_shock_digest_discovery",
            ...
            "state_archetype_summary": "intraday_range_shock_digest_summary",
        }
...
def range_shock_ratio_at(bars: list[base.Bar], index: int, lookback: int, average_range: float) -> float:
...
def strongest_range_shock_index(
    bars: list[base.Bar],
    range_average: list[float | None],
    index: int,
    lookback: int,
...
def build_range_shock_digest_context(
    ...
    for index, bar in enumerate(bars):
        ...
        shock_index = strongest_range_shock_index(bars, range_average, index, 24, average_range)
        ...
        range_shock_12 = range_shock_ratio_at(bars, index, 12, average_range)
        contexts[index] = {
            "average_range": average_range,
            ...
            "range_shock_6": range_shock_ratio_at(bars, index, 6, average_range),
            "range_shock_12": range_shock_12,
            "range_shock_24": range_shock_ratio_at(bars, index, 24, average_range),
            "range_shock_48": range_shock_ratio_at(bars, index, 48, average_range),
            ...
            * (1.0 + range_shock_12 + shock_cluster_density_at(bars, index, 12, average_range)),
            ...
            "shock_regime_contrast_12_48": range_shock_12
            / max(range_shock_ratio_at(bars, index, 48, average_range), 1e-9)
            - 1.0,
...
def intraday_range_shock_digest_features(
    ...
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    range_shock_context: list[dict[str, float] | None],
    index: int,
    direction: int,
) -> list[float] | None:
    average_range = range_average[index]
    day = day_context[index]
    context = range_shock_context[index]
    ...
    features = [
        ...
        context["range_shock_6"],
        context["range_shock_12"],
        context["range_shock_24"],
        context["range_shock_48"],
        ...
        context["session_progress"] * max(context["range_shock_12"], 0.0),
