"""C0124 R0001 proxy evidence for intraday percentile recoil fade."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0124_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0124_r0001_percentile_recoil_fade_summary.json"
...
FEATURE_NAMES = (
    "session_extreme_percentile",
    "range_stretch_pressure",
    "recoil_body_balance",
    "adverse_wick_rejection",
    "vwap_distance_reversion_pressure",
    "prior_session_extension_context",
    "volume_climax_pressure",
    "micro_path_reversal_efficiency",
    "time_of_day_reversal_window",
    "reversion_room_to_session_mid",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_percentile_recoil_fade_rank"
LABEL_SHAPE = "target_first_quality_after_percentile_recoil_fade"
SELECTION_RULE = "top_fold_local_percentile_recoil_fade_scores_per_active_day"
...
MIN_DAY_BARS = 18
PERCENTILE_RECOIL_FADE_LOOKBACK_BARS = 54
PERCENTILE_RECOIL_FADE_RECOIL_BARS = 9
MIN_SESSION_EXTREME_PERCENTILE = 0.62
MIN_PERCENTILE_RECOIL_FADE_ACTIVITY = 0.05
...
class LinearPercentileRecoilFadeModel:
    fold_id: str
    ...
    percentile_recoil_fade_failure_rate: float
...
def run_c0124_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_percentile_recoil_fade_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_percentile_recoil_fade_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        PERCENTILE_RECOIL_FADE_LOOKBACK_BARS,
        PERCENTILE_RECOIL_FADE_RECOIL_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = percentile_recoil_fade_metrics(
    ...
    features = (
        metrics["session_extreme_percentile"],
        metrics["range_stretch_pressure"],
        metrics["recoil_body_balance"],
        metrics["adverse_wick_rejection"],
        metrics["vwap_distance_reversion_pressure"],
        metrics["prior_session_extension_context"],
        metrics["volume_climax_pressure"],
        metrics["micro_path_reversal_efficiency"],
        metrics["time_of_day_reversal_window"],
        metrics["reversion_room_to_session_mid"],
        metrics["session_maturity"],
        metrics["spread_stress_inverse"],
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        range_position_pressure_bucket(metrics["session_extreme_percentile"]),
        breakout_pressure_bucket(metrics["range_stretch_pressure"]),
        absorption_bucket(metrics["recoil_body_balance"]),
        absorption_bucket(metrics["adverse_wick_rejection"]),
        vwap_alignment_bucket(metrics["vwap_distance_reversion_pressure"]),
        prior_day_position_context_bucket(metrics["prior_session_extension_context"]),
        volume_stability_bucket(1.0 - abs(metrics["volume_climax_pressure"])),
        pullback_containment_bucket(metrics["micro_path_reversal_efficiency"]),
        session_maturity_bucket(metrics["time_of_day_reversal_window"]),
        range_position_pressure_bucket(metrics["reversion_room_to_session_mid"]),
...
def percentile_recoil_fade_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    recoil_start = max(index - PERCENTILE_RECOIL_FADE_RECOIL_BARS + 1, context.day_start_index)
    lookback_start = max(index - PERCENTILE_RECOIL_FADE_LOOKBACK_BARS + 1, context.day_start_index)
    if recoil_start < 0 or lookback_start < 0:
        return {"event_count": 0.0}
    recoil_window = bars[recoil_start : index + 1]
    lookback_window = bars[lookback_start : index + 1]
    day_window = bars[context.day_start_index : index + 1]
    if len(recoil_window) < 5 or len(lookback_window) < 24 or len(day_window) < MIN_DAY_BARS:
        return {"event_count": 0.0}
    average_volume = volume_average[index]
    if average_volume is None or average_volume <= 0:
        return {"event_count": 0.0}

    recoil_ranges = [bar.high - bar.low for bar in recoil_window]
    lookback_ranges = [bar.high - bar.low for bar in lookback_window]
    recoil_range_mean = base.mean(recoil_ranges) or average_range
    lookback_range_mean = base.mean(lookback_ranges) or average_range
    recoil_volume_mean = base.mean(volumes[recoil_start : index + 1]) or average_volume
    lookback_volume_mean = base.mean(volumes[lookback_start : index + 1]) or average_volume
    if recoil_range_mean <= 0 or lookback_range_mean <= 0:
        return {"event_count": 0.0}
    ...
    if session_range <= average_range * 0.35:
        return {"event_count": 0.0}
    ...
    session_extreme_percentile = 1.0 - directional_position
    if session_extreme_percentile < MIN_SESSION_EXTREME_PERCENTILE:
        return {"event_count": 0.0}

    range_stretch_pressure = session_range / max(lookback_range_mean * math.sqrt(min(context.bars_so_far, 96)) / 3.0, average_range, 1e-9)
    recoil_body_balance = (base.mean([directional_body_score(bar, direction) for bar in recoil_window]) or 0.0) + (
        directional_bar_settlement(signal_bar, direction) - 0.5
    )
    ...
    adverse_wick_rejection = base.mean([adverse_wick_share(bar) for bar in recoil_window]) or 0.0
    if max(recoil_body_balance, adverse_wick_rejection) < -0.20:
        return {"event_count": 0.0}

    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    vwap_distance_reversion_pressure = -direction * (signal_bar.close - session_vwap) / max(average_range * 3.0, 1e-9)
    prior_session_extension_context = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_session_extension_context = -direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
    if context.prior_day_shadow_balance is not None:
        prior_session_extension_context += -direction * context.prior_day_shadow_balance * 0.08
    volume_climax_pressure = recoil_volume_mean / max(lookback_volume_mean, average_volume, 1e-9) - 1.0
    adverse_extreme = min(bar.low for bar in recoil_window) if direction > 0 else max(bar.high for bar in recoil_window)
    favorable_recoil = direction * (signal_bar.close - adverse_extreme)
    path_travel = sum(abs(recoil_window[pos].close - recoil_window[pos - 1].close) for pos in range(1, len(recoil_window)))
    micro_path_reversal_efficiency = favorable_recoil / max(path_travel, average_range, 1e-9)
    time_of_day_reversal_window = bounded(1.0 - abs(session_phase_maturity - 0.52) / 0.40, 0.0, 1.0)
    session_mid = (context.high_so_far + context.low_so_far) / 2.0
    reversion_room_to_session_mid = direction * (session_mid - signal_bar.close) / max(average_range * 4.0, 1e-9)
    ...
    activity = max(
        session_extreme_percentile * 0.18,
        max(range_stretch_pressure, 0.0) * 0.10,
        max(recoil_body_balance, 0.0) * 0.18,
        adverse_wick_rejection * 0.13,
        max(vwap_distance_reversion_pressure, 0.0) * 0.12,
        max(reversion_room_to_session_mid, 0.0) * 0.11,
        max(volume_climax_pressure, 0.0) * 0.06,
        max(micro_path_reversal_efficiency, 0.0) * 0.14,
        time_of_day_reversal_window * 0.07,
    )
    if activity < MIN_PERCENTILE_RECOIL_FADE_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "session_extreme_percentile": bounded(session_extreme_percentile, 0.0, 1.0),
        "range_stretch_pressure": bounded(range_stretch_pressure, 0.0, 8.0),
        "recoil_body_balance": bounded(recoil_body_balance, -2.0, 2.0),
        "adverse_wick_rejection": bounded(adverse_wick_rejection, 0.0, 1.0),
        "vwap_distance_reversion_pressure": bounded(vwap_distance_reversion_pressure, -4.0, 4.0),
        "prior_session_extension_context": bounded(prior_session_extension_context, -3.0, 3.0),
        "volume_climax_pressure": bounded(volume_climax_pressure, -4.0, 8.0),
        "micro_path_reversal_efficiency": bounded(micro_path_reversal_efficiency, -2.0, 2.0),
        "time_of_day_reversal_window": bounded(time_of_day_reversal_window, 0.0, 1.0),
        "reversion_room_to_session_mid": bounded(reversion_room_to_session_mid, -4.0, 4.0),
        "session_maturity": bounded(session_phase_maturity, 0.0, 1.0),
        "spread_stress_inverse": bounded(spread_stress_inverse, 0.0, 1.0),
        "recent_range_ratio": bounded(recoil_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        ...
        "recent_volume_ratio": bounded(recoil_volume_mean / max(average_volume, 1e-9), 0.0, 8.0),
        ...
        "adverse_extreme": rounded(adverse_extreme) or 0.0,
        "percentile_recoil_fade_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0124_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0124_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0124_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0124_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0124_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0124_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0124_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0124_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0124_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0124_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0124_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0124_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0124_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0124_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0124_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0124_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0124_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0124_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0124_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def c0124_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    extreme_reward = 0.12 if metrics["session_extreme_percentile"] > 0.72 and terminal > 0 else 0.0
    stretch_reward = 0.07 if metrics["range_stretch_pressure"] > 0.85 and terminal > 0 else 0.0
    recoil_reward = 0.14 if metrics["recoil_body_balance"] > 0.03 and terminal > 0 else 0.0
    wick_rejection_reward = 0.08 if metrics["adverse_wick_rejection"] > 0.22 and terminal > 0 else 0.0
    vwap_reward = 0.08 if metrics["vwap_distance_reversion_pressure"] > -0.02 and terminal > 0 else 0.0
    prior_extension_reward = 0.04 if metrics["prior_session_extension_context"] > -0.25 and terminal > 0 else 0.0
    volume_reward = 0.04 if -0.25 <= metrics["volume_climax_pressure"] <= 2.50 and terminal > 0 else 0.0
    path_recoil_reward = 0.11 if metrics["micro_path_reversal_efficiency"] > 0.08 and terminal > 0 else 0.0
    timing_reward = 0.04 if metrics["time_of_day_reversal_window"] > 0.22 and terminal > 0 else 0.0
    room_reward = 0.08 if metrics["reversion_room_to_session_mid"] > 0.08 and terminal > 0 else 0.0
    weak_extreme_penalty = 0.08 if metrics["session_extreme_percentile"] < 0.68 and terminal <= 0 else 0.0
    weak_recoil_penalty = 0.12 if metrics["recoil_body_balance"] < -0.08 and terminal <= 0 else 0.0
    weak_vwap_penalty = 0.08 if metrics["vwap_distance_reversion_pressure"] < -0.20 and terminal <= 0 else 0.0
    weak_room_penalty = 0.10 if metrics["reversion_room_to_session_mid"] < 0.00 and terminal <= 0 else 0.0
    excessive_climax_penalty = 0.05 if metrics["volume_climax_pressure"] > 3.50 and terminal <= 0 else 0.0
    ...
    return (
        ...
        + stretch_reward
        + recoil_reward
        + wick_rejection_reward
        + vwap_reward
        + prior_extension_reward
        + volume_reward
        + path_recoil_reward
        + timing_reward
        + room_reward
        ...
        - max(0.0, MIN_SESSION_EXTREME_PERCENTILE - metrics["session_extreme_percentile"]) * 0.16
    )


def fit_linear_percentile_recoil_fade_model(candidates: list[base.Candidate], fold_id: str) -> LinearPercentileRecoilFadeModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearPercentileRecoilFadeModel(
            fold_id,
    ...
    percentile_recoil_fade_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearPercentileRecoilFadeModel(
        fold_id=fold_id,
        ...
        percentile_recoil_fade_failure_rate=percentile_recoil_fade_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearPercentileRecoilFadeModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_percentile_recoil_fade_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0124_r0001_intraday_percentile_recoil_fade",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0124_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0124_r0001_percentile_recoil_fade_summary.json",
        ],
        ...
        "conditional_profiles": {
            "percentile_recoil_fade_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0124 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0124_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0124 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "percentile_recoil_fade_lookback_bars": PERCENTILE_RECOIL_FADE_LOOKBACK_BARS,
        "percentile_recoil_fade_recoil_bars": PERCENTILE_RECOIL_FADE_RECOIL_BARS,
        "min_session_extreme_percentile": MIN_SESSION_EXTREME_PERCENTILE,
        "min_percentile_recoil_fade_activity": MIN_PERCENTILE_RECOIL_FADE_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_percentile_recoil_fade_model_summary(model: LinearPercentileRecoilFadeModel) -> dict[str, object]:
    return {
        ...
        "percentile_recoil_fade_failure_rate": rounded(model.percentile_recoil_fade_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearPercentileRecoilFadeModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_percentile_recoil_fade_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "percentile_recoil_fade_profile": profiles["percentile_recoil_fade_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0124_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0124_r0001_proxy_trades.csv"
    evidence["percentile_recoil_fade_summary"] = "artifacts/c0124_r0001_percentile_recoil_fade_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0124_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0124_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0124 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0124_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0124_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0124_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0124_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0124_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0124_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0124_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0124_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0124_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0124 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0124 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260708_produce_c0124_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0124_r0001_proxy_evidence
  refines:
  - dec_20260708_open_c0124_r0001_intraday_percentile_recoil_fade_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0124_r0001_proxy_records_percentile_recoil_fade_reference_surface
  ...
  - next_work_is_produce_c0124_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260708_produce_c0124_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0124-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0124_r0001_intraday_percentile_recoil_fade",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0124-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0124-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0124-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0124_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0124_r0001_intraday_percentile_recoil_fade",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0124-R0001-PERCENTILE-RECOIL-FADE-SUMMARY",
            "artifact_role": "percentile_recoil_fade_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0124_r0001_percentile_recoil_fade_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0124_r0001_intraday_percentile_recoil_fade",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0124_r0001_mt5_logic_parity_evidence",
        }
    ]
