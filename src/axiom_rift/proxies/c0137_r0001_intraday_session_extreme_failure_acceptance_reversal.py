"""C0137 R0001 proxy evidence for intraday session extreme failure acceptance reversal."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0137_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0137_r0001_session_extreme_failure_acceptance_reversal_summary.json"
...
FEATURE_NAMES = (
    "failed_extreme_extension",
    "reclaim_depth",
    "reversal_acceptance_persistence",
    "opposite_side_close_efficiency",
    "exhaustion_wick_pressure",
    "range_reentry_quality",
    "micro_rotation_reversal",
    "session_room_to_mid",
    "volume_exhaustion_reengagement",
    "prior_day_contrast",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_session_extreme_failure_acceptance_reversal_rank"
LABEL_SHAPE = "target_first_quality_after_session_extreme_failure_acceptance_reversal"
SELECTION_RULE = "top_fold_local_session_extreme_failure_acceptance_reversal_scores_per_active_day"
...
MIN_DAY_BARS = 18
SESSION_EXTREME_FAILURE_ACCEPTANCE_REVERSAL_LOOKBACK_BARS = 54
SESSION_EXTREME_FAILURE_ACCEPTANCE_REVERSAL_MICRO_BARS = 16
MIN_FAILED_EXTREME_EXTENSION = 0.02
MIN_REVERSAL_ACCEPTANCE_PERSISTENCE = 0.20
MIN_EXHAUSTION_WICK_PRESSURE = -0.12
MIN_SESSION_EXTREME_FAILURE_ACCEPTANCE_REVERSAL_ACTIVITY = 0.03
...
class LinearSessionExtremeFailureAcceptanceReversalModel:
    fold_id: str
    ...
    session_extreme_failure_acceptance_reversal_failure_rate: float
...
def run_c0137_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_session_extreme_failure_acceptance_reversal_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_session_extreme_failure_acceptance_reversal_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        SESSION_EXTREME_FAILURE_ACCEPTANCE_REVERSAL_LOOKBACK_BARS,
        SESSION_EXTREME_FAILURE_ACCEPTANCE_REVERSAL_MICRO_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = session_extreme_failure_acceptance_reversal_metrics(
    ...
    features = (
        metrics["failed_extreme_extension"],
        metrics["reclaim_depth"],
        metrics["reversal_acceptance_persistence"],
        metrics["opposite_side_close_efficiency"],
        metrics["exhaustion_wick_pressure"],
        metrics["range_reentry_quality"],
        metrics["micro_rotation_reversal"],
        metrics["session_room_to_mid"],
        metrics["volume_exhaustion_reengagement"],
        metrics["prior_day_contrast"],
        metrics["session_maturity"],
        metrics["spread_stress_inverse"],
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        extension_pressure_bucket(metrics["failed_extreme_extension"]),
        c0137_reclaim_bucket(metrics["reclaim_depth"]),
        retention_bucket(metrics["reversal_acceptance_persistence"]),
        close_location_confirm_bucket(metrics["opposite_side_close_efficiency"]),
        c0137_tail_rejection_bucket(metrics["exhaustion_wick_pressure"]),
        session_reversion_bucket(metrics["range_reentry_quality"]),
        reversal_commitment_bucket(metrics["micro_rotation_reversal"]),
        session_pressure_bucket(metrics["session_room_to_mid"]),
        c0137_volume_exhaustion_bucket(metrics["volume_exhaustion_reengagement"]),
        prior_day_contrarian_bucket(metrics["prior_day_contrast"]),
...
def session_extreme_failure_acceptance_reversal_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    micro_start = max(index - SESSION_EXTREME_FAILURE_ACCEPTANCE_REVERSAL_MICRO_BARS + 1, context.day_start_index)
    lookback_start = max(index - SESSION_EXTREME_FAILURE_ACCEPTANCE_REVERSAL_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    session_mid = (context.high_so_far + context.low_so_far) * 0.5

    if direction > 0:
        failed_extreme_extension = (reference_low - micro_low) / max(average_range, 1e-9)
        reclaim_depth = (signal_bar.close - reference_low) / max(average_range, 1e-9)
        session_room_to_mid = (session_mid - signal_bar.close) / max(average_range, 1e-9)
        reentry_price = reference_low
        accepted_closes = sum(1 for bar in micro_window[-5:] if bar.close >= reentry_price)
    else:
        failed_extreme_extension = (micro_high - reference_high) / max(average_range, 1e-9)
        reclaim_depth = (reference_high - signal_bar.close) / max(average_range, 1e-9)
        session_room_to_mid = (signal_bar.close - session_mid) / max(average_range, 1e-9)
        reentry_price = reference_high
        accepted_closes = sum(1 for bar in micro_window[-5:] if bar.close <= reentry_price)

    if failed_extreme_extension < MIN_FAILED_EXTREME_EXTENSION:
        return {"event_count": 0.0}

    reversal_acceptance_persistence = accepted_closes / min(len(micro_window[-5:]), 5)
    opposite_side_close_efficiency = directional_bar_settlement(signal_bar, direction)
    exhaustion_wick_pressure = directional_wick_rejection(signal_bar, direction)
    range_reentry_quality = directional_session_percentile_position(
        signal_bar.close,
        context.low_so_far,
        context.high_so_far,
        direction,
    ...
    micro_rotation_reversal = (
        (micro_net / max(average_range, 1e-9)) * 0.45
        + direction * (current_delta - previous_delta) / max(average_range, 1e-9) * 0.55
    )
    if reversal_acceptance_persistence < MIN_REVERSAL_ACCEPTANCE_PERSISTENCE and reclaim_depth < -0.05:
        return {"event_count": 0.0}
    if exhaustion_wick_pressure < MIN_EXHAUSTION_WICK_PRESSURE and opposite_side_close_efficiency < 0.42:
        return {"event_count": 0.0}
    ...
    volume_exhaustion_reengagement = current_volume_ratio - (micro_volume_ratio * 0.35) - 0.65
    ...
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_day_contrast = -direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
    if context.prior_day_shadow_balance is not None:
        prior_day_contrast += direction * context.prior_day_shadow_balance * 0.04
    ...
    activity = (
        max(failed_extreme_extension, -0.05) * 0.18
        + max(reclaim_depth, -0.20) * 0.14
        + max(reversal_acceptance_persistence - 0.20, -0.20) * 0.14
        + max(opposite_side_close_efficiency - 0.35, -0.25) * 0.12
        + max(exhaustion_wick_pressure, -0.20) * 0.10
        + max(range_reentry_quality - 0.20, -0.20) * 0.08
        + max(micro_rotation_reversal, -0.16) * 0.10
        + max(session_room_to_mid, -0.20) * 0.06
        + max(min(volume_exhaustion_reengagement, 1.50), -0.75) * 0.04
        + max(prior_day_contrast, -0.30) * 0.04
    )
    if activity < MIN_SESSION_EXTREME_FAILURE_ACCEPTANCE_REVERSAL_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "failed_extreme_extension": bounded(failed_extreme_extension, -2.0, 8.0),
        "reclaim_depth": bounded(reclaim_depth, -4.0, 6.0),
        "reversal_acceptance_persistence": bounded(reversal_acceptance_persistence, 0.0, 1.0),
        "opposite_side_close_efficiency": bounded(opposite_side_close_efficiency, 0.0, 1.0),
        "exhaustion_wick_pressure": bounded(exhaustion_wick_pressure, -1.0, 1.0),
        "range_reentry_quality": bounded(range_reentry_quality, 0.0, 1.0),
        "micro_rotation_reversal": bounded(micro_rotation_reversal, -4.0, 4.0),
        "session_room_to_mid": bounded(session_room_to_mid, -3.0, 8.0),
        "volume_exhaustion_reengagement": bounded(volume_exhaustion_reengagement, -6.0, 8.0),
        "prior_day_contrast": bounded(prior_day_contrast, -3.0, 3.0),
        ...
        "reference_level": rounded(reentry_price) or 0.0,
        "micro_net": bounded(micro_net / max(average_range, 1e-9), -4.0, 4.0),
        "session_extreme_failure_acceptance_reversal_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0137_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0137_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0137_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0137_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0137_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0137_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0137_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0137_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0137_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0137_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0137_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0137_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0137_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0137_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0137_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0137_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0137_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0137_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0137_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def c0137_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    extension_reward = 0.12 if metrics["failed_extreme_extension"] > 0.08 and terminal > 0 else 0.0
    reclaim_reward = 0.12 if metrics["reclaim_depth"] > 0.00 and terminal > 0 else 0.0
    acceptance_reward = 0.12 if metrics["reversal_acceptance_persistence"] >= 0.40 and terminal > 0 else 0.0
    close_reward = 0.10 if metrics["opposite_side_close_efficiency"] > 0.52 and terminal > 0 else 0.0
    wick_reward = 0.08 if metrics["exhaustion_wick_pressure"] > 0.05 and terminal > 0 else 0.0
    reentry_reward = 0.08 if metrics["range_reentry_quality"] > 0.28 and terminal > 0 else 0.0
    rotation_reward = 0.10 if metrics["micro_rotation_reversal"] > 0.02 and terminal > 0 else 0.0
    room_reward = 0.05 if metrics["session_room_to_mid"] > 0.20 and terminal > 0 else 0.0
    volume_reward = 0.04 if metrics["volume_exhaustion_reengagement"] > -0.45 and terminal > 0 else 0.0
    prior_day_reward = 0.04 if metrics["prior_day_contrast"] > -0.35 and terminal > 0 else 0.0
    weak_extension_penalty = 0.08 if metrics["failed_extreme_extension"] < 0.02 and terminal <= 0 else 0.0
    failed_reclaim_penalty = 0.10 if metrics["reclaim_depth"] < -0.08 and terminal <= 0 else 0.0
    weak_acceptance_penalty = 0.09 if metrics["reversal_acceptance_persistence"] < 0.20 and terminal <= 0 else 0.0
    weak_close_penalty = 0.08 if metrics["opposite_side_close_efficiency"] < 0.40 and terminal <= 0 else 0.0
    weak_wick_penalty = 0.06 if metrics["exhaustion_wick_pressure"] < -0.10 and terminal <= 0 else 0.0
    weak_rotation_penalty = 0.08 if metrics["micro_rotation_reversal"] < -0.08 and terminal <= 0 else 0.0
    no_room_penalty = 0.05 if metrics["session_room_to_mid"] < 0.05 and terminal <= 0 else 0.0
    volume_drought_penalty = 0.05 if metrics["volume_exhaustion_reengagement"] < -0.85 and terminal <= 0 else 0.0
    ...
    return (
        ...
        - weak_acceptance_penalty
        - weak_close_penalty
        - weak_wick_penalty
        - weak_rotation_penalty
        - no_room_penalty
        ...
        - max(0.0, -metrics["volume_exhaustion_reengagement"] - 0.75) * 0.015
        - max(0.0, MIN_FAILED_EXTREME_EXTENSION - metrics["failed_extreme_extension"]) * 0.08
        - max(0.0, MIN_REVERSAL_ACCEPTANCE_PERSISTENCE - metrics["reversal_acceptance_persistence"]) * 0.06
    )


def fit_linear_session_extreme_failure_acceptance_reversal_model(candidates: list[base.Candidate], fold_id: str) -> LinearSessionExtremeFailureAcceptanceReversalModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearSessionExtremeFailureAcceptanceReversalModel(
            fold_id,
    ...
    session_extreme_failure_acceptance_reversal_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearSessionExtremeFailureAcceptanceReversalModel(
        fold_id=fold_id,
        ...
        session_extreme_failure_acceptance_reversal_failure_rate=session_extreme_failure_acceptance_reversal_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearSessionExtremeFailureAcceptanceReversalModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_session_extreme_failure_acceptance_reversal_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0137_r0001_intraday_session_extreme_failure_acceptance_reversal",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0137_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0137_r0001_session_extreme_failure_acceptance_reversal_summary.json",
        ],
        ...
        "conditional_profiles": {
            "session_extreme_failure_acceptance_reversal_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0137 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0137_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0137 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "session_extreme_failure_acceptance_reversal_lookback_bars": SESSION_EXTREME_FAILURE_ACCEPTANCE_REVERSAL_LOOKBACK_BARS,
        "session_extreme_failure_acceptance_reversal_micro_bars": SESSION_EXTREME_FAILURE_ACCEPTANCE_REVERSAL_MICRO_BARS,
        "min_failed_extreme_extension": MIN_FAILED_EXTREME_EXTENSION,
        "min_reversal_acceptance_persistence": MIN_REVERSAL_ACCEPTANCE_PERSISTENCE,
        "min_exhaustion_wick_pressure": MIN_EXHAUSTION_WICK_PRESSURE,
        "min_session_extreme_failure_acceptance_reversal_activity": MIN_SESSION_EXTREME_FAILURE_ACCEPTANCE_REVERSAL_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_session_extreme_failure_acceptance_reversal_model_summary(model: LinearSessionExtremeFailureAcceptanceReversalModel) -> dict[str, object]:
    return {
        ...
        "session_extreme_failure_acceptance_reversal_failure_rate": rounded(model.session_extreme_failure_acceptance_reversal_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearSessionExtremeFailureAcceptanceReversalModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_session_extreme_failure_acceptance_reversal_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "session_extreme_failure_acceptance_reversal_profile": profiles["session_extreme_failure_acceptance_reversal_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0137_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0137_r0001_proxy_trades.csv"
    evidence["session_extreme_failure_acceptance_reversal_summary"] = "artifacts/c0137_r0001_session_extreme_failure_acceptance_reversal_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0137_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0137_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0137 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0137_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    next_work["tasks"] = ["produce_c0137_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0137_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0137_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0137_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0137_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0137_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0137_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0137 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0137 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    block = f"""
- decision_id: dec_20260708_produce_c0137_r0001_proxy_evidence
  created_local_date: '{local_date}'
  status: active
  decision: produce_c0137_r0001_proxy_evidence
  refines:
  - dec_20260708_open_c0137_r0001_intraday_session_extreme_failure_acceptance_reversal_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0137_r0001_proxy_records_session_extreme_failure_acceptance_reversal_reference_surface
  ...
  - next_work_is_produce_c0137_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260708_produce_c0137_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0137-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0137_r0001_intraday_session_extreme_failure_acceptance_reversal",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0137-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0137-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0137-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0137_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0137_r0001_intraday_session_extreme_failure_acceptance_reversal",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0137-R0001-SWING-PIVOT-RETEST-CONTINUATION-SUMMARY",
            "artifact_role": "session_extreme_failure_acceptance_reversal_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0137_r0001_session_extreme_failure_acceptance_reversal_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0137_r0001_intraday_session_extreme_failure_acceptance_reversal",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0137_r0001_mt5_logic_parity_evidence",
        }
    ]
