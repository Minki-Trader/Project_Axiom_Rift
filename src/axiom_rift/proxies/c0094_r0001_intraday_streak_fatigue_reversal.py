"""C0094 R0001 proxy evidence for intraday streak fatigue reversal."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0094_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0094_r0001_streak_fatigue_reversal_summary.json"
...
FEATURE_NAMES = (
    "streak_length_pressure",
    "streak_displacement_stretch",
    "body_effort_decay",
    "close_progression_erosion",
    "counter_wick_rejection",
    "participation_fade",
    "path_efficiency_loss",
    "vwap_extension_pressure",
    "reversal_commitment",
    "session_phase_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_streak_fatigue_reversal_rank"
LABEL_SHAPE = "target_first_reversal_quality_conditioned_on_streak_fatigue_reversal"
SELECTION_RULE = "top_fold_local_streak_fatigue_reversal_scores_per_active_day"
...
MIN_DAY_BARS = 30
STREAK_FATIGUE_REVERSAL_LOOKBACK_BARS = 18
STREAK_FATIGUE_REVERSAL_SHORT_LOOKBACK_BARS = 6
MIN_STREAK_FATIGUE_REVERSAL_ACTIVITY = 0.10
...
class LinearStreakFatigueReversalModel:
    fold_id: str
    ...
    streak_fatigue_reversal_failure_rate: float
...
def run_c0094_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_streak_fatigue_reversal_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_streak_fatigue_reversal_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        STREAK_FATIGUE_REVERSAL_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = streak_fatigue_reversal_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        metrics["streak_length_pressure"],
        metrics["streak_displacement_stretch"],
        metrics["body_effort_decay"],
        metrics["close_progression_erosion"],
        metrics["counter_wick_rejection"],
        metrics["participation_fade"],
        metrics["path_efficiency_loss"],
        metrics["vwap_extension_pressure"],
        metrics["reversal_commitment"],
        metrics["session_phase_maturity"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        streak_length_pressure_bucket(metrics["streak_length_pressure"]),
        streak_displacement_stretch_bucket(metrics["streak_displacement_stretch"]),
        body_effort_decay_streak_bucket(metrics["body_effort_decay"]),
        close_progression_erosion_bucket(metrics["close_progression_erosion"]),
        counter_wick_rejection_streak_bucket(metrics["counter_wick_rejection"]),
        participation_fade_streak_bucket(metrics["participation_fade"]),
        path_efficiency_loss_streak_bucket(metrics["path_efficiency_loss"]),
        vwap_extension_pressure_bucket(metrics["vwap_extension_pressure"]),
        reversal_commitment_bucket(metrics["reversal_commitment"]),
...
def streak_fatigue_reversal_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    short_start = index - STREAK_FATIGUE_REVERSAL_SHORT_LOOKBACK_BARS + 1
    prior_start = index - STREAK_FATIGUE_REVERSAL_LOOKBACK_BARS + 1
    if short_start < 0 or prior_start < 0 or context.day_start_index >= index:
        return {"event_count": 0.0}
    prior_short_start = short_start - STREAK_FATIGUE_REVERSAL_SHORT_LOOKBACK_BARS
    ...
    exhaustion_streak_count = 0
    for pos in range(index - 1, max(context.day_start_index, index - 12) - 1, -1):
        body = exhaustion_direction * (bars[pos].close - bars[pos].open)
        close_step = exhaustion_direction * (bars[pos].close - bars[pos - 1].close) if pos > 0 else 0.0
        if body > -0.08 * average_range or close_step > 0.02 * average_range:
            exhaustion_streak_count += 1
            continue
        break

    lookback_start = max(context.day_start_index, index - STREAK_FATIGUE_REVERSAL_LOOKBACK_BARS)
    recent_start = max(context.day_start_index, index - STREAK_FATIGUE_REVERSAL_SHORT_LOOKBACK_BARS)
    lookback_window = bars[lookback_start:index]
    recent_window = bars[recent_start:index]
    if len(lookback_window) < 8 or len(recent_window) < 3:
        return {"event_count": 0.0}

    exhaustion_anchor = lookback_window[0].open
    exhaustion_displacement = exhaustion_direction * (bars[index - 1].close - exhaustion_anchor)
    streak_displacement_stretch = exhaustion_displacement / max(average_range, 1e-9)
    streak_length_pressure = exhaustion_streak_count / max(float(STREAK_FATIGUE_REVERSAL_SHORT_LOOKBACK_BARS), 1.0)

    body_sizes = [abs(bar.close - bar.open) for bar in lookback_window]
    early_bodies = body_sizes[: max(2, len(body_sizes) // 2)]
    recent_bodies = body_sizes[-max(2, len(body_sizes) // 3) :]
    early_body = base.mean(early_bodies) or average_range
    recent_body = base.mean(recent_bodies) or 0.0
    body_effort_decay = (early_body - recent_body) / max(early_body, 1e-9)

    prior_close = bars[index - 1].close
    close_progression_erosion = reversal_direction * (signal_bar.close - prior_close) / max(average_range, 1e-9)
    counter_wick_rejection = directional_wick_rejection(signal_bar, reversal_direction)

    prior_volumes = volumes[lookback_start:recent_start]
    recent_volumes = volumes[recent_start:index]
    prior_volume_average = base.mean(prior_volumes) or average_volume
    recent_volume_average = base.mean(recent_volumes) or average_volume
    participation_fade = (prior_volume_average - recent_volume_average) / max(prior_volume_average, 1e-9)

    travel = sum(abs(bars[pos].close - bars[pos - 1].close) for pos in range(lookback_start + 1, index))
    directional_efficiency = exhaustion_displacement / max(travel, average_range * 0.25)
    path_efficiency_loss = 1.0 - max(0.0, min(1.0, directional_efficiency))

    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    vwap_extension_pressure = -reversal_direction * (signal_bar.close - session_vwap) / max(average_range, 1e-9)
    reversal_commitment = reversal_direction * (signal_bar.close - signal_bar.open) / signal_range
    session_phase_maturity = bounded_session_progress(signal_bar.time)
    fatigue_pressure_raw = max(
        max(streak_length_pressure - 0.25, 0.0),
        max(streak_displacement_stretch - 0.35, 0.0) * 0.55,
        max(body_effort_decay, 0.0),
        max(close_progression_erosion, 0.0) * 1.3,
        max(counter_wick_rejection, 0.0) * 1.2,
        max(participation_fade, 0.0) * 0.7,
        max(path_efficiency_loss - 0.25, 0.0),
        max(vwap_extension_pressure, 0.0) * 0.35,
        max(reversal_commitment, 0.0),
    )
    fatigue_pressure = session_phase_maturity * fatigue_pressure_raw
    activity = max(fatigue_pressure_raw, fatigue_pressure)
    if activity < MIN_STREAK_FATIGUE_REVERSAL_ACTIVITY:
        return {"event_count": 0.0}
    if streak_length_pressure < 0.25 and streak_displacement_stretch < 0.30:
        return {"event_count": 0.0}
    if body_effort_decay < -0.35 and participation_fade < -0.35:
        return {"event_count": 0.0}
    if close_progression_erosion < -0.20 and counter_wick_rejection < -0.12 and reversal_commitment < -0.10:
        return {"event_count": 0.0}
    return {
        "streak_length_pressure": bounded(streak_length_pressure, 0.0, 2.0),
        "streak_displacement_stretch": bounded(streak_displacement_stretch, -2.0, 6.0),
        "body_effort_decay": bounded(body_effort_decay, -2.0, 1.0),
        "close_progression_erosion": bounded(close_progression_erosion, -3.0, 3.0),
        "counter_wick_rejection": bounded(counter_wick_rejection, -1.0, 1.0),
        "participation_fade": bounded(participation_fade, -3.0, 1.0),
        "path_efficiency_loss": bounded(path_efficiency_loss, 0.0, 1.0),
        "vwap_extension_pressure": bounded(vwap_extension_pressure, -4.0, 4.0),
        "reversal_commitment": bounded(reversal_commitment, -1.0, 1.0),
        "session_phase_maturity": bounded(session_phase_maturity, 0.0, 1.0),
        "fatigue_pressure": bounded(fatigue_pressure, 0.0, 5.0),
        "event_count": 1.0,
    }
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    streak_reward = 0.10 if metrics["streak_length_pressure"] > 0.50 and terminal > 0 else 0.0
    stretch_reward = 0.12 if metrics["streak_displacement_stretch"] > 0.65 and terminal > 0 else 0.0
    effort_decay_reward = 0.12 if metrics["body_effort_decay"] > 0.12 and terminal > 0 else 0.0
    erosion_reward = 0.12 if metrics["close_progression_erosion"] > 0.12 and terminal > 0 else 0.0
    wick_reward = 0.10 if metrics["counter_wick_rejection"] > 0.16 and terminal > 0 else 0.0
    participation_reward = 0.06 if metrics["participation_fade"] > 0.08 and terminal > 0 else 0.0
    path_loss_reward = 0.08 if metrics["path_efficiency_loss"] > 0.35 and terminal > 0 else 0.0
    vwap_reward = 0.08 if metrics["vwap_extension_pressure"] > 0.30 and terminal > 0 else 0.0
    reversal_reward = 0.10 if metrics["reversal_commitment"] > 0.12 and terminal > 0 else 0.0
    continuation_streak_penalty = 0.14 if metrics["streak_displacement_stretch"] > 0.85 and terminal <= 0 else 0.0
    effort_reload_penalty = 0.12 if metrics["body_effort_decay"] < -0.10 and terminal <= 0 else 0.0
    weak_erosion_penalty = 0.12 if metrics["close_progression_erosion"] < -0.06 and terminal <= 0 else 0.0
    weak_wick_penalty = 0.10 if metrics["counter_wick_rejection"] < -0.06 and terminal <= 0 else 0.0
    participation_reload_penalty = 0.08 if metrics["participation_fade"] < -0.15 and terminal <= 0 else 0.0
    vwap_inside_penalty = 0.06 if metrics["vwap_extension_pressure"] < 0.05 and terminal <= 0 else 0.0
    reversal_against_penalty = 0.12 if metrics["reversal_commitment"] < -0.08 and terminal <= 0 else 0.0
    ...
    streak_fatigue_reversal_path_quality = (0.54 * terminal + 0.30 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + streak_fatigue_reversal_path_quality
        + streak_reward
        + stretch_reward
        + effort_decay_reward
        + erosion_reward
        ...
        + vwap_reward
        + reversal_reward
        - continuation_streak_penalty
        - effort_reload_penalty
        - weak_erosion_penalty
        - weak_wick_penalty
        - participation_reload_penalty
        - vwap_inside_penalty
        - reversal_against_penalty
        - stall_penalty
        - unfavorable_path_penalty
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["streak_displacement_stretch"] - 4.0) * 0.04
    )


def fit_linear_streak_fatigue_reversal_model(candidates: list[base.Candidate], fold_id: str) -> LinearStreakFatigueReversalModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearStreakFatigueReversalModel(
            fold_id,
    ...
    streak_fatigue_reversal_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearStreakFatigueReversalModel(
        fold_id=fold_id,
        ...
        streak_fatigue_reversal_failure_rate=streak_fatigue_reversal_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearStreakFatigueReversalModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_streak_fatigue_reversal_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0094_r0001_intraday_streak_fatigue_reversal",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0094_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0094_r0001_streak_fatigue_reversal_summary.json",
        ],
        ...
        "conditional_profiles": {
            "streak_fatigue_reversal_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0094 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0094_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0094 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "streak_fatigue_reversal_lookback_bars": STREAK_FATIGUE_REVERSAL_LOOKBACK_BARS,
        "streak_fatigue_reversal_short_lookback_bars": STREAK_FATIGUE_REVERSAL_SHORT_LOOKBACK_BARS,
        "min_streak_fatigue_reversal_activity": MIN_STREAK_FATIGUE_REVERSAL_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_streak_fatigue_reversal_model_summary(model: LinearStreakFatigueReversalModel) -> dict[str, object]:
    return {
        ...
        "streak_fatigue_reversal_failure_rate": rounded(model.streak_fatigue_reversal_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearStreakFatigueReversalModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_streak_fatigue_reversal_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "streak_fatigue_reversal_profile": profiles["streak_fatigue_reversal_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0094_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0094_r0001_proxy_trades.csv"
    evidence["streak_fatigue_reversal_summary"] = "artifacts/c0094_r0001_streak_fatigue_reversal_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0094_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0094_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0094 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0094_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0094_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0094_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0094_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0094_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0094_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0094_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0094_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0094_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0094 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0094 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0094_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0094_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0094_r0001_intraday_streak_fatigue_reversal_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0094_r0001_proxy_records_streak_fatigue_reversal_reference_surface
  ...
  - next_work_is_produce_c0094_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0094_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0094-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0094_r0001_intraday_streak_fatigue_reversal",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0094-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0094-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0094-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0094_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0094_r0001_intraday_streak_fatigue_reversal",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0094-R0001-STREAK-FATIGUE-REVERSAL-SUMMARY",
            "artifact_role": "streak_fatigue_reversal_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0094_r0001_streak_fatigue_reversal_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0094_r0001_intraday_streak_fatigue_reversal",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0094_r0001_mt5_logic_parity_evidence",
        }
    ]
