"""C0068 R0001 proxy evidence for intraday failure to accept."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0068_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0068_r0001_failure_to_accept_summary.json"
...
FEATURE_NAMES = (
    "reference_probe_distance",
    "failed_acceptance_settlement",
    "rejection_body_pressure",
    "return_to_value_pressure",
    "opposing_follow_through",
    "local_acceptance_density",
    "post_probe_stall_energy",
    "volume_rejection_confirmation",
    "prior_day_overextension_context",
    "session_maturity_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_failure_to_accept_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_failure_to_accept_state"
SELECTION_RULE = "top_fold_local_failure_to_accept_scores_per_active_day"
...
MIN_DAY_BARS = 30
FAILURE_TO_ACCEPT_LOOKBACK_BARS = 20
FAILURE_TO_ACCEPT_SHORT_LOOKBACK_BARS = 6
MIN_FAILURE_TO_ACCEPT_ACTIVITY = 0.10
...
class LinearFailureToAcceptModel:
    fold_id: str
    ...
    failure_to_accept_failure_rate: float
...
def run_c0068_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_failure_to_accept_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_failure_to_accept_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        FAILURE_TO_ACCEPT_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        fta["reference_probe_distance"],
        fta["failed_acceptance_settlement"],
        fta["rejection_body_pressure"],
        fta["return_to_value_pressure"],
        fta["opposing_follow_through"],
        fta["local_acceptance_density"],
        fta["post_probe_stall_energy"],
        fta["volume_rejection_confirmation"],
        fta["prior_day_overextension_context"],
        fta["session_maturity_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        reference_probe_bucket(fta["reference_probe_distance"]),
        failed_acceptance_bucket(fta["failed_acceptance_settlement"]),
        rejection_body_bucket(fta["rejection_body_pressure"]),
        return_value_bucket(fta["return_to_value_pressure"]),
        opposing_follow_bucket(fta["opposing_follow_through"]),
        acceptance_density_bucket(fta["local_acceptance_density"]),
        stall_energy_bucket(fta["post_probe_stall_energy"]),
        volume_rejection_bucket(fta["volume_rejection_confirmation"]),
        prior_overextension_bucket(fta["prior_day_overextension_context"]),
        session_pressure_bucket(fta["session_maturity_pressure"]),
...
def failure_to_accept_metrics(
    ...
    window_start = index - FAILURE_TO_ACCEPT_LOOKBACK_BARS
    short_start = index - FAILURE_TO_ACCEPT_SHORT_LOOKBACK_BARS + 1
    if window_start < 0 or short_start < 0:
        return {"event_count": 0.0}
    short_window = bars[short_start : index + 1]
    prior_window = bars[window_start:index]
    ...
    prior_mid = 0.5 * (prior_high + prior_low)
    signal_range = max(signal_bar.high - signal_bar.low, 1e-9)
    acceptance_hits = 0
    for item in prior_window:
        location = (item.close - prior_low) / prior_width
        if 0.18 <= location <= 0.82:
            acceptance_hits += 1
    local_acceptance_density = acceptance_hits / len(prior_window)

    if direction > 0:
        reference_probe_distance = max(0.0, prior_low - signal_bar.low) / average_range
        failed_acceptance_settlement = (signal_bar.close - prior_low) / average_range
        rejection_body_pressure = (signal_bar.close - signal_bar.open) / average_range
        return_to_value_pressure = (signal_bar.close - prior_low) / prior_width
        prior_day_direction = -1.0
    else:
        reference_probe_distance = max(0.0, signal_bar.high - prior_high) / average_range
        failed_acceptance_settlement = (prior_high - signal_bar.close) / average_range
        rejection_body_pressure = (signal_bar.open - signal_bar.close) / average_range
        return_to_value_pressure = (prior_high - signal_bar.close) / prior_width
        prior_day_direction = 1.0
    if reference_probe_distance <= 0.0 and failed_acceptance_settlement < 0.04:
        return {"event_count": 0.0}
    if failed_acceptance_settlement < -0.08:
        return {"event_count": 0.0}
    opposing_follow_through = sum(
        direction * (item.close - item.open) for item in short_window
    ) / max(average_range * len(short_window), 1e-9)
    post_probe_stall_energy = (1.0 - min(abs(signal_bar.close - signal_bar.open) / signal_range, 1.0)) * min(
        reference_probe_distance + max(failed_acceptance_settlement, 0.0),
        2.0,
    )
    ...
    volume_rejection_confirmation = (directional_volume - opposing_volume) / max(expected_recent_volume, 1e-9)
    if context.prior_day_range is None or context.prior_day_range <= 0 or context.prior_day_return is None:
        prior_day_overextension_context = 0.0
    else:
        prior_day_overextension_context = prior_day_direction * context.prior_day_return / context.prior_day_range
    boundary_distance = abs(signal_bar.close - prior_mid) / prior_width
    session_maturity_pressure = bounded_session_progress(signal_bar.time) * max(
        reference_probe_distance,
        max(failed_acceptance_settlement, 0.0),
        max(rejection_body_pressure, 0.0),
        return_to_value_pressure,
        local_acceptance_density,
        max(volume_rejection_confirmation, 0.0),
    )
    activity = max(
        reference_probe_distance,
        max(failed_acceptance_settlement, 0.0),
        max(rejection_body_pressure, 0.0),
        max(return_to_value_pressure - 0.15, 0.0),
        abs(opposing_follow_through),
        max(local_acceptance_density - 0.35, 0.0),
        post_probe_stall_energy,
        abs(volume_rejection_confirmation),
        max(prior_day_overextension_context, 0.0),
        session_maturity_pressure,
    )
    if activity < MIN_FAILURE_TO_ACCEPT_ACTIVITY:
        return {"event_count": 0.0}
    if (
        reference_probe_distance < 0.03
        and failed_acceptance_settlement < 0.04
        and rejection_body_pressure < 0.05
        and local_acceptance_density < 0.40
    ):
        return {"event_count": 0.0}
    return {
        "reference_probe_distance": bounded(reference_probe_distance, 0.0, 3.0),
        "failed_acceptance_settlement": bounded(failed_acceptance_settlement, -1.0, 3.0),
        "rejection_body_pressure": bounded(rejection_body_pressure, -4.0, 4.0),
        "return_to_value_pressure": bounded(return_to_value_pressure, -0.50, 1.50),
        "opposing_follow_through": bounded(opposing_follow_through, -4.0, 4.0),
        "local_acceptance_density": bounded(local_acceptance_density, 0.0, 1.0),
        "post_probe_stall_energy": bounded(post_probe_stall_energy, 0.0, 2.0),
        "volume_rejection_confirmation": bounded(volume_rejection_confirmation, -4.0, 4.0),
        "prior_day_overextension_context": bounded(prior_day_overextension_context, -1.0, 1.0),
        "session_maturity_pressure": bounded(session_maturity_pressure, 0.0, 4.0),
        "boundary_distance_from_value": bounded(boundary_distance, 0.0, 2.0),
        "event_count": 1.0,
    }
...
def reference_probe_bucket(value: float) -> str:
    if value < 0.04:
        return "reference_probe_thin"
    if value < 0.28:
        return "reference_probe_clear"
    return "reference_probe_extended"


def failed_acceptance_bucket(value: float) -> str:
    if value < 0.02:
        return "acceptance_failure_weak"
    if value < 0.30:
        return "acceptance_failure_returned"
    return "acceptance_failure_deep_return"


def rejection_body_bucket(value: float) -> str:
    if value < -0.12:
        return "rejection_body_against"
    if value < 0.16:
        return "rejection_body_mixed"
    return "rejection_body_with_fade"


def return_value_bucket(value: float) -> str:
    if value < 0.30:
        return "return_value_edge"
    if value < 0.72:
        return "return_value_mid"
    return "return_value_deep"


def opposing_follow_bucket(value: float) -> str:
    if value < -0.18:
        return "opposing_follow_against_fade"
    if value < 0.20:
        return "opposing_follow_mixed"
    return "opposing_follow_with_fade"


def acceptance_density_bucket(value: float) -> str:
    if value < 0.45:
        return "acceptance_density_sparse"
    if value < 0.68:
        return "acceptance_density_mixed"
    return "acceptance_density_dense"


def stall_energy_bucket(value: float) -> str:
    if value < 0.12:
        return "post_probe_stall_low"
    if value < 0.40:
        return "post_probe_stall_mid"
    return "post_probe_stall_high"


def volume_rejection_bucket(value: float) -> str:
    if value < -0.18:
        return "volume_rejection_against"
    if value < 0.22:
        return "volume_rejection_mixed"
    return "volume_rejection_with_fade"


def prior_overextension_bucket(value: float) -> str:
    if value < 0.12:
        return "prior_overextension_low"
    if value < 0.38:
        return "prior_overextension_mid"
    return "prior_overextension_high"
...
def candidate_label(
    ...
    metrics = failure_to_accept_metrics(
        bars,
        volumes,
    ...
    if metrics["event_count"] <= 0:
        return None
    probe_reward = 0.14 if metrics["reference_probe_distance"] > 0.05 and terminal > 0 else 0.0
    failed_acceptance_reward = 0.20 if metrics["failed_acceptance_settlement"] > 0.06 and terminal > 0 else 0.0
    rejection_body_reward = 0.16 if metrics["rejection_body_pressure"] > 0.10 and terminal > 0 else 0.0
    return_value_reward = 0.16 if metrics["return_to_value_pressure"] > 0.32 and terminal > 0 else 0.0
    follow_reward = 0.12 if metrics["opposing_follow_through"] > 0.08 and terminal > 0 else 0.0
    acceptance_density_reward = 0.10 if metrics["local_acceptance_density"] > 0.50 and terminal > 0 else 0.0
    stall_reward = 0.08 if metrics["post_probe_stall_energy"] > 0.14 and terminal > 0 else 0.0
    volume_reward = 0.10 if metrics["volume_rejection_confirmation"] > 0.15 and terminal > 0 else 0.0
    prior_overextension_reward = 0.06 if metrics["prior_day_overextension_context"] > 0.18 and terminal > 0 else 0.0
    accepted_breakout_penalty = (
        0.24
        if metrics["reference_probe_distance"] > 0.06 and metrics["failed_acceptance_settlement"] < 0.0 and terminal <= 0
        else 0.0
    )
    weak_rejection_penalty = 0.20 if metrics["rejection_body_pressure"] < 0.02 and terminal <= 0 else 0.0
    failed_return_penalty = 0.18 if metrics["return_to_value_pressure"] < 0.18 and terminal <= 0 else 0.0
    thin_acceptance_penalty = 0.14 if metrics["local_acceptance_density"] < 0.34 and terminal <= 0 else 0.0
    ...
    failure_to_accept_path_quality = (0.44 * terminal + 0.36 * mfe - 0.72 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + failure_to_accept_path_quality
        + probe_reward
        + failed_acceptance_reward
        + rejection_body_reward
        + return_value_reward
        + follow_reward
        + acceptance_density_reward
        + stall_reward
        + volume_reward
        + prior_overextension_reward
        - accepted_breakout_penalty
        - weak_rejection_penalty
        - failed_return_penalty
        - thin_acceptance_penalty
        - stall_penalty
...
def fit_linear_failure_to_accept_model(candidates: list[base.Candidate], fold_id: str) -> LinearFailureToAcceptModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearFailureToAcceptModel(
            fold_id,
    ...
    failure_to_accept_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearFailureToAcceptModel(
        fold_id=fold_id,
        ...
        failure_to_accept_failure_rate=failure_to_accept_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearFailureToAcceptModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_failure_to_accept_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0068_r0001_intraday_failure_to_accept",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0068_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0068_r0001_failure_to_accept_summary.json",
        ],
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0068 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0068_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0068 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "failure_to_accept_lookback_bars": FAILURE_TO_ACCEPT_LOOKBACK_BARS,
        "failure_to_accept_short_lookback_bars": FAILURE_TO_ACCEPT_SHORT_LOOKBACK_BARS,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_failure_to_accept_model_summary(model: LinearFailureToAcceptModel) -> dict[str, object]:
    return {
        ...
        "failure_to_accept_failure_rate": rounded(model.failure_to_accept_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearFailureToAcceptModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_failure_to_accept_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "failure_to_accept_profile": profiles["failure_to_accept_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0068_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0068_r0001_proxy_trades.csv"
    evidence["failure_to_accept_summary"] = "artifacts/c0068_r0001_failure_to_accept_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0068_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0068_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0068 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0068_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0068_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0068_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0068_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0068_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0068_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0068_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0068_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0068_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0068 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0068 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260706_produce_c0068_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0068_r0001_proxy_evidence
  refines:
  - dec_20260706_open_c0068_r0001_intraday_failure_to_accept_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0068_r0001_proxy_records_failure_to_accept_reference_surface
  ...
  - next_work_is_produce_c0068_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260706_produce_c0068_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0068-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0068_r0001_intraday_failure_to_accept",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0068-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0068-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0068-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0068_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0068_r0001_intraday_failure_to_accept",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0068-R0001-failure-to-accept-SUMMARY",
            "artifact_role": "failure_to_accept_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0068_r0001_failure_to_accept_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0068_r0001_intraday_failure_to_accept",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0068_r0001_mt5_logic_parity_evidence",
        }
    ]
