"""C0105 R0001 proxy evidence for intraday close pressure followthrough."""
...
CAMPAIGN_ID = "C0105"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0105_intraday_close_pressure_followthrough_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0105_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0105_r0001_close_pressure_followthrough_summary.json"
...
FEATURE_NAMES = (
    "close_pressure_imbalance",
    "range_followthrough_pressure",
    "opposing_tail_absorption",
    "open_to_close_drift_alignment",
    "compression_to_close_drive",
    "participation_followthrough",
    "local_extreme_clearance",
    "prior_day_close_pressure_context",
    "time_pressure_alignment",
    "session_phase_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_close_pressure_followthrough_rank"
LABEL_SHAPE = "target_first_quality_after_close_pressure_followthrough"
SELECTION_RULE = "top_fold_local_close_pressure_followthrough_scores_per_active_day"
...
MIN_DAY_BARS = 30
CLOSE_PRESSURE_FOLLOWTHROUGH_LOOKBACK_BARS = 60
CLOSE_PRESSURE_FOLLOWTHROUGH_RECENT_BARS = 10
CLOSE_PRESSURE_FOLLOWTHROUGH_PRIOR_BARS = 30
MIN_CLOSE_PRESSURE_FOLLOWTHROUGH_ACTIVITY = 0.07
...
class LinearClosePressureFollowthroughModel:
    fold_id: str
    ...
    close_pressure_followthrough_failure_rate: float
...
def run_c0105_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_close_pressure_followthrough_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_close_pressure_followthrough_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        CLOSE_PRESSURE_FOLLOWTHROUGH_LOOKBACK_BARS,
        CLOSE_PRESSURE_FOLLOWTHROUGH_RECENT_BARS + CLOSE_PRESSURE_FOLLOWTHROUGH_PRIOR_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = close_pressure_followthrough_metrics(
    ...
    features = (
        metrics["close_pressure_imbalance"],
        metrics["range_followthrough_pressure"],
        metrics["opposing_tail_absorption"],
        metrics["open_to_close_drift_alignment"],
        metrics["compression_to_close_drive"],
        metrics["participation_followthrough"],
        metrics["local_extreme_clearance"],
        metrics["prior_day_close_pressure_context"],
        metrics["time_pressure_alignment"],
        metrics["session_phase_maturity"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        c0105_close_pressure_bucket(metrics["close_pressure_imbalance"]),
        c0105_followthrough_bucket(metrics["range_followthrough_pressure"]),
        c0105_tail_absorption_bucket(metrics["opposing_tail_absorption"]),
        c0105_day_drift_bucket(metrics["open_to_close_drift_alignment"]),
        c0105_compression_drive_bucket(metrics["compression_to_close_drive"]),
        c0105_participation_followthrough_bucket(metrics["participation_followthrough"]),
        c0105_extreme_clearance_bucket(metrics["local_extreme_clearance"]),
        c0105_prior_close_pressure_bucket(metrics["prior_day_close_pressure_context"]),
        c0105_time_pressure_bucket(metrics["time_pressure_alignment"]),
...
def close_pressure_followthrough_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    recent_start = max(index - CLOSE_PRESSURE_FOLLOWTHROUGH_RECENT_BARS + 1, context.day_start_index)
    prior_start = max(recent_start - CLOSE_PRESSURE_FOLLOWTHROUGH_PRIOR_BARS, context.day_start_index)
    lookback_start = max(index - CLOSE_PRESSURE_FOLLOWTHROUGH_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    if len(prior_window) < 10 or len(recent_window) < 6 or len(lookback_window) < 24:
        return {"event_count": 0.0}
    ...
    prior_return = prior_window[-1].close - prior_window[0].open
    recent_scale = max(recent_range_mean * math.sqrt(len(recent_window)), average_range * 0.25)
    close_pressure_imbalance = 2.0 * (
        base.mean([directional_bar_settlement(bar, direction) for bar in recent_window]) or 0.5
    ) - 1.0
    range_followthrough_pressure = direction * recent_return / recent_scale
    opposing_tail_absorption = base.mean([directional_wick_rejection(bar, direction) for bar in recent_window]) or 0.0
    open_to_close_drift_alignment = direction * (signal_bar.close - context.day_open) / day_range_so_far
    compression_to_close_drive = (
        prior_range_mean / max(recent_range_mean, average_range * 0.25) - 1.0
    ) * max(close_pressure_imbalance, 0.0)

    half = max(1, len(recent_window) // 2)
    early_volume_mean = base.mean(volumes[recent_start : recent_start + half]) or average_volume
    late_volume_mean = base.mean(volumes[recent_start + half : index + 1]) or average_volume
    participation_followthrough = (
        late_volume_mean / max(early_volume_mean, average_volume * 0.25) - 1.0
    ) * max(close_pressure_imbalance, 0.0)

    local_high = max(bar.high for bar in lookback_window)
    local_low = min(bar.low for bar in lookback_window)
    local_width = max(local_high - local_low, average_range * 0.25)
    if direction > 0:
        local_extreme_clearance = (signal_bar.close - local_low) / local_width * 2.0 - 1.0
    else:
        local_extreme_clearance = (local_high - signal_bar.close) / local_width * 2.0 - 1.0

    prior_day_close_pressure_context = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        shadow_balance = abs(context.prior_day_shadow_balance or 0.0)
        prior_day_close_pressure_context = (
            direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
            * max(0.0, 1.0 - shadow_balance * 0.5)
        )

    time_pressure_alignment = session_phase_maturity * (
        0.34 * close_pressure_imbalance
        + 0.26 * range_followthrough_pressure
        + 0.15 * opposing_tail_absorption
        + 0.12 * max(participation_followthrough, 0.0)
        + 0.08 * open_to_close_drift_alignment
        + 0.05 * local_extreme_clearance
    )
    activity = max(
        abs(range_followthrough_pressure),
        max(close_pressure_imbalance, 0.0) * 0.35,
        max(opposing_tail_absorption, 0.0) * 0.20,
        abs(open_to_close_drift_alignment) * 0.18,
        max(compression_to_close_drive, 0.0) * 0.16,
        max(participation_followthrough, 0.0) * 0.14,
    )
    if activity < MIN_CLOSE_PRESSURE_FOLLOWTHROUGH_ACTIVITY:
        return {"event_count": 0.0}
    if close_pressure_imbalance < 0.04 and abs(range_followthrough_pressure) < 0.06:
        return {"event_count": 0.0}
    return {
        "close_pressure_imbalance": bounded(close_pressure_imbalance, -1.0, 1.0),
        "range_followthrough_pressure": bounded(range_followthrough_pressure, -4.0, 4.0),
        "opposing_tail_absorption": bounded(opposing_tail_absorption, -1.0, 1.0),
        "open_to_close_drift_alignment": bounded(open_to_close_drift_alignment, -2.0, 2.0),
        "compression_to_close_drive": bounded(compression_to_close_drive, -4.0, 4.0),
        "participation_followthrough": bounded(participation_followthrough, -4.0, 4.0),
        "local_extreme_clearance": bounded(local_extreme_clearance, -1.5, 1.5),
        "prior_day_close_pressure_context": bounded(prior_day_close_pressure_context, -2.0, 2.0),
        "time_pressure_alignment": bounded(time_pressure_alignment, -4.0, 4.0),
        ...
        "close_pressure_followthrough_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0105_close_pressure_bucket(value: float) -> str:
    if value < -0.08:
        return "close_pressure_against"
    if value < 0.18:
        return "close_pressure_mixed"
    return "close_pressure_with"


def c0105_followthrough_bucket(value: float) -> str:
    if value < -0.08:
        return "followthrough_against"
    if value < 0.22:
        return "followthrough_mixed"
    return "followthrough_with"


def c0105_tail_absorption_bucket(value: float) -> str:
    if value < -0.08:
        return "tail_absorption_failed"
    if value < 0.12:
        return "tail_absorption_mixed"
    return "tail_absorption_supportive"


def c0105_day_drift_bucket(value: float) -> str:
    if value < -0.12:
        return "day_drift_against"
    if value < 0.25:
        return "day_drift_mixed"
    return "day_drift_with"


def c0105_compression_drive_bucket(value: float) -> str:
    if value < 0.02:
        return "compression_drive_absent"
    if value < 0.22:
        return "compression_drive_defined"
    return "compression_drive_heavy"


def c0105_participation_followthrough_bucket(value: float) -> str:
    if value < -0.06:
        return "participation_followthrough_fading"
    if value < 0.16:
        return "participation_followthrough_flat"
    return "participation_followthrough_active"


def c0105_extreme_clearance_bucket(value: float) -> str:
    if value < -0.10:
        return "extreme_clearance_against"
    if value < 0.45:
        return "extreme_clearance_middle"
    return "extreme_clearance_clear"


def c0105_prior_close_pressure_bucket(value: float) -> str:
    if value < -0.12:
        return "prior_close_pressure_against"
    if value < 0.18:
        return "prior_close_pressure_neutral"
    return "prior_close_pressure_supportive"


def c0105_time_pressure_bucket(value: float) -> str:
    if value < -0.05:
        return "time_pressure_against"
    if value < 0.18:
        return "time_pressure_mixed"
    return "time_pressure_aligned"
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    close_reward = 0.14 if metrics["close_pressure_imbalance"] > 0.18 and terminal > 0 else 0.0
    followthrough_reward = 0.15 if metrics["range_followthrough_pressure"] > 0.18 and terminal > 0 else 0.0
    tail_reward = 0.09 if metrics["opposing_tail_absorption"] > 0.08 and terminal > 0 else 0.0
    drift_reward = 0.08 if metrics["open_to_close_drift_alignment"] > 0.12 and terminal > 0 else 0.0
    compression_reward = 0.07 if metrics["compression_to_close_drive"] > 0.08 and terminal > 0 else 0.0
    participation_reward = 0.08 if metrics["participation_followthrough"] > 0.10 and terminal > 0 else 0.0
    clearance_reward = 0.06 if metrics["local_extreme_clearance"] > 0.12 and terminal > 0 else 0.0
    prior_reward = 0.05 if metrics["prior_day_close_pressure_context"] > 0.08 and terminal > 0 else 0.0
    time_reward = 0.06 if metrics["time_pressure_alignment"] > 0.08 and terminal > 0 else 0.0
    weak_close_penalty = 0.12 if metrics["close_pressure_imbalance"] < -0.08 and terminal <= 0 else 0.0
    failed_followthrough_penalty = 0.12 if metrics["range_followthrough_pressure"] < -0.08 and terminal <= 0 else 0.0
    adverse_tail_penalty = 0.09 if metrics["opposing_tail_absorption"] < -0.12 and terminal <= 0 else 0.0
    participation_drop_penalty = 0.08 if metrics["participation_followthrough"] < -0.16 and terminal <= 0 else 0.0
    overextension_penalty = 0.05 if metrics["local_extreme_clearance"] > 1.10 and terminal <= 0 else 0.0
    ...
    close_pressure_followthrough_path_quality = (0.54 * terminal + 0.30 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + close_pressure_followthrough_path_quality
        + close_reward
        ...
        + time_reward
        - weak_close_penalty
        - failed_followthrough_penalty
        - adverse_tail_penalty
        - participation_drop_penalty
...
def fit_linear_close_pressure_followthrough_model(candidates: list[base.Candidate], fold_id: str) -> LinearClosePressureFollowthroughModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearClosePressureFollowthroughModel(
            fold_id,
    ...
    close_pressure_followthrough_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearClosePressureFollowthroughModel(
        fold_id=fold_id,
        ...
        close_pressure_followthrough_failure_rate=close_pressure_followthrough_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearClosePressureFollowthroughModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0105-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_close_pressure_followthrough_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0105_r0001_intraday_close_pressure_followthrough",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0105_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0105_r0001_close_pressure_followthrough_summary.json",
        ],
        ...
        "conditional_profiles": {
            "close_pressure_followthrough_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0105 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0105_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0105 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "close_pressure_followthrough_lookback_bars": CLOSE_PRESSURE_FOLLOWTHROUGH_LOOKBACK_BARS,
        "close_pressure_followthrough_recent_bars": CLOSE_PRESSURE_FOLLOWTHROUGH_RECENT_BARS,
        "close_pressure_followthrough_prior_bars": CLOSE_PRESSURE_FOLLOWTHROUGH_PRIOR_BARS,
        "min_close_pressure_followthrough_activity": MIN_CLOSE_PRESSURE_FOLLOWTHROUGH_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_close_pressure_followthrough_model_summary(model: LinearClosePressureFollowthroughModel) -> dict[str, object]:
    return {
        ...
        "close_pressure_followthrough_failure_rate": rounded(model.close_pressure_followthrough_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearClosePressureFollowthroughModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_close_pressure_followthrough_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "close_pressure_followthrough_profile": profiles["close_pressure_followthrough_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0105_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0105_r0001_proxy_trades.csv"
    evidence["close_pressure_followthrough_summary"] = "artifacts/c0105_r0001_close_pressure_followthrough_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0105_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0105_r0001_proxy_trades.csv",
        "artifacts/c0105_r0001_close_pressure_followthrough_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0105_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0105 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0105_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0105_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0105_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0105_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0105_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0105_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0105_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0105_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0105_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0105 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0105 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0105_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0105_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0105_r0001_intraday_close_pressure_followthrough_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0105_r0001_proxy_records_close_pressure_followthrough_reference_surface
  ...
  - next_work_is_produce_c0105_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0105_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0105-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0105_r0001_intraday_close_pressure_followthrough",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0105-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0105-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0105-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0105_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0105_r0001_intraday_close_pressure_followthrough",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0105-R0001-close-pressure-followthrough-SUMMARY",
            "artifact_role": "close_pressure_followthrough_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0105_r0001_close_pressure_followthrough_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0105_r0001_intraday_close_pressure_followthrough",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0105_r0001_mt5_logic_parity_evidence",
        }
    ]
