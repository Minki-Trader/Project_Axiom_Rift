"""R0002 proxy evidence for failed-continuation reversal."""
...
PRICE_DIGITS = 2
MAX_CLOSED_BAR_EVALUATION_GAP_SECONDS = 7200
...
def run_r0002_proxy(write: bool = True) -> dict[str, object]:
                ...
                if active is None:
                    ...
                    if state == 1:
                        ...
                        if stateful_pullback_confirmed(
                            ...
                            pending_direction = reversal_direction(direction)
...
def reversal_direction(expansion_direction_value: int) -> int:
    if expansion_direction_value > 0:
        return -1
    if expansion_direction_value < 0:
        return 1
    return 0
...
def build_proxy_payload(trades: list[Trade], windows: list[Window]) -> dict[str, object]:
    ...
    return {
        ...
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.r0002_failed_continuation_reversal",
        "proxy_config_path": "campaigns/C0001_regime_response_discovery/runs/R0002/run_manifest.json",
        ...
        "deferred_with_reason": [
            {
                ...
                "reason": "R0002 proxy currently measures raw US100 index points only; USD conversion requires MT5 symbol contract, tick value, and fixed-lot discovery sizing",
                "blocking_condition": "record the matching MT5 symbol specification and fixed-lot sizing for the 500 USD account",
                "revisit_when": "during R0002 MT5 probe evidence production",
            ...
            },
            {
                ...
                "revisit_when": "before R0002 closeout",
                "claim_boundary": {"claim_authority": False},
            }
        ],
        "claim_boundary": {
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "response_mode": "failed_continuation_reversal",
        "trade_direction_policy": "reverse_expansion_direction_after_pullback_confirmation",
    }
...
def update_artifact_lineage(proxy_hash: str) -> None:
    ...
    for record in data.get("artifact_records", []):
        if record.get("artifact_id") == "A0001":
            ...
            record["produced_by"] = "axiom_rift.proxies.r0002_failed_continuation_reversal"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_matching_mt5_probe_for_R0002"
    data["deferred_with_reason"] = [
        {
            "field": "decision",
            "reason": "proxy evidence is recorded but MT5 and proxy-vs-MT5 evidence are still missing",
            "blocking_condition": "R0002 cannot close until the mandatory MT5 probe and proxy-vs-MT5 comparison are recorded",
            "revisit_when": "after R0002 MT5 and proxy-vs-MT5 evidence files contain measured results",
        }
    ]
