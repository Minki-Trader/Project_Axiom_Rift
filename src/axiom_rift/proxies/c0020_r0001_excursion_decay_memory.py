"""C0020 R0001 proxy evidence for excursion decay memory discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0020_excursion_decay_memory_discovery" / "runs" / "R0001"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0020_excursion_decay_memory_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0020_r0001_proxy_trades.csv"
EXCURSION_DECAY_MEMORY_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0020_r0001_excursion_decay_memory_summary.json"
...
FEATURE_NAMES = (
    "directional_return_1_over_range",
    "directional_return_3_over_range",
    "directional_return_6_over_range",
    "directional_return_12_over_range",
    "acceleration_3_vs_12",
    "acceleration_6_vs_24",
    "impulse_density_6",
    "impulse_density_12",
    "excursion_from_opposite_24_extreme",
    "distance_to_directional_24_extreme",
    "distance_to_directional_72_extreme",
    "time_since_directional_48_extreme",
    "pullback_fraction_from_24_extreme",
    "adverse_retrace_3_over_range",
    "adverse_retrace_6_over_range",
    "favorable_extension_3_over_range",
    "favorable_extension_6_over_range",
    "realized_volatility_12_over_96",
    "range_shock_3_over_48",
    "range_decay_12_vs_48",
    "close_efficiency_6",
    "close_efficiency_12",
    "path_chop_ratio_12",
    "path_chop_ratio_24",
    "day_excursion_position",
    "spread_over_range",
    "spread_pressure_48",
...
MODEL_FAMILY = "fold_local_excursion_decay_memory_rank"
LABEL_SHAPE = "directional_excursion_decay_memory_followthrough_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_excursion_decay_memory_scores_per_active_day"
...
def run_c0020_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = excursion_decay_memory_features(
                bars,
                range_average,
                short_range_average,
                day_context,
            ...
            if features is None:
                continue
            label = (
                excursion_decay_memory_label(bars, range_average, index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|excursion_decay_memory",
                    features=features,
                    label=label,
                )
            )
...
def excursion_decay_memory_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    return_1 = direction * (bar.close - bars[index - 1].close) / average_range
    return_3 = direction * (bar.close - bars[index - 3].close) / average_range
    return_6 = direction * (bar.close - bars[index - 6].close) / average_range
    return_12 = direction * (bar.close - bars[index - 12].close) / average_range
    acceleration_3_vs_12 = return_3 - return_12 / 4.0
    return_24 = direction * (bar.close - bars[index - 24].close) / average_range
    acceleration_6_vs_24 = return_6 - return_24 / 4.0
    impulse_density_6 = directional_impulse_density(bars, index, direction, 6)
    impulse_density_12 = directional_impulse_density(bars, index, direction, 12)
    low_24 = min(bars[offset].low for offset in range(index - 23, index + 1))
    high_24 = max(bars[offset].high for offset in range(index - 23, index + 1))
    low_72 = min(bars[offset].low for offset in range(index - 71, index + 1))
    high_72 = max(bars[offset].high for offset in range(index - 71, index + 1))
    if direction > 0:
        excursion_from_opposite = (bar.close - low_24) / average_range
        distance_to_24_extreme = (high_24 - bar.close) / average_range
        distance_to_72_extreme = (high_72 - bar.close) / average_range
        time_since_extreme = bars_since_extreme(bars, index, 48, high=True)
    else:
        excursion_from_opposite = (high_24 - bar.close) / average_range
        distance_to_24_extreme = (bar.close - low_24) / average_range
        distance_to_72_extreme = (bar.close - low_72) / average_range
        time_since_extreme = bars_since_extreme(bars, index, 48, high=False)
    excursion_span_24 = max((high_24 - low_24) / average_range, 1e-9)
    pullback_fraction = bounded(distance_to_24_extreme / excursion_span_24, 0.0, 3.0)
    adverse_retrace_3 = max(0.0, -direction * (bar.close - bars[index - 3].close)) / average_range
    adverse_retrace_6 = max(0.0, -direction * (bar.close - bars[index - 6].close)) / average_range
    favorable_extension_3 = max(0.0, direction * (bar.close - bars[index - 3].close)) / average_range
    favorable_extension_6 = max(0.0, direction * (bar.close - bars[index - 6].close)) / average_range
    realized_volatility_12 = close_return_volatility(bars, index, 12)
    realized_volatility_96 = close_return_volatility(bars, index, 96)
    realized_volatility_ratio = realized_volatility_12 / max(realized_volatility_96, 1e-9)
    range_shock = range_3 / max(range_48, 1e-9)
    range_decay = range_12 / max(range_48, 1e-9)
    close_efficiency_6 = directional_close_efficiency(bars, index, direction, 6)
    close_efficiency_12 = directional_close_efficiency(bars, index, direction, 12)
    path_chop_12 = path_chop_ratio(bars, index, 12)
    path_chop_24 = path_chop_ratio(bars, index, 24)
    day_span = max(context["day_high"] - context["day_low"], 1e-9)
    day_excursion_position = (
        (bar.close - context["day_low"]) / day_span
        if direction > 0
        else (context["day_high"] - bar.close) / day_span
    )
    ...
    core_start = 8 * 60
    core_end = 21 * 60
    session_progress = bounded((base.minute_of_day(bar.time) - core_start) / max(core_end - core_start, 1), 0.0, 1.0)
    ...
    if spread_over_range > 0.65 or spread_pressure > 2.20 or range_48 <= 0 or range_96 <= 0:
        return None
    return (
        return_1,
        return_3,
        ...
        impulse_density_6,
        impulse_density_12,
        excursion_from_opposite,
        distance_to_24_extreme,
        distance_to_72_extreme,
        time_since_extreme / 48.0,
        pullback_fraction,
        adverse_retrace_3,
        adverse_retrace_6,
        favorable_extension_3,
        favorable_extension_6,
...
def directional_impulse_density(bars: list[base.Bar], index: int, direction: int, lookback: int) -> float:
    favorable = 0.0
    absolute = 0.0
    for offset in range(index - lookback + 1, index + 1):
        delta = bars[offset].close - bars[offset - 1].close
        favorable += max(direction * delta, 0.0)
        absolute += abs(delta)
    return favorable / max(absolute, 1e-9)


def bars_since_extreme(bars: list[base.Bar], index: int, lookback: int, high: bool) -> float:
    best_offset = index
    best_value = bars[index].high if high else bars[index].low
    for offset in range(index - lookback + 1, index + 1):
        value = bars[offset].high if high else bars[offset].low
        if (high and value >= best_value) or ((not high) and value <= best_value):
            best_value = value
            best_offset = offset
    return float(index - best_offset)


def close_return_volatility(bars: list[base.Bar], index: int, lookback: int) -> float:
    returns = [bars[offset].close - bars[offset - 1].close for offset in range(index - lookback + 1, index + 1)]
    if not returns:
        return 0.0
    mean = sum(returns) / len(returns)
    variance = sum((value - mean) * (value - mean) for value in returns) / len(returns)
    return math.sqrt(max(variance, 0.0))


def directional_close_efficiency(bars: list[base.Bar], index: int, direction: int, lookback: int) -> float:
...
def path_chop_ratio(bars: list[base.Bar], index: int, lookback: int) -> float:
    path = sum(abs(bars[offset].close - bars[offset - 1].close) for offset in range(index - lookback + 1, index + 1))
    span = max(
...
def body_fraction_at(bar: base.Bar) -> float:
    bar_range = max(bar.high - bar.low, 1e-9)
    return (bar.close - bar.open) / bar_range


def wick_fraction_at(bar: base.Bar, direction: int, favorable: bool) -> float:
    bar_range = max(bar.high - bar.low, 1e-9)
    lower_wick = max(min(bar.open, bar.close) - bar.low, 0.0)
    upper_wick = max(bar.high - max(bar.open, bar.close), 0.0)
    if (direction > 0 and favorable) or (direction < 0 and not favorable):
        return lower_wick / bar_range
    return upper_wick / bar_range


def directional_close_location_at(bar: base.Bar, direction: int) -> float:
    bar_range = max(bar.high - bar.low, 1e-9)
    close_location = (bar.close - bar.low) / bar_range
    return close_location if direction > 0 else 1.0 - close_location


def directional_body_sum(bars: list[base.Bar], index: int, direction: int, lookback: int) -> float:
    return sum(direction * body_fraction_at(bars[offset]) for offset in range(index - lookback + 1, index + 1))


def wick_cluster_average(
    bars: list[base.Bar],
    index: int,
    direction: int,
    lookback: int,
    ...
    return sum(wick_fraction_at(bars[offset], direction, favorable) for offset in range(index - lookback + 1, index + 1)) / lookback


def rejection_count(bars: list[base.Bar], index: int, direction: int, lookback: int) -> float:
    count = 0
    for offset in range(index - lookback + 1, index + 1):
        favorable = wick_fraction_at(bars[offset], direction, favorable=True)
        adverse = wick_fraction_at(bars[offset], direction, favorable=False)
        if favorable > adverse and directional_close_location_at(bars[offset], direction) >= 0.45:
            count += 1
    return count / lookback


def acceptance_count(bars: list[base.Bar], index: int, direction: int, lookback: int) -> float:
    count = 0
    for offset in range(index - lookback + 1, index + 1):
        bar = bars[offset]
        if direction * (bar.close - bar.open) > 0 and directional_close_location_at(bar, direction) >= 0.55:
            count += 1
    return count / lookback


def alternating_body_count(bars: list[base.Bar], index: int, lookback: int) -> float:
    signs: list[int] = []
    for offset in range(index - lookback + 1, index + 1):
        body = bars[offset].close - bars[offset].open
        signs.append(1 if body > 0 else -1 if body < 0 else 0)
    changes = sum(1 for offset in range(1, len(signs)) if signs[offset] and signs[offset - 1] and signs[offset] != signs[offset - 1])
    return changes / max(lookback - 1, 1)


def close_flip_count(bars: list[base.Bar], index: int, lookback: int) -> float:
    signs: list[int] = []
    for offset in range(index - lookback + 2, index + 1):
        close_delta = bars[offset].close - bars[offset - 1].close
        signs.append(1 if close_delta > 0 else -1 if close_delta < 0 else 0)
    changes = sum(1 for offset in range(1, len(signs)) if signs[offset] and signs[offset - 1] and signs[offset] != signs[offset - 1])
    return changes / max(len(signs) - 1, 1)


def excursion_decay_memory_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    index: int,
    direction: int,
    ...
    return (
        ...
        - 0.34 * giveback
        + 0.42 * path_alignment_edge
        - 0.28 * adverse_close_flips
        - 1.08 * spread_norm
        - 0.20 * spread_drift
    )
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0020"
    payload["work_unit_id"] = "C0020"
    payload["run_id"] = "R0001"
    payload["proxy_id"] = "PX-C0020-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0020_r0001_excursion_decay_memory"
    payload["proxy_config_path"] = "campaigns/C0020_excursion_decay_memory_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_excursion_decay_memory_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0020_excursion_decay_memory_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0020_excursion_decay_memory_discovery/runs/R0001/artifacts/c0020_r0001_proxy_trades.csv",
        "campaigns/C0020_excursion_decay_memory_discovery/runs/R0001/artifacts/c0020_r0001_excursion_decay_memory_summary.json",
    ]
    ...
    profiles["excursion_decay_memory_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0020_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "feature_weight_floor": FEATURE_WEIGHT_FLOOR,
            "feature_weight_ceiling": FEATURE_WEIGHT_CEILING,
            "score_interpretation": "higher_score_means_direct_fold_local_excursion_decay_memory_rank",
            "variant_boundary": "excursion_decay_memory_rank_not_auction_threshold_monthly_memory_lifecycle_structural_trap_score_floor_stop_target_hold_or_retry_nudge",
        }
    )
    return config
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_excursion_decay_memory_summary_artifact(payload, EXCURSION_DECAY_MEMORY_SUMMARY_PATH)
    PROXY_PATH.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="ascii")
    trade_hash = base.sha256_file(TRADE_ARTIFACT_PATH)
    summary_hash = base.sha256_file(EXCURSION_DECAY_MEMORY_SUMMARY_PATH)
...
def write_excursion_decay_memory_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_excursion_decay_memory_summary_v1",
        "template": False,
        "work_unit_id": "C0020",
        "campaign_id": "C0020",
        "run_id": "R0001",
        ...
        "excursion_decay_memory_profile": profiles["excursion_decay_memory_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "excursion_decay_memory_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0020-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0020_excursion_decay_memory_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0020_excursion_decay_memory_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0020-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0020-R0001-EXCURSION-DECAY-MEMORY-SUMMARY",
                "excursion_decay_memory_summary_artifact",
                "json",
                "campaigns/C0020_excursion_decay_memory_discovery/runs/R0001/artifacts/c0020_r0001_excursion_decay_memory_summary.json",
                summary_hash,
                ["campaigns/C0020_excursion_decay_memory_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0020_r0001_excursion_decay_memory",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0020_r0001_proxy_trades.csv"
    evidence["excursion_decay_memory_summary"] = "artifacts/c0020_r0001_excursion_decay_memory_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0020_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0020_r0001_proxy_trades.csv",
        "artifacts/c0020_r0001_excursion_decay_memory_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0020 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0020_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0020_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0020_excursion_decay_memory_discovery"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0020_excursion_decay_memory_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0020_excursion_decay_memory_discovery",
        "open_c0020_r0001_fold_local_excursion_decay_memory_rank_run",
        "produce_c0020_r0001_proxy_evidence",
    ):
    ...
    data["active_run"] = "campaigns/C0020_excursion_decay_memory_discovery/runs/R0001"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["active_campaign"] = "campaigns/C0020_excursion_decay_memory_discovery"
    data["active_run"] = "campaigns/C0020_excursion_decay_memory_discovery/runs/R0001"
    data["latest_operation"] = {
        "id": "produce_c0020_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0020_excursion_decay_memory_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0020_r0001_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0020",
            "c0004_r0001_fold_local_state_archetype": "c0020_r0001_excursion_decay_memory",
            "c0004_r0001": "c0020_r0001",
            "fold_local_state_archetype_discovery": "excursion_decay_memory_discovery",
            ...
            "state_archetype_summary": "excursion_decay_memory_summary",
        }
