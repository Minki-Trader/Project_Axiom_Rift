"""C0135 R0001 proxy evidence for intraday vwap acceptance continuation."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0135_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0135_r0001_vwap_acceptance_continuation_summary.json"
...
FEATURE_NAMES = (
    ...
    "directional_close_efficiency",
    "pullback_containment",
    "acceptance_reacceleration",
    "micro_range_stability",
    "session_vwap_clearance",
    "session_room",
...
MODEL_FAMILY = "fold_local_intraday_vwap_acceptance_continuation_rank"
LABEL_SHAPE = "target_first_quality_after_vwap_acceptance_continuation"
SELECTION_RULE = "top_fold_local_vwap_acceptance_continuation_scores_per_active_day"
...
MIN_DAY_BARS = 18
VWAP_ACCEPTANCE_CONTINUATION_LOOKBACK_BARS = 36
VWAP_ACCEPTANCE_CONTINUATION_MICRO_BARS = 10
MIN_VWAP_ACCEPTANCE_PERSISTENCE = 0.52
MIN_SESSION_VWAP_CLEARANCE = -0.08
MIN_VWAP_ACCEPTANCE_CONTINUATION_ACTIVITY = 0.04
...
class LinearVwapAcceptanceContinuationModel:
    fold_id: str
    ...
    vwap_acceptance_continuation_failure_rate: float
...
def run_c0135_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_vwap_acceptance_continuation_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_vwap_acceptance_continuation_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        VWAP_ACCEPTANCE_CONTINUATION_LOOKBACK_BARS,
        VWAP_ACCEPTANCE_CONTINUATION_MICRO_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = vwap_acceptance_continuation_metrics(
    ...
    features = (
        metrics["vwap_acceptance_persistence"],
        metrics["vwap_slope_alignment"],
        metrics["directional_close_efficiency"],
        metrics["pullback_containment"],
        metrics["acceptance_reacceleration"],
        metrics["micro_range_stability"],
        metrics["session_vwap_clearance"],
    ...
    dimensions = (
        vwap_reclaim_bucket(metrics["vwap_acceptance_persistence"]),
        vwap_slope_alignment_bucket(metrics["vwap_slope_alignment"]),
        close_drive_bucket(metrics["directional_close_efficiency"]),
        shallow_pullback_bucket(metrics["pullback_containment"]),
        acceleration_pressure_bucket(metrics["acceptance_reacceleration"]),
        range_balance_bucket(metrics["micro_range_stability"]),
        vwap_close_commitment_bucket(metrics["session_vwap_clearance"]),
        session_pressure_bucket(metrics["session_room"]),
        participation_bucket(metrics["volume_participation_balance"]),
...
def vwap_acceptance_continuation_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    micro_start = max(index - VWAP_ACCEPTANCE_CONTINUATION_MICRO_BARS + 1, context.day_start_index)
    lookback_start = max(index - VWAP_ACCEPTANCE_CONTINUATION_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    earlier_vwap = session_vwap_until(bars, volumes, context.day_start_index, lookback_start)
    close_clearances = [
        direction * (bar.close - session_vwap) / max(average_range, 1e-9)
        for bar in lookback_window
    ]
    micro_clearances = close_clearances[-len(micro_window) :]
    vwap_acceptance_persistence = sum(1 for value in close_clearances if value > -0.02) / len(close_clearances)
    micro_acceptance_persistence = sum(1 for value in micro_clearances if value > -0.02) / len(micro_clearances)
    session_vwap_clearance = direction * (signal_bar.close - session_vwap) / max(average_range * 2.0, 1e-9)
    vwap_slope_alignment = direction * (session_vwap - earlier_vwap) / max(average_range, 1e-9)
    ...
    directional_close_efficiency = directional_net / max(directional_travel, average_range, 1e-9)
    micro_net = direction * (signal_bar.close - prior_micro[0].close)
    if direction > 0:
        pullback_containment = (signal_bar.close - prior_micro_low) / prior_micro_range
        session_room = (context.high_so_far - signal_bar.close) / max(average_range, 1e-9)
    else:
        pullback_containment = (prior_micro_high - signal_bar.close) / prior_micro_range
    ...
    acceptance_reacceleration = direction * (current_delta - previous_delta) / max(average_range, 1e-9)
    micro_range_stability = 1.0 - abs(micro_range_mean / max(lookback_range_mean, 1e-9) - 1.0)
    if vwap_acceptance_persistence < MIN_VWAP_ACCEPTANCE_PERSISTENCE:
        return {"event_count": 0.0}
    if session_vwap_clearance < MIN_SESSION_VWAP_CLEARANCE and acceptance_reacceleration < 0.02:
        return {"event_count": 0.0}
    ...
    activity = (
        max(vwap_acceptance_persistence - 0.50, 0.0) * 0.22
        + max(vwap_slope_alignment, -0.05) * 0.13
        + max(directional_close_efficiency, -0.05) * 0.14
        + max(pullback_containment, 0.0) * 0.09
        + max(acceptance_reacceleration, -0.08) * 0.10
        + max(micro_range_stability, -0.50) * 0.08
        + max(session_vwap_clearance, -0.20) * 0.09
        + max(session_room, 0.0) * 0.06
        + max(volume_participation_balance, -0.50) * 0.05
        + max(prior_day_direction_context, -0.20) * 0.04
    )
    if activity < MIN_VWAP_ACCEPTANCE_CONTINUATION_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "vwap_acceptance_persistence": bounded(vwap_acceptance_persistence, 0.0, 1.0),
        "micro_acceptance_persistence": bounded(micro_acceptance_persistence, 0.0, 1.0),
        "vwap_slope_alignment": bounded(vwap_slope_alignment, -4.0, 4.0),
        "directional_close_efficiency": bounded(directional_close_efficiency, -2.0, 2.0),
        "pullback_containment": bounded(pullback_containment, -2.0, 3.0),
        "acceptance_reacceleration": bounded(acceptance_reacceleration, -4.0, 4.0),
        "micro_range_stability": bounded(micro_range_stability, -3.0, 1.0),
        "session_vwap_clearance": bounded(session_vwap_clearance, -4.0, 4.0),
        ...
        "earlier_session_vwap": rounded(earlier_vwap) or 0.0,
        "micro_net": bounded(micro_net / max(average_range, 1e-9), -4.0, 4.0),
        "vwap_acceptance_continuation_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0135_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0135_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0135_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0135_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0135_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0135_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0135_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0135_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0135_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0135_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0135_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0135_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0135_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0135_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0135_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0135_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0135_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0135_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0135_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def c0135_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    acceptance_reward = 0.14 if metrics["vwap_acceptance_persistence"] > 0.62 and terminal > 0 else 0.0
    slope_reward = 0.10 if metrics["vwap_slope_alignment"] > 0.02 and terminal > 0 else 0.0
    close_efficiency_reward = 0.11 if metrics["directional_close_efficiency"] > 0.08 and terminal > 0 else 0.0
    containment_reward = 0.09 if metrics["pullback_containment"] > 0.55 and terminal > 0 else 0.0
    reacceleration_reward = 0.10 if metrics["acceptance_reacceleration"] > 0.04 and terminal > 0 else 0.0
    stability_reward = 0.07 if metrics["micro_range_stability"] > 0.35 and terminal > 0 else 0.0
    clearance_reward = 0.08 if metrics["session_vwap_clearance"] > -0.02 and terminal > 0 else 0.0
    ...
    weak_acceptance_penalty = 0.10 if metrics["vwap_acceptance_persistence"] < 0.58 and terminal <= 0 else 0.0
    weak_slope_penalty = 0.06 if metrics["vwap_slope_alignment"] < -0.08 and terminal <= 0 else 0.0
    weak_efficiency_penalty = 0.07 if metrics["directional_close_efficiency"] < -0.02 and terminal <= 0 else 0.0
    weak_reacceleration_penalty = 0.08 if metrics["acceptance_reacceleration"] < -0.08 and terminal <= 0 else 0.0
    weak_clearance_penalty = 0.09 if metrics["session_vwap_clearance"] < -0.08 and terminal <= 0 else 0.0
    ...
    return (
        ...
        - max(0.0, MIN_VWAP_ACCEPTANCE_PERSISTENCE - metrics["vwap_acceptance_persistence"]) * 0.10
        - max(0.0, MIN_SESSION_VWAP_CLEARANCE - metrics["session_vwap_clearance"]) * 0.06
    )


def fit_linear_vwap_acceptance_continuation_model(candidates: list[base.Candidate], fold_id: str) -> LinearVwapAcceptanceContinuationModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearVwapAcceptanceContinuationModel(
            fold_id,
    ...
    vwap_acceptance_continuation_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearVwapAcceptanceContinuationModel(
        fold_id=fold_id,
        ...
        vwap_acceptance_continuation_failure_rate=vwap_acceptance_continuation_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearVwapAcceptanceContinuationModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_vwap_acceptance_continuation_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0135_r0001_intraday_vwap_acceptance_continuation",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0135_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0135_r0001_vwap_acceptance_continuation_summary.json",
        ],
        ...
        "conditional_profiles": {
            "vwap_acceptance_continuation_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0135 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0135_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0135 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "vwap_acceptance_continuation_lookback_bars": VWAP_ACCEPTANCE_CONTINUATION_LOOKBACK_BARS,
        "vwap_acceptance_continuation_micro_bars": VWAP_ACCEPTANCE_CONTINUATION_MICRO_BARS,
        "min_vwap_acceptance_persistence": MIN_VWAP_ACCEPTANCE_PERSISTENCE,
        "min_session_vwap_clearance": MIN_SESSION_VWAP_CLEARANCE,
        "min_vwap_acceptance_continuation_activity": MIN_VWAP_ACCEPTANCE_CONTINUATION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_vwap_acceptance_continuation_model_summary(model: LinearVwapAcceptanceContinuationModel) -> dict[str, object]:
    return {
        ...
        "vwap_acceptance_continuation_failure_rate": rounded(model.vwap_acceptance_continuation_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearVwapAcceptanceContinuationModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_vwap_acceptance_continuation_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "vwap_acceptance_continuation_profile": profiles["vwap_acceptance_continuation_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0135_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0135_r0001_proxy_trades.csv"
    evidence["vwap_acceptance_continuation_summary"] = "artifacts/c0135_r0001_vwap_acceptance_continuation_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0135_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0135_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0135 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0135_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    next_work["tasks"] = ["produce_c0135_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0135_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0135_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0135_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0135_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0135_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0135_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0135 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0135 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    block = f"""
- decision_id: dec_20260708_produce_c0135_r0001_proxy_evidence
  created_local_date: '{local_date}'
  status: active
  decision: produce_c0135_r0001_proxy_evidence
  refines:
  - dec_20260708_open_c0135_r0001_intraday_vwap_acceptance_continuation_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0135_r0001_proxy_records_vwap_acceptance_continuation_reference_surface
  ...
  - next_work_is_produce_c0135_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260708_produce_c0135_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0135-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0135_r0001_intraday_vwap_acceptance_continuation",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0135-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0135-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0135-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0135_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0135_r0001_intraday_vwap_acceptance_continuation",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0135-R0001-VWAP-ACCEPTANCE-CONTINUATION-SUMMARY",
            "artifact_role": "vwap_acceptance_continuation_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0135_r0001_vwap_acceptance_continuation_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0135_r0001_intraday_vwap_acceptance_continuation",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0135_r0001_mt5_logic_parity_evidence",
        }
    ]
