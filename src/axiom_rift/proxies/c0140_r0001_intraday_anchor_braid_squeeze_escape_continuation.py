"""C0140 R0001 proxy evidence for intraday anchor braid squeeze escape continuation."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0140_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0140_r0001_anchor_braid_squeeze_escape_continuation_summary.json"
...
FEATURE_NAMES = (
    "anchor_braid_tightness",
    "escape_impulse",
    "vwap_escape_alignment",
    "day_open_escape_alignment",
    "prior_close_escape_alignment",
    "compression_quality",
    "post_escape_settlement",
    "pullback_containment",
    "participation_confirmation",
    "adverse_tail_control",
    "session_room_after_escape",
    "prior_day_support",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_anchor_braid_squeeze_escape_continuation_rank"
LABEL_SHAPE = "target_first_quality_after_anchor_braid_squeeze_escape_continuation"
SELECTION_RULE = "top_fold_local_anchor_braid_squeeze_escape_continuation_scores_per_active_day"
...
MIN_DAY_BARS = 18
ANCHOR_BRAID_SQUEEZE_ESCAPE_CONTINUATION_LOOKBACK_BARS = 72
ANCHOR_BRAID_SQUEEZE_ESCAPE_CONTINUATION_MICRO_BARS = 16
MIN_ANCHOR_BRAID_TIGHTNESS = -0.65
MIN_ESCAPE_IMPULSE = -0.08
MIN_PULLBACK_CONTAINMENT = 0.24
MIN_ANCHOR_BRAID_SQUEEZE_ESCAPE_CONTINUATION_ACTIVITY = 0.02
...
class LinearAnchorBraidSqueezeEscapeContinuationModel:
    fold_id: str
    ...
    anchor_braid_squeeze_escape_continuation_failure_rate: float
...
def run_c0140_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_anchor_braid_squeeze_escape_continuation_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_anchor_braid_squeeze_escape_continuation_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        ANCHOR_BRAID_SQUEEZE_ESCAPE_CONTINUATION_LOOKBACK_BARS,
        ANCHOR_BRAID_SQUEEZE_ESCAPE_CONTINUATION_MICRO_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = anchor_braid_squeeze_escape_continuation_metrics(
    ...
    features = (
        metrics["anchor_braid_tightness"],
        metrics["escape_impulse"],
        metrics["vwap_escape_alignment"],
        metrics["day_open_escape_alignment"],
        metrics["prior_close_escape_alignment"],
        metrics["compression_quality"],
        metrics["post_escape_settlement"],
        metrics["pullback_containment"],
        metrics["participation_confirmation"],
        metrics["adverse_tail_control"],
        metrics["session_room_after_escape"],
        metrics["prior_day_support"],
        metrics["session_maturity"],
        metrics["spread_stress_inverse"],
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        anchor_braid_tightness_bucket(metrics["anchor_braid_tightness"]),
        anchor_escape_impulse_bucket(metrics["escape_impulse"]),
        vwap_side_confirm_bucket(metrics["vwap_escape_alignment"]),
        day_open_tension_bucket(metrics["day_open_escape_alignment"]),
        prior_anchor_bucket(metrics["prior_close_escape_alignment"]),
        coil_compression_bucket(metrics["compression_quality"]),
        close_location_drive_bucket(metrics["post_escape_settlement"]),
        pullback_containment_bucket(metrics["pullback_containment"]),
        participation_followthrough_bucket(metrics["participation_confirmation"]),
        adverse_wick_control_bucket(metrics["adverse_tail_control"]),
        session_pressure_bucket(metrics["session_room_after_escape"]),
        prior_direction_context_bucket(metrics["prior_day_support"]),
...
def anchor_braid_squeeze_escape_continuation_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    micro_start = max(index - ANCHOR_BRAID_SQUEEZE_ESCAPE_CONTINUATION_MICRO_BARS + 1, context.day_start_index)
    lookback_start = max(index - ANCHOR_BRAID_SQUEEZE_ESCAPE_CONTINUATION_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    anchors = (session_vwap, context.day_open, prior_close_anchor)
    anchor_span = max(anchors) - min(anchors)
    anchor_center = sum(anchors) / len(anchors)
    anchor_braid_tightness = 1.0 - anchor_span / max(average_range, 1e-9)
    escape_impulse = direction * (signal_bar.close - previous_bar.close) / max(average_range, 1e-9)
    vwap_escape_alignment = direction * (signal_bar.close - session_vwap) / max(average_range, 1e-9)
    day_open_escape_alignment = direction * (signal_bar.close - context.day_open) / max(average_range, 1e-9)
    prior_close_escape_alignment = direction * (signal_bar.close - prior_close_anchor) / max(average_range, 1e-9)
    anchor_center_escape = direction * (signal_bar.close - anchor_center) / max(average_range, 1e-9)
    compression_quality = (lookback_range_mean - micro_range_mean) / max(average_range, 1e-9)
    post_escape_settlement = directional_bar_settlement(signal_bar, direction)
    if direction > 0:
        pullback_containment = (signal_bar.close - micro_low) / micro_span
        session_room_after_escape = (context.high_so_far - signal_bar.close) / max(average_range, 1e-9)
    else:
        pullback_containment = (micro_high - signal_bar.close) / micro_span
        session_room_after_escape = (signal_bar.close - context.low_so_far) / max(average_range, 1e-9)
    ...
    participation_confirmation = current_volume_ratio - micro_volume_ratio
    adverse_tail_control = adverse_wick_absorption(signal_bar, direction)
    prior_day_support = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_day_support = direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
    if context.prior_day_shadow_balance is not None:
        prior_day_support -= direction * context.prior_day_shadow_balance * 0.03

    if anchor_braid_tightness < MIN_ANCHOR_BRAID_TIGHTNESS and compression_quality < -0.25:
        return {"event_count": 0.0}
    if escape_impulse < MIN_ESCAPE_IMPULSE and anchor_center_escape < -0.18:
        return {"event_count": 0.0}
    if pullback_containment < MIN_PULLBACK_CONTAINMENT and adverse_tail_control < 0.22:
        return {"event_count": 0.0}
    ...
    activity = (
        max(anchor_braid_tightness, -0.75) * 0.14
        + max(escape_impulse, -0.18) * 0.17
        + max(vwap_escape_alignment, -0.35) * 0.10
        + max(day_open_escape_alignment, -0.40) * 0.08
        + max(prior_close_escape_alignment, -0.45) * 0.07
        + max(compression_quality, -0.30) * 0.10
        + max(post_escape_settlement - 0.42, -0.22) * 0.10
        + max(pullback_containment - 0.35, -0.22) * 0.09
        + max(participation_confirmation, -0.55) * 0.08
        + max(adverse_tail_control - 0.20, -0.20) * 0.07
        + max(session_room_after_escape, -0.25) * 0.03
        + max(prior_day_support, -0.35) * 0.02
    )
    if activity < MIN_ANCHOR_BRAID_SQUEEZE_ESCAPE_CONTINUATION_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "anchor_braid_tightness": bounded(anchor_braid_tightness, -8.0, 1.0),
        "escape_impulse": bounded(escape_impulse, -4.0, 4.0),
        "vwap_escape_alignment": bounded(vwap_escape_alignment, -5.0, 5.0),
        "day_open_escape_alignment": bounded(day_open_escape_alignment, -5.0, 5.0),
        "prior_close_escape_alignment": bounded(prior_close_escape_alignment, -5.0, 5.0),
        "compression_quality": bounded(compression_quality, -3.0, 6.0),
        "post_escape_settlement": bounded(post_escape_settlement, 0.0, 1.0),
        "pullback_containment": bounded(pullback_containment, 0.0, 1.0),
        "participation_confirmation": bounded(participation_confirmation, -6.0, 8.0),
        "adverse_tail_control": bounded(adverse_tail_control, 0.0, 1.0),
        "session_room_after_escape": bounded(session_room_after_escape, -3.0, 8.0),
        "prior_day_support": bounded(prior_day_support, -3.0, 3.0),
        ...
        "anchor_span_ratio": bounded(anchor_span / max(average_range, 1e-9), 0.0, 12.0),
        "anchor_center_escape": bounded(anchor_center_escape, -5.0, 5.0),
        ...
        "anchor_braid_squeeze_escape_continuation_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def C0140_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def C0140_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def C0140_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def C0140_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def C0140_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def C0140_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def C0140_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def C0140_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def C0140_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def C0140_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def C0140_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def C0140_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def C0140_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def C0140_pullback_containment_bucket(value: float) -> str:
    if value < 0.35:
...
def C0140_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def C0140_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def C0140_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def C0140_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def C0140_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def C0140_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    braid_reward = 0.10 if metrics["anchor_braid_tightness"] > 0.18 and terminal > 0 else 0.0
    impulse_reward = 0.13 if metrics["escape_impulse"] > 0.05 and terminal > 0 else 0.0
    vwap_reward = 0.09 if metrics["vwap_escape_alignment"] > -0.05 and terminal > 0 else 0.0
    open_reward = 0.07 if metrics["day_open_escape_alignment"] > -0.12 and terminal > 0 else 0.0
    prior_close_reward = 0.07 if metrics["prior_close_escape_alignment"] > -0.16 and terminal > 0 else 0.0
    compression_reward = 0.08 if metrics["compression_quality"] > 0.04 and terminal > 0 else 0.0
    settlement_reward = 0.10 if metrics["post_escape_settlement"] > 0.58 and terminal > 0 else 0.0
    containment_reward = 0.08 if metrics["pullback_containment"] > 0.52 and terminal > 0 else 0.0
    participation_reward = 0.07 if metrics["participation_confirmation"] > -0.10 and terminal > 0 else 0.0
    tail_reward = 0.06 if metrics["adverse_tail_control"] > 0.42 and terminal > 0 else 0.0
    room_reward = 0.04 if metrics["session_room_after_escape"] > 0.10 and terminal > 0 else 0.0
    prior_day_reward = 0.03 if metrics["prior_day_support"] > -0.28 and terminal > 0 else 0.0
    loose_braid_penalty = 0.07 if metrics["anchor_braid_tightness"] < -0.35 and terminal <= 0 else 0.0
    weak_impulse_penalty = 0.10 if metrics["escape_impulse"] < -0.05 and terminal <= 0 else 0.0
    failed_anchor_penalty = 0.08 if metrics["anchor_center_escape"] < -0.22 and terminal <= 0 else 0.0
    weak_settlement_penalty = 0.07 if metrics["post_escape_settlement"] < 0.42 and terminal <= 0 else 0.0
    weak_containment_penalty = 0.07 if metrics["pullback_containment"] < 0.34 and terminal <= 0 else 0.0
    noisy_compression_penalty = 0.05 if metrics["compression_quality"] < -0.20 and terminal <= 0 else 0.0
    no_participation_penalty = 0.05 if metrics["participation_confirmation"] < -0.55 and terminal <= 0 else 0.0
    no_room_penalty = 0.04 if metrics["session_room_after_escape"] < 0.04 and terminal <= 0 else 0.0
    ...
    return (
        ...
        - weak_settlement_penalty
        - weak_containment_penalty
        - noisy_compression_penalty
        ...
        - max(0.0, -metrics["participation_confirmation"] - 0.75) * 0.015
        - max(0.0, MIN_ANCHOR_BRAID_TIGHTNESS - metrics["anchor_braid_tightness"]) * 0.04
        - max(0.0, MIN_ESCAPE_IMPULSE - metrics["escape_impulse"]) * 0.08
    )


def fit_linear_anchor_braid_squeeze_escape_continuation_model(candidates: list[base.Candidate], fold_id: str) -> LinearAnchorBraidSqueezeEscapeContinuationModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearAnchorBraidSqueezeEscapeContinuationModel(
            fold_id,
    ...
    anchor_braid_squeeze_escape_continuation_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearAnchorBraidSqueezeEscapeContinuationModel(
        fold_id=fold_id,
        ...
        anchor_braid_squeeze_escape_continuation_failure_rate=anchor_braid_squeeze_escape_continuation_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearAnchorBraidSqueezeEscapeContinuationModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_anchor_braid_squeeze_escape_continuation_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0140_r0001_intraday_anchor_braid_squeeze_escape_continuation",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0140_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0140_r0001_anchor_braid_squeeze_escape_continuation_summary.json",
        ],
        ...
        "conditional_profiles": {
            "anchor_braid_squeeze_escape_continuation_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0140 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0140_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0140 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "anchor_braid_squeeze_escape_continuation_lookback_bars": ANCHOR_BRAID_SQUEEZE_ESCAPE_CONTINUATION_LOOKBACK_BARS,
        "anchor_braid_squeeze_escape_continuation_micro_bars": ANCHOR_BRAID_SQUEEZE_ESCAPE_CONTINUATION_MICRO_BARS,
        "min_anchor_braid_tightness": MIN_ANCHOR_BRAID_TIGHTNESS,
        "min_escape_impulse": MIN_ESCAPE_IMPULSE,
        "min_pullback_containment": MIN_PULLBACK_CONTAINMENT,
        "min_anchor_braid_squeeze_escape_continuation_activity": MIN_ANCHOR_BRAID_SQUEEZE_ESCAPE_CONTINUATION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_anchor_braid_squeeze_escape_continuation_model_summary(model: LinearAnchorBraidSqueezeEscapeContinuationModel) -> dict[str, object]:
    return {
        ...
        "anchor_braid_squeeze_escape_continuation_failure_rate": rounded(model.anchor_braid_squeeze_escape_continuation_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearAnchorBraidSqueezeEscapeContinuationModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_anchor_braid_squeeze_escape_continuation_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "anchor_braid_squeeze_escape_continuation_profile": profiles["anchor_braid_squeeze_escape_continuation_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["next_required_action"] = "produce_c0140_r0001_mt5_logic_parity_evidence"
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0140_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0140_r0001_proxy_trades.csv"
    evidence["anchor_braid_squeeze_escape_continuation_summary"] = "artifacts/c0140_r0001_anchor_braid_squeeze_escape_continuation_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0140_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0140_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0140 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0140_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    next_work["tasks"] = ["produce_c0140_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0140_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0140_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0140_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0140_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0140_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0140_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0140 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0140 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    block = f"""
- decision_id: dec_20260708_produce_c0140_r0001_proxy_evidence
  created_local_date: '{local_date}'
  status: active
  decision: produce_c0140_r0001_proxy_evidence
  refines:
  - dec_20260708_open_c0140_r0001_intraday_anchor_braid_squeeze_escape_continuation_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0140_r0001_proxy_records_anchor_braid_squeeze_escape_continuation_reference_surface
  ...
  - next_work_is_produce_c0140_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260708_produce_c0140_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0140-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0140_r0001_intraday_anchor_braid_squeeze_escape_continuation",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0140-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0140-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0140-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0140_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0140_r0001_intraday_anchor_braid_squeeze_escape_continuation",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0140-R0001-ANCHOR-BRAID-SQUEEZE-ESCAPE-CONTINUATION-SUMMARY",
            "artifact_role": "anchor_braid_squeeze_escape_continuation_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0140_r0001_anchor_braid_squeeze_escape_continuation_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0140_r0001_intraday_anchor_braid_squeeze_escape_continuation",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0140_r0001_mt5_logic_parity_evidence",
        }
    ]
