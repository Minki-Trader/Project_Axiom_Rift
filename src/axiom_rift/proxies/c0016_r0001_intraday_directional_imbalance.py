"""C0016 R0001 proxy evidence for intraday directional imbalance discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0016_intraday_directional_imbalance_discovery" / "runs" / "R0001"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0016_intraday_directional_imbalance_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0016_r0001_proxy_trades.csv"
DIRECTIONAL_IMBALANCE_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0016_r0001_intraday_directional_imbalance_summary.json"
...
FEATURE_NAMES = (
    ...
    "close_sign_imbalance_12",
    "close_sign_imbalance_36",
    "same_direction_close_count_12",
    "opposite_direction_close_count_12",
    "close_streak_directional",
    "alternating_churn_12",
    "impulse_vs_churn_36",
...
MODEL_FAMILY = "fold_local_intraday_directional_imbalance_rank"
LABEL_SHAPE = "directional_intraday_imbalance_continuation_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_intraday_directional_imbalance_scores_per_active_day"
...
def run_c0016_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = directional_imbalance_features(
                bars,
                range_average,
                short_range_average,
                day_context,
            ...
            if features is None:
                continue
            label = (
                directional_imbalance_label(bars, range_average, index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|intraday_directional_imbalance",
                    features=features,
                    label=label,
                )
            )
...
def directional_imbalance_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    if bar_range <= 0:
        return None
    ret_1 = directional_return(bars, index, direction, 1, average_range)
    ret_3 = directional_return(bars, index, direction, 3, average_range)
    ...
    ret_24 = directional_return(bars, index, direction, 24, average_range)
    day_high = context["day_high"]
    day_low = context["day_low"]
    day_range = max(day_high - day_low, 1e-9)
    close_stats_12 = close_sign_stats(bars, index, direction, 12)
    close_stats_36 = close_sign_stats(bars, index, direction, 36)
    ...
    close_streak = directional_close_streak(bars, index, direction, 12)
    ...
    impulse_vs_churn = ret_12 * (1.0 - close_stats_12["alternating_churn"])
    ...
    return (
        ...
        efficiency_36,
        efficiency_72,
        close_stats_12["imbalance"],
        close_stats_36["imbalance"],
        close_stats_12["same_count"],
        close_stats_12["opposite_count"],
        close_streak,
        close_stats_12["alternating_churn"],
        impulse_vs_churn,
        body_fraction,
        directional_close_location,
        favorable_wick_fraction,
...
def directional_imbalance_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    index: int,
    direction: int,
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0016"
    payload["work_unit_id"] = "C0016"
    payload["run_id"] = "R0001"
    payload["proxy_id"] = "PX-C0016-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0016_r0001_intraday_directional_imbalance"
    payload["proxy_config_path"] = "campaigns/C0016_intraday_directional_imbalance_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_intraday_directional_imbalance_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0016_intraday_directional_imbalance_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0016_intraday_directional_imbalance_discovery/runs/R0001/artifacts/c0016_r0001_proxy_trades.csv",
        "campaigns/C0016_intraday_directional_imbalance_discovery/runs/R0001/artifacts/c0016_r0001_intraday_directional_imbalance_summary.json",
    ]
    ...
    profiles["intraday_directional_imbalance_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0016_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "feature_weight_floor": FEATURE_WEIGHT_FLOOR,
            "feature_weight_ceiling": FEATURE_WEIGHT_CEILING,
            "score_interpretation": "higher_score_means_direct_fold_local_intraday_directional_imbalance_rank",
            "variant_boundary": "intraday_directional_imbalance_rank_not_auction_threshold_monthly_memory_lifecycle_structural_trap_score_floor_stop_target_hold_or_retry_nudge",
        }
    )
    return config
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_directional_imbalance_summary_artifact(payload, DIRECTIONAL_IMBALANCE_SUMMARY_PATH)
    PROXY_PATH.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="ascii")
    trade_hash = base.sha256_file(TRADE_ARTIFACT_PATH)
    summary_hash = base.sha256_file(DIRECTIONAL_IMBALANCE_SUMMARY_PATH)
...
def write_directional_imbalance_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_intraday_directional_imbalance_summary_v1",
        "template": False,
        "work_unit_id": "C0016",
        "campaign_id": "C0016",
        "run_id": "R0001",
        ...
        "intraday_directional_imbalance_profile": profiles["intraday_directional_imbalance_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "intraday_directional_imbalance_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0016-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0016_intraday_directional_imbalance_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0016_intraday_directional_imbalance_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0016-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0016-R0001-LIQUIDITY-VACUUM-REBOUND-SUMMARY",
                "intraday_directional_imbalance_summary_artifact",
                "json",
                "campaigns/C0016_intraday_directional_imbalance_discovery/runs/R0001/artifacts/c0016_r0001_intraday_directional_imbalance_summary.json",
                summary_hash,
                ["campaigns/C0016_intraday_directional_imbalance_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0016_r0001_intraday_directional_imbalance",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0016_r0001_proxy_trades.csv"
    evidence["intraday_directional_imbalance_summary"] = "artifacts/c0016_r0001_intraday_directional_imbalance_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0016_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0016_r0001_proxy_trades.csv",
        "artifacts/c0016_r0001_intraday_directional_imbalance_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0016 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0016_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0016_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0016_intraday_directional_imbalance_discovery"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0016_intraday_directional_imbalance_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0016_intraday_directional_imbalance_discovery",
        "open_c0016_r0001_fold_local_intraday_directional_imbalance_rank_run",
        "produce_c0016_r0001_proxy_evidence",
    ):
    ...
    data["active_run"] = "campaigns/C0016_intraday_directional_imbalance_discovery/runs/R0001"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["active_campaign"] = "campaigns/C0016_intraday_directional_imbalance_discovery"
    data["active_run"] = "campaigns/C0016_intraday_directional_imbalance_discovery/runs/R0001"
    data["latest_operation"] = {
        "id": "produce_c0016_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0016_intraday_directional_imbalance_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0016_r0001_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0016",
            "c0004_r0001_fold_local_state_archetype": "c0016_r0001_intraday_directional_imbalance",
            "c0004_r0001": "c0016_r0001",
            "fold_local_state_archetype_discovery": "intraday_directional_imbalance_discovery",
            ...
            "state_archetype_summary": "intraday_directional_imbalance_summary",
        }
