"""C0019 R0001 proxy evidence for bar-quality asymmetry discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0019_bar_quality_asymmetry_discovery" / "runs" / "R0001"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0019_bar_quality_asymmetry_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0019_r0001_proxy_trades.csv"
BAR_QUALITY_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0019_r0001_bar_quality_asymmetry_summary.json"
...
FEATURE_NAMES = (
    ...
    "body_over_range_average",
    "favorable_wick_fraction",
    "adverse_wick_fraction",
    "wick_asymmetry",
    "close_to_extreme_fraction",
    "body_efficiency_over_range",
    ...
    "directional_body_sum_3",
    "directional_body_sum_6",
    "favorable_wick_cluster_3",
    "adverse_wick_cluster_3",
    "favorable_wick_cluster_6",
    "adverse_wick_cluster_6",
    "rejection_count_6",
    "acceptance_count_6",
    "alternating_body_count_12",
    "close_flip_count_12",
    "adverse_close_drift_12",
    "day_range_position",
    "spread_over_range",
    "spread_pressure_48",
...
MODEL_FAMILY = "fold_local_bar_quality_asymmetry_rank"
LABEL_SHAPE = "directional_bar_quality_asymmetry_followthrough_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_bar_quality_asymmetry_scores_per_active_day"
...
def run_c0019_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = bar_quality_asymmetry_features(
                bars,
                range_average,
                short_range_average,
                day_context,
            ...
            if features is None:
                continue
            label = (
                bar_quality_asymmetry_label(bars, range_average, index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|bar_quality_asymmetry",
                    features=features,
                    label=label,
                )
            )
...
def bar_quality_asymmetry_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    body_fraction = bounded(direction * (bar.close - bar.open) / bar_range, -1.0, 1.0)
    body_over_range_average = direction * (bar.close - bar.open) / average_range
    ...
    favorable_wick_fraction = lower_wick / bar_range if direction > 0 else upper_wick / bar_range
    adverse_tail_fraction = upper_wick / bar_range if direction > 0 else lower_wick / bar_range
    wick_asymmetry = favorable_wick_fraction - adverse_tail_fraction
    close_to_extreme_fraction = directional_close_location
    body_efficiency = direction * (bar.close - bar.open) / average_range
    ...
    directional_body_sum_3 = directional_body_sum(bars, index, direction, 3)
    directional_body_sum_6 = directional_body_sum(bars, index, direction, 6)
    favorable_cluster_3 = wick_cluster_average(bars, index, direction, 3, favorable=True)
    adverse_cluster_3 = wick_cluster_average(bars, index, direction, 3, favorable=False)
    favorable_cluster_6 = wick_cluster_average(bars, index, direction, 6, favorable=True)
    adverse_cluster_6 = wick_cluster_average(bars, index, direction, 6, favorable=False)
    rejection_count_6 = rejection_count(bars, index, direction, 6)
    acceptance_count_6 = acceptance_count(bars, index, direction, 6)
    alternating_body_count_12 = alternating_body_count(bars, index, 12)
    close_flip_count_12 = close_flip_count(bars, index, 12)
    adverse_close_drift_12 = max(0.0, -direction * (bar.close - bars[index - 12].close)) / average_range
...
def bar_quality_asymmetry_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    index: int,
    direction: int,
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0019"
    payload["work_unit_id"] = "C0019"
    payload["run_id"] = "R0001"
    payload["proxy_id"] = "PX-C0019-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0019_r0001_bar_quality_asymmetry"
    payload["proxy_config_path"] = "campaigns/C0019_bar_quality_asymmetry_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_bar_quality_asymmetry_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0019_bar_quality_asymmetry_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0019_bar_quality_asymmetry_discovery/runs/R0001/artifacts/c0019_r0001_proxy_trades.csv",
        "campaigns/C0019_bar_quality_asymmetry_discovery/runs/R0001/artifacts/c0019_r0001_bar_quality_asymmetry_summary.json",
    ]
    ...
    profiles["bar_quality_asymmetry_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0019_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "feature_weight_floor": FEATURE_WEIGHT_FLOOR,
            "feature_weight_ceiling": FEATURE_WEIGHT_CEILING,
            "score_interpretation": "higher_score_means_direct_fold_local_bar_quality_asymmetry_rank",
            "variant_boundary": "bar_quality_asymmetry_rank_not_auction_threshold_monthly_memory_lifecycle_structural_trap_score_floor_stop_target_hold_or_retry_nudge",
        }
    )
    return config
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_bar_quality_asymmetry_summary_artifact(payload, BAR_QUALITY_SUMMARY_PATH)
    PROXY_PATH.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="ascii")
    trade_hash = base.sha256_file(TRADE_ARTIFACT_PATH)
    summary_hash = base.sha256_file(BAR_QUALITY_SUMMARY_PATH)
...
def write_bar_quality_asymmetry_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_bar_quality_asymmetry_summary_v1",
        "template": False,
        "work_unit_id": "C0019",
        "campaign_id": "C0019",
        "run_id": "R0001",
        ...
        "bar_quality_asymmetry_profile": profiles["bar_quality_asymmetry_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "bar_quality_asymmetry_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0019-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0019_bar_quality_asymmetry_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0019_bar_quality_asymmetry_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0019-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0019-R0001-BAR-QUALITY-ASYMMETRY-SUMMARY",
                "bar_quality_asymmetry_summary_artifact",
                "json",
                "campaigns/C0019_bar_quality_asymmetry_discovery/runs/R0001/artifacts/c0019_r0001_bar_quality_asymmetry_summary.json",
                summary_hash,
                ["campaigns/C0019_bar_quality_asymmetry_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0019_r0001_bar_quality_asymmetry",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0019_r0001_proxy_trades.csv"
    evidence["bar_quality_asymmetry_summary"] = "artifacts/c0019_r0001_bar_quality_asymmetry_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0019_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0019_r0001_proxy_trades.csv",
        "artifacts/c0019_r0001_bar_quality_asymmetry_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0019 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0019_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0019_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0019_bar_quality_asymmetry_discovery"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0019_bar_quality_asymmetry_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0019_bar_quality_asymmetry_discovery",
        "open_c0019_r0001_fold_local_bar_quality_asymmetry_rank_run",
        "produce_c0019_r0001_proxy_evidence",
    ):
    ...
    data["active_run"] = "campaigns/C0019_bar_quality_asymmetry_discovery/runs/R0001"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["active_campaign"] = "campaigns/C0019_bar_quality_asymmetry_discovery"
    data["active_run"] = "campaigns/C0019_bar_quality_asymmetry_discovery/runs/R0001"
    data["latest_operation"] = {
        "id": "produce_c0019_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0019_bar_quality_asymmetry_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0019_r0001_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0019",
            "c0004_r0001_fold_local_state_archetype": "c0019_r0001_bar_quality_asymmetry",
            "c0004_r0001": "c0019_r0001",
            "fold_local_state_archetype_discovery": "bar_quality_asymmetry_discovery",
            ...
            "state_archetype_summary": "bar_quality_asymmetry_summary",
        }
