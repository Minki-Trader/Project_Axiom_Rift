"""C0102 R0001 proxy evidence for intraday open close energy transfer."""
...
CAMPAIGN_ID = "C0102"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0102_intraday_open_close_energy_transfer_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0102_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0102_r0001_open_close_energy_transfer_summary.json"
...
FEATURE_NAMES = (
    "opening_energy_ratio",
    "session_energy_transfer",
    "midday_energy_decay",
    "late_pressure_alignment",
    "prior_close_open_impulse",
    "early_range_carry",
    "directional_energy_efficiency",
    "time_phase_transfer",
    "prior_day_energy_context",
    "session_phase_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_open_close_energy_transfer_rank"
LABEL_SHAPE = "target_first_quality_after_open_close_energy_transfer"
SELECTION_RULE = "top_fold_local_open_close_energy_transfer_scores_per_active_day"
...
MIN_DAY_BARS = 30
OPEN_CLOSE_ENERGY_TRANSFER_LOOKBACK_BARS = 72
OPEN_CLOSE_ENERGY_TRANSFER_RECENT_BARS = 12
OPEN_CLOSE_ENERGY_TRANSFER_MID_BARS = 36
OPEN_WINDOW_BARS = 18
MIN_OPEN_CLOSE_ENERGY_TRANSFER_ACTIVITY = 0.08
...
class LinearOpenCloseEnergyTransferModel:
    fold_id: str
    ...
    open_close_energy_transfer_failure_rate: float
...
def run_c0102_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_open_close_energy_transfer_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_open_close_energy_transfer_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        OPEN_CLOSE_ENERGY_TRANSFER_LOOKBACK_BARS,
        OPEN_CLOSE_ENERGY_TRANSFER_RECENT_BARS + OPEN_WINDOW_BARS,
        OPEN_CLOSE_ENERGY_TRANSFER_MID_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = open_close_energy_transfer_metrics(
    ...
    features = (
        metrics["opening_energy_ratio"],
        metrics["session_energy_transfer"],
        metrics["midday_energy_decay"],
        metrics["late_pressure_alignment"],
        metrics["prior_close_open_impulse"],
        metrics["early_range_carry"],
        metrics["directional_energy_efficiency"],
        metrics["time_phase_transfer"],
        metrics["prior_day_energy_context"],
        metrics["session_phase_maturity"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        c0102_opening_energy_bucket(metrics["opening_energy_ratio"]),
        c0102_transfer_bucket(metrics["session_energy_transfer"]),
        c0102_midday_decay_bucket(metrics["midday_energy_decay"]),
        c0102_late_pressure_bucket(metrics["late_pressure_alignment"]),
        c0102_prior_open_impulse_bucket(metrics["prior_close_open_impulse"]),
        c0102_early_range_carry_bucket(metrics["early_range_carry"]),
        c0102_efficiency_bucket(metrics["directional_energy_efficiency"]),
        c0102_time_phase_transfer_bucket(metrics["time_phase_transfer"]),
        c0102_prior_energy_context_bucket(metrics["prior_day_energy_context"]),
...
def open_close_energy_transfer_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    open_end = min(context.day_start_index + OPEN_WINDOW_BARS - 1, index)
    if index <= open_end or context.day_start_index >= index:
        return {"event_count": 0.0}
    recent_start = max(index - OPEN_CLOSE_ENERGY_TRANSFER_RECENT_BARS + 1, context.day_start_index)
    lookback_start = max(index - OPEN_CLOSE_ENERGY_TRANSFER_LOOKBACK_BARS + 1, context.day_start_index)
    mid_end = recent_start - 1
    mid_start = max(open_end + 1, mid_end - OPEN_CLOSE_ENERGY_TRANSFER_MID_BARS + 1)
    if recent_start < 0 or lookback_start < 0:
        return {"event_count": 0.0}
    recent_start = max(recent_start, context.day_start_index)
    lookback_start = max(lookback_start, context.day_start_index)
    opening_window = bars[context.day_start_index : open_end + 1]
    recent_window = bars[recent_start : index + 1]
    mid_window = bars[mid_start : mid_end + 1] if mid_end >= mid_start else []
    lookback_window = bars[lookback_start : index + 1]
    if len(opening_window) < 8 or len(recent_window) < 6 or len(lookback_window) < 24:
        return {"event_count": 0.0}
    average_volume = volume_average[index]
    if average_volume is None or average_volume <= 0:
        return {"event_count": 0.0}

    opening_ranges = [bar.high - bar.low for bar in opening_window]
    recent_ranges = [bar.high - bar.low for bar in recent_window]
    lookback_ranges = [bar.high - bar.low for bar in lookback_window]
    mid_ranges = [bar.high - bar.low for bar in mid_window] if mid_window else recent_ranges
    opening_range = max(bar.high for bar in opening_window) - min(bar.low for bar in opening_window)
    opening_range_mean = base.mean(opening_ranges) or average_range
    recent_range_mean = base.mean(recent_ranges) or average_range
    lookback_range_mean = base.mean(lookback_ranges) or average_range
    mid_range_mean = base.mean(mid_ranges) or average_range
    opening_volume_mean = base.mean(volumes[context.day_start_index : open_end + 1]) or average_volume
    recent_volume_mean = base.mean(volumes[recent_start : index + 1]) or average_volume
    if opening_range <= 0 or opening_range_mean <= 0:
        return {"event_count": 0.0}

    session_phase_maturity = bounded_session_progress(signal_bar.time)
    if session_phase_maturity < 0.20:
        return {"event_count": 0.0}
    opening_volume_ratio = opening_volume_mean / max(average_volume, 1e-9)
    recent_range_ratio = recent_range_mean / max(average_range, 1e-9)
    opening_energy_ratio = (
        opening_range
        / max(average_range * math.sqrt(max(len(opening_window), 1)), 1e-9)
        * min(max(opening_volume_ratio, 0.25), 4.0)
    )
    recent_move = signal_bar.close - recent_window[0].open
    recent_travel = sum(abs(recent_window[pos].close - recent_window[pos - 1].close) for pos in range(1, len(recent_window)))
    directional_energy_efficiency = direction * recent_move / max(recent_travel, average_range * 0.25)
    opening_close = opening_window[-1].close
    opening_mid = (max(bar.high for bar in opening_window) + min(bar.low for bar in opening_window)) / 2.0
    opening_directional_bias = direction * (opening_close - context.day_open) / max(opening_range, average_range)
    early_range_carry = direction * (signal_bar.close - context.day_open) / max(opening_range, average_range)
    late_pressure_alignment = direction * (signal_bar.close - opening_mid) / max(average_range, 1e-9) * session_phase_maturity
    midday_energy_decay = 1.0 - mid_range_mean / max(opening_range_mean, average_range * 0.25)
    prior_close_open_impulse = 0.0
    if context.prior_day_close is not None:
        prior_close_open_impulse = direction * (context.day_open - context.prior_day_close) / max(average_range, 1e-9)
    prior_day_energy_context = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_volume_ratio = (context.prior_day_volume_per_bar or average_volume) / max(average_volume, 1e-9)
        prior_day_energy_context = (
            direction
            * context.prior_day_return
            / max(context.prior_day_range, 1e-9)
            * min(max(prior_volume_ratio, 0.25), 3.0)
        )
    session_energy_transfer = (
        opening_energy_ratio
        * (
            0.40 * opening_directional_bias
            + 0.30 * early_range_carry
            + 0.20 * late_pressure_alignment
            + 0.10 * directional_energy_efficiency
        )
        * max(session_phase_maturity, 0.10)
    )
    time_phase_transfer = session_phase_maturity * (session_energy_transfer - 0.20 * recent_range_ratio)
    activity = max(
        abs(session_energy_transfer),
        opening_energy_ratio * 0.20,
        abs(late_pressure_alignment) * 0.18,
        abs(early_range_carry) * 0.16,
        max(midday_energy_decay, 0.0) * 0.14,
        max(directional_energy_efficiency, 0.0) * 0.12,
    )
    if activity < MIN_OPEN_CLOSE_ENERGY_TRANSFER_ACTIVITY:
        return {"event_count": 0.0}
    if opening_energy_ratio < 0.18 and abs(early_range_carry) < 0.08:
        return {"event_count": 0.0}
    if abs(session_energy_transfer) < 0.035 and abs(late_pressure_alignment) < 0.08:
        return {"event_count": 0.0}
    return {
        "opening_energy_ratio": bounded(opening_energy_ratio, 0.0, 8.0),
        "session_energy_transfer": bounded(session_energy_transfer, -8.0, 8.0),
        "midday_energy_decay": bounded(midday_energy_decay, -4.0, 4.0),
        "late_pressure_alignment": bounded(late_pressure_alignment, -8.0, 8.0),
        "prior_close_open_impulse": bounded(prior_close_open_impulse, -8.0, 8.0),
        "early_range_carry": bounded(early_range_carry, -6.0, 6.0),
        "directional_energy_efficiency": bounded(directional_energy_efficiency, -2.0, 2.0),
        "time_phase_transfer": bounded(time_phase_transfer, -8.0, 8.0),
        "prior_day_energy_context": bounded(prior_day_energy_context, -4.0, 4.0),
        "session_phase_maturity": bounded(session_phase_maturity, 0.0, 1.0),
        "opening_range_ratio": bounded(opening_range / max(average_range, 1e-9), 0.0, 16.0),
        ...
        "open_close_energy_transfer_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0102_opening_energy_bucket(value: float) -> str:
    if value < 0.45:
        return "opening_energy_quiet"
    if value < 1.15:
        return "opening_energy_defined"
    return "opening_energy_heavy"


def c0102_transfer_bucket(value: float) -> str:
    if value < -0.18:
        return "transfer_against"
    if value < 0.18:
        return "transfer_mixed"
    return "transfer_aligned"


def c0102_midday_decay_bucket(value: float) -> str:
    if value < -0.15:
        return "midday_energy_reexpanding"
    if value < 0.20:
        return "midday_energy_neutral"
    return "midday_energy_decayed"


def c0102_late_pressure_bucket(value: float) -> str:
    if value < -0.30:
        return "late_pressure_against"
    if value < 0.30:
        return "late_pressure_mixed"
    return "late_pressure_aligned"


def c0102_prior_open_impulse_bucket(value: float) -> str:
    if value < -0.35:
        return "prior_open_impulse_against"
    if value < 0.35:
        return "prior_open_impulse_neutral"
    return "prior_open_impulse_aligned"


def c0102_early_range_carry_bucket(value: float) -> str:
    if value < -0.20:
        return "early_range_carry_failed"
    if value < 0.28:
        return "early_range_carry_mixed"
    return "early_range_carry_sustained"


def c0102_efficiency_bucket(value: float) -> str:
    if value < -0.10:
        return "directional_efficiency_against"
    if value < 0.18:
        return "directional_efficiency_choppy"
    return "directional_efficiency_clean"


def c0102_time_phase_transfer_bucket(value: float) -> str:
    if value < -0.10:
        return "time_phase_transfer_fading"
    if value < 0.20:
        return "time_phase_transfer_mixed"
    return "time_phase_transfer_building"


def c0102_prior_energy_context_bucket(value: float) -> str:
    if value < -0.25:
        return "prior_energy_against"
    if value < 0.25:
        return "prior_energy_neutral"
    return "prior_energy_aligned"
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    transfer_reward = 0.18 if metrics["session_energy_transfer"] > 0.25 and terminal > 0 else 0.0
    opening_reward = 0.09 if metrics["opening_energy_ratio"] > 0.60 and terminal > 0 else 0.0
    decay_reward = 0.08 if metrics["midday_energy_decay"] > 0.18 and terminal > 0 else 0.0
    pressure_reward = 0.13 if metrics["late_pressure_alignment"] > 0.28 and terminal > 0 else 0.0
    impulse_reward = 0.07 if metrics["prior_close_open_impulse"] > 0.20 and terminal > 0 else 0.0
    carry_reward = 0.11 if metrics["early_range_carry"] > 0.25 and terminal > 0 else 0.0
    efficiency_reward = 0.10 if metrics["directional_energy_efficiency"] > 0.18 and terminal > 0 else 0.0
    time_transfer_reward = 0.06 if metrics["time_phase_transfer"] > 0.12 and terminal > 0 else 0.0
    prior_reward = 0.05 if metrics["prior_day_energy_context"] > 0.15 and terminal > 0 else 0.0
    weak_transfer_penalty = 0.14 if metrics["session_energy_transfer"] < -0.10 and terminal <= 0 else 0.0
    anti_pressure_penalty = 0.12 if metrics["late_pressure_alignment"] < -0.20 and terminal <= 0 else 0.0
    failed_carry_penalty = 0.11 if metrics["early_range_carry"] < -0.12 and terminal <= 0 else 0.0
    inefficient_path_penalty = 0.10 if metrics["directional_energy_efficiency"] < -0.08 and terminal <= 0 else 0.0
    anti_context_penalty = 0.06 if metrics["prior_day_energy_context"] < -0.25 and terminal <= 0 else 0.0
    ...
    open_close_energy_transfer_path_quality = (0.54 * terminal + 0.30 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + open_close_energy_transfer_path_quality
        + transfer_reward
        + opening_reward
        + decay_reward
        + pressure_reward
        ...
        + carry_reward
        + efficiency_reward
        + time_transfer_reward
        + prior_reward
        - weak_transfer_penalty
        - anti_pressure_penalty
        - failed_carry_penalty
        ...
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["recent_range_ratio"] - 2.60) * 0.05
        - max(0.0, metrics["opening_range_ratio"] - 8.00) * 0.02
    )


def fit_linear_open_close_energy_transfer_model(candidates: list[base.Candidate], fold_id: str) -> LinearOpenCloseEnergyTransferModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearOpenCloseEnergyTransferModel(
            fold_id,
    ...
    open_close_energy_transfer_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearOpenCloseEnergyTransferModel(
        fold_id=fold_id,
        ...
        open_close_energy_transfer_failure_rate=open_close_energy_transfer_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearOpenCloseEnergyTransferModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0102-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_open_close_energy_transfer_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0102_r0001_intraday_open_close_energy_transfer",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0102_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0102_r0001_open_close_energy_transfer_summary.json",
        ],
        ...
        "conditional_profiles": {
            "open_close_energy_transfer_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0102 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0102_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0102 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "open_close_energy_transfer_lookback_bars": OPEN_CLOSE_ENERGY_TRANSFER_LOOKBACK_BARS,
        "open_close_energy_transfer_recent_bars": OPEN_CLOSE_ENERGY_TRANSFER_RECENT_BARS,
        "open_close_energy_transfer_mid_bars": OPEN_CLOSE_ENERGY_TRANSFER_MID_BARS,
        "open_window_bars": OPEN_WINDOW_BARS,
        "min_open_close_energy_transfer_activity": MIN_OPEN_CLOSE_ENERGY_TRANSFER_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_open_close_energy_transfer_model_summary(model: LinearOpenCloseEnergyTransferModel) -> dict[str, object]:
    return {
        ...
        "open_close_energy_transfer_failure_rate": rounded(model.open_close_energy_transfer_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearOpenCloseEnergyTransferModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_open_close_energy_transfer_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "open_close_energy_transfer_profile": profiles["open_close_energy_transfer_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0102_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0102_r0001_proxy_trades.csv"
    evidence["open_close_energy_transfer_summary"] = "artifacts/c0102_r0001_open_close_energy_transfer_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0102_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0102_r0001_proxy_trades.csv",
        "artifacts/c0102_r0001_open_close_energy_transfer_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0102_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0102 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0102_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0102_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0102_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0102_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0102_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0102_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0102_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0102_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0102_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0102 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0102 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0102_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0102_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0102_r0001_intraday_open_close_energy_transfer_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0102_r0001_proxy_records_open_close_energy_transfer_reference_surface
  ...
  - next_work_is_produce_c0102_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0102_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0102-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0102_r0001_intraday_open_close_energy_transfer",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0102-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0102-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0102-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0102_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0102_r0001_intraday_open_close_energy_transfer",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0102-R0001-open-close-energy-transfer-SUMMARY",
            "artifact_role": "open_close_energy_transfer_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0102_r0001_open_close_energy_transfer_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0102_r0001_intraday_open_close_energy_transfer",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0102_r0001_mt5_logic_parity_evidence",
        }
    ]
