"""C0111 R0001 proxy evidence for intraday cost efficient drift continuation."""
...
CAMPAIGN_ID = "C0111"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0111_intraday_cost_efficient_drift_continuation_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0111_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0111_r0001_cost_efficient_drift_continuation_summary.json"
...
FEATURE_NAMES = (
    ...
    "close_commitment",
    "session_vwap_alignment",
    "prior_session_tailwind",
    "adverse_wick_control",
    "session_phase_maturity",
    "continuation_room",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_cost_efficient_drift_continuation_rank"
LABEL_SHAPE = "target_first_quality_after_cost_efficient_drift_continuation"
SELECTION_RULE = "top_fold_local_cost_efficient_drift_continuation_scores_per_active_day"
...
MIN_DAY_BARS = 18
COST_EFFICIENT_DRIFT_CONTINUATION_LOOKBACK_BARS = 36
COST_EFFICIENT_DRIFT_CONTINUATION_RECENT_BARS = 10
COST_EFFICIENT_DRIFT_CONTINUATION_PRIOR_BARS = 18
MIN_DRIFT_EFFICIENCY = 0.10
MIN_COST_EFFICIENT_DRIFT_CONTINUATION_ACTIVITY = 0.05
...
class LinearCostEfficientDriftContinuationModel:
    fold_id: str
    ...
    cost_efficient_drift_continuation_failure_rate: float
...
def run_c0111_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_cost_efficient_drift_continuation_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_cost_efficient_drift_continuation_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        COST_EFFICIENT_DRIFT_CONTINUATION_LOOKBACK_BARS,
        COST_EFFICIENT_DRIFT_CONTINUATION_RECENT_BARS + COST_EFFICIENT_DRIFT_CONTINUATION_PRIOR_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = cost_efficient_drift_continuation_metrics(
    ...
    features = (
        metrics["drift_efficiency"],
        metrics["range_moderation"],
        metrics["spread_relief"],
        metrics["volume_stability"],
        metrics["pullback_containment"],
        metrics["close_commitment"],
        metrics["session_vwap_alignment"],
        metrics["prior_session_tailwind"],
        metrics["adverse_wick_control"],
        metrics["session_phase_maturity"],
        metrics["continuation_room"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        drift_efficiency_bucket(metrics["drift_efficiency"]),
        range_moderation_bucket(metrics["range_moderation"]),
        spread_relief_bucket(metrics["spread_relief"]),
        volume_stability_bucket(metrics["volume_stability"]),
        pullback_containment_bucket(metrics["pullback_containment"]),
        close_commitment_bucket(metrics["close_commitment"]),
        vwap_alignment_bucket(metrics["session_vwap_alignment"]),
        prior_tailwind_bucket(metrics["prior_session_tailwind"]),
        adverse_wick_control_bucket(metrics["adverse_wick_control"]),
        session_phase_maturity_bucket(metrics["session_phase_maturity"]),
        continuation_room_bucket(metrics["continuation_room"]),
...
def cost_efficient_drift_continuation_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    recent_start = max(index - COST_EFFICIENT_DRIFT_CONTINUATION_RECENT_BARS + 1, context.day_start_index)
    prior_start = max(recent_start - COST_EFFICIENT_DRIFT_CONTINUATION_PRIOR_BARS, context.day_start_index)
    lookback_start = max(index - COST_EFFICIENT_DRIFT_CONTINUATION_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    if len(prior_window) < 12 or len(recent_window) < 8 or len(lookback_window) < 24 or len(day_window) < MIN_DAY_BARS:
        return {"event_count": 0.0}
    ...
    if session_phase_maturity < 0.06 or session_phase_maturity > 0.78:
        return {"event_count": 0.0}
    ...
    drift_travel = sum(
        abs(recent_window[pos].close - recent_window[pos - 1].close)
        for pos in range(1, len(recent_window))
    )
    if drift_travel <= 0:
        return {"event_count": 0.0}
    drift_efficiency = direction * (signal_bar.close - recent_window[0].open) / drift_travel
    if drift_efficiency < MIN_DRIFT_EFFICIENCY:
        return {"event_count": 0.0}

    if direction > 0:
        pullback_containment = (signal_bar.close - recent_low) / average_range
        continuation_room = (context.high_so_far - signal_bar.close) / day_range_so_far
    else:
        pullback_containment = (recent_high - signal_bar.close) / average_range
        continuation_room = (signal_bar.close - context.low_so_far) / day_range_so_far
    if continuation_room < 0.08 or pullback_containment < 0.08:
        return {"event_count": 0.0}

    range_moderation = 1.0 - recent_range_mean / max(prior_range_mean, average_range * 0.10)
    spread_stress = signal_bar.spread_points / average_range
    spread_relief = 1.0 / (1.0 + max(spread_stress, 0.0))
    volume_stability_value = 1.0 - abs(recent_volume_mean - prior_volume_mean) / max(average_volume, 1e-9)
    close_commitment = directional_bar_settlement(signal_bar, direction)
    if close_commitment < 0.42:
        return {"event_count": 0.0}
    session_vwap_alignment = direction * (
        signal_bar.close - session_vwap_until(bars, volumes, context.day_start_index, index)
    ) / average_range
    if session_vwap_alignment < -0.20:
        return {"event_count": 0.0}
    ...
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        ...
        prior_session_tailwind += direction * (context.prior_day_shadow_balance or 0.0) * 0.15
    adverse_wick_control = 1.0 - adverse_wick_absorption(signal_bar, direction)

    activity = max(
        max(drift_efficiency, 0.0) * 0.55,
        max(range_moderation, 0.0) * 0.25,
        max(spread_relief - 0.45, 0.0) * 0.60,
        max(volume_stability_value, 0.0) * 0.22,
        max(pullback_containment, 0.0) * 0.22,
        max(close_commitment - 0.42, 0.0) * 0.50,
        max(session_vwap_alignment, 0.0) * 0.25,
        max(continuation_room, 0.0) * 0.20,
    )
    if activity < MIN_COST_EFFICIENT_DRIFT_CONTINUATION_ACTIVITY:
        return {"event_count": 0.0}
    if range_moderation < -0.75 or spread_relief < 0.35 or volume_stability_value < -0.75:
        return {"event_count": 0.0}
    return {
        "drift_efficiency": bounded(drift_efficiency, -1.0, 1.0),
        "range_moderation": bounded(range_moderation, -2.0, 2.0),
        "spread_relief": bounded(spread_relief, 0.0, 1.0),
        "volume_stability": bounded(volume_stability_value, -2.0, 1.0),
        "pullback_containment": bounded(pullback_containment, 0.0, 8.0),
        "close_commitment": bounded(close_commitment, 0.0, 1.0),
        "session_vwap_alignment": bounded(session_vwap_alignment, -4.0, 4.0),
        "prior_session_tailwind": bounded(prior_session_tailwind, -2.0, 2.0),
        "adverse_wick_control": bounded(adverse_wick_control, 0.0, 1.0),
        "session_phase_maturity": bounded(session_phase_maturity, 0.0, 1.0),
        "continuation_room": bounded(continuation_room, 0.0, 1.0),
        ...
        "cost_efficient_drift_continuation_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0111_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0111_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0111_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0111_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0111_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0111_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0111_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0111_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0111_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0111_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0111_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0111_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0111_boundary_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0111_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0111_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0111_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0111_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0111_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0111_prior_boundary_bucket(value: float) -> str:
    if value < -0.12:
...
def c0111_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    drift_reward = 0.14 if metrics["drift_efficiency"] > 0.24 and terminal > 0 else 0.0
    range_reward = 0.06 if metrics["range_moderation"] > 0.10 and terminal > 0 else 0.0
    spread_reward = 0.08 if metrics["spread_relief"] > 0.62 and terminal > 0 else 0.0
    volume_reward = 0.05 if metrics["volume_stability"] > 0.35 and terminal > 0 else 0.0
    pullback_reward = 0.07 if metrics["pullback_containment"] > 0.28 and terminal > 0 else 0.0
    commitment_reward = 0.09 if metrics["close_commitment"] > 0.62 and terminal > 0 else 0.0
    vwap_reward = 0.07 if metrics["session_vwap_alignment"] > 0.05 and terminal > 0 else 0.0
    tailwind_reward = 0.04 if metrics["prior_session_tailwind"] > 0.08 and terminal > 0 else 0.0
    wick_control_reward = 0.05 if metrics["adverse_wick_control"] > 0.50 and terminal > 0 else 0.0
    room_reward = 0.07 if metrics["continuation_room"] > 0.22 and terminal > 0 else 0.0
    weak_drift_penalty = 0.10 if metrics["drift_efficiency"] < 0.16 and terminal <= 0 else 0.0
    range_expansion_penalty = 0.08 if metrics["range_moderation"] < -0.18 and terminal <= 0 else 0.0
    spread_relief_penalty = 0.10 if metrics["spread_relief"] < 0.48 and terminal <= 0 else 0.0
    volume_disruption_penalty = 0.06 if metrics["volume_stability"] < -0.10 and terminal <= 0 else 0.0
    pullback_penalty = 0.07 if metrics["pullback_containment"] < 0.20 and terminal <= 0 else 0.0
    commitment_penalty = 0.08 if metrics["close_commitment"] < 0.52 and terminal <= 0 else 0.0
    vwap_penalty = 0.08 if metrics["session_vwap_alignment"] < -0.08 and terminal <= 0 else 0.0
    room_absent_penalty = 0.08 if metrics["continuation_room"] < 0.18 and terminal <= 0 else 0.0
    ...
    cost_efficient_drift_continuation_path_quality = (0.62 * terminal + 0.28 * mfe - 0.65 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + cost_efficient_drift_continuation_path_quality
        + drift_reward
        + range_reward
        + spread_reward
        + volume_reward
        ...
        + room_reward
        - weak_drift_penalty
        - range_expansion_penalty
        - spread_relief_penalty
        - volume_disruption_penalty
        - pullback_penalty
        - commitment_penalty
        - vwap_penalty
        - room_absent_penalty
        ...
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["recent_range_ratio"] - 2.20) * 0.07
        - max(0.0, metrics["recent_volume_ratio"] - 4.00) * 0.02
        - max(0.0, 0.20 - metrics["volume_stability"]) * 0.03
    )


def fit_linear_cost_efficient_drift_continuation_model(candidates: list[base.Candidate], fold_id: str) -> LinearCostEfficientDriftContinuationModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearCostEfficientDriftContinuationModel(
            fold_id,
    ...
    cost_efficient_drift_continuation_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearCostEfficientDriftContinuationModel(
        fold_id=fold_id,
        ...
        cost_efficient_drift_continuation_failure_rate=cost_efficient_drift_continuation_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearCostEfficientDriftContinuationModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0111-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_cost_efficient_drift_continuation_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0111_r0001_intraday_cost_efficient_drift_continuation",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0111_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0111_r0001_cost_efficient_drift_continuation_summary.json",
        ],
        ...
        "conditional_profiles": {
            "cost_efficient_drift_continuation_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0111 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0111_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0111 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "cost_efficient_drift_continuation_lookback_bars": COST_EFFICIENT_DRIFT_CONTINUATION_LOOKBACK_BARS,
        "cost_efficient_drift_continuation_recent_bars": COST_EFFICIENT_DRIFT_CONTINUATION_RECENT_BARS,
        "cost_efficient_drift_continuation_prior_bars": COST_EFFICIENT_DRIFT_CONTINUATION_PRIOR_BARS,
        "min_drift_efficiency": MIN_DRIFT_EFFICIENCY,
        "min_cost_efficient_drift_continuation_activity": MIN_COST_EFFICIENT_DRIFT_CONTINUATION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_cost_efficient_drift_continuation_model_summary(model: LinearCostEfficientDriftContinuationModel) -> dict[str, object]:
    return {
        ...
        "cost_efficient_drift_continuation_failure_rate": rounded(model.cost_efficient_drift_continuation_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearCostEfficientDriftContinuationModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_cost_efficient_drift_continuation_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "cost_efficient_drift_continuation_profile": profiles["cost_efficient_drift_continuation_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0111_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0111_r0001_proxy_trades.csv"
    evidence["cost_efficient_drift_continuation_summary"] = "artifacts/c0111_r0001_cost_efficient_drift_continuation_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0111_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0111_r0001_proxy_trades.csv",
        "artifacts/c0111_r0001_cost_efficient_drift_continuation_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0111_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0111 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0111_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0111_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0111_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0111_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0111_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0111_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0111_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0111_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0111_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0111 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0111 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0111_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0111_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0111_r0001_intraday_cost_efficient_drift_continuation_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0111_r0001_proxy_records_cost_efficient_drift_continuation_reference_surface
  ...
  - next_work_is_produce_c0111_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0111_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0111-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0111_r0001_intraday_cost_efficient_drift_continuation",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0111-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0111-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0111-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0111_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0111_r0001_intraday_cost_efficient_drift_continuation",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0111-R0001-COST-EFFICIENT-DRIFT-CONTINUATION-SUMMARY",
            "artifact_role": "cost_efficient_drift_continuation_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0111_r0001_cost_efficient_drift_continuation_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0111_r0001_intraday_cost_efficient_drift_continuation",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0111_r0001_mt5_logic_parity_evidence",
        }
    ]
