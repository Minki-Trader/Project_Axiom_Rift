"""C0070 R0001 proxy evidence for intraday volatility signature transition."""
...
CAMPAIGN_ID = "C0070"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0070_intraday_volatility_signature_transition_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0070_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0070_r0001_vst_summary.json"
...
FEATURE_NAMES = (
    "short_range_transition",
    "medium_range_transition",
    "range_acceleration",
    "body_share_transition",
    "wick_imbalance_pressure",
    "shock_digestion_score",
    "volatility_alignment_score",
    "participation_confirmation",
    "prior_day_volatility_context",
    "session_maturity_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_volatility_signature_transition_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_volatility_signature_transition_state"
SELECTION_RULE = "top_fold_local_volatility_signature_transition_scores_per_active_day"
...
MIN_DAY_BARS = 30
VOLATILITY_SIGNATURE_TRANSITION_LOOKBACK_BARS = 20
VOLATILITY_SIGNATURE_TRANSITION_SHORT_LOOKBACK_BARS = 6
MIN_VOLATILITY_SIGNATURE_TRANSITION_ACTIVITY = 0.10
...
class LinearVolatilitySignatureTransitionModel:
    fold_id: str
    ...
    volatility_signature_transition_failure_rate: float
...
def run_c0070_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_volatility_signature_transition_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_volatility_signature_transition_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        VOLATILITY_SIGNATURE_TRANSITION_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    transition = volatility_signature_transition_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        transition["short_range_transition"],
        transition["medium_range_transition"],
        transition["range_acceleration"],
        transition["body_share_transition"],
        transition["wick_imbalance_pressure"],
        transition["shock_digestion_score"],
        transition["volatility_alignment_score"],
        transition["participation_confirmation"],
        transition["prior_day_volatility_context"],
        transition["session_maturity_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        short_range_transition_bucket(transition["short_range_transition"]),
        medium_range_transition_bucket(transition["medium_range_transition"]),
        range_acceleration_bucket(transition["range_acceleration"]),
        body_share_transition_bucket(transition["body_share_transition"]),
        wick_imbalance_bucket(transition["wick_imbalance_pressure"]),
        shock_digestion_bucket(transition["shock_digestion_score"]),
        volatility_alignment_bucket(transition["volatility_alignment_score"]),
        participation_confirmation_bucket(transition["participation_confirmation"]),
        prior_day_volatility_bucket(transition["prior_day_volatility_context"]),
        session_pressure_bucket(transition["session_maturity_pressure"]),
...
def volatility_signature_transition_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    window_start = index - VOLATILITY_SIGNATURE_TRANSITION_LOOKBACK_BARS + 1
    short_start = index - VOLATILITY_SIGNATURE_TRANSITION_SHORT_LOOKBACK_BARS + 1
    previous_short_start = index - VOLATILITY_SIGNATURE_TRANSITION_SHORT_LOOKBACK_BARS
    ...
    previous_short_window = bars[previous_short_start:index]
    if len(short_window) < 3 or len(medium_window) < 8 or len(previous_short_window) < 3:
        return {"event_count": 0.0}
    signal_range = max(signal_bar.high - signal_bar.low, 1e-9)
    short_ranges = [max(item.high - item.low, 0.0) for item in short_window]
    medium_ranges = [max(item.high - item.low, 0.0) for item in medium_window]
    previous_short_ranges = [max(item.high - item.low, 0.0) for item in previous_short_window]
    short_mean_range = base.mean(short_ranges) or average_range
    medium_mean_range = base.mean(medium_ranges) or average_range
    previous_short_mean_range = base.mean(previous_short_ranges) or average_range
    short_range_transition = (short_mean_range - previous_short_mean_range) / average_range
    medium_range_transition = (short_mean_range - medium_mean_range) / average_range
    range_acceleration = short_range_transition - medium_range_transition
    current_body_share = abs(signal_bar.close - signal_bar.open) / signal_range
    previous_body_shares = [
        abs(item.close - item.open) / max(item.high - item.low, 1e-9)
        for item in previous_short_window
    ]
    previous_body_share = base.mean(previous_body_shares) or 0.0
    body_share_transition = current_body_share - previous_body_share
    upper_wick_sum = 0.0
    lower_wick_sum = 0.0
    for item in short_window:
        upper_wick, lower_wick = wick_lengths(item)
        upper_wick_sum += upper_wick
        lower_wick_sum += lower_wick
    wick_total = max(upper_wick_sum + lower_wick_sum, 1e-9)
    if direction > 0:
        wick_imbalance_pressure = (lower_wick_sum - upper_wick_sum) / wick_total
    else:
        wick_imbalance_pressure = (upper_wick_sum - lower_wick_sum) / wick_total
    signal_body_pressure = direction * (signal_bar.close - signal_bar.open) / average_range
    medium_direction_pressure = direction * (signal_bar.close - medium_window[0].open) / average_range
    session_direction_pressure = direction * (signal_bar.close - context.day_open) / average_range
    positive_components = sum(
        1
        for value in (
            signal_body_pressure,
            medium_direction_pressure,
            session_direction_pressure,
        )
        if value > 0.0
    )
    volatility_alignment_score = positive_components / 3.0
    shock_level = max(0.0, (short_mean_range / average_range) - 1.0)
    shock_digestion_score = shock_level * max(0.0, signal_body_pressure) + max(0.0, -range_acceleration)
    volume_window = volumes[short_start : index + 1]
    directional_volume = 0.0
    opposing_volume = 0.0
    for item, volume in zip(short_window, volume_window):
        directional_body = direction * (item.close - item.open)
        if directional_body > 0:
            directional_volume += volume
        elif directional_body < 0:
            opposing_volume += volume
    expected_recent_volume = prior_volume_per_bar * len(short_window)
    participation_confirmation = (directional_volume - opposing_volume) / max(expected_recent_volume, 1e-9)
    if context.prior_day_range is None or context.prior_day_range <= 0:
        prior_day_volatility_context = 0.0
    else:
        prior_day_volatility_context = context.prior_day_range / max(average_range * 48.0, 1e-9)
    signature_pressure = max(
        abs(short_range_transition),
        abs(medium_range_transition),
        abs(range_acceleration),
        abs(body_share_transition),
        abs(wick_imbalance_pressure),
        shock_digestion_score,
        volatility_alignment_score,
        abs(participation_confirmation),
        prior_day_volatility_context,
    )
    session_maturity_pressure = bounded_session_progress(signal_bar.time) * max(
        signature_pressure,
        max(signal_body_pressure, 0.0),
        max(medium_direction_pressure, 0.0),
        max(session_direction_pressure, 0.0),
    )
    activity = max(
        abs(short_range_transition),
        abs(medium_range_transition),
    ...
    if activity < MIN_VOLATILITY_SIGNATURE_TRANSITION_ACTIVITY:
        return {"event_count": 0.0}
    if (
        abs(short_range_transition) < 0.04
        and abs(body_share_transition) < 0.03
        and abs(wick_imbalance_pressure) < 0.06
        and volatility_alignment_score < 0.34
    ):
        return {"event_count": 0.0}
    return {
        "short_range_transition": bounded(short_range_transition, -5.0, 5.0),
        "medium_range_transition": bounded(medium_range_transition, -5.0, 5.0),
        "range_acceleration": bounded(range_acceleration, -5.0, 5.0),
        "body_share_transition": bounded(body_share_transition, -1.0, 1.0),
        "wick_imbalance_pressure": bounded(wick_imbalance_pressure, -1.0, 1.0),
        "shock_digestion_score": bounded(shock_digestion_score, 0.0, 6.0),
        "volatility_alignment_score": bounded(volatility_alignment_score, 0.0, 1.0),
        "participation_confirmation": bounded(participation_confirmation, -4.0, 4.0),
        "prior_day_volatility_context": bounded(prior_day_volatility_context, 0.0, 4.0),
...
def short_range_transition_bucket(value: float) -> str:
    if value < -0.18:
        return "short_range_contracting"
    if value < 0.18:
        return "short_range_flat"
    return "short_range_expanding"


def medium_range_transition_bucket(value: float) -> str:
    if value < -0.22:
        return "medium_range_below_context"
    if value < 0.22:
        return "medium_range_near_context"
    return "medium_range_above_context"


def range_acceleration_bucket(value: float) -> str:
    if value < -0.16:
        return "range_decelerating"
    if value < 0.16:
        return "range_acceleration_flat"
    return "range_accelerating"


def body_share_transition_bucket(value: float) -> str:
    if value < -0.10:
        return "body_share_fading"
    if value < 0.10:
        return "body_share_stable"
    return "body_share_expanding"


def wick_imbalance_bucket(value: float) -> str:
    if value < -0.16:
        return "wick_imbalance_adverse"
    if value < 0.16:
        return "wick_imbalance_mixed"
    return "wick_imbalance_favorable"


def shock_digestion_bucket(value: float) -> str:
    if value < 0.08:
        return "shock_digestion_low"
    if value < 0.30:
        return "shock_digestion_mid"
    return "shock_digestion_high"


def volatility_alignment_bucket(value: float) -> str:
    if value < 0.34:
        return "vol_alignment_low"
    if value < 0.67:
        return "vol_alignment_partial"
    return "vol_alignment_full"


def participation_confirmation_bucket(value: float) -> str:
    if value < -0.18:
        return "participation_against"
    if value < 0.22:
        return "participation_mixed"
    return "participation_with"


def prior_day_volatility_bucket(value: float) -> str:
    if value < 0.55:
        return "prior_day_vol_quiet"
    if value < 1.45:
        return "prior_day_vol_normal"
    return "prior_day_vol_hot"
...
def candidate_label(
    ...
    metrics = volatility_signature_transition_metrics(
        bars,
        volumes,
    ...
    if metrics["event_count"] <= 0:
        return None
    short_reward = 0.12 if metrics["short_range_transition"] > 0.08 and terminal > 0 else 0.0
    medium_reward = 0.12 if metrics["medium_range_transition"] > 0.06 and terminal > 0 else 0.0
    acceleration_reward = 0.14 if metrics["range_acceleration"] > 0.04 and terminal > 0 else 0.0
    body_share_reward = 0.12 if metrics["body_share_transition"] > 0.06 and terminal > 0 else 0.0
    wick_reward = 0.10 if metrics["wick_imbalance_pressure"] > 0.08 and terminal > 0 else 0.0
    shock_reward = 0.12 if metrics["shock_digestion_score"] > 0.08 and terminal > 0 else 0.0
    alignment_reward = 0.18 if metrics["volatility_alignment_score"] >= 0.67 and terminal > 0 else 0.0
    participation_reward = 0.10 if metrics["participation_confirmation"] > 0.15 and terminal > 0 else 0.0
    prior_volatility_reward = (
        0.06 if 0.45 <= metrics["prior_day_volatility_context"] <= 2.40 and terminal > 0 else 0.0
    )
    transition_mismatch_penalty = 0.20 if metrics["short_range_transition"] < -0.08 and terminal <= 0 else 0.0
    weak_body_penalty = 0.18 if metrics["body_share_transition"] < -0.06 and terminal <= 0 else 0.0
    adverse_wick_penalty = 0.16 if metrics["wick_imbalance_pressure"] < -0.08 and terminal <= 0 else 0.0
    weak_alignment_penalty = 0.16 if metrics["volatility_alignment_score"] < 0.34 and terminal <= 0 else 0.0
    participation_penalty = 0.14 if metrics["participation_confirmation"] < -0.12 and terminal <= 0 else 0.0
    ...
    volatility_signature_transition_path_quality = (0.44 * terminal + 0.36 * mfe - 0.72 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + volatility_signature_transition_path_quality
        + short_reward
        + medium_reward
        + acceleration_reward
        + body_share_reward
...
def fit_linear_volatility_signature_transition_model(candidates: list[base.Candidate], fold_id: str) -> LinearVolatilitySignatureTransitionModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearVolatilitySignatureTransitionModel(
            fold_id,
    ...
    volatility_signature_transition_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearVolatilitySignatureTransitionModel(
        fold_id=fold_id,
        ...
        volatility_signature_transition_failure_rate=volatility_signature_transition_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearVolatilitySignatureTransitionModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0070-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_volatility_signature_transition_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0070_r0001_intraday_volatility_signature_transition",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0070_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0070_r0001_vst_summary.json",
        ],
        ...
        "conditional_profiles": {
            "volatility_signature_transition_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0070 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0070_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0070 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "volatility_signature_transition_lookback_bars": VOLATILITY_SIGNATURE_TRANSITION_LOOKBACK_BARS,
        "volatility_signature_transition_short_lookback_bars": VOLATILITY_SIGNATURE_TRANSITION_SHORT_LOOKBACK_BARS,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_volatility_signature_transition_model_summary(model: LinearVolatilitySignatureTransitionModel) -> dict[str, object]:
    return {
        ...
        "volatility_signature_transition_failure_rate": rounded(model.volatility_signature_transition_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearVolatilitySignatureTransitionModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_volatility_signature_transition_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "volatility_signature_transition_profile": profiles["volatility_signature_transition_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0070_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0070_r0001_proxy_trades.csv"
    evidence["volatility_signature_transition_summary"] = "artifacts/c0070_r0001_vst_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0070_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0070_r0001_proxy_trades.csv",
        "artifacts/c0070_r0001_vst_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0070_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0070 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0070_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0070_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0070_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0070_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0070_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0070_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0070_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0070_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0070_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0070 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0070 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260706_produce_c0070_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0070_r0001_proxy_evidence
  refines:
  - dec_20260706_open_c0070_r0001_intraday_volatility_signature_transition_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0070_r0001_proxy_records_volatility_signature_transition_reference_surface
  ...
  - next_work_is_produce_c0070_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260706_produce_c0070_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0070-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0070_r0001_intraday_volatility_signature_transition",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0070-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0070-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0070-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0070_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0070_r0001_intraday_volatility_signature_transition",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0070-R0001-volatility-signature-transition-SUMMARY",
            "artifact_role": "volatility_signature_transition_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0070_r0001_vst_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0070_r0001_intraday_volatility_signature_transition",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0070_r0001_mt5_logic_parity_evidence",
        }
    ]
