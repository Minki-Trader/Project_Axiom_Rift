"""C0052 R0001 proxy evidence for intraday orderflow dislocation recovery."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0052_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0052_r0001_orderflow_dislocation_recovery_summary.json"
...
FEATURE_NAMES = (
    "directional_pressure_residual_current",
    "directional_pressure_residual_stack_12",
    "orderflow_dislocation_zscore_12",
    "residual_decay_slope_6",
    "directional_recovery_pressure_6",
    "adverse_residual_pressure_12",
    "volume_dryup_after_dislocation",
    "range_compression_12_vs_48",
    "directional_close_response_3",
    "participation_residual_confirmation",
    "spread_dislocation_stress",
    "session_progress",
)
MODEL_FAMILY = "fold_local_intraday_orderflow_dislocation_recovery_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_orderflow_dislocation_recovery_state"
SELECTION_RULE = "top_fold_local_orderflow_dislocation_recovery_scores_per_active_day"
...
class LinearOrderflowDislocationRecoveryModel:
    fold_id: str
    ...
    dislocation_failure_rate: float
...
def run_c0052_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_orderflow_dislocation_recovery_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_orderflow_dislocation_recovery_model_summary(model))
...
def candidate_state_and_features(
    ...
    if average_range is None or average_range <= 0 or short_average_range is None or short_average_range <= 0:
        return None, None
    bar = bars[index]
    _context = daily_contexts[index]
    orderflow = orderflow_dislocation_metrics(bars, volumes, volume_average, index, direction, average_range)
    if orderflow["dislocation_event_count"] <= 0:
        return None, None
    ...
    session_progress = bounded_session_progress(bar.time)
    if volume_ratio is None:
        return None, None
    participation_confirmation = bounded(orderflow["directional_pressure_residual_stack_12"] * volume_ratio, -4.0, 4.0)
    features = (
        orderflow["directional_pressure_residual_current"],
        orderflow["directional_pressure_residual_stack_12"],
        orderflow["orderflow_dislocation_zscore_12"],
        orderflow["residual_decay_slope_6"],
        orderflow["directional_recovery_pressure_6"],
        orderflow["adverse_residual_pressure_12"],
        orderflow["volume_dryup_after_dislocation"],
        compression,
        orderflow["directional_close_response_3"],
    ...
    dimensions = (
        residual_bucket(orderflow["directional_pressure_residual_current"]),
        residual_stack_bucket(orderflow["directional_pressure_residual_stack_12"]),
        dislocation_bucket(orderflow["orderflow_dislocation_zscore_12"]),
        residual_decay_bucket(orderflow["residual_decay_slope_6"]),
        compression_bucket(compression),
        momentum_bucket(orderflow["directional_close_response_3"]),
...
def orderflow_dislocation_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    volume_average: list[float | None],
    index: int,
    ...
    start = max(1, index - 12 + 1)
    residuals: list[float] = []
    directional_moves: list[float] = []
    volume_ratios: list[float] = []
    for pos in range(start, index + 1):
        prior_close = bars[pos - 1].close
        move = bars[pos].close - prior_close
        directional_move = direction * move / average_range
        volume_base = volume_average[pos] or volume_average[index] or 1.0
        participation = volumes[pos] / max(volume_base, 1.0)
        signed_flow = direction * price_sign(move) * participation
        body_share = abs(bars[pos].close - bars[pos].open) / max(bars[pos].high - bars[pos].low, 1e-9)
        residual = signed_flow * (0.45 + 0.55 * body_share) - directional_move
        residuals.append(residual)
        directional_moves.append(directional_move)
        volume_ratios.append(participation)
    current = residuals[-1]
    stack_12 = sum(residuals)
    mean_12 = base.mean(residuals) or 0.0
    std_12 = stddev(residuals, mean_12) or 1.0
    zscore = (current - mean_12) / std_12
    last3 = residuals[-3:]
    prev3 = residuals[-6:-3] or residuals[-3:]
    residual_decay = (sum(last3) / len(last3)) - (sum(prev3) / len(prev3))
    close_response = direction * (bars[index].close - bars[index - 3].close) / average_range
    recovery_pressure = close_response - max(0.0, -residual_decay)
    adverse_pressure = sum(max(0.0, -value) for value in residuals)
    large_residuals = [abs(value) for value in residuals if abs(value) >= 0.60]
    peak_pos = max(range(len(residuals)), key=lambda offset: abs(residuals[offset]))
    post_peak_volumes = volume_ratios[peak_pos:]
    current_participation = volume_ratios[-1] if volume_ratios else 1.0
    peak_participation = max(post_peak_volumes) if post_peak_volumes else current_participation
    volume_dryup = max(0.0, 1.0 - current_participation / max(peak_participation, 1e-9))
    return {
        "directional_pressure_residual_current": current,
        "directional_pressure_residual_stack_12": stack_12,
        "orderflow_dislocation_zscore_12": zscore,
        "residual_decay_slope_6": residual_decay,
        "directional_recovery_pressure_6": recovery_pressure,
        "adverse_residual_pressure_12": adverse_pressure,
        "volume_dryup_after_dislocation": volume_dryup,
        "directional_close_response_3": close_response,
        "dislocation_event_count": float(len(large_residuals)),
    }
...
def residual_bucket(value: float) -> str:
    if value < -0.60:
        return "residual_adverse"
    if value < 0.60:
        return "residual_neutral"
    return "residual_favorable"


def residual_stack_bucket(value: float) -> str:
    if value < -1.50:
        return "stack_adverse"
    if value < 1.50:
        return "stack_neutral"
    return "stack_favorable"


def dislocation_bucket(value: float) -> str:
    if value < -0.80:
        return "dislocation_negative"
    if value < 0.80:
        return "dislocation_mid"
    return "dislocation_positive"


def residual_decay_bucket(value: float) -> str:
    if value < -0.35:
        return "residual_decay"
    if value < 0.35:
        return "residual_stable"
    return "residual_building"
...
def candidate_label(
    ...
    future_range = max(bar.high for bar in path) - min(bar.low for bar in path)
    recent_response = direction * (bars[index].close - bars[index - 3].close) / average_range
    recovery_reward = 0.15 if recent_response > 0.20 and terminal > 0 else 0.0
    unresolved_dislocation_penalty = 0.20 if recent_response < -0.20 and mae > mfe else 0.0
    stall_penalty = 0.25 if future_range < 0.65 * average_range else 0.0
    adverse_expansion_penalty = max(0.0, mae - mfe) / average_range * 0.35
    residual_path_quality = (0.30 * terminal + 0.30 * mfe - 0.60 * mae + 0.15 * future_range) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + residual_path_quality
        + recovery_reward
        - unresolved_dislocation_penalty
        - stall_penalty
...
def fit_linear_orderflow_dislocation_recovery_model(candidates: list[base.Candidate], fold_id: str) -> LinearOrderflowDislocationRecoveryModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearOrderflowDislocationRecoveryModel(fold_id, tuple(0.0 for _ in FEATURE_NAMES), tuple(1.0 for _ in FEATURE_NAMES), tuple(0.0 for _ in FEATURE_NAMES), 0, 0.0, 0.0, 0.0)
    ...
    dislocation_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearOrderflowDislocationRecoveryModel(
        fold_id=fold_id,
        ...
        dislocation_failure_rate=dislocation_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearOrderflowDislocationRecoveryModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_orderflow_dislocation_recovery_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0052_r0001_intraday_orderflow_dislocation_recovery",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0052_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0052_r0001_orderflow_dislocation_recovery_summary.json",
        ],
        ...
        "conditional_profiles": {
            "orderflow_dislocation_recovery_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0052 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0052_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0052 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def linear_orderflow_dislocation_recovery_model_summary(model: LinearOrderflowDislocationRecoveryModel) -> dict[str, object]:
    return {
        ...
        "dislocation_failure_rate": rounded(model.dislocation_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearOrderflowDislocationRecoveryModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_orderflow_dislocation_recovery_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "orderflow_dislocation_recovery_profile": profiles["orderflow_dislocation_recovery_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0052_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_trade_artifact"] = "artifacts/c0052_r0001_proxy_trades.csv"
    evidence["orderflow_dislocation_recovery_summary"] = "artifacts/c0052_r0001_orderflow_dislocation_recovery_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0052_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0052_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0052 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0052_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0052_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0052_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0052_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0052_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0052_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0052_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0052_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0052_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0052 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0052 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260706_produce_c0052_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0052_r0001_proxy_evidence
  refines:
  - dec_20260706_open_c0052_r0001_intraday_orderflow_dislocation_recovery_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0052_r0001_proxy_records_orderflow_dislocation_recovery_reference_surface
  ...
  - next_work_is_produce_c0052_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260706_produce_c0052_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0052-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0052_r0001_intraday_orderflow_dislocation_recovery",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0052-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0052-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0052-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0052_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0052_r0001_intraday_orderflow_dislocation_recovery",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0052-R0001-GAP-CONTINUITY-DECAY-SUMMARY",
            "artifact_role": "orderflow_dislocation_recovery_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0052_r0001_orderflow_dislocation_recovery_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0052_r0001_intraday_orderflow_dislocation_recovery",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0052_r0001_mt5_logic_parity_evidence",
        }
    ]
