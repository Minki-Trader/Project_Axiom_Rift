"""C0051 R0001 proxy evidence for intraday gap continuity decay."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0051_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0051_r0001_gap_continuity_decay_summary.json"
...
FEATURE_NAMES = (
    "directional_current_gap_atr",
    "directional_gap_stack_12_atr",
    "favorable_unfilled_gap_ratio_12",
    "favorable_gap_fill_latency_12",
    "adverse_gap_pressure_12_atr",
    "gap_decay_slope_12",
    "directional_gap_anchor_distance_atr",
    "range_compression_12_vs_48",
    "directional_momentum_after_gap_6",
    "participation_gap_confirmation",
    "spread_gap_stress",
    "session_progress",
)
MODEL_FAMILY = "fold_local_intraday_gap_continuity_decay_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_gap_continuity_decay_state"
SELECTION_RULE = "top_fold_local_gap_continuity_decay_scores_per_active_day"
...
class LinearGapContinuityDecayModel:
    fold_id: str
...
def run_c0051_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_gap_continuity_decay_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_gap_continuity_decay_model_summary(model))
...
def candidate_state_and_features(
    ...
    gap_metrics = recent_gap_metrics(bars, index, direction, average_range, lookback=12)
    if gap_metrics["gap_event_count"] <= 0:
        return None, None
    compression = short_average_range / average_range
    directional_momentum = direction * (bar.close - bars[index - 6].close) / short_average_range
    ...
    if volume_ratio is None:
        return None, None
    participation_confirmation = bounded(gap_metrics["directional_gap_stack_12_atr"] * volume_ratio, -3.0, 3.0)
    features = (
        gap_metrics["directional_current_gap_atr"],
        gap_metrics["directional_gap_stack_12_atr"],
        gap_metrics["favorable_unfilled_gap_ratio_12"],
        gap_metrics["favorable_gap_fill_latency_12"],
        gap_metrics["adverse_gap_pressure_12_atr"],
        gap_metrics["gap_decay_slope_12"],
        gap_metrics["directional_gap_anchor_distance_atr"],
        compression,
    ...
    dimensions = (
        current_gap_bucket(gap_metrics["directional_current_gap_atr"]),
        gap_stack_bucket(gap_metrics["directional_gap_stack_12_atr"]),
        unfilled_gap_bucket(gap_metrics["favorable_unfilled_gap_ratio_12"]),
        gap_decay_bucket(gap_metrics["gap_decay_slope_12"]),
        compression_bucket(compression),
        momentum_bucket(directional_momentum),
...
def recent_gap_metrics(
    ...
    gaps = [(pos, bars[pos].open - bars[pos - 1].close) for pos in range(start, index + 1)]
    material = [(pos, gap) for pos, gap in gaps if abs(gap) >= 0.02 * average_range]
    directional_values = [direction * gap for _, gap in gaps]
    recent_values = directional_values[-3:]
    prior_values = directional_values[:-3] or directional_values[-3:]
    favorable_total = 0
    favorable_unfilled = 0
    latency_sum = 0.0
    adverse_pressure = 0.0
    latest_favorable_anchor = bars[index - 1].close
    for pos, raw_gap in material:
        signed_gap = direction * raw_gap
        if signed_gap > 0:
            favorable_total += 1
            anchor = bars[pos - 1].close
            latest_favorable_anchor = anchor
            if direction > 0:
                filled = min(item.low for item in bars[pos : index + 1]) <= anchor
            else:
                filled = max(item.high for item in bars[pos : index + 1]) >= anchor
            if not filled:
                favorable_unfilled += 1
                latency_sum += (index - pos + 1) / max(float(lookback), 1.0)
        else:
            adverse_pressure += abs(raw_gap)
    unfilled_ratio = favorable_unfilled / favorable_total if favorable_total else 0.0
    latency = latency_sum / favorable_total if favorable_total else 0.0
    recent_avg = sum(recent_values) / max(float(len(recent_values)), 1.0)
    prior_avg = sum(prior_values) / max(float(len(prior_values)), 1.0)
    return {
        "directional_current_gap_atr": directional_values[-1] / average_range,
        "directional_gap_stack_12_atr": sum(directional_values) / average_range,
        "favorable_unfilled_gap_ratio_12": unfilled_ratio,
        "favorable_gap_fill_latency_12": latency,
        "adverse_gap_pressure_12_atr": adverse_pressure / average_range,
        "gap_decay_slope_12": (recent_avg - prior_avg) / average_range,
        "directional_gap_anchor_distance_atr": direction * (bars[index].close - latest_favorable_anchor) / average_range,
        "gap_event_count": float(len(material)),
    }
...
def current_gap_bucket(value: float) -> str:
    if value < -0.08:
        return "current_gap_against"
    if value < 0.08:
        return "current_gap_flat"
    return "current_gap_with"


def gap_stack_bucket(value: float) -> str:
    if value < -0.25:
        return "gap_stack_against"
    if value < 0.25:
        return "gap_stack_neutral"
    return "gap_stack_with"


def unfilled_gap_bucket(value: float) -> str:
    if value <= 0.0:
        return "no_unfilled_gap"
    if value < 0.50:
        return "partial_unfilled_gap"
    return "persistent_unfilled_gap"


def gap_decay_bucket(value: float) -> str:
    if value < -0.08:
        return "gap_decay"
    if value < 0.08:
        return "gap_stable"
    return "gap_reaccelerating"
...
def candidate_label(
    ...
    signal_gap = bars[index].open - bars[index - 1].close if index > 0 else 0.0
    gap_fill_penalty = 0.0
    gap_hold_reward = 0.0
    if direction * signal_gap > 0.02 * average_range:
        anchor = bars[index - 1].close
        if direction > 0:
            filled = min(bar.low for bar in path) <= anchor
        else:
            filled = max(bar.high for bar in path) >= anchor
        gap_fill_penalty = 0.35 if filled else 0.0
        gap_hold_reward = 0.12 if not filled and terminal > 0 else 0.0
    ...
    gap_path_quality = (0.30 * terminal + 0.30 * mfe - 0.60 * mae + 0.15 * future_range) / average_range
...
def fit_linear_gap_continuity_decay_model(candidates: list[base.Candidate], fold_id: str) -> LinearGapContinuityDecayModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearGapContinuityDecayModel(fold_id, tuple(0.0 for _ in FEATURE_NAMES), tuple(1.0 for _ in FEATURE_NAMES), tuple(0.0 for _ in FEATURE_NAMES), 0, 0.0, 0.0, 0.0)
    ...
    gap_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearGapContinuityDecayModel(
        fold_id=fold_id,
        ...
        gap_failure_rate=gap_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearGapContinuityDecayModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_gap_continuity_decay_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0051_r0001_intraday_gap_continuity_decay",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0051_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0051_r0001_gap_continuity_decay_summary.json",
        ],
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0051 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0051_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0051 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def linear_gap_continuity_decay_model_summary(model: LinearGapContinuityDecayModel) -> dict[str, object]:
    return {
        ...
        "gap_failure_rate": rounded(model.gap_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearGapContinuityDecayModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_gap_continuity_decay_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "gap_continuity_decay_profile": profiles["gap_continuity_decay_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0051_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_trade_artifact"] = "artifacts/c0051_r0001_proxy_trades.csv"
    evidence["gap_continuity_decay_summary"] = "artifacts/c0051_r0001_gap_continuity_decay_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0051_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0051_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0051 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0051_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0051_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0051_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0051_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0051_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0051_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0051_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0051_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0051_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0051 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0051 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260706_produce_c0051_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0051_r0001_proxy_evidence
  refines:
  - dec_20260706_open_c0051_r0001_intraday_gap_continuity_decay_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0051_r0001_proxy_records_gap_continuity_decay_reference_surface
  ...
  - next_work_is_produce_c0051_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260706_produce_c0051_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0051-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0051_r0001_intraday_gap_continuity_decay",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0051-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0051-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0051-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0051_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0051_r0001_intraday_gap_continuity_decay",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0051-R0001-GAP-CONTINUITY-DECAY-SUMMARY",
            "artifact_role": "gap_continuity_decay_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0051_r0001_gap_continuity_decay_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0051_r0001_intraday_gap_continuity_decay",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0051_r0001_mt5_logic_parity_evidence",
        }
    ]
