"""C0098 R0001 proxy evidence for intraday range variance displacement decoupling reversal."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0098_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0098_r0001_range_variance_displacement_decoupling_reversal_summary.json"
...
FEATURE_NAMES = (
    "variance_displacement_decoupling",
    "adverse_displacement_pressure",
    "range_energy_excess",
    "path_churn_density",
    "close_recenter_pressure",
    "counter_settlement_confirmation",
    "participation_dislocation",
    "vwap_variance_reversion_pressure",
    "session_phase_maturity",
    "prior_day_energy_context",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_range_variance_displacement_decoupling_reversal_rank"
LABEL_SHAPE = "target_first_reversal_quality_conditioned_on_range_variance_displacement_decoupling_reversal"
SELECTION_RULE = "top_fold_local_range_variance_displacement_decoupling_reversal_scores_per_active_day"
...
MIN_DAY_BARS = 30
RANGE_VARIANCE_DISPLACEMENT_DECOUPLING_LOOKBACK_BARS = 28
RANGE_VARIANCE_DISPLACEMENT_DECOUPLING_STACK_BARS = 10
MIN_RANGE_VARIANCE_DISPLACEMENT_DECOUPLING_ACTIVITY = 0.08
...
class LinearRangeVarianceDisplacementDecouplingReversalModel:
    fold_id: str
    ...
    range_variance_displacement_decoupling_reversal_failure_rate: float
...
def run_c0098_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_range_variance_displacement_decoupling_reversal_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_range_variance_displacement_decoupling_reversal_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        RANGE_VARIANCE_DISPLACEMENT_DECOUPLING_LOOKBACK_BARS,
        RANGE_VARIANCE_DISPLACEMENT_DECOUPLING_STACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = range_variance_displacement_decoupling_reversal_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        metrics["variance_displacement_decoupling"],
        metrics["adverse_displacement_pressure"],
        metrics["range_energy_excess"],
        metrics["path_churn_density"],
        metrics["close_recenter_pressure"],
        metrics["counter_settlement_confirmation"],
        metrics["participation_dislocation"],
        metrics["vwap_variance_reversion_pressure"],
    ...
    dimensions = (
        variance_decoupling_bucket(metrics["variance_displacement_decoupling"]),
        adverse_displacement_bucket(metrics["adverse_displacement_pressure"]),
        range_energy_excess_bucket(metrics["range_energy_excess"]),
        path_churn_bucket(metrics["path_churn_density"]),
        recenter_pressure_bucket(metrics["close_recenter_pressure"]),
        counter_settlement_bucket(metrics["counter_settlement_confirmation"]),
        participation_dislocation_bucket(metrics["participation_dislocation"]),
        vwap_variance_pressure_bucket(metrics["vwap_variance_reversion_pressure"]),
        session_phase_maturity_bucket(metrics["session_phase_maturity"]),
        prior_energy_context_bucket(metrics["prior_day_energy_context"]),
...
def range_variance_displacement_decoupling_reversal_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    stack_start = index - RANGE_VARIANCE_DISPLACEMENT_DECOUPLING_STACK_BARS
    lookback_start = index - RANGE_VARIANCE_DISPLACEMENT_DECOUPLING_LOOKBACK_BARS
    if stack_start < 0 or lookback_start < 0 or context.day_start_index >= index:
        return {"event_count": 0.0}
    stack_start = max(stack_start, context.day_start_index)
    lookback_start = max(lookback_start, context.day_start_index)
    stack_window = bars[stack_start:index]
    prior_window = bars[lookback_start:stack_start]
    if len(stack_window) < 5 or len(prior_window) < 6:
        return {"event_count": 0.0}
    average_volume = volume_average[index]
    if average_volume is None or average_volume <= 0:
        return {"event_count": 0.0}

    stack_ranges = [bar.high - bar.low for bar in stack_window]
    prior_ranges = [bar.high - bar.low for bar in prior_window]
    stack_range_mean = base.mean(stack_ranges) or average_range
    prior_range_mean = base.mean(prior_ranges) or average_range
    stack_high = max(bar.high for bar in stack_window)
    stack_low = min(bar.low for bar in stack_window)
    stack_mid = (stack_high + stack_low) / 2.0
    stack_closes = [bar.close for bar in stack_window]
    net_move = stack_closes[-1] - stack_window[0].open
    close_travel = abs(stack_closes[0] - stack_window[0].open) + sum(
        abs(stack_closes[pos] - stack_closes[pos - 1]) for pos in range(1, len(stack_closes))
    )
    range_travel = sum(stack_ranges)
    close_efficiency = abs(net_move) / max(range_travel, average_range * 0.25)
    range_energy = stack_range_mean / max(average_range, 1e-9)
    variance_displacement_decoupling = range_energy - close_efficiency
    adverse_displacement_pressure = -direction * net_move / max(average_range, 1e-9)
    range_energy_excess = (stack_range_mean / max(prior_range_mean, 1e-9)) - close_efficiency
    path_churn_density = close_travel / max(abs(net_move), average_range * 0.35)
    close_recenter_pressure = direction * (stack_mid - signal_bar.close) / max(average_range, 1e-9)
    counter_settlement_confirmation = directional_bar_settlement(signal_bar, direction)
    stack_volume_average = base.mean(volumes[stack_start:index]) or average_volume
    participation_dislocation = volumes[index] / max(stack_volume_average, 1e-9) - 1.0
    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    vwap_variance_reversion_pressure = direction * (session_vwap - signal_bar.close) / max(average_range, 1e-9)
    session_phase_maturity = bounded_session_progress(signal_bar.time)
    prior_day_energy_context = 0.0
    if context.prior_day_volume_per_bar is not None:
        prior_day_energy_context = context.prior_day_volume_per_bar / max(average_volume, 1e-9) - 1.0
    variance_reversal_pressure_raw = max(
        max(variance_displacement_decoupling - 0.22, 0.0) * 0.95,
        max(adverse_displacement_pressure, 0.0) * 0.80,
        max(range_energy_excess, 0.0) * 0.55,
        max(path_churn_density - 1.25, 0.0) * 0.16,
        max(close_recenter_pressure, 0.0) * 0.60,
        max(counter_settlement_confirmation - 0.50, 0.0) * 0.42,
        max(participation_dislocation, 0.0) * 0.16,
        max(vwap_variance_reversion_pressure, 0.0) * 0.20,
    )
    activity = max(variance_reversal_pressure_raw, variance_reversal_pressure_raw * session_phase_maturity)
    if activity < MIN_RANGE_VARIANCE_DISPLACEMENT_DECOUPLING_ACTIVITY:
        return {"event_count": 0.0}
    if variance_displacement_decoupling < 0.10 and adverse_displacement_pressure < 0.05:
        return {"event_count": 0.0}
    if path_churn_density < 1.18 and range_energy_excess < 0.05:
        return {"event_count": 0.0}
    if close_recenter_pressure < -0.35 and counter_settlement_confirmation < 0.58:
        return {"event_count": 0.0}
    return {
        "variance_displacement_decoupling": bounded(variance_displacement_decoupling, -3.0, 6.0),
        "adverse_displacement_pressure": bounded(adverse_displacement_pressure, -6.0, 6.0),
        "range_energy_excess": bounded(range_energy_excess, -3.0, 6.0),
        "path_churn_density": bounded(path_churn_density, 0.0, 12.0),
        "close_recenter_pressure": bounded(close_recenter_pressure, -6.0, 6.0),
        "counter_settlement_confirmation": bounded(counter_settlement_confirmation, 0.0, 1.0),
        "participation_dislocation": bounded(participation_dislocation, -3.0, 6.0),
        "vwap_variance_reversion_pressure": bounded(vwap_variance_reversion_pressure, -5.0, 5.0),
        "session_phase_maturity": bounded(session_phase_maturity, 0.0, 1.0),
        "prior_day_energy_context": bounded(prior_day_energy_context, -3.0, 6.0),
        "stack_range_ratio": bounded(stack_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        "prior_range_ratio": bounded(prior_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        "variance_reversal_pressure": bounded(variance_reversal_pressure_raw, 0.0, 5.0),
        "event_count": 1.0,
    }
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    decoupling_reward = 0.14 if metrics["variance_displacement_decoupling"] > 0.35 and terminal > 0 else 0.0
    adverse_displacement_reward = 0.11 if metrics["adverse_displacement_pressure"] > 0.16 and terminal > 0 else 0.0
    range_energy_reward = 0.09 if metrics["range_energy_excess"] > 0.14 and terminal > 0 else 0.0
    churn_reward = 0.07 if metrics["path_churn_density"] > 1.80 and terminal > 0 else 0.0
    recenter_reward = 0.12 if metrics["close_recenter_pressure"] > 0.10 and terminal > 0 else 0.0
    settlement_reward = 0.09 if metrics["counter_settlement_confirmation"] > 0.62 and terminal > 0 else 0.0
    volume_reward = 0.06 if metrics["participation_dislocation"] > 0.10 and terminal > 0 else 0.0
    vwap_reward = 0.07 if metrics["vwap_variance_reversion_pressure"] > 0.08 and terminal > 0 else 0.0
    failed_reversal_penalty = 0.16 if metrics["close_recenter_pressure"] < -0.20 and terminal <= 0 else 0.0
    displacement_continuation_penalty = (
        0.12 if metrics["adverse_displacement_pressure"] > 0.34 and terminal <= 0 else 0.0
    )
    low_churn_penalty = 0.07 if metrics["path_churn_density"] < 1.25 and terminal <= 0 else 0.0
    weak_volume_penalty = 0.06 if metrics["participation_dislocation"] < -0.30 and terminal <= 0 else 0.0
    vwap_continuation_penalty = (
        0.08 if metrics["vwap_variance_reversion_pressure"] < -0.18 and terminal <= 0 else 0.0
    )
    ...
    range_variance_displacement_decoupling_reversal_path_quality = (0.54 * terminal + 0.30 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + range_variance_displacement_decoupling_reversal_path_quality
        + decoupling_reward
        + adverse_displacement_reward
        + range_energy_reward
        + churn_reward
        + recenter_reward
        + settlement_reward
        ...
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["stack_range_ratio"] - 2.20) * 0.05
    )


def fit_linear_range_variance_displacement_decoupling_reversal_model(candidates: list[base.Candidate], fold_id: str) -> LinearRangeVarianceDisplacementDecouplingReversalModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearRangeVarianceDisplacementDecouplingReversalModel(
            fold_id,
    ...
    range_variance_displacement_decoupling_reversal_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearRangeVarianceDisplacementDecouplingReversalModel(
        fold_id=fold_id,
        ...
        range_variance_displacement_decoupling_reversal_failure_rate=range_variance_displacement_decoupling_reversal_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearRangeVarianceDisplacementDecouplingReversalModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_range_variance_displacement_decoupling_reversal_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0098_r0001_intraday_range_variance_displacement_decoupling_reversal",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0098_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0098_r0001_range_variance_displacement_decoupling_reversal_summary.json",
        ],
        ...
        "conditional_profiles": {
            "range_variance_displacement_decoupling_reversal_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0098 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0098_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0098 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "range_variance_displacement_decoupling_reversal_lookback_bars": RANGE_VARIANCE_DISPLACEMENT_DECOUPLING_LOOKBACK_BARS,
        "range_variance_displacement_decoupling_reversal_stack_bars": RANGE_VARIANCE_DISPLACEMENT_DECOUPLING_STACK_BARS,
        "min_range_variance_displacement_decoupling_reversal_activity": MIN_RANGE_VARIANCE_DISPLACEMENT_DECOUPLING_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_range_variance_displacement_decoupling_reversal_model_summary(model: LinearRangeVarianceDisplacementDecouplingReversalModel) -> dict[str, object]:
    return {
        ...
        "range_variance_displacement_decoupling_reversal_failure_rate": rounded(model.range_variance_displacement_decoupling_reversal_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearRangeVarianceDisplacementDecouplingReversalModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_range_variance_displacement_decoupling_reversal_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "range_variance_displacement_decoupling_reversal_profile": profiles["range_variance_displacement_decoupling_reversal_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0098_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0098_r0001_proxy_trades.csv"
    evidence["range_variance_displacement_decoupling_reversal_summary"] = "artifacts/c0098_r0001_range_variance_displacement_decoupling_reversal_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0098_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0098_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0098 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0098_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0098_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0098_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0098_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0098_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0098_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0098_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0098_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0098_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0098 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0098 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0098_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0098_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0098_r0001_intraday_range_variance_displacement_decoupling_reversal_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0098_r0001_proxy_records_range_variance_displacement_decoupling_reversal_reference_surface
  ...
  - next_work_is_produce_c0098_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0098_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0098-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0098_r0001_intraday_range_variance_displacement_decoupling_reversal",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0098-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0098-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0098-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0098_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0098_r0001_intraday_range_variance_displacement_decoupling_reversal",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0098-R0001-range-variance-displacement-decoupling-reversal-SUMMARY",
            "artifact_role": "range_variance_displacement_decoupling_reversal_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0098_r0001_range_variance_displacement_decoupling_reversal_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0098_r0001_intraday_range_variance_displacement_decoupling_reversal",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0098_r0001_mt5_logic_parity_evidence",
        }
    ]
