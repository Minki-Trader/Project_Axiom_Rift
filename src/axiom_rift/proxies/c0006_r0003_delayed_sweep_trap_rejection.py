"""C0006 R0003 proxy evidence for delayed sweep trap rejection events."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0006_liquidity_sweep_reclaim_event_discovery" / "runs" / "R0003"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0006_r0003_proxy_trades.csv"
EVENT_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0006_r0003_delayed_sweep_trap_rejection_summary.json"
...
FEATURE_NAMES = (
    ...
    "confirmation_depth_over_range",
    "rejection_wick_fraction",
    "confirmation_delay_bars",
    "event_close_displacement_over_range",
    ...
    "prior_push_against_direction_36",
    "event_body_fraction",
...
MODEL_FAMILY = "fold_local_train_only_delayed_sweep_trap_rejection_event_utility"
LABEL_SHAPE = "delayed_trap_rejection_target_before_stop_plus_path_quality"
...
MIN_CONFIRMATION_OVER_BAR_RANGE = 0.025
CONFIRMATION_MIN_DELAY_BARS = 1
CONFIRMATION_MAX_DELAY_BARS = 3
SCORE_COMPONENT_NAMES = (
    "signed_feature_utility",
    "delayed_confirmation_bonus",
    "scope_bonus",
    "spread_penalty",
)
...
def run_c0006_r0003_proxy(write: bool = True) -> dict[str, object]:
...
def build_candidates(
    ...
    end_index = min(
        ...
        len(bars) - base.LABEL_HORIZON_BARS - CONFIRMATION_MAX_DELAY_BARS - 2,
    )
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            event = delayed_sweep_trap_rejection_features(bars, range_average, short_range_average, index, direction)
            if event is None:
                continue
            confirmation_index, scope, features = event
            if bars[confirmation_index].time > window.end:
                continue
            label = (
                delayed_sweep_trap_rejection_label(bars, range_average, confirmation_index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    direction=direction,
                    day=bars[confirmation_index].time.strftime("%Y-%m-%d"),
                    state_key=f"{side}|{scope}|delayed_sweep_trap_rejection",
                    features=features,
                    label=label,
                )
            )
...
def delayed_sweep_trap_rejection_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    index: int,
    ...
    confirmation_depth = 0.0
    confirmation_bar_range = 0.0
    for candidate_index in range(index + CONFIRMATION_MIN_DELAY_BARS, index + CONFIRMATION_MAX_DELAY_BARS + 1):
        confirm = bars[candidate_index]
        candidate_range = max(confirm.high - confirm.low, 0.0)
        if candidate_range <= 0:
            continue
        depth = max(confirm.close - level, 0.0) if direction > 0 else max(level - confirm.close, 0.0)
        if depth / candidate_range < MIN_CONFIRMATION_OVER_BAR_RANGE:
            continue
        confirmation_index = candidate_index
        confirmation_depth = depth
        confirmation_bar_range = candidate_range
        break
    if confirmation_index is None or confirmation_bar_range <= 0:
        return None
    confirmation = bars[confirmation_index]
    delay_bars = confirmation_index - index
    ...
    prior_push_36 = -direction * (bars[index - 1].close - bars[index - 36].close) / average_range
    body_fraction = direction * (bar.close - bar.open) / bar_range
    confirmation_body_fraction = direction * (confirmation.close - confirmation.open) / confirmation_bar_range
    close_location = (confirmation.close - confirmation.low) / confirmation_bar_range
    ...
    event_close_displacement = direction * (bar.close - level) / average_range
    mid = (local_high + local_low) / 2.0
    distance_from_mid = direction * (confirmation.close - mid) / max(local_high - local_low, 1e-9)
    minute_fraction = base.minute_of_day(confirmation.time) / (24.0 * 60.0)
    session_sin = math.sin(2.0 * math.pi * minute_fraction)
    session_cos = math.cos(2.0 * math.pi * minute_fraction)
    spread_over_range = confirmation.spread_points / average_range
    return confirmation_index, scope, (
        sweep_depth / average_range,
        confirmation_depth / average_range,
        rejection_wick / bar_range,
        float(delay_bars),
        event_close_displacement,
        confirmation_body_fraction,
...
def delayed_sweep_trap_rejection_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    index: int,
    direction: int,
...
def score_event_candidates(candidates: list[base.Candidate], model: EventUtilityModel) -> list[base.Candidate]:
    ...
    confirmation_depth = features[:, FEATURE_NAMES.index("confirmation_depth_over_range")]
    rejection_wick = features[:, FEATURE_NAMES.index("rejection_wick_fraction")]
    confirmation_body = features[:, FEATURE_NAMES.index("confirmation_body_fraction")]
    event_displacement = features[:, FEATURE_NAMES.index("event_close_displacement_over_range")]
    ...
    scores = (
        0.62 * signed_utility
        + 0.22 * np.tanh(2.2 * confirmation_depth)
        + 0.18 * np.tanh(1.6 * sweep_depth)
        + 0.13 * rejection_wick
        + 0.09 * confirmation_body
        - 0.05 * np.tanh(abs(event_displacement))
        + 0.05 * session_scope
        + 0.03 * wide_scope
        - 0.18 * spread_penalty
    )
...
def build_proxy_payload(
    ...
    payload["proxy_id"] = "PX-C0006-R0003"
    payload["proxy_engine"] = "axiom_rift.proxies.c0006_r0003_delayed_sweep_trap_rejection"
    payload["proxy_config_path"] = "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0003/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_delayed_sweep_trap_rejection_event_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0003/kpi/proxy.json",
        "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0003/artifacts/c0006_r0003_proxy_trades.csv",
        "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0003/artifacts/c0006_r0003_delayed_sweep_trap_rejection_summary.json",
    ]
    ...
    profiles["delayed_sweep_trap_rejection_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
            ...
            "min_confirmation_over_bar_range": MIN_CONFIRMATION_OVER_BAR_RANGE,
            "confirmation_min_delay_bars": CONFIRMATION_MIN_DELAY_BARS,
            "confirmation_max_delay_bars": CONFIRMATION_MAX_DELAY_BARS,
            "score_component_names": list(SCORE_COMPONENT_NAMES),
            "fold_models": fold_models,
            "state_distributions": state_distributions,
            "candidates_by_fold": candidates_by_fold,
            "selection_rule": "top_fold_local_delayed_sweep_trap_rejection_scores_per_active_day",
    ...
    config.update(
        {
            ...
            "confirmation_max_delay_bars": CONFIRMATION_MAX_DELAY_BARS,
            "score_component_names": list(SCORE_COMPONENT_NAMES),
            "score_interpretation": "higher_score_means_train_fold_event_utility_supports_delayed_trap_rejection_follow_through",
            "variant_boundary": "new_delayed_sweep_trap_rejection_event_grammar_not_analog_memory_state_bucket_score_cell_exit_shape_or_parameter_nudge",
        }
    )
    return config
...
def write_event_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_delayed_sweep_trap_rejection_summary_v1",
        "template": False,
        "work_unit_id": "C0006",
        "campaign_id": "C0006",
        "run_id": "R0003",
        ...
        "delayed_sweep_trap_rejection_profile": profiles["delayed_sweep_trap_rejection_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "delayed_sweep_trap_rejection_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0006-R0003-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0003/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0003/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0006-R0003-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0006-R0003-EVENT-UTILITY-SUMMARY",
                "delayed_sweep_trap_rejection_summary_artifact",
                "json",
                "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0003/artifacts/c0006_r0003_delayed_sweep_trap_rejection_summary.json",
                summary_hash,
                ["campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0003/kpi/proxy.json"],
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0006_r0003_mt5_logic_parity_evidence",
        }
    ]
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0006_r0003_delayed_sweep_trap_rejection",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0006_r0003_proxy_trades.csv"
    evidence["delayed_sweep_trap_rejection_summary"] = "artifacts/c0006_r0003_delayed_sweep_trap_rejection_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0006_r0003_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0006_r0003_proxy_trades.csv",
        "artifacts/c0006_r0003_delayed_sweep_trap_rejection_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0006 R0003 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0006_r0003_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0006_r0003_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    if "produce_c0006_r0003_proxy_evidence" not in completed:
        completed.append("produce_c0006_r0003_proxy_evidence")
    next_action = "produce_c0006_r0003_mt5_logic_parity_evidence"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0006_r0003_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0003/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0006_r0003_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0006",
            "R0003": "R0003",
            "c0004_r0003_fold_local_state_archetype": "c0006_r0003_delayed_sweep_trap_rejection",
            "c0004_r0003": "c0006_r0003",
