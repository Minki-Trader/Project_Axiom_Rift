"""C0132 R0001 proxy evidence for intraday inside outside bar expansion."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0132_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0132_r0001_inside_outside_bar_expansion_summary.json"
...
FEATURE_NAMES = (
    ...
    "expansion_direction_alignment",
    "mother_bar_boundary_pressure",
    "close_break_efficiency",
    "body_engulfment_commitment",
    "wick_clearance",
    "compression_depth",
    "volume_expansion_confirmation",
    "vwap_directional_room",
    "prior_day_range_alignment",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_inside_outside_bar_expansion_rank"
LABEL_SHAPE = "target_first_quality_after_inside_outside_bar_expansion"
SELECTION_RULE = "top_fold_local_inside_outside_bar_expansion_scores_per_active_day"
...
MIN_DAY_BARS = 18
INSIDE_OUTSIDE_BAR_EXPANSION_LOOKBACK_BARS = 44
INSIDE_OUTSIDE_BAR_EXPANSION_MICRO_BARS = 10
MIN_INSIDE_BAR_DENSITY = 0.18
MIN_EXPANSION_PRESSURE = 0.05
MIN_INSIDE_OUTSIDE_BAR_EXPANSION_ACTIVITY = 0.045
...
class LinearInsideOutsideBarExpansionModel:
    fold_id: str
    ...
    inside_outside_bar_expansion_failure_rate: float
...
def run_c0132_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_inside_outside_bar_expansion_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_inside_outside_bar_expansion_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        INSIDE_OUTSIDE_BAR_EXPANSION_LOOKBACK_BARS,
        INSIDE_OUTSIDE_BAR_EXPANSION_MICRO_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = inside_outside_bar_expansion_metrics(
    ...
    features = (
        metrics["inside_bar_density"],
        metrics["outside_bar_expansion"],
        metrics["expansion_direction_alignment"],
        metrics["mother_bar_boundary_pressure"],
        metrics["close_break_efficiency"],
        metrics["body_engulfment_commitment"],
        metrics["wick_clearance"],
        metrics["compression_depth"],
        metrics["volume_expansion_confirmation"],
        metrics["vwap_directional_room"],
        metrics["prior_day_range_alignment"],
        metrics["session_maturity"],
        metrics["spread_stress_inverse"],
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        inside_bar_density_bucket(metrics["inside_bar_density"]),
        outside_bar_expansion_bucket(metrics["outside_bar_expansion"]),
        expansion_alignment_bucket(metrics["expansion_direction_alignment"]),
        mother_bar_boundary_bucket(metrics["mother_bar_boundary_pressure"]),
        close_break_efficiency_bucket(metrics["close_break_efficiency"]),
        body_engulfment_bucket(metrics["body_engulfment_commitment"]),
        wick_clearance_bucket(metrics["wick_clearance"]),
        compression_depth_bucket(metrics["compression_depth"]),
        volume_expansion_bucket(metrics["volume_expansion_confirmation"]),
        vwap_directional_room_bucket(metrics["vwap_directional_room"]),
        prior_day_range_alignment_bucket(metrics["prior_day_range_alignment"]),
...
def inside_outside_bar_expansion_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    micro_start = max(index - INSIDE_OUTSIDE_BAR_EXPANSION_MICRO_BARS + 1, context.day_start_index)
    lookback_start = max(index - INSIDE_OUTSIDE_BAR_EXPANSION_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    for pos in range(1, len(lookback_window)):
        current = lookback_window[pos]
        previous = lookback_window[pos - 1]
        inside_flags.append(current.high <= previous.high and current.low >= previous.low)
        outside_flags.append(current.high >= previous.high and current.low <= previous.low)
    inside_bar_density = sum(1 for value in inside_flags if value) / max(len(inside_flags), 1)
    recent_inside_density = sum(
        1
        for pos in range(1, len(micro_window))
        if micro_window[pos].high <= micro_window[pos - 1].high
        and micro_window[pos].low >= micro_window[pos - 1].low
    ) / max(len(micro_window) - 1, 1)
    ...
    prior_micro_range = max(prior_micro_high - prior_micro_low, 1e-9)
    directional_break = (
        signal_bar.close - prior_micro_high if direction > 0 else prior_micro_low - signal_bar.close
    )
    wick_boundary_break = (
        signal_bar.high - prior_micro_high if direction > 0 else prior_micro_low - signal_bar.low
    )
    outside_bar_expansion = (
        (1.0 if signal_bar.high >= previous_bar.high and signal_bar.low <= previous_bar.low else 0.0)
        + max(wick_boundary_break, 0.0) / max(average_range * 1.5, 1e-9)
        + recent_outside_count * 0.08
    )
    directional_net = direction * (signal_bar.close - micro_window[0].close)
    directional_travel = sum(
        abs(micro_window[pos].close - micro_window[pos - 1].close)
        for pos in range(1, len(micro_window))
    )
    expansion_direction_alignment = directional_net / max(directional_travel, average_range, 1e-9)
    mother_bar_mid = (prior_micro_high + prior_micro_low) / 2.0
    mother_bar_boundary_pressure = direction * (signal_bar.close - mother_bar_mid) / max(prior_micro_range, average_range, 1e-9)
    close_break_efficiency = directional_break / max(signal_bar.high - signal_bar.low, average_range, 1e-9)
    body_engulfment_commitment = (
        directional_body_score(signal_bar, direction)
        + max(direction * (signal_bar.close - previous_bar.open), 0.0) / max(average_range * 2.0, 1e-9)
    )
    wick_clearance = adverse_wick_absorption(signal_bar, direction)
    compression_depth = 1.0 - base.mean(micro_ranges[:-1] or micro_ranges) / max(lookback_range_mean, 1e-9)
    if (
        max(inside_bar_density, recent_inside_density) < MIN_INSIDE_BAR_DENSITY
        and max(outside_bar_expansion, close_break_efficiency, mother_bar_boundary_pressure) < MIN_EXPANSION_PRESSURE
    ):
        return {"event_count": 0.0}

    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    vwap_directional_room = direction * (signal_bar.close - session_vwap) / max(average_range * 2.5, 1e-9)
    volume_expansion_confirmation = volumes[index] / max(lookback_volume_mean, average_volume, 1e-9) - 1.0
    prior_day_range_alignment = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_day_range_alignment = direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
    if context.prior_day_shadow_balance is not None:
        prior_day_range_alignment += direction * context.prior_day_shadow_balance * 0.05
    ...
    activity = (
        max(inside_bar_density, recent_inside_density) * 0.15
        + max(outside_bar_expansion, 0.0) * 0.15
        + max(expansion_direction_alignment, 0.0) * 0.12
        + max(mother_bar_boundary_pressure, 0.0) * 0.10
        + max(close_break_efficiency, 0.0) * 0.12
        + max(body_engulfment_commitment, 0.0) * 0.10
        + wick_clearance * 0.07
        + max(compression_depth, 0.0) * 0.08
        + max(volume_expansion_confirmation, -0.10) * 0.04
        + max(vwap_directional_room, -0.20) * 0.04
    )
    if activity < MIN_INSIDE_OUTSIDE_BAR_EXPANSION_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "inside_bar_density": bounded(inside_bar_density, 0.0, 1.0),
        "outside_bar_expansion": bounded(outside_bar_expansion, 0.0, 5.0),
        "expansion_direction_alignment": bounded(expansion_direction_alignment, -2.0, 2.0),
        "mother_bar_boundary_pressure": bounded(mother_bar_boundary_pressure, -3.0, 3.0),
        "close_break_efficiency": bounded(close_break_efficiency, -3.0, 3.0),
        "body_engulfment_commitment": bounded(body_engulfment_commitment, -1.0, 4.0),
        "wick_clearance": bounded(wick_clearance, 0.0, 1.0),
        "compression_depth": bounded(compression_depth, -2.0, 2.0),
        "volume_expansion_confirmation": bounded(volume_expansion_confirmation, -2.0, 8.0),
        "vwap_directional_room": bounded(vwap_directional_room, -4.0, 4.0),
        "prior_day_range_alignment": bounded(prior_day_range_alignment, -3.0, 3.0),
        ...
        "inside_outside_bar_expansion_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0132_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0132_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0132_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0132_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0132_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0132_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0132_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0132_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0132_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0132_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0132_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0132_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0132_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0132_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0132_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0132_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0132_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0132_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0132_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def c0132_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    inside_reward = 0.09 if metrics["inside_bar_density"] > 0.24 and terminal > 0 else 0.0
    outside_reward = 0.12 if metrics["outside_bar_expansion"] > 0.36 and terminal > 0 else 0.0
    alignment_reward = 0.11 if metrics["expansion_direction_alignment"] > 0.16 and terminal > 0 else 0.0
    boundary_reward = 0.10 if metrics["mother_bar_boundary_pressure"] > 0.18 and terminal > 0 else 0.0
    close_break_reward = 0.11 if metrics["close_break_efficiency"] > 0.08 and terminal > 0 else 0.0
    body_reward = 0.10 if metrics["body_engulfment_commitment"] > 0.22 and terminal > 0 else 0.0
    wick_reward = 0.07 if metrics["wick_clearance"] > 0.24 and terminal > 0 else 0.0
    compression_reward = 0.07 if metrics["compression_depth"] > 0.04 and terminal > 0 else 0.0
    volume_reward = 0.05 if -0.18 <= metrics["volume_expansion_confirmation"] <= 2.20 and terminal > 0 else 0.0
    vwap_room_reward = 0.04 if metrics["vwap_directional_room"] > -0.30 and terminal > 0 else 0.0
    prior_day_reward = 0.04 if metrics["prior_day_range_alignment"] > -0.35 and terminal > 0 else 0.0
    weak_inside_penalty = 0.06 if metrics["inside_bar_density"] < 0.12 and terminal <= 0 else 0.0
    weak_outside_penalty = 0.08 if metrics["outside_bar_expansion"] < 0.10 and terminal <= 0 else 0.0
    weak_alignment_penalty = 0.08 if metrics["expansion_direction_alignment"] < -0.08 and terminal <= 0 else 0.0
    weak_boundary_penalty = 0.08 if metrics["mother_bar_boundary_pressure"] < -0.05 and terminal <= 0 else 0.0
    weak_body_penalty = 0.07 if metrics["body_engulfment_commitment"] < -0.04 and terminal <= 0 else 0.0
    overheated_volume_penalty = 0.05 if metrics["volume_expansion_confirmation"] > 3.00 and terminal <= 0 else 0.0
    ...
    return (
        ...
        - max(0.0, metrics["volume_expansion_confirmation"] - 3.00) * 0.015
        - max(0.0, MIN_INSIDE_BAR_DENSITY - metrics["inside_bar_density"]) * 0.10
        - max(0.0, MIN_EXPANSION_PRESSURE - max(metrics["outside_bar_expansion"], metrics["close_break_efficiency"])) * 0.08
    )


def fit_linear_inside_outside_bar_expansion_model(candidates: list[base.Candidate], fold_id: str) -> LinearInsideOutsideBarExpansionModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearInsideOutsideBarExpansionModel(
            fold_id,
    ...
    inside_outside_bar_expansion_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearInsideOutsideBarExpansionModel(
        fold_id=fold_id,
        ...
        inside_outside_bar_expansion_failure_rate=inside_outside_bar_expansion_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearInsideOutsideBarExpansionModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_inside_outside_bar_expansion_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0132_r0001_intraday_inside_outside_bar_expansion",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0132_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0132_r0001_inside_outside_bar_expansion_summary.json",
        ],
        ...
        "conditional_profiles": {
            "inside_outside_bar_expansion_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0132 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0132_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0132 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "inside_outside_bar_expansion_lookback_bars": INSIDE_OUTSIDE_BAR_EXPANSION_LOOKBACK_BARS,
        "inside_outside_bar_expansion_micro_bars": INSIDE_OUTSIDE_BAR_EXPANSION_MICRO_BARS,
        "min_inside_bar_density": MIN_INSIDE_BAR_DENSITY,
        "min_expansion_pressure": MIN_EXPANSION_PRESSURE,
        "min_inside_outside_bar_expansion_activity": MIN_INSIDE_OUTSIDE_BAR_EXPANSION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_inside_outside_bar_expansion_model_summary(model: LinearInsideOutsideBarExpansionModel) -> dict[str, object]:
    return {
        ...
        "inside_outside_bar_expansion_failure_rate": rounded(model.inside_outside_bar_expansion_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearInsideOutsideBarExpansionModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_inside_outside_bar_expansion_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "inside_outside_bar_expansion_profile": profiles["inside_outside_bar_expansion_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0132_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0132_r0001_proxy_trades.csv"
    evidence["inside_outside_bar_expansion_summary"] = "artifacts/c0132_r0001_inside_outside_bar_expansion_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0132_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0132_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0132 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0132_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0132_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0132_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0132_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0132_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0132_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0132_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0132_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0132_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0132 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0132 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    block = f"""
- decision_id: dec_20260708_produce_c0132_r0001_proxy_evidence
  created_local_date: '{local_date}'
  status: active
  decision: produce_c0132_r0001_proxy_evidence
  refines:
  - dec_20260708_open_c0132_r0001_intraday_inside_outside_bar_expansion_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0132_r0001_proxy_records_inside_outside_bar_expansion_reference_surface
  ...
  - next_work_is_produce_c0132_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260708_produce_c0132_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0132-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0132_r0001_intraday_inside_outside_bar_expansion",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0132-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0132-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0132-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0132_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0132_r0001_intraday_inside_outside_bar_expansion",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0132-R0001-INSIDE-OUTSIDE-BAR-EXPANSION-SUMMARY",
            "artifact_role": "inside_outside_bar_expansion_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0132_r0001_inside_outside_bar_expansion_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0132_r0001_intraday_inside_outside_bar_expansion",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0132_r0001_mt5_logic_parity_evidence",
        }
    ]
