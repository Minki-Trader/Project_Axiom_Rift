"""C0002 R0003 proxy evidence for a train-only dual-direction cell score."""
...
class ScoreModel:
    ...
    cell_means: dict[tuple[int, tuple[int, ...]], float]
    cell_counts: dict[tuple[int, tuple[int, ...]], int]
    direction_means: dict[int, float]
...
def run_c0002_r0003_proxy(write: bool = True) -> dict[str, object]:
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        if average_range is None or average_range <= 0:
            continue
        features = candidate_features(bars, range_average, index)
        if features is None:
            continue
...
def fit_score_model(candidates: list[Candidate]) -> ScoreModel:
    ...
    if not usable:
        return ScoreModel(
            FEATURE_NAMES,
            tuple((0.0, 0.0) for _ in FEATURE_NAMES),
            {},
            {},
            {1: 0.0, -1: 0.0},
            {1: 0, -1: 0},
    ...
    cuts = tuple(feature_cuts([candidate.features[i] for candidate in usable]) for i in range(len(FEATURE_NAMES)))
    cell_values: dict[tuple[int, tuple[int, ...]], list[float]] = defaultdict(list)
    direction_values: dict[int, list[float]] = defaultdict(list)
    for candidate in usable:
        label = candidate.label or 0.0
        cell_values[(candidate.direction, feature_bins(candidate.features, cuts))].append(label)
        direction_values[candidate.direction].append(label)
    cell_means = {key: mean(values) or 0.0 for key, values in cell_values.items()}
    cell_counts = {key: len(values) for key, values in cell_values.items()}
    global_mean = mean(labels) or 0.0
    direction_means = {direction: mean(direction_values.get(direction, [])) or global_mean for direction in (1, -1)}
    direction_counts = {direction: len(direction_values.get(direction, [])) for direction in (1, -1)}
...
def feature_cuts(values: list[float]) -> tuple[float, float]:
    lower = percentile(values, 1.0 / CELL_BIN_COUNT)
    upper = percentile(values, 2.0 / CELL_BIN_COUNT)
    return (lower or 0.0, upper or 0.0)


def feature_bins(features: tuple[float, ...], cuts: tuple[tuple[float, float], ...]) -> tuple[int, ...]:
    bins: list[int] = []
    for value, (lower, upper) in zip(features, cuts, strict=True):
        if value <= lower:
            bins.append(0)
        elif value <= upper:
            bins.append(1)
...
def score_candidate(candidate: Candidate, model: ScoreModel) -> Candidate:
    bins = feature_bins(candidate.features, model.cuts)
    key = (candidate.direction, bins)
    count = model.cell_counts.get(key, 0)
    fallback = model.direction_means.get(candidate.direction, model.global_mean)
    if count <= 0:
        score = fallback
    else:
        score = ((model.cell_means[key] * count) + (fallback * CELL_PRIOR_WEIGHT)) / (count + CELL_PRIOR_WEIGHT)
    return Candidate(
...
def build_proxy_payload(
    ...
    return {
        ...
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0002_r0003_dual_direction_cell_score",
        "proxy_config_path": "campaigns/C0002_score_conditioned_intraday_edge/runs/R0003/run_manifest.json",
        ...
        "conditional_profiles": {
            "score_surface_profile": {
                "applies": True,
                "fields": {
                    "score_type": "train_only_discrete_quantile_cell_score",
                    "training_scope": "rolling_train_is_only",
                    "feature_count": len(FEATURE_NAMES),
                    "feature_names": list(FEATURE_NAMES),
                    "feature_family_summary": "closed_bar_price_session_volatility_path_context",
                    ...
                    "min_score": MIN_SCORE,
                    "model_family": "discrete_quantile_cell_score",
                    "fold_models": fold_models,
                    "score_distributions": score_distributions,
                    "candidates_by_fold": candidates_by_fold,
        ...
        "deferred_with_reason": [
            {
                ...
                "revisit_when": "during C0002 R0003 MT5 evidence production",
            ...
            },
            {
                ...
                "revisit_when": "before C0002 R0003 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
        "claim_boundary": {
...
def score_model_summary(fold_id: str, model: ScoreModel) -> dict[str, object]:
    return {
        ...
        "cuts": [[rounded(value) for value in pair] for pair in model.cuts],
        "cell_count": len(model.cell_counts),
        "min_cell_count": MIN_CELL_COUNT,
        "cell_prior_weight": CELL_PRIOR_WEIGHT,
        "direction_means": {str(direction): rounded(value) for direction, value in sorted(model.direction_means.items())},
        "direction_counts": {str(direction): count for direction, count in sorted(model.direction_counts.items())},
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "score_model": "train_only_discrete_quantile_cell_mean_with_direction_fallback",
...
def update_artifact_lineage(proxy_hash: str) -> None:
    ...
    for record in data.get("artifact_records", []):
        if record.get("artifact_id") == "A0002":
            ...
            record["produced_by"] = "axiom_rift.proxies.c0002_r0003_dual_direction_cell_score"
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0002_r0003_mt5_logic_parity_evidence",
        }
    ]
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0002_r0003_mt5_logic_parity_evidence"
    data["deferred_with_reason"] = [
        {
            "field": "decision",
            ...
            "revisit_when": "after C0002 R0003 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    text = text.replace("remaining_question: produce_c0002_r0003_proxy_evidence", "remaining_question: produce_c0002_r0003_mt5_logic_parity_evidence")
    text = text.replace("remaining_question: decide_r0003_or_close_c0002_after_r0002", "remaining_question: produce_c0002_r0003_mt5_logic_parity_evidence")
...
def update_reentry_after_proxy() -> None:
    ...
    if "produce_c0002_r0003_proxy_evidence" not in completed:
        completed.append("produce_c0002_r0003_proxy_evidence")
    next_action = "produce_c0002_r0003_mt5_logic_parity_evidence"
