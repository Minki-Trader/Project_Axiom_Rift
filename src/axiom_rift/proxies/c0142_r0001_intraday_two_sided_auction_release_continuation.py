"""C0142 R0001 proxy evidence for intraday two sided auction release continuation."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0142_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0142_r0001_two_sided_auction_release_continuation_summary.json"
...
FEATURE_NAMES = (
    "two_sided_probe_balance",
    "equilibrium_compression",
    "release_impulse",
    "vwap_release_alignment",
    "day_open_release_alignment",
    "prior_close_release_alignment",
    "range_expansion_efficiency",
    "close_drive_commitment",
    "counter_wick_containment",
    "participation_reengagement",
    "pre_release_chop",
    "session_room_after_release",
    "prior_day_break_context",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_two_sided_auction_release_continuation_rank"
LABEL_SHAPE = "target_first_quality_after_two_sided_auction_release_continuation"
SELECTION_RULE = "top_fold_local_two_sided_auction_release_continuation_scores_per_active_day"
...
MIN_DAY_BARS = 18
TWO_SIDED_AUCTION_RELEASE_CONTINUATION_LOOKBACK_BARS = 72
TWO_SIDED_AUCTION_RELEASE_CONTINUATION_MICRO_BARS = 14
MIN_TWO_SIDED_PROBE_BALANCE = 0.20
MIN_RELEASE_IMPULSE = -0.04
MIN_CLOSE_DRIVE_COMMITMENT = 0.38
MIN_TWO_SIDED_AUCTION_RELEASE_CONTINUATION_ACTIVITY = 0.025
...
class LinearTwoSidedAuctionReleaseContinuationModel:
    fold_id: str
    ...
    two_sided_auction_release_continuation_failure_rate: float
...
def run_c0142_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_two_sided_auction_release_continuation_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_two_sided_auction_release_continuation_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        TWO_SIDED_AUCTION_RELEASE_CONTINUATION_LOOKBACK_BARS,
        TWO_SIDED_AUCTION_RELEASE_CONTINUATION_MICRO_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = two_sided_auction_release_continuation_metrics(
    ...
    features = (
        metrics["two_sided_probe_balance"],
        metrics["equilibrium_compression"],
        metrics["release_impulse"],
        metrics["vwap_release_alignment"],
        metrics["day_open_release_alignment"],
        metrics["prior_close_release_alignment"],
        metrics["range_expansion_efficiency"],
        metrics["close_drive_commitment"],
        metrics["counter_wick_containment"],
        metrics["participation_reengagement"],
        metrics["pre_release_chop"],
        metrics["session_room_after_release"],
        metrics["prior_day_break_context"],
        metrics["session_maturity"],
        metrics["spread_stress_inverse"],
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        touch_density_bucket(metrics["two_sided_probe_balance"]),
        range_compression_bucket(metrics["equilibrium_compression"]),
        impulse_response_bucket(metrics["release_impulse"]),
        vwap_side_confirm_bucket(metrics["vwap_release_alignment"]),
        day_open_tension_bucket(metrics["day_open_release_alignment"]),
        prior_direction_context_bucket(metrics["prior_close_release_alignment"]),
        range_expansion_bucket(metrics["range_expansion_efficiency"]),
        close_drive_bucket(metrics["close_drive_commitment"]),
        adverse_wick_control_bucket(metrics["counter_wick_containment"]),
        participation_followthrough_bucket(metrics["participation_reengagement"]),
        path_tortuosity_bucket(metrics["pre_release_chop"]),
        session_pressure_bucket(metrics["session_room_after_release"]),
        prior_day_range_alignment_bucket(metrics["prior_day_break_context"]),
...
def two_sided_auction_release_continuation_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    micro_start = max(index - TWO_SIDED_AUCTION_RELEASE_CONTINUATION_MICRO_BARS + 1, context.day_start_index)
    lookback_start = max(index - TWO_SIDED_AUCTION_RELEASE_CONTINUATION_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    if len(prior_micro) < 5:
        return {"event_count": 0.0}
    previous_bar = bars[index - 1]
    micro_high = max(bar.high for bar in micro_window)
    micro_low = min(bar.low for bar in micro_window)
    micro_span = max(micro_high - micro_low, 1e-9)
    ...
    release_side_position = directional_session_position(signal_bar.close, context, direction)
    if release_side_position is None:
        return {"event_count": 0.0}

    upper_probe_count = sum(1 for bar in lookback_window if bar.high >= lookback_low + 0.70 * lookback_span)
    lower_probe_count = sum(1 for bar in lookback_window if bar.low <= lookback_low + 0.30 * lookback_span)
    two_sided_probe_balance = min(upper_probe_count, lower_probe_count) / max(len(lookback_window), 1) * 2.0
    equilibrium_compression = (lookback_span - micro_span) / max(average_range, 1e-9)
    release_impulse = direction * (signal_bar.close - previous_bar.close) / max(average_range, 1e-9)
    vwap_release_alignment = direction * (signal_bar.close - session_vwap) / max(average_range, 1e-9)
    day_open_release_alignment = direction * (signal_bar.close - context.day_open) / max(average_range, 1e-9)
    prior_close_release_alignment = direction * (signal_bar.close - prior_close_anchor) / max(average_range, 1e-9)
    micro_efficiency = directional_path_efficiency(
        bars,
        index,
        direction,
        min(TWO_SIDED_AUCTION_RELEASE_CONTINUATION_MICRO_BARS, index),
    ) or 0.0
    range_expansion_efficiency = micro_range_mean / max(lookback_range_mean, average_range, 1e-9) + max(
        micro_efficiency,
        -0.50,
    )
    close_drive_commitment = (
        directional_bar_settlement(signal_bar, direction) * 0.48
        + max(0.0, directional_body_score(signal_bar, direction)) * 0.34
        + max(0.0, release_impulse) * 0.18
    )
    counter_wick_containment = adverse_wick_absorption(signal_bar, direction)
    current_volume_ratio = volumes[index] / max(average_volume, 1e-9)
    previous_volume_ratio = volumes[index - 1] / max(average_volume, 1e-9)
    micro_volume_ratio = micro_volume_mean / max(lookback_volume_mean, average_volume, 1e-9)
    participation_reengagement = current_volume_ratio - min(previous_volume_ratio, micro_volume_ratio)
    pre_release_efficiency = directional_path_efficiency(
        bars,
        index - 1,
        direction,
        min(TWO_SIDED_AUCTION_RELEASE_CONTINUATION_LOOKBACK_BARS // 2, index - 1),
    )
    pre_release_chop = 1.0 - abs(pre_release_efficiency or 0.0)
    if direction > 0:
        ...
        session_room_after_release = max(
    ...
    prior_day_break_context = 0.0
    if context.prior_day_range is not None and context.prior_day_range > 0:
        prior_day_break_context = direction * (signal_bar.close - prior_close_anchor) / context.prior_day_range

    if two_sided_probe_balance < MIN_TWO_SIDED_PROBE_BALANCE and pre_release_chop < 0.35:
        return {"event_count": 0.0}
    if equilibrium_compression < -0.15 and range_expansion_efficiency < 0.45:
        return {"event_count": 0.0}
    if release_impulse < MIN_RELEASE_IMPULSE and close_drive_commitment < MIN_CLOSE_DRIVE_COMMITMENT:
        return {"event_count": 0.0}
    if release_side_position < 0.52 and vwap_release_alignment < -0.15:
        return {"event_count": 0.0}
    ...
    activity = (
        max(two_sided_probe_balance - 0.18, -0.18) * 0.12
        + max(equilibrium_compression, -0.30) * 0.09
        + max(release_impulse, -0.18) * 0.15
        + max(vwap_release_alignment, -0.28) * 0.09
        + max(day_open_release_alignment, -0.35) * 0.07
        + max(prior_close_release_alignment, -0.40) * 0.06
        + max(range_expansion_efficiency - 0.45, -0.22) * 0.10
        + max(close_drive_commitment - 0.38, -0.20) * 0.13
        + max(counter_wick_containment - 0.24, -0.18) * 0.07
        + max(participation_reengagement, -0.55) * 0.07
        + max(pre_release_chop - 0.35, -0.18) * 0.05
        + max(session_room_after_release, -0.25) * 0.03
        + max(prior_day_break_context, -0.35) * 0.04
        + max(release_side_position - 0.52, -0.16) * 0.08
    )
    if activity < MIN_TWO_SIDED_AUCTION_RELEASE_CONTINUATION_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "two_sided_probe_balance": bounded(two_sided_probe_balance, 0.0, 2.0),
        "equilibrium_compression": bounded(equilibrium_compression, -3.0, 8.0),
        "release_impulse": bounded(release_impulse, -4.0, 4.0),
        "vwap_release_alignment": bounded(vwap_release_alignment, -5.0, 5.0),
        "day_open_release_alignment": bounded(day_open_release_alignment, -5.0, 5.0),
        "prior_close_release_alignment": bounded(prior_close_release_alignment, -5.0, 5.0),
        "range_expansion_efficiency": bounded(range_expansion_efficiency, -2.0, 6.0),
        "close_drive_commitment": bounded(close_drive_commitment, -1.0, 2.0),
        "counter_wick_containment": bounded(counter_wick_containment, 0.0, 1.0),
        "participation_reengagement": bounded(participation_reengagement, -6.0, 8.0),
        "pre_release_chop": bounded(pre_release_chop, 0.0, 2.0),
        "session_room_after_release": bounded(session_room_after_release, -3.0, 8.0),
        "prior_day_break_context": bounded(prior_day_break_context, -3.0, 3.0),
        ...
        "release_side_position": bounded(release_side_position, 0.0, 1.0),
        ...
        "two_sided_auction_release_continuation_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def C0142_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def C0142_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def C0142_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def C0142_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def C0142_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def C0142_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def C0142_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def C0142_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def C0142_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def C0142_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def C0142_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def C0142_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def C0142_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def C0142_pullback_containment_bucket(value: float) -> str:
    if value < 0.35:
...
def C0142_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def C0142_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def C0142_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def C0142_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def C0142_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def C0142_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    balance_reward = 0.08 if metrics["two_sided_probe_balance"] > 0.32 and terminal > 0 else 0.0
    compression_reward = 0.08 if metrics["equilibrium_compression"] > 0.20 and terminal > 0 else 0.0
    impulse_reward = 0.13 if metrics["release_impulse"] > 0.04 and terminal > 0 else 0.0
    vwap_reward = 0.09 if metrics["vwap_release_alignment"] > -0.04 and terminal > 0 else 0.0
    open_reward = 0.07 if metrics["day_open_release_alignment"] > -0.10 and terminal > 0 else 0.0
    prior_close_reward = 0.06 if metrics["prior_close_release_alignment"] > -0.14 and terminal > 0 else 0.0
    expansion_reward = 0.09 if metrics["range_expansion_efficiency"] > 0.55 and terminal > 0 else 0.0
    drive_reward = 0.11 if metrics["close_drive_commitment"] > 0.48 and terminal > 0 else 0.0
    wick_reward = 0.06 if metrics["counter_wick_containment"] > 0.35 and terminal > 0 else 0.0
    participation_reward = 0.07 if metrics["participation_reengagement"] > -0.10 and terminal > 0 else 0.0
    chop_reward = 0.05 if metrics["pre_release_chop"] > 0.38 and terminal > 0 else 0.0
    room_reward = 0.04 if metrics["session_room_after_release"] > 0.08 and terminal > 0 else 0.0
    prior_day_reward = 0.04 if metrics["prior_day_break_context"] > -0.18 and terminal > 0 else 0.0
    weak_balance_penalty = 0.06 if metrics["two_sided_probe_balance"] < 0.16 and terminal <= 0 else 0.0
    weak_compression_penalty = 0.05 if metrics["equilibrium_compression"] < -0.15 and terminal <= 0 else 0.0
    weak_impulse_penalty = 0.10 if metrics["release_impulse"] < -0.06 and terminal <= 0 else 0.0
    weak_vwap_penalty = 0.06 if metrics["vwap_release_alignment"] < -0.18 and terminal <= 0 else 0.0
    weak_expansion_penalty = 0.07 if metrics["range_expansion_efficiency"] < 0.28 and terminal <= 0 else 0.0
    weak_drive_penalty = 0.08 if metrics["close_drive_commitment"] < 0.30 and terminal <= 0 else 0.0
    weak_wick_penalty = 0.05 if metrics["counter_wick_containment"] < 0.18 and terminal <= 0 else 0.0
    no_participation_penalty = 0.05 if metrics["participation_reengagement"] < -0.50 and terminal <= 0 else 0.0
    no_room_penalty = 0.04 if metrics["session_room_after_release"] < 0.02 and terminal <= 0 else 0.0
    ...
    return (
        ...
        - max(0.0, -metrics["participation_reengagement"] - 0.75) * 0.015
        - max(0.0, MIN_TWO_SIDED_PROBE_BALANCE - metrics["two_sided_probe_balance"]) * 0.04
        - max(0.0, MIN_RELEASE_IMPULSE - metrics["release_impulse"]) * 0.08
    )


def fit_linear_two_sided_auction_release_continuation_model(candidates: list[base.Candidate], fold_id: str) -> LinearTwoSidedAuctionReleaseContinuationModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearTwoSidedAuctionReleaseContinuationModel(
            fold_id,
    ...
    two_sided_auction_release_continuation_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearTwoSidedAuctionReleaseContinuationModel(
        fold_id=fold_id,
        ...
        two_sided_auction_release_continuation_failure_rate=two_sided_auction_release_continuation_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearTwoSidedAuctionReleaseContinuationModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_two_sided_auction_release_continuation_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0142_r0001_intraday_two_sided_auction_release_continuation",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0142_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0142_r0001_two_sided_auction_release_continuation_summary.json",
        ],
        ...
        "conditional_profiles": {
            "two_sided_auction_release_continuation_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0142 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0142_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0142 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "two_sided_auction_release_continuation_lookback_bars": TWO_SIDED_AUCTION_RELEASE_CONTINUATION_LOOKBACK_BARS,
        "two_sided_auction_release_continuation_micro_bars": TWO_SIDED_AUCTION_RELEASE_CONTINUATION_MICRO_BARS,
        "min_two_sided_probe_balance": MIN_TWO_SIDED_PROBE_BALANCE,
        "min_release_impulse": MIN_RELEASE_IMPULSE,
        "min_close_drive_commitment": MIN_CLOSE_DRIVE_COMMITMENT,
        "min_two_sided_auction_release_continuation_activity": MIN_TWO_SIDED_AUCTION_RELEASE_CONTINUATION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_two_sided_auction_release_continuation_model_summary(model: LinearTwoSidedAuctionReleaseContinuationModel) -> dict[str, object]:
    return {
        ...
        "two_sided_auction_release_continuation_failure_rate": rounded(model.two_sided_auction_release_continuation_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearTwoSidedAuctionReleaseContinuationModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_two_sided_auction_release_continuation_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "two_sided_auction_release_continuation_profile": profiles["two_sided_auction_release_continuation_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["next_required_action"] = "produce_c0142_r0001_mt5_logic_parity_evidence"
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0142_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0142_r0001_proxy_trades.csv"
    evidence["two_sided_auction_release_continuation_summary"] = "artifacts/c0142_r0001_two_sided_auction_release_continuation_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0142_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0142_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0142 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0142_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    next_work["tasks"] = ["produce_c0142_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0142_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0142_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0142_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0142_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0142_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0142_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0142 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0142 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    block = f"""
- decision_id: dec_20260708_produce_c0142_r0001_proxy_evidence
  created_local_date: '{local_date}'
  status: active
  decision: produce_c0142_r0001_proxy_evidence
  refines:
  - dec_20260708_open_c0142_r0001_intraday_two_sided_auction_release_continuation_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0142_r0001_proxy_records_two_sided_auction_release_continuation_reference_surface
  ...
  - next_work_is_produce_c0142_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260708_produce_c0142_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0142-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0142_r0001_intraday_two_sided_auction_release_continuation",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0142-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0142-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0142-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0142_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0142_r0001_intraday_two_sided_auction_release_continuation",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0142-R0001-ANCHOR-BRAID-SQUEEZE-ESCAPE-CONTINUATION-SUMMARY",
            "artifact_role": "two_sided_auction_release_continuation_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0142_r0001_two_sided_auction_release_continuation_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0142_r0001_intraday_two_sided_auction_release_continuation",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0142_r0001_mt5_logic_parity_evidence",
        }
    ]
