"""C0083 R0001 proxy evidence for intraday shelf acceptance continuation."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0083_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0083_r0001_shelf_acceptance_summary.json"
...
FEATURE_NAMES = (
    "impulse_strength",
    "shelf_compression",
    "shelf_close_retention",
    "shallow_adverse_overlap",
    "touch_density",
    "participation_renewal",
    "impulse_efficiency",
    "prior_day_alignment",
    "session_continuation_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_shelf_acceptance_continuation_rank"
LABEL_SHAPE = "target_first_continuation_quality_conditioned_on_post_impulse_shelf_acceptance_state"
SELECTION_RULE = "top_fold_local_shelf_acceptance_continuation_scores_per_active_day"
...
MIN_DAY_BARS = 30
SHELF_ACCEPTANCE_CONTINUATION_LOOKBACK_BARS = 18
SHELF_ACCEPTANCE_CONTINUATION_SHORT_LOOKBACK_BARS = 6
MIN_SHELF_ACCEPTANCE_CONTINUATION_ACTIVITY = 0.16
...
class LinearShelfAcceptanceContinuationModel:
    fold_id: str
    ...
    shelf_acceptance_continuation_failure_rate: float
...
def run_c0083_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_shelf_acceptance_continuation_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_shelf_acceptance_continuation_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        SHELF_ACCEPTANCE_CONTINUATION_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = shelf_acceptance_continuation_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        metrics["impulse_strength"],
        metrics["shelf_compression"],
        metrics["shelf_close_retention"],
        metrics["shallow_adverse_overlap"],
        metrics["touch_density"],
        metrics["participation_renewal"],
        metrics["impulse_efficiency"],
        metrics["prior_day_alignment"],
        metrics["session_continuation_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        impulse_range_bucket(metrics["impulse_strength"]),
        digestion_compression_bucket(metrics["shelf_compression"]),
        close_hold_bucket(metrics["shelf_close_retention"]),
        shallow_pullback_bucket(metrics["shallow_adverse_overlap"]),
        touch_density_bucket(metrics["touch_density"]),
        volume_confirmation_bucket(metrics["participation_renewal"]),
        impulse_efficiency_bucket(metrics["impulse_efficiency"]),
        prior_impulse_context_bucket(metrics["prior_day_alignment"]),
        session_escape_bucket(metrics["session_continuation_pressure"]),
...
def shelf_acceptance_continuation_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    short_start = index - SHELF_ACCEPTANCE_CONTINUATION_SHORT_LOOKBACK_BARS + 1
    prior_start = index - SHELF_ACCEPTANCE_CONTINUATION_LOOKBACK_BARS + 1
    if short_start < 0 or prior_start < 0:
        return {"event_count": 0.0}
    short_window = bars[short_start : index + 1]
    impulse_window = bars[prior_start:short_start]
    prior_window = bars[prior_start : index + 1]
    if len(short_window) < 3 or len(impulse_window) < 6 or len(prior_window) < 8:
        return {"event_count": 0.0}
    ...
    impulse_end_price = impulse_window[-1].close
    impulse_net = direction * (impulse_end_price - impulse_start_price)
    impulse_travel = sum(
        abs(impulse_window[pos].close - impulse_window[pos - 1].close)
        for pos in range(1, len(impulse_window))
    )
    impulse_efficiency = impulse_net / impulse_travel if impulse_travel > 0 else 0.0
    ...
    impulse_strength = impulse_net / average_range
    shelf_compression = max(0.0, 1.0 - short_width / prior_width)
    if direction > 0:
        shelf_close_retention = (signal_bar.close - short_low) / short_width
        adverse_retrace = max(0.0, impulse_end_price - short_low)
    else:
        shelf_close_retention = (short_high - signal_bar.close) / short_width
        adverse_retrace = max(0.0, short_high - impulse_end_price)
    shallow_adverse_overlap = 1.0 - adverse_retrace / max(abs(impulse_net), average_range, 1e-9)
    edge_touch_count = 0
    for bar in short_window:
        if direction > 0:
            close_location = (bar.close - short_low) / short_width
        else:
            close_location = (short_high - bar.close) / short_width
        if close_location >= 0.62:
            edge_touch_count += 1
    touch_density = edge_touch_count / len(short_window)
    ...
    participation_renewal = (current_volume_ratio - early_volume_ratio) * max(shelf_close_retention, 0.0)
    prior_day_alignment = direction * prior_day_return / max(prior_day_range, 1e-9)
    continuation_pressure = max(
        max(impulse_strength, 0.0) / 2.0,
        max(shelf_compression, 0.0),
        max(shelf_close_retention - 0.45, 0.0),
        max(shallow_adverse_overlap - 0.35, 0.0),
        max(touch_density - 0.20, 0.0),
        max(impulse_efficiency, 0.0),
        min(max(participation_renewal, 0.0), 1.0),
    )
    ...
    if activity < MIN_SHELF_ACCEPTANCE_CONTINUATION_ACTIVITY:
        return {"event_count": 0.0}
    if impulse_strength < 0.18 or impulse_efficiency < 0.18:
        return {"event_count": 0.0}
    if shelf_compression < 0.04 or shelf_close_retention < 0.48:
        return {"event_count": 0.0}
    if shallow_adverse_overlap < 0.22:
        return {"event_count": 0.0}
    if participation_renewal < -0.25 and touch_density < 0.20:
        return {"event_count": 0.0}
    return {
        "impulse_strength": bounded(impulse_strength, -3.0, 8.0),
        "shelf_compression": bounded(shelf_compression, 0.0, 1.0),
        "shelf_close_retention": bounded(shelf_close_retention, 0.0, 1.0),
        "shallow_adverse_overlap": bounded(shallow_adverse_overlap, -1.0, 1.0),
        "touch_density": bounded(touch_density, 0.0, 1.0),
        "participation_renewal": bounded(participation_renewal, -3.0, 6.0),
        "impulse_efficiency": bounded(impulse_efficiency, -1.0, 1.0),
        "prior_day_alignment": bounded(prior_day_alignment, -2.0, 2.0),
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    impulse_reward = 0.15 if metrics["impulse_strength"] > 0.45 and terminal > 0 else 0.0
    compression_reward = 0.12 if metrics["shelf_compression"] > 0.14 and terminal > 0 else 0.0
    retention_reward = 0.13 if metrics["shelf_close_retention"] > 0.64 and terminal > 0 else 0.0
    shallow_overlap_reward = 0.12 if metrics["shallow_adverse_overlap"] > 0.54 and terminal > 0 else 0.0
    touch_density_reward = 0.10 if metrics["touch_density"] > 0.34 and terminal > 0 else 0.0
    participation_reward = 0.08 if metrics["participation_renewal"] > 0.04 and terminal > 0 else 0.0
    efficiency_reward = 0.10 if metrics["impulse_efficiency"] > 0.52 and terminal > 0 else 0.0
    prior_context_reward = 0.06 if metrics["prior_day_alignment"] > -0.20 and terminal > 0 else 0.0
    weak_impulse_penalty = 0.12 if metrics["impulse_strength"] < 0.22 and terminal <= 0 else 0.0
    loose_shelf_penalty = 0.10 if metrics["shelf_compression"] < 0.05 and terminal <= 0 else 0.0
    weak_retention_penalty = 0.12 if metrics["shelf_close_retention"] < 0.50 and terminal <= 0 else 0.0
    deep_overlap_penalty = 0.13 if metrics["shallow_adverse_overlap"] < 0.34 and terminal <= 0 else 0.0
    sparse_touch_penalty = 0.08 if metrics["touch_density"] < 0.20 and terminal <= 0 else 0.0
    no_participation_penalty = 0.08 if metrics["participation_renewal"] < -0.08 and terminal <= 0 else 0.0
    inefficient_impulse_penalty = 0.10 if metrics["impulse_efficiency"] < 0.25 and terminal <= 0 else 0.0
    ...
    shelf_acceptance_continuation_path_quality = (0.50 * terminal + 0.35 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + shelf_acceptance_continuation_path_quality
        + impulse_reward
        + compression_reward
        + retention_reward
        + shallow_overlap_reward
        + touch_density_reward
        + participation_reward
        + efficiency_reward
        + prior_context_reward
        - weak_impulse_penalty
        - loose_shelf_penalty
        - weak_retention_penalty
        ...
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["impulse_strength"] - 3.20) * 0.04
    )


def fit_linear_shelf_acceptance_continuation_model(candidates: list[base.Candidate], fold_id: str) -> LinearShelfAcceptanceContinuationModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearShelfAcceptanceContinuationModel(
            fold_id,
    ...
    shelf_acceptance_continuation_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearShelfAcceptanceContinuationModel(
        fold_id=fold_id,
        ...
        shelf_acceptance_continuation_failure_rate=shelf_acceptance_continuation_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearShelfAcceptanceContinuationModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_shelf_acceptance_continuation_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0083_r0001_intraday_shelf_acceptance_continuation",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0083_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0083_r0001_shelf_acceptance_summary.json",
        ],
        ...
        "conditional_profiles": {
            "shelf_acceptance_continuation_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0083 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0083_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0083 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "shelf_acceptance_continuation_lookback_bars": SHELF_ACCEPTANCE_CONTINUATION_LOOKBACK_BARS,
        "shelf_acceptance_continuation_short_lookback_bars": SHELF_ACCEPTANCE_CONTINUATION_SHORT_LOOKBACK_BARS,
        "min_shelf_acceptance_continuation_activity": MIN_SHELF_ACCEPTANCE_CONTINUATION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_shelf_acceptance_continuation_model_summary(model: LinearShelfAcceptanceContinuationModel) -> dict[str, object]:
    return {
        ...
        "shelf_acceptance_continuation_failure_rate": rounded(model.shelf_acceptance_continuation_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearShelfAcceptanceContinuationModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_shelf_acceptance_continuation_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "shelf_acceptance_continuation_profile": profiles["shelf_acceptance_continuation_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0083_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0083_r0001_proxy_trades.csv"
    evidence["shelf_acceptance_continuation_summary"] = "artifacts/c0083_r0001_shelf_acceptance_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0083_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0083_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0083 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0083_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0083_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0083_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0083_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0083_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0083_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0083_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0083_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0083_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0083 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0083 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0083_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0083_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0083_r0001_intraday_shelf_acceptance_continuation_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0083_r0001_proxy_records_shelf_acceptance_continuation_reference_surface
  ...
  - next_work_is_produce_c0083_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0083_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0083-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0083_r0001_intraday_shelf_acceptance_continuation",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0083-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0083-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0083-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0083_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0083_r0001_intraday_shelf_acceptance_continuation",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0083-R0001-shelf-acceptance-continuation-SUMMARY",
            "artifact_role": "shelf_acceptance_continuation_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0083_r0001_shelf_acceptance_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0083_r0001_intraday_shelf_acceptance_continuation",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0083_r0001_mt5_logic_parity_evidence",
        }
    ]
