"""C0074 R0001 proxy evidence for intraday session range reset."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0074_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0074_r0001_srr_summary.json"
...
FEATURE_NAMES = (
    "adverse_excursion_ratio",
    "anchor_reset_proximity",
    "opening_anchor_reclaim",
    "day_range_progress",
    "short_recovery_slope",
    "reset_settlement",
    "range_compression_ratio",
    "volume_confirmation_after_reset",
    "prior_day_range_context",
    "session_maturity_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_session_range_reset_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_session_range_reset_state"
SELECTION_RULE = "top_fold_local_session_range_reset_scores_per_active_day"
...
MIN_DAY_BARS = 30
SESSION_RANGE_RESET_LOOKBACK_BARS = 20
SESSION_RANGE_RESET_SHORT_LOOKBACK_BARS = 6
MIN_SESSION_RANGE_RESET_ACTIVITY = 0.06
...
class LinearSessionRangeResetModel:
    fold_id: str
    ...
    session_range_reset_failure_rate: float
...
def run_c0074_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_session_range_reset_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_session_range_reset_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        SESSION_RANGE_RESET_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    release = session_range_reset_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        release["adverse_excursion_ratio"],
        release["anchor_reset_proximity"],
        release["opening_anchor_reclaim"],
        release["day_range_progress"],
        release["short_recovery_slope"],
        release["reset_settlement"],
        release["range_compression_ratio"],
        release["volume_confirmation_after_reset"],
    ...
    dimensions = (
        adverse_excursion_bucket(release["adverse_excursion_ratio"]),
        anchor_reset_bucket(release["anchor_reset_proximity"]),
        opening_reclaim_bucket(release["opening_anchor_reclaim"]),
        day_range_progress_bucket(release["day_range_progress"]),
        short_recovery_bucket(release["short_recovery_slope"]),
        reset_settlement_bucket(release["reset_settlement"]),
        range_compression_bucket(release["range_compression_ratio"]),
        reset_volume_bucket(release["volume_confirmation_after_reset"]),
...
def session_range_reset_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    window_start = index - SESSION_RANGE_RESET_LOOKBACK_BARS + 1
    short_start = index - SESSION_RANGE_RESET_SHORT_LOOKBACK_BARS + 1
    previous_short_start = index - (2 * SESSION_RANGE_RESET_SHORT_LOOKBACK_BARS) + 1
    ...
    opening_mid = (context.opening_high + context.opening_low) * 0.5
    anchor_price = (context.day_open + opening_mid) * 0.5
    if direction > 0:
        adverse_extreme = min(bar.low for bar in medium_window)
        recovery_from_extreme = signal_bar.close - adverse_extreme
        adverse_excursion = max(0.0, anchor_price - adverse_extreme)
    else:
        adverse_extreme = max(bar.high for bar in medium_window)
        recovery_from_extreme = adverse_extreme - signal_bar.close
        adverse_excursion = max(0.0, adverse_extreme - anchor_price)
    adverse_excursion_ratio = adverse_excursion / max(average_range, 1e-9)
    anchor_distance = abs(signal_bar.close - anchor_price) / max(average_range, 1e-9)
    anchor_reset_proximity = 1.0 / (1.0 + anchor_distance)
    opening_anchor_reclaim = direction * (signal_bar.close - opening_mid) / max(average_range, 1e-9)
    day_range_progress = day_range / max(prior_day_range, average_range, 1e-9)
    short_recovery_slope = recovery_from_extreme / max(average_range, 1e-9)
    range_compression_ratio = previous_range_mean / max(short_range_mean, 1e-9)
    signal_body_efficiency = direction * (signal_bar.close - signal_bar.open) / signal_range
    reset_settlement = directional_bar_settlement(signal_bar, direction)
    ...
    volume_confirmation_after_reset = (short_volume_ratio - previous_volume_ratio) * max(
        short_recovery_slope,
        0.0,
    )
    ...
    reset_pressure = max(
        adverse_excursion_ratio,
        anchor_reset_proximity,
        abs(opening_anchor_reclaim),
        abs(day_range_progress - 1.0),
        short_recovery_slope,
        abs(reset_settlement - 0.5),
        abs(range_compression_ratio - 1.0),
        abs(volume_confirmation_after_reset),
        abs(prior_day_range_context - 1.0),
    )
    session_maturity_pressure = bounded_session_progress(signal_bar.time) * max(
        reset_pressure,
        max(short_recovery_slope, 0.0),
    ...
    if activity < MIN_SESSION_RANGE_RESET_ACTIVITY:
        return {"event_count": 0.0}
    if (
        adverse_excursion_ratio < 0.45
        and anchor_reset_proximity < 0.45
        and abs(opening_anchor_reclaim) < 0.08
        and short_recovery_slope < 0.20
        and abs(volume_confirmation_after_reset) < 0.04
    ):
        return {"event_count": 0.0}
    if adverse_excursion_ratio < 0.20 and day_range_progress < 0.35:
        return {"event_count": 0.0}
    return {
        "adverse_excursion_ratio": bounded(adverse_excursion_ratio, 0.0, 8.0),
        "anchor_reset_proximity": bounded(anchor_reset_proximity, 0.0, 1.0),
        "opening_anchor_reclaim": bounded(opening_anchor_reclaim, -6.0, 6.0),
        "day_range_progress": bounded(day_range_progress, 0.0, 6.0),
        "short_recovery_slope": bounded(short_recovery_slope, -2.0, 8.0),
        "reset_settlement": bounded(reset_settlement, 0.0, 1.0),
        "range_compression_ratio": bounded(range_compression_ratio, 0.0, 6.0),
        "volume_confirmation_after_reset": bounded(volume_confirmation_after_reset, -4.0, 4.0),
...
def candidate_label(
    ...
    metrics = session_range_reset_metrics(
        bars,
        volumes,
    ...
    if metrics["event_count"] <= 0:
        return None
    excursion_reward = (
        0.16 if metrics["adverse_excursion_ratio"] > 0.75 and terminal > 0 else 0.0
    )
    anchor_reset_reward = (
        0.18 if metrics["anchor_reset_proximity"] > 0.55 and terminal > 0 else 0.0
    )
    opening_reclaim_reward = (
        0.14 if metrics["opening_anchor_reclaim"] > 0.10 and terminal > 0 else 0.0
    )
    recovery_reward = (
        0.14 if metrics["short_recovery_slope"] > 0.45 and terminal > 0 else 0.0
    )
    settlement_reward = (
        0.12 if metrics["reset_settlement"] > 0.62 and terminal > 0 else 0.0
    )
    compression_reward = (
        0.08 if metrics["range_compression_ratio"] > 1.10 and terminal > 0 else 0.0
    )
    volume_reset_reward = (
        0.08 if metrics["volume_confirmation_after_reset"] > 0.06 and terminal > 0 else 0.0
    )
    prior_range_reward = (
        0.06 if 0.70 <= metrics["prior_day_range_context"] <= 2.20 and terminal > 0 else 0.0
    )
    thin_excursion_penalty = (
        0.14 if metrics["adverse_excursion_ratio"] < 0.35 and terminal <= 0 else 0.0
    )
    anchor_miss_penalty = (
        0.18 if metrics["anchor_reset_proximity"] < 0.32 and terminal <= 0 else 0.0
    )
    failed_opening_reclaim_penalty = (
        0.16 if metrics["opening_anchor_reclaim"] < -0.12 and terminal <= 0 else 0.0
    )
    weak_settlement_penalty = (
        0.14 if metrics["reset_settlement"] < 0.40 and terminal <= 0 else 0.0
    )
    stalled_recovery_penalty = (
        0.12 if metrics["short_recovery_slope"] < 0.18 and terminal <= 0 else 0.0
    )
    volume_fade_penalty = (
        0.10 if metrics["volume_confirmation_after_reset"] < -0.06 and terminal <= 0 else 0.0
    )
    ...
    session_range_reset_path_quality = (0.44 * terminal + 0.36 * mfe - 0.72 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + session_range_reset_path_quality
        + excursion_reward
        + anchor_reset_reward
        + opening_reclaim_reward
        + recovery_reward
        ...
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["day_range_progress"] - 2.4) * 0.05
    )


def fit_linear_session_range_reset_model(candidates: list[base.Candidate], fold_id: str) -> LinearSessionRangeResetModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearSessionRangeResetModel(
            fold_id,
    ...
    session_range_reset_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearSessionRangeResetModel(
        fold_id=fold_id,
        ...
        session_range_reset_failure_rate=session_range_reset_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearSessionRangeResetModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_session_range_reset_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0074_r0001_intraday_session_range_reset",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0074_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0074_r0001_srr_summary.json",
        ],
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0074 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0074_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0074 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "session_range_reset_lookback_bars": SESSION_RANGE_RESET_LOOKBACK_BARS,
        "session_range_reset_short_lookback_bars": SESSION_RANGE_RESET_SHORT_LOOKBACK_BARS,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_session_range_reset_model_summary(model: LinearSessionRangeResetModel) -> dict[str, object]:
    return {
        ...
        "session_range_reset_failure_rate": rounded(model.session_range_reset_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearSessionRangeResetModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_session_range_reset_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "session_range_reset_profile": profiles["session_range_reset_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0074_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0074_r0001_proxy_trades.csv"
    evidence["session_range_reset_summary"] = "artifacts/c0074_r0001_srr_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0074_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0074_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0074 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0074_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0074_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0074_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0074_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0074_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0074_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0074_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0074_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0074_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0074 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0074 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260706_produce_c0074_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0074_r0001_proxy_evidence
  refines:
  - dec_20260706_open_c0074_r0001_intraday_session_range_reset_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0074_r0001_proxy_records_session_range_reset_reference_surface
  ...
  - next_work_is_produce_c0074_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260706_produce_c0074_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0074-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0074_r0001_intraday_session_range_reset",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0074-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0074-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0074-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0074_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0074_r0001_intraday_session_range_reset",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0074-R0001-session-range-reset-SUMMARY",
            "artifact_role": "session_range_reset_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0074_r0001_srr_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0074_r0001_intraday_session_range_reset",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0074_r0001_mt5_logic_parity_evidence",
        }
    ]
