"""C0091 R0001 proxy evidence for intraday path asymmetry hazard."""
...
CAMPAIGN_ID = "C0091"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0091_intraday_path_asymmetry_hazard_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0091_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0091_r0001_path_asymmetry_hazard_summary.json"
...
FEATURE_NAMES = (
    "adverse_path_ratio",
    "recovery_asymmetry",
    "directional_recovery_drive",
    "counter_wick_absorption",
    "close_recovery_commitment",
    "volume_recovery_confirmation",
    "prior_adverse_cluster",
    "path_efficiency_repair",
    "session_phase_maturity",
    "hazard_release_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_path_asymmetry_hazard_rank"
LABEL_SHAPE = "target_first_escape_quality_conditioned_on_path_asymmetry_hazard_state"
SELECTION_RULE = "top_fold_local_path_asymmetry_hazard_scores_per_active_day"
...
MIN_DAY_BARS = 30
PATH_ASYMMETRY_HAZARD_LOOKBACK_BARS = 30
PATH_ASYMMETRY_HAZARD_SHORT_LOOKBACK_BARS = 9
MIN_PATH_ASYMMETRY_HAZARD_ACTIVITY = 0.14
...
class LinearPathAsymmetryHazardModel:
    fold_id: str
    ...
    path_asymmetry_hazard_failure_rate: float
...
def run_c0091_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_path_asymmetry_hazard_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_path_asymmetry_hazard_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        PATH_ASYMMETRY_HAZARD_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = path_asymmetry_hazard_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        metrics["adverse_path_ratio"],
        metrics["recovery_asymmetry"],
        metrics["directional_recovery_drive"],
        metrics["counter_wick_absorption"],
        metrics["close_recovery_commitment"],
        metrics["volume_recovery_confirmation"],
        metrics["prior_adverse_cluster"],
        metrics["path_efficiency_repair"],
        metrics["session_phase_maturity"],
        metrics["hazard_release_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        adverse_path_ratio_bucket(metrics["adverse_path_ratio"]),
        recovery_asymmetry_bucket(metrics["recovery_asymmetry"]),
        directional_recovery_drive_bucket(metrics["directional_recovery_drive"]),
        counter_wick_absorption_bucket(metrics["counter_wick_absorption"]),
        close_recovery_commitment_bucket(metrics["close_recovery_commitment"]),
        volume_recovery_confirmation_bucket(metrics["volume_recovery_confirmation"]),
        prior_adverse_cluster_bucket(metrics["prior_adverse_cluster"]),
        path_efficiency_repair_bucket(metrics["path_efficiency_repair"]),
        session_phase_maturity_bucket(metrics["session_phase_maturity"]),
        hazard_release_pressure_bucket(metrics["hazard_release_pressure"]),
...
def path_asymmetry_hazard_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    short_start = index - PATH_ASYMMETRY_HAZARD_SHORT_LOOKBACK_BARS + 1
    prior_start = index - PATH_ASYMMETRY_HAZARD_LOOKBACK_BARS + 1
    if short_start < 0 or prior_start < 0 or context.day_start_index >= index:
        return {"event_count": 0.0}
    prior_short_start = short_start - PATH_ASYMMETRY_HAZARD_SHORT_LOOKBACK_BARS
    if prior_short_start < 0:
        return {"event_count": 0.0}
    short_window = bars[short_start:index]
    prior_window = bars[prior_start:short_start]
    current_window = bars[short_start : index + 1]
    if len(short_window) < 4 or len(prior_window) < 12 or len(current_window) < 5:
        return {"event_count": 0.0}
    average_volume = volume_average[index]
    if average_volume is None or average_volume <= 0:
        return {"event_count": 0.0}

    anchor = current_window[0].open
    if direction > 0:
        adverse_extreme = max(anchor - bar.low for bar in current_window)
        favorable_extreme = max(bar.high - anchor for bar in current_window)
        recovery_from_adverse = signal_bar.close - min(bar.low for bar in current_window)
        terminal_move = signal_bar.close - anchor
    else:
        adverse_extreme = max(bar.high - anchor for bar in current_window)
        favorable_extreme = max(anchor - bar.low for bar in current_window)
        recovery_from_adverse = max(bar.high for bar in current_window) - signal_bar.close
        terminal_move = anchor - signal_bar.close
    adverse_extreme = max(adverse_extreme, 0.0)
    favorable_extreme = max(favorable_extreme, 0.0)
    recovery_from_adverse = max(recovery_from_adverse, 0.0)
    signal_range = max(signal_bar.high - signal_bar.low, 1e-9)
    path_total = sum(max(bar.high - bar.low, 0.0) for bar in current_window)
    adverse_path_ratio = adverse_extreme / max(adverse_extreme + favorable_extreme, average_range * 0.20)
    recovery_asymmetry = (recovery_from_adverse - adverse_extreme * 0.35) / average_range
    directional_recovery_drive = direction * (signal_bar.close - signal_bar.open) / signal_range
    close_recovery_commitment = directional_bar_settlement(signal_bar, direction)
    path_efficiency_repair = terminal_move / max(path_total, average_range * 0.25)
    path_efficiency = directional_path_efficiency(
        bars,
        index,
        direction,
        PATH_ASYMMETRY_HAZARD_SHORT_LOOKBACK_BARS,
    )
    if path_efficiency is None:
        return {"event_count": 0.0}
    upper_wick_sum = 0.0
    lower_wick_sum = 0.0
    for bar in current_window:
        upper_wick, lower_wick = wick_lengths(bar)
        upper_wick_sum += upper_wick
        lower_wick_sum += lower_wick
    if direction > 0:
        counter_wick_absorption = (lower_wick_sum - upper_wick_sum) / max(path_total, 1e-9)
    else:
        counter_wick_absorption = (upper_wick_sum - lower_wick_sum) / max(path_total, 1e-9)
    prior_adverse_cluster = sum(
        1
        for bar in short_window
        if direction * (bar.close - bar.open) < -0.12 * max(bar.high - bar.low, 1e-9)
    ) / max(len(short_window), 1)
    ...
    volume_recovery_confirmation = (current_volume_ratio - early_volume_ratio) * max(directional_recovery_drive, 0.0)
    session_phase_maturity = bounded_session_progress(signal_bar.time)
    hazard_release_pressure_raw = max(
        max(adverse_path_ratio - 0.20, 0.0),
        max(recovery_asymmetry, 0.0),
        max(directional_recovery_drive, 0.0),
        max(counter_wick_absorption, 0.0),
        max(close_recovery_commitment - 0.45, 0.0) * 2.0,
        min(max(volume_recovery_confirmation, 0.0), 1.5),
        max(path_efficiency_repair, 0.0),
        min(max(prior_adverse_cluster - 0.12, 0.0), 1.0),
    )
    hazard_release_pressure = session_phase_maturity * hazard_release_pressure_raw
    activity = max(hazard_release_pressure_raw, hazard_release_pressure)
    if activity < MIN_PATH_ASYMMETRY_HAZARD_ACTIVITY:
        return {"event_count": 0.0}
    if adverse_extreme < 0.08 * average_range and prior_adverse_cluster < 0.18:
        return {"event_count": 0.0}
    if recovery_from_adverse < 0.05 * average_range and close_recovery_commitment < 0.42:
        return {"event_count": 0.0}
    if directional_recovery_drive < -0.24 and close_recovery_commitment < 0.42:
        return {"event_count": 0.0}
    return {
        "adverse_path_ratio": bounded(adverse_path_ratio, 0.0, 1.0),
        "recovery_asymmetry": bounded(recovery_asymmetry, -2.0, 3.0),
        "directional_recovery_drive": bounded(directional_recovery_drive, -1.0, 1.0),
        "counter_wick_absorption": bounded(counter_wick_absorption, -1.0, 1.0),
        "close_recovery_commitment": bounded(close_recovery_commitment, 0.0, 1.0),
        "volume_recovery_confirmation": bounded(volume_recovery_confirmation, -3.0, 6.0),
        "prior_adverse_cluster": bounded(prior_adverse_cluster, 0.0, 1.0),
        "path_efficiency_repair": bounded((path_efficiency_repair + path_efficiency) * 0.5, -1.0, 1.0),
        "session_phase_maturity": bounded(session_phase_maturity, 0.0, 1.0),
        "hazard_release_pressure": bounded(hazard_release_pressure, 0.0, 4.0),
        "event_count": 1.0,
    }
...
def adverse_path_ratio_bucket(value: float) -> str:
    if value < 0.28:
        return "adverse_path_light"
    if value < 0.56:
        return "adverse_path_defined"
    return "adverse_path_heavy"


def recovery_asymmetry_bucket(value: float) -> str:
    if value < 0.02:
        return "recovery_asymmetry_weak"
    if value < 0.28:
        return "recovery_asymmetry_mid"
    return "recovery_asymmetry_strong"


def directional_recovery_drive_bucket(value: float) -> str:
    if value < -0.08:
        return "recovery_drive_against"
    if value < 0.28:
        return "recovery_drive_mixed"
    return "recovery_drive_with"


def counter_wick_absorption_bucket(value: float) -> str:
    if value < -0.08:
        return "counter_wick_unhelpful"
    if value < 0.16:
        return "counter_wick_mixed"
    return "counter_wick_absorbing"


def close_recovery_commitment_bucket(value: float) -> str:
    if value < 0.42:
        return "close_recovery_weak"
    if value < 0.66:
        return "close_recovery_mid"
    return "close_recovery_strong"


def volume_recovery_confirmation_bucket(value: float) -> str:
    if value < -0.04:
        return "volume_recovery_fading"
    if value < 0.20:
        return "volume_recovery_flat"
    return "volume_recovery_confirming"


def prior_adverse_cluster_bucket(value: float) -> str:
    if value < 0.18:
        return "prior_adverse_sparse"
    if value < 0.42:
        return "prior_adverse_defined"
    return "prior_adverse_clustered"


def path_efficiency_repair_bucket(value: float) -> str:
    if value < -0.05:
        return "path_repair_failed"
    if value < 0.24:
        return "path_repair_mixed"
    return "path_repair_clean"


def hazard_release_pressure_bucket(value: float) -> str:
    if value < 0.08:
        return "hazard_release_light"
    if value < 0.46:
        return "hazard_release_defined"
    return "hazard_release_heavy"
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    recovery_reward = 0.14 if metrics["recovery_asymmetry"] > 0.18 and terminal > 0 else 0.0
    drive_reward = 0.12 if metrics["directional_recovery_drive"] > 0.22 and terminal > 0 else 0.0
    wick_reward = 0.10 if metrics["counter_wick_absorption"] > 0.12 and terminal > 0 else 0.0
    close_reward = 0.12 if metrics["close_recovery_commitment"] > 0.60 and terminal > 0 else 0.0
    volume_reward = 0.08 if metrics["volume_recovery_confirmation"] > 0.08 and terminal > 0 else 0.0
    repair_reward = 0.10 if metrics["path_efficiency_repair"] > 0.12 and terminal > 0 else 0.0
    hazard_reward = 0.06 if metrics["hazard_release_pressure"] > 0.12 and terminal > 0 else 0.0
    weak_recovery_penalty = 0.12 if metrics["recovery_asymmetry"] < 0.02 and terminal <= 0 else 0.0
    adverse_path_penalty = 0.12 if metrics["adverse_path_ratio"] > 0.60 and terminal <= 0 else 0.0
    weak_close_penalty = 0.10 if metrics["close_recovery_commitment"] < 0.45 and terminal <= 0 else 0.0
    adverse_drive_penalty = 0.10 if metrics["directional_recovery_drive"] < -0.08 and terminal <= 0 else 0.0
    no_wick_absorption_penalty = 0.08 if metrics["counter_wick_absorption"] < -0.08 and terminal <= 0 else 0.0
    no_volume_confirmation_penalty = 0.08 if metrics["volume_recovery_confirmation"] < -0.05 and terminal <= 0 else 0.0
    ...
    path_asymmetry_hazard_path_quality = (0.52 * terminal + 0.34 * mfe - 0.68 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + path_asymmetry_hazard_path_quality
        + recovery_reward
        + drive_reward
        + wick_reward
        + close_reward
        ...
        + repair_reward
        + hazard_reward
        - weak_recovery_penalty
        - adverse_path_penalty
        - weak_close_penalty
        - adverse_drive_penalty
        - no_wick_absorption_penalty
        - no_volume_confirmation_penalty
        - stall_penalty
        - unfavorable_path_penalty
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["adverse_path_ratio"] - 0.72) * 0.18
    )


def fit_linear_path_asymmetry_hazard_model(candidates: list[base.Candidate], fold_id: str) -> LinearPathAsymmetryHazardModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearPathAsymmetryHazardModel(
            fold_id,
    ...
    path_asymmetry_hazard_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearPathAsymmetryHazardModel(
        fold_id=fold_id,
        ...
        path_asymmetry_hazard_failure_rate=path_asymmetry_hazard_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearPathAsymmetryHazardModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0091-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_path_asymmetry_hazard_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0091_r0001_intraday_path_asymmetry_hazard",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0091_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0091_r0001_path_asymmetry_hazard_summary.json",
        ],
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0091 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0091_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0091 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "path_asymmetry_hazard_lookback_bars": PATH_ASYMMETRY_HAZARD_LOOKBACK_BARS,
        "path_asymmetry_hazard_short_lookback_bars": PATH_ASYMMETRY_HAZARD_SHORT_LOOKBACK_BARS,
        "min_path_asymmetry_hazard_activity": MIN_PATH_ASYMMETRY_HAZARD_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_path_asymmetry_hazard_model_summary(model: LinearPathAsymmetryHazardModel) -> dict[str, object]:
    return {
        ...
        "path_asymmetry_hazard_failure_rate": rounded(model.path_asymmetry_hazard_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearPathAsymmetryHazardModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_path_asymmetry_hazard_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "path_asymmetry_hazard_profile": profiles["path_asymmetry_hazard_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0091_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0091_r0001_proxy_trades.csv"
    evidence["path_asymmetry_hazard_summary"] = "artifacts/c0091_r0001_path_asymmetry_hazard_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0091_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0091_r0001_proxy_trades.csv",
        "artifacts/c0091_r0001_path_asymmetry_hazard_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0091_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0091 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0091_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0091_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0091_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0091_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0091_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0091_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0091_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0091_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0091_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0091 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0091 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0091_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0091_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0091_r0001_intraday_path_asymmetry_hazard_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0091_r0001_proxy_records_path_asymmetry_hazard_reference_surface
  ...
  - next_work_is_produce_c0091_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0091_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0091-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0091_r0001_intraday_path_asymmetry_hazard",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0091-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0091-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0091-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0091_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0091_r0001_intraday_path_asymmetry_hazard",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0091-R0001-path-asymmetry-hazard-SUMMARY",
            "artifact_role": "path_asymmetry_hazard_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0091_r0001_path_asymmetry_hazard_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0091_r0001_intraday_path_asymmetry_hazard",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0091_r0001_mt5_logic_parity_evidence",
        }
    ]
