"""C0141 R0001 proxy evidence for intraday range overextension snapback reversal."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0141_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0141_r0001_range_overextension_snapback_reversal_summary.json"
...
FEATURE_NAMES = (
    "session_range_extension",
    "extreme_close_failure",
    "snapback_impulse",
    "vwap_return_pressure",
    "day_open_return_pressure",
    "prior_close_return_pressure",
    "range_efficiency_decay",
    "body_efficiency_failure",
    "adverse_wick_rejection",
    "participation_exhaustion",
    "micro_reversal_followthrough",
    "room_to_midpoint",
    "prior_day_excess_context",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_range_overextension_snapback_reversal_rank"
LABEL_SHAPE = "target_first_quality_after_range_overextension_snapback_reversal"
SELECTION_RULE = "top_fold_local_range_overextension_snapback_reversal_scores_per_active_day"
...
MIN_DAY_BARS = 18
RANGE_OVEREXTENSION_SNAPBACK_REVERSAL_LOOKBACK_BARS = 72
RANGE_OVEREXTENSION_SNAPBACK_REVERSAL_MICRO_BARS = 12
MIN_SESSION_RANGE_EXTENSION = 0.45
MIN_EXTREME_CLOSE_FAILURE = 0.08
MIN_SNAPBACK_IMPULSE = -0.05
MIN_RANGE_OVEREXTENSION_SNAPBACK_REVERSAL_ACTIVITY = 0.03
...
class LinearRangeOverextensionSnapbackReversalModel:
    fold_id: str
    ...
    range_overextension_snapback_reversal_failure_rate: float
...
def run_c0141_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_range_overextension_snapback_reversal_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_range_overextension_snapback_reversal_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        RANGE_OVEREXTENSION_SNAPBACK_REVERSAL_LOOKBACK_BARS,
        RANGE_OVEREXTENSION_SNAPBACK_REVERSAL_MICRO_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = range_overextension_snapback_reversal_metrics(
    ...
    features = (
        metrics["session_range_extension"],
        metrics["extreme_close_failure"],
        metrics["snapback_impulse"],
        metrics["vwap_return_pressure"],
        metrics["day_open_return_pressure"],
        metrics["prior_close_return_pressure"],
        metrics["range_efficiency_decay"],
        metrics["body_efficiency_failure"],
        metrics["adverse_wick_rejection"],
        metrics["participation_exhaustion"],
        metrics["micro_reversal_followthrough"],
        metrics["room_to_midpoint"],
        metrics["prior_day_excess_context"],
        metrics["session_maturity"],
        metrics["spread_stress_inverse"],
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        range_extension_bucket(metrics["session_range_extension"]),
        close_location_decay_bucket(metrics["extreme_close_failure"]),
        reversion_impulse_bucket(metrics["snapback_impulse"]),
        vwap_reversion_depth_bucket(metrics["vwap_return_pressure"]),
        day_open_tension_bucket(metrics["day_open_return_pressure"]),
        prior_direction_context_bucket(metrics["prior_close_return_pressure"]),
        range_efficiency_decay_bucket(metrics["range_efficiency_decay"]),
        body_decay_bucket(metrics["body_efficiency_failure"]),
        adverse_wick_control_bucket(metrics["adverse_wick_rejection"]),
        participation_decay_bucket(metrics["participation_exhaustion"]),
        counter_settlement_bucket(metrics["micro_reversal_followthrough"]),
        midpoint_distance_bucket(metrics["room_to_midpoint"]),
        prior_extension_context_bucket(metrics["prior_day_excess_context"]),
...
def range_overextension_snapback_reversal_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    micro_start = max(index - RANGE_OVEREXTENSION_SNAPBACK_REVERSAL_MICRO_BARS + 1, context.day_start_index)
    lookback_start = max(index - RANGE_OVEREXTENSION_SNAPBACK_REVERSAL_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    session_midpoint = (context.high_so_far + context.low_so_far) / 2.0
    close_position = directional_session_position(signal_bar.close, context, direction)
    if close_position is None:
        return {"event_count": 0.0}
    extreme_exposure = 1.0 - close_position
    session_range_extension = session_range / max(average_range, 1e-9)
    snapback_impulse = direction * (signal_bar.close - previous_bar.close) / max(average_range, 1e-9)
    vwap_return_pressure = direction * (session_vwap - signal_bar.close) / max(average_range, 1e-9)
    day_open_return_pressure = direction * (context.day_open - signal_bar.close) / max(average_range, 1e-9)
    prior_close_return_pressure = direction * (prior_close_anchor - signal_bar.close) / max(average_range, 1e-9)
    adverse_wick_rejection = directional_wick_rejection(signal_bar, direction)
    body_efficiency_failure = directional_body_score(signal_bar, direction)
    extreme_close_failure = (
        directional_bar_settlement(signal_bar, direction) * 0.42
        + max(0.0, adverse_wick_rejection) * 0.36
        + max(0.0, body_efficiency_failure) * 0.22
    )
    adverse_efficiency = directional_path_efficiency(
        bars,
        index,
        -direction,
        min(RANGE_OVEREXTENSION_SNAPBACK_REVERSAL_MICRO_BARS, index),
    )
    range_efficiency_decay = 1.0 - max(0.0, adverse_efficiency or 0.0)
    micro_reversal_followthrough = direction * (signal_bar.close - prior_micro[0].close) / max(average_range, 1e-9)
    if direction > 0:
        room_to_midpoint = (session_midpoint - signal_bar.close) / max(average_range, 1e-9)
    else:
        room_to_midpoint = (signal_bar.close - session_midpoint) / max(average_range, 1e-9)
    ...
    participation_exhaustion = max(previous_volume_ratio, micro_volume_ratio) - current_volume_ratio
    prior_day_excess_context = 0.0
    if context.prior_day_range is not None and context.prior_day_range > 0:
        if direction > 0 and context.prior_day_low is not None:
            prior_day_excess_context = (context.prior_day_low - signal_bar.close) / context.prior_day_range
        if direction < 0 and context.prior_day_high is not None:
            prior_day_excess_context = (signal_bar.close - context.prior_day_high) / context.prior_day_range

    if session_range_extension < MIN_SESSION_RANGE_EXTENSION and max(
        vwap_return_pressure,
        day_open_return_pressure,
        prior_close_return_pressure,
    ) < 0.12:
        return {"event_count": 0.0}
    if extreme_exposure < 0.52 and prior_day_excess_context < 0.02:
        return {"event_count": 0.0}
    if extreme_close_failure < MIN_EXTREME_CLOSE_FAILURE and micro_reversal_followthrough < -0.08:
        return {"event_count": 0.0}
    if snapback_impulse < MIN_SNAPBACK_IMPULSE and body_efficiency_failure < -0.14:
        return {"event_count": 0.0}
    ...
    activity = (
        max(session_range_extension - 0.65, -0.22) * 0.12
        + max(extreme_exposure - 0.48, -0.16) * 0.13
        + max(extreme_close_failure - 0.22, -0.18) * 0.16
        + max(snapback_impulse, -0.16) * 0.14
        + max(vwap_return_pressure, -0.30) * 0.10
        + max(day_open_return_pressure, -0.35) * 0.07
        + max(prior_close_return_pressure, -0.40) * 0.07
        + max(range_efficiency_decay - 0.18, -0.20) * 0.08
        + max(body_efficiency_failure, -0.28) * 0.08
        + max(adverse_wick_rejection, -0.25) * 0.06
        + max(participation_exhaustion, -0.55) * 0.05
        + max(micro_reversal_followthrough, -0.25) * 0.07
        + max(room_to_midpoint, -0.25) * 0.04
        + max(prior_day_excess_context, -0.20) * 0.03
    )
    if activity < MIN_RANGE_OVEREXTENSION_SNAPBACK_REVERSAL_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "session_range_extension": bounded(session_range_extension, 0.0, 16.0),
        "extreme_close_failure": bounded(extreme_close_failure, -1.0, 1.0),
        "snapback_impulse": bounded(snapback_impulse, -4.0, 4.0),
        "vwap_return_pressure": bounded(vwap_return_pressure, -5.0, 5.0),
        "day_open_return_pressure": bounded(day_open_return_pressure, -5.0, 5.0),
        "prior_close_return_pressure": bounded(prior_close_return_pressure, -5.0, 5.0),
        "range_efficiency_decay": bounded(range_efficiency_decay, -1.0, 1.0),
        "body_efficiency_failure": bounded(body_efficiency_failure, -1.0, 1.0),
        "adverse_wick_rejection": bounded(adverse_wick_rejection, -1.0, 1.0),
        "participation_exhaustion": bounded(participation_exhaustion, -6.0, 8.0),
        "micro_reversal_followthrough": bounded(micro_reversal_followthrough, -4.0, 4.0),
        "room_to_midpoint": bounded(room_to_midpoint, -3.0, 8.0),
        "prior_day_excess_context": bounded(prior_day_excess_context, -3.0, 3.0),
        ...
        "extreme_exposure": bounded(extreme_exposure, 0.0, 1.0),
        ...
        "micro_net": bounded(micro_reversal_followthrough, -4.0, 4.0),
        "range_overextension_snapback_reversal_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def C0141_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def C0141_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def C0141_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def C0141_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def C0141_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def C0141_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def C0141_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def C0141_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def C0141_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def C0141_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def C0141_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def C0141_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def C0141_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def C0141_pullback_containment_bucket(value: float) -> str:
    if value < 0.35:
...
def C0141_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def C0141_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def C0141_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def C0141_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def C0141_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def C0141_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    extension_reward = 0.08 if metrics["session_range_extension"] > 0.85 and terminal > 0 else 0.0
    failure_reward = 0.13 if metrics["extreme_close_failure"] > 0.22 and terminal > 0 else 0.0
    impulse_reward = 0.12 if metrics["snapback_impulse"] > 0.02 and terminal > 0 else 0.0
    vwap_reward = 0.09 if metrics["vwap_return_pressure"] > 0.05 and terminal > 0 else 0.0
    open_reward = 0.07 if metrics["day_open_return_pressure"] > -0.10 and terminal > 0 else 0.0
    prior_close_reward = 0.07 if metrics["prior_close_return_pressure"] > -0.12 and terminal > 0 else 0.0
    decay_reward = 0.08 if metrics["range_efficiency_decay"] > 0.18 and terminal > 0 else 0.0
    body_reward = 0.09 if metrics["body_efficiency_failure"] > -0.02 and terminal > 0 else 0.0
    wick_reward = 0.07 if metrics["adverse_wick_rejection"] > 0.10 and terminal > 0 else 0.0
    exhaustion_reward = 0.06 if metrics["participation_exhaustion"] > -0.10 and terminal > 0 else 0.0
    followthrough_reward = 0.07 if metrics["micro_reversal_followthrough"] > -0.04 and terminal > 0 else 0.0
    room_reward = 0.04 if metrics["room_to_midpoint"] > 0.08 and terminal > 0 else 0.0
    prior_day_reward = 0.03 if metrics["prior_day_excess_context"] > -0.12 and terminal > 0 else 0.0
    weak_extension_penalty = 0.06 if metrics["session_range_extension"] < 0.55 and terminal <= 0 else 0.0
    weak_failure_penalty = 0.10 if metrics["extreme_close_failure"] < 0.02 and terminal <= 0 else 0.0
    weak_impulse_penalty = 0.10 if metrics["snapback_impulse"] < -0.06 and terminal <= 0 else 0.0
    weak_vwap_penalty = 0.06 if metrics["vwap_return_pressure"] < -0.18 and terminal <= 0 else 0.0
    weak_decay_penalty = 0.06 if metrics["range_efficiency_decay"] < -0.05 and terminal <= 0 else 0.0
    weak_body_penalty = 0.07 if metrics["body_efficiency_failure"] < -0.18 and terminal <= 0 else 0.0
    weak_wick_penalty = 0.05 if metrics["adverse_wick_rejection"] < -0.18 and terminal <= 0 else 0.0
    no_exhaustion_penalty = 0.05 if metrics["participation_exhaustion"] < -0.50 and terminal <= 0 else 0.0
    no_room_penalty = 0.04 if metrics["room_to_midpoint"] < 0.02 and terminal <= 0 else 0.0
    ...
    return (
        ...
        + wick_reward
        + exhaustion_reward
        + followthrough_reward
        + room_reward
        + prior_day_reward
        - weak_extension_penalty
        - weak_failure_penalty
        - weak_impulse_penalty
        - weak_vwap_penalty
        - weak_decay_penalty
        - weak_body_penalty
        - weak_wick_penalty
        - no_exhaustion_penalty
        - no_room_penalty
        - stall_penalty
        ...
        - max(0.0, -metrics["participation_exhaustion"] - 0.75) * 0.015
        - max(0.0, MIN_EXTREME_CLOSE_FAILURE - metrics["extreme_close_failure"]) * 0.05
        - max(0.0, MIN_SNAPBACK_IMPULSE - metrics["snapback_impulse"]) * 0.08
    )


def fit_linear_range_overextension_snapback_reversal_model(candidates: list[base.Candidate], fold_id: str) -> LinearRangeOverextensionSnapbackReversalModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearRangeOverextensionSnapbackReversalModel(
            fold_id,
    ...
    range_overextension_snapback_reversal_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearRangeOverextensionSnapbackReversalModel(
        fold_id=fold_id,
        ...
        range_overextension_snapback_reversal_failure_rate=range_overextension_snapback_reversal_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearRangeOverextensionSnapbackReversalModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_range_overextension_snapback_reversal_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0141_r0001_intraday_range_overextension_snapback_reversal",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0141_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0141_r0001_range_overextension_snapback_reversal_summary.json",
        ],
        ...
        "conditional_profiles": {
            "range_overextension_snapback_reversal_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0141 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0141_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0141 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "range_overextension_snapback_reversal_lookback_bars": RANGE_OVEREXTENSION_SNAPBACK_REVERSAL_LOOKBACK_BARS,
        "range_overextension_snapback_reversal_micro_bars": RANGE_OVEREXTENSION_SNAPBACK_REVERSAL_MICRO_BARS,
        "min_session_range_extension": MIN_SESSION_RANGE_EXTENSION,
        "min_extreme_close_failure": MIN_EXTREME_CLOSE_FAILURE,
        "min_snapback_impulse": MIN_SNAPBACK_IMPULSE,
        "min_range_overextension_snapback_reversal_activity": MIN_RANGE_OVEREXTENSION_SNAPBACK_REVERSAL_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_range_overextension_snapback_reversal_model_summary(model: LinearRangeOverextensionSnapbackReversalModel) -> dict[str, object]:
    return {
        ...
        "range_overextension_snapback_reversal_failure_rate": rounded(model.range_overextension_snapback_reversal_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearRangeOverextensionSnapbackReversalModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_range_overextension_snapback_reversal_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "range_overextension_snapback_reversal_profile": profiles["range_overextension_snapback_reversal_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["next_required_action"] = "produce_c0141_r0001_mt5_logic_parity_evidence"
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0141_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0141_r0001_proxy_trades.csv"
    evidence["range_overextension_snapback_reversal_summary"] = "artifacts/c0141_r0001_range_overextension_snapback_reversal_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0141_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0141_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0141 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0141_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    next_work["tasks"] = ["produce_c0141_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0141_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0141_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0141_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0141_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0141_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0141_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0141 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0141 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    block = f"""
- decision_id: dec_20260708_produce_c0141_r0001_proxy_evidence
  created_local_date: '{local_date}'
  status: active
  decision: produce_c0141_r0001_proxy_evidence
  refines:
  - dec_20260708_open_c0141_r0001_intraday_range_overextension_snapback_reversal_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0141_r0001_proxy_records_range_overextension_snapback_reversal_reference_surface
  ...
  - next_work_is_produce_c0141_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260708_produce_c0141_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0141-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0141_r0001_intraday_range_overextension_snapback_reversal",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0141-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0141-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0141-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0141_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0141_r0001_intraday_range_overextension_snapback_reversal",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0141-R0001-ANCHOR-BRAID-SQUEEZE-ESCAPE-CONTINUATION-SUMMARY",
            "artifact_role": "range_overextension_snapback_reversal_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0141_r0001_range_overextension_snapback_reversal_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0141_r0001_intraday_range_overextension_snapback_reversal",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0141_r0001_mt5_logic_parity_evidence",
        }
    ]
