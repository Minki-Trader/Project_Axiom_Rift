"""C0024 R0001 proxy evidence for calendar phase rhythm discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0024_calendar_phase_rhythm_discovery" / "runs" / "R0001"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0024_calendar_phase_rhythm_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0024_r0001_proxy_trades.csv"
CALENDAR_PHASE_RHYTHM_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0024_r0001_calendar_phase_rhythm_summary.json"
...
FEATURE_NAMES = (
    ...
    "core_session_progress",
    "core_session_progress_squared",
    "minutes_from_core_mid",
    "minutes_from_us_cash_open",
    "minutes_to_core_close",
    "minute_of_day_sin",
    "minute_of_day_cos",
    "half_day_sin",
    "half_day_cos",
    ...
    "is_friday",
    "week_progress",
    "week_progress_squared",
    "month_day_sin",
    "month_day_cos",
    "month_progress",
    "month_progress_squared",
    "month_of_year_sin",
    "month_of_year_cos",
    "quarter_progress",
    "day_of_year_sin",
    "day_of_year_cos",
    "session_early",
    "session_europe_morning",
    "session_us_overlap",
    "session_late",
    "spread_over_range",
    "spread_relief_3_vs_24",
)
MODEL_FAMILY = "fold_local_calendar_phase_rhythm_rank"
LABEL_SHAPE = "directional_calendar_phase_rhythm_transition_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_calendar_phase_rhythm_scores_per_active_day"
...
def run_c0024_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = calendar_phase_rhythm_features(
                bars,
                range_average,
                short_range_average,
                day_context,
            ...
            if features is None:
                continue
            label = (
                calendar_phase_rhythm_label(bars, range_average, index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|calendar_phase_rhythm",
                    features=features,
                    label=label,
                )
            )
...
def calendar_phase_rhythm_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    minute = base.minute_of_day(bar.time)
    core_start = 6 * 60
    core_end = 21 * 60
    core_span = core_end - core_start
    core_progress = bounded((minute - core_start) / core_span, 0.0, 1.0)
    minute_fraction = minute / (24.0 * 60.0)
    half_day_fraction = (minute % (12 * 60)) / (12.0 * 60.0)
    hour_fraction = (minute % 60) / 60.0
    quarter_hour_fraction = (minute % 15) / 15.0
    weekday = bar.time.weekday()
    weekday_fraction = weekday / 5.0
    week_progress = bounded((weekday * 24.0 * 60.0 + minute) / (5.0 * 24.0 * 60.0), 0.0, 1.2)
    month_days = calendar.monthrange(bar.time.year, bar.time.month)[1]
    month_progress = (bar.time.day - 1 + minute_fraction) / month_days
    month_day_fraction = (bar.time.day - 1) / month_days
    month_fraction = (bar.time.month - 1) / 12.0
    quarter_progress = (((bar.time.month - 1) % 3) + month_progress) / 3.0
    day_of_year_fraction = (bar.time.timetuple().tm_yday - 1 + minute_fraction) / 366.0
    ...
    minutes_from_core_mid = (minute - (14 * 60 + 30)) / (24.0 * 60.0)
    minutes_from_us_cash_open = (minute - (14 * 60 + 30)) / (24.0 * 60.0)
    minutes_to_core_close = (core_end - minute) / (24.0 * 60.0)
    session_early = 1.0 if minute < 8 * 60 else 0.0
    session_europe_morning = 1.0 if 8 * 60 <= minute < 13 * 60 else 0.0
    session_us_overlap = 1.0 if 13 * 60 <= minute < 17 * 60 else 0.0
    session_late = 1.0 if minute >= 17 * 60 else 0.0
    ...
    return (
        ...
        minutes_from_us_cash_open,
        minutes_to_core_close,
        math.sin(2.0 * math.pi * minute_fraction),
        math.cos(2.0 * math.pi * minute_fraction),
        math.sin(2.0 * math.pi * half_day_fraction),
        math.cos(2.0 * math.pi * half_day_fraction),
        math.sin(2.0 * math.pi * hour_fraction),
        math.cos(2.0 * math.pi * hour_fraction),
        math.sin(2.0 * math.pi * quarter_hour_fraction),
        math.cos(2.0 * math.pi * quarter_hour_fraction),
        math.sin(2.0 * math.pi * weekday_fraction),
        math.cos(2.0 * math.pi * weekday_fraction),
        1.0 if weekday == 0 else 0.0,
        1.0 if weekday == 1 else 0.0,
        1.0 if weekday == 2 else 0.0,
        1.0 if weekday == 3 else 0.0,
        1.0 if weekday == 4 else 0.0,
        week_progress,
        week_progress * week_progress,
        math.sin(2.0 * math.pi * month_day_fraction),
        math.cos(2.0 * math.pi * month_day_fraction),
        month_progress,
        month_progress * month_progress,
        math.sin(2.0 * math.pi * month_fraction),
        math.cos(2.0 * math.pi * month_fraction),
        quarter_progress,
        math.sin(2.0 * math.pi * day_of_year_fraction),
        math.cos(2.0 * math.pi * day_of_year_fraction),
        session_early,
        session_europe_morning,
        session_us_overlap,
        session_late,
...
def calendar_phase_rhythm_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    index: int,
    direction: int,
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0024"
    payload["work_unit_id"] = "C0024"
    payload["run_id"] = "R0001"
    payload["proxy_id"] = "PX-C0024-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0024_r0001_calendar_phase_rhythm"
    payload["proxy_config_path"] = "campaigns/C0024_calendar_phase_rhythm_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_calendar_phase_rhythm_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0024_calendar_phase_rhythm_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0024_calendar_phase_rhythm_discovery/runs/R0001/artifacts/c0024_r0001_proxy_trades.csv",
        "campaigns/C0024_calendar_phase_rhythm_discovery/runs/R0001/artifacts/c0024_r0001_calendar_phase_rhythm_summary.json",
    ]
    ...
    profiles["calendar_phase_rhythm_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0024_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "feature_weight_floor": FEATURE_WEIGHT_FLOOR,
            "feature_weight_ceiling": FEATURE_WEIGHT_CEILING,
            "score_interpretation": "higher_score_means_direct_fold_local_calendar_phase_rhythm_rank",
            "variant_boundary": "calendar_phase_rhythm_rank_not_auction_threshold_monthly_memory_lifecycle_structural_trap_score_floor_stop_target_hold_or_retry_nudge",
        }
    )
    return config
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_calendar_phase_rhythm_summary_artifact(payload, CALENDAR_PHASE_RHYTHM_SUMMARY_PATH)
    PROXY_PATH.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="ascii")
    trade_hash = base.sha256_file(TRADE_ARTIFACT_PATH)
    summary_hash = base.sha256_file(CALENDAR_PHASE_RHYTHM_SUMMARY_PATH)
...
def write_calendar_phase_rhythm_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_calendar_phase_rhythm_summary_v1",
        "template": False,
        "work_unit_id": "C0024",
        "campaign_id": "C0024",
        "run_id": "R0001",
        ...
        "calendar_phase_rhythm_profile": profiles["calendar_phase_rhythm_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "calendar_phase_rhythm_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0024-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0024_calendar_phase_rhythm_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0024_calendar_phase_rhythm_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0024-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0024-R0001-CALENDAR-PHASE-RHYTHM-SUMMARY",
                "calendar_phase_rhythm_summary_artifact",
                "json",
                "campaigns/C0024_calendar_phase_rhythm_discovery/runs/R0001/artifacts/c0024_r0001_calendar_phase_rhythm_summary.json",
                summary_hash,
                ["campaigns/C0024_calendar_phase_rhythm_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0024_r0001_calendar_phase_rhythm",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0024_r0001_proxy_trades.csv"
    evidence["calendar_phase_rhythm_summary"] = "artifacts/c0024_r0001_calendar_phase_rhythm_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0024_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0024_r0001_proxy_trades.csv",
        "artifacts/c0024_r0001_calendar_phase_rhythm_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0024 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0024_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0024_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0024_calendar_phase_rhythm_discovery"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0024_calendar_phase_rhythm_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0024_calendar_phase_rhythm_discovery",
        "open_c0024_r0001_fold_local_calendar_phase_rhythm_rank_run",
        "produce_c0024_r0001_proxy_evidence",
    ):
    ...
    data["active_run"] = "campaigns/C0024_calendar_phase_rhythm_discovery/runs/R0001"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["active_campaign"] = "campaigns/C0024_calendar_phase_rhythm_discovery"
    data["active_run"] = "campaigns/C0024_calendar_phase_rhythm_discovery/runs/R0001"
    data["latest_operation"] = {
        "id": "produce_c0024_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0024_calendar_phase_rhythm_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0024_r0001_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0024",
            "c0004_r0001_fold_local_state_archetype": "c0024_r0001_calendar_phase_rhythm",
            "c0004_r0001": "c0024_r0001",
            "fold_local_state_archetype_discovery": "calendar_phase_rhythm_discovery",
            ...
            "state_archetype_summary": "calendar_phase_rhythm_summary",
        }
