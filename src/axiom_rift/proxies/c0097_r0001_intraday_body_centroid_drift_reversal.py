"""C0097 R0001 proxy evidence for intraday body centroid drift reversal."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0097_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0097_r0001_body_centroid_drift_reversal_summary.json"
...
FEATURE_NAMES = (
    "body_centroid_drift_pressure",
    "centroid_acceptance_failure",
    "body_range_contraction",
    "close_rejection_from_centroid",
    "counter_wick_confirmation",
    "volume_centroid_engagement",
    "vwap_centroid_reversion_pressure",
    "session_phase_maturity",
    "prior_day_centroid_context",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_body_centroid_drift_reversal_rank"
LABEL_SHAPE = "target_first_reversal_quality_conditioned_on_body_centroid_drift_reversal"
SELECTION_RULE = "top_fold_local_body_centroid_drift_reversal_scores_per_active_day"
...
MIN_DAY_BARS = 30
BODY_CENTROID_DRIFT_LOOKBACK_BARS = 22
BODY_CENTROID_DRIFT_STACK_BARS = 7
MIN_BODY_CENTROID_DRIFT_ACTIVITY = 0.06
...
class LinearBodyCentroidDriftReversalModel:
    fold_id: str
    ...
    body_centroid_drift_reversal_failure_rate: float
...
def run_c0097_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_body_centroid_drift_reversal_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_body_centroid_drift_reversal_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        BODY_CENTROID_DRIFT_LOOKBACK_BARS,
        BODY_CENTROID_DRIFT_STACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = body_centroid_drift_reversal_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        metrics["body_centroid_drift_pressure"],
        metrics["centroid_acceptance_failure"],
        metrics["body_range_contraction"],
        metrics["close_rejection_from_centroid"],
        metrics["counter_wick_confirmation"],
        metrics["volume_centroid_engagement"],
        metrics["vwap_centroid_reversion_pressure"],
        metrics["session_phase_maturity"],
        metrics["prior_day_centroid_context"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        centroid_drift_bucket(metrics["body_centroid_drift_pressure"]),
        centroid_failure_bucket(metrics["centroid_acceptance_failure"]),
        body_contraction_bucket(metrics["body_range_contraction"]),
        centroid_rejection_bucket(metrics["close_rejection_from_centroid"]),
        counter_wick_confirmation_bucket(metrics["counter_wick_confirmation"]),
        volume_centroid_bucket(metrics["volume_centroid_engagement"]),
        vwap_centroid_pressure_bucket(metrics["vwap_centroid_reversion_pressure"]),
        session_phase_maturity_bucket(metrics["session_phase_maturity"]),
        prior_centroid_context_bucket(metrics["prior_day_centroid_context"]),
...
def body_centroid_drift_reversal_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    stack_start = index - BODY_CENTROID_DRIFT_STACK_BARS
    lookback_start = index - BODY_CENTROID_DRIFT_LOOKBACK_BARS
    ...
    stack_range_mean = base.mean([bar.high - bar.low for bar in stack_window]) or average_range
    prior_range_mean = base.mean([bar.high - bar.low for bar in prior_window]) or average_range
    recent_centroid = base.mean([body_centroid(bar) for bar in stack_window]) or body_centroid(signal_bar)
    prior_centroid = base.mean([body_centroid(bar) for bar in prior_window]) or recent_centroid
    signal_centroid = body_centroid(signal_bar)
    recent_body_fraction = base.mean([body_fraction(bar) for bar in stack_window]) or 0.0
    prior_body_fraction = base.mean([body_fraction(bar) for bar in prior_window]) or 0.0
    body_centroid_drift_pressure = direction * (prior_centroid - recent_centroid) / max(average_range, 1e-9)
    centroid_acceptance_failure = direction * (signal_bar.close - recent_centroid) / max(average_range, 1e-9)
    body_range_contraction = prior_body_fraction - recent_body_fraction
    close_rejection_from_centroid = direction * (signal_bar.close - signal_centroid) / max(
        signal_bar.high - signal_bar.low,
        1e-9,
    )
    counter_wick_confirmation = directional_wick_rejection(signal_bar, direction)
    stack_volume_average = base.mean(volumes[stack_start:index]) or average_volume
    volume_centroid_engagement = volumes[index] / max(stack_volume_average, 1e-9) - 1.0
    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    vwap_centroid_reversion_pressure = direction * (session_vwap - signal_bar.close) / max(average_range, 1e-9)
    session_phase_maturity = bounded_session_progress(signal_bar.time)
    prior_day_centroid_context = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None:
        prior_day_centroid_context = -direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
    centroid_reversal_pressure_raw = max(
        max(body_centroid_drift_pressure - 0.08, 0.0) * 1.20,
        max(centroid_acceptance_failure, 0.0) * 0.95,
        max(body_range_contraction, 0.0) * 0.65,
        max(close_rejection_from_centroid, 0.0) * 0.85,
        max(counter_wick_confirmation, 0.0) * 0.45,
        max(volume_centroid_engagement, 0.0) * 0.18,
        max(vwap_centroid_reversion_pressure, 0.0) * 0.18,
    )
    activity = max(centroid_reversal_pressure_raw, centroid_reversal_pressure_raw * session_phase_maturity)
    if activity < MIN_BODY_CENTROID_DRIFT_ACTIVITY:
        return {"event_count": 0.0}
    if body_centroid_drift_pressure < 0.05 and centroid_acceptance_failure < 0.04:
        return {"event_count": 0.0}
    if close_rejection_from_centroid < -0.08 and centroid_acceptance_failure < 0.14:
        return {"event_count": 0.0}
    if volume_centroid_engagement < -0.65 and counter_wick_confirmation < 0.08:
        return {"event_count": 0.0}
    return {
        "body_centroid_drift_pressure": bounded(body_centroid_drift_pressure, -5.0, 5.0),
        "centroid_acceptance_failure": bounded(centroid_acceptance_failure, -5.0, 5.0),
        "body_range_contraction": bounded(body_range_contraction, -1.0, 1.0),
        "close_rejection_from_centroid": bounded(close_rejection_from_centroid, -1.0, 1.0),
        "counter_wick_confirmation": bounded(counter_wick_confirmation, -1.0, 1.0),
        "volume_centroid_engagement": bounded(volume_centroid_engagement, -3.0, 6.0),
        "vwap_centroid_reversion_pressure": bounded(vwap_centroid_reversion_pressure, -5.0, 5.0),
        "session_phase_maturity": bounded(session_phase_maturity, 0.0, 1.0),
        "prior_day_centroid_context": bounded(prior_day_centroid_context, -3.0, 3.0),
        ...
        "centroid_reversal_pressure": bounded(centroid_reversal_pressure_raw, 0.0, 5.0),
        "event_count": 1.0,
    }
...
def centroid_drift_bucket(value: float) -> str:
    if value < 0.08:
        return "centroid_drift_thin"
    if value < 0.65:
        return "centroid_drift_defined"
    return "centroid_drift_stretched"


def centroid_failure_bucket(value: float) -> str:
    if value < -0.05:
        return "centroid_failure_absent"
    if value < 0.20:
        return "centroid_failure_mixed"
    return "centroid_failure_clear"


def body_contraction_bucket(value: float) -> str:
    if value < -0.08:
        return "body_expanding"
    if value < 0.12:
        return "body_contraction_mixed"
    return "body_contraction_clear"


def centroid_rejection_bucket(value: float) -> str:
    if value < -0.08:
        return "centroid_rejection_against"
    if value < 0.12:
        return "centroid_rejection_mixed"
    return "centroid_rejection_confirmed"
...
def volume_centroid_bucket(value: float) -> str:
    if value < -0.20:
        return "centroid_volume_fading"
    if value < 0.25:
        return "centroid_volume_even"
    return "centroid_volume_engaged"


def vwap_centroid_pressure_bucket(value: float) -> str:
    if value < 0.08:
        return "vwap_centroid_pressure_light"
    if value < 0.75:
        return "vwap_centroid_pressure_defined"
    return "vwap_centroid_pressure_heavy"


def prior_centroid_context_bucket(value: float) -> str:
    if value < -0.20:
        return "prior_centroid_context_conflict"
    if value < 0.35:
        return "prior_centroid_context_neutral"
    return "prior_centroid_context_supportive"
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    drift_reward = 0.13 if metrics["body_centroid_drift_pressure"] > 0.22 and terminal > 0 else 0.0
    acceptance_failure_reward = 0.14 if metrics["centroid_acceptance_failure"] > 0.10 and terminal > 0 else 0.0
    contraction_reward = 0.08 if metrics["body_range_contraction"] > 0.08 and terminal > 0 else 0.0
    rejection_reward = 0.12 if metrics["close_rejection_from_centroid"] > 0.12 and terminal > 0 else 0.0
    counter_wick_reward = 0.07 if metrics["counter_wick_confirmation"] > 0.10 and terminal > 0 else 0.0
    volume_reward = 0.06 if metrics["volume_centroid_engagement"] > 0.10 and terminal > 0 else 0.0
    vwap_reward = 0.07 if metrics["vwap_centroid_reversion_pressure"] > 0.08 and terminal > 0 else 0.0
    failed_reversal_penalty = 0.16 if metrics["centroid_acceptance_failure"] < -0.10 and terminal <= 0 else 0.0
    drift_trap_penalty = 0.13 if metrics["body_centroid_drift_pressure"] > 0.30 and terminal <= 0 else 0.0
    expansion_penalty = 0.09 if metrics["body_range_contraction"] < -0.08 and terminal <= 0 else 0.0
    weak_volume_penalty = 0.06 if metrics["volume_centroid_engagement"] < -0.30 and terminal <= 0 else 0.0
    vwap_continuation_penalty = (
        0.08 if metrics["vwap_centroid_reversion_pressure"] < -0.18 and terminal <= 0 else 0.0
    )
    ...
    body_centroid_drift_reversal_path_quality = (0.54 * terminal + 0.30 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + body_centroid_drift_reversal_path_quality
        + drift_reward
        + acceptance_failure_reward
        + contraction_reward
        + rejection_reward
        + counter_wick_reward
        + volume_reward
...
def fit_linear_body_centroid_drift_reversal_model(candidates: list[base.Candidate], fold_id: str) -> LinearBodyCentroidDriftReversalModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearBodyCentroidDriftReversalModel(
            fold_id,
    ...
    body_centroid_drift_reversal_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearBodyCentroidDriftReversalModel(
        fold_id=fold_id,
        ...
        body_centroid_drift_reversal_failure_rate=body_centroid_drift_reversal_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearBodyCentroidDriftReversalModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_body_centroid_drift_reversal_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0097_r0001_intraday_body_centroid_drift_reversal",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0097_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0097_r0001_body_centroid_drift_reversal_summary.json",
        ],
        ...
        "conditional_profiles": {
            "body_centroid_drift_reversal_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0097 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0097_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0097 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "body_centroid_drift_reversal_lookback_bars": BODY_CENTROID_DRIFT_LOOKBACK_BARS,
        "body_centroid_drift_reversal_stack_bars": BODY_CENTROID_DRIFT_STACK_BARS,
        "min_body_centroid_drift_reversal_activity": MIN_BODY_CENTROID_DRIFT_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_body_centroid_drift_reversal_model_summary(model: LinearBodyCentroidDriftReversalModel) -> dict[str, object]:
    return {
        ...
        "body_centroid_drift_reversal_failure_rate": rounded(model.body_centroid_drift_reversal_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearBodyCentroidDriftReversalModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_body_centroid_drift_reversal_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "body_centroid_drift_reversal_profile": profiles["body_centroid_drift_reversal_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0097_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0097_r0001_proxy_trades.csv"
    evidence["body_centroid_drift_reversal_summary"] = "artifacts/c0097_r0001_body_centroid_drift_reversal_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0097_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0097_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0097 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0097_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0097_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0097_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0097_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0097_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0097_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0097_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0097_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0097_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0097 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0097 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0097_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0097_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0097_r0001_intraday_body_centroid_drift_reversal_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0097_r0001_proxy_records_body_centroid_drift_reversal_reference_surface
  ...
  - next_work_is_produce_c0097_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0097_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0097-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0097_r0001_intraday_body_centroid_drift_reversal",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0097-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0097-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0097-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0097_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0097_r0001_intraday_body_centroid_drift_reversal",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0097-R0001-body-centroid-drift-reversal-SUMMARY",
            "artifact_role": "body_centroid_drift_reversal_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0097_r0001_body_centroid_drift_reversal_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0097_r0001_intraday_body_centroid_drift_reversal",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0097_r0001_mt5_logic_parity_evidence",
        }
    ]
