"""C0006 R0002 proxy evidence for sweep acceptance continuation events."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0006_liquidity_sweep_reclaim_event_discovery" / "runs" / "R0002"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0006_r0002_proxy_trades.csv"
EVENT_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0006_r0002_sweep_acceptance_continuation_summary.json"
...
FEATURE_NAMES = (
    ...
    "acceptance_depth_over_range",
    "breakout_wick_fraction",
    "close_beyond_level_fraction",
...
MODEL_FAMILY = "fold_local_train_only_sweep_acceptance_continuation_event_utility"
LABEL_SHAPE = "event_acceptance_directional_target_before_stop_plus_path_quality"
...
MIN_ACCEPTANCE_OVER_BAR_RANGE = 0.03
SCORE_COMPONENT_NAMES = (
    "signed_feature_utility",
    "event_acceptance_bonus",
    "scope_bonus",
    "spread_penalty",
)
...
def run_c0006_r0002_proxy(write: bool = True) -> dict[str, object]:
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            event = sweep_acceptance_features(bars, range_average, short_range_average, index, direction)
            if event is None:
                continue
            scope, features = event
            label = sweep_acceptance_continuation_label(bars, range_average, index, direction) if include_labels else None
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|{scope}|sweep_acceptance_continuation",
                    features=features,
                    label=label,
                )
            )
...
def sweep_acceptance_features(
    ...
    if direction > 0:
        candidates.extend(
            [
                ("local", local_high, max(bar.high - local_high, 0.0), max(bar.close - local_high, 0.0), local_high - local_low),
                ("wide", wide_high, max(bar.high - wide_high, 0.0), max(bar.close - wide_high, 0.0), wide_high - wide_low),
                ("session", session_extreme, max(bar.high - session_extreme, 0.0), max(bar.close - session_extreme, 0.0), wide_high - wide_low),
            ]
        )
    else:
        candidates.extend(
            [
                ("local", local_low, max(local_low - bar.low, 0.0), max(local_low - bar.close, 0.0), local_high - local_low),
                ("wide", wide_low, max(wide_low - bar.low, 0.0), max(wide_low - bar.close, 0.0), wide_high - wide_low),
                ("session", session_extreme, max(session_extreme - bar.low, 0.0), max(session_extreme - bar.close, 0.0), wide_high - wide_low),
            ]
        )
    valid = [
        row
        for row in candidates
        if row[2] / average_range >= MIN_SWEEP_DEPTH_OVER_RANGE and row[3] / bar_range >= MIN_ACCEPTANCE_OVER_BAR_RANGE
    ]
    if not valid:
        return None
    scope, level, sweep_depth, acceptance_depth, event_range_width = max(
        valid,
        key=lambda row: ({"session": 3, "wide": 2, "local": 1}[row[0]], row[2], row[3]),
    )
    ...
    breakout_wick = upper_wick if direction > 0 else lower_wick
    close_beyond_level = acceptance_depth / bar_range
    mid = (local_high + local_low) / 2.0
    distance_from_mid = direction * (bar.close - mid) / max(local_high - local_low, 1e-9)
    ...
    return scope, (
        ...
        acceptance_depth / average_range,
        breakout_wick / bar_range,
        close_beyond_level,
...
def prior_session_extreme(bars: list[base.Bar], index: int, direction: int) -> float | None:
    ...
    if len(session_bars) < 6:
        return None
    return max(bar.high for bar in session_bars) if direction > 0 else min(bar.low for bar in session_bars)


def sweep_acceptance_continuation_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    index: int,
    direction: int,
...
def score_event_candidates(candidates: list[base.Candidate], model: EventUtilityModel) -> list[base.Candidate]:
    ...
    acceptance_depth = features[:, FEATURE_NAMES.index("acceptance_depth_over_range")]
    breakout_wick = features[:, FEATURE_NAMES.index("breakout_wick_fraction")]
    close_beyond = features[:, FEATURE_NAMES.index("close_beyond_level_fraction")]
    ...
    scores = (
        0.62 * signed_utility
        + 0.22 * np.tanh(2.2 * acceptance_depth)
...
def build_proxy_payload(
    ...
    payload["proxy_id"] = "PX-C0006-R0002"
    payload["proxy_engine"] = "axiom_rift.proxies.c0006_r0002_sweep_acceptance_continuation"
    payload["proxy_config_path"] = "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0002/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_sweep_acceptance_continuation_event_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0002/kpi/proxy.json",
        "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0002/artifacts/c0006_r0002_proxy_trades.csv",
        "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0002/artifacts/c0006_r0002_sweep_acceptance_continuation_summary.json",
    ]
    ...
    profiles["sweep_acceptance_continuation_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
            ...
            "min_acceptance_over_bar_range": MIN_ACCEPTANCE_OVER_BAR_RANGE,
            "score_component_names": list(SCORE_COMPONENT_NAMES),
            "fold_models": fold_models,
            "state_distributions": state_distributions,
            "candidates_by_fold": candidates_by_fold,
            "selection_rule": "top_fold_local_sweep_acceptance_continuation_scores_per_active_day",
    ...
    config.update(
        {
            ...
            "min_acceptance_over_bar_range": MIN_ACCEPTANCE_OVER_BAR_RANGE,
            "score_component_names": list(SCORE_COMPONENT_NAMES),
            "score_interpretation": "higher_score_means_train_fold_event_utility_supports_breakout_acceptance_follow_through",
            "variant_boundary": "new_sweep_acceptance_continuation_event_grammar_not_analog_memory_state_bucket_score_cell_exit_shape_or_parameter_nudge",
        }
    )
    return config
...
def write_event_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_sweep_acceptance_continuation_summary_v1",
        "template": False,
        "work_unit_id": "C0006",
        "campaign_id": "C0006",
        "run_id": "R0002",
        ...
        "sweep_acceptance_continuation_profile": profiles["sweep_acceptance_continuation_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "sweep_acceptance_continuation_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0006-R0002-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0002/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0002/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0006-R0002-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0006-R0002-EVENT-UTILITY-SUMMARY",
                "sweep_acceptance_continuation_summary_artifact",
                "json",
                "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0002/artifacts/c0006_r0002_sweep_acceptance_continuation_summary.json",
                summary_hash,
                ["campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0002/kpi/proxy.json"],
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0006_r0002_mt5_logic_parity_evidence",
        }
    ]
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0006_r0002_sweep_acceptance_continuation",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0006_r0002_proxy_trades.csv"
    evidence["sweep_acceptance_continuation_summary"] = "artifacts/c0006_r0002_sweep_acceptance_continuation_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0006_r0002_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0006_r0002_proxy_trades.csv",
        "artifacts/c0006_r0002_sweep_acceptance_continuation_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0006 R0002 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0006_r0002_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0006_r0002_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    if "produce_c0006_r0002_proxy_evidence" not in completed:
        completed.append("produce_c0006_r0002_proxy_evidence")
    next_action = "produce_c0006_r0002_mt5_logic_parity_evidence"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0006_r0002_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0002/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0006_r0002_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            ...
            "R0002": "R0002",
            "R0002": "R0002",
            "c0004_r0001_fold_local_state_archetype": "c0006_r0002_sweep_acceptance_continuation",
            "c0004_r0001": "c0006_r0002",
            "fold_local_state_archetype_discovery": "sweep_acceptance_continuation_event_discovery",
