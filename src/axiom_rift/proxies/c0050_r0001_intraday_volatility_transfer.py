"""C0050 R0001 proxy evidence for intraday volatility transfer."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0050_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0050_r0001_volatility_transfer_summary.json"
...
FEATURE_NAMES = (
    "prior_session_range_atr",
    "prior_session_directional_return_atr",
    "current_session_opening_range_atr",
    "current_session_opening_directional_impulse_atr",
    "session_realized_volatility_transfer",
    "range_compression_12_vs_48",
    "directional_volatility_adjusted_momentum_12",
    "directional_range_transfer_position",
    "intraday_volatility_normalized_location",
    "participation_transfer_rate",
    "spread_volatility_stress",
    "session_progress",
)
MODEL_FAMILY = "fold_local_intraday_volatility_transfer_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_volatility_transfer_state"
SELECTION_RULE = "top_fold_local_volatility_transfer_scores_per_active_day"
...
class LinearVolatilityTransferModel:
    fold_id: str
...
def run_c0050_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_volatility_transfer_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_volatility_transfer_model_summary(model))
...
def candidate_state_and_features(
    ...
    if (
        ...
        or context.prior_day_volume_per_bar is None
        or context.prior_day_volume_per_bar <= 0
    ):
        return None, None
    session_range = context.high_so_far - context.low_so_far
    if session_range <= 0:
        return None, None
    prior_range_atr = context.prior_day_range / average_range
    prior_directional_return = direction * context.prior_day_return / average_range
    opening_range_atr = (context.opening_high - context.opening_low) / average_range
    opening_impulse = direction * (bar.close - context.day_open) / average_range
    realized_transfer = session_range / max(context.prior_day_range, average_range)
    compression = short_average_range / average_range
    directional_momentum = direction * (bar.close - bars[index - 12].close) / short_average_range
    range_position = directional_session_position(bar.close, context, direction)
    normalized_location = direction * (bar.close - context.day_open) / max(session_range, average_range)
    participation_transfer = (context.volume_so_far / max(float(context.bars_so_far), 1.0)) / context.prior_day_volume_per_bar
    ...
    if range_position is None or volume_ratio is None:
        return None, None
    features = (
        prior_range_atr,
        prior_directional_return,
        opening_range_atr,
        opening_impulse,
        realized_transfer,
        compression,
        ...
        range_position,
        normalized_location,
        participation_transfer,
        -spread_stress,
        session_progress,
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        prior_range_bucket(prior_range_atr),
        transfer_bucket(realized_transfer),
        impulse_bucket(opening_impulse),
        compression_bucket(compression),
        location_bucket(range_position),
        participation_bucket(participation_transfer),
...
def transfer_bucket(value: float) -> str:
    if value < 0.45:
        return "transfer_low"
    if value < 0.95:
        return "transfer_mid"
    return "transfer_high"
...
def candidate_label(
    ...
    transfer_quality = (0.30 * terminal + 0.30 * mfe - 0.60 * mae + 0.15 * future_range) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return first_hit + transfer_quality - stall_penalty - adverse_expansion_penalty - same_bar_penalty - spread_penalty


def fit_linear_volatility_transfer_model(candidates: list[base.Candidate], fold_id: str) -> LinearVolatilityTransferModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearVolatilityTransferModel(fold_id, tuple(0.0 for _ in FEATURE_NAMES), tuple(1.0 for _ in FEATURE_NAMES), tuple(0.0 for _ in FEATURE_NAMES), 0, 0.0, 0.0, 0.0)
    ...
    transfer_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearVolatilityTransferModel(
        fold_id=fold_id,
...
def score_candidate(candidate: base.Candidate, model: LinearVolatilityTransferModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_volatility_transfer_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0050_r0001_intraday_volatility_transfer",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0050_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0050_r0001_volatility_transfer_summary.json",
        ],
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0050 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0050_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0050 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def linear_volatility_transfer_model_summary(model: LinearVolatilityTransferModel) -> dict[str, object]:
    return {
        ...
        "transfer_failure_rate": rounded(model.transfer_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearVolatilityTransferModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_volatility_transfer_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "volatility_transfer_profile": profiles["volatility_transfer_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0050_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_trade_artifact"] = "artifacts/c0050_r0001_proxy_trades.csv"
    evidence["volatility_transfer_summary"] = "artifacts/c0050_r0001_volatility_transfer_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0050_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0050_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0050 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0050_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0050_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0050_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0050_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0050_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0050_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0050_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0050_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0050_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0050 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0050 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260706_produce_c0050_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0050_r0001_proxy_evidence
  refines:
  - dec_20260706_open_c0050_r0001_intraday_volatility_transfer_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0050_r0001_proxy_records_volatility_transfer_reference_surface
  ...
  - next_work_is_produce_c0050_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260706_produce_c0050_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0050-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0050_r0001_intraday_volatility_transfer",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0050-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0050-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0050-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0050_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0050_r0001_intraday_volatility_transfer",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0050-R0001-VOLATILITY-TRANSFER-SUMMARY",
            "artifact_role": "volatility_transfer_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0050_r0001_volatility_transfer_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0050_r0001_intraday_volatility_transfer",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0050_r0001_mt5_logic_parity_evidence",
        }
    ]
