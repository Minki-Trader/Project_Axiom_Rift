"""C0077 R0001 proxy evidence for intraday impulse digestion asymmetry."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0077_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0077_r0001_ida_summary.json"
...
FEATURE_NAMES = (
    "impulse_directional_range",
    "impulse_body_efficiency",
    "digestion_compression_ratio",
    "retained_impulse_ratio",
    "adverse_overlap_inverse",
    "digestion_volume_cooldown",
    "rejection_absorption_balance",
    "close_hold_ratio",
    "prior_day_impulse_context",
    "session_maturity_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_impulse_digestion_asymmetry_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_impulse_digestion_asymmetry_state"
SELECTION_RULE = "top_fold_local_impulse_digestion_asymmetry_scores_per_active_day"
...
MIN_DAY_BARS = 30
IMPULSE_DIGESTION_ASYMMETRY_LOOKBACK_BARS = 28
IMPULSE_DIGESTION_ASYMMETRY_SHORT_LOOKBACK_BARS = 8
IMPULSE_DIGESTION_ASYMMETRY_IMPULSE_BARS = 12
MIN_IMPULSE_DIGESTION_ASYMMETRY_ACTIVITY = 0.28
...
class LinearImpulseDigestionAsymmetryModel:
    fold_id: str
    ...
    impulse_digestion_asymmetry_failure_rate: float
...
def run_c0077_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_impulse_digestion_asymmetry_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_impulse_digestion_asymmetry_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        IMPULSE_DIGESTION_ASYMMETRY_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    release = impulse_digestion_asymmetry_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        release["impulse_directional_range"],
        release["impulse_body_efficiency"],
        release["digestion_compression_ratio"],
        release["retained_impulse_ratio"],
        release["adverse_overlap_inverse"],
        release["digestion_volume_cooldown"],
        release["rejection_absorption_balance"],
        release["close_hold_ratio"],
        release["prior_day_impulse_context"],
        release["session_maturity_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        impulse_range_bucket(release["impulse_directional_range"]),
        impulse_efficiency_bucket(release["impulse_body_efficiency"]),
        digestion_compression_bucket(release["digestion_compression_ratio"]),
        retention_bucket(release["retained_impulse_ratio"]),
        adverse_overlap_bucket(release["adverse_overlap_inverse"]),
        volume_cooldown_bucket(release["digestion_volume_cooldown"]),
        absorption_balance_bucket(release["rejection_absorption_balance"]),
        close_hold_bucket(release["close_hold_ratio"]),
        prior_impulse_context_bucket(release["prior_day_impulse_context"]),
...
def impulse_digestion_asymmetry_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    if prior_day_range is None or prior_day_range <= 0 or prior_day_return is None:
        return {"event_count": 0.0}
    window_start = index - IMPULSE_DIGESTION_ASYMMETRY_LOOKBACK_BARS + 1
    short_start = index - IMPULSE_DIGESTION_ASYMMETRY_SHORT_LOOKBACK_BARS + 1
    impulse_start = max(window_start, short_start - IMPULSE_DIGESTION_ASYMMETRY_IMPULSE_BARS)
    impulse_end = short_start
    if window_start < 0 or short_start < 0 or impulse_start < 0 or impulse_end <= impulse_start:
        return {"event_count": 0.0}
    impulse_window = bars[impulse_start:impulse_end]
    digestion_window = bars[short_start : index + 1]
    if len(impulse_window) < 5 or len(digestion_window) < 3:
        return {"event_count": 0.0}
    ...
    impulse_points_raw = direction * (impulse_close - impulse_open)
    impulse_directional_range = impulse_points_raw / max(average_range, 1e-9)
    impulse_travel = abs(impulse_close - impulse_open) + sum(
        abs(impulse_window[pos].close - impulse_window[pos - 1].close)
        for pos in range(1, len(impulse_window))
    )
    impulse_body_efficiency = max(0.0, impulse_points_raw / max(impulse_travel, 1e-9))
    digestion_range = max(bar.high for bar in digestion_window) - min(bar.low for bar in digestion_window)
    digestion_compression_ratio = 1.0 / (1.0 + digestion_range / max(average_range, 1e-9))
    impulse_points = max(impulse_points_raw, average_range)
    retained_impulse_ratio = direction * (signal_bar.close - impulse_open) / max(impulse_points, 1e-9)
    if direction > 0:
        adverse_retrace = max(0.0, max(impulse_close, impulse_open) - min(bar.low for bar in digestion_window))
    else:
        adverse_retrace = max(0.0, max(bar.high for bar in digestion_window) - min(impulse_close, impulse_open))
    adverse_overlap_inverse = 1.0 / (1.0 + adverse_retrace / max(impulse_points, average_range, 1e-9))
    impulse_volume_mean = base.mean(volumes[impulse_start:impulse_end]) or average_volume
    digestion_volume_mean = base.mean(volumes[short_start : index + 1]) or average_volume
    digestion_volume_cooldown = (impulse_volume_mean - digestion_volume_mean) / max(impulse_volume_mean, 1e-9)
    rejection_absorption_balance = (
        base.mean([directional_wick_rejection(bar, direction) for bar in digestion_window]) or 0.0
    )
    close_hold_ratio = base.mean([directional_bar_settlement(bar, direction) for bar in digestion_window]) or 0.0
    prior_day_impulse_context = direction * prior_day_return / max(prior_day_range, 1e-9)
    digestion_pressure = max(
        min(max(impulse_directional_range, 0.0) / 3.0, 1.0),
        impulse_body_efficiency,
        digestion_compression_ratio,
        min(max(retained_impulse_ratio, 0.0), 1.0),
        adverse_overlap_inverse,
        max(0.0, digestion_volume_cooldown),
        max(0.0, rejection_absorption_balance),
        close_hold_ratio,
    )
    session_maturity_pressure = bounded_session_progress(signal_bar.time) * digestion_pressure
    activity = max(digestion_pressure, session_maturity_pressure)
    if activity < MIN_IMPULSE_DIGESTION_ASYMMETRY_ACTIVITY:
        return {"event_count": 0.0}
    if impulse_directional_range < 0.55:
        return {"event_count": 0.0}
    if retained_impulse_ratio < 0.30 and close_hold_ratio < 0.52:
        return {"event_count": 0.0}
    if digestion_compression_ratio < 0.22 and adverse_overlap_inverse < 0.35:
        return {"event_count": 0.0}
    return {
        "impulse_directional_range": bounded(impulse_directional_range, -4.0, 8.0),
        "impulse_body_efficiency": bounded(impulse_body_efficiency, 0.0, 1.0),
        "digestion_compression_ratio": bounded(digestion_compression_ratio, 0.0, 1.0),
        "retained_impulse_ratio": bounded(retained_impulse_ratio, -1.0, 2.0),
        "adverse_overlap_inverse": bounded(adverse_overlap_inverse, 0.0, 1.0),
        "digestion_volume_cooldown": bounded(digestion_volume_cooldown, -1.0, 1.0),
        "rejection_absorption_balance": bounded(rejection_absorption_balance, -1.0, 1.0),
        "close_hold_ratio": bounded(close_hold_ratio, 0.0, 1.0),
        "prior_day_impulse_context": bounded(prior_day_impulse_context, -2.0, 2.0),
...
def candidate_label(
    ...
    metrics = impulse_digestion_asymmetry_metrics(
        bars,
        volumes,
    ...
    if metrics["event_count"] <= 0:
        return None
    impulse_range_reward = 0.15 if 0.55 <= metrics["impulse_directional_range"] <= 3.20 and terminal > 0 else 0.0
    efficiency_reward = 0.13 if metrics["impulse_body_efficiency"] > 0.45 and terminal > 0 else 0.0
    compression_reward = 0.14 if metrics["digestion_compression_ratio"] > 0.45 and terminal > 0 else 0.0
    retention_reward = 0.16 if metrics["retained_impulse_ratio"] > 0.62 and terminal > 0 else 0.0
    shallow_overlap_reward = 0.13 if metrics["adverse_overlap_inverse"] > 0.55 and terminal > 0 else 0.0
    cooldown_reward = 0.08 if metrics["digestion_volume_cooldown"] > 0.05 and terminal > 0 else 0.0
    absorption_reward = 0.10 if metrics["rejection_absorption_balance"] > 0.08 and terminal > 0 else 0.0
    close_hold_reward = 0.12 if metrics["close_hold_ratio"] > 0.62 and terminal > 0 else 0.0
    prior_context_reward = 0.06 if metrics["prior_day_impulse_context"] > -0.25 and terminal > 0 else 0.0
    thin_impulse_penalty = 0.14 if metrics["impulse_directional_range"] < 0.70 and terminal <= 0 else 0.0
    inefficient_impulse_penalty = 0.10 if metrics["impulse_body_efficiency"] < 0.28 and terminal <= 0 else 0.0
    poor_compression_penalty = 0.13 if metrics["digestion_compression_ratio"] < 0.34 and terminal <= 0 else 0.0
    low_retention_penalty = 0.16 if metrics["retained_impulse_ratio"] < 0.42 and terminal <= 0 else 0.0
    deep_overlap_penalty = 0.15 if metrics["adverse_overlap_inverse"] < 0.45 and terminal <= 0 else 0.0
    rising_volume_penalty = 0.10 if metrics["digestion_volume_cooldown"] < -0.05 and terminal <= 0 else 0.0
    weak_absorption_penalty = 0.10 if metrics["rejection_absorption_balance"] < -0.06 and terminal <= 0 else 0.0
    weak_hold_penalty = 0.12 if metrics["close_hold_ratio"] < 0.48 and terminal <= 0 else 0.0
    ...
    impulse_digestion_asymmetry_path_quality = (0.44 * terminal + 0.36 * mfe - 0.72 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + impulse_digestion_asymmetry_path_quality
        + impulse_range_reward
        + efficiency_reward
        + compression_reward
        + retention_reward
        + shallow_overlap_reward
        + cooldown_reward
        + absorption_reward
        + close_hold_reward
        + prior_context_reward
        - thin_impulse_penalty
        - inefficient_impulse_penalty
        - poor_compression_penalty
        - low_retention_penalty
        - deep_overlap_penalty
        - rising_volume_penalty
        ...
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["impulse_directional_range"] - 3.20) * 0.06
    )


def fit_linear_impulse_digestion_asymmetry_model(candidates: list[base.Candidate], fold_id: str) -> LinearImpulseDigestionAsymmetryModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearImpulseDigestionAsymmetryModel(
            fold_id,
    ...
    impulse_digestion_asymmetry_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearImpulseDigestionAsymmetryModel(
        fold_id=fold_id,
        ...
        impulse_digestion_asymmetry_failure_rate=impulse_digestion_asymmetry_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearImpulseDigestionAsymmetryModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_impulse_digestion_asymmetry_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0077_r0001_intraday_impulse_digestion_asymmetry",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0077_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0077_r0001_ida_summary.json",
        ],
        ...
        "conditional_profiles": {
            "impulse_digestion_asymmetry_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0077 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0077_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0077 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "impulse_digestion_asymmetry_lookback_bars": IMPULSE_DIGESTION_ASYMMETRY_LOOKBACK_BARS,
        "impulse_digestion_asymmetry_short_lookback_bars": IMPULSE_DIGESTION_ASYMMETRY_SHORT_LOOKBACK_BARS,
        "impulse_digestion_asymmetry_impulse_bars": IMPULSE_DIGESTION_ASYMMETRY_IMPULSE_BARS,
        "min_impulse_digestion_asymmetry_activity": MIN_IMPULSE_DIGESTION_ASYMMETRY_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_impulse_digestion_asymmetry_model_summary(model: LinearImpulseDigestionAsymmetryModel) -> dict[str, object]:
    return {
        ...
        "impulse_digestion_asymmetry_failure_rate": rounded(model.impulse_digestion_asymmetry_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearImpulseDigestionAsymmetryModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_impulse_digestion_asymmetry_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "impulse_digestion_asymmetry_profile": profiles["impulse_digestion_asymmetry_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0077_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0077_r0001_proxy_trades.csv"
    evidence["impulse_digestion_asymmetry_summary"] = "artifacts/c0077_r0001_ida_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0077_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0077_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0077 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0077_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0077_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0077_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0077_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0077_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0077_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0077_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0077_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0077_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0077 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0077 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0077_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0077_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0077_r0001_intraday_impulse_digestion_asymmetry_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0077_r0001_proxy_records_impulse_digestion_asymmetry_reference_surface
  ...
  - next_work_is_produce_c0077_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0077_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0077-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0077_r0001_intraday_impulse_digestion_asymmetry",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0077-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0077-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0077-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0077_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0077_r0001_intraday_impulse_digestion_asymmetry",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0077-R0001-impulse-digestion-asymmetry-SUMMARY",
            "artifact_role": "impulse_digestion_asymmetry_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0077_r0001_ida_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0077_r0001_intraday_impulse_digestion_asymmetry",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0077_r0001_mt5_logic_parity_evidence",
        }
    ]
