"""C0095 R0001 proxy evidence for intraday close cluster escape."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0095_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0095_r0001_close_cluster_escape_summary.json"
...
FEATURE_NAMES = (
    "close_cluster_tension",
    "cluster_range_compression",
    "escape_body_commitment",
    "escape_close_displacement",
    "pre_escape_directional_drift",
    "volume_reengagement",
...
MODEL_FAMILY = "fold_local_intraday_close_cluster_escape_rank"
LABEL_SHAPE = "target_first_continuation_quality_conditioned_on_close_cluster_escape"
SELECTION_RULE = "top_fold_local_close_cluster_escape_scores_per_active_day"
...
MIN_DAY_BARS = 30
CLOSE_CLUSTER_ESCAPE_LOOKBACK_BARS = 20
CLOSE_CLUSTER_ESCAPE_CLUSTER_BARS = 8
MIN_CLOSE_CLUSTER_ESCAPE_ACTIVITY = 0.08
...
class LinearCloseClusterEscapeModel:
    fold_id: str
    ...
    close_cluster_escape_failure_rate: float
...
def run_c0095_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_close_cluster_escape_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_close_cluster_escape_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        CLOSE_CLUSTER_ESCAPE_LOOKBACK_BARS,
        CLOSE_CLUSTER_ESCAPE_CLUSTER_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = close_cluster_escape_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        metrics["close_cluster_tension"],
        metrics["cluster_range_compression"],
        metrics["escape_body_commitment"],
        metrics["escape_close_displacement"],
        metrics["pre_escape_directional_drift"],
    ...
    dimensions = (
        close_commitment_bucket(metrics["close_cluster_tension"]),
        range_compression_bucket(metrics["cluster_range_compression"]),
        directional_body_commitment_bucket(metrics["escape_body_commitment"]),
        close_escape_bucket(metrics["escape_close_displacement"]),
        path_efficiency_bucket(metrics["pre_escape_directional_drift"]),
        volume_reengagement_bucket(metrics["volume_reengagement"]),
        vwap_slope_alignment_bucket(metrics["vwap_escape_alignment"]),
        wick_containment_bucket(metrics["wick_containment"]),
        session_phase_maturity_bucket(metrics["session_phase_maturity"]),
        prior_day_energy_bucket(metrics["prior_day_energy_context"]),
...
def close_cluster_escape_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    cluster_start = index - CLOSE_CLUSTER_ESCAPE_CLUSTER_BARS
    lookback_start = index - CLOSE_CLUSTER_ESCAPE_LOOKBACK_BARS
    if cluster_start < 0 or lookback_start < 0 or context.day_start_index >= index:
        return {"event_count": 0.0}
    cluster_start = max(cluster_start, context.day_start_index)
    lookback_start = max(lookback_start, context.day_start_index)
    cluster_window = bars[cluster_start:index]
    prior_window = bars[lookback_start:cluster_start]
    if len(cluster_window) < 5 or len(prior_window) < 6:
        return {"event_count": 0.0}
    ...
    cluster_closes = [bar.close for bar in cluster_window]
    cluster_high = max(cluster_closes)
    cluster_low = min(cluster_closes)
    cluster_mid = (cluster_high + cluster_low) / 2.0
    cluster_close_width = (cluster_high - cluster_low) / max(average_range, 1e-9)
    cluster_range_mean = base.mean([bar.high - bar.low for bar in cluster_window]) or average_range
    prior_range_mean = base.mean([bar.high - bar.low for bar in prior_window]) or average_range
    cluster_range_compression = (prior_range_mean - cluster_range_mean) / max(prior_range_mean, 1e-9)
    close_cluster_tension = (1.0 / (1.0 + max(cluster_close_width, 0.0))) + max(cluster_range_compression, 0.0)
    escape_body_commitment = direction * (signal_bar.close - signal_bar.open) / signal_range
    if direction > 0:
        escape_close_displacement = (signal_bar.close - cluster_high) / max(average_range, 1e-9)
    else:
        escape_close_displacement = (cluster_low - signal_bar.close) / max(average_range, 1e-9)
    pre_travel = sum(
        abs(cluster_window[pos].close - cluster_window[pos - 1].close)
        for pos in range(1, len(cluster_window))
    )
    pre_escape_directional_drift = direction * (cluster_window[-1].close - cluster_window[0].close) / max(
        pre_travel,
        average_range * 0.25,
    )
    cluster_volume_average = base.mean(volumes[cluster_start:index]) or average_volume
    volume_reengagement = volumes[index] / max(cluster_volume_average, 1e-9) - 1.0
    ...
    wick_containment = directional_bar_settlement(signal_bar, direction)
    ...
    if context.prior_day_return is not None and context.prior_day_range is not None:
        prior_day_energy_context = abs(context.prior_day_return) / max(context.prior_day_range, average_range, 1e-9)
    cluster_escape_pressure_raw = max(
        max(close_cluster_tension - 0.70, 0.0),
        max(cluster_range_compression, 0.0),
        max(escape_body_commitment - 0.08, 0.0) * 1.2,
        max(escape_close_displacement, 0.0) * 1.5,
        max(volume_reengagement, 0.0) * 0.45,
        max(vwap_escape_alignment, 0.0) * 0.25,
        max(wick_containment - 0.52, 0.0),
    )
    activity = max(cluster_escape_pressure_raw, cluster_escape_pressure_raw * session_phase_maturity)
    if activity < MIN_CLOSE_CLUSTER_ESCAPE_ACTIVITY:
        return {"event_count": 0.0}
    if cluster_close_width > 0.95 and cluster_range_compression < 0.05:
        return {"event_count": 0.0}
    if escape_body_commitment < 0.10 and escape_close_displacement < 0.04:
        return {"event_count": 0.0}
    if volume_reengagement < -0.45 and escape_body_commitment < 0.30:
        return {"event_count": 0.0}
    return {
        "close_cluster_tension": bounded(close_cluster_tension, 0.0, 3.0),
        "cluster_range_compression": bounded(cluster_range_compression, -2.0, 1.0),
        "escape_body_commitment": bounded(escape_body_commitment, -1.0, 1.0),
        "escape_close_displacement": bounded(escape_close_displacement, -3.0, 4.0),
        "pre_escape_directional_drift": bounded(pre_escape_directional_drift, -1.0, 1.0),
        "volume_reengagement": bounded(volume_reengagement, -3.0, 6.0),
        ...
        "prior_day_energy_context": bounded(prior_day_energy_context, 0.0, 3.0),
        "cluster_close_width": bounded(cluster_close_width, 0.0, 5.0),
        "cluster_mid_escape": bounded(direction * (signal_bar.close - cluster_mid) / max(average_range, 1e-9), -4.0, 4.0),
        "cluster_escape_pressure": bounded(cluster_escape_pressure_raw, 0.0, 5.0),
        "event_count": 1.0,
    }
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    tension_reward = 0.08 if metrics["close_cluster_tension"] > 0.90 and terminal > 0 else 0.0
    compression_reward = 0.10 if metrics["cluster_range_compression"] > 0.08 and terminal > 0 else 0.0
    body_reward = 0.14 if metrics["escape_body_commitment"] > 0.22 and terminal > 0 else 0.0
    displacement_reward = 0.16 if metrics["escape_close_displacement"] > 0.10 and terminal > 0 else 0.0
    drift_reward = 0.08 if metrics["pre_escape_directional_drift"] > -0.10 and terminal > 0 else 0.0
    volume_reward = 0.08 if metrics["volume_reengagement"] > 0.15 and terminal > 0 else 0.0
    vwap_reward = 0.08 if metrics["vwap_escape_alignment"] > 0.15 and terminal > 0 else 0.0
    wick_reward = 0.08 if metrics["wick_containment"] > 0.58 and terminal > 0 else 0.0
    false_escape_penalty = 0.18 if metrics["escape_close_displacement"] > 0.10 and terminal <= 0 else 0.0
    weak_body_penalty = 0.12 if metrics["escape_body_commitment"] < 0.04 and terminal <= 0 else 0.0
    failed_volume_penalty = 0.08 if metrics["volume_reengagement"] < -0.20 and terminal <= 0 else 0.0
    vwap_absorption_penalty = 0.08 if metrics["vwap_escape_alignment"] < -0.10 and terminal <= 0 else 0.0
    wick_failure_penalty = 0.08 if metrics["wick_containment"] < 0.44 and terminal <= 0 else 0.0
    ...
    close_cluster_escape_path_quality = (0.54 * terminal + 0.30 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + close_cluster_escape_path_quality
        + tension_reward
        + compression_reward
        + body_reward
        + displacement_reward
        ...
        + vwap_reward
        + wick_reward
        - false_escape_penalty
        - weak_body_penalty
        - failed_volume_penalty
        - vwap_absorption_penalty
        - wick_failure_penalty
        - stall_penalty
        - unfavorable_path_penalty
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["cluster_close_width"] - 1.25) * 0.06
    )


def fit_linear_close_cluster_escape_model(candidates: list[base.Candidate], fold_id: str) -> LinearCloseClusterEscapeModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearCloseClusterEscapeModel(
            fold_id,
    ...
    close_cluster_escape_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearCloseClusterEscapeModel(
        fold_id=fold_id,
        ...
        close_cluster_escape_failure_rate=close_cluster_escape_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearCloseClusterEscapeModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_close_cluster_escape_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0095_r0001_intraday_close_cluster_escape",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0095_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0095_r0001_close_cluster_escape_summary.json",
        ],
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0095 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0095_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0095 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "close_cluster_escape_lookback_bars": CLOSE_CLUSTER_ESCAPE_LOOKBACK_BARS,
        "close_cluster_escape_cluster_bars": CLOSE_CLUSTER_ESCAPE_CLUSTER_BARS,
        "min_close_cluster_escape_activity": MIN_CLOSE_CLUSTER_ESCAPE_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_close_cluster_escape_model_summary(model: LinearCloseClusterEscapeModel) -> dict[str, object]:
    return {
        ...
        "close_cluster_escape_failure_rate": rounded(model.close_cluster_escape_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearCloseClusterEscapeModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_close_cluster_escape_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "close_cluster_escape_profile": profiles["close_cluster_escape_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0095_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0095_r0001_proxy_trades.csv"
    evidence["close_cluster_escape_summary"] = "artifacts/c0095_r0001_close_cluster_escape_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0095_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0095_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0095 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0095_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0095_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0095_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0095_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0095_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0095_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0095_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0095_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0095_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0095 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0095 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0095_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0095_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0095_r0001_intraday_close_cluster_escape_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0095_r0001_proxy_records_close_cluster_escape_reference_surface
  ...
  - next_work_is_produce_c0095_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0095_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0095-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0095_r0001_intraday_close_cluster_escape",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0095-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0095-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0095-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0095_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0095_r0001_intraday_close_cluster_escape",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0095-R0001-close-cluster-escape-SUMMARY",
            "artifact_role": "close_cluster_escape_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0095_r0001_close_cluster_escape_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0095_r0001_intraday_close_cluster_escape",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0095_r0001_mt5_logic_parity_evidence",
        }
    ]
