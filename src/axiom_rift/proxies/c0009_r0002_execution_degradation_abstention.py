"""C0009 R0002 proxy evidence for execution-degradation abstention discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0009_execution_friction_aware_entry_discovery" / "runs" / "R0002"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0009_execution_friction_aware_entry_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0009_r0002_proxy_trades.csv"
DEGRADATION_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0009_r0002_execution_degradation_abstention_summary.json"
...
FEATURE_NAMES = (
    ...
    "directional_favorable_wick_fraction",
    "directional_adverse_wick_fraction",
    "prior_directional_drawdown_6",
    "prior_opposite_tail_pressure_12",
    "spread_over_range",
    "spread_over_short_range",
    "spread_over_bar_range",
    "spread_pressure_96",
    "spread_slope_6",
    "low_friction_flag",
    "friction_adjusted_momentum",
    "session_sin",
    "session_cos",
    "minutes_from_core_mid",
)
MODEL_FAMILY = "fold_local_execution_degradation_abstention_rank"
LABEL_SHAPE = "directional_target_first_minus_execution_degradation_risk"
POSITIVE_LABEL_THRESHOLD = 0.12
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SCORE_FLOOR_QUANTILE = 0.90
SCORE_FLOOR_MINIMUM = -4.00
SELECTION_RULE = "model_score_floor_then_top_fold_local_degradation_abstention_scores_per_active_day"
...
def run_c0009_r0002_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    candidates_by_fold: dict[str, dict[str, object]] = {}
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        floor_candidates = apply_score_floor(scored_candidates, model)
        selected = base.select_daily_candidates(floor_candidates)
        ...
        state_distributions[fold_id] = score_distribution(scored_candidates, floor_candidates, selected, model)
        eligible_count = sum(1 for candidate in scored_candidates if candidate.score is not None)
        candidates_by_fold[fold_id] = {
            ...
            "score_floor_candidate_count": len(floor_candidates),
            "score_floor_rejected_count": max(eligible_count - len(floor_candidates), 0),
            "score_floor": base.rounded(model.score_floor),
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = execution_degradation_abstention_features(bars, range_average, short_range_average, index, direction)
            if features is None:
                continue
            label = execution_degradation_abstention_label(bars, range_average, index, direction) if include_labels else None
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|execution_degradation_abstention",
                    features=features,
                    label=label,
                )
            )
...
def execution_degradation_abstention_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    index: int,
    ...
    ret_6 = direction * (bar.close - bars[index - 6].close) / average_range
    ret_12 = direction * (bar.close - bars[index - 12].close) / average_range
    ret_24 = direction * (bar.close - bars[index - 24].close) / average_range
    ...
    range_36_over_day = range_36 / day_range
    range_compression_pressure = max(1.0 - range_compression, 0.0)
    range_expansion_pressure = max(range_expansion - 1.0, 0.0)
    ...
    prior_directional_drawdown = max(
        0.0,
        max(direction * (bars[offset].close - bar.close) for offset in range(index - 6, index + 1)) / average_range,
    )
    prior_opposite_tail_pressure = sum(
        max(0.0, -direction * (bars[offset].close - bars[offset - 1].close) / average_range)
        for offset in range(index - 11, index + 1)
    ) / 12.0
    spread_96 = sum(bars[offset].spread_points for offset in range(index - 95, index + 1)) / 96.0
    spread_6 = sum(bars[offset].spread_points for offset in range(index - 5, index + 1)) / 6.0
    prev_spread_6 = sum(bars[offset].spread_points for offset in range(index - 11, index - 5)) / 6.0
    spread_over_range = bar.spread_points / average_range
    spread_over_short_range = bar.spread_points / short_average_range
    spread_over_bar_range = bar.spread_points / max(bar_range, 1e-9)
    spread_pressure_96 = bar.spread_points / max(spread_96, 1e-9)
    spread_slope_6 = (spread_6 - prev_spread_6) / max(spread_96, 1e-9)
    low_friction_flag = 1.0 if spread_over_range <= 0.18 and spread_pressure_96 <= 1.15 else 0.0
    friction_adjusted_momentum = ret_6 - 0.55 * spread_over_range - 0.25 * max(spread_pressure_96 - 1.0, 0.0)
    candidate_quality = (
        max(body_fraction, 0.0)
        + max(trend_consistency, 0.0)
        + max(directional_close_location - 0.48, 0.0)
        + 0.18 * low_friction_flag
        - 0.35 * max(spread_over_range - 0.20, 0.0)
    )
    if spread_over_range > 0.70 or spread_over_short_range > 1.15 or spread_over_bar_range > 0.70:
        return None
    if candidate_quality < 0.03 and friction_adjusted_momentum < -0.08:
        return None
...
def execution_degradation_abstention_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    index: int,
    direction: int,
    ...
    path_instability = max(0.0, path_range_peak - 2.0)
    cost_buffer = max(0.0, favorable - spread_norm - spread_drift)
    degradation_risk = 0.70 * adverse_tail + 0.45 * spread_drift + 0.35 * path_instability + 0.22 * giveback
    target_first_edge = 1.10 * first_hit + 0.45 * target_speed - 0.45 * stop_speed
    return (
        target_first_edge
        + 0.26 * favorable
        - 0.42 * adverse
...
def fit_linear_edge_model(candidates: list[base.Candidate], fold_id: str) -> LinearEdgeModel:
    ...
    if not labeled:
        return LinearEdgeModel(
            ...
            label_std=0.0,
            score_floor=SCORE_FLOOR_MINIMUM,
        )
    ...
    score_vector = direction * weights
    denominator = math.sqrt(max(len(score_vector), 1))
    train_scores = float(labels.mean()) + (scaled @ score_vector) / denominator
    finite_scores = train_scores[np.isfinite(train_scores)]
    if finite_scores.size:
        score_floor = max(float(np.quantile(finite_scores, SCORE_FLOOR_QUANTILE)), SCORE_FLOOR_MINIMUM)
    else:
        score_floor = SCORE_FLOOR_MINIMUM
    return LinearEdgeModel(
        fold_id=fold_id,
...
def apply_score_floor(candidates: list[base.Candidate], model: LinearEdgeModel) -> list[base.Candidate]:
    return [
        candidate
        for candidate in candidates
        if candidate.score is not None and math.isfinite(candidate.score) and candidate.score >= model.score_floor
    ]
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0009"
    payload["work_unit_id"] = "C0009"
    payload["run_id"] = "R0002"
    payload["proxy_id"] = "PX-C0009-R0002"
    payload["proxy_engine"] = "axiom_rift.proxies.c0009_r0002_execution_degradation_abstention"
    payload["proxy_config_path"] = "campaigns/C0009_execution_friction_aware_entry_discovery/runs/R0002/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_execution_degradation_abstention_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0009_execution_friction_aware_entry_discovery/runs/R0002/kpi/proxy.json",
        "campaigns/C0009_execution_friction_aware_entry_discovery/runs/R0002/artifacts/c0009_r0002_proxy_trades.csv",
        "campaigns/C0009_execution_friction_aware_entry_discovery/runs/R0002/artifacts/c0009_r0002_execution_degradation_abstention_summary.json",
    ]
    ...
    profiles["execution_degradation_abstention_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
            ...
            "score_floor_minimum": SCORE_FLOOR_MINIMUM,
            "fold_models": fold_models,
            "state_distributions": state_distributions,
            "candidates_by_fold": candidates_by_fold,
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0009_r0002_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "score_floor_quantile": SCORE_FLOOR_QUANTILE,
            "score_floor_minimum": SCORE_FLOOR_MINIMUM,
            "score_interpretation": "higher_score_means_fold_local_execution_degradation_abstention_quality",
            "variant_boundary": "train_fold_score_floor_abstention_on_execution_degradation_risk_not_threshold_window_sltp_or_session_nudge",
        }
    )
    return config
...
def score_distribution(
    scored: list[base.Candidate],
    floor_candidates: list[base.Candidate],
    ...
    floor_scores = [candidate.score or 0.0 for candidate in floor_candidates]
    ...
    floor_rejections = max(eligible_hits - len(floor_candidates), 0)
    return {
        ...
        "score_floor": base.rounded(model.score_floor),
        "score_floor_candidate_count": len(floor_candidates),
        "score_floor_candidate_rate": base.rounded(len(floor_candidates) / eligible_hits) if eligible_hits else None,
        "score_floor_rejected_count": floor_rejections,
        "score_floor_rejected_rate": base.rounded(floor_rejections / eligible_hits) if eligible_hits else None,
        ...
        "floor_score_min": base.rounded(min(floor_scores)) if floor_scores else None,
        "floor_score_p50": base.rounded(base.percentile(floor_scores, 0.50)),
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_degradation_summary_artifact(payload, DEGRADATION_SUMMARY_PATH)
    PROXY_PATH.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="ascii")
    trade_hash = base.sha256_file(TRADE_ARTIFACT_PATH)
    summary_hash = base.sha256_file(DEGRADATION_SUMMARY_PATH)
...
def write_degradation_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_execution_degradation_abstention_summary_v1",
        "template": False,
        "work_unit_id": "C0009",
        "campaign_id": "C0009",
        "run_id": "R0002",
        ...
        "execution_degradation_abstention_profile": profiles["execution_degradation_abstention_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "execution_degradation_abstention_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0009-R0002-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0009_execution_friction_aware_entry_discovery/runs/R0002/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0009_execution_friction_aware_entry_discovery/runs/R0002/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0009-R0002-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0009-R0002-EXECUTION-DEGRADATION-ABSTENTION-SUMMARY",
                "execution_degradation_abstention_summary_artifact",
                "json",
                "campaigns/C0009_execution_friction_aware_entry_discovery/runs/R0002/artifacts/c0009_r0002_execution_degradation_abstention_summary.json",
                summary_hash,
                ["campaigns/C0009_execution_friction_aware_entry_discovery/runs/R0002/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0009_r0002_execution_degradation_abstention",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0009_r0002_proxy_trades.csv"
    evidence["execution_degradation_abstention_summary"] = "artifacts/c0009_r0002_execution_degradation_abstention_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0009_r0002_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0009_r0002_proxy_trades.csv",
        "artifacts/c0009_r0002_execution_degradation_abstention_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0009 R0002 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0009_r0002_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0009_r0002_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0009_execution_friction_aware_entry_discovery"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0009_execution_friction_aware_entry_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0009_execution_friction_aware_entry_discovery",
        "open_c0009_r0002_execution_degradation_abstention_run",
        "produce_c0009_r0002_proxy_evidence",
    ):
    ...
    data["active_run"] = "campaigns/C0009_execution_friction_aware_entry_discovery/runs/R0002"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["active_campaign"] = "campaigns/C0009_execution_friction_aware_entry_discovery"
    data["active_run"] = "campaigns/C0009_execution_friction_aware_entry_discovery/runs/R0002"
    data["latest_operation"] = {
        "id": "produce_c0009_r0002_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0009_execution_friction_aware_entry_discovery/runs/R0002/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0009_r0002_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0009",
            "c0004_r0004_fold_local_state_archetype": "c0009_r0002_execution_degradation_abstention",
            "c0004_r0004": "c0009_r0002",
            "fold_local_state_archetype_discovery": "execution_friction_aware_entry_discovery",
            ...
            "state_archetype_summary": "execution_degradation_abstention_summary",
        }
