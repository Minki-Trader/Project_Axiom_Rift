"""C0054 R0001 proxy evidence for intraday late session inventory unwind."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0054_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0054_r0001_late_session_inventory_unwind_summary.json"
...
FEATURE_NAMES = (
    "directional_day_inventory_imbalance",
    "late_session_progress_pressure",
    "directional_distance_from_day_mid",
    "directional_range_edge_pressure",
    "inventory_reversal_pressure_6",
    "inventory_continuation_pressure_12",
    "late_session_wick_absorption",
    "day_range_completion_ratio",
    "range_compression_12_vs_48",
    "volume_confirmation_on_inventory_shift",
    "prior_day_inventory_context",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_late_session_inventory_unwind_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_late_session_inventory_unwind_state"
SELECTION_RULE = "top_fold_local_late_session_inventory_unwind_scores_per_active_day"
...
LATE_SESSION_MIN_PROGRESS = 0.45
MIN_ABS_DAY_INVENTORY = 0.25
...
class LinearLateSessionInventoryUnwindModel:
    fold_id: str
...
def run_c0054_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_late_session_inventory_unwind_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_late_session_inventory_unwind_model_summary(model))
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < 48:
        return None, None
    inventory = late_session_inventory_unwind_metrics(bars, context, index, direction, average_range)
    if inventory["inventory_event_count"] <= 0:
        return None, None
    ...
    volume_confirmation = bounded(
        volume_ratio
        * (
            0.55 * inventory["inventory_continuation_pressure_12"]
            + 0.45 * inventory["inventory_reversal_pressure_6"]
        ),
        -4.0,
        4.0,
    )
    features = (
        inventory["directional_day_inventory_imbalance"],
        inventory["late_session_progress_pressure"],
        inventory["directional_distance_from_day_mid"],
        inventory["directional_range_edge_pressure"],
        inventory["inventory_reversal_pressure_6"],
        inventory["inventory_continuation_pressure_12"],
        inventory["late_session_wick_absorption"],
        inventory["day_range_completion_ratio"],
        compression,
        volume_confirmation,
        inventory["prior_day_inventory_context"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        inventory_bucket(inventory["directional_day_inventory_imbalance"]),
        progress_bucket(inventory["late_session_progress_pressure"]),
        mid_distance_bucket(inventory["directional_distance_from_day_mid"]),
        edge_pressure_bucket(inventory["directional_range_edge_pressure"]),
        reversal_bucket(inventory["inventory_reversal_pressure_6"]),
        continuation_bucket(inventory["inventory_continuation_pressure_12"]),
        wick_bucket(inventory["late_session_wick_absorption"]),
...
def late_session_inventory_unwind_metrics(
    bars: list[base.Bar],
    ...
    session_progress = bounded_session_progress(bars[index].time)
    if session_progress < LATE_SESSION_MIN_PROGRESS:
        return {"inventory_event_count": 0.0}

    close = bars[index].close
    day_move = close - context.day_open
    raw_day_inventory = day_move / average_range
    if abs(raw_day_inventory) < MIN_ABS_DAY_INVENTORY:
        return {"inventory_event_count": 0.0}
    ...
    day_mid = (context.high_so_far + context.low_so_far) * 0.5
    directional_day_inventory = direction * raw_day_inventory
    directional_mid_distance = direction * (close - day_mid) / average_range
    directional_edge_pressure = directional_session_position(close, context, direction) or 0.5
    reversal_pressure = -direction * (close - bars[index - 6].close) / average_range
    continuation_pressure = direction * (close - bars[index - 12].close) / average_range
    wick_absorption = adverse_wick_absorption(bars[index], direction)
    range_completion = day_width / average_range
    prior_context = 0.0
    if context.prior_day_range is not None and context.prior_day_range > 0 and context.prior_day_return is not None:
        prior_context = direction * context.prior_day_return / context.prior_day_range
    return {
        "directional_day_inventory_imbalance": bounded(directional_day_inventory, -4.0, 4.0),
        "late_session_progress_pressure": bounded(session_progress * abs(raw_day_inventory), 0.0, 4.0),
        "directional_distance_from_day_mid": bounded(directional_mid_distance, -4.0, 4.0),
        "directional_range_edge_pressure": bounded(directional_edge_pressure, 0.0, 1.0),
        "inventory_reversal_pressure_6": bounded(reversal_pressure, -4.0, 4.0),
        "inventory_continuation_pressure_12": bounded(continuation_pressure, -4.0, 4.0),
        "late_session_wick_absorption": wick_absorption,
        "day_range_completion_ratio": bounded(range_completion, 0.0, 8.0),
        "prior_day_inventory_context": bounded(prior_context, -3.0, 3.0),
        "inventory_event_count": 1.0,
    }
...
def inventory_bucket(value: float) -> str:
    if value < -0.75:
        return "against_day_inventory"
    if value < 0.75:
        return "mixed_day_inventory"
    return "with_day_inventory"


def progress_bucket(value: float) -> str:
    if value < 0.55:
        return "early_inventory_pressure"
    if value < 1.25:
        return "late_inventory_pressure"
    return "final_inventory_pressure"


def mid_distance_bucket(value: float) -> str:
    if value < -0.45:
        return "opposite_side_of_day_mid"
    if value < 0.45:
        return "near_day_mid"
    return "beyond_day_mid"
...
def reversal_bucket(value: float) -> str:
    if value < -0.35:
        return "reversal_pressure_against"
    if value < 0.35:
        return "reversal_pressure_neutral"
    return "reversal_pressure_with"


def continuation_bucket(value: float) -> str:
    if value < -0.35:
        return "continuation_pressure_against"
    if value < 0.35:
        return "continuation_pressure_neutral"
    return "continuation_pressure_with"
...
def candidate_label(
    ...
    acceptance_or_unwind_reward = 0.20 if recent_response > 0.15 and terminal > 0 else 0.0
    failed_inventory_shift_penalty = 0.20 if recent_response < -0.15 and mae > mfe else 0.0
    ...
    inventory_path_quality = (0.35 * terminal + 0.35 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + inventory_path_quality
        + acceptance_or_unwind_reward
        - failed_inventory_shift_penalty
        - stall_penalty
...
def fit_linear_late_session_inventory_unwind_model(candidates: list[base.Candidate], fold_id: str) -> LinearLateSessionInventoryUnwindModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearLateSessionInventoryUnwindModel(fold_id, tuple(0.0 for _ in FEATURE_NAMES), tuple(1.0 for _ in FEATURE_NAMES), tuple(0.0 for _ in FEATURE_NAMES), 0, 0.0, 0.0, 0.0)
    ...
    inventory_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearLateSessionInventoryUnwindModel(
        fold_id=fold_id,
        ...
        inventory_failure_rate=inventory_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearLateSessionInventoryUnwindModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_late_session_inventory_unwind_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0054_r0001_intraday_late_session_inventory_unwind",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0054_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0054_r0001_late_session_inventory_unwind_summary.json",
        ],
        ...
        "conditional_profiles": {
            "late_session_inventory_unwind_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0054 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0054_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0054 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def linear_late_session_inventory_unwind_model_summary(model: LinearLateSessionInventoryUnwindModel) -> dict[str, object]:
    return {
        ...
        "inventory_failure_rate": rounded(model.inventory_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearLateSessionInventoryUnwindModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_late_session_inventory_unwind_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "late_session_inventory_unwind_profile": profiles["late_session_inventory_unwind_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0054_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_trade_artifact"] = "artifacts/c0054_r0001_proxy_trades.csv"
    evidence["late_session_inventory_unwind_summary"] = "artifacts/c0054_r0001_late_session_inventory_unwind_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0054_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0054_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0054 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0054_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0054_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0054_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0054_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0054_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0054_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0054_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0054_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0054_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0054 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0054 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260706_produce_c0054_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0054_r0001_proxy_evidence
  refines:
  - dec_20260706_open_c0054_r0001_intraday_late_session_inventory_unwind_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0054_r0001_proxy_records_late_session_inventory_unwind_reference_surface
  ...
  - next_work_is_produce_c0054_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260706_produce_c0054_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0054-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0054_r0001_intraday_late_session_inventory_unwind",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0054-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0054-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0054-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0054_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0054_r0001_intraday_late_session_inventory_unwind",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0054-R0001-LATE-SESSION-INVENTORY-UNWIND-SUMMARY",
            "artifact_role": "late_session_inventory_unwind_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0054_r0001_late_session_inventory_unwind_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0054_r0001_intraday_late_session_inventory_unwind",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0054_r0001_mt5_logic_parity_evidence",
        }
    ]
