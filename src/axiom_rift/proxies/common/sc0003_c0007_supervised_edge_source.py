"""C0007 R0001 proxy evidence for fold-local supervised edge discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0007_fold_local_supervised_edge_discovery" / "runs" / "R0001"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0007_r0001_proxy_trades.csv"
SUPERVISED_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0007_r0001_supervised_edge_summary.json"
...
MODEL_FAMILY = "fold_local_direct_supervised_linear_rank_edge"
LABEL_SHAPE = "directional_target_before_stop_path_quality_cost_adjusted"
POSITIVE_LABEL_THRESHOLD = 0.08
ADVERSE_LABEL_THRESHOLD = -0.42
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_supervised_linear_rank_scores_per_active_day"
...
def run_c0007_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = supervised_features(bars, range_average, short_range_average, index, direction)
            if features is None:
                continue
            label = supervised_path_label(bars, range_average, index, direction) if include_labels else None
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|supervised_linear_rank_edge",
                    features=features,
                    label=label,
                )
            )
...
def build_proxy_payload(
    ...
    payload["proxy_id"] = "PX-C0007-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0007_r0001_fold_local_supervised_edge"
    payload["proxy_config_path"] = "campaigns/C0007_fold_local_supervised_edge_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_supervised_linear_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0007_fold_local_supervised_edge_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0007_fold_local_supervised_edge_discovery/runs/R0001/artifacts/c0007_r0001_proxy_trades.csv",
        "campaigns/C0007_fold_local_supervised_edge_discovery/runs/R0001/artifacts/c0007_r0001_supervised_edge_summary.json",
    ]
    ...
    profiles["supervised_edge_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0007_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "feature_weight_floor": FEATURE_WEIGHT_FLOOR,
            "feature_weight_ceiling": FEATURE_WEIGHT_CEILING,
            "score_interpretation": "higher_score_means_direct_fold_local_supervised_edge_rank",
            "variant_boundary": "direct_supervised_linear_rank_not_analog_memory_event_grammar_state_archetype_score_cell_exit_shape_or_retry_nudge",
        }
    )
    return config
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_supervised_summary_artifact(payload, SUPERVISED_SUMMARY_PATH)
    PROXY_PATH.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="ascii")
    trade_hash = base.sha256_file(TRADE_ARTIFACT_PATH)
    summary_hash = base.sha256_file(SUPERVISED_SUMMARY_PATH)
...
def write_supervised_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_supervised_edge_summary_v1",
        "template": False,
        "work_unit_id": "C0007",
        "campaign_id": "C0007",
        "run_id": "R0001",
        ...
        "supervised_edge_profile": profiles["supervised_edge_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "supervised_edge_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0007-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0007_fold_local_supervised_edge_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0007_fold_local_supervised_edge_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0007-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0007-R0001-SUPERVISED-EDGE-SUMMARY",
                "supervised_edge_summary_artifact",
                "json",
                "campaigns/C0007_fold_local_supervised_edge_discovery/runs/R0001/artifacts/c0007_r0001_supervised_edge_summary.json",
                summary_hash,
                ["campaigns/C0007_fold_local_supervised_edge_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0007_r0001_fold_local_supervised_edge",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0007_r0001_proxy_trades.csv"
    evidence["supervised_edge_summary"] = "artifacts/c0007_r0001_supervised_edge_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0007_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0007_r0001_proxy_trades.csv",
        "artifacts/c0007_r0001_supervised_edge_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0007 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0007_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0007_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    for item in (
        "open_c0007_fold_local_supervised_edge_discovery",
        "open_c0007_r0001_fold_local_supervised_linear_rank_edge_run",
        "produce_c0007_r0001_proxy_evidence",
    ):
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0007_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0007_fold_local_supervised_edge_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0007_r0001_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0007",
            "c0004_r0001_fold_local_state_archetype": "c0007_r0001_fold_local_supervised_edge",
            "c0004_r0001": "c0007_r0001",
            "fold_local_state_archetype_discovery": "fold_local_supervised_edge_discovery",
            ...
            "state_archetype_summary": "supervised_edge_summary",
        }
