"""C0004 R0003 proxy evidence for adverse archetype inversion."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0004_fold_local_state_archetype_discovery" / "runs" / "R0003"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0004_r0003_proxy_trades.csv"
PATH_QUALITY_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0004_r0003_adverse_archetype_inversion_summary.json"
...
def run_c0004_r0003_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        train_candidates = build_adverse_inversion_candidates(
            bars,
            range_average,
            short_range_average,
            split["train_is"],
        ...
        model = fit_adverse_inversion_model(train_candidates, fold_id)
        test_candidates = build_adverse_inversion_candidates(
            bars,
            range_average,
            short_range_average,
            split["test_oos"],
...
def build_adverse_inversion_candidates(
    bars: list[base.Bar],
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            state_key, features = adverse_inversion_state_and_features(
                bars,
                range_average,
                short_range_average,
                index,
            ...
            if state_key is None or features is None:
                continue
            label = adverse_inversion_label(bars, range_average, index, direction) if include_labels else None
            candidates.append(
                base.Candidate(
                    fold_id=fold_id,
                    index=index,
...
def adverse_inversion_state_and_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    index: int,
...
def fit_adverse_inversion_model(candidates: list[base.Candidate], fold_id: str) -> base.ArchetypeModel:
    ...
    for archetype_id, bucket in grouped.items():
        ...
        adverse_lift = global_mean - mean_label
        negative_lift = global_positive_rate - positive_rate
        downside_tail = min(bucket)
        activity_weight = math.log(1.0 + len(bucket) / base.MIN_TRAIN_ARCHETYPE_COUNT)
        score = -mean_label + 0.30 * negative_lift + 0.02 * activity_weight + 0.04 * max(-downside_tail, 0.0)
        if adverse_lift <= 0.0 or negative_lift <= 0.0:
            continue
        stats.append(
            base.ArchetypeStats(
...
def score_candidate(candidate: base.Candidate, model: base.ArchetypeModel) -> base.Candidate:
    ...
    return base.Candidate(
        ...
        direction=-candidate.direction,
        day=candidate.day,
        state_key="invert|" + candidate.state_key,
        features=candidate.features,
        label=candidate.label,
        score=stat.score,
    )
...
def build_proxy_payload(
    ...
    payload["proxy_id"] = "PX-C0004-R0003"
    payload["proxy_engine"] = "axiom_rift.proxies.c0004_r0003_adverse_archetype_inversion"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0003/kpi/proxy.json",
        "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0003/artifacts/c0004_r0003_proxy_trades.csv",
        "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0003/artifacts/c0004_r0003_adverse_archetype_inversion_summary.json",
    ]
    ...
    profile["label_shape"] = "directional_forward_path_quality_used_as_adverse_state_signal"
    profile["selection_rule"] = "top_fold_local_adverse_directional_archetypes_inverted_per_active_day"
    profile["model_family"] = "fold_local_adverse_archetype_inversion"
    payload["conditional_profiles"]["adverse_inversion_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
            ...
            "inverts_direction_for_execution": True,
            "source_direction_state_key_preserved_with_invert_prefix": True,
            "score_interpretation": "higher_score_means_more_adverse_source_direction_selected_for_inversion",
...
def proxy_config() -> dict[str, object]:
    ...
    config.update(
        {
            "state_model": "fold_local_adverse_archetype_inversion",
            "label_shape": "directional_forward_path_quality_used_as_adverse_state_signal",
            "selection_rule": "top_fold_local_adverse_directional_archetypes_inverted_per_active_day",
            "prior_path_bars": PRIOR_PATH_BARS,
            "early_adverse_bars": EARLY_ADVERSE_BARS,
            "inverts_direction_for_execution": True,
            "feature_names": list(FEATURE_NAMES),
            "state_dimensions": list(STATE_DIMENSIONS),
            "variant_boundary": "adverse_archetype_direction_inversion_not_adjacent_r0001_or_r0002_nudge",
        }
    )
    return config
...
def archetype_model_summary(model: base.ArchetypeModel) -> dict[str, object]:
    ...
    summary["model_family"] = "fold_local_adverse_archetype_inversion"
    summary["score_interpretation"] = "higher_score_means_more_adverse_source_direction_selected_for_inversion"
    return summary
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_adverse_inversion_summary_artifact(payload, PATH_QUALITY_SUMMARY_PATH)
...
def write_adverse_inversion_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_adverse_inversion_archetype_summary_v1",
        "template": False,
        "work_unit_id": "C0004",
        "run_id": "R0003",
        ...
        "adverse_inversion_profile": profiles["adverse_inversion_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "adverse_inversion_archetype_summary_artifact"}
    ]
    records.extend(
        [
            {
                "artifact_id": "A-C0004-R0003-PROXY-KPI",
                "artifact_role": "proxy_kpi",
                "artifact_type": "json",
                "repo_relative_path": "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0003/kpi/proxy.json",
                "sha256": proxy_hash,
                "produced_by": "axiom_rift.proxies.c0004_r0003_adverse_archetype_inversion",
                "source_inputs": [
                    "data/processed/datasets/us100_m5_base_frame.csv",
                    "data/processed/coverage_audits/us100_m5_rolling_windows.csv",
                    "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0003/run_manifest.json",
                ],
            ...
            {
                "artifact_id": "A-C0004-R0003-PROXY-TRADES",
                "artifact_role": "proxy_trade_artifact",
                "artifact_type": "csv",
                "repo_relative_path": "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0003/artifacts/c0004_r0003_proxy_trades.csv",
                "sha256": trade_hash,
                "produced_by": "axiom_rift.proxies.c0004_r0003_adverse_archetype_inversion",
                "source_inputs": [
                    "data/processed/datasets/us100_m5_base_frame.csv",
            ...
            {
                "artifact_id": "A-C0004-R0003-ADVERSE-ARCHETYPE-INVERSION-SUMMARY",
                "artifact_role": "adverse_inversion_archetype_summary_artifact",
                "artifact_type": "json",
                "repo_relative_path": "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0003/artifacts/c0004_r0003_adverse_archetype_inversion_summary.json",
                "sha256": summary_hash,
                "produced_by": "axiom_rift.proxies.c0004_r0003_adverse_archetype_inversion",
                "source_inputs": [
                    "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0003/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0004_r0003_mt5_logic_parity_evidence",
        }
    ]
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0004_r0003_proxy_trades.csv"
    evidence["adverse_inversion_archetype_summary"] = "artifacts/c0004_r0003_adverse_archetype_inversion_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0004_r0003_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0004_r0003_proxy_trades.csv",
        "artifacts/c0004_r0003_adverse_archetype_inversion_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0004 R0003 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_r0003_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0004_r0003_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    if "produce_c0004_r0003_proxy_evidence" not in completed:
        completed.append("produce_c0004_r0003_proxy_evidence")
    next_action = "produce_c0004_r0003_mt5_logic_parity_evidence"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0004_r0003_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0003/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0004_r0003_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "c0004_r0001_fold_local_state_archetype": "c0004_r0003_adverse_archetype_inversion",
            "c0004_r0001": "c0004_r0003",
            "C0004_R0001": "C0004_R0003",
            "R0001": "R0003",
            "r0001": "r0003",
        }
