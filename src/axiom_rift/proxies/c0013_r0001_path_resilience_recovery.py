"""C0013 R0001 proxy evidence for path resilience recovery discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0013_path_resilience_recovery_discovery" / "runs" / "R0001"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0013_path_resilience_recovery_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0013_r0001_proxy_trades.csv"
PATH_RESILIENCE_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0013_r0001_path_resilience_recovery_summary.json"
...
FEATURE_NAMES = (
    ...
    "directional_ret_24_over_range",
    "directional_trend_consistency_24",
    "prior_adverse_excursion_6",
    "prior_recovery_fraction_12",
    "pullback_absorption_12",
    "wick_rejection_balance",
    "range_expansion_3_over_36",
    "range_compression_12_over_96",
    "directional_range_position_72",
...
MODEL_FAMILY = "fold_local_path_resilience_recovery_rank"
LABEL_SHAPE = "directional_adverse_first_avoidance_and_recovery_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_path_resilience_recovery_scores_per_active_day"
...
def run_c0013_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = path_resilience_features(bars, range_average, short_range_average, index, direction)
            if features is None:
                continue
            label = path_resilience_label(bars, range_average, index, direction) if include_labels else None
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|path_resilience_recovery",
                    features=features,
                    label=label,
                )
            )
...
def path_resilience_features(
    ...
    if bar_range <= 0:
        return None
    range_position = directional_range_position(bars, index, direction, 72)
    ...
    directional_steps = [
        ...
        for offset in range(index - 23, index + 1)
    ]
    trend_consistency = sum(1.0 if value > 0 else -1.0 if value < 0 else 0.0 for value in directional_steps) / len(
        directional_steps
    )
    prior_6 = bars[index - 5 : index + 1]
    if direction > 0:
        prior_adverse = max(max(bar.close - prior.low, 0.0) for prior in prior_6) / average_range
        favorable_wick_values = [(min(prior.open, prior.close) - prior.low) / max(prior.high - prior.low, 1e-9) for prior in prior_6]
        adverse_wick_values = [(prior.high - max(prior.open, prior.close)) / max(prior.high - prior.low, 1e-9) for prior in prior_6]
    else:
        prior_adverse = max(max(prior.high - bar.close, 0.0) for prior in prior_6) / average_range
        favorable_wick_values = [(prior.high - max(prior.open, prior.close)) / max(prior.high - prior.low, 1e-9) for prior in prior_6]
        adverse_wick_values = [(min(prior.open, prior.close) - prior.low) / max(prior.high - prior.low, 1e-9) for prior in prior_6]
    prior_12 = bars[index - 11 : index + 1]
    adverse_closes = sum(1 for prior in prior_12 if direction * (prior.close - bar.close) < 0)
    recovery_closes = sum(1 for prior in prior_12 if direction * (bar.close - prior.close) > 0)
    recovery_fraction = recovery_closes / max(adverse_closes + recovery_closes, 1)
    pullback_depth = max(0.0, max(-direction * (bars[offset].close - bars[offset - 1].close) for offset in range(index - 11, index + 1)))
    current_recovery = max(ret_1, 0.0) + max(ret_3, 0.0)
    pullback_absorption = current_recovery / max(pullback_depth / average_range, 1e-9)
    wick_rejection_balance = sum(favorable_wick_values) / len(favorable_wick_values) - sum(adverse_wick_values) / len(
        adverse_wick_values
    )
...
def path_resilience_label(
    ...
    recovery_quality = max(terminal_norm, 0.0) / max(adverse, 0.12)
    capture_quality = max(terminal_norm, 0.0) / max(favorable, 0.12)
    ...
    return (
        ...
        - 0.34 * giveback
        + 0.34 * terminal_norm
        + 0.22 * recovery_quality
        + 0.18 * capture_quality
        + 0.24 * path_alignment_edge
        - 1.18 * spread_norm
    )
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0013"
    payload["work_unit_id"] = "C0013"
    payload["run_id"] = "R0001"
    payload["proxy_id"] = "PX-C0013-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0013_r0001_path_resilience_recovery"
    payload["proxy_config_path"] = "campaigns/C0013_path_resilience_recovery_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_path_resilience_recovery_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0013_path_resilience_recovery_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0013_path_resilience_recovery_discovery/runs/R0001/artifacts/c0013_r0001_proxy_trades.csv",
        "campaigns/C0013_path_resilience_recovery_discovery/runs/R0001/artifacts/c0013_r0001_path_resilience_recovery_summary.json",
    ]
    ...
    profiles["path_resilience_recovery_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0013_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "feature_weight_floor": FEATURE_WEIGHT_FLOOR,
            "feature_weight_ceiling": FEATURE_WEIGHT_CEILING,
            "score_interpretation": "higher_score_means_direct_fold_local_path_resilience_recovery_rank",
            "variant_boundary": "path_resilience_recovery_rank_not_auction_threshold_monthly_memory_lifecycle_structural_trap_score_floor_stop_target_hold_or_retry_nudge",
        }
    )
    return config
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_path_resilience_summary_artifact(payload, PATH_RESILIENCE_SUMMARY_PATH)
    PROXY_PATH.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="ascii")
    trade_hash = base.sha256_file(TRADE_ARTIFACT_PATH)
    summary_hash = base.sha256_file(PATH_RESILIENCE_SUMMARY_PATH)
...
def write_path_resilience_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_path_resilience_recovery_summary_v1",
        "template": False,
        "work_unit_id": "C0013",
        "campaign_id": "C0013",
        "run_id": "R0001",
        ...
        "path_resilience_recovery_profile": profiles["path_resilience_recovery_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "path_resilience_recovery_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0013-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0013_path_resilience_recovery_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0013_path_resilience_recovery_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0013-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0013-R0001-PATH-RESILIENCE-RECOVERY-SUMMARY",
                "path_resilience_recovery_summary_artifact",
                "json",
                "campaigns/C0013_path_resilience_recovery_discovery/runs/R0001/artifacts/c0013_r0001_path_resilience_recovery_summary.json",
                summary_hash,
                ["campaigns/C0013_path_resilience_recovery_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0013_r0001_path_resilience_recovery",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0013_r0001_proxy_trades.csv"
    evidence["path_resilience_recovery_summary"] = "artifacts/c0013_r0001_path_resilience_recovery_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0013_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0013_r0001_proxy_trades.csv",
        "artifacts/c0013_r0001_path_resilience_recovery_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0013 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0013_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0013_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0013_path_resilience_recovery_discovery"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0013_path_resilience_recovery_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0013_path_resilience_recovery_discovery",
        "open_c0013_r0001_fold_local_path_resilience_recovery_rank_run",
        "produce_c0013_r0001_proxy_evidence",
    ):
    ...
    data["active_run"] = "campaigns/C0013_path_resilience_recovery_discovery/runs/R0001"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["active_campaign"] = "campaigns/C0013_path_resilience_recovery_discovery"
    data["active_run"] = "campaigns/C0013_path_resilience_recovery_discovery/runs/R0001"
    data["latest_operation"] = {
        "id": "produce_c0013_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0013_path_resilience_recovery_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0013_r0001_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0013",
            "c0004_r0001_fold_local_state_archetype": "c0013_r0001_path_resilience_recovery",
            "c0004_r0001": "c0013_r0001",
            "fold_local_state_archetype_discovery": "path_resilience_recovery_discovery",
            ...
            "state_archetype_summary": "path_resilience_recovery_summary",
        }
