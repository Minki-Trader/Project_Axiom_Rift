"""C0053 R0001 proxy evidence for intraday opening range retest."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0053_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0053_r0001_opening_range_retest_summary.json"
...
FEATURE_NAMES = (
    "opening_boundary_proximity",
    "directional_opening_range_reclaim",
    "opening_range_extension_beyond_boundary",
    "recent_retest_recency",
    "retest_rejection_wick",
    "directional_close_response_3",
    "opening_drive_alignment",
    "session_range_position",
    "range_compression_12_vs_48",
    "volume_confirmation_on_retest",
    "prior_day_directional_context",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_opening_range_retest_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_opening_range_retest_state"
SELECTION_RULE = "top_fold_local_opening_range_retest_scores_per_active_day"
...
class LinearOpeningRangeRetestModel:
    fold_id: str
...
def run_c0053_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_opening_range_retest_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_opening_range_retest_model_summary(model))
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < 18:
        return None, None
    opening = opening_range_retest_metrics(bars, context, index, direction, average_range)
    if opening["retest_event_count"] <= 0:
        return None, None
    ...
    if volume_ratio is None:
        return None, None
    volume_confirmation = bounded(volume_ratio * opening["directional_close_response_3"], -4.0, 4.0)
    features = (
        opening["opening_boundary_proximity"],
        opening["directional_opening_range_reclaim"],
        opening["opening_range_extension_beyond_boundary"],
        opening["recent_retest_recency"],
        opening["retest_rejection_wick"],
        opening["directional_close_response_3"],
        opening["opening_drive_alignment"],
        opening["session_range_position"],
        compression,
        volume_confirmation,
        opening["prior_day_directional_context"],
        -spread_stress,
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        boundary_proximity_bucket(opening["opening_boundary_proximity"]),
        reclaim_bucket(opening["directional_opening_range_reclaim"]),
        extension_bucket(opening["opening_range_extension_beyond_boundary"]),
        recency_bucket(opening["recent_retest_recency"]),
        wick_bucket(opening["retest_rejection_wick"]),
        compression_bucket(compression),
        momentum_bucket(opening["directional_close_response_3"]),
...
def opening_range_retest_metrics(
    bars: list[base.Bar],
    ...
    opening_width = context.opening_high - context.opening_low
    if opening_width <= 0:
        return {"retest_event_count": 0.0}
    boundary = context.opening_low if direction > 0 else context.opening_high
    close = bars[index].close
    if direction > 0:
        directional_distance = (close - boundary) / average_range
    else:
        directional_distance = (boundary - close) / average_range
    boundary_proximity = 1.0 - min(abs(directional_distance), 1.5) / 1.5
    boundary_proximity = bounded(boundary_proximity, -0.50, 1.0)
    extension_beyond_boundary = max(0.0, -directional_distance)
    recent_start = max(context.day_start_index, index - 12 + 1)
    touch_positions: list[int] = []
    for pos in range(recent_start, index + 1):
        if direction > 0:
            touched = bars[pos].low <= boundary + 0.25 * average_range
        else:
            touched = bars[pos].high >= boundary - 0.25 * average_range
        if touched:
            touch_positions.append(pos)
    if not touch_positions:
        return {"retest_event_count": 0.0}
    age = index - touch_positions[-1]
    retest_recency = 1.0 - min(age / 12.0, 1.0)
    rejection_wick = adverse_wick_absorption(bars[index], direction)
    close_response = direction * (bars[index].close - bars[index - 3].close) / average_range
    opening_close_index = min(context.day_start_index + 11, index)
    opening_drive = direction * (bars[opening_close_index].close - context.day_open) / max(opening_width, 1e-9)
    session_position = directional_session_position(close, context, direction) or 0.5
    prior_context = 0.0
    ...
    return {
        "opening_boundary_proximity": boundary_proximity,
        "directional_opening_range_reclaim": bounded(directional_distance, -2.0, 2.0),
        "opening_range_extension_beyond_boundary": min(extension_beyond_boundary, 2.0),
        "recent_retest_recency": retest_recency,
        "retest_rejection_wick": rejection_wick,
        "directional_close_response_3": close_response,
        "opening_drive_alignment": bounded(opening_drive, -3.0, 3.0),
        "session_range_position": session_position,
        "prior_day_directional_context": bounded(prior_context, -3.0, 3.0),
        "retest_event_count": float(len(touch_positions)),
    }
...
def boundary_proximity_bucket(value: float) -> str:
    if value < 0.15:
        return "boundary_far"
    if value < 0.55:
        return "boundary_near"
    return "boundary_retest"


def reclaim_bucket(value: float) -> str:
    if value < -0.25:
        return "outside_opening_range"
    if value < 0.35:
        return "on_opening_boundary"
    return "reclaimed_inside_range"


def extension_bucket(value: float) -> str:
    if value <= 0.05:
        return "no_boundary_extension"
    if value < 0.45:
        return "shallow_boundary_extension"
    return "deep_boundary_extension"


def recency_bucket(value: float) -> str:
    if value < 0.35:
        return "old_retest"
    if value < 0.75:
        return "recent_retest"
    return "fresh_retest"
...
def candidate_label(
    ...
    acceptance_reward = 0.20 if recent_response > 0.15 and terminal > 0 else 0.0
    rejection_failure_penalty = 0.20 if recent_response < -0.15 and mae > mfe else 0.0
    ...
    retest_path_quality = (0.35 * terminal + 0.35 * mfe - 0.70 * mae) / average_range
...
def fit_linear_opening_range_retest_model(candidates: list[base.Candidate], fold_id: str) -> LinearOpeningRangeRetestModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearOpeningRangeRetestModel(fold_id, tuple(0.0 for _ in FEATURE_NAMES), tuple(1.0 for _ in FEATURE_NAMES), tuple(0.0 for _ in FEATURE_NAMES), 0, 0.0, 0.0, 0.0)
    ...
    retest_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearOpeningRangeRetestModel(
        fold_id=fold_id,
        ...
        retest_failure_rate=retest_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearOpeningRangeRetestModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_opening_range_retest_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0053_r0001_intraday_opening_range_retest",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0053_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0053_r0001_opening_range_retest_summary.json",
        ],
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0053 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0053_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0053 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def linear_opening_range_retest_model_summary(model: LinearOpeningRangeRetestModel) -> dict[str, object]:
    return {
        ...
        "retest_failure_rate": rounded(model.retest_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearOpeningRangeRetestModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_opening_range_retest_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "opening_range_retest_profile": profiles["opening_range_retest_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0053_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_trade_artifact"] = "artifacts/c0053_r0001_proxy_trades.csv"
    evidence["opening_range_retest_summary"] = "artifacts/c0053_r0001_opening_range_retest_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0053_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0053_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0053 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0053_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0053_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0053_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0053_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0053_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0053_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0053_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0053_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0053_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0053 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0053 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260706_produce_c0053_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0053_r0001_proxy_evidence
  refines:
  - dec_20260706_open_c0053_r0001_intraday_opening_range_retest_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0053_r0001_proxy_records_opening_range_retest_reference_surface
  ...
  - next_work_is_produce_c0053_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260706_produce_c0053_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0053-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0053_r0001_intraday_opening_range_retest",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0053-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0053-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0053-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0053_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0053_r0001_intraday_opening_range_retest",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0053-R0001-OPENING-RANGE-RETEST-SUMMARY",
            "artifact_role": "opening_range_retest_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0053_r0001_opening_range_retest_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0053_r0001_intraday_opening_range_retest",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0053_r0001_mt5_logic_parity_evidence",
        }
    ]
