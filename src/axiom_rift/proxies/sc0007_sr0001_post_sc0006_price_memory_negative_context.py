"""SC0007 SR0001 proxy for post-SC0006 price-memory negative-context synthesis."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "sc0007_sr0001_proxy_trades.csv"
SUMMARY_ARTIFACT_PATH = RUN_DIR / "artifacts" / "sc0007_sr0001_negative_context_synthesis_summary.json"
...
MODEL_FAMILY = "fold_local_post_sc0006_price_memory_negative_context_synthesis"
LABEL_SHAPE = "price_memory_density_followthrough_survival_under_recent_negative_memory_context"
SELECTION_RULE = "top_c0037_r0003_candidate_schedule_entries_reranked_by_negative_context_per_active_day"
BASE_CANDIDATE_SOURCE = (
    "C0037_R0003",
    "c0037_ig001_intraday_price_memory_density_candidate_evidence",
    "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003/artifacts/c0037_r0003_proxy_trades.csv",
)
C0037_R0003_ROBUSTNESS_AUDIT = (
    "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003/kpi/candidate_robustness_audit.json"
)
C0037_R0004_TICK_BY_FOLD = (
    "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0004/kpi/mt5_tick_by_fold.json"
)

NEGATIVE_SOURCE_INGREDIENTS = (
    (
        "C0031",
        "c0031_ig001_intraday_entropy_release_negative_memory",
        "campaigns/C0031_intraday_entropy_release_discovery/runs/R0001/artifacts/c0031_r0001_proxy_trades.csv",
        "campaigns/C0031_intraday_entropy_release_discovery/runs/R0001/kpi/mt5_tick_by_fold.json",
    ),
    (
        "C0032",
        "c0032_ig001_intraday_tail_risk_skew_negative_memory",
        "campaigns/C0032_intraday_tail_risk_skew_discovery/runs/R0001/artifacts/c0032_r0001_proxy_trades.csv",
        "campaigns/C0032_intraday_tail_risk_skew_discovery/runs/R0001/kpi/mt5_tick_by_fold.json",
    ),
    (
        "C0033",
        "c0033_ig001_intraday_seasonal_residual_dislocation_negative_memory",
        "campaigns/C0033_intraday_seasonal_residual_dislocation_discovery/runs/R0001/artifacts/c0033_r0001_proxy_trades.csv",
        "campaigns/C0033_intraday_seasonal_residual_dislocation_discovery/runs/R0001/kpi/mt5_tick_by_fold.json",
    ),
    (
        "C0034",
        "c0034_ig001_intraday_effort_price_absorption_negative_memory",
        "campaigns/C0034_intraday_effort_price_absorption_discovery/runs/R0001/artifacts/c0034_r0001_proxy_trades.csv",
        "campaigns/C0034_intraday_effort_price_absorption_discovery/runs/R0001/kpi/mt5_tick_by_fold.json",
    ),
    (
        "C0035",
        "c0035_ig001_intraday_cycle_phase_resonance_negative_memory",
        "campaigns/C0035_intraday_cycle_phase_resonance_discovery/runs/R0001/artifacts/c0035_r0001_proxy_trades.csv",
        "campaigns/C0035_intraday_cycle_phase_resonance_discovery/runs/R0001/kpi/mt5_tick_by_fold.json",
    ),
    (
        "C0036",
        "c0036_ig001_intraday_range_impulse_memory_negative_memory",
        "campaigns/C0036_intraday_range_impulse_memory_discovery/runs/R0001/artifacts/c0036_r0001_proxy_trades.csv",
        "campaigns/C0036_intraday_range_impulse_memory_discovery/runs/R0001/kpi/mt5_tick_by_fold.json",
    ),
)
SOURCE_INGREDIENT_IDS = tuple(row[1] for row in NEGATIVE_SOURCE_INGREDIENTS) + (
    BASE_CANDIDATE_SOURCE[1],
    "c0037_ig002_price_memory_density_robustness_fragility_lesson",
    "c0037_ig003_price_memory_density_monthly_distribution_hardening_negative_memory",
)
...
PRICE_MEMORY_FRAGILITY_PENALTY_WEIGHT = 0.10
SAME_FAILURE_CONTEXT_VETO_COUNT = 3
FRAGILITY_CONTEXT_VETO_LEVEL = 0.85

SCORE_COMPONENT_NAMES = (
    "c0037_r0003_source_score_z",
    ...
    "price_memory_fragility_penalty",
)
...
def run_sc0007_sr0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    fragility_profile = load_price_memory_fragility_profile()
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        candidates_by_fold[fold_id] = {
            "c0037_r0003_generator_candidate_count": len(fold_generators),
...
def load_price_memory_fragility_profile() -> dict[str, object]:
    audit = json.loads((PROJECT_ROOT / C0037_R0003_ROBUSTNESS_AUDIT).read_text(encoding="ascii"))
    required = audit.get("required_kpis", {})
    monthly = audit.get("monthly_tick_deal_audit", {})
    month_count = float(monthly.get("month_count") or 0.0)
    losing_month_count = float(monthly.get("losing_month_count") or 0.0)
    r0004 = json.loads((PROJECT_ROOT / C0037_R0004_TICK_BY_FOLD).read_text(encoding="ascii"))
    r0004_folds = r0004["conditional_profiles"]["fold_profile"]["fields"]["folds"]
    r0004_losing_fold_ids = [
        str(fold["fold_id"])
        for fold in r0004_folds
        if float(fold.get("required_kpis", {}).get("mt5_net_pnl", 0.0)) <= 0.0
    ]
    return {
        "monthly_loss_rate": losing_month_count / month_count if month_count > 0 else 0.0,
        "r0003_worst_fold_id": required.get("worst_fold_id"),
        "r0003_weakest_positive_fold_id": required.get("weakest_positive_fold_id"),
        "r0004_losing_fold_ids": r0004_losing_fold_ids,
    }
...
def synthesize_fold_candidates(
    ...
    fragility_profile: dict[str, object],
) -> list[base.Candidate]:
    ...
    for candidate in generator_candidates:
        ...
        fragility_pressure = price_memory_fragility_pressure(candidate.fold_id, fragility_profile)
        ...
        score = (
            ...
            - PRICE_MEMORY_FRAGILITY_PENALTY_WEIGHT * fragility_pressure
        )
        context_veto = same_count >= SAME_FAILURE_CONTEXT_VETO_COUNT or fragility_pressure >= FRAGILITY_CONTEXT_VETO_LEVEL
        ...
        synthesized.append(
            base.Candidate(
                ...
                state_key=(
                    f"{side}|post_sc0006_price_memory_negative_context|same_{same_count}|"
                    ...
                    f"frag_{int(round(100.0 * fragility_pressure))}"
                ),
                features=(
...
def price_memory_fragility_pressure(fold_id: str, profile: dict[str, object]) -> float:
    pressure = float(profile.get("monthly_loss_rate") or 0.0)
    if fold_id == profile.get("r0003_worst_fold_id"):
        pressure += 0.35
    if fold_id == profile.get("r0003_weakest_positive_fold_id"):
        pressure += 0.35
    if fold_id in set(profile.get("r0004_losing_fold_ids") or []):
        pressure += 0.50
...
def fold_model_summary(
    ...
    return {
        ...
        "c0037_r0003_generator_candidate_count": len(generator_candidates),
...
def synthesis_distribution(
    ...
    fragility_values = [candidate.features[8] for candidate in synthesized]
    ...
    return {
        ...
        "price_memory_fragility_pressure_p50": base.rounded(base.percentile(fragility_values, 0.50)),
...
def build_proxy_payload(
    ...
    payload["proxy_id"] = "PX-SC0007-SR0001"
    payload["proxy_engine"] = "axiom_rift.proxies.sc0007_sr0001_post_sc0006_price_memory_negative_context"
    payload["proxy_config_path"] = f"{RUN_REL}/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_c0037_r0003_schedule_negative_context_rerank"
    payload["proxy_artifact_paths"] = [
        f"{RUN_REL}/kpi/proxy.json",
        f"{RUN_REL}/artifacts/sc0007_sr0001_proxy_trades.csv",
        f"{RUN_REL}/artifacts/sc0007_sr0001_negative_context_synthesis_summary.json",
    ]
    ...
    profiles["price_memory_negative_context_synthesis_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
            "model_family": MODEL_FAMILY,
            ...
            "fragility_context_veto_level": FRAGILITY_CONTEXT_VETO_LEVEL,
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_sc0007_sr0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "source_oos_pnl_used_for_selection": False,
            "source_proxy_pnl_used_for_selection": False,
            "variant_boundary": "post_sc0006_price_memory_negative_context_synthesis_not_c0037_threshold_score_cost_buffer_stop_target_hold_session_activity_spread_capital_monthly_filter_or_retry_nudge",
...
def score_weights() -> dict[str, float]:
    return {
        ...
        "price_memory_fragility_penalty": PRICE_MEMORY_FRAGILITY_PENALTY_WEIGHT,
    }
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_sc0007_price_memory_negative_context_synthesis_summary_v1",
        "template": False,
        ...
        "price_memory_negative_context_synthesis_profile": profiles[
            "price_memory_negative_context_synthesis_profile"
        ]["fields"],
...
def update_artifact_lineage(
    ...
    filtered_roles = {
        ...
        "negative_context_synthesis_summary_artifact",
    }
    ...
    records.extend(
        [
            ...
            artifact_record(
                "A-SC0007-SR0001-NEGATIVE-CONTEXT-SYNTHESIS-SUMMARY",
                "negative_context_synthesis_summary_artifact",
                "json",
                f"{RUN_REL}/artifacts/sc0007_sr0001_negative_context_synthesis_summary.json",
                summary_hash,
    ...
    data["lineage_id"] = data.get("lineage_id") or "L-SC0007-SR0001"
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.sc0007_sr0001_post_sc0006_price_memory_negative_context",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/sc0007_sr0001_proxy_trades.csv"
    evidence["negative_context_synthesis_summary"] = "artifacts/sc0007_sr0001_negative_context_synthesis_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_sc0007_sr0001_mt5_logic_parity_evidence"
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "produce_sc0007_sr0001_mt5_logic_parity_evidence",
        }
    ]
...
def update_synthesis_status() -> None:
    ...
    next_candidate["direction"] = "produce_sc0007_sr0001_mt5_logic_parity_evidence"
...
def update_synthesis_queue_after_proxy() -> None:
    ...
    for item in data.get("queue", []):
        if item.get("synthesis_run_id") == RUN_ID:
            ...
            item["last_completed_step"] = "produce_sc0007_sr0001_proxy_evidence"
            item["next_action"] = "produce_sc0007_sr0001_mt5_logic_parity_evidence"
...
def update_selected_after_proxy() -> None:
    ...
    latest["next_required_action"] = "produce_sc0007_sr0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    if "produce_sc0007_sr0001_proxy_evidence" not in completed:
        completed.append("produce_sc0007_sr0001_proxy_evidence")
    next_action = "produce_sc0007_sr0001_mt5_logic_parity_evidence"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_sc0007_sr0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_sc0007_sr0001_mt5_logic_parity_evidence",
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_sc0007_sr0001_mt5_logic_parity_evidence"
    data["active_campaign"] = None
    data["active_synthesis"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["next_required_action"] = "produce_sc0007_sr0001_mt5_logic_parity_evidence"
    current = data.setdefault("current_evidence_summary", {})
    current.update(
        {
            "source_campaign": None,
            "source_synthesis": WORK_UNIT_REL,
            "current_task": "produce_sc0007_sr0001_mt5_logic_parity_evidence",
            "active_run": RUN_REL,
            "active_run_status": "proxy_recorded_pending_mt5",
            "evidence_status": "proxy_recorded_pending_mt5",
            "hypothesis_family": MODEL_FAMILY,
            "label_surface": LABEL_SHAPE,
            "feature_surface": "c0037_r0003_schedule_score_plus_c0031_c0036_negative_memory_context",
            "trade_logic_surface": "fixed_lot_c0037_r0003_schedule_reranked_by_post_sc0006_negative_context",
            "entries_per_active_day": summary.get("entries_per_active_day") if isinstance(summary, dict) else None,
            "proxy_net_pnl_points": required.get("proxy_net_pnl_points") if isinstance(required, dict) else None,
            "proxy_profit_factor": required.get("proxy_profit_factor") if isinstance(required, dict) else None,
            "proxy_trade_count": required.get("proxy_trade_count") if isinstance(required, dict) else None,
            "note": "SC0007 SR0001 proxy is recorded; MT5 paired validation is mandatory before judgment.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "SC0007 SR0001 proxy evidence is recorded; MT5 logic parity is next and proxy is not a go/no-go gate.",
        },
        {
        ...
        },
        {
            ...
            "summary": "Ingredient refs record C0031-C0036 negative memory and C0037 candidate/fragility evidence used by SR0001.",
        },
    ]
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    decision_id = "dec_20260705_produce_sc0007_sr0001_proxy_evidence"
...
- decision_id: {decision_id}
  ...
  status: active
  decision: produce_sc0007_sr0001_proxy_evidence
  refines:
  - dec_20260705_open_sc0007_sr0001_post_sc0006_price_memory_mixed_evidence_synthesis_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - sc0007_sr0001_proxy_reranks_c0037_r0003_candidate_schedule_entries_with_c0031_through_c0036_negative_memory_context
  ...
  - next_work_is_produce_sc0007_sr0001_mt5_logic_parity_evidence
  claim_boundary:
    claim_authority: false
    selected: false
    label_selected: false
...
def replace_base_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004 R0001": "SC0007 SR0001",
            "C0004": "SC0007",
            "R0001": "SR0001",
            "c0004_r0001_fold_local_state_archetype": "sc0007_sr0001_post_sc0006_price_memory_negative_context",
            "c0004_r0001": "sc0007_sr0001",
            "fold_local_state_archetype_discovery": "post_sc0006_price_memory_negative_context_synthesis",
            "fold_local_state_archetype_membership": MODEL_FAMILY,
            "directional_target_before_stop_plus_path_quality": LABEL_SHAPE,
            "state_archetype_summary": "negative_context_synthesis_summary",
        }
...
if __name__ == "__main__":
    print(json.dumps(run_sc0007_sr0001_proxy(write=True), indent=2, sort_keys=True))
