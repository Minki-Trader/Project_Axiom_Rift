"""C0026 R0001 proxy evidence for price acceleration curvature discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0026_price_acceleration_curvature_discovery" / "runs" / "R0001"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0026_price_acceleration_curvature_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0026_r0001_proxy_trades.csv"
PRICE_ACCELERATION_CURVATURE_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0026_r0001_price_acceleration_curvature_summary.json"
...
FEATURE_NAMES = (
    ...
    "directional_velocity_6",
    "directional_velocity_12",
    "directional_velocity_24",
    "acceleration_3_6",
    "acceleration_6_12",
    "acceleration_12_24",
    "directional_acceleration_3_6",
    "directional_acceleration_6_12",
    "directional_acceleration_12_24",
    "curvature_3",
    "curvature_6",
    "curvature_12",
    "directional_curvature_3",
    "directional_curvature_6",
    "directional_curvature_12",
    "jerk_3_6",
    "jerk_6_12",
    "directional_jerk_3_6",
    "directional_jerk_6_12",
    "path_efficiency_6",
    "path_efficiency_12",
    "path_efficiency_24",
    "bend_ratio_6",
    ...
    "alternation_rate_6",
    "alternation_rate_12",
    "close_streak_directional_6",
    "close_streak_directional_12",
    "body_acceleration_3_6",
    "directional_body_acceleration_3_6",
    "range_rel_12",
    "range_rel_48",
    "spread_over_range",
    "spread_relief_3_vs_24",
)
MODEL_FAMILY = "fold_local_price_acceleration_curvature_rank"
LABEL_SHAPE = "directional_price_acceleration_curvature_transition_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_price_acceleration_curvature_scores_per_active_day"
...
def run_c0026_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = price_acceleration_curvature_features(
                bars,
                range_average,
                short_range_average,
                day_context,
            ...
            if features is None:
                continue
            label = (
                price_acceleration_curvature_label(bars, range_average, index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|price_acceleration_curvature",
                    features=features,
                    label=label,
                )
            )
...
def price_acceleration_curvature_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    if spread_over_range > 0.65:
        return None
    v3 = close_velocity(bars, index, 3, average_range)
    v6 = close_velocity(bars, index, 6, average_range)
    v12 = close_velocity(bars, index, 12, average_range)
    v24 = close_velocity(bars, index, 24, average_range)
    accel_3_6 = v3 - v6
    accel_6_12 = v6 - v12
    accel_12_24 = v12 - v24
    curve_3 = price_curvature(bars, index, 3, average_range)
    curve_6 = price_curvature(bars, index, 6, average_range)
    curve_12 = price_curvature(bars, index, 12, average_range)
    jerk_3_6 = curve_3 - curve_6
    jerk_6_12 = curve_6 - curve_12
    body_accel_3_6 = body_velocity(bars, index, 3, average_range) - body_velocity(bars, index, 6, average_range)
    sign_6 = close_sign_stats(bars, index, direction, 6)
    sign_12 = close_sign_stats(bars, index, direction, 12)
    return (
        float(direction),
        v3,
        v6,
        ...
        accel_12_24,
        direction * accel_3_6,
        direction * accel_6_12,
        direction * accel_12_24,
        curve_3,
        curve_6,
        curve_12,
        direction * curve_3,
        ...
        direction * jerk_3_6,
        direction * jerk_6_12,
        directional_efficiency(bars, index, direction, 6),
        directional_efficiency(bars, index, direction, 12),
        directional_efficiency(bars, index, direction, 24),
        path_bend_ratio(bars, index, 6),
        path_bend_ratio(bars, index, 12),
        sign_6["alternating_churn"],
        sign_12["alternating_churn"],
        directional_close_streak(bars, index, direction, 6),
        directional_close_streak(bars, index, direction, 12),
        body_accel_3_6,
        direction * body_accel_3_6,
        average_bar_range(bars, index, 12) / average_range,
        average_bar_range(bars, index, 48) / average_range,
        spread_over_range,
        spread_relief,
    )


def close_velocity(bars: list[base.Bar], index: int, lookback: int, average_range: float) -> float:
    return (bars[index].close - bars[index - lookback].close) / max(average_range, 1e-9)


def body_velocity(bars: list[base.Bar], index: int, lookback: int, average_range: float) -> float:
    body_sum = sum(bars[offset].close - bars[offset].open for offset in range(index - lookback + 1, index + 1))
    return body_sum / max(average_range, 1e-9)


def price_curvature(bars: list[base.Bar], index: int, lookback: int, average_range: float) -> float:
    return (
        bars[index].close
        - 2.0 * bars[index - lookback].close
        + bars[index - (2 * lookback)].close
    ) / max(average_range, 1e-9)


def path_bend_ratio(bars: list[base.Bar], index: int, lookback: int) -> float:
    path = sum(abs(bars[offset].close - bars[offset - 1].close) for offset in range(index - lookback + 1, index + 1))
    net = abs(bars[index].close - bars[index - lookback].close)
    if path <= 0:
        return 0.0
    return bounded((path - net) / path, 0.0, 1.0)
...
def price_acceleration_curvature_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    index: int,
    direction: int,
    ...
    future_closes = [entry] + [bar.close for bar in path]
    future_velocity_3 = future_directional_velocity(future_closes, direction, 3, average_range)
    future_velocity_6 = future_directional_velocity(future_closes, direction, 6, average_range)
    future_acceleration = future_velocity_3 - future_velocity_6
    future_curvature = future_directional_curvature(future_closes, direction, 3, average_range)
    late_velocity = future_directional_velocity(future_closes, direction, min(10, len(future_closes) - 1), average_range)
    acceleration_decay = max(0.0, future_velocity_3 - late_velocity)
    return (
        0.82 * first_hit
        + 0.34 * target_speed
        - 0.46 * stop_speed
        ...
        + 0.24 * future_velocity_3
        + 0.20 * future_acceleration
        + 0.16 * future_curvature
        - 0.18 * acceleration_decay
        - 1.08 * spread_norm
        - 0.20 * spread_drift
    )


def future_directional_velocity(closes: list[float], direction: int, lookback: int, average_range: float) -> float:
    if len(closes) <= 1:
        return 0.0
    actual_lookback = min(lookback, len(closes) - 1)
    return direction * (closes[actual_lookback] - closes[0]) / max(average_range, 1e-9)


def future_directional_curvature(closes: list[float], direction: int, lookback: int, average_range: float) -> float:
    if len(closes) <= (2 * lookback):
        return 0.0
    return direction * (closes[2 * lookback] - 2.0 * closes[lookback] + closes[0]) / max(average_range, 1e-9)
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0026"
    payload["work_unit_id"] = "C0026"
    payload["run_id"] = "R0001"
    payload["proxy_id"] = "PX-C0026-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0026_r0001_price_acceleration_curvature"
    payload["proxy_config_path"] = "campaigns/C0026_price_acceleration_curvature_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_price_acceleration_curvature_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0026_price_acceleration_curvature_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0026_price_acceleration_curvature_discovery/runs/R0001/artifacts/c0026_r0001_proxy_trades.csv",
        "campaigns/C0026_price_acceleration_curvature_discovery/runs/R0001/artifacts/c0026_r0001_price_acceleration_curvature_summary.json",
    ]
    ...
    profiles["price_acceleration_curvature_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0026_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "feature_weight_floor": FEATURE_WEIGHT_FLOOR,
            "feature_weight_ceiling": FEATURE_WEIGHT_CEILING,
            "score_interpretation": "higher_score_means_direct_fold_local_price_acceleration_curvature_rank",
            "variant_boundary": (
                "price_acceleration_curvature_rank_not_range_overlap_calendar_phase_tick_participation_"
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_price_acceleration_curvature_summary_artifact(payload, PRICE_ACCELERATION_CURVATURE_SUMMARY_PATH)
    PROXY_PATH.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="ascii")
    trade_hash = base.sha256_file(TRADE_ARTIFACT_PATH)
    summary_hash = base.sha256_file(PRICE_ACCELERATION_CURVATURE_SUMMARY_PATH)
...
def write_price_acceleration_curvature_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_price_acceleration_curvature_summary_v1",
        "template": False,
        "work_unit_id": "C0026",
        "campaign_id": "C0026",
        "run_id": "R0001",
        ...
        "price_acceleration_curvature_profile": profiles["price_acceleration_curvature_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "price_acceleration_curvature_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0026-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0026_price_acceleration_curvature_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0026_price_acceleration_curvature_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0026-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0026-R0001-PRICE-ACCELERATION-CURVATURE-SUMMARY",
                "price_acceleration_curvature_summary_artifact",
                "json",
                "campaigns/C0026_price_acceleration_curvature_discovery/runs/R0001/artifacts/c0026_r0001_price_acceleration_curvature_summary.json",
                summary_hash,
                ["campaigns/C0026_price_acceleration_curvature_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0026_r0001_price_acceleration_curvature",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0026_r0001_proxy_trades.csv"
    evidence["price_acceleration_curvature_summary"] = "artifacts/c0026_r0001_price_acceleration_curvature_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0026_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0026_r0001_proxy_trades.csv",
        "artifacts/c0026_r0001_price_acceleration_curvature_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0026 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0026_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0026_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0026_price_acceleration_curvature_discovery"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0026_price_acceleration_curvature_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0026_price_acceleration_curvature_discovery",
        "open_c0026_r0001_fold_local_price_acceleration_curvature_rank_run",
        "produce_c0026_r0001_proxy_evidence",
    ):
    ...
    data["active_run"] = "campaigns/C0026_price_acceleration_curvature_discovery/runs/R0001"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["active_campaign"] = "campaigns/C0026_price_acceleration_curvature_discovery"
    data["active_run"] = "campaigns/C0026_price_acceleration_curvature_discovery/runs/R0001"
    data["latest_operation"] = {
        "id": "produce_c0026_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0026_price_acceleration_curvature_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0026_r0001_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0026",
            "c0004_r0001_fold_local_state_archetype": "c0026_r0001_price_acceleration_curvature",
            "c0004_r0001": "c0026_r0001",
            "fold_local_state_archetype_discovery": "price_acceleration_curvature_discovery",
            ...
            "state_archetype_summary": "price_acceleration_curvature_summary",
        }
