"""C0129 R0001 proxy evidence for intraday directional curvature continuation."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0129_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0129_r0001_directional_curvature_continuation_summary.json"
...
FEATURE_NAMES = (
    "directional_range_curvature",
    "cumulative_close_slope",
    "pullback_absorption",
    ...
    "micro_path_smoothness",
    "prior_day_alignment",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_directional_curvature_continuation_rank"
LABEL_SHAPE = "target_first_quality_after_directional_curvature_continuation"
SELECTION_RULE = "top_fold_local_directional_curvature_continuation_scores_per_active_day"
...
MIN_DAY_BARS = 18
DIRECTIONAL_CURVATURE_CONTINUATION_LOOKBACK_BARS = 42
DIRECTIONAL_CURVATURE_CONTINUATION_MICRO_BARS = 10
MIN_DIRECTIONAL_RANGE_CURVATURE = 0.18
MIN_VWAP_TREND_ALIGNMENT = -0.25
MIN_DIRECTIONAL_CURVATURE_CONTINUATION_ACTIVITY = 0.05
...
class LinearDirectionalCurvatureContinuationModel:
    fold_id: str
    ...
    directional_curvature_continuation_failure_rate: float
...
def run_c0129_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_directional_curvature_continuation_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_directional_curvature_continuation_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        DIRECTIONAL_CURVATURE_CONTINUATION_LOOKBACK_BARS,
        DIRECTIONAL_CURVATURE_CONTINUATION_MICRO_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = directional_curvature_continuation_metrics(
    ...
    features = (
        metrics["directional_range_curvature"],
        metrics["cumulative_close_slope"],
        metrics["pullback_absorption"],
        metrics["vwap_trend_alignment"],
        metrics["session_extreme_pressure"],
        metrics["range_expansion_efficiency"],
        metrics["volume_confirmation_ratio"],
        metrics["close_location_persistence"],
        metrics["micro_path_smoothness"],
        metrics["prior_day_alignment"],
        metrics["session_maturity"],
        metrics["spread_stress_inverse"],
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        breakout_pressure_bucket(metrics["directional_range_curvature"]),
        drift_efficiency_bucket(metrics["cumulative_close_slope"]),
        absorption_balance_bucket(metrics["pullback_absorption"]),
        vwap_alignment_bucket(metrics["vwap_trend_alignment"]),
        range_position_pressure_bucket(metrics["session_extreme_pressure"]),
        range_growth_efficiency_bucket(metrics["range_expansion_efficiency"]),
        volume_confirmation_bucket(metrics["volume_confirmation_ratio"]),
        close_location_confirm_bucket(metrics["close_location_persistence"]),
        path_efficiency_bucket(metrics["micro_path_smoothness"]),
        prior_tailwind_bucket(metrics["prior_day_alignment"]),
...
def directional_curvature_continuation_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    micro_start = max(index - DIRECTIONAL_CURVATURE_CONTINUATION_MICRO_BARS + 1, context.day_start_index)
    lookback_start = max(index - DIRECTIONAL_CURVATURE_CONTINUATION_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    close_delta = direction * (signal_bar.close - lookback_window[0].close)
    micro_delta = direction * (signal_bar.close - micro_window[0].close)
    directional_range_curvature = close_delta / max(average_range * 2.5, 1e-9)
    if directional_range_curvature < MIN_DIRECTIONAL_RANGE_CURVATURE:
        return {"event_count": 0.0}
    ...
    cumulative_delta = sum(
        direction * (bars[pos].close - bars[pos - 1].close) for pos in range(lookback_start + 1, index + 1)
    )
    cumulative_close_slope = cumulative_delta / max(average_range * len(lookback_window) / 12.0, 1e-9)
    path_travel = sum(abs(micro_window[pos].close - micro_window[pos - 1].close) for pos in range(1, len(micro_window)))
    micro_path_smoothness = micro_delta / max(path_travel, average_range, 1e-9)
    adverse_step = max(
        [max(0.0, -direction * (micro_window[pos].close - micro_window[pos - 1].close)) for pos in range(1, len(micro_window))]
        or [0.0]
    )
    pullback_absorption = (
        base.mean([adverse_wick_absorption(bar, direction) for bar in micro_window]) or 0.0
    ) + max(0.0, 1.0 - adverse_step / max(average_range, 1e-9)) * 0.25
    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    vwap_trend_alignment = direction * (signal_bar.close - session_vwap) / max(average_range * 2.5, 1e-9)
    if vwap_trend_alignment < MIN_VWAP_TREND_ALIGNMENT:
        return {"event_count": 0.0}
    session_extreme_pressure = directional_session_percentile_position(
        signal_bar.close,
        context.low_so_far,
        context.high_so_far,
        direction,
    )
    range_expansion_efficiency = directional_range_curvature / max(lookback_range_mean / max(average_range, 1e-9), 0.25)
    volume_confirmation_ratio = micro_volume_mean / max(lookback_volume_mean, average_volume, 1e-9)
    close_location_persistence = base.mean([directional_bar_settlement(bar) for bar in micro_window]) or 0.0
    prior_day_alignment = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_day_alignment = direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
    if context.prior_day_shadow_balance is not None:
        prior_day_alignment += direction * context.prior_day_shadow_balance * 0.05
    ...
    activity = (
        max(directional_range_curvature, 0.0) * 0.18
        + max(cumulative_close_slope, 0.0) * 0.13
        + pullback_absorption * 0.11
        + max(vwap_trend_alignment, 0.0) * 0.12
        + session_extreme_pressure * 0.09
        + max(range_expansion_efficiency, 0.0) * 0.10
        + max(volume_confirmation_ratio - 1.0, 0.0) * 0.06
        + close_location_persistence * 0.09
        + max(micro_path_smoothness, 0.0) * 0.08
        + max(prior_day_alignment, 0.0) * 0.04
    )
    if activity < MIN_DIRECTIONAL_CURVATURE_CONTINUATION_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "directional_range_curvature": bounded(directional_range_curvature, -4.0, 6.0),
        "cumulative_close_slope": bounded(cumulative_close_slope, -4.0, 6.0),
        "pullback_absorption": bounded(pullback_absorption, 0.0, 1.5),
        "vwap_trend_alignment": bounded(vwap_trend_alignment, -4.0, 4.0),
        "session_extreme_pressure": bounded(session_extreme_pressure, 0.0, 1.0),
        "range_expansion_efficiency": bounded(range_expansion_efficiency, -4.0, 6.0),
        "volume_confirmation_ratio": bounded(volume_confirmation_ratio, 0.0, 8.0),
        "close_location_persistence": bounded(close_location_persistence, 0.0, 1.0),
        "micro_path_smoothness": bounded(micro_path_smoothness, -2.0, 2.0),
        "prior_day_alignment": bounded(prior_day_alignment, -3.0, 3.0),
        "range_stretch_pressure": bounded(directional_range_curvature, 0.0, 8.0),
        ...
        "directional_curvature_continuation_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0129_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0129_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0129_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0129_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0129_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0129_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0129_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0129_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0129_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0129_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0129_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0129_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0129_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0129_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0129_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0129_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0129_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0129_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0129_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def c0129_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    curvature_reward = 0.14 if metrics["directional_range_curvature"] > 0.55 and terminal > 0 else 0.0
    slope_reward = 0.11 if metrics["cumulative_close_slope"] > 0.40 and terminal > 0 else 0.0
    absorption_reward = 0.08 if metrics["pullback_absorption"] > 0.55 and terminal > 0 else 0.0
    vwap_alignment_reward = 0.10 if metrics["vwap_trend_alignment"] > 0.05 and terminal > 0 else 0.0
    extreme_pressure_reward = 0.08 if metrics["session_extreme_pressure"] > 0.58 and terminal > 0 else 0.0
    efficiency_reward = 0.09 if metrics["range_expansion_efficiency"] > 0.25 and terminal > 0 else 0.0
    volume_reward = 0.05 if 0.85 <= metrics["volume_confirmation_ratio"] <= 3.80 and terminal > 0 else 0.0
    persistence_reward = 0.10 if metrics["close_location_persistence"] > 0.56 and terminal > 0 else 0.0
    smoothness_reward = 0.10 if metrics["micro_path_smoothness"] > 0.10 and terminal > 0 else 0.0
    prior_day_reward = 0.04 if metrics["prior_day_alignment"] > -0.15 and terminal > 0 else 0.0
    weak_curvature_penalty = 0.11 if metrics["directional_range_curvature"] < 0.28 and terminal <= 0 else 0.0
    weak_slope_penalty = 0.08 if metrics["cumulative_close_slope"] < 0.05 and terminal <= 0 else 0.0
    weak_vwap_penalty = 0.07 if metrics["vwap_trend_alignment"] < -0.12 and terminal <= 0 else 0.0
    weak_persistence_penalty = 0.08 if metrics["close_location_persistence"] < 0.48 and terminal <= 0 else 0.0
    weak_smoothness_penalty = 0.08 if metrics["micro_path_smoothness"] < -0.05 and terminal <= 0 else 0.0
    overheated_volume_penalty = 0.05 if metrics["volume_confirmation_ratio"] > 4.25 and terminal <= 0 else 0.0
    ...
    return (
        ...
        + slope_reward
        + absorption_reward
        + vwap_alignment_reward
        + extreme_pressure_reward
        + efficiency_reward
        + volume_reward
        + persistence_reward
        + smoothness_reward
        + prior_day_reward
        - weak_curvature_penalty
        - weak_slope_penalty
        - weak_vwap_penalty
        ...
        - max(0.0, metrics["volume_confirmation_ratio"] - 4.20) * 0.015
        - max(0.0, MIN_DIRECTIONAL_RANGE_CURVATURE - metrics["directional_range_curvature"]) * 0.12
        - max(0.0, MIN_VWAP_TREND_ALIGNMENT - metrics["vwap_trend_alignment"]) * 0.08
    )


def fit_linear_directional_curvature_continuation_model(candidates: list[base.Candidate], fold_id: str) -> LinearDirectionalCurvatureContinuationModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearDirectionalCurvatureContinuationModel(
            fold_id,
    ...
    directional_curvature_continuation_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearDirectionalCurvatureContinuationModel(
        fold_id=fold_id,
        ...
        directional_curvature_continuation_failure_rate=directional_curvature_continuation_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearDirectionalCurvatureContinuationModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_directional_curvature_continuation_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0129_r0001_intraday_directional_curvature_continuation",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0129_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0129_r0001_directional_curvature_continuation_summary.json",
        ],
        ...
        "conditional_profiles": {
            "directional_curvature_continuation_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0129 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0129_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0129 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "directional_curvature_continuation_lookback_bars": DIRECTIONAL_CURVATURE_CONTINUATION_LOOKBACK_BARS,
        "directional_curvature_continuation_micro_bars": DIRECTIONAL_CURVATURE_CONTINUATION_MICRO_BARS,
        "min_directional_range_curvature": MIN_DIRECTIONAL_RANGE_CURVATURE,
        "min_vwap_trend_alignment": MIN_VWAP_TREND_ALIGNMENT,
        "min_directional_curvature_continuation_activity": MIN_DIRECTIONAL_CURVATURE_CONTINUATION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_directional_curvature_continuation_model_summary(model: LinearDirectionalCurvatureContinuationModel) -> dict[str, object]:
    return {
        ...
        "directional_curvature_continuation_failure_rate": rounded(model.directional_curvature_continuation_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearDirectionalCurvatureContinuationModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_directional_curvature_continuation_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "directional_curvature_continuation_profile": profiles["directional_curvature_continuation_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0129_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0129_r0001_proxy_trades.csv"
    evidence["directional_curvature_continuation_summary"] = "artifacts/c0129_r0001_directional_curvature_continuation_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0129_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0129_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0129 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0129_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0129_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0129_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0129_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0129_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0129_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0129_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0129_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0129_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0129 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0129 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    block = f"""
- decision_id: dec_20260708_produce_c0129_r0001_proxy_evidence
  created_local_date: '{local_date}'
  status: active
  decision: produce_c0129_r0001_proxy_evidence
  refines:
  - dec_20260708_open_c0129_r0001_intraday_directional_curvature_continuation_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0129_r0001_proxy_records_directional_curvature_continuation_reference_surface
  ...
  - next_work_is_produce_c0129_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260708_produce_c0129_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0129-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0129_r0001_intraday_directional_curvature_continuation",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0129-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0129-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0129-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0129_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0129_r0001_intraday_directional_curvature_continuation",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0129-R0001-DIRECTIONAL-CURVATURE-CONTINUATION-SUMMARY",
            "artifact_role": "directional_curvature_continuation_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0129_r0001_directional_curvature_continuation_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0129_r0001_intraday_directional_curvature_continuation",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0129_r0001_mt5_logic_parity_evidence",
        }
    ]
