"""C0071 R0001 proxy evidence for intraday path tortuosity release."""
...
CAMPAIGN_ID = "C0071"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0071_intraday_path_tortuosity_release_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0071_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0071_r0001_ptr_summary.json"
...
FEATURE_NAMES = (
    "short_tortuosity_release",
    "medium_tortuosity_release",
    "path_efficiency_rebuild",
    "turn_density_pressure",
    "overlap_compression_release",
    "directional_excursion_bias",
    "close_location_release",
    "volume_followthrough",
    "prior_day_tortuosity_context",
    "session_maturity_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_path_tortuosity_release_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_path_tortuosity_release_state"
SELECTION_RULE = "top_fold_local_path_tortuosity_release_scores_per_active_day"
...
MIN_DAY_BARS = 30
PATH_TORTUOSITY_RELEASE_LOOKBACK_BARS = 20
PATH_TORTUOSITY_RELEASE_SHORT_LOOKBACK_BARS = 6
MIN_PATH_TORTUOSITY_RELEASE_ACTIVITY = 0.10
...
class LinearPathTortuosityReleaseModel:
    fold_id: str
    ...
    path_tortuosity_release_failure_rate: float
...
def run_c0071_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_path_tortuosity_release_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_path_tortuosity_release_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        PATH_TORTUOSITY_RELEASE_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    release = path_tortuosity_release_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        release["short_tortuosity_release"],
        release["medium_tortuosity_release"],
        release["path_efficiency_rebuild"],
        release["turn_density_pressure"],
        release["overlap_compression_release"],
        release["directional_excursion_bias"],
        release["close_location_release"],
        release["volume_followthrough"],
        release["prior_day_tortuosity_context"],
        release["session_maturity_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        short_tortuosity_release_bucket(release["short_tortuosity_release"]),
        medium_tortuosity_release_bucket(release["medium_tortuosity_release"]),
        efficiency_rebuild_bucket(release["path_efficiency_rebuild"]),
        turn_density_bucket(release["turn_density_pressure"]),
        overlap_release_bucket(release["overlap_compression_release"]),
        excursion_bias_bucket(release["directional_excursion_bias"]),
        close_location_release_bucket(release["close_location_release"]),
        volume_followthrough_bucket(release["volume_followthrough"]),
        prior_day_tortuosity_bucket(release["prior_day_tortuosity_context"]),
...
def path_tortuosity_release_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    window_start = index - PATH_TORTUOSITY_RELEASE_LOOKBACK_BARS + 1
    short_start = index - PATH_TORTUOSITY_RELEASE_SHORT_LOOKBACK_BARS + 1
    previous_short_start = index - PATH_TORTUOSITY_RELEASE_SHORT_LOOKBACK_BARS
    ...
    def window_close_travel(window: list[base.Bar]) -> float:
        if not window:
            return 0.0
        travel = abs(window[0].close - window[0].open)
        previous_close = window[0].close
        for item in window[1:]:
            travel += abs(item.close - previous_close)
            previous_close = item.close
        return travel

    def directional_efficiency(window: list[base.Bar]) -> float:
        if not window:
            return 0.0
        net = direction * (window[-1].close - window[0].open)
        return net / max(window_close_travel(window), 1e-9)

    def turn_density(window: list[base.Bar]) -> float:
        signs: list[int] = []
        previous_close = window[0].close
        for item in window[1:]:
            delta = item.close - previous_close
            if abs(delta) > 1e-9:
                signs.append(1 if delta > 0.0 else -1)
            previous_close = item.close
        if len(signs) < 2:
            return 0.0
        turns = sum(1 for offset in range(1, len(signs)) if signs[offset] != signs[offset - 1])
        return turns / max(len(signs) - 1, 1)

    def average_range_overlap(window: list[base.Bar]) -> float:
        overlaps: list[float] = []
        for left, right in zip(window, window[1:]):
            overlap = max(0.0, min(left.high, right.high) - max(left.low, right.low))
            overlaps.append(overlap / max(average_range, 1e-9))
        return base.mean(overlaps) or 0.0

    short_efficiency = directional_efficiency(short_window)
    medium_efficiency = directional_efficiency(medium_window)
    previous_short_efficiency = directional_efficiency(previous_short_window)
    short_tortuosity_release = short_efficiency - previous_short_efficiency
    medium_tortuosity_release = short_efficiency - medium_efficiency
    signal_body_efficiency = direction * (signal_bar.close - signal_bar.open) / signal_range
    path_efficiency_rebuild = (
        max(0.0, -previous_short_efficiency) * max(0.0, short_efficiency)
        + max(0.0, signal_body_efficiency)
    )
    short_turn_density = turn_density(short_window)
    medium_turn_density = turn_density(medium_window)
    turn_density_pressure = medium_turn_density - short_turn_density
    previous_overlap = average_range_overlap(previous_short_window)
    current_overlap = average_range_overlap(short_window)
    overlap_compression_release = max(0.0, previous_overlap - current_overlap) + max(
        0.0, signal_body_efficiency
    )
    favorable_body = sum(max(0.0, direction * (item.close - item.open)) for item in short_window)
    adverse_body = sum(max(0.0, -direction * (item.close - item.open)) for item in short_window)
    directional_excursion_bias = (favorable_body - adverse_body) / max(
        average_range * len(short_window),
        1e-9,
    )
    previous_close_locations = [
        directional_bar_settlement(item, direction) for item in previous_short_window
    ]
    close_location_release = directional_bar_settlement(signal_bar, direction) - (
        base.mean(previous_close_locations) or 0.5
    )
    ...
    volume_followthrough = (directional_volume - opposing_volume) / max(expected_recent_volume, 1e-9)
    if (
        context.prior_day_range is None
        or context.prior_day_range <= 0
        or context.prior_day_return is None
    ):
        prior_day_tortuosity_context = 0.0
    else:
        prior_day_tortuosity_context = 1.0 - min(
            1.0,
            abs(context.prior_day_return) / context.prior_day_range,
        )
    path_pressure = max(
        abs(short_tortuosity_release),
        abs(medium_tortuosity_release),
        path_efficiency_rebuild,
        abs(turn_density_pressure),
        overlap_compression_release,
        abs(directional_excursion_bias),
        abs(close_location_release),
        abs(volume_followthrough),
        prior_day_tortuosity_context,
    )
    session_maturity_pressure = bounded_session_progress(signal_bar.time) * max(
        path_pressure,
        max(short_efficiency, 0.0),
        max(signal_body_efficiency, 0.0),
    )
    activity = max(
        path_pressure,
        session_maturity_pressure,
    )
    if activity < MIN_PATH_TORTUOSITY_RELEASE_ACTIVITY:
        return {"event_count": 0.0}
    if (
        short_tortuosity_release < 0.03
        and path_efficiency_rebuild < 0.05
        and turn_density_pressure < 0.03
        and abs(close_location_release) < 0.05
    ):
        return {"event_count": 0.0}
    return {
        "short_tortuosity_release": bounded(short_tortuosity_release, -2.0, 2.0),
        "medium_tortuosity_release": bounded(medium_tortuosity_release, -2.0, 2.0),
        "path_efficiency_rebuild": bounded(path_efficiency_rebuild, 0.0, 3.0),
        "turn_density_pressure": bounded(turn_density_pressure, -1.0, 1.0),
        "overlap_compression_release": bounded(overlap_compression_release, 0.0, 3.0),
        "directional_excursion_bias": bounded(directional_excursion_bias, -4.0, 4.0),
        "close_location_release": bounded(close_location_release, -1.0, 1.0),
        "volume_followthrough": bounded(volume_followthrough, -4.0, 4.0),
        "prior_day_tortuosity_context": bounded(prior_day_tortuosity_context, 0.0, 1.0),
...
def short_tortuosity_release_bucket(value: float) -> str:
    if value < -0.12:
        return "short_tortuosity_recoiling"
    if value < 0.12:
        return "short_tortuosity_flat"
    return "short_tortuosity_releasing"


def medium_tortuosity_release_bucket(value: float) -> str:
    if value < -0.16:
        return "medium_tortuosity_below_context"
    if value < 0.16:
        return "medium_tortuosity_near_context"
    return "medium_tortuosity_above_context"


def efficiency_rebuild_bucket(value: float) -> str:
    if value < 0.12:
        return "efficiency_rebuild_low"
    if value < 0.40:
        return "efficiency_rebuild_mid"
    return "efficiency_rebuild_high"


def turn_density_bucket(value: float) -> str:
    if value < -0.12:
        return "turn_density_rising"
    if value < 0.12:
        return "turn_density_neutral"
    return "turn_density_releasing"


def overlap_release_bucket(value: float) -> str:
    if value < 0.10:
        return "overlap_release_low"
    if value < 0.35:
        return "overlap_release_mid"
    return "overlap_release_high"


def excursion_bias_bucket(value: float) -> str:
    if value < -0.15:
        return "excursion_bias_adverse"
    if value < 0.18:
        return "excursion_bias_mixed"
    return "excursion_bias_favorable"


def close_location_release_bucket(value: float) -> str:
    if value < -0.10:
        return "close_location_release_adverse"
    if value < 0.12:
        return "close_location_release_mixed"
    return "close_location_release_favorable"


def volume_followthrough_bucket(value: float) -> str:
    if value < -0.18:
        return "volume_against"
    if value < 0.22:
        return "volume_mixed"
    return "volume_with"


def prior_day_tortuosity_bucket(value: float) -> str:
    if value < 0.33:
        return "prior_day_tortuosity_low"
    if value < 0.67:
        return "prior_day_tortuosity_mid"
    return "prior_day_tortuosity_high"
...
def candidate_label(
    ...
    metrics = path_tortuosity_release_metrics(
        bars,
        volumes,
    ...
    if metrics["event_count"] <= 0:
        return None
    short_reward = 0.12 if metrics["short_tortuosity_release"] > 0.08 and terminal > 0 else 0.0
    medium_reward = 0.10 if metrics["medium_tortuosity_release"] > 0.06 and terminal > 0 else 0.0
    efficiency_reward = 0.18 if metrics["path_efficiency_rebuild"] > 0.18 and terminal > 0 else 0.0
    turn_reward = 0.12 if metrics["turn_density_pressure"] > 0.08 and terminal > 0 else 0.0
    overlap_reward = 0.10 if metrics["overlap_compression_release"] > 0.12 and terminal > 0 else 0.0
    excursion_reward = 0.12 if metrics["directional_excursion_bias"] > 0.10 and terminal > 0 else 0.0
    close_location_reward = 0.10 if metrics["close_location_release"] > 0.08 and terminal > 0 else 0.0
    volume_reward = 0.10 if metrics["volume_followthrough"] > 0.12 and terminal > 0 else 0.0
    prior_tortuosity_reward = (
        0.06 if metrics["prior_day_tortuosity_context"] > 0.55 and terminal > 0 else 0.0
    )
    renewed_tortuosity_penalty = (
        0.20 if metrics["short_tortuosity_release"] < -0.08 and terminal <= 0 else 0.0
    )
    weak_rebuild_penalty = 0.18 if metrics["path_efficiency_rebuild"] < 0.06 and terminal <= 0 else 0.0
    turn_density_penalty = 0.16 if metrics["turn_density_pressure"] < -0.08 and terminal <= 0 else 0.0
    adverse_excursion_penalty = (
        0.16 if metrics["directional_excursion_bias"] < -0.08 and terminal <= 0 else 0.0
    )
    weak_close_location_penalty = (
        0.14 if metrics["close_location_release"] < -0.08 and terminal <= 0 else 0.0
    )
    volume_penalty = 0.14 if metrics["volume_followthrough"] < -0.12 and terminal <= 0 else 0.0
    ...
    path_tortuosity_release_path_quality = (0.44 * terminal + 0.36 * mfe - 0.72 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + path_tortuosity_release_path_quality
        + short_reward
        + medium_reward
        + efficiency_reward
        + turn_reward
        + overlap_reward
        + excursion_reward
        + close_location_reward
        + volume_reward
        + prior_tortuosity_reward
        - renewed_tortuosity_penalty
        - weak_rebuild_penalty
        - turn_density_penalty
        - adverse_excursion_penalty
        - weak_close_location_penalty
        - volume_penalty
        - stall_penalty
...
def fit_linear_path_tortuosity_release_model(candidates: list[base.Candidate], fold_id: str) -> LinearPathTortuosityReleaseModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearPathTortuosityReleaseModel(
            fold_id,
    ...
    path_tortuosity_release_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearPathTortuosityReleaseModel(
        fold_id=fold_id,
        ...
        path_tortuosity_release_failure_rate=path_tortuosity_release_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearPathTortuosityReleaseModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0071-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_path_tortuosity_release_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0071_r0001_intraday_path_tortuosity_release",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0071_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0071_r0001_ptr_summary.json",
        ],
        ...
        "conditional_profiles": {
            "path_tortuosity_release_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0071 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0071_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0071 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "path_tortuosity_release_lookback_bars": PATH_TORTUOSITY_RELEASE_LOOKBACK_BARS,
        "path_tortuosity_release_short_lookback_bars": PATH_TORTUOSITY_RELEASE_SHORT_LOOKBACK_BARS,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_path_tortuosity_release_model_summary(model: LinearPathTortuosityReleaseModel) -> dict[str, object]:
    return {
        ...
        "path_tortuosity_release_failure_rate": rounded(model.path_tortuosity_release_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearPathTortuosityReleaseModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_path_tortuosity_release_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "path_tortuosity_release_profile": profiles["path_tortuosity_release_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0071_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0071_r0001_proxy_trades.csv"
    evidence["path_tortuosity_release_summary"] = "artifacts/c0071_r0001_ptr_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0071_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0071_r0001_proxy_trades.csv",
        "artifacts/c0071_r0001_ptr_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0071_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0071 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0071_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0071_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0071_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0071_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0071_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0071_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0071_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0071_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0071_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0071 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0071 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260706_produce_c0071_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0071_r0001_proxy_evidence
  refines:
  - dec_20260706_open_c0071_r0001_intraday_path_tortuosity_release_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0071_r0001_proxy_records_path_tortuosity_release_reference_surface
  ...
  - next_work_is_produce_c0071_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260706_produce_c0071_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0071-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0071_r0001_intraday_path_tortuosity_release",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0071-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0071-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0071-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0071_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0071_r0001_intraday_path_tortuosity_release",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0071-R0001-path-tortuosity-release-SUMMARY",
            "artifact_role": "path_tortuosity_release_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0071_r0001_ptr_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0071_r0001_intraday_path_tortuosity_release",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0071_r0001_mt5_logic_parity_evidence",
        }
    ]
