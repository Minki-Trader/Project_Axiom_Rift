"""C0128 R0001 proxy evidence for intraday close location torque reversal."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0128_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0128_r0001_close_location_torque_reversal_summary.json"
...
FEATURE_NAMES = (
    "adverse_displacement_extension",
    "range_displacement_misfire",
    "close_location_torque",
    "close_location_torque_shift",
    "counter_tail_torque",
    "vwap_centroid_tether",
    "session_centroid_pull",
    "range_energy_torque",
    "volume_effort_misfire",
    "micro_recenter_velocity",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_close_location_torque_reversal_rank"
LABEL_SHAPE = "target_first_quality_after_close_location_torque_reversal"
SELECTION_RULE = "top_fold_local_close_location_torque_reversal_scores_per_active_day"
...
MIN_DAY_BARS = 18
CLOSE_LOCATION_TORQUE_REVERSAL_LOOKBACK_BARS = 42
CLOSE_LOCATION_TORQUE_REVERSAL_MICRO_BARS = 10
MIN_ADVERSE_DISPLACEMENT_EXTENSION = 0.32
MIN_CLOSE_LOCATION_TORQUE = 0.01
MIN_CLOSE_LOCATION_TORQUE_REVERSAL_ACTIVITY = 0.05
...
class LinearCloseLocationTorqueReversalModel:
    fold_id: str
    ...
    close_location_torque_reversal_failure_rate: float
...
def run_c0128_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_close_location_torque_reversal_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_close_location_torque_reversal_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        CLOSE_LOCATION_TORQUE_REVERSAL_LOOKBACK_BARS,
        CLOSE_LOCATION_TORQUE_REVERSAL_MICRO_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = close_location_torque_reversal_metrics(
    ...
    features = (
        metrics["adverse_displacement_extension"],
        metrics["range_displacement_misfire"],
        metrics["close_location_torque"],
        metrics["close_location_torque_shift"],
        metrics["counter_tail_torque"],
        metrics["vwap_centroid_tether"],
        metrics["session_centroid_pull"],
        metrics["range_energy_torque"],
        metrics["volume_effort_misfire"],
        metrics["micro_recenter_velocity"],
        metrics["session_maturity"],
        metrics["spread_stress_inverse"],
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        breakout_pressure_bucket(metrics["adverse_displacement_extension"]),
        range_energy_excess_bucket(metrics["range_displacement_misfire"]),
        close_location_torque_bucket(metrics["close_location_torque"]),
        close_progression_erosion_bucket(metrics["close_location_torque_shift"]),
        counter_wick_pressure_bucket(metrics["counter_tail_torque"]),
        vwap_alignment_bucket(metrics["vwap_centroid_tether"]),
        range_position_pressure_bucket(metrics["session_centroid_pull"]),
        range_growth_efficiency_bucket(metrics["range_energy_torque"]),
        participation_decay_bucket(metrics["volume_effort_misfire"]),
        pullback_containment_bucket(metrics["micro_recenter_velocity"]),
...
def close_location_torque_reversal_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    micro_start = max(index - CLOSE_LOCATION_TORQUE_REVERSAL_MICRO_BARS + 1, context.day_start_index)
    lookback_start = max(index - CLOSE_LOCATION_TORQUE_REVERSAL_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    if session_range <= average_range * 0.55:
        return {"event_count": 0.0}

    session_mid = (context.high_so_far + context.low_so_far) / 2.0
    adverse_displacement_extension = -direction * (signal_bar.close - lookback_window[0].close) / max(
        average_range * 2.5,
        1e-9,
    )
    if adverse_displacement_extension < MIN_ADVERSE_DISPLACEMENT_EXTENSION:
        return {"event_count": 0.0}

    earlier_start = max(lookback_start, micro_start - CLOSE_LOCATION_TORQUE_REVERSAL_MICRO_BARS)
    earlier_window = bars[earlier_start:micro_start] or lookback_window[: len(micro_window)]

    def favorable_close_location(window_bar: base.Bar) -> float:
        width = max(window_bar.high - window_bar.low, 1e-9)
        if direction > 0:
            return (window_bar.close - window_bar.low) / width
        return (window_bar.high - window_bar.close) / width

    prior_close_location = base.mean([favorable_close_location(bar) for bar in earlier_window]) or 0.0
    micro_close_location = base.mean([favorable_close_location(bar) for bar in micro_window]) or 0.0
    signal_close_location = favorable_close_location(signal_bar)
    close_location_torque_shift = micro_close_location - prior_close_location
    close_location_torque = 0.55 * signal_close_location + 0.45 * micro_close_location
    if close_location_torque < MIN_CLOSE_LOCATION_TORQUE and close_location_torque_shift < 0.0:
        return {"event_count": 0.0}
    counter_tail_torque = base.mean(
        [adverse_wick_absorption(bar, direction) for bar in micro_window]
    ) or 0.0
    micro_displacement = abs(signal_bar.close - micro_window[0].close) / max(average_range * 1.2, 1e-9)
    range_displacement_misfire = micro_range_mean / max(average_range, 1e-9) - micro_displacement
    range_energy_torque = (micro_range_mean - lookback_range_mean) / max(average_range, 1e-9)
    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    session_centroid = (session_mid + session_vwap) / 2.0
    vwap_centroid_tether = direction * (session_vwap - signal_bar.close) / max(
        average_range * 3.0,
        1e-9,
    )
    session_centroid_pull = direction * (session_centroid - signal_bar.close) / max(
        average_range * 3.0,
        1e-9,
    )
    prior_day_closure_pressure = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_day_closure_pressure = -direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
    if context.prior_day_shadow_balance is not None:
        prior_day_closure_pressure += direction * context.prior_day_shadow_balance * 0.08
    path_travel = sum(abs(micro_window[pos].close - micro_window[pos - 1].close) for pos in range(1, len(micro_window)))
    micro_recenter_velocity = direction * (
        signal_bar.close - micro_window[0].close
    ) / max(path_travel, average_range, 1e-9)
    volume_effort_misfire = micro_volume_mean / max(lookback_volume_mean, average_volume, 1e-9) - max(
        micro_displacement,
        0.0,
    )
    ...
    activity = (
        max(adverse_displacement_extension, 0.0) * 0.15
        + max(range_displacement_misfire, 0.0) * 0.12
        + max(close_location_torque, 0.0) * 0.15
        + max(close_location_torque_shift, 0.0) * 0.12
        + counter_tail_torque * 0.12
        + max(vwap_centroid_tether, 0.0) * 0.10
        + max(session_centroid_pull, 0.0) * 0.09
        + max(range_energy_torque, 0.0) * 0.06
        + max(volume_effort_misfire, 0.0) * 0.05
        + max(micro_recenter_velocity, 0.0) * 0.04
    )
    if activity < MIN_CLOSE_LOCATION_TORQUE_REVERSAL_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "adverse_displacement_extension": bounded(adverse_displacement_extension, -4.0, 6.0),
        "range_displacement_misfire": bounded(range_displacement_misfire, -4.0, 6.0),
        "close_location_torque": bounded(close_location_torque, -1.0, 1.0),
        "close_location_torque_shift": bounded(close_location_torque_shift, -1.0, 1.0),
        "counter_tail_torque": bounded(counter_tail_torque, 0.0, 1.0),
        "vwap_centroid_tether": bounded(vwap_centroid_tether, -4.0, 4.0),
        "session_centroid_pull": bounded(session_centroid_pull, -4.0, 4.0),
        "prior_day_closure_pressure": bounded(prior_day_closure_pressure, -3.0, 3.0),
        "range_energy_torque": bounded(range_energy_torque, -4.0, 4.0),
        "volume_effort_misfire": bounded(volume_effort_misfire, -4.0, 4.0),
        "micro_recenter_velocity": bounded(micro_recenter_velocity, -2.0, 2.0),
        "range_stretch_pressure": bounded(adverse_displacement_extension, 0.0, 8.0),
        ...
        "session_mid": rounded(session_mid) or 0.0,
        "session_vwap": rounded(session_vwap) or 0.0,
        "session_centroid": rounded(session_centroid) or 0.0,
        "close_location_torque_reversal_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0128_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0128_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0128_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0128_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0128_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0128_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0128_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0128_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0128_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0128_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0128_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0128_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0128_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0128_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0128_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0128_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0128_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0128_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0128_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def c0128_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def close_location_torque_gap_bucket(value: float) -> str:
    if value < -0.08:
        return "close_location_torque_confirming"
    if value < 0.16:
        return "close_location_torque_mixed"
    return "close_location_torque_diverging"


def close_location_torque_bucket(value: float) -> str:
    if value < 0.28:
        return "close_torque_weak"
    if value < 0.58:
        return "close_torque_mixed"
    return "close_torque_defined"
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    extension_reward = 0.10 if metrics["adverse_displacement_extension"] > 0.60 and terminal > 0 else 0.0
    misfire_reward = 0.10 if metrics["range_displacement_misfire"] > 0.20 and terminal > 0 else 0.0
    torque_reward = 0.15 if metrics["close_location_torque"] > 0.55 and terminal > 0 else 0.0
    torque_shift_reward = 0.11 if metrics["close_location_torque_shift"] > 0.06 and terminal > 0 else 0.0
    tail_reward = 0.10 if metrics["counter_tail_torque"] > 0.20 and terminal > 0 else 0.0
    vwap_reward = 0.08 if metrics["vwap_centroid_tether"] > -0.05 and terminal > 0 else 0.0
    centroid_reward = 0.08 if metrics["session_centroid_pull"] > 0.02 and terminal > 0 else 0.0
    range_energy_reward = 0.05 if metrics["range_energy_torque"] > -0.05 and terminal > 0 else 0.0
    volume_misfire_reward = 0.06 if metrics["volume_effort_misfire"] > -0.15 and terminal > 0 else 0.0
    recenter_reward = 0.11 if metrics["micro_recenter_velocity"] > 0.02 and terminal > 0 else 0.0
    weak_extension_penalty = 0.10 if metrics["adverse_displacement_extension"] < 0.42 and terminal <= 0 else 0.0
    weak_misfire_penalty = 0.08 if metrics["range_displacement_misfire"] < -0.15 and terminal <= 0 else 0.0
    weak_torque_penalty = 0.12 if metrics["close_location_torque"] < 0.34 and terminal <= 0 else 0.0
    weak_shift_penalty = 0.08 if metrics["close_location_torque_shift"] < -0.08 and terminal <= 0 else 0.0
    weak_tail_penalty = 0.08 if metrics["counter_tail_torque"] < 0.10 and terminal <= 0 else 0.0
    weak_vwap_penalty = 0.06 if metrics["vwap_centroid_tether"] < -0.22 and terminal <= 0 else 0.0
    weak_recenter_penalty = 0.10 if metrics["micro_recenter_velocity"] < -0.08 and terminal <= 0 else 0.0
    excessive_volume_penalty = 0.05 if metrics["recent_volume_ratio"] > 4.25 and terminal <= 0 else 0.0
    ...
    return (
        ...
        + centroid_reward
        + range_energy_reward
        + volume_misfire_reward
        + recenter_reward
        - weak_extension_penalty
        - weak_misfire_penalty
        - weak_torque_penalty
        - weak_shift_penalty
        - weak_tail_penalty
        - weak_vwap_penalty
        - weak_recenter_penalty
        - excessive_volume_penalty
        - stall_penalty
        - unfavorable_path_penalty
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["range_stretch_pressure"] - 4.50) * 0.035
        - max(0.0, metrics["recent_volume_ratio"] - 4.20) * 0.015
        - max(0.0, MIN_ADVERSE_DISPLACEMENT_EXTENSION - metrics["adverse_displacement_extension"]) * 0.12
        - max(0.0, MIN_CLOSE_LOCATION_TORQUE - metrics["close_location_torque"]) * 0.12
    )


def fit_linear_close_location_torque_reversal_model(candidates: list[base.Candidate], fold_id: str) -> LinearCloseLocationTorqueReversalModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearCloseLocationTorqueReversalModel(
            fold_id,
    ...
    close_location_torque_reversal_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearCloseLocationTorqueReversalModel(
        fold_id=fold_id,
        ...
        close_location_torque_reversal_failure_rate=close_location_torque_reversal_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearCloseLocationTorqueReversalModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_close_location_torque_reversal_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0128_r0001_intraday_close_location_torque_reversal",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0128_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0128_r0001_close_location_torque_reversal_summary.json",
        ],
        ...
        "conditional_profiles": {
            "close_location_torque_reversal_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0128 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0128_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0128 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "close_location_torque_reversal_lookback_bars": CLOSE_LOCATION_TORQUE_REVERSAL_LOOKBACK_BARS,
        "close_location_torque_reversal_micro_bars": CLOSE_LOCATION_TORQUE_REVERSAL_MICRO_BARS,
        "min_adverse_displacement_extension": MIN_ADVERSE_DISPLACEMENT_EXTENSION,
        "min_close_location_torque": MIN_CLOSE_LOCATION_TORQUE,
        "min_close_location_torque_reversal_activity": MIN_CLOSE_LOCATION_TORQUE_REVERSAL_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_close_location_torque_reversal_model_summary(model: LinearCloseLocationTorqueReversalModel) -> dict[str, object]:
    return {
        ...
        "close_location_torque_reversal_failure_rate": rounded(model.close_location_torque_reversal_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearCloseLocationTorqueReversalModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_close_location_torque_reversal_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "close_location_torque_reversal_profile": profiles["close_location_torque_reversal_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0128_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0128_r0001_proxy_trades.csv"
    evidence["close_location_torque_reversal_summary"] = "artifacts/c0128_r0001_close_location_torque_reversal_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0128_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0128_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0128 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0128_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0128_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0128_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0128_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0128_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0128_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0128_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0128_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0128_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0128 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0128 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    block = f"""
- decision_id: dec_20260708_produce_c0128_r0001_proxy_evidence
  created_local_date: '{local_date}'
  status: active
  decision: produce_c0128_r0001_proxy_evidence
  refines:
  - dec_20260708_open_c0128_r0001_intraday_close_location_torque_reversal_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0128_r0001_proxy_records_close_location_torque_reversal_reference_surface
  ...
  - next_work_is_produce_c0128_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260708_produce_c0128_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0128-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0128_r0001_intraday_close_location_torque_reversal",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0128-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0128-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0128-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0128_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0128_r0001_intraday_close_location_torque_reversal",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0128-R0001-CLOSE-LOCATION-TORQUE-REVERSAL-SUMMARY",
            "artifact_role": "close_location_torque_reversal_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0128_r0001_close_location_torque_reversal_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0128_r0001_intraday_close_location_torque_reversal",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0128_r0001_mt5_logic_parity_evidence",
        }
    ]
