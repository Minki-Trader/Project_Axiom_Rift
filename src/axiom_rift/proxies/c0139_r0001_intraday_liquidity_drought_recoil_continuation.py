"""C0139 R0001 proxy evidence for intraday liquidity drought recoil continuation."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0139_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0139_r0001_liquidity_drought_recoil_continuation_summary.json"
...
FEATURE_NAMES = (
    "liquidity_drought",
    "recoil_impulse",
    "vwap_reclaim_alignment",
    "day_open_reclaim_alignment",
    "micro_pullback_containment",
    "range_quieting",
    "participation_reawakening",
    "adverse_tail_absorption",
    "session_mid_recoil",
    "prior_day_buffer",
    "session_room_after_recoil",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_liquidity_drought_recoil_continuation_rank"
LABEL_SHAPE = "target_first_quality_after_liquidity_drought_recoil_continuation"
SELECTION_RULE = "top_fold_local_liquidity_drought_recoil_continuation_scores_per_active_day"
...
MIN_DAY_BARS = 18
LIQUIDITY_DROUGHT_RECOIL_CONTINUATION_LOOKBACK_BARS = 72
LIQUIDITY_DROUGHT_RECOIL_CONTINUATION_MICRO_BARS = 16
MIN_LIQUIDITY_DROUGHT = -0.12
MIN_RECOIL_IMPULSE = -0.08
MIN_MICRO_PULLBACK_CONTAINMENT = 0.24
MIN_LIQUIDITY_DROUGHT_RECOIL_CONTINUATION_ACTIVITY = 0.02
...
class LinearLiquidityDroughtRecoilContinuationModel:
    fold_id: str
    ...
    liquidity_drought_recoil_continuation_failure_rate: float
...
def run_c0139_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_liquidity_drought_recoil_continuation_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_liquidity_drought_recoil_continuation_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        LIQUIDITY_DROUGHT_RECOIL_CONTINUATION_LOOKBACK_BARS,
        LIQUIDITY_DROUGHT_RECOIL_CONTINUATION_MICRO_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = liquidity_drought_recoil_continuation_metrics(
    ...
    features = (
        metrics["liquidity_drought"],
        metrics["recoil_impulse"],
        metrics["vwap_reclaim_alignment"],
        metrics["day_open_reclaim_alignment"],
        metrics["micro_pullback_containment"],
        metrics["range_quieting"],
        metrics["participation_reawakening"],
        metrics["adverse_tail_absorption"],
        metrics["session_mid_recoil"],
        metrics["prior_day_buffer"],
        metrics["session_room_after_recoil"],
        metrics["session_maturity"],
        metrics["spread_stress_inverse"],
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        participation_decay_bucket(metrics["liquidity_drought"]),
        reversion_impulse_bucket(metrics["recoil_impulse"]),
        vwap_reclaim_bucket(metrics["vwap_reclaim_alignment"]),
        day_open_tension_bucket(metrics["day_open_reclaim_alignment"]),
        pullback_containment_bucket(metrics["micro_pullback_containment"]),
        coil_compression_bucket(metrics["range_quieting"]),
        participation_followthrough_bucket(metrics["participation_reawakening"]),
        adverse_wick_control_bucket(metrics["adverse_tail_absorption"]),
        midpoint_distance_bucket(metrics["session_mid_recoil"]),
        prior_direction_context_bucket(metrics["prior_day_buffer"]),
        session_pressure_bucket(metrics["session_room_after_recoil"]),
...
def liquidity_drought_recoil_continuation_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    micro_start = max(index - LIQUIDITY_DROUGHT_RECOIL_CONTINUATION_MICRO_BARS + 1, context.day_start_index)
    lookback_start = max(index - LIQUIDITY_DROUGHT_RECOIL_CONTINUATION_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    if direction > 0:
        micro_pullback_containment = (signal_bar.close - micro_low) / micro_span
        session_room_after_recoil = (context.high_so_far - signal_bar.close) / max(average_range, 1e-9)
    else:
        micro_pullback_containment = (micro_high - signal_bar.close) / micro_span
        session_room_after_recoil = (signal_bar.close - context.low_so_far) / max(average_range, 1e-9)
    ...
    liquidity_drought = (lookback_volume_mean - micro_volume_mean) / max(average_volume, 1e-9)
    participation_reawakening = current_volume_ratio - micro_volume_ratio
    range_quieting = (lookback_range_mean - micro_range_mean) / max(average_range, 1e-9)
    recoil_impulse = direction * (signal_bar.close - previous_bar.close) / max(average_range, 1e-9)
    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    vwap_reclaim_alignment = direction * (signal_bar.close - session_vwap) / max(average_range, 1e-9)
    day_open_reclaim_alignment = direction * (signal_bar.close - context.day_open) / max(average_range, 1e-9)
    session_mid = (context.high_so_far + context.low_so_far) / 2.0
    session_mid_recoil = 1.0 - abs(signal_bar.close - session_mid) / max(session_range, 1e-9)
    adverse_tail_absorption = adverse_wick_absorption(signal_bar, direction)
    prior_day_buffer = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_day_buffer = direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
    if context.prior_day_shadow_balance is not None:
        prior_day_buffer -= direction * context.prior_day_shadow_balance * 0.03

    if liquidity_drought < MIN_LIQUIDITY_DROUGHT and participation_reawakening < -0.15:
        return {"event_count": 0.0}
    if recoil_impulse < MIN_RECOIL_IMPULSE and vwap_reclaim_alignment < -0.18:
        return {"event_count": 0.0}
    if micro_pullback_containment < MIN_MICRO_PULLBACK_CONTAINMENT and adverse_tail_absorption < 0.22:
        return {"event_count": 0.0}
    ...
    activity = (
        max(liquidity_drought, -0.30) * 0.15
        + max(recoil_impulse, -0.18) * 0.16
        + max(vwap_reclaim_alignment, -0.25) * 0.11
        + max(day_open_reclaim_alignment, -0.35) * 0.08
        + max(micro_pullback_containment - 0.35, -0.22) * 0.12
        + max(range_quieting, -0.20) * 0.08
        + max(participation_reawakening, -0.50) * 0.10
        + max(adverse_tail_absorption - 0.18, -0.18) * 0.08
        + max(session_mid_recoil - 0.25, -0.22) * 0.06
        + max(prior_day_buffer, -0.35) * 0.03
        + max(session_room_after_recoil, -0.20) * 0.03
    )
    if activity < MIN_LIQUIDITY_DROUGHT_RECOIL_CONTINUATION_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "liquidity_drought": bounded(liquidity_drought, -4.0, 6.0),
        "recoil_impulse": bounded(recoil_impulse, -4.0, 4.0),
        "vwap_reclaim_alignment": bounded(vwap_reclaim_alignment, -5.0, 5.0),
        "day_open_reclaim_alignment": bounded(day_open_reclaim_alignment, -5.0, 5.0),
        "micro_pullback_containment": bounded(micro_pullback_containment, 0.0, 1.0),
        "range_quieting": bounded(range_quieting, -3.0, 6.0),
        "participation_reawakening": bounded(participation_reawakening, -6.0, 8.0),
        "adverse_tail_absorption": bounded(adverse_tail_absorption, 0.0, 1.0),
        "session_mid_recoil": bounded(session_mid_recoil, -1.0, 1.0),
        "prior_day_buffer": bounded(prior_day_buffer, -3.0, 3.0),
        "session_room_after_recoil": bounded(session_room_after_recoil, -3.0, 8.0),
        ...
        "liquidity_drought_recoil_continuation_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def C0139_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def C0139_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def C0139_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def C0139_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def C0139_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def C0139_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def C0139_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def C0139_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def C0139_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def C0139_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def C0139_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def C0139_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def C0139_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def C0139_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def C0139_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def C0139_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def C0139_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def C0139_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def C0139_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def C0139_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    drought_reward = 0.09 if metrics["liquidity_drought"] > 0.10 and terminal > 0 else 0.0
    recoil_reward = 0.13 if metrics["recoil_impulse"] > 0.04 and terminal > 0 else 0.0
    vwap_reward = 0.11 if metrics["vwap_reclaim_alignment"] > -0.04 and terminal > 0 else 0.0
    open_reward = 0.06 if metrics["day_open_reclaim_alignment"] > -0.12 and terminal > 0 else 0.0
    containment_reward = 0.10 if metrics["micro_pullback_containment"] > 0.52 and terminal > 0 else 0.0
    quieting_reward = 0.06 if metrics["range_quieting"] > -0.05 and terminal > 0 else 0.0
    reawakening_reward = 0.08 if metrics["participation_reawakening"] > -0.12 and terminal > 0 else 0.0
    tail_reward = 0.08 if metrics["adverse_tail_absorption"] > 0.42 and terminal > 0 else 0.0
    midpoint_reward = 0.05 if metrics["session_mid_recoil"] > 0.20 and terminal > 0 else 0.0
    room_reward = 0.04 if metrics["session_room_after_recoil"] > 0.12 and terminal > 0 else 0.0
    prior_day_reward = 0.04 if metrics["prior_day_buffer"] > -0.30 and terminal > 0 else 0.0
    failed_drought_penalty = 0.06 if metrics["liquidity_drought"] < -0.18 and terminal <= 0 else 0.0
    weak_recoil_penalty = 0.10 if metrics["recoil_impulse"] < -0.05 and terminal <= 0 else 0.0
    failed_vwap_penalty = 0.08 if metrics["vwap_reclaim_alignment"] < -0.28 and terminal <= 0 else 0.0
    weak_containment_penalty = 0.08 if metrics["micro_pullback_containment"] < 0.34 and terminal <= 0 else 0.0
    noisy_range_penalty = 0.06 if metrics["range_quieting"] < -0.24 and terminal <= 0 else 0.0
    no_reawakening_penalty = 0.05 if metrics["participation_reawakening"] < -0.55 and terminal <= 0 else 0.0
    no_room_penalty = 0.05 if metrics["session_room_after_recoil"] < 0.04 and terminal <= 0 else 0.0
    ...
    return (
        ...
        + room_reward
        + prior_day_reward
        - failed_drought_penalty
        - weak_recoil_penalty
        - failed_vwap_penalty
        - weak_containment_penalty
        - noisy_range_penalty
        - no_reawakening_penalty
        - no_room_penalty
        - stall_penalty
        ...
        - max(0.0, -metrics["participation_reawakening"] - 0.75) * 0.015
        - max(0.0, MIN_LIQUIDITY_DROUGHT - metrics["liquidity_drought"]) * 0.04
        - max(0.0, MIN_RECOIL_IMPULSE - metrics["recoil_impulse"]) * 0.08
    )


def fit_linear_liquidity_drought_recoil_continuation_model(candidates: list[base.Candidate], fold_id: str) -> LinearLiquidityDroughtRecoilContinuationModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearLiquidityDroughtRecoilContinuationModel(
            fold_id,
    ...
    liquidity_drought_recoil_continuation_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearLiquidityDroughtRecoilContinuationModel(
        fold_id=fold_id,
        ...
        liquidity_drought_recoil_continuation_failure_rate=liquidity_drought_recoil_continuation_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearLiquidityDroughtRecoilContinuationModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_liquidity_drought_recoil_continuation_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0139_r0001_intraday_liquidity_drought_recoil_continuation",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0139_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0139_r0001_liquidity_drought_recoil_continuation_summary.json",
        ],
        ...
        "conditional_profiles": {
            "liquidity_drought_recoil_continuation_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0139 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0139_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0139 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "liquidity_drought_recoil_continuation_lookback_bars": LIQUIDITY_DROUGHT_RECOIL_CONTINUATION_LOOKBACK_BARS,
        "liquidity_drought_recoil_continuation_micro_bars": LIQUIDITY_DROUGHT_RECOIL_CONTINUATION_MICRO_BARS,
        "min_liquidity_drought": MIN_LIQUIDITY_DROUGHT,
        "min_recoil_impulse": MIN_RECOIL_IMPULSE,
        "min_micro_pullback_containment": MIN_MICRO_PULLBACK_CONTAINMENT,
        "min_liquidity_drought_recoil_continuation_activity": MIN_LIQUIDITY_DROUGHT_RECOIL_CONTINUATION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_liquidity_drought_recoil_continuation_model_summary(model: LinearLiquidityDroughtRecoilContinuationModel) -> dict[str, object]:
    return {
        ...
        "liquidity_drought_recoil_continuation_failure_rate": rounded(model.liquidity_drought_recoil_continuation_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearLiquidityDroughtRecoilContinuationModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_liquidity_drought_recoil_continuation_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "liquidity_drought_recoil_continuation_profile": profiles["liquidity_drought_recoil_continuation_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["next_required_action"] = "produce_c0139_r0001_mt5_logic_parity_evidence"
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0139_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0139_r0001_proxy_trades.csv"
    evidence["liquidity_drought_recoil_continuation_summary"] = "artifacts/c0139_r0001_liquidity_drought_recoil_continuation_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0139_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0139_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0139 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0139_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    next_work["tasks"] = ["produce_c0139_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0139_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0139_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0139_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0139_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0139_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0139_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0139 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0139 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    block = f"""
- decision_id: dec_20260708_produce_c0139_r0001_proxy_evidence
  created_local_date: '{local_date}'
  status: active
  decision: produce_c0139_r0001_proxy_evidence
  refines:
  - dec_20260708_open_c0139_r0001_intraday_liquidity_drought_recoil_continuation_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0139_r0001_proxy_records_liquidity_drought_recoil_continuation_reference_surface
  ...
  - next_work_is_produce_c0139_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260708_produce_c0139_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0139-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0139_r0001_intraday_liquidity_drought_recoil_continuation",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0139-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0139-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0139-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0139_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0139_r0001_intraday_liquidity_drought_recoil_continuation",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0139-R0001-LIQUIDITY-DROUGHT-RECOIL-CONTINUATION-SUMMARY",
            "artifact_role": "liquidity_drought_recoil_continuation_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0139_r0001_liquidity_drought_recoil_continuation_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0139_r0001_intraday_liquidity_drought_recoil_continuation",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0139_r0001_mt5_logic_parity_evidence",
        }
    ]
