"""C0028 R0001 proxy evidence for intraday swing leg maturity discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0028_intraday_swing_leg_maturity_discovery" / "runs" / "R0001"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0028_intraday_swing_leg_maturity_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0028_r0001_proxy_trades.csv"
INTRADAY_SWING_LEG_MATURITY_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0028_r0001_intraday_swing_leg_maturity_summary.json"
...
FEATURE_NAMES = (
    "direction_bias",
    "leg_direction_alignment_24",
    "swing_leg_age_24",
    "swing_leg_extreme_age_24",
    "swing_leg_distance_atr",
    "swing_leg_current_distance_atr",
    "swing_leg_efficiency",
    "swing_leg_range_capture",
    "swing_leg_close_retention",
    "swing_leg_pullback_fraction",
    "swing_leg_extension_maturity",
    "swing_leg_pullback_bars_12",
    "swing_leg_new_extreme_rate_12",
    "swing_leg_failed_extension_rate_6",
    "directional_extension_distance_3",
    "directional_extension_distance_6",
    "directional_extension_distance_12",
    "directional_box_position_24",
    ...
    "rejection_wick_at_extreme_1",
    "rejection_wick_mean_3",
    "follow_wick_mean_3",
    "wick_asymmetry_3",
    "range_tier_1",
    "range_tier_mean_6",
    "range_contraction_after_leg_3",
    "inside_rate_6",
    "average_overlap_ratio_6",
    "churn_ratio_12",
    "directional_path_efficiency_12",
    "counter_move_pressure_6",
    "breakout_pressure_12",
    "failed_break_pressure_12",
...
MODEL_FAMILY = "fold_local_intraday_swing_leg_maturity_rank"
LABEL_SHAPE = "directional_intraday_swing_leg_maturity_survival_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_intraday_swing_leg_maturity_scores_per_active_day"
...
def run_c0028_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = intraday_swing_leg_maturity_features(
                bars,
                range_average,
                short_range_average,
                day_context,
            ...
            if features is None:
                continue
            label = (
                intraday_swing_leg_maturity_label(bars, range_average, index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|intraday_swing_leg_maturity",
                    features=features,
                    label=label,
                )
            )
...
def intraday_swing_leg_maturity_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    if spread_over_range > 0.65:
        return None
    leg = swing_leg_state(bars, index, direction, 24, average_range)
    if leg is None:
        return None
    ...
    return (
        float(direction),
        direction * (bars[index].close - bars[index - 24].close) / average_range,
        leg["age"],
        leg["extreme_age"],
        leg["distance_atr"],
        leg["current_distance_atr"],
        leg["efficiency"],
        leg["range_capture"],
        leg["close_retention"],
        leg["pullback_fraction"],
        leg["extension_maturity"],
        directional_pullback_bars(bars, index, direction, 12),
        directional_new_extreme_rate(bars, index, direction, 12),
        directional_failed_extension_rate(bars, index, direction, 6),
        ...
        directional_box_position(bars, index, direction, 24),
        ...
        range_contraction_after_leg(bars, index, average_range),
        ...
        directional_breakout_pressure(bars, index, direction, 12, average_range),
        directional_failed_break_pressure(bars, index, direction, 12),
...
def swing_leg_state(
    ...
    if start < 1 or average_range <= 0:
        return None
    indices = list(range(start, index + 1))
    if direction > 0:
        origin_index = min(indices, key=lambda offset: bars[offset].low)
        leg_indices = list(range(origin_index, index + 1))
        extreme_index = max(leg_indices, key=lambda offset: bars[offset].high)
        origin_price = bars[origin_index].low
        extreme_price = bars[extreme_index].high
        close_distance = bars[index].close - origin_price
        pullback_distance = max(0.0, extreme_price - bars[index].close)
        box_low = min(bars[offset].low for offset in indices)
        box_high = max(bars[offset].high for offset in indices)
    else:
        origin_index = max(indices, key=lambda offset: bars[offset].high)
        leg_indices = list(range(origin_index, index + 1))
        extreme_index = min(leg_indices, key=lambda offset: bars[offset].low)
        origin_price = bars[origin_index].high
        extreme_price = bars[extreme_index].low
        close_distance = origin_price - bars[index].close
        pullback_distance = max(0.0, bars[index].close - extreme_price)
        box_low = min(bars[offset].low for offset in indices)
        box_high = max(bars[offset].high for offset in indices)
    leg_span = abs(extreme_price - origin_price)
    if leg_span <= 0:
        return None
    path_distance = sum(abs(bars[offset].close - bars[offset - 1].close) for offset in range(origin_index + 1, index + 1))
    return {
        "age": bounded((index - origin_index) / lookback, 0.0, 1.5),
        "extreme_age": bounded((index - extreme_index) / lookback, 0.0, 1.5),
        "distance_atr": leg_span / average_range,
        "current_distance_atr": close_distance / average_range,
        "efficiency": abs(bars[index].close - bars[origin_index].close) / max(path_distance, 1e-9),
        "range_capture": leg_span / max(box_high - box_low, 1e-9),
        "close_retention": bounded(close_distance / max(leg_span, 1e-9), -0.5, 1.5),
        "pullback_fraction": bounded(pullback_distance / max(leg_span, 1e-9), 0.0, 1.5),
        "extension_maturity": bounded((index - extreme_index) / 12.0, 0.0, 1.5),
    }
...
def intraday_swing_leg_maturity_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    index: int,
    direction: int,
    ...
    future_extension_quality = future_directional_extension_quality(path, entry, direction, average_range)
    future_pullback_penalty = future_directional_pullback_penalty(path, entry, direction, average_range)
    future_leg_retention = future_directional_leg_retention(path, entry, direction)
    future_failed_extension_penalty = future_directional_failed_break_penalty(bars, entry_index, exit_index, direction)
    return (
        0.82 * first_hit
        + 0.34 * target_speed
        - 0.46 * stop_speed
        ...
        + 0.28 * future_extension_quality
        - 0.34 * future_pullback_penalty
        + 0.24 * future_leg_retention
        - 0.30 * future_failed_extension_penalty
        - 1.08 * spread_norm
        - 0.20 * spread_drift
    )


def future_directional_extension_quality(
    path: list[base.Bar],
    entry: float,
    direction: int,
    average_range: float,
...
def future_directional_pullback_penalty(
    path: list[base.Bar],
    entry: float,
    direction: int,
    average_range: float,
...
def future_directional_leg_retention(path: list[base.Bar], entry: float, direction: int) -> float:
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0028"
    payload["work_unit_id"] = "C0028"
    payload["run_id"] = "R0001"
    payload["proxy_id"] = "PX-C0028-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0028_r0001_intraday_swing_leg_maturity"
    payload["proxy_config_path"] = "campaigns/C0028_intraday_swing_leg_maturity_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_intraday_swing_leg_maturity_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0028_intraday_swing_leg_maturity_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0028_intraday_swing_leg_maturity_discovery/runs/R0001/artifacts/c0028_r0001_proxy_trades.csv",
        "campaigns/C0028_intraday_swing_leg_maturity_discovery/runs/R0001/artifacts/c0028_r0001_intraday_swing_leg_maturity_summary.json",
    ]
    ...
    profiles["intraday_swing_leg_maturity_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0028_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "feature_weight_floor": FEATURE_WEIGHT_FLOOR,
            "feature_weight_ceiling": FEATURE_WEIGHT_CEILING,
            "score_interpretation": "higher_score_means_direct_fold_local_intraday_swing_leg_maturity_rank",
            "variant_boundary": (
                "intraday_swing_leg_maturity_rank_not_range_overlap_calendar_phase_tick_participation_"
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_intraday_swing_leg_maturity_summary_artifact(payload, INTRADAY_SWING_LEG_MATURITY_SUMMARY_PATH)
    PROXY_PATH.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="ascii")
    trade_hash = base.sha256_file(TRADE_ARTIFACT_PATH)
    summary_hash = base.sha256_file(INTRADAY_SWING_LEG_MATURITY_SUMMARY_PATH)
...
def write_intraday_swing_leg_maturity_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_intraday_swing_leg_maturity_summary_v1",
        "template": False,
        "work_unit_id": "C0028",
        "campaign_id": "C0028",
        "run_id": "R0001",
        ...
        "intraday_swing_leg_maturity_profile": profiles["intraday_swing_leg_maturity_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "intraday_swing_leg_maturity_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0028-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0028_intraday_swing_leg_maturity_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0028_intraday_swing_leg_maturity_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0028-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0028-R0001-INTRADAY-SWING-LEG-MATURITY-SUMMARY",
                "intraday_swing_leg_maturity_summary_artifact",
                "json",
                "campaigns/C0028_intraday_swing_leg_maturity_discovery/runs/R0001/artifacts/c0028_r0001_intraday_swing_leg_maturity_summary.json",
                summary_hash,
                ["campaigns/C0028_intraday_swing_leg_maturity_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0028_r0001_intraday_swing_leg_maturity",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0028_r0001_proxy_trades.csv"
    evidence["intraday_swing_leg_maturity_summary"] = "artifacts/c0028_r0001_intraday_swing_leg_maturity_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0028_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0028_r0001_proxy_trades.csv",
        "artifacts/c0028_r0001_intraday_swing_leg_maturity_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0028 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0028_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0028_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0028_intraday_swing_leg_maturity_discovery"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0028_intraday_swing_leg_maturity_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0028_intraday_swing_leg_maturity_discovery",
        "open_c0028_r0001_fold_local_intraday_swing_leg_maturity_rank_run",
        "produce_c0028_r0001_proxy_evidence",
    ):
    ...
    data["active_run"] = "campaigns/C0028_intraday_swing_leg_maturity_discovery/runs/R0001"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["active_campaign"] = "campaigns/C0028_intraday_swing_leg_maturity_discovery"
    data["active_run"] = "campaigns/C0028_intraday_swing_leg_maturity_discovery/runs/R0001"
    data["latest_operation"] = {
        "id": "produce_c0028_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0028_intraday_swing_leg_maturity_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0028_r0001_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0028",
            "c0004_r0001_fold_local_state_archetype": "c0028_r0001_intraday_swing_leg_maturity",
            "c0004_r0001": "c0028_r0001",
            "fold_local_state_archetype_discovery": "intraday_swing_leg_maturity_discovery",
            ...
            "state_archetype_summary": "intraday_swing_leg_maturity_summary",
        }
