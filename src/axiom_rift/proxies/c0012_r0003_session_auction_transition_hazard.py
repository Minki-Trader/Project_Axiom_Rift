"""C0012 R0003 proxy evidence for session auction transition hazard discovery."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0012_r0003_proxy_trades.csv"
SUMMARY_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0012_r0003_session_auction_transition_hazard_summary.json"
...
FEATURE_NAMES = (
    ...
    "event_transition_alignment",
    "event_persistence",
    "value_mid_slope_over_range",
    "opening_range_migration",
    "rotation_acceleration",
    "prior_event_pressure",
)
MODEL_FAMILY = "fold_local_session_auction_transition_hazard_rank"
LABEL_SHAPE = "target_first_auction_transition_resolution_penalized_by_adverse_persistence"
MIN_SESSION_BARS = 12
...
class SessionSnapshot:
    ...
    prior_event_pressure: float


def run_c0012_r0003_proxy(write: bool = True) -> dict[str, object]:
...
def build_context(bars: list[base.Bar]) -> dict[str, list[Any]]:
    ...
    value_mid_slope: list[float] = [0.0] * len(bars)
    opening_mid: list[float | None] = [None] * len(bars)
    rotation_count: list[int] = [0] * len(bars)
    rotation_delta: list[int] = [0] * len(bars)
    last_day = ""
    day_open = 0.0
    day_high = 0.0
    day_low = 0.0
    ...
    for index, bar in enumerate(bars):
        ...
        opening_mid[index] = 0.5 * (open_high + open_low)
        if count > OPENING_RANGE_BARS and index >= OPENING_RANGE_BARS:
            prior_mid = value_mid[index - OPENING_RANGE_BARS]
            if prior_mid is not None:
                value_mid_slope[index] = mid - float(prior_mid)
            rotation_delta[index] = rotations - rotation_count[index - OPENING_RANGE_BARS]
    ...
    return {
        ...
        "value_mid_slope": value_mid_slope,
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            ...
            if snapshot is None:
                continue
            label = transition_hazard_label(bars, context, index, direction, snapshot) if include_labels else None
            candidates.append(
                base.Candidate(
                    fold_id=fold_id,
                    index=index,
...
def session_auction_snapshot(
    ...
    opening_mid = context["opening_mid"][index]
    ...
    prior_event_pressure = direction * (bar.close - previous.close) / float(range_48)
    value_mid_slope_over_range = direction * float(context["value_mid_slope"][index]) / float(range_48)
    opening_range_migration = direction * (bar.close - float(opening_mid)) / float(range_48)
    rotation_acceleration = clamp(float(context["rotation_delta"][index]) / 3.0, -2.0, 2.0)
    event_transition_alignment = (
        edge_rejection
        + edge_acceptance
        - failed_return
        + 0.25 * prior_event_pressure
        + 0.12 * value_mid_slope_over_range
    )
    event_persistence = clamp(
        0.50 * max(0.0, prior_event_pressure)
        + 0.35 * max(0.0, previous_acceptance)
        + 0.15 * abs(rotation_acceleration),
        0.0,
        2.0,
    )
    transition_score = (
        event_transition_alignment
        + 0.25 * opening_range_migration
        - 0.12 * spread_over_range
        - 0.08 * abs(rotation_acceleration)
    )
    event = auction_event(
        edge_rejection,
        edge_acceptance,
    ...
    features = (
        ...
        session_progress,
        body_fraction,
        clamp(event_transition_alignment, -3.0, 3.0),
        event_persistence,
        clamp(value_mid_slope_over_range, -3.0, 3.0),
        clamp(opening_range_migration, -5.0, 5.0),
        rotation_acceleration,
        clamp(prior_event_pressure, -3.0, 3.0),
    )
    return SessionSnapshot(
        features=features,
        state_key=auction_state_key(
            direction,
            event,
            directional_value_location,
            rotation_norm,
        ...
        prior_event_pressure=prior_event_pressure,
    )
...
def transition_hazard_label(
    ...
    favorable_close_count = 0
    adverse_close_count = 0
    adverse_streak = 0
    ...
    if direction > 0:
        ...
        for offset, bar in enumerate(path, start=1):
            favorable_close = bar.close >= entry
            favorable_close_count += 1 if favorable_close else 0
            adverse_close_count += 0 if favorable_close else 1
            adverse_move = entry - bar.low
            adverse_streak = adverse_streak + 1 if adverse_move >= 0.20 * average_range or not favorable_close else 0
            ...
            if first_adverse_bar == len(path) and adverse_move >= 0.30 * average_range:
    ...
    favorable_delay = first_favorable_bar / len(path) if first_favorable_bar <= len(path) else 1.0
    adverse_first_penalty = 1.0 if first_adverse_bar < first_favorable_bar else 0.0
    path_alignment = favorable_close_count / len(path)
    adverse_persistence = max_adverse_streak / len(path)
    adverse_close_pressure = adverse_close_count / len(path)
    spread_norm = bars[entry_index].spread_points / average_range
    event_bonus = {
        "edge_rejection": 0.10,
        "edge_acceptance": 0.06,
        "inventory_pressure": 0.02,
        "failed_value_return": -0.04,
        "value_rotation": -0.02,
    }.get(snapshot.auction_event, 0.0)
    return (
        1.16 * first_hit
        + 0.46 * target_speed
        - 0.58 * stop_speed
        + 0.25 * favorable
        + 0.22 * terminal_norm
        + 0.18 * (path_alignment - 0.5)
        + 0.16 * snapshot.transition_score
        + 0.10 * snapshot.event_persistence
        + event_bonus
        - 0.72 * adverse
        - 0.50 * adverse_persistence
        - 0.22 * adverse_close_pressure
        - 0.44 * adverse_first_penalty
        - 0.30 * favorable_delay
        - 0.68 * spread_norm
    )
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0012-R0003",
        "dataset_identity": "us100_m5_base_frame_sha256_80938f8f37fa0ef4fff7c2b5adfe45186153bc0bbc54d7544fec3d44e2c34ee3",
        "split_policy": "rolling_window_test_oos_only_with_train_is_session_auction_transition_hazard_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0012_r0003_session_auction_transition_hazard",
        "proxy_config_path": "campaigns/C0012_session_auction_rotation_discovery/runs/R0003/run_manifest.json",
        "proxy_code_version_or_commit": "uncommitted_local_worktree_at_proxy_creation",
        "proxy_artifact_paths": [
            "campaigns/C0012_session_auction_rotation_discovery/runs/R0003/kpi/proxy.json",
            "campaigns/C0012_session_auction_rotation_discovery/runs/R0003/artifacts/c0012_r0003_proxy_trades.csv",
            "campaigns/C0012_session_auction_rotation_discovery/runs/R0003/artifacts/c0012_r0003_session_auction_transition_hazard_summary.json"
        ],
        ...
        "conditional_profiles": {
            "session_auction_transition_hazard_profile": {
                "applies": True,
                "fields": {
                    "state_fit_scope": "rolling_train_is_only",
                    "feature_count": len(FEATURE_NAMES),
                    "feature_names": list(FEATURE_NAMES),
                    "candidate_direction": "dual_direction_long_and_short_session_auction_transition_events",
                    "label_horizon_bars": LABEL_HORIZON_BARS,
                    "label_shape": LABEL_SHAPE,
                    "entry_count_per_active_day_target": "5_to_10",
                    "entries_per_active_day": base.rounded(entries_per_active_day),
                    "max_entries_per_active_day": base.MAX_ENTRIES_PER_ACTIVE_DAY,
                    "selection_rule": "top_fold_local_session_auction_transition_hazard_scores_per_active_day",
        ...
        "deferred_with_reason": [
            {
                ...
                "revisit_when": "during C0012 R0003 MT5 evidence production",
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_session_auction_transition_hazard_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "campaign_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        ...
        "session_auction_transition_hazard_profile": profiles["session_auction_transition_hazard_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        not in {"proxy_kpi", "proxy_trade_artifact", "session_auction_transition_hazard_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0012-R0003-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0012_session_auction_rotation_discovery/runs/R0003/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0012_session_auction_rotation_discovery/runs/R0003/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0012-R0003-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0012-R0003-SESSION-AUCTION-TRANSITION-HAZARD-SUMMARY",
                "session_auction_transition_hazard_summary_artifact",
                "json",
                "campaigns/C0012_session_auction_rotation_discovery/runs/R0003/artifacts/c0012_r0003_session_auction_transition_hazard_summary.json",
                summary_hash,
                ["campaigns/C0012_session_auction_rotation_discovery/runs/R0003/kpi/proxy.json"],
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0012_r0003_mt5_logic_parity_evidence",
        }
    ]
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0012_r0003_session_auction_transition_hazard",
...
def update_run_manifest_after_proxy() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0012_r0003_proxy_trades.csv"
    evidence["session_auction_transition_hazard_summary"] = "artifacts/c0012_r0003_session_auction_transition_hazard_summary.json"
...
def update_gate_report_after_proxy() -> None:
    ...
    data["next_action"] = "produce_c0012_r0003_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0012_r0003_proxy_trades.csv",
        "artifacts/c0012_r0003_session_auction_transition_hazard_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0012 R0003 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_after_proxy() -> None:
    ...
    next_candidate["direction"] = "produce_c0012_r0003_mt5_logic_parity_evidence"
    ...
    closeout["remaining_question"] = "produce_c0012_r0003_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    for item in (
        "design_c0012_r0003_session_auction_transition_hazard_run",
        "open_c0012_r0003_session_auction_transition_hazard_run",
        "produce_c0012_r0003_proxy_evidence",
    ):
    ...
    data["active_run"] = "campaigns/C0012_session_auction_rotation_discovery/runs/R0003"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0012_r0003_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0012_session_auction_rotation_discovery/runs/R0003/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0012_r0003_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def update_decision_cursor_after_proxy() -> None:
    ...
    data["canonical_source"] = "campaigns/C0012_session_auction_rotation_discovery/runs/R0003/kpi/proxy.json"
    data["canonical_status"] = "proxy_recorded_pending_mt5"
    data["current_decision"] = "produce_c0012_r0003_mt5_logic_parity_evidence"
    data["active_run"] = "campaigns/C0012_session_auction_rotation_discovery/runs/R0003"
    data["next_required_action"] = "produce_c0012_r0003_mt5_logic_parity_evidence"
    current = data.setdefault("current_evidence_summary", {})
    current["source_run"] = "campaigns/C0012_session_auction_rotation_discovery/runs/R0003"
    current["current_task"] = "produce_c0012_r0003_mt5_logic_parity_evidence"
...
def linear_model_summary(model: LinearAuctionModel) -> dict[str, object]:
    return {
        ...
        "score_interpretation": "higher_score_means_fold_local_session_auction_transition_resolution_quality",
    }
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "candidate_direction": "dual_direction_session_auction_transition_events",
        "model_family": MODEL_FAMILY,
        "feature_names": list(FEATURE_NAMES),
        "label_shape": LABEL_SHAPE,
...
def auction_state_key(
    ...
    transition = bucket(transition_score, -0.15, 0.35, "adverse_transition", "mixed_transition", "aligned_transition")
    persistence = bucket(event_persistence, 0.08, 0.24, "low_persistence", "mid_persistence", "high_persistence")
    opening = bucket(opening_range_migration, -0.40, 0.40, "below_opening_mid", "near_opening_mid", "above_opening_mid")
    return f"{side}|event_{event}|{value}|{rotation}|{expansion}|{session}|{transition}|{persistence}|{opening}"
