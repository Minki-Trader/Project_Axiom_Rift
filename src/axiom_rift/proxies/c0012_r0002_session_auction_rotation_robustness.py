"""C0012 R0002 proxy evidence for session auction rotation robustness conditioning."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0012_r0002_proxy_trades.csv"
SUMMARY_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0012_r0002_session_auction_rotation_robustness_summary.json"
...
SOURCE_AUDIT_PATH = (
    "campaigns/C0012_session_auction_rotation_discovery/runs/R0001/kpi/candidate_robustness_audit.json"
)
...
CONDITIONER_FEATURE_NAMES = (
    "base_auction_score",
    "cost_pressure",
    "fragile_adverse_path_pressure",
    "edge_quality_balance",
    "balance_expansion_pressure",
    "absolute_inventory_pressure",
    "absolute_volatility_transition",
    "session_lateness",
    "rotation_intensity",
    "directional_body_alignment",
)
MODEL_FAMILY = "fold_local_session_auction_rotation_robustness_conditioner"
LABEL_SHAPE = "fold_local_robust_positive_minus_fragile_adverse_auction_rotation_quality"
SELECTION_RULE = "top_fold_local_conditioned_session_auction_rotation_scores_per_active_day"
CONDITIONER_WEIGHT = 0.42
MIN_SESSION_BARS = 12
...
def run_c0012_r0002_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        state_distributions[fold_id] = auction_distribution(scored_candidates, selected, fold_trades, conditioner)
...
def fit_robustness_conditioner(candidates: list[base.Candidate], fold_id: str) -> RobustnessConditioner:
    base_model = fit_linear_auction_model(candidates, fold_id)
    scored_train = score_linear_candidates(candidates, base_model)
    ...
    labels = np.asarray([robustness_adjusted_label(candidate) for candidate in labeled], dtype=float)
    base_scores = np.asarray([float(candidate.score or 0.0) for candidate in labeled], dtype=float)
    feature_mean = matrix.mean(axis=0)
    feature_std = matrix.std(axis=0)
    feature_std = np.where(feature_std < 1e-9, 1.0, feature_std)
    scaled = (matrix - feature_mean) / feature_std
    robust_threshold = float(np.quantile(labels, 0.72))
    fragile_threshold = float(np.quantile(labels, 0.28))
    ...
    feature_weights = np.clip(separation / max(float(separation.mean()), 1e-9), 0.20, 2.80)
    return RobustnessConditioner(
        fold_id=fold_id,
        base_model=base_model,
        feature_mean=feature_mean,
...
def robustness_adjusted_label(candidate: base.Candidate) -> float:
    ...
    cost_pressure = feature_value(candidate, "spread_over_range")
    failed_return = max(0.0, feature_value(candidate, "failed_value_return_pressure"))
    volatility_shock = abs(feature_value(candidate, "volatility_transition"))
    balance_expansion = max(0.0, feature_value(candidate, "balance_expansion") - 1.0)
    body_alignment = feature_value(candidate, "directional_body_fraction")
    return (
        label
        + 0.18 * base_score
        + 0.10 * body_alignment
...
def conditioner_features(candidate: base.Candidate) -> tuple[float, ...]:
    base_score = float(candidate.score or 0.0)
    cost_pressure = feature_value(candidate, "spread_over_range")
    edge_rejection = feature_value(candidate, "edge_rejection_strength")
    edge_acceptance = feature_value(candidate, "edge_acceptance_strength")
    failed_return = feature_value(candidate, "failed_value_return_pressure")
    balance_expansion = feature_value(candidate, "balance_expansion")
    inventory_pressure = feature_value(candidate, "inventory_pressure")
    volatility_transition = feature_value(candidate, "volatility_transition")
    session_progress = feature_value(candidate, "session_progress")
    rotation_norm = feature_value(candidate, "rotation_count_norm")
    body_alignment = feature_value(candidate, "directional_body_fraction")
    fragile_adverse_path = max(0.0, failed_return) + max(0.0, -body_alignment) + max(0.0, -base_score)
    edge_quality_balance = edge_rejection + 0.65 * edge_acceptance - 0.35 * fragile_adverse_path
    return (
        base_score,
        cost_pressure,
        fragile_adverse_path,
        edge_quality_balance,
        balance_expansion,
        abs(inventory_pressure),
        abs(volatility_transition),
        session_progress,
        rotation_norm,
        body_alignment,
    )
...
def feature_value(candidate: base.Candidate, name: str) -> float:
    ...
    return float(candidate.features[index])


def score_linear_candidates(candidates: list[base.Candidate], model: LinearAuctionModel) -> list[base.Candidate]:
...
def score_candidates(candidates: list[base.Candidate], conditioner: RobustnessConditioner) -> list[base.Candidate]:
    ...
    for index, candidate in enumerate(base_scored):
        ...
        score = (
            ...
            + 0.12 * (conditioner.robust_label_rate - conditioner.fragile_label_rate)
        )
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0012-R0002-ROBUSTNESS",
        "dataset_identity": "us100_m5_base_frame_sha256_80938f8f37fa0ef4fff7c2b5adfe45186153bc0bbc54d7544fec3d44e2c34ee3",
        "split_policy": "rolling_window_test_oos_only_with_train_is_session_auction_robustness_conditioner_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0012_r0002_session_auction_rotation_robustness",
        "proxy_config_path": "campaigns/C0012_session_auction_rotation_discovery/runs/R0002/run_manifest.json",
        "proxy_code_version_or_commit": "uncommitted_local_worktree_at_proxy_creation",
        "proxy_artifact_paths": [
            "campaigns/C0012_session_auction_rotation_discovery/runs/R0002/kpi/proxy.json",
            "campaigns/C0012_session_auction_rotation_discovery/runs/R0002/artifacts/c0012_r0002_proxy_trades.csv",
            "campaigns/C0012_session_auction_rotation_discovery/runs/R0002/artifacts/c0012_r0002_session_auction_rotation_robustness_summary.json"
        ],
        ...
        "conditional_profiles": {
            "session_auction_rotation_robustness_conditioner_profile": {
                "applies": True,
                "fields": {
                    ...
                    "source_failure_memory": SOURCE_AUDIT_PATH,
                    ...
                    "state_distributions": state_distributions,
                    "candidates_by_fold": candidates_by_fold,
                    "variant_boundary": "R0002 conditions R0001 auction rotation rank with fold-local robust-positive versus fragile-adverse state centroids; it is not a threshold, stop, target, hold, session, monthly, activity, or retry nudge",
            ...
            "mt5_pairing_profile": {
                ...
                "fields": {
                    ...
                    "next_action": "produce_c0012_r0002_mt5_logic_parity_evidence",
                },
            },
        ...
        "deferred_with_reason": [
            {
                ...
                "revisit_when": "during C0012 R0002 MT5 evidence production",
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_session_auction_rotation_robustness_conditioner_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "campaign_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        ...
        "session_auction_rotation_robustness_conditioner_profile": profiles[
            "session_auction_rotation_robustness_conditioner_profile"
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        not in {"proxy_kpi", "proxy_trade_artifact", "session_auction_rotation_robustness_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0012-R0002-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0012_session_auction_rotation_discovery/runs/R0002/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0012_session_auction_rotation_discovery/runs/R0002/run_manifest.json",
                    SOURCE_AUDIT_PATH,
                ],
            ),
            artifact_record(
                "A-C0012-R0002-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0012-R0002-SESSION-AUCTION-ROTATION-ROBUSTNESS-SUMMARY",
                "session_auction_rotation_robustness_summary_artifact",
                "json",
                "campaigns/C0012_session_auction_rotation_discovery/runs/R0002/artifacts/c0012_r0002_session_auction_rotation_robustness_summary.json",
                summary_hash,
                ["campaigns/C0012_session_auction_rotation_discovery/runs/R0002/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0012_r0002_session_auction_rotation_robustness",
...
def update_run_manifest_after_proxy() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0012_r0002_proxy_trades.csv"
    evidence["session_auction_rotation_robustness_summary"] = (
        "artifacts/c0012_r0002_session_auction_rotation_robustness_summary.json"
    )
...
def update_gate_report_after_proxy() -> None:
    ...
    data["next_action"] = "produce_c0012_r0002_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0012_r0002_proxy_trades.csv",
        "artifacts/c0012_r0002_session_auction_rotation_robustness_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0012 R0002 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_after_proxy() -> None:
    ...
    next_candidate["direction"] = "produce_c0012_r0002_mt5_logic_parity_evidence"
    ...
    closeout["remaining_question"] = "produce_c0012_r0002_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    for item in (
        "design_c0012_r0002_session_auction_rotation_robustness_conditioner_run",
        "open_c0012_r0002_session_auction_rotation_robustness_conditioner_run",
        "produce_c0012_r0002_proxy_evidence",
    ):
    ...
    data["active_run"] = "campaigns/C0012_session_auction_rotation_discovery/runs/R0002"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0012_r0002_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0012_session_auction_rotation_discovery/runs/R0002/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0012_r0002_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def update_decision_cursor_after_proxy() -> None:
    ...
    data["canonical_source"] = "campaigns/C0012_session_auction_rotation_discovery/runs/R0002/kpi/proxy.json"
    data["canonical_status"] = "proxy_recorded_pending_mt5"
    data["current_decision"] = "produce_c0012_r0002_mt5_logic_parity_evidence"
    data["active_run"] = "campaigns/C0012_session_auction_rotation_discovery/runs/R0002"
    data["next_required_action"] = "produce_c0012_r0002_mt5_logic_parity_evidence"
    current = data.setdefault("current_evidence_summary", {})
    current["source_run"] = "campaigns/C0012_session_auction_rotation_discovery/runs/R0002"
    current["current_task"] = "produce_c0012_r0002_mt5_logic_parity_evidence"
...
def linear_model_summary(model: LinearAuctionModel) -> dict[str, object]:
    return {
        ...
        "model_family": "fold_local_session_auction_rotation_rank_base",
...
def conditioner_summary(conditioner: RobustnessConditioner) -> dict[str, object]:
    return {
        ...
        "model_family": MODEL_FAMILY,
        "base_model_family": "fold_local_session_auction_rotation_rank_base",
        "base_feature_names": list(FEATURE_NAMES),
        "conditioner_feature_names": list(CONDITIONER_FEATURE_NAMES),
        ...
        "base_auction_model": linear_model_summary(conditioner.base_model),
        "score_interpretation": "higher_score_means_closer_to_fold_local_robust_auction_rotation_and_farther_from_fragile_adverse_conditioner",
    }
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "conditioner_weight": CONDITIONER_WEIGHT,
        "source_failure_memory": SOURCE_AUDIT_PATH,
        "variant_boundary": "session auction rotation robustness conditioner over train-only robust-positive and fragile-adverse centroid state; not a threshold, stop, target, hold, session, cost-buffer, monthly, activity, or retry nudge",
