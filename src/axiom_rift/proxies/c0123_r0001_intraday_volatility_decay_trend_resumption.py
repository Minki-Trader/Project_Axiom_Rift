"""C0123 R0001 proxy evidence for intraday volatility decay trend resumption."""
...
CAMPAIGN_ID = "C0123"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0123_intraday_volatility_decay_trend_resumption_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0123_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0123_r0001_volatility_decay_trend_resumption_summary.json"
...
FEATURE_NAMES = (
    ...
    "pullback_depth_control",
    "shelf_acceptance_balance",
    "vwap_trend_alignment",
    "close_location_persistence",
    "session_extension_room",
    "prior_session_directional_bias",
    "volume_stability",
    "adverse_wick_absorption",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_volatility_decay_trend_resumption_rank"
LABEL_SHAPE = "target_first_quality_after_volatility_decay_trend_resumption"
SELECTION_RULE = "top_fold_local_volatility_decay_trend_resumption_scores_per_active_day"
...
MIN_DAY_BARS = 18
VOLATILITY_DECAY_TREND_RESUMPTION_LOOKBACK_BARS = 48
VOLATILITY_DECAY_TREND_RESUMPTION_TREND_BARS = 14
VOLATILITY_DECAY_TREND_RESUMPTION_DECAY_BARS = 10
MIN_TREND_RUN_STRENGTH = 0.16
MIN_VOLATILITY_DECAY_TREND_RESUMPTION_ACTIVITY = 0.055
...
class LinearVolatilityDecayTrendResumptionModel:
    fold_id: str
    ...
    volatility_decay_trend_resumption_failure_rate: float
...
def run_c0123_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_volatility_decay_trend_resumption_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_volatility_decay_trend_resumption_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        VOLATILITY_DECAY_TREND_RESUMPTION_LOOKBACK_BARS,
        VOLATILITY_DECAY_TREND_RESUMPTION_TREND_BARS + VOLATILITY_DECAY_TREND_RESUMPTION_DECAY_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = volatility_decay_trend_resumption_metrics(
    ...
    features = (
        metrics["trend_run_strength"],
        metrics["volatility_decay_ratio"],
        metrics["pullback_depth_control"],
        metrics["shelf_acceptance_balance"],
        metrics["vwap_trend_alignment"],
        metrics["close_location_persistence"],
        metrics["session_extension_room"],
        metrics["prior_session_directional_bias"],
        metrics["volume_stability"],
        metrics["adverse_wick_absorption"],
        metrics["session_maturity"],
        metrics["spread_stress_inverse"],
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        breakout_pressure_bucket(metrics["trend_run_strength"]),
        range_moderation_bucket(metrics["volatility_decay_ratio"]),
        pullback_containment_bucket(metrics["pullback_depth_control"]),
        breakout_pressure_bucket(metrics["shelf_acceptance_balance"]),
        vwap_alignment_bucket(metrics["vwap_trend_alignment"]),
        absorption_bucket(metrics["close_location_persistence"]),
        range_position_pressure_bucket(metrics["session_extension_room"]),
        prior_day_position_context_bucket(metrics["prior_session_directional_bias"]),
        volume_stability_bucket(metrics["volume_stability"]),
        absorption_bucket(metrics["adverse_wick_absorption"]),
...
def volatility_decay_trend_resumption_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    decay_start = max(index - VOLATILITY_DECAY_TREND_RESUMPTION_DECAY_BARS + 1, context.day_start_index)
    trend_start = max(decay_start - VOLATILITY_DECAY_TREND_RESUMPTION_TREND_BARS, context.day_start_index)
    lookback_start = max(index - VOLATILITY_DECAY_TREND_RESUMPTION_LOOKBACK_BARS + 1, context.day_start_index)
    if trend_start < 0 or decay_start < 0 or lookback_start < 0:
        return {"event_count": 0.0}
    trend_window = bars[trend_start:decay_start]
    decay_window = bars[decay_start : index + 1]
    lookback_window = bars[lookback_start : index + 1]
    day_window = bars[context.day_start_index : index + 1]
    if len(trend_window) < 8 or len(decay_window) < 5 or len(lookback_window) < 24 or len(day_window) < MIN_DAY_BARS:
        return {"event_count": 0.0}
    average_volume = volume_average[index]
    if average_volume is None or average_volume <= 0:
        return {"event_count": 0.0}

    trend_ranges = [bar.high - bar.low for bar in trend_window]
    decay_ranges = [bar.high - bar.low for bar in decay_window]
    lookback_ranges = [bar.high - bar.low for bar in lookback_window]
    trend_range_mean = base.mean(trend_ranges) or average_range
    decay_range_mean = base.mean(decay_ranges) or average_range
    lookback_range_mean = base.mean(lookback_ranges) or average_range
    trend_volume_mean = base.mean(volumes[trend_start:decay_start]) or average_volume
    decay_volume_mean = base.mean(volumes[decay_start : index + 1]) or average_volume
    lookback_volume_mean = base.mean(volumes[lookback_start : index + 1]) or average_volume
    if trend_range_mean <= 0 or decay_range_mean <= 0 or lookback_range_mean <= 0:
        return {"event_count": 0.0}
    ...
    trend_move = direction * (trend_window[-1].close - trend_window[0].close)
    trend_run_strength = trend_move / max(trend_range_mean * math.sqrt(len(trend_window)), average_range, 1e-9)
    if trend_run_strength < MIN_TREND_RUN_STRENGTH:
        return {"event_count": 0.0}
    volatility_decay_ratio = 1.0 - decay_range_mean / max(lookback_range_mean, average_range, 1e-9)
    if volatility_decay_ratio < -0.40:
        return {"event_count": 0.0}

    trend_end_close = trend_window[-1].close
    pullback_extreme = min(bar.low for bar in decay_window) if direction > 0 else max(bar.high for bar in decay_window)
    pullback_depth = -direction * (pullback_extreme - trend_end_close) / max(abs(trend_move), average_range, 1e-9)
    if pullback_depth < -0.15 or pullback_depth > 1.45:
        return {"event_count": 0.0}
    pullback_depth_control = 1.0 - pullback_depth

    decay_high = max(bar.high for bar in decay_window)
    decay_low = min(bar.low for bar in decay_window)
    decay_mid = (decay_high + decay_low) / 2.0
    decay_settlement_balance = base.mean([directional_bar_settlement(bar, direction) for bar in decay_window]) or 0.0
    shelf_acceptance_balance = decay_settlement_balance + direction * (signal_bar.close - decay_mid) / max(average_range * 2.0, 1e-9)

    def adverse_wick_share(bar: base.Bar) -> float:
    ...
    adverse_wick_absorption = base.mean([adverse_wick_share(bar) for bar in decay_window]) or 0.0
    close_location_persistence = base.mean([directional_bar_settlement(bar, direction) for bar in day_window[-min(6, len(day_window)) :]]) or 0.0
    if max(shelf_acceptance_balance, close_location_persistence, adverse_wick_absorption) < -0.20:
        return {"event_count": 0.0}

    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    vwap_trend_alignment = direction * (signal_bar.close - session_vwap) / max(average_range * 3.0, 1e-9)
    prior_session_directional_bias = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_session_directional_bias = direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
    if context.prior_day_shadow_balance is not None:
        prior_session_directional_bias += direction * context.prior_day_shadow_balance * 0.08
    ...
    session_extension_room = 1.0 - directional_position
    volume_stability = 1.0 - abs(decay_volume_mean / max(lookback_volume_mean, average_volume, 1e-9) - 1.0)
    ...
    activity = max(
        trend_run_strength * 0.22,
        max(volatility_decay_ratio, 0.0) * 0.16,
        max(pullback_depth_control, 0.0) * 0.13,
        max(shelf_acceptance_balance, 0.0) * 0.19,
        max(vwap_trend_alignment, 0.0) * 0.10,
        max(close_location_persistence, 0.0) * 0.12,
        max(volume_stability, 0.0) * 0.07,
        adverse_wick_absorption * 0.10,
    )
    if activity < MIN_VOLATILITY_DECAY_TREND_RESUMPTION_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "trend_run_strength": bounded(trend_run_strength, -4.0, 8.0),
        "volatility_decay_ratio": bounded(volatility_decay_ratio, -4.0, 1.0),
        "pullback_depth_control": bounded(pullback_depth_control, -2.0, 2.0),
        "shelf_acceptance_balance": bounded(shelf_acceptance_balance, -4.0, 8.0),
        "vwap_trend_alignment": bounded(vwap_trend_alignment, -4.0, 4.0),
        "close_location_persistence": bounded(close_location_persistence, -1.0, 1.0),
        "session_extension_room": bounded(session_extension_room, 0.0, 1.0),
        "prior_session_directional_bias": bounded(prior_session_directional_bias, -3.0, 3.0),
        "volume_stability": bounded(volume_stability, -4.0, 1.0),
        "adverse_wick_absorption": bounded(adverse_wick_absorption, 0.0, 1.0),
        "session_maturity": bounded(session_phase_maturity, 0.0, 1.0),
        "spread_stress_inverse": bounded(spread_stress_inverse, 0.0, 1.0),
        "recent_range_ratio": bounded(decay_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        "base_range_ratio": bounded(trend_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        "lookback_range_ratio": bounded(lookback_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        "recent_volume_ratio": bounded(decay_volume_mean / max(average_volume, 1e-9), 0.0, 8.0),
        "base_volume_ratio": bounded(trend_volume_mean / max(average_volume, 1e-9), 0.0, 8.0),
        "lookback_volume_ratio": bounded(lookback_volume_mean / max(average_volume, 1e-9), 0.0, 8.0),
        "decay_mid": rounded(decay_mid) or 0.0,
        "pullback_extreme": rounded(pullback_extreme) or 0.0,
        "volatility_decay_trend_resumption_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0123_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0123_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0123_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0123_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0123_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0123_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0123_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0123_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0123_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0123_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0123_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0123_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0123_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0123_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0123_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0123_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0123_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0123_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0123_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def c0123_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    trend_reward = 0.11 if metrics["trend_run_strength"] > 0.32 and terminal > 0 else 0.0
    decay_reward = 0.08 if metrics["volatility_decay_ratio"] > 0.04 and terminal > 0 else 0.0
    pullback_reward = 0.09 if metrics["pullback_depth_control"] > 0.12 and terminal > 0 else 0.0
    shelf_reward = 0.12 if metrics["shelf_acceptance_balance"] > 0.04 and terminal > 0 else 0.0
    vwap_reward = 0.07 if metrics["vwap_trend_alignment"] > -0.02 and terminal > 0 else 0.0
    close_persistence_reward = 0.06 if metrics["close_location_persistence"] > 0.05 and terminal > 0 else 0.0
    room_reward = 0.04 if metrics["session_extension_room"] > 0.18 and terminal > 0 else 0.0
    prior_bias_reward = 0.04 if metrics["prior_session_directional_bias"] > -0.25 and terminal > 0 else 0.0
    volume_reward = 0.05 if metrics["volume_stability"] > -0.05 and terminal > 0 else 0.0
    wick_absorption_reward = 0.06 if metrics["adverse_wick_absorption"] > 0.22 and terminal > 0 else 0.0
    weak_trend_penalty = 0.08 if metrics["trend_run_strength"] < 0.24 and terminal <= 0 else 0.0
    expansion_penalty = 0.08 if metrics["volatility_decay_ratio"] < -0.10 and terminal <= 0 else 0.0
    deep_pullback_penalty = 0.10 if metrics["pullback_depth_control"] < -0.10 and terminal <= 0 else 0.0
    weak_shelf_penalty = 0.10 if metrics["shelf_acceptance_balance"] < -0.08 and terminal <= 0 else 0.0
    weak_close_penalty = 0.06 if metrics["close_location_persistence"] < -0.08 and terminal <= 0 else 0.0
    ...
    return (
        ...
        + shelf_reward
        + vwap_reward
        + close_persistence_reward
        + room_reward
        + prior_bias_reward
        + volume_reward
        + wick_absorption_reward
        - weak_trend_penalty
        - expansion_penalty
        ...
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["recent_range_ratio"] - 2.40) * 0.04
        - max(0.0, metrics["recent_volume_ratio"] - 4.20) * 0.015
        - max(0.0, 0.16 - metrics["trend_run_strength"]) * 0.12
    )


def fit_linear_volatility_decay_trend_resumption_model(candidates: list[base.Candidate], fold_id: str) -> LinearVolatilityDecayTrendResumptionModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearVolatilityDecayTrendResumptionModel(
            fold_id,
    ...
    volatility_decay_trend_resumption_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearVolatilityDecayTrendResumptionModel(
        fold_id=fold_id,
        ...
        volatility_decay_trend_resumption_failure_rate=volatility_decay_trend_resumption_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearVolatilityDecayTrendResumptionModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0123-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_volatility_decay_trend_resumption_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0123_r0001_intraday_volatility_decay_trend_resumption",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0123_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0123_r0001_volatility_decay_trend_resumption_summary.json",
        ],
        ...
        "conditional_profiles": {
            "volatility_decay_trend_resumption_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0123 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0123_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0123 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "volatility_decay_trend_resumption_lookback_bars": VOLATILITY_DECAY_TREND_RESUMPTION_LOOKBACK_BARS,
        "volatility_decay_trend_resumption_trend_bars": VOLATILITY_DECAY_TREND_RESUMPTION_TREND_BARS,
        "volatility_decay_trend_resumption_decay_bars": VOLATILITY_DECAY_TREND_RESUMPTION_DECAY_BARS,
        "min_trend_run_strength": MIN_TREND_RUN_STRENGTH,
        "min_volatility_decay_trend_resumption_activity": MIN_VOLATILITY_DECAY_TREND_RESUMPTION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_volatility_decay_trend_resumption_model_summary(model: LinearVolatilityDecayTrendResumptionModel) -> dict[str, object]:
    return {
        ...
        "volatility_decay_trend_resumption_failure_rate": rounded(model.volatility_decay_trend_resumption_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearVolatilityDecayTrendResumptionModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_volatility_decay_trend_resumption_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "volatility_decay_trend_resumption_profile": profiles["volatility_decay_trend_resumption_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0123_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0123_r0001_proxy_trades.csv"
    evidence["volatility_decay_trend_resumption_summary"] = "artifacts/c0123_r0001_volatility_decay_trend_resumption_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0123_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0123_r0001_proxy_trades.csv",
        "artifacts/c0123_r0001_volatility_decay_trend_resumption_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0123_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0123 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0123_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0123_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0123_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0123_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0123_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0123_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0123_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0123_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0123_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0123 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0123 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260708_produce_c0123_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0123_r0001_proxy_evidence
  refines:
  - dec_20260708_open_c0123_r0001_intraday_volatility_decay_trend_resumption_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0123_r0001_proxy_records_volatility_decay_trend_resumption_reference_surface
  ...
  - next_work_is_produce_c0123_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260708_produce_c0123_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0123-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0123_r0001_intraday_volatility_decay_trend_resumption",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0123-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0123-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0123-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0123_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0123_r0001_intraday_volatility_decay_trend_resumption",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0123-R0001-VOLATILITY-DECAY-TREND-RESUMPTION-SUMMARY",
            "artifact_role": "volatility_decay_trend_resumption_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0123_r0001_volatility_decay_trend_resumption_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0123_r0001_intraday_volatility_decay_trend_resumption",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0123_r0001_mt5_logic_parity_evidence",
        }
    ]
