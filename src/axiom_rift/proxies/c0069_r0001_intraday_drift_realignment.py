"""C0069 R0001 proxy evidence for intraday drift realignment."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0069_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0069_r0001_drift_realignment_summary.json"
...
FEATURE_NAMES = (
    "short_drift_pressure",
    "medium_drift_pressure",
    "session_drift_pressure",
    "drift_alignment_score",
    "fracture_repair_pressure",
    "pullback_absorption_depth",
    "impulse_quality",
    "participation_confirmation",
    "prior_day_drift_context",
    "session_maturity_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_drift_realignment_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_drift_realignment_state"
SELECTION_RULE = "top_fold_local_drift_realignment_scores_per_active_day"
...
MIN_DAY_BARS = 30
DRIFT_REALIGNMENT_LOOKBACK_BARS = 20
DRIFT_REALIGNMENT_SHORT_LOOKBACK_BARS = 6
MIN_DRIFT_REALIGNMENT_ACTIVITY = 0.10
...
class LinearDriftRealignmentModel:
    fold_id: str
    ...
    drift_realignment_failure_rate: float
...
def run_c0069_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_drift_realignment_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_drift_realignment_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        DRIFT_REALIGNMENT_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        drift["short_drift_pressure"],
        drift["medium_drift_pressure"],
        drift["session_drift_pressure"],
        drift["drift_alignment_score"],
        drift["fracture_repair_pressure"],
        drift["pullback_absorption_depth"],
        drift["impulse_quality"],
        drift["participation_confirmation"],
        drift["prior_day_drift_context"],
        drift["session_maturity_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        short_drift_bucket(drift["short_drift_pressure"]),
        medium_drift_bucket(drift["medium_drift_pressure"]),
        session_drift_bucket(drift["session_drift_pressure"]),
        drift_alignment_bucket(drift["drift_alignment_score"]),
        fracture_repair_bucket(drift["fracture_repair_pressure"]),
        pullback_absorption_bucket(drift["pullback_absorption_depth"]),
        impulse_quality_bucket(drift["impulse_quality"]),
        participation_confirmation_bucket(drift["participation_confirmation"]),
        prior_day_drift_bucket(drift["prior_day_drift_context"]),
        session_pressure_bucket(drift["session_maturity_pressure"]),
...
def drift_realignment_metrics(
    ...
    window_start = index - DRIFT_REALIGNMENT_LOOKBACK_BARS + 1
    short_start = index - DRIFT_REALIGNMENT_SHORT_LOOKBACK_BARS + 1
    previous_short_start = index - DRIFT_REALIGNMENT_SHORT_LOOKBACK_BARS
    ...
    short_drift_pressure = direction * (signal_bar.close - short_window[0].open) / average_range
    medium_drift_pressure = direction * (signal_bar.close - medium_window[0].open) / average_range
    session_drift_pressure = direction * (signal_bar.close - context.day_open) / average_range
    previous_short_drift = direction * (bars[index - 1].close - previous_short_window[0].open) / average_range
    ...
    drift_alignment_score = positive_components / 3.0
    signal_body_pressure = direction * (signal_bar.close - signal_bar.open) / average_range
    fracture_repair_pressure = max(0.0, -previous_short_drift) * max(0.0, medium_drift_pressure)
    fracture_repair_pressure += max(0.0, signal_body_pressure)
    if direction > 0:
        pullback_absorption_depth = (signal_bar.close - min(item.low for item in short_window)) / average_range
    else:
        pullback_absorption_depth = (max(item.high for item in short_window) - signal_bar.close) / average_range
    impulse_quality = direction * (signal_bar.close - signal_bar.open) / signal_range
    recent_directional_pressure = sum(
    ...
    if context.prior_day_range is None or context.prior_day_range <= 0 or context.prior_day_return is None:
        prior_day_drift_context = 0.0
    else:
        prior_day_drift_context = direction * context.prior_day_return / context.prior_day_range
    session_maturity_pressure = bounded_session_progress(signal_bar.time) * max(
        max(short_drift_pressure, 0.0),
        max(medium_drift_pressure, 0.0),
        max(session_drift_pressure, 0.0),
        drift_alignment_score,
        max(fracture_repair_pressure, 0.0),
        max(participation_confirmation, 0.0),
    )
    activity = max(
        abs(short_drift_pressure),
        abs(medium_drift_pressure),
        abs(session_drift_pressure),
        drift_alignment_score,
        max(fracture_repair_pressure, 0.0),
        max(pullback_absorption_depth, 0.0),
        abs(impulse_quality),
        abs(recent_directional_pressure),
        abs(participation_confirmation),
        abs(prior_day_drift_context),
        session_maturity_pressure,
    )
    if activity < MIN_DRIFT_REALIGNMENT_ACTIVITY:
        return {"event_count": 0.0}
    if (
        drift_alignment_score < 0.34
        and fracture_repair_pressure < 0.04
        and abs(short_drift_pressure) < 0.08
        and abs(medium_drift_pressure) < 0.12
    ):
        return {"event_count": 0.0}
    return {
        "short_drift_pressure": bounded(short_drift_pressure, -5.0, 5.0),
        "medium_drift_pressure": bounded(medium_drift_pressure, -8.0, 8.0),
        "session_drift_pressure": bounded(session_drift_pressure, -12.0, 12.0),
        "drift_alignment_score": bounded(drift_alignment_score, 0.0, 1.0),
        "fracture_repair_pressure": bounded(fracture_repair_pressure, 0.0, 6.0),
        "pullback_absorption_depth": bounded(pullback_absorption_depth, 0.0, 6.0),
        "impulse_quality": bounded(impulse_quality, -1.0, 1.0),
        "participation_confirmation": bounded(participation_confirmation, -4.0, 4.0),
        "prior_day_drift_context": bounded(prior_day_drift_context, -1.0, 1.0),
        "session_maturity_pressure": bounded(session_maturity_pressure, 0.0, 4.0),
        "recent_directional_pressure": bounded(recent_directional_pressure, -4.0, 4.0),
        "event_count": 1.0,
    }
...
def short_drift_bucket(value: float) -> str:
    if value < -0.25:
        return "short_drift_against"
    if value < 0.25:
        return "short_drift_mixed"
    return "short_drift_with"


def medium_drift_bucket(value: float) -> str:
    if value < -0.45:
        return "medium_drift_against"
    if value < 0.45:
        return "medium_drift_mixed"
    return "medium_drift_with"


def session_drift_bucket(value: float) -> str:
    if value < -0.80:
        return "session_drift_against"
    if value < 0.80:
        return "session_drift_mixed"
    return "session_drift_with"


def drift_alignment_bucket(value: float) -> str:
    if value < 0.34:
        return "drift_alignment_low"
    if value < 0.67:
        return "drift_alignment_partial"
    return "drift_alignment_full"


def fracture_repair_bucket(value: float) -> str:
    if value < 0.10:
        return "fracture_repair_low"
    if value < 0.55:
        return "fracture_repair_mid"
    return "fracture_repair_high"


def pullback_absorption_bucket(value: float) -> str:
    if value < 0.25:
        return "pullback_absorption_shallow"
    if value < 0.85:
        return "pullback_absorption_mid"
    return "pullback_absorption_deep"


def impulse_quality_bucket(value: float) -> str:
    if value < -0.18:
        return "impulse_against"
    if value < 0.24:
        return "impulse_mixed"
    return "impulse_with"
...
def prior_day_drift_bucket(value: float) -> str:
    if value < -0.18:
        return "prior_day_drift_against"
    if value < 0.18:
        return "prior_day_drift_mixed"
    return "prior_day_drift_with"
...
def candidate_label(
    ...
    metrics = drift_realignment_metrics(
        bars,
        volumes,
    ...
    if metrics["event_count"] <= 0:
        return None
    short_reward = 0.12 if metrics["short_drift_pressure"] > 0.18 and terminal > 0 else 0.0
    medium_reward = 0.16 if metrics["medium_drift_pressure"] > 0.36 and terminal > 0 else 0.0
    session_reward = 0.10 if metrics["session_drift_pressure"] > 0.55 and terminal > 0 else 0.0
    alignment_reward = 0.18 if metrics["drift_alignment_score"] >= 0.67 and terminal > 0 else 0.0
    fracture_reward = 0.16 if metrics["fracture_repair_pressure"] > 0.18 and terminal > 0 else 0.0
    absorption_reward = 0.12 if metrics["pullback_absorption_depth"] > 0.32 and terminal > 0 else 0.0
    impulse_reward = 0.12 if metrics["impulse_quality"] > 0.18 and terminal > 0 else 0.0
    participation_reward = 0.10 if metrics["participation_confirmation"] > 0.15 and terminal > 0 else 0.0
    prior_drift_reward = 0.06 if metrics["prior_day_drift_context"] > 0.16 and terminal > 0 else 0.0
    misalignment_penalty = 0.22 if metrics["drift_alignment_score"] < 0.34 and terminal <= 0 else 0.0
    failed_repair_penalty = 0.20 if metrics["fracture_repair_pressure"] < 0.06 and terminal <= 0 else 0.0
    weak_impulse_penalty = 0.18 if metrics["impulse_quality"] < 0.0 and terminal <= 0 else 0.0
    ...
    drift_realignment_path_quality = (0.44 * terminal + 0.36 * mfe - 0.72 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + drift_realignment_path_quality
        + short_reward
        + medium_reward
        + session_reward
        + alignment_reward
...
def fit_linear_drift_realignment_model(candidates: list[base.Candidate], fold_id: str) -> LinearDriftRealignmentModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearDriftRealignmentModel(
            fold_id,
    ...
    drift_realignment_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearDriftRealignmentModel(
        fold_id=fold_id,
        ...
        drift_realignment_failure_rate=drift_realignment_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearDriftRealignmentModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_drift_realignment_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0069_r0001_intraday_drift_realignment",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0069_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0069_r0001_drift_realignment_summary.json",
        ],
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0069 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0069_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0069 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "drift_realignment_lookback_bars": DRIFT_REALIGNMENT_LOOKBACK_BARS,
        "drift_realignment_short_lookback_bars": DRIFT_REALIGNMENT_SHORT_LOOKBACK_BARS,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_drift_realignment_model_summary(model: LinearDriftRealignmentModel) -> dict[str, object]:
    return {
        ...
        "drift_realignment_failure_rate": rounded(model.drift_realignment_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearDriftRealignmentModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_drift_realignment_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "drift_realignment_profile": profiles["drift_realignment_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0069_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0069_r0001_proxy_trades.csv"
    evidence["drift_realignment_summary"] = "artifacts/c0069_r0001_drift_realignment_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0069_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0069_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0069 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0069_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0069_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0069_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0069_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0069_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0069_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0069_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0069_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0069_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0069 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0069 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260706_produce_c0069_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0069_r0001_proxy_evidence
  refines:
  - dec_20260706_open_c0069_r0001_intraday_drift_realignment_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0069_r0001_proxy_records_drift_realignment_reference_surface
  ...
  - next_work_is_produce_c0069_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260706_produce_c0069_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0069-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0069_r0001_intraday_drift_realignment",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0069-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0069-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0069-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0069_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0069_r0001_intraday_drift_realignment",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0069-R0001-drift-realignment-SUMMARY",
            "artifact_role": "drift_realignment_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0069_r0001_drift_realignment_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0069_r0001_intraday_drift_realignment",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0069_r0001_mt5_logic_parity_evidence",
        }
    ]
