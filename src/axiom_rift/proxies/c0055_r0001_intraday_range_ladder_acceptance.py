"""C0055 R0001 proxy evidence for intraday range ladder acceptance."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0055_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0055_r0001_range_ladder_acceptance_summary.json"
...
FEATURE_NAMES = (
    "directional_ladder_acceptance",
    "nearest_ladder_proximity",
    "ladder_cross_recency",
    "directional_ladder_cross_impulse",
    "post_cross_dwell_ratio",
    "failed_recross_pressure",
    "directional_range_position",
    "day_range_completion_ratio",
    "range_expansion_pace_12",
    "range_compression_12_vs_48",
    "volume_confirmation_on_ladder_shift",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_range_ladder_acceptance_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_range_ladder_acceptance_state"
SELECTION_RULE = "top_fold_local_range_ladder_acceptance_scores_per_active_day"
...
LADDER_LEVEL_FRACTIONS = (0.25, 0.50, 0.75)
MIN_DAY_BARS = 36
LADDER_CROSS_LOOKBACK_BARS = 12
LADDER_DWELL_LOOKBACK_BARS = 6
MIN_LADDER_ACTIVITY = 0.18
...
class LinearRangeLadderAcceptanceModel:
    fold_id: str
...
def run_c0055_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_range_ladder_acceptance_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_range_ladder_acceptance_model_summary(model))
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    ladder = range_ladder_acceptance_metrics(bars, context, index, direction, average_range)
    if ladder["ladder_event_count"] <= 0:
        return None, None
    ...
    volume_confirmation = bounded(
        volume_ratio
        * (
            0.45 * ladder["directional_ladder_acceptance"]
            + 0.35 * ladder["directional_ladder_cross_impulse"]
            + 0.20 * ladder["post_cross_dwell_ratio"]
        ),
        -4.0,
        4.0,
    )
    features = (
        ladder["directional_ladder_acceptance"],
        ladder["nearest_ladder_proximity"],
        ladder["ladder_cross_recency"],
        ladder["directional_ladder_cross_impulse"],
        ladder["post_cross_dwell_ratio"],
        ladder["failed_recross_pressure"],
        ladder["directional_range_position"],
        ladder["day_range_completion_ratio"],
        ladder["range_expansion_pace_12"],
        compression,
        volume_confirmation,
    ...
    dimensions = (
        ladder_acceptance_bucket(ladder["directional_ladder_acceptance"]),
        ladder_proximity_bucket(ladder["nearest_ladder_proximity"]),
        ladder_recency_bucket(ladder["ladder_cross_recency"]),
        ladder_impulse_bucket(ladder["directional_ladder_cross_impulse"]),
        ladder_dwell_bucket(ladder["post_cross_dwell_ratio"]),
        failed_recross_bucket(ladder["failed_recross_pressure"]),
        edge_pressure_bucket(ladder["directional_range_position"]),
        expansion_bucket(ladder["range_expansion_pace_12"]),
...
def range_ladder_acceptance_metrics(
    bars: list[base.Bar],
    ...
    if day_width <= 0:
        return {"ladder_event_count": 0.0}

    levels = ladder_levels(context)
    nearest_level = min(levels, key=lambda level: abs(close - level))
    nearest_distance = abs(close - nearest_level)
    nearest_proximity = 1.0 - min(nearest_distance / max(0.75 * average_range, 1e-9), 1.0)
    cross_age, crossed_level = recent_ladder_cross(bars, index, direction, levels, LADDER_CROSS_LOOKBACK_BARS)
    cross_recency = 0.0 if cross_age is None else 1.0 - min(cross_age / LADDER_CROSS_LOOKBACK_BARS, 1.0)
    reference_level = crossed_level if crossed_level is not None else nearest_level
    directional_acceptance = direction * (close - reference_level) / average_range
    cross_impulse = direction * (close - bars[index - 3].close) / average_range
    dwell_ratio = post_cross_dwell_ratio(bars, index, direction, reference_level, LADDER_DWELL_LOOKBACK_BARS)
    failed_recross = failed_recross_pressure(bars, index, direction, reference_level, LADDER_DWELL_LOOKBACK_BARS)
    directional_position = directional_session_position(close, context, direction) or 0.5
    range_completion = day_width / average_range
    previous_width = historical_day_width(bars, context, index, 12)
    expansion_pace = 0.0 if previous_width is None else (day_width - previous_width) / average_range
    activity = max(nearest_proximity, cross_recency, abs(directional_acceptance))
    if activity < MIN_LADDER_ACTIVITY:
        return {"ladder_event_count": 0.0}
    return {
        "directional_ladder_acceptance": bounded(directional_acceptance, -4.0, 4.0),
        "nearest_ladder_proximity": bounded(nearest_proximity, 0.0, 1.0),
        "ladder_cross_recency": bounded(cross_recency, 0.0, 1.0),
        "directional_ladder_cross_impulse": bounded(cross_impulse, -4.0, 4.0),
        "post_cross_dwell_ratio": bounded(dwell_ratio, 0.0, 1.0),
        "failed_recross_pressure": bounded(failed_recross, 0.0, 1.0),
        "directional_range_position": bounded(directional_position, 0.0, 1.0),
        "day_range_completion_ratio": bounded(range_completion, 0.0, 8.0),
        "range_expansion_pace_12": bounded(expansion_pace, -4.0, 4.0),
        "ladder_event_count": 1.0,
    }


def ladder_levels(context: DailyContext) -> tuple[float, ...]:
    width = context.high_so_far - context.low_so_far
    return tuple(context.low_so_far + fraction * width for fraction in LADDER_LEVEL_FRACTIONS)


def recent_ladder_cross(
    bars: list[base.Bar],
    index: int,
    direction: int,
    levels: tuple[float, ...],
    ...
    earliest = max(1, index - lookback + 1)
    for position in range(index, earliest - 1, -1):
        previous_close = bars[position - 1].close
        current_close = bars[position].close
        crossed: list[float] = []
        for level in levels:
            if direction > 0 and previous_close <= level < current_close:
                crossed.append(level)
            elif direction < 0 and previous_close >= level > current_close:
                crossed.append(level)
        if crossed:
            current = bars[index].close
            level = min(crossed, key=lambda item: abs(current - item))
            return index - position, level
    return None, None


def post_cross_dwell_ratio(
    bars: list[base.Bar],
    index: int,
    direction: int,
    level: float,
    ...
    closes = [bars[position].close for position in range(start, index + 1)]
    if not closes:
        return 0.0
    if direction > 0:
        return sum(1 for close in closes if close > level) / len(closes)
    return sum(1 for close in closes if close < level) / len(closes)


def failed_recross_pressure(
    bars: list[base.Bar],
    index: int,
    direction: int,
    level: float,
...
def ladder_acceptance_bucket(value: float) -> str:
    if value < -0.40:
        return "rejected_ladder_side"
    if value < 0.25:
        return "near_ladder_balance"
    return "accepted_ladder_side"


def ladder_proximity_bucket(value: float) -> str:
    if value < 0.35:
        return "away_from_ladder"
    if value < 0.70:
        return "near_ladder"
    return "touching_ladder"


def ladder_recency_bucket(value: float) -> str:
    if value <= 0.0:
        return "no_recent_ladder_cross"
    if value < 0.50:
        return "older_ladder_cross"
    return "fresh_ladder_cross"
...
def ladder_impulse_bucket(value: float) -> str:
    if value < -0.35:
        return "ladder_impulse_against"
    if value < 0.35:
        return "ladder_impulse_neutral"
    return "ladder_impulse_with"


def ladder_dwell_bucket(value: float) -> str:
    if value < 0.40:
        return "failed_ladder_dwell"
    if value < 0.75:
        return "mixed_ladder_dwell"
    return "accepted_ladder_dwell"


def failed_recross_bucket(value: float) -> str:
    if value < 0.15:
        return "low_failed_recross"
    if value < 0.40:
        return "mixed_failed_recross"
    return "high_failed_recross"


def expansion_bucket(value: float) -> str:
    if value < -0.20:
...
def candidate_label(
    ...
    ladder_acceptance_reward = 0.20 if recent_response > 0.15 and terminal > 0 else 0.0
    failed_ladder_shift_penalty = 0.20 if recent_response < -0.15 and mae > mfe else 0.0
    ...
    ladder_path_quality = (0.35 * terminal + 0.35 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + ladder_path_quality
        + ladder_acceptance_reward
        - failed_ladder_shift_penalty
        - stall_penalty
...
def fit_linear_range_ladder_acceptance_model(candidates: list[base.Candidate], fold_id: str) -> LinearRangeLadderAcceptanceModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearRangeLadderAcceptanceModel(fold_id, tuple(0.0 for _ in FEATURE_NAMES), tuple(1.0 for _ in FEATURE_NAMES), tuple(0.0 for _ in FEATURE_NAMES), 0, 0.0, 0.0, 0.0)
    ...
    ladder_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearRangeLadderAcceptanceModel(
        fold_id=fold_id,
        ...
        ladder_failure_rate=ladder_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearRangeLadderAcceptanceModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_range_ladder_acceptance_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0055_r0001_intraday_range_ladder_acceptance",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0055_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0055_r0001_range_ladder_acceptance_summary.json",
        ],
        ...
        "conditional_profiles": {
            "range_ladder_acceptance_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0055 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0055_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0055 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def linear_range_ladder_acceptance_model_summary(model: LinearRangeLadderAcceptanceModel) -> dict[str, object]:
    return {
        ...
        "ladder_failure_rate": rounded(model.ladder_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearRangeLadderAcceptanceModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_range_ladder_acceptance_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "range_ladder_acceptance_profile": profiles["range_ladder_acceptance_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0055_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_trade_artifact"] = "artifacts/c0055_r0001_proxy_trades.csv"
    evidence["range_ladder_acceptance_summary"] = "artifacts/c0055_r0001_range_ladder_acceptance_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0055_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0055_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0055 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0055_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0055_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0055_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0055_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0055_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0055_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0055_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0055_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0055_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0055 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0055 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260706_produce_c0055_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0055_r0001_proxy_evidence
  refines:
  - dec_20260706_open_c0055_r0001_intraday_range_ladder_acceptance_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0055_r0001_proxy_records_range_ladder_acceptance_reference_surface
  ...
  - next_work_is_produce_c0055_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260706_produce_c0055_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0055-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0055_r0001_intraday_range_ladder_acceptance",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0055-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0055-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0055-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0055_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0055_r0001_intraday_range_ladder_acceptance",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0055-R0001-RANGE-LADDER-ACCEPTANCE-SUMMARY",
            "artifact_role": "range_ladder_acceptance_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0055_r0001_range_ladder_acceptance_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0055_r0001_intraday_range_ladder_acceptance",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0055_r0001_mt5_logic_parity_evidence",
        }
    ]
