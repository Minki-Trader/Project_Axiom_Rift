"""C0096 R0001 proxy evidence for intraday wick stack pressure reversal."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0096_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0096_r0001_wick_stack_pressure_reversal_summary.json"
...
FEATURE_NAMES = (
    "adverse_wick_stack_pressure",
    "body_followthrough_failure",
    "failed_extreme_progress",
    "range_expansion_without_close_progress",
    "reversal_settlement",
    "wick_pressure_density",
    "volume_tail_engagement",
    "vwap_reversion_pressure",
    "session_phase_maturity",
    "prior_day_shadow_context",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_wick_stack_pressure_reversal_rank"
LABEL_SHAPE = "target_first_reversal_quality_conditioned_on_wick_stack_pressure_reversal"
SELECTION_RULE = "top_fold_local_wick_stack_pressure_reversal_scores_per_active_day"
...
MIN_DAY_BARS = 30
WICK_STACK_PRESSURE_LOOKBACK_BARS = 20
WICK_STACK_PRESSURE_STACK_BARS = 8
MIN_WICK_STACK_PRESSURE_ACTIVITY = 0.08
...
class LinearWickStackPressureReversalModel:
    fold_id: str
    ...
    wick_stack_pressure_reversal_failure_rate: float
...
def run_c0096_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_wick_stack_pressure_reversal_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_wick_stack_pressure_reversal_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        WICK_STACK_PRESSURE_LOOKBACK_BARS,
        WICK_STACK_PRESSURE_STACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = wick_stack_pressure_reversal_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        metrics["adverse_wick_stack_pressure"],
        metrics["body_followthrough_failure"],
        metrics["failed_extreme_progress"],
        metrics["range_expansion_without_close_progress"],
        metrics["reversal_settlement"],
        metrics["wick_pressure_density"],
        metrics["volume_tail_engagement"],
        metrics["vwap_reversion_pressure"],
        metrics["session_phase_maturity"],
        metrics["prior_day_shadow_context"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        wick_bucket(metrics["adverse_wick_stack_pressure"]),
        failed_followthrough_bucket(metrics["body_followthrough_failure"]),
        failed_cross_bucket(metrics["failed_extreme_progress"]),
        range_expansion_bucket(metrics["range_expansion_without_close_progress"]),
        reversal_commitment_bucket(metrics["reversal_settlement"] - 0.50),
        shadow_cluster_bucket(metrics["wick_pressure_density"]),
        participation_bucket(1.0 + metrics["volume_tail_engagement"]),
        vwap_reversion_depth_bucket(metrics["vwap_reversion_pressure"]),
        session_phase_maturity_bucket(metrics["session_phase_maturity"]),
        prior_shadow_contrast_bucket(metrics["prior_day_shadow_context"]),
...
def wick_stack_pressure_reversal_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    stack_start = index - WICK_STACK_PRESSURE_STACK_BARS
    lookback_start = index - WICK_STACK_PRESSURE_LOOKBACK_BARS
    ...
    close_travel = sum(abs(stack_closes[pos] - stack_closes[pos - 1]) for pos in range(1, len(stack_closes)))
    close_progress = abs(stack_closes[-1] - stack_closes[0]) / max(close_travel, average_range * 0.25)
    range_expansion_without_close_progress = (stack_range_mean / max(prior_range_mean, 1e-9)) - close_progress
    wick_scores = [directional_wick_rejection(bar, direction) for bar in stack_window]
    positive_wicks = [max(value, 0.0) for value in wick_scores]
    adverse_wick_stack_pressure = base.mean(positive_wicks) or 0.0
    wick_pressure_density = sum(1 for value in wick_scores if value > 0.12) / len(wick_scores)
    body_scores = [directional_body_score(bar, direction) for bar in stack_window]
    body_mean = base.mean(body_scores) or 0.0
    body_followthrough_failure = adverse_wick_stack_pressure - max(-body_mean, 0.0)
    if direction > 0:
        failed_extreme_progress = (signal_bar.close - stack_low) / max(average_range, 1e-9)
    else:
        failed_extreme_progress = (stack_high - signal_bar.close) / max(average_range, 1e-9)
    reversal_settlement = directional_bar_settlement(signal_bar, direction)
    stack_volume_average = base.mean(volumes[stack_start:index]) or average_volume
    volume_tail_engagement = volumes[index] / max(stack_volume_average, 1e-9) - 1.0
    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    vwap_reversion_pressure = direction * (session_vwap - signal_bar.close) / max(average_range, 1e-9)
    session_phase_maturity = bounded_session_progress(signal_bar.time)
    prior_day_shadow_context = 0.0
    if context.prior_day_shadow_balance is not None:
        prior_day_shadow_context = direction * context.prior_day_shadow_balance
    wick_reversal_pressure_raw = max(
        max(adverse_wick_stack_pressure - 0.12, 0.0) * 1.25,
        max(body_followthrough_failure, 0.0),
        max(failed_extreme_progress - 0.35, 0.0) * 0.70,
        max(range_expansion_without_close_progress, 0.0) * 0.45,
        max(reversal_settlement - 0.50, 0.0),
        max(wick_pressure_density - 0.28, 0.0),
        max(volume_tail_engagement, 0.0) * 0.25,
        max(vwap_reversion_pressure, 0.0) * 0.20,
    )
    activity = max(wick_reversal_pressure_raw, wick_reversal_pressure_raw * session_phase_maturity)
    if activity < MIN_WICK_STACK_PRESSURE_ACTIVITY:
        return {"event_count": 0.0}
    if adverse_wick_stack_pressure < 0.08 and wick_pressure_density < 0.25:
        return {"event_count": 0.0}
    if body_followthrough_failure < -0.10 and reversal_settlement < 0.55:
        return {"event_count": 0.0}
    if volume_tail_engagement < -0.55 and reversal_settlement < 0.62:
        return {"event_count": 0.0}
    return {
        "adverse_wick_stack_pressure": bounded(adverse_wick_stack_pressure, -1.0, 1.0),
        "body_followthrough_failure": bounded(body_followthrough_failure, -1.0, 1.0),
        "failed_extreme_progress": bounded(failed_extreme_progress, -3.0, 5.0),
        "range_expansion_without_close_progress": bounded(range_expansion_without_close_progress, -3.0, 5.0),
        "reversal_settlement": bounded(reversal_settlement, 0.0, 1.0),
        "wick_pressure_density": bounded(wick_pressure_density, 0.0, 1.0),
        "volume_tail_engagement": bounded(volume_tail_engagement, -3.0, 6.0),
        "vwap_reversion_pressure": bounded(vwap_reversion_pressure, -5.0, 5.0),
        "session_phase_maturity": bounded(session_phase_maturity, 0.0, 1.0),
        "prior_day_shadow_context": bounded(prior_day_shadow_context, -3.0, 3.0),
        "stack_range_ratio": bounded(stack_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        "stack_mid_reversal": bounded(direction * (signal_bar.close - stack_mid) / max(average_range, 1e-9), -4.0, 4.0),
        "wick_reversal_pressure": bounded(wick_reversal_pressure_raw, 0.0, 5.0),
        "event_count": 1.0,
    }
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    wick_stack_reward = 0.12 if metrics["adverse_wick_stack_pressure"] > 0.24 and terminal > 0 else 0.0
    followthrough_failure_reward = 0.12 if metrics["body_followthrough_failure"] > 0.06 and terminal > 0 else 0.0
    failed_extreme_reward = 0.12 if metrics["failed_extreme_progress"] > 0.55 and terminal > 0 else 0.0
    range_rejection_reward = (
        0.08 if metrics["range_expansion_without_close_progress"] > 0.10 and terminal > 0 else 0.0
    )
    settlement_reward = 0.14 if metrics["reversal_settlement"] > 0.62 and terminal > 0 else 0.0
    density_reward = 0.08 if metrics["wick_pressure_density"] > 0.40 and terminal > 0 else 0.0
    volume_reward = 0.06 if metrics["volume_tail_engagement"] > 0.10 and terminal > 0 else 0.0
    vwap_reward = 0.07 if metrics["vwap_reversion_pressure"] > 0.08 and terminal > 0 else 0.0
    failed_reversal_penalty = 0.18 if metrics["reversal_settlement"] < 0.48 and terminal <= 0 else 0.0
    wick_trap_penalty = 0.12 if metrics["adverse_wick_stack_pressure"] > 0.28 and terminal <= 0 else 0.0
    body_followthrough_penalty = 0.10 if metrics["body_followthrough_failure"] < -0.08 and terminal <= 0 else 0.0
    weak_volume_penalty = 0.06 if metrics["volume_tail_engagement"] < -0.30 and terminal <= 0 else 0.0
    vwap_continuation_penalty = 0.08 if metrics["vwap_reversion_pressure"] < -0.18 and terminal <= 0 else 0.0
    ...
    wick_stack_pressure_reversal_path_quality = (0.54 * terminal + 0.30 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + wick_stack_pressure_reversal_path_quality
        + wick_stack_reward
        + followthrough_failure_reward
        + failed_extreme_reward
        + range_rejection_reward
        + settlement_reward
        + density_reward
        + volume_reward
...
def fit_linear_wick_stack_pressure_reversal_model(candidates: list[base.Candidate], fold_id: str) -> LinearWickStackPressureReversalModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearWickStackPressureReversalModel(
            fold_id,
    ...
    wick_stack_pressure_reversal_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearWickStackPressureReversalModel(
        fold_id=fold_id,
        ...
        wick_stack_pressure_reversal_failure_rate=wick_stack_pressure_reversal_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearWickStackPressureReversalModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_wick_stack_pressure_reversal_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0096_r0001_intraday_wick_stack_pressure_reversal",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0096_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0096_r0001_wick_stack_pressure_reversal_summary.json",
        ],
        ...
        "conditional_profiles": {
            "wick_stack_pressure_reversal_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0096 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0096_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0096 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "wick_stack_pressure_reversal_lookback_bars": WICK_STACK_PRESSURE_LOOKBACK_BARS,
        "wick_stack_pressure_reversal_stack_bars": WICK_STACK_PRESSURE_STACK_BARS,
        "min_wick_stack_pressure_reversal_activity": MIN_WICK_STACK_PRESSURE_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_wick_stack_pressure_reversal_model_summary(model: LinearWickStackPressureReversalModel) -> dict[str, object]:
    return {
        ...
        "wick_stack_pressure_reversal_failure_rate": rounded(model.wick_stack_pressure_reversal_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearWickStackPressureReversalModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_wick_stack_pressure_reversal_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "wick_stack_pressure_reversal_profile": profiles["wick_stack_pressure_reversal_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0096_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0096_r0001_proxy_trades.csv"
    evidence["wick_stack_pressure_reversal_summary"] = "artifacts/c0096_r0001_wick_stack_pressure_reversal_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0096_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0096_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0096 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0096_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0096_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0096_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0096_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0096_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0096_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0096_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0096_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0096_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0096 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0096 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0096_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0096_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0096_r0001_intraday_wick_stack_pressure_reversal_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0096_r0001_proxy_records_wick_stack_pressure_reversal_reference_surface
  ...
  - next_work_is_produce_c0096_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0096_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0096-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0096_r0001_intraday_wick_stack_pressure_reversal",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0096-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0096-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0096-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0096_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0096_r0001_intraday_wick_stack_pressure_reversal",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0096-R0001-wick-stack-pressure-reversal-SUMMARY",
            "artifact_role": "wick_stack_pressure_reversal_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0096_r0001_wick_stack_pressure_reversal_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0096_r0001_intraday_wick_stack_pressure_reversal",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0096_r0001_mt5_logic_parity_evidence",
        }
    ]
