"""C0112 R0001 proxy evidence for intraday micro consolidation breakout continuation."""
...
CAMPAIGN_ID = "C0112"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0112_intraday_micro_consolidation_breakout_continuation_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0112_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0112_r0001_micro_consolidation_breakout_continuation_summary.json"
...
FEATURE_NAMES = (
    "impulse_efficiency",
    "digestion_compression",
    "range_retention",
    "adverse_overlap_control",
    "volume_cooldown",
    "absorption_balance",
    "close_hold",
    "vwap_hold",
    "prior_impulse_context",
    "dwell_ratio",
    "touch_density",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_micro_consolidation_breakout_continuation_rank"
LABEL_SHAPE = "target_first_quality_after_micro_consolidation_breakout_continuation"
SELECTION_RULE = "top_fold_local_micro_consolidation_breakout_continuation_scores_per_active_day"
...
MIN_DAY_BARS = 18
MICRO_CONSOLIDATION_BREAKOUT_LOOKBACK_BARS = 42
MICRO_CONSOLIDATION_DIGESTION_BARS = 8
MICRO_CONSOLIDATION_IMPULSE_BARS = 14
MIN_IMPULSE_EFFICIENCY = 0.16
MIN_MICRO_CONSOLIDATION_BREAKOUT_ACTIVITY = 0.05
...
class LinearMicroConsolidationBreakoutContinuationModel:
    fold_id: str
    ...
    micro_consolidation_breakout_continuation_failure_rate: float
...
def run_c0112_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_micro_consolidation_breakout_continuation_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_micro_consolidation_breakout_continuation_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        MICRO_CONSOLIDATION_BREAKOUT_LOOKBACK_BARS,
        MICRO_CONSOLIDATION_DIGESTION_BARS + MICRO_CONSOLIDATION_IMPULSE_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = micro_consolidation_breakout_continuation_metrics(
    ...
    features = (
        metrics["impulse_efficiency"],
        metrics["digestion_compression"],
        metrics["range_retention"],
        metrics["adverse_overlap_control"],
        metrics["volume_cooldown"],
        metrics["absorption_balance"],
        metrics["close_hold"],
        metrics["vwap_hold"],
        metrics["prior_impulse_context"],
        metrics["dwell_ratio"],
        metrics["touch_density"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        impulse_efficiency_bucket(metrics["impulse_efficiency"]),
        digestion_compression_bucket(metrics["digestion_compression"]),
        retention_bucket(metrics["range_retention"]),
        adverse_overlap_bucket(metrics["adverse_overlap_control"]),
        volume_cooldown_bucket(metrics["volume_cooldown"]),
        absorption_balance_bucket(metrics["absorption_balance"]),
        close_hold_bucket(metrics["close_hold"]),
        vwap_hold_bucket(metrics["vwap_hold"]),
        prior_impulse_context_bucket(metrics["prior_impulse_context"]),
        dwell_ratio_bucket(metrics["dwell_ratio"]),
        touch_density_bucket(metrics["touch_density"]),
...
def micro_consolidation_breakout_continuation_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    digestion_start = max(index - MICRO_CONSOLIDATION_DIGESTION_BARS + 1, context.day_start_index)
    impulse_start = max(digestion_start - MICRO_CONSOLIDATION_IMPULSE_BARS, context.day_start_index)
    lookback_start = max(index - MICRO_CONSOLIDATION_BREAKOUT_LOOKBACK_BARS + 1, context.day_start_index)
    if impulse_start < 0 or digestion_start < 0 or lookback_start < 0:
        return {"event_count": 0.0}
    impulse_window = bars[impulse_start:digestion_start]
    digestion_window = bars[digestion_start : index + 1]
    lookback_window = bars[lookback_start : index + 1]
    day_window = bars[context.day_start_index : index + 1]
    if len(impulse_window) < 10 or len(digestion_window) < 6 or len(lookback_window) < 24 or len(day_window) < MIN_DAY_BARS:
        return {"event_count": 0.0}
    average_volume = volume_average[index]
    if average_volume is None or average_volume <= 0:
        return {"event_count": 0.0}

    digestion_ranges = [bar.high - bar.low for bar in digestion_window]
    impulse_ranges = [bar.high - bar.low for bar in impulse_window]
    lookback_ranges = [bar.high - bar.low for bar in lookback_window]
    digestion_range_mean = base.mean(digestion_ranges) or average_range
    impulse_range_mean = base.mean(impulse_ranges) or average_range
    lookback_range_mean = base.mean(lookback_ranges) or average_range
    impulse_volume_mean = base.mean(volumes[impulse_start:digestion_start]) or average_volume
    digestion_volume_mean = base.mean(volumes[digestion_start : index + 1]) or average_volume
    lookback_volume_mean = base.mean(volumes[lookback_start : index + 1]) or average_volume
    if digestion_range_mean <= 0 or impulse_range_mean <= 0 or lookback_range_mean <= 0:
        return {"event_count": 0.0}

    session_phase_maturity = bounded_session_progress(signal_bar.time)
    if session_phase_maturity < 0.10 or session_phase_maturity > 0.84:
        return {"event_count": 0.0}

    day_range_so_far = max(context.high_so_far - context.low_so_far, average_range * 0.25)
    digestion_high = max(bar.high for bar in digestion_window)
    digestion_low = min(bar.low for bar in digestion_window)
    impulse_high = max(bar.high for bar in impulse_window)
    impulse_low = min(bar.low for bar in impulse_window)
    impulse_travel = sum(
        abs(impulse_window[pos].close - impulse_window[pos - 1].close)
        for pos in range(1, len(impulse_window))
    )
    if impulse_travel <= 0:
        return {"event_count": 0.0}
    impulse_start_price = impulse_window[0].open
    impulse_terminal = impulse_window[-1].close
    impulse_net = direction * (impulse_terminal - impulse_start_price)
    impulse_efficiency = impulse_net / impulse_travel
    if impulse_efficiency < MIN_IMPULSE_EFFICIENCY:
        return {"event_count": 0.0}

    impulse_distance = max(abs(impulse_terminal - impulse_start_price), average_range * 0.50)
    if direction > 0:
        adverse_overlap = max(0.0, impulse_terminal - digestion_low) / impulse_distance
        range_retention = (signal_bar.close - impulse_start_price) / impulse_distance
        breakout_pressure = (signal_bar.close - digestion_high) / average_range
        side_touch_count = sum(1 for bar in digestion_window if bar.high >= digestion_high - 0.18 * average_range)
        dwell_count = sum(1 for bar in digestion_window if bar.close >= digestion_low + 0.58 * max(digestion_high - digestion_low, average_range * 0.15))
    else:
        adverse_overlap = max(0.0, digestion_high - impulse_terminal) / impulse_distance
        range_retention = (impulse_start_price - signal_bar.close) / impulse_distance
        breakout_pressure = (digestion_low - signal_bar.close) / average_range
        side_touch_count = sum(1 for bar in digestion_window if bar.low <= digestion_low + 0.18 * average_range)
        dwell_count = sum(1 for bar in digestion_window if bar.close <= digestion_high - 0.58 * max(digestion_high - digestion_low, average_range * 0.15))
    if range_retention < 0.22:
        return {"event_count": 0.0}

    digestion_width = digestion_high - digestion_low
    digestion_compression = 1.0 - digestion_range_mean / max(impulse_range_mean, average_range * 0.10)
    if digestion_width > max(average_range * 3.10, (impulse_high - impulse_low) * 1.45):
        return {"event_count": 0.0}
    spread_stress = signal_bar.spread_points / average_range
    spread_stress_inverse = 1.0 / (1.0 + max(spread_stress, 0.0))
    volume_cooldown = 1.0 - digestion_volume_mean / max(impulse_volume_mean, average_volume * 0.20)
    absorption_balance = base.mean([directional_wick_rejection(bar, direction) for bar in digestion_window]) or 0.0
    close_hold = directional_bar_settlement(signal_bar, direction)
    if close_hold < 0.40:
        return {"event_count": 0.0}
    vwap_hold = direction * (
        signal_bar.close - session_vwap_until(bars, volumes, context.day_start_index, index)
    ) / average_range
    if vwap_hold < -0.45:
        return {"event_count": 0.0}
    prior_impulse_context = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_impulse_context = direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
        prior_impulse_context += direction * (context.prior_day_shadow_balance or 0.0) * 0.12
    adverse_overlap_control = 1.0 - adverse_overlap
    dwell_ratio = dwell_count / max(len(digestion_window), 1)
    touch_density = side_touch_count / max(len(digestion_window), 1)
    if max(dwell_ratio, touch_density) < 0.18:
        return {"event_count": 0.0}

    activity = max(
        max(impulse_efficiency, 0.0) * 0.52,
        max(digestion_compression, 0.0) * 0.34,
        max(range_retention - 0.22, 0.0) * 0.34,
        max(adverse_overlap_control, 0.0) * 0.24,
        max(volume_cooldown, 0.0) * 0.18,
        max(absorption_balance, 0.0) * 0.20,
        max(close_hold - 0.40, 0.0) * 0.44,
        max(vwap_hold, 0.0) * 0.20,
        max(dwell_ratio - 0.18, 0.0) * 0.28,
        max(touch_density - 0.18, 0.0) * 0.28,
        max(breakout_pressure, 0.0) * 0.24,
    )
    if activity < MIN_MICRO_CONSOLIDATION_BREAKOUT_ACTIVITY:
        return {"event_count": 0.0}
    if digestion_compression < -0.55 or spread_stress_inverse < 0.35 or adverse_overlap_control < -0.50:
        return {"event_count": 0.0}
    return {
        "impulse_efficiency": bounded(impulse_efficiency, -1.0, 1.0),
        "digestion_compression": bounded(digestion_compression, -2.0, 2.0),
        "range_retention": bounded(range_retention, -2.0, 2.0),
        "adverse_overlap_control": bounded(adverse_overlap_control, -2.0, 1.0),
        "volume_cooldown": bounded(volume_cooldown, -3.0, 1.0),
        "absorption_balance": bounded(absorption_balance, -1.0, 1.0),
        "close_hold": bounded(close_hold, 0.0, 1.0),
        "vwap_hold": bounded(vwap_hold, -4.0, 4.0),
        "prior_impulse_context": bounded(prior_impulse_context, -2.0, 2.0),
        "dwell_ratio": bounded(dwell_ratio, 0.0, 1.0),
        "touch_density": bounded(touch_density, 0.0, 1.0),
        "session_phase_maturity": bounded(session_phase_maturity, 0.0, 1.0),
        "breakout_pressure": bounded(breakout_pressure, -4.0, 4.0),
        "digestion_range_ratio": bounded(digestion_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        "impulse_range_ratio": bounded(impulse_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        "lookback_range_ratio": bounded(lookback_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        "digestion_volume_ratio": bounded(digestion_volume_mean / max(average_volume, 1e-9), 0.0, 8.0),
        "impulse_volume_ratio": bounded(impulse_volume_mean / max(average_volume, 1e-9), 0.0, 8.0),
        "lookback_volume_ratio": bounded(lookback_volume_mean / max(average_volume, 1e-9), 0.0, 8.0),
        "day_range_so_far": day_range_so_far,
        "micro_consolidation_breakout_continuation_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0112_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0112_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0112_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0112_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0112_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0112_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0112_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0112_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0112_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0112_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0112_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0112_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0112_boundary_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0112_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0112_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0112_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0112_reclaim_bucket(value: float) -> str:
    if value < -0.05:
        return "boundary_not_reclaimed"
    if value < 0.16:
        return "boundary_reclaimed_thin"
    return "boundary_reclaimed_clear"
...
def c0112_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0112_prior_boundary_bucket(value: float) -> str:
    if value < -0.12:
        return "prior_boundary_against"
    if value < 0.18:
        return "prior_boundary_neutral"
    return "prior_boundary_supportive"


def c0112_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    impulse_reward = 0.12 if metrics["impulse_efficiency"] > 0.28 and terminal > 0 else 0.0
    compression_reward = 0.08 if metrics["digestion_compression"] > 0.12 and terminal > 0 else 0.0
    retention_reward = 0.08 if metrics["range_retention"] > 0.45 and terminal > 0 else 0.0
    overlap_reward = 0.06 if metrics["adverse_overlap_control"] > 0.35 and terminal > 0 else 0.0
    cooldown_reward = 0.05 if metrics["volume_cooldown"] > 0.06 and terminal > 0 else 0.0
    absorption_reward = 0.05 if metrics["absorption_balance"] > 0.05 and terminal > 0 else 0.0
    close_reward = 0.08 if metrics["close_hold"] > 0.62 and terminal > 0 else 0.0
    vwap_reward = 0.06 if metrics["vwap_hold"] > 0.02 and terminal > 0 else 0.0
    dwell_reward = 0.05 if metrics["dwell_ratio"] > 0.35 and terminal > 0 else 0.0
    touch_reward = 0.05 if metrics["touch_density"] > 0.35 and terminal > 0 else 0.0
    weak_impulse_penalty = 0.10 if metrics["impulse_efficiency"] < 0.20 and terminal <= 0 else 0.0
    expansion_penalty = 0.08 if metrics["digestion_compression"] < -0.12 and terminal <= 0 else 0.0
    retention_penalty = 0.08 if metrics["range_retention"] < 0.32 and terminal <= 0 else 0.0
    overlap_penalty = 0.08 if metrics["adverse_overlap_control"] < 0.05 and terminal <= 0 else 0.0
    close_penalty = 0.07 if metrics["close_hold"] < 0.50 and terminal <= 0 else 0.0
    vwap_penalty = 0.07 if metrics["vwap_hold"] < -0.10 and terminal <= 0 else 0.0
    dwell_penalty = 0.06 if metrics["dwell_ratio"] < 0.22 and terminal <= 0 else 0.0
    touch_penalty = 0.06 if metrics["touch_density"] < 0.22 and terminal <= 0 else 0.0
    ...
    micro_consolidation_breakout_continuation_path_quality = (0.62 * terminal + 0.28 * mfe - 0.65 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + micro_consolidation_breakout_continuation_path_quality
        + impulse_reward
        + compression_reward
        + retention_reward
        + overlap_reward
        ...
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["digestion_range_ratio"] - 1.90) * 0.06
        - max(0.0, metrics["digestion_volume_ratio"] - 3.20) * 0.02
        - max(0.0, 0.18 - metrics["dwell_ratio"]) * 0.03
    )


def fit_linear_micro_consolidation_breakout_continuation_model(candidates: list[base.Candidate], fold_id: str) -> LinearMicroConsolidationBreakoutContinuationModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearMicroConsolidationBreakoutContinuationModel(
            fold_id,
    ...
    micro_consolidation_breakout_continuation_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearMicroConsolidationBreakoutContinuationModel(
        fold_id=fold_id,
        ...
        micro_consolidation_breakout_continuation_failure_rate=micro_consolidation_breakout_continuation_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearMicroConsolidationBreakoutContinuationModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0112-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_micro_consolidation_breakout_continuation_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0112_r0001_intraday_micro_consolidation_breakout_continuation",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0112_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0112_r0001_micro_consolidation_breakout_continuation_summary.json",
        ],
        ...
        "conditional_profiles": {
            "micro_consolidation_breakout_continuation_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0112 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0112_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0112 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "micro_consolidation_breakout_continuation_lookback_bars": MICRO_CONSOLIDATION_BREAKOUT_LOOKBACK_BARS,
        "micro_consolidation_breakout_continuation_recent_bars": MICRO_CONSOLIDATION_DIGESTION_BARS,
        "micro_consolidation_breakout_continuation_prior_bars": MICRO_CONSOLIDATION_IMPULSE_BARS,
        "min_impulse_efficiency": MIN_IMPULSE_EFFICIENCY,
        "min_micro_consolidation_breakout_continuation_activity": MIN_MICRO_CONSOLIDATION_BREAKOUT_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_micro_consolidation_breakout_continuation_model_summary(model: LinearMicroConsolidationBreakoutContinuationModel) -> dict[str, object]:
    return {
        ...
        "micro_consolidation_breakout_continuation_failure_rate": rounded(model.micro_consolidation_breakout_continuation_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearMicroConsolidationBreakoutContinuationModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_micro_consolidation_breakout_continuation_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "micro_consolidation_breakout_continuation_profile": profiles["micro_consolidation_breakout_continuation_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0112_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0112_r0001_proxy_trades.csv"
    evidence["micro_consolidation_breakout_continuation_summary"] = "artifacts/c0112_r0001_micro_consolidation_breakout_continuation_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0112_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0112_r0001_proxy_trades.csv",
        "artifacts/c0112_r0001_micro_consolidation_breakout_continuation_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0112_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0112 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0112_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0112_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0112_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0112_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0112_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0112_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0112_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0112_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0112_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0112 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0112 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0112_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0112_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0112_r0001_intraday_micro_consolidation_breakout_continuation_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0112_r0001_proxy_records_micro_consolidation_breakout_continuation_reference_surface
  ...
  - next_work_is_produce_c0112_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0112_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0112-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0112_r0001_intraday_micro_consolidation_breakout_continuation",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0112-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0112-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0112-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0112_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0112_r0001_intraday_micro_consolidation_breakout_continuation",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0112-R0001-MICRO-CONSOLIDATION-BREAKOUT-CONTINUATION-SUMMARY",
            "artifact_role": "micro_consolidation_breakout_continuation_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0112_r0001_micro_consolidation_breakout_continuation_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0112_r0001_intraday_micro_consolidation_breakout_continuation",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0112_r0001_mt5_logic_parity_evidence",
        }
    ]
