"""C0092 R0001 proxy evidence for intraday cost pressure absorption."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0092_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0092_r0001_cost_pressure_absorption_summary.json"
...
FEATURE_NAMES = (
    ...
    "spread_relief_intensity",
    "cost_absorption_drive",
    "close_after_cost_pressure",
    "volume_cost_confirmation",
    "spread_cluster_pressure",
    "price_efficiency_under_cost",
    "prior_cost_context",
    "session_phase_maturity",
    "cost_resolution_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_cost_pressure_absorption_rank"
LABEL_SHAPE = "target_first_escape_quality_conditioned_on_cost_pressure_absorption_state"
SELECTION_RULE = "top_fold_local_cost_pressure_absorption_scores_per_active_day"
...
MIN_DAY_BARS = 30
COST_PRESSURE_ABSORPTION_LOOKBACK_BARS = 34
COST_PRESSURE_ABSORPTION_SHORT_LOOKBACK_BARS = 8
MIN_COST_PRESSURE_ABSORPTION_ACTIVITY = 0.10
...
class LinearCostPressureAbsorptionModel:
    fold_id: str
    ...
    cost_pressure_absorption_failure_rate: float
...
def run_c0092_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_cost_pressure_absorption_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_cost_pressure_absorption_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        COST_PRESSURE_ABSORPTION_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = cost_pressure_absorption_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        metrics["spread_pressure_ratio"],
        metrics["spread_relief_intensity"],
        metrics["cost_absorption_drive"],
        metrics["close_after_cost_pressure"],
        metrics["volume_cost_confirmation"],
        metrics["spread_cluster_pressure"],
        metrics["price_efficiency_under_cost"],
        metrics["prior_cost_context"],
        metrics["session_phase_maturity"],
        metrics["cost_resolution_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        spread_pressure_ratio_bucket(metrics["spread_pressure_ratio"]),
        spread_relief_intensity_bucket(metrics["spread_relief_intensity"]),
        cost_absorption_drive_bucket(metrics["cost_absorption_drive"]),
        close_after_cost_pressure_bucket(metrics["close_after_cost_pressure"]),
        volume_cost_confirmation_bucket(metrics["volume_cost_confirmation"]),
        spread_cluster_pressure_bucket(metrics["spread_cluster_pressure"]),
        price_efficiency_under_cost_bucket(metrics["price_efficiency_under_cost"]),
        prior_cost_context_bucket(metrics["prior_cost_context"]),
        session_phase_maturity_bucket(metrics["session_phase_maturity"]),
        cost_resolution_pressure_bucket(metrics["cost_resolution_pressure"]),
...
def cost_pressure_absorption_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    short_start = index - COST_PRESSURE_ABSORPTION_SHORT_LOOKBACK_BARS + 1
    prior_start = index - COST_PRESSURE_ABSORPTION_LOOKBACK_BARS + 1
    if short_start < 0 or prior_start < 0 or context.day_start_index >= index:
        return {"event_count": 0.0}
    prior_short_start = short_start - COST_PRESSURE_ABSORPTION_SHORT_LOOKBACK_BARS
    ...
    prior_spreads = [max(bar.spread_points, 0.0) / average_range for bar in prior_window]
    short_spreads = [max(bar.spread_points, 0.0) / average_range for bar in short_window]
    current_spread = max(signal_bar.spread_points, 0.0) / average_range
    prior_spread_average = base.mean(prior_spreads) or current_spread
    short_spread_average = base.mean(short_spreads) or current_spread
    spread_pressure_ratio = short_spread_average / max(prior_spread_average, 1e-9)
    spread_relief_intensity = (short_spread_average - current_spread) / max(prior_spread_average, 1e-9)
    cost_absorption_drive = direction * (signal_bar.close - signal_bar.open) / signal_range
    close_after_cost_pressure = directional_bar_settlement(signal_bar, direction)
    price_efficiency_under_cost = terminal_move / max(path_total, average_range * 0.25)
    path_efficiency = directional_path_efficiency(
        bars,
        index,
        direction,
        COST_PRESSURE_ABSORPTION_SHORT_LOOKBACK_BARS,
    )
    if path_efficiency is None:
        return {"event_count": 0.0}
    spread_cluster_pressure = sum(1 for value in short_spreads if value > prior_spread_average * 1.08) / max(
        len(short_spreads),
        1,
    )
    ...
    volume_cost_confirmation = (current_volume_ratio - early_volume_ratio) * max(cost_absorption_drive, 0.0)
    prior_cost_context = prior_spread_average
    session_phase_maturity = bounded_session_progress(signal_bar.time)
    cost_resolution_pressure_raw = max(
        max(spread_pressure_ratio - 1.0, 0.0),
        max(spread_relief_intensity, 0.0),
        max(cost_absorption_drive, 0.0),
        max(close_after_cost_pressure - 0.45, 0.0) * 2.0,
        min(max(volume_cost_confirmation, 0.0), 1.5),
        max(price_efficiency_under_cost, 0.0),
        max(path_efficiency, 0.0),
        min(max(spread_cluster_pressure - 0.12, 0.0), 1.0),
    )
    cost_resolution_pressure = session_phase_maturity * cost_resolution_pressure_raw
    activity = max(cost_resolution_pressure_raw, cost_resolution_pressure)
    if activity < MIN_COST_PRESSURE_ABSORPTION_ACTIVITY:
        return {"event_count": 0.0}
    if spread_pressure_ratio < 0.92 and spread_cluster_pressure < 0.18:
        return {"event_count": 0.0}
    if spread_relief_intensity < -0.08 and close_after_cost_pressure < 0.42:
        return {"event_count": 0.0}
    if cost_absorption_drive < -0.24 and close_after_cost_pressure < 0.42:
        return {"event_count": 0.0}
    return {
        "spread_pressure_ratio": bounded(spread_pressure_ratio, 0.0, 6.0),
        "spread_relief_intensity": bounded(spread_relief_intensity, -3.0, 3.0),
        "cost_absorption_drive": bounded(cost_absorption_drive, -1.0, 1.0),
        "close_after_cost_pressure": bounded(close_after_cost_pressure, 0.0, 1.0),
        "volume_cost_confirmation": bounded(volume_cost_confirmation, -3.0, 6.0),
        "spread_cluster_pressure": bounded(spread_cluster_pressure, 0.0, 1.0),
        "price_efficiency_under_cost": bounded((price_efficiency_under_cost + path_efficiency) * 0.5, -1.0, 1.0),
        "prior_cost_context": bounded(prior_cost_context, 0.0, 1.0),
        "session_phase_maturity": bounded(session_phase_maturity, 0.0, 1.0),
        "cost_resolution_pressure": bounded(cost_resolution_pressure, 0.0, 4.0),
        "event_count": 1.0,
    }
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    pressure_reward = 0.10 if metrics["spread_pressure_ratio"] > 1.05 and terminal > 0 else 0.0
    relief_reward = 0.14 if metrics["spread_relief_intensity"] > 0.10 and terminal > 0 else 0.0
    drive_reward = 0.12 if metrics["cost_absorption_drive"] > 0.22 and terminal > 0 else 0.0
    close_reward = 0.12 if metrics["close_after_cost_pressure"] > 0.60 and terminal > 0 else 0.0
    volume_reward = 0.08 if metrics["volume_cost_confirmation"] > 0.08 and terminal > 0 else 0.0
    efficiency_reward = 0.10 if metrics["price_efficiency_under_cost"] > 0.12 and terminal > 0 else 0.0
    resolution_reward = 0.06 if metrics["cost_resolution_pressure"] > 0.12 and terminal > 0 else 0.0
    no_relief_penalty = 0.12 if metrics["spread_relief_intensity"] < -0.02 and terminal <= 0 else 0.0
    heavy_cluster_penalty = 0.10 if metrics["spread_cluster_pressure"] > 0.55 and terminal <= 0 else 0.0
    weak_close_penalty = 0.10 if metrics["close_after_cost_pressure"] < 0.45 and terminal <= 0 else 0.0
    adverse_drive_penalty = 0.10 if metrics["cost_absorption_drive"] < -0.08 and terminal <= 0 else 0.0
    poor_efficiency_penalty = 0.08 if metrics["price_efficiency_under_cost"] < -0.06 and terminal <= 0 else 0.0
    no_volume_confirmation_penalty = 0.08 if metrics["volume_cost_confirmation"] < -0.05 and terminal <= 0 else 0.0
    ...
    cost_pressure_absorption_path_quality = (0.54 * terminal + 0.30 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + cost_pressure_absorption_path_quality
        + pressure_reward
        + relief_reward
        + drive_reward
        + close_reward
        ...
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["prior_cost_context"] - 0.18) * 0.12
    )


def fit_linear_cost_pressure_absorption_model(candidates: list[base.Candidate], fold_id: str) -> LinearCostPressureAbsorptionModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearCostPressureAbsorptionModel(
            fold_id,
    ...
    cost_pressure_absorption_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearCostPressureAbsorptionModel(
        fold_id=fold_id,
        ...
        cost_pressure_absorption_failure_rate=cost_pressure_absorption_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearCostPressureAbsorptionModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_cost_pressure_absorption_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0092_r0001_intraday_cost_pressure_absorption",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0092_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0092_r0001_cost_pressure_absorption_summary.json",
        ],
        ...
        "conditional_profiles": {
            "cost_pressure_absorption_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0092 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0092_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0092 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "cost_pressure_absorption_lookback_bars": COST_PRESSURE_ABSORPTION_LOOKBACK_BARS,
        "cost_pressure_absorption_short_lookback_bars": COST_PRESSURE_ABSORPTION_SHORT_LOOKBACK_BARS,
        "min_cost_pressure_absorption_activity": MIN_COST_PRESSURE_ABSORPTION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_cost_pressure_absorption_model_summary(model: LinearCostPressureAbsorptionModel) -> dict[str, object]:
    return {
        ...
        "cost_pressure_absorption_failure_rate": rounded(model.cost_pressure_absorption_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearCostPressureAbsorptionModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_cost_pressure_absorption_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "cost_pressure_absorption_profile": profiles["cost_pressure_absorption_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0092_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0092_r0001_proxy_trades.csv"
    evidence["cost_pressure_absorption_summary"] = "artifacts/c0092_r0001_cost_pressure_absorption_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0092_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0092_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0092 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0092_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0092_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0092_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0092_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0092_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0092_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0092_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0092_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0092_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0092 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0092 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0092_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0092_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0092_r0001_intraday_cost_pressure_absorption_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0092_r0001_proxy_records_cost_pressure_absorption_reference_surface
  ...
  - next_work_is_produce_c0092_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0092_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0092-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0092_r0001_intraday_cost_pressure_absorption",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0092-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0092-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0092-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0092_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0092_r0001_intraday_cost_pressure_absorption",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0092-R0001-cost-pressure-absorption-SUMMARY",
            "artifact_role": "cost_pressure_absorption_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0092_r0001_cost_pressure_absorption_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0092_r0001_intraday_cost_pressure_absorption",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0092_r0001_mt5_logic_parity_evidence",
        }
    ]
