"""C0114 R0001 proxy evidence for intraday momentum volume divergence reversal."""
...
CAMPAIGN_ID = "C0114"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0114_intraday_momentum_volume_divergence_reversal_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0114_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0114_r0001_momentum_volume_divergence_reversal_summary.json"
...
FEATURE_NAMES = (
    ...
    "volume_response_gap",
    "body_efficiency_gap",
    "close_location_decay",
    "range_efficiency_decay",
    "micro_slope_deceleration",
    "counter_wick_pressure",
    "vwap_stretch_pressure",
    "prior_extension_burden",
    "divergence_consensus",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_momentum_volume_divergence_reversal_rank"
LABEL_SHAPE = "target_first_quality_after_momentum_volume_divergence_reversal"
SELECTION_RULE = "top_fold_local_momentum_volume_divergence_reversal_scores_per_active_day"
...
MIN_DAY_BARS = 18
MOMENTUM_VOLUME_DIVERGENCE_REVERSAL_LOOKBACK_BARS = 42
DIVERGENCE_RECENT_BARS = 8
DIVERGENCE_BASE_BARS = 14
MIN_EXTENSION_PRESSURE = 0.55
MIN_MOMENTUM_VOLUME_DIVERGENCE_REVERSAL_ACTIVITY = 0.055
...
class LinearMomentumVolumeDivergenceReversalModel:
    fold_id: str
    ...
    momentum_volume_divergence_reversal_failure_rate: float
...
def run_c0114_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_momentum_volume_divergence_reversal_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_momentum_volume_divergence_reversal_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        MOMENTUM_VOLUME_DIVERGENCE_REVERSAL_LOOKBACK_BARS,
        DIVERGENCE_RECENT_BARS + DIVERGENCE_BASE_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = momentum_volume_divergence_reversal_metrics(
    ...
    features = (
        metrics["extension_pressure"],
        metrics["volume_response_gap"],
        metrics["body_efficiency_gap"],
        metrics["close_location_decay"],
        metrics["range_efficiency_decay"],
        metrics["micro_slope_deceleration"],
        metrics["counter_wick_pressure"],
        metrics["vwap_stretch_pressure"],
        metrics["prior_extension_burden"],
        metrics["divergence_consensus"],
        metrics["session_maturity"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        extension_pressure_bucket(metrics["extension_pressure"]),
        volume_response_gap_bucket(metrics["volume_response_gap"]),
        body_efficiency_gap_bucket(metrics["body_efficiency_gap"]),
        close_location_decay_bucket(metrics["close_location_decay"]),
        range_efficiency_decay_bucket(metrics["range_efficiency_decay"]),
        micro_slope_deceleration_bucket(metrics["micro_slope_deceleration"]),
        counter_wick_pressure_bucket(metrics["counter_wick_pressure"]),
        vwap_stretch_pressure_bucket(metrics["vwap_stretch_pressure"]),
        prior_extension_burden_bucket(metrics["prior_extension_burden"]),
        divergence_consensus_bucket(metrics["divergence_consensus"]),
...
def momentum_volume_divergence_reversal_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    recent_start = max(index - DIVERGENCE_RECENT_BARS + 1, context.day_start_index)
    base_start = max(recent_start - DIVERGENCE_BASE_BARS, context.day_start_index)
    lookback_start = max(index - MOMENTUM_VOLUME_DIVERGENCE_REVERSAL_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    if len(base_window) < 10 or len(recent_window) < 6 or len(lookback_window) < 24 or len(day_window) < MIN_DAY_BARS:
        return {"event_count": 0.0}
    ...
    base_start_close = base_window[0].close
    extension_pressure = extension_direction * (signal_bar.close - base_start_close) / average_range
    if extension_pressure < MIN_EXTENSION_PRESSURE:
        return {"event_count": 0.0}

    base_body_alignment = base.mean([directional_body_score(bar, extension_direction) for bar in base_window]) or 0.0
    recent_body_alignment = base.mean([directional_body_score(bar, extension_direction) for bar in recent_window]) or 0.0
    body_efficiency_gap = base_body_alignment - recent_body_alignment
    volume_response_gap = 1.0 - recent_volume_mean / max(base_volume_mean, average_volume * 0.25)
    extension_settlement = directional_bar_settlement(signal_bar, extension_direction)
    close_location_decay = 1.0 - extension_settlement

    recent_net = extension_direction * (recent_window[-1].close - recent_window[0].close)
    recent_efficiency = recent_net / max(close_travel, average_range * 0.10)
    range_efficiency_decay = 1.0 - recent_efficiency
    base_net = extension_direction * (base_window[-1].close - base_window[0].close)
    base_slope = base_net / max(len(base_window), 1) / average_range
    recent_slope = recent_net / max(len(recent_window), 1) / average_range
    micro_slope_deceleration = base_slope - recent_slope
    counter_wick_pressure = directional_wick_rejection(signal_bar, direction)
    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    vwap_stretch_pressure = extension_direction * (signal_bar.close - session_vwap) / average_range
    prior_extension_burden = 0.0
    if context.prior_day_close is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_extension_burden = extension_direction * (
            signal_bar.close - context.prior_day_close
        ) / context.prior_day_range
        prior_extension_burden += extension_direction * (context.prior_day_shadow_balance or 0.0) * 0.08
    ...
    divergence_components = [
        max(volume_response_gap, 0.0),
        max(body_efficiency_gap, 0.0),
        max(close_location_decay - 0.30, 0.0),
        max(range_efficiency_decay - 0.40, 0.0),
        max(micro_slope_deceleration, 0.0),
        max(counter_wick_pressure, 0.0),
        max(vwap_stretch_pressure - 0.20, 0.0),
    ]
    divergence_consensus = sum(divergence_components) / len(divergence_components)
    activity = max(
        max(extension_pressure - 0.45, 0.0) * 0.32,
        max(volume_response_gap, 0.0) * 0.30,
        max(body_efficiency_gap, 0.0) * 0.30,
        max(close_location_decay - 0.25, 0.0) * 0.24,
        max(range_efficiency_decay - 0.25, 0.0) * 0.20,
        max(micro_slope_deceleration, 0.0) * 0.55,
        max(counter_wick_pressure, 0.0) * 0.24,
        max(vwap_stretch_pressure - 0.15, 0.0) * 0.16,
        max(divergence_consensus, 0.0) * 0.55,
    )
    if activity < MIN_MOMENTUM_VOLUME_DIVERGENCE_REVERSAL_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "extension_pressure": bounded(extension_pressure, -4.0, 8.0),
        "volume_response_gap": bounded(volume_response_gap, -4.0, 4.0),
        "body_efficiency_gap": bounded(body_efficiency_gap, -2.0, 2.0),
        "close_location_decay": bounded(close_location_decay, 0.0, 1.0),
        "range_efficiency_decay": bounded(range_efficiency_decay, -2.0, 3.0),
        "micro_slope_deceleration": bounded(micro_slope_deceleration, -2.0, 2.0),
        "counter_wick_pressure": bounded(counter_wick_pressure, -1.0, 1.0),
        "vwap_stretch_pressure": bounded(vwap_stretch_pressure, -4.0, 8.0),
        "prior_extension_burden": bounded(prior_extension_burden, -4.0, 4.0),
        "divergence_consensus": bounded(divergence_consensus, 0.0, 4.0),
        ...
        "momentum_volume_divergence_reversal_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0114_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0114_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0114_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0114_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0114_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0114_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0114_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0114_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0114_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0114_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0114_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0114_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0114_boundary_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0114_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0114_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0114_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0114_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0114_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0114_prior_boundary_bucket(value: float) -> str:
    if value < -0.12:
...
def c0114_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    extension_reward = 0.08 if metrics["extension_pressure"] > 0.90 and terminal > 0 else 0.0
    volume_gap_reward = 0.12 if metrics["volume_response_gap"] > 0.12 and terminal > 0 else 0.0
    body_gap_reward = 0.10 if metrics["body_efficiency_gap"] > 0.12 and terminal > 0 else 0.0
    close_decay_reward = 0.08 if metrics["close_location_decay"] > 0.42 and terminal > 0 else 0.0
    range_decay_reward = 0.07 if metrics["range_efficiency_decay"] > 0.48 and terminal > 0 else 0.0
    slope_reward = 0.07 if metrics["micro_slope_deceleration"] > 0.02 and terminal > 0 else 0.0
    wick_reward = 0.08 if metrics["counter_wick_pressure"] > 0.10 and terminal > 0 else 0.0
    vwap_reward = 0.06 if metrics["vwap_stretch_pressure"] > 0.35 and terminal > 0 else 0.0
    consensus_reward = 0.14 if metrics["divergence_consensus"] > 0.18 and terminal > 0 else 0.0
    weak_extension_penalty = 0.08 if metrics["extension_pressure"] < 0.70 and terminal <= 0 else 0.0
    volume_confirm_penalty = 0.08 if metrics["volume_response_gap"] < -0.05 and terminal <= 0 else 0.0
    body_confirm_penalty = 0.08 if metrics["body_efficiency_gap"] < -0.05 and terminal <= 0 else 0.0
    close_follow_penalty = 0.07 if metrics["close_location_decay"] < 0.22 and terminal <= 0 else 0.0
    wick_failure_penalty = 0.07 if metrics["counter_wick_pressure"] < -0.08 and terminal <= 0 else 0.0
    consensus_penalty = 0.12 if metrics["divergence_consensus"] < 0.08 and terminal <= 0 else 0.0
    ...
    momentum_volume_divergence_reversal_path_quality = (0.62 * terminal + 0.28 * mfe - 0.65 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + momentum_volume_divergence_reversal_path_quality
        + extension_reward
        + volume_gap_reward
        + body_gap_reward
        + close_decay_reward
        ...
        - body_confirm_penalty
        - close_follow_penalty
        - wick_failure_penalty
        - consensus_penalty
        - stall_penalty
        ...
        - max(0.0, 0.08 - metrics["divergence_consensus"]) * 0.10
    )


def fit_linear_momentum_volume_divergence_reversal_model(candidates: list[base.Candidate], fold_id: str) -> LinearMomentumVolumeDivergenceReversalModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearMomentumVolumeDivergenceReversalModel(
            fold_id,
    ...
    momentum_volume_divergence_reversal_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearMomentumVolumeDivergenceReversalModel(
        fold_id=fold_id,
        ...
        momentum_volume_divergence_reversal_failure_rate=momentum_volume_divergence_reversal_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearMomentumVolumeDivergenceReversalModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0114-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_momentum_volume_divergence_reversal_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0114_r0001_intraday_momentum_volume_divergence_reversal",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0114_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0114_r0001_momentum_volume_divergence_reversal_summary.json",
        ],
        ...
        "conditional_profiles": {
            "momentum_volume_divergence_reversal_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0114 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0114_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0114 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "momentum_volume_divergence_reversal_lookback_bars": MOMENTUM_VOLUME_DIVERGENCE_REVERSAL_LOOKBACK_BARS,
        "momentum_volume_divergence_reversal_recent_bars": DIVERGENCE_RECENT_BARS,
        "momentum_volume_divergence_reversal_prior_bars": DIVERGENCE_BASE_BARS,
        "min_extension_pressure": MIN_EXTENSION_PRESSURE,
        "min_momentum_volume_divergence_reversal_activity": MIN_MOMENTUM_VOLUME_DIVERGENCE_REVERSAL_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_momentum_volume_divergence_reversal_model_summary(model: LinearMomentumVolumeDivergenceReversalModel) -> dict[str, object]:
    return {
        ...
        "momentum_volume_divergence_reversal_failure_rate": rounded(model.momentum_volume_divergence_reversal_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearMomentumVolumeDivergenceReversalModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_momentum_volume_divergence_reversal_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "momentum_volume_divergence_reversal_profile": profiles["momentum_volume_divergence_reversal_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0114_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0114_r0001_proxy_trades.csv"
    evidence["momentum_volume_divergence_reversal_summary"] = "artifacts/c0114_r0001_momentum_volume_divergence_reversal_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0114_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0114_r0001_proxy_trades.csv",
        "artifacts/c0114_r0001_momentum_volume_divergence_reversal_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0114_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0114 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0114_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0114_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0114_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0114_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0114_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0114_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0114_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0114_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0114_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0114 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0114 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0114_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0114_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0114_r0001_intraday_momentum_volume_divergence_reversal_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0114_r0001_proxy_records_momentum_volume_divergence_reversal_reference_surface
  ...
  - next_work_is_produce_c0114_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0114_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0114-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0114_r0001_intraday_momentum_volume_divergence_reversal",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0114-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0114-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0114-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0114_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0114_r0001_intraday_momentum_volume_divergence_reversal",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0114-R0001-MOMENTUM-VOLUME-DIVERGENCE-REVERSAL-SUMMARY",
            "artifact_role": "momentum_volume_divergence_reversal_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0114_r0001_momentum_volume_divergence_reversal_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0114_r0001_intraday_momentum_volume_divergence_reversal",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0114_r0001_mt5_logic_parity_evidence",
        }
    ]
