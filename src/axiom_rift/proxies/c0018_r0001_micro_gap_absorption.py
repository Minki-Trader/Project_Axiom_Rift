"""C0018 R0001 proxy evidence for micro-gap absorption discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0018_micro_gap_absorption_discovery" / "runs" / "R0001"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0018_micro_gap_absorption_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0018_r0001_proxy_trades.csv"
MICRO_GAP_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0018_r0001_micro_gap_absorption_summary.json"
...
FEATURE_NAMES = (
    "directional_gap_1_over_range",
    "opposite_gap_1_over_range",
    "gap_abs_size_over_range",
    "gap_fill_fraction_current",
    "gap_reclaim_close_over_range",
    "gap_breach_over_range",
    "gap_followthrough_1_over_range",
    "approach_into_gap_3_over_range",
    "approach_into_gap_6_over_range",
    "approach_into_gap_12_over_range",
    "consecutive_gap_pressure_6",
    "gap_same_direction_count_12",
    "gap_opposite_direction_count_12",
    "gap_cross_count_12",
    "time_since_gap_anchor_touch_72",
    "directional_body_fraction",
    "directional_close_location",
    "gap_rejection_wick_fraction",
    "adverse_wick_fraction",
    "range_expansion_3_over_36",
    "range_compression_12_over_96",
    "day_gap_anchor_position",
    "spread_over_range",
    "spread_pressure_48",
...
MODEL_FAMILY = "fold_local_micro_gap_absorption_rank"
LABEL_SHAPE = "directional_micro_gap_absorption_followthrough_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_micro_gap_absorption_scores_per_active_day"
...
def run_c0018_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = micro_gap_absorption_features(
                bars,
                range_average,
                short_range_average,
                day_context,
            ...
            if features is None:
                continue
            label = (
                micro_gap_absorption_label(bars, range_average, index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|micro_gap_absorption",
                    features=features,
                    label=label,
                )
            )
...
def micro_gap_absorption_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    gap = bar.open - anchor
    abs_gap = abs(gap)
    directional_gap = max(-direction * gap, 0.0) / average_range
    opposite_gap = max(direction * gap, 0.0) / average_range
    gap_abs_size = abs_gap / average_range
    gap_fill_fraction = bounded(direction * (bar.close - bar.open) / max(abs_gap, 1e-9), -3.0, 3.0)
    gap_reclaim_close = direction * (bar.close - anchor) / average_range
    if direction > 0:
        gap_breach = max(anchor - bar.low, 0.0) / average_range
        day_anchor_position = (anchor - context["day_low"]) / max(context["day_high"] - context["day_low"], 1e-9)
    else:
        gap_breach = max(bar.high - anchor, 0.0) / average_range
        day_anchor_position = (context["day_high"] - anchor) / max(context["day_high"] - context["day_low"], 1e-9)
    gap_followthrough = direction * (bar.close - bar.open) / average_range
    approach_3 = approach_into_gap(bars, index, direction, 3, average_range)
    approach_6 = approach_into_gap(bars, index, direction, 6, average_range)
    approach_12 = approach_into_gap(bars, index, direction, 12, average_range)
    consecutive_gap_pressure = directional_gap_pressure(bars, index, direction, 6, average_range)
    same_gap_count = directional_gap_count(bars, index, direction, 12, desired=True)
    opposite_gap_count = directional_gap_count(bars, index, direction, 12, desired=False)
    gap_cross_count_value = gap_cross_count(bars, index, 12)
    ...
    return (
        ...
        approach_6,
        approach_12,
        consecutive_gap_pressure,
        same_gap_count,
        opposite_gap_count,
        gap_cross_count_value,
        time_since_touch,
        body_fraction,
        directional_close_location,
...
def approach_into_gap(
    ...
    return -direction * (bars[index - 1].close - bars[index - lookback].close) / average_range


def gap_at(bars: list[base.Bar], index: int) -> float:
    return bars[index].open - bars[index - 1].close


def directional_gap_pressure(
    bars: list[base.Bar],
    index: int,
    direction: int,
    lookback: int,
    ...
    for offset in range(index - lookback + 1, index + 1):
        total += -direction * gap_at(bars, offset) / average_range
    return total / lookback


def directional_gap_count(
    bars: list[base.Bar],
    index: int,
    direction: int,
    lookback: int,
    ...
    count = 0
    for offset in range(index - lookback + 1, index + 1):
        signed_gap = -direction * gap_at(bars, offset)
        if desired and signed_gap > 0:
            count += 1
        if not desired and signed_gap < 0:
            count += 1
    return count / lookback


def gap_cross_count(bars: list[base.Bar], index: int, lookback: int) -> float:
    crosses = 0
    for offset in range(index - lookback + 1, index + 1):
        anchor = bars[offset - 1].close
        open_side = bars[offset].open - anchor
        close_side = bars[offset].close - anchor
        if open_side == 0.0 or close_side == 0.0 or open_side * close_side < 0.0:
            crosses += 1
...
def micro_gap_absorption_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    index: int,
    direction: int,
    ...
    if direction > 0:
        ...
        anchor_failure_depth = max(max(anchor - bar.low, 0.0) for bar in path) / average_range
        anchor_recross_count = sum(1 for bar in path if bar.close < anchor) / len(path)
        anchor_reclaim_finish = (path[-1].close - anchor) / average_range
    else:
        anchor_hold = sum(1 for bar in path if bar.close <= anchor) / len(path)
        anchor_failure_depth = max(max(bar.high - anchor, 0.0) for bar in path) / average_range
        anchor_recross_count = sum(1 for bar in path if bar.close > anchor) / len(path)
        anchor_reclaim_finish = (anchor - path[-1].close) / average_range
    return (
        0.82 * first_hit
        + 0.34 * target_speed
        - 0.46 * stop_speed
        ...
        - 0.34 * early_adverse
        + 0.28 * close_through
        + 0.44 * (anchor_hold - 0.5)
        + 0.22 * anchor_reclaim_finish
        - 0.50 * anchor_failure_depth
        - 0.30 * anchor_recross_count
        + 0.28 * favorable
        - 0.62 * adverse
        + 0.38 * terminal_norm
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0018"
    payload["work_unit_id"] = "C0018"
    payload["run_id"] = "R0001"
    payload["proxy_id"] = "PX-C0018-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0018_r0001_micro_gap_absorption"
    payload["proxy_config_path"] = "campaigns/C0018_micro_gap_absorption_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_micro_gap_absorption_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0018_micro_gap_absorption_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0018_micro_gap_absorption_discovery/runs/R0001/artifacts/c0018_r0001_proxy_trades.csv",
        "campaigns/C0018_micro_gap_absorption_discovery/runs/R0001/artifacts/c0018_r0001_micro_gap_absorption_summary.json",
    ]
    ...
    profiles["micro_gap_absorption_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0018_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "feature_weight_floor": FEATURE_WEIGHT_FLOOR,
            "feature_weight_ceiling": FEATURE_WEIGHT_CEILING,
            "score_interpretation": "higher_score_means_direct_fold_local_micro_gap_absorption_rank",
            "variant_boundary": "micro_gap_absorption_rank_not_auction_threshold_monthly_memory_lifecycle_structural_trap_score_floor_stop_target_hold_or_retry_nudge",
        }
    )
    return config
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_micro_gap_absorption_summary_artifact(payload, MICRO_GAP_SUMMARY_PATH)
    PROXY_PATH.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="ascii")
    trade_hash = base.sha256_file(TRADE_ARTIFACT_PATH)
    summary_hash = base.sha256_file(MICRO_GAP_SUMMARY_PATH)
...
def write_micro_gap_absorption_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_micro_gap_absorption_summary_v1",
        "template": False,
        "work_unit_id": "C0018",
        "campaign_id": "C0018",
        "run_id": "R0001",
        ...
        "micro_gap_absorption_profile": profiles["micro_gap_absorption_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "micro_gap_absorption_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0018-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0018_micro_gap_absorption_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0018_micro_gap_absorption_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0018-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0018-R0001-MICRO-GAP-ABSORPTION-SUMMARY",
                "micro_gap_absorption_summary_artifact",
                "json",
                "campaigns/C0018_micro_gap_absorption_discovery/runs/R0001/artifacts/c0018_r0001_micro_gap_absorption_summary.json",
                summary_hash,
                ["campaigns/C0018_micro_gap_absorption_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0018_r0001_micro_gap_absorption",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0018_r0001_proxy_trades.csv"
    evidence["micro_gap_absorption_summary"] = "artifacts/c0018_r0001_micro_gap_absorption_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0018_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0018_r0001_proxy_trades.csv",
        "artifacts/c0018_r0001_micro_gap_absorption_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0018 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0018_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0018_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0018_micro_gap_absorption_discovery"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0018_micro_gap_absorption_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0018_micro_gap_absorption_discovery",
        "open_c0018_r0001_fold_local_micro_gap_absorption_rank_run",
        "produce_c0018_r0001_proxy_evidence",
    ):
    ...
    data["active_run"] = "campaigns/C0018_micro_gap_absorption_discovery/runs/R0001"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["active_campaign"] = "campaigns/C0018_micro_gap_absorption_discovery"
    data["active_run"] = "campaigns/C0018_micro_gap_absorption_discovery/runs/R0001"
    data["latest_operation"] = {
        "id": "produce_c0018_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0018_micro_gap_absorption_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0018_r0001_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0018",
            "c0004_r0001_fold_local_state_archetype": "c0018_r0001_micro_gap_absorption",
            "c0004_r0001": "c0018_r0001",
            "fold_local_state_archetype_discovery": "micro_gap_absorption_discovery",
            ...
            "state_archetype_summary": "micro_gap_absorption_summary",
        }
