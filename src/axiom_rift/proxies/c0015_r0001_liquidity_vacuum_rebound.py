"""C0015 R0001 proxy evidence for liquidity vacuum rebound discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0015_liquidity_vacuum_rebound_discovery" / "runs" / "R0001"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0015_liquidity_vacuum_rebound_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0015_r0001_proxy_trades.csv"
LIQUIDITY_VACUUM_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0015_r0001_liquidity_vacuum_rebound_summary.json"
...
FEATURE_NAMES = (
    ...
    "opposite_impulse_12_over_range",
    "micro_range_drought_6_over_48",
    ...
    "distance_from_directional_extreme_72",
    "minutes_since_directional_extreme_72",
    "recoil_from_extreme_over_range",
    "failed_extension_fraction",
    "day_directional_position",
...
MODEL_FAMILY = "fold_local_liquidity_vacuum_rebound_rank"
LABEL_SHAPE = "directional_liquidity_vacuum_rebound_followthrough_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_liquidity_vacuum_rebound_scores_per_active_day"
...
def run_c0015_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    liquidity_context = build_liquidity_context(bars)
    trades: list[base.Trade] = []
...
def build_candidates(
    ...
    liquidity_context: list[dict[str, float] | None],
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = liquidity_vacuum_features(
                bars,
                range_average,
                short_range_average,
                liquidity_context,
            ...
            if features is None:
                continue
            label = (
                liquidity_vacuum_label(bars, range_average, index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|liquidity_vacuum_rebound",
                    features=features,
                    label=label,
                )
            )
...
def build_liquidity_context(bars: list[base.Bar]) -> list[dict[str, float] | None]:
...
def liquidity_vacuum_features(
    ...
    if average_range is None or average_range <= 0 or short_average_range is None or short_average_range <= 0:
        return None
    context = liquidity_context[index]
    if context is None:
        return None
    bar = bars[index]
    ...
    opposite_impulse = max(0.0, -ret_12)
    range_3 = sum(max(bars[offset].high - bars[offset].low, 0.0) for offset in range(index - 2, index + 1)) / 3.0
    range_6 = sum(max(bars[offset].high - bars[offset].low, 0.0) for offset in range(index - 5, index + 1)) / 6.0
    ...
    micro_range_drought = range_6 / max(range_48, 1e-9)
    ...
    directional_wick_rejection = lower_wick / bar_range if direction > 0 else upper_wick / bar_range
    ...
    if range_position is None:
        return None
    lookback_window = bars[index - 71 : index + 1]
    if direction > 0:
        extreme_offset, extreme_bar = min(enumerate(lookback_window), key=lambda item: (item[1].low, item[0]))
        prior_extreme = min(prior.low for prior in bars[index - 71 : index])
        distance_from_extreme = (bar.close - extreme_bar.low) / average_range
        failed_extension = max(prior_extreme - bar.low, 0.0) / average_range
        recoil_from_extreme = (bar.close - bar.low) / average_range
        day_position = (bar.close - day_low) / day_range
    else:
        extreme_offset, extreme_bar = max(enumerate(lookback_window), key=lambda item: (item[1].high, -item[0]))
        prior_extreme = max(prior.high for prior in bars[index - 71 : index])
        distance_from_extreme = (extreme_bar.high - bar.close) / average_range
        failed_extension = max(bar.high - prior_extreme, 0.0) / average_range
        recoil_from_extreme = (bar.high - bar.close) / average_range
        day_position = (day_high - bar.close) / day_range
    minutes_since_extreme = (len(lookback_window) - 1 - extreme_offset) / 72.0
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0015"
    payload["work_unit_id"] = "C0015"
    payload["run_id"] = "R0001"
    payload["proxy_id"] = "PX-C0015-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0015_r0001_liquidity_vacuum_rebound"
    payload["proxy_config_path"] = "campaigns/C0015_liquidity_vacuum_rebound_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_liquidity_vacuum_rebound_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0015_liquidity_vacuum_rebound_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0015_liquidity_vacuum_rebound_discovery/runs/R0001/artifacts/c0015_r0001_proxy_trades.csv",
        "campaigns/C0015_liquidity_vacuum_rebound_discovery/runs/R0001/artifacts/c0015_r0001_liquidity_vacuum_rebound_summary.json",
    ]
    ...
    profiles["liquidity_vacuum_rebound_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0015_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "feature_weight_floor": FEATURE_WEIGHT_FLOOR,
            "feature_weight_ceiling": FEATURE_WEIGHT_CEILING,
            "score_interpretation": "higher_score_means_direct_fold_local_liquidity_vacuum_rebound_rank",
            "variant_boundary": "liquidity_vacuum_rebound_rank_not_auction_threshold_monthly_memory_lifecycle_structural_trap_score_floor_stop_target_hold_or_retry_nudge",
        }
    )
    return config
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_liquidity_vacuum_summary_artifact(payload, LIQUIDITY_VACUUM_SUMMARY_PATH)
    PROXY_PATH.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="ascii")
    trade_hash = base.sha256_file(TRADE_ARTIFACT_PATH)
    summary_hash = base.sha256_file(LIQUIDITY_VACUUM_SUMMARY_PATH)
...
def write_liquidity_vacuum_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_liquidity_vacuum_rebound_summary_v1",
        "template": False,
        "work_unit_id": "C0015",
        "campaign_id": "C0015",
        "run_id": "R0001",
        ...
        "liquidity_vacuum_rebound_profile": profiles["liquidity_vacuum_rebound_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "liquidity_vacuum_rebound_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0015-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0015_liquidity_vacuum_rebound_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0015_liquidity_vacuum_rebound_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0015-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0015-R0001-LIQUIDITY-VACUUM-REBOUND-SUMMARY",
                "liquidity_vacuum_rebound_summary_artifact",
                "json",
                "campaigns/C0015_liquidity_vacuum_rebound_discovery/runs/R0001/artifacts/c0015_r0001_liquidity_vacuum_rebound_summary.json",
                summary_hash,
                ["campaigns/C0015_liquidity_vacuum_rebound_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0015_r0001_liquidity_vacuum_rebound",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0015_r0001_proxy_trades.csv"
    evidence["liquidity_vacuum_rebound_summary"] = "artifacts/c0015_r0001_liquidity_vacuum_rebound_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0015_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0015_r0001_proxy_trades.csv",
        "artifacts/c0015_r0001_liquidity_vacuum_rebound_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0015 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0015_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0015_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0015_liquidity_vacuum_rebound_discovery"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0015_liquidity_vacuum_rebound_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0015_liquidity_vacuum_rebound_discovery",
        "open_c0015_r0001_fold_local_liquidity_vacuum_rebound_rank_run",
        "produce_c0015_r0001_proxy_evidence",
    ):
    ...
    data["active_run"] = "campaigns/C0015_liquidity_vacuum_rebound_discovery/runs/R0001"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["active_campaign"] = "campaigns/C0015_liquidity_vacuum_rebound_discovery"
    data["active_run"] = "campaigns/C0015_liquidity_vacuum_rebound_discovery/runs/R0001"
    data["latest_operation"] = {
        "id": "produce_c0015_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0015_liquidity_vacuum_rebound_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0015_r0001_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0015",
            "c0004_r0001_fold_local_state_archetype": "c0015_r0001_liquidity_vacuum_rebound",
            "c0004_r0001": "c0015_r0001",
            "fold_local_state_archetype_discovery": "liquidity_vacuum_rebound_discovery",
            ...
            "state_archetype_summary": "liquidity_vacuum_rebound_summary",
        }
