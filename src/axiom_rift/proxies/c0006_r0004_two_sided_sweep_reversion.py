"""C0006 R0004 proxy evidence for two-sided sweep reversion events."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0006_liquidity_sweep_reclaim_event_discovery" / "runs" / "R0004"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0006_r0004_proxy_trades.csv"
EVENT_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0006_r0004_two_sided_sweep_reversion_summary.json"
...
FEATURE_NAMES = (
    "first_sweep_depth_over_range",
    "second_sweep_depth_over_range",
    "sweep_span_bars",
    "rotation_width_over_range",
    "second_reclaim_depth_over_range",
    "first_rejection_wick_fraction",
    "second_rejection_wick_fraction",
    "second_body_fraction",
    "second_directional_close_location",
    "distance_from_range_mid_over_range",
    "local_range_width_over_avg",
    "wide_range_width_over_avg",
    "compression_ratio_12_over_48",
    "first_range_expansion_ratio",
    "second_range_expansion_ratio",
    "prior_push_against_final_direction_12",
    "session_scope_flag",
    "session_sin",
    "session_cos",
    "spread_over_range",
)
MODEL_FAMILY = "fold_local_train_only_two_sided_sweep_reversion_event_utility"
LABEL_SHAPE = "two_sided_sweep_reversion_target_before_stop_plus_path_quality"
...
MIN_SWEEP_DEPTH_OVER_RANGE = 0.012
MIN_RECLAIM_INSIDE_OVER_BAR_RANGE = 0.020
SECOND_SWEEP_MIN_DELAY_BARS = 2
SECOND_SWEEP_MAX_DELAY_BARS = 12
SCORE_COMPONENT_NAMES = (
    "signed_feature_utility",
    "two_sided_reversion_bonus",
    "scope_bonus",
    "spread_penalty",
)
...
def run_c0006_r0004_proxy(write: bool = True) -> dict[str, object]:
...
def build_candidates(
    ...
    end_index = min(
        ...
        len(bars) - base.LABEL_HORIZON_BARS - SECOND_SWEEP_MAX_DELAY_BARS - 2,
    )
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            event = two_sided_sweep_reversion_features(bars, range_average, short_range_average, index, direction)
            if event is None:
                continue
            entry_anchor_index, scope, features = event
            if bars[entry_anchor_index].time > window.end:
                continue
            label = (
                two_sided_sweep_reversion_label(bars, range_average, entry_anchor_index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    fold_id=fold_id,
                    index=entry_anchor_index,
                    direction=direction,
                    day=bars[entry_anchor_index].time.strftime("%Y-%m-%d"),
                    state_key=f"{side}|{scope}|two_sided_sweep_reversion",
                    features=features,
                    label=label,
                )
            )
...
def two_sided_sweep_reversion_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    index: int,
    ...
    session_extremes = prior_session_extremes(bars, index)
    if session_extremes is None:
        return None
    session_high, session_low = session_extremes

    levels: list[tuple[str, float, float, float]] = [
        ("local", local_high, local_low, local_high - local_low),
        ("wide", wide_high, wide_low, wide_high - wide_low),
        ("session", session_high, session_low, session_high - session_low),
    ]
    if direction > 0:
        # Final long: first sweep high-side liquidity, then opposite low-side sweep reclaims inside range.
        first_depths = [
            (scope, high_level, low_level, width, max(first.high - high_level, 0.0))
            for scope, high_level, low_level, width in levels
            if width > 0
        ]
    else:
        # Final short: first sweep low-side liquidity, then opposite high-side sweep reclaims inside range.
        first_depths = [
            (scope, high_level, low_level, width, max(low_level - first.low, 0.0))
            for scope, high_level, low_level, width in levels
            if width > 0
        ]
    valid_first = [row for row in first_depths if row[4] / average_range >= MIN_SWEEP_DEPTH_OVER_RANGE]
    if not valid_first:
        return None
    scope, high_level, low_level, event_range_width, first_depth = max(
        valid_first,
        key=lambda row: ({"session": 3, "wide": 2, "local": 1}[row[0]], row[4]),
    )
    ...
    second_range = 0.0
    rotation_width = 0.0
    for candidate_index in range(index + SECOND_SWEEP_MIN_DELAY_BARS, index + SECOND_SWEEP_MAX_DELAY_BARS + 1):
        if candidate_index >= len(bars):
            break
        candidate = bars[candidate_index]
        if candidate.time.date() != first.time.date() or not base.in_core_session(candidate.time):
            continue
        candidate_range = max(candidate.high - candidate.low, 0.0)
        if candidate_range <= 0:
            continue
        if direction > 0:
            candidate_depth = max(low_level - candidate.low, 0.0)
            candidate_reclaim = max(candidate.close - low_level, 0.0)
            candidate_rotation = max(first.high - candidate.low, 0.0)
        else:
            candidate_depth = max(candidate.high - high_level, 0.0)
            candidate_reclaim = max(high_level - candidate.close, 0.0)
            candidate_rotation = max(candidate.high - first.low, 0.0)
        if candidate_depth / average_range < MIN_SWEEP_DEPTH_OVER_RANGE:
            continue
        if candidate_reclaim / candidate_range < MIN_RECLAIM_INSIDE_OVER_BAR_RANGE:
            continue
        second_index = candidate_index
        second_depth = candidate_depth
        reclaim_depth = candidate_reclaim
        second_range = candidate_range
        rotation_width = candidate_rotation
        break
    if second_index is None or second_range <= 0:
        return None
    second = bars[second_index]
    span_bars = second_index - index
    compression_ratio = short_average_range / average_range
    first_range_expansion = first_range / average_range
    second_range_expansion = second_range / average_range
    prior_push_12 = -direction * (bars[index - 1].close - bars[index - 12].close) / average_range
    second_body_fraction = direction * (second.close - second.open) / second_range
    close_location = (second.close - second.low) / second_range
    directional_close_location = close_location if direction > 0 else 1.0 - close_location
    first_upper_wick = first.high - max(first.open, first.close)
    first_lower_wick = min(first.open, first.close) - first.low
    second_upper_wick = second.high - max(second.open, second.close)
    second_lower_wick = min(second.open, second.close) - second.low
    first_rejection_wick = first_upper_wick if direction > 0 else first_lower_wick
    second_rejection_wick = second_lower_wick if direction > 0 else second_upper_wick
    mid = (local_high + local_low) / 2.0
    distance_from_mid = direction * (second.close - mid) / max(local_high - local_low, 1e-9)
    minute_fraction = base.minute_of_day(second.time) / (24.0 * 60.0)
    session_sin = math.sin(2.0 * math.pi * minute_fraction)
    session_cos = math.cos(2.0 * math.pi * minute_fraction)
    spread_over_range = second.spread_points / average_range
    return second_index, scope, (
        first_depth / average_range,
        second_depth / average_range,
        float(span_bars),
        rotation_width / max(event_range_width, 1e-9),
        reclaim_depth / average_range,
        first_rejection_wick / first_range,
        second_rejection_wick / second_range,
        second_body_fraction,
        ...
        compression_ratio,
        first_range_expansion,
        second_range_expansion,
        prior_push_12,
...
def prior_session_extremes(bars: list[base.Bar], index: int) -> tuple[float, float] | None:
    ...
    if len(session_bars) < 6:
        return None
    return max(bar.high for bar in session_bars), min(bar.low for bar in session_bars)


def two_sided_sweep_reversion_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    index: int,
    direction: int,
...
def score_event_candidates(candidates: list[base.Candidate], model: EventUtilityModel) -> list[base.Candidate]:
    ...
    first_depth = features[:, FEATURE_NAMES.index("first_sweep_depth_over_range")]
    second_depth = features[:, FEATURE_NAMES.index("second_sweep_depth_over_range")]
    reclaim_depth = features[:, FEATURE_NAMES.index("second_reclaim_depth_over_range")]
    rotation_width = features[:, FEATURE_NAMES.index("rotation_width_over_range")]
    second_rejection = features[:, FEATURE_NAMES.index("second_rejection_wick_fraction")]
    directional_close = features[:, FEATURE_NAMES.index("second_directional_close_location")]
    distance_from_mid = features[:, FEATURE_NAMES.index("distance_from_range_mid_over_range")]
    ...
    scores = (
        0.62 * signed_utility
        + 0.18 * np.tanh(1.6 * second_depth)
        + 0.14 * np.tanh(1.4 * first_depth)
        + 0.14 * np.tanh(2.0 * reclaim_depth)
        + 0.08 * np.tanh(rotation_width)
        + 0.08 * second_rejection
        + 0.05 * directional_close
        - 0.04 * np.tanh(abs(distance_from_mid))
        + 0.05 * session_scope
        - 0.18 * spread_penalty
    )
...
def build_proxy_payload(
    ...
    payload["proxy_id"] = "PX-C0006-R0004"
    payload["proxy_engine"] = "axiom_rift.proxies.c0006_r0004_two_sided_sweep_reversion"
    payload["proxy_config_path"] = "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0004/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_two_sided_sweep_reversion_event_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0004/kpi/proxy.json",
        "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0004/artifacts/c0006_r0004_proxy_trades.csv",
        "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0004/artifacts/c0006_r0004_two_sided_sweep_reversion_summary.json",
    ]
    ...
    profiles["two_sided_sweep_reversion_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
            ...
            "min_reclaim_inside_over_bar_range": MIN_RECLAIM_INSIDE_OVER_BAR_RANGE,
            "second_sweep_min_delay_bars": SECOND_SWEEP_MIN_DELAY_BARS,
            "second_sweep_max_delay_bars": SECOND_SWEEP_MAX_DELAY_BARS,
            "score_component_names": list(SCORE_COMPONENT_NAMES),
            "fold_models": fold_models,
            "state_distributions": state_distributions,
            "candidates_by_fold": candidates_by_fold,
            "selection_rule": "top_fold_local_two_sided_sweep_reversion_scores_per_active_day",
    ...
    config.update(
        {
            ...
            "second_sweep_max_delay_bars": SECOND_SWEEP_MAX_DELAY_BARS,
            "score_component_names": list(SCORE_COMPONENT_NAMES),
            "score_interpretation": "higher_score_means_train_fold_event_utility_supports_two_sided_sweep_reversion_follow_through",
            "variant_boundary": "new_two_sided_sweep_reversion_event_grammar_not_analog_memory_state_bucket_score_cell_exit_shape_or_parameter_nudge",
        }
    )
    return config
...
def write_event_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_two_sided_sweep_reversion_summary_v1",
        "template": False,
        "work_unit_id": "C0006",
        "campaign_id": "C0006",
        "run_id": "R0004",
        ...
        "two_sided_sweep_reversion_profile": profiles["two_sided_sweep_reversion_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "two_sided_sweep_reversion_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0006-R0004-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0004/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0004/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0006-R0004-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0006-R0004-EVENT-UTILITY-SUMMARY",
                "two_sided_sweep_reversion_summary_artifact",
                "json",
                "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0004/artifacts/c0006_r0004_two_sided_sweep_reversion_summary.json",
                summary_hash,
                ["campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0004/kpi/proxy.json"],
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0006_r0004_mt5_logic_parity_evidence",
        }
    ]
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0006_r0004_two_sided_sweep_reversion",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0006_r0004_proxy_trades.csv"
    evidence["two_sided_sweep_reversion_summary"] = "artifacts/c0006_r0004_two_sided_sweep_reversion_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0006_r0004_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0006_r0004_proxy_trades.csv",
        "artifacts/c0006_r0004_two_sided_sweep_reversion_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0006 R0004 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0006_r0004_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0006_r0004_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    if "produce_c0006_r0004_proxy_evidence" not in completed:
        completed.append("produce_c0006_r0004_proxy_evidence")
    next_action = "produce_c0006_r0004_mt5_logic_parity_evidence"
    completed = [item for item in completed if item != next_action]
    next_work["completed"] = completed
    next_work["tasks"] = [next_action]
    data["active_run"] = "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0004"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0006_r0004_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0006_liquidity_sweep_reclaim_event_discovery/runs/R0004/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0006_r0004_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0006",
            "R0004": "R0004",
            "c0004_r0003_fold_local_state_archetype": "c0006_r0004_two_sided_sweep_reversion",
            "c0004_r0003": "c0006_r0004",
