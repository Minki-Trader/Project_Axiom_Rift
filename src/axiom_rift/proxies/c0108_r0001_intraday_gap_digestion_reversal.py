"""C0108 R0001 proxy evidence for intraday gap digestion reversal."""
...
CAMPAIGN_ID = "C0108"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0108_intraday_gap_digestion_reversal_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0108_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0108_r0001_gap_digestion_reversal_summary.json"
...
FEATURE_NAMES = (
    ...
    "vwap_magnet",
    "reversal_body_turn",
    "opening_band_reclaim",
    "session_phase_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_gap_digestion_reversal_rank"
LABEL_SHAPE = "target_first_quality_after_gap_digestion_reversal"
SELECTION_RULE = "top_fold_local_gap_digestion_reversal_scores_per_active_day"
...
MIN_DAY_BARS = 18
GAP_DIGESTION_REVERSAL_LOOKBACK_BARS = 48
GAP_DIGESTION_REVERSAL_RECENT_BARS = 8
GAP_DIGESTION_REVERSAL_PRIOR_BARS = 24
MIN_GAP_SIZE_RANGE = 0.18
MIN_GAP_DIGESTION_REVERSAL_ACTIVITY = 0.04
...
class LinearGapDigestionReversalModel:
    fold_id: str
    ...
    gap_digestion_reversal_failure_rate: float
...
def run_c0108_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_gap_digestion_reversal_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_gap_digestion_reversal_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        GAP_DIGESTION_REVERSAL_LOOKBACK_BARS,
        GAP_DIGESTION_REVERSAL_RECENT_BARS + GAP_DIGESTION_REVERSAL_PRIOR_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = gap_digestion_reversal_metrics(
    ...
    features = (
        metrics["overnight_gap_size"],
        metrics["gap_side_residual"],
        metrics["gap_fill_progress"],
        metrics["gap_extreme_recoil"],
        metrics["digestion_compression"],
        metrics["volume_cooling"],
        metrics["prior_close_magnet"],
        metrics["vwap_magnet"],
        metrics["reversal_body_turn"],
        metrics["opening_band_reclaim"],
        metrics["session_phase_maturity"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        c0108_gap_size_bucket(metrics["overnight_gap_size"]),
        c0108_gap_residual_bucket(metrics["gap_side_residual"]),
        c0108_gap_fill_bucket(metrics["gap_fill_progress"]),
        c0108_recoil_bucket(metrics["gap_extreme_recoil"]),
        compression_bucket(metrics["digestion_compression"]),
        c0108_volume_cooling_bucket(metrics["volume_cooling"]),
        c0108_prior_close_magnet_bucket(metrics["prior_close_magnet"]),
        c0108_vwap_magnet_bucket(metrics["vwap_magnet"]),
        counter_body_turn_bucket(metrics["reversal_body_turn"]),
        c0108_opening_reclaim_bucket(metrics["opening_band_reclaim"]),
...
def gap_digestion_reversal_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    if context.prior_day_close is None:
        return {"event_count": 0.0}
    gap_points = context.day_open - context.prior_day_close
    gap_direction = int(price_sign(gap_points))
    if gap_direction == 0 or direction != -gap_direction:
        return {"event_count": 0.0}
    overnight_gap_size = abs(gap_points) / average_range
    if overnight_gap_size < MIN_GAP_SIZE_RANGE:
        return {"event_count": 0.0}

    recent_start = max(index - GAP_DIGESTION_REVERSAL_RECENT_BARS + 1, context.day_start_index)
    prior_start = max(recent_start - GAP_DIGESTION_REVERSAL_PRIOR_BARS, context.day_start_index)
    lookback_start = max(index - GAP_DIGESTION_REVERSAL_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    opening_boundary = context.opening_high if direction > 0 else context.opening_low
    recent_high = max(bar.high for bar in recent_window)
    recent_low = min(bar.low for bar in recent_window)
    gap_side_distance = gap_direction * (signal_bar.close - context.prior_day_close)
    gap_side_residual = gap_side_distance / average_range
    if gap_side_residual <= 0.03:
        return {"event_count": 0.0}
    gap_fill_progress = (abs(gap_points) - gap_side_distance) / max(abs(gap_points), 1e-9)
    if gap_fill_progress < -0.60 or gap_fill_progress > 0.92:
        return {"event_count": 0.0}

    if gap_direction > 0:
        gap_extreme_recoil = (context.high_so_far - signal_bar.close) / average_range
        recent_gap_recoil = (recent_high - signal_bar.close) / average_range
    else:
        gap_extreme_recoil = (signal_bar.close - context.low_so_far) / average_range
        recent_gap_recoil = (signal_bar.close - recent_low) / average_range
    digestion_compression = 1.0 - recent_range_mean / max(prior_range_mean, average_range * 0.10)
    volume_cooling = prior_volume_mean / max(average_volume, 1e-9) - recent_volume_mean / max(
        average_volume,
        1e-9,
    )
    prior_close_magnet = 1.0 - min(
        abs(signal_bar.close - context.prior_day_close) / max(context.prior_day_range or average_range, average_range),
        2.0,
    )
    vwap_magnet = direction * (session_vwap - signal_bar.close) / average_range
    opening_band_reclaim = direction * (signal_bar.close - opening_boundary) / average_range
    recent_body_turn = base.mean([directional_body_score(bar, direction) for bar in recent_window[-3:]]) or 0.0
    reversal_body_turn = 0.60 * directional_body_score(signal_bar, direction) + 0.40 * recent_body_turn
    activity = max(
        overnight_gap_size * 0.45,
        max(gap_extreme_recoil, recent_gap_recoil, 0.0) * 0.65,
        max(digestion_compression, 0.0) * 0.55,
        max(vwap_magnet, 0.0) * 0.45,
        max(opening_band_reclaim, 0.0) * 0.40,
        max(reversal_body_turn, 0.0) * 0.35,
        max(volume_cooling, 0.0) * 0.25,
    )
    if activity < MIN_GAP_DIGESTION_REVERSAL_ACTIVITY:
        return {"event_count": 0.0}
    if gap_extreme_recoil < -0.12 or vwap_magnet < -0.80:
        return {"event_count": 0.0}
    return {
        "overnight_gap_size": bounded(overnight_gap_size, 0.0, 8.0),
        "gap_side_residual": bounded(gap_side_residual, 0.0, 8.0),
        "gap_fill_progress": bounded(gap_fill_progress, -1.0, 1.0),
        "gap_extreme_recoil": bounded(gap_extreme_recoil, -2.0, 8.0),
        "digestion_compression": bounded(digestion_compression, -2.0, 2.0),
        "volume_cooling": bounded(volume_cooling, -4.0, 4.0),
        "prior_close_magnet": bounded(prior_close_magnet, -1.0, 1.0),
        "vwap_magnet": bounded(vwap_magnet, -4.0, 4.0),
        "reversal_body_turn": bounded(reversal_body_turn, -1.0, 1.0),
        "opening_band_reclaim": bounded(opening_band_reclaim, -4.0, 4.0),
        ...
        "gap_direction": float(gap_direction),
        "recent_gap_recoil": bounded(recent_gap_recoil, -2.0, 8.0),
        "gap_digestion_reversal_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0108_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0108_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0108_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0108_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0108_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0108_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0108_boundary_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0108_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0108_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0108_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0108_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0108_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
        return "volume_still_expanding"
    if value < 0.18:
        return "volume_flat"
...
def c0108_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0108_opening_reclaim_bucket(value: float) -> str:
    if value < -0.08:
...
def c0108_prior_boundary_bucket(value: float) -> str:
    if value < -0.12:
...
def c0108_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    gap_reward = 0.12 if metrics["overnight_gap_size"] > 0.35 and terminal > 0 else 0.0
    residual_reward = 0.10 if metrics["gap_side_residual"] > 0.18 and terminal > 0 else 0.0
    fill_reward = 0.10 if 0.08 <= metrics["gap_fill_progress"] <= 0.70 and terminal > 0 else 0.0
    recoil_reward = 0.12 if metrics["gap_extreme_recoil"] > 0.20 and terminal > 0 else 0.0
    compression_reward = 0.09 if metrics["digestion_compression"] > 0.08 and terminal > 0 else 0.0
    volume_reward = 0.07 if metrics["volume_cooling"] > 0.08 and terminal > 0 else 0.0
    prior_reward = 0.08 if metrics["prior_close_magnet"] > 0.12 and terminal > 0 else 0.0
    vwap_reward = 0.09 if metrics["vwap_magnet"] > 0.12 and terminal > 0 else 0.0
    body_reward = 0.08 if metrics["reversal_body_turn"] > 0.08 and terminal > 0 else 0.0
    reclaim_reward = 0.08 if metrics["opening_band_reclaim"] > 0.08 and terminal > 0 else 0.0
    weak_gap_penalty = 0.08 if metrics["overnight_gap_size"] < 0.24 and terminal <= 0 else 0.0
    depleted_residual_penalty = 0.10 if metrics["gap_side_residual"] < 0.10 and terminal <= 0 else 0.0
    overfilled_gap_penalty = 0.10 if metrics["gap_fill_progress"] > 0.78 and terminal <= 0 else 0.0
    recoil_absent_penalty = 0.11 if metrics["gap_extreme_recoil"] < 0.05 and terminal <= 0 else 0.0
    vwap_against_penalty = 0.10 if metrics["vwap_magnet"] < -0.10 and terminal <= 0 else 0.0
    body_against_penalty = 0.08 if metrics["reversal_body_turn"] < -0.08 and terminal <= 0 else 0.0
    ...
    gap_digestion_reversal_path_quality = (0.54 * terminal + 0.30 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + gap_digestion_reversal_path_quality
        + gap_reward
        + residual_reward
        + fill_reward
        + recoil_reward
        ...
        + reclaim_reward
        - weak_gap_penalty
        - depleted_residual_penalty
        - overfilled_gap_penalty
        - recoil_absent_penalty
...
def fit_linear_gap_digestion_reversal_model(candidates: list[base.Candidate], fold_id: str) -> LinearGapDigestionReversalModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearGapDigestionReversalModel(
            fold_id,
    ...
    gap_digestion_reversal_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearGapDigestionReversalModel(
        fold_id=fold_id,
        ...
        gap_digestion_reversal_failure_rate=gap_digestion_reversal_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearGapDigestionReversalModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0108-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_gap_digestion_reversal_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0108_r0001_intraday_gap_digestion_reversal",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0108_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0108_r0001_gap_digestion_reversal_summary.json",
        ],
        ...
        "conditional_profiles": {
            "gap_digestion_reversal_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0108 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0108_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0108 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "gap_digestion_reversal_lookback_bars": GAP_DIGESTION_REVERSAL_LOOKBACK_BARS,
        "gap_digestion_reversal_recent_bars": GAP_DIGESTION_REVERSAL_RECENT_BARS,
        "gap_digestion_reversal_prior_bars": GAP_DIGESTION_REVERSAL_PRIOR_BARS,
        "min_gap_digestion_reversal_activity": MIN_GAP_DIGESTION_REVERSAL_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_gap_digestion_reversal_model_summary(model: LinearGapDigestionReversalModel) -> dict[str, object]:
    return {
        ...
        "gap_digestion_reversal_failure_rate": rounded(model.gap_digestion_reversal_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearGapDigestionReversalModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_gap_digestion_reversal_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "gap_digestion_reversal_profile": profiles["gap_digestion_reversal_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0108_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0108_r0001_proxy_trades.csv"
    evidence["gap_digestion_reversal_summary"] = "artifacts/c0108_r0001_gap_digestion_reversal_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0108_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0108_r0001_proxy_trades.csv",
        "artifacts/c0108_r0001_gap_digestion_reversal_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0108_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0108 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0108_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0108_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0108_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0108_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0108_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0108_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0108_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0108_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0108_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0108 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0108 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0108_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0108_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0108_r0001_intraday_gap_digestion_reversal_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0108_r0001_proxy_records_gap_digestion_reversal_reference_surface
  ...
  - next_work_is_produce_c0108_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0108_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0108-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0108_r0001_intraday_gap_digestion_reversal",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0108-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0108-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0108-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0108_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0108_r0001_intraday_gap_digestion_reversal",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0108-R0001-GAP-DIGESTION-REVERSAL-SUMMARY",
            "artifact_role": "gap_digestion_reversal_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0108_r0001_gap_digestion_reversal_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0108_r0001_intraday_gap_digestion_reversal",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0108_r0001_mt5_logic_parity_evidence",
        }
    ]
