"""C0106 R0001 proxy evidence for intraday range acceptance failure reversal."""
...
CAMPAIGN_ID = "C0106"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0106_intraday_range_acceptance_failure_reversal_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0106_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0106_r0001_range_acceptance_failure_reversal_summary.json"
...
FEATURE_NAMES = (
    "boundary_probe_depth",
    "acceptance_failure_recoil",
    "failed_followthrough_distance",
    "opposing_tail_rejection",
    "boundary_reclaim_strength",
    "volume_exhaustion_divergence",
    "session_range_extension_stress",
    "opening_range_reclaim_context",
    "prior_day_boundary_failure_context",
    "time_reversal_alignment",
    "session_phase_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_range_acceptance_failure_reversal_rank"
LABEL_SHAPE = "target_first_quality_after_range_acceptance_failure_reversal"
SELECTION_RULE = "top_fold_local_range_acceptance_failure_reversal_scores_per_active_day"
...
MIN_DAY_BARS = 30
RANGE_ACCEPTANCE_FAILURE_REVERSAL_LOOKBACK_BARS = 60
RANGE_ACCEPTANCE_FAILURE_REVERSAL_RECENT_BARS = 10
RANGE_ACCEPTANCE_FAILURE_REVERSAL_PRIOR_BARS = 30
MIN_RANGE_ACCEPTANCE_FAILURE_REVERSAL_ACTIVITY = 0.05
...
class LinearRangeAcceptanceFailureReversalModel:
    fold_id: str
    ...
    range_acceptance_failure_reversal_failure_rate: float
...
def run_c0106_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_range_acceptance_failure_reversal_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_range_acceptance_failure_reversal_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        RANGE_ACCEPTANCE_FAILURE_REVERSAL_LOOKBACK_BARS,
        RANGE_ACCEPTANCE_FAILURE_REVERSAL_RECENT_BARS + RANGE_ACCEPTANCE_FAILURE_REVERSAL_PRIOR_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = range_acceptance_failure_reversal_metrics(
    ...
    features = (
        metrics["boundary_probe_depth"],
        metrics["acceptance_failure_recoil"],
        metrics["failed_followthrough_distance"],
        metrics["opposing_tail_rejection"],
        metrics["boundary_reclaim_strength"],
        metrics["volume_exhaustion_divergence"],
        metrics["session_range_extension_stress"],
        metrics["opening_range_reclaim_context"],
        metrics["prior_day_boundary_failure_context"],
        metrics["time_reversal_alignment"],
        metrics["session_phase_maturity"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        c0106_boundary_probe_bucket(metrics["boundary_probe_depth"]),
        c0106_recoil_bucket(metrics["acceptance_failure_recoil"]),
        c0106_failed_followthrough_bucket(metrics["failed_followthrough_distance"]),
        c0106_tail_rejection_bucket(metrics["opposing_tail_rejection"]),
        c0106_reclaim_bucket(metrics["boundary_reclaim_strength"]),
        c0106_volume_exhaustion_bucket(metrics["volume_exhaustion_divergence"]),
        c0106_extension_stress_bucket(metrics["session_range_extension_stress"]),
        c0106_opening_reclaim_bucket(metrics["opening_range_reclaim_context"]),
        c0106_prior_boundary_bucket(metrics["prior_day_boundary_failure_context"]),
        c0106_time_reversal_bucket(metrics["time_reversal_alignment"]),
...
def range_acceptance_failure_reversal_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    recent_start = max(index - RANGE_ACCEPTANCE_FAILURE_REVERSAL_RECENT_BARS + 1, context.day_start_index)
    prior_start = max(recent_start - RANGE_ACCEPTANCE_FAILURE_REVERSAL_PRIOR_BARS, context.day_start_index)
    lookback_start = max(index - RANGE_ACCEPTANCE_FAILURE_REVERSAL_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    history_window = bars[lookback_start:recent_start]
    if len(prior_window) < 10 or len(recent_window) < 6 or len(lookback_window) < 24 or len(history_window) < 14:
        return {"event_count": 0.0}
    ...
    if session_phase_maturity < 0.12:
        return {"event_count": 0.0}

    day_range_so_far = max(context.high_so_far - context.low_so_far, average_range * 0.25)
    recent_return = signal_bar.close - recent_window[0].open
    recent_scale = max(recent_range_mean * math.sqrt(len(recent_window)), average_range * 0.25)
    history_high = max(bar.high for bar in history_window)
    history_low = min(bar.low for bar in history_window)
    recent_high = max(bar.high for bar in recent_window)
    recent_low = min(bar.low for bar in recent_window)
    recent_width = max(recent_high - recent_low, average_range * 0.25)
    if direction > 0:
        boundary_probe_depth = max(0.0, history_low - recent_low) / average_range
        acceptance_failure_recoil = (signal_bar.close - recent_low) / recent_width
        boundary_reclaim_strength = (signal_bar.close - history_low) / average_range
        session_range_extension_stress = max(0.0, context.day_open - recent_low) / day_range_so_far
        opening_range_reclaim_context = (signal_bar.close - context.opening_low) / day_range_so_far
        probe_boundary = history_low
    else:
        boundary_probe_depth = max(0.0, recent_high - history_high) / average_range
        acceptance_failure_recoil = (recent_high - signal_bar.close) / recent_width
        boundary_reclaim_strength = (history_high - signal_bar.close) / average_range
        session_range_extension_stress = max(0.0, recent_high - context.day_open) / day_range_so_far
        opening_range_reclaim_context = (context.opening_high - signal_bar.close) / day_range_so_far
        probe_boundary = history_high
    failed_followthrough_distance = direction * recent_return / recent_scale
    opposing_tail_rejection = base.mean([directional_wick_rejection(bar, direction) for bar in recent_window]) or 0.0
    probe_index = min(
        range(recent_start, index + 1),
        key=lambda pos: bars[pos].low if direction > 0 else -bars[pos].high,
    )
    probe_volume_ratio = volumes[probe_index] / max(average_volume, 1e-9)
    late_start = max(recent_start, index - 2)
    late_volume_mean = base.mean(volumes[late_start : index + 1]) or average_volume
    volume_exhaustion_divergence = probe_volume_ratio - late_volume_mean / max(average_volume, 1e-9)
    prior_day_boundary_failure_context = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        shadow_balance = context.prior_day_shadow_balance or 0.0
        prior_day_boundary_failure_context = (
            -direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
            + direction * shadow_balance * 0.35
        )

    time_reversal_alignment = session_phase_maturity * (
        0.26 * boundary_probe_depth
        + 0.22 * acceptance_failure_recoil
        + 0.18 * max(boundary_reclaim_strength, 0.0)
        + 0.12 * opposing_tail_rejection
        + 0.10 * max(volume_exhaustion_divergence, 0.0)
        - 0.10 * max(failed_followthrough_distance, 0.0)
    )
    activity = max(
        boundary_probe_depth,
        max(boundary_reclaim_strength, 0.0) * 0.45,
        max(acceptance_failure_recoil, 0.0) * 0.30,
        max(opposing_tail_rejection, 0.0) * 0.22,
        max(volume_exhaustion_divergence, 0.0) * 0.16,
        max(time_reversal_alignment, 0.0) * 0.14,
    )
    if activity < MIN_RANGE_ACCEPTANCE_FAILURE_REVERSAL_ACTIVITY:
        return {"event_count": 0.0}
    if boundary_probe_depth < 0.01 or boundary_reclaim_strength < -0.18:
        return {"event_count": 0.0}
    return {
        "boundary_probe_depth": bounded(boundary_probe_depth, 0.0, 4.0),
        "acceptance_failure_recoil": bounded(acceptance_failure_recoil, -1.0, 2.0),
        "failed_followthrough_distance": bounded(failed_followthrough_distance, -4.0, 4.0),
        "opposing_tail_rejection": bounded(opposing_tail_rejection, -1.0, 1.0),
        "boundary_reclaim_strength": bounded(boundary_reclaim_strength, -4.0, 4.0),
        "volume_exhaustion_divergence": bounded(volume_exhaustion_divergence, -4.0, 4.0),
        "session_range_extension_stress": bounded(session_range_extension_stress, 0.0, 4.0),
        "opening_range_reclaim_context": bounded(opening_range_reclaim_context, -2.0, 2.0),
        "prior_day_boundary_failure_context": bounded(prior_day_boundary_failure_context, -2.0, 2.0),
        "time_reversal_alignment": bounded(time_reversal_alignment, -4.0, 4.0),
        ...
        "range_acceptance_failure_reversal_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0106_boundary_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0106_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0106_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0106_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0106_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0106_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0106_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0106_opening_reclaim_bucket(value: float) -> str:
    if value < -0.08:
...
def c0106_prior_boundary_bucket(value: float) -> str:
    if value < -0.12:
...
def c0106_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    probe_reward = 0.13 if metrics["boundary_probe_depth"] > 0.08 and terminal > 0 else 0.0
    recoil_reward = 0.14 if metrics["acceptance_failure_recoil"] > 0.55 and terminal > 0 else 0.0
    reclaim_reward = 0.13 if metrics["boundary_reclaim_strength"] > 0.08 and terminal > 0 else 0.0
    tail_reward = 0.09 if metrics["opposing_tail_rejection"] > 0.08 and terminal > 0 else 0.0
    volume_reward = 0.08 if metrics["volume_exhaustion_divergence"] > 0.16 and terminal > 0 else 0.0
    extension_reward = 0.05 if metrics["session_range_extension_stress"] > 0.12 and terminal > 0 else 0.0
    opening_reward = 0.06 if metrics["opening_range_reclaim_context"] > 0.10 and terminal > 0 else 0.0
    prior_reward = 0.05 if metrics["prior_day_boundary_failure_context"] > 0.08 and terminal > 0 else 0.0
    time_reward = 0.06 if metrics["time_reversal_alignment"] > 0.08 and terminal > 0 else 0.0
    weak_probe_penalty = 0.10 if metrics["boundary_probe_depth"] < 0.04 and terminal <= 0 else 0.0
    failed_reclaim_penalty = 0.14 if metrics["boundary_reclaim_strength"] < -0.08 and terminal <= 0 else 0.0
    adverse_tail_penalty = 0.09 if metrics["opposing_tail_rejection"] < -0.12 and terminal <= 0 else 0.0
    volume_continuation_penalty = 0.08 if metrics["volume_exhaustion_divergence"] < -0.18 and terminal <= 0 else 0.0
    continuation_penalty = 0.06 if metrics["failed_followthrough_distance"] > 0.24 and terminal <= 0 else 0.0
    ...
    range_acceptance_failure_reversal_path_quality = (0.54 * terminal + 0.30 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + range_acceptance_failure_reversal_path_quality
        + probe_reward
        + recoil_reward
        + reclaim_reward
        + tail_reward
        ...
        + time_reward
        - weak_probe_penalty
        - failed_reclaim_penalty
        - adverse_tail_penalty
        - volume_continuation_penalty
...
def fit_linear_range_acceptance_failure_reversal_model(candidates: list[base.Candidate], fold_id: str) -> LinearRangeAcceptanceFailureReversalModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearRangeAcceptanceFailureReversalModel(
            fold_id,
    ...
    range_acceptance_failure_reversal_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearRangeAcceptanceFailureReversalModel(
        fold_id=fold_id,
        ...
        range_acceptance_failure_reversal_failure_rate=range_acceptance_failure_reversal_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearRangeAcceptanceFailureReversalModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0106-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_range_acceptance_failure_reversal_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0106_r0001_intraday_range_acceptance_failure_reversal",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0106_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0106_r0001_range_acceptance_failure_reversal_summary.json",
        ],
        ...
        "conditional_profiles": {
            "range_acceptance_failure_reversal_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0106 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0106_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0106 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "range_acceptance_failure_reversal_lookback_bars": RANGE_ACCEPTANCE_FAILURE_REVERSAL_LOOKBACK_BARS,
        "range_acceptance_failure_reversal_recent_bars": RANGE_ACCEPTANCE_FAILURE_REVERSAL_RECENT_BARS,
        "range_acceptance_failure_reversal_prior_bars": RANGE_ACCEPTANCE_FAILURE_REVERSAL_PRIOR_BARS,
        "min_range_acceptance_failure_reversal_activity": MIN_RANGE_ACCEPTANCE_FAILURE_REVERSAL_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_range_acceptance_failure_reversal_model_summary(model: LinearRangeAcceptanceFailureReversalModel) -> dict[str, object]:
    return {
        ...
        "range_acceptance_failure_reversal_failure_rate": rounded(model.range_acceptance_failure_reversal_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearRangeAcceptanceFailureReversalModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_range_acceptance_failure_reversal_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "range_acceptance_failure_reversal_profile": profiles["range_acceptance_failure_reversal_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0106_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0106_r0001_proxy_trades.csv"
    evidence["range_acceptance_failure_reversal_summary"] = "artifacts/c0106_r0001_range_acceptance_failure_reversal_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0106_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0106_r0001_proxy_trades.csv",
        "artifacts/c0106_r0001_range_acceptance_failure_reversal_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0106_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0106 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0106_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0106_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0106_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0106_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0106_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0106_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0106_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0106_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0106_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0106 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0106 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0106_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0106_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0106_r0001_intraday_range_acceptance_failure_reversal_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0106_r0001_proxy_records_range_acceptance_failure_reversal_reference_surface
  ...
  - next_work_is_produce_c0106_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0106_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0106-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0106_r0001_intraday_range_acceptance_failure_reversal",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0106-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0106-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0106-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0106_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0106_r0001_intraday_range_acceptance_failure_reversal",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0106-R0001-range-acceptance-failure-reversal-SUMMARY",
            "artifact_role": "range_acceptance_failure_reversal_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0106_r0001_range_acceptance_failure_reversal_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0106_r0001_intraday_range_acceptance_failure_reversal",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0106_r0001_mt5_logic_parity_evidence",
        }
    ]
