"""C0127 R0001 proxy evidence for intraday body efficiency cliff reversal."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0127_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0127_r0001_body_efficiency_cliff_reversal_summary.json"
...
FEATURE_NAMES = (
    "adverse_impulse_progress",
    "prior_body_efficiency",
    "body_efficiency_cliff",
    "close_progression_erosion",
    "counter_wick_rejection_pressure",
    "range_progression_divergence",
    "vwap_reversion_tether",
...
MODEL_FAMILY = "fold_local_intraday_body_efficiency_cliff_reversal_rank"
LABEL_SHAPE = "target_first_quality_after_body_efficiency_cliff_reversal"
SELECTION_RULE = "top_fold_local_body_efficiency_cliff_reversal_scores_per_active_day"
...
MIN_DAY_BARS = 18
BODY_EFFICIENCY_CLIFF_REVERSAL_LOOKBACK_BARS = 48
BODY_EFFICIENCY_CLIFF_REVERSAL_MICRO_BARS = 12
MIN_ADVERSE_IMPULSE_PROGRESS = 0.35
MIN_BODY_EFFICIENCY_CLIFF = -0.08
MIN_BODY_EFFICIENCY_CLIFF_REVERSAL_ACTIVITY = 0.05
...
class LinearBodyEfficiencyCliffReversalModel:
    fold_id: str
    ...
    body_efficiency_cliff_reversal_failure_rate: float
...
def run_c0127_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_body_efficiency_cliff_reversal_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_body_efficiency_cliff_reversal_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        BODY_EFFICIENCY_CLIFF_REVERSAL_LOOKBACK_BARS,
        BODY_EFFICIENCY_CLIFF_REVERSAL_MICRO_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = body_efficiency_cliff_reversal_metrics(
    ...
    features = (
        metrics["adverse_impulse_progress"],
        metrics["prior_body_efficiency"],
        metrics["body_efficiency_cliff"],
        metrics["close_progression_erosion"],
        metrics["counter_wick_rejection_pressure"],
        metrics["range_progression_divergence"],
        metrics["vwap_reversion_tether"],
        metrics["session_extension_context"],
        metrics["micro_turn_quality"],
        metrics["volume_effort_fade"],
        metrics["session_maturity"],
        metrics["spread_stress_inverse"],
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        breakout_pressure_bucket(metrics["adverse_impulse_progress"]),
        impulse_efficiency_bucket(metrics["prior_body_efficiency"]),
        body_efficiency_gap_bucket(metrics["body_efficiency_cliff"]),
        close_progression_erosion_bucket(metrics["close_progression_erosion"]),
        counter_wick_pressure_bucket(metrics["counter_wick_rejection_pressure"]),
        range_efficiency_decay_bucket(metrics["range_progression_divergence"]),
        vwap_alignment_bucket(metrics["vwap_reversion_tether"]),
        range_position_pressure_bucket(metrics["session_extension_context"]),
        pullback_containment_bucket(metrics["micro_turn_quality"]),
        participation_decay_bucket(metrics["volume_effort_fade"]),
...
def body_efficiency_cliff_reversal_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    micro_start = max(index - BODY_EFFICIENCY_CLIFF_REVERSAL_MICRO_BARS + 1, context.day_start_index)
    lookback_start = max(index - BODY_EFFICIENCY_CLIFF_REVERSAL_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    adverse_impulse_progress = -direction * (signal_bar.close - lookback_window[0].close) / max(
        average_range * 2.5,
        1e-9,
    )
    if adverse_impulse_progress < MIN_ADVERSE_IMPULSE_PROGRESS:
        return {"event_count": 0.0}

    earlier_start = max(lookback_start, micro_start - BODY_EFFICIENCY_CLIFF_REVERSAL_MICRO_BARS)
    ...
    prior_body_efficiency = base.mean(
        [max(-direction * (bar.close - bar.open) / max(bar.high - bar.low, 1e-9), 0.0) for bar in earlier_window]
    ) or 0.0
    micro_adverse_body_efficiency = base.mean(
        [max(-direction * (bar.close - bar.open) / max(bar.high - bar.low, 1e-9), 0.0) for bar in micro_window]
    ) or 0.0
    body_efficiency_cliff = prior_body_efficiency - micro_adverse_body_efficiency
    if body_efficiency_cliff < MIN_BODY_EFFICIENCY_CLIFF:
        return {"event_count": 0.0}
    ...
    close_progression_erosion = micro_close_location - prior_close_location
    counter_wick_rejection_pressure = base.mean(
        [adverse_wick_absorption(bar, direction) for bar in micro_window]
    ) or 0.0
    adverse_micro_progress = -direction * (signal_bar.close - micro_window[0].close) / max(
        average_range * 1.5,
        1e-9,
    )
    range_progression_divergence = adverse_impulse_progress - max(adverse_micro_progress, 0.0)
    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    vwap_reversion_tether = direction * (session_vwap - signal_bar.close) / max(
        average_range * 3.0,
        1e-9,
    )
    session_extension_context = direction * (session_mid - signal_bar.close) / max(average_range * 3.0, 1e-9)
    ...
    micro_turn_quality = direction * (
        signal_bar.close - micro_window[0].close
    ) / max(path_travel, average_range, 1e-9)
    volume_effort_fade = 1.0 - micro_volume_mean / max(lookback_volume_mean, average_volume, 1e-9)
    ...
    activity = (
        max(adverse_impulse_progress, 0.0) * 0.16
        + max(prior_body_efficiency, 0.0) * 0.13
        + max(body_efficiency_cliff, 0.0) * 0.14
        + max(close_progression_erosion, 0.0) * 0.12
        + counter_wick_rejection_pressure * 0.12
        + max(range_progression_divergence, 0.0) * 0.10
        + max(vwap_reversion_tether, 0.0) * 0.08
        + max(session_extension_context, 0.0) * 0.06
        + max(micro_turn_quality, 0.0) * 0.06
        + max(volume_effort_fade, 0.0) * 0.03
    )
    if activity < MIN_BODY_EFFICIENCY_CLIFF_REVERSAL_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "adverse_impulse_progress": bounded(adverse_impulse_progress, -4.0, 6.0),
        "prior_body_efficiency": bounded(prior_body_efficiency, 0.0, 1.0),
        "body_efficiency_cliff": bounded(body_efficiency_cliff, -1.0, 1.0),
        "close_progression_erosion": bounded(close_progression_erosion, -1.0, 1.0),
        "counter_wick_rejection_pressure": bounded(counter_wick_rejection_pressure, 0.0, 1.0),
        "range_progression_divergence": bounded(range_progression_divergence, -4.0, 6.0),
        "vwap_reversion_tether": bounded(vwap_reversion_tether, -4.0, 4.0),
        "session_extension_context": bounded(session_extension_context, -4.0, 4.0),
        "prior_day_closure_pressure": bounded(prior_day_closure_pressure, -3.0, 3.0),
        "micro_turn_quality": bounded(micro_turn_quality, -2.0, 2.0),
        "volume_effort_fade": bounded(volume_effort_fade, -4.0, 4.0),
        "range_stretch_pressure": bounded(adverse_impulse_progress, 0.0, 8.0),
        ...
        "body_efficiency_cliff_reversal_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0127_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0127_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0127_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0127_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0127_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0127_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0127_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0127_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0127_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0127_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0127_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0127_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0127_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0127_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0127_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0127_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0127_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0127_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0127_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def c0127_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    impulse_reward = 0.10 if metrics["adverse_impulse_progress"] > 0.65 and terminal > 0 else 0.0
    prior_efficiency_reward = 0.08 if metrics["prior_body_efficiency"] > 0.28 and terminal > 0 else 0.0
    cliff_reward = 0.14 if metrics["body_efficiency_cliff"] > 0.08 and terminal > 0 else 0.0
    close_erosion_reward = 0.12 if metrics["close_progression_erosion"] > 0.08 and terminal > 0 else 0.0
    wick_reward = 0.10 if metrics["counter_wick_rejection_pressure"] > 0.22 and terminal > 0 else 0.0
    progression_reward = 0.08 if metrics["range_progression_divergence"] > 0.24 and terminal > 0 else 0.0
    vwap_reward = 0.08 if metrics["vwap_reversion_tether"] > -0.05 and terminal > 0 else 0.0
    extension_reward = 0.05 if metrics["session_extension_context"] > 0.03 and terminal > 0 else 0.0
    path_reversal_reward = 0.12 if metrics["micro_turn_quality"] > 0.02 and terminal > 0 else 0.0
    effort_fade_reward = 0.05 if metrics["volume_effort_fade"] > -0.10 and terminal > 0 else 0.0
    weak_impulse_penalty = 0.10 if metrics["adverse_impulse_progress"] < 0.45 and terminal <= 0 else 0.0
    weak_cliff_penalty = 0.12 if metrics["body_efficiency_cliff"] < 0.00 and terminal <= 0 else 0.0
    weak_close_penalty = 0.10 if metrics["close_progression_erosion"] < -0.06 and terminal <= 0 else 0.0
    weak_wick_penalty = 0.08 if metrics["counter_wick_rejection_pressure"] < 0.12 and terminal <= 0 else 0.0
    weak_vwap_penalty = 0.06 if metrics["vwap_reversion_tether"] < -0.22 and terminal <= 0 else 0.0
    weak_reversal_penalty = 0.10 if metrics["micro_turn_quality"] < -0.08 and terminal <= 0 else 0.0
    ...
    return (
        ...
        + path_quality
        + impulse_reward
        + prior_efficiency_reward
        + cliff_reward
        + close_erosion_reward
        + wick_reward
        + progression_reward
        + vwap_reward
        + extension_reward
        ...
        - max(0.0, MIN_ADVERSE_IMPULSE_PROGRESS - metrics["adverse_impulse_progress"]) * 0.12
        - max(0.0, MIN_BODY_EFFICIENCY_CLIFF - metrics["body_efficiency_cliff"]) * 0.12
    )


def fit_linear_body_efficiency_cliff_reversal_model(candidates: list[base.Candidate], fold_id: str) -> LinearBodyEfficiencyCliffReversalModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearBodyEfficiencyCliffReversalModel(
            fold_id,
    ...
    body_efficiency_cliff_reversal_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearBodyEfficiencyCliffReversalModel(
        fold_id=fold_id,
        ...
        body_efficiency_cliff_reversal_failure_rate=body_efficiency_cliff_reversal_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearBodyEfficiencyCliffReversalModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_body_efficiency_cliff_reversal_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0127_r0001_intraday_body_efficiency_cliff_reversal",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0127_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0127_r0001_body_efficiency_cliff_reversal_summary.json",
        ],
        ...
        "conditional_profiles": {
            "body_efficiency_cliff_reversal_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0127 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0127_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0127 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "body_efficiency_cliff_reversal_lookback_bars": BODY_EFFICIENCY_CLIFF_REVERSAL_LOOKBACK_BARS,
        "body_efficiency_cliff_reversal_micro_bars": BODY_EFFICIENCY_CLIFF_REVERSAL_MICRO_BARS,
        "min_adverse_impulse_progress": MIN_ADVERSE_IMPULSE_PROGRESS,
        "min_body_efficiency_cliff": MIN_BODY_EFFICIENCY_CLIFF,
        "min_body_efficiency_cliff_reversal_activity": MIN_BODY_EFFICIENCY_CLIFF_REVERSAL_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_body_efficiency_cliff_reversal_model_summary(model: LinearBodyEfficiencyCliffReversalModel) -> dict[str, object]:
    return {
        ...
        "body_efficiency_cliff_reversal_failure_rate": rounded(model.body_efficiency_cliff_reversal_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearBodyEfficiencyCliffReversalModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_body_efficiency_cliff_reversal_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "body_efficiency_cliff_reversal_profile": profiles["body_efficiency_cliff_reversal_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0127_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0127_r0001_proxy_trades.csv"
    evidence["body_efficiency_cliff_reversal_summary"] = "artifacts/c0127_r0001_body_efficiency_cliff_reversal_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0127_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0127_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0127 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0127_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0127_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0127_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0127_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0127_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0127_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0127_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0127_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0127_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0127 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0127 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    block = f"""
- decision_id: dec_20260708_produce_c0127_r0001_proxy_evidence
  created_local_date: '{local_date}'
  status: active
  decision: produce_c0127_r0001_proxy_evidence
  refines:
  - dec_20260708_open_c0127_r0001_intraday_body_efficiency_cliff_reversal_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0127_r0001_proxy_records_body_efficiency_cliff_reversal_reference_surface
  ...
  - next_work_is_produce_c0127_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260708_produce_c0127_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0127-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0127_r0001_intraday_body_efficiency_cliff_reversal",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0127-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0127-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0127-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0127_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0127_r0001_intraday_body_efficiency_cliff_reversal",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0127-R0001-BODY-EFFICIENCY-CLIFF-REVERSAL-SUMMARY",
            "artifact_role": "body_efficiency_cliff_reversal_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0127_r0001_body_efficiency_cliff_reversal_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0127_r0001_intraday_body_efficiency_cliff_reversal",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0127_r0001_mt5_logic_parity_evidence",
        }
    ]
