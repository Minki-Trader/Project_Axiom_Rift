"""C0064 R0001 proxy evidence for intraday polarity flip persistence."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0064_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0064_r0001_polarity_flip_summary.json"
...
FEATURE_NAMES = (
    "flip_pressure",
    "flip_cluster_density",
    "continuation_failure_pressure",
    "two_bar_settlement",
    "body_direction_quality",
    "wick_conflict_pressure",
    "close_to_mid_displacement",
    "signed_range_response",
    "volume_confirmation",
    "prior_day_polarity_contrast",
    "session_maturity_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_polarity_flip_persistence_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_polarity_flip_persistence_state"
SELECTION_RULE = "top_fold_local_polarity_flip_persistence_scores_per_active_day"
...
MIN_DAY_BARS = 30
POLARITY_FLIP_LOOKBACK_BARS = 14
POLARITY_FLIP_SHORT_LOOKBACK_BARS = 5
MIN_POLARITY_FLIP_ACTIVITY = 0.10
...
class LinearPolarityFlipPersistenceModel:
    fold_id: str
    ...
    polarity_flip_persistence_failure_rate: float
...
def run_c0064_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_polarity_flip_persistence_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_polarity_flip_persistence_model_summary(model))
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    polarity = polarity_flip_persistence_metrics(
        bars, volumes, context, volume_average, index, direction, average_range
    )
    if polarity["event_count"] <= 0:
        return None, None
    ...
    if volume_ratio is None:
        return None, None
    features = (
        polarity["flip_pressure"],
        polarity["flip_cluster_density"],
        polarity["continuation_failure_pressure"],
        polarity["two_bar_settlement"],
        polarity["body_direction_quality"],
        polarity["wick_conflict_pressure"],
        polarity["close_to_mid_displacement"],
        polarity["signed_range_response"],
        polarity["volume_confirmation"],
        polarity["prior_day_polarity_contrast"],
        polarity["session_maturity_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        flip_pressure_bucket(polarity["flip_pressure"]),
        flip_cluster_bucket(polarity["flip_cluster_density"]),
        continuation_failure_bucket(polarity["continuation_failure_pressure"]),
        settlement_pressure_bucket(polarity["two_bar_settlement"]),
        body_bucket(polarity["body_direction_quality"]),
        wick_conflict_bucket(polarity["wick_conflict_pressure"]),
        close_to_mid_bucket(polarity["close_to_mid_displacement"]),
        signed_range_response_bucket(polarity["signed_range_response"]),
        volume_confirmation_bucket(polarity["volume_confirmation"]),
        prior_day_contrast_bucket(polarity["prior_day_polarity_contrast"]),
        session_pressure_bucket(polarity["session_maturity_pressure"]),
...
def polarity_flip_persistence_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    if prior_volume_per_bar is None or prior_volume_per_bar <= 0:
        return {"event_count": 0.0}
    window_start = index - POLARITY_FLIP_LOOKBACK_BARS + 1
    short_start = index - POLARITY_FLIP_SHORT_LOOKBACK_BARS + 1
    ...
    if len(window) < 6 or len(short_window) < 3:
        return {"event_count": 0.0}
    polarities = [price_sign(item.close - item.open) for item in window]
    short_polarities = polarities[-len(short_window) :]
    pair_count = max(len(polarities) - 1, 1)
    flip_count = sum(
        1
        for previous, current in zip(polarities, polarities[1:])
        if previous != 0.0 and current != 0.0 and previous != current
    )
    flip_into_direction_count = sum(
        1
        for previous, current in zip(polarities, polarities[1:])
        if previous == -direction and current == direction
    )
    failure_count = sum(
        1
        for previous, current in zip(polarities, polarities[1:])
    ...
    flip_pressure = flip_into_direction_count / pair_count
    flip_cluster_density = flip_count / pair_count
    continuation_failure_pressure = failure_count / pair_count
    two_bar_settlement = base.mean([directional_bar_settlement(item, direction) for item in window[-2:]]) or 0.0
    current_body = direction * (signal_bar.close - signal_bar.open) / average_range
    short_body = sum(direction * (item.close - item.open) for item in short_window)
    body_direction_quality = 0.55 * current_body + 0.45 * short_body / max(
        average_range * len(short_window),
        1e-9,
    )
    wick_conflict_pressure = favorable_wick_giveback(signal_bar, direction) - adverse_wick_absorption(
        signal_bar,
        direction,
    )
    ...
    close_to_mid_displacement = direction * (close - window_mid) / average_range
    signed_range_response = direction * (close - window[0].close) / average_range
    volume_window = volumes[short_start : index + 1]
    directional_volume = sum(
        volume for volume, polarity in zip(volume_window, short_polarities) if polarity == direction
    )
    opposing_volume = sum(
        volume for volume, polarity in zip(volume_window, short_polarities) if polarity == -direction
    )
    ...
    if context.prior_day_return is None or context.prior_day_range is None or context.prior_day_range <= 0:
        prior_day_polarity_contrast = 0.0
    else:
        prior_day_polarity_contrast = -direction * context.prior_day_return / max(
    ...
    session_maturity_pressure = bounded_session_progress(signal_bar.time) * max(
        ...
        flip_cluster_density,
        continuation_failure_pressure,
        max(body_direction_quality, 0.0),
    ...
    activity = max(
        ...
        continuation_failure_pressure,
        two_bar_settlement,
        abs(body_direction_quality),
        abs(wick_conflict_pressure),
        abs(close_to_mid_displacement),
        abs(signed_range_response),
        abs(volume_confirmation),
        abs(prior_day_polarity_contrast),
        session_maturity_pressure,
    )
    if activity < MIN_POLARITY_FLIP_ACTIVITY:
        return {"event_count": 0.0}
    if polarities[-1] == -direction and two_bar_settlement < 0.45 and continuation_failure_pressure < 0.10:
        return {"event_count": 0.0}
    return {
        "flip_pressure": bounded(flip_pressure, 0.0, 1.0),
        "flip_cluster_density": bounded(flip_cluster_density, 0.0, 1.0),
        "continuation_failure_pressure": bounded(continuation_failure_pressure, 0.0, 1.0),
        "two_bar_settlement": bounded(two_bar_settlement, 0.0, 1.0),
        "body_direction_quality": bounded(body_direction_quality, -4.0, 4.0),
        "wick_conflict_pressure": bounded(wick_conflict_pressure, -1.0, 1.0),
        "close_to_mid_displacement": bounded(close_to_mid_displacement, -4.0, 4.0),
        "signed_range_response": bounded(signed_range_response, -6.0, 6.0),
        "volume_confirmation": bounded(volume_confirmation, -4.0, 4.0),
        "prior_day_polarity_contrast": bounded(prior_day_polarity_contrast, -2.0, 2.0),
...
def flip_pressure_bucket(value: float) -> str:
    if value < 0.08:
        return "flip_pressure_low"
    if value < 0.22:
        return "flip_pressure_mid"
    return "flip_pressure_high"


def flip_cluster_bucket(value: float) -> str:
    if value < 0.18:
        return "flip_cluster_sparse"
    if value < 0.38:
        return "flip_cluster_present"
    return "flip_cluster_dense"


def continuation_failure_bucket(value: float) -> str:
    if value < 0.08:
        return "continuation_failure_low"
    if value < 0.22:
        return "continuation_failure_mid"
    return "continuation_failure_high"
...
def wick_conflict_bucket(value: float) -> str:
    if value < -0.12:
        return "wick_conflict_favorable"
    if value < 0.18:
        return "wick_conflict_mixed"
    return "wick_conflict_adverse"


def close_to_mid_bucket(value: float) -> str:
    if value < -0.35:
        return "below_polarity_mid"
    if value < 0.35:
        return "near_polarity_mid"
    return "beyond_polarity_mid"


def signed_range_response_bucket(value: float) -> str:
    if value < -0.45:
        return "range_response_against"
    if value < 0.45:
        return "range_response_flat"
    return "range_response_with"
...
def candidate_label(
    ...
    metrics = polarity_flip_persistence_metrics(
        bars,
        volumes,
    ...
    if metrics["event_count"] <= 0:
        return None
    flip_reward = 0.22 if metrics["flip_pressure"] > 0.08 and terminal > 0 else 0.0
    cluster_reward = 0.12 if metrics["flip_cluster_density"] > 0.18 and terminal > 0 else 0.0
    failure_reward = 0.18 if metrics["continuation_failure_pressure"] > 0.12 and terminal > 0 else 0.0
    settlement_reward = 0.14 if metrics["two_bar_settlement"] > 0.62 and terminal > 0 else 0.0
    body_reward = 0.14 if metrics["body_direction_quality"] > 0.18 and terminal > 0 else 0.0
    volume_reward = 0.10 if metrics["volume_confirmation"] > 0.18 and terminal > 0 else 0.0
    prior_contrast_reward = 0.06 if metrics["prior_day_polarity_contrast"] > 0.18 and terminal > 0 else 0.0
    failed_flip_penalty = (
        0.26
        if metrics["flip_pressure"] < 0.05
        and metrics["continuation_failure_pressure"] < 0.08
        and terminal <= 0
        else 0.0
    )
    adverse_wick_penalty = 0.18 if metrics["wick_conflict_pressure"] > 0.25 and terminal <= 0 else 0.0
    weak_body_penalty = 0.18 if metrics["body_direction_quality"] < -0.08 and terminal <= 0 else 0.0
    failed_settlement_penalty = 0.20 if metrics["two_bar_settlement"] < 0.42 and terminal <= 0 else 0.0
    chase_penalty = 0.16 if metrics["close_to_mid_displacement"] > 1.35 and terminal <= 0 else 0.0
...
def fit_linear_polarity_flip_persistence_model(candidates: list[base.Candidate], fold_id: str) -> LinearPolarityFlipPersistenceModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearPolarityFlipPersistenceModel(
            fold_id,
    ...
    polarity_flip_persistence_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearPolarityFlipPersistenceModel(
        fold_id=fold_id,
        ...
        polarity_flip_persistence_failure_rate=polarity_flip_persistence_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearPolarityFlipPersistenceModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_polarity_flip_persistence_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0064_r0001_intraday_polarity_flip_persistence",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0064_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0064_r0001_polarity_flip_summary.json",
        ],
        ...
        "conditional_profiles": {
            "polarity_flip_persistence_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0064 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0064_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0064 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "polarity_flip_persistence_lookback_bars": POLARITY_FLIP_LOOKBACK_BARS,
        "polarity_flip_persistence_short_lookback_bars": POLARITY_FLIP_SHORT_LOOKBACK_BARS,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_polarity_flip_persistence_model_summary(model: LinearPolarityFlipPersistenceModel) -> dict[str, object]:
    return {
        ...
        "polarity_flip_persistence_failure_rate": rounded(model.polarity_flip_persistence_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearPolarityFlipPersistenceModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_polarity_flip_persistence_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "polarity_flip_persistence_profile": profiles["polarity_flip_persistence_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0064_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0064_r0001_proxy_trades.csv"
    evidence["polarity_flip_persistence_summary"] = "artifacts/c0064_r0001_polarity_flip_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0064_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0064_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0064 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0064_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0064_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0064_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0064_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0064_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0064_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0064_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0064_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0064_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0064 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0064 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260706_produce_c0064_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0064_r0001_proxy_evidence
  refines:
  - dec_20260706_open_c0064_r0001_intraday_polarity_flip_persistence_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0064_r0001_proxy_records_polarity_flip_persistence_reference_surface
  ...
  - next_work_is_produce_c0064_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260706_produce_c0064_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0064-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0064_r0001_intraday_polarity_flip_persistence",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0064-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0064-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0064-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0064_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0064_r0001_intraday_polarity_flip_persistence",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0064-R0001-polarity-flip-persistence-SUMMARY",
            "artifact_role": "polarity_flip_persistence_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0064_r0001_polarity_flip_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0064_r0001_intraday_polarity_flip_persistence",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0064_r0001_mt5_logic_parity_evidence",
        }
    ]
