"""C0086 R0001 proxy evidence for intraday opening balance reanchor."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0086_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0086_r0001_opening_balance_reanchor_summary.json"
...
FEATURE_NAMES = (
    "opening_anchor_distance",
    "opposite_side_probe_depth",
    "reanchor_reclaim_strength",
    "opening_range_memory",
    "anchor_wick_absorption",
    "close_location_reanchor",
    "impulse_after_reanchor",
    "participation_reengagement",
    "prior_day_opening_context",
    "session_reanchor_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_opening_balance_reanchor_rank"
LABEL_SHAPE = "target_first_escape_quality_conditioned_on_opening_balance_reanchor_state"
SELECTION_RULE = "top_fold_local_opening_balance_reanchor_scores_per_active_day"
...
MIN_DAY_BARS = 30
OPENING_BALANCE_REANCHOR_LOOKBACK_BARS = 24
OPENING_BALANCE_REANCHOR_SHORT_LOOKBACK_BARS = 8
MIN_OPENING_BALANCE_REANCHOR_ACTIVITY = 0.16
...
class LinearOpeningBalanceReanchorModel:
    fold_id: str
    ...
    opening_balance_reanchor_failure_rate: float
...
def run_c0086_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_opening_balance_reanchor_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_opening_balance_reanchor_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        OPENING_BALANCE_REANCHOR_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = opening_balance_reanchor_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        metrics["opening_anchor_distance"],
        metrics["opposite_side_probe_depth"],
        metrics["reanchor_reclaim_strength"],
        metrics["opening_range_memory"],
        metrics["anchor_wick_absorption"],
        metrics["close_location_reanchor"],
        metrics["impulse_after_reanchor"],
        metrics["participation_reengagement"],
        metrics["prior_day_opening_context"],
        metrics["session_reanchor_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        opening_anchor_distance_bucket(metrics["opening_anchor_distance"]),
        opposite_probe_bucket(metrics["opposite_side_probe_depth"]),
        reanchor_reclaim_bucket(metrics["reanchor_reclaim_strength"]),
        opening_range_memory_bucket(metrics["opening_range_memory"]),
        anchor_wick_absorption_bucket(metrics["anchor_wick_absorption"]),
        close_reanchor_bucket(metrics["close_location_reanchor"]),
        impulse_reanchor_bucket(metrics["impulse_after_reanchor"]),
        volume_reengagement_bucket(metrics["participation_reengagement"]),
        prior_opening_context_bucket(metrics["prior_day_opening_context"]),
        session_reanchor_bucket(metrics["session_reanchor_pressure"]),
...
def opening_balance_reanchor_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    short_start = index - OPENING_BALANCE_REANCHOR_SHORT_LOOKBACK_BARS + 1
    prior_start = index - OPENING_BALANCE_REANCHOR_LOOKBACK_BARS + 1
    ...
    opening_width = max(context.opening_high - context.opening_low, 1e-9)
    opening_midpoint = (context.opening_high + context.opening_low) / 2.0
    if direction > 0:
        opening_anchor_distance = (signal_bar.close - opening_midpoint) / average_range
        opposite_side_probe_depth = (opening_midpoint - short_low) / average_range
        reanchor_reclaim_strength = (signal_bar.close - opening_midpoint) / average_range
        crossed_anchor = short_low <= opening_midpoint and signal_bar.close >= opening_midpoint - 0.05 * average_range
    else:
        opening_anchor_distance = (opening_midpoint - signal_bar.close) / average_range
        opposite_side_probe_depth = (short_high - opening_midpoint) / average_range
        reanchor_reclaim_strength = (opening_midpoint - signal_bar.close) / average_range
        crossed_anchor = short_high >= opening_midpoint and signal_bar.close <= opening_midpoint + 0.05 * average_range
    opening_range_memory = opening_width / average_range
    anchor_wick_absorption = directional_wick_rejection(signal_bar, direction)
    close_location_reanchor = directional_bar_settlement(signal_bar, direction)
    impulse_after_reanchor = direction * (signal_bar.close - bars[index - 1].close) / average_range
    ...
    prior_day_opening_context = direction * (context.day_open - prior_day_close) / max(prior_day_range, 1e-9)
    reanchor_pressure = max(
        max(opposite_side_probe_depth, 0.0) / 2.0,
        max(reanchor_reclaim_strength, 0.0),
        max(anchor_wick_absorption, 0.0),
        max(close_location_reanchor - 0.50, 0.0) * 2.0,
        max(impulse_after_reanchor, 0.0),
        min(max(participation_reengagement, 0.0), 1.0),
    )
    session_reanchor_pressure = bounded_session_progress(signal_bar.time) * reanchor_pressure
    activity = max(reanchor_pressure, session_reanchor_pressure)
    if activity < MIN_OPENING_BALANCE_REANCHOR_ACTIVITY:
        return {"event_count": 0.0}
    if not crossed_anchor and opposite_side_probe_depth < 0.16:
        return {"event_count": 0.0}
    if opposite_side_probe_depth < 0.08:
        return {"event_count": 0.0}
    if reanchor_reclaim_strength < -0.08 and close_location_reanchor < 0.46:
        return {"event_count": 0.0}
    if impulse_after_reanchor < -0.35 and anchor_wick_absorption < -0.05:
        return {"event_count": 0.0}
    return {
        "opening_anchor_distance": bounded(opening_anchor_distance, -2.0, 6.0),
        "opposite_side_probe_depth": bounded(opposite_side_probe_depth, -2.0, 6.0),
        "reanchor_reclaim_strength": bounded(reanchor_reclaim_strength, -2.0, 6.0),
        "opening_range_memory": bounded(opening_range_memory, 0.0, 8.0),
        "anchor_wick_absorption": bounded(anchor_wick_absorption, -1.0, 1.0),
        "close_location_reanchor": bounded(close_location_reanchor, 0.0, 1.0),
        "impulse_after_reanchor": bounded(impulse_after_reanchor, -4.0, 6.0),
        "participation_reengagement": bounded(participation_reengagement, -3.0, 6.0),
        "prior_day_opening_context": bounded(prior_day_opening_context, -2.0, 2.0),
        "session_reanchor_pressure": bounded(session_reanchor_pressure, 0.0, 4.0),
        "event_count": 1.0,
    }
...
def opening_anchor_distance_bucket(value: float) -> str:
    if value < 0.10:
        return "opening_anchor_inside"
    if value < 0.85:
        return "opening_anchor_reclaimed"
    return "opening_anchor_extended"


def opposite_probe_bucket(value: float) -> str:
    if value < 0.15:
        return "opposite_probe_shallow"
    if value < 0.95:
        return "opposite_probe_defined"
    return "opposite_probe_deep"


def reanchor_reclaim_bucket(value: float) -> str:
    if value < -0.04:
        return "reanchor_reclaim_failed"
    if value < 0.35:
        return "reanchor_reclaim_mixed"
    return "reanchor_reclaim_clear"


def opening_range_memory_bucket(value: float) -> str:
    if value < 1.20:
        return "opening_range_quiet"
    if value < 3.20:
        return "opening_range_defined"
    return "opening_range_hot"


def anchor_wick_absorption_bucket(value: float) -> str:
    if value < -0.08:
        return "anchor_wick_against"
    if value < 0.12:
        return "anchor_wick_mixed"
    return "anchor_wick_absorbing"


def close_reanchor_bucket(value: float) -> str:
    if value < 0.45:
        return "close_reanchor_weak"
    if value < 0.66:
        return "close_reanchor_mid"
    return "close_reanchor_strong"


def impulse_reanchor_bucket(value: float) -> str:
    if value < -0.10:
        return "reanchor_impulse_against"
    if value < 0.45:
        return "reanchor_impulse_mixed"
    return "reanchor_impulse_with"


def prior_opening_context_bucket(value: float) -> str:
    if value < -0.18:
        return "prior_opening_conflict"
    if value < 0.35:
        return "prior_opening_neutral"
    return "prior_opening_aligned"


def session_reanchor_bucket(value: float) -> str:
    if value < 0.08:
        return "session_reanchor_light"
    if value < 0.65:
        return "session_reanchor_defined"
    return "session_reanchor_heavy"
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    probe_reward = 0.12 if metrics["opposite_side_probe_depth"] > 0.18 and terminal > 0 else 0.0
    reclaim_reward = 0.14 if metrics["reanchor_reclaim_strength"] > 0.05 and terminal > 0 else 0.0
    close_reward = 0.12 if metrics["close_location_reanchor"] > 0.58 and terminal > 0 else 0.0
    impulse_reward = 0.10 if metrics["impulse_after_reanchor"] > 0.10 and terminal > 0 else 0.0
    wick_reward = 0.08 if metrics["anchor_wick_absorption"] > 0.05 and terminal > 0 else 0.0
    participation_reward = 0.08 if metrics["participation_reengagement"] > 0.03 and terminal > 0 else 0.0
    prior_context_reward = 0.05 if metrics["prior_day_opening_context"] > -0.15 and terminal > 0 else 0.0
    session_reward = 0.05 if metrics["session_reanchor_pressure"] > 0.12 and terminal > 0 else 0.0
    shallow_probe_penalty = 0.10 if metrics["opposite_side_probe_depth"] < 0.12 and terminal <= 0 else 0.0
    failed_reclaim_penalty = 0.12 if metrics["reanchor_reclaim_strength"] < -0.04 and terminal <= 0 else 0.0
    weak_close_penalty = 0.10 if metrics["close_location_reanchor"] < 0.45 and terminal <= 0 else 0.0
    adverse_impulse_penalty = 0.10 if metrics["impulse_after_reanchor"] < -0.12 and terminal <= 0 else 0.0
    weak_wick_penalty = 0.08 if metrics["anchor_wick_absorption"] < -0.08 and terminal <= 0 else 0.0
    no_participation_penalty = 0.08 if metrics["participation_reengagement"] < -0.06 and terminal <= 0 else 0.0
    hot_opening_penalty = 0.04 if metrics["opening_range_memory"] > 4.00 and terminal <= 0 else 0.0
    ...
    opening_balance_reanchor_path_quality = (0.52 * terminal + 0.34 * mfe - 0.68 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + opening_balance_reanchor_path_quality
        + probe_reward
        + reclaim_reward
        + close_reward
        + impulse_reward
        ...
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["opening_anchor_distance"] - 3.00) * 0.04
    )


def fit_linear_opening_balance_reanchor_model(candidates: list[base.Candidate], fold_id: str) -> LinearOpeningBalanceReanchorModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearOpeningBalanceReanchorModel(
            fold_id,
    ...
    opening_balance_reanchor_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearOpeningBalanceReanchorModel(
        fold_id=fold_id,
        ...
        opening_balance_reanchor_failure_rate=opening_balance_reanchor_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearOpeningBalanceReanchorModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_opening_balance_reanchor_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0086_r0001_intraday_opening_balance_reanchor",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0086_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0086_r0001_opening_balance_reanchor_summary.json",
        ],
        ...
        "conditional_profiles": {
            "opening_balance_reanchor_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0086 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0086_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0086 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "opening_balance_reanchor_lookback_bars": OPENING_BALANCE_REANCHOR_LOOKBACK_BARS,
        "opening_balance_reanchor_short_lookback_bars": OPENING_BALANCE_REANCHOR_SHORT_LOOKBACK_BARS,
        "min_opening_balance_reanchor_activity": MIN_OPENING_BALANCE_REANCHOR_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_opening_balance_reanchor_model_summary(model: LinearOpeningBalanceReanchorModel) -> dict[str, object]:
    return {
        ...
        "opening_balance_reanchor_failure_rate": rounded(model.opening_balance_reanchor_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearOpeningBalanceReanchorModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_opening_balance_reanchor_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "opening_balance_reanchor_profile": profiles["opening_balance_reanchor_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0086_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0086_r0001_proxy_trades.csv"
    evidence["opening_balance_reanchor_summary"] = "artifacts/c0086_r0001_opening_balance_reanchor_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0086_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0086_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0086 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0086_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0086_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0086_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0086_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0086_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0086_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0086_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0086_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0086_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0086 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0086 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0086_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0086_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0086_r0001_intraday_opening_balance_reanchor_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0086_r0001_proxy_records_opening_balance_reanchor_reference_surface
  ...
  - next_work_is_produce_c0086_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0086_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0086-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0086_r0001_intraday_opening_balance_reanchor",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0086-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0086-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0086-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0086_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0086_r0001_intraday_opening_balance_reanchor",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0086-R0001-opening-balance-reanchor-SUMMARY",
            "artifact_role": "opening_balance_reanchor_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0086_r0001_opening_balance_reanchor_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0086_r0001_intraday_opening_balance_reanchor",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0086_r0001_mt5_logic_parity_evidence",
        }
    ]
