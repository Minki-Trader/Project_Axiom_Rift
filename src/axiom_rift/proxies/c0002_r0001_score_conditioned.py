"""C0002 R0001 proxy evidence for score-conditioned candidate selection."""
...
MIN_SCORE = 0.0
STOP_RANGE_MULTIPLE = 0.8
TARGET_RANGE_MULTIPLE = 1.2
...
def run_c0002_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        if average_range is None or average_range <= 0:
            continue
        direction = momentum_direction(bars, index)
        if direction == 0:
            continue
...
def candidate_label(
    ...
    return (0.55 * terminal + 0.35 * mfe - 0.45 * mae - bars[entry_index].spread_points) / average_range
...
def build_proxy_payload(
    ...
    return {
        ...
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0002_r0001_score_conditioned",
        "proxy_config_path": "campaigns/C0002_score_conditioned_intraday_edge/runs/R0001/run_manifest.json",
        ...
        "conditional_profiles": {
            "score_surface_profile": {
                "applies": True,
                "fields": {
                    "score_type": "train_only_linear_correlation_score",
                    "training_scope": "rolling_train_is_only",
                    "feature_count": len(FEATURE_NAMES),
                    "feature_names": list(FEATURE_NAMES),
                    "feature_family_summary": "closed_bar_price_session_volatility_momentum_path_context",
        ...
        "deferred_with_reason": [
            {
                ...
                "revisit_when": "during C0002 R0001 MT5 evidence production",
            ...
            },
            {
                ...
                "revisit_when": "before C0002 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
        "claim_boundary": {
...
def update_artifact_lineage(proxy_hash: str) -> None:
    ...
    for record in data.get("artifact_records", []):
        if record.get("artifact_id") == "A0002":
            ...
            record["produced_by"] = "axiom_rift.proxies.c0002_r0001_score_conditioned"
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0002_r0001_mt5_logic_parity_evidence",
        }
    ]
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0002_r0001_mt5_logic_parity_evidence"
    data["deferred_with_reason"] = [
        {
            "field": "decision",
            ...
            "revisit_when": "after C0002 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    text = text.replace("remaining_question: produce_r0001_score_conditioned_proxy_evidence", "remaining_question: produce_c0002_r0001_mt5_logic_parity_evidence")
