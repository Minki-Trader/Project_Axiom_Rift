"""C0130 R0001 proxy evidence for intraday session skew normalization reversal."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0130_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0130_r0001_session_skew_normalization_reversal_summary.json"
...
FEATURE_NAMES = (
    "opposing_session_extension",
    "skew_normalization_velocity",
    "vwap_reversion_room",
    "midpoint_reversion_room",
    "range_skew_pressure",
    "tail_rejection_bias",
    "close_settlement_repair",
    "volume_normalization_ratio",
    "path_normalization_smoothness",
    "prior_day_opposition_context",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_session_skew_normalization_reversal_rank"
LABEL_SHAPE = "target_first_quality_after_session_skew_normalization_reversal"
SELECTION_RULE = "top_fold_local_session_skew_normalization_reversal_scores_per_active_day"
...
MIN_DAY_BARS = 18
SESSION_SKEW_NORMALIZATION_REVERSAL_LOOKBACK_BARS = 48
SESSION_SKEW_NORMALIZATION_REVERSAL_MICRO_BARS = 12
MIN_OPPOSING_SESSION_EXTENSION = 0.48
MIN_REVERSION_ROOM = -0.10
MIN_SESSION_SKEW_NORMALIZATION_REVERSAL_ACTIVITY = 0.05
...
class LinearSessionSkewNormalizationReversalModel:
    fold_id: str
    ...
    session_skew_normalization_reversal_failure_rate: float
...
def run_c0130_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_session_skew_normalization_reversal_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_session_skew_normalization_reversal_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        SESSION_SKEW_NORMALIZATION_REVERSAL_LOOKBACK_BARS,
        SESSION_SKEW_NORMALIZATION_REVERSAL_MICRO_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = session_skew_normalization_reversal_metrics(
    ...
    features = (
        metrics["opposing_session_extension"],
        metrics["skew_normalization_velocity"],
        metrics["vwap_reversion_room"],
        metrics["midpoint_reversion_room"],
        metrics["range_skew_pressure"],
        metrics["tail_rejection_bias"],
        metrics["close_settlement_repair"],
        metrics["volume_normalization_ratio"],
        metrics["path_normalization_smoothness"],
        metrics["prior_day_opposition_context"],
        metrics["session_maturity"],
        metrics["spread_stress_inverse"],
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        extension_pressure_bucket(metrics["opposing_session_extension"]),
        drift_efficiency_bucket(metrics["skew_normalization_velocity"]),
        vwap_reversion_depth_bucket(metrics["vwap_reversion_room"]),
        midpoint_distance_bucket(metrics["midpoint_reversion_room"]),
        range_position_pressure_bucket(metrics["range_skew_pressure"]),
        adverse_absorption_bucket(metrics["tail_rejection_bias"]),
        close_location_confirm_bucket(metrics["close_settlement_repair"]),
        volume_confirmation_bucket(metrics["volume_normalization_ratio"]),
        path_efficiency_bucket(metrics["path_normalization_smoothness"]),
        prior_day_contrarian_bucket(metrics["prior_day_opposition_context"]),
...
def session_skew_normalization_reversal_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    micro_start = max(index - SESSION_SKEW_NORMALIZATION_REVERSAL_MICRO_BARS + 1, context.day_start_index)
    lookback_start = max(index - SESSION_SKEW_NORMALIZATION_REVERSAL_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    if session_range <= average_range * 0.50:
        return {"event_count": 0.0}

    session_mid = (context.high_so_far + context.low_so_far) / 2.0
    session_position = (signal_bar.close - context.low_so_far) / max(session_range, 1e-9)
    directional_position = directional_session_percentile_position(
        signal_bar.close,
        context.low_so_far,
        context.high_so_far,
        direction,
    )
    opposing_session_extension = 1.0 - directional_position
    if opposing_session_extension < MIN_OPPOSING_SESSION_EXTENSION:
        return {"event_count": 0.0}

    micro_delta = direction * (signal_bar.close - micro_window[0].close)
    skew_normalization_velocity = micro_delta / max(average_range * 1.5, 1e-9)

    def directional_bar_settlement(window_bar: base.Bar) -> float:
    ...
    path_normalization_smoothness = micro_delta / max(path_travel, average_range, 1e-9)
    tail_rejection_bias = (
        base.mean([adverse_wick_absorption(bar, direction) for bar in micro_window]) or 0.0
    )
    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    vwap_reversion_room = direction * (session_vwap - signal_bar.close) / max(average_range * 2.5, 1e-9)
    midpoint_reversion_room = direction * (session_mid - signal_bar.close) / max(average_range * 2.5, 1e-9)
    if max(vwap_reversion_room, midpoint_reversion_room) < MIN_REVERSION_ROOM:
        return {"event_count": 0.0}
    range_skew_pressure = abs(session_position - 0.5) * 2.0
    volume_normalization_ratio = micro_volume_mean / max(lookback_volume_mean, average_volume, 1e-9)
    close_settlement_repair = base.mean([directional_bar_settlement(bar) for bar in micro_window]) or 0.0
    prior_day_opposition_context = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_day_opposition_context = -direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
    if context.prior_day_shadow_balance is not None:
        prior_day_opposition_context -= direction * context.prior_day_shadow_balance * 0.05
    ...
    activity = (
        opposing_session_extension * 0.16
        + max(skew_normalization_velocity, 0.0) * 0.14
        + max(vwap_reversion_room, 0.0) * 0.13
        + max(midpoint_reversion_room, 0.0) * 0.11
        + range_skew_pressure * 0.10
        + tail_rejection_bias * 0.11
        + close_settlement_repair * 0.09
        + max(1.80 - volume_normalization_ratio, 0.0) * 0.04
        + max(path_normalization_smoothness, 0.0) * 0.08
        + max(prior_day_opposition_context, 0.0) * 0.04
    )
    if activity < MIN_SESSION_SKEW_NORMALIZATION_REVERSAL_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "opposing_session_extension": bounded(opposing_session_extension, 0.0, 1.0),
        "skew_normalization_velocity": bounded(skew_normalization_velocity, -4.0, 6.0),
        "vwap_reversion_room": bounded(vwap_reversion_room, -4.0, 4.0),
        "midpoint_reversion_room": bounded(midpoint_reversion_room, -4.0, 4.0),
        "range_skew_pressure": bounded(range_skew_pressure, 0.0, 1.0),
        "tail_rejection_bias": bounded(tail_rejection_bias, 0.0, 1.0),
        "close_settlement_repair": bounded(close_settlement_repair, 0.0, 1.0),
        "volume_normalization_ratio": bounded(volume_normalization_ratio, 0.0, 8.0),
        "path_normalization_smoothness": bounded(path_normalization_smoothness, -2.0, 2.0),
        "prior_day_opposition_context": bounded(prior_day_opposition_context, -3.0, 3.0),
        "range_stretch_pressure": bounded(opposing_session_extension * session_range / max(average_range, 1e-9), 0.0, 8.0),
        ...
        "session_skew_normalization_reversal_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0130_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0130_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0130_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0130_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0130_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0130_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0130_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0130_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0130_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0130_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0130_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0130_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0130_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0130_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0130_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0130_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0130_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0130_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0130_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def c0130_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    extension_reward = 0.12 if metrics["opposing_session_extension"] > 0.62 and terminal > 0 else 0.0
    velocity_reward = 0.11 if metrics["skew_normalization_velocity"] > 0.10 and terminal > 0 else 0.0
    vwap_room_reward = 0.10 if metrics["vwap_reversion_room"] > 0.04 and terminal > 0 else 0.0
    midpoint_room_reward = 0.09 if metrics["midpoint_reversion_room"] > 0.04 and terminal > 0 else 0.0
    skew_pressure_reward = 0.07 if metrics["range_skew_pressure"] > 0.44 and terminal > 0 else 0.0
    tail_reward = 0.09 if metrics["tail_rejection_bias"] > 0.18 and terminal > 0 else 0.0
    settlement_reward = 0.10 if metrics["close_settlement_repair"] > 0.54 and terminal > 0 else 0.0
    volume_reward = 0.05 if 0.55 <= metrics["volume_normalization_ratio"] <= 2.80 and terminal > 0 else 0.0
    smoothness_reward = 0.09 if metrics["path_normalization_smoothness"] > 0.05 and terminal > 0 else 0.0
    prior_day_reward = 0.04 if metrics["prior_day_opposition_context"] > -0.10 and terminal > 0 else 0.0
    weak_extension_penalty = 0.09 if metrics["opposing_session_extension"] < 0.54 and terminal <= 0 else 0.0
    weak_velocity_penalty = 0.08 if metrics["skew_normalization_velocity"] < -0.04 and terminal <= 0 else 0.0
    weak_vwap_room_penalty = 0.07 if metrics["vwap_reversion_room"] < -0.04 and terminal <= 0 else 0.0
    weak_midpoint_room_penalty = 0.07 if metrics["midpoint_reversion_room"] < -0.04 and terminal <= 0 else 0.0
    weak_settlement_penalty = 0.08 if metrics["close_settlement_repair"] < 0.45 and terminal <= 0 else 0.0
    weak_smoothness_penalty = 0.08 if metrics["path_normalization_smoothness"] < -0.08 and terminal <= 0 else 0.0
    overheated_volume_penalty = 0.05 if metrics["volume_normalization_ratio"] > 3.80 and terminal <= 0 else 0.0
    ...
    return (
        ...
        + velocity_reward
        + vwap_room_reward
        + midpoint_room_reward
        + skew_pressure_reward
        + tail_reward
        + settlement_reward
        + volume_reward
        + smoothness_reward
        ...
        - weak_vwap_room_penalty
        - weak_midpoint_room_penalty
        - weak_settlement_penalty
        - weak_smoothness_penalty
        ...
        - max(0.0, metrics["volume_normalization_ratio"] - 3.80) * 0.015
        - max(0.0, MIN_OPPOSING_SESSION_EXTENSION - metrics["opposing_session_extension"]) * 0.12
        - max(0.0, MIN_REVERSION_ROOM - max(metrics["vwap_reversion_room"], metrics["midpoint_reversion_room"])) * 0.08
    )


def fit_linear_session_skew_normalization_reversal_model(candidates: list[base.Candidate], fold_id: str) -> LinearSessionSkewNormalizationReversalModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearSessionSkewNormalizationReversalModel(
            fold_id,
    ...
    session_skew_normalization_reversal_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearSessionSkewNormalizationReversalModel(
        fold_id=fold_id,
        ...
        session_skew_normalization_reversal_failure_rate=session_skew_normalization_reversal_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearSessionSkewNormalizationReversalModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_session_skew_normalization_reversal_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0130_r0001_intraday_session_skew_normalization_reversal",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0130_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0130_r0001_session_skew_normalization_reversal_summary.json",
        ],
        ...
        "conditional_profiles": {
            "session_skew_normalization_reversal_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0130 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0130_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0130 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "session_skew_normalization_reversal_lookback_bars": SESSION_SKEW_NORMALIZATION_REVERSAL_LOOKBACK_BARS,
        "session_skew_normalization_reversal_micro_bars": SESSION_SKEW_NORMALIZATION_REVERSAL_MICRO_BARS,
        "min_opposing_session_extension": MIN_OPPOSING_SESSION_EXTENSION,
        "min_reversion_room": MIN_REVERSION_ROOM,
        "min_session_skew_normalization_reversal_activity": MIN_SESSION_SKEW_NORMALIZATION_REVERSAL_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_session_skew_normalization_reversal_model_summary(model: LinearSessionSkewNormalizationReversalModel) -> dict[str, object]:
    return {
        ...
        "session_skew_normalization_reversal_failure_rate": rounded(model.session_skew_normalization_reversal_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearSessionSkewNormalizationReversalModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_session_skew_normalization_reversal_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "session_skew_normalization_reversal_profile": profiles["session_skew_normalization_reversal_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0130_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0130_r0001_proxy_trades.csv"
    evidence["session_skew_normalization_reversal_summary"] = "artifacts/c0130_r0001_session_skew_normalization_reversal_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0130_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0130_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0130 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0130_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0130_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0130_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0130_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0130_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0130_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0130_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0130_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0130_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0130 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0130 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    block = f"""
- decision_id: dec_20260708_produce_c0130_r0001_proxy_evidence
  created_local_date: '{local_date}'
  status: active
  decision: produce_c0130_r0001_proxy_evidence
  refines:
  - dec_20260708_open_c0130_r0001_intraday_session_skew_normalization_reversal_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0130_r0001_proxy_records_session_skew_normalization_reversal_reference_surface
  ...
  - next_work_is_produce_c0130_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260708_produce_c0130_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0130-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0130_r0001_intraday_session_skew_normalization_reversal",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0130-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0130-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0130-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0130_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0130_r0001_intraday_session_skew_normalization_reversal",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0130-R0001-DIRECTIONAL-CURVATURE-CONTINUATION-SUMMARY",
            "artifact_role": "session_skew_normalization_reversal_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0130_r0001_session_skew_normalization_reversal_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0130_r0001_intraday_session_skew_normalization_reversal",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0130_r0001_mt5_logic_parity_evidence",
        }
    ]
