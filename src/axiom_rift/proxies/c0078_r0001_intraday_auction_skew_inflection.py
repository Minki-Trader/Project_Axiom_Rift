"""C0078 R0001 proxy evidence for intraday auction skew inflection."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0078_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0078_r0001_asi_summary.json"
...
FEATURE_NAMES = (
    "short_body_skew",
    "medium_body_skew",
    "skew_inflection_delta",
    "wick_rejection_flip",
    "close_location_shift",
    "range_expansion_confirmation",
    "participation_followthrough",
    "adverse_tail_inverse",
    "prior_day_skew_context",
    "session_maturity_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_auction_skew_inflection_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_auction_skew_inflection_state"
SELECTION_RULE = "top_fold_local_auction_skew_inflection_scores_per_active_day"
...
MIN_DAY_BARS = 30
AUCTION_SKEW_INFLECTION_LOOKBACK_BARS = 24
AUCTION_SKEW_INFLECTION_SHORT_LOOKBACK_BARS = 8
MIN_AUCTION_SKEW_INFLECTION_ACTIVITY = 0.22
...
class LinearAuctionSkewInflectionModel:
    fold_id: str
    ...
    auction_skew_inflection_failure_rate: float
...
def run_c0078_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_auction_skew_inflection_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_auction_skew_inflection_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        AUCTION_SKEW_INFLECTION_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    release = auction_skew_inflection_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        release["short_body_skew"],
        release["medium_body_skew"],
        release["skew_inflection_delta"],
        release["wick_rejection_flip"],
        release["close_location_shift"],
        release["range_expansion_confirmation"],
        release["participation_followthrough"],
        release["adverse_tail_inverse"],
        release["prior_day_skew_context"],
        release["session_maturity_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        body_skew_bucket(release["short_body_skew"], "short_body"),
        body_skew_bucket(release["medium_body_skew"], "medium_body"),
        skew_delta_bucket(release["skew_inflection_delta"]),
        wick_flip_bucket(release["wick_rejection_flip"]),
        close_shift_bucket(release["close_location_shift"]),
        range_confirmation_bucket(release["range_expansion_confirmation"]),
        participation_followthrough_bucket(release["participation_followthrough"]),
        adverse_tail_bucket(release["adverse_tail_inverse"]),
        prior_skew_context_bucket(release["prior_day_skew_context"]),
...
def auction_skew_inflection_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    medium_start = index - AUCTION_SKEW_INFLECTION_LOOKBACK_BARS + 1
    short_start = index - AUCTION_SKEW_INFLECTION_SHORT_LOOKBACK_BARS + 1
    previous_start = short_start - AUCTION_SKEW_INFLECTION_SHORT_LOOKBACK_BARS
    previous_end = short_start
    if medium_start < 0 or short_start < 0 or previous_start < 0:
        return {"event_count": 0.0}
    medium_window = bars[medium_start : index + 1]
    short_window = bars[short_start : index + 1]
    previous_window = bars[previous_start:previous_end]
    if len(medium_window) < 12 or len(short_window) < 3 or len(previous_window) < 3:
        return {"event_count": 0.0}
    average_volume = volume_average[index]
    if average_volume is None or average_volume <= 0:
        return {"event_count": 0.0}

    short_body_skew = base.mean([directional_body_score(bar, direction) for bar in short_window]) or 0.0
    medium_body_skew = base.mean([directional_body_score(bar, direction) for bar in medium_window]) or 0.0
    previous_wick_rejection = base.mean([directional_wick_rejection(bar, direction) for bar in previous_window]) or 0.0
    short_wick_rejection = base.mean([directional_wick_rejection(bar, direction) for bar in short_window]) or 0.0
    previous_close_location = base.mean([directional_bar_settlement(bar, direction) for bar in previous_window]) or 0.0
    short_close_location = base.mean([directional_bar_settlement(bar, direction) for bar in short_window]) or 0.0
    short_range_mean = base.mean([bar.high - bar.low for bar in short_window]) or average_range
    medium_range_mean = base.mean([bar.high - bar.low for bar in medium_window]) or average_range
    short_volume_mean = base.mean(volumes[short_start : index + 1]) or average_volume
    skew_inflection_delta = short_body_skew - medium_body_skew
    wick_rejection_flip = short_wick_rejection - previous_wick_rejection
    close_location_shift = short_close_location - previous_close_location
    range_expansion_confirmation = short_range_mean / max(medium_range_mean, 1e-9)
    participation_followthrough = (short_volume_mean / max(average_volume, 1e-9)) - 1.0
    adverse_tail_pressure = base.mean([favorable_wick_giveback(bar, direction) for bar in short_window]) or 0.0
    adverse_tail_inverse = 1.0 / (1.0 + adverse_tail_pressure * 3.0)
    prior_day_skew_context = direction * prior_day_return / max(prior_day_range, 1e-9)
    skew_pressure = max(
        max(short_body_skew, 0.0),
        max(skew_inflection_delta, 0.0),
        max(wick_rejection_flip, 0.0),
        max(close_location_shift, 0.0),
        min(max(range_expansion_confirmation, 0.0) / 2.0, 1.0),
        min(max(participation_followthrough, 0.0) / 2.0, 1.0),
        adverse_tail_inverse,
    )
    session_maturity_pressure = bounded_session_progress(signal_bar.time) * skew_pressure
    activity = max(skew_pressure, session_maturity_pressure)
    if activity < MIN_AUCTION_SKEW_INFLECTION_ACTIVITY:
        return {"event_count": 0.0}
    if short_body_skew < -0.05 and skew_inflection_delta < 0.03:
        return {"event_count": 0.0}
    if skew_inflection_delta < -0.08 and close_location_shift < 0.03:
        return {"event_count": 0.0}
    return {
        "short_body_skew": bounded(short_body_skew, -1.0, 1.0),
        "medium_body_skew": bounded(medium_body_skew, -1.0, 1.0),
        "skew_inflection_delta": bounded(skew_inflection_delta, -1.0, 1.0),
        "wick_rejection_flip": bounded(wick_rejection_flip, -1.0, 1.0),
        "close_location_shift": bounded(close_location_shift, -1.0, 1.0),
        "range_expansion_confirmation": bounded(range_expansion_confirmation, 0.0, 6.0),
        "participation_followthrough": bounded(participation_followthrough, -1.0, 4.0),
        "adverse_tail_inverse": bounded(adverse_tail_inverse, 0.0, 1.0),
        "prior_day_skew_context": bounded(prior_day_skew_context, -2.0, 2.0),
...
def candidate_label(
    ...
    metrics = auction_skew_inflection_metrics(
        bars,
        volumes,
    ...
    if metrics["event_count"] <= 0:
        return None
    short_skew_reward = 0.15 if metrics["short_body_skew"] > 0.10 and terminal > 0 else 0.0
    inflection_reward = 0.16 if metrics["skew_inflection_delta"] > 0.08 and terminal > 0 else 0.0
    wick_flip_reward = 0.11 if metrics["wick_rejection_flip"] > 0.06 and terminal > 0 else 0.0
    close_shift_reward = 0.13 if metrics["close_location_shift"] > 0.06 and terminal > 0 else 0.0
    range_confirmation_reward = (
        0.10 if 0.82 <= metrics["range_expansion_confirmation"] <= 2.40 and terminal > 0 else 0.0
    )
    participation_reward = 0.08 if metrics["participation_followthrough"] > -0.05 and terminal > 0 else 0.0
    adverse_tail_reward = 0.12 if metrics["adverse_tail_inverse"] > 0.60 and terminal > 0 else 0.0
    prior_context_reward = 0.06 if metrics["prior_day_skew_context"] > -0.25 and terminal > 0 else 0.0
    short_skew_penalty = 0.13 if metrics["short_body_skew"] < -0.08 and terminal <= 0 else 0.0
    inflection_fade_penalty = 0.16 if metrics["skew_inflection_delta"] < -0.08 and terminal <= 0 else 0.0
    wick_worsen_penalty = 0.11 if metrics["wick_rejection_flip"] < -0.08 and terminal <= 0 else 0.0
    close_shift_penalty = 0.13 if metrics["close_location_shift"] < -0.06 and terminal <= 0 else 0.0
    range_underconfirm_penalty = 0.09 if metrics["range_expansion_confirmation"] < 0.70 and terminal <= 0 else 0.0
    participation_fade_penalty = 0.09 if metrics["participation_followthrough"] < -0.18 and terminal <= 0 else 0.0
    adverse_tail_penalty = 0.13 if metrics["adverse_tail_inverse"] < 0.44 and terminal <= 0 else 0.0
    ...
    auction_skew_inflection_path_quality = (0.44 * terminal + 0.36 * mfe - 0.72 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + auction_skew_inflection_path_quality
        + short_skew_reward
        + inflection_reward
        + wick_flip_reward
        + close_shift_reward
        ...
        - short_skew_penalty
        - inflection_fade_penalty
        - wick_worsen_penalty
        - close_shift_penalty
        - range_underconfirm_penalty
        - participation_fade_penalty
        - adverse_tail_penalty
        - stall_penalty
        - unfavorable_path_penalty
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["range_expansion_confirmation"] - 2.40) * 0.05
    )


def fit_linear_auction_skew_inflection_model(candidates: list[base.Candidate], fold_id: str) -> LinearAuctionSkewInflectionModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearAuctionSkewInflectionModel(
            fold_id,
    ...
    auction_skew_inflection_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearAuctionSkewInflectionModel(
        fold_id=fold_id,
        ...
        auction_skew_inflection_failure_rate=auction_skew_inflection_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearAuctionSkewInflectionModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_auction_skew_inflection_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0078_r0001_intraday_auction_skew_inflection",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0078_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0078_r0001_asi_summary.json",
        ],
        ...
        "conditional_profiles": {
            "auction_skew_inflection_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0078 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0078_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0078 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "auction_skew_inflection_lookback_bars": AUCTION_SKEW_INFLECTION_LOOKBACK_BARS,
        "auction_skew_inflection_short_lookback_bars": AUCTION_SKEW_INFLECTION_SHORT_LOOKBACK_BARS,
        "min_auction_skew_inflection_activity": MIN_AUCTION_SKEW_INFLECTION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_auction_skew_inflection_model_summary(model: LinearAuctionSkewInflectionModel) -> dict[str, object]:
    return {
        ...
        "auction_skew_inflection_failure_rate": rounded(model.auction_skew_inflection_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearAuctionSkewInflectionModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_auction_skew_inflection_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "auction_skew_inflection_profile": profiles["auction_skew_inflection_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0078_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0078_r0001_proxy_trades.csv"
    evidence["auction_skew_inflection_summary"] = "artifacts/c0078_r0001_asi_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0078_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0078_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0078 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0078_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0078_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0078_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0078_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0078_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0078_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0078_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0078_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0078_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0078 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0078 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0078_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0078_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0078_r0001_intraday_auction_skew_inflection_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0078_r0001_proxy_records_auction_skew_inflection_reference_surface
  ...
  - next_work_is_produce_c0078_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0078_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0078-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0078_r0001_intraday_auction_skew_inflection",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0078-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0078-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0078-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0078_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0078_r0001_intraday_auction_skew_inflection",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0078-R0001-auction-skew-inflection-SUMMARY",
            "artifact_role": "auction_skew_inflection_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0078_r0001_asi_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0078_r0001_intraday_auction_skew_inflection",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0078_r0001_mt5_logic_parity_evidence",
        }
    ]
