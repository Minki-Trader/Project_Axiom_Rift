"""C0087 R0001 proxy evidence for intraday range third transition."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0087_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0087_r0001_range_third_transition_summary.json"
...
FEATURE_NAMES = (
    "range_third_distance",
    "boundary_transition_depth",
    "third_reclaim_strength",
    "day_range_third_memory",
    "boundary_wick_absorption",
    "close_location_commitment",
    "transition_impulse",
    "participation_reengagement",
    "prior_day_range_context",
    "session_transition_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_range_third_transition_rank"
LABEL_SHAPE = "target_first_escape_quality_conditioned_on_range_third_transition_state"
SELECTION_RULE = "top_fold_local_range_third_transition_scores_per_active_day"
...
MIN_DAY_BARS = 30
RANGE_THIRD_TRANSITION_LOOKBACK_BARS = 24
RANGE_THIRD_TRANSITION_SHORT_LOOKBACK_BARS = 8
MIN_RANGE_THIRD_TRANSITION_ACTIVITY = 0.16
...
class LinearRangeThirdTransitionModel:
    fold_id: str
    ...
    range_third_transition_failure_rate: float
...
def run_c0087_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_range_third_transition_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_range_third_transition_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        RANGE_THIRD_TRANSITION_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = range_third_transition_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        metrics["range_third_distance"],
        metrics["boundary_transition_depth"],
        metrics["third_reclaim_strength"],
        metrics["day_range_third_memory"],
        metrics["boundary_wick_absorption"],
        metrics["close_location_commitment"],
        metrics["transition_impulse"],
        metrics["participation_reengagement"],
        metrics["prior_day_range_context"],
        metrics["session_transition_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        range_third_distance_bucket(metrics["range_third_distance"]),
        boundary_transition_depth_bucket(metrics["boundary_transition_depth"]),
        third_reclaim_strength_bucket(metrics["third_reclaim_strength"]),
        range_third_memory_bucket(metrics["day_range_third_memory"]),
        boundary_wick_absorption_bucket(metrics["boundary_wick_absorption"]),
        third_close_commitment_bucket(metrics["close_location_commitment"]),
        transition_impulse_bucket(metrics["transition_impulse"]),
        ...
        session_transition_bucket(metrics["session_transition_pressure"]),
...
def range_third_transition_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    if prior_day_range is None or prior_day_range <= 0 or prior_day_close is None:
        return {"event_count": 0.0}
    short_start = index - RANGE_THIRD_TRANSITION_SHORT_LOOKBACK_BARS + 1
    prior_start = index - RANGE_THIRD_TRANSITION_LOOKBACK_BARS + 1
    ...
    if day_width < max(2.0 * average_range, 1e-9):
        return {"event_count": 0.0}
    lower_third = context.low_so_far + day_width / 3.0
    upper_third = context.low_so_far + 2.0 * day_width / 3.0
    previous_window = bars[short_start:index]
    if not previous_window:
        return {"event_count": 0.0}
    previous_closes = [bar.close for bar in previous_window]
    previous_high = max(bar.high for bar in previous_window)
    previous_low = min(bar.low for bar in previous_window)

    boundary_candidates: list[tuple[float, float, float, float, bool]] = []
    if direction > 0:
        for boundary in (lower_third, upper_third):
            depth = (boundary - previous_low) / average_range
            reclaim = (signal_bar.close - boundary) / average_range
            dwell = sum(1 for close in previous_closes if close <= boundary) / len(previous_closes)
            transitioned = (
                previous_low <= boundary
                and signal_bar.high >= boundary
                and signal_bar.close >= boundary - 0.05 * average_range
                and (bars[index - 1].close <= boundary or dwell >= 0.35)
            )
            boundary_candidates.append((boundary, depth, reclaim, dwell, transitioned))
    else:
        for boundary in (upper_third, lower_third):
            depth = (previous_high - boundary) / average_range
            reclaim = (boundary - signal_bar.close) / average_range
            dwell = sum(1 for close in previous_closes if close >= boundary) / len(previous_closes)
            transitioned = (
                previous_high >= boundary
                and signal_bar.low <= boundary
                and signal_bar.close <= boundary + 0.05 * average_range
                and (bars[index - 1].close >= boundary or dwell >= 0.35)
            )
            boundary_candidates.append((boundary, depth, reclaim, dwell, transitioned))
    boundary, boundary_transition_depth, third_reclaim_strength, dwell_imbalance, transitioned = max(
        boundary_candidates,
        key=lambda item: (1.0 if item[4] else 0.0, item[1] + item[2] + 0.35 * item[3]),
    )

    range_third_distance = direction * (signal_bar.close - boundary) / average_range
    day_range_third_memory = day_width / average_range
    boundary_wick_absorption = directional_wick_rejection(signal_bar, direction)
    close_location_commitment = directional_bar_settlement(signal_bar, direction)
    transition_impulse = direction * (signal_bar.close - bars[index - 1].close) / average_range
    ...
    prior_day_range_context = prior_day_range / max(48.0 * average_range, 1e-9)
    transition_pressure = max(
        max(boundary_transition_depth, 0.0) / 2.0,
        max(third_reclaim_strength, 0.0),
        max(boundary_wick_absorption, 0.0),
        max(close_location_commitment - 0.50, 0.0) * 2.0,
        max(transition_impulse, 0.0),
        min(max(participation_reengagement, 0.0), 1.0),
        max(dwell_imbalance - 0.35, 0.0),
    )
    session_transition_pressure = bounded_session_progress(signal_bar.time) * transition_pressure
    activity = max(transition_pressure, session_transition_pressure)
    if activity < MIN_RANGE_THIRD_TRANSITION_ACTIVITY:
        return {"event_count": 0.0}
    if not transitioned and boundary_transition_depth < 0.16:
        return {"event_count": 0.0}
    if boundary_transition_depth < 0.08:
        return {"event_count": 0.0}
    if third_reclaim_strength < -0.08 and close_location_commitment < 0.46:
        return {"event_count": 0.0}
    if transition_impulse < -0.35 and boundary_wick_absorption < -0.05:
        return {"event_count": 0.0}
    return {
        "range_third_distance": bounded(range_third_distance, -2.0, 6.0),
        "boundary_transition_depth": bounded(boundary_transition_depth, -2.0, 6.0),
        "third_reclaim_strength": bounded(third_reclaim_strength, -2.0, 6.0),
        "day_range_third_memory": bounded(day_range_third_memory, 0.0, 12.0),
        "boundary_wick_absorption": bounded(boundary_wick_absorption, -1.0, 1.0),
        "close_location_commitment": bounded(close_location_commitment, 0.0, 1.0),
        "transition_impulse": bounded(transition_impulse, -4.0, 6.0),
        "participation_reengagement": bounded(participation_reengagement, -3.0, 6.0),
        "prior_day_range_context": bounded(prior_day_range_context, 0.0, 6.0),
        "session_transition_pressure": bounded(session_transition_pressure, 0.0, 4.0),
        "event_count": 1.0,
    }
...
def range_third_distance_bucket(value: float) -> str:
    if value < 0.10:
        return "third_boundary_inside"
    if value < 0.85:
        return "third_boundary_reclaimed"
    return "third_boundary_extended"


def boundary_transition_depth_bucket(value: float) -> str:
    if value < 0.15:
        return "boundary_probe_shallow"
    if value < 0.95:
        return "boundary_probe_defined"
    return "boundary_probe_deep"


def third_reclaim_strength_bucket(value: float) -> str:
    if value < -0.04:
        return "third_reclaim_failed"
    if value < 0.35:
        return "third_reclaim_mixed"
    return "third_reclaim_clear"


def range_third_memory_bucket(value: float) -> str:
    if value < 2.40:
        return "third_range_quiet"
    if value < 5.20:
        return "third_range_defined"
    return "third_range_extended"


def boundary_wick_absorption_bucket(value: float) -> str:
    if value < -0.08:
        return "boundary_wick_against"
    if value < 0.12:
        return "boundary_wick_mixed"
    return "boundary_wick_absorbing"


def third_close_commitment_bucket(value: float) -> str:
    if value < 0.45:
        return "third_close_weak"
    if value < 0.66:
        return "third_close_mid"
    return "third_close_strong"


def transition_impulse_bucket(value: float) -> str:
    if value < -0.10:
        return "transition_impulse_against"
    if value < 0.45:
        return "transition_impulse_mixed"
    return "transition_impulse_with"


def session_transition_bucket(value: float) -> str:
    if value < 0.08:
        return "session_transition_light"
    if value < 0.65:
        return "session_transition_defined"
    return "session_transition_heavy"
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    probe_reward = 0.12 if metrics["boundary_transition_depth"] > 0.18 and terminal > 0 else 0.0
    reclaim_reward = 0.14 if metrics["third_reclaim_strength"] > 0.05 and terminal > 0 else 0.0
    close_reward = 0.12 if metrics["close_location_commitment"] > 0.58 and terminal > 0 else 0.0
    impulse_reward = 0.10 if metrics["transition_impulse"] > 0.10 and terminal > 0 else 0.0
    wick_reward = 0.08 if metrics["boundary_wick_absorption"] > 0.05 and terminal > 0 else 0.0
    participation_reward = 0.08 if metrics["participation_reengagement"] > 0.03 and terminal > 0 else 0.0
    prior_context_reward = 0.05 if metrics["prior_day_range_context"] > 0.35 and terminal > 0 else 0.0
    session_reward = 0.05 if metrics["session_transition_pressure"] > 0.12 and terminal > 0 else 0.0
    shallow_probe_penalty = 0.10 if metrics["boundary_transition_depth"] < 0.12 and terminal <= 0 else 0.0
    failed_reclaim_penalty = 0.12 if metrics["third_reclaim_strength"] < -0.04 and terminal <= 0 else 0.0
    weak_close_penalty = 0.10 if metrics["close_location_commitment"] < 0.45 and terminal <= 0 else 0.0
    adverse_impulse_penalty = 0.10 if metrics["transition_impulse"] < -0.12 and terminal <= 0 else 0.0
    weak_wick_penalty = 0.08 if metrics["boundary_wick_absorption"] < -0.08 and terminal <= 0 else 0.0
    no_participation_penalty = 0.08 if metrics["participation_reengagement"] < -0.06 and terminal <= 0 else 0.0
    hot_range_penalty = 0.04 if metrics["day_range_third_memory"] > 6.50 and terminal <= 0 else 0.0
    ...
    range_third_transition_path_quality = (0.52 * terminal + 0.34 * mfe - 0.68 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + range_third_transition_path_quality
        + probe_reward
        + reclaim_reward
        + close_reward
        + impulse_reward
        ...
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["range_third_distance"] - 3.00) * 0.04
    )


def fit_linear_range_third_transition_model(candidates: list[base.Candidate], fold_id: str) -> LinearRangeThirdTransitionModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearRangeThirdTransitionModel(
            fold_id,
    ...
    range_third_transition_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearRangeThirdTransitionModel(
        fold_id=fold_id,
        ...
        range_third_transition_failure_rate=range_third_transition_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearRangeThirdTransitionModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_range_third_transition_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0087_r0001_intraday_range_third_transition",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0087_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0087_r0001_range_third_transition_summary.json",
        ],
        ...
        "conditional_profiles": {
            "range_third_transition_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0087 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0087_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0087 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "range_third_transition_lookback_bars": RANGE_THIRD_TRANSITION_LOOKBACK_BARS,
        "range_third_transition_short_lookback_bars": RANGE_THIRD_TRANSITION_SHORT_LOOKBACK_BARS,
        "min_range_third_transition_activity": MIN_RANGE_THIRD_TRANSITION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_range_third_transition_model_summary(model: LinearRangeThirdTransitionModel) -> dict[str, object]:
    return {
        ...
        "range_third_transition_failure_rate": rounded(model.range_third_transition_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearRangeThirdTransitionModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_range_third_transition_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "range_third_transition_profile": profiles["range_third_transition_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0087_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0087_r0001_proxy_trades.csv"
    evidence["range_third_transition_summary"] = "artifacts/c0087_r0001_range_third_transition_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0087_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0087_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0087 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0087_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0087_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0087_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0087_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0087_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0087_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0087_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0087_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0087_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0087 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0087 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0087_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0087_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0087_r0001_intraday_range_third_transition_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0087_r0001_proxy_records_range_third_transition_reference_surface
  ...
  - next_work_is_produce_c0087_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0087_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0087-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0087_r0001_intraday_range_third_transition",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0087-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0087-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0087-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0087_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0087_r0001_intraday_range_third_transition",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0087-R0001-range-third-transition-SUMMARY",
            "artifact_role": "range_third_transition_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0087_r0001_range_third_transition_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0087_r0001_intraday_range_third_transition",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0087_r0001_mt5_logic_parity_evidence",
        }
    ]
