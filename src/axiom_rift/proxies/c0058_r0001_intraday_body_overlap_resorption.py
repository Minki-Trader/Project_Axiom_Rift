"""C0058 R0001 proxy evidence for intraday body overlap resorption."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0058_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0058_r0001_body_overlap_resorption_summary.json"
...
FEATURE_NAMES = (
    "body_overlap_density_12",
    "directional_body_cluster_pressure",
    "failed_break_resorption_6",
    "wick_invasion_pressure_6",
    "midbody_reclaim_efficiency",
    "range_box_compression_12",
    "cluster_edge_distance",
    "post_break_body_followthrough",
    "day_range_completion_ratio",
    "body_volume_confirmation",
    "session_progress_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_body_overlap_resorption_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_body_overlap_resorption_state"
SELECTION_RULE = "top_fold_local_body_overlap_resorption_scores_per_active_day"
...
MIN_DAY_BARS = 36
BODY_OVERLAP_LOOKBACK_BARS = 12
BODY_OVERLAP_SHORT_LOOKBACK_BARS = 6
MIN_BODY_OVERLAP_ACTIVITY = 0.18
...
class LinearBodyOverlapResorptionModel:
    fold_id: str
...
def run_c0058_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_body_overlap_resorption_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_body_overlap_resorption_model_summary(model))
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    resorption = body_overlap_resorption_metrics(bars, volumes, context, volume_average, index, direction, average_range)
    if resorption["event_count"] <= 0:
        return None, None
    ...
    volume_confirmation = bounded(
        volume_ratio
        * (
            0.35 * resorption["body_overlap_density_12"]
            + 0.30 * resorption["failed_break_resorption_6"]
            + 0.25 * resorption["midbody_reclaim_efficiency"]
            + 0.10 * resorption["post_break_body_followthrough"]
        ),
        -4.0,
        4.0,
    )
    features = (
        resorption["body_overlap_density_12"],
        resorption["directional_body_cluster_pressure"],
        resorption["failed_break_resorption_6"],
        resorption["wick_invasion_pressure_6"],
        resorption["midbody_reclaim_efficiency"],
        resorption["range_box_compression_12"],
        resorption["cluster_edge_distance"],
        resorption["post_break_body_followthrough"],
        resorption["day_range_completion_ratio"],
        volume_confirmation,
        resorption["session_progress_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        overlap_density_bucket(resorption["body_overlap_density_12"]),
        cluster_pressure_bucket(resorption["directional_body_cluster_pressure"]),
        resorption_bucket(resorption["failed_break_resorption_6"]),
        wick_invasion_bucket(resorption["wick_invasion_pressure_6"]),
        reclaim_bucket(resorption["midbody_reclaim_efficiency"]),
        box_compression_bucket(resorption["range_box_compression_12"]),
        edge_distance_bucket(resorption["cluster_edge_distance"]),
        followthrough_bucket(resorption["post_break_body_followthrough"]),
        participation_bucket(volume_ratio),
        session_pressure_bucket(resorption["session_progress_pressure"]),
...
def body_overlap_resorption_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    if prior_volume_per_bar is None or prior_volume_per_bar <= 0:
        return {"event_count": 0.0}
    window = bars[index - BODY_OVERLAP_LOOKBACK_BARS : index + 1]
    short_window = bars[index - BODY_OVERLAP_SHORT_LOOKBACK_BARS + 1 : index + 1]
    body_highs = [max(item.open, item.close) for item in window]
    body_lows = [min(item.open, item.close) for item in window]
    body_sizes = [max(high - low, 1e-9) for high, low in zip(body_highs, body_lows)]
    pairwise_overlap: list[float] = []
    for left in range(len(window)):
        for right in range(left + 1, len(window)):
            overlap = max(0.0, min(body_highs[left], body_highs[right]) - max(body_lows[left], body_lows[right]))
            denominator = max(body_sizes[left], body_sizes[right], average_range * 0.05, 1e-9)
            pairwise_overlap.append(overlap / denominator)
    body_overlap_density = base.mean(pairwise_overlap) or 0.0
    cluster_high = base.percentile(body_highs, 0.60) or max(body_highs)
    cluster_low = base.percentile(body_lows, 0.40) or min(body_lows)
    cluster_mid = (cluster_high + cluster_low) / 2.0
    cluster_span = max(cluster_high - cluster_low, average_range * 0.15, 1e-9)
    directional_cluster_pressure = direction * (close - cluster_mid) / average_range
    range_box_width = max(item.high for item in window) - min(item.low for item in window)
    range_box_compression = range_box_width / average_range
    short_high = max(item.high for item in short_window)
    short_low = min(item.low for item in short_window)
    if direction > 0:
        adverse_break = max(0.0, cluster_low - short_low) / cluster_span
        reclaim = max(0.0, close - cluster_mid) / cluster_span
        edge_distance = (cluster_high - close) / average_range
        wick_invasion = sum(max(0.0, cluster_low - item.low) / cluster_span for item in short_window) / len(short_window)
    else:
        adverse_break = max(0.0, short_high - cluster_high) / cluster_span
        reclaim = max(0.0, cluster_mid - close) / cluster_span
        edge_distance = (close - cluster_low) / average_range
        wick_invasion = sum(max(0.0, item.high - cluster_high) / cluster_span for item in short_window) / len(short_window)
    failed_break_resorption = min(4.0, adverse_break * min(reclaim, 4.0))
    midbody_reclaim_efficiency = reclaim / max(adverse_break + 0.20, 1e-9)
    response_start = bars[index - BODY_OVERLAP_SHORT_LOOKBACK_BARS].close
    post_break_followthrough = direction * (close - response_start) / average_range
    range_completion = day_width / average_range
    recent_volume = sum(volumes[max(0, index - BODY_OVERLAP_SHORT_LOOKBACK_BARS + 1) : index + 1])
    expected_recent_volume = prior_volume_per_bar * BODY_OVERLAP_SHORT_LOOKBACK_BARS
    ...
    session_pressure = bounded_session_progress(bars[index].time) * max(
        body_overlap_density,
        failed_break_resorption,
        abs(directional_cluster_pressure),
        max(post_break_followthrough, 0.0),
    )
    activity = max(
        body_overlap_density,
        abs(directional_cluster_pressure),
        failed_break_resorption,
        wick_invasion,
        abs(midbody_reclaim_efficiency - 0.5),
        abs(post_break_followthrough),
        session_pressure,
    )
    if activity < MIN_BODY_OVERLAP_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "body_overlap_density_12": bounded(body_overlap_density, 0.0, 2.0),
        "directional_body_cluster_pressure": bounded(directional_cluster_pressure, -4.0, 4.0),
        "failed_break_resorption_6": bounded(failed_break_resorption, 0.0, 4.0),
        "wick_invasion_pressure_6": bounded(wick_invasion, 0.0, 4.0),
        "midbody_reclaim_efficiency": bounded(midbody_reclaim_efficiency, 0.0, 4.0),
        "range_box_compression_12": bounded(range_box_compression, 0.0, 8.0),
        "cluster_edge_distance": bounded(edge_distance, -4.0, 4.0),
        "post_break_body_followthrough": bounded(post_break_followthrough, -4.0, 4.0),
        "day_range_completion_ratio": bounded(range_completion, 0.0, 8.0),
        "session_progress_pressure": bounded(session_pressure * recent_intensity, 0.0, 4.0),
        "event_count": 1.0,
    }
...
def overlap_density_bucket(value: float) -> str:
    if value < 0.20:
        return "overlap_sparse"
    if value < 0.55:
        return "overlap_moderate"
    return "overlap_dense"


def cluster_pressure_bucket(value: float) -> str:
    if value < -0.35:
        return "cluster_pressure_against"
    if value < 0.35:
        return "cluster_pressure_neutral"
    return "cluster_pressure_with"


def resorption_bucket(value: float) -> str:
    if value < 0.10:
        return "resorption_absent"
    if value < 0.65:
        return "resorption_partial"
    return "resorption_strong"


def wick_invasion_bucket(value: float) -> str:
    if value < 0.15:
        return "wick_invasion_thin"
    if value < 0.70:
        return "wick_invasion_mid"
    return "wick_invasion_deep"


def reclaim_bucket(value: float) -> str:
    if value < 0.45:
        return "weak_reclaim"
    if value < 1.20:
        return "partial_reclaim"
    return "strong_reclaim"


def box_compression_bucket(value: float) -> str:
    if value < 1.65:
        return "body_box_compressed"
    if value < 3.00:
        return "body_box_normal"
    return "body_box_expanded"


def edge_distance_bucket(value: float) -> str:
    if value < -0.20:
        return "past_cluster_edge"
    if value < 0.60:
        return "near_cluster_edge"
    return "inside_cluster_far_edge"
...
def candidate_label(
    ...
    close = bars[index].close
    if direction > 0:
        adverse_break = max(0.0, cluster_low - min(item.low for item in short_window)) / cluster_span
        reclaim = max(0.0, close - cluster_mid) / cluster_span
        accepted_edge = close > cluster_high
    else:
        adverse_break = max(0.0, max(item.high for item in short_window) - cluster_high) / cluster_span
        reclaim = max(0.0, cluster_mid - close) / cluster_span
        accepted_edge = close < cluster_low
    post_followthrough = direction * (close - bars[index - BODY_OVERLAP_SHORT_LOOKBACK_BARS].close) / average_range
    cluster_reclaim_reward = 0.22 if adverse_break > 0.10 and reclaim > 0.55 and terminal > 0 else 0.0
    edge_acceptance_reward = 0.12 if accepted_edge and post_followthrough > 0.10 and terminal > 0 else 0.0
    failed_resorption_penalty = 0.24 if adverse_break > 0.10 and reclaim < 0.30 and mae > mfe else 0.0
    trapped_reentry_penalty = 0.18 if post_followthrough < -0.25 and terminal <= 0 else 0.0
    ...
    resorption_path_quality = (0.35 * terminal + 0.35 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + resorption_path_quality
        + cluster_reclaim_reward
        + edge_acceptance_reward
        - failed_resorption_penalty
        - trapped_reentry_penalty
        - stall_penalty
...
def fit_linear_body_overlap_resorption_model(candidates: list[base.Candidate], fold_id: str) -> LinearBodyOverlapResorptionModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearBodyOverlapResorptionModel(
            fold_id,
    ...
    resorption_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearBodyOverlapResorptionModel(
        fold_id=fold_id,
        ...
        resorption_failure_rate=resorption_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearBodyOverlapResorptionModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_body_overlap_resorption_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0058_r0001_intraday_body_overlap_resorption",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0058_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0058_r0001_body_overlap_resorption_summary.json",
        ],
        ...
        "conditional_profiles": {
            "body_overlap_resorption_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0058 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0058_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0058 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "body_overlap_lookback_bars": BODY_OVERLAP_LOOKBACK_BARS,
        "body_overlap_short_lookback_bars": BODY_OVERLAP_SHORT_LOOKBACK_BARS,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_body_overlap_resorption_model_summary(model: LinearBodyOverlapResorptionModel) -> dict[str, object]:
    return {
        ...
        "resorption_failure_rate": rounded(model.resorption_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearBodyOverlapResorptionModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_body_overlap_resorption_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "body_overlap_resorption_profile": profiles["body_overlap_resorption_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0058_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_trade_artifact"] = "artifacts/c0058_r0001_proxy_trades.csv"
    evidence["body_overlap_resorption_summary"] = "artifacts/c0058_r0001_body_overlap_resorption_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0058_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0058_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0058 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0058_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0058_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0058_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0058_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0058_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0058_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0058_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0058_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0058_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0058 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0058 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260706_produce_c0058_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0058_r0001_proxy_evidence
  refines:
  - dec_20260706_open_c0058_r0001_intraday_body_overlap_resorption_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0058_r0001_proxy_records_body_overlap_resorption_reference_surface
  ...
  - next_work_is_produce_c0058_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260706_produce_c0058_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0058-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0058_r0001_intraday_body_overlap_resorption",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0058-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0058-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0058-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0058_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0058_r0001_intraday_body_overlap_resorption",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0058-R0001-body-overlap-resorption-SUMMARY",
            "artifact_role": "body_overlap_resorption_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0058_r0001_body_overlap_resorption_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0058_r0001_intraday_body_overlap_resorption",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0058_r0001_mt5_logic_parity_evidence",
        }
    ]
