"""C0136 R0001 proxy evidence for intraday swing pivot retest continuation."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0136_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0136_r0001_swing_pivot_retest_continuation_summary.json"
...
FEATURE_NAMES = (
    ...
    "retest_depth",
    "retest_rejection_strength",
    "reclaim_close_efficiency",
    "pullback_range_compression",
    "swing_structure_alignment",
    "micro_reacceleration",
    "session_room",
    "volume_reengagement",
...
MODEL_FAMILY = "fold_local_intraday_swing_pivot_retest_continuation_rank"
LABEL_SHAPE = "target_first_quality_after_swing_pivot_retest_continuation"
SELECTION_RULE = "top_fold_local_swing_pivot_retest_continuation_scores_per_active_day"
...
MIN_DAY_BARS = 18
SWING_PIVOT_RETEST_CONTINUATION_LOOKBACK_BARS = 48
SWING_PIVOT_RETEST_CONTINUATION_MICRO_BARS = 12
MIN_RETEST_REJECTION_STRENGTH = 0.04
MIN_RECLAIM_CLOSE_EFFICIENCY = 0.04
MIN_SWING_PIVOT_RETEST_CONTINUATION_ACTIVITY = 0.04
...
class LinearSwingPivotRetestContinuationModel:
    fold_id: str
    ...
    swing_pivot_retest_continuation_failure_rate: float
...
def run_c0136_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_swing_pivot_retest_continuation_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_swing_pivot_retest_continuation_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        SWING_PIVOT_RETEST_CONTINUATION_LOOKBACK_BARS,
        SWING_PIVOT_RETEST_CONTINUATION_MICRO_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = swing_pivot_retest_continuation_metrics(
    ...
    features = (
        metrics["pivot_break_strength"],
        metrics["retest_depth"],
        metrics["retest_rejection_strength"],
        metrics["reclaim_close_efficiency"],
        metrics["pullback_range_compression"],
        metrics["swing_structure_alignment"],
        metrics["micro_reacceleration"],
        metrics["session_room"],
        metrics["volume_reengagement"],
    ...
    dimensions = (
        breakout_pressure_bucket(metrics["pivot_break_strength"]),
        shallow_pullback_bucket(metrics["retest_depth"]),
        wick_containment_bucket(metrics["retest_rejection_strength"]),
        close_reclaim_bucket(metrics["reclaim_close_efficiency"]),
        compression_bucket(metrics["pullback_range_compression"]),
        trend_persistence_bucket(metrics["swing_structure_alignment"]),
        acceleration_pressure_bucket(metrics["micro_reacceleration"]),
        session_pressure_bucket(metrics["session_room"]),
        volume_reengagement_bucket(metrics["volume_reengagement"]),
        prior_direction_context_bucket(metrics["prior_day_direction_context"]),
...
def swing_pivot_retest_continuation_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    micro_start = max(index - SWING_PIVOT_RETEST_CONTINUATION_MICRO_BARS + 1, context.day_start_index)
    lookback_start = max(index - SWING_PIVOT_RETEST_CONTINUATION_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    if len(prior_micro) < 5:
        return {"event_count": 0.0}
    pivot_window = lookback_window[: -len(micro_window)] if len(lookback_window) > len(micro_window) else lookback_window[:-1]
    if len(pivot_window) < 12:
        return {"event_count": 0.0}
    previous_bar = bars[index - 1]
    previous_previous_bar = bars[index - 2]

    if direction > 0:
        pivot_level = max(bar.high for bar in pivot_window)
        pivot_break_strength = (max(bar.high for bar in prior_micro) - pivot_level) / max(average_range, 1e-9)
        retest_depth = (pivot_level - min(bar.low for bar in micro_window)) / max(average_range, 1e-9)
        reclaim_close_efficiency = (signal_bar.close - pivot_level) / max(average_range, 1e-9)
        retest_rejection_strength = max(signal_bar.close - min(bar.low for bar in micro_window), 0.0) / max(
            average_range,
            1e-9,
        )
        session_room = (context.high_so_far - signal_bar.close) / max(average_range, 1e-9)
    else:
        pivot_level = min(bar.low for bar in pivot_window)
        pivot_break_strength = (pivot_level - min(bar.low for bar in prior_micro)) / max(average_range, 1e-9)
        retest_depth = (max(bar.high for bar in micro_window) - pivot_level) / max(average_range, 1e-9)
        reclaim_close_efficiency = (pivot_level - signal_bar.close) / max(average_range, 1e-9)
        retest_rejection_strength = max(max(bar.high for bar in micro_window) - signal_bar.close, 0.0) / max(
            average_range,
            1e-9,
        )
        session_room = (signal_bar.close - context.low_so_far) / max(average_range, 1e-9)

    directional_net = direction * (signal_bar.close - lookback_window[0].close)
    directional_travel = sum(
        abs(lookback_window[pos].close - lookback_window[pos - 1].close)
        for pos in range(1, len(lookback_window))
    )
    swing_structure_alignment = directional_net / max(directional_travel, average_range, 1e-9)
    ...
    micro_reacceleration = direction * (current_delta - previous_delta) / max(average_range, 1e-9)
    pullback_range_compression = 1.0 - micro_range_mean / max(lookback_range_mean, 1e-9)
    if pivot_break_strength < -0.20 and reclaim_close_efficiency < -0.12:
        return {"event_count": 0.0}
    if retest_rejection_strength < MIN_RETEST_REJECTION_STRENGTH:
        return {"event_count": 0.0}
    if reclaim_close_efficiency < MIN_RECLAIM_CLOSE_EFFICIENCY and micro_reacceleration < 0.02:
        return {"event_count": 0.0}

    volume_ratio = volumes[index] / max(lookback_volume_mean, average_volume, 1e-9)
    volume_reengagement = volume_ratio - 1.0
    ...
    activity = (
        max(pivot_break_strength, -0.05) * 0.16
        + max(0.65 - abs(retest_depth), -0.20) * 0.12
        + max(retest_rejection_strength, 0.0) * 0.14
        + max(reclaim_close_efficiency, -0.10) * 0.16
        + max(pullback_range_compression, -0.50) * 0.08
        + max(swing_structure_alignment, -0.05) * 0.10
        + max(micro_reacceleration, -0.08) * 0.10
        + max(session_room, -0.20) * 0.05
        + max(min(volume_reengagement, 1.50), -0.75) * 0.05
        + max(prior_day_direction_context, -0.20) * 0.04
    )
    if activity < MIN_SWING_PIVOT_RETEST_CONTINUATION_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "pivot_break_strength": bounded(pivot_break_strength, -4.0, 4.0),
        "retest_depth": bounded(retest_depth, -4.0, 6.0),
        "retest_rejection_strength": bounded(retest_rejection_strength, 0.0, 8.0),
        "reclaim_close_efficiency": bounded(reclaim_close_efficiency, -4.0, 4.0),
        "pullback_range_compression": bounded(pullback_range_compression, -3.0, 1.0),
        "swing_structure_alignment": bounded(swing_structure_alignment, -2.0, 2.0),
        "micro_reacceleration": bounded(micro_reacceleration, -4.0, 4.0),
        "session_room": bounded(session_room, -2.0, 12.0),
        "volume_reengagement": bounded(volume_reengagement, -6.0, 8.0),
        ...
        "pivot_level": rounded(pivot_level) or 0.0,
        "micro_net": bounded(micro_net / max(average_range, 1e-9), -4.0, 4.0),
        "swing_pivot_retest_continuation_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0136_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0136_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0136_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0136_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0136_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0136_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0136_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0136_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0136_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0136_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0136_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0136_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0136_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0136_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0136_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0136_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0136_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0136_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0136_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def c0136_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    break_reward = 0.13 if metrics["pivot_break_strength"] > 0.05 and terminal > 0 else 0.0
    retest_reward = 0.12 if -0.20 <= metrics["retest_depth"] <= 0.75 and terminal > 0 else 0.0
    rejection_reward = 0.12 if metrics["retest_rejection_strength"] > 0.22 and terminal > 0 else 0.0
    reclaim_reward = 0.11 if metrics["reclaim_close_efficiency"] > 0.08 and terminal > 0 else 0.0
    compression_reward = 0.07 if metrics["pullback_range_compression"] > -0.10 and terminal > 0 else 0.0
    alignment_reward = 0.08 if metrics["swing_structure_alignment"] > 0.04 and terminal > 0 else 0.0
    reacceleration_reward = 0.10 if metrics["micro_reacceleration"] > 0.03 and terminal > 0 else 0.0
    session_room_reward = 0.05 if metrics["session_room"] > 0.30 and terminal > 0 else 0.0
    volume_reward = 0.05 if metrics["volume_reengagement"] > -0.35 and terminal > 0 else 0.0
    prior_day_reward = 0.04 if metrics["prior_day_direction_context"] > -0.35 and terminal > 0 else 0.0
    weak_break_penalty = 0.09 if metrics["pivot_break_strength"] < -0.04 and terminal <= 0 else 0.0
    deep_retest_penalty = 0.08 if metrics["retest_depth"] > 1.10 and terminal <= 0 else 0.0
    weak_rejection_penalty = 0.08 if metrics["retest_rejection_strength"] < 0.10 and terminal <= 0 else 0.0
    weak_reclaim_penalty = 0.09 if metrics["reclaim_close_efficiency"] < -0.04 and terminal <= 0 else 0.0
    weak_alignment_penalty = 0.06 if metrics["swing_structure_alignment"] < -0.05 and terminal <= 0 else 0.0
    weak_reacceleration_penalty = 0.08 if metrics["micro_reacceleration"] < -0.08 and terminal <= 0 else 0.0
    no_room_penalty = 0.05 if metrics["session_room"] < 0.10 and terminal <= 0 else 0.0
    volume_drought_penalty = 0.05 if metrics["volume_reengagement"] < -0.75 and terminal <= 0 else 0.0
    ...
    return (
        ...
        - max(0.0, -metrics["volume_reengagement"] - 0.75) * 0.015
        - max(0.0, MIN_RETEST_REJECTION_STRENGTH - metrics["retest_rejection_strength"]) * 0.08
        - max(0.0, MIN_RECLAIM_CLOSE_EFFICIENCY - metrics["reclaim_close_efficiency"]) * 0.06
    )


def fit_linear_swing_pivot_retest_continuation_model(candidates: list[base.Candidate], fold_id: str) -> LinearSwingPivotRetestContinuationModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearSwingPivotRetestContinuationModel(
            fold_id,
    ...
    swing_pivot_retest_continuation_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearSwingPivotRetestContinuationModel(
        fold_id=fold_id,
        ...
        swing_pivot_retest_continuation_failure_rate=swing_pivot_retest_continuation_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearSwingPivotRetestContinuationModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_swing_pivot_retest_continuation_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0136_r0001_intraday_swing_pivot_retest_continuation",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0136_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0136_r0001_swing_pivot_retest_continuation_summary.json",
        ],
        ...
        "conditional_profiles": {
            "swing_pivot_retest_continuation_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0136 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0136_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0136 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "swing_pivot_retest_continuation_lookback_bars": SWING_PIVOT_RETEST_CONTINUATION_LOOKBACK_BARS,
        "swing_pivot_retest_continuation_micro_bars": SWING_PIVOT_RETEST_CONTINUATION_MICRO_BARS,
        "min_retest_rejection_strength": MIN_RETEST_REJECTION_STRENGTH,
        "min_reclaim_close_efficiency": MIN_RECLAIM_CLOSE_EFFICIENCY,
        "min_swing_pivot_retest_continuation_activity": MIN_SWING_PIVOT_RETEST_CONTINUATION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_swing_pivot_retest_continuation_model_summary(model: LinearSwingPivotRetestContinuationModel) -> dict[str, object]:
    return {
        ...
        "swing_pivot_retest_continuation_failure_rate": rounded(model.swing_pivot_retest_continuation_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearSwingPivotRetestContinuationModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_swing_pivot_retest_continuation_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "swing_pivot_retest_continuation_profile": profiles["swing_pivot_retest_continuation_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0136_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0136_r0001_proxy_trades.csv"
    evidence["swing_pivot_retest_continuation_summary"] = "artifacts/c0136_r0001_swing_pivot_retest_continuation_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0136_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0136_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0136 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0136_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    next_work["tasks"] = ["produce_c0136_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0136_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0136_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0136_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0136_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0136_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0136_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0136 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0136 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    block = f"""
- decision_id: dec_20260708_produce_c0136_r0001_proxy_evidence
  created_local_date: '{local_date}'
  status: active
  decision: produce_c0136_r0001_proxy_evidence
  refines:
  - dec_20260708_open_c0136_r0001_intraday_swing_pivot_retest_continuation_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0136_r0001_proxy_records_swing_pivot_retest_continuation_reference_surface
  ...
  - next_work_is_produce_c0136_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260708_produce_c0136_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0136-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0136_r0001_intraday_swing_pivot_retest_continuation",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0136-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0136-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0136-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0136_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0136_r0001_intraday_swing_pivot_retest_continuation",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0136-R0001-SWING-PIVOT-RETEST-CONTINUATION-SUMMARY",
            "artifact_role": "swing_pivot_retest_continuation_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0136_r0001_swing_pivot_retest_continuation_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0136_r0001_intraday_swing_pivot_retest_continuation",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0136_r0001_mt5_logic_parity_evidence",
        }
    ]
