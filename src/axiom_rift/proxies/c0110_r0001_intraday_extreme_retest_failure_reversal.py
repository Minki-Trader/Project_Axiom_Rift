"""C0110 R0001 proxy evidence for intraday extreme retest failure reversal."""
...
CAMPAIGN_ID = "C0110"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0110_intraday_extreme_retest_failure_reversal_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0110_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0110_r0001_extreme_retest_failure_reversal_summary.json"
...
FEATURE_NAMES = (
    "extreme_retest_pressure",
    "rejection_wick",
    "failed_breakout_recovery",
    "reversal_body_turn",
    "close_reclaim",
    "range_compression",
    "volume_exhaustion",
    ...
    "prior_session_contrast",
    "session_phase_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_extreme_retest_failure_reversal_rank"
LABEL_SHAPE = "target_first_quality_after_extreme_retest_failure_reversal"
SELECTION_RULE = "top_fold_local_extreme_retest_failure_reversal_scores_per_active_day"
...
MIN_DAY_BARS = 18
EXTREME_RETEST_FAILURE_REVERSAL_LOOKBACK_BARS = 42
EXTREME_RETEST_FAILURE_REVERSAL_RECENT_BARS = 7
EXTREME_RETEST_FAILURE_REVERSAL_PRIOR_BARS = 20
EXTREME_RETEST_FAILURE_REVERSAL_TREND_BARS = 16
MIN_EXTREME_RETEST_PRESSURE = 0.20
MIN_EXTREME_RETEST_FAILURE_REVERSAL_ACTIVITY = 0.05
...
class LinearExtremeRetestFailureReversalModel:
    fold_id: str
    ...
    extreme_retest_failure_reversal_failure_rate: float
...
def run_c0110_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_extreme_retest_failure_reversal_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_extreme_retest_failure_reversal_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        EXTREME_RETEST_FAILURE_REVERSAL_LOOKBACK_BARS,
        EXTREME_RETEST_FAILURE_REVERSAL_RECENT_BARS + EXTREME_RETEST_FAILURE_REVERSAL_PRIOR_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = extreme_retest_failure_reversal_metrics(
    ...
    features = (
        metrics["extreme_retest_pressure"],
        metrics["rejection_wick"],
        metrics["failed_breakout_recovery"],
        metrics["reversal_body_turn"],
        metrics["close_reclaim"],
        metrics["range_compression"],
        metrics["volume_exhaustion"],
        metrics["opposite_room"],
        metrics["opening_range_reclaim"],
        metrics["prior_session_contrast"],
        metrics["session_phase_maturity"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        c0110_extreme_retest_bucket(metrics["extreme_retest_pressure"]),
        adverse_absorption_bucket(metrics["rejection_wick"]),
        c0110_recovery_bucket(metrics["failed_breakout_recovery"]),
        counter_body_turn_bucket(metrics["reversal_body_turn"]),
        close_reclaim_bucket(metrics["close_reclaim"]),
        compression_bucket(metrics["range_compression"]),
        c0110_volume_exhaustion_bucket(metrics["volume_exhaustion"]),
        c0110_opposite_room_bucket(metrics["opposite_room"]),
        c0110_opening_reclaim_bucket(metrics["opening_range_reclaim"]),
        c0110_prior_contrast_bucket(metrics["prior_session_contrast"]),
...
def extreme_retest_failure_reversal_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    recent_start = max(index - EXTREME_RETEST_FAILURE_REVERSAL_RECENT_BARS + 1, context.day_start_index)
    prior_start = max(recent_start - EXTREME_RETEST_FAILURE_REVERSAL_PRIOR_BARS, context.day_start_index)
    lookback_start = max(index - EXTREME_RETEST_FAILURE_REVERSAL_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    if session_phase_maturity < 0.08 or session_phase_maturity > 0.76:
        return {"event_count": 0.0}
    ...
    if direction > 0:
        distance_from_extreme = max(0.0, recent_low - context.low_so_far) / average_range
        failed_breakout_recovery = (signal_bar.close - recent_low) / average_range
        opposite_room = (context.high_so_far - signal_bar.close) / day_range_so_far
        opening_range_reclaim = (signal_bar.close - context.opening_low) / average_range
    else:
        distance_from_extreme = max(0.0, context.high_so_far - recent_high) / average_range
        failed_breakout_recovery = (recent_high - signal_bar.close) / average_range
        opposite_room = (signal_bar.close - context.low_so_far) / day_range_so_far
        opening_range_reclaim = (context.opening_high - signal_bar.close) / average_range
    extreme_retest_pressure = max(0.0, 1.25 - distance_from_extreme)
    if extreme_retest_pressure < MIN_EXTREME_RETEST_PRESSURE or failed_breakout_recovery < 0.08:
        return {"event_count": 0.0}

    rejection_wick = adverse_wick_absorption(signal_bar, direction)
    recent_body_turn = base.mean([directional_body_score(bar, direction) for bar in recent_window[-3:]]) or 0.0
    reversal_body_turn = 0.62 * directional_body_score(signal_bar, direction) + 0.38 * recent_body_turn
    close_reclaim = directional_bar_settlement(signal_bar, direction)
    trend_start = max(context.day_start_index, index - EXTREME_RETEST_FAILURE_REVERSAL_TREND_BARS)
    trend_into_extreme = -direction * (signal_bar.close - bars[trend_start].close) / average_range
    range_compression = 1.0 - recent_range_mean / max(prior_range_mean, average_range * 0.10)
    volume_exhaustion = prior_volume_mean / max(average_volume, 1e-9) - recent_volume_mean / max(average_volume, 1e-9)
    prior_session_contrast = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_session_contrast = -direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
        prior_session_contrast -= direction * (context.prior_day_shadow_balance or 0.0) * 0.15

    activity = max(
        extreme_retest_pressure * 0.50,
        failed_breakout_recovery * 0.45,
        max(rejection_wick, 0.0) * 0.45,
        max(reversal_body_turn, 0.0) * 0.45,
        max(close_reclaim, 0.0) * 0.30,
        max(range_compression, 0.0) * 0.25,
        max(volume_exhaustion, 0.0) * 0.20,
        max(opposite_room, 0.0) * 0.20,
    )
    if activity < MIN_EXTREME_RETEST_FAILURE_REVERSAL_ACTIVITY:
        return {"event_count": 0.0}
    if reversal_body_turn < -0.42 or close_reclaim < 0.24 or opposite_room < 0.12:
        return {"event_count": 0.0}
    return {
        "extreme_retest_pressure": bounded(extreme_retest_pressure, 0.0, 2.0),
        "rejection_wick": bounded(rejection_wick, 0.0, 1.0),
        "failed_breakout_recovery": bounded(failed_breakout_recovery, 0.0, 8.0),
        "reversal_body_turn": bounded(reversal_body_turn, -1.0, 1.0),
        "close_reclaim": bounded(close_reclaim, 0.0, 1.0),
        "range_compression": bounded(range_compression, -2.0, 2.0),
        "volume_exhaustion": bounded(volume_exhaustion, -4.0, 4.0),
        "opposite_room": bounded(opposite_room, 0.0, 1.0),
        "opening_range_reclaim": bounded(opening_range_reclaim, -4.0, 4.0),
        "prior_session_contrast": bounded(prior_session_contrast, -2.0, 2.0),
        ...
        "trend_into_extreme": bounded(trend_into_extreme, -4.0, 4.0),
        "distance_from_extreme": bounded(distance_from_extreme, 0.0, 8.0),
        "extreme_retest_failure_reversal_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0110_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0110_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0110_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0110_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0110_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0110_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0110_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0110_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0110_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0110_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0110_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0110_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0110_boundary_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0110_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0110_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0110_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0110_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0110_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0110_prior_boundary_bucket(value: float) -> str:
    if value < -0.12:
...
def c0110_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    retest_reward = 0.12 if metrics["extreme_retest_pressure"] > 0.55 and terminal > 0 else 0.0
    wick_reward = 0.09 if metrics["rejection_wick"] > 0.24 and terminal > 0 else 0.0
    recovery_reward = 0.11 if metrics["failed_breakout_recovery"] > 0.22 and terminal > 0 else 0.0
    body_reward = 0.10 if metrics["reversal_body_turn"] > 0.08 and terminal > 0 else 0.0
    reclaim_reward = 0.09 if metrics["close_reclaim"] > 0.58 and terminal > 0 else 0.0
    compression_reward = 0.06 if metrics["range_compression"] > 0.10 and terminal > 0 else 0.0
    volume_reward = 0.05 if metrics["volume_exhaustion"] > 0.10 and terminal > 0 else 0.0
    room_reward = 0.08 if metrics["opposite_room"] > 0.34 and terminal > 0 else 0.0
    opening_reward = 0.05 if metrics["opening_range_reclaim"] > 0.10 and terminal > 0 else 0.0
    prior_reward = 0.04 if metrics["prior_session_contrast"] > 0.08 and terminal > 0 else 0.0
    weak_retest_penalty = 0.08 if metrics["extreme_retest_pressure"] < 0.35 and terminal <= 0 else 0.0
    weak_wick_penalty = 0.08 if metrics["rejection_wick"] < 0.12 and terminal <= 0 else 0.0
    recovery_penalty = 0.09 if metrics["failed_breakout_recovery"] < 0.14 and terminal <= 0 else 0.0
    body_against_penalty = 0.10 if metrics["reversal_body_turn"] < -0.08 and terminal <= 0 else 0.0
    reclaim_absent_penalty = 0.10 if metrics["close_reclaim"] < 0.42 and terminal <= 0 else 0.0
    room_absent_penalty = 0.08 if metrics["opposite_room"] < 0.20 and terminal <= 0 else 0.0
    ...
    extreme_retest_failure_reversal_path_quality = (0.54 * terminal + 0.30 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + extreme_retest_failure_reversal_path_quality
        + retest_reward
        + wick_reward
        + recovery_reward
        + body_reward
...
def fit_linear_extreme_retest_failure_reversal_model(candidates: list[base.Candidate], fold_id: str) -> LinearExtremeRetestFailureReversalModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearExtremeRetestFailureReversalModel(
            fold_id,
    ...
    extreme_retest_failure_reversal_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearExtremeRetestFailureReversalModel(
        fold_id=fold_id,
        ...
        extreme_retest_failure_reversal_failure_rate=extreme_retest_failure_reversal_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearExtremeRetestFailureReversalModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0110-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_extreme_retest_failure_reversal_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0110_r0001_intraday_extreme_retest_failure_reversal",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0110_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0110_r0001_extreme_retest_failure_reversal_summary.json",
        ],
        ...
        "conditional_profiles": {
            "extreme_retest_failure_reversal_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0110 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0110_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0110 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "extreme_retest_failure_reversal_lookback_bars": EXTREME_RETEST_FAILURE_REVERSAL_LOOKBACK_BARS,
        "extreme_retest_failure_reversal_recent_bars": EXTREME_RETEST_FAILURE_REVERSAL_RECENT_BARS,
        "extreme_retest_failure_reversal_prior_bars": EXTREME_RETEST_FAILURE_REVERSAL_PRIOR_BARS,
        "min_extreme_retest_failure_reversal_activity": MIN_EXTREME_RETEST_FAILURE_REVERSAL_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_extreme_retest_failure_reversal_model_summary(model: LinearExtremeRetestFailureReversalModel) -> dict[str, object]:
    return {
        ...
        "extreme_retest_failure_reversal_failure_rate": rounded(model.extreme_retest_failure_reversal_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearExtremeRetestFailureReversalModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_extreme_retest_failure_reversal_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "extreme_retest_failure_reversal_profile": profiles["extreme_retest_failure_reversal_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0110_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0110_r0001_proxy_trades.csv"
    evidence["extreme_retest_failure_reversal_summary"] = "artifacts/c0110_r0001_extreme_retest_failure_reversal_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0110_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0110_r0001_proxy_trades.csv",
        "artifacts/c0110_r0001_extreme_retest_failure_reversal_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0110_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0110 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0110_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0110_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0110_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0110_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0110_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0110_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0110_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0110_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0110_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0110 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0110 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0110_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0110_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0110_r0001_intraday_extreme_retest_failure_reversal_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0110_r0001_proxy_records_extreme_retest_failure_reversal_reference_surface
  ...
  - next_work_is_produce_c0110_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0110_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0110-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0110_r0001_intraday_extreme_retest_failure_reversal",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0110-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0110-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0110-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0110_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0110_r0001_intraday_extreme_retest_failure_reversal",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0110-R0001-EXTREME-RETEST-FAILURE-REVERSAL-SUMMARY",
            "artifact_role": "extreme_retest_failure_reversal_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0110_r0001_extreme_retest_failure_reversal_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0110_r0001_intraday_extreme_retest_failure_reversal",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0110_r0001_mt5_logic_parity_evidence",
        }
    ]
