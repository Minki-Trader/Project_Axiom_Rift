"""C0057 R0001 proxy evidence for intraday impulse response latency."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0057_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0057_r0001_impulse_response_latency_summary.json"
...
FEATURE_NAMES = (
    "prior_impulse_extension_12",
    "counter_response_latency_6",
    "reclaim_efficiency_12",
    "extreme_hold_decay",
    "post_impulse_compression",
    "directional_extreme_pressure",
    "counter_wick_absorption_6",
    "day_range_completion_ratio",
    "range_compression_12_vs_48",
    "volume_confirmation_on_latency_resolution",
    "session_progress_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_impulse_response_latency_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_impulse_response_latency_state"
SELECTION_RULE = "top_fold_local_impulse_response_latency_scores_per_active_day"
...
RESPONSE_LOOKBACK_BARS = 6
MIN_IMPULSE_RESPONSE_ACTIVITY = 0.18
...
class LinearImpulseResponseLatencyModel:
    fold_id: str
...
def run_c0057_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_impulse_response_latency_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_impulse_response_latency_model_summary(model))
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    latency = impulse_response_latency_metrics(bars, volumes, context, volume_average, index, direction, average_range)
    if latency["event_count"] <= 0:
        return None, None
    ...
    volume_confirmation = bounded(
        volume_ratio
        * (
            0.40 * latency["prior_impulse_extension_12"]
            + 0.35 * latency["reclaim_efficiency_12"]
            - 0.25 * latency["counter_response_latency_6"]
        ),
        -4.0,
        4.0,
    )
    features = (
        latency["prior_impulse_extension_12"],
        latency["counter_response_latency_6"],
        latency["reclaim_efficiency_12"],
        latency["extreme_hold_decay"],
        latency["post_impulse_compression"],
        latency["directional_extreme_pressure"],
        latency["counter_wick_absorption_6"],
        latency["day_range_completion_ratio"],
        compression,
        volume_confirmation,
        latency["session_progress_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        impulse_bucket(latency["prior_impulse_extension_12"]),
        latency_bucket(latency["counter_response_latency_6"]),
        reclaim_bucket(latency["reclaim_efficiency_12"]),
        extreme_decay_bucket(latency["extreme_hold_decay"]),
        post_impulse_bucket(latency["post_impulse_compression"]),
        edge_pressure_bucket(latency["directional_extreme_pressure"]),
        absorption_bucket(latency["counter_wick_absorption_6"]),
        compression_bucket(compression),
        participation_bucket(volume_ratio),
        session_pressure_bucket(latency["session_progress_pressure"]),
...
def impulse_response_latency_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    if prior_volume_per_bar is None or prior_volume_per_bar <= 0:
        return {"event_count": 0.0}
    impulse_start = bars[index - IMPULSE_LOOKBACK_BARS].close
    response_start = bars[index - RESPONSE_LOOKBACK_BARS].close
    prior_impulse = direction * (response_start - impulse_start) / average_range
    response_move = direction * (close - response_start) / average_range
    counter_response_latency = max(0.0, -response_move) * (1.0 + min(abs(prior_impulse), 4.0))
    impulse_window = bars[index - IMPULSE_LOOKBACK_BARS : index + 1]
    response_window = bars[index - RESPONSE_LOOKBACK_BARS + 1 : index + 1]
    if direction > 0:
        extreme_position, extreme_value = max(
            enumerate(impulse_window),
            key=lambda item: item[1].high,
        )
        opposite_extreme = min(bar.low for bar in impulse_window)
        pullback_extreme = min(bar.low for bar in response_window)
        reclaim_distance = close - pullback_extreme
        reclaim_span = max(extreme_value.high - pullback_extreme, 1e-9)
        distance_from_extreme = max(0.0, extreme_value.high - close) / average_range
        directional_pressure = (close - opposite_extreme) / max(extreme_value.high - opposite_extreme, 1e-9)
    else:
        extreme_position, extreme_value = min(
            enumerate(impulse_window),
            key=lambda item: item[1].low,
        )
        opposite_extreme = max(bar.high for bar in impulse_window)
        pullback_extreme = max(bar.high for bar in response_window)
        reclaim_distance = pullback_extreme - close
        reclaim_span = max(pullback_extreme - extreme_value.low, 1e-9)
        distance_from_extreme = max(0.0, close - extreme_value.low) / average_range
        directional_pressure = (opposite_extreme - close) / max(opposite_extreme - extreme_value.low, 1e-9)
    extreme_age = len(impulse_window) - 1 - extreme_position
    extreme_hold_decay = (extreme_age / IMPULSE_LOOKBACK_BARS) * min(distance_from_extreme, 4.0)
    reclaim_efficiency = reclaim_distance / reclaim_span
    response_high = max(bar.high for bar in response_window)
    response_low = min(bar.low for bar in response_window)
    post_impulse_compression = (response_high - response_low) / average_range
    counter_wick_absorption = sum(adverse_wick_absorption(item, direction) for item in response_window) / len(response_window)
    range_completion = day_width / average_range
    recent_volume = sum(volumes[max(0, index - RESPONSE_LOOKBACK_BARS + 1) : index + 1])
    expected_recent_volume = prior_volume_per_bar * RESPONSE_LOOKBACK_BARS
    recent_intensity = recent_volume / max(expected_recent_volume, 1e-9)
    session_pressure = bounded_session_progress(bars[index].time) * max(abs(prior_impulse), abs(response_move))
    activity = max(
        abs(prior_impulse),
        counter_response_latency,
        abs(reclaim_efficiency - 0.5),
        extreme_hold_decay,
        counter_wick_absorption,
        session_pressure,
    )
    if activity < MIN_IMPULSE_RESPONSE_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "prior_impulse_extension_12": bounded(prior_impulse, -4.0, 4.0),
        "counter_response_latency_6": bounded(counter_response_latency, 0.0, 4.0),
        "reclaim_efficiency_12": bounded(reclaim_efficiency, 0.0, 1.5),
        "extreme_hold_decay": bounded(extreme_hold_decay, 0.0, 4.0),
        "post_impulse_compression": bounded(post_impulse_compression, 0.0, 4.0),
        "directional_extreme_pressure": bounded(directional_pressure, 0.0, 1.0),
        "counter_wick_absorption_6": bounded(counter_wick_absorption, 0.0, 1.0),
...
def extreme_decay_bucket(value: float) -> str:
    if value < 0.10:
        return "extreme_fresh"
    if value < 0.55:
        return "extreme_holding"
    return "extreme_decaying"


def post_impulse_bucket(value: float) -> str:
    if value < 0.65:
        return "post_impulse_compressed"
    if value < 1.25:
        return "post_impulse_normal"
    return "post_impulse_expanded"
...
def candidate_label(
    ...
    prior_impulse = direction * (bars[index - RESPONSE_LOOKBACK_BARS].close - bars[index - IMPULSE_LOOKBACK_BARS].close) / average_range
    recent_response = direction * (bars[index].close - bars[index - RESPONSE_LOOKBACK_BARS].close) / average_range
    counter_latency = max(0.0, -recent_response)
    continuation_after_latency_reward = 0.18 if prior_impulse > 0.20 and counter_latency > 0.05 and terminal > 0 else 0.0
    reversal_after_latency_reward = 0.18 if prior_impulse < -0.20 and recent_response > 0.10 and terminal > 0 else 0.0
    failed_latency_penalty = 0.22 if prior_impulse > 0.20 and recent_response < -0.20 and mae > mfe else 0.0
    ...
    latency_path_quality = (0.35 * terminal + 0.35 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + latency_path_quality
        + continuation_after_latency_reward
        + reversal_after_latency_reward
        - failed_latency_penalty
        - stall_penalty
...
def fit_linear_impulse_response_latency_model(candidates: list[base.Candidate], fold_id: str) -> LinearImpulseResponseLatencyModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearImpulseResponseLatencyModel(
            fold_id,
    ...
    latency_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearImpulseResponseLatencyModel(
        fold_id=fold_id,
        ...
        latency_failure_rate=latency_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearImpulseResponseLatencyModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_impulse_response_latency_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0057_r0001_intraday_impulse_response_latency",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0057_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0057_r0001_impulse_response_latency_summary.json",
        ],
        ...
        "conditional_profiles": {
            "impulse_response_latency_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0057 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0057_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0057 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def linear_impulse_response_latency_model_summary(model: LinearImpulseResponseLatencyModel) -> dict[str, object]:
    return {
        ...
        "latency_failure_rate": rounded(model.latency_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearImpulseResponseLatencyModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_impulse_response_latency_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "impulse_response_latency_profile": profiles["impulse_response_latency_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0057_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_trade_artifact"] = "artifacts/c0057_r0001_proxy_trades.csv"
    evidence["impulse_response_latency_summary"] = "artifacts/c0057_r0001_impulse_response_latency_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0057_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0057_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0057 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0057_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0057_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0057_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0057_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0057_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0057_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0057_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0057_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0057_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0057 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0057 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260706_produce_c0057_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0057_r0001_proxy_evidence
  refines:
  - dec_20260706_open_c0057_r0001_intraday_impulse_response_latency_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0057_r0001_proxy_records_impulse_response_latency_reference_surface
  ...
  - next_work_is_produce_c0057_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260706_produce_c0057_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0057-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0057_r0001_intraday_impulse_response_latency",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0057-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0057-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0057-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0057_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0057_r0001_intraday_impulse_response_latency",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0057-R0001-impulse-response-latency-SUMMARY",
            "artifact_role": "impulse_response_latency_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0057_r0001_impulse_response_latency_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0057_r0001_intraday_impulse_response_latency",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0057_r0001_mt5_logic_parity_evidence",
        }
    ]
