"""C0131 R0001 proxy evidence for intraday bar sequence alternation pressure release."""
...
CAMPAIGN_ID = "C0131"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0131_intraday_bar_sequence_alternation_pressure_release_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0131_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0131_r0001_bar_sequence_alternation_pressure_release_summary.json"
...
FEATURE_NAMES = (
    "alternation_density",
    "alternation_break_bias",
    "release_run_acceleration",
    "compression_before_release",
    "directional_body_followthrough",
    "close_sequence_settlement",
    "wick_opposition_clearance",
    "volume_release_confirmation",
    "vwap_release_room",
    "prior_day_sequence_context",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_bar_sequence_alternation_pressure_release_rank"
LABEL_SHAPE = "target_first_quality_after_bar_sequence_alternation_pressure_release"
SELECTION_RULE = "top_fold_local_bar_sequence_alternation_pressure_release_scores_per_active_day"
...
MIN_DAY_BARS = 18
BAR_SEQUENCE_ALTERNATION_PRESSURE_RELEASE_LOOKBACK_BARS = 48
BAR_SEQUENCE_ALTERNATION_PRESSURE_RELEASE_MICRO_BARS = 12
MIN_ALTERNATION_DENSITY = 0.36
MIN_RELEASE_PRESSURE = 0.04
MIN_BAR_SEQUENCE_ALTERNATION_PRESSURE_RELEASE_ACTIVITY = 0.045
...
class LinearBarSequenceAlternationPressureReleaseModel:
    fold_id: str
    ...
    bar_sequence_alternation_pressure_release_failure_rate: float
...
def run_c0131_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_bar_sequence_alternation_pressure_release_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_bar_sequence_alternation_pressure_release_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        BAR_SEQUENCE_ALTERNATION_PRESSURE_RELEASE_LOOKBACK_BARS,
        BAR_SEQUENCE_ALTERNATION_PRESSURE_RELEASE_MICRO_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = bar_sequence_alternation_pressure_release_metrics(
    ...
    features = (
        metrics["alternation_density"],
        metrics["alternation_break_bias"],
        metrics["release_run_acceleration"],
        metrics["compression_before_release"],
        metrics["directional_body_followthrough"],
        metrics["close_sequence_settlement"],
        metrics["wick_opposition_clearance"],
        metrics["volume_release_confirmation"],
        metrics["vwap_release_room"],
        metrics["prior_day_sequence_context"],
        metrics["session_maturity"],
        metrics["spread_stress_inverse"],
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        alternation_density_bucket(metrics["alternation_density"]),
        alternation_break_bucket(metrics["alternation_break_bias"]),
        release_acceleration_bucket(metrics["release_run_acceleration"]),
        pre_release_compression_bucket(metrics["compression_before_release"]),
        sequence_body_follow_bucket(metrics["directional_body_followthrough"]),
        sequence_settlement_bucket(metrics["close_sequence_settlement"]),
        wick_clearance_bucket(metrics["wick_opposition_clearance"]),
        volume_release_bucket(metrics["volume_release_confirmation"]),
        vwap_release_bucket(metrics["vwap_release_room"]),
        prior_sequence_context_bucket(metrics["prior_day_sequence_context"]),
...
def bar_sequence_alternation_pressure_release_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    micro_start = max(index - BAR_SEQUENCE_ALTERNATION_PRESSURE_RELEASE_MICRO_BARS + 1, context.day_start_index)
    lookback_start = max(index - BAR_SEQUENCE_ALTERNATION_PRESSURE_RELEASE_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    close_deltas = [lookback_window[pos].close - lookback_window[pos - 1].close for pos in range(1, len(lookback_window))]
    close_signs = [price_sign(value) for value in close_deltas if price_sign(value) != 0.0]
    if len(close_signs) < 8:
        return {"event_count": 0.0}
    alternation_count = sum(1 for pos in range(1, len(close_signs)) if close_signs[pos] != close_signs[pos - 1])
    alternation_density = alternation_count / max(len(close_signs) - 1, 1)
    directional_micro_deltas = [
        direction * (micro_window[pos].close - micro_window[pos - 1].close)
        for pos in range(1, len(micro_window))
    ]
    if len(directional_micro_deltas) < 5:
        return {"event_count": 0.0}
    recent_leg = sum(directional_micro_deltas[-4:])
    previous_leg = sum(directional_micro_deltas[-8:-4]) if len(directional_micro_deltas) >= 8 else 0.0
    recent_abs_travel = sum(abs(value) for value in directional_micro_deltas[-4:])
    alternation_break_bias = recent_leg / max(recent_abs_travel, average_range, 1e-9)
    release_run_acceleration = (recent_leg - previous_leg) / max(average_range * 1.5, 1e-9)
    trailing_run = 0
    for value in reversed(directional_micro_deltas):
        if value <= 0:
            break
        trailing_run += 1
    release_run_length_pressure = trailing_run / 5.0
    compression_before_release = 1.0 - micro_range_mean / max(lookback_range_mean, 1e-9)
    directional_body_followthrough = base.mean(
        [directional_body_score(bar, direction) for bar in micro_window[-4:]]
    ) or 0.0
    close_sequence_settlement = base.mean(
        [directional_bar_settlement(bar, direction) for bar in micro_window[-4:]]
    ) or 0.0
    wick_opposition_clearance = base.mean(
        [adverse_wick_absorption(bar, direction) for bar in micro_window[-6:]]
    ) or 0.0
    if alternation_density < MIN_ALTERNATION_DENSITY and max(alternation_break_bias, release_run_acceleration) < MIN_RELEASE_PRESSURE:
        return {"event_count": 0.0}

    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    vwap_release_room = direction * (signal_bar.close - session_vwap) / max(average_range * 2.5, 1e-9)
    volume_release_confirmation = micro_volume_mean / max(lookback_volume_mean, average_volume, 1e-9) - 1.0
    prior_day_sequence_context = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_day_sequence_context = direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
    if context.prior_day_shadow_balance is not None:
        prior_day_sequence_context += direction * context.prior_day_shadow_balance * 0.05
    ...
    activity = (
        alternation_density * 0.16
        + max(alternation_break_bias, 0.0) * 0.14
        + max(release_run_acceleration, 0.0) * 0.12
        + max(compression_before_release, 0.0) * 0.10
        + max(directional_body_followthrough, 0.0) * 0.11
        + close_sequence_settlement * 0.08
        + wick_opposition_clearance * 0.08
        + max(volume_release_confirmation, -0.10) * 0.04
        + max(vwap_release_room, -0.20) * 0.04
        + release_run_length_pressure * 0.07
    )
    if activity < MIN_BAR_SEQUENCE_ALTERNATION_PRESSURE_RELEASE_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "alternation_density": bounded(alternation_density, 0.0, 1.0),
        "alternation_break_bias": bounded(alternation_break_bias, -2.0, 2.0),
        "release_run_acceleration": bounded(release_run_acceleration, -4.0, 6.0),
        "compression_before_release": bounded(compression_before_release, -2.0, 2.0),
        "directional_body_followthrough": bounded(directional_body_followthrough, -1.0, 1.0),
        "close_sequence_settlement": bounded(close_sequence_settlement, 0.0, 1.0),
        "wick_opposition_clearance": bounded(wick_opposition_clearance, 0.0, 1.0),
        "volume_release_confirmation": bounded(volume_release_confirmation, -2.0, 6.0),
        "vwap_release_room": bounded(vwap_release_room, -4.0, 4.0),
        "prior_day_sequence_context": bounded(prior_day_sequence_context, -3.0, 3.0),
        ...
        "bar_sequence_alternation_pressure_release_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0131_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0131_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0131_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0131_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0131_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0131_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0131_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0131_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0131_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0131_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0131_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0131_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0131_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0131_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0131_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0131_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0131_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0131_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0131_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def c0131_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def alternation_density_bucket(value: float) -> str:
    if value < 0.40:
        return "alternation_sparse"
    if value < 0.62:
        return "alternation_defined"
    return "alternation_dense"


def alternation_break_bucket(value: float) -> str:
    if value < -0.08:
        return "alternation_break_against"
    if value < 0.12:
        return "alternation_break_mixed"
    return "alternation_break_with"


def release_acceleration_bucket(value: float) -> str:
    if value < -0.08:
        return "release_accel_fading"
    if value < 0.18:
        return "release_accel_mixed"
    return "release_accel_expanding"


def pre_release_compression_bucket(value: float) -> str:
    if value < -0.05:
        return "pre_release_expanding"
    if value < 0.18:
        return "pre_release_even"
    return "pre_release_compressed"


def sequence_body_follow_bucket(value: float) -> str:
    if value < -0.08:
        return "sequence_body_against"
    if value < 0.24:
        return "sequence_body_mixed"
    return "sequence_body_with"


def sequence_settlement_bucket(value: float) -> str:
    if value < 0.44:
        return "sequence_settlement_weak"
    if value < 0.66:
        return "sequence_settlement_mixed"
    return "sequence_settlement_strong"
...
def volume_release_bucket(value: float) -> str:
    if value < -0.22:
        return "volume_release_fading"
    if value < 0.28:
        return "volume_release_even"
    return "volume_release_confirmed"


def vwap_release_bucket(value: float) -> str:
    if value < -0.35:
        return "vwap_release_against"
    if value < 0.35:
        return "vwap_release_near"
    return "vwap_release_extended"


def prior_sequence_context_bucket(value: float) -> str:
    if value < -0.25:
        return "prior_sequence_against"
    if value < 0.25:
        return "prior_sequence_mixed"
    return "prior_sequence_with"
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    alternation_reward = 0.10 if metrics["alternation_density"] > 0.48 and terminal > 0 else 0.0
    break_bias_reward = 0.12 if metrics["alternation_break_bias"] > 0.10 and terminal > 0 else 0.0
    acceleration_reward = 0.11 if metrics["release_run_acceleration"] > 0.08 and terminal > 0 else 0.0
    compression_reward = 0.08 if metrics["compression_before_release"] > 0.04 and terminal > 0 else 0.0
    body_reward = 0.10 if metrics["directional_body_followthrough"] > 0.14 and terminal > 0 else 0.0
    settlement_reward = 0.09 if metrics["close_sequence_settlement"] > 0.54 and terminal > 0 else 0.0
    wick_reward = 0.08 if metrics["wick_opposition_clearance"] > 0.24 and terminal > 0 else 0.0
    volume_reward = 0.05 if -0.18 <= metrics["volume_release_confirmation"] <= 1.80 and terminal > 0 else 0.0
    vwap_room_reward = 0.05 if metrics["vwap_release_room"] > -0.25 and terminal > 0 else 0.0
    prior_day_reward = 0.04 if metrics["prior_day_sequence_context"] > -0.35 and terminal > 0 else 0.0
    weak_alternation_penalty = 0.07 if metrics["alternation_density"] < 0.38 and terminal <= 0 else 0.0
    weak_break_penalty = 0.09 if metrics["alternation_break_bias"] < -0.06 and terminal <= 0 else 0.0
    weak_acceleration_penalty = 0.08 if metrics["release_run_acceleration"] < -0.05 and terminal <= 0 else 0.0
    weak_body_penalty = 0.08 if metrics["directional_body_followthrough"] < -0.05 and terminal <= 0 else 0.0
    weak_settlement_penalty = 0.07 if metrics["close_sequence_settlement"] < 0.45 and terminal <= 0 else 0.0
    overheated_volume_penalty = 0.05 if metrics["volume_release_confirmation"] > 2.60 and terminal <= 0 else 0.0
    ...
    return (
        ...
        + vwap_room_reward
        + prior_day_reward
        - weak_alternation_penalty
        - weak_break_penalty
        - weak_acceleration_penalty
        - weak_body_penalty
        ...
        - max(0.0, metrics["volume_release_confirmation"] - 2.60) * 0.015
        - max(0.0, MIN_ALTERNATION_DENSITY - metrics["alternation_density"]) * 0.12
        - max(0.0, MIN_RELEASE_PRESSURE - max(metrics["alternation_break_bias"], metrics["release_run_acceleration"])) * 0.08
    )


def fit_linear_bar_sequence_alternation_pressure_release_model(candidates: list[base.Candidate], fold_id: str) -> LinearBarSequenceAlternationPressureReleaseModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearBarSequenceAlternationPressureReleaseModel(
            fold_id,
    ...
    bar_sequence_alternation_pressure_release_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearBarSequenceAlternationPressureReleaseModel(
        fold_id=fold_id,
        ...
        bar_sequence_alternation_pressure_release_failure_rate=bar_sequence_alternation_pressure_release_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearBarSequenceAlternationPressureReleaseModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0131-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_bar_sequence_alternation_pressure_release_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0131_r0001_intraday_bar_sequence_alternation_pressure_release",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0131_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0131_r0001_bar_sequence_alternation_pressure_release_summary.json",
        ],
        ...
        "conditional_profiles": {
            "bar_sequence_alternation_pressure_release_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0131 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0131_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0131 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "bar_sequence_alternation_pressure_release_lookback_bars": BAR_SEQUENCE_ALTERNATION_PRESSURE_RELEASE_LOOKBACK_BARS,
        "bar_sequence_alternation_pressure_release_micro_bars": BAR_SEQUENCE_ALTERNATION_PRESSURE_RELEASE_MICRO_BARS,
        "min_alternation_density": MIN_ALTERNATION_DENSITY,
        "min_release_pressure": MIN_RELEASE_PRESSURE,
        "min_bar_sequence_alternation_pressure_release_activity": MIN_BAR_SEQUENCE_ALTERNATION_PRESSURE_RELEASE_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_bar_sequence_alternation_pressure_release_model_summary(model: LinearBarSequenceAlternationPressureReleaseModel) -> dict[str, object]:
    return {
        ...
        "bar_sequence_alternation_pressure_release_failure_rate": rounded(model.bar_sequence_alternation_pressure_release_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearBarSequenceAlternationPressureReleaseModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_bar_sequence_alternation_pressure_release_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "bar_sequence_alternation_pressure_release_profile": profiles["bar_sequence_alternation_pressure_release_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0131_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0131_r0001_proxy_trades.csv"
    evidence["bar_sequence_alternation_pressure_release_summary"] = "artifacts/c0131_r0001_bar_sequence_alternation_pressure_release_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0131_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0131_r0001_proxy_trades.csv",
        "artifacts/c0131_r0001_bar_sequence_alternation_pressure_release_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0131_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0131 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0131_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0131_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0131_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0131_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0131_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0131_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0131_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0131_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0131_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0131 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0131 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    block = f"""
- decision_id: dec_20260708_produce_c0131_r0001_proxy_evidence
  created_local_date: '{local_date}'
  status: active
  decision: produce_c0131_r0001_proxy_evidence
  refines:
  - dec_20260708_open_c0131_r0001_intraday_bar_sequence_alternation_pressure_release_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0131_r0001_proxy_records_bar_sequence_alternation_pressure_release_reference_surface
  ...
  - next_work_is_produce_c0131_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260708_produce_c0131_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0131-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0131_r0001_intraday_bar_sequence_alternation_pressure_release",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0131-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0131-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0131-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0131_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0131_r0001_intraday_bar_sequence_alternation_pressure_release",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0131-R0001-BAR-SEQUENCE-ALTERNATION-PRESSURE-RELEASE-SUMMARY",
            "artifact_role": "bar_sequence_alternation_pressure_release_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0131_r0001_bar_sequence_alternation_pressure_release_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0131_r0001_intraday_bar_sequence_alternation_pressure_release",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0131_r0001_mt5_logic_parity_evidence",
        }
    ]
