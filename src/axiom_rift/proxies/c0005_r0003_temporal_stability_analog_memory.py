"""C0005 R0003 proxy evidence for temporal-stability analog-memory entries."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0005_r0003_proxy_trades.csv"
ANALOG_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0005_r0003_analog_memory_summary.json"
...
MODEL_FAMILY = "fold_local_temporal_stability_dispersion_analog_memory"
LABEL_SHAPE = "directional_path_stability_risk_adjusted_excursion"
ANALOG_NEIGHBOR_COUNT = 80
INNER_NEIGHBOR_COUNT = 32
MAX_ANALOG_DISTANCE = 5.75
MIN_STABILITY_LIFT = -0.01
MAX_ADVERSE_RATE_LIFT = 0.06
MAX_INNER_OUTER_GAP = 0.38
...
def run_c0005_r0003_proxy(write: bool = True) -> dict[str, object]:
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            ...
            if features is None:
                continue
            label = temporal_stability_label(bars, range_average, index, direction) if include_labels else None
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|temporal_stability_analog_memory",
                    features=features,
                    label=label,
                )
            )
...
def temporal_stability_label(
    ...
    stability_edge = path_alignment - 0.5
...
def fit_analog_model(candidates: list[base.Candidate], fold_id: str) -> AnalogModel:
    ...
    return AnalogModel(
        ...
        global_adverse_rate=float(np.mean(labels < -0.45)),
    )
...
def score_candidates(candidates: list[base.Candidate], model: AnalogModel) -> list[base.Candidate]:
    ...
    local_adverse_rate = (neighbor_labels < -0.45).mean(axis=1)
    ...
    scores = (
        weighted_mean
        + 0.22 * (local_positive_rate - model.global_positive_rate)
        - 0.52 * (local_adverse_rate - model.global_adverse_rate)
        + 0.16 * (recent_mean - model.global_recent_mean)
        + 0.12 * local_upper_tail
        - 0.10 * local_std
        - 0.18 * inner_outer_gap
        - 0.014 * mean_distance
    )
    ...
    for index, candidate in enumerate(candidates):
        if mean_distance[index] > MAX_ANALOG_DISTANCE:
            score = None
        elif weighted_mean[index] - model.global_recent_mean < MIN_STABILITY_LIFT:
            score = None
        elif local_adverse_rate[index] - model.global_adverse_rate > MAX_ADVERSE_RATE_LIFT:
            score = None
        elif inner_outer_gap[index] > MAX_INNER_OUTER_GAP:
            score = None
        else:
...
def build_proxy_payload(
    ...
    payload["proxy_id"] = "PX-C0005-R0003"
    payload["proxy_engine"] = "axiom_rift.proxies.c0005_r0003_temporal_stability_analog_memory"
    payload["proxy_config_path"] = "campaigns/C0005_continuous_analog_memory_discovery/runs/R0003/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_temporal_stability_analog_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0005_continuous_analog_memory_discovery/runs/R0003/kpi/proxy.json",
        "campaigns/C0005_continuous_analog_memory_discovery/runs/R0003/artifacts/c0005_r0003_proxy_trades.csv",
        "campaigns/C0005_continuous_analog_memory_discovery/runs/R0003/artifacts/c0005_r0003_analog_memory_summary.json",
    ]
    ...
    profiles["analog_memory_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "min_stability_lift": MIN_STABILITY_LIFT,
            "max_adverse_rate_lift": MAX_ADVERSE_RATE_LIFT,
            "max_inner_outer_gap": MAX_INNER_OUTER_GAP,
            "fold_models": fold_models,
            "state_distributions": state_distributions,
            "candidates_by_fold": candidates_by_fold,
            "selection_rule": "top_fold_local_temporal_stability_analog_scores_per_active_day",
    ...
    config.update(
        {
            ...
            "max_adverse_rate_lift": MAX_ADVERSE_RATE_LIFT,
            "max_inner_outer_gap": MAX_INNER_OUTER_GAP,
            "score_interpretation": "higher_score_means_recency_weighted_stable_low_dispersion_local_analog_edge",
            "variant_boundary": "temporal_stability_dispersion_analog_memory_not_r0001_or_r0002_parameter_nudge_or_retry",
        }
    )
    return config
...
def analog_model_summary(model: AnalogModel) -> dict[str, object]:
    return {
        ...
        "global_adverse_rate": base.rounded(model.global_adverse_rate),
...
def write_analog_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_temporal_stability_analog_memory_summary_v1",
        "template": False,
        "work_unit_id": "C0005",
        "campaign_id": "C0005",
        "run_id": "R0003",
        ...
        [
            artifact_record(
                ...
                [
                    ...
                    "campaigns/C0005_continuous_analog_memory_discovery/runs/R0003/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0005-R0003-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0005-R0003-ANALOG-MEMORY-SUMMARY",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0005_r0003_mt5_logic_parity_evidence",
        }
    ]
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0005_r0003_temporal_stability_analog_memory",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0005_r0003_proxy_trades.csv"
    evidence["analog_memory_summary"] = "artifacts/c0005_r0003_analog_memory_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0005_r0003_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0005_r0003_proxy_trades.csv",
        "artifacts/c0005_r0003_analog_memory_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0005 R0003 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0005_r0003_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0005_r0003_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    if "produce_c0005_r0003_proxy_evidence" not in completed:
        completed.append("produce_c0005_r0003_proxy_evidence")
    next_action = "produce_c0005_r0003_mt5_logic_parity_evidence"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0005_r0003_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0005_continuous_analog_memory_discovery/runs/R0003/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0005_r0003_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            ...
            "R0004": "R0003",
            "R0001": "R0003",
            "c0004_r0001_fold_local_state_archetype": "c0005_r0003_temporal_stability_analog_memory",
            "c0004_r0001": "c0005_r0003",
