"""C0022 R0001 proxy evidence for volatility term structure discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0022_volatility_term_structure_discovery" / "runs" / "R0001"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0022_volatility_term_structure_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0022_r0001_proxy_trades.csv"
VOLATILITY_TERM_STRUCTURE_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0022_r0001_volatility_term_structure_summary.json"
...
FEATURE_NAMES = (
    ...
    "range_24_over_96",
    "range_3_over_96",
    "term_structure_slope_3_12",
    "term_structure_slope_12_48",
    "term_structure_convexity_3_12_48",
    "compression_depth_12_over_96",
    "expansion_impulse_3_over_36",
    "expansion_persistence_6_over_24",
    "expansion_cooling_3_vs_12",
    "range_variability_12",
    "range_variability_48",
    "range_zscore_3_vs_96",
    "range_zscore_12_vs_96",
    "range_slope_3_minus_12",
    "range_slope_12_minus_48",
    "range_acceleration_3_12_48",
    "directional_return_6_over_range",
    "directional_return_18_over_range",
    "directional_return_48_over_range",
...
MODEL_FAMILY = "fold_local_volatility_term_structure_rank"
LABEL_SHAPE = "directional_volatility_term_structure_transition_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_volatility_term_structure_scores_per_active_day"
...
def run_c0022_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    term_context = build_term_structure_context(bars)
    trades: list[base.Trade] = []
...
def build_candidates(
    ...
    term_context: list[dict[str, float] | None],
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = volatility_term_structure_features(
                bars,
                range_average,
                short_range_average,
                day_context,
            ...
            if features is None:
                continue
            label = (
                volatility_term_structure_label(bars, range_average, term_context, index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|volatility_term_structure",
                    features=features,
                    label=label,
                )
            )
...
def build_term_structure_context(bars: list[base.Bar]) -> list[dict[str, float] | None]:
    ...
    def rolling_mean(index: int, lookback: int) -> float:
        start = index - lookback + 1
        return (prefix[index + 1] - prefix[start]) / lookback

    def rolling_std(index: int, lookback: int, mean_value: float) -> float:
        start = index - lookback + 1
        mean_sq = (prefix_sq[index + 1] - prefix_sq[start]) / lookback
        return math.sqrt(max(mean_sq - mean_value * mean_value, 0.0))

    contexts: list[dict[str, float] | None] = [None] * len(bars)
    for index in range(95, len(bars)):
        range_3 = rolling_mean(index, 3)
        range_6 = rolling_mean(index, 6)
        range_12 = rolling_mean(index, 12)
        range_24 = rolling_mean(index, 24)
        range_36 = rolling_mean(index, 36)
        range_48 = rolling_mean(index, 48)
        range_96 = rolling_mean(index, 96)
        std_12 = rolling_std(index, 12, range_12)
        std_48 = rolling_std(index, 48, range_48)
        std_96 = rolling_std(index, 96, range_96)
        contexts[index] = {
            "range_3": range_3,
            "range_6": range_6,
            "range_12": range_12,
...
def volatility_term_structure_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    range_3 = term["range_3"]
    range_6 = term["range_6"]
    range_12 = term["range_12"]
    range_24 = term["range_24"]
    range_36 = term["range_36"]
    range_48 = term["range_48"]
    range_96 = term["range_96"]
    range_3_over_12 = range_3 / max(range_12, 1e-9)
    range_6_over_24 = range_6 / max(range_24, 1e-9)
    range_12_over_48 = range_12 / max(range_48, 1e-9)
    range_24_over_96 = range_24 / max(range_96, 1e-9)
    range_3_over_96 = range_3 / max(range_96, 1e-9)
    short_mid_slope = math.log(max(range_3, 1e-9) / max(range_12, 1e-9))
    mid_long_slope = math.log(max(range_12, 1e-9) / max(range_48, 1e-9))
    term_structure_convexity = short_mid_slope - mid_long_slope
    compression_depth = 1.0 - range_12_over_48
    expansion_impulse = range_3 / max(range_36, 1e-9)
    expansion_persistence = range_6 / max(range_24, 1e-9)
    expansion_cooling = (range_12 - range_3) / max(range_12, 1e-9)
    range_variability_12 = term["std_12"] / max(range_12, 1e-9)
    range_variability_48 = term["std_48"] / max(range_48, 1e-9)
    range_zscore_3 = (range_3 - range_96) / max(term["std_96"], 1e-9)
    range_zscore_12 = (range_12 - range_96) / max(term["std_96"], 1e-9)
    range_slope_3_minus_12 = (range_3 - range_12) / max(average_range, 1e-9)
    range_slope_12_minus_48 = (range_12 - range_48) / max(average_range, 1e-9)
    range_acceleration = range_slope_3_minus_12 - range_slope_12_minus_48
    ...
    ret_48 = directional_return(bars, index, direction, 48, average_range)
    ...
    breakout_pressure_12 = directional_breakout_pressure(bars, index, direction, 12, average_range)
    compression_breakout_distance = breakout_pressure_12 * max(0.0, compression_depth)
    ...
    if spread_over_range > 0.65 or spread_pressure > 2.20 or range_96 <= 0:
        return None
    return (
        day_progress,
...
def volatility_term_structure_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    term_context: list[dict[str, float] | None],
    index: int,
    ...
    average_range = range_average[index]
    term = term_context[index]
    if average_range is None or average_range <= 0 or entry_index >= exit_index or term is None:
        return None
    ...
    pre_range_12 = term["range_12"]
    pre_range_48 = term["range_48"]
    future_ranges = [max(bar.high - bar.low, 0.0) for bar in path[: min(6, len(path))]]
    future_range_6 = sum(future_ranges) / max(len(future_ranges), 1)
    transition_expansion = future_range_6 / max(pre_range_12, 1e-9)
    transition_slope_shift = (future_range_6 - pre_range_12) / max(pre_range_48, 1e-9)
    aligned_expansion = transition_expansion if terminal_norm > 0.0 else -transition_expansion
    runaway_adverse_expansion = transition_expansion if terminal_norm < 0.0 else 0.0
    return (
        0.82 * first_hit
        + 0.34 * target_speed
        - 0.46 * stop_speed
        ...
        + 0.20 * aligned_expansion
        + 0.22 * transition_slope_shift
        - 0.30 * runaway_adverse_expansion
        - 1.08 * spread_norm
        - 0.20 * spread_drift
    )
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0022"
    payload["work_unit_id"] = "C0022"
    payload["run_id"] = "R0001"
    payload["proxy_id"] = "PX-C0022-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0022_r0001_volatility_term_structure"
    payload["proxy_config_path"] = "campaigns/C0022_volatility_term_structure_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_volatility_term_structure_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0022_volatility_term_structure_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0022_volatility_term_structure_discovery/runs/R0001/artifacts/c0022_r0001_proxy_trades.csv",
        "campaigns/C0022_volatility_term_structure_discovery/runs/R0001/artifacts/c0022_r0001_volatility_term_structure_summary.json",
    ]
    ...
    profiles["volatility_term_structure_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0022_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "feature_weight_floor": FEATURE_WEIGHT_FLOOR,
            "feature_weight_ceiling": FEATURE_WEIGHT_CEILING,
            "score_interpretation": "higher_score_means_direct_fold_local_volatility_term_structure_rank",
            "variant_boundary": "volatility_term_structure_rank_not_auction_threshold_monthly_memory_lifecycle_structural_trap_score_floor_stop_target_hold_or_retry_nudge",
        }
    )
    return config
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_volatility_term_structure_summary_artifact(payload, VOLATILITY_TERM_STRUCTURE_SUMMARY_PATH)
    PROXY_PATH.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="ascii")
    trade_hash = base.sha256_file(TRADE_ARTIFACT_PATH)
    summary_hash = base.sha256_file(VOLATILITY_TERM_STRUCTURE_SUMMARY_PATH)
...
def write_volatility_term_structure_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_volatility_term_structure_summary_v1",
        "template": False,
        "work_unit_id": "C0022",
        "campaign_id": "C0022",
        "run_id": "R0001",
        ...
        "volatility_term_structure_profile": profiles["volatility_term_structure_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "volatility_term_structure_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0022-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0022_volatility_term_structure_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0022_volatility_term_structure_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0022-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0022-R0001-VOLATILITY-TERM-STRUCTURE-SUMMARY",
                "volatility_term_structure_summary_artifact",
                "json",
                "campaigns/C0022_volatility_term_structure_discovery/runs/R0001/artifacts/c0022_r0001_volatility_term_structure_summary.json",
                summary_hash,
                ["campaigns/C0022_volatility_term_structure_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0022_r0001_volatility_term_structure",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0022_r0001_proxy_trades.csv"
    evidence["volatility_term_structure_summary"] = "artifacts/c0022_r0001_volatility_term_structure_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0022_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0022_r0001_proxy_trades.csv",
        "artifacts/c0022_r0001_volatility_term_structure_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0022 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0022_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0022_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0022_volatility_term_structure_discovery"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0022_volatility_term_structure_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0022_volatility_term_structure_discovery",
        "open_c0022_r0001_fold_local_volatility_term_structure_rank_run",
        "produce_c0022_r0001_proxy_evidence",
    ):
    ...
    data["active_run"] = "campaigns/C0022_volatility_term_structure_discovery/runs/R0001"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["active_campaign"] = "campaigns/C0022_volatility_term_structure_discovery"
    data["active_run"] = "campaigns/C0022_volatility_term_structure_discovery/runs/R0001"
    data["latest_operation"] = {
        "id": "produce_c0022_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0022_volatility_term_structure_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0022_r0001_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0022",
            "c0004_r0001_fold_local_state_archetype": "c0022_r0001_volatility_term_structure",
            "c0004_r0001": "c0022_r0001",
            "fold_local_state_archetype_discovery": "volatility_term_structure_discovery",
            ...
            "state_archetype_summary": "volatility_term_structure_summary",
        }
