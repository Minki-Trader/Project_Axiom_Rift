"""C0047 R0002 proxy evidence for intraday liquidity void rejection continuation discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0047_intraday_liquidity_void_reversion_discovery" / "runs" / "R0002"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0047_intraday_liquidity_void_reversion_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0047_r0002_proxy_trades.csv"
LIQUIDITY_VOID_REVERSION_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0047_r0002_liquidity_void_reversion_summary.json"
...
FEATURE_NAMES = (
    ...
    "continuation_displacement_3",
    "continuation_displacement_6",
    "failed_fill_return_1",
    "failed_fill_return_3",
    "void_gap_3",
    "void_gap_6",
    "low_overlap_3",
    "low_overlap_6",
    ...
    "body_imbalance_6",
    "body_imbalance_12",
    "continuation_void_energy_6",
    "continuation_void_energy_12",
    "void_rejection_impulse_3",
    "void_rejection_impulse_6",
    "failed_fill_pressure_3",
    "failed_fill_pressure_6",
    "failed_fill_pressure_12",
    "continuation_age_pressure_12",
    "continuation_decay_pressure_24",
    ...
    "spread_over_range",
    "spread_void_stress",
    "continuation_to_spread_margin",
    "continuation_cost_buffer",
    "late_session_void_pressure",
    "wick_continuation_fragility",
    "adverse_churn_pressure",
    "quantized_continuation_margin",
    "continuation_survival_interaction",
    "void_continuation_alignment",
    "void_rejection_interaction",
    "void_energy_interaction",
)
MODEL_FAMILY = "fold_local_intraday_liquidity_void_rejection_continuation_rank"
LABEL_SHAPE = "directional_intraday_liquidity_void_rejection_continuation_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_intraday_liquidity_void_rejection_continuation_scores_per_active_day"
...
def run_c0047_r0002_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    price_context = build_liquidity_void_reversion_context(bars, range_average, short_range_average, day_context)
    trades: list[base.Trade] = []
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = intraday_liquidity_void_reversion_features(
                bars,
                range_average,
                short_range_average,
                day_context,
            ...
            if features is None:
                continue
            label = (
                intraday_liquidity_void_reversion_label(bars, range_average, price_context, index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|intraday_liquidity_void_reversion",
                    features=features,
                    label=label,
                )
            )
...
def build_liquidity_void_reversion_context(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
...
def intraday_liquidity_void_reversion_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    if len(features) != len(FEATURE_NAMES):
        raise RuntimeError(f"C0047 feature length mismatch: {len(features)} != {len(FEATURE_NAMES)}")
    return tuple(float(value) for value in features)


def intraday_liquidity_void_reversion_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    ribbon_context: list[dict[str, float] | None],
    index: int,
    ...
    future_release = future_liquidity_void_reversion_retention(ribbon_context, entry_index, exit_index, direction)
...
def future_liquidity_void_reversion_retention(
    ribbon_context: list[dict[str, float] | None],
    entry_index: int,
    exit_index: int,
    direction: int,
...
def linear_model_summary(model: LinearEdgeModel) -> dict[str, object]:
    return {
        ...
        "score_interpretation": "higher_score_means_fold_local_intraday_liquidity_void_reversion_fill_rejection_rank",
    }
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0047"
    payload["work_unit_id"] = "C0047"
    payload["run_id"] = "R0002"
    payload["proxy_id"] = "PX-C0047-R0002"
    payload["proxy_engine"] = "axiom_rift.proxies.c0047_r0002_intraday_liquidity_void_rejection_continuation"
    payload["proxy_config_path"] = "campaigns/C0047_intraday_liquidity_void_reversion_discovery/runs/R0002/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_intraday_liquidity_void_reversion_fill_rejection_rank_fit"
    payload["claim_boundary"] = dict(CLAIM_BOUNDARY)
    payload["proxy_artifact_paths"] = [
        "campaigns/C0047_intraday_liquidity_void_reversion_discovery/runs/R0002/kpi/proxy.json",
        "campaigns/C0047_intraday_liquidity_void_reversion_discovery/runs/R0002/artifacts/c0047_r0002_proxy_trades.csv",
        "campaigns/C0047_intraday_liquidity_void_reversion_discovery/runs/R0002/artifacts/c0047_r0002_liquidity_void_reversion_summary.json",
    ]
    ...
    profiles["intraday_liquidity_void_reversion_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
            "model_family": MODEL_FAMILY,
            "feature_count": len(FEATURE_NAMES),
            ...
            "candidate_direction": "dual_direction_long_and_short_cost_fill_rejection_per_closed_bar",
            "model_selected": False,
            "feature_set_selected": False,
            "label_selected": False,
        },
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0047_r0002_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "score_interpretation": "higher_score_means_fold_local_intraday_liquidity_void_reversion_fill_rejection_rank",
            "variant_boundary": (
                "intraday_liquidity_void_reversion_fill_rejection_rank_changes_feature_label_model_and_scoring_surface_"
                "not_threshold_window_stop_target_hold_session_activity_spread_capital_monthly_filter_materialization_"
                "onnx_selected_runtime_or_retry_nudge"
            ),
        }
    )
    return config
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_liquidity_void_reversion_summary_artifact(payload, LIQUIDITY_VOID_REVERSION_SUMMARY_PATH)
    write_text_local(PROXY_PATH, json.dumps(payload, indent=2, sort_keys=True) + "\n")
    trade_hash = sha256_file_local(TRADE_ARTIFACT_PATH)
    summary_hash = sha256_file_local(LIQUIDITY_VOID_REVERSION_SUMMARY_PATH)
...
def write_liquidity_void_reversion_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_intraday_liquidity_void_reversion_summary_v1",
        "template": False,
        "work_unit_id": "C0047",
        "campaign_id": "C0047",
        "run_id": "R0002",
        ...
        "intraday_liquidity_void_reversion_profile": profiles["intraday_liquidity_void_reversion_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "intraday_liquidity_void_reversion_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0047-R0002-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0047_intraday_liquidity_void_reversion_discovery/runs/R0002/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0047_intraday_liquidity_void_reversion_discovery/runs/R0002/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0047-R0002-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0047-R0002-LIQUIDITY-VOID-REVERSION-SUMMARY",
                "intraday_liquidity_void_reversion_summary_artifact",
                "json",
                "campaigns/C0047_intraday_liquidity_void_reversion_discovery/runs/R0002/artifacts/c0047_r0002_liquidity_void_reversion_summary.json",
                summary_hash,
                ["campaigns/C0047_intraday_liquidity_void_reversion_discovery/runs/R0002/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0047_r0002_intraday_liquidity_void_rejection_continuation",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0047_r0002_proxy_trades.csv"
    evidence["intraday_liquidity_void_reversion_summary"] = "artifacts/c0047_r0002_liquidity_void_reversion_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0047_r0002_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0047_r0002_proxy_trades.csv",
        "artifacts/c0047_r0002_liquidity_void_reversion_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0047 R0002 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0047_r0002_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0047_r0002_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0047_intraday_liquidity_void_reversion_discovery"
    data["project"]["active_run"] = "campaigns/C0047_intraday_liquidity_void_reversion_discovery/runs/R0002"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0047_intraday_liquidity_void_reversion_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0047_intraday_liquidity_void_reversion_discovery",
        "open_c0047_r0002_intraday_liquidity_void_rejection_continuation_run",
        "produce_c0047_r0002_proxy_evidence",
    ):
    ...
    data["active_campaign"] = "campaigns/C0047_intraday_liquidity_void_reversion_discovery"
    data["active_run"] = "campaigns/C0047_intraday_liquidity_void_reversion_discovery/runs/R0002"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0047_r0002_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0047_intraday_liquidity_void_reversion_discovery/runs/R0002/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0047_r0002_mt5_logic_parity_evidence",
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["canonical_source"] = "campaigns/C0047_intraday_liquidity_void_reversion_discovery/runs/R0002/kpi/proxy.json"
    data["canonical_status"] = "proxy_recorded_pending_mt5"
    data["current_decision"] = "produce_c0047_r0002_mt5_logic_parity_evidence"
    data["active_campaign"] = "campaigns/C0047_intraday_liquidity_void_reversion_discovery"
    data["active_run"] = "campaigns/C0047_intraday_liquidity_void_reversion_discovery/runs/R0002"
    data["next_required_action"] = "produce_c0047_r0002_mt5_logic_parity_evidence"
    data["current_evidence_summary"] = {
        "source_campaign": "campaigns/C0047_intraday_liquidity_void_reversion_discovery",
        "current_task": "produce_c0047_r0002_mt5_logic_parity_evidence",
        "active_run": "campaigns/C0047_intraday_liquidity_void_reversion_discovery/runs/R0002",
        "active_run_status": "proxy_recorded_pending_mt5",
        "evidence_status": "proxy_recorded_pending_mt5",
        "campaign_family": "intraday_liquidity_void_reversion",
        "entries_per_active_day": summary.get("entries_per_active_day") if isinstance(summary, dict) else None,
        "proxy_trade_count": required.get("proxy_trade_count") if isinstance(required, dict) else None,
        "proxy_net_pnl_points": required.get("proxy_net_pnl_points") if isinstance(required, dict) else None,
        "proxy_profit_factor": required.get("proxy_profit_factor") if isinstance(required, dict) else None,
        "next_required_action": "produce_c0047_r0002_mt5_logic_parity_evidence",
        "note": "C0047 R0002 proxy evidence recorded; MT5 paired validation remains mandatory regardless of proxy result.",
        "source_campaign_manifest": "campaigns/C0047_intraday_liquidity_void_reversion_discovery/campaign.yaml",
    }
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0047",
            "c0004_r0002_fold_local_state_archetype": "c0047_r0002_intraday_liquidity_void_rejection_continuation",
            "c0004_r0002": "c0047_r0002",
            "fold_local_state_archetype_discovery": "intraday_liquidity_void_reversion_discovery",
            ...
            "state_archetype_summary": "intraday_liquidity_void_reversion_summary",
        }
...
def intraday_liquidity_void_reversion_features(
    ...
    favorable_wick = favorable_wick_acceptance(bars, index, direction, 6, average_range)
    opposing_wick = opposing_wick_rejection(bars, index, direction, 6, average_range)
    path_efficiency = directional_path_efficiency(bars, index, direction, 12)
    path_churn = flow_path_churn_ratio(bars, index, 12)
    spread_flow_stress = spread_over_range * (1.0 + context["energy_6"] + pressure_build_12 + release_3)
    release_to_spread_margin = release_3 + 0.50 * release_6 - 2.0 * spread_over_range
    fragility_cost_buffer = max(
        0.0,
        release_3 + 0.35 * body_align_6 + 0.25 * pressure_release_alignment - spread_flow_stress,
    )
    late_session_spread_pressure = context["session_progress"] * spread_flow_stress
    wick_execution_fragility = max(0.0, opposing_wick - favorable_wick) + spread_over_range * (1.0 + path_churn)
    adverse_churn_pressure = path_churn * (1.0 + pressure_build_12 + opposing_pressure_12) + max(0.0, -path_efficiency)
    quantized_margin_proxy = math.trunc((pressure_release_alignment - spread_flow_stress) * 50.0) / 50.0
    pressure_release_survival_interaction = (
        (pressure_release_alignment - spread_flow_stress) * (0.5 + release_3)
        - 0.25 * wick_execution_fragility
    )
    features = (
        float(direction),
        ...
        spread_over_range,
        spread_flow_stress,
        release_to_spread_margin,
        fragility_cost_buffer,
        late_session_spread_pressure,
        wick_execution_fragility,
        adverse_churn_pressure,
        quantized_margin_proxy,
        pressure_release_survival_interaction,
        pressure_release_alignment,
...
def intraday_liquidity_void_reversion_label(
    ...
    spread_norm = bars[entry_index].spread_points / average_range
    path_spread_peak = max(bar.spread_points for bar in path) / average_range
    churn = future_path_churn(path, entry)
    spread_spike = max(0.0, path_spread_peak - spread_norm)
    survival_buffer = max(0.0, early_followthrough + 0.40 * favorable + 0.25 * pre_release - spread_norm - 0.50 * path_spread_peak)
    adverse_cost_drag = spread_norm + 0.80 * path_spread_peak + 0.25 * context["session_progress"] * path_spread_peak
    ambiguous_exit_drag = max(0.0, early_adverse + churn - early_followthrough)
    return (
        0.86 * first_hit
        + 0.34 * target_speed
        - 0.48 * stop_speed
        ...
        - 1.08 * spread_norm
        - 0.20 * spread_spike
        + 0.22 * survival_buffer
        - 0.34 * adverse_cost_drag
        - 0.20 * ambiguous_exit_drag
    )
...
def _c0047_r0002_final_void_context(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
...
def _c0047_r0002_recent_void_gap(
    bars: list[base.Bar],
    index: int,
    direction: int,
    lookback: int,
    ...
    for cursor in range(start, index + 1):
        if direction > 0:
            gap = bars[cursor - 2].low - bars[cursor].high
        else:
            gap = bars[cursor].low - bars[cursor - 2].high
        value = max(value, gap / average_range)
    return max(0.0, value)


def _c0047_r0002_low_overlap_pressure(
    bars: list[base.Bar],
    index: int,
    direction: int,
    lookback: int,
    ...
    for cursor in range(start, index + 1):
        ...
        current_range = max(bar.high - bar.low, 1e-9)
        overlap = max(0.0, min(bar.high, previous.high) - max(bar.low, previous.low))
        low_overlap = 1.0 - bounded(overlap / current_range, 0.0, 1.0)
        continuation_body = max(0.0, direction * (bar.close - bar.open) / current_range)
        weighted_gap += low_overlap * (0.35 + continuation_body)
        weight += 1.0
    return weighted_gap / max(weight, 1.0)


def _c0047_r0002_body_imbalance(
    bars: list[base.Bar],
    index: int,
    direction: int,
    lookback: int,
    ...
    for cursor in range(start, index + 1):
        bar = bars[cursor]
        body_sum += max(0.0, direction * (bar.close - bar.open))
        range_sum += max(bar.high - bar.low, 1e-9)
    return body_sum / max(range_sum, 1e-9)


def _c0047_r0002_final_void_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    if spread_over_range > 0.65:
        return None
    directional_ret_1 = direction * context["ret_1"]
    directional_ret_3 = direction * context["ret_3"]
    directional_ret_6 = direction * context["ret_6"]
    continuation_displacement_3 = max(0.0, directional_ret_3)
    continuation_displacement_6 = max(0.0, directional_ret_6)
    failed_fill_1 = max(0.0, -directional_ret_1)
    failed_fill_3 = max(0.0, -directional_ret_3)
    void_gap_3 = _c0047_r0002_recent_void_gap(bars, index, -direction, 3, average_range)
    void_gap_6 = _c0047_r0002_recent_void_gap(bars, index, -direction, 6, average_range)
    low_overlap_3 = _c0047_r0002_low_overlap_pressure(bars, index, direction, 3)
    low_overlap_6 = _c0047_r0002_low_overlap_pressure(bars, index, direction, 6)
    low_overlap_12 = _c0047_r0002_low_overlap_pressure(bars, index, direction, 12)
    body_imbalance_3 = _c0047_r0002_body_imbalance(bars, index, direction, 3)
    body_imbalance_6 = _c0047_r0002_body_imbalance(bars, index, direction, 6)
    body_imbalance_12 = _c0047_r0002_body_imbalance(bars, index, direction, 12)
    continuation_void_energy_6 = continuation_displacement_6 * (0.5 + low_overlap_6 + body_imbalance_6)
    continuation_void_energy_12 = max(0.0, direction * context["ret_12"]) * (
        0.5 + low_overlap_12 + body_imbalance_12
    )
    rejection_3 = continuation_displacement_3 * (0.5 + void_gap_3 + low_overlap_3)
    rejection_6 = max(0.0, directional_ret_6 + 0.35 * continuation_displacement_3) * (
        0.5 + void_gap_6 + low_overlap_6
    )
    failed_fill_pressure_3 = failed_fill_3 * (0.5 + void_gap_3 + low_overlap_3 + body_imbalance_3)
    failed_fill_pressure_6 = max(0.0, -directional_ret_6) * (0.5 + void_gap_6 + low_overlap_6 + body_imbalance_6)
    failed_fill_pressure_12 = max(0.0, -direction * context["ret_12"]) * (
        0.5 + low_overlap_12 + body_imbalance_12
    )
    void_age_pressure = 0.55 * continuation_void_energy_12 + 0.30 * continuation_void_energy_6 - 0.20 * failed_fill_3
    void_decay_pressure = max(0.0, direction * context["ret_24"]) - 0.35 * max(0.0, direction * context["ret_6"])
    day_return = direction * (bars[index].close - context["day_open"]) / average_range
    if direction > 0:
        day_void_position = (bars[index].close - context["day_low"]) / context["day_range"]
    else:
        day_void_position = (context["day_high"] - bars[index].close) / context["day_range"]
    day_mid_pressure = direction * (bars[index].close - context["day_mid"]) / average_range
    favorable_wick = favorable_wick_acceptance(bars, index, direction, 6, average_range)
    opposing_wick = opposing_wick_rejection(bars, index, direction, 6, average_range)
    path_efficiency = directional_path_efficiency(bars, index, direction, 12)
    path_churn = flow_path_churn_ratio(bars, index, 12)
    void_continuation_alignment = (
        0.35 * continuation_void_energy_6
        + 0.28 * rejection_3
        + 0.18 * favorable_wick
        + 0.12 * max(0.0, day_mid_pressure)
        - 0.25 * opposing_wick
    )
    void_rejection_interaction = rejection_6 * (0.5 + low_overlap_6) - 0.35 * failed_fill_pressure_12
    void_energy_interaction = (continuation_void_energy_6 + rejection_3) * (
        0.5 + context["energy_6"] + 0.25 * context["range_expansion_6_24"]
    )
    spread_void_stress = spread_over_range * (1.0 + continuation_void_energy_6 + low_overlap_6 + body_imbalance_6)
    continuation_to_spread_margin = continuation_displacement_3 + 0.45 * rejection_6 - 2.0 * spread_over_range
    void_cost_buffer = max(
        0.0,
        continuation_to_spread_margin + 0.30 * void_continuation_alignment - spread_void_stress,
    )
    late_session_void_pressure = context["session_progress"] * spread_void_stress
    wick_continuation_fragility = max(0.0, opposing_wick - favorable_wick) + spread_over_range * (1.0 + path_churn)
    adverse_churn_pressure = path_churn * (1.0 + failed_fill_pressure_12 + continuation_void_energy_12) + max(
        0.0, -path_efficiency
    )
    quantized_continuation_margin = math.trunc((void_continuation_alignment - spread_void_stress) * 50.0) / 50.0
    continuation_survival_interaction = (
        (void_continuation_alignment + 0.25 * void_rejection_interaction - spread_void_stress)
        * (0.5 + continuation_displacement_3)
        - 0.25 * wick_continuation_fragility
    )
    features = (
        float(direction),
        directional_ret_1,
        ...
        path_churn,
        day_return,
        bounded(day_void_position, 0.0, 1.0),
        day_mid_pressure,
        context["range_expansion_6_24"],
        context["session_progress"],
        spread_over_range,
...
def _c0047_r0002_final_void_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    flow_context: list[dict[str, float] | None],
    index: int,
    ...
    favorable_body_share = sum(1 for bar in path if direction * (bar.close - bar.open) > 0.0) / len(path)
    opposing_close_share = sum(1 for bar in path if direction * (bar.close - entry) < -0.22 * average_range) / len(path)
    pre_continuation = max(0.0, direction * context["ret_6"])
    failed_fill = max(0.0, -direction * context["ret_3"])
    pre_gap = _c0047_r0002_recent_void_gap(bars, index, -direction, 6, average_range)
    pre_overlap = _c0047_r0002_low_overlap_pressure(bars, index, direction, 6)
    continuation_completion = min(2.5, favorable + max(0.0, terminal_norm) + 0.35 * first_hit)
    void_state_reward = pre_continuation * (0.5 + pre_gap + pre_overlap)
    spread_norm = bars[entry_index].spread_points / average_range
    path_spread_peak = max(bar.spread_points for bar in path) / average_range
    churn = future_path_churn(path, entry)
    spread_spike = max(0.0, path_spread_peak - spread_norm)
    survival_buffer = max(
        0.0,
        early_followthrough + 0.35 * favorable + 0.25 * pre_continuation - spread_norm - 0.50 * path_spread_peak,
    )
    adverse_cost_drag = spread_norm + 0.80 * path_spread_peak + 0.25 * context["session_progress"] * path_spread_peak
    ambiguous_exit_drag = max(0.0, early_adverse + churn - early_followthrough)
    return (
        ...
        - 0.48 * stop_speed
        - 0.56 * adverse_first
        + 0.30 * void_state_reward
        + 0.22 * pre_continuation
        - 0.16 * failed_fill
        + 0.34 * early_followthrough
        - 0.36 * early_adverse
        + 0.28 * favorable
...
build_liquidity_void_reversion_context = _c0047_r0002_final_void_context
intraday_liquidity_void_reversion_features = _c0047_r0002_final_void_features
intraday_liquidity_void_reversion_label = _c0047_r0002_final_void_label
