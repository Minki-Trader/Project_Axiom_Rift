"""C0116 R0001 proxy evidence for intraday day open anchor reclaim."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0116_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0116_r0001_day_open_anchor_reclaim_summary.json"
...
FEATURE_NAMES = (
    "day_open_distance",
    "anchor_reclaim_depth",
    "same_side_reclaim_ratio",
    "failed_auction_absorption",
    "close_reclaim_strength",
    "anchor_impulse_efficiency",
    "volume_reclaim_ratio",
    "vwap_anchor_alignment",
    "opening_range_reclaim_alignment",
    "prior_day_direction_context",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_day_open_anchor_reclaim_rank"
LABEL_SHAPE = "target_first_quality_after_day_open_anchor_reclaim"
SELECTION_RULE = "top_fold_local_day_open_anchor_reclaim_scores_per_active_day"
...
MIN_DAY_BARS = 18
DAY_OPEN_ANCHOR_RECLAIM_LOOKBACK_BARS = 42
DAY_OPEN_ANCHOR_RECLAIM_RECENT_BARS = 10
DAY_OPEN_ANCHOR_RECLAIM_BASE_BARS = 14
MIN_DAY_OPEN_ANCHOR_EXCURSION = 0.18
MIN_DAY_OPEN_ANCHOR_RECLAIM_ACTIVITY = 0.060
...
class LinearDayOpenAnchorReclaimModel:
    fold_id: str
    ...
    day_open_anchor_reclaim_failure_rate: float
...
def run_c0116_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_day_open_anchor_reclaim_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_day_open_anchor_reclaim_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        DAY_OPEN_ANCHOR_RECLAIM_LOOKBACK_BARS,
        DAY_OPEN_ANCHOR_RECLAIM_RECENT_BARS + DAY_OPEN_ANCHOR_RECLAIM_BASE_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = day_open_anchor_reclaim_metrics(
    ...
    features = (
        metrics["day_open_distance"],
        metrics["anchor_reclaim_depth"],
        metrics["same_side_reclaim_ratio"],
        metrics["failed_auction_absorption"],
        metrics["close_reclaim_strength"],
        metrics["anchor_impulse_efficiency"],
        metrics["volume_reclaim_ratio"],
        metrics["vwap_anchor_alignment"],
        metrics["opening_range_reclaim_alignment"],
    ...
    dimensions = (
        day_open_distance_bucket(metrics["day_open_distance"]),
        anchor_reclaim_depth_bucket(metrics["anchor_reclaim_depth"]),
        same_side_reclaim_ratio_bucket(metrics["same_side_reclaim_ratio"]),
        failed_auction_absorption_bucket(metrics["failed_auction_absorption"]),
        close_reclaim_strength_bucket(metrics["close_reclaim_strength"]),
        anchor_impulse_efficiency_bucket(metrics["anchor_impulse_efficiency"]),
        volume_reclaim_ratio_bucket(metrics["volume_reclaim_ratio"]),
        vwap_anchor_alignment_bucket(metrics["vwap_anchor_alignment"]),
        opening_range_reclaim_alignment_bucket(metrics["opening_range_reclaim_alignment"]),
...
def day_open_anchor_reclaim_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    recent_start = max(index - DAY_OPEN_ANCHOR_RECLAIM_RECENT_BARS + 1, context.day_start_index)
    base_start = max(recent_start - DAY_OPEN_ANCHOR_RECLAIM_BASE_BARS, context.day_start_index)
    lookback_start = max(index - DAY_OPEN_ANCHOR_RECLAIM_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    anchor = context.day_open
    anchor_signed_closes = [direction * (bar.close - anchor) for bar in recent_window]
    anchor_signed_extremes = [
        (bar.high - anchor) if direction > 0 else (anchor - bar.low) for bar in recent_window
    ]
    anchor_signed_adverse = [
        (bar.low - anchor) if direction > 0 else (anchor - bar.high) for bar in recent_window
    ]
    day_open_distance = direction * (signal_bar.close - anchor) / average_range
    anchor_reclaim_depth = max(anchor_signed_extremes) / average_range
    if anchor_reclaim_depth < MIN_DAY_OPEN_ANCHOR_EXCURSION:
        return {"event_count": 0.0}
    same_side_reclaim_ratio = sum(1 for value in anchor_signed_closes if value > 0) / len(anchor_signed_closes)
    if day_open_distance < -0.16 and same_side_reclaim_ratio < 0.45:
        return {"event_count": 0.0}

    deepest_retest = min(anchor_signed_adverse) / average_range
    retest_penalty = max(0.0, -deepest_retest)
    failed_auction_absorption = day_open_distance + same_side_reclaim_ratio - 0.52 * retest_penalty
    close_reclaim_strength = 0.60 * directional_bar_settlement(signal_bar, direction) + 0.40 * same_side_reclaim_ratio
    ...
    anchor_impulse_efficiency = recent_net / max(close_travel, average_range * 0.12)
    volume_reclaim_ratio = recent_volume_mean / max(average_volume, 1e-9)
    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    vwap_anchor_alignment = 1.0 - abs(session_vwap - anchor) / max(average_range, 1e-9)
    opening_reference = context.opening_high if direction > 0 else context.opening_low
    opening_range_reclaim_alignment = direction * (signal_bar.close - opening_reference) / average_range
    ...
    activity = max(
        max(day_open_distance, 0.0) * 0.36,
        max(anchor_reclaim_depth - 0.15, 0.0) * 0.32,
        max(same_side_reclaim_ratio - 0.45, 0.0) * 0.44,
        max(failed_auction_absorption - 0.15, 0.0) * 0.28,
        max(close_reclaim_strength - 0.45, 0.0) * 0.26,
        max(anchor_impulse_efficiency, 0.0) * 0.22,
        max(volume_reclaim_ratio - 0.80, 0.0) * 0.08,
        max(vwap_anchor_alignment - 0.20, 0.0) * 0.12,
        max(opening_range_reclaim_alignment, 0.0) * 0.12,
    )
    if activity < MIN_DAY_OPEN_ANCHOR_RECLAIM_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "day_open_distance": bounded(day_open_distance, -4.0, 8.0),
        "anchor_reclaim_depth": bounded(anchor_reclaim_depth, -2.0, 8.0),
        "same_side_reclaim_ratio": bounded(same_side_reclaim_ratio, 0.0, 1.0),
        "failed_auction_absorption": bounded(failed_auction_absorption, -4.0, 8.0),
        "close_reclaim_strength": bounded(close_reclaim_strength, 0.0, 1.0),
        "anchor_impulse_efficiency": bounded(anchor_impulse_efficiency, -2.0, 2.0),
        "volume_reclaim_ratio": bounded(volume_reclaim_ratio, 0.0, 8.0),
        "vwap_anchor_alignment": bounded(vwap_anchor_alignment, -4.0, 1.0),
        "opening_range_reclaim_alignment": bounded(opening_range_reclaim_alignment, -4.0, 8.0),
        ...
        "day_open_anchor_reclaim_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def day_open_distance_bucket(value: float) -> str:
    if value < -0.08:
        return "day_open_not_reclaimed"
    if value < 0.25:
        return "day_open_marginal_reclaim"
    return "day_open_clear_reclaim"


def anchor_reclaim_depth_bucket(value: float) -> str:
    if value < 0.28:
        return "anchor_probe_shallow"
    if value < 0.85:
        return "anchor_probe_defined"
    return "anchor_probe_extended"


def same_side_reclaim_ratio_bucket(value: float) -> str:
    if value < 0.45:
        return "anchor_hold_unstable"
    if value < 0.75:
        return "anchor_hold_mixed"
    return "anchor_hold_persistent"


def failed_auction_absorption_bucket(value: float) -> str:
    if value < 0.10:
        return "failed_auction_absorption_weak"
    if value < 0.72:
        return "failed_auction_absorption_defined"
    return "failed_auction_absorption_strong"


def close_reclaim_strength_bucket(value: float) -> str:
    if value < 0.45:
        return "close_reclaim_soft"
    if value < 0.72:
        return "close_reclaim_tradeable"
    return "close_reclaim_firm"


def anchor_impulse_efficiency_bucket(value: float) -> str:
    if value < -0.05:
        return "anchor_impulse_fading"
    if value < 0.35:
        return "anchor_impulse_mixed"
    return "anchor_impulse_efficient"


def volume_reclaim_ratio_bucket(value: float) -> str:
    if value < 0.78:
        return "volume_reclaim_thin"
    if value < 1.55:
        return "volume_reclaim_orderly"
    return "volume_reclaim_hot"


def vwap_anchor_alignment_bucket(value: float) -> str:
    if value < -0.45:
        return "vwap_anchor_far"
    if value < 0.30:
        return "vwap_anchor_near"
    return "vwap_anchor_tight"


def opening_range_reclaim_alignment_bucket(value: float) -> str:
    if value < -0.10:
...
def c0116_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0116_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0116_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0116_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0116_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0116_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0116_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0116_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0116_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0116_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0116_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0116_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0116_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0116_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0116_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0116_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0116_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0116_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0116_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def c0116_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    anchor_reward = 0.12 if metrics["day_open_distance"] > 0.18 and terminal > 0 else 0.0
    reclaim_reward = 0.12 if metrics["anchor_reclaim_depth"] > 0.35 and terminal > 0 else 0.0
    hold_reward = 0.10 if metrics["same_side_reclaim_ratio"] > 0.62 and terminal > 0 else 0.0
    retest_reward = 0.10 if metrics["failed_auction_absorption"] > 0.25 and terminal > 0 else 0.0
    close_reward = 0.08 if metrics["close_reclaim_strength"] > 0.58 and terminal > 0 else 0.0
    impulse_reward = 0.08 if metrics["anchor_impulse_efficiency"] > 0.18 and terminal > 0 else 0.0
    volume_reward = 0.06 if 0.85 <= metrics["volume_reclaim_ratio"] <= 1.90 and terminal > 0 else 0.0
    vwap_reward = 0.06 if metrics["vwap_anchor_alignment"] > 0.20 and terminal > 0 else 0.0
    opening_reward = 0.06 if metrics["opening_range_reclaim_alignment"] > 0.12 and terminal > 0 else 0.0
    weak_anchor_penalty = 0.10 if metrics["day_open_distance"] < 0.02 and terminal <= 0 else 0.0
    weak_hold_penalty = 0.10 if metrics["same_side_reclaim_ratio"] < 0.45 and terminal <= 0 else 0.0
    weak_retest_penalty = 0.08 if metrics["failed_auction_absorption"] < 0.00 and terminal <= 0 else 0.0
    weak_close_penalty = 0.07 if metrics["close_reclaim_strength"] < 0.42 and terminal <= 0 else 0.0
    weak_impulse_penalty = 0.07 if metrics["anchor_impulse_efficiency"] < -0.05 and terminal <= 0 else 0.0
    overheated_volume_penalty = 0.06 if metrics["volume_reclaim_ratio"] > 2.60 and terminal <= 0 else 0.0
    ...
    day_open_anchor_reclaim_path_quality = (0.62 * terminal + 0.28 * mfe - 0.65 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + day_open_anchor_reclaim_path_quality
        + anchor_reward
        + reclaim_reward
        + hold_reward
        + retest_reward
        ...
        - max(0.0, 0.38 - metrics["same_side_reclaim_ratio"]) * 0.12
    )


def fit_linear_day_open_anchor_reclaim_model(candidates: list[base.Candidate], fold_id: str) -> LinearDayOpenAnchorReclaimModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearDayOpenAnchorReclaimModel(
            fold_id,
    ...
    day_open_anchor_reclaim_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearDayOpenAnchorReclaimModel(
        fold_id=fold_id,
        ...
        day_open_anchor_reclaim_failure_rate=day_open_anchor_reclaim_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearDayOpenAnchorReclaimModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_day_open_anchor_reclaim_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0116_r0001_intraday_day_open_anchor_reclaim",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0116_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0116_r0001_day_open_anchor_reclaim_summary.json",
        ],
        ...
        "conditional_profiles": {
            "day_open_anchor_reclaim_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0116 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0116_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0116 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "day_open_anchor_reclaim_lookback_bars": DAY_OPEN_ANCHOR_RECLAIM_LOOKBACK_BARS,
        "day_open_anchor_reclaim_recent_bars": DAY_OPEN_ANCHOR_RECLAIM_RECENT_BARS,
        "day_open_anchor_reclaim_base_bars": DAY_OPEN_ANCHOR_RECLAIM_BASE_BARS,
        "min_day_open_anchor_excursion": MIN_DAY_OPEN_ANCHOR_EXCURSION,
        "min_day_open_anchor_reclaim_activity": MIN_DAY_OPEN_ANCHOR_RECLAIM_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_day_open_anchor_reclaim_model_summary(model: LinearDayOpenAnchorReclaimModel) -> dict[str, object]:
    return {
        ...
        "day_open_anchor_reclaim_failure_rate": rounded(model.day_open_anchor_reclaim_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearDayOpenAnchorReclaimModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_day_open_anchor_reclaim_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "day_open_anchor_reclaim_profile": profiles["day_open_anchor_reclaim_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0116_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0116_r0001_proxy_trades.csv"
    evidence["day_open_anchor_reclaim_summary"] = "artifacts/c0116_r0001_day_open_anchor_reclaim_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0116_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0116_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0116 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0116_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0116_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0116_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0116_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0116_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0116_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0116_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0116_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0116_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0116 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0116 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0116_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0116_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0116_r0001_intraday_day_open_anchor_reclaim_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0116_r0001_proxy_records_day_open_anchor_reclaim_reference_surface
  ...
  - next_work_is_produce_c0116_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0116_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0116-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0116_r0001_intraday_day_open_anchor_reclaim",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0116-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0116-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0116-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0116_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0116_r0001_intraday_day_open_anchor_reclaim",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0116-R0001-DAY-OPEN-ANCHOR-RECLAIM-SUMMARY",
            "artifact_role": "day_open_anchor_reclaim_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0116_r0001_day_open_anchor_reclaim_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0116_r0001_intraday_day_open_anchor_reclaim",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0116_r0001_mt5_logic_parity_evidence",
        }
    ]
