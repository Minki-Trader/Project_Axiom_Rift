"""C0099 R0001 proxy evidence for intraday failed pullback continuation."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0099_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0099_r0001_failed_pullback_continuation_summary.json"
...
FEATURE_NAMES = (
    "impulse_range_pressure",
    "impulse_efficiency",
    "shallow_pullback_strength",
    "pullback_compression",
    "trend_retention_ratio",
...
MODEL_FAMILY = "fold_local_intraday_failed_pullback_continuation_rank"
LABEL_SHAPE = "target_first_continuation_quality_conditioned_on_failed_pullback_after_directional_expansion"
SELECTION_RULE = "top_fold_local_failed_pullback_continuation_scores_per_active_day"
...
MIN_DAY_BARS = 30
FAILED_PULLBACK_CONTINUATION_LOOKBACK_BARS = 28
FAILED_PULLBACK_CONTINUATION_IMPULSE_BARS = 14
FAILED_PULLBACK_CONTINUATION_PULLBACK_BARS = 6
MIN_FAILED_PULLBACK_CONTINUATION_ACTIVITY = 0.10
...
class LinearFailedPullbackContinuationModel:
    fold_id: str
    ...
    failed_pullback_continuation_failure_rate: float
...
def run_c0099_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_failed_pullback_continuation_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_failed_pullback_continuation_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        FAILED_PULLBACK_CONTINUATION_LOOKBACK_BARS,
        FAILED_PULLBACK_CONTINUATION_IMPULSE_BARS + FAILED_PULLBACK_CONTINUATION_PULLBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = failed_pullback_continuation_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        metrics["impulse_range_pressure"],
        metrics["impulse_efficiency"],
        metrics["shallow_pullback_strength"],
        metrics["pullback_compression"],
        metrics["trend_retention_ratio"],
        metrics["adverse_overlap_control"],
        metrics["participation_reacceleration"],
        metrics["vwap_hold_pressure"],
    ...
    dimensions = (
        impulse_range_bucket(metrics["impulse_range_pressure"]),
        impulse_efficiency_bucket(metrics["impulse_efficiency"]),
        shallow_pullback_bucket(metrics["shallow_pullback_strength"]),
        digestion_compression_bucket(metrics["pullback_compression"]),
        retention_bucket(metrics["trend_retention_ratio"]),
        adverse_overlap_bucket(metrics["adverse_overlap_control"]),
        participation_followthrough_bucket(metrics["participation_reacceleration"]),
        vwap_hold_bucket(metrics["vwap_hold_pressure"]),
...
def failed_pullback_continuation_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    pullback_start = index - FAILED_PULLBACK_CONTINUATION_PULLBACK_BARS
    impulse_start = pullback_start - FAILED_PULLBACK_CONTINUATION_IMPULSE_BARS
    lookback_start = index - FAILED_PULLBACK_CONTINUATION_LOOKBACK_BARS
    if pullback_start < 0 or impulse_start < 0 or lookback_start < 0 or context.day_start_index >= index:
        return {"event_count": 0.0}
    impulse_start = max(impulse_start, context.day_start_index)
    pullback_start = max(pullback_start, context.day_start_index)
    lookback_start = max(lookback_start, context.day_start_index)
    impulse_window = bars[impulse_start:pullback_start]
    pullback_window = bars[pullback_start:index]
    prior_window = bars[lookback_start:impulse_start]
    if len(impulse_window) < 6 or len(pullback_window) < 3 or len(prior_window) < 5:
        return {"event_count": 0.0}
    ...
    impulse_closes = [bar.close for bar in impulse_window]
    impulse_move = impulse_closes[-1] - impulse_window[0].open
    directional_impulse = direction * impulse_move
    impulse_travel = abs(impulse_closes[0] - impulse_window[0].open) + sum(
        abs(impulse_closes[pos] - impulse_closes[pos - 1]) for pos in range(1, len(impulse_closes))
    )
    ...
    if direction > 0:
        pullback_depth = max(0.0, impulse_high - min(bar.low for bar in pullback_window + [signal_bar]))
        retained_move = signal_bar.close - impulse_window[0].open
    else:
        pullback_depth = max(0.0, max(bar.high for bar in pullback_window + [signal_bar]) - impulse_low)
        retained_move = impulse_window[0].open - signal_bar.close
    impulse_range_pressure = directional_impulse / max(average_range, 1e-9)
    impulse_efficiency = directional_impulse / max(impulse_travel, average_range * 0.25)
    shallow_pullback_strength = 1.0 - pullback_depth / max(directional_impulse, average_range * 0.65)
    pullback_compression = 1.0 - pullback_range_mean / max(impulse_range_mean, average_range * 0.35)
    trend_retention_ratio = retained_move / max(directional_impulse, average_range * 0.35)
    adverse_overlap_control = 1.0 - pullback_depth / max(average_range * 1.35, directional_impulse)
    pullback_volume_average = base.mean(volumes[pullback_start:index]) or average_volume
    participation_reacceleration = volumes[index] / max(pullback_volume_average, 1e-9) - 1.0
    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    vwap_hold_pressure = direction * (signal_bar.close - session_vwap) / max(average_range, 1e-9)
    ...
    failed_pullback_continuation_pressure_raw = max(
        max(impulse_range_pressure - 0.35, 0.0) * 0.55,
        max(impulse_efficiency - 0.30, 0.0) * 0.70,
        max(shallow_pullback_strength - 0.18, 0.0) * 0.80,
        max(pullback_compression, 0.0) * 0.45,
        max(trend_retention_ratio - 0.38, 0.0) * 0.65,
        max(adverse_overlap_control - 0.12, 0.0) * 0.40,
        max(participation_reacceleration, 0.0) * 0.16,
        max(vwap_hold_pressure + 0.05, 0.0) * 0.22,
    )
    activity = max(
        failed_pullback_continuation_pressure_raw,
        failed_pullback_continuation_pressure_raw * session_phase_maturity,
    )
    if activity < MIN_FAILED_PULLBACK_CONTINUATION_ACTIVITY:
        return {"event_count": 0.0}
    if impulse_range_pressure < 0.28 or impulse_efficiency < 0.18:
        return {"event_count": 0.0}
    if shallow_pullback_strength < -0.15 or trend_retention_ratio < 0.10:
        return {"event_count": 0.0}
    if vwap_hold_pressure < -1.20 and adverse_overlap_control < -0.10:
        return {"event_count": 0.0}
    return {
        "impulse_range_pressure": bounded(impulse_range_pressure, 0.0, 8.0),
        "impulse_efficiency": bounded(impulse_efficiency, 0.0, 1.5),
        "shallow_pullback_strength": bounded(shallow_pullback_strength, -3.0, 1.5),
        "pullback_compression": bounded(pullback_compression, -3.0, 1.5),
        "trend_retention_ratio": bounded(trend_retention_ratio, -3.0, 3.0),
        "adverse_overlap_control": bounded(adverse_overlap_control, -3.0, 1.5),
        "participation_reacceleration": bounded(participation_reacceleration, -3.0, 6.0),
        "vwap_hold_pressure": bounded(vwap_hold_pressure, -5.0, 5.0),
        "session_phase_maturity": bounded(session_phase_maturity, 0.0, 1.0),
        "prior_impulse_context": bounded(prior_impulse_context, -3.0, 3.0),
        "impulse_range_ratio": bounded(impulse_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        "pullback_range_ratio": bounded(pullback_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        "prior_range_ratio": bounded(prior_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        "failed_pullback_continuation_pressure": bounded(failed_pullback_continuation_pressure_raw, 0.0, 5.0),
        "event_count": 1.0,
    }
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    impulse_reward = 0.12 if metrics["impulse_range_pressure"] > 0.85 and terminal > 0 else 0.0
    efficiency_reward = 0.12 if metrics["impulse_efficiency"] > 0.42 and terminal > 0 else 0.0
    shallow_pullback_reward = 0.13 if metrics["shallow_pullback_strength"] > 0.38 and terminal > 0 else 0.0
    compression_reward = 0.08 if metrics["pullback_compression"] > 0.10 and terminal > 0 else 0.0
    retention_reward = 0.13 if metrics["trend_retention_ratio"] > 0.52 and terminal > 0 else 0.0
    overlap_reward = 0.08 if metrics["adverse_overlap_control"] > 0.18 and terminal > 0 else 0.0
    volume_reward = 0.06 if metrics["participation_reacceleration"] > 0.08 and terminal > 0 else 0.0
    vwap_reward = 0.08 if metrics["vwap_hold_pressure"] > -0.05 and terminal > 0 else 0.0
    deep_pullback_penalty = 0.16 if metrics["shallow_pullback_strength"] < 0.05 and terminal <= 0 else 0.0
    failed_retention_penalty = 0.14 if metrics["trend_retention_ratio"] < 0.30 and terminal <= 0 else 0.0
    weak_impulse_penalty = 0.09 if metrics["impulse_efficiency"] < 0.25 and terminal <= 0 else 0.0
    weak_volume_penalty = 0.06 if metrics["participation_reacceleration"] < -0.25 and terminal <= 0 else 0.0
    vwap_loss_penalty = 0.10 if metrics["vwap_hold_pressure"] < -0.55 and terminal <= 0 else 0.0
    ...
    failed_pullback_continuation_path_quality = (0.54 * terminal + 0.30 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + failed_pullback_continuation_path_quality
        + impulse_reward
        + efficiency_reward
        ...
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["pullback_range_ratio"] - 1.65) * 0.05
    )


def fit_linear_failed_pullback_continuation_model(candidates: list[base.Candidate], fold_id: str) -> LinearFailedPullbackContinuationModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearFailedPullbackContinuationModel(
            fold_id,
    ...
    failed_pullback_continuation_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearFailedPullbackContinuationModel(
        fold_id=fold_id,
        ...
        failed_pullback_continuation_failure_rate=failed_pullback_continuation_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearFailedPullbackContinuationModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_failed_pullback_continuation_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0099_r0001_intraday_failed_pullback_continuation",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0099_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0099_r0001_failed_pullback_continuation_summary.json",
        ],
        ...
        "conditional_profiles": {
            "failed_pullback_continuation_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0099 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0099_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0099 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "failed_pullback_continuation_lookback_bars": FAILED_PULLBACK_CONTINUATION_LOOKBACK_BARS,
        "failed_pullback_continuation_impulse_bars": FAILED_PULLBACK_CONTINUATION_IMPULSE_BARS,
        "failed_pullback_continuation_pullback_bars": FAILED_PULLBACK_CONTINUATION_PULLBACK_BARS,
        "min_failed_pullback_continuation_activity": MIN_FAILED_PULLBACK_CONTINUATION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_failed_pullback_continuation_model_summary(model: LinearFailedPullbackContinuationModel) -> dict[str, object]:
    return {
        ...
        "failed_pullback_continuation_failure_rate": rounded(model.failed_pullback_continuation_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearFailedPullbackContinuationModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_failed_pullback_continuation_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "failed_pullback_continuation_profile": profiles["failed_pullback_continuation_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0099_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0099_r0001_proxy_trades.csv"
    evidence["failed_pullback_continuation_summary"] = "artifacts/c0099_r0001_failed_pullback_continuation_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0099_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0099_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0099 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0099_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0099_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0099_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0099_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0099_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0099_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0099_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0099_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0099_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0099 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0099 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0099_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0099_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0099_r0001_intraday_failed_pullback_continuation_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0099_r0001_proxy_records_failed_pullback_continuation_reference_surface
  ...
  - next_work_is_produce_c0099_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0099_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0099-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0099_r0001_intraday_failed_pullback_continuation",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0099-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0099-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0099-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0099_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0099_r0001_intraday_failed_pullback_continuation",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0099-R0001-failed-pullback-continuation-SUMMARY",
            "artifact_role": "failed_pullback_continuation_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0099_r0001_failed_pullback_continuation_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0099_r0001_intraday_failed_pullback_continuation",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0099_r0001_mt5_logic_parity_evidence",
        }
    ]
