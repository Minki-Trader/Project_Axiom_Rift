"""R0007 proxy evidence for core-session expansion continuation."""
...
BREAKOUT_BARS = 24
EXPANSION_RANGE_MULTIPLE = 1.8
MIN_BODY_RANGE_FRACTION = 0.55
PULLBACK_SEARCH_BARS = 6
PULLBACK_MIN_RATIO = 0.20
PULLBACK_MAX_RATIO = 0.65
...
def run_r0007_proxy(write: bool = True) -> dict[str, object]:
...
def simulate_trades(bars: list[Bar], windows: list[Window]) -> list[Trade]:
    ...
    previous_high, previous_low = previous_extrema(bars, BREAKOUT_BARS)
    trades: list[Trade] = []
    state = 0
    direction = 0
    ...
    for window in windows:
        ...
        index = max(start_index, LOOKBACK_RANGE_BARS, BREAKOUT_BARS)
        while index <= end_index:
            if index > start_index:
                if active is not None:
                    active_bars_held += 1
                    if active_bars_held >= MAX_HOLD_BARS:
                ...
                if active is None:
                    ...
                    if state == 1:
                        if not in_core_session(bars[closed_index].time):
                            state = 0
                            direction = 0
                            index += 1
                            continue
                        bars_since_expansion += 1
                        if stateful_pullback_confirmed(
                            bars[closed_index],
                            direction,
                            expansion_open,
                            expansion_close,
                            ...
                            if in_core_session(bars[index].time):
                                pending_direction = direction
                                pending_average_range = expansion_average_range
                            state = 0
                        elif bars_since_expansion >= PULLBACK_SEARCH_BARS:
                            state = 0
                            index += 1
                            continue
                        else:
                        ...
                        if in_core_session(bars[closed_index].time):
                            next_direction = expansion_direction(
                                bars[closed_index],
                                ranges[closed_index],
                                range_average[closed_index],
                                previous_high[closed_index],
                                previous_low[closed_index],
                            )
                            if next_direction != 0:
                                closed_bar = bars[closed_index]
                                state = 1
                                direction = next_direction
                                bars_since_expansion = 0
                                expansion_open = closed_bar.open
                                expansion_close = closed_bar.close
                                expansion_range = max(
                                    abs(closed_bar.close - closed_bar.open),
                                    closed_bar.high - closed_bar.low,
                                    range_average[closed_index] or 0.0,
                                )
                                expansion_average_range = range_average[closed_index] or 0.0
...
def stateful_pullback_confirmed(
    bar: Bar,
    direction: int,
    ...
    if expansion_range <= 0:
        return False
    if direction > 0:
        retrace = expansion_close - bar.low
        still_valid = bar.close > expansion_open
    else:
        retrace = bar.high - expansion_close
        still_valid = bar.close < expansion_open
    ratio = retrace / expansion_range
    return still_valid and PULLBACK_MIN_RATIO <= ratio <= PULLBACK_MAX_RATIO
...
def open_active_trade(
    ...
    stop_price = round(entry_price - direction * STOP_ATR_MULTIPLE * average_range, PRICE_DIGITS)
    target_price = round(entry_price + direction * TARGET_ATR_MULTIPLE * average_range, PRICE_DIGITS)
    return ActiveTrade(
        fold_id=fold_id,
...
def expansion_direction(
    ...
    if average_range is None or average_range <= 0 or previous_high is None or previous_low is None:
        return 0
    body = bar.close - bar.open
    if range_points < EXPANSION_RANGE_MULTIPLE * average_range:
        return 0
    if abs(body) < MIN_BODY_RANGE_FRACTION * range_points:
        return 0
    if body > 0 and bar.close > previous_high:
        return 1
    if body < 0 and bar.close < previous_low:
        return -1
    return 0


def find_pullback_index(
    bars: list[Bar],
    expansion_index: int,
    direction: int,
    average_range: float | None,
    ...
    body_points = abs(expansion.close - expansion.open)
    anchor_range = max(body_points, expansion.high - expansion.low, average_range or 0.0)
    if anchor_range <= 0:
        return None
    limit = min(expansion_index + PULLBACK_SEARCH_BARS, end_index - MAX_HOLD_BARS)
    for index in range(expansion_index + 1, limit + 1):
        bar = bars[index]
        if direction == 1:
            retrace = expansion.close - bar.low
            still_valid = bar.close > expansion.open
        else:
            retrace = bar.high - expansion.close
            still_valid = bar.close < expansion.open
        ratio = retrace / anchor_range
        if PULLBACK_MIN_RATIO <= ratio <= PULLBACK_MAX_RATIO and still_valid:
            return index
    return None
...
def build_proxy_payload(trades: list[Trade], windows: list[Window]) -> dict[str, object]:
    ...
    return {
        ...
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.r0007_core_session_expansion_continuation",
        "proxy_config_path": "campaigns/C0001_regime_response_discovery/runs/R0007/run_manifest.json",
        ...
        "deferred_with_reason": [
            {
                ...
                "reason": "R0007 proxy currently measures raw US100 index points only; USD conversion requires MT5 symbol contract, tick value, and fixed-lot discovery sizing",
                "blocking_condition": "record the matching MT5 symbol specification and fixed-lot sizing for the 500 USD account",
                "revisit_when": "during R0007 MT5 probe evidence production",
            ...
            },
            {
                ...
                "revisit_when": "before R0007 closeout",
                "claim_boundary": {"claim_authority": False},
            }
        ],
        "claim_boundary": {
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "context_gate": "core_session",
        "session_filter_applies": True,
        "session_start_minute": CORE_SESSION_START_MINUTE,
        "session_end_minute": CORE_SESSION_END_MINUTE,
...
def update_artifact_lineage(proxy_hash: str) -> None:
    ...
    for record in data.get("artifact_records", []):
        if record.get("artifact_id") == "A0001":
            ...
            record["produced_by"] = "axiom_rift.proxies.r0007_core_session_expansion_continuation"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_matching_mt5_probe_for_r0007"
    data["deferred_with_reason"] = [
        {
            "field": "decision",
            "reason": "proxy evidence is recorded but MT5 and proxy-vs-MT5 evidence are still missing",
            "blocking_condition": "R0007 cannot close until the mandatory MT5 probe and proxy-vs-MT5 comparison are recorded",
            "revisit_when": "after R0007 MT5 and proxy-vs-MT5 evidence files contain measured results",
        }
    ]
