"""C0033 R0001 proxy evidence for intraday seasonal residual dislocation discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0033_intraday_seasonal_residual_dislocation_discovery" / "runs" / "R0001"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0033_intraday_seasonal_residual_dislocation_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0033_r0001_proxy_trades.csv"
INTRADAY_SEASONAL_RESIDUAL_DISLOCATION_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0033_r0001_seasonal_residual_summary.json"
...
FEATURE_NAMES = (
    "direction_bias",
    "seasonal_return_residual_1",
    "seasonal_return_residual_3",
    "seasonal_return_residual_12",
    "seasonal_range_residual_1",
    "seasonal_range_residual_3",
    "seasonal_range_residual_12",
    "seasonal_volume_residual_1",
    "seasonal_volume_residual_3",
    "seasonal_volume_residual_12",
    "seasonal_spread_residual_1",
    "seasonal_spread_relief_3",
    "seasonal_cum_range_residual",
    "seasonal_cum_volume_residual",
    "seasonal_day_return_residual",
    "seasonal_volatility_residual_12",
    "seasonal_activity_dislocation",
    "seasonal_quiet_dislocation",
    "directional_residual_alignment_3",
    "directional_residual_alignment_12",
    "residual_momentum_shift_3_vs_12",
    "seasonal_reversion_pressure_6",
    "seasonal_breakout_pressure_6",
    "range_volume_residual_agreement",
    "spread_volume_residual_stress",
    "session_progress",
...
MODEL_FAMILY = "fold_local_intraday_seasonal_residual_dislocation_rank"
LABEL_SHAPE = "directional_intraday_seasonal_residual_dislocation_survival_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_intraday_seasonal_residual_dislocation_scores_per_active_day"
...
def run_c0033_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    seasonal_context = build_seasonal_context(bars, volumes, range_average, short_range_average)
    trades: list[base.Trade] = []
...
def build_candidates(
    ...
    volumes: list[float],
    seasonal_context: list[dict[str, float] | None],
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = intraday_seasonal_residual_dislocation_features(
                bars,
                range_average,
                short_range_average,
                day_context,
            ...
            if features is None:
                continue
            label = (
                intraday_seasonal_residual_dislocation_label(bars, range_average, seasonal_context, index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|intraday_seasonal_residual_dislocation",
                    features=features,
                    label=label,
                )
            )
...
def build_seasonal_context(
    ...
    contexts: list[dict[str, float] | None] = [None] * len(bars)
    range_history_by_minute: dict[int, list[float]] = {}
    volume_history_by_minute: dict[int, list[float]] = {}
    spread_history_by_minute: dict[int, list[float]] = {}
    abs_return_history_by_minute: dict[int, list[float]] = {}
    cumulative_range_history_by_minute: dict[int, list[float]] = {}
    cumulative_volume_history_by_minute: dict[int, list[float]] = {}
    day_return_history_by_minute: dict[int, list[float]] = {}
    abs_day_return_history_by_minute: dict[int, list[float]] = {}
    current_day: str | None = None
    day_open = 0.0
    day_volume = 0.0
    cumulative_range = 0.0
    ...
    for index, bar in enumerate(bars):
        day = bar.time.strftime("%Y-%m-%d")
        bar_range = max(bar.high - bar.low, 0.0)
        close_delta = bar.close - bars[index - 1].close if index > 0 else 0.0
        if current_day != day:
            current_day = day
            day_open = bar.open
            day_volume = 0.0
        ...
        average_range = range_average[index]
        short_average_range = short_range_average[index]
        minute = minute_key(bar.time)
        fallback_range = average_range or prior_bar_range_mean(bars, index, 24, max(bar_range, 1e-9))
        fallback_volume = prior_scalar_mean(volumes, index, 24, max(volumes[index], 1.0))
        fallback_spread = prior_spread_mean(bars, index, 24, max(bar.spread_points, 1e-9))
        fallback_abs_return = prior_abs_close_delta_mean(bars, index, 24, max(fallback_range, 1e-9))
        expected_range = prior_mean(range_history_by_minute.get(minute, []), fallback_range)
        expected_volume = prior_mean(volume_history_by_minute.get(minute, []), fallback_volume)
        expected_spread = prior_mean(spread_history_by_minute.get(minute, []), fallback_spread)
        expected_abs_return = prior_mean(abs_return_history_by_minute.get(minute, []), fallback_abs_return)
        expected_cumulative_range = prior_mean(
            cumulative_range_history_by_minute.get(minute, []),
            max(fallback_range * max(bars_since_day_open + 1, 1), cumulative_range),
        )
        expected_cumulative_volume = prior_mean(
            cumulative_volume_history_by_minute.get(minute, []),
            max(fallback_volume * max(bars_since_day_open + 1, 1), day_volume),
        )
        day_return = bar.close - day_open
        expected_day_return = prior_mean(day_return_history_by_minute.get(minute, []), 0.0)
        expected_abs_day_return = prior_mean(
            abs_day_return_history_by_minute.get(minute, []),
            max(fallback_abs_return * math.sqrt(max(bars_since_day_open + 1, 1)), fallback_range),
        )
        if index >= 48 and average_range is not None and average_range > 0 and short_average_range is not None:
            ...
            recent_ranges = [
                max(bars[offset].high - bars[offset].low, 0.0) / max(range_average[offset] or average_range, 1e-9)
                for offset in range(index - 11, index + 1)
            ]
            realized_12 = realized_return_volatility(bars, range_average, index, 12, average_range)
            realized_36 = realized_return_volatility(bars, range_average, index, 36, average_range)
            return_residual = close_delta / max(expected_abs_return, 1e-9)
            range_residual = residual_ratio(bar_range, expected_range)
            volume_residual = residual_ratio(volumes[index], expected_volume)
            spread_residual = residual_ratio(bar.spread_points, expected_spread)
            return_abs_residual = residual_ratio(abs(close_delta), expected_abs_return)
            activity_dislocation = bounded(
                0.36 * max(range_residual, 0.0)
                + 0.34 * max(volume_residual, 0.0)
                + 0.20 * max(return_abs_residual, 0.0)
                - 0.10 * max(spread_residual, 0.0),
                -2.0,
                4.0,
            )
            quiet_dislocation = bounded(
                0.42 * max(-range_residual, 0.0)
                + 0.38 * max(-volume_residual, 0.0)
                + 0.20 * max(-return_abs_residual, 0.0),
                0.0,
                4.0,
            )
            contexts[index] = {
                "return_residual": bounded(return_residual, -5.0, 5.0),
                "return_abs_residual": return_abs_residual,
                "range_residual": range_residual,
                "volume_residual": volume_residual,
                "spread_residual": spread_residual,
                "cum_range_residual": residual_ratio(cumulative_range, expected_cumulative_range),
                "cum_volume_residual": residual_ratio(day_volume, expected_cumulative_volume),
                "day_return_residual": bounded(
                    (day_return - expected_day_return) / max(expected_abs_day_return, 1e-9),
                    -5.0,
                    5.0,
                ),
                ...
                "realized_vol_12_atr": realized_12,
                "realized_vol_36_atr": realized_36,
                "vol_of_vol_12": volatility_of_volatility(recent_ranges),
                "seasonal_activity_dislocation": activity_dislocation,
                "seasonal_quiet_dislocation": quiet_dislocation,
                "session_progress": bounded(float(bars_since_day_open) / 288.0, 0.0, 1.0),
            }
        append_limited(range_history_by_minute.setdefault(minute, []), bar_range)
        append_limited(volume_history_by_minute.setdefault(minute, []), volumes[index])
        append_limited(spread_history_by_minute.setdefault(minute, []), bar.spread_points)
        append_limited(abs_return_history_by_minute.setdefault(minute, []), abs(close_delta))
        append_limited(cumulative_range_history_by_minute.setdefault(minute, []), cumulative_range)
        append_limited(cumulative_volume_history_by_minute.setdefault(minute, []), day_volume)
        append_limited(day_return_history_by_minute.setdefault(minute, []), day_return)
        append_limited(abs_day_return_history_by_minute.setdefault(minute, []), abs(day_return))
    return contexts
...
def residual_ratio(value: float, expected: float) -> float:
    return bounded((value - expected) / max(abs(expected), 1e-9), -4.0, 4.0)


def context_mean(contexts: list[dict[str, float] | None], index: int, field: str, lookback: int) -> float:
    values: list[float] = []
...
def residual_agreement(left: float, right: float) -> float:
    if left == 0.0 or right == 0.0:
        return 0.0
    sign = 1.0 if left * right > 0.0 else -1.0
    return sign * min(abs(left), abs(right))


def directional_residual_alignment(
    ...
    return bounded(
        direction
        * (
            context_mean(contexts, index, "return_residual", lookback)
            + 0.25 * context_mean(contexts, index, "day_return_residual", lookback)
        ),
        -5.0,
        5.0,
    )
...
def seasonal_reversion_pressure(contexts: list[dict[str, float] | None], index: int, direction: int, lookback: int) -> float:
    context = contexts[index]
    if context is None:
        return 0.0
    adverse_residual = max(0.0, -directional_residual_alignment(contexts, index, direction, lookback))
    excess_activity = max(0.0, context["range_residual"]) + 0.5 * max(0.0, context["volume_residual"])
    return bounded(adverse_residual * (1.0 + excess_activity), 0.0, 6.0)


def seasonal_breakout_pressure(contexts: list[dict[str, float] | None], index: int, direction: int, lookback: int) -> float:
    context = contexts[index]
    if context is None:
        return 0.0
    aligned_residual = max(0.0, directional_residual_alignment(contexts, index, direction, lookback))
    excess_activity = max(0.0, context["range_residual"]) + 0.5 * max(0.0, context["volume_residual"])
    spread_drag = max(0.0, context["spread_residual"])
    return bounded(aligned_residual * (1.0 + excess_activity) - 0.25 * spread_drag, -2.0, 6.0)
...
def intraday_seasonal_residual_dislocation_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    seasonal = seasonal_context[index]
    if context is None or seasonal is None:
        return None
    bar = bars[index]
    spread_over_range = bar.spread_points / average_range
    if spread_over_range > 0.65:
        return None
    return_residual_3 = context_mean(seasonal_context, index, "return_residual", 3)
    return_residual_12 = context_mean(seasonal_context, index, "return_residual", 12)
    range_residual_3 = context_mean(seasonal_context, index, "range_residual", 3)
    range_residual_12 = context_mean(seasonal_context, index, "range_residual", 12)
    volume_residual_3 = context_mean(seasonal_context, index, "volume_residual", 3)
    volume_residual_12 = context_mean(seasonal_context, index, "volume_residual", 12)
    residual_alignment_3 = directional_residual_alignment(seasonal_context, index, direction, 3)
    residual_alignment_12 = directional_residual_alignment(seasonal_context, index, direction, 12)
    close_streak = directional_close_streak(bars, index, direction, 6)
    return (
        float(direction),
        direction * seasonal["return_residual"],
        direction * return_residual_3,
        direction * return_residual_12,
        seasonal["range_residual"],
        range_residual_3,
        range_residual_12,
        seasonal["volume_residual"],
        volume_residual_3,
        volume_residual_12,
        seasonal["spread_residual"],
        seasonal["spread_relief_3_vs_24"],
        seasonal["cum_range_residual"],
        seasonal["cum_volume_residual"],
        direction * seasonal["day_return_residual"],
        seasonal["realized_vol_12_atr"] - seasonal["realized_vol_36_atr"],
        seasonal["seasonal_activity_dislocation"],
        seasonal["seasonal_quiet_dislocation"],
        residual_alignment_3,
        residual_alignment_12,
        residual_alignment_3 - residual_alignment_12,
        seasonal_reversion_pressure(seasonal_context, index, direction, 6),
        seasonal_breakout_pressure(seasonal_context, index, direction, 6),
        residual_agreement(seasonal["range_residual"], seasonal["volume_residual"]),
        max(0.0, seasonal["spread_residual"]) * max(0.0, seasonal["volume_residual"]),
        seasonal["session_progress"],
        ...
        seasonal["range_expansion_3_vs_24"],
        seasonal["range_compression_6_vs_48"],
        spread_over_range,
    )
...
def intraday_seasonal_residual_dislocation_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    seasonal_context: list[dict[str, float] | None],
    index: int,
    ...
    future_residual_quality = future_directional_residual_quality(seasonal_context, entry_index, exit_index, direction)
    future_residual_retention = future_seasonal_residual_retention(seasonal_context, entry_index, exit_index, direction)
    ...
    return (
        ...
        + 0.30 * future_residual_quality
        + 0.24 * future_directional_bias
        + 0.20 * future_residual_retention
...
def future_seasonal_residual_retention(
    seasonal_context: list[dict[str, float] | None],
    entry_index: int,
    exit_index: int,
    direction: int,
    ...
    values: list[float] = []
    for offset in range(entry_index, exit_index + 1):
        context = seasonal_context[offset]
        if context is not None:
            values.append(
                0.70 * direction * context["return_residual"]
                + 0.20 * direction * context["day_return_residual"]
                + 0.10 * residual_agreement(context["range_residual"], context["volume_residual"])
            )
    return sum(values) / max(len(values), 1)


def future_directional_residual_quality(
    seasonal_context: list[dict[str, float] | None],
    entry_index: int,
    exit_index: int,
    direction: int,
    ...
    for offset in range(entry_index, exit_index + 1):
        ...
        if context is None:
            continue
        aligned = direction * context["return_residual"]
        day_alignment = direction * context["day_return_residual"]
        activity = max(0.0, context["range_residual"]) + 0.5 * max(0.0, context["volume_residual"])
        spread_drag = max(0.0, context["spread_residual"])
        values.append(aligned + 0.25 * day_alignment + 0.10 * activity - 0.14 * spread_drag)
    if not values:
        return 0.0
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0033"
    payload["work_unit_id"] = "C0033"
    payload["run_id"] = "R0001"
    payload["proxy_id"] = "PX-C0033-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0033_r0001_intraday_seasonal_residual_dislocation"
    payload["proxy_config_path"] = "campaigns/C0033_intraday_seasonal_residual_dislocation_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_intraday_seasonal_residual_dislocation_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0033_intraday_seasonal_residual_dislocation_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0033_intraday_seasonal_residual_dislocation_discovery/runs/R0001/artifacts/c0033_r0001_proxy_trades.csv",
        "campaigns/C0033_intraday_seasonal_residual_dislocation_discovery/runs/R0001/artifacts/c0033_r0001_seasonal_residual_summary.json",
    ]
    ...
    profiles["intraday_seasonal_residual_dislocation_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0033_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "feature_weight_floor": FEATURE_WEIGHT_FLOOR,
            "feature_weight_ceiling": FEATURE_WEIGHT_CEILING,
            "score_interpretation": "higher_score_means_direct_fold_local_intraday_seasonal_residual_dislocation_rank",
            "variant_boundary": (
                "intraday_seasonal_residual_dislocation_rank_not_vwap_source_synthesis_range_overlap_calendar_phase_tick_participation_"
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_intraday_seasonal_residual_dislocation_summary_artifact(payload, INTRADAY_SEASONAL_RESIDUAL_DISLOCATION_SUMMARY_PATH)
    write_text_local(PROXY_PATH, json.dumps(payload, indent=2, sort_keys=True) + "\n")
    trade_hash = sha256_file_local(TRADE_ARTIFACT_PATH)
    summary_hash = sha256_file_local(INTRADAY_SEASONAL_RESIDUAL_DISLOCATION_SUMMARY_PATH)
...
def write_intraday_seasonal_residual_dislocation_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_intraday_seasonal_residual_dislocation_summary_v1",
        "template": False,
        "work_unit_id": "C0033",
        "campaign_id": "C0033",
        "run_id": "R0001",
        ...
        "intraday_seasonal_residual_dislocation_profile": profiles["intraday_seasonal_residual_dislocation_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "intraday_seasonal_residual_dislocation_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0033-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0033_intraday_seasonal_residual_dislocation_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0033_intraday_seasonal_residual_dislocation_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0033-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0033-R0001-SEASONAL-RESIDUAL-SUMMARY",
                "intraday_seasonal_residual_dislocation_summary_artifact",
                "json",
                "campaigns/C0033_intraday_seasonal_residual_dislocation_discovery/runs/R0001/artifacts/c0033_r0001_seasonal_residual_summary.json",
                summary_hash,
                ["campaigns/C0033_intraday_seasonal_residual_dislocation_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0033_r0001_intraday_seasonal_residual_dislocation",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0033_r0001_proxy_trades.csv"
    evidence["intraday_seasonal_residual_dislocation_summary"] = "artifacts/c0033_r0001_seasonal_residual_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0033_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0033_r0001_proxy_trades.csv",
        "artifacts/c0033_r0001_seasonal_residual_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0033 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0033_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0033_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0033_intraday_seasonal_residual_dislocation_discovery"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0033_intraday_seasonal_residual_dislocation_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0033_intraday_seasonal_residual_dislocation_discovery",
        "open_c0033_r0001_fold_local_intraday_seasonal_residual_dislocation_rank_run",
        "produce_c0033_r0001_proxy_evidence",
    ):
    ...
    data["active_run"] = "campaigns/C0033_intraday_seasonal_residual_dislocation_discovery/runs/R0001"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["active_campaign"] = "campaigns/C0033_intraday_seasonal_residual_dislocation_discovery"
    data["active_run"] = "campaigns/C0033_intraday_seasonal_residual_dislocation_discovery/runs/R0001"
    data["latest_operation"] = {
        "id": "produce_c0033_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0033_intraday_seasonal_residual_dislocation_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0033_r0001_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0033",
            "c0004_r0001_fold_local_state_archetype": "c0033_r0001_intraday_seasonal_residual_dislocation",
            "c0004_r0001": "c0033_r0001",
            "fold_local_state_archetype_discovery": "intraday_seasonal_residual_dislocation_discovery",
            ...
            "state_archetype_summary": "intraday_seasonal_residual_dislocation_summary",
        }
