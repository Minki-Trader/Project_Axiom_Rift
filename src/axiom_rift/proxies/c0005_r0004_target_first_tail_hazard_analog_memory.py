"""C0005 R0004 proxy evidence for target-first tail-hazard analog-memory entries."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0005_continuous_analog_memory_discovery" / "runs" / "R0004"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0005_r0004_proxy_trades.csv"
ANALOG_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0005_r0004_analog_memory_summary.json"
...
MODEL_FAMILY = "fold_local_target_first_tail_hazard_dispersion_analog_memory"
LABEL_SHAPE = "directional_target_first_tail_hazard_risk_adjusted_event"
ANALOG_NEIGHBOR_COUNT = 96
INNER_NEIGHBOR_COUNT = 40
MAX_ANALOG_DISTANCE = 12.0
MIN_TARGET_HAZARD_LIFT = -0.08
MAX_STOP_FIRST_RATE_LIFT = 0.12
MIN_LOCAL_LOWER_TAIL_MEAN = -0.95
MAX_INNER_OUTER_GAP = 0.46
ELIGIBILITY_FILTER = "distance_only_after_empty_schedule_repair"
...
def run_c0005_r0004_proxy(write: bool = True) -> dict[str, object]:
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            ...
            if features is None:
                continue
            label = target_first_tail_hazard_label(bars, range_average, index, direction) if include_labels else None
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|target_first_tail_hazard_analog_memory",
                    features=features,
                    label=label,
                )
            )
...
def target_first_tail_hazard_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    index: int,
    direction: int,
...
def fit_analog_model(candidates: list[base.Candidate], fold_id: str) -> AnalogModel:
    ...
    return AnalogModel(
        ...
        global_positive_rate=float(np.mean(labels > 0.65)),
        global_adverse_rate=float(np.mean(labels < -0.65)),
    )
...
def score_candidates(candidates: list[base.Candidate], model: AnalogModel) -> list[base.Candidate]:
    ...
    neighbor_weights = recency_weights(neighbor_train_indices)
    weighted_mean = (neighbor_labels * neighbor_weights).sum(axis=1) / np.maximum(neighbor_weights.sum(axis=1), 1e-9)
    local_mean = neighbor_labels.mean(axis=1)
    local_std = neighbor_labels.std(axis=1)
    local_target_first_rate = (neighbor_labels > 0.65).mean(axis=1)
    local_stop_first_rate = (neighbor_labels < -0.65).mean(axis=1)
    ...
    inner_outer_gap = np.abs(inner_mean - outer_mean)
    recent_order = np.argsort(neighbor_train_indices, axis=1)
    ...
    scores = (
        weighted_mean
        + 0.42 * (local_target_first_rate - model.global_positive_rate)
        - 0.75 * (local_stop_first_rate - model.global_adverse_rate)
        + 0.16 * (recent_mean - model.global_recent_mean)
        + 0.18 * local_upper_tail
        + 0.28 * local_lower_tail
        - 0.14 * local_std
        - 0.20 * inner_outer_gap
        - 0.012 * mean_distance
    )
...
def build_proxy_payload(
    ...
    payload["proxy_id"] = "PX-C0005-R0004"
    payload["proxy_engine"] = "axiom_rift.proxies.c0005_r0004_target_first_tail_hazard_analog_memory"
    payload["proxy_config_path"] = "campaigns/C0005_continuous_analog_memory_discovery/runs/R0004/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_target_first_tail_hazard_analog_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0005_continuous_analog_memory_discovery/runs/R0004/kpi/proxy.json",
        "campaigns/C0005_continuous_analog_memory_discovery/runs/R0004/artifacts/c0005_r0004_proxy_trades.csv",
        "campaigns/C0005_continuous_analog_memory_discovery/runs/R0004/artifacts/c0005_r0004_analog_memory_summary.json",
    ]
    ...
    profiles["analog_memory_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "min_target_hazard_lift": MIN_TARGET_HAZARD_LIFT,
            "max_stop_first_rate_lift": MAX_STOP_FIRST_RATE_LIFT,
            "min_local_lower_tail_mean": MIN_LOCAL_LOWER_TAIL_MEAN,
            "max_inner_outer_gap": MAX_INNER_OUTER_GAP,
            "eligibility_filter": ELIGIBILITY_FILTER,
            "fold_models": fold_models,
            "state_distributions": state_distributions,
            "candidates_by_fold": candidates_by_fold,
            "selection_rule": "top_fold_local_target_first_tail_hazard_analog_scores_per_active_day",
    ...
    config.update(
        {
            ...
            "max_inner_outer_gap": MAX_INNER_OUTER_GAP,
            "eligibility_filter": ELIGIBILITY_FILTER,
            "score_interpretation": "higher_score_means_target_first_low_tail_risk_local_analog_hazard_edge",
            "variant_boundary": "target_first_tail_hazard_analog_memory_not_r0001_r0002_r0003_parameter_nudge_or_retry",
        }
    )
    return config
...
def analog_model_summary(model: AnalogModel) -> dict[str, object]:
    return {
        ...
        "global_target_first_rate": base.rounded(model.global_positive_rate),
        "global_stop_first_rate": base.rounded(model.global_adverse_rate),
...
def write_analog_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_target_first_tail_hazard_analog_memory_summary_v1",
        "template": False,
        "work_unit_id": "C0005",
        "campaign_id": "C0005",
        "run_id": "R0004",
        ...
        [
            artifact_record(
                ...
                [
                    ...
                    "campaigns/C0005_continuous_analog_memory_discovery/runs/R0004/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0005-R0004-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0005-R0004-ANALOG-MEMORY-SUMMARY",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0005_r0004_mt5_logic_parity_evidence",
        }
    ]
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0005_r0004_target_first_tail_hazard_analog_memory",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0005_r0004_proxy_trades.csv"
    evidence["analog_memory_summary"] = "artifacts/c0005_r0004_analog_memory_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0005_r0004_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0005_r0004_proxy_trades.csv",
        "artifacts/c0005_r0004_analog_memory_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0005 R0004 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0005_r0004_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0005_r0004_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    if "produce_c0005_r0004_proxy_evidence" not in completed:
        completed.append("produce_c0005_r0004_proxy_evidence")
    next_action = "produce_c0005_r0004_mt5_logic_parity_evidence"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0005_r0004_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0005_continuous_analog_memory_discovery/runs/R0004/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0005_r0004_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            ...
            "R0004": "R0004",
            "R0001": "R0004",
            "c0004_r0001_fold_local_state_archetype": "c0005_r0004_target_first_tail_hazard_analog_memory",
            "c0004_r0001": "c0005_r0004",
