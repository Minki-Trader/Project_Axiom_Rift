"""C0037 R0001 proxy evidence for intraday price memory density discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0037_intraday_price_memory_density_discovery" / "runs" / "R0001"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0037_r0001_proxy_trades.csv"
PRICE_MEMORY_DENSITY_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0037_r0001_price_memory_density_summary.json"
...
MODEL_FAMILY = "fold_local_intraday_price_memory_density_rank"
LABEL_SHAPE = "directional_intraday_price_memory_density_survival_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_intraday_price_memory_density_scores_per_active_day"
...
def run_c0037_r0001_proxy(write: bool = True) -> dict[str, object]:
        ...
        for direction in (1, -1):
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|intraday_price_memory_density",
                    features=features,
                    label=label,
                )
            )
...
def linear_model_summary(model: LinearEdgeModel) -> dict[str, object]:
    return {
        ...
        "score_interpretation": "higher_score_means_direct_fold_local_intraday_price_memory_density_rank",
    }
...
def build_proxy_payload(
    ...
    payload["proxy_id"] = "PX-C0037-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0037_r0001_intraday_price_memory_density"
    payload["proxy_config_path"] = "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_intraday_price_memory_density_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0001/artifacts/c0037_r0001_proxy_trades.csv",
        "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0001/artifacts/c0037_r0001_price_memory_density_summary.json",
    ]
    ...
    profiles["intraday_price_memory_density_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
            "model_family": MODEL_FAMILY,
            "feature_count": len(FEATURE_NAMES),
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0037_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "score_interpretation": "higher_score_means_direct_fold_local_intraday_price_memory_density_rank",
            "variant_boundary": (
                "intraday_price_memory_density_rank_not_effort_absorption_seasonal_residual_tail_risk_skew_entropy_"
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_price_memory_density_summary_artifact(payload, PRICE_MEMORY_DENSITY_SUMMARY_PATH)
    write_text_local(PROXY_PATH, json.dumps(payload, indent=2, sort_keys=True) + "\n")
    trade_hash = sha256_file_local(TRADE_ARTIFACT_PATH)
    summary_hash = sha256_file_local(PRICE_MEMORY_DENSITY_SUMMARY_PATH)
...
def write_price_memory_density_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_intraday_price_memory_density_summary_v1",
        "template": False,
        "work_unit_id": "C0037",
        "campaign_id": "C0037",
        "run_id": "R0001",
        ...
        "intraday_price_memory_density_profile": profiles["intraday_price_memory_density_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "intraday_price_memory_density_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0037-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0037-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0037-R0001-PRICE-MEMORY-DENSITY-SUMMARY",
                "intraday_price_memory_density_summary_artifact",
                "json",
                "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0001/artifacts/c0037_r0001_price_memory_density_summary.json",
                summary_hash,
                ["campaigns/C0037_intraday_price_memory_density_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0037_r0001_intraday_price_memory_density",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0037_r0001_proxy_trades.csv"
    evidence["intraday_price_memory_density_summary"] = "artifacts/c0037_r0001_price_memory_density_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0037_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0037_r0001_proxy_trades.csv",
        "artifacts/c0037_r0001_price_memory_density_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0037 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0037_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0037_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_run"] = "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0001"
    ...
    for item in (
        ...
        "open_c0037_r0001_fold_local_intraday_price_memory_density_rank_run",
        "produce_c0037_r0001_proxy_evidence",
    ):
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0037_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0037_r0001_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["canonical_source"] = "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0001/kpi/proxy.json"
    data["canonical_status"] = "proxy_recorded_pending_mt5"
    data["current_decision"] = "produce_c0037_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = "campaigns/C0037_intraday_price_memory_density_discovery"
    data["active_run"] = "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0001"
    data["next_required_action"] = "produce_c0037_r0001_mt5_logic_parity_evidence"
    data["current_evidence_summary"] = {
        "source_campaign": "campaigns/C0037_intraday_price_memory_density_discovery",
        "current_task": "produce_c0037_r0001_mt5_logic_parity_evidence",
        "active_run": "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0001",
        ...
        "proxy_profit_factor": required.get("proxy_profit_factor") if isinstance(required, dict) else None,
        "next_required_action": "produce_c0037_r0001_mt5_logic_parity_evidence",
        "note": "C0037 R0001 proxy evidence recorded; MT5 paired validation remains mandatory regardless of proxy result.",
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0037",
            "c0004_r0001_fold_local_state_archetype": "c0037_r0001_intraday_price_memory_density",
            "c0004_r0001": "c0037_r0001",
            ...
            "state_archetype_summary": "intraday_price_memory_density_summary",
        }
