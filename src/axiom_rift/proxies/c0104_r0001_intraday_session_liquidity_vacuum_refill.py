"""C0104 R0001 proxy evidence for intraday session liquidity vacuum refill."""
...
CAMPAIGN_ID = "C0104"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0104_intraday_session_liquidity_vacuum_refill_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0104_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0104_r0001_session_liquidity_vacuum_refill_summary.json"
...
FEATURE_NAMES = (
    "liquidity_vacuum_ratio",
    "refill_pressure",
    "adverse_void_recoil",
    "session_depth_gap",
    "post_void_settlement",
    "vacuum_range_recompression",
    "participation_recovery",
    "prior_day_liquidity_context",
    "time_refill_alignment",
    "session_phase_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_session_liquidity_vacuum_refill_rank"
LABEL_SHAPE = "target_first_quality_after_session_liquidity_vacuum_refill"
SELECTION_RULE = "top_fold_local_liquidity_vacuum_refill_scores_per_active_day"
...
MIN_DAY_BARS = 30
LIQUIDITY_VACUUM_REFILL_LOOKBACK_BARS = 72
LIQUIDITY_VACUUM_REFILL_RECENT_BARS = 12
LIQUIDITY_VACUUM_REFILL_PRIOR_BARS = 30
MIN_LIQUIDITY_VACUUM_REFILL_ACTIVITY = 0.07
...
class LinearSessionLiquidityVacuumRefillModel:
    fold_id: str
    ...
    session_liquidity_vacuum_refill_failure_rate: float
...
def run_c0104_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_session_liquidity_vacuum_refill_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_session_liquidity_vacuum_refill_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        LIQUIDITY_VACUUM_REFILL_LOOKBACK_BARS,
        LIQUIDITY_VACUUM_REFILL_RECENT_BARS + LIQUIDITY_VACUUM_REFILL_PRIOR_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = session_liquidity_vacuum_refill_metrics(
    ...
    features = (
        metrics["liquidity_vacuum_ratio"],
        metrics["refill_pressure"],
        metrics["adverse_void_recoil"],
        metrics["session_depth_gap"],
        metrics["post_void_settlement"],
        metrics["vacuum_range_recompression"],
        metrics["participation_recovery"],
        metrics["prior_day_liquidity_context"],
        metrics["time_refill_alignment"],
        metrics["session_phase_maturity"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        c0104_vacuum_bucket(metrics["liquidity_vacuum_ratio"]),
        c0104_refill_pressure_bucket(metrics["refill_pressure"]),
        c0104_recoil_bucket(metrics["adverse_void_recoil"]),
        c0104_depth_gap_bucket(metrics["session_depth_gap"]),
        c0104_settlement_bucket(metrics["post_void_settlement"]),
        c0104_recompression_bucket(metrics["vacuum_range_recompression"]),
        c0104_participation_recovery_bucket(metrics["participation_recovery"]),
        c0104_prior_liquidity_bucket(metrics["prior_day_liquidity_context"]),
        c0104_time_refill_bucket(metrics["time_refill_alignment"]),
...
def session_liquidity_vacuum_refill_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    recent_start = max(index - LIQUIDITY_VACUUM_REFILL_RECENT_BARS + 1, context.day_start_index)
    prior_start = max(recent_start - LIQUIDITY_VACUUM_REFILL_PRIOR_BARS, context.day_start_index)
    lookback_start = max(index - LIQUIDITY_VACUUM_REFILL_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    session_volume_per_bar = context.volume_so_far / max(context.bars_so_far, 1)
    volume_vacuum = 1.0 - recent_volume_mean / max(prior_volume_mean, average_volume * 0.25)
    range_vacuum = 1.0 - recent_range_mean / max(prior_range_mean, average_range * 0.25)
    liquidity_vacuum_ratio = volume_vacuum + 0.50 * range_vacuum
    session_depth_gap = 1.0 - recent_volume_mean / max(session_volume_per_bar, average_volume * 0.25)
    ...
    prior_scale = max(prior_range_mean * math.sqrt(len(prior_window)), average_range * 0.25)
    directional_recent_return = direction * recent_return / recent_scale
    adverse_prior = max(0.0, -direction * prior_return / prior_scale)
    refill_pressure = directional_recent_return * max(liquidity_vacuum_ratio, 0.0)
    adverse_void_recoil = adverse_prior * max(directional_recent_return, 0.0)
    post_void_settlement = base.mean([directional_bar_settlement(bar, direction) for bar in recent_window]) or 0.0
    vacuum_range_recompression = 1.0 - recent_range_mean / max(lookback_range_mean, average_range * 0.25)
    ...
    participation_recovery = late_volume_mean / max(early_volume_mean, average_volume * 0.25) - 1.0

    prior_day_liquidity_context = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_day_volume_ratio = (context.prior_day_volume_per_bar or average_volume) / max(average_volume, 1e-9)
        prior_day_liquidity_context = (
            (1.0 - min(max(prior_day_volume_ratio, 0.0), 4.0) / 4.0)
            + direction * context.prior_day_return / max(context.prior_day_range, 1e-9) * 0.35
        )

    time_refill_alignment = session_phase_maturity * (
        0.35 * refill_pressure
        + 0.25 * adverse_void_recoil
        + 0.20 * max(liquidity_vacuum_ratio, 0.0)
        + 0.15 * max(participation_recovery, 0.0)
        - 0.08 * max(session_depth_gap, 0.0)
    )
    activity = max(
        abs(refill_pressure),
        adverse_void_recoil * 0.35,
        max(liquidity_vacuum_ratio, 0.0) * 0.25,
        abs(session_depth_gap) * 0.18,
        max(participation_recovery, 0.0) * 0.14,
        max(vacuum_range_recompression, 0.0) * 0.12,
    )
    if activity < MIN_LIQUIDITY_VACUUM_REFILL_ACTIVITY:
        return {"event_count": 0.0}
    if max(liquidity_vacuum_ratio, 0.0) < 0.05 and abs(refill_pressure) < 0.06 and adverse_void_recoil < 0.05:
        return {"event_count": 0.0}
    return {
        "liquidity_vacuum_ratio": bounded(liquidity_vacuum_ratio, -3.0, 3.0),
        "refill_pressure": bounded(refill_pressure, -4.0, 4.0),
        "adverse_void_recoil": bounded(adverse_void_recoil, 0.0, 4.0),
        "session_depth_gap": bounded(session_depth_gap, -3.0, 3.0),
        "post_void_settlement": bounded(post_void_settlement, 0.0, 1.0),
        "vacuum_range_recompression": bounded(vacuum_range_recompression, -3.0, 3.0),
        "participation_recovery": bounded(participation_recovery, -4.0, 4.0),
        "prior_day_liquidity_context": bounded(prior_day_liquidity_context, -2.0, 2.0),
        "time_refill_alignment": bounded(time_refill_alignment, -4.0, 4.0),
        ...
        "session_liquidity_vacuum_refill_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0104_vacuum_bucket(value: float) -> str:
    if value < 0.05:
        return "vacuum_absent"
    if value < 0.35:
        return "vacuum_defined"
    return "vacuum_deep"


def c0104_refill_pressure_bucket(value: float) -> str:
    if value < -0.05:
        return "refill_pressure_against"
    if value < 0.18:
        return "refill_pressure_mixed"
    return "refill_pressure_with"


def c0104_recoil_bucket(value: float) -> str:
    if value < 0.04:
        return "void_recoil_absent"
    if value < 0.22:
        return "void_recoil_defined"
    return "void_recoil_heavy"


def c0104_depth_gap_bucket(value: float) -> str:
    if value < 0.03:
        return "depth_gap_absent"
    if value < 0.28:
        return "depth_gap_defined"
    return "depth_gap_wide"


def c0104_settlement_bucket(value: float) -> str:
    if value < 0.42:
...
def c0104_recompression_bucket(value: float) -> str:
    if value < 0.02:
        return "recompression_absent"
    if value < 0.22:
        return "recompression_defined"
    return "recompression_tight"


def c0104_participation_recovery_bucket(value: float) -> str:
    if value < -0.08:
        return "participation_recovery_fading"
    if value < 0.18:
        return "participation_recovery_flat"
    return "participation_recovery_active"


def c0104_prior_liquidity_bucket(value: float) -> str:
    if value < -0.10:
        return "prior_liquidity_against"
    if value < 0.24:
        return "prior_liquidity_neutral"
    return "prior_liquidity_supportive"


def c0104_time_refill_bucket(value: float) -> str:
    if value < -0.04:
        return "time_refill_against"
    if value < 0.18:
        return "time_refill_mixed"
    return "time_refill_aligned"
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    refill_reward = 0.16 if metrics["refill_pressure"] > 0.14 and terminal > 0 else 0.0
    vacuum_reward = 0.09 if metrics["liquidity_vacuum_ratio"] > 0.18 and terminal > 0 else 0.0
    recoil_reward = 0.13 if metrics["adverse_void_recoil"] > 0.12 and terminal > 0 else 0.0
    depth_gap_reward = 0.06 if metrics["session_depth_gap"] > 0.10 and terminal > 0 else 0.0
    settlement_reward = 0.10 if metrics["post_void_settlement"] > 0.62 and terminal > 0 else 0.0
    recompression_reward = 0.07 if metrics["vacuum_range_recompression"] > 0.10 and terminal > 0 else 0.0
    participation_reward = 0.08 if metrics["participation_recovery"] > 0.12 and terminal > 0 else 0.0
    prior_reward = 0.05 if metrics["prior_day_liquidity_context"] > 0.10 and terminal > 0 else 0.0
    time_reward = 0.06 if metrics["time_refill_alignment"] > 0.08 and terminal > 0 else 0.0
    weak_refill_penalty = 0.12 if metrics["refill_pressure"] < -0.04 and terminal <= 0 else 0.0
    no_recoil_penalty = 0.08 if metrics["adverse_void_recoil"] < 0.03 and terminal <= 0 else 0.0
    adverse_settlement_penalty = 0.10 if metrics["post_void_settlement"] < 0.42 and terminal <= 0 else 0.0
    participation_drop_penalty = 0.08 if metrics["participation_recovery"] < -0.18 and terminal <= 0 else 0.0
    excess_depth_penalty = 0.05 if metrics["session_depth_gap"] > 0.85 and terminal <= 0 else 0.0
    ...
    session_liquidity_vacuum_refill_path_quality = (0.54 * terminal + 0.30 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + session_liquidity_vacuum_refill_path_quality
        + refill_reward
        + vacuum_reward
        + recoil_reward
        + depth_gap_reward
        + settlement_reward
        + recompression_reward
        + participation_reward
        + prior_reward
        + time_reward
        - weak_refill_penalty
        - no_recoil_penalty
        - adverse_settlement_penalty
        - participation_drop_penalty
        - excess_depth_penalty
        - stall_penalty
...
def fit_linear_session_liquidity_vacuum_refill_model(candidates: list[base.Candidate], fold_id: str) -> LinearSessionLiquidityVacuumRefillModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearSessionLiquidityVacuumRefillModel(
            fold_id,
    ...
    session_liquidity_vacuum_refill_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearSessionLiquidityVacuumRefillModel(
        fold_id=fold_id,
        ...
        session_liquidity_vacuum_refill_failure_rate=session_liquidity_vacuum_refill_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearSessionLiquidityVacuumRefillModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0104-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_session_liquidity_vacuum_refill_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0104_r0001_intraday_session_liquidity_vacuum_refill",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0104_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0104_r0001_session_liquidity_vacuum_refill_summary.json",
        ],
        ...
        "conditional_profiles": {
            "session_liquidity_vacuum_refill_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0104 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0104_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0104 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "session_liquidity_vacuum_refill_lookback_bars": LIQUIDITY_VACUUM_REFILL_LOOKBACK_BARS,
        "session_liquidity_vacuum_refill_recent_bars": LIQUIDITY_VACUUM_REFILL_RECENT_BARS,
        "session_liquidity_vacuum_refill_prior_bars": LIQUIDITY_VACUUM_REFILL_PRIOR_BARS,
        "min_session_liquidity_vacuum_refill_activity": MIN_LIQUIDITY_VACUUM_REFILL_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_session_liquidity_vacuum_refill_model_summary(model: LinearSessionLiquidityVacuumRefillModel) -> dict[str, object]:
    return {
        ...
        "session_liquidity_vacuum_refill_failure_rate": rounded(model.session_liquidity_vacuum_refill_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearSessionLiquidityVacuumRefillModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_session_liquidity_vacuum_refill_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "session_liquidity_vacuum_refill_profile": profiles["session_liquidity_vacuum_refill_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0104_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0104_r0001_proxy_trades.csv"
    evidence["session_liquidity_vacuum_refill_summary"] = "artifacts/c0104_r0001_session_liquidity_vacuum_refill_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0104_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0104_r0001_proxy_trades.csv",
        "artifacts/c0104_r0001_session_liquidity_vacuum_refill_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0104_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0104 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0104_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0104_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0104_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0104_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0104_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0104_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0104_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0104_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0104_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0104 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0104 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0104_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0104_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0104_r0001_intraday_session_liquidity_vacuum_refill_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0104_r0001_proxy_records_session_liquidity_vacuum_refill_reference_surface
  ...
  - next_work_is_produce_c0104_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0104_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0104-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0104_r0001_intraday_session_liquidity_vacuum_refill",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0104-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0104-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0104-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0104_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0104_r0001_intraday_session_liquidity_vacuum_refill",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0104-R0001-session-liquidity-vacuum-refill-SUMMARY",
            "artifact_role": "session_liquidity_vacuum_refill_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0104_r0001_session_liquidity_vacuum_refill_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0104_r0001_intraday_session_liquidity_vacuum_refill",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0104_r0001_mt5_logic_parity_evidence",
        }
    ]
