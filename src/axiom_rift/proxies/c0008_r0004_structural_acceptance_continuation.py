"""C0008 R0004 proxy evidence for structural acceptance continuation discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0008_multi_timeframe_structural_context_discovery" / "runs" / "R0004"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0008_r0004_proxy_trades.csv"
ACCEPTANCE_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0008_r0004_structural_acceptance_continuation_summary.json"
...
FEATURE_NAMES = (
    ...
    "accepted_break_m15",
    "accepted_break_h1",
    "accepted_break_session",
    "accepted_break_day",
    "hold_depth_h1",
    "hold_depth_session",
    "hold_depth_day",
    "continuation_alignment",
    "compression_before_break",
    "expansion_quality",
    "session_sin",
    "session_cos",
    "spread_over_range",
)
MODEL_FAMILY = "fold_local_structural_acceptance_continuation_rank"
LABEL_SHAPE = "directional_structural_acceptance_continuation_quality"
POSITIVE_LABEL_THRESHOLD = 0.12
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_structural_acceptance_continuation_scores_per_active_day"
...
def run_c0008_r0004_proxy(write: bool = True) -> dict[str, object]:
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = structural_acceptance_continuation_features(bars, range_average, short_range_average, index, direction)
            if features is None:
                continue
            label = structural_acceptance_continuation_label(bars, range_average, index, direction) if include_labels else None
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|structural_acceptance_continuation",
                    features=features,
                    label=label,
                )
            )
...
def structural_acceptance_continuation_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    index: int,
    ...
    accepted_break_m15 = max(break_m15, 0.0)
    accepted_break_h1 = max(break_h1, 0.0)
    accepted_break_session = max(break_session, 0.0)
    accepted_break_day = max(break_day, 0.0)
    prior_close = bars[index - 1].close
    if direction > 0:
        hold_depth_h1 = (min(bar.close, prior_close) - h1_high) / average_range
        hold_depth_session = (min(bar.close, prior_close) - session_high) / average_range
        hold_depth_day = (min(bar.close, prior_close) - day_high) / average_range
    else:
        hold_depth_h1 = (h1_low - max(bar.close, prior_close)) / average_range
        hold_depth_session = (session_low - max(bar.close, prior_close)) / average_range
        hold_depth_day = (day_low - max(bar.close, prior_close)) / average_range
    hold_depth_h1 = max(hold_depth_h1, 0.0)
    hold_depth_session = max(hold_depth_session, 0.0)
    hold_depth_day = max(hold_depth_day, 0.0)
    ...
    continuation_alignment = max(trend_consistency, 0.0) + max(body_fraction, 0.0) + max(directional_close_location - 0.5, 0.0)
    compression_before_break = max(1.0 - range_compression, 0.0)
    expansion_quality = max(range_expansion - 1.0, 0.0)
    accepted_break_strength = max(accepted_break_h1, accepted_break_session, accepted_break_day, 0.0)
    hold_strength = max(hold_depth_h1, hold_depth_session, hold_depth_day, 0.0)
    if accepted_break_strength < 0.06 and hold_strength < 0.04:
        return None
    if continuation_alignment < 0.16 and expansion_quality < 0.05:
        return None
...
def structural_acceptance_continuation_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    index: int,
    direction: int,
    ...
    if direction > 0:
        accepted_level = max(
            level
            for level in (m15_high, h1_high, session_high, day_high)
            if bars[index].close >= level
        )
    else:
        accepted_level = min(
            level
            for level in (m15_low, h1_low, session_low, day_low)
            if bars[index].close <= level
        )
    ...
    if direction > 0:
        ...
        failed_hold_count = sum(1 for bar in path if bar.close < accepted_level)
    ...
    failed_hold_rate = failed_hold_count / len(path)
...
def build_proxy_payload(
    ...
    payload["proxy_id"] = "PX-C0008-R0004"
    payload["proxy_engine"] = "axiom_rift.proxies.c0008_r0004_structural_acceptance_continuation"
    payload["proxy_config_path"] = "campaigns/C0008_multi_timeframe_structural_context_discovery/runs/R0004/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_structural_acceptance_continuation_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0008_multi_timeframe_structural_context_discovery/runs/R0004/kpi/proxy.json",
        "campaigns/C0008_multi_timeframe_structural_context_discovery/runs/R0004/artifacts/c0008_r0004_proxy_trades.csv",
        "campaigns/C0008_multi_timeframe_structural_context_discovery/runs/R0004/artifacts/c0008_r0004_structural_acceptance_continuation_summary.json",
    ]
    ...
    profiles["structural_acceptance_continuation_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0008_r0004_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "feature_weight_floor": FEATURE_WEIGHT_FLOOR,
            "feature_weight_ceiling": FEATURE_WEIGHT_CEILING,
            "score_interpretation": "higher_score_means_fold_local_structural_acceptance_continuation_quality",
            "variant_boundary": "accepted_structural_break_continuation_not_direct_context_rank_trap_reversal_robustness_filter_or_retry_nudge",
        }
    )
    return config
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_structural_summary_artifact(payload, ACCEPTANCE_SUMMARY_PATH)
    PROXY_PATH.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="ascii")
    trade_hash = base.sha256_file(TRADE_ARTIFACT_PATH)
    summary_hash = base.sha256_file(ACCEPTANCE_SUMMARY_PATH)
...
def write_structural_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_structural_acceptance_continuation_summary_v1",
        "template": False,
        "work_unit_id": "C0008",
        "campaign_id": "C0008",
        "run_id": "R0004",
        ...
        "structural_acceptance_continuation_profile": profiles["structural_acceptance_continuation_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "structural_acceptance_continuation_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0008-R0004-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0008_multi_timeframe_structural_context_discovery/runs/R0004/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0008_multi_timeframe_structural_context_discovery/runs/R0004/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0008-R0004-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0008-R0004-STRUCTURAL-ACCEPTANCE-CONTINUATION-SUMMARY",
                "structural_acceptance_continuation_summary_artifact",
                "json",
                "campaigns/C0008_multi_timeframe_structural_context_discovery/runs/R0004/artifacts/c0008_r0004_structural_acceptance_continuation_summary.json",
                summary_hash,
                ["campaigns/C0008_multi_timeframe_structural_context_discovery/runs/R0004/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0008_r0004_structural_acceptance_continuation",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0008_r0004_proxy_trades.csv"
    evidence["structural_acceptance_continuation_summary"] = "artifacts/c0008_r0004_structural_acceptance_continuation_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0008_r0004_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0008_r0004_proxy_trades.csv",
        "artifacts/c0008_r0004_structural_acceptance_continuation_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0008 R0004 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0008_r0004_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0008_r0004_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    for item in (
        ...
        "open_c0008_r0004_structural_acceptance_continuation_run",
        "produce_c0008_r0004_proxy_evidence",
    ):
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0008_r0004_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0008_multi_timeframe_structural_context_discovery/runs/R0004/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0008_r0004_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0008",
            "c0004_r0004_fold_local_state_archetype": "c0008_r0004_structural_acceptance_continuation",
            "c0004_r0004": "c0008_r0004",
            ...
            "state_archetype_summary": "structural_acceptance_continuation_summary",
        }
