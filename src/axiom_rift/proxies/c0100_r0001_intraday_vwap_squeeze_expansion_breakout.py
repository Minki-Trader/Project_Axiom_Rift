"""C0100 R0001 proxy evidence for intraday vwap squeeze expansion breakout."""
...
CAMPAIGN_ID = "C0100"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0100_intraday_vwap_squeeze_expansion_breakout_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0100_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0100_r0001_vwap_squeeze_expansion_breakout_summary.json"
...
FEATURE_NAMES = (
    "vwap_squeeze_pressure",
    "compression_score",
    "directional_release_pressure",
    "release_efficiency",
    "close_displacement_strength",
    "participation_reexpansion",
    "range_expansion_ratio",
    "session_vwap_side_alignment",
    "prior_day_directional_context",
    "session_phase_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_vwap_squeeze_expansion_breakout_rank"
LABEL_SHAPE = "target_first_breakout_quality_after_vwap_centered_compression_release"
SELECTION_RULE = "top_fold_local_vwap_squeeze_expansion_breakout_scores_per_active_day"
...
MIN_DAY_BARS = 30
VWAP_SQUEEZE_EXPANSION_BREAKOUT_LOOKBACK_BARS = 36
VWAP_SQUEEZE_EXPANSION_BREAKOUT_SQUEEZE_BARS = 18
VWAP_SQUEEZE_EXPANSION_BREAKOUT_RELEASE_BARS = 4
MIN_VWAP_SQUEEZE_EXPANSION_BREAKOUT_ACTIVITY = 0.08
...
class LinearVwapSqueezeExpansionBreakoutModel:
    fold_id: str
    ...
    vwap_squeeze_expansion_breakout_failure_rate: float
...
def run_c0100_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_vwap_squeeze_expansion_breakout_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_vwap_squeeze_expansion_breakout_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        VWAP_SQUEEZE_EXPANSION_BREAKOUT_LOOKBACK_BARS,
        VWAP_SQUEEZE_EXPANSION_BREAKOUT_SQUEEZE_BARS + VWAP_SQUEEZE_EXPANSION_BREAKOUT_RELEASE_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = vwap_squeeze_expansion_breakout_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        metrics["vwap_squeeze_pressure"],
        metrics["compression_score"],
        metrics["directional_release_pressure"],
        metrics["release_efficiency"],
        metrics["close_displacement_strength"],
        metrics["participation_reexpansion"],
        metrics["range_expansion_ratio"],
        metrics["session_vwap_side_alignment"],
        metrics["prior_day_directional_context"],
        metrics["session_phase_maturity"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        c0100_vwap_squeeze_bucket(metrics["vwap_squeeze_pressure"]),
        c0100_compression_bucket(metrics["compression_score"]),
        c0100_release_pressure_bucket(metrics["directional_release_pressure"]),
        c0100_release_efficiency_bucket(metrics["release_efficiency"]),
        c0100_close_displacement_bucket(metrics["close_displacement_strength"]),
        c0100_participation_reexpansion_bucket(metrics["participation_reexpansion"]),
        range_expansion_bucket(metrics["range_expansion_ratio"]),
        c0100_vwap_alignment_bucket(metrics["session_vwap_side_alignment"]),
        c0100_prior_context_bucket(metrics["prior_day_directional_context"]),
...
def vwap_squeeze_expansion_breakout_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    release_start = index - VWAP_SQUEEZE_EXPANSION_BREAKOUT_RELEASE_BARS + 1
    squeeze_start = release_start - VWAP_SQUEEZE_EXPANSION_BREAKOUT_SQUEEZE_BARS
    lookback_start = index - VWAP_SQUEEZE_EXPANSION_BREAKOUT_LOOKBACK_BARS
    if release_start < 0 or squeeze_start < 0 or lookback_start < 0 or context.day_start_index >= index:
        return {"event_count": 0.0}
    release_start = max(release_start, context.day_start_index)
    squeeze_start = max(squeeze_start, context.day_start_index)
    lookback_start = max(lookback_start, context.day_start_index)
    squeeze_window = bars[squeeze_start:release_start]
    release_window = bars[release_start : index + 1]
    prior_window = bars[lookback_start:squeeze_start]
    if len(squeeze_window) < 8 or len(release_window) < 2 or len(prior_window) < 5:
        return {"event_count": 0.0}
    average_volume = volume_average[index]
    if average_volume is None or average_volume <= 0:
        return {"event_count": 0.0}

    squeeze_ranges = [bar.high - bar.low for bar in squeeze_window]
    release_ranges = [bar.high - bar.low for bar in release_window]
    prior_ranges = [bar.high - bar.low for bar in prior_window]
    squeeze_range_mean = base.mean(squeeze_ranges) or average_range
    release_range_mean = base.mean(release_ranges) or average_range
    prior_range_mean = base.mean(prior_ranges) or average_range
    release_closes = [bar.close for bar in release_window]
    release_move = signal_bar.close - release_window[0].open
    directional_release_pressure = direction * release_move / max(average_range, 1e-9)
    release_travel = abs(release_closes[0] - release_window[0].open) + sum(
        abs(release_closes[pos] - release_closes[pos - 1]) for pos in range(1, len(release_closes))
    )
    release_efficiency = direction * release_move / max(release_travel, average_range * 0.25)
    if directional_release_pressure <= 0:
        return {"event_count": 0.0}
    squeeze_high = max(bar.high for bar in squeeze_window)
    squeeze_low = min(bar.low for bar in squeeze_window)
    squeeze_width = max(squeeze_high - squeeze_low, 1e-9)
    compression_score = 1.0 - squeeze_range_mean / max(prior_range_mean, average_range * 0.35)
    range_expansion_ratio = release_range_mean / max(squeeze_range_mean, average_range * 0.25)
    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    pre_release_vwap = session_vwap_until(bars, volumes, context.day_start_index, max(release_start - 1, context.day_start_index))
    squeeze_vwap_distance = (
        base.mean([abs(bar.close - pre_release_vwap) for bar in squeeze_window]) or average_range
    )
    vwap_squeeze_pressure = 1.0 - squeeze_vwap_distance / max(squeeze_width, average_range * 0.75)
    close_displacement_strength = direction * (signal_bar.close - session_vwap) / max(average_range, 1e-9)
    vwap_slope_alignment = direction * (session_vwap - pre_release_vwap) / max(average_range, 1e-9)
    session_vwap_side_alignment = close_displacement_strength + 0.35 * vwap_slope_alignment
    squeeze_volume_average = base.mean(volumes[squeeze_start:release_start]) or average_volume
    participation_reexpansion = volumes[index] / max(squeeze_volume_average, 1e-9) - 1.0
    session_phase_maturity = bounded_session_progress(signal_bar.time)
    prior_day_directional_context = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_day_directional_context = direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
    vwap_squeeze_expansion_breakout_pressure_raw = max(
        max(vwap_squeeze_pressure - 0.05, 0.0) * 0.35,
        max(compression_score, 0.0) * 0.55,
        max(directional_release_pressure - 0.10, 0.0) * 0.55,
        max(release_efficiency - 0.10, 0.0) * 0.45,
        max(close_displacement_strength + 0.05, 0.0) * 0.22,
        max(participation_reexpansion, 0.0) * 0.12,
        max(range_expansion_ratio - 0.90, 0.0) * 0.22,
        max(session_vwap_side_alignment, 0.0) * 0.18,
    )
    activity = max(
        vwap_squeeze_expansion_breakout_pressure_raw,
        vwap_squeeze_expansion_breakout_pressure_raw * session_phase_maturity,
    )
    if activity < MIN_VWAP_SQUEEZE_EXPANSION_BREAKOUT_ACTIVITY:
        return {"event_count": 0.0}
    if compression_score < -0.35:
        return {"event_count": 0.0}
    if directional_release_pressure < 0.12 or release_efficiency < -0.05:
        return {"event_count": 0.0}
    if close_displacement_strength < -0.80 and range_expansion_ratio < 1.05:
        return {"event_count": 0.0}
    return {
        "vwap_squeeze_pressure": bounded(vwap_squeeze_pressure, -3.0, 1.5),
        "compression_score": bounded(compression_score, -3.0, 1.5),
        "directional_release_pressure": bounded(directional_release_pressure, 0.0, 8.0),
        "release_efficiency": bounded(release_efficiency, -1.5, 1.5),
        "close_displacement_strength": bounded(close_displacement_strength, -5.0, 5.0),
        "participation_reexpansion": bounded(participation_reexpansion, -3.0, 6.0),
        "range_expansion_ratio": bounded(range_expansion_ratio, 0.0, 6.0),
        "session_vwap_side_alignment": bounded(session_vwap_side_alignment, -5.0, 5.0),
        "prior_day_directional_context": bounded(prior_day_directional_context, -3.0, 3.0),
        "session_phase_maturity": bounded(session_phase_maturity, 0.0, 1.0),
        "squeeze_range_ratio": bounded(squeeze_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        "release_range_ratio": bounded(release_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        "prior_range_ratio": bounded(prior_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        "vwap_squeeze_expansion_breakout_pressure": bounded(vwap_squeeze_expansion_breakout_pressure_raw, 0.0, 5.0),
        "event_count": 1.0,
    }
...
def c0100_vwap_squeeze_bucket(value: float) -> str:
    if value < -0.20:
        return "vwap_squeeze_loose"
    if value < 0.30:
        return "vwap_squeeze_mixed"
    return "vwap_squeeze_tight"


def c0100_compression_bucket(value: float) -> str:
    if value < -0.10:
        return "compression_expanding"
    if value < 0.18:
        return "compression_moderate"
    return "compression_defined"


def c0100_release_pressure_bucket(value: float) -> str:
    if value < 0.35:
        return "release_pressure_light"
    if value < 0.95:
        return "release_pressure_defined"
    return "release_pressure_heavy"


def c0100_release_efficiency_bucket(value: float) -> str:
    if value < 0.15:
        return "release_efficiency_low"
    if value < 0.45:
        return "release_efficiency_mixed"
    return "release_efficiency_clean"


def c0100_close_displacement_bucket(value: float) -> str:
    if value < -0.10:
        return "close_displacement_against_vwap"
    if value < 0.55:
        return "close_displacement_near_vwap"
    return "close_displacement_beyond_vwap"


def c0100_participation_reexpansion_bucket(value: float) -> str:
    if value < -0.20:
        return "participation_cooling"
    if value < 0.35:
        return "participation_stable"
    return "participation_reexpanding"


def c0100_vwap_alignment_bucket(value: float) -> str:
    if value < -0.15:
        return "vwap_alignment_against"
    if value < 0.45:
        return "vwap_alignment_mixed"
    return "vwap_alignment_clear"


def c0100_prior_context_bucket(value: float) -> str:
    if value < -0.25:
        return "prior_context_against"
    if value < 0.25:
        return "prior_context_neutral"
    return "prior_context_aligned"
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    squeeze_reward = 0.10 if metrics["vwap_squeeze_pressure"] > 0.20 and terminal > 0 else 0.0
    compression_reward = 0.12 if metrics["compression_score"] > 0.12 and terminal > 0 else 0.0
    release_reward = 0.14 if metrics["directional_release_pressure"] > 0.75 and terminal > 0 else 0.0
    efficiency_reward = 0.10 if metrics["release_efficiency"] > 0.35 and terminal > 0 else 0.0
    displacement_reward = 0.12 if metrics["close_displacement_strength"] > 0.35 and terminal > 0 else 0.0
    expansion_reward = 0.08 if metrics["range_expansion_ratio"] > 1.15 and terminal > 0 else 0.0
    volume_reward = 0.06 if metrics["participation_reexpansion"] > 0.10 and terminal > 0 else 0.0
    vwap_reward = 0.08 if metrics["session_vwap_side_alignment"] > 0.10 and terminal > 0 else 0.0
    loose_squeeze_penalty = 0.10 if metrics["vwap_squeeze_pressure"] < -0.20 and terminal <= 0 else 0.0
    weak_compression_penalty = 0.12 if metrics["compression_score"] < -0.05 and terminal <= 0 else 0.0
    weak_release_penalty = 0.14 if metrics["directional_release_pressure"] < 0.35 and terminal <= 0 else 0.0
    weak_volume_penalty = 0.06 if metrics["participation_reexpansion"] < -0.25 and terminal <= 0 else 0.0
    vwap_loss_penalty = 0.10 if metrics["close_displacement_strength"] < -0.45 and terminal <= 0 else 0.0
    ...
    vwap_squeeze_expansion_breakout_path_quality = (0.54 * terminal + 0.30 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + vwap_squeeze_expansion_breakout_path_quality
        + squeeze_reward
        + compression_reward
        + release_reward
        + efficiency_reward
        ...
        + volume_reward
        + vwap_reward
        - loose_squeeze_penalty
        - weak_compression_penalty
        - weak_release_penalty
        - weak_volume_penalty
        - vwap_loss_penalty
        - stall_penalty
        ...
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["release_range_ratio"] - 2.20) * 0.05
    )


def fit_linear_vwap_squeeze_expansion_breakout_model(candidates: list[base.Candidate], fold_id: str) -> LinearVwapSqueezeExpansionBreakoutModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearVwapSqueezeExpansionBreakoutModel(
            fold_id,
    ...
    vwap_squeeze_expansion_breakout_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearVwapSqueezeExpansionBreakoutModel(
        fold_id=fold_id,
        ...
        vwap_squeeze_expansion_breakout_failure_rate=vwap_squeeze_expansion_breakout_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearVwapSqueezeExpansionBreakoutModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0100-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_vwap_squeeze_expansion_breakout_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0100_r0001_intraday_vwap_squeeze_expansion_breakout",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0100_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0100_r0001_vwap_squeeze_expansion_breakout_summary.json",
        ],
        ...
        "conditional_profiles": {
            "vwap_squeeze_expansion_breakout_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0100 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0100_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0100 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "vwap_squeeze_expansion_breakout_lookback_bars": VWAP_SQUEEZE_EXPANSION_BREAKOUT_LOOKBACK_BARS,
        "vwap_squeeze_expansion_breakout_squeeze_bars": VWAP_SQUEEZE_EXPANSION_BREAKOUT_SQUEEZE_BARS,
        "vwap_squeeze_expansion_breakout_release_bars": VWAP_SQUEEZE_EXPANSION_BREAKOUT_RELEASE_BARS,
        "min_vwap_squeeze_expansion_breakout_activity": MIN_VWAP_SQUEEZE_EXPANSION_BREAKOUT_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_vwap_squeeze_expansion_breakout_model_summary(model: LinearVwapSqueezeExpansionBreakoutModel) -> dict[str, object]:
    return {
        ...
        "vwap_squeeze_expansion_breakout_failure_rate": rounded(model.vwap_squeeze_expansion_breakout_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearVwapSqueezeExpansionBreakoutModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_vwap_squeeze_expansion_breakout_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "vwap_squeeze_expansion_breakout_profile": profiles["vwap_squeeze_expansion_breakout_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0100_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0100_r0001_proxy_trades.csv"
    evidence["vwap_squeeze_expansion_breakout_summary"] = "artifacts/c0100_r0001_vwap_squeeze_expansion_breakout_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0100_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0100_r0001_proxy_trades.csv",
        "artifacts/c0100_r0001_vwap_squeeze_expansion_breakout_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0100_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0100 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0100_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0100_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0100_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0100_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0100_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0100_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0100_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0100_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0100_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0100 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0100 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0100_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0100_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0100_r0001_intraday_vwap_squeeze_expansion_breakout_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0100_r0001_proxy_records_vwap_squeeze_expansion_breakout_reference_surface
  ...
  - next_work_is_produce_c0100_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0100_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0100-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0100_r0001_intraday_vwap_squeeze_expansion_breakout",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0100-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0100-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0100-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0100_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0100_r0001_intraday_vwap_squeeze_expansion_breakout",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0100-R0001-vwap-squeeze-expansion-breakout-SUMMARY",
            "artifact_role": "vwap_squeeze_expansion_breakout_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0100_r0001_vwap_squeeze_expansion_breakout_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0100_r0001_intraday_vwap_squeeze_expansion_breakout",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0100_r0001_mt5_logic_parity_evidence",
        }
    ]
