"""C0084 R0001 proxy evidence for intraday local equilibrium displacement reversion."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0084_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0084_r0001_local_equilibrium_reversion_summary.json"
...
FEATURE_NAMES = (
    "equilibrium_displacement_stretch",
    "equilibrium_reclaim_strength",
    "failed_extension_decay",
    "short_compression",
    "adverse_wick_rejection",
    "participation_turn",
    "path_efficiency_loss",
    "prior_day_contrarian_context",
    "session_reversion_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_local_equilibrium_displacement_reversion_rank"
LABEL_SHAPE = "target_first_mean_reversion_quality_conditioned_on_local_equilibrium_displacement_failure_state"
SELECTION_RULE = "top_fold_local_local_equilibrium_displacement_reversion_scores_per_active_day"
...
MIN_DAY_BARS = 30
LOCAL_EQUILIBRIUM_DISPLACEMENT_REVERSION_LOOKBACK_BARS = 36
LOCAL_EQUILIBRIUM_DISPLACEMENT_REVERSION_SHORT_LOOKBACK_BARS = 8
MIN_LOCAL_EQUILIBRIUM_DISPLACEMENT_REVERSION_ACTIVITY = 0.14
...
class LinearLocalEquilibriumDisplacementReversionModel:
    fold_id: str
    ...
    local_equilibrium_displacement_reversion_failure_rate: float
...
def run_c0084_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_local_equilibrium_displacement_reversion_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_local_equilibrium_displacement_reversion_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        LOCAL_EQUILIBRIUM_DISPLACEMENT_REVERSION_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = local_equilibrium_displacement_reversion_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        metrics["equilibrium_displacement_stretch"],
        metrics["equilibrium_reclaim_strength"],
        metrics["failed_extension_decay"],
        metrics["short_compression"],
        metrics["adverse_wick_rejection"],
        metrics["participation_turn"],
        metrics["path_efficiency_loss"],
        metrics["prior_day_contrarian_context"],
        metrics["session_reversion_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        displacement_stretch_bucket(metrics["equilibrium_displacement_stretch"]),
        equilibrium_reclaim_bucket(metrics["equilibrium_reclaim_strength"]),
        failed_extension_decay_bucket(metrics["failed_extension_decay"]),
        range_compression_bucket(metrics["short_compression"]),
        boundary_wick_bucket(metrics["adverse_wick_rejection"]),
        volume_confirmation_bucket(metrics["participation_turn"]),
        path_efficiency_loss_bucket(metrics["path_efficiency_loss"]),
        prior_day_contrarian_bucket(metrics["prior_day_contrarian_context"]),
        session_reversion_bucket(metrics["session_reversion_pressure"]),
...
def local_equilibrium_displacement_reversion_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    short_start = index - LOCAL_EQUILIBRIUM_DISPLACEMENT_REVERSION_SHORT_LOOKBACK_BARS + 1
    prior_start = index - LOCAL_EQUILIBRIUM_DISPLACEMENT_REVERSION_LOOKBACK_BARS + 1
    ...
    equilibrium = volume_weighted_close(prior_window, volumes[prior_start : index + 1])
    ...
    short_compression = max(0.0, 1.0 - short_width / prior_width)
    if direction > 0:
        displacement_stretch = (equilibrium - short_low) / average_range
        current_distance = (equilibrium - signal_bar.close) / average_range
        prior_distance = (equilibrium - bars[index - 1].close) / average_range
    else:
        displacement_stretch = (short_high - equilibrium) / average_range
        current_distance = (signal_bar.close - equilibrium) / average_range
        prior_distance = (bars[index - 1].close - equilibrium) / average_range
    equilibrium_reclaim_strength = prior_distance - current_distance
    failed_extension_decay = displacement_stretch - max(current_distance, 0.0)
    adverse_wick_rejection = directional_wick_rejection(signal_bar, direction)
    path_efficiency = directional_path_efficiency(bars, index, direction, LOCAL_EQUILIBRIUM_DISPLACEMENT_REVERSION_SHORT_LOOKBACK_BARS)
    path_efficiency_loss = 1.0 - abs(path_efficiency or 0.0)
    ...
    participation_turn = (current_volume_ratio - early_volume_ratio) * max(directional_body_score(signal_bar, direction), 0.0)
    prior_day_contrarian_context = -direction * prior_day_return / max(prior_day_range, 1e-9)
    reversion_pressure = max(
        max(displacement_stretch, 0.0) / 2.0,
        max(equilibrium_reclaim_strength, 0.0),
        max(failed_extension_decay, 0.0) / 2.0,
        max(short_compression, 0.0),
        max(adverse_wick_rejection, 0.0),
        max(path_efficiency_loss - 0.25, 0.0),
        min(max(participation_turn, 0.0), 1.0),
    )
    session_reversion_pressure = bounded_session_progress(signal_bar.time) * reversion_pressure
    activity = max(reversion_pressure, session_reversion_pressure)
    if activity < MIN_LOCAL_EQUILIBRIUM_DISPLACEMENT_REVERSION_ACTIVITY:
        return {"event_count": 0.0}
    if displacement_stretch < 0.18:
        return {"event_count": 0.0}
    if equilibrium_reclaim_strength < -0.04:
        return {"event_count": 0.0}
    if failed_extension_decay < 0.05 and adverse_wick_rejection < -0.10:
        return {"event_count": 0.0}
    if path_efficiency_loss < 0.18 and short_compression < 0.04:
        return {"event_count": 0.0}
    return {
        "equilibrium_displacement_stretch": bounded(displacement_stretch, -3.0, 8.0),
        "equilibrium_reclaim_strength": bounded(equilibrium_reclaim_strength, -3.0, 6.0),
        "failed_extension_decay": bounded(failed_extension_decay, -3.0, 8.0),
        "short_compression": bounded(short_compression, 0.0, 1.0),
        "adverse_wick_rejection": bounded(adverse_wick_rejection, -1.0, 1.0),
        "participation_turn": bounded(participation_turn, -3.0, 6.0),
        ...
        "session_reversion_pressure": bounded(session_reversion_pressure, 0.0, 4.0),
        "event_count": 1.0,
    }
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    stretch_reward = 0.14 if metrics["equilibrium_displacement_stretch"] > 0.35 and terminal > 0 else 0.0
    reclaim_reward = 0.14 if metrics["equilibrium_reclaim_strength"] > 0.08 and terminal > 0 else 0.0
    decay_reward = 0.12 if metrics["failed_extension_decay"] > 0.20 and terminal > 0 else 0.0
    compression_reward = 0.09 if metrics["short_compression"] > 0.10 and terminal > 0 else 0.0
    wick_reward = 0.10 if metrics["adverse_wick_rejection"] > 0.06 and terminal > 0 else 0.0
    participation_reward = 0.08 if metrics["participation_turn"] > 0.02 and terminal > 0 else 0.0
    efficiency_loss_reward = 0.08 if metrics["path_efficiency_loss"] > 0.40 and terminal > 0 else 0.0
    prior_context_reward = 0.06 if metrics["prior_day_contrarian_context"] > -0.20 and terminal > 0 else 0.0
    session_reward = 0.05 if metrics["session_reversion_pressure"] > 0.10 and terminal > 0 else 0.0
    weak_stretch_penalty = 0.12 if metrics["equilibrium_displacement_stretch"] < 0.22 and terminal <= 0 else 0.0
    adverse_reclaim_penalty = 0.12 if metrics["equilibrium_reclaim_strength"] < -0.08 and terminal <= 0 else 0.0
    weak_decay_penalty = 0.10 if metrics["failed_extension_decay"] < 0.08 and terminal <= 0 else 0.0
    loose_compression_penalty = 0.08 if metrics["short_compression"] < 0.04 and terminal <= 0 else 0.0
    weak_wick_penalty = 0.08 if metrics["adverse_wick_rejection"] < -0.08 and terminal <= 0 else 0.0
    no_participation_penalty = 0.08 if metrics["participation_turn"] < -0.08 and terminal <= 0 else 0.0
    efficient_continuation_penalty = 0.09 if metrics["path_efficiency_loss"] < 0.18 and terminal <= 0 else 0.0
    ...
    local_equilibrium_displacement_reversion_path_quality = (0.48 * terminal + 0.32 * mfe - 0.72 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + local_equilibrium_displacement_reversion_path_quality
        + stretch_reward
        + reclaim_reward
        + decay_reward
        + compression_reward
        ...
        + efficiency_loss_reward
        + prior_context_reward
        + session_reward
        - weak_stretch_penalty
        - adverse_reclaim_penalty
        - weak_decay_penalty
        - loose_compression_penalty
        - weak_wick_penalty
        - no_participation_penalty
        - efficient_continuation_penalty
        - stall_penalty
        - unfavorable_path_penalty
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["equilibrium_displacement_stretch"] - 3.20) * 0.04
    )


def fit_linear_local_equilibrium_displacement_reversion_model(candidates: list[base.Candidate], fold_id: str) -> LinearLocalEquilibriumDisplacementReversionModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearLocalEquilibriumDisplacementReversionModel(
            fold_id,
    ...
    local_equilibrium_displacement_reversion_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearLocalEquilibriumDisplacementReversionModel(
        fold_id=fold_id,
        ...
        local_equilibrium_displacement_reversion_failure_rate=local_equilibrium_displacement_reversion_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearLocalEquilibriumDisplacementReversionModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_local_equilibrium_displacement_reversion_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0084_r0001_intraday_local_equilibrium_displacement_reversion",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0084_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0084_r0001_local_equilibrium_reversion_summary.json",
        ],
        ...
        "conditional_profiles": {
            "local_equilibrium_displacement_reversion_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0084 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0084_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0084 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "local_equilibrium_displacement_reversion_lookback_bars": LOCAL_EQUILIBRIUM_DISPLACEMENT_REVERSION_LOOKBACK_BARS,
        "local_equilibrium_displacement_reversion_short_lookback_bars": LOCAL_EQUILIBRIUM_DISPLACEMENT_REVERSION_SHORT_LOOKBACK_BARS,
        "min_local_equilibrium_displacement_reversion_activity": MIN_LOCAL_EQUILIBRIUM_DISPLACEMENT_REVERSION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_local_equilibrium_displacement_reversion_model_summary(model: LinearLocalEquilibriumDisplacementReversionModel) -> dict[str, object]:
    return {
        ...
        "local_equilibrium_displacement_reversion_failure_rate": rounded(model.local_equilibrium_displacement_reversion_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearLocalEquilibriumDisplacementReversionModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_local_equilibrium_displacement_reversion_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "local_equilibrium_displacement_reversion_profile": profiles["local_equilibrium_displacement_reversion_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0084_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0084_r0001_proxy_trades.csv"
    evidence["local_equilibrium_displacement_reversion_summary"] = "artifacts/c0084_r0001_local_equilibrium_reversion_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0084_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0084_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0084 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0084_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0084_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0084_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0084_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0084_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0084_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0084_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0084_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0084_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0084 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0084 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0084_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0084_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0084_r0001_intraday_local_equilibrium_displacement_reversion_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0084_r0001_proxy_records_local_equilibrium_displacement_reversion_reference_surface
  ...
  - next_work_is_produce_c0084_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0084_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0084-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0084_r0001_intraday_local_equilibrium_displacement_reversion",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0084-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0084-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0084-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0084_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0084_r0001_intraday_local_equilibrium_displacement_reversion",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0084-R0001-local-equilibrium-displacement-reversion-SUMMARY",
            "artifact_role": "local_equilibrium_displacement_reversion_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0084_r0001_local_equilibrium_reversion_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0084_r0001_intraday_local_equilibrium_displacement_reversion",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0084_r0001_mt5_logic_parity_evidence",
        }
    ]
