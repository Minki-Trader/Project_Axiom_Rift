"""C0081 R0001 proxy evidence for intraday failed acceleration absorption."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0081_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0081_r0001_faa_summary.json"
...
FEATURE_NAMES = (
    ...
    "followthrough_decay",
    "opposing_wick_absorption",
    "close_back_inside_pressure",
    "failed_extension_body_ratio",
    "range_extension_saturation",
    "volume_climax_absorption",
    "prior_day_exhaustion_context",
    "session_reversal_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_failed_acceleration_absorption_rank"
LABEL_SHAPE = "target_first_reversal_quality_conditioned_on_failed_acceleration_absorption_state"
SELECTION_RULE = "top_fold_local_failed_acceleration_absorption_scores_per_active_day"
...
MIN_DAY_BARS = 30
FAILED_ACCELERATION_ABSORPTION_LOOKBACK_BARS = 18
FAILED_ACCELERATION_ABSORPTION_SHORT_LOOKBACK_BARS = 6
MIN_FAILED_ACCELERATION_ABSORPTION_ACTIVITY = 0.18
...
class LinearFailedAccelerationAbsorptionModel:
    fold_id: str
    ...
    failed_acceleration_absorption_failure_rate: float
...
def run_c0081_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_failed_acceleration_absorption_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_failed_acceleration_absorption_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        FAILED_ACCELERATION_ABSORPTION_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    release = failed_acceleration_absorption_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        release["acceleration_range_pressure"],
        release["followthrough_decay"],
        release["opposing_wick_absorption"],
        release["close_back_inside_pressure"],
        release["failed_extension_body_ratio"],
        release["range_extension_saturation"],
        release["volume_climax_absorption"],
        release["prior_day_exhaustion_context"],
        release["session_reversal_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        acceleration_pressure_bucket(release["acceleration_range_pressure"]),
        followthrough_decay_bucket(release["followthrough_decay"]),
        wick_bucket(release["opposing_wick_absorption"]),
        close_commitment_bucket(release["close_back_inside_pressure"]),
        body_bucket(release["failed_extension_body_ratio"]),
        saturation_bucket(release["range_extension_saturation"]),
        volume_confirmation_bucket(release["volume_climax_absorption"]),
        prior_direction_context_bucket(release["prior_day_exhaustion_context"]),
        session_pressure_bucket(release["session_reversal_pressure"]),
...
def failed_acceleration_absorption_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    short_start = index - FAILED_ACCELERATION_ABSORPTION_SHORT_LOOKBACK_BARS + 1
    prior_start = index - FAILED_ACCELERATION_ABSORPTION_LOOKBACK_BARS + 1
    ...
    prior_width = max(max(bar.high for bar in prior_window) - min(bar.low for bar in prior_window), 1e-9)
    impulse_direction = -direction
    acceleration_range_pressure = impulse_direction * (signal_bar.open - prior_window[0].open) / average_range
    prior_impulse_body = (
        base.mean([max(directional_body_score(bar, impulse_direction), 0.0) for bar in prior_window[:-2]]) or 0.0
    )
    recent_impulse_body = (
        base.mean([max(directional_body_score(bar, impulse_direction), 0.0) for bar in short_window[:-1]]) or 0.0
    )
    current_impulse_body = max(directional_body_score(signal_bar, impulse_direction), 0.0)
    current_reversal_body = max(directional_body_score(signal_bar, direction), 0.0)
    followthrough_decay = max(prior_impulse_body - current_impulse_body, recent_impulse_body - current_impulse_body, 0.0)
    opposing_wick_absorption = directional_wick_rejection(signal_bar, direction)
    close_back_inside_pressure = directional_bar_settlement(signal_bar, direction)
    failed_extension_body_ratio = followthrough_decay * (1.0 + current_reversal_body)
    if direction > 0:
        range_extension_saturation = (max(bar.high for bar in prior_window) - signal_bar.close) / prior_width
    else:
        range_extension_saturation = (signal_bar.close - min(bar.low for bar in prior_window)) / prior_width
    ...
    volume_climax_absorption = max(current_volume_ratio - min(early_volume_ratio, current_volume_ratio), 0.0) * max(
        opposing_wick_absorption + current_reversal_body,
        0.0,
    )
    prior_day_exhaustion_context = -direction * prior_day_return / max(prior_day_range, 1e-9)
    reversal_pressure = max(
        max(acceleration_range_pressure, 0.0) / 3.0,
        max(followthrough_decay, 0.0),
        max(opposing_wick_absorption, 0.0),
        max(close_back_inside_pressure - 0.40, 0.0),
        max(failed_extension_body_ratio, 0.0),
        min(max(range_extension_saturation, 0.0), 1.0),
        min(max(volume_climax_absorption, 0.0) / 2.0, 1.0),
    )
    session_reversal_pressure = bounded_session_progress(signal_bar.time) * reversal_pressure
    activity = max(reversal_pressure, session_reversal_pressure)
    if activity < MIN_FAILED_ACCELERATION_ABSORPTION_ACTIVITY:
        return {"event_count": 0.0}
    if acceleration_range_pressure < 0.12 or range_extension_saturation < 0.20:
        return {"event_count": 0.0}
    if close_back_inside_pressure < 0.25 and opposing_wick_absorption < -0.04:
        return {"event_count": 0.0}
    return {
        "acceleration_range_pressure": bounded(acceleration_range_pressure, -3.0, 8.0),
        "followthrough_decay": bounded(followthrough_decay, 0.0, 2.0),
        "opposing_wick_absorption": bounded(opposing_wick_absorption, -1.0, 1.0),
        "close_back_inside_pressure": bounded(close_back_inside_pressure, 0.0, 1.0),
        "failed_extension_body_ratio": bounded(failed_extension_body_ratio, 0.0, 3.0),
        "range_extension_saturation": bounded(range_extension_saturation, 0.0, 1.0),
        "volume_climax_absorption": bounded(volume_climax_absorption, 0.0, 6.0),
        "prior_day_exhaustion_context": bounded(prior_day_exhaustion_context, -2.0, 2.0),
        "session_reversal_pressure": bounded(session_reversal_pressure, 0.0, 4.0),
        "event_count": 1.0,
    }
...
def candidate_label(
    ...
    metrics = failed_acceleration_absorption_metrics(
        bars,
        volumes,
    ...
    if metrics["event_count"] <= 0:
        return None
    acceleration_reward = 0.15 if metrics["acceleration_range_pressure"] > 0.45 and terminal > 0 else 0.0
    decay_reward = 0.13 if metrics["followthrough_decay"] > 0.10 and terminal > 0 else 0.0
    absorption_reward = 0.14 if metrics["opposing_wick_absorption"] > 0.08 and terminal > 0 else 0.0
    close_inside_reward = 0.13 if metrics["close_back_inside_pressure"] > 0.52 and terminal > 0 else 0.0
    failed_extension_reward = 0.12 if metrics["failed_extension_body_ratio"] > 0.08 and terminal > 0 else 0.0
    saturation_reward = 0.10 if metrics["range_extension_saturation"] > 0.45 and terminal > 0 else 0.0
    volume_reward = 0.08 if metrics["volume_climax_absorption"] > 0.06 and terminal > 0 else 0.0
    prior_context_reward = 0.06 if metrics["prior_day_exhaustion_context"] > -0.20 and terminal > 0 else 0.0
    weak_acceleration_penalty = 0.12 if metrics["acceleration_range_pressure"] < 0.18 and terminal <= 0 else 0.0
    no_decay_penalty = 0.12 if metrics["followthrough_decay"] < 0.04 and terminal <= 0 else 0.0
    wick_against_penalty = 0.12 if metrics["opposing_wick_absorption"] < -0.08 and terminal <= 0 else 0.0
    weak_close_inside_penalty = 0.12 if metrics["close_back_inside_pressure"] < 0.30 and terminal <= 0 else 0.0
    weak_failed_extension_penalty = 0.10 if metrics["failed_extension_body_ratio"] < 0.03 and terminal <= 0 else 0.0
    low_saturation_penalty = 0.10 if metrics["range_extension_saturation"] < 0.28 and terminal <= 0 else 0.0
    no_volume_absorption_penalty = 0.08 if metrics["volume_climax_absorption"] < 0.02 and terminal <= 0 else 0.0
    ...
    failed_acceleration_absorption_path_quality = (0.46 * terminal + 0.34 * mfe - 0.74 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + failed_acceleration_absorption_path_quality
        + acceleration_reward
        + decay_reward
        + absorption_reward
        + close_inside_reward
        + failed_extension_reward
        + saturation_reward
        + volume_reward
        ...
        - weak_close_inside_penalty
        - weak_failed_extension_penalty
        - low_saturation_penalty
        - no_volume_absorption_penalty
        - stall_penalty
        - unfavorable_path_penalty
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["acceleration_range_pressure"] - 3.20) * 0.04
    )


def fit_linear_failed_acceleration_absorption_model(candidates: list[base.Candidate], fold_id: str) -> LinearFailedAccelerationAbsorptionModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearFailedAccelerationAbsorptionModel(
            fold_id,
    ...
    failed_acceleration_absorption_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearFailedAccelerationAbsorptionModel(
        fold_id=fold_id,
        ...
        failed_acceleration_absorption_failure_rate=failed_acceleration_absorption_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearFailedAccelerationAbsorptionModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_failed_acceleration_absorption_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0081_r0001_intraday_failed_acceleration_absorption",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0081_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0081_r0001_faa_summary.json",
        ],
        ...
        "conditional_profiles": {
            "failed_acceleration_absorption_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0081 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0081_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0081 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "failed_acceleration_absorption_lookback_bars": FAILED_ACCELERATION_ABSORPTION_LOOKBACK_BARS,
        "failed_acceleration_absorption_short_lookback_bars": FAILED_ACCELERATION_ABSORPTION_SHORT_LOOKBACK_BARS,
        "min_failed_acceleration_absorption_activity": MIN_FAILED_ACCELERATION_ABSORPTION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_failed_acceleration_absorption_model_summary(model: LinearFailedAccelerationAbsorptionModel) -> dict[str, object]:
    return {
        ...
        "failed_acceleration_absorption_failure_rate": rounded(model.failed_acceleration_absorption_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearFailedAccelerationAbsorptionModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_failed_acceleration_absorption_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "failed_acceleration_absorption_profile": profiles["failed_acceleration_absorption_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0081_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0081_r0001_proxy_trades.csv"
    evidence["failed_acceleration_absorption_summary"] = "artifacts/c0081_r0001_faa_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0081_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0081_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0081 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0081_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0081_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0081_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0081_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0081_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0081_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0081_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0081_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0081_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0081 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0081 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0081_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0081_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0081_r0001_intraday_failed_acceleration_absorption_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0081_r0001_proxy_records_failed_acceleration_absorption_reference_surface
  ...
  - next_work_is_produce_c0081_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0081_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0081-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0081_r0001_intraday_failed_acceleration_absorption",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0081-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0081-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0081-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0081_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0081_r0001_intraday_failed_acceleration_absorption",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0081-R0001-failed-acceleration-absorption-SUMMARY",
            "artifact_role": "failed_acceleration_absorption_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0081_r0001_faa_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0081_r0001_intraday_failed_acceleration_absorption",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0081_r0001_mt5_logic_parity_evidence",
        }
    ]
