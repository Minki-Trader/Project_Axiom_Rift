"""C0143 R0001 proxy evidence for intraday value migration pullback continuation."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0143_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0143_r0001_value_migration_pullback_continuation_summary.json"
...
FEATURE_NAMES = (
    "value_migration_slope",
    "vwap_persistence",
    "close_location_consensus",
    "pullback_shallowness",
    "pullback_absorption",
    "counter_rotation_decay",
    "trend_orderliness",
    "range_progress_efficiency",
    "participation_consistency",
    "day_open_tailwind",
    "prior_close_tailwind",
    "prior_day_acceptance_tailwind",
    "session_room_after_pullback",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_value_migration_pullback_continuation_rank"
LABEL_SHAPE = "target_first_quality_after_value_migration_pullback_continuation"
SELECTION_RULE = "top_fold_local_value_migration_pullback_continuation_scores_per_active_day"
...
MIN_DAY_BARS = 18
VALUE_MIGRATION_PULLBACK_CONTINUATION_LOOKBACK_BARS = 78
VALUE_MIGRATION_PULLBACK_CONTINUATION_MICRO_BARS = 16
VALUE_MIGRATION_PULLBACK_CONTINUATION_PULLBACK_BARS = 7
MIN_VALUE_MIGRATION_SLOPE = -0.02
MIN_PULLBACK_ABSORPTION = 0.20
MIN_TREND_ORDERLINESS = 0.05
MIN_VALUE_MIGRATION_PULLBACK_CONTINUATION_ACTIVITY = 0.025
...
class LinearValueMigrationPullbackContinuationModel:
    fold_id: str
    ...
    value_migration_pullback_continuation_failure_rate: float
...
def run_c0143_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_value_migration_pullback_continuation_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_value_migration_pullback_continuation_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        VALUE_MIGRATION_PULLBACK_CONTINUATION_LOOKBACK_BARS,
        VALUE_MIGRATION_PULLBACK_CONTINUATION_MICRO_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = value_migration_pullback_continuation_metrics(
    ...
    features = (
        metrics["value_migration_slope"],
        metrics["vwap_persistence"],
        metrics["close_location_consensus"],
        metrics["pullback_shallowness"],
        metrics["pullback_absorption"],
        metrics["counter_rotation_decay"],
        metrics["trend_orderliness"],
        metrics["range_progress_efficiency"],
        metrics["participation_consistency"],
        metrics["day_open_tailwind"],
        metrics["prior_close_tailwind"],
        metrics["prior_day_acceptance_tailwind"],
        metrics["session_room_after_pullback"],
        metrics["session_maturity"],
        metrics["spread_stress_inverse"],
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        impulse_response_bucket(metrics["value_migration_slope"]),
        vwap_side_confirm_bucket(metrics["vwap_persistence"]),
        close_drive_bucket(metrics["close_location_consensus"]),
        range_compression_bucket(metrics["pullback_shallowness"]),
        adverse_wick_control_bucket(metrics["pullback_absorption"]),
        path_tortuosity_bucket(metrics["counter_rotation_decay"]),
        drift_efficiency_bucket(metrics["trend_orderliness"]),
        range_expansion_bucket(metrics["range_progress_efficiency"]),
        volume_stability_bucket(metrics["participation_consistency"]),
        day_open_tension_bucket(metrics["day_open_tailwind"]),
        prior_direction_context_bucket(metrics["prior_close_tailwind"]),
        prior_day_range_alignment_bucket(metrics["prior_day_acceptance_tailwind"]),
        session_pressure_bucket(metrics["session_room_after_pullback"]),
...
def value_migration_pullback_continuation_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    pullback_start = max(index - VALUE_MIGRATION_PULLBACK_CONTINUATION_PULLBACK_BARS + 1, context.day_start_index)
    micro_start = max(index - VALUE_MIGRATION_PULLBACK_CONTINUATION_MICRO_BARS + 1, context.day_start_index)
    lookback_start = max(index - VALUE_MIGRATION_PULLBACK_CONTINUATION_LOOKBACK_BARS + 1, context.day_start_index)
    if pullback_start < 0 or micro_start < 0 or lookback_start < 0:
        return {"event_count": 0.0}
    pullback_window = bars[pullback_start : index + 1]
    ...
    if pullback_range_mean <= 0 or micro_range_mean <= 0 or lookback_range_mean <= 0:
        return {"event_count": 0.0}
    ...
    lookback_span = max(lookback_high - lookback_low, 1e-9)

    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    prior_close_anchor = context.prior_day_close if context.prior_day_close is not None else context.day_open
    directional_position = directional_session_position(signal_bar.close, context, direction)
    if directional_position is None:
        return {"event_count": 0.0}

    lookback_start_close = lookback_window[0].close
    micro_start_close = micro_window[0].close
    value_migration_slope = direction * (signal_bar.close - lookback_start_close) / max(
        average_range * math.sqrt(len(lookback_window)),
        1e-9,
    )
    vwap_persistence = direction * (signal_bar.close - session_vwap) / max(average_range, 1e-9)
    day_open_tailwind = direction * (signal_bar.close - context.day_open) / max(average_range, 1e-9)
    prior_close_tailwind = direction * (signal_bar.close - prior_close_anchor) / max(average_range, 1e-9)
    close_location_consensus = base.mean([directional_bar_settlement(bar, direction) for bar in micro_window]) or 0.0
    trend_orderliness = directional_path_efficiency(
        bars,
        index,
        direction,
        min(VALUE_MIGRATION_PULLBACK_CONTINUATION_LOOKBACK_BARS // 2, index),
    ) or 0.0
    micro_progress = direction * (signal_bar.close - micro_start_close) / max(average_range, 1e-9)
    range_progress_efficiency = micro_progress / max(micro_range_mean / max(average_range, 1e-9), 1e-9)

    adverse_pullback = 0.0
    if direction > 0:
        adverse_pullback = max(0.0, max(bar.high for bar in pullback_window[:-1]) - min(bar.low for bar in pullback_window))
        room_to_prior_day_extreme = (
            (context.prior_day_high - signal_bar.close) / max(average_range, 1e-9)
            if context.prior_day_high is not None
            else 0.0
        )
        session_room_after_pullback = max(
            (context.high_so_far - signal_bar.close) / max(average_range, 1e-9),
            room_to_prior_day_extreme,
        )
    else:
        adverse_pullback = max(0.0, max(bar.high for bar in pullback_window) - min(bar.low for bar in pullback_window[:-1]))
        room_to_prior_day_extreme = (
            (signal_bar.close - context.prior_day_low) / max(average_range, 1e-9)
            if context.prior_day_low is not None
            else 0.0
        )
        session_room_after_pullback = max(
            (signal_bar.close - context.low_so_far) / max(average_range, 1e-9),
            room_to_prior_day_extreme,
        )
    pullback_shallowness = 1.0 - adverse_pullback / max(lookback_span, average_range, 1e-9)
    pullback_absorption = adverse_wick_absorption(signal_bar, direction)
    previous_progress = direction * (previous_bar.close - micro_start_close) / max(average_range, 1e-9)
    counter_rotation_decay = micro_progress - previous_progress + pullback_shallowness
    current_volume_ratio = volumes[index] / max(average_volume, 1e-9)
    pullback_volume_ratio = pullback_volume_mean / max(average_volume, 1e-9)
    micro_volume_ratio = micro_volume_mean / max(average_volume, 1e-9)
    participation_consistency = 1.0 - abs(current_volume_ratio - max(pullback_volume_ratio, micro_volume_ratio))
    prior_day_acceptance_tailwind = 0.0
    if context.prior_day_range is not None and context.prior_day_range > 0:
        prior_day_acceptance_tailwind = direction * (signal_bar.close - prior_close_anchor) / context.prior_day_range

    if value_migration_slope < MIN_VALUE_MIGRATION_SLOPE and trend_orderliness < MIN_TREND_ORDERLINESS:
        return {"event_count": 0.0}
    if pullback_absorption < MIN_PULLBACK_ABSORPTION and pullback_shallowness < 0.20:
        return {"event_count": 0.0}
    if close_location_consensus < 0.42 and range_progress_efficiency < 0.05:
        return {"event_count": 0.0}
    if directional_position < 0.48 and vwap_persistence < -0.18:
        return {"event_count": 0.0}
    ...
    activity = (
        max(value_migration_slope, -0.12) * 0.12
        + max(vwap_persistence, -0.28) * 0.10
        + max(close_location_consensus - 0.44, -0.18) * 0.12
        + max(pullback_shallowness - 0.22, -0.22) * 0.08
        + max(pullback_absorption - 0.24, -0.20) * 0.09
        + max(counter_rotation_decay, -0.45) * 0.08
        + max(trend_orderliness, -0.35) * 0.11
        + max(range_progress_efficiency, -0.40) * 0.09
        + max(participation_consistency - 0.30, -0.28) * 0.05
        + max(day_open_tailwind, -0.35) * 0.06
        + max(prior_close_tailwind, -0.40) * 0.05
        + max(prior_day_acceptance_tailwind, -0.35) * 0.04
        + max(session_room_after_pullback, -0.25) * 0.03
        + max(directional_position - 0.48, -0.18) * 0.08
    )
    if activity < MIN_VALUE_MIGRATION_PULLBACK_CONTINUATION_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "value_migration_slope": bounded(value_migration_slope, -4.0, 4.0),
        "vwap_persistence": bounded(vwap_persistence, -5.0, 5.0),
        "close_location_consensus": bounded(close_location_consensus, 0.0, 1.0),
        "pullback_shallowness": bounded(pullback_shallowness, -2.0, 2.0),
        "pullback_absorption": bounded(pullback_absorption, 0.0, 1.0),
        "counter_rotation_decay": bounded(counter_rotation_decay, -5.0, 5.0),
        "trend_orderliness": bounded(trend_orderliness, -1.0, 1.0),
        "range_progress_efficiency": bounded(range_progress_efficiency, -4.0, 4.0),
        "participation_consistency": bounded(participation_consistency, -6.0, 2.0),
        "day_open_tailwind": bounded(day_open_tailwind, -5.0, 5.0),
        "prior_close_tailwind": bounded(prior_close_tailwind, -5.0, 5.0),
        "prior_day_acceptance_tailwind": bounded(prior_day_acceptance_tailwind, -3.0, 3.0),
        "session_room_after_pullback": bounded(session_room_after_pullback, -3.0, 8.0),
        ...
        "release_side_position": bounded(directional_position, 0.0, 1.0),
        ...
        "reference_level": rounded(session_vwap) or 0.0,
        "micro_net": bounded(direction * (signal_bar.close - prior_micro[0].close) / max(average_range, 1e-9), -4.0, 4.0),
        "value_migration_pullback_continuation_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def C0143_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def C0143_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def C0143_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def C0143_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def C0143_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def C0143_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def C0143_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def C0143_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def C0143_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def C0143_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def C0143_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def C0143_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def C0143_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def C0143_pullback_containment_bucket(value: float) -> str:
    if value < 0.35:
        return "pullback_containment_weak"
    if value < 0.68:
        return "pullback_containment_partial"
    return "pullback_containment_full"


def C0143_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def C0143_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def C0143_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def C0143_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def C0143_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def C0143_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def compression_bucket(value: float) -> str:
    if value < -0.08:
        return "tempo_expanding"
    if value < 0.18:
        return "tempo_neutral"
    return "tempo_contracting"


def trend_pressure_bucket(value: float) -> str:
    if value < -0.08:
        return "drift_against"
    if value < 0.18:
        return "drift_mixed"
    return "drift_with"


def impulse_response_bucket(value: float) -> str:
    if value < -0.08:
        return "acceleration_fading"
    if value < 0.16:
        return "acceleration_mixed"
    return "acceleration_active"


def persistence_bucket(value: float) -> str:
    if value < 0.35:
        return "close_persistence_low"
    if value < 0.70:
        return "close_persistence_mid"
    return "close_persistence_high"


def volume_alignment_bucket(value: float) -> str:
    if value < -0.10:
        return "volume_phase_against"
    if value < 0.35:
        return "volume_phase_mixed"
    return "volume_phase_aligned"


def opening_anchor_bucket(value: float) -> str:
    if value < -0.20:
        return "opening_anchor_against"
    if value < 0.35:
        return "opening_anchor_neutral"
    return "opening_anchor_with"
...
def prior_participation_quieting_bucket(value: float) -> str:
    if value < 0.04:
        return "prior_participation_quieting_absent"
    if value < 0.18:
        return "prior_participation_quieting_defined"
    return "prior_participation_quieting_deep"
...
def anchor_braid_tightness_bucket(value: float) -> str:
    if value < -0.30:
        return "anchor_braid_wide"
    if value < 0.35:
        return "anchor_braid_defined"
    return "anchor_braid_tight"


def anchor_escape_impulse_bucket(value: float) -> str:
    if value < -0.05:
        return "anchor_escape_failed"
    if value < 0.22:
        return "anchor_escape_probe"
    return "anchor_escape_committed"


def prior_anchor_bucket(value: float) -> str:
    if value < -0.30:
        return "prior_anchor_against_escape"
    if value < 0.25:
        return "prior_anchor_neutral"
    return "prior_anchor_with_escape"
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    migration_reward = 0.10 if metrics["value_migration_slope"] > 0.04 and terminal > 0 else 0.0
    vwap_reward = 0.09 if metrics["vwap_persistence"] > -0.04 and terminal > 0 else 0.0
    close_consensus_reward = 0.08 if metrics["close_location_consensus"] > 0.52 and terminal > 0 else 0.0
    shallow_pullback_reward = 0.08 if metrics["pullback_shallowness"] > 0.28 and terminal > 0 else 0.0
    absorption_reward = 0.08 if metrics["pullback_absorption"] > 0.34 and terminal > 0 else 0.0
    rotation_decay_reward = 0.07 if metrics["counter_rotation_decay"] > 0.05 and terminal > 0 else 0.0
    trend_reward = 0.10 if metrics["trend_orderliness"] > 0.12 and terminal > 0 else 0.0
    progress_reward = 0.08 if metrics["range_progress_efficiency"] > 0.10 and terminal > 0 else 0.0
    participation_reward = 0.05 if metrics["participation_consistency"] > 0.18 and terminal > 0 else 0.0
    open_reward = 0.06 if metrics["day_open_tailwind"] > -0.10 and terminal > 0 else 0.0
    prior_close_reward = 0.05 if metrics["prior_close_tailwind"] > -0.14 and terminal > 0 else 0.0
    prior_day_reward = 0.04 if metrics["prior_day_acceptance_tailwind"] > -0.18 and terminal > 0 else 0.0
    room_reward = 0.04 if metrics["session_room_after_pullback"] > 0.08 and terminal > 0 else 0.0
    weak_migration_penalty = 0.07 if metrics["value_migration_slope"] < -0.05 and terminal <= 0 else 0.0
    weak_vwap_penalty = 0.06 if metrics["vwap_persistence"] < -0.18 and terminal <= 0 else 0.0
    weak_consensus_penalty = 0.06 if metrics["close_location_consensus"] < 0.38 and terminal <= 0 else 0.0
    deep_pullback_penalty = 0.07 if metrics["pullback_shallowness"] < 0.05 and terminal <= 0 else 0.0
    weak_absorption_penalty = 0.06 if metrics["pullback_absorption"] < 0.18 and terminal <= 0 else 0.0
    weak_trend_penalty = 0.08 if metrics["trend_orderliness"] < -0.08 and terminal <= 0 else 0.0
    weak_progress_penalty = 0.06 if metrics["range_progress_efficiency"] < -0.10 and terminal <= 0 else 0.0
    no_participation_penalty = 0.05 if metrics["participation_consistency"] < -0.45 and terminal <= 0 else 0.0
    no_room_penalty = 0.04 if metrics["session_room_after_pullback"] < 0.02 and terminal <= 0 else 0.0
    ...
    return (
        ...
        + migration_reward
        + vwap_reward
        + close_consensus_reward
        + shallow_pullback_reward
        + absorption_reward
        + rotation_decay_reward
        + trend_reward
        + progress_reward
        ...
        - max(0.0, -metrics["participation_consistency"] - 0.75) * 0.015
        - max(0.0, MIN_VALUE_MIGRATION_SLOPE - metrics["value_migration_slope"]) * 0.04
        - max(0.0, MIN_TREND_ORDERLINESS - metrics["trend_orderliness"]) * 0.06
        - max(0.0, MIN_PULLBACK_ABSORPTION - metrics["pullback_absorption"]) * 0.05
    )


def fit_linear_value_migration_pullback_continuation_model(candidates: list[base.Candidate], fold_id: str) -> LinearValueMigrationPullbackContinuationModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearValueMigrationPullbackContinuationModel(
            fold_id,
    ...
    value_migration_pullback_continuation_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearValueMigrationPullbackContinuationModel(
        fold_id=fold_id,
        ...
        value_migration_pullback_continuation_failure_rate=value_migration_pullback_continuation_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearValueMigrationPullbackContinuationModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_value_migration_pullback_continuation_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0143_r0001_intraday_value_migration_pullback_continuation",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0143_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0143_r0001_value_migration_pullback_continuation_summary.json",
        ],
        ...
        "conditional_profiles": {
            "value_migration_pullback_continuation_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0143 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0143_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0143 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "value_migration_pullback_continuation_lookback_bars": VALUE_MIGRATION_PULLBACK_CONTINUATION_LOOKBACK_BARS,
        "value_migration_pullback_continuation_micro_bars": VALUE_MIGRATION_PULLBACK_CONTINUATION_MICRO_BARS,
        "pullback_bars": VALUE_MIGRATION_PULLBACK_CONTINUATION_PULLBACK_BARS,
        "min_value_migration_slope": MIN_VALUE_MIGRATION_SLOPE,
        "min_pullback_absorption": MIN_PULLBACK_ABSORPTION,
        "min_trend_orderliness": MIN_TREND_ORDERLINESS,
        "min_value_migration_pullback_continuation_activity": MIN_VALUE_MIGRATION_PULLBACK_CONTINUATION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_value_migration_pullback_continuation_model_summary(model: LinearValueMigrationPullbackContinuationModel) -> dict[str, object]:
    return {
        ...
        "value_migration_pullback_continuation_failure_rate": rounded(model.value_migration_pullback_continuation_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearValueMigrationPullbackContinuationModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_value_migration_pullback_continuation_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "value_migration_pullback_continuation_profile": profiles["value_migration_pullback_continuation_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["next_required_action"] = "produce_c0143_r0001_mt5_logic_parity_evidence"
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0143_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0143_r0001_proxy_trades.csv"
    evidence["value_migration_pullback_continuation_summary"] = "artifacts/c0143_r0001_value_migration_pullback_continuation_summary.json"
...
def update_gate_report() -> None:
    ...
    data["gate_status"] = "proxy_recorded_pending_mt5"
    data["required_kpi_status"] = "proxy_recorded_pending_mt5"
    ...
    data["next_action"] = "produce_c0143_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0143_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0143 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0143_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    next_work["tasks"] = ["produce_c0143_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0143_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0143_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0143_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0143_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0143_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0143_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0143 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0143 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    block = f"""
- decision_id: dec_20260708_produce_c0143_r0001_proxy_evidence
  created_local_date: '{local_date}'
  status: active
  decision: produce_c0143_r0001_proxy_evidence
  refines:
  - dec_20260708_open_c0143_r0001_intraday_value_migration_pullback_continuation_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0143_r0001_proxy_records_value_migration_pullback_continuation_reference_surface
  ...
  - next_work_is_produce_c0143_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260708_produce_c0143_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0143-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0143_r0001_intraday_value_migration_pullback_continuation",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0143-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0143-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0143-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0143_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0143_r0001_intraday_value_migration_pullback_continuation",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0143-R0001-ANCHOR-BRAID-SQUEEZE-ESCAPE-CONTINUATION-SUMMARY",
            "artifact_role": "value_migration_pullback_continuation_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0143_r0001_value_migration_pullback_continuation_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0143_r0001_intraday_value_migration_pullback_continuation",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0143_r0001_mt5_logic_parity_evidence",
        }
    ]
