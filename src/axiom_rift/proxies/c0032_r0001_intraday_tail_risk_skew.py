"""C0032 R0001 proxy evidence for intraday tail risk skew discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0032_intraday_tail_risk_skew_discovery" / "runs" / "R0001"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0032_intraday_tail_risk_skew_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0032_r0001_proxy_trades.csv"
INTRADAY_TAIL_RISK_SKEW_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0032_r0001_intraday_tail_risk_skew_summary.json"
...
FEATURE_NAMES = (
    ...
    "directional_return_skew_12",
    "directional_return_skew_36",
    "skew_shift_12_vs_36",
    "positive_tail_share_12",
    "negative_tail_share_12",
    "tail_share_imbalance_12",
    "extreme_cluster_6",
    "adverse_extreme_cluster_6",
    "tail_impulse_3",
    "tail_exhaustion_12",
    "tail_reversal_pressure_6",
    "realized_vol_12_atr",
    "realized_vol_36_atr",
    "vol_of_vol_12",
    "tail_kurtosis_24",
    ...
    "volume_tail_confirmation_6",
    "volume_tail_exhaustion_6",
    "spread_relief_3_vs_24",
    "directional_run_ratio_6",
    "adverse_run_ratio_6",
...
MODEL_FAMILY = "fold_local_intraday_tail_risk_skew_rank"
LABEL_SHAPE = "directional_intraday_tail_risk_skew_survival_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_intraday_tail_risk_skew_scores_per_active_day"
...
def run_c0032_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    tail_context = build_tail_context(bars, volumes, range_average, short_range_average)
    trades: list[base.Trade] = []
...
def build_candidates(
    ...
    volumes: list[float],
    tail_context: list[dict[str, float] | None],
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = intraday_tail_risk_skew_features(
                bars,
                range_average,
                short_range_average,
                day_context,
            ...
            if features is None:
                continue
            label = (
                intraday_tail_risk_skew_label(bars, range_average, tail_context, index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|intraday_tail_risk_skew",
                    features=features,
                    label=label,
                )
            )
...
def build_tail_context(
    ...
    contexts: list[dict[str, float] | None] = [None] * len(bars)
    volume_average_24 = base.previous_rolling_average(volumes, 24)
    current_day: str | None = None
    day_volume = 0.0
    bars_since_day_open = 0
    for index, bar in enumerate(bars):
        ...
        average_range = range_average[index]
        short_average_range = short_range_average[index]
        if index < 48 or average_range is None or average_range <= 0 or short_average_range is None:
            continue
        range_3 = sum(max(bars[offset].high - bars[offset].low, 0.0) for offset in range(index - 2, index + 1)) / 3.0
        range_24 = sum(max(bars[offset].high - bars[offset].low, 0.0) for offset in range(index - 23, index + 1)) / 24.0
        ...
        volume_6 = sum(volumes[offset] for offset in range(index - 5, index + 1)) / 6.0
        volume_24 = sum(volumes[offset] for offset in range(index - 23, index + 1)) / 24.0
        volume_3 = sum(volumes[offset] for offset in range(index - 2, index + 1)) / 3.0
        volume_12 = sum(volumes[offset] for offset in range(index - 11, index + 1)) / 12.0
        ...
        contexts[index] = {
            ...
            "volume_burst_vs_24": volumes[index] / max(volume_average_24[index] or 1.0, 1.0),
            "volume_order_shift_6_vs_24": volume_6 / max(volume_24, 1.0),
            "volume_fade_3_vs_12": max(0.0, 1.0 - (volume_3 / max(volume_12, 1.0))),
            ...
            "day_volume_progress": volumes[index] / max(day_volume, 1.0),
...
def directional_tail_stats(
    ...
    moves = [direction * value for value in normalized_close_moves(bars, range_average, index, lookback, fallback_range)]
    if not moves:
        return {
            "skew": 0.0,
            "tail_kurtosis": 0.0,
            ...
            "tail_share_imbalance": 0.0,
            "positive_tail_magnitude": 0.0,
            "negative_tail_magnitude": 0.0,
            "extreme_cluster": 0.0,
            "adverse_extreme_cluster": 0.0,
        }
    mean = sum(moves) / len(moves)
    variance = sum((value - mean) ** 2 for value in moves) / len(moves)
    std = math.sqrt(max(variance, 1e-12))
    skew = sum(((value - mean) / std) ** 3 for value in moves) / len(moves)
    kurtosis = sum(((value - mean) / std) ** 4 for value in moves) / len(moves)
    positive = [value for value in moves if value >= 0.22]
    negative = [value for value in moves if value <= -0.22]
    positive_share = len(positive) / len(moves)
    negative_share = len(negative) / len(moves)
    return {
        "skew": bounded(skew / 3.0, -3.0, 3.0),
        "tail_kurtosis": bounded((kurtosis - 3.0) / 6.0, -2.0, 3.0),
        "positive_tail_share": positive_share,
        "negative_tail_share": negative_share,
        "tail_share_imbalance": positive_share - negative_share,
        "positive_tail_magnitude": sum(positive) / len(moves),
        "negative_tail_magnitude": abs(sum(negative)) / len(moves),
        "extreme_cluster": consecutive_tail_rate(moves, threshold=0.22),
        "adverse_extreme_cluster": consecutive_tail_rate([-value for value in moves], threshold=0.22),
    }


def consecutive_tail_rate(moves: list[float], threshold: float) -> float:
    if len(moves) < 2:
        return 0.0
    pairs = list(zip(moves, moves[1:]))
    return sum(1 for left, right in pairs if left >= threshold and right >= threshold) / len(pairs)
...
def intraday_tail_risk_skew_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    tail = tail_context[index]
    if context is None or tail is None:
        return None
    bar = bars[index]
    spread_over_range = bar.spread_points / average_range
    if spread_over_range > 0.65:
        return None
    tail_6 = directional_tail_stats(bars, range_average, index, direction, 6, average_range)
    tail_12 = directional_tail_stats(bars, range_average, index, direction, 12, average_range)
    tail_24 = directional_tail_stats(bars, range_average, index, direction, 24, average_range)
    tail_36 = directional_tail_stats(bars, range_average, index, direction, 36, average_range)
    directional_return_3 = direction * (bar.close - bars[index - 3].close) / average_range
    directional_return_12 = direction * (bar.close - bars[index - 12].close) / average_range
    tail_impulse = max(0.0, tail_12["tail_share_imbalance"]) * directional_return_3
    tail_exhaustion = max(0.0, tail_24["positive_tail_share"] - tail_6["positive_tail_share"])
    tail_reversal_pressure = tail_6["negative_tail_magnitude"] * max(0.0, directional_return_12)
    volume_tail_confirmation = tail["volume_order_shift_6_vs_24"] * max(0.0, tail_6["tail_share_imbalance"])
    volume_tail_exhaustion = tail["volume_fade_3_vs_12"] * max(0.0, tail_12["positive_tail_share"])
    ...
    return (
        ...
        tail_12["skew"],
        tail_36["skew"],
        tail_12["skew"] - tail_36["skew"],
        tail_12["positive_tail_share"],
        tail_12["negative_tail_share"],
        tail_12["tail_share_imbalance"],
        tail_6["extreme_cluster"],
        tail_6["adverse_extreme_cluster"],
        tail_impulse,
        tail_exhaustion,
        tail_reversal_pressure,
        tail["realized_vol_12_atr"],
        tail["realized_vol_36_atr"],
        tail["vol_of_vol_12"],
        tail_24["tail_kurtosis"],
        tail["range_expansion_3_vs_24"],
        tail["range_compression_6_vs_48"],
        volume_tail_confirmation,
        volume_tail_exhaustion,
        tail["spread_relief_3_vs_24"],
        directional_run_ratio(bars, index, direction, 6),
        directional_run_ratio(bars, index, -direction, 6),
        ...
        tail["day_volume_progress"],
        tail["session_progress"],
        spread_over_range,
    )
...
def intraday_tail_risk_skew_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    tail_context: list[dict[str, float] | None],
    index: int,
    ...
    future_tail_quality = future_directional_tail_quality(bars, range_average, entry_index, exit_index, direction, average_range)
    future_tail_retention = future_tail_skew_retention(tail_context, entry_index, exit_index)
    ...
    return (
        ...
        + 0.30 * future_tail_quality
        + 0.24 * future_directional_bias
        + 0.20 * future_tail_retention
...
def future_tail_skew_retention(
    tail_context: list[dict[str, float] | None],
    entry_index: int,
    exit_index: int,
) -> float:
    values: list[float] = []
    for offset in range(entry_index, exit_index + 1):
        context = tail_context[offset]
        if context is not None:
            values.append(max(0.0, context["realized_vol_12_atr"] - context["realized_vol_36_atr"]))
    return sum(values) / max(len(values), 1)


def future_directional_tail_quality(
    bars: list[base.Bar],
    range_average: list[float | None],
    entry_index: int,
    exit_index: int,
    ...
    for offset in range(entry_index, exit_index + 1):
        ...
        if denominator <= 0:
            continue
        moves.append(direction * (bars[offset].close - bars[offset - 1].close) / denominator)
    if not moves:
        return 0.0
    positive_tail = sum(value for value in moves if value >= 0.18)
    adverse_tail = abs(sum(value for value in moves if value <= -0.18))
    ordered_share = sum(1 for value in moves if value > 0.0) / len(moves)
    return (positive_tail - adverse_tail) / len(moves) + (ordered_share - 0.5)
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0032"
    payload["work_unit_id"] = "C0032"
    payload["run_id"] = "R0001"
    payload["proxy_id"] = "PX-C0032-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0032_r0001_intraday_tail_risk_skew"
    payload["proxy_config_path"] = "campaigns/C0032_intraday_tail_risk_skew_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_intraday_tail_risk_skew_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0032_intraday_tail_risk_skew_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0032_intraday_tail_risk_skew_discovery/runs/R0001/artifacts/c0032_r0001_proxy_trades.csv",
        "campaigns/C0032_intraday_tail_risk_skew_discovery/runs/R0001/artifacts/c0032_r0001_intraday_tail_risk_skew_summary.json",
    ]
    ...
    profiles["intraday_tail_risk_skew_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0032_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "feature_weight_floor": FEATURE_WEIGHT_FLOOR,
            "feature_weight_ceiling": FEATURE_WEIGHT_CEILING,
            "score_interpretation": "higher_score_means_direct_fold_local_intraday_tail_risk_skew_rank",
            "variant_boundary": (
                "intraday_tail_risk_skew_rank_not_vwap_source_synthesis_range_overlap_calendar_phase_tick_participation_"
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_intraday_tail_risk_skew_summary_artifact(payload, INTRADAY_TAIL_RISK_SKEW_SUMMARY_PATH)
    PROXY_PATH.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="ascii")
    trade_hash = base.sha256_file(TRADE_ARTIFACT_PATH)
    summary_hash = base.sha256_file(INTRADAY_TAIL_RISK_SKEW_SUMMARY_PATH)
...
def write_intraday_tail_risk_skew_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_intraday_tail_risk_skew_summary_v1",
        "template": False,
        "work_unit_id": "C0032",
        "campaign_id": "C0032",
        "run_id": "R0001",
        ...
        "intraday_tail_risk_skew_profile": profiles["intraday_tail_risk_skew_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "intraday_tail_risk_skew_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0032-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0032_intraday_tail_risk_skew_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0032_intraday_tail_risk_skew_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0032-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0032-R0001-INTRADAY-TAIL-RISK-SKEW-SUMMARY",
                "intraday_tail_risk_skew_summary_artifact",
                "json",
                "campaigns/C0032_intraday_tail_risk_skew_discovery/runs/R0001/artifacts/c0032_r0001_intraday_tail_risk_skew_summary.json",
                summary_hash,
                ["campaigns/C0032_intraday_tail_risk_skew_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0032_r0001_intraday_tail_risk_skew",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0032_r0001_proxy_trades.csv"
    evidence["intraday_tail_risk_skew_summary"] = "artifacts/c0032_r0001_intraday_tail_risk_skew_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0032_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0032_r0001_proxy_trades.csv",
        "artifacts/c0032_r0001_intraday_tail_risk_skew_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0032 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0032_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0032_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0032_intraday_tail_risk_skew_discovery"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0032_intraday_tail_risk_skew_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0032_intraday_tail_risk_skew_discovery",
        "open_c0032_r0001_fold_local_intraday_tail_risk_skew_rank_run",
        "produce_c0032_r0001_proxy_evidence",
    ):
    ...
    data["active_run"] = "campaigns/C0032_intraday_tail_risk_skew_discovery/runs/R0001"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["active_campaign"] = "campaigns/C0032_intraday_tail_risk_skew_discovery"
    data["active_run"] = "campaigns/C0032_intraday_tail_risk_skew_discovery/runs/R0001"
    data["latest_operation"] = {
        "id": "produce_c0032_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0032_intraday_tail_risk_skew_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0032_r0001_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0032",
            "c0004_r0001_fold_local_state_archetype": "c0032_r0001_intraday_tail_risk_skew",
            "c0004_r0001": "c0032_r0001",
            "fold_local_state_archetype_discovery": "intraday_tail_risk_skew_discovery",
            ...
            "state_archetype_summary": "intraday_tail_risk_skew_summary",
        }
