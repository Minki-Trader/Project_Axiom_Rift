"""C0089 R0001 proxy evidence for intraday session vwap dislocation reversion."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0089_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0089_r0001_session_vwap_dislocation_reversion_summary.json"
...
FEATURE_NAMES = (
    "vwap_dislocation_distance",
    "vwap_reversion_depth",
    ...
    "session_vwap_maturity",
    "dislocation_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_session_vwap_dislocation_reversion_rank"
LABEL_SHAPE = "target_first_escape_quality_conditioned_on_session_vwap_dislocation_reversion_state"
SELECTION_RULE = "top_fold_local_session_vwap_dislocation_reversion_scores_per_active_day"
...
MIN_DAY_BARS = 30
SESSION_VWAP_DISLOCATION_REVERSION_LOOKBACK_BARS = 24
SESSION_VWAP_DISLOCATION_REVERSION_SHORT_LOOKBACK_BARS = 8
MIN_SESSION_VWAP_DISLOCATION_REVERSION_ACTIVITY = 0.16
...
class LinearSessionVwapDislocationReversionModel:
    fold_id: str
    ...
    session_vwap_dislocation_reversion_failure_rate: float
...
def run_c0089_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_session_vwap_dislocation_reversion_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_session_vwap_dislocation_reversion_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        SESSION_VWAP_DISLOCATION_REVERSION_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = session_vwap_dislocation_reversion_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        metrics["vwap_dislocation_distance"],
        metrics["vwap_reversion_depth"],
        metrics["vwap_reclaim_strength"],
        metrics["vwap_slope_alignment"],
        metrics["vwap_wick_rejection"],
        metrics["close_location_commitment"],
        metrics["reversion_impulse"],
        metrics["participation_reengagement"],
        metrics["session_vwap_maturity"],
        metrics["dislocation_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        vwap_dislocation_distance_bucket(metrics["vwap_dislocation_distance"]),
        vwap_reversion_depth_bucket(metrics["vwap_reversion_depth"]),
        vwap_reclaim_bucket(metrics["vwap_reclaim_strength"]),
        vwap_slope_alignment_bucket(metrics["vwap_slope_alignment"]),
        vwap_wick_rejection_bucket(metrics["vwap_wick_rejection"]),
        vwap_close_commitment_bucket(metrics["close_location_commitment"]),
        reversion_impulse_bucket(metrics["reversion_impulse"]),
        volume_reengagement_bucket(metrics["participation_reengagement"]),
        session_vwap_maturity_bucket(metrics["session_vwap_maturity"]),
        dislocation_pressure_bucket(metrics["dislocation_pressure"]),
...
def session_vwap_dislocation_reversion_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    short_start = index - SESSION_VWAP_DISLOCATION_REVERSION_SHORT_LOOKBACK_BARS + 1
    prior_start = index - SESSION_VWAP_DISLOCATION_REVERSION_LOOKBACK_BARS + 1
    ...
    previous_vwap = session_vwap_until(bars, volumes, context.day_start_index, index - 1)
    if direction > 0:
        vwap_dislocation_distance = (session_vwap - previous_low) / average_range
        vwap_reversion_depth = (session_vwap - previous_low) / average_range
        vwap_reclaim_strength = (signal_bar.close - session_vwap) / average_range
        crossed_anchor = previous_low <= session_vwap and signal_bar.close >= session_vwap - 0.05 * average_range
    else:
        vwap_dislocation_distance = (previous_high - session_vwap) / average_range
        vwap_reversion_depth = (previous_high - session_vwap) / average_range
        vwap_reclaim_strength = (session_vwap - signal_bar.close) / average_range
        crossed_anchor = previous_high >= session_vwap and signal_bar.close <= session_vwap + 0.05 * average_range
    vwap_slope_alignment = direction * (session_vwap - previous_vwap) / average_range
    vwap_wick_rejection = directional_wick_rejection(signal_bar, direction)
    close_location_commitment = directional_bar_settlement(signal_bar, direction)
    reversion_impulse = direction * (signal_bar.close - bars[index - 1].close) / average_range
    ...
    session_vwap_maturity = min(context.bars_so_far / 72.0, 3.0)
    dislocation_pressure_raw = max(
        max(vwap_reversion_depth, 0.0) / 2.0,
        max(vwap_reclaim_strength, 0.0),
        max(vwap_wick_rejection, 0.0),
        max(close_location_commitment - 0.50, 0.0) * 2.0,
        max(reversion_impulse, 0.0),
        min(max(participation_reengagement, 0.0), 1.0),
        max(session_vwap_maturity, 0.0),
    )
    dislocation_pressure = bounded_session_progress(signal_bar.time) * dislocation_pressure_raw
    activity = max(dislocation_pressure_raw, dislocation_pressure)
    if activity < MIN_SESSION_VWAP_DISLOCATION_REVERSION_ACTIVITY:
        return {"event_count": 0.0}
    if not crossed_anchor and vwap_reversion_depth < 0.16:
        return {"event_count": 0.0}
    if vwap_reversion_depth < 0.08:
        return {"event_count": 0.0}
    if vwap_reclaim_strength < -0.08 and close_location_commitment < 0.46:
        return {"event_count": 0.0}
    if reversion_impulse < -0.35 and vwap_wick_rejection < -0.05:
        return {"event_count": 0.0}
    return {
        "vwap_dislocation_distance": bounded(vwap_dislocation_distance, -2.0, 6.0),
        "vwap_reversion_depth": bounded(vwap_reversion_depth, -2.0, 6.0),
        "vwap_reclaim_strength": bounded(vwap_reclaim_strength, -2.0, 6.0),
        "vwap_slope_alignment": bounded(vwap_slope_alignment, -4.0, 4.0),
        "vwap_wick_rejection": bounded(vwap_wick_rejection, -1.0, 1.0),
        "close_location_commitment": bounded(close_location_commitment, 0.0, 1.0),
        "reversion_impulse": bounded(reversion_impulse, -4.0, 6.0),
        "participation_reengagement": bounded(participation_reengagement, -3.0, 6.0),
        "session_vwap_maturity": bounded(session_vwap_maturity, 0.0, 3.0),
        "dislocation_pressure": bounded(dislocation_pressure, 0.0, 4.0),
        "event_count": 1.0,
    }
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    probe_reward = 0.12 if metrics["vwap_reversion_depth"] > 0.18 and terminal > 0 else 0.0
    reclaim_reward = 0.14 if metrics["vwap_reclaim_strength"] > 0.05 and terminal > 0 else 0.0
    close_reward = 0.12 if metrics["close_location_commitment"] > 0.58 and terminal > 0 else 0.0
    impulse_reward = 0.10 if metrics["reversion_impulse"] > 0.10 and terminal > 0 else 0.0
    wick_reward = 0.08 if metrics["vwap_wick_rejection"] > 0.05 and terminal > 0 else 0.0
    participation_reward = 0.08 if metrics["participation_reengagement"] > 0.03 and terminal > 0 else 0.0
    prior_context_reward = 0.05 if metrics["session_vwap_maturity"] > 0.08 and terminal > 0 else 0.0
    session_reward = 0.05 if metrics["dislocation_pressure"] > 0.12 and terminal > 0 else 0.0
    shallow_probe_penalty = 0.10 if metrics["vwap_reversion_depth"] < 0.12 and terminal <= 0 else 0.0
    failed_reclaim_penalty = 0.12 if metrics["vwap_reclaim_strength"] < -0.04 and terminal <= 0 else 0.0
    weak_close_penalty = 0.10 if metrics["close_location_commitment"] < 0.45 and terminal <= 0 else 0.0
    adverse_impulse_penalty = 0.10 if metrics["reversion_impulse"] < -0.12 and terminal <= 0 else 0.0
    weak_wick_penalty = 0.08 if metrics["vwap_wick_rejection"] < -0.08 and terminal <= 0 else 0.0
    no_participation_penalty = 0.08 if metrics["participation_reengagement"] < -0.06 and terminal <= 0 else 0.0
    wide_gap_penalty = 0.04 if metrics["vwap_slope_alignment"] > 0.85 and terminal <= 0 else 0.0
    ...
    session_vwap_dislocation_reversion_path_quality = (0.52 * terminal + 0.34 * mfe - 0.68 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + session_vwap_dislocation_reversion_path_quality
        + probe_reward
        + reclaim_reward
        + close_reward
        + impulse_reward
        ...
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["vwap_dislocation_distance"] - 3.00) * 0.04
    )


def fit_linear_session_vwap_dislocation_reversion_model(candidates: list[base.Candidate], fold_id: str) -> LinearSessionVwapDislocationReversionModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearSessionVwapDislocationReversionModel(
            fold_id,
    ...
    session_vwap_dislocation_reversion_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearSessionVwapDislocationReversionModel(
        fold_id=fold_id,
        ...
        session_vwap_dislocation_reversion_failure_rate=session_vwap_dislocation_reversion_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearSessionVwapDislocationReversionModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_session_vwap_dislocation_reversion_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0089_r0001_intraday_session_vwap_dislocation_reversion",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0089_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0089_r0001_session_vwap_dislocation_reversion_summary.json",
        ],
        ...
        "conditional_profiles": {
            "session_vwap_dislocation_reversion_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0089 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0089_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0089 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "session_vwap_dislocation_reversion_lookback_bars": SESSION_VWAP_DISLOCATION_REVERSION_LOOKBACK_BARS,
        "session_vwap_dislocation_reversion_short_lookback_bars": SESSION_VWAP_DISLOCATION_REVERSION_SHORT_LOOKBACK_BARS,
        "min_session_vwap_dislocation_reversion_activity": MIN_SESSION_VWAP_DISLOCATION_REVERSION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_session_vwap_dislocation_reversion_model_summary(model: LinearSessionVwapDislocationReversionModel) -> dict[str, object]:
    return {
        ...
        "session_vwap_dislocation_reversion_failure_rate": rounded(model.session_vwap_dislocation_reversion_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearSessionVwapDislocationReversionModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_session_vwap_dislocation_reversion_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "session_vwap_dislocation_reversion_profile": profiles["session_vwap_dislocation_reversion_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0089_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0089_r0001_proxy_trades.csv"
    evidence["session_vwap_dislocation_reversion_summary"] = "artifacts/c0089_r0001_session_vwap_dislocation_reversion_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0089_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0089_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0089 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0089_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0089_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0089_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0089_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0089_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0089_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0089_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0089_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0089_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0089 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0089 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0089_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0089_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0089_r0001_intraday_session_vwap_dislocation_reversion_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0089_r0001_proxy_records_session_vwap_dislocation_reversion_reference_surface
  ...
  - next_work_is_produce_c0089_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0089_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0089-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0089_r0001_intraday_session_vwap_dislocation_reversion",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0089-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0089-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0089-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0089_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0089_r0001_intraday_session_vwap_dislocation_reversion",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0089-R0001-session-vwap-dislocation-reversion-SUMMARY",
            "artifact_role": "session_vwap_dislocation_reversion_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0089_r0001_session_vwap_dislocation_reversion_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0089_r0001_intraday_session_vwap_dislocation_reversion",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0089_r0001_mt5_logic_parity_evidence",
        }
    ]
