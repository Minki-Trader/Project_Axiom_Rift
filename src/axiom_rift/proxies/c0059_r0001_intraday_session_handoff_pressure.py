"""C0059 R0001 proxy evidence for intraday session handoff pressure."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0059_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0059_r0001_session_handoff_pressure_summary.json"
...
FEATURE_NAMES = (
    "handoff_proximity",
    "pre_handoff_directional_drift_18",
    "post_handoff_acceptance_6",
    "handoff_reversal_pressure",
    "prior_handoff_range_carry",
    "session_transition_pressure",
    "intraday_position_pressure",
    "handoff_wick_rejection_balance",
    "volume_participation_shift",
    "day_range_completion_ratio",
    "session_progress_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_session_handoff_pressure_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_session_handoff_pressure_state"
SELECTION_RULE = "top_fold_local_session_handoff_pressure_scores_per_active_day"
...
MIN_DAY_BARS = 36
SESSION_HANDOFF_LOOKBACK_BARS = 18
SESSION_HANDOFF_SHORT_LOOKBACK_BARS = 6
MIN_SESSION_HANDOFF_ACTIVITY = 0.16
...
class LinearSessionHandoffPressureModel:
    fold_id: str
...
def run_c0059_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_session_handoff_pressure_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_session_handoff_pressure_model_summary(model))
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    handoff = session_handoff_pressure_metrics(bars, volumes, context, volume_average, index, direction, average_range)
    if handoff["event_count"] <= 0:
        return None, None
    ...
    if volume_ratio is None:
        return None, None
    features = (
        handoff["handoff_proximity"],
        handoff["pre_handoff_directional_drift_18"],
        handoff["post_handoff_acceptance_6"],
        handoff["handoff_reversal_pressure"],
        handoff["prior_handoff_range_carry"],
        handoff["session_transition_pressure"],
        handoff["intraday_position_pressure"],
        handoff["handoff_wick_rejection_balance"],
        handoff["volume_participation_shift"],
        handoff["day_range_completion_ratio"],
        handoff["session_progress_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        handoff_proximity_bucket(handoff["handoff_proximity"]),
        drift_bucket(handoff["pre_handoff_directional_drift_18"]),
        acceptance_bucket(handoff["post_handoff_acceptance_6"]),
        reversal_pressure_bucket(handoff["handoff_reversal_pressure"]),
        range_carry_bucket(handoff["prior_handoff_range_carry"]),
        transition_pressure_bucket(handoff["session_transition_pressure"]),
        intraday_position_bucket(handoff["intraday_position_pressure"]),
        wick_rejection_bucket(handoff["handoff_wick_rejection_balance"]),
        participation_bucket(volume_ratio),
        session_pressure_bucket(handoff["session_progress_pressure"]),
...
def session_handoff_pressure_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    if prior_volume_per_bar is None or prior_volume_per_bar <= 0:
        return {"event_count": 0.0}
    prior_window = bars[index - SESSION_HANDOFF_LOOKBACK_BARS : index - SESSION_HANDOFF_SHORT_LOOKBACK_BARS + 1]
    short_window = bars[index - SESSION_HANDOFF_SHORT_LOOKBACK_BARS + 1 : index + 1]
    minute = base.minute_of_day(bars[index].time)
    handoff_proximity = session_handoff_proximity(minute)
    pre_reference = bars[index - SESSION_HANDOFF_SHORT_LOOKBACK_BARS].close
    pre_handoff_drift = direction * (pre_reference - bars[index - SESSION_HANDOFF_LOOKBACK_BARS].close) / average_range
    post_handoff_acceptance = direction * (close - pre_reference) / average_range
    handoff_reversal_pressure = max(0.0, -pre_handoff_drift) * max(0.0, post_handoff_acceptance)
    prior_handoff_range_carry = (max(item.high for item in prior_window) - min(item.low for item in prior_window)) / average_range
    session_transition_pressure = handoff_proximity * (post_handoff_acceptance - 0.35 * pre_handoff_drift)
    intraday_position = direction * (close - context.day_open) / average_range
    handoff_wick_rejection = sum(directional_wick_rejection(item, direction) for item in short_window) / len(short_window)
    range_completion = day_width / average_range
    recent_volume = sum(volumes[max(0, index - SESSION_HANDOFF_SHORT_LOOKBACK_BARS + 1) : index + 1])
    expected_recent_volume = prior_volume_per_bar * SESSION_HANDOFF_SHORT_LOOKBACK_BARS
    recent_intensity = recent_volume / max(expected_recent_volume, 1e-9)
    volume_participation_shift = recent_intensity - 1.0
    session_pressure = handoff_proximity * max(
        abs(pre_handoff_drift),
        abs(post_handoff_acceptance),
        handoff_reversal_pressure,
        abs(handoff_wick_rejection),
    )
    activity = max(
        handoff_proximity,
        abs(pre_handoff_drift),
        abs(post_handoff_acceptance),
        handoff_reversal_pressure,
        abs(session_transition_pressure),
        abs(volume_participation_shift),
        session_pressure,
    )
    if activity < MIN_SESSION_HANDOFF_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "handoff_proximity": bounded(handoff_proximity, 0.0, 1.0),
        "pre_handoff_directional_drift_18": bounded(pre_handoff_drift, -4.0, 4.0),
        "post_handoff_acceptance_6": bounded(post_handoff_acceptance, -4.0, 4.0),
        "handoff_reversal_pressure": bounded(handoff_reversal_pressure, 0.0, 4.0),
        "prior_handoff_range_carry": bounded(prior_handoff_range_carry, 0.0, 8.0),
        "session_transition_pressure": bounded(session_transition_pressure, -4.0, 4.0),
        "intraday_position_pressure": bounded(intraday_position, -8.0, 8.0),
        "handoff_wick_rejection_balance": bounded(handoff_wick_rejection, -1.0, 1.0),
        "volume_participation_shift": bounded(volume_participation_shift, -2.0, 4.0),
        "day_range_completion_ratio": bounded(range_completion, 0.0, 8.0),
        "session_progress_pressure": bounded(session_pressure * max(recent_intensity, 0.0), 0.0, 4.0),
        "event_count": 1.0,
    }
...
def session_handoff_proximity(minute: int) -> float:
    boundaries = (6 * 60, 8 * 60, 13 * 60, 17 * 60, 21 * 60)
    distance = min(abs(minute - boundary) for boundary in boundaries)
...
def handoff_proximity_bucket(value: float) -> str:
    if value < 0.20:
        return "handoff_far"
    if value < 0.65:
        return "handoff_near"
...
def drift_bucket(value: float) -> str:
    if value < -0.35:
        return "pre_handoff_drift_against"
    if value < 0.35:
        return "pre_handoff_drift_flat"
    return "pre_handoff_drift_with"


def acceptance_bucket(value: float) -> str:
    if value < -0.25:
        return "post_handoff_rejected"
    if value < 0.35:
        return "post_handoff_neutral"
    return "post_handoff_accepted"


def reversal_pressure_bucket(value: float) -> str:
    if value < 0.08:
        return "handoff_reversal_absent"
    if value < 0.55:
        return "handoff_reversal_partial"
    return "handoff_reversal_strong"


def range_carry_bucket(value: float) -> str:
    if value < 1.15:
        return "handoff_range_quiet"
    if value < 2.60:
        return "handoff_range_normal"
    return "handoff_range_wide"


def transition_pressure_bucket(value: float) -> str:
    if value < -0.25:
        return "transition_pressure_against"
    if value < 0.35:
        return "transition_pressure_neutral"
    return "transition_pressure_with"


def intraday_position_bucket(value: float) -> str:
    if value < -0.75:
        return "intraday_position_against"
    if value < 0.75:
        return "intraday_position_mid"
    return "intraday_position_with"


def wick_rejection_bucket(value: float) -> str:
    if value < -0.20:
        return "handoff_wick_rejects"
    if value < 0.25:
        return "handoff_wick_balanced"
    return "handoff_wick_supports"
...
def candidate_label(
    ...
    handoff_proximity = session_handoff_proximity(minute)
    pre_reference = bars[index - SESSION_HANDOFF_SHORT_LOOKBACK_BARS].close
    pre_handoff_drift = direction * (
        pre_reference - bars[index - SESSION_HANDOFF_LOOKBACK_BARS].close
    ) / average_range
    post_handoff_acceptance = direction * (close - pre_reference) / average_range
    continuation_reward = 0.16 if handoff_proximity > 0.35 and pre_handoff_drift > 0.20 and terminal > 0 else 0.0
    reversal_reward = (
        0.22
        if handoff_proximity > 0.35 and pre_handoff_drift < -0.20 and post_handoff_acceptance > 0.08 and terminal > 0
        else 0.0
    )
    failed_handoff_penalty = 0.24 if handoff_proximity > 0.35 and post_handoff_acceptance < -0.20 and mae > mfe else 0.0
    false_transition_penalty = 0.16 if pre_handoff_drift > 0.20 and post_handoff_acceptance < -0.20 and terminal <= 0 else 0.0
    ...
    handoff_path_quality = (0.35 * terminal + 0.35 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + handoff_path_quality
        + continuation_reward
        + reversal_reward
        - failed_handoff_penalty
        - false_transition_penalty
        - stall_penalty
...
def fit_linear_session_handoff_pressure_model(candidates: list[base.Candidate], fold_id: str) -> LinearSessionHandoffPressureModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearSessionHandoffPressureModel(
            fold_id,
    ...
    handoff_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearSessionHandoffPressureModel(
        fold_id=fold_id,
        ...
        handoff_failure_rate=handoff_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearSessionHandoffPressureModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_session_handoff_pressure_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0059_r0001_intraday_session_handoff_pressure",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0059_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0059_r0001_session_handoff_pressure_summary.json",
        ],
        ...
        "conditional_profiles": {
            "session_handoff_pressure_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0059 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0059_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0059 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "session_handoff_lookback_bars": SESSION_HANDOFF_LOOKBACK_BARS,
        "session_handoff_short_lookback_bars": SESSION_HANDOFF_SHORT_LOOKBACK_BARS,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_session_handoff_pressure_model_summary(model: LinearSessionHandoffPressureModel) -> dict[str, object]:
    return {
        ...
        "handoff_failure_rate": rounded(model.handoff_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearSessionHandoffPressureModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_session_handoff_pressure_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "session_handoff_pressure_profile": profiles["session_handoff_pressure_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0059_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_trade_artifact"] = "artifacts/c0059_r0001_proxy_trades.csv"
    evidence["session_handoff_pressure_summary"] = "artifacts/c0059_r0001_session_handoff_pressure_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0059_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0059_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0059 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0059_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0059_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0059_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0059_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0059_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0059_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0059_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0059_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0059_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0059 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0059 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260706_produce_c0059_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0059_r0001_proxy_evidence
  refines:
  - dec_20260706_open_c0059_r0001_intraday_session_handoff_pressure_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0059_r0001_proxy_records_session_handoff_pressure_reference_surface
  ...
  - next_work_is_produce_c0059_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260706_produce_c0059_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0059-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0059_r0001_intraday_session_handoff_pressure",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0059-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0059-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0059-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0059_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0059_r0001_intraday_session_handoff_pressure",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0059-R0001-session-handoff-pressure-SUMMARY",
            "artifact_role": "session_handoff_pressure_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0059_r0001_session_handoff_pressure_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0059_r0001_intraday_session_handoff_pressure",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0059_r0001_mt5_logic_parity_evidence",
        }
    ]
