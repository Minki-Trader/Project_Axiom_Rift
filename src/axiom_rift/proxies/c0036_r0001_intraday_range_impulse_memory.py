"""C0036 R0001 proxy evidence for intraday range impulse memory discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0036_intraday_range_impulse_memory_discovery" / "runs" / "R0001"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0036_intraday_range_impulse_memory_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0036_r0001_proxy_trades.csv"
RANGE_IMPULSE_MEMORY_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0036_r0001_range_impulse_memory_summary.json"
...
FEATURE_NAMES = (
    ...
    "directional_range_impulse_1",
    "directional_range_impulse_3",
    "directional_range_impulse_6",
    "directional_range_impulse_12",
    "range_impulse_memory_6",
    "range_impulse_memory_12",
    "range_impulse_memory_24",
    "range_impulse_slope_6",
    "range_impulse_slope_12",
    "range_impulse_curvature_6",
    "directional_range_dominance_6",
    "directional_range_dominance_12",
    "opposite_range_relief_6",
    "opposite_range_relief_12",
    "close_location_impulse_3",
    "close_location_impulse_6",
    "wick_rejection_memory_6",
    "wick_follow_memory_6",
    ...
    "path_efficiency_12",
    "churn_ratio_12",
    "adverse_range_pressure_6",
    "favorable_range_pressure_6",
    "day_range_balance_directional",
    "day_range_balance_slope_12",
    "day_extreme_distance_directional",
    "session_progress",
    "spread_over_range",
    "spread_range_stress_3",
)
MODEL_FAMILY = "fold_local_intraday_range_impulse_memory_rank"
LABEL_SHAPE = "directional_intraday_range_impulse_memory_survival_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_intraday_range_impulse_memory_scores_per_active_day"
...
def run_c0036_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    range_context = build_range_impulse_context(bars, range_average, short_range_average, day_context)
    trades: list[base.Trade] = []
...
def build_candidates(
    ...
    range_context: list[dict[str, float] | None],
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = intraday_range_impulse_memory_features(
                bars,
                range_average,
                short_range_average,
                day_context,
            ...
            if features is None:
                continue
            label = (
                intraday_range_impulse_memory_label(bars, range_average, range_context, index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|intraday_range_impulse_memory",
                    features=features,
                    label=label,
                )
            )
...
def build_range_impulse_context(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    for index, bar in enumerate(bars):
        ...
        if index > 0 and average_range is not None and average_range > 0:
            day_impulse_sum += signed_range_impulse(bars, index, average_range)
        ...
        range_12 = mean_bar_range(bars, index, 12)
        range_24 = mean_bar_range(bars, index, 24)
        range_48 = mean_bar_range(bars, index, 48)
        day_balance = day_impulse_sum / max(math.sqrt(day["bars_since_day_open"] + 1.0), 1.0)
        previous_day_balance = 0.0
        if index >= 12 and bars[index - 12].time.strftime("%Y-%m-%d") == day_key:
            previous_context = contexts[index - 12]
            if previous_context is not None:
                previous_day_balance = previous_context["day_range_balance"]
        contexts[index] = {
            "signed_impulse_1": signed_range_impulse(bars, index, average_range),
            "signed_impulse_3": mean_signed_range_impulse(bars, index, 3, average_range),
            "signed_impulse_6": mean_signed_range_impulse(bars, index, 6, average_range),
            "signed_impulse_12": mean_signed_range_impulse(bars, index, 12, average_range),
            "signed_impulse_24": mean_signed_range_impulse(bars, index, 24, average_range),
            "signed_impulse_slope_6": impulse_memory_slope(bars, index, 6, average_range),
            "signed_impulse_slope_12": impulse_memory_slope(bars, index, 12, average_range),
            "signed_impulse_curvature_6": impulse_memory_curvature(bars, index, average_range),
            "range_expansion_3_vs_24": range_3 / max(range_24, 1e-9),
            "range_expansion_6_vs_48": range_6 / max(range_48, 1e-9),
            "range_compression_12_vs_48": range_12 / max(range_48, 1e-9),
            "day_range_balance": day_balance,
            "day_range_balance_slope_12": day_balance - previous_day_balance,
            "session_progress": bounded(day["bars_since_day_open"] / 288.0, 0.0, 1.0),
            "spread_over_range": bar.spread_points / average_range,
            "spread_range_stress_3": bar.spread_points / max(range_3, 1e-9),
        }
    return contexts


def signed_range_impulse(bars: list[base.Bar], index: int, average_range: float) -> float:
    if index <= 0:
        return 0.0
    bar = bars[index]
    bar_range = max(bar.high - bar.low, 1e-9)
    range_weight = bar_range / max(average_range, 1e-9)
    body_component = (bar.close - bar.open) / bar_range
    close_location_component = close_location_signed(bar)
    close_move_component = bounded((bar.close - bars[index - 1].close) / max(average_range, 1e-9), -2.0, 2.0)
    impulse = (0.45 * body_component + 0.35 * close_location_component + 0.20 * close_move_component) * range_weight
    return bounded(impulse, -4.0, 4.0)


def mean_signed_range_impulse(bars: list[base.Bar], index: int, lookback: int, average_range: float) -> float:
    return sum(signed_range_impulse(bars, offset, average_range) for offset in range(index - lookback + 1, index + 1)) / lookback


def segment_mean_signed_range_impulse(
    bars: list[base.Bar],
    start: int,
    end: int,
    average_range: float,
    ...
    if start < 1 or end < start:
        return 0.0
    count = end - start + 1
    return sum(signed_range_impulse(bars, offset, average_range) for offset in range(start, end + 1)) / count


def impulse_memory_slope(bars: list[base.Bar], index: int, lookback: int, average_range: float) -> float:
    recent = segment_mean_signed_range_impulse(bars, index - lookback + 1, index, average_range)
    prior = segment_mean_signed_range_impulse(bars, index - 2 * lookback + 1, index - lookback, average_range)
    return recent - prior


def impulse_memory_curvature(bars: list[base.Bar], index: int, average_range: float) -> float:
    recent = segment_mean_signed_range_impulse(bars, index - 2, index, average_range)
    prior = segment_mean_signed_range_impulse(bars, index - 5, index - 3, average_range)
    older = segment_mean_signed_range_impulse(bars, index - 8, index - 6, average_range)
    return (recent - prior) - (prior - older)


def intraday_range_impulse_memory_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    features = (
        ...
        direction * context["signed_impulse_1"],
        direction * context["signed_impulse_3"],
        direction * context["signed_impulse_6"],
        direction * context["signed_impulse_12"],
        direction * context["signed_impulse_6"],
        direction * context["signed_impulse_12"],
        direction * context["signed_impulse_24"],
        direction * context["signed_impulse_slope_6"],
        direction * context["signed_impulse_slope_12"],
        direction * context["signed_impulse_curvature_6"],
        directional_range_dominance(bars, index, direction, 6, average_range),
        directional_range_dominance(bars, index, direction, 12, average_range),
        opposite_range_relief(bars, index, direction, 6, average_range),
        opposite_range_relief(bars, index, direction, 12, average_range),
        close_location_impulse(bars, index, direction, 3, average_range),
        close_location_impulse(bars, index, direction, 6, average_range),
        wick_rejection_memory(bars, index, direction, 6),
        wick_follow_memory(bars, index, direction, 6),
        body_range_alignment(bars, index, direction, 6, average_range),
        body_range_alignment(bars, index, direction, 12, average_range),
        context["range_expansion_3_vs_24"],
        context["range_expansion_6_vs_48"],
        context["range_compression_12_vs_48"],
        directional_efficiency(bars, index, direction, 12),
        path_churn_ratio(bars, index, 12),
        adverse_range_pressure(bars, index, direction, 6, average_range),
        favorable_range_pressure(bars, index, direction, 6, average_range),
        direction * context["day_range_balance"],
        direction * context["day_range_balance_slope_12"],
        day_extreme_distance_directional(bars, day, index, direction, average_range),
        context["session_progress"],
        spread_over_range,
        context["spread_range_stress_3"],
    )
    if len(features) != len(FEATURE_NAMES):
        raise RuntimeError(f"C0036 feature length mismatch: {len(features)} != {len(FEATURE_NAMES)}")
    return tuple(float(value) for value in features)


def intraday_range_impulse_memory_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    range_context: list[dict[str, float] | None],
    index: int,
    ...
    future_impulse_acceptance = future_directional_range_impulse_acceptance(
        bars, range_average, entry_index, exit_index, direction, average_range
    )
    future_impulse_retention = future_range_impulse_memory_retention(range_context, entry_index, exit_index, direction)
    future_adverse_impulse = future_adverse_range_pressure(bars, entry_index, exit_index, direction, average_range)
    ...
    return (
        ...
        + 0.32 * future_impulse_acceptance
        + 0.22 * future_impulse_retention
        - 0.24 * future_adverse_impulse
...
def future_range_impulse_memory_retention(
    range_context: list[dict[str, float] | None],
    entry_index: int,
    exit_index: int,
    direction: int,
    ...
    for offset in range(entry_index, exit_index + 1):
        context = range_context[offset]
        if context is None:
            continue
        values.append(
            0.30 * direction * context["signed_impulse_3"]
            + 0.24 * direction * context["signed_impulse_6"]
            + 0.20 * direction * context["signed_impulse_slope_6"]
            + 0.16 * direction * context["day_range_balance_slope_12"]
            - 0.12 * context["spread_over_range"]
        )
    return sum(values) / max(len(values), 1)


def future_directional_range_impulse_acceptance(
    bars: list[base.Bar],
    range_average: list[float | None],
    entry_index: int,
    exit_index: int,
    ...
    values: list[float] = []
    for offset in range(entry_index, exit_index + 1):
        local_average = range_average[offset] if offset < len(range_average) else average_range
        if local_average is None or local_average <= 0:
            local_average = average_range
        move = direction * (bars[offset].close - bars[offset - 1].close) / max(average_range, 1e-9)
        impulse = direction * signed_range_impulse(bars, offset, local_average)
        close_acceptance = direction * close_location_signed(bars[offset])
        values.append(0.44 * impulse + 0.34 * move + 0.22 * close_acceptance)
    if not values:
        return 0.0
...
def future_adverse_range_pressure(
    bars: list[base.Bar],
    entry_index: int,
    exit_index: int,
    direction: int,
    ...
    pressures = [
        max(0.0, -direction * signed_range_impulse(bars, offset, average_range))
        for offset in range(entry_index, exit_index + 1)
    ]
    return sum(pressures) / max(len(pressures), 1)


def directional_range_dominance(
    bars: list[base.Bar],
    index: int,
    direction: int,
    lookback: int,
    ...
    favorable = favorable_range_pressure(bars, index, direction, lookback, average_range)
    adverse = adverse_range_pressure(bars, index, direction, lookback, average_range)
    return (favorable - adverse) / max(favorable + adverse, 1e-9)


def opposite_range_relief(
    bars: list[base.Bar],
    index: int,
    direction: int,
    lookback: int,
    ...
    recent = adverse_range_pressure(bars, index, direction, lookback, average_range)
    prior = adverse_range_pressure(bars, index - lookback, direction, lookback, average_range)
    return max(0.0, prior - recent)


def close_location_impulse(
    bars: list[base.Bar],
    index: int,
    direction: int,
    lookback: int,
    ...
    for offset in range(index - lookback + 1, index + 1):
        bar_range = max(bars[offset].high - bars[offset].low, 1e-9)
        total += direction * close_location_signed(bars[offset]) * bar_range / max(average_range, 1e-9)
    return total / lookback


def wick_rejection_memory(bars: list[base.Bar], index: int, direction: int, lookback: int) -> float:
    total = 0.0
    for offset in range(index - lookback + 1, index + 1):
        bar = bars[offset]
        bar_range = max(bar.high - bar.low, 1e-9)
        upper = (bar.high - max(bar.open, bar.close)) / bar_range
        lower = (min(bar.open, bar.close) - bar.low) / bar_range
        location = close_location_signed(bar)
        if direction > 0:
            total += lower * max(0.0, location)
        else:
            total += upper * max(0.0, -location)
    return total / lookback


def wick_follow_memory(bars: list[base.Bar], index: int, direction: int, lookback: int) -> float:
    total = 0.0
    for offset in range(index - lookback + 1, index + 1):
        bar = bars[offset]
        bar_range = max(bar.high - bar.low, 1e-9)
...
def day_extreme_distance_directional(
    bars: list[base.Bar],
...
def linear_model_summary(model: LinearEdgeModel) -> dict[str, object]:
    return {
        ...
        "score_interpretation": "higher_score_means_direct_fold_local_intraday_range_impulse_memory_rank",
    }
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0036"
    payload["work_unit_id"] = "C0036"
    payload["run_id"] = "R0001"
    payload["proxy_id"] = "PX-C0036-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0036_r0001_intraday_range_impulse_memory"
    payload["proxy_config_path"] = "campaigns/C0036_intraday_range_impulse_memory_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_intraday_range_impulse_memory_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0036_intraday_range_impulse_memory_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0036_intraday_range_impulse_memory_discovery/runs/R0001/artifacts/c0036_r0001_proxy_trades.csv",
        "campaigns/C0036_intraday_range_impulse_memory_discovery/runs/R0001/artifacts/c0036_r0001_range_impulse_memory_summary.json",
    ]
    ...
    profiles["intraday_range_impulse_memory_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
            "model_family": MODEL_FAMILY,
            "feature_count": len(FEATURE_NAMES),
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0036_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "score_interpretation": "higher_score_means_direct_fold_local_intraday_range_impulse_memory_rank",
            "variant_boundary": (
                "intraday_range_impulse_memory_rank_not_effort_absorption_seasonal_residual_tail_risk_skew_entropy_"
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_range_impulse_memory_summary_artifact(payload, RANGE_IMPULSE_MEMORY_SUMMARY_PATH)
    write_text_local(PROXY_PATH, json.dumps(payload, indent=2, sort_keys=True) + "\n")
    trade_hash = sha256_file_local(TRADE_ARTIFACT_PATH)
    summary_hash = sha256_file_local(RANGE_IMPULSE_MEMORY_SUMMARY_PATH)
...
def write_range_impulse_memory_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_intraday_range_impulse_memory_summary_v1",
        "template": False,
        "work_unit_id": "C0036",
        "campaign_id": "C0036",
        "run_id": "R0001",
        ...
        "intraday_range_impulse_memory_profile": profiles["intraday_range_impulse_memory_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "intraday_range_impulse_memory_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0036-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0036_intraday_range_impulse_memory_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0036_intraday_range_impulse_memory_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0036-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0036-R0001-CYCLE-PHASE-SUMMARY",
                "intraday_range_impulse_memory_summary_artifact",
                "json",
                "campaigns/C0036_intraday_range_impulse_memory_discovery/runs/R0001/artifacts/c0036_r0001_range_impulse_memory_summary.json",
                summary_hash,
                ["campaigns/C0036_intraday_range_impulse_memory_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0036_r0001_intraday_range_impulse_memory",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0036_r0001_proxy_trades.csv"
    evidence["intraday_range_impulse_memory_summary"] = "artifacts/c0036_r0001_range_impulse_memory_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0036_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0036_r0001_proxy_trades.csv",
        "artifacts/c0036_r0001_range_impulse_memory_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0036 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0036_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0036_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0036_intraday_range_impulse_memory_discovery"
    data["project"]["active_run"] = "campaigns/C0036_intraday_range_impulse_memory_discovery/runs/R0001"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0036_intraday_range_impulse_memory_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0036_intraday_range_impulse_memory_discovery",
        "open_c0036_r0001_fold_local_intraday_range_impulse_memory_rank_run",
        "produce_c0036_r0001_proxy_evidence",
    ):
    ...
    data["active_campaign"] = "campaigns/C0036_intraday_range_impulse_memory_discovery"
    data["active_run"] = "campaigns/C0036_intraday_range_impulse_memory_discovery/runs/R0001"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0036_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0036_intraday_range_impulse_memory_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0036_r0001_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["canonical_source"] = "campaigns/C0036_intraday_range_impulse_memory_discovery/runs/R0001/kpi/proxy.json"
    data["canonical_status"] = "proxy_recorded_pending_mt5"
    data["current_decision"] = "produce_c0036_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = "campaigns/C0036_intraday_range_impulse_memory_discovery"
    data["active_run"] = "campaigns/C0036_intraday_range_impulse_memory_discovery/runs/R0001"
    data["next_required_action"] = "produce_c0036_r0001_mt5_logic_parity_evidence"
    data["current_evidence_summary"] = {
        "source_campaign": "campaigns/C0036_intraday_range_impulse_memory_discovery",
        "current_task": "produce_c0036_r0001_mt5_logic_parity_evidence",
        "active_run": "campaigns/C0036_intraday_range_impulse_memory_discovery/runs/R0001",
        "active_run_status": "proxy_recorded_pending_mt5",
        "evidence_status": "proxy_recorded_pending_mt5",
        "campaign_family": "intraday_range_impulse_memory",
        "entries_per_active_day": summary.get("entries_per_active_day") if isinstance(summary, dict) else None,
        "proxy_trade_count": required.get("proxy_trade_count") if isinstance(required, dict) else None,
        "proxy_net_pnl_points": required.get("proxy_net_pnl_points") if isinstance(required, dict) else None,
        "proxy_profit_factor": required.get("proxy_profit_factor") if isinstance(required, dict) else None,
        "next_required_action": "produce_c0036_r0001_mt5_logic_parity_evidence",
        "note": "C0036 R0001 proxy evidence recorded; MT5 paired validation remains mandatory regardless of proxy result.",
        "source_campaign_manifest": "campaigns/C0036_intraday_range_impulse_memory_discovery/campaign.yaml",
    }
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0036",
            "c0004_r0001_fold_local_state_archetype": "c0036_r0001_intraday_range_impulse_memory",
            "c0004_r0001": "c0036_r0001",
            "fold_local_state_archetype_discovery": "intraday_range_impulse_memory_discovery",
            ...
            "state_archetype_summary": "intraday_range_impulse_memory_summary",
        }
