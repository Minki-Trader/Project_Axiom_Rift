"""C0031 R0001 proxy evidence for intraday entropy release discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0031_intraday_entropy_release_discovery" / "runs" / "R0001"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0031_intraday_entropy_release_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0031_r0001_proxy_trades.csv"
INTRADAY_ENTROPY_RELEASE_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0031_r0001_intraday_entropy_release_summary.json"
...
FEATURE_NAMES = (
    ...
    "sign_entropy_6",
    "sign_entropy_24",
    "entropy_collapse_6_vs_24",
    "entropy_release_impulse_3",
...
MODEL_FAMILY = "fold_local_intraday_entropy_release_rank"
LABEL_SHAPE = "directional_intraday_entropy_release_survival_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_intraday_entropy_release_scores_per_active_day"
...
def run_c0031_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    entropy_context = build_entropy_context(bars, volumes, range_average, short_range_average)
    trades: list[base.Trade] = []
...
def build_candidates(
    ...
    volumes: list[float],
    entropy_context: list[dict[str, float] | None],
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = intraday_entropy_release_features(
                bars,
                range_average,
                short_range_average,
                day_context,
            ...
            if features is None:
                continue
            label = (
                intraday_entropy_release_label(bars, range_average, entropy_context, index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|intraday_entropy_release",
                    features=features,
                    label=label,
                )
            )
...
def build_entropy_context(
    ...
    for index, bar in enumerate(bars):
        ...
        if index < 48 or average_range is None or average_range <= 0 or short_average_range is None:
            continue
        sign_entropy_6 = close_sign_entropy(bars, index, 6)
        sign_entropy_24 = close_sign_entropy(bars, index, 24)
        range_entropy_12 = tier_entropy(
            [range_ratio_at(bars, range_average, offset) for offset in range(index - 11, index + 1)],
            (0.70, 1.20),
        )
        volume_entropy_12 = tier_entropy(
            [volume_ratio_at(volumes, volume_average_24, offset) for offset in range(index - 11, index + 1)],
            (0.75, 1.35),
        )
        spread_entropy_12 = tier_entropy(
            [bars[offset].spread_points / max(range_average[offset] or average_range, 1e-9) for offset in range(index - 11, index + 1)],
            (0.18, 0.42),
        )
        ...
        contexts[index] = {
            "sign_entropy_6": sign_entropy_6,
            "sign_entropy_24": sign_entropy_24,
            "entropy_collapse_6_vs_24": sign_entropy_24 - sign_entropy_6,
            "range_entropy_12": range_entropy_12,
            "range_compression_6_vs_48": short_average_range / average_range,
            "range_expansion_3_vs_24": range_3 / max(range_24, 1e-9),
            "volume_entropy_12": volume_entropy_12,
            ...
            "spread_entropy_12": spread_entropy_12,
...
def intraday_entropy_release_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    entropy = entropy_context[index]
    if context is None or entropy is None:
        return None
    bar = bars[index]
    ...
    entropy_release_impulse = max(0.0, entropy["entropy_collapse_6_vs_24"]) * directional_return_3
    close_streak = directional_close_streak(bars, index, direction, 6)
    return (
        float(direction),
        directional_return_3,
        directional_return_12,
        entropy["sign_entropy_6"],
        entropy["sign_entropy_24"],
        entropy["entropy_collapse_6_vs_24"],
        entropy_release_impulse,
        directional_run_ratio(bars, index, direction, 6),
        directional_run_ratio(bars, index, -direction, 6),
        direction * body_direction_sum(bars, index, 6) / 6.0,
        entropy["range_entropy_12"],
        entropy["range_compression_6_vs_48"],
        entropy["range_expansion_3_vs_24"],
        entropy["volume_entropy_12"],
        entropy["volume_burst_vs_24"],
        entropy["volume_order_shift_6_vs_24"],
        entropy["spread_entropy_12"],
        entropy["spread_relief_3_vs_24"],
        ...
        entropy["day_volume_progress"],
        entropy["session_progress"],
        spread_over_range,
...
def intraday_entropy_release_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    entropy_context: list[dict[str, float] | None],
    index: int,
    ...
    future_entropy_order = 1.0 - future_close_entropy(bars, entry_index, exit_index)
    future_directional_bias = future_directional_move_ratio(bars, entry_index, exit_index, direction) - 0.5
    future_entropy_retention = future_entropy_order_retention(entropy_context, entry_index, exit_index)
    ...
    return (
        ...
        + 0.30 * future_entropy_order
        + 0.24 * future_directional_bias
        + 0.20 * future_entropy_retention
...
def future_entropy_order_retention(
    entropy_context: list[dict[str, float] | None],
    entry_index: int,
    exit_index: int,
) -> float:
    values: list[float] = []
    for offset in range(entry_index, exit_index + 1):
        context = entropy_context[offset]
        if context is not None:
            values.append(max(0.0, context["entropy_collapse_6_vs_24"]))
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0031"
    payload["work_unit_id"] = "C0031"
    payload["run_id"] = "R0001"
    payload["proxy_id"] = "PX-C0031-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0031_r0001_intraday_entropy_release"
    payload["proxy_config_path"] = "campaigns/C0031_intraday_entropy_release_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_intraday_entropy_release_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0031_intraday_entropy_release_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0031_intraday_entropy_release_discovery/runs/R0001/artifacts/c0031_r0001_proxy_trades.csv",
        "campaigns/C0031_intraday_entropy_release_discovery/runs/R0001/artifacts/c0031_r0001_intraday_entropy_release_summary.json",
    ]
    ...
    profiles["intraday_entropy_release_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0031_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "feature_weight_floor": FEATURE_WEIGHT_FLOOR,
            "feature_weight_ceiling": FEATURE_WEIGHT_CEILING,
            "score_interpretation": "higher_score_means_direct_fold_local_intraday_entropy_release_rank",
            "variant_boundary": (
                "intraday_entropy_release_rank_not_vwap_source_synthesis_range_overlap_calendar_phase_tick_participation_"
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_intraday_entropy_release_summary_artifact(payload, INTRADAY_ENTROPY_RELEASE_SUMMARY_PATH)
    PROXY_PATH.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="ascii")
    trade_hash = base.sha256_file(TRADE_ARTIFACT_PATH)
    summary_hash = base.sha256_file(INTRADAY_ENTROPY_RELEASE_SUMMARY_PATH)
...
def write_intraday_entropy_release_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_intraday_entropy_release_summary_v1",
        "template": False,
        "work_unit_id": "C0031",
        "campaign_id": "C0031",
        "run_id": "R0001",
        ...
        "intraday_entropy_release_profile": profiles["intraday_entropy_release_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "intraday_entropy_release_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0031-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0031_intraday_entropy_release_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0031_intraday_entropy_release_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0031-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0031-R0001-INTRADAY-ENTROPY-RELEASE-SUMMARY",
                "intraday_entropy_release_summary_artifact",
                "json",
                "campaigns/C0031_intraday_entropy_release_discovery/runs/R0001/artifacts/c0031_r0001_intraday_entropy_release_summary.json",
                summary_hash,
                ["campaigns/C0031_intraday_entropy_release_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0031_r0001_intraday_entropy_release",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0031_r0001_proxy_trades.csv"
    evidence["intraday_entropy_release_summary"] = "artifacts/c0031_r0001_intraday_entropy_release_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0031_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0031_r0001_proxy_trades.csv",
        "artifacts/c0031_r0001_intraday_entropy_release_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0031 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0031_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0031_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0031_intraday_entropy_release_discovery"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0031_intraday_entropy_release_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0031_intraday_entropy_release_discovery",
        "open_c0031_r0001_fold_local_intraday_entropy_release_rank_run",
        "produce_c0031_r0001_proxy_evidence",
    ):
    ...
    data["active_run"] = "campaigns/C0031_intraday_entropy_release_discovery/runs/R0001"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["active_campaign"] = "campaigns/C0031_intraday_entropy_release_discovery"
    data["active_run"] = "campaigns/C0031_intraday_entropy_release_discovery/runs/R0001"
    data["latest_operation"] = {
        "id": "produce_c0031_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0031_intraday_entropy_release_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0031_r0001_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0031",
            "c0004_r0001_fold_local_state_archetype": "c0031_r0001_intraday_entropy_release",
            "c0004_r0001": "c0031_r0001",
            "fold_local_state_archetype_discovery": "intraday_entropy_release_discovery",
            ...
            "state_archetype_summary": "intraday_entropy_release_summary",
        }
