"""C0034 R0001 proxy evidence for intraday effort absorption dislocation discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0034_intraday_effort_price_absorption_discovery" / "runs" / "R0001"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0034_intraday_effort_price_absorption_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0034_r0001_proxy_trades.csv"
INTRADAY_EFFORT_PRICE_ABSORPTION_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0034_r0001_effort_absorption_summary.json"
...
FEATURE_NAMES = (
    ...
    "volume_effort_ratio_1",
    "volume_effort_ratio_3",
    "volume_effort_ratio_6",
    "volume_effort_ratio_12",
    "range_effort_ratio_1",
    "range_effort_ratio_3",
    "spread_effort_stress_3",
    "effort_price_efficiency_3",
    "effort_price_efficiency_6",
    "effort_price_efficiency_12",
    "absorption_pressure_3",
    "absorption_pressure_6",
    "absorption_pressure_12",
    "directional_absorption_reversal_pressure",
    "directional_effort_continuation_pressure",
    "effort_momentum_shift_3_vs_12",
    "wick_rejection_pressure_3",
    "wick_follow_pressure_3",
...
MODEL_FAMILY = "fold_local_intraday_effort_price_absorption_rank"
LABEL_SHAPE = "directional_intraday_effort_price_absorption_survival_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_intraday_effort_price_absorption_scores_per_active_day"
...
def run_c0034_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    effort_context = build_effort_context(bars, volumes, range_average, short_range_average)
    trades: list[base.Trade] = []
...
def build_candidates(
    ...
    volumes: list[float],
    effort_context: list[dict[str, float] | None],
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = intraday_effort_price_absorption_features(
                bars,
                range_average,
                short_range_average,
                day_context,
            ...
            if features is None:
                continue
            label = (
                intraday_effort_price_absorption_label(bars, range_average, effort_context, index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|intraday_effort_price_absorption",
                    features=features,
                    label=label,
                )
            )
...
def build_effort_context(
    ...
    for index, bar in enumerate(bars):
        ...
        if index >= 48 and average_range is not None and average_range > 0 and short_average_range is not None:
            ...
            volume_effort_ratio_1 = volume_1 / max(volume_24, 1.0)
            volume_effort_ratio_3 = volume_3 / max(volume_24, 1.0)
            volume_effort_ratio_6 = volume_6 / max(volume_24, 1.0)
            volume_effort_ratio_12 = volume_12 / max(volume_48, 1.0)
            ...
            range_effort_ratio_1 = range_ratio_1 / max(volume_effort_ratio_1, 0.10)
            range_effort_ratio_3 = range_ratio_3 / max(volume_effort_ratio_3, 0.10)
            effort_price_efficiency_3 = abs_progress_3 / max(path_3 * volume_effort_ratio_3, 0.10)
            effort_price_efficiency_6 = abs_progress_6 / max(path_6 * volume_effort_ratio_6, 0.10)
            effort_price_efficiency_12 = abs_progress_12 / max(path_12 * volume_effort_ratio_12, 0.10)
            absorption_pressure_3 = bounded(
                max(0.0, volume_effort_ratio_3 - 1.0) + max(0.0, 1.0 - effort_price_efficiency_3),
                0.0,
                6.0,
            )
            absorption_pressure_6 = bounded(
                max(0.0, volume_effort_ratio_6 - 1.0) + max(0.0, 1.0 - effort_price_efficiency_6),
                0.0,
                6.0,
            )
            absorption_pressure_12 = bounded(
                max(0.0, volume_effort_ratio_12 - 1.0) + max(0.0, 1.0 - effort_price_efficiency_12),
                0.0,
                6.0,
            )
            expected_day_volume = volume_24 * max(bars_since_day_open + 1, 1)
            contexts[index] = {
                "volume_effort_ratio_1": volume_effort_ratio_1,
                "volume_effort_ratio_3": volume_effort_ratio_3,
                "volume_effort_ratio_6": volume_effort_ratio_6,
                "volume_effort_ratio_12": volume_effort_ratio_12,
                "range_effort_ratio_1": range_effort_ratio_1,
                "range_effort_ratio_3": range_effort_ratio_3,
                "spread_effort_stress_3": spread_3 / max(average_range, 1e-9) * max(volume_effort_ratio_3, 0.25),
                "abs_price_progress_3": abs_progress_3,
                "abs_price_progress_6": abs_progress_6,
                "abs_price_progress_12": abs_progress_12,
                "effort_price_efficiency_3": effort_price_efficiency_3,
                "effort_price_efficiency_6": effort_price_efficiency_6,
                "effort_price_efficiency_12": effort_price_efficiency_12,
                ...
                "effort_momentum_shift_3_vs_12": effort_price_efficiency_3 - effort_price_efficiency_12,
...
def intraday_effort_price_absorption_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    effort = effort_context[index]
    if context is None or effort is None:
        return None
    bar = bars[index]
    ...
    directional_return_12 = direction * (bars[index].close - bars[index - 12].close) / average_range
    directional_efficiency_3 = directional_return_3 / max(effort["volume_effort_ratio_3"], 0.10)
    directional_efficiency_6 = directional_return_6 / max(effort["volume_effort_ratio_6"], 0.10)
    directional_efficiency_12 = directional_return_12 / max(effort["volume_effort_ratio_12"], 0.10)
    absorption_reversal = effort["absorption_pressure_6"] * max(0.0, -directional_return_3)
    effort_continuation = max(0.0, directional_return_3) * max(0.0, effort["volume_effort_ratio_3"] - 1.0)
    wick_rejection = directional_rejection_wick(bars, index, direction, 3) * (1.0 + effort["absorption_pressure_3"])
    wick_follow = directional_follow_wick(bars, index, direction, 3) * max(effort["volume_effort_ratio_3"], 0.25)
    ...
    return (
        ...
        effort["volume_effort_ratio_1"],
        effort["volume_effort_ratio_3"],
        effort["volume_effort_ratio_6"],
        effort["volume_effort_ratio_12"],
        effort["range_effort_ratio_1"],
        effort["range_effort_ratio_3"],
        effort["spread_effort_stress_3"],
        directional_efficiency_3,
        directional_efficiency_6,
        directional_efficiency_12,
        effort["absorption_pressure_3"],
        effort["absorption_pressure_6"],
        effort["absorption_pressure_12"],
        absorption_reversal,
        effort_continuation,
        directional_efficiency_3 - directional_efficiency_12,
        wick_rejection,
        wick_follow,
        ...
        effort["range_expansion_3_vs_24"],
        effort["range_compression_6_vs_48"],
        ...
        effort["day_volume_progress"],
        effort["session_progress"],
        spread_over_range,
        effort["spread_relief_3_vs_24"],
    )
...
def intraday_effort_price_absorption_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    effort_context: list[dict[str, float] | None],
    index: int,
    ...
    future_effort_acceptance = future_directional_effort_acceptance(
        bars,
        effort_context,
        entry_index,
        exit_index,
    ...
    future_absorption_retention = future_effort_absorption_retention(effort_context, entry_index, exit_index)
...
def future_effort_absorption_retention(
    effort_context: list[dict[str, float] | None],
    entry_index: int,
    exit_index: int,
) -> float:
    values: list[float] = []
    for offset in range(entry_index, exit_index + 1):
        context = effort_context[offset]
        if context is not None:
            values.append(
                0.48 * context["effort_price_efficiency_3"]
                + 0.22 * context["effort_price_efficiency_6"]
                + 0.16 * context["spread_relief_3_vs_24"]
                - 0.30 * context["absorption_pressure_3"]
                - 0.18 * context["spread_effort_stress_3"]
            )
    return sum(values) / max(len(values), 1)


def future_directional_effort_acceptance(
    bars: list[base.Bar],
    effort_context: list[dict[str, float] | None],
    entry_index: int,
    exit_index: int,
    ...
    for offset in range(entry_index, exit_index + 1):
        ...
        values.append(
            move * max(context["volume_effort_ratio_3"], 0.25)
            + 0.22 * context["effort_price_efficiency_3"]
            - 0.22 * context["absorption_pressure_3"]
            - 0.14 * context["spread_effort_stress_3"]
        )
    if not values:
        return 0.0
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0034"
    payload["work_unit_id"] = "C0034"
    payload["run_id"] = "R0001"
    payload["proxy_id"] = "PX-C0034-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0034_r0001_intraday_effort_price_absorption"
    payload["proxy_config_path"] = "campaigns/C0034_intraday_effort_price_absorption_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_intraday_effort_price_absorption_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0034_intraday_effort_price_absorption_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0034_intraday_effort_price_absorption_discovery/runs/R0001/artifacts/c0034_r0001_proxy_trades.csv",
        "campaigns/C0034_intraday_effort_price_absorption_discovery/runs/R0001/artifacts/c0034_r0001_effort_absorption_summary.json",
    ]
    ...
    profiles["intraday_effort_price_absorption_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0034_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "feature_weight_floor": FEATURE_WEIGHT_FLOOR,
            "feature_weight_ceiling": FEATURE_WEIGHT_CEILING,
            "score_interpretation": "higher_score_means_direct_fold_local_intraday_effort_price_absorption_rank",
            "variant_boundary": (
                "intraday_effort_price_absorption_rank_not_vwap_source_synthesis_range_overlap_calendar_phase_tick_participation_"
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_intraday_effort_price_absorption_summary_artifact(payload, INTRADAY_EFFORT_PRICE_ABSORPTION_SUMMARY_PATH)
    write_text_local(PROXY_PATH, json.dumps(payload, indent=2, sort_keys=True) + "\n")
    trade_hash = sha256_file_local(TRADE_ARTIFACT_PATH)
    summary_hash = sha256_file_local(INTRADAY_EFFORT_PRICE_ABSORPTION_SUMMARY_PATH)
...
def write_intraday_effort_price_absorption_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_intraday_effort_price_absorption_summary_v1",
        "template": False,
        "work_unit_id": "C0034",
        "campaign_id": "C0034",
        "run_id": "R0001",
        ...
        "intraday_effort_price_absorption_profile": profiles["intraday_effort_price_absorption_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "intraday_effort_price_absorption_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0034-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0034_intraday_effort_price_absorption_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0034_intraday_effort_price_absorption_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0034-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0034-R0001-EFFORT-ABSORPTION-SUMMARY",
                "intraday_effort_price_absorption_summary_artifact",
                "json",
                "campaigns/C0034_intraday_effort_price_absorption_discovery/runs/R0001/artifacts/c0034_r0001_effort_absorption_summary.json",
                summary_hash,
                ["campaigns/C0034_intraday_effort_price_absorption_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0034_r0001_intraday_effort_price_absorption",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0034_r0001_proxy_trades.csv"
    evidence["intraday_effort_price_absorption_summary"] = "artifacts/c0034_r0001_effort_absorption_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0034_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0034_r0001_proxy_trades.csv",
        "artifacts/c0034_r0001_effort_absorption_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0034 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0034_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0034_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0034_intraday_effort_price_absorption_discovery"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0034_intraday_effort_price_absorption_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0034_intraday_effort_price_absorption_discovery",
        "open_c0034_r0001_fold_local_intraday_effort_price_absorption_rank_run",
        "produce_c0034_r0001_proxy_evidence",
    ):
    ...
    data["active_run"] = "campaigns/C0034_intraday_effort_price_absorption_discovery/runs/R0001"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["active_campaign"] = "campaigns/C0034_intraday_effort_price_absorption_discovery"
    data["active_run"] = "campaigns/C0034_intraday_effort_price_absorption_discovery/runs/R0001"
    data["latest_operation"] = {
        "id": "produce_c0034_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0034_intraday_effort_price_absorption_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0034_r0001_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0034",
            "c0004_r0001_fold_local_state_archetype": "c0034_r0001_intraday_effort_price_absorption",
            "c0004_r0001": "c0034_r0001",
            "fold_local_state_archetype_discovery": "intraday_effort_price_absorption_discovery",
            ...
            "state_archetype_summary": "intraday_effort_price_absorption_summary",
        }
