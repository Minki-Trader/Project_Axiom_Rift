"""C0101 R0001 proxy evidence for intraday volume clock desynchronization."""
...
CAMPAIGN_ID = "C0101"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0101_intraday_volume_clock_desynchronization_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0101_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0101_r0001_volume_clock_desynchronization_summary.json"
...
FEATURE_NAMES = (
    "volume_clock_lead_lag",
    "participation_desync_pressure",
    "directional_displacement_per_volume",
    "range_per_volume_efficiency",
    "volume_reacceleration",
    "range_absorption_ratio",
    "session_progress_mismatch",
    "prior_day_participation_context",
    "session_phase_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_volume_clock_desynchronization_rank"
LABEL_SHAPE = "target_first_quality_after_volume_time_desynchronization"
SELECTION_RULE = "top_fold_local_volume_clock_desynchronization_scores_per_active_day"
...
MIN_DAY_BARS = 30
VOLUME_CLOCK_DESYNCHRONIZATION_LOOKBACK_BARS = 48
VOLUME_CLOCK_DESYNCHRONIZATION_RECENT_BARS = 12
VOLUME_CLOCK_DESYNCHRONIZATION_PRIOR_BARS = 24
MIN_VOLUME_CLOCK_DESYNCHRONIZATION_ACTIVITY = 0.06
...
class LinearVolumeClockDesynchronizationModel:
    fold_id: str
    ...
    volume_clock_desynchronization_failure_rate: float
...
def run_c0101_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_volume_clock_desynchronization_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_volume_clock_desynchronization_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        VOLUME_CLOCK_DESYNCHRONIZATION_LOOKBACK_BARS,
        VOLUME_CLOCK_DESYNCHRONIZATION_RECENT_BARS + VOLUME_CLOCK_DESYNCHRONIZATION_PRIOR_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = volume_clock_desynchronization_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        metrics["volume_clock_lead_lag"],
        metrics["participation_desync_pressure"],
        metrics["directional_displacement_per_volume"],
        metrics["range_per_volume_efficiency"],
        metrics["volume_reacceleration"],
        metrics["range_absorption_ratio"],
        metrics["session_progress_mismatch"],
        metrics["prior_day_participation_context"],
        metrics["session_phase_maturity"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        c0101_volume_clock_lead_lag_bucket(metrics["volume_clock_lead_lag"]),
        c0101_desync_pressure_bucket(metrics["participation_desync_pressure"]),
        c0101_displacement_per_volume_bucket(metrics["directional_displacement_per_volume"]),
        c0101_range_efficiency_bucket(metrics["range_per_volume_efficiency"]),
        c0101_volume_reacceleration_bucket(metrics["volume_reacceleration"]),
        c0101_absorption_bucket(metrics["range_absorption_ratio"]),
        c0101_progress_mismatch_bucket(metrics["session_progress_mismatch"]),
        c0101_prior_participation_bucket(metrics["prior_day_participation_context"]),
...
def volume_clock_desynchronization_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    recent_start = index - VOLUME_CLOCK_DESYNCHRONIZATION_RECENT_BARS + 1
    prior_start = recent_start - VOLUME_CLOCK_DESYNCHRONIZATION_PRIOR_BARS
    lookback_start = index - VOLUME_CLOCK_DESYNCHRONIZATION_LOOKBACK_BARS + 1
    if recent_start < 0 or prior_start < 0 or lookback_start < 0 or context.day_start_index >= index:
        return {"event_count": 0.0}
    recent_start = max(recent_start, context.day_start_index)
    prior_start = max(prior_start, context.day_start_index)
    ...
    if len(recent_window) < 6 or len(prior_window) < 8 or len(lookback_window) < 18:
        return {"event_count": 0.0}
    ...
    expected_volume_per_bar = context.prior_day_volume_per_bar or average_volume
    if expected_volume_per_bar <= 0 or prior_volume_mean <= 0:
        return {"event_count": 0.0}

    expected_volume_so_far = expected_volume_per_bar * max(float(context.bars_so_far), 1.0)
    volume_clock_lead_lag = context.volume_so_far / max(expected_volume_so_far, 1e-9) - 1.0
    session_phase_maturity = bounded_session_progress(signal_bar.time)
    session_progress_mismatch = session_phase_maturity * volume_clock_lead_lag
    recent_volume_ratio = recent_volume_mean / max(average_volume, 1e-9)
    volume_reacceleration = recent_volume_mean / max(prior_volume_mean, 1e-9) - 1.0
    recent_move = signal_bar.close - recent_window[0].open
    directional_displacement = direction * recent_move / max(average_range, 1e-9)
    directional_displacement_per_volume = directional_displacement / max(recent_volume_ratio, 0.25)
    range_per_volume_efficiency = (recent_range_mean / max(lookback_range_mean, average_range * 0.25)) / max(
        recent_volume_ratio,
        0.25,
    )
    range_absorption_ratio = 1.0 - recent_range_mean / max(prior_range_mean, average_range * 0.35)
    prior_day_participation_context = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_participation_ratio = (context.prior_day_volume_per_bar or average_volume) / max(average_volume, 1e-9)
        prior_day_participation_context = (
            direction
            * context.prior_day_return
            / max(context.prior_day_range, 1e-9)
            * min(max(prior_participation_ratio, 0.25), 3.0)
        )
    participation_desync_pressure = (
        abs(volume_clock_lead_lag)
        * (
            abs(directional_displacement_per_volume)
            + max(range_per_volume_efficiency - 0.25, 0.0)
            + abs(volume_reacceleration) * 0.35
            + max(range_absorption_ratio, 0.0) * 0.25
        )
    )
    activity = max(
        abs(volume_clock_lead_lag) * 0.45,
        participation_desync_pressure,
        abs(volume_reacceleration) * 0.20,
        abs(directional_displacement_per_volume) * 0.16,
        max(range_absorption_ratio, 0.0) * 0.15,
    )
    if activity < MIN_VOLUME_CLOCK_DESYNCHRONIZATION_ACTIVITY:
        return {"event_count": 0.0}
    if abs(volume_clock_lead_lag) < 0.04 and abs(volume_reacceleration) < 0.12:
        return {"event_count": 0.0}
    if abs(directional_displacement_per_volume) < 0.03 and range_per_volume_efficiency < 0.45:
        return {"event_count": 0.0}
    return {
        "volume_clock_lead_lag": bounded(volume_clock_lead_lag, -4.0, 4.0),
        "participation_desync_pressure": bounded(participation_desync_pressure, 0.0, 8.0),
        "directional_displacement_per_volume": bounded(directional_displacement_per_volume, -8.0, 8.0),
        "range_per_volume_efficiency": bounded(range_per_volume_efficiency, 0.0, 8.0),
        "volume_reacceleration": bounded(volume_reacceleration, -4.0, 6.0),
        "range_absorption_ratio": bounded(range_absorption_ratio, -4.0, 1.5),
        "session_progress_mismatch": bounded(session_progress_mismatch, -3.0, 3.0),
        "prior_day_participation_context": bounded(prior_day_participation_context, -4.0, 4.0),
        ...
        "recent_volume_ratio": bounded(recent_volume_ratio, 0.0, 8.0),
        "volume_clock_desynchronization_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0101_volume_clock_lead_lag_bucket(value: float) -> str:
    if value < -0.18:
        return "volume_clock_lagging"
    if value < 0.18:
        return "volume_clock_synchronous"
    return "volume_clock_leading"


def c0101_desync_pressure_bucket(value: float) -> str:
    if value < 0.08:
        return "desync_pressure_light"
    if value < 0.45:
        return "desync_pressure_defined"
    return "desync_pressure_heavy"


def c0101_displacement_per_volume_bucket(value: float) -> str:
    if value < -0.20:
        return "displacement_per_volume_against"
    if value < 0.20:
        return "displacement_per_volume_flat"
    return "displacement_per_volume_with"


def c0101_range_efficiency_bucket(value: float) -> str:
    if value < 0.45:
        return "range_per_volume_absorbed"
    if value < 1.15:
        return "range_per_volume_balanced"
    return "range_per_volume_efficient"


def c0101_volume_reacceleration_bucket(value: float) -> str:
    if value < -0.18:
        return "volume_reacceleration_fading"
    if value < 0.25:
        return "volume_reacceleration_stable"
    return "volume_reacceleration_building"


def c0101_absorption_bucket(value: float) -> str:
    if value < -0.20:
        return "range_absorption_absent"
    if value < 0.25:
        return "range_absorption_mixed"
    return "range_absorption_defined"


def c0101_progress_mismatch_bucket(value: float) -> str:
    if value < -0.08:
        return "session_progress_volume_behind"
    if value < 0.08:
        return "session_progress_matched"
    return "session_progress_volume_ahead"


def c0101_prior_participation_bucket(value: float) -> str:
    if value < -0.30:
        return "prior_participation_against"
    if value < 0.30:
        return "prior_participation_neutral"
    return "prior_participation_aligned"
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    desync_reward = 0.14 if metrics["participation_desync_pressure"] > 0.25 and terminal > 0 else 0.0
    lead_lag_reward = 0.08 if abs(metrics["volume_clock_lead_lag"]) > 0.18 and terminal > 0 else 0.0
    displacement_reward = (
        0.13 if metrics["directional_displacement_per_volume"] > 0.22 and terminal > 0 else 0.0
    )
    efficient_range_reward = 0.10 if metrics["range_per_volume_efficiency"] > 0.80 and terminal > 0 else 0.0
    reacceleration_reward = 0.08 if metrics["volume_reacceleration"] > 0.15 and terminal > 0 else 0.0
    absorption_reward = 0.07 if metrics["range_absorption_ratio"] > 0.20 and terminal > 0 else 0.0
    progress_reward = 0.05 if abs(metrics["session_progress_mismatch"]) > 0.06 and terminal > 0 else 0.0
    prior_reward = 0.05 if metrics["prior_day_participation_context"] > 0.15 and terminal > 0 else 0.0
    weak_desync_penalty = 0.12 if metrics["participation_desync_pressure"] < 0.10 and terminal <= 0 else 0.0
    adverse_displacement_penalty = (
        0.14 if metrics["directional_displacement_per_volume"] < -0.20 and terminal <= 0 else 0.0
    )
    inefficient_range_penalty = 0.10 if metrics["range_per_volume_efficiency"] < 0.35 and terminal <= 0 else 0.0
    fading_volume_penalty = 0.08 if metrics["volume_reacceleration"] < -0.25 and terminal <= 0 else 0.0
    anti_context_penalty = 0.06 if metrics["prior_day_participation_context"] < -0.30 and terminal <= 0 else 0.0
    ...
    volume_clock_desynchronization_path_quality = (0.54 * terminal + 0.30 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + volume_clock_desynchronization_path_quality
        + desync_reward
        + lead_lag_reward
        + displacement_reward
        + efficient_range_reward
        ...
        - weak_desync_penalty
        - adverse_displacement_penalty
        - inefficient_range_penalty
        - fading_volume_penalty
        - anti_context_penalty
        - stall_penalty
        - unfavorable_path_penalty
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["recent_range_ratio"] - 2.40) * 0.05
    )


def fit_linear_volume_clock_desynchronization_model(candidates: list[base.Candidate], fold_id: str) -> LinearVolumeClockDesynchronizationModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearVolumeClockDesynchronizationModel(
            fold_id,
    ...
    volume_clock_desynchronization_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearVolumeClockDesynchronizationModel(
        fold_id=fold_id,
        ...
        volume_clock_desynchronization_failure_rate=volume_clock_desynchronization_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearVolumeClockDesynchronizationModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0101-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_volume_clock_desynchronization_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0101_r0001_intraday_volume_clock_desynchronization",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0101_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0101_r0001_volume_clock_desynchronization_summary.json",
        ],
        ...
        "conditional_profiles": {
            "volume_clock_desynchronization_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0101 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0101_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0101 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "volume_clock_desynchronization_lookback_bars": VOLUME_CLOCK_DESYNCHRONIZATION_LOOKBACK_BARS,
        "volume_clock_desynchronization_recent_bars": VOLUME_CLOCK_DESYNCHRONIZATION_RECENT_BARS,
        "volume_clock_desynchronization_prior_bars": VOLUME_CLOCK_DESYNCHRONIZATION_PRIOR_BARS,
        "min_volume_clock_desynchronization_activity": MIN_VOLUME_CLOCK_DESYNCHRONIZATION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_volume_clock_desynchronization_model_summary(model: LinearVolumeClockDesynchronizationModel) -> dict[str, object]:
    return {
        ...
        "volume_clock_desynchronization_failure_rate": rounded(model.volume_clock_desynchronization_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearVolumeClockDesynchronizationModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_volume_clock_desynchronization_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "volume_clock_desynchronization_profile": profiles["volume_clock_desynchronization_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0101_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0101_r0001_proxy_trades.csv"
    evidence["volume_clock_desynchronization_summary"] = "artifacts/c0101_r0001_volume_clock_desynchronization_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0101_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0101_r0001_proxy_trades.csv",
        "artifacts/c0101_r0001_volume_clock_desynchronization_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0101_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0101 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0101_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0101_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0101_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0101_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0101_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0101_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0101_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0101_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0101_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0101 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0101 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0101_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0101_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0101_r0001_intraday_volume_clock_desynchronization_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0101_r0001_proxy_records_volume_clock_desynchronization_reference_surface
  ...
  - next_work_is_produce_c0101_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0101_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0101-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0101_r0001_intraday_volume_clock_desynchronization",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0101-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0101-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0101-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0101_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0101_r0001_intraday_volume_clock_desynchronization",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0101-R0001-volume-clock-desynchronization-SUMMARY",
            "artifact_role": "volume_clock_desynchronization_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0101_r0001_volume_clock_desynchronization_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0101_r0001_intraday_volume_clock_desynchronization",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0101_r0001_mt5_logic_parity_evidence",
        }
    ]
