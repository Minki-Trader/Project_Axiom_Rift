"""C0065 R0001 proxy evidence for intraday anchor tension resolution."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0065_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0065_r0001_anchor_tension_summary.json"
...
FEATURE_NAMES = (
    ...
    "opening_mid_tension",
    "rolling_mid_tension",
    "anchor_cross_density",
    "anchor_rejection_pressure",
    "anchor_compression_pressure",
    "settlement_alignment",
    "volume_confirmation",
    "prior_day_anchor_contrast",
    "session_maturity_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_anchor_tension_resolution_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_anchor_tension_resolution_state"
SELECTION_RULE = "top_fold_local_anchor_tension_resolution_scores_per_active_day"
...
MIN_DAY_BARS = 30
ANCHOR_TENSION_LOOKBACK_BARS = 14
ANCHOR_TENSION_SHORT_LOOKBACK_BARS = 6
MIN_ANCHOR_TENSION_ACTIVITY = 0.10
...
class LinearAnchorTensionResolutionModel:
    fold_id: str
    ...
    anchor_tension_resolution_failure_rate: float
...
def run_c0065_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_anchor_tension_resolution_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_anchor_tension_resolution_model_summary(model))
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    anchor = anchor_tension_resolution_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        anchor["day_open_tension"],
        anchor["prior_close_tension"],
        anchor["opening_mid_tension"],
        anchor["rolling_mid_tension"],
        anchor["anchor_cross_density"],
        anchor["anchor_rejection_pressure"],
        anchor["anchor_compression_pressure"],
        anchor["settlement_alignment"],
        anchor["volume_confirmation"],
        anchor["prior_day_anchor_contrast"],
        anchor["session_maturity_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        anchor_tension_bucket(anchor["day_open_tension"], "day_open"),
        anchor_tension_bucket(anchor["prior_close_tension"], "prior_close"),
        anchor_tension_bucket(anchor["opening_mid_tension"], "opening_mid"),
        anchor_tension_bucket(anchor["rolling_mid_tension"], "rolling_mid"),
        anchor_cross_bucket(anchor["anchor_cross_density"]),
        anchor_rejection_bucket(anchor["anchor_rejection_pressure"]),
        anchor_compression_bucket(anchor["anchor_compression_pressure"]),
        anchor_settlement_bucket(anchor["settlement_alignment"]),
        volume_confirmation_bucket(anchor["volume_confirmation"]),
        prior_anchor_contrast_bucket(anchor["prior_day_anchor_contrast"]),
        session_pressure_bucket(anchor["session_maturity_pressure"]),
...
def anchor_tension_resolution_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    if context.prior_day_close is None or prior_volume_per_bar is None or prior_volume_per_bar <= 0:
        return {"event_count": 0.0}
    window_start = index - ANCHOR_TENSION_LOOKBACK_BARS + 1
    short_start = index - ANCHOR_TENSION_SHORT_LOOKBACK_BARS + 1
    if window_start < 0 or short_start < 0:
        return {"event_count": 0.0}
    window = bars[window_start : index + 1]
    short_window = bars[short_start : index + 1]
    if len(window) < 6 or len(short_window) < 3:
        return {"event_count": 0.0}
    ...
    rolling_mid = (window_high + window_low) / 2.0
    anchors = (
        context.day_open,
        context.prior_day_close,
        opening_mid,
        rolling_mid,
    )
    day_open_tension = direction * (close - context.day_open) / average_range
    prior_close_tension = direction * (close - context.prior_day_close) / average_range
    opening_mid_tension = direction * (close - opening_mid) / average_range
    rolling_mid_tension = direction * (close - rolling_mid) / average_range
    cross_count = 0
    cross_into_direction_count = 0
    for previous_bar, current_bar in zip(window, window[1:]):
        for anchor in anchors:
            previous_side = price_sign(previous_bar.close - anchor)
            current_side = price_sign(current_bar.close - anchor)
            if previous_side == 0.0 or current_side == 0.0 or previous_side == current_side:
                continue
            cross_count += 1
            if current_side == direction:
                cross_into_direction_count += 1
    cross_denominator = max((len(window) - 1) * len(anchors), 1)
    anchor_cross_density = (0.65 * cross_into_direction_count + 0.35 * cross_count) / cross_denominator
    rejection_scores: list[float] = []
    for item in short_window:
        for anchor in anchors:
            if item.low <= anchor <= item.high:
                side_strength = direction * (item.close - anchor) / average_range
                wick_rejection = directional_wick_rejection(item, direction)
                settlement = directional_bar_settlement(item, direction)
                rejection_scores.append(0.45 * side_strength + 0.35 * wick_rejection + 0.20 * (2.0 * settlement - 1.0))
    anchor_rejection_pressure = base.mean(rejection_scores) or 0.0
    average_anchor_distance = base.mean([abs(close - anchor) for anchor in anchors]) or 0.0
    anchor_compression_pressure = 1.0 / (1.0 + average_anchor_distance / average_range)
    settlement_alignment = directional_bar_settlement(signal_bar, direction)
    volume_window = volumes[short_start : index + 1]
    directional_volume = 0.0
    opposing_volume = 0.0
    for item, volume in zip(short_window, volume_window):
        anchor_bias = base.mean([direction * (item.close - anchor) / average_range for anchor in anchors]) or 0.0
        if anchor_bias > 0.05:
            directional_volume += volume
        elif anchor_bias < -0.05:
            opposing_volume += volume
    expected_recent_volume = prior_volume_per_bar * len(short_window)
    volume_confirmation = (directional_volume - opposing_volume) / max(expected_recent_volume, 1e-9)
    if context.prior_day_return is None or context.prior_day_range is None or context.prior_day_range <= 0:
        prior_day_anchor_contrast = 0.0
    else:
        prior_day_anchor_contrast = -direction * context.prior_day_return / max(
            context.prior_day_range,
            average_range,
        )
    positive_tension = max(
        day_open_tension,
        prior_close_tension,
        opening_mid_tension,
        rolling_mid_tension,
    ...
    session_maturity_pressure = bounded_session_progress(signal_bar.time) * max(
        positive_tension,
        anchor_cross_density,
        max(anchor_rejection_pressure, 0.0),
        anchor_compression_pressure,
        settlement_alignment,
        max(volume_confirmation, 0.0),
    )
    activity = max(
        abs(day_open_tension),
        abs(prior_close_tension),
        abs(opening_mid_tension),
        abs(rolling_mid_tension),
        anchor_cross_density,
        abs(anchor_rejection_pressure),
        anchor_compression_pressure,
        settlement_alignment,
        abs(volume_confirmation),
        abs(prior_day_anchor_contrast),
        session_maturity_pressure,
    )
    if activity < MIN_ANCHOR_TENSION_ACTIVITY:
        return {"event_count": 0.0}
    if (
        anchor_cross_density < 0.035
        and anchor_rejection_pressure < 0.05
        and anchor_compression_pressure < 0.28
        and positive_tension < 0.20
    ):
        return {"event_count": 0.0}
    return {
        "day_open_tension": bounded(day_open_tension, -6.0, 6.0),
        "prior_close_tension": bounded(prior_close_tension, -6.0, 6.0),
        "opening_mid_tension": bounded(opening_mid_tension, -6.0, 6.0),
        "rolling_mid_tension": bounded(rolling_mid_tension, -6.0, 6.0),
        "anchor_cross_density": bounded(anchor_cross_density, 0.0, 1.0),
        "anchor_rejection_pressure": bounded(anchor_rejection_pressure, -2.0, 2.0),
        "anchor_compression_pressure": bounded(anchor_compression_pressure, 0.0, 1.0),
        "settlement_alignment": bounded(settlement_alignment, 0.0, 1.0),
        "volume_confirmation": bounded(volume_confirmation, -4.0, 4.0),
        "prior_day_anchor_contrast": bounded(prior_day_anchor_contrast, -2.0, 2.0),
...
def anchor_tension_bucket(value: float, prefix: str) -> str:
    if value < -0.45:
        return f"{prefix}_against"
    if value < 0.45:
        return f"{prefix}_near"
    return f"{prefix}_with"


def anchor_cross_bucket(value: float) -> str:
    if value < 0.04:
        return "anchor_cross_sparse"
    if value < 0.16:
        return "anchor_cross_present"
    return "anchor_cross_dense"


def anchor_rejection_bucket(value: float) -> str:
    if value < -0.12:
        return "anchor_rejection_failed"
    if value < 0.18:
        return "anchor_rejection_mixed"
    return "anchor_rejection_favorable"


def anchor_compression_bucket(value: float) -> str:
    if value < 0.28:
        return "anchor_compression_loose"
    if value < 0.52:
        return "anchor_compression_mid"
    return "anchor_compression_tight"


def anchor_settlement_bucket(value: float) -> str:
    if value < 0.45:
        return "anchor_settlement_weak"
    if value < 0.68:
        return "anchor_settlement_mid"
    return "anchor_settlement_strong"
...
def prior_anchor_contrast_bucket(value: float) -> str:
    if value < -0.20:
        return "prior_anchor_with_trade"
    if value < 0.20:
        return "prior_anchor_neutral"
    return "prior_anchor_against_trade"
...
def candidate_label(
    ...
    metrics = anchor_tension_resolution_metrics(
        bars,
        volumes,
    ...
    max_tension = max(
        ...
        metrics["prior_close_tension"],
        metrics["opening_mid_tension"],
        metrics["rolling_mid_tension"],
    )
    min_tension = min(
        metrics["day_open_tension"],
        metrics["prior_close_tension"],
    ...
    cross_reward = 0.22 if metrics["anchor_cross_density"] > 0.06 and terminal > 0 else 0.0
    rejection_reward = 0.18 if metrics["anchor_rejection_pressure"] > 0.14 and terminal > 0 else 0.0
    compression_reward = 0.10 if metrics["anchor_compression_pressure"] > 0.42 and terminal > 0 else 0.0
    settlement_reward = 0.14 if metrics["settlement_alignment"] > 0.62 and terminal > 0 else 0.0
    volume_reward = 0.10 if metrics["volume_confirmation"] > 0.18 and terminal > 0 else 0.0
    prior_contrast_reward = 0.06 if metrics["prior_day_anchor_contrast"] > 0.18 and terminal > 0 else 0.0
    failed_anchor_penalty = 0.26 if max_tension < 0.05 and terminal <= 0 else 0.0
    adverse_anchor_penalty = 0.18 if min_tension < -0.45 and terminal <= 0 else 0.0
    failed_rejection_penalty = 0.18 if metrics["anchor_rejection_pressure"] < -0.10 and terminal <= 0 else 0.0
    failed_settlement_penalty = 0.20 if metrics["settlement_alignment"] < 0.42 and terminal <= 0 else 0.0
    chase_penalty = 0.16 if max_tension > 2.20 and terminal <= 0 else 0.0
    ...
    anchor_path_quality = (0.44 * terminal + 0.36 * mfe - 0.72 * mae) / average_range
    ...
    return (
        ...
        + settlement_reward
        + volume_reward
        + prior_contrast_reward
        - failed_anchor_penalty
        - adverse_anchor_penalty
        - failed_rejection_penalty
...
def fit_linear_anchor_tension_resolution_model(candidates: list[base.Candidate], fold_id: str) -> LinearAnchorTensionResolutionModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearAnchorTensionResolutionModel(
            fold_id,
    ...
    anchor_tension_resolution_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearAnchorTensionResolutionModel(
        fold_id=fold_id,
        ...
        anchor_tension_resolution_failure_rate=anchor_tension_resolution_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearAnchorTensionResolutionModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_anchor_tension_resolution_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0065_r0001_intraday_anchor_tension_resolution",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0065_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0065_r0001_anchor_tension_summary.json",
        ],
        ...
        "conditional_profiles": {
            "anchor_tension_resolution_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0065 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0065_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0065 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "anchor_tension_resolution_lookback_bars": ANCHOR_TENSION_LOOKBACK_BARS,
        "anchor_tension_resolution_short_lookback_bars": ANCHOR_TENSION_SHORT_LOOKBACK_BARS,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_anchor_tension_resolution_model_summary(model: LinearAnchorTensionResolutionModel) -> dict[str, object]:
    return {
        ...
        "anchor_tension_resolution_failure_rate": rounded(model.anchor_tension_resolution_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearAnchorTensionResolutionModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_anchor_tension_resolution_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "anchor_tension_resolution_profile": profiles["anchor_tension_resolution_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0065_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0065_r0001_proxy_trades.csv"
    evidence["anchor_tension_resolution_summary"] = "artifacts/c0065_r0001_anchor_tension_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0065_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0065_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0065 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0065_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0065_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0065_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0065_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0065_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0065_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0065_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0065_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0065_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0065 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0065 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260706_produce_c0065_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0065_r0001_proxy_evidence
  refines:
  - dec_20260706_open_c0065_r0001_intraday_anchor_tension_resolution_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0065_r0001_proxy_records_anchor_tension_resolution_reference_surface
  ...
  - next_work_is_produce_c0065_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260706_produce_c0065_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0065-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0065_r0001_intraday_anchor_tension_resolution",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0065-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0065-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0065-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0065_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0065_r0001_intraday_anchor_tension_resolution",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0065-R0001-anchor-tension-resolution-SUMMARY",
            "artifact_role": "anchor_tension_resolution_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0065_r0001_anchor_tension_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0065_r0001_intraday_anchor_tension_resolution",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0065_r0001_mt5_logic_parity_evidence",
        }
    ]
