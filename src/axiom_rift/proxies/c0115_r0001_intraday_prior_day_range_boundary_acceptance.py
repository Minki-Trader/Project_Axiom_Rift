"""C0115 R0001 proxy evidence for intraday prior day range boundary acceptance."""
...
CAMPAIGN_ID = "C0115"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0115_intraday_prior_day_range_boundary_acceptance_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0115_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0115_r0001_prior_day_range_boundary_acceptance_summary.json"
...
FEATURE_NAMES = (
    "prior_boundary_distance",
    "boundary_acceptance_depth",
    "same_side_hold_ratio",
    "boundary_retest_absorption",
    "close_acceptance_strength",
    "breakout_impulse_efficiency",
    "volume_acceptance_ratio",
    "vwap_boundary_alignment",
    "opening_range_alignment",
    "prior_day_direction_context",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_prior_day_range_boundary_acceptance_rank"
LABEL_SHAPE = "target_first_quality_after_prior_day_range_boundary_acceptance"
SELECTION_RULE = "top_fold_local_prior_day_range_boundary_acceptance_scores_per_active_day"
...
MIN_DAY_BARS = 18
PRIOR_DAY_RANGE_BOUNDARY_ACCEPTANCE_LOOKBACK_BARS = 42
BOUNDARY_RECENT_BARS = 10
BOUNDARY_BASE_BARS = 14
MIN_PRIOR_BOUNDARY_EXCURSION = 0.18
MIN_PRIOR_DAY_RANGE_BOUNDARY_ACCEPTANCE_ACTIVITY = 0.060
...
class LinearPriorDayRangeBoundaryAcceptanceModel:
    fold_id: str
    ...
    prior_day_range_boundary_acceptance_failure_rate: float
...
def run_c0115_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_prior_day_range_boundary_acceptance_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_prior_day_range_boundary_acceptance_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        PRIOR_DAY_RANGE_BOUNDARY_ACCEPTANCE_LOOKBACK_BARS,
        BOUNDARY_RECENT_BARS + BOUNDARY_BASE_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = prior_day_range_boundary_acceptance_metrics(
    ...
    features = (
        metrics["prior_boundary_distance"],
        metrics["boundary_acceptance_depth"],
        metrics["same_side_hold_ratio"],
        metrics["boundary_retest_absorption"],
        metrics["close_acceptance_strength"],
        metrics["breakout_impulse_efficiency"],
        metrics["volume_acceptance_ratio"],
        metrics["vwap_boundary_alignment"],
        metrics["opening_range_alignment"],
    ...
    dimensions = (
        prior_boundary_distance_bucket(metrics["prior_boundary_distance"]),
        boundary_acceptance_depth_bucket(metrics["boundary_acceptance_depth"]),
        same_side_hold_ratio_bucket(metrics["same_side_hold_ratio"]),
        boundary_retest_absorption_bucket(metrics["boundary_retest_absorption"]),
        close_acceptance_strength_bucket(metrics["close_acceptance_strength"]),
        breakout_impulse_efficiency_bucket(metrics["breakout_impulse_efficiency"]),
        volume_acceptance_ratio_bucket(metrics["volume_acceptance_ratio"]),
        vwap_boundary_alignment_bucket(metrics["vwap_boundary_alignment"]),
        opening_range_alignment_bucket(metrics["opening_range_alignment"]),
        prior_day_direction_context_bucket(metrics["prior_day_direction_context"]),
...
def prior_day_range_boundary_acceptance_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    if (
        context.prior_day_high is None
        or context.prior_day_low is None
        or context.prior_day_range is None
        or context.prior_day_range <= 0
    ):
        return {"event_count": 0.0}
    recent_start = max(index - BOUNDARY_RECENT_BARS + 1, context.day_start_index)
    base_start = max(recent_start - BOUNDARY_BASE_BARS, context.day_start_index)
    lookback_start = max(index - PRIOR_DAY_RANGE_BOUNDARY_ACCEPTANCE_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    boundary = context.prior_day_high if direction > 0 else context.prior_day_low
    if boundary is None:
        return {"event_count": 0.0}
    boundary_signed_closes = [direction * (bar.close - boundary) for bar in recent_window]
    boundary_signed_extremes = [
        (bar.high - boundary) if direction > 0 else (boundary - bar.low) for bar in recent_window
    ]
    boundary_signed_adverse = [
        (bar.low - boundary) if direction > 0 else (boundary - bar.high) for bar in recent_window
    ]
    prior_boundary_distance = direction * (signal_bar.close - boundary) / average_range
    boundary_acceptance_depth = max(boundary_signed_extremes) / average_range
    if boundary_acceptance_depth < MIN_PRIOR_BOUNDARY_EXCURSION:
        return {"event_count": 0.0}
    same_side_hold_ratio = sum(1 for value in boundary_signed_closes if value > 0) / len(boundary_signed_closes)
    if prior_boundary_distance < -0.18 and same_side_hold_ratio < 0.45:
        return {"event_count": 0.0}

    deepest_retest = min(boundary_signed_adverse) / average_range
    retest_penalty = max(0.0, -deepest_retest)
    boundary_retest_absorption = prior_boundary_distance + same_side_hold_ratio - 0.55 * retest_penalty
    close_acceptance_strength = 0.60 * directional_bar_settlement(signal_bar, direction) + 0.40 * same_side_hold_ratio

    close_path = [bar.close for bar in recent_window]
    close_travel = sum(abs(close_path[pos] - close_path[pos - 1]) for pos in range(1, len(close_path)))
    recent_net = direction * (recent_window[-1].close - recent_window[0].close)
    breakout_impulse_efficiency = recent_net / max(close_travel, average_range * 0.12)
    volume_acceptance_ratio = recent_volume_mean / max(average_volume, 1e-9)
    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    vwap_boundary_alignment = 1.0 - abs(session_vwap - boundary) / max(average_range, 1e-9)
    opening_reference = context.opening_high if direction > 0 else context.opening_low
    opening_range_alignment = direction * (signal_bar.close - opening_reference) / average_range
    prior_day_direction_context = 0.0
    if context.prior_day_return is not None:
    ...
    if context.prior_day_shadow_balance is not None:
        prior_day_direction_context += direction * context.prior_day_shadow_balance * 0.08
    ...
    activity = max(
        max(prior_boundary_distance, 0.0) * 0.36,
        max(boundary_acceptance_depth - 0.15, 0.0) * 0.32,
        max(same_side_hold_ratio - 0.45, 0.0) * 0.44,
        max(boundary_retest_absorption - 0.15, 0.0) * 0.28,
        max(close_acceptance_strength - 0.45, 0.0) * 0.26,
        max(breakout_impulse_efficiency, 0.0) * 0.22,
        max(volume_acceptance_ratio - 0.80, 0.0) * 0.08,
        max(vwap_boundary_alignment - 0.20, 0.0) * 0.12,
        max(opening_range_alignment, 0.0) * 0.12,
    )
    if activity < MIN_PRIOR_DAY_RANGE_BOUNDARY_ACCEPTANCE_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "prior_boundary_distance": bounded(prior_boundary_distance, -4.0, 8.0),
        "boundary_acceptance_depth": bounded(boundary_acceptance_depth, -2.0, 8.0),
        "same_side_hold_ratio": bounded(same_side_hold_ratio, 0.0, 1.0),
        "boundary_retest_absorption": bounded(boundary_retest_absorption, -4.0, 8.0),
        "close_acceptance_strength": bounded(close_acceptance_strength, 0.0, 1.0),
        "breakout_impulse_efficiency": bounded(breakout_impulse_efficiency, -2.0, 2.0),
        "volume_acceptance_ratio": bounded(volume_acceptance_ratio, 0.0, 8.0),
        "vwap_boundary_alignment": bounded(vwap_boundary_alignment, -4.0, 1.0),
        "opening_range_alignment": bounded(opening_range_alignment, -4.0, 8.0),
        ...
        "prior_day_range_boundary_acceptance_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def prior_boundary_distance_bucket(value: float) -> str:
    if value < -0.08:
        return "prior_boundary_not_accepted"
    if value < 0.25:
        return "prior_boundary_marginal_acceptance"
    return "prior_boundary_clear_acceptance"


def boundary_acceptance_depth_bucket(value: float) -> str:
    if value < 0.28:
        return "boundary_probe_shallow"
    if value < 0.85:
        return "boundary_probe_defined"
    return "boundary_probe_extended"


def same_side_hold_ratio_bucket(value: float) -> str:
    if value < 0.45:
        return "boundary_hold_unstable"
    if value < 0.75:
        return "boundary_hold_mixed"
    return "boundary_hold_persistent"


def boundary_retest_absorption_bucket(value: float) -> str:
    if value < 0.10:
        return "boundary_retest_absorption_weak"
    if value < 0.72:
        return "boundary_retest_absorption_defined"
    return "boundary_retest_absorption_strong"


def close_acceptance_strength_bucket(value: float) -> str:
    if value < 0.45:
        return "close_acceptance_soft"
    if value < 0.72:
        return "close_acceptance_tradeable"
    return "close_acceptance_firm"


def breakout_impulse_efficiency_bucket(value: float) -> str:
    if value < -0.05:
        return "breakout_impulse_fading"
    if value < 0.35:
        return "breakout_impulse_mixed"
    return "breakout_impulse_efficient"


def volume_acceptance_ratio_bucket(value: float) -> str:
    if value < 0.78:
        return "volume_acceptance_thin"
    if value < 1.55:
        return "volume_acceptance_orderly"
    return "volume_acceptance_hot"


def vwap_boundary_alignment_bucket(value: float) -> str:
    if value < -0.45:
        return "vwap_boundary_far"
    if value < 0.30:
        return "vwap_boundary_near"
    return "vwap_boundary_anchor"


def opening_range_alignment_bucket(value: float) -> str:
    if value < -0.10:
        return "opening_range_against"
    if value < 0.35:
        return "opening_range_neutral"
    return "opening_range_aligned"


def prior_day_direction_context_bucket(value: float) -> str:
    if value < -0.22:
        return "prior_day_context_counter"
    if value < 0.22:
        return "prior_day_context_neutral"
    return "prior_day_context_aligned"
...
def c0115_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0115_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0115_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0115_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0115_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0115_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0115_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0115_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0115_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0115_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0115_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0115_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0115_boundary_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0115_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0115_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0115_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0115_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0115_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0115_prior_boundary_bucket(value: float) -> str:
    if value < -0.12:
...
def c0115_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    boundary_reward = 0.12 if metrics["prior_boundary_distance"] > 0.18 and terminal > 0 else 0.0
    acceptance_reward = 0.12 if metrics["boundary_acceptance_depth"] > 0.35 and terminal > 0 else 0.0
    hold_reward = 0.10 if metrics["same_side_hold_ratio"] > 0.62 and terminal > 0 else 0.0
    retest_reward = 0.10 if metrics["boundary_retest_absorption"] > 0.25 and terminal > 0 else 0.0
    close_reward = 0.08 if metrics["close_acceptance_strength"] > 0.58 and terminal > 0 else 0.0
    impulse_reward = 0.08 if metrics["breakout_impulse_efficiency"] > 0.18 and terminal > 0 else 0.0
    volume_reward = 0.06 if 0.85 <= metrics["volume_acceptance_ratio"] <= 1.90 and terminal > 0 else 0.0
    vwap_reward = 0.06 if metrics["vwap_boundary_alignment"] > 0.20 and terminal > 0 else 0.0
    opening_reward = 0.06 if metrics["opening_range_alignment"] > 0.12 and terminal > 0 else 0.0
    weak_boundary_penalty = 0.10 if metrics["prior_boundary_distance"] < 0.02 and terminal <= 0 else 0.0
    weak_hold_penalty = 0.10 if metrics["same_side_hold_ratio"] < 0.45 and terminal <= 0 else 0.0
    weak_retest_penalty = 0.08 if metrics["boundary_retest_absorption"] < 0.00 and terminal <= 0 else 0.0
    weak_close_penalty = 0.07 if metrics["close_acceptance_strength"] < 0.42 and terminal <= 0 else 0.0
    weak_impulse_penalty = 0.07 if metrics["breakout_impulse_efficiency"] < -0.05 and terminal <= 0 else 0.0
    overheated_volume_penalty = 0.06 if metrics["volume_acceptance_ratio"] > 2.60 and terminal <= 0 else 0.0
    ...
    prior_day_range_boundary_acceptance_path_quality = (0.62 * terminal + 0.28 * mfe - 0.65 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + prior_day_range_boundary_acceptance_path_quality
        + boundary_reward
        + acceptance_reward
        + hold_reward
        + retest_reward
        ...
        - max(0.0, 0.38 - metrics["same_side_hold_ratio"]) * 0.12
    )


def fit_linear_prior_day_range_boundary_acceptance_model(candidates: list[base.Candidate], fold_id: str) -> LinearPriorDayRangeBoundaryAcceptanceModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearPriorDayRangeBoundaryAcceptanceModel(
            fold_id,
    ...
    prior_day_range_boundary_acceptance_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearPriorDayRangeBoundaryAcceptanceModel(
        fold_id=fold_id,
        ...
        prior_day_range_boundary_acceptance_failure_rate=prior_day_range_boundary_acceptance_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearPriorDayRangeBoundaryAcceptanceModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0115-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_prior_day_range_boundary_acceptance_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0115_r0001_intraday_prior_day_range_boundary_acceptance",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0115_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0115_r0001_prior_day_range_boundary_acceptance_summary.json",
        ],
        ...
        "conditional_profiles": {
            "prior_day_range_boundary_acceptance_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0115 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0115_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0115 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "prior_day_range_boundary_acceptance_lookback_bars": PRIOR_DAY_RANGE_BOUNDARY_ACCEPTANCE_LOOKBACK_BARS,
        "prior_day_range_boundary_acceptance_recent_bars": BOUNDARY_RECENT_BARS,
        "prior_day_range_boundary_acceptance_prior_bars": BOUNDARY_BASE_BARS,
        "min_prior_boundary_excursion": MIN_PRIOR_BOUNDARY_EXCURSION,
        "min_prior_day_range_boundary_acceptance_activity": MIN_PRIOR_DAY_RANGE_BOUNDARY_ACCEPTANCE_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_prior_day_range_boundary_acceptance_model_summary(model: LinearPriorDayRangeBoundaryAcceptanceModel) -> dict[str, object]:
    return {
        ...
        "prior_day_range_boundary_acceptance_failure_rate": rounded(model.prior_day_range_boundary_acceptance_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearPriorDayRangeBoundaryAcceptanceModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_prior_day_range_boundary_acceptance_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "prior_day_range_boundary_acceptance_profile": profiles["prior_day_range_boundary_acceptance_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0115_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0115_r0001_proxy_trades.csv"
    evidence["prior_day_range_boundary_acceptance_summary"] = "artifacts/c0115_r0001_prior_day_range_boundary_acceptance_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0115_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0115_r0001_proxy_trades.csv",
        "artifacts/c0115_r0001_prior_day_range_boundary_acceptance_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0115_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0115 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0115_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0115_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0115_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0115_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0115_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0115_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0115_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0115_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0115_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0115 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0115 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0115_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0115_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0115_r0001_intraday_prior_day_range_boundary_acceptance_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0115_r0001_proxy_records_prior_day_range_boundary_acceptance_reference_surface
  ...
  - next_work_is_produce_c0115_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0115_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0115-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0115_r0001_intraday_prior_day_range_boundary_acceptance",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0115-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0115-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0115-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0115_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0115_r0001_intraday_prior_day_range_boundary_acceptance",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0115-R0001-PRIOR-DAY-RANGE-BOUNDARY-ACCEPTANCE-SUMMARY",
            "artifact_role": "prior_day_range_boundary_acceptance_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0115_r0001_prior_day_range_boundary_acceptance_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0115_r0001_intraday_prior_day_range_boundary_acceptance",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0115_r0001_mt5_logic_parity_evidence",
        }
    ]
