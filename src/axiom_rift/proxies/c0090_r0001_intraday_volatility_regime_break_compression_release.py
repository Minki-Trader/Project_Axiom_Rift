"""C0090 R0001 proxy evidence for intraday volatility regime break compression release."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0090_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0090_r0001_volatility_regime_break_compression_release_summary.json"
...
FEATURE_NAMES = (
    ...
    "directional_body_commitment",
    "close_location_drive",
    "participation_expansion",
    "prebreak_range_contraction",
    "path_efficiency",
    "session_phase_maturity",
    "breakout_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_volatility_regime_break_compression_release_rank"
LABEL_SHAPE = "target_first_escape_quality_conditioned_on_volatility_regime_break_compression_release_state"
SELECTION_RULE = "top_fold_local_volatility_regime_break_compression_release_scores_per_active_day"
...
MIN_DAY_BARS = 30
VOLATILITY_REGIME_BREAK_COMPRESSION_RELEASE_LOOKBACK_BARS = 36
VOLATILITY_REGIME_BREAK_COMPRESSION_RELEASE_SHORT_LOOKBACK_BARS = 10
MIN_VOLATILITY_REGIME_BREAK_COMPRESSION_RELEASE_ACTIVITY = 0.16
...
class LinearVolatilityRegimeBreakCompressionReleaseModel:
    fold_id: str
    ...
    volatility_regime_break_compression_release_failure_rate: float
...
def run_c0090_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_volatility_regime_break_compression_release_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_volatility_regime_break_compression_release_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        VOLATILITY_REGIME_BREAK_COMPRESSION_RELEASE_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = volatility_regime_break_compression_release_metrics(
    ...
    features = (
        ...
        metrics["volatility_expansion_ratio"],
        metrics["range_break_strength"],
        metrics["directional_body_commitment"],
        metrics["close_location_drive"],
        metrics["participation_expansion"],
        metrics["prebreak_range_contraction"],
        metrics["path_efficiency"],
        metrics["session_phase_maturity"],
        metrics["breakout_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        break_compression_depth_bucket(metrics["compression_depth"]),
        volatility_expansion_ratio_bucket(metrics["volatility_expansion_ratio"]),
        range_break_strength_bucket(metrics["range_break_strength"]),
        directional_body_commitment_bucket(metrics["directional_body_commitment"]),
        close_location_drive_bucket(metrics["close_location_drive"]),
        participation_expansion_bucket(metrics["participation_expansion"]),
        prebreak_range_contraction_bucket(metrics["prebreak_range_contraction"]),
        path_efficiency_bucket(metrics["path_efficiency"]),
        session_phase_maturity_bucket(metrics["session_phase_maturity"]),
        breakout_pressure_bucket(metrics["breakout_pressure"]),
...
def volatility_regime_break_compression_release_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    short_start = index - VOLATILITY_REGIME_BREAK_COMPRESSION_RELEASE_SHORT_LOOKBACK_BARS + 1
    prior_start = index - VOLATILITY_REGIME_BREAK_COMPRESSION_RELEASE_LOOKBACK_BARS + 1
    if short_start < 0 or prior_start < 0 or context.day_start_index >= index:
        return {"event_count": 0.0}
    prior_short_start = short_start - VOLATILITY_REGIME_BREAK_COMPRESSION_RELEASE_SHORT_LOOKBACK_BARS
    ...
    short_ranges = [bar.high - bar.low for bar in short_window]
    prior_avg_range = base.mean(prior_ranges) or average_range
    short_avg_range = base.mean(short_ranges) or average_range
    signal_range = max(signal_bar.high - signal_bar.low, 1e-9)
    previous_high = max(bar.high for bar in short_window)
    previous_low = min(bar.low for bar in short_window)
    if direction > 0:
        range_break_strength = (signal_bar.close - previous_high) / average_range
    else:
        range_break_strength = (previous_low - signal_bar.close) / average_range
    compression_depth = 1.0 - short_avg_range / max(prior_avg_range, 1e-9)
    volatility_expansion_ratio = signal_range / max(short_avg_range, 1e-9)
    directional_body_commitment = direction * (signal_bar.close - signal_bar.open) / signal_range
    close_location_drive = directional_bar_settlement(signal_bar, direction)
    prebreak_range_contraction = 1.0 - short_avg_range / max(average_range, 1e-9)
    path_efficiency = directional_path_efficiency(
        bars,
        index,
        direction,
        VOLATILITY_REGIME_BREAK_COMPRESSION_RELEASE_SHORT_LOOKBACK_BARS,
    )
    ...
    participation_expansion = (current_volume_ratio - early_volume_ratio) * max(directional_body_commitment, 0.0)
    session_phase_maturity = bounded_session_progress(signal_bar.time)
    breakout_pressure_raw = max(
        max(compression_depth, 0.0),
        max(prebreak_range_contraction, 0.0),
        max(volatility_expansion_ratio - 1.0, 0.0),
        max(range_break_strength, 0.0),
        max(directional_body_commitment, 0.0),
        max(close_location_drive - 0.50, 0.0) * 2.0,
        min(max(participation_expansion, 0.0), 1.0),
        max(path_efficiency, 0.0),
    )
    breakout_pressure = session_phase_maturity * breakout_pressure_raw
    activity = max(breakout_pressure_raw, breakout_pressure)
    if activity < MIN_VOLATILITY_REGIME_BREAK_COMPRESSION_RELEASE_ACTIVITY:
        return {"event_count": 0.0}
    if compression_depth < 0.04 and prebreak_range_contraction < 0.04:
        return {"event_count": 0.0}
    if volatility_expansion_ratio < 0.85:
        return {"event_count": 0.0}
    if range_break_strength < -0.05:
        return {"event_count": 0.0}
    if directional_body_commitment < -0.18 and close_location_drive < 0.45:
        return {"event_count": 0.0}
    return {
        "compression_depth": bounded(compression_depth, -2.0, 2.0),
        "volatility_expansion_ratio": bounded(volatility_expansion_ratio, 0.0, 6.0),
        "range_break_strength": bounded(range_break_strength, -2.0, 6.0),
        "directional_body_commitment": bounded(directional_body_commitment, -1.0, 1.0),
        "close_location_drive": bounded(close_location_drive, 0.0, 1.0),
        "participation_expansion": bounded(participation_expansion, -3.0, 6.0),
        "prebreak_range_contraction": bounded(prebreak_range_contraction, -2.0, 2.0),
        "path_efficiency": bounded(path_efficiency, -1.0, 1.0),
        "session_phase_maturity": bounded(session_phase_maturity, 0.0, 1.0),
        "breakout_pressure": bounded(breakout_pressure, 0.0, 4.0),
        "event_count": 1.0,
    }
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    compression_reward = 0.12 if metrics["compression_depth"] > 0.12 and terminal > 0 else 0.0
    expansion_reward = 0.14 if metrics["volatility_expansion_ratio"] > 1.18 and terminal > 0 else 0.0
    break_reward = 0.14 if metrics["range_break_strength"] > 0.08 and terminal > 0 else 0.0
    body_reward = 0.12 if metrics["directional_body_commitment"] > 0.26 and terminal > 0 else 0.0
    close_reward = 0.10 if metrics["close_location_drive"] > 0.60 and terminal > 0 else 0.0
    participation_reward = 0.08 if metrics["participation_expansion"] > 0.05 and terminal > 0 else 0.0
    path_reward = 0.08 if metrics["path_efficiency"] > 0.18 and terminal > 0 else 0.0
    session_reward = 0.05 if metrics["breakout_pressure"] > 0.12 and terminal > 0 else 0.0
    shallow_compression_penalty = 0.10 if metrics["compression_depth"] < 0.06 and terminal <= 0 else 0.0
    failed_break_penalty = 0.12 if metrics["range_break_strength"] < 0.00 and terminal <= 0 else 0.0
    weak_close_penalty = 0.10 if metrics["close_location_drive"] < 0.45 and terminal <= 0 else 0.0
    adverse_body_penalty = 0.10 if metrics["directional_body_commitment"] < -0.08 and terminal <= 0 else 0.0
    no_expansion_penalty = 0.10 if metrics["volatility_expansion_ratio"] < 0.95 and terminal <= 0 else 0.0
    no_participation_penalty = 0.08 if metrics["participation_expansion"] < -0.05 and terminal <= 0 else 0.0
    ...
    volatility_regime_break_compression_release_path_quality = (0.52 * terminal + 0.34 * mfe - 0.68 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + volatility_regime_break_compression_release_path_quality
        + compression_reward
        + expansion_reward
        + break_reward
        + body_reward
        ...
        + path_reward
        + session_reward
        - shallow_compression_penalty
        - failed_break_penalty
        - weak_close_penalty
        - adverse_body_penalty
        ...
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["volatility_expansion_ratio"] - 3.00) * 0.04
    )


def fit_linear_volatility_regime_break_compression_release_model(candidates: list[base.Candidate], fold_id: str) -> LinearVolatilityRegimeBreakCompressionReleaseModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearVolatilityRegimeBreakCompressionReleaseModel(
            fold_id,
    ...
    volatility_regime_break_compression_release_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearVolatilityRegimeBreakCompressionReleaseModel(
        fold_id=fold_id,
        ...
        volatility_regime_break_compression_release_failure_rate=volatility_regime_break_compression_release_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearVolatilityRegimeBreakCompressionReleaseModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_volatility_regime_break_compression_release_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0090_r0001_intraday_volatility_regime_break_compression_release",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0090_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0090_r0001_volatility_regime_break_compression_release_summary.json",
        ],
        ...
        "conditional_profiles": {
            "volatility_regime_break_compression_release_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0090 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0090_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0090 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "volatility_regime_break_compression_release_lookback_bars": VOLATILITY_REGIME_BREAK_COMPRESSION_RELEASE_LOOKBACK_BARS,
        "volatility_regime_break_compression_release_short_lookback_bars": VOLATILITY_REGIME_BREAK_COMPRESSION_RELEASE_SHORT_LOOKBACK_BARS,
        "min_volatility_regime_break_compression_release_activity": MIN_VOLATILITY_REGIME_BREAK_COMPRESSION_RELEASE_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_volatility_regime_break_compression_release_model_summary(model: LinearVolatilityRegimeBreakCompressionReleaseModel) -> dict[str, object]:
    return {
        ...
        "volatility_regime_break_compression_release_failure_rate": rounded(model.volatility_regime_break_compression_release_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearVolatilityRegimeBreakCompressionReleaseModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_volatility_regime_break_compression_release_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "volatility_regime_break_compression_release_profile": profiles["volatility_regime_break_compression_release_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0090_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0090_r0001_proxy_trades.csv"
    evidence["volatility_regime_break_compression_release_summary"] = "artifacts/c0090_r0001_volatility_regime_break_compression_release_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0090_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0090_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0090 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0090_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0090_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0090_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0090_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0090_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0090_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0090_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0090_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0090_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0090 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0090 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0090_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0090_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0090_r0001_intraday_volatility_regime_break_compression_release_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0090_r0001_proxy_records_volatility_regime_break_compression_release_reference_surface
  ...
  - next_work_is_produce_c0090_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0090_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0090-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0090_r0001_intraday_volatility_regime_break_compression_release",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0090-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0090-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0090-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0090_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0090_r0001_intraday_volatility_regime_break_compression_release",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0090-R0001-volatility-regime-break-compression-release-SUMMARY",
            "artifact_role": "volatility_regime_break_compression_release_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0090_r0001_volatility_regime_break_compression_release_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0090_r0001_intraday_volatility_regime_break_compression_release",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0090_r0001_mt5_logic_parity_evidence",
        }
    ]
