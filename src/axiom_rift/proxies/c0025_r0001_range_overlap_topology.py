"""C0025 R0001 proxy evidence for range overlap topology discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0025_range_overlap_topology_discovery" / "runs" / "R0001"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0025_range_overlap_topology_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0025_r0001_proxy_trades.csv"
RANGE_OVERLAP_TOPOLOGY_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0025_r0001_range_overlap_topology_summary.json"
...
FEATURE_NAMES = (
    ...
    "range_rel_12",
    "body_to_range",
    "directional_body_to_range",
    "directional_close_location",
    "directional_close_from_mid",
    "favorable_wick_to_range",
    "adverse_wick_to_range",
    "overlap_prev_ratio",
    "overlap_3_avg",
    "overlap_6_avg",
    "overlap_12_avg",
    "overlap_24_avg",
    "overlap_decay_3_vs_12",
    "inside_prev",
    "outside_prev",
    "inside_count_6",
    "outside_count_6",
    ...
    "outside_count_18",
    "containment_count_6",
    "containment_count_18",
    "range_center_shift_3",
    "range_center_shift_6",
    "range_center_shift_18",
    "box_position_6",
    "box_position_18",
    "box_position_36",
    "directional_box_position_6",
    "directional_box_position_18",
    "directional_box_position_36",
    "directional_escape_3",
    "directional_escape_6",
    "directional_escape_18",
    "close_inside_prior_3",
    "close_inside_prior_12",
    "return_to_prior_range_3",
    "spread_over_range",
    "spread_relief_3_vs_24",
)
MODEL_FAMILY = "fold_local_range_overlap_topology_rank"
LABEL_SHAPE = "directional_range_overlap_topology_transition_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_range_overlap_topology_scores_per_active_day"
...
def run_c0025_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = range_overlap_topology_features(
                bars,
                range_average,
                short_range_average,
                day_context,
            ...
            if features is None:
                continue
            label = (
                range_overlap_topology_label(bars, range_average, index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|range_overlap_topology",
                    features=features,
                    label=label,
                )
            )
...
def range_overlap_topology_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    close_location = bounded((bar.close - bar.low) / current_range, 0.0, 1.0)
    directional_close_location = close_location if direction > 0 else 1.0 - close_location
    favorable_wick = bar.high - max(bar.open, bar.close) if direction > 0 else min(bar.open, bar.close) - bar.low
    adverse_wick = min(bar.open, bar.close) - bar.low if direction > 0 else bar.high - max(bar.open, bar.close)
    ...
    return (
        ...
        current_range / short_average_range,
        abs(body) / current_range,
        direction * body / current_range,
        directional_close_location,
        2.0 * (directional_close_location - 0.5),
        bounded(favorable_wick / current_range, 0.0, 1.0),
        bounded(adverse_wick / current_range, 0.0, 1.0),
        bar_overlap_ratio(bars, index, index - 1),
        average_overlap_ratio(bars, index, 3),
        average_overlap_ratio(bars, index, 6),
        average_overlap_ratio(bars, index, 12),
        average_overlap_ratio(bars, index, 24),
        average_overlap_ratio(bars, index, 3) - average_overlap_ratio(bars, index, 12),
        1.0 if is_inside_bar(bars, index, index - 1) else 0.0,
        1.0 if is_outside_bar(bars, index, index - 1) else 0.0,
        inside_bar_count(bars, index, 6),
        outside_bar_count(bars, index, 6),
        inside_bar_count(bars, index, 18),
        outside_bar_count(bars, index, 18),
        contained_range_count(bars, index, 6),
        contained_range_count(bars, index, 18),
        range_center_shift(bars, index, direction, 3, average_range),
        range_center_shift(bars, index, direction, 6, average_range),
        range_center_shift(bars, index, direction, 18, average_range),
        box_position(bars, index, 6),
        box_position(bars, index, 18),
        box_position(bars, index, 36),
        directional_box_position(bars, index, direction, 6),
        directional_box_position(bars, index, direction, 18),
        directional_box_position(bars, index, direction, 36),
        directional_escape_pressure(bars, index, direction, 3, average_range),
        directional_escape_pressure(bars, index, direction, 6, average_range),
        directional_escape_pressure(bars, index, direction, 18, average_range),
        close_inside_prior_range(bars, index, 3),
        close_inside_prior_range(bars, index, 12),
        directional_return_to_prior_range(bars, index, direction, 3),
        spread_over_range,
        spread_relief,
    )
...
def range_overlap_topology_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    index: int,
    direction: int,
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0025"
    payload["work_unit_id"] = "C0025"
    payload["run_id"] = "R0001"
    payload["proxy_id"] = "PX-C0025-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0025_r0001_range_overlap_topology"
    payload["proxy_config_path"] = "campaigns/C0025_range_overlap_topology_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_range_overlap_topology_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0025_range_overlap_topology_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0025_range_overlap_topology_discovery/runs/R0001/artifacts/c0025_r0001_proxy_trades.csv",
        "campaigns/C0025_range_overlap_topology_discovery/runs/R0001/artifacts/c0025_r0001_range_overlap_topology_summary.json",
    ]
    ...
    profiles["range_overlap_topology_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0025_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "feature_weight_floor": FEATURE_WEIGHT_FLOOR,
            "feature_weight_ceiling": FEATURE_WEIGHT_CEILING,
            "score_interpretation": "higher_score_means_direct_fold_local_range_overlap_topology_rank",
            "variant_boundary": (
                "range_overlap_topology_rank_not_calendar_phase_tick_participation_volatility_term_"
                "daily_profile_excursion_bar_quality_micro_gap_round_level_liquidity_vacuum_"
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_range_overlap_topology_summary_artifact(payload, RANGE_OVERLAP_TOPOLOGY_SUMMARY_PATH)
    PROXY_PATH.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="ascii")
    trade_hash = base.sha256_file(TRADE_ARTIFACT_PATH)
    summary_hash = base.sha256_file(RANGE_OVERLAP_TOPOLOGY_SUMMARY_PATH)
...
def write_range_overlap_topology_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_range_overlap_topology_summary_v1",
        "template": False,
        "work_unit_id": "C0025",
        "campaign_id": "C0025",
        "run_id": "R0001",
        ...
        "range_overlap_topology_profile": profiles["range_overlap_topology_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "range_overlap_topology_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0025-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0025_range_overlap_topology_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0025_range_overlap_topology_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0025-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0025-R0001-RANGE-OVERLAP-TOPOLOGY-SUMMARY",
                "range_overlap_topology_summary_artifact",
                "json",
                "campaigns/C0025_range_overlap_topology_discovery/runs/R0001/artifacts/c0025_r0001_range_overlap_topology_summary.json",
                summary_hash,
                ["campaigns/C0025_range_overlap_topology_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0025_r0001_range_overlap_topology",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0025_r0001_proxy_trades.csv"
    evidence["range_overlap_topology_summary"] = "artifacts/c0025_r0001_range_overlap_topology_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0025_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0025_r0001_proxy_trades.csv",
        "artifacts/c0025_r0001_range_overlap_topology_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0025 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0025_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0025_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0025_range_overlap_topology_discovery"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0025_range_overlap_topology_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0025_range_overlap_topology_discovery",
        "open_c0025_r0001_fold_local_range_overlap_topology_rank_run",
        "produce_c0025_r0001_proxy_evidence",
    ):
    ...
    data["active_run"] = "campaigns/C0025_range_overlap_topology_discovery/runs/R0001"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["active_campaign"] = "campaigns/C0025_range_overlap_topology_discovery"
    data["active_run"] = "campaigns/C0025_range_overlap_topology_discovery/runs/R0001"
    data["latest_operation"] = {
        "id": "produce_c0025_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0025_range_overlap_topology_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0025_r0001_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0025",
            "c0004_r0001_fold_local_state_archetype": "c0025_r0001_range_overlap_topology",
            "c0004_r0001": "c0025_r0001",
            "fold_local_state_archetype_discovery": "range_overlap_topology_discovery",
            ...
            "state_archetype_summary": "range_overlap_topology_summary",
        }
