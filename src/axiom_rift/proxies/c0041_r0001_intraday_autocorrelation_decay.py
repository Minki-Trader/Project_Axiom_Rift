"""C0041 R0001 proxy evidence for intraday autocorrelation decay discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0041_intraday_autocorrelation_decay_discovery" / "runs" / "R0001"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0041_intraday_autocorrelation_decay_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0041_r0001_proxy_trades.csv"
AUTOCORRELATION_DECAY_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0041_r0001_autocorrelation_decay_summary.json"
...
FEATURE_NAMES = (
    ...
    "directional_return_2_atr",
    "directional_return_3_atr",
    "directional_return_6_atr",
    "directional_return_12_atr",
    "lag1_directional_agreement_12",
    "lag1_directional_agreement_24",
    "lag2_directional_agreement_24",
    "lag3_directional_agreement_48",
    "sign_autocorr_12",
    "sign_autocorr_24",
    "sign_autocorr_48",
    "directional_sign_autocorr_24",
    "alternating_rate_12",
    "alternating_rate_24",
    "continuation_rate_12",
    "continuation_rate_24",
    "directional_alternation_pressure_24",
    "directional_continuation_pressure_24",
    "autocorr_decay_slope_12_48",
    "variance_ratio_3_12",
    "variance_ratio_6_24",
    "variance_ratio_12_48",
    "directional_variance_ratio_6_24",
    "return_z_1_24",
    "lagged_return_z_1_24",
    "directional_reversal_pressure",
    "directional_continuation_impulse",
    "impulse_reversal_pressure_12",
    "impulse_continuation_pressure_12",
    "rolling_mean_return_12",
    "rolling_mean_return_24",
    "rolling_mean_return_48",
    "directional_mean_return_24",
    "range_adjusted_return_rank_24",
    "body_return_agreement_12",
    "body_return_agreement_24",
    "path_efficiency_12",
    "churn_ratio_12",
    "day_autocorrelation_position",
    "session_progress",
    "spread_over_range",
    "spread_serial_stress_24",
    "autocorr_regime_contrast_12_48",
)
MODEL_FAMILY = "fold_local_intraday_autocorrelation_decay_rank"
LABEL_SHAPE = "directional_intraday_autocorrelation_decay_survival_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_intraday_autocorrelation_decay_scores_per_active_day"
...
def run_c0041_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    price_context = build_autocorrelation_decay_context(bars, range_average, short_range_average, day_context)
    trades: list[base.Trade] = []
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = intraday_autocorrelation_decay_features(
                bars,
                range_average,
                short_range_average,
                day_context,
            ...
            if features is None:
                continue
            label = (
                intraday_autocorrelation_decay_label(bars, range_average, price_context, index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|intraday_autocorrelation_decay",
                    features=features,
                    label=label,
                )
            )
...
def build_autocorrelation_decay_context(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
...
def intraday_autocorrelation_decay_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    if len(features) != len(FEATURE_NAMES):
        raise RuntimeError(f"C0041 feature length mismatch: {len(features)} != {len(FEATURE_NAMES)}")
    return tuple(float(value) for value in features)


def intraday_autocorrelation_decay_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    ribbon_context: list[dict[str, float] | None],
    index: int,
    ...
    future_release = future_autocorrelation_decay_retention(ribbon_context, entry_index, exit_index, direction)
...
def future_autocorrelation_decay_retention(
    ribbon_context: list[dict[str, float] | None],
    entry_index: int,
    exit_index: int,
    direction: int,
...
def sign_autocorrelation(values: list[float], lag: int = 1) -> float:
    if len(values) <= lag:
        return 0.0
    total = 0.0
    count = 0
    for offset in range(lag, len(values)):
        left = sign_value(values[offset])
        right = sign_value(values[offset - lag])
...
def lag_directional_agreement(values: list[float], direction: int, lag: int) -> float:
    if len(values) <= lag:
        return 0.0
    total = 0.0
    count = 0
    for offset in range(lag, len(values)):
        current = sign_value(values[offset])
        prior = sign_value(values[offset - lag])
        if current == 0 or prior == 0:
            continue
        total += direction * current * prior
        count += 1
    return total / max(count, 1)


def alternation_rate(values: list[float]) -> float:
    if len(values) < 2:
        return 0.0
    alternations = 0
    count = 0
    for offset in range(1, len(values)):
        ...
        if left == 0 or right == 0:
            continue
        alternations += 1 if left != right else 0
        count += 1
    return alternations / max(count, 1)


def continuation_rate(values: list[float]) -> float:
    if len(values) < 2:
        return 0.0
    continuations = 0
    count = 0
    for offset in range(1, len(values)):
        ...
        if left == 0 or right == 0:
            continue
        continuations += 1 if left == right else 0
        count += 1
    return continuations / max(count, 1)
...
def variance_ratio(values: list[float], short_window: int, long_window: int) -> float:
    if len(values) < long_window or short_window <= 0:
        return 0.0
    short_values = values[-short_window:]
    long_values = values[-long_window:]
    return bounded(variance(short_values) / max(variance(long_values), 1e-9), 0.0, 8.0)
...
def build_autocorrelation_decay_context(
    ...
    for index, bar in enumerate(bars):
        ...
        prior_return = normalized_close_return(bars, range_average, index - 1)
        return_2 = close_delta_atr(bars, index, 2, average_range)
        ...
        ac12 = sign_autocorrelation(returns_12)
        ac24 = sign_autocorrelation(returns_24)
        ac48 = sign_autocorrelation(returns_48)
        alt12 = alternation_rate(returns_12)
        alt24 = alternation_rate(returns_24)
        cont12 = continuation_rate(returns_12)
        cont24 = continuation_rate(returns_24)
        var_ratio_3_12 = variance_ratio(returns_12, 3, 12)
        var_ratio_6_24 = variance_ratio(returns_24, 6, 24)
        var_ratio_12_48 = variance_ratio(returns_48, 12, 48)
        autocorr_decay = bounded(ac12 - ac48, -2.0, 2.0)
        spread_over_range = bar.spread_points / average_range
        session_progress = bounded(day["bars_since_day_open"] / 288.0, 0.0, 1.0)
        contexts[index] = {
            "average_range": average_range,
            ...
            "prior_return_z_24": zscore(prior_return, stats_24["mean"], stats_24["std"]),
            "lag1_agree_12": lag_directional_agreement(returns_12, 1, 1),
            "lag1_agree_24": lag_directional_agreement(returns_24, 1, 1),
            "lag2_agree_24": lag_directional_agreement(returns_24, 1, 2),
            "lag3_agree_48": lag_directional_agreement(returns_48, 1, 3),
            "sign_autocorr_12": ac12,
            "sign_autocorr_24": ac24,
            "sign_autocorr_48": ac48,
            "alternating_rate_12": alt12,
            "alternating_rate_24": alt24,
            "continuation_rate_12": cont12,
            "continuation_rate_24": cont24,
            "autocorr_decay_slope_12_48": autocorr_decay,
            "variance_ratio_3_12": var_ratio_3_12,
            "variance_ratio_6_24": var_ratio_6_24,
            "variance_ratio_12_48": var_ratio_12_48,
            "impulse_reversal_pressure_12": abs(current_return) * alt12,
            "impulse_continuation_pressure_12": abs(current_return) * cont12,
            "mean_return_12": stats_12["mean"],
            "mean_return_24": stats_24["mean"],
            "mean_return_48": stats_48["mean"],
            ...
            "spread_serial_stress_24": spread_over_range * (1.0 + abs(ac24) + alt24 + cont24),
            "autocorr_regime_contrast_12_48": autocorr_decay,
        }
    return contexts
...
def intraday_autocorrelation_decay_features(
    ...
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    autocorrelation_context: list[dict[str, float] | None],
    index: int,
    direction: int,
) -> tuple[float, ...] | None:
    average_range = range_average[index]
    ...
    if average_range is None or average_range <= 0 or short_average_range is None or short_average_range <= 0:
        return None
    day = day_context[index]
    context = autocorrelation_context[index]
    if day is None or context is None:
        return None
    spread_over_range = bars[index].spread_points / average_range
    if spread_over_range > 0.65:
        return None
    directional_reversal = max(0.0, -direction * context["return_1"]) * context["alternating_rate_24"]
    directional_continuation = max(0.0, direction * context["return_1"]) * context["continuation_rate_24"]
    features = (
        float(direction),
        direction * context["return_1"],
        direction * context["return_2"],
        direction * context["return_3"],
        direction * context["return_6"],
        direction * context["return_12"],
        direction * context["lag1_agree_12"],
        direction * context["lag1_agree_24"],
        direction * context["lag2_agree_24"],
        direction * context["lag3_agree_48"],
        context["sign_autocorr_12"],
        context["sign_autocorr_24"],
        context["sign_autocorr_48"],
        direction * context["sign_autocorr_24"],
        context["alternating_rate_12"],
        context["alternating_rate_24"],
        context["continuation_rate_12"],
        context["continuation_rate_24"],
        directional_reversal,
        directional_continuation,
        context["autocorr_decay_slope_12_48"],
        context["variance_ratio_3_12"],
        context["variance_ratio_6_24"],
        context["variance_ratio_12_48"],
        direction * context["return_6"] * context["variance_ratio_6_24"],
        direction * context["return_z_1_24"],
        direction * context["prior_return_z_24"],
        directional_reversal,
        directional_continuation,
        context["impulse_reversal_pressure_12"],
        context["impulse_continuation_pressure_12"],
        context["mean_return_12"],
        context["mean_return_24"],
        context["mean_return_48"],
        direction * context["mean_return_24"],
        ...
        context["session_progress"],
        spread_over_range,
        context["spread_serial_stress_24"],
        context["autocorr_regime_contrast_12_48"],
    )
    if len(features) != len(FEATURE_NAMES):
        raise RuntimeError(f"C0041 feature length mismatch: {len(features)} != {len(FEATURE_NAMES)}")
    return tuple(float(value) for value in features)
...
def intraday_autocorrelation_decay_label(
    ...
    serial_reversal_alignment = max(0.0, -direction * context["return_1"]) * context["alternating_rate_24"]
    serial_continuation_alignment = max(0.0, direction * context["return_1"]) * context["continuation_rate_24"]
    serial_alignment = (
        0.30 * serial_reversal_alignment
        + 0.28 * serial_continuation_alignment
        + 0.20 * direction * context["mean_return_24"]
        + 0.14 * context["autocorr_regime_contrast_12_48"]
    )
    serial_penalty = 0.22 * max(0.0, context["spread_serial_stress_24"] - context["spread_over_range"])
    return (
        0.84 * first_hit
        + 0.34 * target_speed
        - 0.46 * stop_speed
...
def linear_model_summary(model: LinearEdgeModel) -> dict[str, object]:
    return {
        ...
        "score_interpretation": "higher_score_means_direct_fold_local_intraday_autocorrelation_decay_rank",
    }
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0041"
    payload["work_unit_id"] = "C0041"
    payload["run_id"] = "R0001"
    payload["proxy_id"] = "PX-C0041-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0041_r0001_intraday_autocorrelation_decay"
    payload["proxy_config_path"] = "campaigns/C0041_intraday_autocorrelation_decay_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_intraday_autocorrelation_decay_rank_fit"
    payload["claim_boundary"] = dict(CLAIM_BOUNDARY)
    payload["proxy_artifact_paths"] = [
        "campaigns/C0041_intraday_autocorrelation_decay_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0041_intraday_autocorrelation_decay_discovery/runs/R0001/artifacts/c0041_r0001_proxy_trades.csv",
        "campaigns/C0041_intraday_autocorrelation_decay_discovery/runs/R0001/artifacts/c0041_r0001_autocorrelation_decay_summary.json",
    ]
    ...
    profiles["intraday_autocorrelation_decay_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
            "model_family": MODEL_FAMILY,
            "feature_count": len(FEATURE_NAMES),
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0041_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "score_interpretation": "higher_score_means_direct_fold_local_intraday_autocorrelation_decay_rank",
            "variant_boundary": (
                "intraday_autocorrelation_decay_rank_not_effort_absorption_seasonal_residual_tail_risk_skew_entropy_"
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_autocorrelation_decay_summary_artifact(payload, AUTOCORRELATION_DECAY_SUMMARY_PATH)
    write_text_local(PROXY_PATH, json.dumps(payload, indent=2, sort_keys=True) + "\n")
    trade_hash = sha256_file_local(TRADE_ARTIFACT_PATH)
    summary_hash = sha256_file_local(AUTOCORRELATION_DECAY_SUMMARY_PATH)
...
def write_autocorrelation_decay_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_intraday_autocorrelation_decay_summary_v1",
        "template": False,
        "work_unit_id": "C0041",
        "campaign_id": "C0041",
        "run_id": "R0001",
        ...
        "intraday_autocorrelation_decay_profile": profiles["intraday_autocorrelation_decay_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "intraday_autocorrelation_decay_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0041-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0041_intraday_autocorrelation_decay_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0041_intraday_autocorrelation_decay_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0041-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0041-R0001-AUTOCORRELATION-DECAY-SUMMARY",
                "intraday_autocorrelation_decay_summary_artifact",
                "json",
                "campaigns/C0041_intraday_autocorrelation_decay_discovery/runs/R0001/artifacts/c0041_r0001_autocorrelation_decay_summary.json",
                summary_hash,
                ["campaigns/C0041_intraday_autocorrelation_decay_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0041_r0001_intraday_autocorrelation_decay",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0041_r0001_proxy_trades.csv"
    evidence["intraday_autocorrelation_decay_summary"] = "artifacts/c0041_r0001_autocorrelation_decay_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0041_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0041_r0001_proxy_trades.csv",
        "artifacts/c0041_r0001_autocorrelation_decay_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0041 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0041_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0041_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0041_intraday_autocorrelation_decay_discovery"
    data["project"]["active_run"] = "campaigns/C0041_intraday_autocorrelation_decay_discovery/runs/R0001"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0041_intraday_autocorrelation_decay_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0041_intraday_autocorrelation_decay_discovery",
        "open_c0041_r0001_fold_local_intraday_autocorrelation_decay_rank_run",
        "produce_c0041_r0001_proxy_evidence",
    ):
    ...
    data["active_campaign"] = "campaigns/C0041_intraday_autocorrelation_decay_discovery"
    data["active_run"] = "campaigns/C0041_intraday_autocorrelation_decay_discovery/runs/R0001"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0041_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0041_intraday_autocorrelation_decay_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0041_r0001_mt5_logic_parity_evidence",
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["canonical_source"] = "campaigns/C0041_intraday_autocorrelation_decay_discovery/runs/R0001/kpi/proxy.json"
    data["canonical_status"] = "proxy_recorded_pending_mt5"
    data["current_decision"] = "produce_c0041_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = "campaigns/C0041_intraday_autocorrelation_decay_discovery"
    data["active_run"] = "campaigns/C0041_intraday_autocorrelation_decay_discovery/runs/R0001"
    data["next_required_action"] = "produce_c0041_r0001_mt5_logic_parity_evidence"
    data["current_evidence_summary"] = {
        "source_campaign": "campaigns/C0041_intraday_autocorrelation_decay_discovery",
        "current_task": "produce_c0041_r0001_mt5_logic_parity_evidence",
        "active_run": "campaigns/C0041_intraday_autocorrelation_decay_discovery/runs/R0001",
        "active_run_status": "proxy_recorded_pending_mt5",
        "evidence_status": "proxy_recorded_pending_mt5",
        "campaign_family": "intraday_autocorrelation_decay",
        "entries_per_active_day": summary.get("entries_per_active_day") if isinstance(summary, dict) else None,
        "proxy_trade_count": required.get("proxy_trade_count") if isinstance(required, dict) else None,
        "proxy_net_pnl_points": required.get("proxy_net_pnl_points") if isinstance(required, dict) else None,
        "proxy_profit_factor": required.get("proxy_profit_factor") if isinstance(required, dict) else None,
        "next_required_action": "produce_c0041_r0001_mt5_logic_parity_evidence",
        "note": "C0041 R0001 proxy evidence recorded; MT5 paired validation remains mandatory regardless of proxy result.",
        "source_campaign_manifest": "campaigns/C0041_intraday_autocorrelation_decay_discovery/campaign.yaml",
    }
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0041",
            "c0004_r0001_fold_local_state_archetype": "c0041_r0001_intraday_autocorrelation_decay",
            "c0004_r0001": "c0041_r0001",
            "fold_local_state_archetype_discovery": "intraday_autocorrelation_decay_discovery",
            ...
            "state_archetype_summary": "intraday_autocorrelation_decay_summary",
        }
