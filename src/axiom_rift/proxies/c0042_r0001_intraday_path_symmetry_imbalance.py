"""C0042 R0001 proxy evidence for intraday path symmetry imbalance discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0042_intraday_path_symmetry_imbalance_discovery" / "runs" / "R0001"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0042_intraday_path_symmetry_imbalance_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0042_r0001_proxy_trades.csv"
PATH_SYMMETRY_IMBALANCE_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0042_r0001_path_symmetry_imbalance_summary.json"
...
FEATURE_NAMES = (
    ...
    "directional_mid_displacement_12",
    "directional_mid_displacement_24",
    "directional_mid_displacement_48",
    "directional_day_mid_displacement",
    "upper_half_occupancy_12",
    "upper_half_occupancy_24",
    "lower_half_occupancy_12",
    "lower_half_occupancy_24",
    "directional_half_occupancy_12",
    "directional_half_occupancy_24",
    "mirror_residual_12",
    "mirror_residual_24",
    "directional_mirror_residual_24",
    "centerline_cross_rate_12",
    "centerline_cross_rate_24",
    "centerline_rejection_12",
    "centerline_rejection_24",
    "directional_rejection_24",
    "close_to_mid_elasticity_6",
    "close_to_mid_elasticity_12",
    "half_range_migration_12",
    "half_range_migration_24",
    "directional_half_range_migration_24",
    "excursion_asymmetry_12",
    "excursion_asymmetry_24",
    "directional_excursion_asymmetry_24",
    "range_width_ratio_12_48",
    "midline_slope_12",
    "directional_midline_slope_12",
    "center_distance_z_24",
    "directional_center_distance_z_24",
    "center_distance_change_6",
    "directional_center_distance_change_6",
    "path_efficiency_12",
    "path_churn_ratio_12",
    "body_mid_alignment_12",
    "directional_close_streak_12",
    "day_center_position",
    "directional_day_center_position",
    "session_progress",
    "spread_over_range",
    "spread_symmetry_stress_24",
    "symmetry_regime_contrast_12_48",
)
MODEL_FAMILY = "fold_local_intraday_path_symmetry_imbalance_rank"
LABEL_SHAPE = "directional_intraday_path_symmetry_imbalance_survival_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_intraday_path_symmetry_imbalance_scores_per_active_day"
...
def run_c0042_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    price_context = build_path_symmetry_imbalance_context(bars, range_average, short_range_average, day_context)
    trades: list[base.Trade] = []
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = intraday_path_symmetry_imbalance_features(
                bars,
                range_average,
                short_range_average,
                day_context,
            ...
            if features is None:
                continue
            label = (
                intraday_path_symmetry_imbalance_label(bars, range_average, price_context, index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|intraday_path_symmetry_imbalance",
                    features=features,
                    label=label,
                )
            )
...
def build_path_symmetry_imbalance_context(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
...
def intraday_path_symmetry_imbalance_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    if len(features) != len(FEATURE_NAMES):
        raise RuntimeError(f"C0042 feature length mismatch: {len(features)} != {len(FEATURE_NAMES)}")
    return tuple(float(value) for value in features)


def intraday_path_symmetry_imbalance_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    ribbon_context: list[dict[str, float] | None],
    index: int,
    ...
    future_release = future_path_symmetry_imbalance_retention(ribbon_context, entry_index, exit_index, direction)
...
def future_path_symmetry_imbalance_retention(
    ribbon_context: list[dict[str, float] | None],
    entry_index: int,
    exit_index: int,
    direction: int,
...
def window_bounds(bars: list[base.Bar], index: int, lookback: int) -> tuple[float, float, float, float]:
    ...
    return high, low, 0.5 * (high + low), width


def signed_mid_distances(bars: list[base.Bar], index: int, lookback: int) -> list[float]:
    _, _, mid, width = window_bounds(bars, index, lookback)
    return [2.0 * (bars[offset].close - mid) / width for offset in range(index - lookback + 1, index + 1)]


def occupancy_above(values: list[float]) -> float:
    return sum(1 for value in values if value > 0.0) / max(len(values), 1)


def centerline_cross_rate(values: list[float]) -> float:
    crosses = 0
    count = 0
    for offset in range(1, len(values)):
        left = sign_value(values[offset])
        right = sign_value(values[offset - 1])
        if left == 0 or right == 0:
            continue
        crosses += 1 if left != right else 0
        count += 1
    return crosses / max(count, 1)


def centerline_rejection(
    bars: list[base.Bar],
    index: int,
    lookback: int,
    direction: int,
) -> float:
    _, _, mid, _ = window_bounds(bars, index, lookback)
    touches = 0
    accepts = 0
    for offset in range(index - lookback + 1, index + 1):
        bar = bars[offset]
        if bar.low <= mid <= bar.high:
            touches += 1
            close_side = sign_value(bar.close - mid)
            if close_side == direction:
                accepts += 1
            elif close_side == -direction:
                accepts -= 1
    return accepts / max(touches, 1)


def excursion_asymmetry(bars: list[base.Bar], index: int, lookback: int) -> float:
    _, _, mid, width = window_bounds(bars, index, lookback)
    upper = max(max(0.0, bars[offset].high - mid) for offset in range(index - lookback + 1, index + 1))
    lower = max(max(0.0, mid - bars[offset].low) for offset in range(index - lookback + 1, index + 1))
    return (upper - lower) / width


def body_mid_alignment(bars: list[base.Bar], index: int, lookback: int, direction: int) -> float:
    _, _, mid, _ = window_bounds(bars, index, lookback)
    total = 0.0
    for offset in range(index - lookback + 1, index + 1):
        body = bars[offset].close - bars[offset].open
        side = bars[offset].close - mid
        total += direction * sign_value(body) * sign_value(side)
    return total / lookback


def path_symmetry_metrics(
    bars: list[base.Bar],
    index: int,
    lookback: int,
    average_range: float,
    ...
    _, _, mid, width = window_bounds(bars, index, lookback)
    distances = signed_mid_distances(bars, index, lookback)
    above = occupancy_above(distances)
    below = occupancy_above([-value for value in distances])
    signed_occupancy = above - below
    mirror_residual = abs(signed_occupancy)
    current_distance = 2.0 * (bars[index].close - mid) / width
    lag = min(max(lookback // 2, 1), 6)
    _, _, prior_mid, prior_width = window_bounds(bars, index - lag, lookback)
    prior_distance = 2.0 * (bars[index - lag].close - prior_mid) / prior_width
    distance_stats = return_distribution_stats(distances)
    mid_slope = (mid - prior_mid) / max(average_range, 1e-9)
    return {
        "mid": mid,
        "width": width,
        "above": above,
        "below": below,
        "signed_occupancy": signed_occupancy,
        "mirror_residual": mirror_residual,
        "centerline_cross_rate": centerline_cross_rate(distances),
        "current_distance": current_distance,
        "prior_distance": prior_distance,
        "distance_z": zscore(current_distance, distance_stats["mean"], distance_stats["std"]),
        "distance_change": current_distance - prior_distance,
        "excursion_asymmetry": excursion_asymmetry(bars, index, lookback),
        "mid_slope": mid_slope,
    }
...
def build_path_symmetry_imbalance_context(
    ...
    for index, bar in enumerate(bars):
        ...
        if (
            ...
            continue
        m6 = path_symmetry_metrics(bars, index, 6, average_range)
        m12 = path_symmetry_metrics(bars, index, 12, average_range)
        m24 = path_symmetry_metrics(bars, index, 24, average_range)
        m48 = path_symmetry_metrics(bars, index, 48, average_range)
        day_width = max(day["day_high"] - day["day_low"], 1e-9)
        day_mid = 0.5 * (day["day_high"] + day["day_low"])
        day_center_position = 2.0 * (bar.close - day_mid) / day_width
        close_location = (bar.close - bar.low) / max(bar.high - bar.low, 1e-9)
        regime_contrast = m12["signed_occupancy"] - m48["signed_occupancy"]
        spread_over_range = bar.spread_points / average_range
        session_progress = bounded(day["bars_since_day_open"] / 288.0, 0.0, 1.0)
        contexts[index] = {
            "average_range": average_range,
            "close_location": close_location,
            "mid_distance_12": m12["current_distance"],
            "mid_distance_24": m24["current_distance"],
            "mid_distance_48": m48["current_distance"],
            "day_center_position": day_center_position,
            "upper_half_occupancy_12": m12["above"],
            "upper_half_occupancy_24": m24["above"],
            "lower_half_occupancy_12": m12["below"],
            "lower_half_occupancy_24": m24["below"],
            "signed_occupancy_12": m12["signed_occupancy"],
            "signed_occupancy_24": m24["signed_occupancy"],
            "mirror_residual_12": m12["mirror_residual"],
            "mirror_residual_24": m24["mirror_residual"],
            "centerline_cross_rate_12": m12["centerline_cross_rate"],
            "centerline_cross_rate_24": m24["centerline_cross_rate"],
            "centerline_rejection_12_long": centerline_rejection(bars, index, 12, 1),
            "centerline_rejection_24_long": centerline_rejection(bars, index, 24, 1),
            "centerline_rejection_12_short": centerline_rejection(bars, index, 12, -1),
            "centerline_rejection_24_short": centerline_rejection(bars, index, 24, -1),
            "close_to_mid_elasticity_6": m6["distance_change"],
            "close_to_mid_elasticity_12": m12["distance_change"],
            "half_range_migration_12": m12["distance_change"],
            "half_range_migration_24": m24["distance_change"],
            "excursion_asymmetry_12": m12["excursion_asymmetry"],
            "excursion_asymmetry_24": m24["excursion_asymmetry"],
            "range_width_ratio_12_48": m12["width"] / max(m48["width"], 1e-9),
            "midline_slope_12": m12["mid_slope"],
            "center_distance_z_24": m24["distance_z"],
            "center_distance_change_6": m6["distance_change"],
            "session_progress": session_progress,
            "spread_over_range": spread_over_range,
            "spread_symmetry_stress_24": spread_over_range
            * (1.0 + m24["mirror_residual"] + m24["centerline_cross_rate"] + abs(m24["distance_change"])),
            "symmetry_regime_contrast_12_48": regime_contrast,
        }
    return contexts
...
def intraday_path_symmetry_imbalance_features(
    ...
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    path_symmetry_context: list[dict[str, float] | None],
    index: int,
    direction: int,
) -> tuple[float, ...] | None:
    average_range = range_average[index]
    ...
    if average_range is None or average_range <= 0 or short_average_range is None or short_average_range <= 0:
        return None
    day = day_context[index]
    context = path_symmetry_context[index]
    if day is None or context is None:
        return None
    spread_over_range = bars[index].spread_points / average_range
    if spread_over_range > 0.65:
        return None
    directional_rejection_24 = (
        context["centerline_rejection_24_long"] if direction > 0 else context["centerline_rejection_24_short"]
    )
    features = (
        float(direction),
        directional_close_location(bars[index], direction),
        direction * context["mid_distance_12"],
        direction * context["mid_distance_24"],
        direction * context["mid_distance_48"],
        direction * context["day_center_position"],
        context["upper_half_occupancy_12"],
        context["upper_half_occupancy_24"],
        context["lower_half_occupancy_12"],
        context["lower_half_occupancy_24"],
        direction * context["signed_occupancy_12"],
        direction * context["signed_occupancy_24"],
        context["mirror_residual_12"],
        context["mirror_residual_24"],
        direction * context["signed_occupancy_24"] * context["mirror_residual_24"],
        context["centerline_cross_rate_12"],
        context["centerline_cross_rate_24"],
        context["centerline_rejection_12_long"] if direction > 0 else context["centerline_rejection_12_short"],
        directional_rejection_24,
        direction * directional_rejection_24,
        context["close_to_mid_elasticity_6"],
        context["close_to_mid_elasticity_12"],
        context["half_range_migration_12"],
        context["half_range_migration_24"],
        direction * context["half_range_migration_24"],
        context["excursion_asymmetry_12"],
        context["excursion_asymmetry_24"],
        direction * context["excursion_asymmetry_24"],
        context["range_width_ratio_12_48"],
        context["midline_slope_12"],
        direction * context["midline_slope_12"],
        context["center_distance_z_24"],
        direction * context["center_distance_z_24"],
        context["center_distance_change_6"],
        direction * context["center_distance_change_6"],
        directional_efficiency(bars, index, direction, 12),
        path_churn_ratio(bars, index, 12),
        body_mid_alignment(bars, index, 12, direction),
        directional_close_streak(bars, index, direction, 12),
        context["day_center_position"],
        direction * context["day_center_position"],
        context["session_progress"],
        spread_over_range,
        context["spread_symmetry_stress_24"],
        context["symmetry_regime_contrast_12_48"],
    )
    if len(features) != len(FEATURE_NAMES):
        raise RuntimeError(f"C0042 feature length mismatch: {len(features)} != {len(FEATURE_NAMES)}")
    return tuple(float(value) for value in features)
...
def future_midline_escape(
    ...
    if not path:
        return 0.0
    closes = [bar.close for bar in path]
    aligned = sum(1 for close in closes if direction * (close - mid) > 0.0) / len(closes)
    terminal_escape = direction * (closes[-1] - mid) / max(average_range, 1e-9)
    return aligned + 0.25 * terminal_escape


def future_centerline_churn(path: list[base.Bar], mid: float) -> float:
    distances = [bar.close - mid for bar in path]
    return centerline_cross_rate(distances)
...
def intraday_path_symmetry_imbalance_label(
    ...
    stop_price = entry - direction * stop_distance
    target_price = entry + direction * target_distance
    _, _, mid, _ = window_bounds(bars, index, 24)
    first_hit = 0.0
    event_bar = len(path)
    adverse_first = 0.0
    if direction > 0:
        ...
        mae = max(entry - bar.low for bar in path)
        terminal = path[-1].close - entry
        aligned_close_count = sum(1 for bar in path if bar.close >= mid)
        for offset, bar in enumerate(path, start=1):
            if bar.low <= stop_price:
                first_hit = -1.0
                event_bar = offset
    ...
    path_spread_peak = max(bar.spread_points for bar in path) / average_range
    mirror_alignment = (
        0.34 * direction * context["signed_occupancy_24"]
        + 0.26 * direction * context["half_range_migration_24"]
        + 0.22 * direction * context["excursion_asymmetry_24"]
        + 0.18 * direction * context["midline_slope_12"]
    )
    rejection_alignment = (
        context["centerline_rejection_24_long"] if direction > 0 else context["centerline_rejection_24_short"]
    )
    future_escape = future_midline_escape(path, direction, mid, average_range)
    future_churn = future_centerline_churn(path, mid)
    symmetry_penalty = 0.25 * context["mirror_residual_24"] * context["centerline_cross_rate_24"]
    spread_penalty = 0.24 * max(0.0, context["spread_symmetry_stress_24"] - context["spread_over_range"])
    return (
        0.84 * first_hit
        + 0.34 * target_speed
        - 0.46 * stop_speed
...
def linear_model_summary(model: LinearEdgeModel) -> dict[str, object]:
    return {
        ...
        "score_interpretation": "higher_score_means_direct_fold_local_intraday_path_symmetry_imbalance_rank",
    }
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0042"
    payload["work_unit_id"] = "C0042"
    payload["run_id"] = "R0001"
    payload["proxy_id"] = "PX-C0042-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0042_r0001_intraday_path_symmetry_imbalance"
    payload["proxy_config_path"] = "campaigns/C0042_intraday_path_symmetry_imbalance_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_intraday_path_symmetry_imbalance_rank_fit"
    payload["claim_boundary"] = dict(CLAIM_BOUNDARY)
    payload["proxy_artifact_paths"] = [
        "campaigns/C0042_intraday_path_symmetry_imbalance_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0042_intraday_path_symmetry_imbalance_discovery/runs/R0001/artifacts/c0042_r0001_proxy_trades.csv",
        "campaigns/C0042_intraday_path_symmetry_imbalance_discovery/runs/R0001/artifacts/c0042_r0001_path_symmetry_imbalance_summary.json",
    ]
    ...
    profiles["intraday_path_symmetry_imbalance_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
            "model_family": MODEL_FAMILY,
            "feature_count": len(FEATURE_NAMES),
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0042_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "score_interpretation": "higher_score_means_direct_fold_local_intraday_path_symmetry_imbalance_rank",
            "variant_boundary": (
                "intraday_path_symmetry_imbalance_rank_not_effort_absorption_seasonal_residual_tail_risk_skew_entropy_"
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_path_symmetry_imbalance_summary_artifact(payload, PATH_SYMMETRY_IMBALANCE_SUMMARY_PATH)
    write_text_local(PROXY_PATH, json.dumps(payload, indent=2, sort_keys=True) + "\n")
    trade_hash = sha256_file_local(TRADE_ARTIFACT_PATH)
    summary_hash = sha256_file_local(PATH_SYMMETRY_IMBALANCE_SUMMARY_PATH)
...
def write_path_symmetry_imbalance_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_intraday_path_symmetry_imbalance_summary_v1",
        "template": False,
        "work_unit_id": "C0042",
        "campaign_id": "C0042",
        "run_id": "R0001",
        ...
        "intraday_path_symmetry_imbalance_profile": profiles["intraday_path_symmetry_imbalance_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "intraday_path_symmetry_imbalance_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0042-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0042_intraday_path_symmetry_imbalance_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0042_intraday_path_symmetry_imbalance_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0042-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0042-R0001-PATH-SYMMETRY-IMBALANCE-SUMMARY",
                "intraday_path_symmetry_imbalance_summary_artifact",
                "json",
                "campaigns/C0042_intraday_path_symmetry_imbalance_discovery/runs/R0001/artifacts/c0042_r0001_path_symmetry_imbalance_summary.json",
                summary_hash,
                ["campaigns/C0042_intraday_path_symmetry_imbalance_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0042_r0001_intraday_path_symmetry_imbalance",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0042_r0001_proxy_trades.csv"
    evidence["intraday_path_symmetry_imbalance_summary"] = "artifacts/c0042_r0001_path_symmetry_imbalance_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0042_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0042_r0001_proxy_trades.csv",
        "artifacts/c0042_r0001_path_symmetry_imbalance_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0042 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0042_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0042_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0042_intraday_path_symmetry_imbalance_discovery"
    data["project"]["active_run"] = "campaigns/C0042_intraday_path_symmetry_imbalance_discovery/runs/R0001"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0042_intraday_path_symmetry_imbalance_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0042_intraday_path_symmetry_imbalance_discovery",
        "open_c0042_r0001_fold_local_intraday_path_symmetry_imbalance_rank_run",
        "produce_c0042_r0001_proxy_evidence",
    ):
    ...
    data["active_campaign"] = "campaigns/C0042_intraday_path_symmetry_imbalance_discovery"
    data["active_run"] = "campaigns/C0042_intraday_path_symmetry_imbalance_discovery/runs/R0001"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0042_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0042_intraday_path_symmetry_imbalance_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0042_r0001_mt5_logic_parity_evidence",
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["canonical_source"] = "campaigns/C0042_intraday_path_symmetry_imbalance_discovery/runs/R0001/kpi/proxy.json"
    data["canonical_status"] = "proxy_recorded_pending_mt5"
    data["current_decision"] = "produce_c0042_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = "campaigns/C0042_intraday_path_symmetry_imbalance_discovery"
    data["active_run"] = "campaigns/C0042_intraday_path_symmetry_imbalance_discovery/runs/R0001"
    data["next_required_action"] = "produce_c0042_r0001_mt5_logic_parity_evidence"
    data["current_evidence_summary"] = {
        "source_campaign": "campaigns/C0042_intraday_path_symmetry_imbalance_discovery",
        "current_task": "produce_c0042_r0001_mt5_logic_parity_evidence",
        "active_run": "campaigns/C0042_intraday_path_symmetry_imbalance_discovery/runs/R0001",
        "active_run_status": "proxy_recorded_pending_mt5",
        "evidence_status": "proxy_recorded_pending_mt5",
        "campaign_family": "intraday_path_symmetry_imbalance",
        "entries_per_active_day": summary.get("entries_per_active_day") if isinstance(summary, dict) else None,
        "proxy_trade_count": required.get("proxy_trade_count") if isinstance(required, dict) else None,
        "proxy_net_pnl_points": required.get("proxy_net_pnl_points") if isinstance(required, dict) else None,
        "proxy_profit_factor": required.get("proxy_profit_factor") if isinstance(required, dict) else None,
        "next_required_action": "produce_c0042_r0001_mt5_logic_parity_evidence",
        "note": "C0042 R0001 proxy evidence recorded; MT5 paired validation remains mandatory regardless of proxy result.",
        "source_campaign_manifest": "campaigns/C0042_intraday_path_symmetry_imbalance_discovery/campaign.yaml",
    }
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0042",
            "c0004_r0001_fold_local_state_archetype": "c0042_r0001_intraday_path_symmetry_imbalance",
            "c0004_r0001": "c0042_r0001",
            "fold_local_state_archetype_discovery": "intraday_path_symmetry_imbalance_discovery",
            ...
            "state_archetype_summary": "intraday_path_symmetry_imbalance_summary",
        }
