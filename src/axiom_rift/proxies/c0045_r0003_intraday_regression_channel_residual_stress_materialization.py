"""C0045 R0003 stress/materialization proxy for regression channel residual discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0045_intraday_regression_channel_residual_discovery" / "runs" / "R0003"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0045_intraday_regression_channel_residual_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0045_r0003_proxy_trades.csv"
REGRESSION_CHANNEL_RESIDUAL_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0045_r0003_regression_channel_residual_summary.json"
...
FEATURE_NAMES = (
    ...
    "slope_24",
    "slope_48",
    "directional_slope_24",
    "slope_acceleration_12_48",
    "residual_z_12",
    "residual_z_24",
    "residual_z_48",
    "directional_residual_z_24",
    "residual_abs_z_24",
    "residual_z_change_3",
    "residual_z_change_6",
    "residual_persistence_12",
    "residual_persistence_24",
    "channel_width_12",
    "channel_width_24",
    "channel_width_48",
    "channel_width_change_12_48",
    "channel_compression_24_48",
    "channel_efficiency_24",
    "channel_efficiency_48",
    "regression_r2_24",
    "regression_r2_48",
    "residual_position_in_channel_24",
    "directional_channel_position_24",
    "distance_to_upper_channel_24",
    "distance_to_lower_channel_24",
    "directional_channel_edge_distance",
    "residual_mean_reversion_pressure_24",
    "residual_continuation_pressure_24",
    "slope_residual_alignment_24",
    "trend_channel_stress_24",
    "day_slope_proxy",
    "day_residual_pressure",
    "day_channel_position",
    "session_progress",
    "spread_over_range",
    "spread_channel_stress_24",
    "path_churn_ratio_12",
    "path_efficiency_24",
    "normalized_close_return_3",
    "directional_close_streak_12",
    "regression_regime_contrast_12_48",
    "spread_adjusted_channel_edge_distance",
    "spread_adjusted_mean_reversion_pressure",
    "spread_adjusted_continuation_pressure",
    "channel_cost_buffer_positive",
    "tick_fragility_proxy_24",
    "late_session_cost_pressure",
    "short_side_asymmetry_bias",
    "adverse_cost_survival_buffer_001",
    "adverse_cost_survival_buffer_002",
    "materialization_quantization_margin",
    "materialization_input_stability",
)
MODEL_FAMILY = "fold_local_intraday_regression_channel_residual_stress_materialization_rank"
LABEL_SHAPE = "stress_survival_cost_slippage_intraday_regression_channel_residual_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "short_primary_stress_survival_materialization_probe_top_fold_local_intraday_regression_channel_residual_scores_per_active_day"
ADVERSE_COST_STRESS_LEVELS = (0.01, 0.02, 0.04)
MATERIALIZATION_QUANTIZATION_STEP = 0.01
...
def run_c0045_r0003_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    price_context = build_regression_channel_residual_context(bars, range_average, short_range_average, day_context)
    trades: list[base.Trade] = []
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        if average_range is None or average_range <= 0 or short_average_range is None or short_average_range <= 0:
            continue
        for direction in (-1,):
            features = intraday_regression_channel_residual_features(
                bars,
                range_average,
                short_range_average,
                day_context,
            ...
            if features is None:
                continue
            label = (
                intraday_regression_channel_residual_label(bars, range_average, price_context, index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|intraday_regression_channel_residual_stress_materialization",
                    features=features,
                    label=label,
                )
            )
...
def build_regression_channel_residual_context(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
...
def intraday_regression_channel_residual_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    if len(features) != len(FEATURE_NAMES):
        raise RuntimeError(f"C0045 feature length mismatch: {len(features)} != {len(FEATURE_NAMES)}")
    return tuple(float(value) for value in features)


def intraday_regression_channel_residual_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    ribbon_context: list[dict[str, float] | None],
    index: int,
    ...
    future_release = future_regression_channel_residual_retention(ribbon_context, entry_index, exit_index, direction)
...
def future_regression_channel_residual_retention(
    ribbon_context: list[dict[str, float] | None],
    entry_index: int,
    exit_index: int,
    direction: int,
...
def score_candidate(candidate: base.Candidate, model: LinearEdgeModel) -> base.Candidate:
    ...
    raw_score = float(np.dot(normalized * model.score_direction, model.feature_weights) / max(model.feature_weights.sum(), 1e-9))
    score = raw_score + stress_materialization_score_overlay(candidate)
...
def stress_materialization_score_overlay(candidate: base.Candidate) -> float:
    fields = dict(zip(FEATURE_NAMES, candidate.features))
    buffer_001 = float(fields.get("adverse_cost_survival_buffer_001", 0.0))
    buffer_002 = float(fields.get("adverse_cost_survival_buffer_002", 0.0))
    stability = float(fields.get("materialization_input_stability", 0.0))
    quantization_margin = float(fields.get("materialization_quantization_margin", 0.0))
    fragility = float(fields.get("tick_fragility_proxy_24", 0.0))
    late_cost = float(fields.get("late_session_cost_pressure", 0.0))
    return (
        0.14 * bounded(buffer_001, -4.0, 4.0)
        + 0.18 * bounded(buffer_002, -4.0, 4.0)
        + 0.08 * bounded(stability, 0.0, 1.0)
        + 0.05 * bounded(quantization_margin, 0.0, 4.0)
        - 0.12 * bounded(fragility, 0.0, 4.0)
        - 0.06 * bounded(late_cost, 0.0, 4.0)
    )
...
def linear_model_summary(model: LinearEdgeModel) -> dict[str, object]:
    return {
        ...
        "score_interpretation": "higher_score_means_short_primary_stress_survival_materialization_regression_channel_residual_rank",
    }
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0045"
    payload["work_unit_id"] = "C0045"
    payload["run_id"] = "R0003"
    payload["proxy_id"] = "PX-C0045-R0003"
    payload["proxy_engine"] = "axiom_rift.proxies.c0045_r0003_intraday_regression_channel_residual_stress_materialization"
    payload["proxy_config_path"] = "campaigns/C0045_intraday_regression_channel_residual_discovery/runs/R0003/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_intraday_regression_channel_residual_rank_fit"
    payload["claim_boundary"] = dict(CLAIM_BOUNDARY)
    payload["proxy_artifact_paths"] = [
        "campaigns/C0045_intraday_regression_channel_residual_discovery/runs/R0003/kpi/proxy.json",
        "campaigns/C0045_intraday_regression_channel_residual_discovery/runs/R0003/artifacts/c0045_r0003_proxy_trades.csv",
        "campaigns/C0045_intraday_regression_channel_residual_discovery/runs/R0003/artifacts/c0045_r0003_regression_channel_residual_summary.json",
    ]
    ...
    profiles["intraday_regression_channel_residual_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
            "model_family": MODEL_FAMILY,
            "feature_count": len(FEATURE_NAMES),
            ...
            "candidate_direction": "short_primary_cost_slippage_stress_survival_materialization_probe",
            "adverse_cost_stress_levels": list(ADVERSE_COST_STRESS_LEVELS),
            "materialization_quantization_step": MATERIALIZATION_QUANTIZATION_STEP,
            "materialization_portability_probe": "closed_bar_feature_surface_plus_schedule_replay_no_onnx_claim",
            "model_selected": False,
            "feature_set_selected": False,
            "label_selected": False,
        },
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0045_r0003_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "score_interpretation": "higher_score_means_short_primary_stress_survival_materialization_regression_channel_residual_rank",
            "variant_boundary": (
                "short_primary_cost_slippage_stress_survival_materialization_probe_not_selected_claim_not_"
                "synthetic_commission_not_threshold_stop_target_hold_session_activity_capital_monthly_filter_or_retry_nudge"
            ),
        }
    )
    return config
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_regression_channel_residual_summary_artifact(payload, REGRESSION_CHANNEL_RESIDUAL_SUMMARY_PATH)
    write_text_local(PROXY_PATH, json.dumps(payload, indent=2, sort_keys=True) + "\n")
    trade_hash = sha256_file_local(TRADE_ARTIFACT_PATH)
    summary_hash = sha256_file_local(REGRESSION_CHANNEL_RESIDUAL_SUMMARY_PATH)
...
def write_regression_channel_residual_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_intraday_regression_channel_residual_summary_v1",
        "template": False,
        "work_unit_id": "C0045",
        "campaign_id": "C0045",
        "run_id": "R0003",
        ...
        "intraday_regression_channel_residual_profile": profiles["intraday_regression_channel_residual_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "intraday_regression_channel_residual_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0045-R0003-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0045_intraday_regression_channel_residual_discovery/runs/R0003/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0045_intraday_regression_channel_residual_discovery/runs/R0003/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0045-R0003-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0045-R0003-REGRESSION-CHANNEL-RESIDUAL-SUMMARY",
                "intraday_regression_channel_residual_summary_artifact",
                "json",
                "campaigns/C0045_intraday_regression_channel_residual_discovery/runs/R0003/artifacts/c0045_r0003_regression_channel_residual_summary.json",
                summary_hash,
                ["campaigns/C0045_intraday_regression_channel_residual_discovery/runs/R0003/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0045_r0003_intraday_regression_channel_residual_stress_materialization",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0045_r0003_proxy_trades.csv"
    evidence["intraday_regression_channel_residual_summary"] = "artifacts/c0045_r0003_regression_channel_residual_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0045_r0003_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0045_r0003_proxy_trades.csv",
        "artifacts/c0045_r0003_regression_channel_residual_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0045 R0003 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0045_r0003_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0045_r0003_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0045_intraday_regression_channel_residual_discovery"
    data["project"]["active_run"] = "campaigns/C0045_intraday_regression_channel_residual_discovery/runs/R0003"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0045_intraday_regression_channel_residual_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0045_intraday_regression_channel_residual_discovery",
        "open_c0045_r0003_regression_channel_residual_cost_slippage_stress_materialization_probe_run",
        "produce_c0045_r0003_proxy_evidence",
    ):
    ...
    data["active_campaign"] = "campaigns/C0045_intraday_regression_channel_residual_discovery"
    data["active_run"] = "campaigns/C0045_intraday_regression_channel_residual_discovery/runs/R0003"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0045_r0003_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0045_intraday_regression_channel_residual_discovery/runs/R0003/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0045_r0003_mt5_logic_parity_evidence",
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["canonical_source"] = "campaigns/C0045_intraday_regression_channel_residual_discovery/runs/R0003/kpi/proxy.json"
    data["canonical_status"] = "proxy_recorded_pending_mt5"
    data["current_decision"] = "produce_c0045_r0003_mt5_logic_parity_evidence"
    data["active_campaign"] = "campaigns/C0045_intraday_regression_channel_residual_discovery"
    data["active_run"] = "campaigns/C0045_intraday_regression_channel_residual_discovery/runs/R0003"
    data["next_required_action"] = "produce_c0045_r0003_mt5_logic_parity_evidence"
    data["current_evidence_summary"] = {
        "source_campaign": "campaigns/C0045_intraday_regression_channel_residual_discovery",
        "current_task": "produce_c0045_r0003_mt5_logic_parity_evidence",
        "active_run": "campaigns/C0045_intraday_regression_channel_residual_discovery/runs/R0003",
        "active_run_status": "proxy_recorded_pending_mt5",
        "evidence_status": "proxy_recorded_pending_mt5",
        "campaign_family": "intraday_regression_channel_residual",
        "entries_per_active_day": summary.get("entries_per_active_day") if isinstance(summary, dict) else None,
        "proxy_trade_count": required.get("proxy_trade_count") if isinstance(required, dict) else None,
        "proxy_net_pnl_points": required.get("proxy_net_pnl_points") if isinstance(required, dict) else None,
        "proxy_profit_factor": required.get("proxy_profit_factor") if isinstance(required, dict) else None,
        "next_required_action": "produce_c0045_r0003_mt5_logic_parity_evidence",
        "note": "C0045 R0003 proxy evidence recorded; MT5 paired validation remains mandatory regardless of proxy result.",
        "source_campaign_manifest": "campaigns/C0045_intraday_regression_channel_residual_discovery/campaign.yaml",
    }
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0045",
            "c0004_r0001_fold_local_state_archetype": "c0045_r0003_intraday_regression_channel_residual_stress_materialization",
            "c0004_r0001": "c0045_r0003",
            "fold_local_state_archetype_discovery": "intraday_regression_channel_residual_discovery",
            ...
            "state_archetype_summary": "intraday_regression_channel_residual_summary",
        }
...
def regression_channel_residual_ratio_at(bars: list[base.Bar], index: int, lookback: int, average_range: float) -> float:
...
def strongest_regression_channel_residual_index(
    bars: list[base.Bar],
    range_average: list[float | None],
    index: int,
    lookback: int,
...
def build_regression_channel_residual_context(
    ...
    for index, bar in enumerate(bars):
        ...
        shock_index = strongest_regression_channel_residual_index(bars, range_average, index, 24, average_range)
        ...
        regression_channel_residual_12 = regression_channel_residual_ratio_at(bars, index, 12, average_range)
        contexts[index] = {
            "average_range": average_range,
            ...
            "regression_channel_residual_6": regression_channel_residual_ratio_at(bars, index, 6, average_range),
            "regression_channel_residual_12": regression_channel_residual_12,
            "regression_channel_residual_24": regression_channel_residual_ratio_at(bars, index, 24, average_range),
            "regression_channel_residual_48": regression_channel_residual_ratio_at(bars, index, 48, average_range),
            ...
            * (1.0 + regression_channel_residual_12 + shock_cluster_density_at(bars, index, 12, average_range)),
            ...
            "shock_regime_contrast_12_48": regression_channel_residual_12
            / max(regression_channel_residual_ratio_at(bars, index, 48, average_range), 1e-9)
            - 1.0,
...
def intraday_regression_channel_residual_features(
    ...
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    regression_channel_residual_context: list[dict[str, float] | None],
    index: int,
    direction: int,
) -> list[float] | None:
    average_range = range_average[index]
    day = day_context[index]
    context = regression_channel_residual_context[index]
    ...
    features = [
        ...
        context["regression_channel_residual_6"],
        context["regression_channel_residual_12"],
        context["regression_channel_residual_24"],
        context["regression_channel_residual_48"],
        ...
        context["session_progress"] * max(context["regression_channel_residual_12"], 0.0),
...
def intraday_regression_channel_residual_features(
    ...
    day_regression_channel_residual = context["day_high_recency"] if direction > 0 else context["day_low_recency"]
    ...
    features = [
        ...
        day_regression_channel_residual,
...
def ols_channel_stats(
    ...
    if start < 0 or average_range <= 0:
        return None
    closes = [bars[offset].close for offset in range(start, index + 1)]
    n = len(closes)
    if n < 3:
        return None
    x_mean = (n - 1) / 2.0
    y_mean = sum(closes) / n
    denominator = sum((offset - x_mean) ** 2 for offset in range(n))
    if denominator <= 0:
        return None
    slope_raw = sum((offset - x_mean) * (closes[offset] - y_mean) for offset in range(n)) / denominator
    intercept = y_mean - slope_raw * x_mean
    fitted = [intercept + slope_raw * offset for offset in range(n)]
    residuals = [closes[offset] - fitted[offset] for offset in range(n)]
    residual = residuals[-1]
    residual_mean = sum(residuals) / n
    residual_variance = sum((value - residual_mean) ** 2 for value in residuals) / n
    residual_std = math.sqrt(max(residual_variance, 0.0))
    residual_span = max(residuals) - min(residuals)
    avg = max(average_range, 1e-9)
    residual_z = residual / residual_std if residual_std > 1e-9 else 0.0
    residual_norm = residual / avg
    slope_norm = slope_raw / avg
    total_path = sum(abs(closes[offset] - closes[offset - 1]) for offset in range(1, n)) / avg
    endpoint_move = (closes[-1] - closes[0]) / avg
    efficiency = abs(endpoint_move) / max(total_path, 1e-9)
    ss_total = sum((value - y_mean) ** 2 for value in closes)
    ss_residual = sum((closes[offset] - fitted[offset]) ** 2 for offset in range(n))
    r2 = 1.0 - ss_residual / max(ss_total, 1e-9) if ss_total > 1e-9 else 0.0
    if residual_span > 1e-9:
        position = (residual - min(residuals)) / residual_span
    else:
        position = 0.5
    if abs(residual) > 1e-9:
        residual_sign = 1.0 if residual > 0.0 else -1.0
        persistence = sum(1 for value in residuals if value * residual_sign > 0.0) / n
    else:
        persistence = 0.0
    return {
        "slope": slope_norm,
        ...
        "residual_abs_z": abs(residual_z),
        "channel_width": max(residual_span / avg, residual_std / avg, 1e-6),
        "position": bounded(position, 0.0, 1.0),
        "lower_distance": bounded(position, 0.0, 1.0),
        "upper_distance": bounded(1.0 - position, 0.0, 1.0),
        "persistence": bounded(persistence, 0.0, 1.0),
        "efficiency": bounded(efficiency, 0.0, 4.0),
        "r2": bounded(r2, 0.0, 1.0),
...
def build_regression_channel_residual_context(
    ...
    for index, bar in enumerate(bars):
        ...
        if index < 120 or average_range is None or average_range <= 0 or short_average_range is None or day is None:
            continue
        stats_12 = ols_channel_stats(bars, index, 12, average_range)
        stats_24 = ols_channel_stats(bars, index, 24, average_range)
        stats_48 = ols_channel_stats(bars, index, 48, average_range)
        if stats_12 is None or stats_24 is None or stats_48 is None:
            continue
        prev_3 = ols_channel_stats(
            bars,
            index - 3,
            24,
            range_average[index - 3] if index >= 3 and range_average[index - 3] else average_range,
        )
        prev_6 = ols_channel_stats(
            bars,
            index - 6,
            24,
            range_average[index - 6] if index >= 6 and range_average[index - 6] else average_range,
        )
        day_width = max(day["day_high"] - day["day_low"], 1e-9)
        day_position = bounded((bar.close - day["day_low"]) / day_width, 0.0, 1.0)
        bars_since_open = max(day["bars_since_day_open"], 1.0)
        day_slope_proxy = (bar.close - day["day_open"]) / max(average_range * bars_since_open, 1e-9)
        spread_over_range = bar.spread_points / max(average_range, 1e-9)
        contexts[index] = {
            "slope_12": stats_12["slope"],
            "slope_24": stats_24["slope"],
            "slope_48": stats_48["slope"],
            "slope_acceleration_12_48": stats_12["slope"] - stats_48["slope"],
            "residual_z_12": stats_12["residual_z"],
            "residual_z_24": stats_24["residual_z"],
            "residual_z_48": stats_48["residual_z"],
            "residual_abs_z_24": stats_24["residual_abs_z"],
            "residual_z_change_3": stats_24["residual_z"] - (prev_3["residual_z"] if prev_3 is not None else 0.0),
            "residual_z_change_6": stats_24["residual_z"] - (prev_6["residual_z"] if prev_6 is not None else 0.0),
            "residual_persistence_12": stats_12["persistence"],
            "residual_persistence_24": stats_24["persistence"],
            "channel_width_12": stats_12["channel_width"],
            "channel_width_24": stats_24["channel_width"],
            "channel_width_48": stats_48["channel_width"],
            "channel_width_change_12_48": stats_12["channel_width"] - stats_48["channel_width"],
            "channel_compression_24_48": stats_24["channel_width"] / max(stats_48["channel_width"], 1e-9) - 1.0,
            "channel_efficiency_24": stats_24["efficiency"],
            "channel_efficiency_48": stats_48["efficiency"],
            "regression_r2_24": stats_24["r2"],
            "regression_r2_48": stats_48["r2"],
            "residual_position_in_channel_24": stats_24["position"],
            "distance_to_upper_channel_24": stats_24["upper_distance"],
            "distance_to_lower_channel_24": stats_24["lower_distance"],
            "day_slope_proxy": day_slope_proxy,
            "day_residual_pressure": (day_position - 0.5) * 2.0 - 0.25 * bounded(day_slope_proxy, -4.0, 4.0),
            "day_channel_position": day_position,
            "session_progress": bounded(day["bars_since_day_open"] / 288.0, 0.0, 1.0),
            "spread_over_range": spread_over_range,
            "spread_channel_stress_24": spread_over_range * (1.0 + stats_24["channel_width"] + 0.25 * abs(stats_24["residual_z"])),
            "path_churn_ratio_12": path_churn_ratio(bars, index, 12),
            "path_efficiency_24": abs(directional_efficiency(bars, index, 1, 24)),
            "normalized_close_return_3": close_delta_over(bars, index, 3, average_range),
            "regression_regime_contrast_12_48": (stats_12["r2"] - stats_48["r2"])
            + 0.5 * (stats_12["channel_width"] / max(stats_48["channel_width"], 1e-9) - 1.0),
        }
    return contexts
...
def intraday_regression_channel_residual_features(
    ...
    day = day_context[index]
    context = regression_channel_residual_context[index]
    if average_range is None or average_range <= 0 or short_average_range is None or day is None or context is None:
        return None
    residual_z_24 = context["residual_z_24"]
    directional_residual = direction * residual_z_24
    directional_slope = direction * context["slope_24"]
    directional_channel_position = direction * (context["residual_position_in_channel_24"] - 0.5) * 2.0
    directional_edge_distance = (
        context["distance_to_upper_channel_24"] if direction > 0 else context["distance_to_lower_channel_24"]
    )
    mean_reversion_pressure = max(0.0, -directional_residual) * min(abs(residual_z_24), 4.0)
    continuation_pressure = max(0.0, directional_residual) * max(0.0, directional_slope)
    slope_residual_alignment = direction * context["slope_24"] * bounded(residual_z_24 / 4.0, -1.0, 1.0)
    trend_channel_stress = abs(residual_z_24) * abs(context["slope_24"]) * (1.0 + context["channel_width_24"])
    spread_adjusted_edge = directional_edge_distance - context["spread_channel_stress_24"]
    spread_adjusted_mean_reversion = mean_reversion_pressure - context["spread_channel_stress_24"]
    spread_adjusted_continuation = continuation_pressure - context["spread_channel_stress_24"]
    positive_cost_buffer = max(0.0, spread_adjusted_edge)
    tick_fragility_proxy = context["spread_channel_stress_24"] * (1.0 + context["path_churn_ratio_12"])
    late_session_cost_pressure = context["session_progress"] * context["spread_channel_stress_24"]
    short_side_asymmetry_bias = 1.0 if direction < 0 else -1.0
    adverse_cost_survival_buffer_001 = spread_adjusted_edge - ADVERSE_COST_STRESS_LEVELS[0] / max(average_range, 1e-9)
    adverse_cost_survival_buffer_002 = spread_adjusted_edge - ADVERSE_COST_STRESS_LEVELS[1] / max(average_range, 1e-9)
    materialization_quantization_margin = min(
        abs(spread_adjusted_edge),
        abs(spread_adjusted_mean_reversion),
        abs(spread_adjusted_continuation),
        abs(directional_slope),
        abs(directional_residual),
    )
    materialization_input_stability = 1.0 / (
        1.0
        + abs(context["residual_z_change_3"])
        + abs(context["residual_z_change_6"])
        + context["path_churn_ratio_12"]
        + 2.0 * context["spread_channel_stress_24"]
    )
    features = [
        float(direction),
        directional_close_location(bars[index], direction),
        ...
        context["slope_48"],
        directional_slope,
        context["slope_acceleration_12_48"],
        context["residual_z_12"],
        residual_z_24,
        context["residual_z_48"],
        directional_residual,
        context["residual_abs_z_24"],
        context["residual_z_change_3"],
        context["residual_z_change_6"],
        context["residual_persistence_12"],
        context["residual_persistence_24"],
        context["channel_width_12"],
        context["channel_width_24"],
        context["channel_width_48"],
        context["channel_width_change_12_48"],
        context["channel_compression_24_48"],
        context["channel_efficiency_24"],
        context["channel_efficiency_48"],
        context["regression_r2_24"],
        context["regression_r2_48"],
        context["residual_position_in_channel_24"],
        directional_channel_position,
        context["distance_to_upper_channel_24"],
        context["distance_to_lower_channel_24"],
        directional_edge_distance,
        mean_reversion_pressure,
        continuation_pressure,
        slope_residual_alignment,
        trend_channel_stress,
        direction * context["day_slope_proxy"],
        direction * context["day_residual_pressure"],
        direction * (context["day_channel_position"] - 0.5) * 2.0,
        context["session_progress"],
        context["spread_over_range"],
        context["spread_channel_stress_24"],
        context["path_churn_ratio_12"],
        context["path_efficiency_24"],
        direction * context["normalized_close_return_3"],
        directional_close_streak(bars, index, direction, 12),
        context["regression_regime_contrast_12_48"],
        spread_adjusted_edge,
        spread_adjusted_mean_reversion,
        spread_adjusted_continuation,
        positive_cost_buffer,
        tick_fragility_proxy,
        late_session_cost_pressure,
        short_side_asymmetry_bias,
        adverse_cost_survival_buffer_001,
        adverse_cost_survival_buffer_002,
...
def intraday_regression_channel_residual_label(
    ...
    if direction > 0:
        ...
        for offset, bar in enumerate(path, start=1):
            ...
            if bar.low <= target_price:
                ...
                event_bar = offset
                break
    favorable = mfe / max(average_range, 1e-9)
    adverse = mae / max(average_range, 1e-9)
    terminal_norm = terminal / max(average_range, 1e-9)
    close_path = [entry] + [bar.close for bar in path]
    path_distance = sum(abs(close_path[offset] - close_path[offset - 1]) for offset in range(1, len(close_path)))
    path_efficiency = terminal / max(path_distance, 1e-9)
    target_speed = (len(path) - event_bar + 1) / len(path) if first_hit > 0.0 else 0.0
    ...
    early_path = path[: min(3, len(path))]
    if direction > 0:
        early_followthrough = max(bar.high - entry for bar in early_path) / max(average_range, 1e-9)
        early_adverse = max(entry - bar.low for bar in early_path) / max(average_range, 1e-9)
    else:
        early_followthrough = max(entry - bar.low for bar in early_path) / max(average_range, 1e-9)
        early_adverse = max(bar.high - entry for bar in early_path) / max(average_range, 1e-9)
    exit_stats = ols_channel_stats(bars, exit_index, 24, average_range)
    residual_followthrough = 0.0
    if exit_stats is not None:
        residual_followthrough = direction * (exit_stats["residual_z"] - context["residual_z_24"])
    directional_residual = direction * context["residual_z_24"]
    directional_slope = direction * context["slope_24"]
    mean_reversion_setup = max(0.0, -directional_residual) * min(abs(context["residual_z_24"]), 4.0)
    continuation_setup = max(0.0, directional_residual) * max(0.0, directional_slope)
    directional_edge_distance = (
        context["distance_to_upper_channel_24"] if direction > 0 else context["distance_to_lower_channel_24"]
    )
    cost_buffer = max(0.0, directional_edge_distance - context["spread_channel_stress_24"])
    tick_fragility_proxy = context["spread_channel_stress_24"] * (1.0 + context["path_churn_ratio_12"])
    adverse_cost_survival_buffer_001 = cost_buffer - ADVERSE_COST_STRESS_LEVELS[0] / max(average_range, 1e-9)
    adverse_cost_survival_buffer_002 = cost_buffer - ADVERSE_COST_STRESS_LEVELS[1] / max(average_range, 1e-9)
    materialization_input_stability = 1.0 / (
        1.0
        + abs(context["residual_z_change_3"])
        + abs(context["residual_z_change_6"])
    ...
    short_side_quality = 0.10 if direction < 0 else -0.18
    adverse_channel_drift = max(0.0, -residual_followthrough)
    regression_quality = (
        0.40 * context["regression_r2_24"]
        + 0.25 * context["channel_efficiency_24"]
        - 0.12 * context["channel_width_24"]
    )
    spread_norm = bars[entry_index].spread_points / max(average_range, 1e-9)
    path_spread_peak = max(bar.spread_points for bar in path) / max(average_range, 1e-9)
    return (
        0.88 * first_hit
        + 0.30 * target_speed
        - 0.48 * stop_speed
        ...
        - 0.62 * adverse
        + 0.38 * terminal_norm
        + 0.24 * path_efficiency
        + 0.20 * early_followthrough
        - 0.34 * early_adverse
        + 0.16 * mean_reversion_setup
        + 0.14 * continuation_setup
        + 0.18 * cost_buffer
        + 0.18 * max(0.0, adverse_cost_survival_buffer_001)
        + 0.22 * max(0.0, adverse_cost_survival_buffer_002)
        - 0.18 * max(0.0, -adverse_cost_survival_buffer_002)
        + 0.10 * materialization_input_stability
        + 0.22 * residual_followthrough
        - 0.30 * adverse_channel_drift
        + 0.12 * regression_quality
        - 0.25 * giveback
        - 0.26 * future_path_churn(path, entry)
        - 1.45 * spread_norm
        - 0.34 * max(0.0, path_spread_peak - spread_norm)
        - 0.30 * context["spread_channel_stress_24"]
        - 0.16 * tick_fragility_proxy
        - 0.08 * context["session_progress"] * context["spread_channel_stress_24"]
        + short_side_quality
    )
