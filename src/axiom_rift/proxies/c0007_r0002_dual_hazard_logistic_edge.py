"""C0007 R0002 proxy evidence for dual-hazard logistic supervised edge discovery."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0007_r0002_proxy_trades.csv"
LOGISTIC_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0007_r0002_dual_hazard_logistic_summary.json"
...
MODEL_FAMILY = "fold_local_dual_hazard_logistic_edge_classifier"
...
SELECTION_RULE = "top_fold_local_dual_hazard_logistic_edge_scores_per_active_day"
...
class LogisticEdgeModel:
    ...
    positive_classifier: LogisticRegression | None
    adverse_classifier: LogisticRegression | None
...
def run_c0007_r0002_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_logistic_edge_model(train_candidates, fold_id)
        ...
        fold_models.append(logistic_model_summary(model))
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = dual_hazard_features(bars, range_average, short_range_average, index, direction)
            if features is None:
                continue
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|dual_hazard_logistic_edge",
                    features=features,
                    label=label,
                )
            )
...
def fit_logistic_edge_model(candidates: list[base.Candidate], fold_id: str) -> LogisticEdgeModel:
...
def fit_binary_classifier(features: np.ndarray, target: np.ndarray) -> LogisticRegression | None:
...
def score_candidates(candidates: list[base.Candidate], model: LogisticEdgeModel) -> list[base.Candidate]:
    if not candidates:
        return []
    ...
    scores = positive_probability - 0.92 * adverse_probability + 0.10 * positive_lift - 0.06 * adverse_lift
...
def class_probability(classifier: LogisticRegression | None, features: np.ndarray, fallback: float) -> np.ndarray:
...
def build_proxy_payload(
    ...
    payload["proxy_id"] = "PX-C0007-R0002"
    payload["proxy_engine"] = "axiom_rift.proxies.c0007_r0002_dual_hazard_logistic_edge"
    payload["proxy_config_path"] = "campaigns/C0007_fold_local_supervised_edge_discovery/runs/R0002/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_dual_hazard_logistic_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0007_fold_local_supervised_edge_discovery/runs/R0002/kpi/proxy.json",
        "campaigns/C0007_fold_local_supervised_edge_discovery/runs/R0002/artifacts/c0007_r0002_proxy_trades.csv",
        "campaigns/C0007_fold_local_supervised_edge_discovery/runs/R0002/artifacts/c0007_r0002_dual_hazard_logistic_summary.json",
    ]
    ...
    profiles["dual_hazard_logistic_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0007_r0002_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "variant_boundary": "dual_hazard_logistic_classifier_not_r0001_linear_rank_or_parameter_retry",
        }
    )
    return config


def logistic_model_summary(model: LogisticEdgeModel) -> dict[str, object]:
    return {
        ...
        "positive_intercept": classifier_intercept(model.positive_classifier),
        "adverse_intercept": classifier_intercept(model.adverse_classifier),
        "positive_coefficients": classifier_coefficients(model.positive_classifier),
        "adverse_coefficients": classifier_coefficients(model.adverse_classifier),
        "score_interpretation": "higher_score_means_positive_target_probability_minus_adverse_path_hazard",
    }


def classifier_intercept(classifier: LogisticRegression | None) -> float | None:
    if classifier is None:
        return None
    return base.rounded(float(classifier.intercept_[0]))


def classifier_coefficients(classifier: LogisticRegression | None) -> list[float]:
    if classifier is None:
        return []
    return [base.rounded(float(value)) for value in classifier.coef_[0]]
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_logistic_summary_artifact(payload, LOGISTIC_SUMMARY_PATH)
    PROXY_PATH.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="ascii")
    trade_hash = base.sha256_file(TRADE_ARTIFACT_PATH)
    summary_hash = base.sha256_file(LOGISTIC_SUMMARY_PATH)
...
def write_logistic_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_dual_hazard_logistic_edge_summary_v1",
        "template": False,
        "work_unit_id": "C0007",
        "campaign_id": "C0007",
        "run_id": "R0002",
        ...
        "dual_hazard_logistic_profile": profiles["dual_hazard_logistic_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "dual_hazard_logistic_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0007-R0002-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0007_fold_local_supervised_edge_discovery/runs/R0002/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0007_fold_local_supervised_edge_discovery/runs/R0002/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0007-R0002-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0007-R0002-DUAL-HAZARD-LOGISTIC-SUMMARY",
                "dual_hazard_logistic_summary_artifact",
                "json",
                "campaigns/C0007_fold_local_supervised_edge_discovery/runs/R0002/artifacts/c0007_r0002_dual_hazard_logistic_summary.json",
                summary_hash,
                ["campaigns/C0007_fold_local_supervised_edge_discovery/runs/R0002/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0007_r0002_dual_hazard_logistic_edge",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0007_r0002_proxy_trades.csv"
    evidence["dual_hazard_logistic_summary"] = "artifacts/c0007_r0002_dual_hazard_logistic_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0007_r0002_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0007_r0002_proxy_trades.csv",
        "artifacts/c0007_r0002_dual_hazard_logistic_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0007 R0002 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0007_r0002_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0007_r0002_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    for item in (
        "open_c0007_r0002_dual_hazard_logistic_edge_classifier_run",
        "produce_c0007_r0002_proxy_evidence",
    ):
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0007_r0002_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0007_fold_local_supervised_edge_discovery/runs/R0002/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0007_r0002_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "R0001": "R0002",
            "r0001": "r0002",
            "PX-C0007-R0001": "PX-C0007-R0002",
            "c0007_r0001_fold_local_supervised_edge": "c0007_r0002_dual_hazard_logistic_edge",
            "c0007_r0001": "c0007_r0002",
            ...
            "supervised_edge_summary": "dual_hazard_logistic_summary",
            "direct_fold_local_supervised_edge_rank": "positive_target_probability_minus_adverse_path_hazard",
            "supervised_linear_rank": "dual_hazard_logistic_edge",
        }
