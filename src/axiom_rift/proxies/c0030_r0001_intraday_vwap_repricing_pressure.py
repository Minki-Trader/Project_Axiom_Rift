"""C0030 R0001 proxy evidence for intraday vwap repricing pressure discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0030_intraday_vwap_repricing_pressure_discovery" / "runs" / "R0001"
CAMPAIGN_PATH = PROJECT_ROOT / "campaigns" / "C0030_intraday_vwap_repricing_pressure_discovery" / "campaign.yaml"
PROXY_PATH = RUN_DIR / "kpi" / "proxy.json"
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0030_r0001_proxy_trades.csv"
INTRADAY_VWAP_REPRICING_PRESSURE_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0030_r0001_intraday_vwap_repricing_pressure_summary.json"
...
FEATURE_NAMES = (
    "direction_bias",
    "directional_vwap_deviation_atr",
    "abs_vwap_deviation_atr",
    "vwap_slope_6_atr",
    "vwap_slope_24_atr",
    "vwap_curvature_6_24",
    "directional_vwap_reclaim_rate_6",
    "directional_vwap_acceptance_rate_12",
    "vwap_cross_rate_12",
    "directional_vwap_band_position_24",
    "vwap_band_width_atr_24",
    "vwap_pull_distance_atr",
    "volume_weighted_repricing_pressure_6",
    "volume_weighted_repricing_pressure_24",
    "effort_vs_repricing_12",
    "vwap_rejection_wick_mean_3",
    "vwap_follow_wick_mean_3",
    "vwap_wick_asymmetry_3",
    "directional_vwap_side_persistence_6",
    "adverse_vwap_side_run_12",
    "vwap_deviation_change_3_atr",
    "vwap_repricing_impulse_3",
    ...
    "vwap_day_volume_share",
    "vwap_session_progress",
    "spread_over_range",
    "spread_relief_3_vs_24",
    "session_position",
)
MODEL_FAMILY = "fold_local_intraday_vwap_repricing_pressure_rank"
LABEL_SHAPE = "directional_intraday_vwap_repricing_pressure_survival_quality"
POSITIVE_LABEL_THRESHOLD = 0.18
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_intraday_vwap_repricing_pressure_scores_per_active_day"
...
def run_c0030_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    vwap_context = build_vwap_context(bars, volumes)
    trades: list[base.Trade] = []
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = intraday_vwap_repricing_pressure_features(
                bars,
                range_average,
                short_range_average,
                day_context,
            ...
            if features is None:
                continue
            label = (
                intraday_vwap_repricing_pressure_label(bars, range_average, vwap_context, index, direction)
                if include_labels
                else None
            )
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|intraday_vwap_repricing_pressure",
                    features=features,
                    label=label,
                )
            )
...
def intraday_vwap_repricing_pressure_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    day_context: list[dict[str, float] | None],
    ...
    if spread_over_range > 0.65:
        return None
    state = vwap_repricing_pressure_state(bars, vwap_context, index, direction, average_range)
    if state is None:
        return None
    close_streak = directional_close_streak(bars, index, direction, 6)
    rejection_3 = vwap_rejection_wick(bars, vwap_context, index, direction, 3)
    follow_3 = vwap_follow_wick(bars, vwap_context, index, direction, 3)
    return (
        float(direction),
        state["directional_vwap_deviation_atr"],
        state["abs_vwap_deviation_atr"],
        state["vwap_slope_6_atr"],
        state["vwap_slope_24_atr"],
        state["vwap_curvature_6_24"],
        directional_vwap_reclaim_rate(bars, vwap_context, index, direction, 6),
        directional_vwap_acceptance_rate(bars, vwap_context, index, direction, 12),
        vwap_cross_rate(bars, vwap_context, index, 12),
        state["directional_vwap_band_position_24"],
        state["vwap_band_width_atr_24"],
        state["vwap_pull_distance_atr"],
        volume_weighted_repricing_pressure(bars, volumes, vwap_context, index, direction, 6, average_range),
        volume_weighted_repricing_pressure(bars, volumes, vwap_context, index, direction, 24, average_range),
        effort_vs_repricing(bars, volumes, vwap_context, index, direction, 12, average_range),
        rejection_3,
        follow_3,
        rejection_3 - follow_3,
        directional_vwap_side_persistence(bars, vwap_context, index, direction, 6),
        adverse_vwap_side_run(bars, vwap_context, index, direction, 12),
        vwap_deviation_change(bars, vwap_context, index, direction, 3, average_range),
        direction * (vwap_context[index]["vwap"] - vwap_context[index - 3]["vwap"]) / average_range,
        ...
        range_contraction_near_vwap(bars, vwap_context, index, average_range),
        ...
        vwap_context[index]["volume"] / max(vwap_context[index]["cumulative_volume"], 1e-9),
        bounded(vwap_context[index]["bars_since_anchor"] / 288.0, 0.0, 1.0),
        spread_over_range,
        spread_relief,
...
def intraday_vwap_repricing_pressure_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    vwap_context: list[dict[str, float]],
    index: int,
    ...
    future_vwap_acceptance = future_directional_vwap_acceptance(path, vwap_context, entry_index, direction)
    future_vwap_repricing_quality = future_directional_vwap_repricing_quality(
        bars,
        vwap_context,
        entry_index,
        exit_index,
    ...
    future_vwap_retention = future_directional_vwap_retention(path, vwap_context, entry_index, direction)
    future_failed_vwap_penalty = future_directional_failed_vwap_reclaim_penalty(
        bars,
        vwap_context,
        entry_index,
        exit_index,
    ...
    return (
        ...
        + 0.24 * future_vwap_acceptance
        + 0.24 * future_vwap_repricing_quality
        + 0.18 * future_vwap_retention
        - 0.34 * future_failed_vwap_penalty
        - 1.08 * spread_norm
        - 0.20 * spread_drift
    )
...
def build_proxy_payload(
    ...
    payload["campaign_id"] = "C0030"
    payload["work_unit_id"] = "C0030"
    payload["run_id"] = "R0001"
    payload["proxy_id"] = "PX-C0030-R0001"
    payload["proxy_engine"] = "axiom_rift.proxies.c0030_r0001_intraday_vwap_repricing_pressure"
    payload["proxy_config_path"] = "campaigns/C0030_intraday_vwap_repricing_pressure_discovery/runs/R0001/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_intraday_vwap_repricing_pressure_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0030_intraday_vwap_repricing_pressure_discovery/runs/R0001/kpi/proxy.json",
        "campaigns/C0030_intraday_vwap_repricing_pressure_discovery/runs/R0001/artifacts/c0030_r0001_proxy_trades.csv",
        "campaigns/C0030_intraday_vwap_repricing_pressure_discovery/runs/R0001/artifacts/c0030_r0001_intraday_vwap_repricing_pressure_summary.json",
    ]
    ...
    profiles["intraday_vwap_repricing_pressure_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0030_r0001_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "feature_weight_floor": FEATURE_WEIGHT_FLOOR,
            "feature_weight_ceiling": FEATURE_WEIGHT_CEILING,
            "score_interpretation": "higher_score_means_direct_fold_local_intraday_vwap_repricing_pressure_rank",
            "variant_boundary": (
                "intraday_vwap_repricing_pressure_rank_not_range_overlap_calendar_phase_tick_participation_"
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_intraday_vwap_repricing_pressure_summary_artifact(payload, INTRADAY_VWAP_REPRICING_PRESSURE_SUMMARY_PATH)
    PROXY_PATH.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="ascii")
    trade_hash = base.sha256_file(TRADE_ARTIFACT_PATH)
    summary_hash = base.sha256_file(INTRADAY_VWAP_REPRICING_PRESSURE_SUMMARY_PATH)
...
def write_intraday_vwap_repricing_pressure_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_intraday_vwap_repricing_pressure_summary_v1",
        "template": False,
        "work_unit_id": "C0030",
        "campaign_id": "C0030",
        "run_id": "R0001",
        ...
        "intraday_vwap_repricing_pressure_profile": profiles["intraday_vwap_repricing_pressure_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "intraday_vwap_repricing_pressure_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0030-R0001-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0030_intraday_vwap_repricing_pressure_discovery/runs/R0001/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0030_intraday_vwap_repricing_pressure_discovery/runs/R0001/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0030-R0001-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0030-R0001-INTRADAY-VWAP-REPRICING-PRESSURE-SUMMARY",
                "intraday_vwap_repricing_pressure_summary_artifact",
                "json",
                "campaigns/C0030_intraday_vwap_repricing_pressure_discovery/runs/R0001/artifacts/c0030_r0001_intraday_vwap_repricing_pressure_summary.json",
                summary_hash,
                ["campaigns/C0030_intraday_vwap_repricing_pressure_discovery/runs/R0001/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0030_r0001_intraday_vwap_repricing_pressure",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0030_r0001_proxy_trades.csv"
    evidence["intraday_vwap_repricing_pressure_summary"] = "artifacts/c0030_r0001_intraday_vwap_repricing_pressure_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0030_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0030_r0001_proxy_trades.csv",
        "artifacts/c0030_r0001_intraday_vwap_repricing_pressure_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0030 R0001 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0030_r0001_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0030_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    data["project"]["active_campaign"] = "campaigns/C0030_intraday_vwap_repricing_pressure_discovery"
    next_work = data.setdefault("next_work", {})
    next_work["campaign"] = "campaigns/C0030_intraday_vwap_repricing_pressure_discovery"
    completed = list(next_work.get("completed") or [])
    for item in (
        "open_c0030_intraday_vwap_repricing_pressure_discovery",
        "open_c0030_r0001_fold_local_intraday_vwap_repricing_pressure_rank_run",
        "produce_c0030_r0001_proxy_evidence",
    ):
    ...
    data["active_run"] = "campaigns/C0030_intraday_vwap_repricing_pressure_discovery/runs/R0001"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["active_campaign"] = "campaigns/C0030_intraday_vwap_repricing_pressure_discovery"
    data["active_run"] = "campaigns/C0030_intraday_vwap_repricing_pressure_discovery/runs/R0001"
    data["latest_operation"] = {
        "id": "produce_c0030_r0001_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0030_intraday_vwap_repricing_pressure_discovery/runs/R0001/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0030_r0001_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0030",
            "c0004_r0001_fold_local_state_archetype": "c0030_r0001_intraday_vwap_repricing_pressure",
            "c0004_r0001": "c0030_r0001",
            "fold_local_state_archetype_discovery": "intraday_vwap_repricing_pressure_discovery",
            ...
            "state_archetype_summary": "intraday_vwap_repricing_pressure_summary",
        }
