"""R0003 proxy evidence for failed-breakout reclaim reversal."""
...
def run_r0003_proxy(write: bool = True) -> dict[str, object]:
            ...
            if index > start_index:
                ...
                if active is None:
                    ...
                    if state == 1:
                        ...
                        if breakout_reclaim_confirmed(bars[closed_index], direction, expansion_breakout_level):
                            pending_direction = reversal_direction(direction)
                            pending_average_range = expansion_average_range
                            state = 0
                        elif bars_since_expansion >= RECLAIM_SEARCH_BARS:
                            state = 0
                            index += 1
                            continue
                        else:
                        ...
                        next_direction, breakout_level = expansion_signal(
                            bars[closed_index],
                            ranges[closed_index],
                        ...
                        if next_direction != 0:
                            ...
                            expansion_breakout_level = breakout_level
...
def breakout_reclaim_confirmed(bar: Bar, direction: int, breakout_level: float) -> bool:
    if breakout_level <= 0:
        return False
    if direction > 0:
...
def expansion_signal(
    ...
    if body > 0 and bar.close > previous_high:
        return 1, previous_high
    if body < 0 and bar.close < previous_low:
        return -1, previous_low
    return 0, 0.0
...
def build_proxy_payload(trades: list[Trade], windows: list[Window]) -> dict[str, object]:
    ...
    return {
        ...
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.r0003_failed_breakout_reclaim_reversal",
        "proxy_config_path": "campaigns/C0001_regime_response_discovery/runs/R0003/run_manifest.json",
        ...
        "deferred_with_reason": [
            {
                ...
                "reason": "R0003 proxy currently measures raw US100 index points only; USD conversion requires MT5 symbol contract, tick value, and fixed-lot discovery sizing",
                "blocking_condition": "record the matching MT5 symbol specification and fixed-lot sizing for the 500 USD account",
                "revisit_when": "during R0003 MT5 probe evidence production",
            ...
            },
            {
                ...
                "revisit_when": "before R0003 closeout",
                "claim_boundary": {"claim_authority": False},
            }
        ],
        "claim_boundary": {
...
def proxy_config() -> dict[str, object]:
    return {
        "bar_basis": "closed_m5",
        "entry_basis": "next_bar_open_after_breakout_reclaim_confirmation",
        ...
        "reclaim_search_bars": RECLAIM_SEARCH_BARS,
        ...
        "response_mode": "failed_breakout_reclaim_reversal",
        "trade_direction_policy": "reverse_expansion_direction_after_breakout_level_reclaim",
    }
...
def update_artifact_lineage(proxy_hash: str) -> None:
    ...
    for record in data.get("artifact_records", []):
        if record.get("artifact_id") == "A0001":
            ...
            record["produced_by"] = "axiom_rift.proxies.r0003_failed_breakout_reclaim_reversal"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_matching_mt5_probe_for_R0003"
    data["deferred_with_reason"] = [
        {
            "field": "decision",
            "reason": "proxy evidence is recorded but MT5 and proxy-vs-MT5 evidence are still missing",
            "blocking_condition": "R0003 cannot close until the mandatory MT5 probe and proxy-vs-MT5 comparison are recorded",
            "revisit_when": "after R0003 MT5 and proxy-vs-MT5 evidence files contain measured results",
        }
    ]
