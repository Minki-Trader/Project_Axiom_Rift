"""C0009 R0003 proxy evidence for intrabar ambiguity avoidance discovery."""
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0009_execution_friction_aware_entry_discovery" / "runs" / "R0003"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0009_r0003_proxy_trades.csv"
AMBIGUITY_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0009_r0003_intrabar_ambiguity_avoidance_summary.json"
...
FEATURE_NAMES = (
    ...
    "prior_two_sided_wick_pressure_12",
    "prior_mid_close_pressure_12",
    "prior_range_spike_pressure_12",
    "spread_range_interaction",
    "session_sin",
    "session_cos",
    "minutes_from_core_mid",
)
MODEL_FAMILY = "fold_local_intrabar_ambiguity_avoidance_rank"
LABEL_SHAPE = "directional_unambiguous_target_first_minus_exit_path_risk"
POSITIVE_LABEL_THRESHOLD = 0.12
ADVERSE_LABEL_THRESHOLD = -0.36
FEATURE_WEIGHT_FLOOR = 0.25
FEATURE_WEIGHT_CEILING = 2.50
SELECTION_RULE = "top_fold_local_intrabar_ambiguity_avoidance_scores_per_active_day"
...
def run_c0009_r0003_proxy(write: bool = True) -> dict[str, object]:
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            features = intrabar_ambiguity_avoidance_features(bars, range_average, short_range_average, index, direction)
            if features is None:
                continue
            label = intrabar_ambiguity_avoidance_label(bars, range_average, index, direction) if include_labels else None
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|intrabar_ambiguity_avoidance",
                    features=features,
                    label=label,
                )
            )
...
def intrabar_ambiguity_avoidance_features(
    bars: list[base.Bar],
    range_average: list[float | None],
    short_range_average: list[float | None],
    index: int,
    ...
    prior_bars = bars[index - 11 : index + 1]
    two_sided_wicks: list[float] = []
    mid_close_hits = 0
    range_spikes: list[float] = []
    for prior in prior_bars:
        ...
        two_sided_wicks.append(min(prior_upper, prior_lower) / prior_range)
        prior_close_location = (prior.close - prior.low) / prior_range
        if 0.35 <= prior_close_location <= 0.65:
            mid_close_hits += 1
        range_spikes.append(max(0.0, prior_range / average_range - 1.0))
    prior_two_sided_wick_pressure = sum(two_sided_wicks) / len(two_sided_wicks)
    prior_mid_close_pressure = mid_close_hits / len(prior_bars)
    prior_range_spike_pressure = sum(range_spikes) / len(range_spikes)
    ...
    spread_range_interaction = spread_over_range * bar_range_over_average
    candidate_quality = (
        ...
        - 0.22 * prior_two_sided_wick_pressure
    )
    ...
    return (
        ...
        low_friction_flag,
        friction_adjusted_momentum,
        prior_two_sided_wick_pressure,
        prior_mid_close_pressure,
        prior_range_spike_pressure,
        spread_range_interaction,
        session_sin,
        session_cos,
        minutes_from_core_mid,
...
def intrabar_ambiguity_avoidance_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    index: int,
    direction: int,
    ...
    ambiguous_path_pressure = ambiguous_first_touch + 0.18 * ambiguous_bar_count
    clean_target_edge = 1.15 * first_hit + 0.34 * target_speed - 0.44 * stop_speed
    return (
        clean_target_edge
        + 0.26 * favorable
        - 0.42 * adverse
        - 1.30 * adverse_tail
        - 0.30 * giveback
        - 0.95 * ambiguous_path_pressure
        - 0.36 * near_stop_pressure
        + 0.22 * terminal_norm
        + 0.18 * path_alignment_edge
        + 0.14 * cost_buffer
        - 0.88 * spread_norm
        - 0.42 * spread_drift
        - 0.30 * path_instability
    )
...
def build_proxy_payload(
    ...
    payload["proxy_id"] = "PX-C0009-R0003"
    payload["proxy_engine"] = "axiom_rift.proxies.c0009_r0003_intrabar_ambiguity_avoidance"
    payload["proxy_config_path"] = "campaigns/C0009_execution_friction_aware_entry_discovery/runs/R0003/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_intrabar_ambiguity_avoidance_rank_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0009_execution_friction_aware_entry_discovery/runs/R0003/kpi/proxy.json",
        "campaigns/C0009_execution_friction_aware_entry_discovery/runs/R0003/artifacts/c0009_r0003_proxy_trades.csv",
        "campaigns/C0009_execution_friction_aware_entry_discovery/runs/R0003/artifacts/c0009_r0003_intrabar_ambiguity_avoidance_summary.json",
    ]
    ...
    profiles["intrabar_ambiguity_avoidance_profile"] = {  # type: ignore[index]
        "applies": True,
        "fields": {
    ...
    profiles["mt5_pairing_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "next_action": "produce_c0009_r0003_mt5_logic_parity_evidence",
        },
    }
    return payload
    ...
    config.update(
        {
            ...
            "feature_weight_floor": FEATURE_WEIGHT_FLOOR,
            "feature_weight_ceiling": FEATURE_WEIGHT_CEILING,
            "score_interpretation": "higher_score_means_fold_local_intrabar_ambiguity_avoidance_quality",
            "variant_boundary": "intrabar_ambiguity_avoidance_label_and_features_not_score_floor_quantile_daily_count_stop_target_hold_session_or_retry_nudge",
        }
    )
    return config
...
def write_proxy_evidence(payload: dict[str, object], trades: list[base.Trade]) -> None:
    ...
    write_ambiguity_summary_artifact(payload, AMBIGUITY_SUMMARY_PATH)
    PROXY_PATH.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="ascii")
    trade_hash = base.sha256_file(TRADE_ARTIFACT_PATH)
    summary_hash = base.sha256_file(AMBIGUITY_SUMMARY_PATH)
...
def write_ambiguity_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_fold_local_intrabar_ambiguity_avoidance_summary_v1",
        "template": False,
        "work_unit_id": "C0009",
        "campaign_id": "C0009",
        "run_id": "R0003",
        ...
        "intrabar_ambiguity_avoidance_profile": profiles["intrabar_ambiguity_avoidance_profile"]["fields"],  # type: ignore[index]
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    records = [
        ...
        if record.get("artifact_role") not in {"proxy_kpi", "proxy_trade_artifact", "intrabar_ambiguity_avoidance_summary_artifact"}
    ]
    records.extend(
        [
            artifact_record(
                "A-C0009-R0003-PROXY-KPI",
                "proxy_kpi",
                "json",
                "campaigns/C0009_execution_friction_aware_entry_discovery/runs/R0003/kpi/proxy.json",
                proxy_hash,
                [
                    ...
                    "campaigns/C0009_execution_friction_aware_entry_discovery/runs/R0003/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0009-R0003-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0009-R0003-INTRABAR-AMBIGUITY-AVOIDANCE-SUMMARY",
                "intrabar_ambiguity_avoidance_summary_artifact",
                "json",
                "campaigns/C0009_execution_friction_aware_entry_discovery/runs/R0003/artifacts/c0009_r0003_intrabar_ambiguity_avoidance_summary.json",
                summary_hash,
                ["campaigns/C0009_execution_friction_aware_entry_discovery/runs/R0003/kpi/proxy.json"],
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0009_r0003_intrabar_ambiguity_avoidance",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0009_r0003_proxy_trades.csv"
    evidence["intrabar_ambiguity_avoidance_summary"] = "artifacts/c0009_r0003_intrabar_ambiguity_avoidance_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0009_r0003_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0009_r0003_proxy_trades.csv",
        "artifacts/c0009_r0003_intrabar_ambiguity_avoidance_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0009 R0003 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0009_r0003_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0009_r0003_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    for item in (
        ...
        "open_c0009_r0003_intrabar_ambiguity_avoidance_run",
        "produce_c0009_r0003_proxy_evidence",
    ):
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0009_r0003_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0009_execution_friction_aware_entry_discovery/runs/R0003/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0009_r0003_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            "C0004": "C0009",
            "c0004_r0004_fold_local_state_archetype": "c0009_r0003_intrabar_ambiguity_avoidance",
            "c0004_r0004": "c0009_r0003",
            ...
            "state_archetype_summary": "intrabar_ambiguity_avoidance_summary",
        }
