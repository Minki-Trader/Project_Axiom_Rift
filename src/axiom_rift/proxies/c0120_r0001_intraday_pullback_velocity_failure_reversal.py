"""C0120 R0001 proxy evidence for intraday pullback velocity failure reversal."""
...
CAMPAIGN_ID = "C0120"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0120_intraday_pullback_velocity_failure_reversal_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0120_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0120_r0001_pullback_velocity_failure_reversal_summary.json"
...
FEATURE_NAMES = (
    "impulse_leg_strength",
    "pullback_velocity_decay",
    "pullback_depth_efficiency",
    "counter_impulse_rejection",
    "reclaim_settlement_strength",
    "pullback_volume_cooling",
    "followthrough_repair_pressure",
    "vwap_pullback_alignment",
    "prior_session_impulse_context",
    "session_pullback_position",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_pullback_velocity_failure_reversal_rank"
LABEL_SHAPE = "target_first_quality_after_pullback_velocity_failure_reversal"
SELECTION_RULE = "top_fold_local_pullback_velocity_failure_reversal_scores_per_active_day"
...
MIN_DAY_BARS = 18
PULLBACK_VELOCITY_FAILURE_REVERSAL_LOOKBACK_BARS = 48
PULLBACK_VELOCITY_FAILURE_REVERSAL_IMPULSE_BARS = 12
PULLBACK_VELOCITY_FAILURE_REVERSAL_PULLBACK_BARS = 10
MIN_IMPULSE_LEG_STRENGTH = 0.30
MIN_PULLBACK_VELOCITY_FAILURE_REVERSAL_ACTIVITY = 0.055
...
class LinearPullbackVelocityFailureReversalModel:
    fold_id: str
    ...
    pullback_velocity_failure_reversal_failure_rate: float
...
def run_c0120_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_pullback_velocity_failure_reversal_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_pullback_velocity_failure_reversal_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        PULLBACK_VELOCITY_FAILURE_REVERSAL_LOOKBACK_BARS,
        PULLBACK_VELOCITY_FAILURE_REVERSAL_PULLBACK_BARS + PULLBACK_VELOCITY_FAILURE_REVERSAL_IMPULSE_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = pullback_velocity_failure_reversal_metrics(
    ...
    features = (
        metrics["impulse_leg_strength"],
        metrics["pullback_velocity_decay"],
        metrics["pullback_depth_efficiency"],
        metrics["counter_impulse_rejection"],
        metrics["reclaim_settlement_strength"],
        metrics["pullback_volume_cooling"],
        metrics["followthrough_repair_pressure"],
        metrics["vwap_pullback_alignment"],
        metrics["prior_session_impulse_context"],
        metrics["session_pullback_position"],
        metrics["session_maturity"],
        metrics["spread_stress_inverse"],
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        breakout_pressure_bucket(metrics["impulse_leg_strength"]),
        breakout_pressure_bucket(metrics["pullback_velocity_decay"]),
        compression_bucket(metrics["pullback_depth_efficiency"]),
        absorption_bucket(metrics["counter_impulse_rejection"]),
        breakout_pressure_bucket(metrics["reclaim_settlement_strength"]),
        volume_confirmation_bucket(metrics["pullback_volume_cooling"]),
        breakout_pressure_bucket(metrics["followthrough_repair_pressure"]),
        vwap_alignment_bucket(metrics["vwap_pullback_alignment"]),
        prior_day_position_context_bucket(metrics["prior_session_impulse_context"]),
        range_position_pressure_bucket(metrics["session_pullback_position"]),
...
def pullback_velocity_failure_reversal_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    pullback_start = max(index - PULLBACK_VELOCITY_FAILURE_REVERSAL_PULLBACK_BARS + 1, context.day_start_index)
    impulse_start = max(pullback_start - PULLBACK_VELOCITY_FAILURE_REVERSAL_IMPULSE_BARS, context.day_start_index)
    lookback_start = max(index - PULLBACK_VELOCITY_FAILURE_REVERSAL_LOOKBACK_BARS + 1, context.day_start_index)
    if impulse_start < 0 or pullback_start < 0 or lookback_start < 0:
        return {"event_count": 0.0}
    impulse_window = bars[impulse_start:pullback_start]
    ...
    if len(impulse_window) < 6 or len(pullback_window) < 5 or len(lookback_window) < 24 or len(day_window) < MIN_DAY_BARS:
        return {"event_count": 0.0}
    ...
    impulse_volume_mean = base.mean(volumes[impulse_start:pullback_start]) or average_volume
    ...
    if pullback_range_mean <= 0 or impulse_range_mean <= 0 or lookback_range_mean <= 0:
        return {"event_count": 0.0}
    ...
    impulse_leg_strength = impulse_move / max(impulse_range_mean * math.sqrt(len(impulse_window)), average_range)
    if impulse_leg_strength < MIN_IMPULSE_LEG_STRENGTH:
        return {"event_count": 0.0}

    pullback_move = -direction * (pullback_window[-1].close - pullback_window[0].close)
    pullback_depth_ratio = pullback_move / max(abs(impulse_move), average_range)
    if pullback_depth_ratio < 0.03 or pullback_depth_ratio > 1.25:
        return {"event_count": 0.0}

    split_offset = max(2, len(pullback_window) // 2)
    pullback_mid_bar = pullback_window[split_offset]
    first_half_speed = -direction * (pullback_mid_bar.close - pullback_window[0].close)
    second_half_speed = -direction * (pullback_window[-1].close - pullback_mid_bar.close)
    first_half_speed /= max(pullback_range_mean * math.sqrt(split_offset), average_range)
    second_half_speed /= max(pullback_range_mean * math.sqrt(len(pullback_window) - split_offset), average_range)
    pullback_velocity_decay = first_half_speed - second_half_speed

    pullback_depth_efficiency = 1.0 - min(1.0, max(pullback_depth_ratio, 0.0))
    pullback_mid = (min(bar.low for bar in pullback_window) + max(bar.high for bar in pullback_window)) / 2.0
    pullback_extreme = min(bar.low for bar in pullback_window) if direction > 0 else max(bar.high for bar in pullback_window)
    settlement_balance = base.mean([directional_bar_settlement(bar, direction) for bar in pullback_window]) or 0.0
    ...
    wick_rejection = base.mean([rejection_wick_share(bar) for bar in pullback_window]) or 0.0
    counter_impulse_rejection = 0.55 * max(settlement_balance, 0.0) + 0.45 * wick_rejection
    reclaim_settlement_strength = direction * (signal_bar.close - pullback_mid) / max(average_range, 1e-9)
    reclaim_settlement_strength += directional_bar_settlement(signal_bar, direction) * 0.45
    followthrough_repair_pressure = direction * (signal_bar.close - pullback_window[0].close) / max(average_range, 1e-9)
    followthrough_repair_pressure += direction * (signal_bar.close - pullback_extreme) / max(
        pullback_range_mean * len(pullback_window),
        average_range,
    )
    if max(pullback_velocity_decay, counter_impulse_rejection, reclaim_settlement_strength, followthrough_repair_pressure) < 0.02:
        return {"event_count": 0.0}

    pullback_volume_cooling = 1.0 - pullback_volume_mean / max(impulse_volume_mean * 1.10, average_volume, 1e-9)
    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    vwap_pullback_alignment = 1.0 - abs(signal_bar.close - session_vwap) / max(average_range * 3.0, 1e-9)
    prior_session_impulse_context = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_session_impulse_context = direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
    if context.prior_day_shadow_balance is not None:
        prior_session_impulse_context += direction * context.prior_day_shadow_balance * 0.08
    session_pullback_position = directional_session_percentile_position(
        signal_bar.close,
        context.low_so_far,
        context.high_so_far,
        direction,
    ...
    activity = max(
        impulse_leg_strength * 0.22,
        max(pullback_velocity_decay, 0.0) * 0.20,
        pullback_depth_efficiency * 0.12,
        counter_impulse_rejection * 0.20,
        max(reclaim_settlement_strength, 0.0) * 0.24,
        max(pullback_volume_cooling, 0.0) * 0.08,
        max(followthrough_repair_pressure, 0.0) * 0.20,
        max(vwap_pullback_alignment, 0.0) * 0.06,
    )
    if activity < MIN_PULLBACK_VELOCITY_FAILURE_REVERSAL_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "impulse_leg_strength": bounded(impulse_leg_strength, -4.0, 8.0),
        "pullback_velocity_decay": bounded(pullback_velocity_decay, -4.0, 8.0),
        "pullback_depth_efficiency": bounded(pullback_depth_efficiency, -1.0, 1.0),
        "counter_impulse_rejection": bounded(counter_impulse_rejection, 0.0, 1.0),
        "reclaim_settlement_strength": bounded(reclaim_settlement_strength, -4.0, 8.0),
        "pullback_volume_cooling": bounded(pullback_volume_cooling, -4.0, 1.0),
        "followthrough_repair_pressure": bounded(followthrough_repair_pressure, -4.0, 8.0),
        "vwap_pullback_alignment": bounded(vwap_pullback_alignment, -4.0, 1.0),
        "prior_session_impulse_context": bounded(prior_session_impulse_context, -3.0, 3.0),
        "session_pullback_position": bounded(session_pullback_position, 0.0, 1.0),
        "session_maturity": bounded(session_phase_maturity, 0.0, 1.0),
        "spread_stress_inverse": bounded(spread_stress_inverse, 0.0, 1.0),
        "recent_range_ratio": bounded(pullback_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        ...
        "recent_volume_ratio": bounded(pullback_volume_mean / max(average_volume, 1e-9), 0.0, 8.0),
        ...
        "pullback_mid": rounded(pullback_mid) or 0.0,
        "pullback_extreme": rounded(pullback_extreme) or 0.0,
        "pullback_velocity_failure_reversal_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0120_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0120_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0120_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0120_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0120_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0120_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0120_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0120_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0120_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0120_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0120_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0120_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0120_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0120_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0120_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0120_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0120_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0120_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0120_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def c0120_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    impulse_reward = 0.10 if metrics["impulse_leg_strength"] > 0.55 and terminal > 0 else 0.0
    decay_reward = 0.10 if metrics["pullback_velocity_decay"] > 0.08 and terminal > 0 else 0.0
    depth_reward = 0.08 if metrics["pullback_depth_efficiency"] > 0.35 and terminal > 0 else 0.0
    rejection_reward = 0.09 if metrics["counter_impulse_rejection"] > 0.34 and terminal > 0 else 0.0
    reclaim_reward = 0.13 if metrics["reclaim_settlement_strength"] > 0.18 and terminal > 0 else 0.0
    cooling_reward = 0.06 if metrics["pullback_volume_cooling"] > -0.10 and terminal > 0 else 0.0
    followthrough_reward = 0.09 if metrics["followthrough_repair_pressure"] > 0.12 and terminal > 0 else 0.0
    vwap_reward = 0.06 if metrics["vwap_pullback_alignment"] > 0.10 and terminal > 0 else 0.0
    position_reward = 0.05 if metrics["session_pullback_position"] > 0.45 and terminal > 0 else 0.0
    weak_impulse_penalty = 0.08 if metrics["impulse_leg_strength"] < 0.42 and terminal <= 0 else 0.0
    weak_decay_penalty = 0.07 if metrics["pullback_velocity_decay"] < -0.05 and terminal <= 0 else 0.0
    weak_reclaim_penalty = 0.10 if metrics["reclaim_settlement_strength"] < 0.00 and terminal <= 0 else 0.0
    weak_rejection_penalty = 0.08 if metrics["counter_impulse_rejection"] < 0.20 and terminal <= 0 else 0.0
    overheated_volume_penalty = 0.06 if metrics["pullback_volume_cooling"] < -1.20 and terminal <= 0 else 0.0
    ...
    pullback_velocity_failure_reversal_path_quality = (0.62 * terminal + 0.28 * mfe - 0.65 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + pullback_velocity_failure_reversal_path_quality
        + impulse_reward
        + decay_reward
        + depth_reward
        + rejection_reward
        ...
        - max(0.0, 0.30 - metrics["impulse_leg_strength"]) * 0.12
    )


def fit_linear_pullback_velocity_failure_reversal_model(candidates: list[base.Candidate], fold_id: str) -> LinearPullbackVelocityFailureReversalModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearPullbackVelocityFailureReversalModel(
            fold_id,
    ...
    pullback_velocity_failure_reversal_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearPullbackVelocityFailureReversalModel(
        fold_id=fold_id,
        ...
        pullback_velocity_failure_reversal_failure_rate=pullback_velocity_failure_reversal_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearPullbackVelocityFailureReversalModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0120-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_pullback_velocity_failure_reversal_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0120_r0001_intraday_pullback_velocity_failure_reversal",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0120_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0120_r0001_pullback_velocity_failure_reversal_summary.json",
        ],
        ...
        "conditional_profiles": {
            "pullback_velocity_failure_reversal_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0120 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0120_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0120 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "pullback_velocity_failure_reversal_lookback_bars": PULLBACK_VELOCITY_FAILURE_REVERSAL_LOOKBACK_BARS,
        "pullback_velocity_failure_reversal_impulse_bars": PULLBACK_VELOCITY_FAILURE_REVERSAL_IMPULSE_BARS,
        "pullback_velocity_failure_reversal_pullback_bars": PULLBACK_VELOCITY_FAILURE_REVERSAL_PULLBACK_BARS,
        "min_impulse_leg_strength": MIN_IMPULSE_LEG_STRENGTH,
        "min_pullback_velocity_failure_reversal_activity": MIN_PULLBACK_VELOCITY_FAILURE_REVERSAL_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_pullback_velocity_failure_reversal_model_summary(model: LinearPullbackVelocityFailureReversalModel) -> dict[str, object]:
    return {
        ...
        "pullback_velocity_failure_reversal_failure_rate": rounded(model.pullback_velocity_failure_reversal_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearPullbackVelocityFailureReversalModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_pullback_velocity_failure_reversal_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "pullback_velocity_failure_reversal_profile": profiles["pullback_velocity_failure_reversal_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0120_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0120_r0001_proxy_trades.csv"
    evidence["pullback_velocity_failure_reversal_summary"] = "artifacts/c0120_r0001_pullback_velocity_failure_reversal_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0120_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0120_r0001_proxy_trades.csv",
        "artifacts/c0120_r0001_pullback_velocity_failure_reversal_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0120_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0120 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0120_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0120_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0120_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0120_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0120_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0120_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0120_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0120_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0120_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0120 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0120 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0120_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0120_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0120_r0001_intraday_pullback_velocity_failure_reversal_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0120_r0001_proxy_records_pullback_velocity_failure_reversal_reference_surface
  ...
  - next_work_is_produce_c0120_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0120_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0120-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0120_r0001_intraday_pullback_velocity_failure_reversal",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0120-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0120-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0120-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0120_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0120_r0001_intraday_pullback_velocity_failure_reversal",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0120-R0001-PULLBACK-VELOCITY-FAILURE-REVERSAL-SUMMARY",
            "artifact_role": "pullback_velocity_failure_reversal_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0120_r0001_pullback_velocity_failure_reversal_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0120_r0001_intraday_pullback_velocity_failure_reversal",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0120_r0001_mt5_logic_parity_evidence",
        }
    ]
