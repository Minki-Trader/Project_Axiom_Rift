"""C0125 R0001 proxy evidence for intraday halfback inventory transfer."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0125_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0125_r0001_halfback_inventory_transfer_summary.json"
...
FEATURE_NAMES = (
    "halfback_reclaim_pressure",
    "inventory_transfer_strength",
    "range_side_acceptance",
    "vwap_halfback_alignment",
    "opening_range_bias_transfer",
    "prior_session_inventory_context",
    "adverse_rotation_absorption",
    "micro_path_continuation_efficiency",
    "volume_confirmation_pressure",
    "time_window_transfer_quality",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_halfback_inventory_transfer_rank"
LABEL_SHAPE = "target_first_quality_after_halfback_inventory_transfer"
SELECTION_RULE = "top_fold_local_halfback_inventory_transfer_scores_per_active_day"
...
MIN_DAY_BARS = 18
HALFBACK_INVENTORY_TRANSFER_LOOKBACK_BARS = 48
HALFBACK_INVENTORY_TRANSFER_MICRO_BARS = 12
MIN_HALFBACK_RECLAIM_PRESSURE = 0.015
MIN_HALFBACK_INVENTORY_TRANSFER_ACTIVITY = 0.05
...
class LinearHalfbackInventoryTransferModel:
    fold_id: str
    ...
    halfback_inventory_transfer_failure_rate: float
...
def run_c0125_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_halfback_inventory_transfer_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_halfback_inventory_transfer_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        HALFBACK_INVENTORY_TRANSFER_LOOKBACK_BARS,
        HALFBACK_INVENTORY_TRANSFER_MICRO_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = halfback_inventory_transfer_metrics(
    ...
    features = (
        metrics["halfback_reclaim_pressure"],
        metrics["inventory_transfer_strength"],
        metrics["range_side_acceptance"],
        metrics["vwap_halfback_alignment"],
        metrics["opening_range_bias_transfer"],
        metrics["prior_session_inventory_context"],
        metrics["adverse_rotation_absorption"],
        metrics["micro_path_continuation_efficiency"],
        metrics["volume_confirmation_pressure"],
        metrics["time_window_transfer_quality"],
        metrics["session_maturity"],
        metrics["spread_stress_inverse"],
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        range_position_pressure_bucket(metrics["halfback_reclaim_pressure"]),
        breakout_pressure_bucket(metrics["inventory_transfer_strength"]),
        absorption_bucket(metrics["range_side_acceptance"]),
        vwap_alignment_bucket(metrics["vwap_halfback_alignment"]),
        prior_day_position_context_bucket(metrics["opening_range_bias_transfer"]),
        prior_tailwind_bucket(metrics["prior_session_inventory_context"]),
        adverse_wick_control_bucket(metrics["adverse_rotation_absorption"]),
        pullback_containment_bucket(metrics["micro_path_continuation_efficiency"]),
        volume_stability_bucket(1.0 - abs(metrics["volume_confirmation_pressure"])),
        session_maturity_bucket(metrics["time_window_transfer_quality"]),
...
def halfback_inventory_transfer_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    micro_start = max(index - HALFBACK_INVENTORY_TRANSFER_MICRO_BARS + 1, context.day_start_index)
    lookback_start = max(index - HALFBACK_INVENTORY_TRANSFER_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    if session_range <= average_range * 0.45:
        return {"event_count": 0.0}

    session_halfback = (context.high_so_far + context.low_so_far) / 2.0
    halfback_reclaim_pressure = direction * (signal_bar.close - session_halfback) / max(
        average_range * 3.0,
        1e-9,
    )
    if halfback_reclaim_pressure < MIN_HALFBACK_RECLAIM_PRESSURE:
        return {"event_count": 0.0}

    side_closes = [
        1.0 if direction * (bar.close - session_halfback) > 0 else 0.0
        for bar in micro_window
    ]
    range_side_acceptance = base.mean(side_closes) or 0.0
    if range_side_acceptance < 0.35:
        return {"event_count": 0.0}

    range_stretch_pressure = session_range / max(
        lookback_range_mean * math.sqrt(min(context.bars_so_far, 96)) / 3.0,
        average_range,
        1e-9,
    )
    micro_volumes = volumes[micro_start : index + 1]
    transfer_vwap = volume_weighted_close(micro_window, micro_volumes)
    inventory_transfer_strength = direction * (transfer_vwap - session_halfback) / max(
        average_range * 3.0,
        1e-9,
    )
    if inventory_transfer_strength < -0.08:
        return {"event_count": 0.0}

    adverse_rotation_absorption = base.mean(
    ...
    vwap_halfback_alignment = (
        direction * (session_vwap - session_halfback)
        + direction * (signal_bar.close - session_vwap) * 0.35
    ) / max(average_range * 3.0, 1e-9)
    ...
    opening_range_bias_transfer = direction * (signal_bar.close - opening_mid) / max(
        opening_width,
        average_range * 2.0,
        1e-9,
    )
    prior_session_inventory_context = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_session_inventory_context = direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
    if context.prior_day_shadow_balance is not None:
        prior_session_inventory_context += direction * context.prior_day_shadow_balance * 0.08
    volume_confirmation_pressure = micro_volume_mean / max(lookback_volume_mean, average_volume, 1e-9) - 1.0
    path_travel = sum(abs(micro_window[pos].close - micro_window[pos - 1].close) for pos in range(1, len(micro_window)))
    micro_path_continuation_efficiency = direction * (
        signal_bar.close - micro_window[0].close
    ) / max(path_travel, average_range, 1e-9)
    time_window_transfer_quality = bounded(1.0 - abs(session_phase_maturity - 0.56) / 0.44, 0.0, 1.0)
    ...
    activity = (
        max(halfback_reclaim_pressure, 0.0) * 0.18
        + max(inventory_transfer_strength, 0.0) * 0.16
        + range_side_acceptance * 0.13
        + max(vwap_halfback_alignment, 0.0) * 0.11
        + max(opening_range_bias_transfer, 0.0) * 0.09
        + max(prior_session_inventory_context, 0.0) * 0.06
        + adverse_rotation_absorption * 0.08
        + max(micro_path_continuation_efficiency, 0.0) * 0.12
        + max(volume_confirmation_pressure, 0.0) * 0.04
        + time_window_transfer_quality * 0.03
    )
    if activity < MIN_HALFBACK_INVENTORY_TRANSFER_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "halfback_reclaim_pressure": bounded(halfback_reclaim_pressure, -4.0, 4.0),
        "inventory_transfer_strength": bounded(inventory_transfer_strength, -4.0, 4.0),
        "range_side_acceptance": bounded(range_side_acceptance, 0.0, 1.0),
        "vwap_halfback_alignment": bounded(vwap_halfback_alignment, -4.0, 4.0),
        "opening_range_bias_transfer": bounded(opening_range_bias_transfer, -4.0, 4.0),
        "prior_session_inventory_context": bounded(prior_session_inventory_context, -3.0, 3.0),
        "adverse_rotation_absorption": bounded(adverse_rotation_absorption, 0.0, 1.0),
        "micro_path_continuation_efficiency": bounded(micro_path_continuation_efficiency, -2.0, 2.0),
        "volume_confirmation_pressure": bounded(volume_confirmation_pressure, -4.0, 8.0),
        "time_window_transfer_quality": bounded(time_window_transfer_quality, 0.0, 1.0),
        "range_stretch_pressure": bounded(range_stretch_pressure, 0.0, 8.0),
        ...
        "session_halfback": rounded(session_halfback) or 0.0,
        "session_vwap": rounded(session_vwap) or 0.0,
        "opening_mid": rounded(opening_mid) or 0.0,
        "halfback_inventory_transfer_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0125_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0125_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0125_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0125_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0125_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0125_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0125_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0125_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0125_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0125_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0125_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0125_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0125_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0125_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0125_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0125_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0125_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0125_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0125_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def c0125_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    halfback_reward = 0.12 if metrics["halfback_reclaim_pressure"] > 0.05 and terminal > 0 else 0.0
    inventory_reward = 0.13 if metrics["inventory_transfer_strength"] > 0.02 and terminal > 0 else 0.0
    acceptance_reward = 0.10 if metrics["range_side_acceptance"] > 0.58 and terminal > 0 else 0.0
    vwap_reward = 0.08 if metrics["vwap_halfback_alignment"] > -0.02 and terminal > 0 else 0.0
    opening_reward = 0.07 if metrics["opening_range_bias_transfer"] > 0.03 and terminal > 0 else 0.0
    prior_inventory_reward = 0.04 if metrics["prior_session_inventory_context"] > -0.20 and terminal > 0 else 0.0
    absorption_reward = 0.07 if metrics["adverse_rotation_absorption"] > 0.22 and terminal > 0 else 0.0
    volume_reward = 0.04 if -0.30 <= metrics["volume_confirmation_pressure"] <= 2.75 and terminal > 0 else 0.0
    path_continuation_reward = 0.12 if metrics["micro_path_continuation_efficiency"] > 0.05 and terminal > 0 else 0.0
    timing_reward = 0.04 if metrics["time_window_transfer_quality"] > 0.20 and terminal > 0 else 0.0
    weak_halfback_penalty = 0.10 if metrics["halfback_reclaim_pressure"] < 0.03 and terminal <= 0 else 0.0
    weak_inventory_penalty = 0.10 if metrics["inventory_transfer_strength"] < -0.04 and terminal <= 0 else 0.0
    weak_acceptance_penalty = 0.09 if metrics["range_side_acceptance"] < 0.45 and terminal <= 0 else 0.0
    weak_vwap_penalty = 0.08 if metrics["vwap_halfback_alignment"] < -0.18 and terminal <= 0 else 0.0
    weak_path_penalty = 0.10 if metrics["micro_path_continuation_efficiency"] < -0.05 and terminal <= 0 else 0.0
    excessive_volume_penalty = 0.05 if metrics["volume_confirmation_pressure"] > 3.75 and terminal <= 0 else 0.0
    ...
    return (
        ...
        + vwap_reward
        + opening_reward
        + prior_inventory_reward
        + absorption_reward
        + volume_reward
        + path_continuation_reward
        + timing_reward
        - weak_halfback_penalty
        - weak_inventory_penalty
        ...
        - max(0.0, MIN_HALFBACK_RECLAIM_PRESSURE - metrics["halfback_reclaim_pressure"]) * 0.18
    )


def fit_linear_halfback_inventory_transfer_model(candidates: list[base.Candidate], fold_id: str) -> LinearHalfbackInventoryTransferModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearHalfbackInventoryTransferModel(
            fold_id,
    ...
    halfback_inventory_transfer_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearHalfbackInventoryTransferModel(
        fold_id=fold_id,
        ...
        halfback_inventory_transfer_failure_rate=halfback_inventory_transfer_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearHalfbackInventoryTransferModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_halfback_inventory_transfer_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0125_r0001_intraday_halfback_inventory_transfer",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0125_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0125_r0001_halfback_inventory_transfer_summary.json",
        ],
        ...
        "conditional_profiles": {
            "halfback_inventory_transfer_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0125 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0125_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0125 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "halfback_inventory_transfer_lookback_bars": HALFBACK_INVENTORY_TRANSFER_LOOKBACK_BARS,
        "halfback_inventory_transfer_micro_bars": HALFBACK_INVENTORY_TRANSFER_MICRO_BARS,
        "min_halfback_reclaim_pressure": MIN_HALFBACK_RECLAIM_PRESSURE,
        "min_halfback_inventory_transfer_activity": MIN_HALFBACK_INVENTORY_TRANSFER_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_halfback_inventory_transfer_model_summary(model: LinearHalfbackInventoryTransferModel) -> dict[str, object]:
    return {
        ...
        "halfback_inventory_transfer_failure_rate": rounded(model.halfback_inventory_transfer_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearHalfbackInventoryTransferModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_halfback_inventory_transfer_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "halfback_inventory_transfer_profile": profiles["halfback_inventory_transfer_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0125_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0125_r0001_proxy_trades.csv"
    evidence["halfback_inventory_transfer_summary"] = "artifacts/c0125_r0001_halfback_inventory_transfer_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0125_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0125_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0125 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0125_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0125_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0125_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0125_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0125_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0125_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0125_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0125_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0125_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0125 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0125 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260708_produce_c0125_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0125_r0001_proxy_evidence
  refines:
  - dec_20260708_open_c0125_r0001_intraday_halfback_inventory_transfer_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0125_r0001_proxy_records_halfback_inventory_transfer_reference_surface
  ...
  - next_work_is_produce_c0125_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260708_produce_c0125_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0125-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0125_r0001_intraday_halfback_inventory_transfer",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0125-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0125-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0125-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0125_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0125_r0001_intraday_halfback_inventory_transfer",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0125-R0001-HALFBACK-INVENTORY-TRANSFER-SUMMARY",
            "artifact_role": "halfback_inventory_transfer_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0125_r0001_halfback_inventory_transfer_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0125_r0001_intraday_halfback_inventory_transfer",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0125_r0001_mt5_logic_parity_evidence",
        }
    ]
