"""C0093 R0001 proxy evidence for intraday participation pulse efficiency."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0093_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0093_r0001_participation_pulse_efficiency_summary.json"
...
FEATURE_NAMES = (
    ...
    "range_efficiency",
    "wick_containment",
    "prior_participation_drought",
    "session_vwap_impulse",
    "session_phase_maturity",
    "pulse_resolution_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_participation_pulse_efficiency_rank"
LABEL_SHAPE = "target_first_escape_quality_conditioned_on_participation_pulse_efficiency"
SELECTION_RULE = "top_fold_local_participation_pulse_efficiency_scores_per_active_day"
...
MIN_DAY_BARS = 30
PARTICIPATION_PULSE_EFFICIENCY_LOOKBACK_BARS = 30
PARTICIPATION_PULSE_EFFICIENCY_SHORT_LOOKBACK_BARS = 7
MIN_PARTICIPATION_PULSE_EFFICIENCY_ACTIVITY = 0.12
...
class LinearParticipationPulseEfficiencyModel:
    fold_id: str
    ...
    participation_pulse_efficiency_failure_rate: float
...
def run_c0093_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_participation_pulse_efficiency_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_participation_pulse_efficiency_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        PARTICIPATION_PULSE_EFFICIENCY_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = participation_pulse_efficiency_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        metrics["pulse_volume_ratio"],
        metrics["participation_acceleration"],
        metrics["body_commitment"],
        metrics["close_commitment"],
        metrics["range_efficiency"],
        metrics["wick_containment"],
        metrics["prior_participation_drought"],
        metrics["session_vwap_impulse"],
        metrics["session_phase_maturity"],
        metrics["pulse_resolution_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        pulse_volume_ratio_bucket(metrics["pulse_volume_ratio"]),
        participation_acceleration_bucket(metrics["participation_acceleration"]),
        pulse_body_commitment_bucket(metrics["body_commitment"]),
        pulse_close_commitment_bucket(metrics["close_commitment"]),
        pulse_range_efficiency_bucket(metrics["range_efficiency"]),
        wick_containment_bucket(metrics["wick_containment"]),
        prior_participation_drought_bucket(metrics["prior_participation_drought"]),
        session_vwap_impulse_bucket(metrics["session_vwap_impulse"]),
        session_phase_maturity_bucket(metrics["session_phase_maturity"]),
        pulse_resolution_pressure_bucket(metrics["pulse_resolution_pressure"]),
...
def participation_pulse_efficiency_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    short_start = index - PARTICIPATION_PULSE_EFFICIENCY_SHORT_LOOKBACK_BARS + 1
    prior_start = index - PARTICIPATION_PULSE_EFFICIENCY_LOOKBACK_BARS + 1
    if short_start < 0 or prior_start < 0 or context.day_start_index >= index:
        return {"event_count": 0.0}
    prior_short_start = short_start - PARTICIPATION_PULSE_EFFICIENCY_SHORT_LOOKBACK_BARS
    ...
    terminal_move = direction * (signal_bar.close - anchor)
    signal_range = max(signal_bar.high - signal_bar.low, 1e-9)
    path_total = sum(max(bar.high - bar.low, 0.0) for bar in current_window)
    prior_volumes = volumes[prior_start:short_start]
    short_volumes = volumes[short_start:index]
    prior_volume_average = base.mean(prior_volumes) or average_volume
    short_volume_average = base.mean(short_volumes) or average_volume
    current_volume = max(volumes[index], 0.0)
    pulse_volume_ratio = current_volume / max(prior_volume_average, 1e-9)
    participation_acceleration = (current_volume - short_volume_average) / max(prior_volume_average, 1e-9)
    body_commitment = direction * (signal_bar.close - signal_bar.open) / signal_range
    close_commitment = directional_bar_settlement(signal_bar, direction)
    range_efficiency_raw = terminal_move / max(path_total, average_range * 0.25)
    path_efficiency = directional_path_efficiency(
        bars,
        index,
        direction,
        PARTICIPATION_PULSE_EFFICIENCY_SHORT_LOOKBACK_BARS,
    )
    if path_efficiency is None:
        return {"event_count": 0.0}
    range_efficiency = (range_efficiency_raw + path_efficiency) * 0.5
    upper_wick, lower_wick = wick_lengths(signal_bar)
    adverse_wick = lower_wick if direction > 0 else upper_wick
    wick_containment = 1.0 - adverse_wick / signal_range
    prior_participation_drought = max((prior_volume_average - short_volume_average) / max(prior_volume_average, 1e-9), 0.0)
    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    session_vwap_impulse = direction * (signal_bar.close - session_vwap) / max(average_range, 1e-9)
    session_phase_maturity = bounded_session_progress(signal_bar.time)
    pulse_resolution_pressure_raw = max(
        max(pulse_volume_ratio - 1.0, 0.0),
        max(participation_acceleration, 0.0),
        max(body_commitment, 0.0),
        max(close_commitment - 0.45, 0.0) * 2.0,
        max(wick_containment - 0.48, 0.0),
        max(range_efficiency, 0.0),
        max(path_efficiency, 0.0),
        max(prior_participation_drought, 0.0) * 0.5,
        max(session_vwap_impulse, 0.0) * 0.35,
    )
    pulse_resolution_pressure = session_phase_maturity * pulse_resolution_pressure_raw
    activity = max(pulse_resolution_pressure_raw, pulse_resolution_pressure)
    if activity < MIN_PARTICIPATION_PULSE_EFFICIENCY_ACTIVITY:
        return {"event_count": 0.0}
    if pulse_volume_ratio < 1.04 and participation_acceleration < 0.10:
        return {"event_count": 0.0}
    if body_commitment < -0.16 and close_commitment < 0.45:
        return {"event_count": 0.0}
    if range_efficiency < -0.10 and session_vwap_impulse < -0.35:
        return {"event_count": 0.0}
    return {
        "pulse_volume_ratio": bounded(pulse_volume_ratio, 0.0, 8.0),
        "participation_acceleration": bounded(participation_acceleration, -4.0, 8.0),
        "body_commitment": bounded(body_commitment, -1.0, 1.0),
        "close_commitment": bounded(close_commitment, 0.0, 1.0),
        "range_efficiency": bounded(range_efficiency, -1.0, 1.0),
        "wick_containment": bounded(wick_containment, 0.0, 1.0),
        "prior_participation_drought": bounded(prior_participation_drought, 0.0, 1.0),
        "session_vwap_impulse": bounded(session_vwap_impulse, -3.0, 3.0),
        "session_phase_maturity": bounded(session_phase_maturity, 0.0, 1.0),
        "pulse_resolution_pressure": bounded(pulse_resolution_pressure, 0.0, 5.0),
        "event_count": 1.0,
    }
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    pulse_reward = 0.12 if metrics["pulse_volume_ratio"] > 1.18 and terminal > 0 else 0.0
    acceleration_reward = 0.12 if metrics["participation_acceleration"] > 0.16 and terminal > 0 else 0.0
    body_reward = 0.10 if metrics["body_commitment"] > 0.22 and terminal > 0 else 0.0
    close_reward = 0.12 if metrics["close_commitment"] > 0.62 and terminal > 0 else 0.0
    efficiency_reward = 0.12 if metrics["range_efficiency"] > 0.16 and terminal > 0 else 0.0
    wick_reward = 0.08 if metrics["wick_containment"] > 0.62 and terminal > 0 else 0.0
    drought_reward = 0.05 if metrics["prior_participation_drought"] > 0.10 and terminal > 0 else 0.0
    vwap_reward = 0.08 if metrics["session_vwap_impulse"] > 0.20 and terminal > 0 else 0.0
    resolution_reward = 0.06 if metrics["pulse_resolution_pressure"] > 0.16 and terminal > 0 else 0.0
    fading_participation_penalty = 0.12 if metrics["participation_acceleration"] < -0.05 and terminal <= 0 else 0.0
    weak_close_penalty = 0.10 if metrics["close_commitment"] < 0.45 and terminal <= 0 else 0.0
    adverse_body_penalty = 0.10 if metrics["body_commitment"] < -0.08 and terminal <= 0 else 0.0
    poor_efficiency_penalty = 0.10 if metrics["range_efficiency"] < -0.05 and terminal <= 0 else 0.0
    poor_wick_penalty = 0.08 if metrics["wick_containment"] < 0.45 and terminal <= 0 else 0.0
    vwap_against_penalty = 0.08 if metrics["session_vwap_impulse"] < -0.25 and terminal <= 0 else 0.0
    ...
    participation_pulse_efficiency_path_quality = (0.54 * terminal + 0.30 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + participation_pulse_efficiency_path_quality
        + pulse_reward
        + acceleration_reward
        + body_reward
        + close_reward
        ...
        + vwap_reward
        + resolution_reward
        - fading_participation_penalty
        - weak_close_penalty
        - adverse_body_penalty
        - poor_efficiency_penalty
        - poor_wick_penalty
        ...
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["pulse_volume_ratio"] - 5.0) * 0.04
    )


def fit_linear_participation_pulse_efficiency_model(candidates: list[base.Candidate], fold_id: str) -> LinearParticipationPulseEfficiencyModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearParticipationPulseEfficiencyModel(
            fold_id,
    ...
    participation_pulse_efficiency_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearParticipationPulseEfficiencyModel(
        fold_id=fold_id,
        ...
        participation_pulse_efficiency_failure_rate=participation_pulse_efficiency_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearParticipationPulseEfficiencyModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_participation_pulse_efficiency_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0093_r0001_intraday_participation_pulse_efficiency",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0093_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0093_r0001_participation_pulse_efficiency_summary.json",
        ],
        ...
        "conditional_profiles": {
            "participation_pulse_efficiency_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0093 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0093_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0093 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "participation_pulse_efficiency_lookback_bars": PARTICIPATION_PULSE_EFFICIENCY_LOOKBACK_BARS,
        "participation_pulse_efficiency_short_lookback_bars": PARTICIPATION_PULSE_EFFICIENCY_SHORT_LOOKBACK_BARS,
        "min_participation_pulse_efficiency_activity": MIN_PARTICIPATION_PULSE_EFFICIENCY_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_participation_pulse_efficiency_model_summary(model: LinearParticipationPulseEfficiencyModel) -> dict[str, object]:
    return {
        ...
        "participation_pulse_efficiency_failure_rate": rounded(model.participation_pulse_efficiency_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearParticipationPulseEfficiencyModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_participation_pulse_efficiency_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "participation_pulse_efficiency_profile": profiles["participation_pulse_efficiency_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0093_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0093_r0001_proxy_trades.csv"
    evidence["participation_pulse_efficiency_summary"] = "artifacts/c0093_r0001_participation_pulse_efficiency_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0093_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0093_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0093 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0093_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0093_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0093_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0093_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0093_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0093_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0093_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0093_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0093_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0093 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0093 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0093_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0093_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0093_r0001_intraday_participation_pulse_efficiency_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0093_r0001_proxy_records_participation_pulse_efficiency_reference_surface
  ...
  - next_work_is_produce_c0093_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0093_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0093-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0093_r0001_intraday_participation_pulse_efficiency",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0093-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0093-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0093-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0093_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0093_r0001_intraday_participation_pulse_efficiency",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0093-R0001-PARTICIPATION-PULSE-EFFICIENCY-SUMMARY",
            "artifact_role": "participation_pulse_efficiency_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0093_r0001_participation_pulse_efficiency_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0093_r0001_intraday_participation_pulse_efficiency",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0093_r0001_mt5_logic_parity_evidence",
        }
    ]
