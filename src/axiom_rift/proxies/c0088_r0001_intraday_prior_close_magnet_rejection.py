"""C0088 R0001 proxy evidence for intraday prior close magnet rejection."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0088_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0088_r0001_prior_close_magnet_rejection_summary.json"
...
FEATURE_NAMES = (
    "prior_close_anchor_distance",
    "magnet_probe_depth",
    "prior_close_reclaim_strength",
    "prior_close_gap_memory",
    "magnet_wick_rejection",
    "close_location_commitment",
    "reclaim_impulse",
    "participation_reengagement",
    "prior_day_return_alignment",
    "session_magnet_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_prior_close_magnet_rejection_rank"
LABEL_SHAPE = "target_first_escape_quality_conditioned_on_prior_close_magnet_rejection_state"
SELECTION_RULE = "top_fold_local_prior_close_magnet_rejection_scores_per_active_day"
...
MIN_DAY_BARS = 30
PRIOR_CLOSE_MAGNET_REJECTION_LOOKBACK_BARS = 24
PRIOR_CLOSE_MAGNET_REJECTION_SHORT_LOOKBACK_BARS = 8
MIN_PRIOR_CLOSE_MAGNET_REJECTION_ACTIVITY = 0.16
...
class LinearPriorCloseMagnetRejectionModel:
    fold_id: str
    ...
    prior_close_magnet_rejection_failure_rate: float
...
def run_c0088_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_prior_close_magnet_rejection_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_prior_close_magnet_rejection_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        PRIOR_CLOSE_MAGNET_REJECTION_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = prior_close_magnet_rejection_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        metrics["prior_close_anchor_distance"],
        metrics["magnet_probe_depth"],
        metrics["prior_close_reclaim_strength"],
        metrics["prior_close_gap_memory"],
        metrics["magnet_wick_rejection"],
        metrics["close_location_commitment"],
        metrics["reclaim_impulse"],
        metrics["participation_reengagement"],
        metrics["prior_day_return_alignment"],
        metrics["session_magnet_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        prior_close_anchor_distance_bucket(metrics["prior_close_anchor_distance"]),
        magnet_probe_depth_bucket(metrics["magnet_probe_depth"]),
        prior_close_reclaim_bucket(metrics["prior_close_reclaim_strength"]),
        prior_close_gap_memory_bucket(metrics["prior_close_gap_memory"]),
        magnet_wick_rejection_bucket(metrics["magnet_wick_rejection"]),
        magnet_close_commitment_bucket(metrics["close_location_commitment"]),
        reclaim_impulse_bucket(metrics["reclaim_impulse"]),
        volume_reengagement_bucket(metrics["participation_reengagement"]),
        prior_day_return_alignment_bucket(metrics["prior_day_return_alignment"]),
        session_magnet_pressure_bucket(metrics["session_magnet_pressure"]),
...
def prior_close_magnet_rejection_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    prior_day_close = context.prior_day_close
    prior_day_return = context.prior_day_return
    if (
        prior_day_range is None
        or prior_day_range <= 0
        or prior_day_close is None
        or prior_day_return is None
    ):
        return {"event_count": 0.0}
    short_start = index - PRIOR_CLOSE_MAGNET_REJECTION_SHORT_LOOKBACK_BARS + 1
    prior_start = index - PRIOR_CLOSE_MAGNET_REJECTION_LOOKBACK_BARS + 1
    if short_start < 0 or prior_start < 0:
        return {"event_count": 0.0}
    short_window = bars[short_start : index + 1]
    prior_window = bars[prior_start : index + 1]
    if len(short_window) < 4 or len(prior_window) < 16:
        return {"event_count": 0.0}
    average_volume = volume_average[index]
    if average_volume is None or average_volume <= 0:
        return {"event_count": 0.0}

    previous_window = bars[short_start:index]
    ...
    if direction > 0:
        prior_close_anchor_distance = (signal_bar.close - prior_day_close) / average_range
        magnet_probe_depth = (prior_day_close - previous_low) / average_range
        prior_close_reclaim_strength = (signal_bar.close - prior_day_close) / average_range
        crossed_anchor = previous_low <= prior_day_close and signal_bar.close >= prior_day_close - 0.05 * average_range
    else:
        prior_close_anchor_distance = (prior_day_close - signal_bar.close) / average_range
        magnet_probe_depth = (previous_high - prior_day_close) / average_range
        prior_close_reclaim_strength = (prior_day_close - signal_bar.close) / average_range
        crossed_anchor = previous_high >= prior_day_close and signal_bar.close <= prior_day_close + 0.05 * average_range
    prior_close_gap_memory = abs(context.day_open - prior_day_close) / prior_day_range
    magnet_wick_rejection = directional_wick_rejection(signal_bar, direction)
    close_location_commitment = directional_bar_settlement(signal_bar, direction)
    reclaim_impulse = direction * (signal_bar.close - bars[index - 1].close) / average_range
    ...
    body_alignment = directional_body_score(signal_bar, direction)
    participation_reengagement = (current_volume_ratio - early_volume_ratio) * max(body_alignment, 0.0)
    prior_day_return_alignment = direction * prior_day_return / prior_day_range
    magnet_pressure = max(
        max(magnet_probe_depth, 0.0) / 2.0,
        max(prior_close_reclaim_strength, 0.0),
        max(magnet_wick_rejection, 0.0),
        max(close_location_commitment - 0.50, 0.0) * 2.0,
        max(reclaim_impulse, 0.0),
        min(max(participation_reengagement, 0.0), 1.0),
        max(prior_day_return_alignment, 0.0),
    )
    session_magnet_pressure = bounded_session_progress(signal_bar.time) * magnet_pressure
    activity = max(magnet_pressure, session_magnet_pressure)
    if activity < MIN_PRIOR_CLOSE_MAGNET_REJECTION_ACTIVITY:
        return {"event_count": 0.0}
    if not crossed_anchor and magnet_probe_depth < 0.16:
        return {"event_count": 0.0}
    if magnet_probe_depth < 0.08:
        return {"event_count": 0.0}
    if prior_close_reclaim_strength < -0.08 and close_location_commitment < 0.46:
        return {"event_count": 0.0}
    if reclaim_impulse < -0.35 and magnet_wick_rejection < -0.05:
        return {"event_count": 0.0}
    return {
        "prior_close_anchor_distance": bounded(prior_close_anchor_distance, -2.0, 6.0),
        "magnet_probe_depth": bounded(magnet_probe_depth, -2.0, 6.0),
        "prior_close_reclaim_strength": bounded(prior_close_reclaim_strength, -2.0, 6.0),
        "prior_close_gap_memory": bounded(prior_close_gap_memory, 0.0, 6.0),
        "magnet_wick_rejection": bounded(magnet_wick_rejection, -1.0, 1.0),
        "close_location_commitment": bounded(close_location_commitment, 0.0, 1.0),
        "reclaim_impulse": bounded(reclaim_impulse, -4.0, 6.0),
        "participation_reengagement": bounded(participation_reengagement, -3.0, 6.0),
        "prior_day_return_alignment": bounded(prior_day_return_alignment, -4.0, 4.0),
        "session_magnet_pressure": bounded(session_magnet_pressure, 0.0, 4.0),
        "event_count": 1.0,
    }
...
def prior_close_anchor_distance_bucket(value: float) -> str:
    if value < 0.10:
        return "prior_close_inside"
    if value < 0.85:
        return "prior_close_reclaimed"
    return "prior_close_extended"


def magnet_probe_depth_bucket(value: float) -> str:
    if value < 0.15:
        return "magnet_probe_shallow"
    if value < 0.95:
        return "magnet_probe_defined"
    return "magnet_probe_deep"


def prior_close_reclaim_bucket(value: float) -> str:
    if value < -0.04:
        return "prior_close_reclaim_failed"
    if value < 0.35:
        return "prior_close_reclaim_mixed"
    return "prior_close_reclaim_clear"


def prior_close_gap_memory_bucket(value: float) -> str:
    if value < 0.10:
        return "prior_close_gap_tight"
    if value < 0.45:
        return "prior_close_gap_defined"
    return "prior_close_gap_stretched"


def magnet_wick_rejection_bucket(value: float) -> str:
    if value < -0.08:
        return "magnet_wick_against"
    if value < 0.12:
        return "magnet_wick_mixed"
    return "magnet_wick_rejecting"


def magnet_close_commitment_bucket(value: float) -> str:
    if value < 0.45:
        return "magnet_close_weak"
    if value < 0.66:
        return "magnet_close_mid"
    return "magnet_close_strong"


def reclaim_impulse_bucket(value: float) -> str:
    if value < -0.10:
        return "reclaim_impulse_against"
    if value < 0.45:
        return "reclaim_impulse_mixed"
    return "reclaim_impulse_with"


def session_magnet_pressure_bucket(value: float) -> str:
    if value < 0.08:
        return "session_magnet_light"
    if value < 0.65:
        return "session_magnet_defined"
    return "session_magnet_heavy"
...
def prior_day_return_alignment_bucket(value: float) -> str:
    if value < -0.12:
        return "prior_day_return_adverse"
    if value < 0.12:
        return "prior_day_return_mixed"
    return "prior_day_return_aligned"
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    probe_reward = 0.12 if metrics["magnet_probe_depth"] > 0.18 and terminal > 0 else 0.0
    reclaim_reward = 0.14 if metrics["prior_close_reclaim_strength"] > 0.05 and terminal > 0 else 0.0
    close_reward = 0.12 if metrics["close_location_commitment"] > 0.58 and terminal > 0 else 0.0
    impulse_reward = 0.10 if metrics["reclaim_impulse"] > 0.10 and terminal > 0 else 0.0
    wick_reward = 0.08 if metrics["magnet_wick_rejection"] > 0.05 and terminal > 0 else 0.0
    participation_reward = 0.08 if metrics["participation_reengagement"] > 0.03 and terminal > 0 else 0.0
    prior_context_reward = 0.05 if metrics["prior_day_return_alignment"] > 0.08 and terminal > 0 else 0.0
    session_reward = 0.05 if metrics["session_magnet_pressure"] > 0.12 and terminal > 0 else 0.0
    shallow_probe_penalty = 0.10 if metrics["magnet_probe_depth"] < 0.12 and terminal <= 0 else 0.0
    failed_reclaim_penalty = 0.12 if metrics["prior_close_reclaim_strength"] < -0.04 and terminal <= 0 else 0.0
    weak_close_penalty = 0.10 if metrics["close_location_commitment"] < 0.45 and terminal <= 0 else 0.0
    adverse_impulse_penalty = 0.10 if metrics["reclaim_impulse"] < -0.12 and terminal <= 0 else 0.0
    weak_wick_penalty = 0.08 if metrics["magnet_wick_rejection"] < -0.08 and terminal <= 0 else 0.0
    no_participation_penalty = 0.08 if metrics["participation_reengagement"] < -0.06 and terminal <= 0 else 0.0
    wide_gap_penalty = 0.04 if metrics["prior_close_gap_memory"] > 0.85 and terminal <= 0 else 0.0
    ...
    prior_close_magnet_rejection_path_quality = (0.52 * terminal + 0.34 * mfe - 0.68 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + prior_close_magnet_rejection_path_quality
        + probe_reward
        + reclaim_reward
        + close_reward
        + impulse_reward
        ...
        + session_reward
        - shallow_probe_penalty
        - failed_reclaim_penalty
        - weak_close_penalty
        - adverse_impulse_penalty
        - weak_wick_penalty
        ...
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["prior_close_anchor_distance"] - 3.00) * 0.04
    )


def fit_linear_prior_close_magnet_rejection_model(candidates: list[base.Candidate], fold_id: str) -> LinearPriorCloseMagnetRejectionModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearPriorCloseMagnetRejectionModel(
            fold_id,
    ...
    prior_close_magnet_rejection_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearPriorCloseMagnetRejectionModel(
        fold_id=fold_id,
        ...
        prior_close_magnet_rejection_failure_rate=prior_close_magnet_rejection_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearPriorCloseMagnetRejectionModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_prior_close_magnet_rejection_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0088_r0001_intraday_prior_close_magnet_rejection",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0088_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0088_r0001_prior_close_magnet_rejection_summary.json",
        ],
        ...
        "conditional_profiles": {
            "prior_close_magnet_rejection_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0088 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0088_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0088 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "prior_close_magnet_rejection_lookback_bars": PRIOR_CLOSE_MAGNET_REJECTION_LOOKBACK_BARS,
        "prior_close_magnet_rejection_short_lookback_bars": PRIOR_CLOSE_MAGNET_REJECTION_SHORT_LOOKBACK_BARS,
        "min_prior_close_magnet_rejection_activity": MIN_PRIOR_CLOSE_MAGNET_REJECTION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_prior_close_magnet_rejection_model_summary(model: LinearPriorCloseMagnetRejectionModel) -> dict[str, object]:
    return {
        ...
        "prior_close_magnet_rejection_failure_rate": rounded(model.prior_close_magnet_rejection_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearPriorCloseMagnetRejectionModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_prior_close_magnet_rejection_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "prior_close_magnet_rejection_profile": profiles["prior_close_magnet_rejection_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0088_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0088_r0001_proxy_trades.csv"
    evidence["prior_close_magnet_rejection_summary"] = "artifacts/c0088_r0001_prior_close_magnet_rejection_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0088_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0088_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0088 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0088_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0088_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0088_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0088_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0088_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0088_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0088_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0088_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0088_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0088 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0088 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0088_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0088_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0088_r0001_intraday_prior_close_magnet_rejection_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0088_r0001_proxy_records_prior_close_magnet_rejection_reference_surface
  ...
  - next_work_is_produce_c0088_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0088_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0088-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0088_r0001_intraday_prior_close_magnet_rejection",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0088-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0088-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0088-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0088_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0088_r0001_intraday_prior_close_magnet_rejection",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0088-R0001-prior-close-magnet-rejection-SUMMARY",
            "artifact_role": "prior_close_magnet_rejection_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0088_r0001_prior_close_magnet_rejection_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0088_r0001_intraday_prior_close_magnet_rejection",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0088_r0001_mt5_logic_parity_evidence",
        }
    ]
