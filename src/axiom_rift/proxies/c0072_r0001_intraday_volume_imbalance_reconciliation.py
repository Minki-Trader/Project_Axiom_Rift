"""C0072 R0001 proxy evidence for intraday volume imbalance reconciliation."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0072_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0072_r0001_vir_summary.json"
...
FEATURE_NAMES = (
    "short_signed_volume_pressure",
    "medium_signed_volume_pressure",
    "price_response_alignment",
    "effort_result_gap",
    "reconciliation_release",
    "opposing_effort_decay",
    "volume_acceleration",
    "close_location_confirmation",
    "prior_day_volume_context",
    "session_maturity_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_volume_imbalance_reconciliation_rank"
LABEL_SHAPE = "target_first_quality_conditioned_on_volume_imbalance_reconciliation_state"
SELECTION_RULE = "top_fold_local_volume_imbalance_reconciliation_scores_per_active_day"
...
MIN_DAY_BARS = 30
VOLUME_IMBALANCE_RECONCILIATION_LOOKBACK_BARS = 20
VOLUME_IMBALANCE_RECONCILIATION_SHORT_LOOKBACK_BARS = 6
MIN_VOLUME_IMBALANCE_RECONCILIATION_ACTIVITY = 0.10
...
class LinearVolumeImbalanceReconciliationModel:
    fold_id: str
    ...
    volume_imbalance_reconciliation_failure_rate: float
...
def run_c0072_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_volume_imbalance_reconciliation_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_volume_imbalance_reconciliation_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        VOLUME_IMBALANCE_RECONCILIATION_LOOKBACK_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    release = volume_imbalance_reconciliation_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        release["short_signed_volume_pressure"],
        release["medium_signed_volume_pressure"],
        release["price_response_alignment"],
        release["effort_result_gap"],
        release["reconciliation_release"],
        release["opposing_effort_decay"],
        release["volume_acceleration"],
        release["close_location_confirmation"],
        release["prior_day_volume_context"],
        release["session_maturity_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        short_volume_pressure_bucket(release["short_signed_volume_pressure"]),
        medium_volume_pressure_bucket(release["medium_signed_volume_pressure"]),
        price_response_bucket(release["price_response_alignment"]),
        effort_result_gap_bucket(release["effort_result_gap"]),
        reconciliation_release_bucket(release["reconciliation_release"]),
        opposing_effort_decay_bucket(release["opposing_effort_decay"]),
        volume_acceleration_bucket(release["volume_acceleration"]),
        close_location_confirmation_bucket(release["close_location_confirmation"]),
        prior_day_volume_bucket(release["prior_day_volume_context"]),
...
def volume_imbalance_reconciliation_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    window_start = index - VOLUME_IMBALANCE_RECONCILIATION_LOOKBACK_BARS + 1
    short_start = index - VOLUME_IMBALANCE_RECONCILIATION_SHORT_LOOKBACK_BARS + 1
    previous_short_start = index - VOLUME_IMBALANCE_RECONCILIATION_SHORT_LOOKBACK_BARS
    ...
    def signed_volume_pressure(window: list[base.Bar], window_volumes: list[float]) -> float:
        pressure = 0.0
        for item, volume in zip(window, window_volumes):
            bar_range = max(item.high - item.low, 1e-9)
            directional_body_share = direction * (item.close - item.open) / bar_range
            volume_ratio = volume / max(prior_volume_per_bar, 1e-9)
            pressure += directional_body_share * volume_ratio
        return pressure / max(len(window), 1)

    short_volume_window = volumes[short_start : index + 1]
    medium_volume_window = volumes[window_start : index + 1]
    previous_volume_window = volumes[previous_short_start:index]
    short_signed_volume_pressure = signed_volume_pressure(short_window, short_volume_window)
    medium_signed_volume_pressure = signed_volume_pressure(medium_window, medium_volume_window)
    previous_signed_volume_pressure = signed_volume_pressure(previous_short_window, previous_volume_window)
    signal_body_efficiency = direction * (signal_bar.close - signal_bar.open) / signal_range
    price_response_alignment = direction * (signal_bar.close - short_window[0].open) / max(
        average_range,
        1e-9,
    )
    effort_result_gap = short_signed_volume_pressure - price_response_alignment
    reconciliation_release = (
        max(0.0, previous_signed_volume_pressure)
        * max(0.0, signal_body_efficiency)
        + max(0.0, price_response_alignment)
    )
    opposing_effort_decay = max(0.0, -previous_signed_volume_pressure) - max(
        0.0,
        -short_signed_volume_pressure,
    )
    short_volume_ratio = (base.mean(short_volume_window) or average_volume) / max(prior_volume_per_bar, 1e-9)
    previous_volume_ratio = (base.mean(previous_volume_window) or average_volume) / max(
        prior_volume_per_bar,
        1e-9,
    )
    volume_acceleration = short_volume_ratio - previous_volume_ratio
    close_location_confirmation = directional_bar_settlement(signal_bar, direction)
    prior_day_volume_context = prior_volume_per_bar / max(average_volume, 1e-9)
    reconciliation_pressure = max(
        abs(short_signed_volume_pressure),
        abs(medium_signed_volume_pressure),
        abs(price_response_alignment),
        abs(effort_result_gap),
        reconciliation_release,
        abs(opposing_effort_decay),
        abs(volume_acceleration),
        abs(close_location_confirmation - 0.5),
        abs(prior_day_volume_context - 1.0),
    )
    session_maturity_pressure = bounded_session_progress(signal_bar.time) * max(
        reconciliation_pressure,
        max(short_signed_volume_pressure, 0.0),
    ...
    if activity < MIN_VOLUME_IMBALANCE_RECONCILIATION_ACTIVITY:
        return {"event_count": 0.0}
    if (
        abs(short_signed_volume_pressure) < 0.08
        and abs(effort_result_gap) < 0.06
        and reconciliation_release < 0.05
        and abs(volume_acceleration) < 0.08
        and close_location_confirmation < 0.52
    ):
        return {"event_count": 0.0}
    return {
        "short_signed_volume_pressure": bounded(short_signed_volume_pressure, -4.0, 4.0),
        "medium_signed_volume_pressure": bounded(medium_signed_volume_pressure, -4.0, 4.0),
        "price_response_alignment": bounded(price_response_alignment, -4.0, 4.0),
        "effort_result_gap": bounded(effort_result_gap, -4.0, 4.0),
        "reconciliation_release": bounded(reconciliation_release, 0.0, 4.0),
        "opposing_effort_decay": bounded(opposing_effort_decay, -4.0, 4.0),
        "volume_acceleration": bounded(volume_acceleration, -4.0, 4.0),
        "close_location_confirmation": bounded(close_location_confirmation, 0.0, 1.0),
        "prior_day_volume_context": bounded(prior_day_volume_context, 0.0, 4.0),
...
def short_volume_pressure_bucket(value: float) -> str:
    if value < -0.18:
        return "short_volume_pressure_against"
    if value < 0.18:
        return "short_volume_pressure_mixed"
    return "short_volume_pressure_with"


def medium_volume_pressure_bucket(value: float) -> str:
    if value < -0.16:
        return "medium_volume_pressure_against"
    if value < 0.16:
        return "medium_volume_pressure_mixed"
    return "medium_volume_pressure_with"


def price_response_bucket(value: float) -> str:
    if value < -0.20:
        return "price_response_adverse"
    if value < 0.20:
        return "price_response_flat"
    return "price_response_aligned"


def effort_result_gap_bucket(value: float) -> str:
    if value < -0.20:
        return "effort_result_price_ahead"
    if value < 0.20:
        return "effort_result_balanced"
    return "effort_result_volume_ahead"


def reconciliation_release_bucket(value: float) -> str:
    if value < 0.12:
        return "reconciliation_release_low"
    if value < 0.45:
        return "reconciliation_release_mid"
    return "reconciliation_release_high"


def opposing_effort_decay_bucket(value: float) -> str:
    if value < -0.12:
        return "opposing_effort_rebuilding"
    if value < 0.12:
        return "opposing_effort_neutral"
    return "opposing_effort_decaying"


def volume_acceleration_bucket(value: float) -> str:
    if value < -0.16:
        return "volume_acceleration_falling"
    if value < 0.16:
        return "volume_acceleration_flat"
    return "volume_acceleration_rising"


def close_location_confirmation_bucket(value: float) -> str:
    if value < 0.42:
        return "close_location_unconfirmed"
    if value < 0.64:
        return "close_location_mixed"
    return "close_location_confirmed"


def prior_day_volume_bucket(value: float) -> str:
    if value < 0.75:
        return "prior_day_volume_quiet"
    if value < 1.50:
        return "prior_day_volume_normal"
    return "prior_day_volume_hot"
...
def candidate_label(
    ...
    metrics = volume_imbalance_reconciliation_metrics(
        bars,
        volumes,
    ...
    if metrics["event_count"] <= 0:
        return None
    short_pressure_reward = (
        0.12 if metrics["short_signed_volume_pressure"] > 0.18 and terminal > 0 else 0.0
    )
    medium_pressure_reward = (
        0.10 if metrics["medium_signed_volume_pressure"] > 0.12 and terminal > 0 else 0.0
    )
    price_response_reward = (
        0.14 if metrics["price_response_alignment"] > 0.10 and terminal > 0 else 0.0
    )
    effort_gap_reward = 0.10 if metrics["effort_result_gap"] > 0.12 and terminal > 0 else 0.0
    reconciliation_reward = (
        0.18 if metrics["reconciliation_release"] > 0.12 and terminal > 0 else 0.0
    )
    opposing_decay_reward = (
        0.10 if metrics["opposing_effort_decay"] > 0.08 and terminal > 0 else 0.0
    )
    volume_acceleration_reward = (
        0.08 if metrics["volume_acceleration"] > 0.08 and terminal > 0 else 0.0
    )
    close_location_reward = (
        0.10 if metrics["close_location_confirmation"] > 0.62 and terminal > 0 else 0.0
    )
    prior_volume_reward = (
        0.06 if 0.80 <= metrics["prior_day_volume_context"] <= 2.40 and terminal > 0 else 0.0
    )
    adverse_pressure_penalty = (
        0.18 if metrics["short_signed_volume_pressure"] < -0.12 and terminal <= 0 else 0.0
    )
    adverse_price_response_penalty = (
        0.16 if metrics["price_response_alignment"] < -0.08 and terminal <= 0 else 0.0
    )
    adverse_effort_gap_penalty = (
        0.14 if metrics["effort_result_gap"] < -0.15 and terminal <= 0 else 0.0
    )
    weak_reconciliation_penalty = (
        0.18 if metrics["reconciliation_release"] < 0.04 and terminal <= 0 else 0.0
    )
    opposing_rebuild_penalty = (
        0.14 if metrics["opposing_effort_decay"] < -0.08 and terminal <= 0 else 0.0
    )
    volume_fade_penalty = (
        0.12 if metrics["volume_acceleration"] < -0.12 and terminal <= 0 else 0.0
    )
    weak_close_location_penalty = (
        0.14 if metrics["close_location_confirmation"] < 0.38 and terminal <= 0 else 0.0
    )
    ...
    volume_imbalance_reconciliation_path_quality = (0.44 * terminal + 0.36 * mfe - 0.72 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + volume_imbalance_reconciliation_path_quality
        + short_pressure_reward
        + medium_pressure_reward
        + price_response_reward
        + effort_gap_reward
...
def fit_linear_volume_imbalance_reconciliation_model(candidates: list[base.Candidate], fold_id: str) -> LinearVolumeImbalanceReconciliationModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearVolumeImbalanceReconciliationModel(
            fold_id,
    ...
    volume_imbalance_reconciliation_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearVolumeImbalanceReconciliationModel(
        fold_id=fold_id,
        ...
        volume_imbalance_reconciliation_failure_rate=volume_imbalance_reconciliation_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearVolumeImbalanceReconciliationModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_volume_imbalance_reconciliation_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0072_r0001_intraday_volume_imbalance_reconciliation",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0072_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0072_r0001_vir_summary.json",
        ],
        ...
        "conditional_profiles": {
            "volume_imbalance_reconciliation_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0072 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0072_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0072 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "volume_imbalance_reconciliation_lookback_bars": VOLUME_IMBALANCE_RECONCILIATION_LOOKBACK_BARS,
        "volume_imbalance_reconciliation_short_lookback_bars": VOLUME_IMBALANCE_RECONCILIATION_SHORT_LOOKBACK_BARS,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_volume_imbalance_reconciliation_model_summary(model: LinearVolumeImbalanceReconciliationModel) -> dict[str, object]:
    return {
        ...
        "volume_imbalance_reconciliation_failure_rate": rounded(model.volume_imbalance_reconciliation_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearVolumeImbalanceReconciliationModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_volume_imbalance_reconciliation_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "volume_imbalance_reconciliation_profile": profiles["volume_imbalance_reconciliation_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0072_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0072_r0001_proxy_trades.csv"
    evidence["volume_imbalance_reconciliation_summary"] = "artifacts/c0072_r0001_vir_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0072_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0072_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0072 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0072_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0072_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0072_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0072_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0072_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0072_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0072_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0072_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0072_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0072 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0072 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260706_produce_c0072_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0072_r0001_proxy_evidence
  refines:
  - dec_20260706_open_c0072_r0001_intraday_volume_imbalance_reconciliation_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0072_r0001_proxy_records_volume_imbalance_reconciliation_reference_surface
  ...
  - next_work_is_produce_c0072_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260706_produce_c0072_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0072-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0072_r0001_intraday_volume_imbalance_reconciliation",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0072-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0072-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0072-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0072_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0072_r0001_intraday_volume_imbalance_reconciliation",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0072-R0001-volume-imbalance-reconciliation-SUMMARY",
            "artifact_role": "volume_imbalance_reconciliation_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0072_r0001_vir_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0072_r0001_intraday_volume_imbalance_reconciliation",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0072_r0001_mt5_logic_parity_evidence",
        }
    ]
