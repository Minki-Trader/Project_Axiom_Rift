"""C0133 R0001 proxy evidence for intraday failed extreme continuation absorption."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0133_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0133_r0001_failed_extreme_continuation_absorption_summary.json"
...
FEATURE_NAMES = (
    "fresh_extreme_pressure",
    "failed_continuation_absorption",
    "reclaim_distance",
    "adverse_wick_absorption",
    "extension_stretch",
    "post_extreme_settlement",
    "range_reentry_depth",
    "volume_exhaustion_ratio",
    "vwap_reentry_room",
    "prior_day_extreme_context",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_failed_extreme_continuation_absorption_rank"
LABEL_SHAPE = "target_first_quality_after_failed_extreme_continuation_absorption"
SELECTION_RULE = "top_fold_local_failed_extreme_continuation_absorption_scores_per_active_day"
...
MIN_DAY_BARS = 18
FAILED_EXTREME_CONTINUATION_ABSORPTION_LOOKBACK_BARS = 42
FAILED_EXTREME_CONTINUATION_ABSORPTION_MICRO_BARS = 12
MIN_FAILED_EXTREME_PRESSURE = 0.04
MIN_ABSORPTION_PRESSURE = 0.05
MIN_FAILED_EXTREME_CONTINUATION_ABSORPTION_ACTIVITY = 0.045
...
class LinearFailedExtremeContinuationAbsorptionModel:
    fold_id: str
    ...
    failed_extreme_continuation_absorption_failure_rate: float
...
def run_c0133_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_failed_extreme_continuation_absorption_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_failed_extreme_continuation_absorption_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        FAILED_EXTREME_CONTINUATION_ABSORPTION_LOOKBACK_BARS,
        FAILED_EXTREME_CONTINUATION_ABSORPTION_MICRO_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = failed_extreme_continuation_absorption_metrics(
    ...
    features = (
        metrics["fresh_extreme_pressure"],
        metrics["failed_continuation_absorption"],
        metrics["reclaim_distance"],
        metrics["adverse_wick_absorption"],
        metrics["extension_stretch"],
        metrics["post_extreme_settlement"],
        metrics["range_reentry_depth"],
        metrics["volume_exhaustion_ratio"],
        metrics["vwap_reentry_room"],
        metrics["prior_day_extreme_context"],
        metrics["session_maturity"],
        metrics["spread_stress_inverse"],
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        c0133_anchor_probe_bucket(metrics["fresh_extreme_pressure"]),
        c0133_failed_followthrough_bucket(metrics["failed_continuation_absorption"]),
        c0133_reclaim_bucket(metrics["reclaim_distance"]),
        c0133_tail_rejection_bucket(metrics["adverse_wick_absorption"]),
        c0133_extension_stress_bucket(metrics["extension_stretch"]),
        c0133_recoil_bucket(metrics["post_extreme_settlement"]),
        c0133_reclaim_bucket(metrics["range_reentry_depth"]),
        c0133_volume_exhaustion_bucket(metrics["volume_exhaustion_ratio"]),
        c0133_reclaim_bucket(metrics["vwap_reentry_room"]),
        c0133_prior_anchor_bucket(metrics["prior_day_extreme_context"]),
...
def failed_extreme_continuation_absorption_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    micro_start = max(index - FAILED_EXTREME_CONTINUATION_ABSORPTION_MICRO_BARS + 1, context.day_start_index)
    lookback_start = max(index - FAILED_EXTREME_CONTINUATION_ABSORPTION_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    if len(prior_lookback) < 20 or len(prior_micro) < 5:
        return {"event_count": 0.0}
    prior_high = max(bar.high for bar in prior_lookback)
    prior_low = min(bar.low for bar in prior_lookback)
    ...
    if direction > 0:
        fresh_extreme_pressure = max(prior_low - signal_bar.low, 0.0) / max(average_range, 1e-9)
        failed_continuation_absorption = (signal_bar.close - prior_low) / max(average_range, 1e-9)
        reclaim_distance = (signal_bar.close - prior_micro_low) / prior_micro_range
        range_reentry_depth = (signal_bar.close - prior_micro_low) / prior_micro_range
        extension_stretch = (prior_low - signal_bar.low + max(prior_low - signal_bar.close, 0.0)) / max(average_range, 1e-9)
    else:
        fresh_extreme_pressure = max(signal_bar.high - prior_high, 0.0) / max(average_range, 1e-9)
        failed_continuation_absorption = (prior_high - signal_bar.close) / max(average_range, 1e-9)
        reclaim_distance = (prior_micro_high - signal_bar.close) / prior_micro_range
        range_reentry_depth = (prior_micro_high - signal_bar.close) / prior_micro_range
        extension_stretch = (signal_bar.high - prior_high + max(signal_bar.close - prior_high, 0.0)) / max(average_range, 1e-9)

    if (
        fresh_extreme_pressure < MIN_FAILED_EXTREME_PRESSURE
        and failed_continuation_absorption < MIN_ABSORPTION_PRESSURE
    ):
        return {"event_count": 0.0}

    adverse_wick_absorption_score = adverse_wick_absorption(signal_bar, direction)
    post_extreme_settlement = directional_bar_settlement(signal_bar, direction)
    session_vwap = session_vwap_until(bars, volumes, context.day_start_index, index)
    vwap_reentry_room = direction * (session_vwap - signal_bar.close) / max(average_range * 2.5, 1e-9)
    volume_exhaustion_ratio = 1.0 - volumes[index] / max(lookback_volume_mean, average_volume, 1e-9)
    prior_day_extreme_context = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_day_extreme_context = -direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
    if context.prior_day_shadow_balance is not None:
        prior_day_extreme_context += -direction * context.prior_day_shadow_balance * 0.05
    ...
    activity = (
        max(fresh_extreme_pressure, 0.0) * 0.17
        + max(failed_continuation_absorption, 0.0) * 0.18
        + max(reclaim_distance, 0.0) * 0.13
        + adverse_wick_absorption_score * 0.10
        + max(extension_stretch, 0.0) * 0.10
        + max(post_extreme_settlement, 0.0) * 0.10
        + max(range_reentry_depth, 0.0) * 0.09
        + max(volume_exhaustion_ratio, -0.15) * 0.04
        + max(vwap_reentry_room, -0.20) * 0.05
        + max(prior_day_extreme_context, -0.20) * 0.04
    )
    if activity < MIN_FAILED_EXTREME_CONTINUATION_ABSORPTION_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "fresh_extreme_pressure": bounded(fresh_extreme_pressure, 0.0, 5.0),
        "failed_continuation_absorption": bounded(failed_continuation_absorption, -3.0, 4.0),
        "reclaim_distance": bounded(reclaim_distance, -2.0, 3.0),
        "adverse_wick_absorption": bounded(adverse_wick_absorption_score, 0.0, 1.0),
        "extension_stretch": bounded(extension_stretch, 0.0, 6.0),
        "post_extreme_settlement": bounded(post_extreme_settlement, -1.0, 1.0),
        "range_reentry_depth": bounded(range_reentry_depth, -2.0, 3.0),
        "volume_exhaustion_ratio": bounded(volume_exhaustion_ratio, -3.0, 1.0),
        "vwap_reentry_room": bounded(vwap_reentry_room, -4.0, 4.0),
        "prior_day_extreme_context": bounded(prior_day_extreme_context, -3.0, 3.0),
        ...
        "failed_extreme_continuation_absorption_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0133_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def c0133_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def c0133_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def c0133_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def c0133_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def c0133_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def c0133_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0133_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0133_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0133_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0133_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0133_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0133_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0133_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0133_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0133_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0133_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0133_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0133_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def c0133_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    fresh_extreme_reward = 0.11 if metrics["fresh_extreme_pressure"] > 0.10 and terminal > 0 else 0.0
    absorption_reward = 0.14 if metrics["failed_continuation_absorption"] > 0.14 and terminal > 0 else 0.0
    reclaim_reward = 0.12 if metrics["reclaim_distance"] > 0.12 and terminal > 0 else 0.0
    wick_absorption_reward = 0.09 if metrics["adverse_wick_absorption"] > 0.28 and terminal > 0 else 0.0
    stretch_reward = 0.07 if 0.04 <= metrics["extension_stretch"] <= 1.80 and terminal > 0 else 0.0
    settlement_reward = 0.09 if metrics["post_extreme_settlement"] > 0.16 and terminal > 0 else 0.0
    reentry_reward = 0.10 if metrics["range_reentry_depth"] > 0.10 and terminal > 0 else 0.0
    volume_reward = 0.05 if -0.25 <= metrics["volume_exhaustion_ratio"] <= 0.70 and terminal > 0 else 0.0
    vwap_room_reward = 0.05 if metrics["vwap_reentry_room"] > -0.25 and terminal > 0 else 0.0
    prior_day_reward = 0.04 if metrics["prior_day_extreme_context"] > -0.35 and terminal > 0 else 0.0
    weak_extreme_penalty = 0.06 if metrics["fresh_extreme_pressure"] < 0.05 and terminal <= 0 else 0.0
    weak_absorption_penalty = 0.10 if metrics["failed_continuation_absorption"] < 0.02 and terminal <= 0 else 0.0
    weak_reclaim_penalty = 0.09 if metrics["reclaim_distance"] < -0.02 and terminal <= 0 else 0.0
    weak_wick_penalty = 0.07 if metrics["adverse_wick_absorption"] < 0.18 and terminal <= 0 else 0.0
    weak_settlement_penalty = 0.08 if metrics["post_extreme_settlement"] < -0.08 and terminal <= 0 else 0.0
    overheated_volume_penalty = 0.05 if metrics["volume_exhaustion_ratio"] < -1.50 and terminal <= 0 else 0.0
    ...
    return (
        ...
        - max(0.0, -metrics["volume_exhaustion_ratio"] - 1.50) * 0.015
        - max(0.0, MIN_FAILED_EXTREME_PRESSURE - metrics["fresh_extreme_pressure"]) * 0.10
        - max(0.0, MIN_ABSORPTION_PRESSURE - metrics["failed_continuation_absorption"]) * 0.08
    )


def fit_linear_failed_extreme_continuation_absorption_model(candidates: list[base.Candidate], fold_id: str) -> LinearFailedExtremeContinuationAbsorptionModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearFailedExtremeContinuationAbsorptionModel(
            fold_id,
    ...
    failed_extreme_continuation_absorption_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearFailedExtremeContinuationAbsorptionModel(
        fold_id=fold_id,
        ...
        failed_extreme_continuation_absorption_failure_rate=failed_extreme_continuation_absorption_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearFailedExtremeContinuationAbsorptionModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_failed_extreme_continuation_absorption_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0133_r0001_intraday_failed_extreme_continuation_absorption",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0133_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0133_r0001_failed_extreme_continuation_absorption_summary.json",
        ],
        ...
        "conditional_profiles": {
            "failed_extreme_continuation_absorption_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0133 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0133_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0133 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "failed_extreme_continuation_absorption_lookback_bars": FAILED_EXTREME_CONTINUATION_ABSORPTION_LOOKBACK_BARS,
        "failed_extreme_continuation_absorption_micro_bars": FAILED_EXTREME_CONTINUATION_ABSORPTION_MICRO_BARS,
        "min_failed_extreme_pressure": MIN_FAILED_EXTREME_PRESSURE,
        "min_absorption_pressure": MIN_ABSORPTION_PRESSURE,
        "min_failed_extreme_continuation_absorption_activity": MIN_FAILED_EXTREME_CONTINUATION_ABSORPTION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_failed_extreme_continuation_absorption_model_summary(model: LinearFailedExtremeContinuationAbsorptionModel) -> dict[str, object]:
    return {
        ...
        "failed_extreme_continuation_absorption_failure_rate": rounded(model.failed_extreme_continuation_absorption_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearFailedExtremeContinuationAbsorptionModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_failed_extreme_continuation_absorption_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "failed_extreme_continuation_absorption_profile": profiles["failed_extreme_continuation_absorption_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0133_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0133_r0001_proxy_trades.csv"
    evidence["failed_extreme_continuation_absorption_summary"] = "artifacts/c0133_r0001_failed_extreme_continuation_absorption_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0133_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0133_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0133 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0133_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0133_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0133_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0133_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0133_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0133_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0133_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0133_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0133_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0133 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0133 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    block = f"""
- decision_id: dec_20260708_produce_c0133_r0001_proxy_evidence
  created_local_date: '{local_date}'
  status: active
  decision: produce_c0133_r0001_proxy_evidence
  refines:
  - dec_20260708_open_c0133_r0001_intraday_failed_extreme_continuation_absorption_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0133_r0001_proxy_records_failed_extreme_continuation_absorption_reference_surface
  ...
  - next_work_is_produce_c0133_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260708_produce_c0133_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0133-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0133_r0001_intraday_failed_extreme_continuation_absorption",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0133-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0133-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0133-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0133_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0133_r0001_intraday_failed_extreme_continuation_absorption",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0133-R0001-FAILED-EXTREME-CONTINUATION-ABSORPTION-SUMMARY",
            "artifact_role": "failed_extreme_continuation_absorption_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0133_r0001_failed_extreme_continuation_absorption_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0133_r0001_intraday_failed_extreme_continuation_absorption",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0133_r0001_mt5_logic_parity_evidence",
        }
    ]
