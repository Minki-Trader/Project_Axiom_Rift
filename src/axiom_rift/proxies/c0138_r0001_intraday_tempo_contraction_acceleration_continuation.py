"""C0138 R0001 proxy evidence for intraday tempo contraction acceleration continuation."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0138_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0138_r0001_tempo_contraction_acceleration_continuation_summary.json"
...
FEATURE_NAMES = (
    "tempo_contraction",
    "directional_close_drift",
    "acceleration_impulse",
    "pullback_absorption",
    "close_sequence_persistence",
    "range_efficiency",
    "volume_phase_alignment",
    "session_room_in_direction",
    "prior_day_momentum_context",
    "opening_anchor_alignment",
    "session_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_tempo_contraction_acceleration_continuation_rank"
LABEL_SHAPE = "target_first_quality_after_tempo_contraction_acceleration_continuation"
SELECTION_RULE = "top_fold_local_tempo_contraction_acceleration_continuation_scores_per_active_day"
...
MIN_DAY_BARS = 18
TEMPO_CONTRACTION_ACCELERATION_CONTINUATION_LOOKBACK_BARS = 60
TEMPO_CONTRACTION_ACCELERATION_CONTINUATION_MICRO_BARS = 14
MIN_TEMPO_CONTRACTION = -0.10
MIN_DIRECTIONAL_CLOSE_DRIFT = -0.04
MIN_CLOSE_SEQUENCE_PERSISTENCE = 0.20
MIN_TEMPO_CONTRACTION_ACCELERATION_CONTINUATION_ACTIVITY = 0.03
...
class LinearTempoContractionAccelerationContinuationModel:
    fold_id: str
    ...
    tempo_contraction_acceleration_continuation_failure_rate: float
...
def run_c0138_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_tempo_contraction_acceleration_continuation_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_tempo_contraction_acceleration_continuation_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        TEMPO_CONTRACTION_ACCELERATION_CONTINUATION_LOOKBACK_BARS,
        TEMPO_CONTRACTION_ACCELERATION_CONTINUATION_MICRO_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = tempo_contraction_acceleration_continuation_metrics(
    ...
    features = (
        metrics["tempo_contraction"],
        metrics["directional_close_drift"],
        metrics["acceleration_impulse"],
        metrics["pullback_absorption"],
        metrics["close_sequence_persistence"],
        metrics["range_efficiency"],
        metrics["volume_phase_alignment"],
        metrics["session_room_in_direction"],
        metrics["prior_day_momentum_context"],
        metrics["opening_anchor_alignment"],
        metrics["session_maturity"],
        metrics["spread_stress_inverse"],
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        compression_bucket(metrics["tempo_contraction"]),
        trend_pressure_bucket(metrics["directional_close_drift"]),
        impulse_response_bucket(metrics["acceleration_impulse"]),
        absorption_bucket(metrics["pullback_absorption"]),
        persistence_bucket(metrics["close_sequence_persistence"]),
        path_efficiency_bucket(metrics["range_efficiency"]),
        volume_alignment_bucket(metrics["volume_phase_alignment"]),
        session_pressure_bucket(metrics["session_room_in_direction"]),
        prior_direction_context_bucket(metrics["prior_day_momentum_context"]),
        opening_anchor_bucket(metrics["opening_anchor_alignment"]),
...
def tempo_contraction_acceleration_continuation_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    micro_start = max(index - TEMPO_CONTRACTION_ACCELERATION_CONTINUATION_MICRO_BARS + 1, context.day_start_index)
    lookback_start = max(index - TEMPO_CONTRACTION_ACCELERATION_CONTINUATION_LOOKBACK_BARS + 1, context.day_start_index)
    ...
    if len(prior_micro) < 5:
        return {"event_count": 0.0}
    reference_window = lookback_window[: -len(micro_window)] if len(lookback_window) > len(micro_window) else lookback_window[:-1]
    if len(reference_window) < 12:
        return {"event_count": 0.0}
    ...
    if direction > 0:
        pullback_absorption = (signal_bar.close - micro_low) / micro_span
        session_room_in_direction = (context.high_so_far - signal_bar.close) / max(average_range, 1e-9)
    else:
        pullback_absorption = (micro_high - signal_bar.close) / micro_span
        session_room_in_direction = (signal_bar.close - context.low_so_far) / max(average_range, 1e-9)

    tempo_contraction = (lookback_range_mean - micro_range_mean) / max(average_range, 1e-9)
    micro_net = direction * (signal_bar.close - prior_micro[0].close)
    directional_close_drift = micro_net / max(average_range, 1e-9)
    current_delta = signal_bar.close - previous_bar.close
    previous_delta = previous_bar.close - previous_previous_bar.close
    acceleration_impulse = direction * (current_delta - previous_delta) / max(average_range, 1e-9)
    closes = [bar.close for bar in micro_window]
    close_deltas = [direction * (closes[pos] - closes[pos - 1]) for pos in range(1, len(closes))]
    recent_deltas = close_deltas[-5:]
    close_sequence_persistence = sum(1 for delta in recent_deltas if delta > 0.0) / max(len(recent_deltas), 1)
    path_distance = sum(abs(closes[pos] - closes[pos - 1]) for pos in range(1, len(closes)))
    range_efficiency = micro_net / max(path_distance, 1e-9)
    if tempo_contraction < MIN_TEMPO_CONTRACTION and directional_close_drift < 0.02:
        return {"event_count": 0.0}
    if directional_close_drift < MIN_DIRECTIONAL_CLOSE_DRIFT and acceleration_impulse < 0.02:
        return {"event_count": 0.0}
    if close_sequence_persistence < MIN_CLOSE_SEQUENCE_PERSISTENCE and pullback_absorption < 0.42:
        return {"event_count": 0.0}
    ...
    volume_phase_alignment = (current_volume_ratio - micro_volume_ratio * 0.45) + max(acceleration_impulse, -0.25)
    prior_day_momentum_context = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_day_momentum_context = direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
    if context.prior_day_shadow_balance is not None:
        prior_day_momentum_context -= direction * context.prior_day_shadow_balance * 0.03
    opening_anchor_alignment = direction * (signal_bar.close - context.day_open) / max(average_range, 1e-9)
    ...
    activity = (
        max(tempo_contraction, -0.20) * 0.12
        + max(directional_close_drift, -0.20) * 0.18
        + max(acceleration_impulse, -0.18) * 0.14
        + max(pullback_absorption - 0.38, -0.24) * 0.12
        + max(close_sequence_persistence - 0.20, -0.20) * 0.12
        + max(range_efficiency, -0.35) * 0.08
        + max(min(volume_phase_alignment, 1.50), -0.75) * 0.08
        + max(session_room_in_direction, -0.20) * 0.06
        + max(prior_day_momentum_context, -0.30) * 0.04
        + max(opening_anchor_alignment, -0.30) * 0.06
    )
    if activity < MIN_TEMPO_CONTRACTION_ACCELERATION_CONTINUATION_ACTIVITY:
        return {"event_count": 0.0}
    return {
        "tempo_contraction": bounded(tempo_contraction, -3.0, 6.0),
        "directional_close_drift": bounded(directional_close_drift, -4.0, 4.0),
        "acceleration_impulse": bounded(acceleration_impulse, -4.0, 4.0),
        "pullback_absorption": bounded(pullback_absorption, 0.0, 1.0),
        "close_sequence_persistence": bounded(close_sequence_persistence, 0.0, 1.0),
        "range_efficiency": bounded(range_efficiency, -1.0, 1.0),
        "volume_phase_alignment": bounded(volume_phase_alignment, -6.0, 8.0),
        "session_room_in_direction": bounded(session_room_in_direction, -3.0, 8.0),
        "prior_day_momentum_context": bounded(prior_day_momentum_context, -3.0, 3.0),
        "opening_anchor_alignment": bounded(opening_anchor_alignment, -4.0, 4.0),
        ...
        "reference_level": rounded(context.day_open) or 0.0,
        "micro_net": bounded(micro_net / max(average_range, 1e-9), -4.0, 4.0),
        "tempo_contraction_acceleration_continuation_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def C0138_extreme_retest_bucket(value: float) -> str:
    if value < 0.45:
...
def C0138_recovery_bucket(value: float) -> str:
    if value < 0.22:
...
def C0138_opposite_room_bucket(value: float) -> str:
    if value < 0.25:
...
def C0138_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
...
def C0138_opening_reclaim_bucket(value: float) -> str:
    if value < -0.15:
...
def C0138_prior_contrast_bucket(value: float) -> str:
    if value < -0.18:
...
def C0138_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def C0138_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def C0138_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def C0138_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def C0138_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def C0138_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def C0138_anchor_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def C0138_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def C0138_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def C0138_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def C0138_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def C0138_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def C0138_prior_anchor_bucket(value: float) -> str:
    if value < -0.12:
...
def C0138_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    contraction_reward = 0.08 if metrics["tempo_contraction"] > 0.08 and terminal > 0 else 0.0
    drift_reward = 0.13 if metrics["directional_close_drift"] > 0.10 and terminal > 0 else 0.0
    acceleration_reward = 0.12 if metrics["acceleration_impulse"] > 0.04 and terminal > 0 else 0.0
    absorption_reward = 0.10 if metrics["pullback_absorption"] > 0.56 and terminal > 0 else 0.0
    persistence_reward = 0.10 if metrics["close_sequence_persistence"] >= 0.55 and terminal > 0 else 0.0
    efficiency_reward = 0.08 if metrics["range_efficiency"] > 0.28 and terminal > 0 else 0.0
    volume_reward = 0.06 if metrics["volume_phase_alignment"] > -0.10 and terminal > 0 else 0.0
    room_reward = 0.05 if metrics["session_room_in_direction"] > 0.20 and terminal > 0 else 0.0
    prior_day_reward = 0.04 if metrics["prior_day_momentum_context"] > -0.30 and terminal > 0 else 0.0
    opening_reward = 0.04 if metrics["opening_anchor_alignment"] > -0.20 and terminal > 0 else 0.0
    expansion_penalty = 0.06 if metrics["tempo_contraction"] < -0.16 and terminal <= 0 else 0.0
    weak_drift_penalty = 0.10 if metrics["directional_close_drift"] < -0.05 and terminal <= 0 else 0.0
    weak_acceleration_penalty = 0.08 if metrics["acceleration_impulse"] < -0.05 and terminal <= 0 else 0.0
    weak_absorption_penalty = 0.08 if metrics["pullback_absorption"] < 0.42 and terminal <= 0 else 0.0
    weak_persistence_penalty = 0.08 if metrics["close_sequence_persistence"] < 0.35 and terminal <= 0 else 0.0
    weak_efficiency_penalty = 0.07 if metrics["range_efficiency"] < 0.05 and terminal <= 0 else 0.0
    no_room_penalty = 0.05 if metrics["session_room_in_direction"] < 0.05 and terminal <= 0 else 0.0
    volume_drought_penalty = 0.05 if metrics["volume_phase_alignment"] < -0.75 and terminal <= 0 else 0.0
    ...
    return (
        ...
        - weak_efficiency_penalty
        - no_room_penalty
        - volume_drought_penalty
        - stall_penalty
        - unfavorable_path_penalty
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["range_stretch_pressure"] - 8.00) * 0.020
        - max(0.0, -metrics["volume_phase_alignment"] - 0.75) * 0.015
        - max(0.0, MIN_TEMPO_CONTRACTION - metrics["tempo_contraction"]) * 0.05
        - max(0.0, MIN_DIRECTIONAL_CLOSE_DRIFT - metrics["directional_close_drift"]) * 0.08
    )


def fit_linear_tempo_contraction_acceleration_continuation_model(candidates: list[base.Candidate], fold_id: str) -> LinearTempoContractionAccelerationContinuationModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearTempoContractionAccelerationContinuationModel(
            fold_id,
    ...
    tempo_contraction_acceleration_continuation_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearTempoContractionAccelerationContinuationModel(
        fold_id=fold_id,
        ...
        tempo_contraction_acceleration_continuation_failure_rate=tempo_contraction_acceleration_continuation_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearTempoContractionAccelerationContinuationModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_tempo_contraction_acceleration_continuation_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0138_r0001_intraday_tempo_contraction_acceleration_continuation",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0138_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0138_r0001_tempo_contraction_acceleration_continuation_summary.json",
        ],
        ...
        "conditional_profiles": {
            "tempo_contraction_acceleration_continuation_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0138 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0138_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0138 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "tempo_contraction_acceleration_continuation_lookback_bars": TEMPO_CONTRACTION_ACCELERATION_CONTINUATION_LOOKBACK_BARS,
        "tempo_contraction_acceleration_continuation_micro_bars": TEMPO_CONTRACTION_ACCELERATION_CONTINUATION_MICRO_BARS,
        "min_tempo_contraction": MIN_TEMPO_CONTRACTION,
        "min_directional_close_drift": MIN_DIRECTIONAL_CLOSE_DRIFT,
        "min_close_sequence_persistence": MIN_CLOSE_SEQUENCE_PERSISTENCE,
        "min_tempo_contraction_acceleration_continuation_activity": MIN_TEMPO_CONTRACTION_ACCELERATION_CONTINUATION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_tempo_contraction_acceleration_continuation_model_summary(model: LinearTempoContractionAccelerationContinuationModel) -> dict[str, object]:
    return {
        ...
        "tempo_contraction_acceleration_continuation_failure_rate": rounded(model.tempo_contraction_acceleration_continuation_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearTempoContractionAccelerationContinuationModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_tempo_contraction_acceleration_continuation_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "tempo_contraction_acceleration_continuation_profile": profiles["tempo_contraction_acceleration_continuation_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["next_required_action"] = "produce_c0138_r0001_mt5_logic_parity_evidence"
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0138_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0138_r0001_proxy_trades.csv"
    evidence["tempo_contraction_acceleration_continuation_summary"] = "artifacts/c0138_r0001_tempo_contraction_acceleration_continuation_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0138_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0138_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0138 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0138_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    next_work["tasks"] = ["produce_c0138_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0138_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0138_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0138_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0138_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0138_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0138_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0138 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0138 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    block = f"""
- decision_id: dec_20260708_produce_c0138_r0001_proxy_evidence
  created_local_date: '{local_date}'
  status: active
  decision: produce_c0138_r0001_proxy_evidence
  refines:
  - dec_20260708_open_c0138_r0001_intraday_tempo_contraction_acceleration_continuation_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0138_r0001_proxy_records_tempo_contraction_acceleration_continuation_reference_surface
  ...
  - next_work_is_produce_c0138_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260708_produce_c0138_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0138-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0138_r0001_intraday_tempo_contraction_acceleration_continuation",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0138-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0138-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0138-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0138_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0138_r0001_intraday_tempo_contraction_acceleration_continuation",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0138-R0001-TEMPO-CONTRACTION-ACCELERATION-CONTINUATION-SUMMARY",
            "artifact_role": "tempo_contraction_acceleration_continuation_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0138_r0001_tempo_contraction_acceleration_continuation_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0138_r0001_intraday_tempo_contraction_acceleration_continuation",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0138_r0001_mt5_logic_parity_evidence",
        }
    ]
