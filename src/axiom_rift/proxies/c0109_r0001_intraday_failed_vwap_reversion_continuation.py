"""C0109 R0001 proxy evidence for intraday failed VWAP reversion continuation."""
...
CAMPAIGN_ID = "C0109"
RUN_ID = "R0001"
WORK_UNIT_REL = "campaigns/C0109_intraday_failed_vwap_reversion_continuation_discovery"
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0109_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0109_r0001_failed_vwap_reversion_continuation_summary.json"
...
FEATURE_NAMES = (
    "vwap_dislocation",
    "failed_reversion_pullback",
    "residual_dislocation",
    "continuation_body_turn",
    "pullback_rejection_wick",
    "trend_persistence",
    "range_reexpansion",
    "volume_confirmation",
    "day_extreme_alignment",
    "prior_session_tailwind",
    "session_phase_maturity",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_failed_vwap_reversion_continuation_rank"
LABEL_SHAPE = "target_first_quality_after_failed_vwap_reversion_continuation"
SELECTION_RULE = "top_fold_local_failed_vwap_reversion_continuation_scores_per_active_day"
...
MIN_DAY_BARS = 18
FAILED_VWAP_REVERSION_CONTINUATION_LOOKBACK_BARS = 48
FAILED_VWAP_REVERSION_CONTINUATION_RECENT_BARS = 8
FAILED_VWAP_REVERSION_CONTINUATION_PRIOR_BARS = 24
FAILED_VWAP_REVERSION_CONTINUATION_TREND_BARS = 18
MIN_VWAP_DISLOCATION_RANGE = 0.18
MIN_FAILED_VWAP_REVERSION_CONTINUATION_ACTIVITY = 0.05
...
class LinearFailedVwapReversionContinuationModel:
    fold_id: str
    ...
    failed_vwap_reversion_continuation_failure_rate: float
...
def run_c0109_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_failed_vwap_reversion_continuation_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_failed_vwap_reversion_continuation_model_summary(model))
...
def build_candidates(
    ...
    start_index = max(
        ...
        LOOKBACK_RANGE_BARS,
        SHORT_RANGE_BARS,
        FAILED_VWAP_REVERSION_CONTINUATION_LOOKBACK_BARS,
        FAILED_VWAP_REVERSION_CONTINUATION_RECENT_BARS + FAILED_VWAP_REVERSION_CONTINUATION_PRIOR_BARS,
        72,
        LABEL_HORIZON_BARS,
    )
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = failed_vwap_reversion_continuation_metrics(
    ...
    features = (
        metrics["vwap_dislocation"],
        metrics["failed_reversion_pullback"],
        metrics["residual_dislocation"],
        metrics["continuation_body_turn"],
        metrics["pullback_rejection_wick"],
        metrics["trend_persistence"],
        metrics["range_reexpansion"],
        metrics["volume_confirmation"],
        metrics["day_extreme_alignment"],
        metrics["prior_session_tailwind"],
        metrics["session_phase_maturity"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        c0109_vwap_dislocation_bucket(metrics["vwap_dislocation"]),
        c0109_failed_pullback_bucket(metrics["failed_reversion_pullback"]),
        c0109_residual_dislocation_bucket(metrics["residual_dislocation"]),
        counter_body_turn_bucket(metrics["continuation_body_turn"]),
        adverse_absorption_bucket(metrics["pullback_rejection_wick"]),
        trend_persistence_bucket(metrics["trend_persistence"]),
        range_reexpansion_bucket(metrics["range_reexpansion"]),
        c0109_volume_confirmation_bucket(metrics["volume_confirmation"]),
        c0109_day_extreme_alignment_bucket(metrics["day_extreme_alignment"]),
        c0109_prior_tailwind_bucket(metrics["prior_session_tailwind"]),
        session_phase_maturity_bucket(metrics["session_phase_maturity"]),
...
def failed_vwap_reversion_continuation_metrics(
    bars: list[base.Bar],
    volumes: list[float],
    ...
    recent_start = max(index - FAILED_VWAP_REVERSION_CONTINUATION_RECENT_BARS + 1, context.day_start_index)
    prior_start = max(recent_start - FAILED_VWAP_REVERSION_CONTINUATION_PRIOR_BARS, context.day_start_index)
    lookback_start = max(index - FAILED_VWAP_REVERSION_CONTINUATION_LOOKBACK_BARS + 1, context.day_start_index)
    if prior_start < 0 or recent_start < 0 or lookback_start < 0:
        return {"event_count": 0.0}
    prior_window = bars[prior_start:recent_start]
    ...
    if len(prior_window) < 10 or len(recent_window) < 6 or len(lookback_window) < 24 or len(day_window) < MIN_DAY_BARS:
        return {"event_count": 0.0}
    ...
    prior_ranges = [bar.high - bar.low for bar in prior_window]
    lookback_ranges = [bar.high - bar.low for bar in lookback_window]
    recent_range_mean = base.mean(recent_ranges) or average_range
    prior_range_mean = base.mean(prior_ranges) or average_range
    lookback_range_mean = base.mean(lookback_ranges) or average_range
    prior_volume_mean = base.mean(volumes[prior_start:recent_start]) or average_volume
    ...
    if recent_range_mean <= 0 or prior_range_mean <= 0 or lookback_range_mean <= 0:
        return {"event_count": 0.0}

    session_phase_maturity = bounded_session_progress(signal_bar.time)
    if session_phase_maturity < 0.05 or session_phase_maturity > 0.72:
        return {"event_count": 0.0}
    ...
    dislocation_sign = int(price_sign(signal_bar.close - session_vwap))
    if dislocation_sign == 0 or direction != dislocation_sign:
        return {"event_count": 0.0}
    recent_high = max(bar.high for bar in recent_window)
    recent_low = min(bar.low for bar in recent_window)
    vwap_dislocation = direction * (signal_bar.close - session_vwap) / average_range
    if vwap_dislocation < MIN_VWAP_DISLOCATION_RANGE:
        return {"event_count": 0.0}

    if direction > 0:
        failed_reversion_pullback = max(0.0, recent_high - recent_low) / average_range
        pullback_rejection_wick = adverse_wick_absorption(signal_bar, direction)
        day_extreme_alignment = (signal_bar.close - context.low_so_far) / day_range_so_far
    else:
        failed_reversion_pullback = max(0.0, recent_high - recent_low) / average_range
        pullback_rejection_wick = adverse_wick_absorption(signal_bar, direction)
        day_extreme_alignment = (context.high_so_far - signal_bar.close) / day_range_so_far
    if failed_reversion_pullback < 0.08:
        return {"event_count": 0.0}

    residual_dislocation = vwap_dislocation / max(failed_reversion_pullback, 0.10)
    recent_body_turn = base.mean([directional_body_score(bar, direction) for bar in recent_window[-3:]]) or 0.0
    continuation_body_turn = 0.62 * directional_body_score(signal_bar, direction) + 0.38 * recent_body_turn
    trend_start = max(context.day_start_index, index - FAILED_VWAP_REVERSION_CONTINUATION_TREND_BARS)
    trend_persistence = direction * (signal_bar.close - bars[trend_start].close) / average_range
    range_reexpansion = recent_range_mean / max(prior_range_mean, average_range * 0.10) - 1.0
    volume_confirmation = recent_volume_mean / max(average_volume, 1e-9) - prior_volume_mean / max(
        average_volume,
        1e-9,
    )
    prior_session_tailwind = 0.0
    if context.prior_day_return is not None and context.prior_day_range is not None and context.prior_day_range > 0:
        prior_session_tailwind = direction * context.prior_day_return / max(context.prior_day_range, 1e-9)
        prior_session_tailwind += direction * (context.prior_day_shadow_balance or 0.0) * 0.20

    activity = max(
        vwap_dislocation * 0.55,
        failed_reversion_pullback * 0.45,
        residual_dislocation * 0.30,
        max(continuation_body_turn, 0.0) * 0.50,
        max(trend_persistence, 0.0) * 0.40,
        max(range_reexpansion, 0.0) * 0.25,
        max(volume_confirmation, 0.0) * 0.25,
    )
    if activity < MIN_FAILED_VWAP_REVERSION_CONTINUATION_ACTIVITY:
        return {"event_count": 0.0}
    if trend_persistence < -0.35 or continuation_body_turn < -0.35:
        return {"event_count": 0.0}
    return {
        "vwap_dislocation": bounded(vwap_dislocation, 0.0, 8.0),
        "failed_reversion_pullback": bounded(failed_reversion_pullback, 0.0, 8.0),
        "residual_dislocation": bounded(residual_dislocation, 0.0, 8.0),
        "continuation_body_turn": bounded(continuation_body_turn, -1.0, 1.0),
        "pullback_rejection_wick": bounded(pullback_rejection_wick, 0.0, 1.0),
        "trend_persistence": bounded(trend_persistence, -4.0, 4.0),
        "range_reexpansion": bounded(range_reexpansion, -2.0, 4.0),
        "volume_confirmation": bounded(volume_confirmation, -4.0, 4.0),
        "day_extreme_alignment": bounded(day_extreme_alignment, 0.0, 1.0),
        "prior_session_tailwind": bounded(prior_session_tailwind, -2.0, 2.0),
        ...
        "prior_range_ratio": bounded(prior_range_mean / max(average_range, 1e-9), 0.0, 6.0),
        ...
        "prior_volume_ratio": bounded(prior_volume_mean / max(average_volume, 1e-9), 0.0, 8.0),
        "lookback_volume_ratio": bounded(lookback_volume_mean / max(average_volume, 1e-9), 0.0, 8.0),
        "opening_session_vwap": session_vwap,
        "day_range_so_far": day_range_so_far,
        "dislocation_direction": float(dislocation_sign),
        "failed_vwap_reversion_continuation_pressure": bounded(activity, 0.0, 8.0),
        "event_count": 1.0,
    }
...
def c0109_vwap_dislocation_bucket(value: float) -> str:
    if value < 0.35:
        return "vwap_dislocation_light"
    if value < 0.90:
        return "vwap_dislocation_defined"
    return "vwap_dislocation_extreme"


def c0109_failed_pullback_bucket(value: float) -> str:
    if value < 0.25:
        return "failed_pullback_shallow"
    if value < 0.85:
        return "failed_pullback_defined"
    return "failed_pullback_deep"


def c0109_residual_dislocation_bucket(value: float) -> str:
    if value < 0.45:
        return "residual_dislocation_thin"
    if value < 1.20:
        return "residual_dislocation_tradeable"
    return "residual_dislocation_wide"
...
def c0109_volume_confirmation_bucket(value: float) -> str:
    if value < -0.18:
        return "volume_confirmation_absent"
    if value < 0.18:
        return "volume_confirmation_neutral"
    return "volume_confirmation_active"


def c0109_day_extreme_alignment_bucket(value: float) -> str:
    if value < 0.45:
        return "day_extreme_alignment_inside"
    if value < 0.78:
        return "day_extreme_alignment_outer"
    return "day_extreme_alignment_deep"


def c0109_prior_tailwind_bucket(value: float) -> str:
    if value < -0.18:
        return "prior_tailwind_against"
    if value < 0.18:
        return "prior_tailwind_neutral"
    return "prior_tailwind_with"


def c0109_gap_size_bucket(value: float) -> str:
    if value < 0.45:
...
def c0109_gap_residual_bucket(value: float) -> str:
    if value < 0.22:
...
def c0109_gap_fill_bucket(value: float) -> str:
    if value < 0.05:
...
def c0109_volume_cooling_bucket(value: float) -> str:
    if value < -0.18:
...
def c0109_prior_close_magnet_bucket(value: float) -> str:
    if value < -0.05:
...
def c0109_vwap_magnet_bucket(value: float) -> str:
    if value < -0.08:
...
def c0109_boundary_probe_bucket(value: float) -> str:
    if value < 0.05:
...
def c0109_recoil_bucket(value: float) -> str:
    if value < 0.35:
        return "recoil_weak"
    if value < 0.68:
...
def c0109_failed_followthrough_bucket(value: float) -> str:
    if value < -0.08:
...
def c0109_tail_rejection_bucket(value: float) -> str:
    if value < -0.08:
...
def c0109_reclaim_bucket(value: float) -> str:
    if value < -0.05:
...
def c0109_volume_exhaustion_bucket(value: float) -> str:
    if value < -0.18:
        return "volume_still_expanding"
    if value < 0.18:
        return "volume_flat"
...
def c0109_extension_stress_bucket(value: float) -> str:
    if value < 0.16:
...
def c0109_opening_reclaim_bucket(value: float) -> str:
    if value < -0.08:
...
def c0109_prior_boundary_bucket(value: float) -> str:
    if value < -0.12:
...
def c0109_time_reversal_bucket(value: float) -> str:
    if value < 0.04:
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    dislocation_reward = 0.12 if metrics["vwap_dislocation"] > 0.35 and terminal > 0 else 0.0
    pullback_reward = 0.10 if metrics["failed_reversion_pullback"] > 0.20 and terminal > 0 else 0.0
    residual_reward = 0.09 if metrics["residual_dislocation"] > 0.45 and terminal > 0 else 0.0
    body_reward = 0.10 if metrics["continuation_body_turn"] > 0.08 and terminal > 0 else 0.0
    wick_reward = 0.07 if metrics["pullback_rejection_wick"] > 0.24 and terminal > 0 else 0.0
    trend_reward = 0.10 if metrics["trend_persistence"] > 0.25 and terminal > 0 else 0.0
    expansion_reward = 0.07 if metrics["range_reexpansion"] > 0.08 and terminal > 0 else 0.0
    volume_reward = 0.06 if metrics["volume_confirmation"] > 0.08 and terminal > 0 else 0.0
    extreme_reward = 0.06 if metrics["day_extreme_alignment"] > 0.65 and terminal > 0 else 0.0
    prior_reward = 0.04 if metrics["prior_session_tailwind"] > 0.08 and terminal > 0 else 0.0
    weak_dislocation_penalty = 0.08 if metrics["vwap_dislocation"] < 0.24 and terminal <= 0 else 0.0
    shallow_pullback_penalty = 0.08 if metrics["failed_reversion_pullback"] < 0.12 and terminal <= 0 else 0.0
    residual_penalty = 0.09 if metrics["residual_dislocation"] < 0.28 and terminal <= 0 else 0.0
    body_against_penalty = 0.10 if metrics["continuation_body_turn"] < -0.08 and terminal <= 0 else 0.0
    trend_against_penalty = 0.12 if metrics["trend_persistence"] < -0.10 and terminal <= 0 else 0.0
    expansion_absent_penalty = 0.07 if metrics["range_reexpansion"] < -0.15 and terminal <= 0 else 0.0
    ...
    failed_vwap_reversion_continuation_path_quality = (0.54 * terminal + 0.30 * mfe - 0.70 * mae) / average_range
    same_bar_penalty = 0.25 * same_bar_conflict_count
    spread_penalty = bars[entry_index].spread_points / average_range
    return (
        first_hit
        + failed_vwap_reversion_continuation_path_quality
        + dislocation_reward
        + pullback_reward
        + residual_reward
        + body_reward
        ...
        + extreme_reward
        + prior_reward
        - weak_dislocation_penalty
        - shallow_pullback_penalty
        - residual_penalty
        - body_against_penalty
        - trend_against_penalty
        - expansion_absent_penalty
        - stall_penalty
        - unfavorable_path_penalty
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["recent_range_ratio"] - 2.80) * 0.05
        - max(0.0, metrics["recent_volume_ratio"] - 4.00) * 0.02
    )


def fit_linear_failed_vwap_reversion_continuation_model(candidates: list[base.Candidate], fold_id: str) -> LinearFailedVwapReversionContinuationModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearFailedVwapReversionContinuationModel(
            fold_id,
    ...
    failed_vwap_reversion_continuation_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearFailedVwapReversionContinuationModel(
        fold_id=fold_id,
        ...
        failed_vwap_reversion_continuation_failure_rate=failed_vwap_reversion_continuation_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearFailedVwapReversionContinuationModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "proxy_id": "PX-C0109-R0001",
        "dataset_identity": "data/processed/datasets/us100_m5_base_frame.csv",
        "split_policy": "rolling_window_test_oos_only_with_train_is_failed_vwap_reversion_continuation_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0109_r0001_intraday_failed_vwap_reversion_continuation",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0109_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0109_r0001_failed_vwap_reversion_continuation_summary.json",
        ],
        ...
        "conditional_profiles": {
            "failed_vwap_reversion_continuation_profile": {
                "applies": True,
                "fields": {
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0109 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0109_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0109 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "failed_vwap_reversion_continuation_lookback_bars": FAILED_VWAP_REVERSION_CONTINUATION_LOOKBACK_BARS,
        "failed_vwap_reversion_continuation_recent_bars": FAILED_VWAP_REVERSION_CONTINUATION_RECENT_BARS,
        "failed_vwap_reversion_continuation_prior_bars": FAILED_VWAP_REVERSION_CONTINUATION_PRIOR_BARS,
        "min_failed_vwap_reversion_continuation_activity": MIN_FAILED_VWAP_REVERSION_CONTINUATION_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_failed_vwap_reversion_continuation_model_summary(model: LinearFailedVwapReversionContinuationModel) -> dict[str, object]:
    return {
        ...
        "failed_vwap_reversion_continuation_failure_rate": rounded(model.failed_vwap_reversion_continuation_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearFailedVwapReversionContinuationModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_failed_vwap_reversion_continuation_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "failed_vwap_reversion_continuation_profile": profiles["failed_vwap_reversion_continuation_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0109_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0109_r0001_proxy_trades.csv"
    evidence["failed_vwap_reversion_continuation_summary"] = "artifacts/c0109_r0001_failed_vwap_reversion_continuation_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0109_r0001_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "run_manifest.json",
        "artifact_lineage.json",
        "artifacts/c0109_r0001_proxy_trades.csv",
        "artifacts/c0109_r0001_failed_vwap_reversion_continuation_summary.json",
        "kpi/proxy.json",
    ):
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0109_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0109 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0109_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0109_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0109_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0109_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0109_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0109_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0109_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0109_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0109_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0109 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0109 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0109_r0001_proxy_evidence
  created_local_date: '2026-07-07'
  status: active
  decision: produce_c0109_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0109_r0001_intraday_failed_vwap_reversion_continuation_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0109_r0001_proxy_records_failed_vwap_reversion_continuation_reference_surface
  ...
  - next_work_is_produce_c0109_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0109_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0109-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0109_r0001_intraday_failed_vwap_reversion_continuation",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0109-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0109-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0109-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0109_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0109_r0001_intraday_failed_vwap_reversion_continuation",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0109-R0001-FAILED-VWAP-REVERSION-CONTINUATION-SUMMARY",
            "artifact_role": "failed_vwap_reversion_continuation_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0109_r0001_failed_vwap_reversion_continuation_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0109_r0001_intraday_failed_vwap_reversion_continuation",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0109_r0001_mt5_logic_parity_evidence",
        }
    ]
