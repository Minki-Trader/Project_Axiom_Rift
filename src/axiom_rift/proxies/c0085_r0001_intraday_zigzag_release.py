"""C0085 R0001 proxy evidence for intraday zigzag release."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0085_r0001_proxy_trades.csv"
SUMMARY_PATH = RUN_DIR / "artifacts" / "c0085_r0001_zigzag_release_summary.json"
...
FEATURE_NAMES = (
    "zigzag_density",
    "coil_compression",
    "release_body_pressure",
    "close_location_drive",
    "range_expansion",
    "participation_reignite",
    "path_tortuosity",
    "prior_day_energy_context",
    "session_release_pressure",
    "spread_stress_inverse",
)
MODEL_FAMILY = "fold_local_intraday_zigzag_release_rank"
LABEL_SHAPE = "target_first_continuation_quality_conditioned_on_zigzag_pressure_release_state"
SELECTION_RULE = "top_fold_local_zigzag_release_scores_per_active_day"
...
MIN_DAY_BARS = 30
ZIGZAG_RELEASE_LOOKBACK_BARS = 28
ZIGZAG_RELEASE_SHORT_LOOKBACK_BARS = 8
MIN_ZIGZAG_RELEASE_ACTIVITY = 0.16
...
class LinearZigzagReleaseModel:
    fold_id: str
    ...
    zigzag_release_failure_rate: float
...
def run_c0085_r0001_proxy(write: bool = True) -> dict[str, object]:
...
def build_proxy_run_result() -> base.ProxyRunResult:
    ...
    for fold_id in sorted(fold_id for fold_id, split in windows.items() if {"train_is", "test_oos"} <= set(split)):
        ...
        model = fit_linear_zigzag_release_model(train_candidates, fold_id)
        ...
        fold_models.append(linear_zigzag_release_model_summary(model))
...
def candidate_state_and_features(
    ...
    if context.bars_so_far < MIN_DAY_BARS:
        return None, None
    metrics = zigzag_release_metrics(
    ...
    if volume_ratio is None:
        return None, None
    features = (
        metrics["zigzag_density"],
        metrics["coil_compression"],
        metrics["release_body_pressure"],
        metrics["close_location_drive"],
        metrics["range_expansion"],
        metrics["participation_reignite"],
        metrics["path_tortuosity"],
        metrics["prior_day_energy_context"],
        metrics["session_release_pressure"],
        1.0 / (1.0 + max(spread_stress, 0.0)),
    )
    prefix = "long" if direction > 0 else "short"
    dimensions = (
        zigzag_density_bucket(metrics["zigzag_density"]),
        coil_compression_bucket(metrics["coil_compression"]),
        release_body_bucket(metrics["release_body_pressure"]),
        close_drive_bucket(metrics["close_location_drive"]),
        range_expansion_bucket(metrics["range_expansion"]),
        volume_confirmation_bucket(metrics["participation_reignite"]),
        path_tortuosity_bucket(metrics["path_tortuosity"]),
        prior_day_energy_bucket(metrics["prior_day_energy_context"]),
        session_release_bucket(metrics["session_release_pressure"]),
...
def zigzag_release_metrics(
    ...
    if prior_day_range is None or prior_day_range <= 0:
        return {"event_count": 0.0}
    short_start = index - ZIGZAG_RELEASE_SHORT_LOOKBACK_BARS + 1
    prior_start = index - ZIGZAG_RELEASE_LOOKBACK_BARS + 1
    ...
    close_changes = [prior_window[pos].close - prior_window[pos - 1].close for pos in range(1, len(prior_window))]
    signs = [price_sign(change) for change in close_changes]
    nonzero_signs = [sign for sign in signs if sign != 0.0]
    sign_changes = sum(1 for left, right in zip(nonzero_signs, nonzero_signs[1:]) if left != right)
    zigzag_density = sign_changes / max(len(nonzero_signs) - 1, 1) if len(nonzero_signs) > 1 else 0.0
    coil_compression = 1.0 - short_width / prior_width
    release_body_pressure = directional_body_score(signal_bar, direction)
    close_location_drive = directional_bar_settlement(signal_bar, direction)
    range_expansion = (signal_bar.high - signal_bar.low) / average_range
    path_efficiency = directional_path_efficiency(bars, index, direction, ZIGZAG_RELEASE_SHORT_LOOKBACK_BARS)
    path_tortuosity = 1.0 - min(abs(path_efficiency or 0.0), 1.0)
    ...
    participation_reignite = (current_volume_ratio - early_volume_ratio) * max(release_body_pressure, 0.0)
    prior_day_energy_context = prior_day_range / max(average_range * 60.0, 1e-9)
    release_pressure = max(
        max(zigzag_density - 0.20, 0.0),
        max(coil_compression, 0.0),
        max(release_body_pressure, 0.0),
        max(close_location_drive - 0.50, 0.0) * 2.0,
        max(range_expansion - 0.75, 0.0) / 2.0,
        max(participation_reignite, 0.0),
        max(path_tortuosity - 0.25, 0.0),
    )
    session_release_pressure = bounded_session_progress(signal_bar.time) * release_pressure
    activity = max(release_pressure, session_release_pressure)
    if activity < MIN_ZIGZAG_RELEASE_ACTIVITY:
        return {"event_count": 0.0}
    if zigzag_density < 0.24 and path_tortuosity < 0.28:
        return {"event_count": 0.0}
    if coil_compression < -0.10 and path_tortuosity < 0.30:
        return {"event_count": 0.0}
    if release_body_pressure < -0.08 and close_location_drive < 0.45:
        return {"event_count": 0.0}
    if range_expansion < 0.35 and participation_reignite < -0.05:
        return {"event_count": 0.0}
    return {
        "zigzag_density": bounded(zigzag_density, 0.0, 1.0),
        "coil_compression": bounded(coil_compression, -1.0, 1.0),
        "release_body_pressure": bounded(release_body_pressure, -1.0, 1.0),
        "close_location_drive": bounded(close_location_drive, 0.0, 1.0),
        "range_expansion": bounded(range_expansion, 0.0, 6.0),
        "participation_reignite": bounded(participation_reignite, -3.0, 6.0),
        "path_tortuosity": bounded(path_tortuosity, 0.0, 1.0),
        "prior_day_energy_context": bounded(prior_day_energy_context, 0.0, 3.0),
        "session_release_pressure": bounded(session_release_pressure, 0.0, 4.0),
        "event_count": 1.0,
    }
...
def candidate_label(
    ...
    if metrics["event_count"] <= 0:
        return None
    zigzag_reward = 0.12 if metrics["zigzag_density"] > 0.35 and terminal > 0 else 0.0
    compression_reward = 0.10 if metrics["coil_compression"] > 0.08 and terminal > 0 else 0.0
    release_reward = 0.14 if metrics["release_body_pressure"] > 0.08 and terminal > 0 else 0.0
    close_drive_reward = 0.12 if metrics["close_location_drive"] > 0.58 and terminal > 0 else 0.0
    expansion_reward = 0.08 if metrics["range_expansion"] > 0.85 and terminal > 0 else 0.0
    participation_reward = 0.08 if metrics["participation_reignite"] > 0.03 and terminal > 0 else 0.0
    tortuosity_reward = 0.07 if metrics["path_tortuosity"] > 0.35 and terminal > 0 else 0.0
    energy = metrics["prior_day_energy_context"]
    prior_energy_reward = 0.05 if 0.35 <= energy <= 2.20 and terminal > 0 else 0.0
    session_reward = 0.05 if metrics["session_release_pressure"] > 0.12 and terminal > 0 else 0.0
    weak_zigzag_penalty = 0.10 if metrics["zigzag_density"] < 0.28 and terminal <= 0 else 0.0
    loose_compression_penalty = 0.08 if metrics["coil_compression"] < -0.02 and terminal <= 0 else 0.0
    adverse_release_penalty = 0.12 if metrics["release_body_pressure"] < -0.08 and terminal <= 0 else 0.0
    weak_close_drive_penalty = 0.10 if metrics["close_location_drive"] < 0.45 and terminal <= 0 else 0.0
    thin_range_penalty = 0.08 if metrics["range_expansion"] < 0.45 and terminal <= 0 else 0.0
    no_participation_penalty = 0.08 if metrics["participation_reignite"] < -0.06 and terminal <= 0 else 0.0
    low_tortuosity_penalty = 0.07 if metrics["path_tortuosity"] < 0.22 and terminal <= 0 else 0.0
    ...
    return (
        ...
        - adverse_release_penalty
        - weak_close_drive_penalty
        - thin_range_penalty
        - no_participation_penalty
        - low_tortuosity_penalty
        - stall_penalty
        - unfavorable_path_penalty
        - same_bar_penalty
        - spread_penalty
        - max(0.0, metrics["range_expansion"] - 3.00) * 0.04
    )


def fit_linear_zigzag_release_model(candidates: list[base.Candidate], fold_id: str) -> LinearZigzagReleaseModel:
    labeled = [candidate for candidate in candidates if candidate.label is not None]
    if not labeled:
        return LinearZigzagReleaseModel(
            fold_id,
    ...
    zigzag_release_failure_rate = sum(1 for label in labels if label < -0.75) / len(labels)
    return LinearZigzagReleaseModel(
        fold_id=fold_id,
        ...
        zigzag_release_failure_rate=zigzag_release_failure_rate,
    )
...
def score_candidate(candidate: base.Candidate, model: LinearZigzagReleaseModel) -> base.Candidate:
    score = 0.0
...
def build_proxy_payload(
    ...
    return {
        ...
        "split_policy": "rolling_window_test_oos_only_with_train_is_zigzag_release_fit",
        "fold_ids": fold_ids,
        "proxy_engine": "axiom_rift.proxies.c0085_r0001_intraday_zigzag_release",
        ...
        "proxy_artifact_paths": [
            ...
            f"{RUN_REL}/artifacts/c0085_r0001_proxy_trades.csv",
            f"{RUN_REL}/artifacts/c0085_r0001_zigzag_release_summary.json",
        ],
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "produce C0085 R0001 MT5 paired validation evidence",
                "revisit_when": "produce_c0085_r0001_mt5_logic_parity_evidence",
            ...
            },
            {
                ...
                "revisit_when": "before C0085 R0001 closeout",
                "claim_boundary": {"claim_authority": False},
            },
        ],
...
def proxy_config() -> dict[str, object]:
    return {
        ...
        "zigzag_release_lookback_bars": ZIGZAG_RELEASE_LOOKBACK_BARS,
        "zigzag_release_short_lookback_bars": ZIGZAG_RELEASE_SHORT_LOOKBACK_BARS,
        "min_zigzag_release_activity": MIN_ZIGZAG_RELEASE_ACTIVITY,
        "label_horizon_bars": LABEL_HORIZON_BARS,
...
def linear_zigzag_release_model_summary(model: LinearZigzagReleaseModel) -> dict[str, object]:
    return {
        ...
        "zigzag_release_failure_rate": rounded(model.zigzag_release_failure_rate),
    }
...
def score_distribution(
    ...
    model: LinearZigzagReleaseModel,
...
def write_summary_artifact(payload: dict[str, object], path: Path) -> None:
    ...
    summary = {
        "schema": "axiom_rift_zigzag_release_summary_v1",
        "template": False,
        "work_unit_id": CAMPAIGN_ID,
        "run_id": RUN_ID,
        "created_at_utc": payload["created_at_utc"],
        "proxy_config": payload["proxy_config"],
        "zigzag_release_profile": profiles["zigzag_release_profile"]["fields"],  # type: ignore[index]
...
def update_run_manifest_status() -> None:
    ...
    data["mt5_probe_plan"]["next_required_action"] = "produce_c0085_r0001_mt5_logic_parity_evidence"
    evidence = data.setdefault("evidence_paths", {})
    evidence["proxy_kpi"] = "kpi/proxy.json"
    evidence["proxy_trade_artifact"] = "artifacts/c0085_r0001_proxy_trades.csv"
    evidence["zigzag_release_summary"] = "artifacts/c0085_r0001_zigzag_release_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0085_r0001_mt5_logic_parity_evidence"
...
def update_campaign_status() -> None:
    ...
    closeout["remaining_question"] = "produce_c0085_r0001_mt5_logic_parity_evidence"
...
def update_selected_status() -> None:
    ...
    data["selected_reason"] = "C0085 R0001 proxy evidence is recorded as a reference surface only; MT5 paired validation and fold-isolated closeout remain mandatory before judgment."
    data["next_required_action"] = "produce_c0085_r0001_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy(payload: dict[str, object]) -> None:
    ...
    for item in (
        "produce_c0085_r0001_proxy_evidence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["tasks"] = ["produce_c0085_r0001_mt5_logic_parity_evidence"]
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0085_r0001_proxy_evidence",
        "status": "completed",
        ...
        "next_required_action": "produce_c0085_r0001_mt5_logic_parity_evidence",
        "claim_boundary": CLAIM_BOUNDARY,
    }
...
def update_decision_cursor_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["current_decision"] = "produce_c0085_r0001_mt5_logic_parity_evidence"
    data["active_campaign"] = WORK_UNIT_REL
    data["active_run"] = RUN_REL
    data["active_synthesis"] = None
    data["next_required_action"] = "produce_c0085_r0001_mt5_logic_parity_evidence"
    ...
    summary.update(
        {
            ...
            "evidence_status": "proxy_recorded_pending_mt5",
            "current_task": "produce_c0085_r0001_mt5_logic_parity_evidence",
            "next_required_action": "produce_c0085_r0001_mt5_logic_parity_evidence",
            ...
            "note": "C0085 R0001 proxy evidence is recorded; MT5 paired validation remains mandatory and no selection or economics claim is created.",
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0085 R0001 proxy evidence recorded as reference surface only; MT5 logic parity is next.",
        },
        {
...
def append_decision_registry_after_proxy(payload: dict[str, object]) -> None:
    ...
    summary = payload["proxy_summary"]  # type: ignore[index]
    block = f"""
- decision_id: dec_20260707_produce_c0085_r0001_proxy_evidence
  created_local_date: '2026-07-06'
  status: active
  decision: produce_c0085_r0001_proxy_evidence
  refines:
  - dec_20260707_open_c0085_r0001_intraday_zigzag_release_rank_run
  - dec_20260701_mandatory_mt5_paired_run_validation
  rationale:
  - c0085_r0001_proxy_records_zigzag_release_reference_surface
  ...
  - next_work_is_produce_c0085_r0001_mt5_logic_parity_evidence
  ...
  claim_boundary:
    ...
    live_ready: false
"""
    if "dec_20260707_produce_c0085_r0001_proxy_evidence" not in text:
...
def update_artifact_lineage(proxy_hash: str, trade_hash: str, summary_hash: str) -> None:
    ...
    data["artifact_records"] = [
        {
            "artifact_id": "A-C0085-R0001-RUN-MANIFEST",
            "artifact_role": "run_manifest",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/run_manifest.json",
            "sha256": run_manifest_hash,
            "produced_by": "axiom_rift.proxies.c0085_r0001_intraday_zigzag_release",
            "source_inputs": [
        ...
        {
            "artifact_id": "A-C0085-R0001-GATE-REPORT",
        ...
        {
            "artifact_id": "A-C0085-R0001-PROXY-KPI",
        ...
        {
            "artifact_id": "A-C0085-R0001-PROXY-TRADES",
            "artifact_role": "proxy_trade_artifact",
            "artifact_type": "csv",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0085_r0001_proxy_trades.csv",
            "sha256": trade_hash,
            "produced_by": "axiom_rift.proxies.c0085_r0001_intraday_zigzag_release",
            "source_inputs": [
                "data/processed/datasets/us100_m5_base_frame.csv",
        ...
        {
            "artifact_id": "A-C0085-R0001-zigzag-release-SUMMARY",
            "artifact_role": "zigzag_release_summary_artifact",
            "artifact_type": "json",
            "repo_relative_path": f"{RUN_REL}/artifacts/c0085_r0001_zigzag_release_summary.json",
            "sha256": summary_hash,
            "produced_by": "axiom_rift.proxies.c0085_r0001_intraday_zigzag_release",
            "source_inputs": [
                f"{RUN_REL}/kpi/proxy.json",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0085_r0001_mt5_logic_parity_evidence",
        }
    ]
