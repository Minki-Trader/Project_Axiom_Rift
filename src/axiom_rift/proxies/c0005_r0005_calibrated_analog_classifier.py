"""C0005 R0005 proxy evidence for calibrated analog-classifier analog-memory entries."""
...
TRADE_ARTIFACT_PATH = RUN_DIR / "artifacts" / "c0005_r0005_proxy_trades.csv"
ANALOG_SUMMARY_PATH = RUN_DIR / "artifacts" / "c0005_r0005_analog_memory_summary.json"
...
MODEL_FAMILY = "fold_local_calibrated_analog_diagnostic_classifier"
LABEL_SHAPE = "binary_directional_favorable_path_vs_adverse_path"
...
LOGISTIC_C = 0.70
ELIGIBILITY_FILTER = "distance_only_no_probability_gate"
...
def run_c0005_r0005_proxy(write: bool = True) -> dict[str, object]:
...
def build_candidates(
    ...
    for index in range(start_index, end_index + 1):
        ...
        for direction in (1, -1):
            ...
            if features is None:
                continue
            label = calibrated_analog_classifier_label(bars, range_average, index, direction) if include_labels else None
            ...
            candidates.append(
                base.Candidate(
                    ...
                    state_key=f"{side}|calibrated_analog_classifier_analog_memory",
                    features=features,
                    label=label,
                )
            )
...
def calibrated_analog_classifier_label(
    bars: list[base.Bar],
    range_average: list[float | None],
    index: int,
    direction: int,
...
def fit_analog_model(candidates: list[base.Candidate], fold_id: str) -> AnalogModel:
    ...
    if not labeled:
        ...
        return AnalogModel(
            ...
            neighbors=None,
            classifier=None,
            classifier_feature_mean=np.array([], dtype=float),
            classifier_feature_std=np.ones(1, dtype=float),
            neighbor_count=0,
    ...
    train_distances, train_neighbor_indices = neighbors.kneighbors(scaled)
    if fit_neighbor_count > 1:
        train_distances = train_distances[:, 1:]
        train_neighbor_indices = train_neighbor_indices[:, 1:]
    diagnostics = analog_diagnostics(
        train_distances,
        labels[train_neighbor_indices],
        train_indices[train_neighbor_indices],
        labels,
        train_indices,
    )
    classifier_features = np.hstack([scaled, diagnostics])
    classifier_feature_mean = classifier_features.mean(axis=0)
    classifier_feature_std = classifier_features.std(axis=0)
    classifier_feature_std[classifier_feature_std < 1e-9] = 1.0
    classifier_scaled = (classifier_features - classifier_feature_mean) / classifier_feature_std
    classifier: LogisticRegression | None = None
    if len(set(binary_labels.tolist())) == 2:
        ...
        classifier.fit(classifier_scaled, binary_labels)
    return AnalogModel(
        fold_id=fold_id,
...
def score_candidates(candidates: list[base.Candidate], model: AnalogModel) -> list[base.Candidate]:
    ...
    classifier_features = np.hstack([scaled, diagnostics])
    classifier_scaled = (classifier_features - model.classifier_feature_mean) / model.classifier_feature_std
    if model.classifier is not None:
        positive_column = int(np.where(model.classifier.classes_ == 1)[0][0])
        probabilities = model.classifier.predict_proba(classifier_scaled)[:, positive_column]
    else:
        probabilities = np.repeat(model.global_positive_rate, len(candidates))
    scores = (
        (2.0 * probabilities - 1.0)
        + 0.22 * diagnostics[:, 0]
        + 0.16 * (diagnostics[:, 3] - model.global_positive_rate)
        - 0.24 * (diagnostics[:, 4] - model.global_adverse_rate)
        + 0.10 * diagnostics[:, 5]
        + 0.16 * diagnostics[:, 6]
        - 0.10 * diagnostics[:, 2]
        - 0.18 * diagnostics[:, 7]
...
def build_proxy_payload(
    ...
    payload["proxy_id"] = "PX-C0005-R0005"
    payload["proxy_engine"] = "axiom_rift.proxies.c0005_r0005_calibrated_analog_classifier"
    payload["proxy_config_path"] = "campaigns/C0005_continuous_analog_memory_discovery/runs/R0005/run_manifest.json"
    payload["split_policy"] = "rolling_window_test_oos_only_with_train_is_calibrated_analog_classifier_analog_fit"
    payload["proxy_artifact_paths"] = [
        "campaigns/C0005_continuous_analog_memory_discovery/runs/R0005/kpi/proxy.json",
        "campaigns/C0005_continuous_analog_memory_discovery/runs/R0005/artifacts/c0005_r0005_proxy_trades.csv",
        "campaigns/C0005_continuous_analog_memory_discovery/runs/R0005/artifacts/c0005_r0005_analog_memory_summary.json",
    ]
    ...
    profiles["analog_memory_profile"] = {  # type: ignore[index]
        ...
        "fields": {
            ...
            "state_distributions": state_distributions,
            "candidates_by_fold": candidates_by_fold,
            "selection_rule": "top_fold_local_calibrated_analog_classifier_scores_per_active_day",
    ...
    config.update(
        {
            ...
            "logistic_c": LOGISTIC_C,
            "eligibility_filter": ELIGIBILITY_FILTER,
            "score_interpretation": "higher_score_means_calibrated_favorable_path_probability_plus_analog_diagnostic_edge",
            "variant_boundary": "calibrated_analog_classifier_not_r0001_r0002_r0003_r0004_parameter_nudge_or_retry",
        }
    )
    return config
...
def analog_model_summary(model: AnalogModel) -> dict[str, object]:
    return {
        ...
        "classifier_available": model.classifier is not None,
        "classifier_feature_count": int(model.classifier_feature_mean.size),
        "positive_label_threshold": POSITIVE_LABEL_THRESHOLD,
        "adverse_label_threshold": ADVERSE_LABEL_THRESHOLD,
...
def write_analog_summary_artifact(payload: dict[str, object], path: Path) -> None:
    profiles = payload["conditional_profiles"]  # type: ignore[index]
    summary = {
        "schema": "axiom_rift_calibrated_analog_classifier_analog_memory_summary_v1",
        "template": False,
        "work_unit_id": "C0005",
        "campaign_id": "C0005",
        "run_id": "R0005",
        ...
        [
            artifact_record(
                ...
                [
                    ...
                    "campaigns/C0005_continuous_analog_memory_discovery/runs/R0005/run_manifest.json",
                ],
            ),
            artifact_record(
                "A-C0005-R0005-PROXY-TRADES",
            ...
            artifact_record(
                "A-C0005-R0005-ANALOG-MEMORY-SUMMARY",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0005_r0005_mt5_logic_parity_evidence",
        }
    ]
...
def artifact_record(
    ...
    return {
        ...
        "sha256": digest,
        "produced_by": "axiom_rift.proxies.c0005_r0005_calibrated_analog_classifier",
...
def update_run_manifest_status() -> None:
    ...
    evidence["proxy_trade_artifact"] = "artifacts/c0005_r0005_proxy_trades.csv"
    evidence["analog_memory_summary"] = "artifacts/c0005_r0005_analog_memory_summary.json"
...
def update_gate_report() -> None:
    ...
    data["next_action"] = "produce_c0005_r0005_mt5_logic_parity_evidence"
    evidence_paths = list(data.get("evidence_paths") or [])
    for path in (
        "artifacts/c0005_r0005_proxy_trades.csv",
        "artifacts/c0005_r0005_analog_memory_summary.json",
        "kpi/proxy.json",
    ):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0005 R0005 MT5 evidence files contain measured results",
        }
    ]
...
def update_campaign_status() -> None:
    ...
    next_candidate["direction"] = "active_c0005_r0005_mt5_logic_parity"
    ...
    closeout["remaining_question"] = "produce_c0005_r0005_mt5_logic_parity_evidence"
...
def update_reentry_after_proxy() -> None:
    ...
    if "produce_c0005_r0005_proxy_evidence" not in completed:
        completed.append("produce_c0005_r0005_proxy_evidence")
    next_action = "produce_c0005_r0005_mt5_logic_parity_evidence"
...
def update_claim_state_after_proxy(payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0005_r0005_proxy_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0005_continuous_analog_memory_discovery/runs/R0005/kpi/proxy.json",
        ...
        "next_required_action": "produce_c0005_r0005_mt5_logic_parity_evidence",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def replace_run_markers(value: Any) -> Any:
    ...
    if isinstance(value, str):
        replacements = {
            ...
            "R0005": "R0005",
            "R0001": "R0005",
            "c0004_r0001_fold_local_state_archetype": "c0005_r0005_calibrated_analog_classifier",
            "c0004_r0001": "c0005_r0005",
