"""C0008 draft validators for multi-file state updates."""
...
def validate_c0008_transaction(
    tx: StateTransaction,
    *,
    run_dir: Path,
    campaign_path: Path,
    ...
    """Validate the state files that C0008 MT5 recorders update together."""
...
def _assert_run_gate_alignment(run_manifest: dict[str, Any], gate_report: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise TransactionError(
            "C0008 run_manifest.gate_status does not align with gate_report status: "
            f"{run_gate_status!r}"
        )
...
def _assert_evidence_paths_exist(
    ...
    for rel_path in sorted(paths):
        ...
        if not tx.has_draft(target) and not target.exists():
            raise TransactionError(f"C0008 evidence path is missing: {run_dir / rel_path}")
...
def _assert_next_action_alignment(
    ...
    *,
    ...
    if mismatches:
        raise TransactionError(
            f"C0008 next action mismatch, expected {expected_next_action!r}: " + "; ".join(mismatches)
        )
