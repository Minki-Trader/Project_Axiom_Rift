"""Management-only materialization of the STU-0124 status Repair."""
...
from axiom_rift.research.sleeve_exposure_cap_risk_runtime import (
    CALLABLE_IDENTITY,
...
class ProspectivePairStatusRepairContext(Protocol):
...
def materialize_prospective_pair_status_repair_candidate(
    writer: ProspectivePairStatusRepairContext,
    *,
    explanation: str,
    source_root: Any,
    verification_evidence_hashes: tuple[str, ...] = (),
) -> str:
    """Bind the corrected trace closure and independent verification."""

    reason = _ascii("prospective-pair status Repair explanation", explanation)
    with writer.open_stable_index() as (control, index):
        ...
        if (
            ...
            raise ValueError(
                "status implementation Repair requires one interrupted Job"
            )
        ...
        if attempt_head is not None:
            for sequence in range(1, attempt_head.sequence + 1):
                ...
                if record is None:
                    raise ValueError("status Repair attempt stream has a gap")
                attempts.append(record)
        mission_id = (
            None if declaration is None else declaration.payload.get("mission_id")
        )
        if type(mission_id) is not str or opened is None:
            raise ValueError("status Repair Mission authority is unavailable")
    ...
    if not isinstance(spec, Mapping) or not isinstance(reproduction, list):
        raise ValueError("status Repair provenance is unavailable")
    ...
    if len(requested_verification) != len(verification_evidence_hashes):
        raise ValueError("status Repair verification evidence must be unique")
    ...
    new_identity = materialize_prospective_job_implementation(
        writer,
    ...
    if new_identity == previous_identity:
        raise ValueError("status Repair did not change the source closure")
    verification_results = (
        writer.resolve_prospective_pair_status_correction_verification(
    ...
    plan = writer.plan_prospective_pair_status_correction_repair(
        new_implementation_identity=new_identity,
    )
    ...
    result = semantic_equivalence_result_manifest(
        ...
        surface_verdicts={surface_id: "passed" for surface_id in plan["claims"]},
    )
    ...
    ) = writer.materialize_prospective_pair_status_repair_candidate_validation_plan(
        explanation=reason,
    ...
    verification_receipt = writer.evidence.finalize(
        canonical_bytes(
            build_repair_candidate_validation_receipt(
                ...
                result_artifact_hashes=repair_validation_artifact_hashes,
            )
        )
    )
    ...
    if set(reproduction).intersection(attempt_new_evidence) or set(
        ...
        raise ValueError("status Repair evidence surfaces must be distinct")
...
__all__ = [
    "ProspectivePairStatusRepairContext",
