"""MT5 schedule-replay logic parity helpers for C0011 R0001."""
...
from axiom_rift.proxies.c0011_r0001_setup_lifecycle_timing import (
    BASE_FRAME,
    ROLLING_WINDOWS,
    SplitWindow,
    Trade,
...
SCHEDULE_ARTIFACT = RUN_ARTIFACT_DIR / "c0011_r0001_schedule.csv"
SCHEDULE_COMMON_REL = "AxiomRift\\C0011\\R0001\\schedule\\c0011_r0001_schedule.csv"
STARTING_BALANCE_USD = starting_balance_usd()
RESPONSE_MODE = "fold_local_setup_lifecycle_timing_schedule_replay"
MAX_HOLD_BARS = 10
MAGIC = 1011001
...
def compile_c0011_r0001_ea(metaeditor_exe: Path | None = None) -> CompileResult:
    ...
    log = PROJECT_ROOT / "artifacts" / "reports" / "c0011_r0001_mt5_compile.log"
...
def tester_config_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    return PROJECT_ROOT / "artifacts" / "reports" / "c0011_r0001_mt5_tester" / f"c0011_r0001_{scoped_name(mode, output_scope)}_tester.ini"


def tester_report_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    return PROJECT_ROOT / "artifacts" / "reports" / "c0011_r0001_mt5_tester" / f"c0011_r0001_mt5_{scoped_name(mode, output_scope)}_report.htm"
...
def write_tester_config(
    ...
    config_dir = PROJECT_ROOT / "artifacts" / "reports" / "c0011_r0001_mt5_tester"
...
def parse_c0011_r0001_mt5(
    ...
    with state_transaction(f"C0011-r0001-mt5-{mode}"):
        write_json_state(kpi_path, payload)
        if mode == LOGIC_PARITY_MODE:
            upsert_artifact_lineage(
                "A-C0011-R0001-MT5-SCHEDULE",
                "mt5_schedule_csv",
                "mt5_logic_parity_input",
                rel(SCHEDULE_ARTIFACT),
                sha256_file(SCHEDULE_ARTIFACT),
                ["campaigns/C0011_setup_lifecycle_timing_discovery/runs/R0001/kpi/proxy.json"],
            )
            upsert_artifact_lineage(
                "A-C0011-R0001-MT5-LOGIC-KPI",
                "mt5_logic_parity_kpi",
                "mt5_logic_parity",
                rel(MT5_LOGIC_KPI),
                state_sha256(MT5_LOGIC_KPI),
                [
                    "campaigns/C0011_setup_lifecycle_timing_discovery/runs/R0001/artifacts/c0011_r0001_schedule.csv",
                    "configs/market.yaml",
                ],
            )
        else:
            upsert_artifact_lineage(
                "A-C0011-R0001-MT5-TICK-KPI",
                "mt5_tick_kpi",
                "mt5_tick",
                rel(MT5_TICK_KPI),
    ...
    return {
        ...
        **runtime_payload_fields(),
        ...
        "conditional_profiles": {
            ...
            "setup_lifecycle_timing_schedule_profile": {
                "applies": True,
                "fields": {
                    "setup_lifecycle_timing_source": "rolling_train_is_proxy_schedule",
        ...
        "deferred_with_reason": [
            {
                "field": "native_mql_or_onnx_setup_lifecycle_timing_surface",
                "requirement_class": "deferred_with_reason",
                "reason": "C0011 R0001 logic parity replays the proxy Setup lifecycle timing schedule; native model or ONNX materialization is not claimed at this stage",
...
def record_c0011_r0001_parity() -> dict[str, object]:
    ...
    next_action = "produce_c0011_r0001_mt5_tick_execution_evidence" if mechanical_ok else "repair_c0011_r0001_schedule_replay_parity"
    ...
    payload = {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": RUN_ID,
        "parity_id": "P-C0011-R0001",
        "proxy_id": "PX-C0011-R0001",
        "mt5_probe_id": "MT5-C0011-R0001",
        ...
        "mechanical_checks": {
            ...
            "setup_lifecycle_timing_schedule_match": "passed" if entry_compare["key_match_rate"] == 1.0 else "failed",
        ...
        "conditional_profiles": {
            ...
            "setup_lifecycle_timing_schedule_replay_profile": {
                "applies": True,
                "fields": {
                    "schedule_artifact_path": rel(SCHEDULE_ARTIFACT),
                    "schedule_hash": sha256_file(SCHEDULE_ARTIFACT) if SCHEDULE_ARTIFACT.exists() else None,
                    "logic_boundary": "proxy_selected_fold_local_setup_lifecycle_timing_schedule_replayed_in_mt5_closed_bar_mode",
    ...
    with state_transaction("C0011-r0001-logic-parity", expected_next_action=next_action):
        write_json_state(LOGIC_PARITY_KPI, payload)
        upsert_artifact_lineage(
            "A-C0011-R0001-PROXY-VS-MT5-LOGIC-KPI",
            ...
            [
                "campaigns/C0011_setup_lifecycle_timing_discovery/runs/R0001/kpi/proxy.json",
                "campaigns/C0011_setup_lifecycle_timing_discovery/runs/R0001/kpi/mt5_logic_parity.json",
            ],
        )
...
def update_run_after_mt5(mode: str) -> None:
    ...
    evidence_paths["mt5_schedule_artifact"] = "artifacts/c0011_r0001_schedule.csv"
    mt5_plan = data.setdefault("mt5_probe_plan", {})
    mt5_plan["logic_parity_kpi_path"] = "kpi/mt5_logic_parity.json"
    mt5_plan["logic_parity_purpose"] = "proxy_selected_setup_lifecycle_timing_schedule_vs_ea_closed_bar_lifecycle_parity"
...
def update_gate_after_parity(parity_payload: dict[str, object]) -> None:
    ...
    evidence_paths = data.setdefault("evidence_paths", [])
    for path in ("kpi/mt5_logic_parity.json", "kpi/proxy_vs_mt5_logic_parity.json", "artifacts/c0011_r0001_schedule.csv"):
        ...
        data["deferred_with_reason"] = [
            {
                ...
                "blocking_condition": "produce_c0011_r0001_mt5_tick_execution_evidence",
                "revisit_when": "after C0011 R0001 tick execution KPI and execution divergence are recorded",
            }
        ]
...
def update_reentry_after_logic_parity(next_action: str, mechanical_ok: bool) -> None:
    ...
    for item in (
        "produce_c0011_r0001_mt5_logic_parity_evidence",
        "record_c0011_r0001_proxy_vs_mt5_logic_parity_evidence",
    ):
...
def update_campaign_after_logic_parity(next_action: str) -> None:
    ...
    next_candidate["direction"] = "active_c0011_r0001_tick_execution" if "mt5_tick" in next_action else "active_c0011_r0001_parity_repair"
...
def update_claim_state_after_logic_parity(parity_payload: dict[str, object], mt5_payload: dict[str, object]) -> None:
    ...
    data["active_run"] = "campaigns/C0011_setup_lifecycle_timing_discovery/runs/R0001"
    data["latest_operation"] = {
        "id": "produce_c0011_r0001_mt5_logic_parity_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0011_setup_lifecycle_timing_discovery/runs/R0001/kpi/mt5_logic_parity.json",
        "paired_parity_source": "campaigns/C0011_setup_lifecycle_timing_discovery/runs/R0001/kpi/proxy_vs_mt5_logic_parity.json",
...
def record_c0011_r0001_execution_divergence() -> dict[str, object]:
        ...
        "deferred_with_reason": [
            {
                ...
                "revisit_when": "after C0011 R0001 fold-isolated MT5 tick evidence",
    ...
    next_action = "produce_c0011_r0001_fold_isolated_mt5_tick_kpi"
    with state_transaction("C0011-r0001-execution-divergence", expected_next_action=next_action):
        write_json_state(EXECUTION_DIVERGENCE_KPI, payload)
        upsert_artifact_lineage(
            "A-C0011-R0001-EXECUTION-DIVERGENCE-KPI",
            "diagnostic_output",
            ...
            [
                "campaigns/C0011_setup_lifecycle_timing_discovery/runs/R0001/kpi/mt5_logic_parity.json",
                "campaigns/C0011_setup_lifecycle_timing_discovery/runs/R0001/kpi/mt5_tick.json",
            ],
        )
...
def update_gate_after_execution_divergence(divergence_payload: dict[str, object]) -> None:
    ...
    for path in (
        ...
        "artifacts/c0011_r0001_schedule.csv",
    ):
        if path not in evidence_paths:
            evidence_paths.append(path)
    data["decision"] = "defer_with_reason"
    data["next_action"] = "produce_c0011_r0001_fold_isolated_mt5_tick_kpi"
    data["deferred_with_reason"] = [
        {
            "field": "run_closeout",
            ...
            "blocking_condition": "produce_c0011_r0001_fold_isolated_mt5_tick_kpi",
...
def update_reentry_after_tick_and_divergence() -> None:
    ...
    completed = list(next_work.get("completed") or [])
    for item in (
        "produce_c0011_r0001_mt5_tick_execution_evidence",
        "record_c0011_r0001_execution_divergence",
    ):
        if item not in completed:
            completed.append(item)
    next_action = "produce_c0011_r0001_fold_isolated_mt5_tick_kpi"
...
def update_campaign_after_tick_and_divergence() -> None:
    ...
    run_index = data.setdefault("run_index", {})
    next_candidate = run_index.setdefault("next_run_candidate", {})
    next_candidate["direction"] = "active_c0011_r0001_fold_isolated_tick"
...
def update_claim_state_after_tick_and_divergence(divergence_payload: dict[str, object], tick_payload: dict[str, object]) -> None:
    ...
    data["active_run"] = "campaigns/C0011_setup_lifecycle_timing_discovery/runs/R0001"
    data["latest_operation"] = {
        "id": "produce_c0011_r0001_mt5_tick_execution_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0011_setup_lifecycle_timing_discovery/runs/R0001/kpi/mt5_tick.json",
        "paired_divergence_source": "campaigns/C0011_setup_lifecycle_timing_discovery/runs/R0001/kpi/execution_divergence.json",
        ...
        "economics_shift_status": required.get("economics_shift_status"),
        "next_required_action": "produce_c0011_r0001_fold_isolated_mt5_tick_kpi",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def update_gate_after_fold_isolated_evidence(
    ...
    gate_data["next_action"] = "review_c0011_r0001_tick_execution_kpi_and_closeout"
    gate_data["deferred_with_reason"] = [
        {
            ...
            "blocking_condition": "review_c0011_r0001_tick_execution_kpi_and_closeout",
            "revisit_when": "during C0011 R0001 closeout review",
        }
    ]
...
def update_reentry_after_fold_isolated() -> None:
    ...
    completed = list(next_work.get("completed") or [])
    for item in (
        "produce_c0011_r0001_fold_isolated_mt5_tick_kpi",
        "produce_c0011_r0001_fold_isolated_execution_divergence",
    ):
        if item not in completed:
            completed.append(item)
    next_action = "review_c0011_r0001_tick_execution_kpi_and_closeout"
    completed = [item for item in completed if item != next_action]
    next_work["completed"] = completed
    next_work["tasks"] = [next_action]
    write_yaml_state(REENTRY, data)
...
def update_campaign_after_fold_isolated() -> None:
    ...
    run_index = data.setdefault("run_index", {})
    next_candidate = run_index.setdefault("next_run_candidate", {})
    next_candidate["direction"] = "active_c0011_r0001_closeout_review"
...
def update_claim_state_after_fold_isolated(
    ...
    data["active_run"] = "campaigns/C0011_setup_lifecycle_timing_discovery/runs/R0001"
    data["latest_operation"] = {
        "id": "produce_c0011_r0001_fold_isolated_mt5_tick_kpi",
        "status": "completed",
        "recorded_at_source": "campaigns/C0011_setup_lifecycle_timing_discovery/runs/R0001/kpi/mt5_tick_by_fold.json",
        "paired_divergence_source": "campaigns/C0011_setup_lifecycle_timing_discovery/runs/R0001/kpi/execution_divergence_by_fold.json",
        ...
        "next_required_action": "review_c0011_r0001_tick_execution_kpi_and_closeout",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def upsert_artifact_lineage(
    ...
    next_record = {
        ...
        "mutable": False,
        "produced_by": "axiom_rift.mt5.c0011_r0001_probe",
        ...
        data["deferred_with_reason"] = [
            {
                ...
                "next_action": "produce_c0011_r0001_fold_isolated_execution_divergence",
            }
        ]
        ...
        data["deferred_with_reason"] = [
            {
                ...
                "next_action": "produce_c0011_r0001_mt5_tick_execution_evidence",
            }
        ]
    write_json_state(ARTIFACT_LINEAGE, data)


def run_c0011_r0001_mt5_logic_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_c0011_r0001_tester(mode=LOGIC_PARITY_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_c0011_r0001_mt5(result)
    parity_payload = record_c0011_r0001_parity()
    return {
...
def run_c0011_r0001_mt5_tick_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_c0011_r0001_tester(mode=TICK_EXECUTION_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_c0011_r0001_mt5(result)
    divergence_payload = record_c0011_r0001_execution_divergence()
    return {
...
def run_c0011_r0001_mt5_tick_by_fold_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    compile_c0011_r0001_ea()
    write_schedule_files()
    records: list[dict[str, Any]] = []
    for window in load_test_windows():
        from_date, to_date = tester_dates_for_window(window)
        logic_result = run_c0011_r0001_tester(
        ...
        logic_payload = parse_c0011_r0001_mt5(logic_result, write_kpi=False)
        tick_result = run_c0011_r0001_tester(
        ...
        tick_payload = parse_c0011_r0001_mt5(tick_result, write_kpi=False)
        records.append(
            {
                "window": window,
                "from_date": from_date,
    ...
    next_action = "review_c0011_r0001_tick_execution_kpi_and_closeout"
    with state_transaction("C0011-r0001-fold-isolated", expected_next_action=next_action):
        ...
        upsert_artifact_lineage(
            "A-C0011-R0001-MT5-TICK-BY-FOLD-KPI",
        ...
        upsert_artifact_lineage(
            "A-C0011-R0001-EXECUTION-DIVERGENCE-BY-FOLD-KPI",
            "diagnostic_output",
            "execution_divergence_by_fold",
            rel(EXECUTION_DIVERGENCE_BY_FOLD_KPI),
            state_sha256(EXECUTION_DIVERGENCE_BY_FOLD_KPI),
            [
                "campaigns/C0011_setup_lifecycle_timing_discovery/runs/R0001/kpi/mt5_tick_by_fold.json",
                "registries/rolling_windows.yaml",
            ],
        )
...
def build_mt5_tick_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": RUN_ID,
        "mt5_probe_id": "MT5-C0011-R0001-BY-FOLD",
...
def build_execution_divergence_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": RUN_ID,
        "divergence_id": "ED-C0011-R0001-BY-FOLD",
        "split_policy": "rolling_window_test_oos_fold_isolated",
        "split_registry": "registries/rolling_windows.yaml",
