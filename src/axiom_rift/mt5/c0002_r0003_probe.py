"""MT5 schedule-replay logic parity helpers for C0002 R0003."""
...
from axiom_rift.proxies.c0002_r0003_dual_direction_cell_score import (
    BASE_FRAME,
    ROLLING_WINDOWS,
    SplitWindow,
    Trade,
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0002_score_conditioned_intraday_edge" / "runs" / "R0003"
CAMPAIGN = PROJECT_ROOT / "campaigns" / "C0002_score_conditioned_intraday_edge" / "campaign.yaml"
KPI_DIR = RUN_DIR / "kpi"
...
SCHEDULE_ARTIFACT = RUN_ARTIFACT_DIR / "c0002_r0003_schedule.csv"
SCHEDULE_COMMON_REL = "AxiomRift\\C0002\\R0003\\schedule\\c0002_r0003_schedule.csv"
STARTING_BALANCE_USD = 500.0
R0003_RESPONSE_MODE = "score_conditioned_dual_direction_cell_schedule_replay"
R0003_MAX_HOLD_BARS = 12
...
def compile_c0002_r0003_ea(metaeditor_exe: Path = DEFAULT_METAEDITOR_EXE) -> CompileResult:
    ...
    log = PROJECT_ROOT / "artifacts" / "reports" / "C0002_R0003_mt5_compile.log"
...
def tester_config_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    return PROJECT_ROOT / "artifacts" / "reports" / "C0002_R0003_mt5_tester" / f"C0002_R0003_{scoped_name(mode, output_scope)}_tester.ini"


def tester_report_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    return PROJECT_ROOT / "artifacts" / "reports" / "C0002_R0003_mt5_tester" / f"C0002_R0003_mt5_{scoped_name(mode, output_scope)}_report.htm"
...
def write_tester_config(
    ...
    config_dir = PROJECT_ROOT / "artifacts" / "reports" / "C0002_R0003_mt5_tester"
    ...
    lines = [
        ...
        f"InpResponseMode={R0003_RESPONSE_MODE}",
        f"InpSchedulePath={SCHEDULE_COMMON_REL}",
        "InpMagic=200002",
        "InpLot=0.01",
        f"InpMaxHoldBars={R0003_MAX_HOLD_BARS}",
...
def parse_c0002_r0003_mt5(
    ...
    if mode == LOGIC_PARITY_MODE:
        upsert_artifact_lineage(
            ...
            ["campaigns/C0002_score_conditioned_intraday_edge/runs/R0003/kpi/proxy.json"],
        )
        upsert_artifact_lineage(
            "A0004",
            ...
            [
                "campaigns/C0002_score_conditioned_intraday_edge/runs/R0003/artifacts/c0002_r0003_schedule.csv",
                "configs/market.yaml",
            ],
        )
    else:
...
def assert_mt5_mode_matches_status(mode: str, status: dict[str, str], output_scope: str | None = None) -> None:
    ...
    if status.get("response_mode") != R0003_RESPONSE_MODE:
        raise RuntimeError(
            "MT5 response_mode mismatch: "
            f"expected={R0003_RESPONSE_MODE} actual={status.get('response_mode')}"
        )
    ...
    return {
        ...
        "conditional_profiles": {
            ...
            "score_model_profile": {
                "applies": True,
                "fields": {
                    "score_surface_source": "rolling_train_is_proxy_schedule",
        ...
        "deferred_with_reason": [
            {
                "field": "native_mql_score_training",
                "requirement_class": "deferred_with_reason",
                "reason": "C0002 R0003 logic parity replays the train-only score-selected schedule; native model or ONNX materialization is not claimed at this stage",
                "blocking_condition": "candidate quality must be established before materialization work freezes model export requirements",
                "revisit_when": "after a score-conditioned candidate has fold-isolated MT5 tick evidence worth materializing",
...
def record_c0002_r0003_parity() -> dict[str, object]:
    ...
    next_action = "produce_c0002_r0003_mt5_tick_execution_evidence" if mechanical_ok else "repair_c0002_r0003_schedule_replay_parity"
    ...
    payload = {
        ...
        "mechanical_checks": {
            ...
            "score_filtered_entry_match": "passed" if entry_compare["key_match_rate"] == 1.0 else "failed",
        ...
        "conditional_profiles": {
            ...
            "score_schedule_replay_profile": {
                "applies": True,
                "fields": {
                    "schedule_artifact_path": rel(SCHEDULE_ARTIFACT),
                    "schedule_hash": sha256_file(SCHEDULE_ARTIFACT) if SCHEDULE_ARTIFACT.exists() else None,
                    "logic_boundary": "proxy_selected_trade_schedule_replayed_in_mt5_closed_bar_mode",
                    "native_mql_score_training_claim": False,
                },
            },
            "cost_execution_profile": {
                "applies": True,
        ...
        "deferred_with_reason": [
            {
                "field": "native_mql_or_onnx_score_surface",
                "requirement_class": "deferred_with_reason",
                "reason": "logic parity confirms MT5 can reproduce the selected trade lifecycle; it does not freeze model materialization",
                "blocking_condition": "candidate quality and materialization contract are not established yet",
                "revisit_when": "after fold-isolated MT5 tick evidence shows a candidate worth materializing",
                "claim_boundary": {"claim_authority": False, "onnx_ready": False, "runtime_authority": False},
            }
        ],
        "claim_boundary": {
    ...
    upsert_artifact_lineage(
        ...
        [
            "campaigns/C0002_score_conditioned_intraday_edge/runs/R0003/kpi/proxy.json",
            "campaigns/C0002_score_conditioned_intraday_edge/runs/R0003/kpi/mt5_logic_parity.json",
        ],
    )
...
def record_c0002_r0003_execution_divergence() -> dict[str, object]:
        ...
        "deferred_with_reason": [
            {
                ...
                "revisit_when": "after C0002 R0003 fold-isolated MT5 tick evidence",
    ...
    upsert_artifact_lineage(
        ...
        [
            "campaigns/C0002_score_conditioned_intraday_edge/runs/R0003/kpi/mt5_logic_parity.json",
            "campaigns/C0002_score_conditioned_intraday_edge/runs/R0003/kpi/mt5_tick.json",
        ],
    )
    update_gate_after_execution_divergence(payload)
    update_reentry_after_tick_and_divergence()
    update_campaign_remaining_question("produce_c0002_r0003_fold_isolated_mt5_tick_kpi")
    return payload
...
def fold_summary(events: list[dict[str, str]], profits: list[float]) -> dict[str, Any]:
    ...
    schedule_rows_by_entry = {parse_time(row["entry_time"]): row.get("fold_id", "") for row in read_csv_rows(SCHEDULE_ARTIFACT)} if SCHEDULE_ARTIFACT.exists() else {}
    for row in entries:
...
def update_run_after_mt5(mode: str) -> None:
    ...
    evidence_paths["mt5_schedule_artifact"] = "artifacts/c0002_r0003_schedule.csv"
    mt5_plan = data.setdefault("mt5_probe_plan", {})
    mt5_plan["logic_parity_kpi_path"] = "kpi/mt5_logic_parity.json"
    mt5_plan["logic_parity_purpose"] = "proxy_selected_schedule_vs_ea_closed_bar_lifecycle_parity"
...
def update_gate_after_parity(parity_payload: dict[str, object]) -> None:
    ...
    evidence_paths = data.setdefault("evidence_paths", [])
    for path in ("kpi/mt5_logic_parity.json", "kpi/proxy_vs_mt5_logic_parity.json", "artifacts/c0002_r0003_schedule.csv"):
        ...
        data["deferred_with_reason"] = [
            {
                ...
                "blocking_condition": "produce_c0002_r0003_mt5_tick_execution_evidence",
                "revisit_when": "after C0002 R0003 tick execution KPI and execution divergence are recorded",
            }
        ]
...
def update_gate_after_execution_divergence(divergence_payload: dict[str, object]) -> None:
    ...
    for path in (
        ...
        "artifacts/c0002_r0003_schedule.csv",
    ):
        if path not in evidence_paths:
            evidence_paths.append(path)
    data["decision"] = "defer_with_reason"
    data["next_action"] = "produce_c0002_r0003_fold_isolated_mt5_tick_kpi"
    data["deferred_with_reason"] = [
        {
            "field": "run_closeout",
            ...
            "blocking_condition": "produce_c0002_r0003_fold_isolated_mt5_tick_kpi",
...
def update_gate_after_fold_isolated_evidence(
    ...
    gate_data["next_action"] = "review_c0002_r0003_tick_execution_kpi_and_closeout"
    gate_data["deferred_with_reason"] = [
        {
            ...
            "blocking_condition": "review_c0002_r0003_tick_execution_kpi_and_closeout",
            "revisit_when": "during C0002 R0003 closeout review",
        }
    ]
...
def upsert_artifact_lineage(
    ...
    next_record = {
        ...
        "mutable": False,
        "produced_by": "axiom_rift.mt5.c0002_r0003_probe",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0002_r0003_mt5_tick_execution_evidence",
        }
    ]
...
def update_reentry_after_logic_parity(next_action: str, mechanical_ok: bool) -> None:
    ...
    previous_tasks = list(next_work.get("tasks") or [])
    completed = list(next_work.get("completed") or [])
    completed_items = [
        "produce_c0002_r0003_mt5_logic_parity_evidence",
        "record_c0002_r0003_proxy_vs_mt5_logic_parity_evidence",
    ]
    if "repair_c0002_r0003_schedule_replay_parity" in previous_tasks and mechanical_ok:
        completed_items.append("repair_c0002_r0003_schedule_replay_parity")
...
def update_campaign_after_logic_parity(next_action: str) -> None:
    update_campaign_remaining_question(next_action)


def update_campaign_remaining_question(next_action: str) -> None:
    lines = CAMPAIGN.read_text(encoding="ascii").splitlines()
    updated = [
        f"  remaining_question: {next_action}" if line.strip().startswith("remaining_question:") else line
        for line in lines
    ]
    CAMPAIGN.write_text("\n".join(updated) + "\n", encoding="ascii")
...
def update_reentry_after_tick_and_divergence() -> None:
    ...
    completed = list(next_work.get("completed") or [])
    for item in (
        "produce_c0002_r0003_mt5_tick_execution_evidence",
        "record_c0002_r0003_execution_divergence",
    ):
        if item not in completed:
            completed.append(item)
    next_action = "produce_c0002_r0003_fold_isolated_mt5_tick_kpi"
    completed = [item for item in completed if item != next_action]
    next_work["completed"] = completed
    next_work["tasks"] = [next_action]
    path.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def update_reentry_after_fold_isolated() -> None:
    ...
    completed = list(next_work.get("completed") or [])
    for item in (
        "produce_c0002_r0003_fold_isolated_mt5_tick_kpi",
        "produce_c0002_r0003_fold_isolated_execution_divergence",
    ):
        if item not in completed:
            completed.append(item)
    next_action = "review_c0002_r0003_tick_execution_kpi_and_closeout"
    completed = [item for item in completed if item != next_action]
    next_work["completed"] = completed
    next_work["tasks"] = [next_action]
    path.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def run_c0002_r0003_mt5_logic_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_c0002_r0003_tester(mode=LOGIC_PARITY_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_c0002_r0003_mt5(result)
    parity_payload = record_c0002_r0003_parity()
    return {
...
def run_c0002_r0003_mt5_tick_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_c0002_r0003_tester(mode=TICK_EXECUTION_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_c0002_r0003_mt5(result)
    divergence_payload = record_c0002_r0003_execution_divergence()
    return {
...
def run_c0002_r0003_mt5_tick_by_fold_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    compile_c0002_r0003_ea()
    write_schedule_files()
    records: list[dict[str, Any]] = []
    for window in load_test_windows():
        from_date, to_date = tester_dates_for_window(window)
        logic_result = run_c0002_r0003_tester(
        ...
        logic_payload = parse_c0002_r0003_mt5(logic_result, write_kpi=False)
        tick_result = run_c0002_r0003_tester(
        ...
        tick_payload = parse_c0002_r0003_mt5(tick_result, write_kpi=False)
        records.append(
            {
                "window": window,
                "from_date": from_date,
    ...
    upsert_artifact_lineage(
        ...
        [
            "campaigns/C0002_score_conditioned_intraday_edge/runs/R0003/kpi/mt5_tick_by_fold.json",
            "registries/rolling_windows.yaml",
        ],
    )
    ...
    update_campaign_remaining_question("review_c0002_r0003_tick_execution_kpi_and_closeout")
    return {
...
def build_mt5_tick_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": "R0003",
        "mt5_probe_id": "MT5C0002R0003_BY_FOLD",
