"""MT5 schedule-replay logic parity helpers for C0004 R0004."""
...
from axiom_rift.proxies.c0004_r0004_temporal_stability_archetype import (
    BASE_FRAME,
    ROLLING_WINDOWS,
    SplitWindow,
    Trade,
...
SCHEDULE_ARTIFACT = RUN_ARTIFACT_DIR / "c0004_r0004_schedule.csv"
SCHEDULE_COMMON_REL = "AxiomRift\\C0004\\R0004\\schedule\\c0004_r0004_schedule.csv"
STARTING_BALANCE_USD = 500.0
RESPONSE_MODE = "fold_local_temporal_stability_archetype_schedule_replay"
MAX_HOLD_BARS = 10
MAGIC = 400004
...
def compile_c0004_r0004_ea(metaeditor_exe: Path = DEFAULT_METAEDITOR_EXE) -> CompileResult:
    ...
    log = PROJECT_ROOT / "artifacts" / "reports" / "C0004_R0004_mt5_compile.log"
...
def tester_config_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    return PROJECT_ROOT / "artifacts" / "reports" / "C0004_R0004_mt5_tester" / f"C0004_R0004_{scoped_name(mode, output_scope)}_tester.ini"


def tester_report_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    return PROJECT_ROOT / "artifacts" / "reports" / "C0004_R0004_mt5_tester" / f"C0004_R0004_mt5_{scoped_name(mode, output_scope)}_report.htm"
...
def write_tester_config(
    ...
    config_dir = PROJECT_ROOT / "artifacts" / "reports" / "C0004_R0004_mt5_tester"
...
def parse_c0004_r0004_mt5(
    ...
    if mode == LOGIC_PARITY_MODE:
        ...
        upsert_artifact_lineage(
            "A-C0004-R0004-MT5-LOGIC-KPI",
            "mt5_logic_parity_kpi",
            "mt5_logic_parity",
            rel(MT5_LOGIC_KPI),
            sha256_file(MT5_LOGIC_KPI),
            [
                "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0004/artifacts/c0004_r0004_schedule.csv",
                "configs/market.yaml",
            ],
        )
    else:
        upsert_artifact_lineage(
            "A-C0004-R0004-MT5-TICK-KPI",
            "mt5_tick_kpi",
            "mt5_tick",
            rel(MT5_TICK_KPI),
        ...
        "deferred_with_reason": [
            {
                ...
                "reason": "C0004 R0004 logic parity replays the proxy state-archetype schedule; native model or ONNX materialization is not claimed at this stage",
...
def record_c0004_r0004_parity() -> dict[str, object]:
    ...
    next_action = "produce_c0004_r0004_mt5_tick_execution_evidence" if mechanical_ok else "repair_c0004_r0004_schedule_replay_parity"
    ...
    payload = {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": RUN_ID,
        "parity_id": "P-C0004-R0004",
        "proxy_id": "PX-C0004-R0004",
        "mt5_probe_id": "MT5-C0004-R0004",
        ...
        "conditional_profiles": {
            ...
            "state_archetype_schedule_replay_profile": {
                ...
                "fields": {
                    ...
                    "logic_boundary": "proxy_selected_fold_local_temporal_stability_archetype_schedule_replayed_in_mt5_closed_bar_mode",
    ...
    upsert_artifact_lineage(
        "A-C0004-R0004-PROXY-VS-MT5-LOGIC-KPI",
        ...
        [
            "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0004/kpi/proxy.json",
            "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0004/kpi/mt5_logic_parity.json",
        ],
    )
...
def update_run_after_mt5(mode: str) -> None:
    ...
    evidence_paths["mt5_schedule_artifact"] = "artifacts/c0004_r0004_schedule.csv"
    mt5_plan = data.setdefault("mt5_probe_plan", {})
    mt5_plan["logic_parity_kpi_path"] = "kpi/mt5_logic_parity.json"
    mt5_plan["logic_parity_purpose"] = "proxy_selected_temporal_stability_archetype_schedule_vs_ea_closed_bar_lifecycle_parity"
...
def update_gate_after_parity(parity_payload: dict[str, object]) -> None:
    ...
    evidence_paths = data.setdefault("evidence_paths", [])
    for path in ("kpi/mt5_logic_parity.json", "kpi/proxy_vs_mt5_logic_parity.json", "artifacts/c0004_r0004_schedule.csv"):
        ...
        data["deferred_with_reason"] = [
            {
                ...
                "blocking_condition": "produce_c0004_r0004_mt5_tick_execution_evidence",
                "revisit_when": "after C0004 R0004 tick execution KPI and execution divergence are recorded",
            }
        ]
...
def update_reentry_after_logic_parity(next_action: str, mechanical_ok: bool) -> None:
    ...
    for item in (
        "produce_c0004_r0004_mt5_logic_parity_evidence",
        "record_c0004_r0004_proxy_vs_mt5_logic_parity_evidence",
    ):
...
def update_campaign_after_logic_parity(next_action: str) -> None:
    ...
    next_candidate["direction"] = "active_r0004_tick_execution" if "mt5_tick" in next_action else "active_r0004_parity_repair"
...
def update_claim_state_after_logic_parity(parity_payload: dict[str, object], mt5_payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0004_r0004_mt5_logic_parity_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0004/kpi/mt5_logic_parity.json",
        "paired_parity_source": "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0004/kpi/proxy_vs_mt5_logic_parity.json",
...
def record_c0004_r0004_execution_divergence() -> dict[str, object]:
        ...
        "deferred_with_reason": [
            {
                ...
                "revisit_when": "after C0004 R0004 fold-isolated MT5 tick evidence",
    ...
    upsert_artifact_lineage(
        "A-C0004-R0004-EXECUTION-DIVERGENCE-KPI",
        "diagnostic_output",
        ...
        [
            "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0004/kpi/mt5_logic_parity.json",
            "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0004/kpi/mt5_tick.json",
        ],
    )
...
def update_gate_after_execution_divergence(divergence_payload: dict[str, object]) -> None:
    ...
    for path in (
        ...
        "artifacts/c0004_r0004_schedule.csv",
    ):
        if path not in evidence_paths:
            evidence_paths.append(path)
    data["decision"] = "defer_with_reason"
    data["next_action"] = "produce_c0004_r0004_fold_isolated_mt5_tick_kpi"
    data["deferred_with_reason"] = [
        {
            "field": "run_closeout",
            ...
            "blocking_condition": "produce_c0004_r0004_fold_isolated_mt5_tick_kpi",
...
def update_reentry_after_tick_and_divergence() -> None:
    ...
    completed = list(next_work.get("completed") or [])
    for item in (
        "produce_c0004_r0004_mt5_tick_execution_evidence",
        "record_c0004_r0004_execution_divergence",
    ):
        if item not in completed:
            completed.append(item)
    next_action = "produce_c0004_r0004_fold_isolated_mt5_tick_kpi"
    completed = [item for item in completed if item != next_action]
    next_work["completed"] = completed
    next_work["tasks"] = [next_action]
    path.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def update_campaign_after_tick_and_divergence() -> None:
    ...
    run_index = data.setdefault("run_index", {})
    next_candidate = run_index.setdefault("next_run_candidate", {})
    next_candidate["direction"] = "active_r0004_fold_isolated_tick"
...
def update_claim_state_after_tick_and_divergence(divergence_payload: dict[str, object], tick_payload: dict[str, object]) -> None:
    ...
    data["active_run"] = "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0004"
    data["latest_operation"] = {
        "id": "produce_c0004_r0004_mt5_tick_execution_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0004/kpi/mt5_tick.json",
        "paired_divergence_source": "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0004/kpi/execution_divergence.json",
        ...
        "economics_shift_status": required.get("economics_shift_status"),
        "next_required_action": "produce_c0004_r0004_fold_isolated_mt5_tick_kpi",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def update_gate_after_fold_isolated_evidence(
    ...
    gate_data["next_action"] = "review_c0004_r0004_tick_execution_kpi_and_closeout"
    gate_data["deferred_with_reason"] = [
        {
            ...
            "blocking_condition": "review_c0004_r0004_tick_execution_kpi_and_closeout",
            "revisit_when": "during C0004 R0004 closeout review",
        }
    ]
...
def update_reentry_after_fold_isolated() -> None:
    ...
    completed = list(next_work.get("completed") or [])
    for item in (
        "produce_c0004_r0004_fold_isolated_mt5_tick_kpi",
        "produce_c0004_r0004_fold_isolated_execution_divergence",
    ):
        if item not in completed:
            completed.append(item)
    next_action = "review_c0004_r0004_tick_execution_kpi_and_closeout"
    completed = [item for item in completed if item != next_action]
    next_work["completed"] = completed
    next_work["tasks"] = [next_action]
    path.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def update_campaign_after_fold_isolated() -> None:
    ...
    run_index = data.setdefault("run_index", {})
    next_candidate = run_index.setdefault("next_run_candidate", {})
    next_candidate["direction"] = "active_r0004_closeout_review"
...
def update_claim_state_after_fold_isolated(
    ...
    data["active_run"] = "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0004"
    data["latest_operation"] = {
        "id": "produce_c0004_r0004_fold_isolated_mt5_tick_kpi",
        "status": "completed",
        "recorded_at_source": "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0004/kpi/mt5_tick_by_fold.json",
        "paired_divergence_source": "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0004/kpi/execution_divergence_by_fold.json",
        ...
        "next_required_action": "review_c0004_r0004_tick_execution_kpi_and_closeout",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def upsert_artifact_lineage(
    ...
    next_record = {
        ...
        "mutable": False,
        "produced_by": "axiom_rift.mt5.c0004_r0004_probe",
        ...
        data["deferred_with_reason"] = [
            {
                ...
                "next_action": "produce_c0004_r0004_fold_isolated_execution_divergence",
            }
        ]
        ...
        data["deferred_with_reason"] = [
            {
                ...
                "next_action": "produce_c0004_r0004_mt5_tick_execution_evidence",
            }
        ]
    ARTIFACT_LINEAGE.write_text(json.dumps(data, indent=2, sort_keys=True) + "\n", encoding="ascii")


def run_c0004_r0004_mt5_logic_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_c0004_r0004_tester(mode=LOGIC_PARITY_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_c0004_r0004_mt5(result)
    parity_payload = record_c0004_r0004_parity()
    return {
...
def run_c0004_r0004_mt5_tick_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_c0004_r0004_tester(mode=TICK_EXECUTION_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_c0004_r0004_mt5(result)
    divergence_payload = record_c0004_r0004_execution_divergence()
    return {
...
def run_c0004_r0004_mt5_tick_by_fold_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    compile_c0004_r0004_ea()
    write_schedule_files()
    records: list[dict[str, Any]] = []
    for window in load_test_windows():
        from_date, to_date = tester_dates_for_window(window)
        logic_result = run_c0004_r0004_tester(
        ...
        logic_payload = parse_c0004_r0004_mt5(logic_result, write_kpi=False)
        tick_result = run_c0004_r0004_tester(
        ...
        tick_payload = parse_c0004_r0004_mt5(tick_result, write_kpi=False)
        records.append(
            {
                "window": window,
                "from_date": from_date,
    ...
    upsert_artifact_lineage(
        "A-C0004-R0004-MT5-TICK-BY-FOLD-KPI",
    ...
    upsert_artifact_lineage(
        "A-C0004-R0004-EXECUTION-DIVERGENCE-BY-FOLD-KPI",
        "diagnostic_output",
        "execution_divergence_by_fold",
        rel(EXECUTION_DIVERGENCE_BY_FOLD_KPI),
        sha256_file(EXECUTION_DIVERGENCE_BY_FOLD_KPI),
        [
            "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0004/kpi/mt5_tick_by_fold.json",
            "registries/rolling_windows.yaml",
        ],
    )
...
def build_mt5_tick_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": RUN_ID,
        "mt5_probe_id": "MT5-C0004-R0004-BY-FOLD",
...
def build_execution_divergence_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": RUN_ID,
        "divergence_id": "ED-C0004-R0004-BY-FOLD",
        "split_policy": "rolling_window_test_oos_fold_isolated",
        "split_registry": "registries/rolling_windows.yaml",
