"""MT5 schedule-replay logic parity helpers for SC0002 SR0001."""
...
from axiom_rift.proxies.sc0002_sr0001_cross_surface_veto_inversion import (
    BASE_FRAME,
    ROLLING_WINDOWS,
    TIME_FORMAT,
    SplitWindow,
...
WORK_UNIT_ID = "SC0002"
RUN_ID = "SR0001"
RUN_DIR = PROJECT_ROOT / "campaigns" / "SC0002_accumulated_post_sc0001_negative_memory_synthesis" / "runs" / RUN_ID
SYNTHESIS_QUEUE = PROJECT_ROOT / "campaigns" / "SC0002_accumulated_post_sc0001_negative_memory_synthesis" / "synthesis_queue.yaml"
KPI_DIR = RUN_DIR / "kpi"
...
PROXY_TRADE_ARTIFACT = RUN_ARTIFACT_DIR / "sc0002_sr0001_proxy_trades.csv"
SCHEDULE_ARTIFACT = RUN_ARTIFACT_DIR / "sc0002_sr0001_schedule.csv"
SCHEDULE_COMMON_REL = "AxiomRift\\SC0002\\SR0001\\schedule\\sc0002_sr0001_schedule.csv"
STARTING_BALANCE_USD = 500.0
RESPONSE_MODE = "cross_surface_veto_inversion_schedule_replay"
MAX_HOLD_BARS = 10
MAGIC = 920001
...
def compile_sc0002_sr0001_ea(metaeditor_exe: Path = DEFAULT_METAEDITOR_EXE) -> CompileResult:
    ...
    log = PROJECT_ROOT / "artifacts" / "reports" / "SC0002_SR0001_mt5_compile.log"
...
def tester_config_path(mode: str = LOGIC_PARITY_MODE, output_scope: str | None = None) -> Path:
    ...
    return PROJECT_ROOT / "artifacts" / "reports" / "SC0002_SR0001_mt5_tester" / f"SC0002_SR0001_{suffix}_tester.ini"
...
def tester_report_path(mode: str = LOGIC_PARITY_MODE, output_scope: str | None = None) -> Path:
    ...
    output_scope = normalize_output_scope(output_scope)
    suffix = mode if output_scope is None else f"{mode}_{output_scope}"
    return PROJECT_ROOT / "artifacts" / "reports" / "SC0002_SR0001_mt5_tester" / f"SC0002_SR0001_mt5_{suffix}_report.htm"
...
def write_tester_config(
    ...
    config_dir = PROJECT_ROOT / "artifacts" / "reports" / "SC0002_SR0001_mt5_tester"
...
def parse_sc0002_sr0001_mt5(
    ...
    if mode == LOGIC_PARITY_MODE:
        ...
        upsert_artifact_lineage(
            "A-SC0002-SR0001-MT5-LOGIC-KPI",
            "mt5_logic_parity_kpi",
            "mt5_logic_parity",
            rel(MT5_LOGIC_KPI),
            sha256_file(MT5_LOGIC_KPI),
            [
                "campaigns/SC0002_accumulated_post_sc0001_negative_memory_synthesis/runs/SR0001/artifacts/sc0002_sr0001_schedule.csv",
                "configs/market.yaml",
            ],
        )
    else:
        upsert_artifact_lineage(
            "A-SC0002-SR0001-MT5-TICK-KPI",
            "mt5_tick_kpi",
            "mt5_tick",
            rel(MT5_TICK_KPI),
...
def build_mt5_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "mt5_probe_id": "MT5-SC0002-SR0001",
        ...
        "conditional_profiles": {
            ...
            "synthesis_schedule_replay_profile": {
                ...
                "fields": {
                    ...
                    "logic_boundary": "proxy_selected_cross_surface_veto_inversion_schedule_replayed_in_mt5_closed_bar_mode"
                    if mode == LOGIC_PARITY_MODE
        ...
        "deferred_with_reason": [
            {
                "field": "native_mql_cross_surface_veto_inversion_logic",
                "requirement_class": "deferred_with_reason",
                "reason": "SC0002 SR0001 replays the proxy schedule in MT5; native model or ONNX materialization is not claimed",
...
def record_sc0002_sr0001_parity() -> dict[str, object]:
    ...
    next_action = (
        "produce_sc0002_sr0001_mt5_tick_execution_evidence"
        if mechanical_ok
        else "repair_sc0002_sr0001_schedule_replay_parity"
    )
    ...
    payload = {
        ...
        "synthesis_id_when_applicable": WORK_UNIT_ID,
        "run_id": RUN_ID,
        "parity_id": "P-SC0002-SR0001",
        "proxy_id": "PX-SC0002-SR0001",
        "mt5_probe_id": "MT5-SC0002-SR0001",
        ...
        "mechanical_checks": {
            ...
            "cross_surface_schedule_exit_match": "passed" if exit_compare["reason_match_rate"] == 1.0 else "failed",
        ...
        "deferred_with_reason": [
            {
                ...
                "requirement_class": "deferred_with_reason",
                "reason": "logic parity uses schedule replay only; native cross-surface veto-inversion materialization is deferred until candidate quality exists",
    ...
    upsert_artifact_lineage(
        ...
        [
            "campaigns/SC0002_accumulated_post_sc0001_negative_memory_synthesis/runs/SR0001/kpi/proxy.json",
            "campaigns/SC0002_accumulated_post_sc0001_negative_memory_synthesis/runs/SR0001/kpi/mt5_logic_parity.json",
        ],
    )
...
def update_run_after_mt5(mode: str) -> None:
    ...
    evidence_paths["mt5_schedule_artifact"] = "artifacts/sc0002_sr0001_schedule.csv"
    mt5_plan = data.setdefault("mt5_probe_plan", {})
    mt5_plan["logic_parity_kpi_path"] = "kpi/mt5_logic_parity.json"
    mt5_plan["logic_parity_purpose"] = "proxy_cross_surface_veto_inversion_schedule_vs_ea_closed_bar_lifecycle_parity"
...
def update_gate_after_logic_parity(payload: dict[str, object], next_action: str, mechanical_ok: bool) -> None:
    ...
    for path in ("kpi/mt5_logic_parity.json", "kpi/proxy_vs_mt5_logic_parity.json", "artifacts/sc0002_sr0001_schedule.csv"):
...
def update_reentry_after_logic_parity(next_action: str) -> None:
    ...
    for item in (
        "produce_sc0002_sr0001_mt5_logic_parity_evidence",
        "record_sc0002_sr0001_proxy_vs_mt5_logic_parity_evidence",
    ):
...
def update_synthesis_queue_after_logic_parity(next_action: str, mechanical_ok: bool) -> None:
    ...
    for item in data.get("queue", []):
        if item.get("synthesis_run_id") == RUN_ID:
            ...
            item["last_completed_step"] = "produce_sc0002_sr0001_mt5_logic_parity_evidence"
            item["next_action"] = next_action
    SYNTHESIS_QUEUE.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")


def record_sc0002_sr0001_execution_divergence() -> dict[str, object]:
    ...
    payload = {
        ...
        "synthesis_id_when_applicable": WORK_UNIT_ID,
        "run_id": RUN_ID,
        "divergence_id": "ED-SC0002-SR0001",
        ...
        "conditional_profiles": {
            ...
            "synthesis_schedule_replay_profile": {
                ...
                "fields": {
                    "schedule_artifact_path": rel(SCHEDULE_ARTIFACT),
                    "risk_shape_source": "proxy_cross_surface_veto_inversion_schedule",
        ...
        "deferred_with_reason": [
            {
                ...
                "revisit_when": "after SC0002 SR0001 fold-isolated MT5 tick evidence",
    ...
    upsert_artifact_lineage(
        "A-SC0002-SR0001-EXEC-DIVERGENCE-KPI",
        ...
        [
            "campaigns/SC0002_accumulated_post_sc0001_negative_memory_synthesis/runs/SR0001/kpi/mt5_logic_parity.json",
            "campaigns/SC0002_accumulated_post_sc0001_negative_memory_synthesis/runs/SR0001/kpi/mt5_tick.json",
        ],
    )
...
def update_gate_after_execution_divergence(payload: dict[str, object]) -> None:
    ...
    data["decision"] = "defer_with_reason"
    data["next_action"] = "produce_sc0002_sr0001_fold_isolated_mt5_tick_kpi"
    data["execution_gate"] = {
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "produce_sc0002_sr0001_fold_isolated_mt5_tick_kpi",
        }
    ]
    GATE_REPORT.write_text(json.dumps(data, indent=2, sort_keys=True) + "\n", encoding="ascii")
...
def update_reentry_after_execution_divergence() -> None:
    ...
    for item in (
        ...
        if item not in completed:
            completed.append(item)
    next_action = "produce_sc0002_sr0001_fold_isolated_mt5_tick_kpi"
    completed = [item for item in completed if item != next_action]
    next_work["completed"] = completed
    next_work["tasks"] = [next_action]
    path.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def update_synthesis_queue_after_execution_divergence() -> None:
    ...
    for item in data.get("queue", []):
        if item.get("synthesis_run_id") == RUN_ID:
            ...
            item["last_completed_step"] = "record_sc0002_sr0001_execution_divergence"
            item["next_action"] = "produce_sc0002_sr0001_fold_isolated_mt5_tick_kpi"
    SYNTHESIS_QUEUE.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def build_mt5_tick_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": WORK_UNIT_ID,
        "run_id": RUN_ID,
        "mt5_probe_id": "MT5-SC0002-SR0001-BY-FOLD",
...
def build_execution_divergence_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": WORK_UNIT_ID,
        "run_id": RUN_ID,
        "divergence_id": "ED-SC0002-SR0001-BY-FOLD",
        "split_policy": "rolling_window_test_oos_fold_isolated",
        "split_registry": "registries/rolling_windows.yaml",
...
def update_gate_after_fold_isolated_evidence(
    ...
    for path in (
        ...
        "artifacts/sc0002_sr0001_schedule.csv",
    ):
        if path not in evidence_paths:
            evidence_paths.append(path)
    gate_data["decision"] = "defer_with_reason"
    gate_data["next_action"] = "review_sc0002_sr0001_tick_execution_kpi_and_closeout"
    gate_data["deferred_with_reason"] = [
        {
            ...
            "blocking_condition": "review_sc0002_sr0001_tick_execution_kpi_and_closeout",
            "revisit_when": "during SC0002 SR0001 closeout review",
        }
    ]
...
def update_reentry_after_fold_isolated() -> None:
    ...
    completed = list(next_work.get("completed") or [])
    for item in (
        "produce_sc0002_sr0001_fold_isolated_mt5_tick_kpi",
        "produce_sc0002_sr0001_fold_isolated_execution_divergence",
    ):
        if item not in completed:
            completed.append(item)
    next_action = "review_sc0002_sr0001_tick_execution_kpi_and_closeout"
    completed = [item for item in completed if item != next_action]
    next_work["completed"] = completed
    next_work["tasks"] = [next_action]
    path.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def update_synthesis_queue_after_fold_isolated() -> None:
    ...
    for item in data.get("queue", []):
        if item.get("synthesis_run_id") == RUN_ID:
            ...
            item["last_completed_step"] = "produce_sc0002_sr0001_fold_isolated_execution_divergence"
            item["next_action"] = "review_sc0002_sr0001_tick_execution_kpi_and_closeout"
    SYNTHESIS_QUEUE.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def upsert_artifact_lineage(
    ...
    records.append(
        {
            ...
            "sha256": file_hash,
            "produced_by": "axiom_rift.mt5.sc0002_sr0001_probe",
    ...
    if EXECUTION_DIVERGENCE_BY_FOLD_KPI.exists():
        ...
        deferred_next_action = "review_sc0002_sr0001_tick_execution_kpi_and_closeout"
        ...
        deferred_next_action = "produce_sc0002_sr0001_fold_isolated_mt5_tick_kpi"
    else:
        ...
        deferred_next_action = "produce_sc0002_sr0001_mt5_tick_execution_evidence"
    data["deferred_with_reason"] = [
        {
...
def run_sc0002_sr0001_logic_parity_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_sc0002_sr0001_tester(mode=LOGIC_PARITY_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_sc0002_sr0001_mt5(result, mode=LOGIC_PARITY_MODE, write_kpi=True)
    parity_payload = record_sc0002_sr0001_parity()
    return {
...
def run_sc0002_sr0001_mt5_tick_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_sc0002_sr0001_tester(mode=TICK_EXECUTION_MODE, timeout_seconds=timeout_seconds)
    tick_payload = parse_sc0002_sr0001_mt5(result, mode=TICK_EXECUTION_MODE, write_kpi=True)
    divergence_payload = record_sc0002_sr0001_execution_divergence()
    return {
...
def run_sc0002_sr0001_mt5_tick_by_fold_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    compile_sc0002_sr0001_ea()
    write_schedule_files()
    records: list[dict[str, Any]] = []
    for window in load_test_windows():
        from_date, to_date = tester_dates_for_window(window)
        logic_result = run_sc0002_sr0001_tester(
        ...
        logic_payload = parse_sc0002_sr0001_mt5(logic_result, mode=LOGIC_PARITY_MODE, write_kpi=False)
        tick_result = run_sc0002_sr0001_tester(
        ...
        tick_payload = parse_sc0002_sr0001_mt5(tick_result, mode=TICK_EXECUTION_MODE, write_kpi=False)
        records.append(
            {
                "window": window,
                "from_date": from_date,
    ...
    upsert_artifact_lineage(
        "A-SC0002-SR0001-MT5-TICK-BY-FOLD-KPI",
    ...
    upsert_artifact_lineage(
        "A-SC0002-SR0001-EXEC-DIVERGENCE-BY-FOLD-KPI",
        ...
        [
            "campaigns/SC0002_accumulated_post_sc0001_negative_memory_synthesis/runs/SR0001/kpi/mt5_tick_by_fold.json",
            "registries/rolling_windows.yaml",
        ],
    )
...
if __name__ == "__main__":
    result_payload = run_sc0002_sr0001_logic_parity_workflow()
