"""MT5 schedule-replay helpers for C0037 R0003."""
...
from axiom_rift.proxies.c0037_r0003_price_memory_density_tail_stability import (
    BASE_FRAME,
    ROLLING_WINDOWS,
    SplitWindow,
    Trade,
...
SCHEDULE_ARTIFACT = RUN_ARTIFACT_DIR / "c0037_r0003_schedule.csv"
SCHEDULE_COMMON_REL = "AxiomRift\\C0037\\R0003\\schedule\\c0037_r0003_schedule.csv"
STARTING_BALANCE_USD = starting_balance_usd()
LOGIC_PARITY_STARTING_BALANCE_USD = 100000.0
RESPONSE_MODE = "fold_local_price_memory_density_tail_stability_schedule_replay"
MAX_HOLD_BARS = 10
MAGIC = 1037003
...
def compile_c0037_r0003_ea(metaeditor_exe: Path | None = None) -> CompileResult:
    ...
    log = PROJECT_ROOT / "artifacts" / "reports" / "c0037_r0003_mt5_compile.log"
...
def tester_config_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    return PROJECT_ROOT / "artifacts" / "reports" / "c0037_r0003_mt5_tester" / f"c0037_r0003_{scoped_name(mode, output_scope)}_tester.ini"


def tester_report_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    return PROJECT_ROOT / "artifacts" / "reports" / "c0037_r0003_mt5_tester" / f"c0037_r0003_mt5_{scoped_name(mode, output_scope)}_report.htm"
...
def write_tester_config(
    ...
    config_dir = PROJECT_ROOT / "artifacts" / "reports" / "c0037_r0003_mt5_tester"
...
def parse_c0037_r0003_mt5(
    ...
    if write_kpi:
        ...
        if mode == LOGIC_PARITY_MODE:
            ...
            upsert_artifact_lineage(
                "A-C0037-R0003-MT5-LOGIC-KPI",
                "mt5_logic_parity_kpi",
                "mt5_logic_parity",
                rel(MT5_LOGIC_KPI),
                sha256_file(MT5_LOGIC_KPI),
                [
                    "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003/artifacts/c0037_r0003_schedule.csv",
                    "configs/market.yaml",
                ],
            )
        else:
            upsert_artifact_lineage(
                "A-C0037-R0003-MT5-TICK-KPI",
                "mt5_tick_kpi",
                "mt5_tick",
                rel(MT5_TICK_KPI),
...
def record_c0037_r0003_parity() -> dict[str, object]:
    ...
    next_action = "produce_c0037_r0003_mt5_tick_execution_evidence" if mechanical_ok else "repair_c0037_r0003_schedule_replay_parity"
    ...
    payload = {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": RUN_ID,
        "parity_id": "PVMT5-C0037-R0003",
        "proxy_id": "PX-C0037-R0003",
        "mt5_probe_id": "MT5-C0037-R0003",
        ...
        "conditional_profiles": {
            ...
            "intraday_price_memory_density_schedule_replay_profile": {
                ...
                "fields": {
                    ...
                    "logic_boundary": "proxy_selected_fold_local_price_memory_density_tail_stability_schedule_replayed_in_mt5_closed_bar_mode",
        ...
        "deferred_with_reason": [
            {
                ...
                "blocking_condition": "C0037 R0003 does not yet have fold-isolated tick evidence",
    ...
    upsert_artifact_lineage(
        "A-C0037-R0003-PROXY-VS-MT5-LOGIC-KPI",
        ...
        [
            "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003/kpi/proxy.json",
            "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003/kpi/mt5_logic_parity.json",
        ],
    )
...
def record_c0037_r0003_execution_divergence() -> dict[str, object]:
        ...
        "deferred_with_reason": [
            {
                ...
                "next_action": "produce_c0037_r0003_fold_isolated_mt5_tick_kpi",
            }
        ],
        "claim_boundary": {
            "claim_authority": False,
    ...
    upsert_artifact_lineage(
        "A-C0037-R0003-EXECUTION-DIVERGENCE-KPI",
        "diagnostic_output",
        ...
        [
            "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003/kpi/mt5_logic_parity.json",
            "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003/kpi/mt5_tick.json",
        ],
    )
...
def run_c0037_r0003_mt5_logic_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_c0037_r0003_tester(mode=LOGIC_PARITY_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_c0037_r0003_mt5(result)
    parity_payload = record_c0037_r0003_parity()
    return {
...
def run_c0037_r0003_mt5_tick_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_c0037_r0003_tester(mode=TICK_EXECUTION_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_c0037_r0003_mt5(result)
    divergence_payload = record_c0037_r0003_execution_divergence()
    return {
...
def run_c0037_r0003_mt5_tick_by_fold_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    compile_c0037_r0003_ea()
    write_schedule_files()
    records: list[dict[str, Any]] = []
    for window in load_test_windows():
        from_date, to_date = tester_dates_for_window(window)
        logic_result = run_c0037_r0003_tester(
        ...
        logic_payload = parse_c0037_r0003_mt5(logic_result, write_kpi=False)
        tick_result = run_c0037_r0003_tester(
        ...
        tick_payload = parse_c0037_r0003_mt5(tick_result, write_kpi=False)
        records.append(
            {
                "window": window,
                "from_date": from_date,
    ...
    upsert_artifact_lineage(
        "A-C0037-R0003-MT5-TICK-BY-FOLD-KPI",
    ...
    upsert_artifact_lineage(
        "A-C0037-R0003-EXECUTION-DIVERGENCE-BY-FOLD-KPI",
        "diagnostic_output",
        "execution_divergence_by_fold",
        rel(EXECUTION_DIVERGENCE_BY_FOLD_KPI),
        sha256_file(EXECUTION_DIVERGENCE_BY_FOLD_KPI),
        [
            "campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003/kpi/mt5_tick_by_fold.json",
            "registries/rolling_windows.yaml",
        ],
    )
...
def build_mt5_tick_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": RUN_ID,
        "mt5_probe_id": "MT5-C0037-R0003-BY-FOLD",
...
def build_execution_divergence_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": RUN_ID,
        "divergence_id": "ED-C0037-R0003-BY-FOLD",
        "split_policy": "rolling_window_test_oos_fold_isolated",
        "split_registry": "registries/rolling_windows.yaml",
...
def update_after_fold_isolated(tick_payload: dict[str, object], divergence_payload: dict[str, object]) -> None:
    next_action = "review_c0037_r0003_tick_execution_kpi_and_closeout"
    ...
    gate["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "during C0037 R0003 closeout review",
        }
    ]
    ...
    for item in (
        "produce_c0037_r0003_mt5_tick_execution_evidence",
        "record_c0037_r0003_execution_divergence",
        "produce_c0037_r0003_fold_isolated_mt5_tick_kpi",
        "produce_c0037_r0003_fold_isolated_execution_divergence",
    ):
    ...
    update_claim_state(
        operation_id="produce_c0037_r0003_fold_isolated_execution_divergence",
        source="campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003/kpi/execution_divergence_by_fold.json",
...
def update_run_after_mt5(mode: str) -> None:
    ...
    evidence["mt5_schedule_artifact"] = "artifacts/c0037_r0003_schedule.csv"
    if mode == LOGIC_PARITY_MODE:
...
def update_after_parity(payload: dict[str, object], next_action: str) -> None:
    ...
    update_campaign_next(next_action)
    update_reentry_next("record_c0037_r0003_proxy_vs_mt5_logic_parity_evidence", next_action)
    update_claim_state(
        operation_id="record_c0037_r0003_proxy_vs_mt5_logic_parity_evidence",
        source="campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003/kpi/proxy_vs_mt5_logic_parity.json",
...
def update_after_execution_divergence(payload: dict[str, object]) -> None:
    ...
    update_campaign_next(next_action)
    update_reentry_next("record_c0037_r0003_execution_divergence", next_action)
    update_claim_state(
        operation_id="record_c0037_r0003_execution_divergence",
        source="campaigns/C0037_intraday_price_memory_density_discovery/runs/R0003/kpi/execution_divergence.json",
...
def update_gate_next(status: str, next_action: str) -> None:
    ...
    for path in (
        ...
        "kpi/mt5_tick.json",
        "kpi/execution_divergence.json",
        "artifacts/c0037_r0003_schedule.csv",
    ):
        if path not in paths:
            paths.append(path)
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0037 R0003 fold-isolated MT5 evidence files contain measured results",
        }
    ]
...
def update_reentry_next(completed_item: str, next_action: str) -> None:
    ...
    next_work = data.setdefault("next_work", {})
    completed = list(next_work.get("completed") or [])
    for item in ("produce_c0037_r0003_proxy_evidence", completed_item):
        if item not in completed:
            completed.append(item)
    ...
    records.append(
        {
            ...
            "sha256": digest,
            "produced_by": "axiom_rift.mt5.c0037_r0003_probe",
