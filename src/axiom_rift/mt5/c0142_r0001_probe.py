"""MT5 schedule-replay helpers for C0142 R0001."""
...
from axiom_rift.proxies.c0142_r0001_intraday_two_sided_auction_release_continuation import (
    BASE_FRAME,
    ROLLING_WINDOWS,
    SplitWindow,
    Trade,
...
CAMPAIGN_ID = "C0142"
RUN_ID = "R0001"
CAMPAIGN_ROOT = PROJECT_ROOT / "campaigns" / "C0142_intraday_two_sided_auction_release_continuation_discovery"
...
SELECTED = CAMPAIGN_ROOT / "selected.yaml"
...
SCHEDULE_ARTIFACT = RUN_ARTIFACT_DIR / "c0142_r0001_schedule.csv"
SCHEDULE_COMMON_REL = "AxiomRift\\C0142\\R0001\\schedule\\c0142_r0001_schedule.csv"
STARTING_BALANCE_USD = starting_balance_usd()
LOGIC_PARITY_STARTING_BALANCE_USD = 100000.0
RESPONSE_MODE = "fold_local_intraday_two_sided_auction_release_continuation_schedule_replay"
MAX_HOLD_BARS = 10
MAGIC = 1128001
TESTER_FROM_DATE = "2024.02.01"
TESTER_TO_DATE = "2026.05.01"
WORK_UNIT_REL = "campaigns/C0142_intraday_two_sided_auction_release_continuation_discovery"
RUN_REL = f"{WORK_UNIT_REL}/runs/R0001"
NEXT_MAJOR_ACTION = "choose_c0143_new_major_hypothesis_after_c0142_closeout"
PREVIOUS_CAMPAIGN_REL = "campaigns/C0141_intraday_range_overextension_snapback_reversal_discovery"
C0142_CLOSE_REASON = (
    "negative_memory_recorded_after_proxy_mt5_logic_parity_tick_execution_divergence_and_fold_isolated_validation"
)
C0142_HYPOTHESIS_TESTED = (
    "fold-local intraday two sided auction release continuation rank over two-sided probe balance, "
    "equilibrium compression, release impulse, VWAP release alignment, day-open release alignment, "
    "prior-close release alignment, range-expansion efficiency, close-drive commitment, "
    "counter-wick containment, participation reengagement, pre-release chop, session room after "
    "release, prior-day break context, session maturity, and spread-stress-inverse features"
)
C0142_FEATURE_SUMMARY = (
    "two sided probe balance; equilibrium compression; release impulse; vwap release alignment; "
    "day open release alignment; prior close release alignment; range expansion efficiency; "
    "close drive commitment; counter wick containment; participation reengagement; pre release "
    "chop; session room after release; prior day break context; session maturity; "
    "spread stress inverse"
)
...
def compile_c0142_r0001_ea(metaeditor_exe: Path | None = None) -> CompileResult:
    ...
    log = PROJECT_ROOT / "artifacts" / "reports" / "c0142_r0001_mt5_compile.log"
...
def tester_config_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    return PROJECT_ROOT / "artifacts" / "reports" / "c0142_r0001_mt5_tester" / f"c0142_r0001_{scoped_name(mode, output_scope)}_tester.ini"


def tester_report_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    return PROJECT_ROOT / "artifacts" / "reports" / "c0142_r0001_mt5_tester" / f"c0142_r0001_mt5_{scoped_name(mode, output_scope)}_report.htm"
...
def write_tester_config(
    ...
    config_dir = PROJECT_ROOT / "artifacts" / "reports" / "c0142_r0001_mt5_tester"
    ...
    if write_kpi:
        ...
        if mode == LOGIC_PARITY_MODE:
            upsert_artifact_lineage(
                "A-C0142-R0001-MT5-SCHEDULE",
                "mt5_schedule_csv",
                "mt5_logic_parity_input",
                rel(SCHEDULE_ARTIFACT),
                sha256_file(SCHEDULE_ARTIFACT),
                ["campaigns/C0142_intraday_two_sided_auction_release_continuation_discovery/runs/R0001/kpi/proxy.json"],
            )
            upsert_artifact_lineage(
                "A-C0142-R0001-MT5-LOGIC-KPI",
                "mt5_logic_parity_kpi",
                "mt5_logic_parity",
                rel(MT5_LOGIC_KPI),
                sha256_file(MT5_LOGIC_KPI),
                [
                    "campaigns/C0142_intraday_two_sided_auction_release_continuation_discovery/runs/R0001/artifacts/c0142_r0001_schedule.csv",
                    "configs/market.yaml",
                ],
            )
        else:
            upsert_artifact_lineage(
                "A-C0142-R0001-MT5-TICK-KPI",
                "mt5_tick_kpi",
                "mt5_tick",
                rel(MT5_TICK_KPI),
    ...
    return {
        ...
        **runtime_payload_fields(),
        ...
        "conditional_profiles": {
            ...
            "intraday_two_sided_auction_release_continuation_schedule_profile": {
                "applies": True,
                "fields": {
                    "intraday_two_sided_auction_release_continuation_source": "rolling_train_is_proxy_schedule",
        ...
        "deferred_with_reason": [
            {
                "field": "native_mql_or_onnx_intraday_two_sided_auction_release_continuation_surface",
...
def record_c0142_r0001_parity() -> dict[str, object]:
    ...
    next_action = "produce_c0142_r0001_mt5_tick_execution_evidence" if mechanical_ok else "repair_c0142_r0001_schedule_replay_parity"
    ...
    payload = {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": RUN_ID,
        "parity_id": "PVMT5-C0142-R0001",
        "proxy_id": "PX-C0142-R0001",
        "mt5_probe_id": "MT5-C0142-R0001",
        ...
        "mechanical_checks": {
            ...
            "intraday_two_sided_auction_release_continuation_schedule_match": "passed" if entry_compare["key_match_rate"] == 1.0 else "failed",
        ...
        "conditional_profiles": {
            ...
            "intraday_two_sided_auction_release_continuation_schedule_replay_profile": {
                "applies": True,
                "fields": {
                    "schedule_artifact_path": rel(SCHEDULE_ARTIFACT),
                    "schedule_hash": sha256_file(SCHEDULE_ARTIFACT) if SCHEDULE_ARTIFACT.exists() else None,
                    "logic_boundary": "proxy_selected_fold_local_intraday_two_sided_auction_release_continuation_schedule_replayed_in_mt5_closed_bar_mode",
        ...
        "deferred_with_reason": [
            {
                ...
                "requirement_class": "deferred_with_reason",
                "reason": "logic parity uses schedule replay only; native intraday two sided auction release continuation materialization is deferred until candidate quality exists",
                "blocking_condition": "C0142 R0001 does not yet have fold-isolated tick evidence",
    ...
    upsert_artifact_lineage(
        "A-C0142-R0001-PROXY-VS-MT5-LOGIC-KPI",
        ...
        [
            "campaigns/C0142_intraday_two_sided_auction_release_continuation_discovery/runs/R0001/kpi/proxy.json",
            "campaigns/C0142_intraday_two_sided_auction_release_continuation_discovery/runs/R0001/kpi/mt5_logic_parity.json",
        ],
    )
...
def record_c0142_r0001_execution_divergence() -> dict[str, object]:
        ...
        "deferred_with_reason": [
            {
                ...
                "next_action": "produce_c0142_r0001_fold_isolated_mt5_tick_kpi",
            }
        ],
        "claim_boundary": {
            "claim_authority": False,
    ...
    upsert_artifact_lineage(
        "A-C0142-R0001-EXECUTION-DIVERGENCE-KPI",
        "diagnostic_output",
        ...
        [
            "campaigns/C0142_intraday_two_sided_auction_release_continuation_discovery/runs/R0001/kpi/mt5_logic_parity.json",
            "campaigns/C0142_intraday_two_sided_auction_release_continuation_discovery/runs/R0001/kpi/mt5_tick.json",
        ],
    )
...
def run_c0142_r0001_mt5_logic_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_c0142_r0001_tester(mode=LOGIC_PARITY_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_c0142_r0001_mt5(result)
    parity_payload = record_c0142_r0001_parity()
    return {
...
def run_c0142_r0001_mt5_tick_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_c0142_r0001_tester(mode=TICK_EXECUTION_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_c0142_r0001_mt5(result)
    divergence_payload = record_c0142_r0001_execution_divergence()
    return {
...
def run_c0142_r0001_mt5_tick_by_fold_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    compile_c0142_r0001_ea()
    write_schedule_files()
    records: list[dict[str, Any]] = []
    for window in load_test_windows():
        from_date, to_date = tester_dates_for_window(window)
        logic_result = run_or_reuse_c0142_r0001_tester(
        ...
        logic_payload = parse_c0142_r0001_mt5(logic_result, write_kpi=False)
        tick_result = run_or_reuse_c0142_r0001_tester(
        ...
        tick_payload = parse_c0142_r0001_mt5(tick_result, write_kpi=False)
        records.append(
            {
                "window": window,
                "from_date": from_date,
    ...
    upsert_artifact_lineage(
        "A-C0142-R0001-MT5-TICK-BY-FOLD-KPI",
    ...
    upsert_artifact_lineage(
        "A-C0142-R0001-EXECUTION-DIVERGENCE-BY-FOLD-KPI",
        "diagnostic_output",
        "execution_divergence_by_fold",
        rel(EXECUTION_DIVERGENCE_BY_FOLD_KPI),
        sha256_file(EXECUTION_DIVERGENCE_BY_FOLD_KPI),
        [
            "campaigns/C0142_intraday_two_sided_auction_release_continuation_discovery/runs/R0001/kpi/mt5_tick_by_fold.json",
            "registries/rolling_windows.yaml",
        ],
    )
...
def run_or_reuse_c0142_r0001_tester(
    mode: str,
    timeout_seconds: int,
    from_date: str,
    to_date: str,
    ...
    existing = tester_result_for_scope(mode, from_date, to_date, output_scope)
    if completed_mt5_output_exists(existing):
        return existing
    return run_c0142_r0001_tester(
        mode=mode,
        timeout_seconds=timeout_seconds,
        from_date=from_date,
        to_date=to_date,
...
def tester_result_for_scope(mode: str, from_date: str, to_date: str, output_scope: str) -> TesterResult:
    mode = normalize_mt5_mode(mode)
    output_scope = normalize_output_scope(output_scope) or output_scope
    return TesterResult(
        config=tester_config_path_for_mode(mode, output_scope),
        report=tester_report_path_for_mode(mode, output_scope),
        common_dir=common_output_dir(mode, output_scope),
...
def completed_mt5_output_exists(result: TesterResult) -> bool:
    if not result.status_csv.exists() or not result.events_csv.exists() or not result.deals_csv.exists():
        return False
    try:
        status = read_status_csv(result.status_csv)
        assert_mt5_mode_matches_status(result.mode, status, result.output_scope)
    except Exception:
        return False
...
def build_mt5_tick_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": RUN_ID,
        "mt5_probe_id": "MT5-C0142-R0001-BY-FOLD",
...
def build_execution_divergence_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": RUN_ID,
        "divergence_id": "ED-C0142-R0001-BY-FOLD",
        "split_policy": "rolling_window_test_oos_fold_isolated",
        "split_registry": "registries/rolling_windows.yaml",
...
def update_after_fold_isolated(tick_payload: dict[str, object], divergence_payload: dict[str, object]) -> None:
    next_action = "review_c0142_r0001_tick_execution_kpi_and_closeout"
    ...
    gate["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "during C0142 R0001 closeout review",
        }
    ]
    ...
    for item in (
        "produce_c0142_r0001_mt5_tick_execution_evidence",
        "record_c0142_r0001_execution_divergence",
        "produce_c0142_r0001_fold_isolated_mt5_tick_kpi",
        "produce_c0142_r0001_fold_isolated_execution_divergence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["completed"] = [item for item in completed if item != next_action]
    next_work["tasks"] = [next_action]
    data["active_run"] = "campaigns/C0142_intraday_two_sided_auction_release_continuation_discovery/runs/R0001"
    ...
    update_claim_state(
        operation_id="produce_c0142_r0001_fold_isolated_execution_divergence",
        source="campaigns/C0142_intraday_two_sided_auction_release_continuation_discovery/runs/R0001/kpi/execution_divergence_by_fold.json",
...
def update_run_after_mt5(mode: str) -> None:
    ...
    evidence["mt5_schedule_artifact"] = "artifacts/c0142_r0001_schedule.csv"
    if mode == LOGIC_PARITY_MODE:
...
def update_after_parity(payload: dict[str, object], next_action: str) -> None:
    ...
    update_campaign_next(next_action)
    update_reentry_next("record_c0142_r0001_proxy_vs_mt5_logic_parity_evidence", next_action)
    update_claim_state(
        operation_id="record_c0142_r0001_proxy_vs_mt5_logic_parity_evidence",
        source="campaigns/C0142_intraday_two_sided_auction_release_continuation_discovery/runs/R0001/kpi/proxy_vs_mt5_logic_parity.json",
...
def update_after_execution_divergence(payload: dict[str, object]) -> None:
    ...
    update_campaign_next(next_action)
    update_reentry_next("record_c0142_r0001_execution_divergence", next_action)
    update_claim_state(
        operation_id="record_c0142_r0001_execution_divergence",
        source="campaigns/C0142_intraday_two_sided_auction_release_continuation_discovery/runs/R0001/kpi/execution_divergence.json",
...
def update_gate_next(status: str, next_action: str) -> None:
    ...
    for path in (
        ...
        "kpi/mt5_tick.json",
        "kpi/execution_divergence.json",
        "artifacts/c0142_r0001_schedule.csv",
    ):
        if path not in paths:
            paths.append(path)
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0142 R0001 fold-isolated MT5 evidence files contain measured results",
        }
    ]
...
def update_reentry_next(completed_item: str, next_action: str) -> None:
    ...
    next_work = data.setdefault("next_work", {})
    completed = list(next_work.get("completed") or [])
    for item in ("produce_c0142_r0001_proxy_evidence", completed_item):
        if item not in completed:
            completed.append(item)
...
def update_claim_state(operation_id: str, source: str, evidence_status: str, next_action: str, extra: dict[str, object]) -> None:
    ...
    data["active_campaign"] = "campaigns/C0142_intraday_two_sided_auction_release_continuation_discovery"
    data["active_run"] = "campaigns/C0142_intraday_two_sided_auction_release_continuation_discovery/runs/R0001"
    latest = {
        "id": operation_id,
        "status": "completed",
...
def closeout_evidence_paths() -> list[str]:
    return [
        ...
        "run_manifest.json",
        "artifacts/c0142_r0001_proxy_trades.csv",
        "artifacts/c0142_r0001_two_sided_auction_release_continuation_summary.json",
        "kpi/proxy.json",
        "kpi/mt5_logic_parity.json",
        "kpi/proxy_vs_mt5_logic_parity.json",
        "kpi/mt5_tick.json",
...
def C0142_closeout_kpi_summary() -> dict[str, Any]:
    ...
    return {
        ...
        "long_trade_count_proxy": proxy_summary.get("long_trade_count"),
        "short_trade_count_proxy": proxy_summary.get("short_trade_count"),
        ...
        "exit_time_direction_match_rate": parity_required.get("exit_time_direction_match_rate"),
        ...
        "tick_minus_logic_trade_count": divergence_required.get("tick_minus_logic_trade_count"),
        ...
        "missing_required_fold_count": tick_by_fold_required.get("missing_required_fold_count"),
        ...
        "worst_drawdown_fold_id": tick_by_fold_required.get("worst_drawdown_fold_id"),
        "execution_divergence_by_fold_status": divergence_by_fold_required.get(
            "execution_divergence_by_fold_status"
        ),
        "divergence_recorded_fold_count": divergence_by_fold_required.get("divergence_recorded_fold_count"),
        ...
        "total_tick_minus_logic_trade_count": divergence_by_fold_required.get(
            "total_tick_minus_logic_trade_count"
        ),
        ...
        "remaining_work_classification": (
            "no_candidate; further C0142 work would be adjacent threshold, stop, target, hold, session, "
            "activity, spread, capital, or two-sided-auction-release-continuation feature nudge tuning"
        ),
    }


def C0142_negative_memory_asset(summary: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "runtime_authority": False,
        "onnx_ready": False,
        "hypothesis_tested": C0142_HYPOTHESIS_TESTED,
        ...
        "next_boundary": (
            "C0142 R0001 shows the tested two sided auction release continuation surface is not a candidate as implemented; "
            "remaining C0142 variants would be adjacent tuning, so next work should choose a new major C0143 "
        ...
        "supporting_lessons": [
            {
                ...
                "summary": (
                    f"R0001 produced {summary.get('profitable_fold_count')} profitable and "
                    f"{summary.get('losing_fold_count')} losing fold-isolated MT5 tick folds, total tick net PnL "
                    f"{summary.get('total_tick_net_pnl')}, and worst fold {summary.get('worst_fold_id')} net PnL "
                    f"{summary.get('worst_fold_net_pnl')}."
                ),
            },
            {
                ...
                "summary": (
                    "Schedule replay mechanical parity passed with mismatch_count "
                    f"{summary.get('mismatch_count')}, so the weak result is not explained by proxy-vs-MT5 "
                    "logic mismatch."
                ),
            },
            {
                ...
                "summary": (
                    "Tick execution differed from closed-bar logic by "
                    f"{summary.get('total_tick_minus_logic_net_pnl')} total net PnL across "
                    f"{summary.get('tick_better_fold_count')} better and "
                    f"{summary.get('tick_worse_fold_count')} worse folds; closeout remains bounded to "
                    "fold-isolated MT5 tick evidence."
                ),
            },
            {
                ...
                "summary": (
                    "Native MQL signal logic, ONNX materialization, cost/slippage hardening, final sizing, "
                    "and historical lineage debt remain unresolved; no stronger claim is created."
                ),
            },
        ],
    }
...
def C0142_closeout_review(closed_at: str) -> dict[str, Any]:
    summary = C0142_closeout_kpi_summary()
    return {
        "status": "closed_no_candidate",
        "basis": "rolling_window_fold_isolated_mt5_tick_and_execution_divergence",
        "close_reason": C0142_CLOSE_REASON,
        "closed_at_utc": closed_at,
        "required_kpi_summary": summary,
        "negative_memory": C0142_negative_memory_asset(summary),
...
def update_run_manifest_for_closeout(closeout_review: dict[str, Any], closed_at: str) -> None:
    ...
    data["surfaces"] = {
        "model": {
            "status": "variable",
            "summary": "fold-local linear intraday two sided auction release continuation rank",
        },
        "feature": {
            "status": "variable",
            "summary": C0142_FEATURE_SUMMARY,
        },
        "trade_logic": {
            "status": "variable",
            "summary": (
                "dual-direction top fold-local two-sided-auction-release-continuation scores per active day "
                "using next-bar-open fixed-lot discovery entries"
            ),
        },
        "label": {
            "status": "variable",
            "summary": "target-first quality after two sided auction release continuation",
        },
    }
    evidence = data.setdefault("evidence_paths", {})
    evidence.update(
        {
            ...
            "proxy_trade_artifact": "artifacts/c0142_r0001_proxy_trades.csv",
            "two_sided_auction_release_continuation_summary": "artifacts/c0142_r0001_two_sided_auction_release_continuation_summary.json",
            "mt5_schedule_artifact": "artifacts/c0142_r0001_schedule.csv",
...
def update_gate_report_for_closeout(closeout_review: dict[str, Any]) -> None:
    ...
    data["gate_report_id"] = "G-C0142-R0001"
    data["status"] = "closed_no_candidate"
    data["gate_status"] = "closed_no_candidate"
    data["required_kpi_status"] = "completed"
...
def update_selected_for_closeout(closeout_review: dict[str, Any]) -> None:
    data = yaml.safe_load(SELECTED.read_text(encoding="ascii"))
    summary = closeout_review["required_kpi_summary"]
    data["status"] = "negative_memory_recorded_not_selected_closed"
    data["selection_status"] = "not_selected"
    data["selected"] = False
    data["selected_reason"] = (
        "C0142 R0001 closed as no candidate: observed activity and proxy, MT5 logic, "
        "aggregate MT5 tick, and fold-isolated MT5 evidence did not establish a candidate."
    )
    data["next_required_action"] = NEXT_MAJOR_ACTION
    data["claim_boundary"] = claim_boundary_payload()
    data["latest_run_state"] = {
        "status": "closed_no_candidate",
        "source": "runs/R0001/gate_report.json",
    ...
    data["negative_memory"] = {
        "latest_run": "runs/R0001",
        "status": "recorded",
        "evidence_path": "runs/R0001/gate_report.json",
        "basis": "rolling_window_fold_isolated_mt5_tick_plus_execution_divergence",
...
def update_campaign_for_closeout(closeout_review: dict[str, Any], closed_at: str) -> None:
    data = yaml.safe_load(CAMPAIGN.read_text(encoding="ascii"))
    data["status"] = "closed_no_candidate"
    data["closed_at_utc"] = closed_at
    run_index = data.setdefault("run_index", {})
    run_index["active_run"] = None
    closed_runs = list(run_index.get("closed_runs") or [])
    if "runs/R0001" not in closed_runs:
        closed_runs.append("runs/R0001")
    run_index["closed_runs"] = closed_runs
    run_index["next_run_candidate"] = {
        "run_id": None,
        "direction": NEXT_MAJOR_ACTION,
        "reason": (
            "C0142 R0001 closed no-candidate after proxy, MT5 logic parity, MT5 tick, execution divergence, "
            "and fold-isolated MT5 evidence. Further work inside C0142 would be adjacent tuning."
        ),
        "status": "campaign_closed_no_candidate",
    }
    data["synthesis_readiness"] = {
        "check_required_at_closeout": True,
        "result": "candidate_later",
        "usable_ingredient_group_count": 37,
        "readiness_notes": [
            "C0049, C0050, C0051, C0052, C0053, C0054, C0055, C0056, C0057, C0058, C0059, C0060, C0061, C0062, C0063, C0064, C0065, C0066, C0070, C0071, C0072, C0073, C0084, C0085, C0086, C0087, C0088, C0089, C0096, C0097, C0098, C0099, C0100, C0102, C0103, C0104, C0107, C0109, C0110, C0114, C0115, C0116, C0117, C0118, C0119, C0120, C0121, C0122, C0123, C0124, C0125, C0126, C0127, C0128, C0131, C0132, C0133, and C0142 provide post-SC0009 negative-memory ingredients, but default usable ingredient minimums are not met.",
            "Next work can remain a new major C0143 campaign hypothesis, not C0142 threshold or stop/target tuning.",
        ],
    }
    data["closeout"] = {
        "status": "closed_no_candidate",
...
def update_reentry_for_closeout(closeout_review: dict[str, Any]) -> None:
    data = yaml.safe_load(REENTRY.read_text(encoding="ascii"))
    summary = closeout_review["required_kpi_summary"]
    ...
    for item in (
        ...
        "produce_c0142_r0001_fold_isolated_mt5_tick_kpi",
        "produce_c0142_r0001_fold_isolated_execution_divergence",
        "review_c0142_r0001_tick_execution_kpi_and_closeout",
        "close_c0142_r0001_no_candidate",
        "close_c0142_no_candidate_after_r0001",
    ):
        if item not in completed:
            completed.append(item)
...
def update_decision_cursor_for_closeout(closeout_review: dict[str, Any]) -> None:
    ...
    data["active_synthesis"] = None
    data["next_required_action"] = NEXT_MAJOR_ACTION
    resolved = data.setdefault("terminal_evidence_priority", {}).setdefault("resolved_from", {})
    resolved["active_campaign"] = WORK_UNIT_REL
    resolved["active_campaign_status"] = "closed_no_candidate"
    resolved["active_run"] = RUN_REL
    resolved["active_run_status"] = "closed_no_candidate"
    resolved["previous_campaign"] = PREVIOUS_CAMPAIGN_REL
    resolved["previous_campaign_status"] = "closed_no_candidate"
    ...
    repo_scope["note"] = (
        ...
        "current_c0142_r0001_closeout_makes_no_selection_promotion_handoff_or_reproducibility_claims"
    )
    for item in repo_scope.get("nonblocking_debts") or []:
        if isinstance(item, dict):
            item["role"] = "repo_state_debt_not_current_c0142_r0001_closeout"
    ...
    current.update(
        {
            "source_campaign": WORK_UNIT_REL,
            "active_campaign": None,
            "active_synthesis": None,
            "active_run": None,
            "active_run_status": "closed_no_candidate",
            "campaign_status": "closed_no_candidate",
            "evidence_status": "closed_no_candidate",
            ...
            "next_required_action": NEXT_MAJOR_ACTION,
            "note": (
                "C0142 R0001 and C0142 closed no-candidate; two sided auction release continuation surface produced "
    ...
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0142 R0001 closed no-candidate with negative memory recorded.",
        },
        {
            "path": f"{RUN_REL}/kpi/mt5_tick_by_fold.json",
            "role": "fold_isolated_mt5_tick_kpi",
            "summary": "Fold-isolated MT5 tick evidence completed for closeout review.",
        },
        {
            "path": f"{WORK_UNIT_REL}/campaign.yaml",
            "role": "closed_campaign_manifest",
            "summary": "C0142 closed no-candidate; next work is a new major C0143 hypothesis.",
        },
    ]
...
def update_work_unit_registry_for_closeout() -> None:
    ...
    closed_units = list(campaign_stream.get("closed_work_units") or [])
    if WORK_UNIT_REL not in closed_units:
        closed_units.append(WORK_UNIT_REL)
    campaign_stream["closed_work_units"] = closed_units
    WORK_UNIT_REGISTRY.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False, width=88), encoding="ascii")
...
def append_decision_registry_for_closeout(closeout_review: dict[str, Any]) -> None:
    ...
    if "dec_20260708_close_c0142_r0001_no_candidate" not in existing:
        decisions.append(
            {
                "decision_id": "dec_20260708_close_c0142_r0001_no_candidate",
                "created_local_date": local_date,
                "status": "active",
                "decision": "close_c0142_r0001_no_candidate",
                "refines": [
                    "dec_20260708_produce_c0142_r0001_proxy_evidence",
                    "dec_20260701_mandatory_mt5_paired_run_validation",
                ],
                "rationale": [
                    "c0142_r0001_completed_proxy_mt5_logic_parity_proxy_vs_mt5_parity_mt5_tick_execution_divergence_and_fold_isolated_tick_and_divergence_evidence",
    ...
    if "dec_20260708_close_c0142_no_candidate_after_r0001" not in existing:
        decisions.append(
            {
                "decision_id": "dec_20260708_close_c0142_no_candidate_after_r0001",
                "created_local_date": local_date,
                "status": "active",
                "decision": "close_c0142_no_candidate_after_r0001",
                "refines": [
                    "dec_20260708_close_c0142_r0001_no_candidate",
                    "dec_20260708_open_c0142_intraday_two_sided_auction_release_continuation_discovery",
                ],
                "rationale": [
                    "C0142_two_sided_auction_release_continuation_boundary_was_answered_by_r0001_fold_isolated_mt5_evidence",
                    "remaining_C0142_work_would_be_adjacent_threshold_window_stop_target_hold_session_activity_spread_capital_or_feature_nudge_tuning",
                    "synthesis_readiness_candidate_later_because_post_sc0009_usable_ingredient_count_is_below_default_minimum",
                    "next_work_is_choose_c0143_new_major_hypothesis_after_c0142_closeout",
                    "no_selected_economics_runtime_materialization_onnx_promotion_or_live_claim_is_created",
                ],
                "claim_boundary": claim_boundary_payload(),
            }
    ...
    if not isinstance(data, dict):
        data = decisions
    DECISION_REGISTRY.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False, width=88), encoding="ascii")


def C0142_candidate_evidence_asset(summary: dict[str, Any]) -> dict[str, Any]:
    return {
        "asset_type": "candidate_evidence",
        "candidate_status": "candidate_evidence_retained_not_selected",
        "selection_status": "not_selected",
        "economics_pass": False,
        "runtime_authority": False,
        "onnx_ready": False,
        ...
        "evidence_paths": closeout_evidence_paths(),
        "reason_retained": (
            f"R0001 retained target-band activity with {summary.get('entries_per_active_day')} entries per "
            f"active day, proxy PF {summary.get('proxy_profit_factor')}, MT5 tick PF "
            f"{summary.get('mt5_tick_profit_factor')}, total fold-isolated tick net PnL "
            f"{summary.get('total_tick_net_pnl')}, and {summary.get('profitable_fold_count')} of "
            f"{summary.get('fold_count')} profitable folds."
        ),
        "reason_not_selected": (
            "C0142 has useful candidate evidence, but active selected, economics-pass, runtime, "
            "materialization, ONNX, promotion, handoff, and live-readiness authority is absent; cost/slippage "
            "robustness, final sizing, native signal materialization, ONNX artifacts, and historical lineage "
            "debt remain unresolved."
        ),
        "next_boundary": (
            "C0142 should close with retained candidate evidence only. Further C0142 work would be adjacent "
            "threshold, stop/target, hold, session, activity, spread, capital, materialization, ONNX, or retry "
            "hardening rather than a new non-adjacent discovery run."
        ),
        "supporting_lessons": [
            {
                "asset_type": "candidate_evidence",
                ...
                "summary": (
                    f"R0001 produced {summary.get('profitable_fold_count')} profitable and "
                    f"{summary.get('losing_fold_count')} losing fold-isolated MT5 tick folds, total tick net "
                    f"PnL {summary.get('total_tick_net_pnl')}, and worst fold "
                    f"{summary.get('worst_fold_id')} net PnL {summary.get('worst_fold_net_pnl')}."
                ),
            },
            {
                "asset_type": "parity_lesson",
                ...
                "summary": (
                    "Schedule replay mechanical parity passed with mismatch_count "
                    f"{summary.get('mismatch_count')}; retained evidence is not explained by proxy-vs-MT5 "
                    "logic mismatch."
                ),
            },
            {
                ...
                "summary": (
                    ...
                    f"{summary.get('total_tick_minus_logic_net_pnl')} total net PnL across "
                    f"{summary.get('tick_better_fold_count')} better and "
                    f"{summary.get('tick_worse_fold_count')} worse folds."
                ),
            },
            {
                "asset_type": "evidence_gap",
                "source": "gate_report.json",
                "summary": (
                    "R0001 lacks selected-contract authority, cost/slippage robustness, native signal "
                    "materialization, ONNX artifacts, final sizing, and lineage-debt repair."
                ),
            },
        ],
    }
...
def C0142_candidate_closeout_review(closed_at: str) -> dict[str, Any]:
    summary = C0142_closeout_kpi_summary()
    summary["remaining_work_classification"] = (
        "candidate_evidence_retained_not_selected; C0142 R0001 produced target-band activity and "
        "profitable fold-isolated MT5 tick evidence, but remains outside selected, economics-pass, runtime, "
        "materialization, ONNX, promotion, handoff, reproducibility, or live-readiness authority"
    )
    return {
        "status": "closed_with_candidate_evidence",
        "basis": "rolling_window_fold_isolated_mt5_tick_and_execution_divergence",
        "close_reason": (
            "candidate_evidence_retained_not_selected_after_proxy_mt5_logic_parity_tick_execution_divergence_"
            "and_fold_isolated_validation"
        ),
        "closed_at_utc": closed_at,
        "required_kpi_summary": summary,
        "candidate_evidence": C0142_candidate_evidence_asset(summary),
        "claim_boundary": claim_boundary_payload(),
    }


def update_run_manifest_for_candidate_closeout(closeout_review: dict[str, Any], closed_at: str) -> None:
    data = json.loads(RUN_MANIFEST.read_text(encoding="ascii"))
    data["work_unit_id"] = CAMPAIGN_ID
    data["status"] = "closed_with_candidate_evidence"
    data["gate_status"] = "closed_with_candidate_evidence"
    data["next_required_action"] = NEXT_MAJOR_ACTION
    data["closed_at_utc"] = closed_at
    data["closeout_review"] = closeout_review
    data["surfaces"] = {
    ...
    evidence.update(
        {
            ...
            "execution_divergence_kpi": "kpi/execution_divergence.json",
            "proxy_trade_artifact": "artifacts/c0142_r0001_proxy_trades.csv",
            "two_sided_auction_release_continuation_summary": (
                "artifacts/c0142_r0001_two_sided_auction_release_continuation_summary.json"
            ),
            "mt5_schedule_artifact": "artifacts/c0142_r0001_schedule.csv",
            "mt5_tick_by_fold_kpi": "kpi/mt5_tick_by_fold.json",
    ...
    mt5_plan = data.setdefault("mt5_probe_plan", {})
    mt5_plan["closeout_status"] = "closed_with_candidate_evidence"
    mt5_plan["next_required_action"] = NEXT_MAJOR_ACTION
    data["next_required_action"] = NEXT_MAJOR_ACTION
    data["claim_boundary"] = claim_boundary_payload()
    RUN_MANIFEST.write_text(json.dumps(data, indent=2, sort_keys=True) + "\n", encoding="ascii")
...
def update_gate_report_for_candidate_closeout(closeout_review: dict[str, Any]) -> None:
    data = json.loads(GATE_REPORT.read_text(encoding="ascii"))
    summary = closeout_review["required_kpi_summary"]
    data["schema"] = "axiom_rift_gate_report_v2"
    data["gate_report_id"] = "G-C0142-R0001"
    ...
    data["gate_status"] = "closed_with_candidate_evidence"
    data["required_kpi_status"] = "completed"
    data["decision"] = "close_with_candidate_evidence"
    data["role"] = "lightweight_decision_receipt"
    data["next_action"] = NEXT_MAJOR_ACTION
    data["deferred_with_reason"] = []
    data["closeout_review"] = closeout_review
    ...
    data["must_not"] = [
        "claim_selected",
        "claim_economics_pass",
        "claim_runtime_authority",
        "claim_materialization_ready",
        "claim_onnx_ready",
        "claim_promotion_ready",
        "claim_live_ready",
        "skip_mt5_when_proxy_is_weak",
        "close_from_aggregate_mt5",
        "close_from_broken_code",
...
def update_selected_for_candidate_closeout(closeout_review: dict[str, Any]) -> None:
    data = yaml.safe_load(SELECTED.read_text(encoding="ascii"))
    summary = closeout_review["required_kpi_summary"]
    data["status"] = "candidate_evidence_retained_not_selected_closed"
    data["selection_status"] = "not_selected"
    data["selected"] = False
    data["active_run"] = None
    data["selected_reason"] = (
        "C0142 R0001 retained useful candidate evidence, but it is not selected because selected-contract, "
        "economics-pass, runtime, materialization, ONNX, promotion, handoff, and live-readiness authority is "
        "absent."
    )
    data["next_required_action"] = NEXT_MAJOR_ACTION
    data["claim_boundary"] = claim_boundary_payload()
    ...
    data.pop("negative_memory", None)
    data["candidate_evidence"] = {
        "latest_run": "runs/R0001",
        "status": "retained_not_selected",
        "evidence_path": "runs/R0001/gate_report.json",
        "basis": "rolling_window_fold_isolated_mt5_tick_plus_execution_divergence",
        "entries_per_active_day": summary.get("entries_per_active_day"),
        "proxy_net_pnl_points": summary.get("proxy_net_pnl_points"),
        ...
        "profitable_fold_count": summary.get("profitable_fold_count"),
        "losing_fold_count": summary.get("losing_fold_count"),
        "worst_fold_net_pnl": summary.get("worst_fold_net_pnl"),
        "next_required_action": NEXT_MAJOR_ACTION,
        "claim_boundary": claim_boundary_payload(),
    }
    SELECTED.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False, width=88), encoding="ascii")


def update_campaign_for_candidate_closeout(closeout_review: dict[str, Any], closed_at: str) -> None:
    data = yaml.safe_load(CAMPAIGN.read_text(encoding="ascii"))
    data["status"] = "closed_with_candidate_evidence"
    data["closed_at_utc"] = closed_at
    run_index = data.setdefault("run_index", {})
    ...
    run_index["next_run_candidate"] = {
        ...
        "direction": NEXT_MAJOR_ACTION,
        "reason": (
            "C0142 R0001 retained candidate evidence after proxy, MT5 logic parity, MT5 tick, execution "
            "divergence, and fold-isolated MT5 evidence. Further C0142 work would be adjacent hardening or "
            "materialization, not a new non-adjacent discovery run."
        ),
        "status": "campaign_closed_with_candidate_evidence",
    }
    data["synthesis_readiness"] = {
        "check_required_at_closeout": True,
        "result": "candidate_later",
        "open_synthesis_now": False,
        "recommended_next_action": NEXT_MAJOR_ACTION,
        "readiness_notes": [
            "c0142_r0001_candidate_evidence_retained_not_selected",
            "synthesis_not_opened_in_this_closeout",
            "next_work_can_choose_c0143_new_major_hypothesis_without_c0142_adjacent_tuning",
        ],
    }
    data["closeout"] = {
        "status": "closed_with_candidate_evidence",
        "close_reason": closeout_review["close_reason"],
        "evidence_paths": closeout_evidence_paths(),
        "remaining_question": NEXT_MAJOR_ACTION,
        "closed_at_utc": closed_at,
        "decision_basis": closeout_review["required_kpi_summary"],
    ...
    data["campaign_lessons"] = [
        "C0142 R0001 two sided auction release continuation evidence is retained as candidate evidence.",
        "Fold-isolated MT5 tick evidence completed with all folds profitable, but no selected or economics-pass authority is created.",
        "Further C0142 work would be adjacent hardening, materialization, ONNX, sizing, or robustness work.",
    ]
    data["claim_boundary"] = claim_boundary_payload()
    CAMPAIGN.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False, width=88), encoding="ascii")


def update_reentry_for_candidate_closeout(closeout_review: dict[str, Any]) -> None:
    data = yaml.safe_load(REENTRY.read_text(encoding="ascii"))
    summary = closeout_review["required_kpi_summary"]
    data["updated_local_date"] = datetime.now().date().isoformat()
    read_budget = data.setdefault("read_budget", {})
    ...
    completed = list(next_work.get("completed") or [])
    for item in (
        "produce_c0142_r0001_proxy_evidence",
        "produce_c0142_r0001_mt5_logic_parity_evidence",
        "record_c0142_r0001_proxy_vs_mt5_logic_parity_evidence",
        "produce_c0142_r0001_mt5_tick_execution_evidence",
        "record_c0142_r0001_execution_divergence",
        "produce_c0142_r0001_fold_isolated_mt5_tick_kpi",
        "produce_c0142_r0001_fold_isolated_execution_divergence",
        "review_c0142_r0001_tick_execution_kpi_and_closeout",
        "close_c0142_r0001_with_candidate_evidence",
        "close_c0142_with_candidate_evidence_after_r0001",
    ):
        if item not in completed:
            completed.append(item)
    completed = [item for item in completed if item != NEXT_MAJOR_ACTION]
    ...
    data["active_run"] = None
    data["latest_operation"] = {
        "id": "close_c0142_r0001_with_candidate_evidence",
        "status": "completed",
        "recorded_at_source": f"{RUN_REL}/gate_report.json",
        "evidence_status": "closed_with_candidate_evidence",
        "active_campaign": None,
        "active_synthesis": None,
        "active_run": None,
        "negative_memory_recorded": False,
...
def update_claim_state_for_candidate_closeout(closeout_review: dict[str, Any]) -> None:
    data = yaml.safe_load(CLAIM_STATE.read_text(encoding="ascii"))
    summary = closeout_review["required_kpi_summary"]
    data["active_campaign"] = None
    data["active_synthesis"] = None
...
def update_decision_cursor_for_candidate_closeout(closeout_review: dict[str, Any]) -> None:
    data = yaml.safe_load(DECISION_CURSOR.read_text(encoding="ascii"))
    summary = closeout_review["required_kpi_summary"]
    data["updated_local_date"] = datetime.now().date().isoformat()
    data["canonical_source"] = f"{RUN_REL}/gate_report.json"
    data["canonical_status"] = "closed_with_candidate_evidence"
    data["current_decision"] = NEXT_MAJOR_ACTION
    data["active_campaign"] = None
    data["active_run"] = None
    data["active_synthesis"] = None
    ...
    resolved = data.setdefault("terminal_evidence_priority", {}).setdefault("resolved_from", {})
    resolved["active_campaign"] = WORK_UNIT_REL
    resolved["active_campaign_status"] = "closed_with_candidate_evidence"
    resolved["active_run"] = RUN_REL
    resolved["active_run_status"] = "closed_with_candidate_evidence"
    resolved["previous_campaign"] = PREVIOUS_CAMPAIGN_REL
    resolved["previous_campaign_status"] = "closed_no_candidate"
    repo_scope = data.setdefault("repo_debt_scope", {})
    repo_scope["note"] = (
        "repo_state_has_preexisting_nonblocking_artifact_lineage_debt_and_price_quality_warning; "
        "current_c0142_r0001_candidate_closeout_makes_no_selection_promotion_handoff_or_reproducibility_claims"
    )
    for item in repo_scope.get("nonblocking_debts") or []:
        if isinstance(item, dict):
            item["role"] = "repo_state_debt_not_current_c0142_r0001_candidate_closeout"
            item["current_decision_class"] = "known_nonblocking_for_next_work_decision"
    for item in repo_scope.get("warning_observations") or []:
        if isinstance(item, dict):
            item["current_decision_class"] = "known_nonblocking_for_next_work_decision"
    ...
    current.update(
        {
            ...
            "active_synthesis": None,
            "active_run": None,
            "active_run_status": "closed_with_candidate_evidence",
            "campaign_status": "closed_with_candidate_evidence",
            "evidence_status": "closed_with_candidate_evidence",
            "current_task": NEXT_MAJOR_ACTION,
            "entries_per_active_day": summary.get("entries_per_active_day"),
            "proxy_net_pnl_points": summary.get("proxy_net_pnl_points"),
            ...
            "losing_fold_count": summary.get("losing_fold_count"),
            "next_required_action": NEXT_MAJOR_ACTION,
            "note": "C0142 R0001 and C0142 closed with candidate evidence retained, not selected or promoted.",
        }
    )
    data["next_decision_basis"] = [
        {
            "path": f"{RUN_REL}/gate_report.json",
            "role": "closed_run_gate_report",
            "summary": "C0142 R0001 closed with candidate evidence retained, not selected.",
        },
        {
            "path": f"{RUN_REL}/kpi/mt5_tick_by_fold.json",
            "role": "fold_isolated_mt5_tick_kpi",
            "summary": "Fold-isolated MT5 tick evidence completed with 9 profitable and 0 losing folds.",
        },
        {
            "path": f"{WORK_UNIT_REL}/campaign.yaml",
            "role": "closed_campaign_manifest",
            "summary": "C0142 closed with candidate evidence; next work is a new major C0143 hypothesis.",
        },
    ]
    data["claim_boundary_snapshot"] = claim_boundary_payload()
    DECISION_CURSOR.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False, width=88), encoding="ascii")
...
def append_decision_registry_for_candidate_closeout(closeout_review: dict[str, Any]) -> None:
    data = yaml.safe_load(DECISION_REGISTRY.read_text(encoding="ascii"))
    if isinstance(data, dict):
        decisions = data.setdefault("decisions", [])
    else:
    ...
    summary = closeout_review["required_kpi_summary"]
    local_date = datetime.now().date().isoformat()
    if "dec_20260708_close_c0142_r0001_with_candidate_evidence" not in existing:
        decisions.append(
            {
                "decision_id": "dec_20260708_close_c0142_r0001_with_candidate_evidence",
                "created_local_date": local_date,
                "status": "active",
                "decision": "close_c0142_r0001_with_candidate_evidence",
                "refines": [
                    "dec_20260708_produce_c0142_r0001_proxy_evidence",
                    "dec_20260701_mandatory_mt5_paired_run_validation",
                ],
                "rationale": [
                    ...
                    f"mt5_tick_net_pnl_{summary.get('mt5_tick_net_pnl')}_profit_factor_{summary.get('mt5_tick_profit_factor')}",
                    f"fold_isolated_tick_has_{summary.get('profitable_fold_count')}_profitable_and_{summary.get('losing_fold_count')}_losing_folds",
                    "closeout_records_candidate_evidence_retained_not_selection_or_economics_pass",
                    "no_selected_economics_runtime_materialization_onnx_promotion_or_live_claim_is_created",
                ],
                "claim_boundary": claim_boundary_payload(),
            }
        )
    if "dec_20260708_close_c0142_with_candidate_evidence_after_r0001" not in existing:
        decisions.append(
            {
                "decision_id": "dec_20260708_close_c0142_with_candidate_evidence_after_r0001",
                "created_local_date": local_date,
                "status": "active",
                "decision": "close_c0142_with_candidate_evidence_after_r0001",
                "refines": [
                    "dec_20260708_close_c0142_r0001_with_candidate_evidence",
                    "dec_20260708_open_c0142_intraday_two_sided_auction_release_continuation_discovery",
                ],
                "rationale": [
                    "C0142_two_sided_auction_release_continuation_boundary_retains_candidate_evidence_without_claim_authority",
                    "remaining_C0142_work_would_be_adjacent_threshold_window_stop_target_hold_session_activity_spread_capital_materialization_onnx_or_retry_hardening",
                    "next_work_is_choose_c0143_new_major_hypothesis_after_c0142_closeout",
                    "no_selected_economics_runtime_materialization_onnx_promotion_or_live_claim_is_created",
                ],
                "claim_boundary": claim_boundary_payload(),
...
def refresh_state_artifact_hashes(status: str = "closed_no_candidate") -> None:
    data = json.loads(ARTIFACT_LINEAGE.read_text(encoding="ascii"))
    data["status"] = status
    for record in data.get("artifact_records", []):
        path = record.get("repo_relative_path")
...
def close_c0142_r0001_no_candidate() -> dict[str, object]:
    closed_at = utc_now()
    closeout_review = C0142_closeout_review(closed_at)
    ...
    update_campaign_for_closeout(closeout_review, closed_at)
    update_reentry_for_closeout(closeout_review)
...
def close_c0142_r0001_candidate_evidence_retained() -> dict[str, object]:
    closed_at = utc_now()
    closeout_review = C0142_candidate_closeout_review(closed_at)
    update_run_manifest_for_candidate_closeout(closeout_review, closed_at)
    update_gate_report_for_candidate_closeout(closeout_review)
    update_selected_for_candidate_closeout(closeout_review)
    update_campaign_for_candidate_closeout(closeout_review, closed_at)
    update_reentry_for_candidate_closeout(closeout_review)
    update_claim_state_for_candidate_closeout(closeout_review)
    update_decision_cursor_for_candidate_closeout(closeout_review)
    update_work_unit_registry_for_closeout()
    append_decision_registry_for_candidate_closeout(closeout_review)
    refresh_state_artifact_hashes("closed_with_candidate_evidence")
    return {
        "status": "closed_with_candidate_evidence",
        "closed_at_utc": closed_at,
        "next_required_action": NEXT_MAJOR_ACTION,
    ...
    records.append(
        {
            ...
            "sha256": digest,
            "produced_by": "axiom_rift.mt5.c0142_r0001_probe",
