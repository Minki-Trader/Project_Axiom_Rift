"""MT5 schedule-replay logic parity helpers for SC0008 SR0001."""
...
from axiom_rift.proxies.sc0008_sr0001_post_sc0007_regression_channel_residual_mixed_evidence import (
    BASE_FRAME,
    ROLLING_WINDOWS,
    TIME_FORMAT,
)
...
WORK_UNIT_ID = "SC0008"
RUN_ID = "SR0001"
RUN_DIR = PROJECT_ROOT / "campaigns" / "SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis" / "runs" / RUN_ID
SYNTHESIS_QUEUE = PROJECT_ROOT / "campaigns" / "SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis" / "synthesis_queue.yaml"
KPI_DIR = RUN_DIR / "kpi"
...
SYNTHESIS = PROJECT_ROOT / "campaigns" / "SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis" / "synthesis.yaml"
SELECTED = PROJECT_ROOT / "campaigns" / "SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis" / "selected.yaml"
...
PROXY_TRADE_ARTIFACT = RUN_ARTIFACT_DIR / "sc0008_sr0001_proxy_trades.csv"
SCHEDULE_ARTIFACT = RUN_ARTIFACT_DIR / "sc0008_sr0001_schedule.csv"
SCHEDULE_COMMON_REL = "AxiomRift\\SC0008\\SR0001\\schedule\\sc0008_sr0001_schedule.csv"
WORK_UNIT_REL = "campaigns/SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis"
RUN_REL = f"{WORK_UNIT_REL}/runs/{RUN_ID}"
NEXT_MAJOR_ACTION = "choose_c0046_new_major_hypothesis_after_sc0008_closeout"
SC0008_CLOSE_REASON = (
    "candidate_evidence_recorded_with_zero_trade_coverage_gap_after_fold_isolated_mt5_tick_and_execution_divergence"
)
SC0008_REMAINING_WORK_CLASSIFICATION = (
    "candidate_evidence_retained_not_selected_zero_trade_fold_native_materialization_onnx_lineage_and_cost_stress_gaps"
    "_block_stronger_claims"
)
SC0008_CLOSED_STATUSES = {"closed_no_candidate", "closed_with_candidate_evidence", "closed_non_portable"}
STARTING_BALANCE_USD = starting_balance_usd()
RESPONSE_MODE = "post_sc0007_regression_channel_residual_mixed_evidence_schedule_replay"
MAX_HOLD_BARS = 10
MAGIC = 980001
...
def compile_sc0008_sr0001_ea(metaeditor_exe: Path | None = None) -> CompileResult:
    ...
    log = PROJECT_ROOT / "artifacts" / "reports" / "SC0008_SR0001_mt5_compile.log"
...
def tester_config_path(mode: str = LOGIC_PARITY_MODE, output_scope: str | None = None) -> Path:
    ...
    return PROJECT_ROOT / "artifacts" / "reports" / "SC0008_SR0001_mt5_tester" / f"SC0008_SR0001_{suffix}_tester.ini"
...
def tester_report_path(mode: str = LOGIC_PARITY_MODE, output_scope: str | None = None) -> Path:
    ...
    output_scope = normalize_output_scope(output_scope)
    suffix = mode if output_scope is None else f"{mode}_{output_scope}"
    return PROJECT_ROOT / "artifacts" / "reports" / "SC0008_SR0001_mt5_tester" / f"SC0008_SR0001_mt5_{suffix}_report.htm"
...
def write_tester_config(
    ...
    config_dir = PROJECT_ROOT / "artifacts" / "reports" / "SC0008_SR0001_mt5_tester"
...
def parse_sc0008_sr0001_mt5(
    ...
    if mode == LOGIC_PARITY_MODE:
        upsert_artifact_lineage(
            "A-SC0008-SR0001-MT5-SCHEDULE",
            "mt5_schedule_csv",
            "mt5_logic_parity_input",
            rel(SCHEDULE_ARTIFACT),
            sha256_file(SCHEDULE_ARTIFACT),
            ["campaigns/SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis/runs/SR0001/kpi/proxy.json"],
        )
        upsert_artifact_lineage(
            "A-SC0008-SR0001-MT5-LOGIC-KPI",
            "mt5_logic_parity_kpi",
            "mt5_logic_parity",
            rel(MT5_LOGIC_KPI),
            sha256_file(MT5_LOGIC_KPI),
            [
                "campaigns/SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis/runs/SR0001/artifacts/sc0008_sr0001_schedule.csv",
                "configs/market.yaml",
            ],
        )
    else:
        upsert_artifact_lineage(
            "A-SC0008-SR0001-MT5-TICK-KPI",
            "mt5_tick_kpi",
            "mt5_tick",
            rel(MT5_TICK_KPI),
...
def build_mt5_payload(
    ...
    return {
        ...
        "run_id": RUN_ID,
        "mt5_probe_id": "MT5-SC0008-SR0001",
        ...
        **runtime_payload_fields(),
        ...
        "conditional_profiles": {
            ...
            "synthesis_schedule_replay_profile": {
                ...
                "fields": {
                    ...
                    "logic_boundary": "proxy_selected_post_sc0007_regression_channel_residual_mixed_evidence_schedule_replayed_in_mt5_closed_bar_mode"
                    if mode == LOGIC_PARITY_MODE
        ...
        "deferred_with_reason": [
            {
                "field": "native_mql_post_sc0007_regression_channel_residual_mixed_evidence_synthesis_logic",
                "requirement_class": "deferred_with_reason",
                "reason": "SC0008 SR0001 replays the proxy schedule in MT5; native model or ONNX materialization is not claimed",
...
def record_sc0008_sr0001_parity() -> dict[str, object]:
    ...
    next_action = (
        "produce_sc0008_sr0001_mt5_tick_execution_evidence"
        if mechanical_ok
        else "repair_sc0008_sr0001_schedule_replay_parity"
    )
    ...
    payload = {
        ...
        "synthesis_id_when_applicable": WORK_UNIT_ID,
        "run_id": RUN_ID,
        "parity_id": "P-SC0008-SR0001",
        "proxy_id": "PX-SC0008-SR0001",
        "mt5_probe_id": "MT5-SC0008-SR0001",
    ...
    upsert_artifact_lineage(
        ...
        [
            "campaigns/SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis/runs/SR0001/kpi/proxy.json",
            "campaigns/SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis/runs/SR0001/kpi/mt5_logic_parity.json",
        ],
    )
...
def update_run_after_mt5(mode: str) -> None:
    ...
    evidence_paths["mt5_schedule_artifact"] = "artifacts/sc0008_sr0001_schedule.csv"
    mt5_plan = data.setdefault("mt5_probe_plan", {})
    mt5_plan["logic_parity_kpi_path"] = "kpi/mt5_logic_parity.json"
    mt5_plan["logic_parity_purpose"] = "proxy_post_sc0007_regression_channel_residual_mixed_evidence_synthesis_schedule_vs_ea_closed_bar_lifecycle_parity"
...
def update_gate_after_logic_parity(payload: dict[str, object], next_action: str, mechanical_ok: bool) -> None:
    ...
    for path in ("kpi/mt5_logic_parity.json", "kpi/proxy_vs_mt5_logic_parity.json", "artifacts/sc0008_sr0001_schedule.csv"):
...
def update_reentry_after_logic_parity(next_action: str) -> None:
    ...
    for item in (
        "produce_sc0008_sr0001_mt5_logic_parity_evidence",
        "record_sc0008_sr0001_proxy_vs_mt5_logic_parity_evidence",
    ):
...
def update_synthesis_queue_after_logic_parity(next_action: str, mechanical_ok: bool) -> None:
    ...
    for item in data.get("queue", []):
        if item.get("synthesis_run_id") == RUN_ID:
            ...
            item["last_completed_step"] = "produce_sc0008_sr0001_mt5_logic_parity_evidence"
...
def update_claim_state_after_logic_parity(payload: dict[str, object], next_action: str, mechanical_ok: bool) -> None:
    ...
    data["active_synthesis"] = "campaigns/SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis"
    data["active_run"] = "campaigns/SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis/runs/SR0001"
    data["latest_operation"] = {
        "id": "produce_sc0008_sr0001_mt5_logic_parity_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis/runs/SR0001/kpi/proxy_vs_mt5_logic_parity.json",
        "evidence_status": evidence_status,
        "active_synthesis": "campaigns/SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis",
        "active_run": "campaigns/SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis/runs/SR0001",
...
def update_decision_cursor_after_logic_parity(payload: dict[str, object], next_action: str, mechanical_ok: bool) -> None:
    ...
    data["canonical_source"] = "campaigns/SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis/runs/SR0001/kpi/proxy_vs_mt5_logic_parity.json"
    ...
    summary["note"] = (
        "SC0008 SR0001 closed-bar MT5 logic parity is recorded; tick execution KPI is next."
        if mechanical_ok
        else "SC0008 SR0001 logic parity is recorded with mechanical mismatch; repair is next."
    )
    ...
    if isinstance(basis, list):
        ...
        basis.insert(
            0,
            {
                "path": "campaigns/SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis/runs/SR0001/kpi/proxy_vs_mt5_logic_parity.json",
                "role": "active_synthesis_run_logic_parity_kpi",
                "summary": "SC0008 SR0001 closed-bar MT5 logic parity is recorded; next action remains inside mandatory paired validation.",
            },
        )
...
def append_decision_registry_after_logic_parity(payload: dict[str, object], next_action: str, mechanical_ok: bool) -> None:
    ...
    decision_id = "dec_20260706_produce_sc0008_sr0001_mt5_logic_parity_evidence"
    ...
    decisions.append(
        {
            ...
            "status": "active",
            "decision": "produce_sc0008_sr0001_mt5_logic_parity_evidence",
            "refines": [
                "dec_20260706_produce_sc0008_sr0001_proxy_evidence",
                "dec_20260701_mandatory_mt5_paired_run_validation",
            ],
            "rationale": [
                "closed_bar_mt5_logic_parity_and_proxy_vs_mt5_parity_are_recorded_for_sc0008_sr0001",
...
def record_sc0008_sr0001_execution_divergence() -> dict[str, object]:
    ...
    payload = {
        ...
        "synthesis_id_when_applicable": WORK_UNIT_ID,
        "run_id": RUN_ID,
        "divergence_id": "ED-SC0008-SR0001",
        ...
        "conditional_profiles": {
            ...
            "synthesis_schedule_replay_profile": {
                ...
                "fields": {
                    "schedule_artifact_path": rel(SCHEDULE_ARTIFACT),
                    "risk_shape_source": "proxy_post_sc0007_regression_channel_residual_mixed_evidence_synthesis_schedule",
        ...
        "deferred_with_reason": [
            {
                ...
                "revisit_when": "after SC0008 SR0001 fold-isolated MT5 tick evidence",
    ...
    upsert_artifact_lineage(
        "A-SC0008-SR0001-EXEC-DIVERGENCE-KPI",
        ...
        [
            "campaigns/SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis/runs/SR0001/kpi/mt5_logic_parity.json",
            "campaigns/SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis/runs/SR0001/kpi/mt5_tick.json",
        ],
    )
...
def update_gate_after_execution_divergence(payload: dict[str, object]) -> None:
    ...
    data["decision"] = "defer_with_reason"
    data["next_action"] = "produce_sc0008_sr0001_fold_isolated_mt5_tick_kpi"
    data["execution_gate"] = {
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "produce_sc0008_sr0001_fold_isolated_mt5_tick_kpi",
        }
    ]
    GATE_REPORT.write_text(json.dumps(data, indent=2, sort_keys=True) + "\n", encoding="ascii")
...
def update_reentry_after_execution_divergence() -> None:
    ...
    for item in (
        "produce_sc0008_sr0001_mt5_tick_execution_evidence",
        "record_sc0008_sr0001_execution_divergence",
    ):
        if item not in completed:
            completed.append(item)
    next_action = "produce_sc0008_sr0001_fold_isolated_mt5_tick_kpi"
    completed = [item for item in completed if item != next_action]
    next_work["completed"] = completed
    next_work["tasks"] = [next_action]
    path.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def update_synthesis_queue_after_execution_divergence() -> None:
    ...
    for item in data.get("queue", []):
        if item.get("synthesis_run_id") == RUN_ID:
            ...
            item["last_completed_step"] = "record_sc0008_sr0001_execution_divergence"
            item["next_action"] = "produce_sc0008_sr0001_fold_isolated_mt5_tick_kpi"
    SYNTHESIS_QUEUE.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def build_mt5_tick_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": WORK_UNIT_ID,
        "run_id": RUN_ID,
        "mt5_probe_id": "MT5-SC0008-SR0001-BY-FOLD",
...
def build_execution_divergence_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": WORK_UNIT_ID,
        "run_id": RUN_ID,
        "divergence_id": "ED-SC0008-SR0001-BY-FOLD",
        "split_policy": "rolling_window_test_oos_fold_isolated",
        "split_registry": "registries/rolling_windows.yaml",
...
def update_gate_after_fold_isolated_evidence(
    ...
    mt5_plan["next_required_action"] = "review_sc0008_sr0001_tick_execution_kpi_and_closeout"
    ...
    for path in (
        ...
        "artifacts/sc0008_sr0001_schedule.csv",
    ):
        if path not in evidence_paths:
            evidence_paths.append(path)
    gate_data["decision"] = "defer_with_reason"
    gate_data["next_action"] = "review_sc0008_sr0001_tick_execution_kpi_and_closeout"
    gate_data["deferred_with_reason"] = [
        {
            ...
            "blocking_condition": "review_sc0008_sr0001_tick_execution_kpi_and_closeout",
            "revisit_when": "during SC0008 SR0001 closeout review",
        }
    ]
...
def update_reentry_after_fold_isolated() -> None:
    ...
    completed = list(next_work.get("completed") or [])
    for item in (
        "produce_sc0008_sr0001_fold_isolated_mt5_tick_kpi",
        "produce_sc0008_sr0001_fold_isolated_execution_divergence",
    ):
        if item not in completed:
            completed.append(item)
    next_action = "review_sc0008_sr0001_tick_execution_kpi_and_closeout"
    completed = [item for item in completed if item != next_action]
    next_work["completed"] = completed
    next_work["tasks"] = [next_action]
    path.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def update_synthesis_queue_after_fold_isolated() -> None:
    ...
    for item in data.get("queue", []):
        if item.get("synthesis_run_id") == RUN_ID:
            ...
            item["last_completed_step"] = "produce_sc0008_sr0001_fold_isolated_execution_divergence"
            item["next_action"] = "review_sc0008_sr0001_tick_execution_kpi_and_closeout"
    SYNTHESIS_QUEUE.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def upsert_artifact_lineage(
    ...
    records.append(
        {
            ...
            "sha256": file_hash,
            "produced_by": "axiom_rift.mt5.sc0008_sr0001_probe",
    ...
    try:
        ...
        run_status = None
    if run_status in SC0008_CLOSED_STATUSES:
        data["deferred_with_reason"] = []
        ...
        deferred_next_action = "review_sc0008_sr0001_tick_execution_kpi_and_closeout"
        data["deferred_with_reason"] = [
            {
        ...
        deferred_next_action = "produce_sc0008_sr0001_fold_isolated_mt5_tick_kpi"
        data["deferred_with_reason"] = [
            {
                "field": deferred_field,
                "reason": deferred_reason,
        ...
        deferred_next_action = "produce_sc0008_sr0001_mt5_tick_execution_evidence"
        data["deferred_with_reason"] = [
            {
                "field": deferred_field,
                "reason": deferred_reason,
...
def refresh_state_artifact_hashes() -> None:
    upsert_artifact_lineage(
        ...
        [
            "campaigns/SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis/runs/SR0001/kpi/mt5_logic_parity.json",
            "campaigns/SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis/runs/SR0001/kpi/proxy_vs_mt5_logic_parity.json",
        ],
    )
    upsert_artifact_lineage(
        ...
        [
            "campaigns/SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis/runs/SR0001/run_manifest.json",
            "campaigns/SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis/runs/SR0001/kpi/proxy_vs_mt5_logic_parity.json",
        ],
    )
...
def sc0008_closeout_kpi_summary() -> dict[str, Any]:
    ...
    return {
        ...
        "remaining_work_classification": SC0008_REMAINING_WORK_CLASSIFICATION,
    }


def sc0008_candidate_evidence_asset(summary: dict[str, Any]) -> dict[str, Any]:
    return {
        "asset_type": "candidate_evidence",
        "candidate_status": "recorded_not_selected",
        "selection_status": "not_selected",
        "economics_pass": False,
        "runtime_authority": False,
        "onnx_ready": False,
        "hypothesis_tested": (
            "SC0008 SR0001 fold-local post-SC0007 regression-channel residual candidate schedule reweighted by "
            "C0038-C0044 negative-memory context plus C0045 cost/materialization gap evidence without source OOS PnL selection"
        ),
        "evidence_paths": [
            "kpi/proxy.json",
            "kpi/mt5_logic_parity.json",
        ...
        "reason_candidate_evidence": (
            "target-range activity, positive proxy, passed schedule replay parity, positive aggregate MT5 tick net PnL, "
            "eight profitable rolling-window tick folds, zero losing folds, and one zero-trade fold"
        ),
        "next_boundary": (
            "SR0001 remains unselected because rw_001 produced zero trades, native materialization and ONNX portability "
            "are absent, C0045 source cost-stress weakness is avoided rather than proven in the zero-trade fold, and "
            "historical repo lineage debt blocks selected, promotion, handoff, and reproducibility claims; next work "
            "should choose C0046 or a later hardening axis without selected, economics-pass, runtime, ONNX, promotion, "
            "or live claims"
        ),
        "supporting_lessons": [
            {
                ...
                "summary": (
                    "post-SC0007 mixed-evidence rerank produced target-range activity and positive fold-isolated tick "
                    f"evidence; total tick net PnL was {summary.get('total_tick_net_pnl')} with "
            ...
            },
            {
                ...
                "summary": (
                    f"tick execution worsened total net versus closed-bar logic by "
                    f"{summary.get('total_tick_minus_logic_net_pnl')}, with "
                    f"{summary.get('tick_better_fold_count')} tick-better folds and "
                    f"{summary.get('tick_worse_fold_count')} tick-worse folds"
                ),
            },
            {
                ...
                "source": "kpi/mt5_tick_by_fold.json",
                "summary": (
                    "rw_001 produced zero trades; null denominator metrics are recorded as not-applicable, and the "
                    "source C0045 weakest-fold cost weakness is avoided there rather than proven tradable"
                ),
            },
            {
                "asset_type": "non_portable_lesson",
                "source": "kpi/proxy_vs_mt5_logic_parity.json",
                "summary": (
                    "logic parity is schedule replay only; native MQL or ONNX materialization remains deferred and "
                    "unclaimed"
                ),
            },
        ],
...
def sc0008_closeout_review(closed_at: str) -> dict[str, Any]:
    summary = sc0008_closeout_kpi_summary()
    return {
        "status": "closed_with_candidate_evidence",
        "basis": "rolling_window_fold_isolated_mt5_tick",
        "close_reason": SC0008_CLOSE_REASON,
        "closed_at_utc": closed_at,
        "required_kpi_summary": summary,
        "candidate_evidence": sc0008_candidate_evidence_asset(summary),
        "claim_boundary": claim_boundary_payload(),
    }
    ...
    evidence.update(
        {
            ...
            "proxy_trade_artifact": "artifacts/sc0008_sr0001_proxy_trades.csv",
            "regression_channel_mixed_evidence_summary": "artifacts/sc0008_sr0001_summary.json",
            "mt5_schedule_artifact": "artifacts/sc0008_sr0001_schedule.csv",
...
def update_gate_report_for_closeout(closeout_review: dict[str, Any]) -> None:
    ...
    data["gate_report_id"] = "G-SC0008-SR0001"
    data["status"] = "closed_with_candidate_evidence"
    data["decision"] = "closed_with_candidate_evidence"
...
def update_selected_for_closeout() -> None:
    ...
    data["selected_reason"] = (
        "SC0008 SR0001 retained candidate evidence but remains unselected because rw_001 zero-trade coverage, "
        "native materialization, ONNX portability, historical lineage debt, and claim authority remain unresolved."
    )
    data["claim_boundary"] = data.get("claim_boundary", {})
    latest = data.setdefault("latest_synthesis_state", {})
    latest["status"] = "closed_with_candidate_evidence"
    latest["source"] = "runs/SR0001/gate_report.json"
    latest["candidate_evidence_retained"] = True
    ...
    data["candidate_evidence"] = {
        "latest_run": "runs/SR0001",
        "status": "retained_not_selected",
        "evidence_path": "runs/SR0001/gate_report.json",
        ...
        "evidence_gap": "rw_001_zero_trade_fold_and_schedule_replay_only_no_native_onnx_claim",
...
def update_synthesis_for_closeout(closeout_review: dict[str, Any], closed_at: str) -> None:
    ...
    run_index["next_run_candidate"] = {
        ...
        "reason": (
            "SR0001 closed with candidate evidence after positive proxy, passed schedule replay parity, "
            "positive tick execution, and fold-isolated MT5 tick evidence with eight profitable folds, zero losing "
            "folds, and one zero-trade coverage gap; further SC0008 work would be adjacent mixed-evidence weight, "
            "cost-gap, threshold, monthly-filter, stop, target, hold, activity, spread, session, capital, or retry tuning."
        ),
        "status": "synthesis_closed_with_candidate_evidence",
    }
    data["closeout"] = {
        "status": "closed_with_candidate_evidence",
        "close_reason": SC0008_CLOSE_REASON,
...
def update_synthesis_queue_for_closeout(closed_at: str) -> None:
    ...
    for item in data.get("queue", []):
        if item.get("synthesis_run_id") == RUN_ID:
            item["status"] = "closed_with_candidate_evidence"
            item["last_completed_step"] = "close_sc0008_sr0001_with_candidate_evidence"
...
def update_reentry_for_closeout() -> None:
    ...
    completed = list(next_work.get("completed") or [])
    for item in (
        "review_sc0008_sr0001_tick_execution_kpi_and_closeout",
        "close_sc0008_sr0001_with_candidate_evidence",
        "close_sc0008_with_candidate_evidence_after_sr0001",
    ):
        if item not in completed:
            completed.append(item)
...
def update_claim_state_for_closeout(closeout_review: dict[str, Any]) -> None:
    ...
    data["active_run"] = None
    data["latest_operation"] = {
        "id": "close_sc0008_sr0001_with_candidate_evidence",
        "status": "completed",
    ...
    current.update(
        {
            ...
            "synthesis_status": "closed_with_candidate_evidence",
            "entries_per_active_day": summary.get("entries_per_active_day"),
            "proxy_net_pnl_points": summary.get("proxy_net_pnl_points"),
            "proxy_profit_factor": summary.get("proxy_profit_factor"),
            ...
            "next_required_action": NEXT_MAJOR_ACTION,
            "note": (
                "SC0008 SR0001 closed with candidate evidence retained but not selected; rw_001 zero-trade coverage, "
                "native materialization, ONNX portability, and repo lineage debt block stronger claims."
            ),
        }
    )
    data["next_decision_basis"] = [
        {
            ...
            "summary": "SC0008 SR0001 closed with candidate evidence retained but not selected.",
        },
        {
            "path": f"{RUN_REL}/kpi/mt5_tick_by_fold.json",
            "role": "fold_isolated_mt5_tick_kpi",
            "summary": "Fold evidence completed with eight profitable folds, zero losing folds, and one zero-trade fold.",
        },
        {
            "path": f"{WORK_UNIT_REL}/synthesis.yaml",
            "role": "closed_synthesis_manifest",
            "summary": "SC0008 is closed with candidate evidence; remaining work is a new major C0046 hypothesis or later hardening axis.",
        },
    ]
    data["claim_boundary_snapshot"] = claim_boundary_payload()
    DECISION_CURSOR.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def append_decision_registry_for_closeout(closeout_review: dict[str, Any]) -> None:
    ...
    if "dec_20260706_close_sc0008_sr0001_with_candidate_evidence" not in existing:
        decisions.append(
            {
                "decision_id": "dec_20260706_close_sc0008_sr0001_with_candidate_evidence",
                "created_local_date": local_date,
                "status": "active",
                "decision": "close_sc0008_sr0001_with_candidate_evidence",
                "refines": [
                    "dec_20260706_produce_sc0008_sr0001_mt5_logic_parity_evidence",
                    "dec_20260706_produce_sc0008_sr0001_proxy_evidence",
                    "dec_20260701_mandatory_mt5_paired_run_validation",
                ],
                "rationale": [
                    ...
                    "closeout_records_candidate_evidence_not_selection_or_economics_pass",
                    "zero_trade_rw_001_coverage_gap_and_schedule_replay_only_boundary_remain",
                    "no_selected_economics_runtime_materialization_onnx_promotion_or_live_claim_is_created",
                ],
                "claim_boundary": claim_boundary_payload(),
            }
        )
    if "dec_20260706_close_sc0008_with_candidate_evidence_after_sr0001" not in existing:
        decisions.append(
            {
                "decision_id": "dec_20260706_close_sc0008_with_candidate_evidence_after_sr0001",
                "created_local_date": local_date,
                "status": "active",
                "decision": "close_sc0008_with_candidate_evidence_after_sr0001",
                "refines": [
                    "dec_20260706_close_sc0008_sr0001_with_candidate_evidence",
                    "dec_20260706_close_c0045_with_candidate_evidence_and_open_sc0008",
                ],
                "rationale": [
                    "sc0008_synthesis_boundary_was_answered_by_sr0001_fold_isolated_mt5_evidence",
                    "candidate_evidence_is_retained_but_zero_trade_fold_native_materialization_onnx_and_lineage_gaps_block_stronger_claims",
                    "remaining_sc0008_work_is_adjacent_mixed_evidence_weight_cost_gap_threshold_monthly_filter_stop_target_hold_activity_spread_session_capital_or_retry_tuning",
                    "next_work_is_choose_c0046_new_major_hypothesis_after_sc0008_closeout",
                    "no_selected_economics_runtime_materialization_onnx_promotion_or_live_claim_is_created",
                ],
                "claim_boundary": claim_boundary_payload(),
            }
...
def close_sc0008_sr0001_with_candidate_evidence() -> dict[str, object]:
    closed_at = utc_now()
    closeout_review = sc0008_closeout_review(closed_at)
...
def run_sc0008_sr0001_logic_parity_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_sc0008_sr0001_tester(mode=LOGIC_PARITY_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_sc0008_sr0001_mt5(result, mode=LOGIC_PARITY_MODE, write_kpi=True)
    parity_payload = record_sc0008_sr0001_parity()
    return {
...
def run_sc0008_sr0001_mt5_tick_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_sc0008_sr0001_tester(mode=TICK_EXECUTION_MODE, timeout_seconds=timeout_seconds)
    tick_payload = parse_sc0008_sr0001_mt5(result, mode=TICK_EXECUTION_MODE, write_kpi=True)
    divergence_payload = record_sc0008_sr0001_execution_divergence()
    return {
...
def write_sc0008_sr0001_fold_isolated_payloads(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    upsert_artifact_lineage(
        "A-SC0008-SR0001-MT5-TICK-BY-FOLD-KPI",
    ...
    upsert_artifact_lineage(
        "A-SC0008-SR0001-EXEC-DIVERGENCE-BY-FOLD-KPI",
        "execution_divergence_by_fold_kpi",
        "execution_divergence_by_fold",
        rel(EXECUTION_DIVERGENCE_BY_FOLD_KPI),
        sha256_file(EXECUTION_DIVERGENCE_BY_FOLD_KPI),
        [
            "campaigns/SC0008_post_sc0007_c0038_c0045_mixed_evidence_synthesis/runs/SR0001/kpi/mt5_tick_by_fold.json",
            "registries/rolling_windows.yaml",
        ],
    )
...
def record_sc0008_sr0001_fold_isolated_from_outputs() -> dict[str, object]:
    ...
    for window in load_test_windows():
        ...
        logic_payload = parse_sc0008_sr0001_mt5(logic_result, mode=LOGIC_PARITY_MODE, write_kpi=False)
        tick_payload = parse_sc0008_sr0001_mt5(tick_result, mode=TICK_EXECUTION_MODE, write_kpi=False)
        records.append(
            {
                "window": window,
                "from_date": from_date,
    ...
    return write_sc0008_sr0001_fold_isolated_payloads(records)


def run_sc0008_sr0001_mt5_tick_by_fold_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    compile_sc0008_sr0001_ea()
    write_schedule_files()
    records: list[dict[str, Any]] = []
    for window in load_test_windows():
        from_date, to_date = tester_dates_for_window(window)
        logic_result = run_sc0008_sr0001_tester(
        ...
        logic_payload = parse_sc0008_sr0001_mt5(logic_result, mode=LOGIC_PARITY_MODE, write_kpi=False)
        tick_result = run_sc0008_sr0001_tester(
...
if __name__ == "__main__":
    result_payload = run_sc0008_sr0001_logic_parity_workflow()
