"""MT5 compile, tester, and KPI parsing helpers for R0007."""
...
from axiom_rift.proxies.r0007_core_session_expansion_continuation import (
    BASE_FRAME,
    load_bars,
    load_test_windows,
    run_r0007_proxy,
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0001_regime_response_discovery" / "runs" / "R0007"
KPI_DIR = RUN_DIR / "kpi"
...
SESSION_START_MINUTE = 6 * 60
SESSION_END_MINUTE = 21 * 60
...
def compile_r0007_ea(metaeditor_exe: Path = DEFAULT_METAEDITOR_EXE) -> CompileResult:
    ...
    log = PROJECT_ROOT / "artifacts" / "reports" / "R0007_mt5_compile.log"
...
def tester_config_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    mode = normalize_mt5_mode(mode)
    return PROJECT_ROOT / "artifacts" / "reports" / "R0007_mt5_tester" / f"R0007_{scoped_name(mode, output_scope)}_tester.ini"


def tester_report_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    mode = normalize_mt5_mode(mode)
    return PROJECT_ROOT / "artifacts" / "reports" / "R0007_mt5_tester" / f"R0007_mt5_{scoped_name(mode, output_scope)}_report.htm"
...
def write_tester_config(
    ...
    config_dir = PROJECT_ROOT / "artifacts" / "reports" / "R0007_mt5_tester"
    ...
    lines = [
        ...
        "InpResponseMode=core_session_expansion_continuation",
        "InpUseCommonFiles=true",
        f"InpUseClosedBarExit={bool_text(use_closed_bar_exit)}",
        "InpUseSessionFilter=true",
        f"InpSessionStartMinute={SESSION_START_MINUTE}",
        f"InpSessionEndMinute={SESSION_END_MINUTE}",
        "InpLookbackRangeBars=48",
        "InpBreakoutBars=24",
        "InpExpansionRangeMultiple=1.8",
        "InpMinBodyRangeFraction=0.55",
        "InpPullbackSearchBars=6",
        "InpPullbackMinRatio=0.20",
        "InpPullbackMaxRatio=0.65",
...
def assert_mt5_mode_matches_status(mode: str, status: dict[str, str], output_scope: str | None = None) -> None:
    ...
    if status.get("response_mode") != "core_session_expansion_continuation":
        raise RuntimeError(f"MT5 response_mode mismatch: expected=core_session_expansion_continuation actual={status.get('response_mode')}")
    if status.get("session_filter_enabled") != "true":
        raise RuntimeError(f"MT5 session_filter_enabled mismatch: expected=true actual={status.get('session_filter_enabled')}")
    if status.get("session_start_minute") != str(SESSION_START_MINUTE):
        raise RuntimeError(f"MT5 session_start_minute mismatch: expected={SESSION_START_MINUTE} actual={status.get('session_start_minute')}")
    if status.get("session_end_minute") != str(SESSION_END_MINUTE):
        raise RuntimeError(f"MT5 session_end_minute mismatch: expected={SESSION_END_MINUTE} actual={status.get('session_end_minute')}")
...
def record_r0007_parity() -> dict[str, object]:
    mt5 = json.loads(MT5_LOGIC_KPI.read_text(encoding="ascii"))
    proxy = run_r0007_proxy(write=False)
    ...
    upsert_artifact_lineage(
        ...
        [
            "campaigns/C0001_regime_response_discovery/runs/R0007/kpi/proxy.json",
            "campaigns/C0001_regime_response_discovery/runs/R0007/kpi/mt5_logic_parity.json",
        ],
    )
...
def record_r0007_execution_divergence() -> dict[str, object]:
    ...
    payload = {
        ...
        "conditional_profiles": {
            ...
            "spread_slippage_stress_profile": {
                "applies": False,
                "fields": {},
                "deferred_with_reason": "R0007 has baseline MT5 tick execution evidence only; spread and slippage stress sweeps are separate robustness work.",
            },
            "score_model_profile": {
                "applies": False,
                "fields": {},
                "deferred_with_reason": "R0007 is rule/proxy based and has no model score surface.",
            },
        ...
        "deferred_with_reason": [
            {
                ...
                "requirement_class": "deferred_with_reason",
                "reason": "baseline tick KPI is recorded; stress sweeps are not part of the current R0007 parity-to-tick split.",
                "blocking_condition": "requires a separate controlled spread/slippage tester matrix",
                "revisit_when": "after R0007 closeout review if this hypothesis remains worth robustness testing",
                "claim_boundary": {"claim_authority": False},
            }
        ],
        "claim_boundary": {
    ...
    upsert_artifact_lineage(
        ...
        [
            "campaigns/C0001_regime_response_discovery/runs/R0007/kpi/mt5_logic_parity.json",
            "campaigns/C0001_regime_response_discovery/runs/R0007/kpi/mt5_tick.json",
        ],
    )
...
def run_r0007_mt5_tick_by_fold_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    compile_r0007_ea()
    records: list[dict[str, Any]] = []
    for window in load_test_windows(ROLLING_WINDOWS):
        from_date, to_date = tester_dates_for_window(window)
        logic_result = run_r0007_tester(
        ...
        logic_payload = parse_r0007_mt5(logic_result, write_kpi=False)
        tick_result = run_r0007_tester(
        ...
        tick_payload = parse_r0007_mt5(tick_result, write_kpi=False)
        records.append(
            {
                "window": window,
                "from_date": from_date,
    ...
    upsert_artifact_lineage(
        ...
        [
            "campaigns/C0001_regime_response_discovery/runs/R0007/kpi/mt5_tick_by_fold.json",
...
def classify_session_gap_exception(
    ...
    applies = (
        not mt5_only_entries
        and len(proxy_only_entries) <= 2
        and mt5_only_exits
        and all(is_session_gap_boundary_time(key[0]) for key in proxy_only_entries)
        and all(is_session_gap_boundary_time(key[0]) for key in mt5_only_exits)
        and all(
            is_session_gap_boundary_time(key[0]) or is_session_gap_proxy_consequence(key, proxy_only_entries)
            for key in proxy_only_exits
        )
    )
    return {
        "applies": applies,
        ...
        "reason": (
            "MT5 tester session-gap trade availability delays one or more closes and may skip proxy-only follow-on trades"
            if applies
            else ""
        ),
        "blocking_condition": "" if applies else "mismatch pattern is not a recognized single session-gap tick-availability exception",
        "revisit_when": "if R0007 remains useful after tick execution evidence" if applies else "",
        "mismatch_samples": {
...
def is_session_gap_proxy_consequence(
    exit_key: tuple[datetime, int, str],
    proxy_only_entries: list[tuple[datetime, int]],
) -> bool:
    exit_time, direction, _reason = exit_key
    for entry_time, entry_direction in proxy_only_entries:
        if direction != entry_direction:
            continue
        if entry_time <= exit_time and (exit_time - entry_time).total_seconds() <= 4 * 60 * 60:
            return True
    return False
...
def upsert_artifact_lineage(
    ...
    next_record = {
        ...
        "mutable": False,
        "produced_by": "axiom_rift.mt5.r0007_probe",
        ...
        data["deferred_with_reason"] = [
            {
                ...
                "revisit_when": "during R0007 closeout review",
            }
        ]
...
def update_gate_after_execution_divergence(divergence_payload: dict[str, object]) -> None:
    ...
    data["next_action"] = "review_R0007_tick_execution_kpi_and_closeout"
    data["deferred_with_reason"] = [
        {
            "field": "run_closeout",
            "reason": "logic parity, tick execution KPI, and execution divergence are recorded; closeout judgment still requires review",
            "blocking_condition": "review tick economics and divergence before deciding whether R0007 remains useful",
            "revisit_when": "during R0007 closeout review",
        }
    ]
    GATE_REPORT.write_text(json.dumps(data, indent=2, sort_keys=True) + "\n", encoding="ascii")
...
def update_gate_after_fold_isolated_evidence(
    ...
    gate_data["next_action"] = "review_R0007_tick_execution_kpi_and_closeout"
    gate_data["deferred_with_reason"] = [
        {
            ...
            "blocking_condition": "review fold-isolated tick economics and divergence before closing R0007",
            "revisit_when": "during R0007 closeout review",
        }
    ]
...
def run_r0007_mt5_logic_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_r0007_tester(mode=LOGIC_PARITY_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_r0007_mt5(result)
    parity_payload = record_r0007_parity()
    return {
...
def run_r0007_mt5_tick_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_r0007_tester(mode=TICK_EXECUTION_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_r0007_mt5(result)
    divergence_payload = record_r0007_execution_divergence()
    return {
