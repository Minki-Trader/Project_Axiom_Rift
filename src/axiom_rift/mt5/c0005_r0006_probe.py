"""MT5 schedule-replay logic parity helpers for C0005 R0006."""
...
from axiom_rift.proxies.c0005_r0006_metric_rank_ensemble_analog_memory import (
    BASE_FRAME,
    ROLLING_WINDOWS,
    SplitWindow,
    Trade,
...
SCHEDULE_ARTIFACT = RUN_ARTIFACT_DIR / "c0005_r0006_schedule.csv"
SCHEDULE_COMMON_REL = "AxiomRift\\C0005\\R0006\\schedule\\c0005_r0006_schedule.csv"
STARTING_BALANCE_USD = 500.0
RESPONSE_MODE = "fold_local_metric_rank_ensemble_analog_memory_schedule_replay"
MAX_HOLD_BARS = 10
MAGIC = 500006
...
def compile_c0005_r0006_ea(metaeditor_exe: Path = DEFAULT_METAEDITOR_EXE) -> CompileResult:
    ...
    log = PROJECT_ROOT / "artifacts" / "reports" / "C0005_R0006_mt5_compile.log"
...
def tester_config_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    return PROJECT_ROOT / "artifacts" / "reports" / "C0005_R0006_mt5_tester" / f"C0005_R0006_{scoped_name(mode, output_scope)}_tester.ini"


def tester_report_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    return PROJECT_ROOT / "artifacts" / "reports" / "C0005_R0006_mt5_tester" / f"C0005_R0006_mt5_{scoped_name(mode, output_scope)}_report.htm"
...
def write_tester_config(
    ...
    config_dir = PROJECT_ROOT / "artifacts" / "reports" / "C0005_R0006_mt5_tester"
...
def parse_c0005_r0006_mt5(
    ...
    if mode == LOGIC_PARITY_MODE:
        ...
        upsert_artifact_lineage(
            "A-C0005-R0006-MT5-LOGIC-KPI",
            "mt5_logic_parity_kpi",
            "mt5_logic_parity",
            rel(MT5_LOGIC_KPI),
            sha256_file(MT5_LOGIC_KPI),
            [
                "campaigns/C0005_continuous_analog_memory_discovery/runs/R0006/artifacts/c0005_r0006_schedule.csv",
                "configs/market.yaml",
            ],
        )
    else:
        upsert_artifact_lineage(
            "A-C0005-R0006-MT5-TICK-KPI",
            "mt5_tick_kpi",
            "mt5_tick",
            rel(MT5_TICK_KPI),
        ...
        "deferred_with_reason": [
            {
                ...
                "reason": "C0005 R0006 logic parity replays the proxy supervised metric-rank ensemble analog-memory schedule; native model or ONNX materialization is not claimed at this stage",
...
def record_c0005_r0006_parity() -> dict[str, object]:
    ...
    next_action = "produce_c0005_r0006_mt5_tick_execution_evidence" if mechanical_ok else "repair_c0005_r0006_schedule_replay_parity"
    ...
    payload = {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": RUN_ID,
        "parity_id": "P-C0005-R0006",
        "proxy_id": "PX-C0005-R0006",
        "mt5_probe_id": "MT5-C0005-R0006",
        ...
        "conditional_profiles": {
            ...
            "analog_memory_schedule_replay_profile": {
                ...
                "fields": {
                    ...
                    "logic_boundary": "proxy_selected_fold_local_metric_rank_ensemble_analog_memory_schedule_replayed_in_mt5_closed_bar_mode",
    ...
    upsert_artifact_lineage(
        "A-C0005-R0006-PROXY-VS-MT5-LOGIC-KPI",
        ...
        [
            "campaigns/C0005_continuous_analog_memory_discovery/runs/R0006/kpi/proxy.json",
            "campaigns/C0005_continuous_analog_memory_discovery/runs/R0006/kpi/mt5_logic_parity.json",
        ],
    )
...
def update_run_after_mt5(mode: str) -> None:
    ...
    evidence_paths["mt5_schedule_artifact"] = "artifacts/c0005_r0006_schedule.csv"
    mt5_plan = data.setdefault("mt5_probe_plan", {})
    mt5_plan["logic_parity_kpi_path"] = "kpi/mt5_logic_parity.json"
    mt5_plan["logic_parity_purpose"] = "proxy_selected_metric_rank_ensemble_analog_memory_schedule_vs_ea_closed_bar_lifecycle_parity"
...
def update_gate_after_parity(parity_payload: dict[str, object]) -> None:
    ...
    evidence_paths = data.setdefault("evidence_paths", [])
    for path in ("kpi/mt5_logic_parity.json", "kpi/proxy_vs_mt5_logic_parity.json", "artifacts/c0005_r0006_schedule.csv"):
        ...
        data["deferred_with_reason"] = [
            {
                ...
                "blocking_condition": "produce_c0005_r0006_mt5_tick_execution_evidence",
                "revisit_when": "after C0005 R0006 tick execution KPI and execution divergence are recorded",
            }
        ]
...
def update_reentry_after_logic_parity(next_action: str, mechanical_ok: bool) -> None:
    ...
    for item in (
        "produce_c0005_r0006_mt5_logic_parity_evidence",
        "record_c0005_r0006_proxy_vs_mt5_logic_parity_evidence",
    ):
...
def update_campaign_after_logic_parity(next_action: str) -> None:
    ...
    next_candidate["direction"] = "active_c0005_r0006_tick_execution" if "mt5_tick" in next_action else "active_c0005_r0006_parity_repair"
    next_candidate["reason"] = (
        "R0006 closed-bar MT5 logic parity and proxy-vs-MT5 parity are recorded; next work is mandatory tick execution evidence."
        if "mt5_tick" in next_action
        else "R0006 closed-bar MT5 logic parity is recorded but mechanical parity repair is required before hypothesis judgment."
    )
...
def update_claim_state_after_logic_parity(parity_payload: dict[str, object], mt5_payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0005_r0006_mt5_logic_parity_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0005_continuous_analog_memory_discovery/runs/R0006/kpi/mt5_logic_parity.json",
        "paired_parity_source": "campaigns/C0005_continuous_analog_memory_discovery/runs/R0006/kpi/proxy_vs_mt5_logic_parity.json",
...
def record_c0005_r0006_execution_divergence() -> dict[str, object]:
        ...
        "deferred_with_reason": [
            {
                ...
                "revisit_when": "after C0005 R0006 fold-isolated MT5 tick evidence",
    ...
    upsert_artifact_lineage(
        "A-C0005-R0006-EXECUTION-DIVERGENCE-KPI",
        "diagnostic_output",
        ...
        [
            "campaigns/C0005_continuous_analog_memory_discovery/runs/R0006/kpi/mt5_logic_parity.json",
            "campaigns/C0005_continuous_analog_memory_discovery/runs/R0006/kpi/mt5_tick.json",
        ],
    )
...
def update_gate_after_execution_divergence(divergence_payload: dict[str, object]) -> None:
    ...
    for path in (
        ...
        "artifacts/c0005_r0006_schedule.csv",
    ):
        if path not in evidence_paths:
            evidence_paths.append(path)
    data["decision"] = "defer_with_reason"
    data["next_action"] = "produce_c0005_r0006_fold_isolated_mt5_tick_kpi"
    data["deferred_with_reason"] = [
        {
            "field": "run_closeout",
            ...
            "blocking_condition": "produce_c0005_r0006_fold_isolated_mt5_tick_kpi",
...
def update_reentry_after_tick_and_divergence() -> None:
    ...
    completed = list(next_work.get("completed") or [])
    for item in (
        "produce_c0005_r0006_mt5_tick_execution_evidence",
        "record_c0005_r0006_execution_divergence",
    ):
        if item not in completed:
            completed.append(item)
    next_action = "produce_c0005_r0006_fold_isolated_mt5_tick_kpi"
    completed = [item for item in completed if item != next_action]
    next_work["completed"] = completed
    next_work["tasks"] = [next_action]
    path.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def update_campaign_after_tick_and_divergence() -> None:
    ...
    run_index = data.setdefault("run_index", {})
    next_candidate = run_index.setdefault("next_run_candidate", {})
    next_candidate["direction"] = "active_c0005_r0006_fold_isolated_tick"
    next_candidate["reason"] = "R0006 aggregate tick execution and execution divergence are recorded; next work is mandatory fold-isolated MT5 tick evidence."
    next_candidate["status"] = "active_run_open"
    CAMPAIGN.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def update_claim_state_after_tick_and_divergence(divergence_payload: dict[str, object], tick_payload: dict[str, object]) -> None:
    ...
    data["active_run"] = "campaigns/C0005_continuous_analog_memory_discovery/runs/R0006"
    data["latest_operation"] = {
        "id": "produce_c0005_r0006_mt5_tick_execution_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0005_continuous_analog_memory_discovery/runs/R0006/kpi/mt5_tick.json",
        "paired_divergence_source": "campaigns/C0005_continuous_analog_memory_discovery/runs/R0006/kpi/execution_divergence.json",
        ...
        "economics_shift_status": required.get("economics_shift_status"),
        "next_required_action": "produce_c0005_r0006_fold_isolated_mt5_tick_kpi",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def update_gate_after_fold_isolated_evidence(
    ...
    gate_data["next_action"] = "review_c0005_r0006_tick_execution_kpi_and_closeout"
    gate_data["deferred_with_reason"] = [
        {
            ...
            "blocking_condition": "review_c0005_r0006_tick_execution_kpi_and_closeout",
            "revisit_when": "during C0005 R0006 closeout review",
        }
    ]
...
def update_reentry_after_fold_isolated() -> None:
    ...
    completed = list(next_work.get("completed") or [])
    for item in (
        "produce_c0005_r0006_fold_isolated_mt5_tick_kpi",
        "produce_c0005_r0006_fold_isolated_execution_divergence",
    ):
        if item not in completed:
            completed.append(item)
    next_action = "review_c0005_r0006_tick_execution_kpi_and_closeout"
    completed = [item for item in completed if item != next_action]
    next_work["completed"] = completed
    next_work["tasks"] = [next_action]
    path.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def update_campaign_after_fold_isolated() -> None:
    ...
    run_index = data.setdefault("run_index", {})
    next_candidate = run_index.setdefault("next_run_candidate", {})
    next_candidate["direction"] = "active_c0005_r0006_closeout_review"
    next_candidate["reason"] = "R0006 fold-isolated MT5 tick KPI and fold-isolated execution divergence are recorded; next work is closeout review."
    next_candidate["status"] = "active_run_open"
    CAMPAIGN.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def update_claim_state_after_fold_isolated(
    ...
    data["active_run"] = "campaigns/C0005_continuous_analog_memory_discovery/runs/R0006"
    data["latest_operation"] = {
        "id": "produce_c0005_r0006_fold_isolated_mt5_tick_kpi",
        "status": "completed",
        "recorded_at_source": "campaigns/C0005_continuous_analog_memory_discovery/runs/R0006/kpi/mt5_tick_by_fold.json",
        "paired_divergence_source": "campaigns/C0005_continuous_analog_memory_discovery/runs/R0006/kpi/execution_divergence_by_fold.json",
        ...
        "next_required_action": "review_c0005_r0006_tick_execution_kpi_and_closeout",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def upsert_artifact_lineage(
    ...
    next_record = {
        ...
        "mutable": False,
        "produced_by": "axiom_rift.mt5.c0005_r0006_probe",
        ...
        data["deferred_with_reason"] = [
            {
                ...
                "next_action": "produce_c0005_r0006_fold_isolated_execution_divergence",
            }
        ]
        ...
        data["deferred_with_reason"] = [
            {
                ...
                "next_action": "produce_c0005_r0006_mt5_tick_execution_evidence",
            }
        ]
    ARTIFACT_LINEAGE.write_text(json.dumps(data, indent=2, sort_keys=True) + "\n", encoding="ascii")


def run_c0005_r0006_mt5_logic_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_c0005_r0006_tester(mode=LOGIC_PARITY_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_c0005_r0006_mt5(result)
    parity_payload = record_c0005_r0006_parity()
    return {
...
def run_c0005_r0006_mt5_tick_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_c0005_r0006_tester(mode=TICK_EXECUTION_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_c0005_r0006_mt5(result)
    divergence_payload = record_c0005_r0006_execution_divergence()
    return {
...
def run_c0005_r0006_mt5_tick_by_fold_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    compile_c0005_r0006_ea()
    write_schedule_files()
    records: list[dict[str, Any]] = []
    for window in load_test_windows():
        from_date, to_date = tester_dates_for_window(window)
        logic_result = run_c0005_r0006_tester(
        ...
        logic_payload = parse_c0005_r0006_mt5(logic_result, write_kpi=False)
        tick_result = run_c0005_r0006_tester(
        ...
        tick_payload = parse_c0005_r0006_mt5(tick_result, write_kpi=False)
        records.append(
            {
                "window": window,
                "from_date": from_date,
    ...
    upsert_artifact_lineage(
        "A-C0005-R0006-MT5-TICK-BY-FOLD-KPI",
    ...
    upsert_artifact_lineage(
        "A-C0005-R0006-EXECUTION-DIVERGENCE-BY-FOLD-KPI",
        "diagnostic_output",
        "execution_divergence_by_fold",
        rel(EXECUTION_DIVERGENCE_BY_FOLD_KPI),
        sha256_file(EXECUTION_DIVERGENCE_BY_FOLD_KPI),
        [
            "campaigns/C0005_continuous_analog_memory_discovery/runs/R0006/kpi/mt5_tick_by_fold.json",
            "registries/rolling_windows.yaml",
        ],
    )
...
def build_mt5_tick_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": RUN_ID,
        "mt5_probe_id": "MT5-C0005-R0006-BY-FOLD",
...
def build_execution_divergence_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": RUN_ID,
        "divergence_id": "ED-C0005-R0006-BY-FOLD",
        "split_policy": "rolling_window_test_oos_fold_isolated",
        "split_registry": "registries/rolling_windows.yaml",
