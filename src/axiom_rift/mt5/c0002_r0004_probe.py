"""MT5 schedule-replay logic parity helpers for C0002 R0004."""
...
from axiom_rift.proxies.c0002_r0004_stump_rank_ensemble import (
    BASE_FRAME,
    ROLLING_WINDOWS,
    SplitWindow,
    Trade,
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0002_score_conditioned_intraday_edge" / "runs" / "R0004"
...
SCHEDULE_ARTIFACT = RUN_ARTIFACT_DIR / "c0002_r0004_schedule.csv"
SCHEDULE_COMMON_REL = "AxiomRift\\C0002\\R0004\\schedule\\c0002_r0004_schedule.csv"
STARTING_BALANCE_USD = 500.0
R0004_RESPONSE_MODE = "score_conditioned_stump_rank_ensemble_schedule_replay"
R0004_MAX_HOLD_BARS = 12
...
def compile_c0002_r0004_ea(metaeditor_exe: Path = DEFAULT_METAEDITOR_EXE) -> CompileResult:
    ...
    log = PROJECT_ROOT / "artifacts" / "reports" / "C0002_R0004_mt5_compile.log"
...
def tester_config_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    return PROJECT_ROOT / "artifacts" / "reports" / "C0002_R0004_mt5_tester" / f"C0002_R0004_{scoped_name(mode, output_scope)}_tester.ini"


def tester_report_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    return PROJECT_ROOT / "artifacts" / "reports" / "C0002_R0004_mt5_tester" / f"C0002_R0004_mt5_{scoped_name(mode, output_scope)}_report.htm"
...
def write_tester_config(
    ...
    config_dir = PROJECT_ROOT / "artifacts" / "reports" / "C0002_R0004_mt5_tester"
    ...
    lines = [
        ...
        f"InpResponseMode={R0004_RESPONSE_MODE}",
        f"InpSchedulePath={SCHEDULE_COMMON_REL}",
        "InpMagic=200004",
        "InpLot=0.01",
        f"InpMaxHoldBars={R0004_MAX_HOLD_BARS}",
...
def parse_c0002_r0004_mt5(
    ...
    if mode == LOGIC_PARITY_MODE:
        upsert_artifact_lineage(
            ...
            ["campaigns/C0002_score_conditioned_intraday_edge/runs/R0004/kpi/proxy.json"],
        )
        upsert_artifact_lineage(
            "A0004",
            ...
            [
                "campaigns/C0002_score_conditioned_intraday_edge/runs/R0004/artifacts/c0002_r0004_schedule.csv",
                "configs/market.yaml",
            ],
        )
    else:
...
def assert_mt5_mode_matches_status(mode: str, status: dict[str, str], output_scope: str | None = None) -> None:
    ...
    if status.get("response_mode") != R0004_RESPONSE_MODE:
        raise RuntimeError(
            "MT5 response_mode mismatch: "
            f"expected={R0004_RESPONSE_MODE} actual={status.get('response_mode')}"
        )
        ...
        "deferred_with_reason": [
            {
                ...
                "reason": "C0002 R0004 logic parity replays the train-only score-selected schedule; native model or ONNX materialization is not claimed at this stage",
...
def record_c0002_r0004_parity() -> dict[str, object]:
    ...
    next_action = "produce_c0002_r0004_mt5_tick_execution_evidence" if mechanical_ok else "repair_c0002_r0004_schedule_replay_parity"
    ...
    upsert_artifact_lineage(
        ...
        [
            "campaigns/C0002_score_conditioned_intraday_edge/runs/R0004/kpi/proxy.json",
            "campaigns/C0002_score_conditioned_intraday_edge/runs/R0004/kpi/mt5_logic_parity.json",
        ],
    )
...
def record_c0002_r0004_execution_divergence() -> dict[str, object]:
        ...
        "deferred_with_reason": [
            {
                ...
                "revisit_when": "after C0002 R0004 fold-isolated MT5 tick evidence",
    ...
    upsert_artifact_lineage(
        ...
        [
            "campaigns/C0002_score_conditioned_intraday_edge/runs/R0004/kpi/mt5_logic_parity.json",
            "campaigns/C0002_score_conditioned_intraday_edge/runs/R0004/kpi/mt5_tick.json",
        ],
    )
    update_gate_after_execution_divergence(payload)
    update_reentry_after_tick_and_divergence()
    update_campaign_remaining_question("produce_c0002_r0004_fold_isolated_mt5_tick_kpi")
    return payload
...
def update_run_after_mt5(mode: str) -> None:
    ...
    evidence_paths["mt5_schedule_artifact"] = "artifacts/c0002_r0004_schedule.csv"
...
def update_gate_after_parity(parity_payload: dict[str, object]) -> None:
    ...
    evidence_paths = data.setdefault("evidence_paths", [])
    for path in ("kpi/mt5_logic_parity.json", "kpi/proxy_vs_mt5_logic_parity.json", "artifacts/c0002_r0004_schedule.csv"):
        ...
        data["deferred_with_reason"] = [
            {
                ...
                "blocking_condition": "produce_c0002_r0004_mt5_tick_execution_evidence",
                "revisit_when": "after C0002 R0004 tick execution KPI and execution divergence are recorded",
            }
        ]
...
def update_gate_after_execution_divergence(divergence_payload: dict[str, object]) -> None:
    ...
    for path in (
        ...
        "artifacts/c0002_r0004_schedule.csv",
    ):
        if path not in evidence_paths:
            evidence_paths.append(path)
    data["decision"] = "defer_with_reason"
    data["next_action"] = "produce_c0002_r0004_fold_isolated_mt5_tick_kpi"
    data["deferred_with_reason"] = [
        {
            "field": "run_closeout",
            ...
            "blocking_condition": "produce_c0002_r0004_fold_isolated_mt5_tick_kpi",
...
def update_gate_after_fold_isolated_evidence(
    ...
    gate_data["next_action"] = "review_c0002_r0004_tick_execution_kpi_and_closeout"
    gate_data["deferred_with_reason"] = [
        {
            ...
            "blocking_condition": "review_c0002_r0004_tick_execution_kpi_and_closeout",
            "revisit_when": "during C0002 R0004 closeout review",
        }
    ]
...
def upsert_artifact_lineage(
    ...
    next_record = {
        ...
        "mutable": False,
        "produced_by": "axiom_rift.mt5.c0002_r0004_probe",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0002_r0004_mt5_tick_execution_evidence",
        }
    ]
...
def update_reentry_after_logic_parity(next_action: str, mechanical_ok: bool) -> None:
    ...
    completed_items = [
        "produce_c0002_r0004_mt5_logic_parity_evidence",
        "record_c0002_r0004_proxy_vs_mt5_logic_parity_evidence",
    ]
    if "repair_c0002_r0004_schedule_replay_parity" in previous_tasks and mechanical_ok:
        completed_items.append("repair_c0002_r0004_schedule_replay_parity")
...
def update_reentry_after_tick_and_divergence() -> None:
    ...
    completed = list(next_work.get("completed") or [])
    for item in (
        "produce_c0002_r0004_mt5_tick_execution_evidence",
        "record_c0002_r0004_execution_divergence",
    ):
        if item not in completed:
            completed.append(item)
    next_action = "produce_c0002_r0004_fold_isolated_mt5_tick_kpi"
    completed = [item for item in completed if item != next_action]
    next_work["completed"] = completed
    next_work["tasks"] = [next_action]
    path.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def update_reentry_after_fold_isolated() -> None:
    ...
    completed = list(next_work.get("completed") or [])
    for item in (
        "produce_c0002_r0004_fold_isolated_mt5_tick_kpi",
        "produce_c0002_r0004_fold_isolated_execution_divergence",
    ):
        if item not in completed:
            completed.append(item)
    next_action = "review_c0002_r0004_tick_execution_kpi_and_closeout"
    completed = [item for item in completed if item != next_action]
    next_work["completed"] = completed
    next_work["tasks"] = [next_action]
    path.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def run_c0002_r0004_mt5_logic_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_c0002_r0004_tester(mode=LOGIC_PARITY_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_c0002_r0004_mt5(result)
    parity_payload = record_c0002_r0004_parity()
    return {
...
def run_c0002_r0004_mt5_tick_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_c0002_r0004_tester(mode=TICK_EXECUTION_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_c0002_r0004_mt5(result)
    divergence_payload = record_c0002_r0004_execution_divergence()
    return {
...
def run_c0002_r0004_mt5_tick_by_fold_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    compile_c0002_r0004_ea()
    write_schedule_files()
    records: list[dict[str, Any]] = []
    for window in load_test_windows():
        from_date, to_date = tester_dates_for_window(window)
        logic_result = run_c0002_r0004_tester(
        ...
        logic_payload = parse_c0002_r0004_mt5(logic_result, write_kpi=False)
        tick_result = run_c0002_r0004_tester(
        ...
        tick_payload = parse_c0002_r0004_mt5(tick_result, write_kpi=False)
        records.append(
            {
                "window": window,
                "from_date": from_date,
    ...
    upsert_artifact_lineage(
        ...
        [
            "campaigns/C0002_score_conditioned_intraday_edge/runs/R0004/kpi/mt5_tick_by_fold.json",
            "registries/rolling_windows.yaml",
        ],
    )
    ...
    update_campaign_remaining_question("review_c0002_r0004_tick_execution_kpi_and_closeout")
    return {
...
def build_mt5_tick_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": "R0004",
        "mt5_probe_id": "MT5C0002R0004_BY_FOLD",
