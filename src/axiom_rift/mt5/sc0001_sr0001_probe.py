...
from axiom_rift.proxies.sc0001_sr0001_synthesis_constraints import (
    BASE_FRAME,
    ROLLING_WINDOWS,
    TIME_FORMAT,
    SplitWindow,
...
def compile_sc0001_sr0001_ea(metaeditor_exe: Path | None = None) -> CompileResult:
...
def build_mt5_tick_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": WORK_UNIT_ID,
        "run_id": RUN_ID,
        "mt5_probe_id": "MT5-SC0001-SR0001-BY-FOLD",
...
def build_execution_divergence_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": WORK_UNIT_ID,
        "run_id": RUN_ID,
        "divergence_id": "ED-SC0001-SR0001-BY-FOLD",
        "split_policy": "rolling_window_test_oos_fold_isolated",
        "split_registry": "registries/rolling_windows.yaml",
...
def update_gate_after_fold_isolated_evidence(
    ...
    for path in (
        ...
        "artifacts/sc0001_sr0001_schedule.csv",
    ):
        if path not in evidence_paths:
            evidence_paths.append(path)
    gate_data["decision"] = "defer_with_reason"
    gate_data["next_action"] = "review_sc0001_sr0001_tick_execution_kpi_and_closeout"
    gate_data["deferred_with_reason"] = [
        {
            ...
            "blocking_condition": "review_sc0001_sr0001_tick_execution_kpi_and_closeout",
            "revisit_when": "during SC0001 SR0001 closeout review",
        }
    ]
...
def update_reentry_after_fold_isolated() -> None:
    ...
    for item in (
        ...
        "produce_sc0001_sr0001_fold_isolated_execution_divergence",
    ):
        if item not in completed:
            completed.append(item)
    next_action = "review_sc0001_sr0001_tick_execution_kpi_and_closeout"
    completed = [item for item in completed if item != next_action]
    next_work["completed"] = completed
    next_work["tasks"] = [next_action]
    path.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def update_synthesis_queue_after_fold_isolated() -> None:
    ...
    for item in data.get("queue", []):
        if item.get("synthesis_run_id") == RUN_ID:
            ...
            item["last_completed_step"] = "produce_sc0001_sr0001_fold_isolated_execution_divergence"
            item["next_action"] = "review_sc0001_sr0001_tick_execution_kpi_and_closeout"
    SYNTHESIS_QUEUE.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def upsert_artifact_lineage(
    ...
    if EXECUTION_DIVERGENCE_BY_FOLD_KPI.exists():
        ...
        deferred_next_action = "review_sc0001_sr0001_tick_execution_kpi_and_closeout"
        ...
        deferred_next_action = "produce_sc0001_sr0001_fold_isolated_mt5_tick_kpi"
    else:
        ...
        deferred_next_action = "produce_sc0001_sr0001_mt5_tick_execution_evidence"
    data["deferred_with_reason"] = [
        {
...
def run_sc0001_sr0001_mt5_tick_by_fold_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    compile_sc0001_sr0001_ea()
    write_schedule_files()
    records: list[dict[str, Any]] = []
    for window in load_test_windows():
        from_date, to_date = tester_dates_for_window(window)
        logic_result = run_sc0001_sr0001_tester(
        ...
        logic_payload = parse_sc0001_sr0001_mt5(logic_result, mode=LOGIC_PARITY_MODE, write_kpi=False)
        tick_result = run_sc0001_sr0001_tester(
        ...
        tick_payload = parse_sc0001_sr0001_mt5(tick_result, mode=TICK_EXECUTION_MODE, write_kpi=False)
        records.append(
            {
                "window": window,
                "from_date": from_date,
    ...
    upsert_artifact_lineage(
        "A-SC0001-SR0001-MT5-TICK-BY-FOLD-KPI",
    ...
    upsert_artifact_lineage(
        "A-SC0001-SR0001-EXEC-DIVERGENCE-BY-FOLD-KPI",
        ...
        [
            "campaigns/SC0001_accumulated_negative_memory_synthesis/runs/SR0001/kpi/mt5_tick_by_fold.json",
            "registries/rolling_windows.yaml",
        ],
    )
