"""MT5 schedule-replay helpers for C0112 R0001."""
...
from axiom_rift.proxies.c0112_r0001_intraday_micro_consolidation_breakout_continuation import (
    BASE_FRAME,
    ROLLING_WINDOWS,
    SplitWindow,
    Trade,
...
CAMPAIGN_ID = "C0112"
RUN_ID = "R0001"
CAMPAIGN_ROOT = PROJECT_ROOT / "campaigns" / "C0112_intraday_micro_consolidation_breakout_continuation_discovery"
...
SCHEDULE_ARTIFACT = RUN_ARTIFACT_DIR / "c0112_r0001_schedule.csv"
SCHEDULE_COMMON_REL = "AxiomRift\\C0112\\R0001\\schedule\\c0112_r0001_schedule.csv"
STARTING_BALANCE_USD = starting_balance_usd()
LOGIC_PARITY_STARTING_BALANCE_USD = 100000.0
RESPONSE_MODE = "fold_local_intraday_micro_consolidation_breakout_continuation_schedule_replay"
MAX_HOLD_BARS = 10
MAGIC = 1112001
...
NEXT_MAJOR_ACTION = "choose_c0113_new_major_hypothesis_after_c0112_closeout"
PREVIOUS_CAMPAIGN_REL = "campaigns/C0111_intraday_cost_efficient_drift_continuation_discovery"
C0112_CLOSE_REASON = (
...
def compile_c0112_r0001_ea(metaeditor_exe: Path | None = None) -> CompileResult:
    ...
    log = PROJECT_ROOT / "artifacts" / "reports" / "c0112_r0001_mt5_compile.log"
...
def tester_config_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    return PROJECT_ROOT / "artifacts" / "reports" / "c0112_r0001_mt5_tester" / f"c0112_r0001_{scoped_name(mode, output_scope)}_tester.ini"


def tester_report_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    return PROJECT_ROOT / "artifacts" / "reports" / "c0112_r0001_mt5_tester" / f"c0112_r0001_mt5_{scoped_name(mode, output_scope)}_report.htm"
...
def write_tester_config(
    ...
    config_dir = PROJECT_ROOT / "artifacts" / "reports" / "c0112_r0001_mt5_tester"
    ...
    if write_kpi:
        ...
        if mode == LOGIC_PARITY_MODE:
            upsert_artifact_lineage(
                "A-C0112-R0001-MT5-SCHEDULE",
                "mt5_schedule_csv",
                "mt5_logic_parity_input",
                rel(SCHEDULE_ARTIFACT),
                sha256_file(SCHEDULE_ARTIFACT),
                ["campaigns/C0112_intraday_micro_consolidation_breakout_continuation_discovery/runs/R0001/kpi/proxy.json"],
            )
            upsert_artifact_lineage(
                "A-C0112-R0001-MT5-LOGIC-KPI",
                "mt5_logic_parity_kpi",
                "mt5_logic_parity",
                rel(MT5_LOGIC_KPI),
                sha256_file(MT5_LOGIC_KPI),
                [
                    "campaigns/C0112_intraday_micro_consolidation_breakout_continuation_discovery/runs/R0001/artifacts/c0112_r0001_schedule.csv",
                    "configs/market.yaml",
                ],
            )
        else:
            upsert_artifact_lineage(
                "A-C0112-R0001-MT5-TICK-KPI",
                "mt5_tick_kpi",
                "mt5_tick",
                rel(MT5_TICK_KPI),
    ...
    return {
        ...
        **runtime_payload_fields(),
        ...
        "conditional_profiles": {
            ...
            "intraday_micro_consolidation_breakout_continuation_schedule_profile": {
                "applies": True,
                "fields": {
                    "intraday_micro_consolidation_breakout_continuation_source": "rolling_train_is_proxy_schedule",
        ...
        "deferred_with_reason": [
            {
                "field": "native_mql_or_onnx_intraday_micro_consolidation_breakout_continuation_surface",
...
def record_c0112_r0001_parity() -> dict[str, object]:
    ...
    next_action = "produce_c0112_r0001_mt5_tick_execution_evidence" if mechanical_ok else "repair_c0112_r0001_schedule_replay_parity"
    ...
    payload = {
        ...
        "mechanical_checks": {
            ...
            "intraday_micro_consolidation_breakout_continuation_schedule_match": "passed" if entry_compare["key_match_rate"] == 1.0 else "failed",
        ...
        "conditional_profiles": {
            ...
            "intraday_micro_consolidation_breakout_continuation_schedule_replay_profile": {
                "applies": True,
                "fields": {
                    "schedule_artifact_path": rel(SCHEDULE_ARTIFACT),
                    "schedule_hash": sha256_file(SCHEDULE_ARTIFACT) if SCHEDULE_ARTIFACT.exists() else None,
                    "logic_boundary": "proxy_selected_fold_local_intraday_micro_consolidation_breakout_continuation_schedule_replayed_in_mt5_closed_bar_mode",
        ...
        "deferred_with_reason": [
            {
                ...
                "requirement_class": "deferred_with_reason",
                "reason": "logic parity uses schedule replay only; native intraday micro consolidation breakout continuation materialization is deferred until candidate quality exists",
                "blocking_condition": "C0112 R0001 does not yet have fold-isolated tick evidence",
    ...
    upsert_artifact_lineage(
        "A-C0112-R0001-PROXY-VS-MT5-LOGIC-KPI",
        ...
        [
            "campaigns/C0112_intraday_micro_consolidation_breakout_continuation_discovery/runs/R0001/kpi/proxy.json",
            "campaigns/C0112_intraday_micro_consolidation_breakout_continuation_discovery/runs/R0001/kpi/mt5_logic_parity.json",
        ],
    )
...
def record_c0112_r0001_execution_divergence() -> dict[str, object]:
        ...
        "deferred_with_reason": [
            {
                ...
                "next_action": "produce_c0112_r0001_fold_isolated_mt5_tick_kpi",
            }
        ],
        "claim_boundary": {
            "claim_authority": False,
    ...
    upsert_artifact_lineage(
        "A-C0112-R0001-EXECUTION-DIVERGENCE-KPI",
        "diagnostic_output",
        ...
        [
            "campaigns/C0112_intraday_micro_consolidation_breakout_continuation_discovery/runs/R0001/kpi/mt5_logic_parity.json",
            "campaigns/C0112_intraday_micro_consolidation_breakout_continuation_discovery/runs/R0001/kpi/mt5_tick.json",
        ],
    )
...
def run_c0112_r0001_mt5_logic_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_c0112_r0001_tester(mode=LOGIC_PARITY_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_c0112_r0001_mt5(result)
    parity_payload = record_c0112_r0001_parity()
    return {
...
def run_c0112_r0001_mt5_tick_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_c0112_r0001_tester(mode=TICK_EXECUTION_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_c0112_r0001_mt5(result)
    divergence_payload = record_c0112_r0001_execution_divergence()
    return {
...
def run_c0112_r0001_mt5_tick_by_fold_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    compile_c0112_r0001_ea()
    write_schedule_files()
    records: list[dict[str, Any]] = []
    for window in load_test_windows():
        from_date, to_date = tester_dates_for_window(window)
        logic_result = run_c0112_r0001_tester(
        ...
        logic_payload = parse_c0112_r0001_mt5(logic_result, write_kpi=False)
        tick_result = run_c0112_r0001_tester(
        ...
        tick_payload = parse_c0112_r0001_mt5(tick_result, write_kpi=False)
        records.append(
            {
                "window": window,
                "from_date": from_date,
    ...
    upsert_artifact_lineage(
        "A-C0112-R0001-MT5-TICK-BY-FOLD-KPI",
    ...
    upsert_artifact_lineage(
        "A-C0112-R0001-EXECUTION-DIVERGENCE-BY-FOLD-KPI",
        "diagnostic_output",
        "execution_divergence_by_fold",
        rel(EXECUTION_DIVERGENCE_BY_FOLD_KPI),
        sha256_file(EXECUTION_DIVERGENCE_BY_FOLD_KPI),
        [
            "campaigns/C0112_intraday_micro_consolidation_breakout_continuation_discovery/runs/R0001/kpi/mt5_tick_by_fold.json",
            "registries/rolling_windows.yaml",
        ],
    )
...
def build_mt5_tick_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": RUN_ID,
        "mt5_probe_id": "MT5-C0112-R0001-BY-FOLD",
...
def build_execution_divergence_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": RUN_ID,
        "divergence_id": "ED-C0112-R0001-BY-FOLD",
        "split_policy": "rolling_window_test_oos_fold_isolated",
        "split_registry": "registries/rolling_windows.yaml",
...
def update_after_fold_isolated(tick_payload: dict[str, object], divergence_payload: dict[str, object]) -> None:
    next_action = "review_c0112_r0001_tick_execution_kpi_and_closeout"
    ...
    gate["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "during C0112 R0001 closeout review",
        }
    ]
    ...
    for item in (
        "produce_c0112_r0001_mt5_tick_execution_evidence",
        "record_c0112_r0001_execution_divergence",
        "produce_c0112_r0001_fold_isolated_mt5_tick_kpi",
        "produce_c0112_r0001_fold_isolated_execution_divergence",
    ):
        if item not in completed:
            completed.append(item)
    next_work["completed"] = [item for item in completed if item != next_action]
    next_work["tasks"] = [next_action]
    data["active_run"] = "campaigns/C0112_intraday_micro_consolidation_breakout_continuation_discovery/runs/R0001"
    ...
    update_claim_state(
        operation_id="produce_c0112_r0001_fold_isolated_execution_divergence",
        source="campaigns/C0112_intraday_micro_consolidation_breakout_continuation_discovery/runs/R0001/kpi/execution_divergence_by_fold.json",
...
def update_run_after_mt5(mode: str) -> None:
    ...
    evidence["mt5_schedule_artifact"] = "artifacts/c0112_r0001_schedule.csv"
    if mode == LOGIC_PARITY_MODE:
...
def update_after_parity(payload: dict[str, object], next_action: str) -> None:
    ...
    update_campaign_next(next_action)
    update_reentry_next("record_c0112_r0001_proxy_vs_mt5_logic_parity_evidence", next_action)
    update_claim_state(
        operation_id="record_c0112_r0001_proxy_vs_mt5_logic_parity_evidence",
        source="campaigns/C0112_intraday_micro_consolidation_breakout_continuation_discovery/runs/R0001/kpi/proxy_vs_mt5_logic_parity.json",
...
def update_after_execution_divergence(payload: dict[str, object]) -> None:
    ...
    update_campaign_next(next_action)
    update_reentry_next("record_c0112_r0001_execution_divergence", next_action)
    update_claim_state(
        operation_id="record_c0112_r0001_execution_divergence",
        source="campaigns/C0112_intraday_micro_consolidation_breakout_continuation_discovery/runs/R0001/kpi/execution_divergence.json",
...
def update_gate_next(status: str, next_action: str) -> None:
    ...
    for path in (
        ...
        "kpi/mt5_tick.json",
        "kpi/execution_divergence.json",
        "artifacts/c0112_r0001_schedule.csv",
    ):
        if path not in paths:
            paths.append(path)
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "after C0112 R0001 fold-isolated MT5 evidence files contain measured results",
        }
    ]
...
def update_reentry_next(completed_item: str, next_action: str) -> None:
    ...
    next_work = data.setdefault("next_work", {})
    completed = list(next_work.get("completed") or [])
    for item in ("produce_c0112_r0001_proxy_evidence", completed_item):
        if item not in completed:
            completed.append(item)
...
def update_claim_state(operation_id: str, source: str, evidence_status: str, next_action: str, extra: dict[str, object]) -> None:
    ...
    data["active_campaign"] = "campaigns/C0112_intraday_micro_consolidation_breakout_continuation_discovery"
    data["active_run"] = "campaigns/C0112_intraday_micro_consolidation_breakout_continuation_discovery/runs/R0001"
    latest = {
        "id": operation_id,
        "status": "completed",
...
def c0112_closeout_kpi_summary() -> dict[str, Any]:
    ...
    return {
        ...
        "remaining_work_classification": (
            "no_candidate; further C0112 work would be adjacent threshold, stop, target, hold, session, "
            "activity, spread, capital, or micro-consolidation-breakout-continuation feature nudge tuning"
        ),
    }


def c0112_negative_memory_asset(summary: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "onnx_ready": False,
        "hypothesis_tested": (
            "fold-local intraday micro consolidation breakout continuation rank over impulse efficiency, "
            "digestion compression, range retention, adverse overlap control, volume cooldown, absorption "
            "balance, close hold, VWAP hold, prior impulse context, dwell ratio, touch density, and spread "
            "stress inverse features"
        ),
        ...
        "next_boundary": (
            "C0112 R0001 shows the tested micro consolidation breakout continuation surface is not a candidate as implemented; "
            "remaining C0112 variants would be adjacent tuning, so next work should choose a new major C0113 "
...
def c0112_closeout_review(closed_at: str) -> dict[str, Any]:
    summary = c0112_closeout_kpi_summary()
    return {
        "status": "closed_no_candidate",
        "basis": "rolling_window_fold_isolated_mt5_tick_and_execution_divergence",
        "close_reason": C0112_CLOSE_REASON,
        "closed_at_utc": closed_at,
        "required_kpi_summary": summary,
        "negative_memory": c0112_negative_memory_asset(summary),
    ...
    evidence.update(
        {
            ...
            "proxy_trade_artifact": "artifacts/c0112_r0001_proxy_trades.csv",
            "micro_consolidation_breakout_continuation_summary": "artifacts/c0112_r0001_micro_consolidation_breakout_continuation_summary.json",
            "mt5_schedule_artifact": "artifacts/c0112_r0001_schedule.csv",
...
def update_gate_report_for_closeout(closeout_review: dict[str, Any]) -> None:
    ...
    data["gate_report_id"] = "G-C0112-R0001"
    data["status"] = "closed_no_candidate"
...
def update_selected_for_closeout(closeout_review: dict[str, Any]) -> None:
    ...
    data["selected_reason"] = (
        "C0112 R0001 closed as no candidate: observed activity and proxy, MT5 logic, "
...
def update_campaign_for_closeout(closeout_review: dict[str, Any], closed_at: str) -> None:
    ...
    run_index["next_run_candidate"] = {
        ...
        "reason": (
            "C0112 R0001 closed no-candidate after proxy, MT5 logic parity, MT5 tick, execution divergence, "
            "and fold-isolated MT5 evidence. Further work inside C0112 would be adjacent tuning."
        ),
    ...
    data["synthesis_readiness"] = {
        ...
        "usable_ingredient_group_count": 30,
        "readiness_notes": [
            "C0049, C0050, C0051, C0052, C0053, C0054, C0055, C0056, C0057, C0058, C0059, C0060, C0061, C0062, C0063, C0064, C0065, C0066, C0070, C0071, C0072, C0073, C0084, C0085, C0086, C0087, C0088, C0089, C0096, C0097, C0098, C0099, C0100, C0102, C0103, C0104, C0107, C0109, C0110, and C0112 provide post-SC0009 negative-memory ingredients, but default usable ingredient minimums are not met.",
            "Next work can remain a new major C0113 campaign hypothesis, not C0112 threshold or stop/target tuning.",
        ],
    }
    data["closeout"] = {
        "status": "closed_no_candidate",
...
def update_reentry_for_closeout() -> None:
    ...
    for item in (
        ...
        "produce_c0112_r0001_fold_isolated_mt5_tick_kpi",
        "produce_c0112_r0001_fold_isolated_execution_divergence",
        "review_c0112_r0001_tick_execution_kpi_and_closeout",
        "close_c0112_r0001_no_candidate",
        "close_c0112_no_candidate_after_r0001",
    ):
        if item not in completed:
            completed.append(item)
...
def update_claim_state_for_closeout(closeout_review: dict[str, Any]) -> None:
    ...
    data["latest_operation"] = {
        "id": "close_c0112_r0001_no_candidate",
        "status": "completed",
...
def update_decision_cursor_for_closeout(closeout_review: dict[str, Any]) -> None:
    ...
    repo_scope["note"] = (
        ...
        "current_c0112_r0001_closeout_makes_no_selection_promotion_handoff_or_reproducibility_claims"
    )
    for item in repo_scope.get("nonblocking_debts") or []:
        if isinstance(item, dict):
            item["role"] = "repo_state_debt_not_current_c0112_r0001_closeout"
    ...
    current.update(
        {
            ...
            "next_required_action": NEXT_MAJOR_ACTION,
            "note": (
                "C0112 R0001 and C0112 closed no-candidate; micro consolidation breakout continuation surface produced "
    ...
    data["next_decision_basis"] = [
        {
            ...
            "summary": "C0112 R0001 closed no-candidate with negative memory recorded.",
        },
        {
        ...
        },
        {
            ...
            "summary": "C0112 closed no-candidate; next work is a new major C0113 hypothesis.",
        },
    ]
...
def append_decision_registry_for_closeout(closeout_review: dict[str, Any]) -> None:
    ...
    if "dec_20260707_close_c0112_r0001_no_candidate" not in existing:
        decisions.append(
            {
                "decision_id": "dec_20260707_close_c0112_r0001_no_candidate",
                "created_local_date": local_date,
                "status": "active",
                "decision": "close_c0112_r0001_no_candidate",
                "refines": [
                    "dec_20260707_produce_c0112_r0001_proxy_evidence",
                    "dec_20260701_mandatory_mt5_paired_run_validation",
                ],
                "rationale": [
                    "c0112_r0001_completed_proxy_mt5_logic_parity_proxy_vs_mt5_parity_mt5_tick_execution_divergence_and_fold_isolated_tick_and_divergence_evidence",
    ...
    if "dec_20260707_close_c0112_no_candidate_after_r0001" not in existing:
        decisions.append(
            {
                "decision_id": "dec_20260707_close_c0112_no_candidate_after_r0001",
                "created_local_date": local_date,
                "status": "active",
                "decision": "close_c0112_no_candidate_after_r0001",
                "refines": [
                    "dec_20260707_close_c0112_r0001_no_candidate",
                    "dec_20260707_open_c0112_intraday_micro_consolidation_breakout_continuation_discovery",
                ],
                "rationale": [
                    "c0112_boundary_was_answered_by_r0001_fold_isolated_mt5_evidence",
                    "remaining_c0112_work_would_be_adjacent_threshold_window_stop_target_hold_session_activity_spread_capital_or_feature_nudge_tuning",
                    "synthesis_readiness_candidate_later_because_post_sc0009_usable_ingredient_count_is_below_default_minimum",
                    "next_work_is_choose_c0113_new_major_hypothesis_after_c0112_closeout",
                    "no_selected_economics_runtime_materialization_onnx_promotion_or_live_claim_is_created",
                ],
                "claim_boundary": claim_boundary_payload(),
            }
...
def close_c0112_r0001_no_candidate() -> dict[str, object]:
    closed_at = utc_now()
    closeout_review = c0112_closeout_review(closed_at)
    ...
    records.append(
        {
            ...
            "sha256": digest,
            "produced_by": "axiom_rift.mt5.c0112_r0001_probe",
