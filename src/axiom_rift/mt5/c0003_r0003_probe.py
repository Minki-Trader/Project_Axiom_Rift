"""MT5 schedule-replay logic parity helpers for C0003 R0003."""
...
from axiom_rift.proxies.c0003_r0003_fast_favorable_capture import (
    BASE_FRAME,
    ROLLING_WINDOWS,
    TIME_FORMAT,
    Bar,
...
PROXY_TRADE_ARTIFACT = RUN_ARTIFACT_DIR / "c0003_r0003_proxy_trades.csv"
SCHEDULE_ARTIFACT = RUN_ARTIFACT_DIR / "c0003_r0003_schedule.csv"
SCHEDULE_COMMON_REL = "AxiomRift\\C0003\\R0003\\schedule\\c0003_r0003_schedule.csv"
STARTING_BALANCE_USD = 500.0
RESPONSE_MODE = "fast_favorable_capture_schedule_replay"
MAX_HOLD_BARS = 12
MAGIC = 300003
...
def compile_c0003_r0003_ea(metaeditor_exe: Path = DEFAULT_METAEDITOR_EXE) -> CompileResult:
    ...
    log = PROJECT_ROOT / "artifacts" / "reports" / "C0003_R0003_mt5_compile.log"
...
def tester_config_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    return PROJECT_ROOT / "artifacts" / "reports" / "C0003_R0003_mt5_tester" / f"C0003_R0003_{scoped_name(mode, output_scope)}_tester.ini"


def tester_report_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    return PROJECT_ROOT / "artifacts" / "reports" / "C0003_R0003_mt5_tester" / f"C0003_R0003_mt5_{scoped_name(mode, output_scope)}_report.htm"
...
def write_tester_config(
    ...
    config_dir = PROJECT_ROOT / "artifacts" / "reports" / "C0003_R0003_mt5_tester"
...
def parse_c0003_r0003_mt5(
    ...
    if mode == LOGIC_PARITY_MODE:
        ...
        upsert_artifact_lineage(
            ...
            [
                "campaigns/C0003_path_conditioned_exit_risk_shape/runs/R0003/artifacts/c0003_r0003_schedule.csv",
                "configs/market.yaml",
            ],
        )
    else:
    ...
    return {
        ...
        "conditional_profiles": {
            ...
            "risk_shape_profile": {
                "applies": True,
                "fields": {
                    "risk_shape_source": "proxy_fast_favorable_capture_schedule",
        ...
        "deferred_with_reason": [
            {
                "field": "native_mql_fast_favorable_capture_training",
                "requirement_class": "deferred_with_reason",
                "reason": "C0003 R0003 logic parity replays the proxy fast favorable-capture schedule; native model or ONNX materialization is not claimed",
...
def record_c0003_r0003_parity() -> dict[str, object]:
    ...
    next_action = "produce_c0003_r0003_mt5_tick_execution_evidence" if mechanical_ok else "repair_c0003_r0003_schedule_replay_parity"
    ...
    payload = {
        ...
        "mechanical_checks": {
            ...
            "fast_favorable_capture_exit_match": "passed" if exit_compare["reason_match_rate"] == 1.0 else "failed",
        ...
        "conditional_profiles": {
            ...
            "risk_shape_schedule_replay_profile": {
                ...
                "fields": {
                    ...
                    "logic_boundary": "proxy_selected_fast_favorable_capture_schedule_replayed_in_mt5_closed_bar_mode",
        ...
        "deferred_with_reason": [
            {
                ...
                "requirement_class": "deferred_with_reason",
                "reason": "logic parity uses schedule replay only; native fast favorable-capture surface materialization is deferred until candidate quality exists",
    ...
    upsert_artifact_lineage(
        ...
        [
            "campaigns/C0003_path_conditioned_exit_risk_shape/runs/R0003/kpi/proxy.json",
            "campaigns/C0003_path_conditioned_exit_risk_shape/runs/R0003/kpi/mt5_logic_parity.json",
        ],
    )
...
def update_run_after_mt5(mode: str) -> None:
    ...
    evidence_paths["mt5_schedule_artifact"] = "artifacts/c0003_r0003_schedule.csv"
    mt5_plan = data.setdefault("mt5_probe_plan", {})
    mt5_plan["logic_parity_kpi_path"] = "kpi/mt5_logic_parity.json"
    mt5_plan["logic_parity_purpose"] = "proxy_selected_fast_favorable_capture_schedule_vs_ea_closed_bar_lifecycle_parity"
...
def update_gate_after_logic_parity(payload: dict[str, object], next_action: str, mechanical_ok: bool) -> None:
    ...
    for path in ("kpi/mt5_logic_parity.json", "kpi/proxy_vs_mt5_logic_parity.json", "artifacts/c0003_r0003_schedule.csv"):
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "blocking_condition": "R0003 cannot close until mandatory MT5 tick, execution divergence, and fold-isolated evidence are recorded"
            if mechanical_ok
...
def update_reentry_after_logic_parity(next_action: str, mechanical_ok: bool) -> None:
    ...
    for item in (
        "produce_c0003_r0003_mt5_logic_parity_evidence",
        "record_c0003_r0003_proxy_vs_mt5_logic_parity_evidence",
    ):
...
def update_campaign_after_logic_parity(next_action: str) -> None:
    ...
    text = text.replace("remaining_question: produce_c0003_r0003_mt5_logic_parity_evidence", f"remaining_question: {next_action}")
    CAMPAIGN.write_text(text, encoding="ascii")


def record_c0003_r0003_execution_divergence() -> dict[str, object]:
        ...
        "deferred_with_reason": [
            {
                ...
                "revisit_when": "after C0003 R0003 fold-isolated MT5 tick evidence",
    ...
    upsert_artifact_lineage(
        ...
        [
            "campaigns/C0003_path_conditioned_exit_risk_shape/runs/R0003/kpi/mt5_logic_parity.json",
            "campaigns/C0003_path_conditioned_exit_risk_shape/runs/R0003/kpi/mt5_tick.json",
        ],
    )
    ...
    update_campaign_remaining_question("produce_c0003_r0003_fold_isolated_mt5_tick_kpi")
    return payload
...
def update_gate_after_execution_divergence(payload: dict[str, object]) -> None:
    ...
    data["decision"] = "defer_with_reason"
    data["next_action"] = "produce_c0003_r0003_fold_isolated_mt5_tick_kpi"
    data["execution_gate"] = {
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "revisit_when": "produce_c0003_r0003_fold_isolated_mt5_tick_kpi",
        }
    ]
    GATE_REPORT.write_text(json.dumps(data, indent=2, sort_keys=True) + "\n", encoding="ascii")
...
def update_reentry_after_execution_divergence() -> None:
    ...
    completed = list(next_work.get("completed") or [])
    for item in (
        "produce_c0003_r0003_mt5_tick_execution_evidence",
        "record_c0003_r0003_execution_divergence",
    ):
        if item not in completed:
            completed.append(item)
    next_action = "produce_c0003_r0003_fold_isolated_mt5_tick_kpi"
    completed = [item for item in completed if item != next_action]
    next_work["completed"] = completed
    next_work["tasks"] = [next_action]
    path.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def update_campaign_remaining_question(next_action: str) -> None:
    text = CAMPAIGN.read_text(encoding="ascii")
    text = text.replace("remaining_question: produce_c0003_r0003_mt5_tick_execution_evidence", f"remaining_question: {next_action}")
    text = text.replace("remaining_question: produce_c0003_r0003_fold_isolated_mt5_tick_kpi", f"remaining_question: {next_action}")
    CAMPAIGN.write_text(text, encoding="ascii")
...
def build_mt5_tick_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": RUN_ID,
        "mt5_probe_id": "MT5-C0003-R0003-BY-FOLD",
...
def build_execution_divergence_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": RUN_ID,
        "divergence_id": "ED-C0003-R0003-BY-FOLD",
        "split_policy": "rolling_window_test_oos_fold_isolated",
        "split_registry": "registries/rolling_windows.yaml",
...
def update_gate_after_fold_isolated_evidence(
    ...
    for path in (
        ...
        "artifacts/c0003_r0003_schedule.csv",
    ):
        if path not in evidence_paths:
            evidence_paths.append(path)
    gate_data["decision"] = "defer_with_reason"
    gate_data["next_action"] = "review_c0003_r0003_tick_execution_kpi_and_closeout"
    gate_data["deferred_with_reason"] = [
        {
            ...
            "blocking_condition": "review_c0003_r0003_tick_execution_kpi_and_closeout",
            "revisit_when": "during C0003 R0003 closeout review",
        }
    ]
...
def update_reentry_after_fold_isolated() -> None:
    ...
    completed = list(next_work.get("completed") or [])
    for item in (
        "produce_c0003_r0003_fold_isolated_mt5_tick_kpi",
        "produce_c0003_r0003_fold_isolated_execution_divergence",
    ):
        if item not in completed:
            completed.append(item)
    next_action = "review_c0003_r0003_tick_execution_kpi_and_closeout"
    completed = [item for item in completed if item != next_action]
    next_work["completed"] = completed
    next_work["tasks"] = [next_action]
    path.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
    ...
    records.append(
        {
            ...
            "sha256": file_hash,
            "produced_by": "axiom_rift.mt5.c0003_r0003_probe",
    ...
    data["deferred_with_reason"] = [
        {
            ...
            "next_action": "produce_c0003_r0003_mt5_tick_execution_evidence",
        }
    ]
    ARTIFACT_LINEAGE.write_text(json.dumps(data, indent=2, sort_keys=True) + "\n", encoding="ascii")


def run_c0003_r0003_logic_parity_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_c0003_r0003_tester(mode=LOGIC_PARITY_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_c0003_r0003_mt5(result, mode=LOGIC_PARITY_MODE, write_kpi=True)
    parity_payload = record_c0003_r0003_parity()
    return {
...
def run_c0003_r0003_mt5_tick_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_c0003_r0003_tester(mode=TICK_EXECUTION_MODE, timeout_seconds=timeout_seconds)
    tick_payload = parse_c0003_r0003_mt5(result, mode=TICK_EXECUTION_MODE, write_kpi=True)
    divergence_payload = record_c0003_r0003_execution_divergence()
    return {
...
def run_c0003_r0003_mt5_tick_by_fold_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    compile_c0003_r0003_ea()
    write_schedule_files()
    records: list[dict[str, Any]] = []
    for window in load_test_windows():
        from_date, to_date = tester_dates_for_window(window)
        logic_result = run_c0003_r0003_tester(
        ...
        logic_payload = parse_c0003_r0003_mt5(logic_result, write_kpi=False)
        tick_result = run_c0003_r0003_tester(
        ...
        tick_payload = parse_c0003_r0003_mt5(tick_result, write_kpi=False)
        records.append(
            {
                "window": window,
                "from_date": from_date,
    ...
    upsert_artifact_lineage(
        ...
        [
            "campaigns/C0003_path_conditioned_exit_risk_shape/runs/R0003/kpi/mt5_tick_by_fold.json",
            "registries/rolling_windows.yaml",
        ],
    )
    ...
    update_campaign_remaining_question("review_c0003_r0003_tick_execution_kpi_and_closeout")
    return {
...
if __name__ == "__main__":
    result_payload = run_c0003_r0003_logic_parity_workflow()
