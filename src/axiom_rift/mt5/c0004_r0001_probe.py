"""MT5 schedule-replay logic parity helpers for C0004 R0001."""
...
SCHEDULE_ARTIFACT = RUN_ARTIFACT_DIR / "c0004_r0001_schedule.csv"
SCHEDULE_COMMON_REL = "AxiomRift\\C0004\\R0001\\schedule\\c0004_r0001_schedule.csv"
STARTING_BALANCE_USD = 500.0
RESPONSE_MODE = "fold_local_state_archetype_schedule_replay"
MAX_HOLD_BARS = 10
MAGIC = 400001
...
def compile_c0004_r0001_ea(metaeditor_exe: Path = DEFAULT_METAEDITOR_EXE) -> CompileResult:
    ...
    log = PROJECT_ROOT / "artifacts" / "reports" / "C0004_R0001_mt5_compile.log"
...
def tester_config_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    return PROJECT_ROOT / "artifacts" / "reports" / "C0004_R0001_mt5_tester" / f"C0004_R0001_{scoped_name(mode, output_scope)}_tester.ini"


def tester_report_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    return PROJECT_ROOT / "artifacts" / "reports" / "C0004_R0001_mt5_tester" / f"C0004_R0001_mt5_{scoped_name(mode, output_scope)}_report.htm"
...
def write_tester_config(
    ...
    config_dir = PROJECT_ROOT / "artifacts" / "reports" / "C0004_R0001_mt5_tester"
        ...
        "deferred_with_reason": [
            {
                ...
                "reason": "C0004 R0001 logic parity replays the proxy state-archetype schedule; native model or ONNX materialization is not claimed at this stage",
...
def record_c0004_r0001_parity() -> dict[str, object]:
    ...
    next_action = "produce_c0004_r0001_mt5_tick_execution_evidence" if mechanical_ok else "repair_c0004_r0001_schedule_replay_parity"
        ...
        "conditional_profiles": {
            ...
            "state_archetype_schedule_replay_profile": {
                ...
                "fields": {
                    ...
                    "logic_boundary": "proxy_selected_fold_local_state_archetype_schedule_replayed_in_mt5_closed_bar_mode",
    ...
    upsert_artifact_lineage(
        "A-C0004-R0001-PROXY-VS-MT5-LOGIC-KPI",
        ...
        [
            "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0001/kpi/proxy.json",
            "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0001/kpi/mt5_logic_parity.json",
        ],
    )
...
def update_run_after_mt5(mode: str) -> None:
    ...
    evidence_paths["mt5_schedule_artifact"] = "artifacts/c0004_r0001_schedule.csv"
...
def update_gate_after_parity(parity_payload: dict[str, object]) -> None:
    ...
    evidence_paths = data.setdefault("evidence_paths", [])
    for path in ("kpi/mt5_logic_parity.json", "kpi/proxy_vs_mt5_logic_parity.json", "artifacts/c0004_r0001_schedule.csv"):
        ...
        data["deferred_with_reason"] = [
            {
                ...
                "blocking_condition": "produce_c0004_r0001_mt5_tick_execution_evidence",
                "revisit_when": "after C0004 R0001 tick execution KPI and execution divergence are recorded",
            }
        ]
...
def update_reentry_after_logic_parity(next_action: str, mechanical_ok: bool) -> None:
    ...
    for item in (
        "produce_c0004_r0001_mt5_logic_parity_evidence",
        "record_c0004_r0001_proxy_vs_mt5_logic_parity_evidence",
    ):
...
def update_campaign_after_logic_parity(next_action: str) -> None:
    ...
    next_candidate["direction"] = "active_r0001_tick_execution" if "mt5_tick" in next_action else "active_r0001_parity_repair"
...
def update_claim_state_after_logic_parity(parity_payload: dict[str, object], mt5_payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0004_r0001_mt5_logic_parity_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0001/kpi/mt5_logic_parity.json",
        "paired_parity_source": "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0001/kpi/proxy_vs_mt5_logic_parity.json",
...
def record_c0004_r0001_execution_divergence() -> dict[str, object]:
        ...
        "deferred_with_reason": [
            {
                ...
                "revisit_when": "after C0004 R0001 fold-isolated MT5 tick evidence",
    ...
    upsert_artifact_lineage(
        "A-C0004-R0001-EXECUTION-DIVERGENCE-KPI",
        "diagnostic_output",
        ...
        [
            "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0001/kpi/mt5_logic_parity.json",
            "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0001/kpi/mt5_tick.json",
        ],
    )
...
def update_gate_after_execution_divergence(divergence_payload: dict[str, object]) -> None:
    ...
    data["next_action"] = "produce_c0004_r0001_fold_isolated_mt5_tick_kpi"
    data["deferred_with_reason"] = [
        {
            "field": "run_closeout",
            ...
            "blocking_condition": "produce_c0004_r0001_fold_isolated_mt5_tick_kpi",
...
def update_reentry_after_tick_and_divergence() -> None:
    ...
    completed = list(next_work.get("completed") or [])
    for item in (
        "produce_c0004_r0001_mt5_tick_execution_evidence",
        "record_c0004_r0001_execution_divergence",
    ):
        if item not in completed:
            completed.append(item)
    next_action = "produce_c0004_r0001_fold_isolated_mt5_tick_kpi"
    completed = [item for item in completed if item != next_action]
    next_work["completed"] = completed
    next_work["tasks"] = [next_action]
    path.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=False), encoding="ascii")
...
def update_campaign_after_tick_and_divergence() -> None:
    ...
    run_index = data.setdefault("run_index", {})
    next_candidate = run_index.setdefault("next_run_candidate", {})
    next_candidate["direction"] = "active_r0001_fold_isolated_tick"
...
def update_claim_state_after_tick_and_divergence(divergence_payload: dict[str, object], tick_payload: dict[str, object]) -> None:
    ...
    data["active_run"] = "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0001"
    data["latest_operation"] = {
        "id": "produce_c0004_r0001_mt5_tick_execution_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0001/kpi/mt5_tick.json",
        "paired_divergence_source": "campaigns/C0004_fold_local_state_archetype_discovery/runs/R0001/kpi/execution_divergence.json",
        ...
        "economics_shift_status": required.get("economics_shift_status"),
        "next_required_action": "produce_c0004_r0001_fold_isolated_mt5_tick_kpi",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def upsert_artifact_lineage(
    ...
    next_record = {
        ...
        "mutable": False,
        "produced_by": "axiom_rift.mt5.c0004_r0001_probe",
        ...
        data["deferred_with_reason"] = [
            {
                ...
                "next_action": "produce_c0004_r0001_mt5_tick_execution_evidence",
            }
        ]
    ARTIFACT_LINEAGE.write_text(json.dumps(data, indent=2, sort_keys=True) + "\n", encoding="ascii")


def run_c0004_r0001_mt5_logic_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_c0004_r0001_tester(mode=LOGIC_PARITY_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_c0004_r0001_mt5(result)
    parity_payload = record_c0004_r0001_parity()
    return {
...
def run_c0004_r0001_mt5_tick_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_c0004_r0001_tester(mode=TICK_EXECUTION_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_c0004_r0001_mt5(result)
    divergence_payload = record_c0004_r0001_execution_divergence()
    return {
