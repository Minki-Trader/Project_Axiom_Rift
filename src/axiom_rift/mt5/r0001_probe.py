"""MT5 compile, tester, and KPI parsing helpers for R0001."""
...
from axiom_rift.proxies.r0001_volatility_expansion import (
    BASE_FRAME,
    load_bars,
    load_test_windows,
    run_r0001_proxy,
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0001_regime_response_discovery" / "runs" / "R0001"
KPI_DIR = RUN_DIR / "kpi"
...
def compile_r0001_ea(metaeditor_exe: Path | None = None) -> CompileResult:
    ...
    log = PROJECT_ROOT / "artifacts" / "reports" / "r0001_mt5_compile.log"
...
def tester_config_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    mode = normalize_mt5_mode(mode)
    return PROJECT_ROOT / "artifacts" / "reports" / "r0001_mt5_tester" / f"r0001_{scoped_name(mode, output_scope)}_tester.ini"


def tester_report_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    mode = normalize_mt5_mode(mode)
    return PROJECT_ROOT / "artifacts" / "reports" / "r0001_mt5_tester" / f"r0001_mt5_{scoped_name(mode, output_scope)}_report.htm"
...
def write_tester_config(
    ...
    config_dir = PROJECT_ROOT / "artifacts" / "reports" / "r0001_mt5_tester"
...
def record_r0001_parity() -> dict[str, object]:
    mt5 = json.loads(MT5_LOGIC_KPI.read_text(encoding="ascii"))
    proxy = run_r0001_proxy(write=False)
    ...
    upsert_artifact_lineage(
        ...
        [
            "campaigns/C0001_regime_response_discovery/runs/R0001/kpi/proxy.json",
            "campaigns/C0001_regime_response_discovery/runs/R0001/kpi/mt5_logic_parity.json",
        ],
    )
...
def record_r0001_execution_divergence() -> dict[str, object]:
    ...
    payload = {
        ...
        "conditional_profiles": {
            ...
            "spread_slippage_stress_profile": {
                "applies": False,
                "fields": {},
                "deferred_with_reason": "R0001 has baseline MT5 tick execution evidence only; spread and slippage stress sweeps are separate robustness work.",
            },
            "score_model_profile": {
                "applies": False,
                "fields": {},
                "deferred_with_reason": "R0001 is rule/proxy based and has no model score surface.",
            },
        ...
        "deferred_with_reason": [
            {
                ...
                "requirement_class": "deferred_with_reason",
                "reason": "baseline tick KPI is recorded; stress sweeps are not part of the current R0001 parity-to-tick split.",
                "blocking_condition": "requires a separate controlled spread/slippage tester matrix",
                "revisit_when": "after R0001 closeout review if this hypothesis remains worth robustness testing",
                "claim_boundary": {"claim_authority": False},
            }
        ],
        "claim_boundary": {
    ...
    upsert_artifact_lineage(
        ...
        [
            "campaigns/C0001_regime_response_discovery/runs/R0001/kpi/mt5_logic_parity.json",
            "campaigns/C0001_regime_response_discovery/runs/R0001/kpi/mt5_tick.json",
        ],
    )
...
def run_r0001_mt5_tick_by_fold_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    compile_r0001_ea()
    records: list[dict[str, Any]] = []
    for window in load_test_windows(ROLLING_WINDOWS):
        from_date, to_date = tester_dates_for_window(window)
        logic_result = run_r0001_tester(
        ...
        logic_payload = parse_r0001_mt5(logic_result, write_kpi=False)
        tick_result = run_r0001_tester(
        ...
        tick_payload = parse_r0001_mt5(tick_result, write_kpi=False)
        records.append(
            {
                "window": window,
                "from_date": from_date,
    ...
    upsert_artifact_lineage(
        ...
        [
            "campaigns/C0001_regime_response_discovery/runs/R0001/kpi/mt5_tick_by_fold.json",
...
def upsert_artifact_lineage(
    ...
    next_record = {
        ...
        "mutable": False,
        "produced_by": "axiom_rift.mt5.r0001_probe",
        ...
        data["deferred_with_reason"] = [
            {
                ...
                "revisit_when": "during R0001 closeout review",
            }
        ]
...
def update_gate_after_execution_divergence(divergence_payload: dict[str, object]) -> None:
    ...
    data["next_action"] = "review_r0001_tick_execution_kpi_and_closeout"
    data["deferred_with_reason"] = [
        {
            "field": "run_closeout",
            "reason": "logic parity, tick execution KPI, and execution divergence are recorded; closeout judgment still requires review",
            "blocking_condition": "review tick economics and divergence before deciding whether R0001 remains useful",
            "revisit_when": "during R0001 closeout review",
        }
    ]
    GATE_REPORT.write_text(json.dumps(data, indent=2, sort_keys=True) + "\n", encoding="ascii")
...
def update_gate_after_fold_isolated_evidence(
    ...
    gate_data["next_action"] = "review_r0001_tick_execution_kpi_and_closeout"
    gate_data["deferred_with_reason"] = [
        {
            ...
            "blocking_condition": "review fold-isolated tick economics and divergence before closing R0001",
            "revisit_when": "during R0001 closeout review",
        }
    ]
...
def run_r0001_mt5_logic_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_r0001_tester(mode=LOGIC_PARITY_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_r0001_mt5(result)
    parity_payload = record_r0001_parity()
    return {
...
def run_r0001_mt5_tick_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_r0001_tester(mode=TICK_EXECUTION_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_r0001_mt5(result)
    divergence_payload = record_r0001_execution_divergence()
    return {
