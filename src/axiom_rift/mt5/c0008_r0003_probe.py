"""MT5 schedule-replay logic parity helpers for C0008 R0003."""
...
from axiom_rift.proxies.c0008_r0003_structural_trap_robustness import (
    BASE_FRAME,
    ROLLING_WINDOWS,
    SplitWindow,
    Trade,
...
def compile_c0008_r0003_ea(metaeditor_exe: Path | None = None) -> CompileResult:
    ...
    log = PROJECT_ROOT / "artifacts" / "reports" / "c0008_r0003_mt5_compile.log"
...
def tester_config_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    return PROJECT_ROOT / "artifacts" / "reports" / "c0008_r0003_mt5_tester" / f"c0008_r0003_{scoped_name(mode, output_scope)}_tester.ini"


def tester_report_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    return PROJECT_ROOT / "artifacts" / "reports" / "c0008_r0003_mt5_tester" / f"c0008_r0003_mt5_{scoped_name(mode, output_scope)}_report.htm"
    ...
    return {
        ...
        **runtime_payload_fields(),
        ...
        "conditional_profiles": {
            ...
            "structural_trap_robustness_schedule_profile": {
                "applies": True,
                "fields": {
                    "structural_trap_robustness_source": "rolling_train_is_proxy_schedule",
        ...
        "deferred_with_reason": [
            {
                ...
                "reason": "C0008 R0003 logic parity replays the proxy structural trap robustness schedule; native model or ONNX materialization is not claimed at this stage",
...
def record_c0008_r0003_parity() -> dict[str, object]:
    ...
    next_action = "produce_c0008_r0003_mt5_tick_execution_evidence" if mechanical_ok else "repair_c0008_r0003_schedule_replay_parity"
    ...
    payload = {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": RUN_ID,
        "parity_id": "P-C0008-R0003",
        "proxy_id": "PX-C0008-R0003",
        "mt5_probe_id": "MT5-C0008-R0003",
        ...
        "conditional_profiles": {
            ...
            "structural_trap_reversal_schedule_replay_profile": {
                ...
                "fields": {
                    ...
                    "logic_boundary": "proxy_selected_fold_local_structural_trap_robustness_schedule_replayed_in_mt5_closed_bar_mode",
        ...
        "deferred_with_reason": [
            {
                ...
                "requirement_class": "deferred_with_reason",
                "reason": "logic parity uses schedule replay only; native structural trap robustness surface materialization is deferred until candidate quality exists",
...
def update_gate_after_parity(parity_payload: dict[str, object]) -> None:
    ...
    evidence_paths = data.setdefault("evidence_paths", [])
    for path in ("kpi/mt5_logic_parity.json", "kpi/proxy_vs_mt5_logic_parity.json", "artifacts/c0008_r0003_schedule.csv"):
        ...
        data["deferred_with_reason"] = [
            {
                ...
                "blocking_condition": "produce_c0008_r0003_mt5_tick_execution_evidence",
...
def update_reentry_after_logic_parity(next_action: str, mechanical_ok: bool) -> None:
    ...
    for item in (
        "produce_c0008_r0003_mt5_logic_parity_evidence",
        "record_c0008_r0003_proxy_vs_mt5_logic_parity_evidence",
    ):
...
def update_campaign_after_logic_parity(next_action: str) -> None:
    ...
    next_candidate["direction"] = "active_c0008_r0003_tick_execution" if "mt5_tick" in next_action else "active_c0008_r0003_parity_repair"
...
def update_claim_state_after_logic_parity(parity_payload: dict[str, object], mt5_payload: dict[str, object]) -> None:
    ...
    data["latest_operation"] = {
        "id": "produce_c0008_r0003_mt5_logic_parity_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0008_multi_timeframe_structural_context_discovery/runs/R0003/kpi/mt5_logic_parity.json",
        "paired_parity_source": "campaigns/C0008_multi_timeframe_structural_context_discovery/runs/R0003/kpi/proxy_vs_mt5_logic_parity.json",
        ...
        "deferred_with_reason": [
            {
                ...
                "revisit_when": "after C0008 R0003 fold-isolated MT5 tick evidence",
...
def update_gate_after_execution_divergence(divergence_payload: dict[str, object]) -> None:
    ...
    data["next_action"] = "produce_c0008_r0003_fold_isolated_mt5_tick_kpi"
    data["deferred_with_reason"] = [
        {
            "field": "run_closeout",
            ...
            "blocking_condition": "produce_c0008_r0003_fold_isolated_mt5_tick_kpi",
...
def update_reentry_after_tick_and_divergence() -> None:
    ...
    completed = list(next_work.get("completed") or [])
    for item in (
        "produce_c0008_r0003_mt5_tick_execution_evidence",
...
def update_claim_state_after_tick_and_divergence(divergence_payload: dict[str, object], tick_payload: dict[str, object]) -> None:
    ...
    data["active_run"] = "campaigns/C0008_multi_timeframe_structural_context_discovery/runs/R0003"
    data["latest_operation"] = {
        "id": "produce_c0008_r0003_mt5_tick_execution_evidence",
        "status": "completed",
        "recorded_at_source": "campaigns/C0008_multi_timeframe_structural_context_discovery/runs/R0003/kpi/mt5_tick.json",
        "paired_divergence_source": "campaigns/C0008_multi_timeframe_structural_context_discovery/runs/R0003/kpi/execution_divergence.json",
        ...
        "economics_shift_status": required.get("economics_shift_status"),
        "next_required_action": "produce_c0008_r0003_fold_isolated_mt5_tick_kpi",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def update_gate_after_fold_isolated_evidence(
    ...
    gate_data["next_action"] = "review_c0008_r0003_tick_execution_kpi_and_closeout"
    gate_data["deferred_with_reason"] = [
        {
            ...
            "blocking_condition": "review_c0008_r0003_tick_execution_kpi_and_closeout",
...
def update_reentry_after_fold_isolated() -> None:
    ...
    completed = list(next_work.get("completed") or [])
    for item in (
        "produce_c0008_r0003_fold_isolated_mt5_tick_kpi",
        "produce_c0008_r0003_fold_isolated_execution_divergence",
    ):
        if item not in completed:
            completed.append(item)
...
def update_claim_state_after_fold_isolated(
    ...
    data["active_run"] = "campaigns/C0008_multi_timeframe_structural_context_discovery/runs/R0003"
    data["latest_operation"] = {
        "id": "produce_c0008_r0003_fold_isolated_mt5_tick_kpi",
        "status": "completed",
        "recorded_at_source": "campaigns/C0008_multi_timeframe_structural_context_discovery/runs/R0003/kpi/mt5_tick_by_fold.json",
        "paired_divergence_source": "campaigns/C0008_multi_timeframe_structural_context_discovery/runs/R0003/kpi/execution_divergence_by_fold.json",
        ...
        "next_required_action": "review_c0008_r0003_tick_execution_kpi_and_closeout",
        "claim_boundary": {
            "claim_authority": False,
            "selected": False,
            "label_selected": False,
...
def upsert_artifact_lineage(
    ...
    next_record = {
        ...
        "mutable": False,
        "produced_by": "axiom_rift.mt5.c0008_r0003_probe",
        ...
        data["deferred_with_reason"] = [
            {
                ...
                "next_action": "produce_c0008_r0003_fold_isolated_execution_divergence",
            }
        ]
...
def run_c0008_r0003_mt5_logic_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_c0008_r0003_tester(mode=LOGIC_PARITY_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_c0008_r0003_mt5(result)
    parity_payload = record_c0008_r0003_parity()
    return {
...
def run_c0008_r0003_mt5_tick_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_c0008_r0003_tester(mode=TICK_EXECUTION_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_c0008_r0003_mt5(result)
    divergence_payload = record_c0008_r0003_execution_divergence()
    return {
...
def run_c0008_r0003_mt5_tick_by_fold_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    compile_c0008_r0003_ea()
    write_schedule_files()
    records: list[dict[str, Any]] = []
    for window in load_test_windows():
        from_date, to_date = tester_dates_for_window(window)
        logic_result = run_c0008_r0003_tester(
...
def build_mt5_tick_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": RUN_ID,
        "mt5_probe_id": "MT5-C0008-R0003-BY-FOLD",
...
def build_execution_divergence_by_fold_payload(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    return {
        ...
        "synthesis_id_when_applicable": None,
        "run_id": RUN_ID,
        "divergence_id": "ED-C0008-R0003-BY-FOLD",
        "split_policy": "rolling_window_test_oos_fold_isolated",
        "split_registry": "registries/rolling_windows.yaml",
