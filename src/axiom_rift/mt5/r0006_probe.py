"""MT5 compile, tester, and KPI parsing helpers for R0006."""
...
from axiom_rift.proxies.r0006_compression_breakout_reversal import (
    BASE_FRAME,
    load_bars,
    load_test_windows,
    run_r0006_proxy,
...
RUN_DIR = PROJECT_ROOT / "campaigns" / "C0001_regime_response_discovery" / "runs" / "R0006"
KPI_DIR = RUN_DIR / "kpi"
...
def compile_r0006_ea(metaeditor_exe: Path = DEFAULT_METAEDITOR_EXE) -> CompileResult:
    ...
    log = PROJECT_ROOT / "artifacts" / "reports" / "R0006_mt5_compile.log"
...
def tester_config_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    mode = normalize_mt5_mode(mode)
    return PROJECT_ROOT / "artifacts" / "reports" / "R0006_mt5_tester" / f"R0006_{scoped_name(mode, output_scope)}_tester.ini"


def tester_report_path_for_mode(mode: str, output_scope: str | None = None) -> Path:
    mode = normalize_mt5_mode(mode)
    return PROJECT_ROOT / "artifacts" / "reports" / "R0006_mt5_tester" / f"R0006_mt5_{scoped_name(mode, output_scope)}_report.htm"
...
def write_tester_config(
    ...
    config_dir = PROJECT_ROOT / "artifacts" / "reports" / "R0006_mt5_tester"
    ...
    lines = [
        ...
        "InpResponseMode=compression_breakout_reversal",
...
def assert_mt5_mode_matches_status(mode: str, status: dict[str, str], output_scope: str | None = None) -> None:
    ...
    if status.get("response_mode") != "compression_breakout_reversal":
        raise RuntimeError(f"MT5 response_mode mismatch: expected=compression_breakout_reversal actual={status.get('response_mode')}")
...
def record_r0006_parity() -> dict[str, object]:
    mt5 = json.loads(MT5_LOGIC_KPI.read_text(encoding="ascii"))
    proxy = run_r0006_proxy(write=False)
    ...
    upsert_artifact_lineage(
        ...
        [
            "campaigns/C0001_regime_response_discovery/runs/R0006/kpi/proxy.json",
            "campaigns/C0001_regime_response_discovery/runs/R0006/kpi/mt5_logic_parity.json",
        ],
    )
...
def record_r0006_execution_divergence() -> dict[str, object]:
    ...
    payload = {
        ...
        "conditional_profiles": {
            ...
            "spread_slippage_stress_profile": {
                "applies": False,
                "fields": {},
                "deferred_with_reason": "R0006 has baseline MT5 tick execution evidence only; spread and slippage stress sweeps are separate robustness work.",
            },
            "score_model_profile": {
                "applies": False,
                "fields": {},
                "deferred_with_reason": "R0006 is rule/proxy based and has no model score surface.",
            },
        ...
        "deferred_with_reason": [
            {
                ...
                "requirement_class": "deferred_with_reason",
                "reason": "baseline tick KPI is recorded; stress sweeps are not part of the current R0006 parity-to-tick split.",
                "blocking_condition": "requires a separate controlled spread/slippage tester matrix",
                "revisit_when": "after R0006 closeout review if this hypothesis remains worth robustness testing",
                "claim_boundary": {"claim_authority": False},
            }
        ],
        "claim_boundary": {
    ...
    upsert_artifact_lineage(
        ...
        [
            "campaigns/C0001_regime_response_discovery/runs/R0006/kpi/mt5_logic_parity.json",
            "campaigns/C0001_regime_response_discovery/runs/R0006/kpi/mt5_tick.json",
        ],
    )
...
def run_r0006_mt5_tick_by_fold_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    compile_r0006_ea()
    records: list[dict[str, Any]] = []
    for window in load_test_windows(ROLLING_WINDOWS):
        from_date, to_date = tester_dates_for_window(window)
        logic_result = run_r0006_tester(
        ...
        logic_payload = parse_r0006_mt5(logic_result, write_kpi=False)
        tick_result = run_r0006_tester(
        ...
        tick_payload = parse_r0006_mt5(tick_result, write_kpi=False)
        records.append(
            {
                "window": window,
                "from_date": from_date,
    ...
    upsert_artifact_lineage(
        ...
        [
            "campaigns/C0001_regime_response_discovery/runs/R0006/kpi/mt5_tick_by_fold.json",
...
def classify_session_gap_exception(
    ...
    return {
        ...
        "revisit_when": "if R0006 remains useful after tick execution evidence" if applies else "",
        "mismatch_samples": {
...
def upsert_artifact_lineage(
    ...
    next_record = {
        ...
        "mutable": False,
        "produced_by": "axiom_rift.mt5.r0006_probe",
        ...
        data["deferred_with_reason"] = [
            {
                ...
                "revisit_when": "during R0006 closeout review",
            }
        ]
...
def update_gate_after_execution_divergence(divergence_payload: dict[str, object]) -> None:
    ...
    data["next_action"] = "review_R0006_tick_execution_kpi_and_closeout"
    data["deferred_with_reason"] = [
        {
            "field": "run_closeout",
            "reason": "logic parity, tick execution KPI, and execution divergence are recorded; closeout judgment still requires review",
            "blocking_condition": "review tick economics and divergence before deciding whether R0006 remains useful",
            "revisit_when": "during R0006 closeout review",
        }
    ]
    GATE_REPORT.write_text(json.dumps(data, indent=2, sort_keys=True) + "\n", encoding="ascii")
...
def update_gate_after_fold_isolated_evidence(
    ...
    gate_data["next_action"] = "review_R0006_tick_execution_kpi_and_closeout"
    gate_data["deferred_with_reason"] = [
        {
            ...
            "blocking_condition": "review fold-isolated tick economics and divergence before closing R0006",
            "revisit_when": "during R0006 closeout review",
        }
    ]
...
def run_r0006_mt5_logic_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_r0006_tester(mode=LOGIC_PARITY_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_r0006_mt5(result)
    parity_payload = record_r0006_parity()
    return {
...
def run_r0006_mt5_tick_workflow(timeout_seconds: int = 1800) -> dict[str, object]:
    result = run_r0006_tester(mode=TICK_EXECUTION_MODE, timeout_seconds=timeout_seconds)
    mt5_payload = parse_r0006_mt5(result)
    divergence_payload = record_r0006_execution_divergence()
    return {
