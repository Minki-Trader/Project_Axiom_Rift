"""Writer-gated evidence Job for the fixed USDJPY carry-unwind exit."""
...
CALLABLE_IDENTITY = (
    "axiom_rift.research.usdjpy_carry_exit_study."
    "execute_usdjpy_carry_exit_job.v1"
)
...
def build_usdjpy_carry_exit_validation_plan(
    executable_id: str,
    *,
    mission_id: str,
) -> dict[str, object]:
...
def surface_output_name(*, study_id: str) -> str:
    return f"scientific/{study_id}/usdjpy-carry-exit-surface.json"


def surface_manifest_output_name(*, study_id: str) -> str:
    return f"scientific/{study_id}/usdjpy-carry-exit-surface-manifest.json"
...
def build_environment_manifest() -> dict[str, object]:
    ...
    value = {
        ...
        "source_raw_sha256": USDJPY_RAW_SHA256,
...
class USDJPYCarryExitJobPacket:
...
def execute_usdjpy_carry_exit_job(
    *,
    repository_root: str | Path,
    execution: RunningJobExecution,
) -> USDJPYCarryExitJobPacket:
    ...
    if (
        ...
        raise ValueError("USDJPY carry-exit binding is invalid")
    ...
    if not required.issubset(spec["input_hashes"]):
        raise ValueError("USDJPY carry-exit Job inputs are incomplete")
    surface = compute_registered_usdjpy_carry_exit_surface(root)
    surface_hash = writer.evidence.finalize(canonical_bytes(surface)).sha256
    surface_manifest = {
        "schema": "usdjpy_carry_exit_surface_manifest.v1",
    ...
    return USDJPYCarryExitJobPacket(
