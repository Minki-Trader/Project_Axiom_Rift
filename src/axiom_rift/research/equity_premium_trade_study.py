"""Writer-gated evidence Job for the equity-premium trade contrast."""
...
from axiom_rift.research.equity_premium_trade_discovery import (
    compute_registered_equity_premium_trade_surface,
    equity_premium_trade_discovery_implementation_sha256,
    project_equity_premium_trade_evaluation,
)
...
from axiom_rift.research.scientific_study import (
    EVIDENCE_MODES, PLANNED_CLAIMS, claim_metrics, discovery_criteria,
    planned_verdict,
)
from axiom_rift.research.validation import (
    SCIENTIFIC_DISCOVERY_VALIDATOR_ID, SCIENTIFIC_MEASUREMENT_SCHEMA,
    SCIENTIFIC_RESULT_SCHEMA, build_validation_plan,
)
...
CALLABLE_IDENTITY = (
    "axiom_rift.research.equity_premium_trade_study."
    "execute_equity_premium_trade_job.v1"
)
EVIDENCE_DEPTH = "discovery"
_CONTROL_DELTA_METRIC = "trade_control_delta_net_profit_micropoints"
_CONTROL_PVALUE_METRIC = "trade_control_pvalue_upper_ppm"
CRITERIA = discovery_criteria(
    control_delta_metric=_CONTROL_DELTA_METRIC,
    control_pvalue_metric=_CONTROL_PVALUE_METRIC,
    include_opposite_sign=False,
)


def build_equity_premium_trade_validation_plan(
    executable_id: str, *, mission_id: str,
) -> dict[str, object]:
    return build_validation_plan(
        mission_id=mission_id, executable_id=executable_id,
        evidence_depth=EVIDENCE_DEPTH, planned_claims=PLANNED_CLAIMS,
        evidence_modes=EVIDENCE_MODES, criteria=CRITERIA,
...
def surface_output_name(*, study_id: str) -> str:
    return f"scientific/{study_id}/equity-premium-trade-surface.json"


def surface_manifest_output_name(*, study_id: str) -> str:
    return f"scientific/{study_id}/equity-premium-trade-surface-manifest.json"
...
def _claim_metrics(evaluation: Mapping[str, Any]) -> dict[str, dict[str, int | None]]:
    return claim_metrics(
        evaluation, control_delta_metric=_CONTROL_DELTA_METRIC,
        control_pvalue_metric=_CONTROL_PVALUE_METRIC,
        include_opposite_sign=False,
    )
...
def build_measurement(
    *, executable_id: str, job_id: str, job_hash: str,
    evaluation_artifact_hash: str, evaluation: Mapping[str, Any], mission_id: str,
) -> dict[str, object]:
    value = {
        "claims": list(PLANNED_CLAIMS), "evidence_depth": EVIDENCE_DEPTH,
        "evidence_modes": list(EVIDENCE_MODES),
        "evaluation_artifact_hash": evaluation_artifact_hash,
        "executable_id": executable_id, "job_hash": job_hash, "job_id": job_id,
        "metrics": _claim_metrics(evaluation), "mission_id": mission_id,
...
def build_result_manifest(
    *, executable_id: str, job_id: str, job_hash: str,
    measurement_artifact_hash: str, mission_id: str,
) -> dict[str, object]:
    value = {
        "evidence_depth": EVIDENCE_DEPTH, "executable_id": executable_id,
        "job_hash": job_hash, "job_id": job_id, "mission_id": mission_id,
        "observations": [
...
class EquityPremiumTradeJobPacket:
...
def _load_surface(
    writer: StateWriter, input_hashes: tuple[str, ...],
    ...
    for artifact_hash in input_hashes:
        try:
            ...
            continue
        if isinstance(value, dict) and value.get("schema") == "equity_premium_trade_surface.v1":
            surface = (value, artifact_hash)
        if isinstance(value, dict) and value.get("schema") == "equity_premium_trade_surface_manifest.v1":
            manifest = (value, artifact_hash)
    if (
        surface is None or manifest is None
        or manifest[0].get("surface_artifact_hash") != surface[1]
    ):
        raise ValueError("equity-premium trade surface is missing")
    return surface[0], surface[1], manifest[1]


def execute_equity_premium_trade_job(
    *, repository_root: str | Path, execution: RunningJobExecution,
) -> EquityPremiumTradeJobPacket:
    ...
    spec = binding["spec"]
    subject, mission_id, study_id = (
        spec.get("evidence_subject"), binding.get("mission_id"), binding.get("study_id")
    )
    if (
        not isinstance(mission_id, str) or not isinstance(study_id, str)
        or not isinstance(subject, dict)
        or subject.get("id") not in executable_configuration_map()
    ):
        raise ValueError("equity-premium trade binding is invalid")
    executable_id = subject["id"]
    plan = build_equity_premium_trade_validation_plan(
        executable_id, mission_id=mission_id
    )
    ...
    required = {
        DATASET_SHA256, OBSERVED_MATERIAL_ID, ROLLING_SPLIT_SHA256, plan_hash,
        equity_premium_trade_chassis_implementation_sha256(),
        equity_premium_trade_discovery_implementation_sha256(),
        volatility_clock_label_chassis_implementation_sha256(),
        event_label_implementation_sha256(), loader_implementation_sha256(),
        discovery_implementation_sha256(),
    }
    if not required.issubset(inputs):
        raise ValueError("equity-premium trade inputs are missing")
    expected = set(spec["expected_outputs"])
    produces_surface = surface_output_name(study_id=study_id) in expected
    if produces_surface:
        surface = compute_registered_equity_premium_trade_surface(root)
        surface_hash = writer.evidence.finalize(canonical_bytes(surface)).sha256
        manifest_value = {
            "schema": "equity_premium_trade_surface_manifest.v1",
    ...
    evaluation = project_equity_premium_trade_evaluation(
        surface, job_execution={**execution.payload(), "identity": execution.identity},
        subject_executable_id=executable_id, surface_artifact_hash=surface_hash,
        surface_manifest_hash=manifest_hash,
    )
    evaluation_hash = writer.evidence.finalize(canonical_bytes(evaluation)).sha256
    measurement = build_measurement(
        executable_id=executable_id, job_id=execution.job_id,
        job_hash=execution.job_hash, evaluation_artifact_hash=evaluation_hash,
        evaluation=evaluation, mission_id=mission_id,
    )
    measurement_hash = writer.evidence.finalize(canonical_bytes(measurement)).sha256
    result = build_result_manifest(
        executable_id=executable_id, job_id=execution.job_id,
        job_hash=execution.job_hash, measurement_artifact_hash=measurement_hash,
    ...
    return EquityPremiumTradeJobPacket(
...
__all__ = [
    "CALLABLE_IDENTITY", "CRITERIA", "EVIDENCE_DEPTH", "EVIDENCE_MODES",
    "PLANNED_CLAIMS", "build_environment_manifest",
    "build_equity_premium_trade_validation_plan",
    "execute_equity_premium_trade_job", "output_names",
    "surface_manifest_output_name", "surface_output_name",
]
