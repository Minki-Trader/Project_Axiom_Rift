"""FPMarkets US30 M5 source contract and eligibility measurements."""
...
US30_SYMBOL = "US30"
US30_SERVER = "FPMarketsSC-Live"
US30_RAW_RELATIVE_PATH = "data/raw/mt5_bars/m5/US30_M5_max.csv"
US30_HISTORICAL_SNAPSHOT_SHA256 = (
    "6d638467069a756a7a3897b587ec16a4b9ff76df8718186c2a81905d6d0488d4"
)
US30_START_UTC = datetime(2018, 5, 7, 1, 0, tzinfo=timezone.utc)
US30_END_UTC = datetime(2026, 6, 26, 23, 50, tzinfo=timezone.utc)
US30_COLUMNS = (
    "time",
    "open",
    "high",
...
class US30SourceError(RuntimeError):
    """The US30 source could not satisfy an eligibility boundary."""


def us30_source_contract() -> SourceContract:
    return SourceContract(
        display_name="FPMarkets US30 cash index CFD M5 bid bars",
        canonical_instrument="FPMarkets_US30_cash_index_CFD_M5",
        runtime_identifier=US30_SYMBOL,
        ...
        availability_semantics={
            ...
            "content_hash": f"sha256:{US30_HISTORICAL_SNAPSHOT_SHA256}",
            "coverage": (
...
def _environment(root: Path) -> Mapping[str, Any]:
    ...
    if not isinstance(value, dict):
        raise US30SourceError("environment foundation is invalid")
    return value
...
def _terminal_path(root: Path) -> str:
    ...
    if not isinstance(value, dict) or not isinstance(value.get("terminal"), str):
        raise US30SourceError("MT5 terminal path is absent")
    return value["terminal"]


def _render_rates_csv(rates: np.ndarray) -> bytes:
    lines = [",".join(US30_COLUMNS)]
    for row in rates:
...
def acquire_us30_historical_snapshot(repository_root: str | Path) -> bytes:
    root = Path(repository_root).resolve()
    try:
        import MetaTrader5 as mt5
    except ImportError as exc:  # pragma: no cover - production dependency
        raise US30SourceError("MetaTrader5 Python package is unavailable") from exc
    if not mt5.initialize(path=_terminal_path(root)):
        raise US30SourceError(f"MT5 initialization failed: {mt5.last_error()!r}")
    try:
        if not mt5.symbol_select(US30_SYMBOL, True):
            raise US30SourceError("US30 symbol selection failed")
        ...
        if rates is None or len(rates) == 0:
            raise US30SourceError(f"US30 historical retrieval failed: {mt5.last_error()!r}")
        content = _render_rates_csv(rates)
    finally:
        mt5.shutdown()
    try:
        publish_immutable_raw_snapshot(root, "US30", content)
    except ExternalObservedDevelopmentError as exc:
        raise US30SourceError("US30 immutable raw publication failed") from exc
    return content
...
def _parse_historical(content: bytes) -> pd.DataFrame:
    try:
        ...
        raise US30SourceError("US30 historical CSV cannot be parsed") from exc
    if tuple(frame.columns) != US30_COLUMNS or frame.empty:
        raise US30SourceError("US30 historical CSV schema is invalid")
    return frame


def audit_us30_historical_bytes(
    content: bytes,
    *,
    observed_at_utc: str,
) -> dict[str, Any]:
    ...
    expected_first = US30_START_UTC.replace(tzinfo=None).isoformat()
    expected_last = US30_END_UTC.replace(tzinfo=None).isoformat()
    ...
    facts = {
        ...
        "content_hash_verified": raw_sha256 == US30_HISTORICAL_SNAPSHOT_SHA256,
    ...
    return {
        "schema": "us30_historical_audit_measurement.v3",
...
def build_us30_historical_audit(repository_root: str | Path) -> tuple[bytes, dict[str, Any]]:
    content = acquire_us30_historical_snapshot(repository_root)
    observed = datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z")
    measurement = audit_us30_historical_bytes(content, observed_at_utc=observed)
...
def derive_runtime_facts(probe: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not required.issubset(probe):
        raise US30SourceError("US30 runtime probe schema is incomplete")
    integer_fields = (
        "digits",
        "latest_rate_mt5_epoch_seconds",
        "mt5_epoch_minus_observed_utc_seconds",
    ...
    if any(type(probe[name]) is not int for name in integer_fields):
        raise US30SourceError("US30 runtime probe integer fields are invalid")
    exact_spec = (
        probe["server"] == US30_SERVER
        and probe["symbol"] == US30_SYMBOL
        and probe["digits"] == 2
...
def probe_us30_runtime(repository_root: str | Path) -> dict[str, Any]:
    root = Path(repository_root).resolve()
    try:
        import MetaTrader5 as mt5
    except ImportError as exc:  # pragma: no cover - production dependency
    ...
    try:
        ...
        mt5.symbol_select(US30_SYMBOL, True)
        info = mt5.symbol_info(US30_SYMBOL)
        rates = mt5.copy_rates_from_pos(US30_SYMBOL, mt5.TIMEFRAME_M5, 0, 8)
    ...
    if account is None or terminal is None or info is None or rates is None or tick is None:
        raise US30SourceError("US30 runtime probe returned an incomplete surface")
    ...
    if any(
        ...
        raise US30SourceError("US30 runtime availability proof did not pass")
    return probe
...
def source_validation_plan(transition_evidence: str) -> dict[str, Any]:
    ...
    return {
        "schema": "us30_source_validation_plan.v3",
        "source_contract_id": us30_source_contract().source_contract_id,
...
__all__ = [
    ...
    "US30_SYMBOL",
    "US30SourceError",
    "acquire_us30_historical_snapshot",
