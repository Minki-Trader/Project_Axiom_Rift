...
def audit_us30_historical_bytes(
    ...
    *,
    ...
    expected_first = US30_START_UTC.isoformat().replace("+00:00", "Z")
    expected_last = US30_END_UTC.isoformat().replace("+00:00", "Z")
    ...
    return {
        "schema": "us30_historical_audit_measurement.v1",
...
def build_us30_historical_audit(repository_root: str | Path) -> tuple[bytes, dict[str, Any]]:
    ...
    if any(measurement["facts"].get(name) is not True for name in HISTORICAL_FACT_FIELDS):
        raise US30SourceError("US30 historical source audit did not pass")
...
def source_validation_plan(transition_evidence: str) -> dict[str, Any]:
    ...
    return {
        "schema": "us30_source_validation_plan.v1",
        "source_contract_id": us30_source_contract().source_contract_id,
