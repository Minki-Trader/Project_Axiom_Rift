"""Thin running-Job adapter for prospective volatility fixed-hold research."""
...
import axiom_rift.research.volatility_duration_fixed_hold as replay_module
...
JOB_IMPLEMENTATION_PROTOCOL = (
    "python.source.volatility_duration_fixed_hold.v1"
)
ARTIFACT_NAMESPACE = "volatility-duration-fixed-hold-v1"
...
def _definition(
    ...
    return volatility_duration_fixed_hold_protocol_definition(context)
...
def _trace(
    ...
    return compute_volatility_duration_fixed_hold_family_trace(
        repository_root,
        definition,
    )
...
def volatility_duration_fixed_hold_job_implementation_artifact() -> bytes:
    return fixed_hold_replay_job_implementation_artifact(RUNTIME_ADAPTER)


def volatility_duration_fixed_hold_job_implementation_sha256() -> str:
    return fixed_hold_replay_job_implementation_sha256(RUNTIME_ADAPTER)


def materialize_volatility_duration_fixed_hold_job_implementation(
...
def materialize_volatility_duration_fixed_hold_running_job_repair_proof(
    writer: object,
    *,
    ...
    return materialize_running_job_implementation_repair_proof(
        ...
        adapter=RUNTIME_ADAPTER,
        explanation=(
            "repair the prospective volatility fixed-hold implementation "
            "without changing its registered scientific family"
        ),
        verification_evidence_hashes=verification_evidence_hashes,
    )


def build_volatility_duration_fixed_hold_job_plan(
    *,
    mission_id: str,
    study_id: str,
    executable_id: str,
...
def execute_volatility_duration_fixed_hold_job(
    *,
