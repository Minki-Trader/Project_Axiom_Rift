"""Writer-gated scientific-v2 Job for the sleeve loss-skip risk pair."""
...
def sleeve_loss_skip_risk_criteria() -> tuple[dict[str, object], ...]:
    ...
    if set(by_id) != set(_EVIDENCE_MODE_BY_CRITERION) or len(legacy) != 18:
        raise RuntimeError("sleeve loss-skip criterion inventory drifted")
    return tuple(
        {
            **item,
...
SLEEVE_LOSS_SKIP_RISK_CRITERIA = sleeve_loss_skip_risk_criteria()
...
def sleeve_loss_skip_risk_multiplicity_registrations(
    *,
    definition: ProspectivePairProtocolDefinition,
    subject_executable_id: str,
) -> tuple[dict[str, object], ...]:
    if subject_executable_id not in definition.prospective_executable_ids:
        raise ValueError("sleeve loss-skip subject is outside the pair")
...
def build_sleeve_loss_skip_risk_validation_plan(
    *,
    definition: ProspectivePairProtocolDefinition,
    mission_id: str,
    executable_id: str,
    ...
    return build_validation_plan_v2(
        ...
        criteria=SLEEVE_LOSS_SKIP_RISK_CRITERIA,
...
class SleeveLossSkipRiskJobPlan:
    mission_id: str
    study_id: str
    executable_id: str
    definition: ProspectivePairProtocolDefinition
    ...
    def validated_recomputed_criterion_ids(
        ...
        return validated_recomputed_criterion_ids(
            ...
            expected_criteria=SLEEVE_LOSS_SKIP_RISK_CRITERIA,
            context="prospective sleeve loss-skip risk",
        )


def build_sleeve_loss_skip_risk_job_plan(
    *,
    ...
    return SleeveLossSkipRiskJobPlan(
        mission_id=mission_id,
        study_id=study_id,
        executable_id=executable_id,
        definition=definition,
...
def build_environment_manifest() -> dict[str, object]:
    value = {
        ...
        "sleeve_loss_skip_risk_chassis_implementation_sha256": sleeve_loss_skip_risk_chassis_implementation_sha256(),
        "sleeve_loss_skip_risk_study_implementation_sha256": sleeve_loss_skip_risk_study_implementation_sha256(),
        "sleeve_loss_skip_risk_trace_implementation_sha256": sleeve_loss_skip_risk_trace_implementation_sha256(),
        "validator_id": SCIENTIFIC_ADJUDICATION_VALIDATOR_V2_ID,
    }
...
def _hypothesis(
    ...
    if len(matches) != 1:
        raise ValueError("sleeve loss-skip inference subject is ambiguous")
    return matches[0]


def build_measurement(
    *,
    scoped_plan: SleeveLossSkipRiskJobPlan,
    job_id: str,
    job_hash: str,
    ...
    if not isinstance(metrics, Mapping) or not isinstance(statistics, Mapping):
        raise ValueError("sleeve loss-skip calculation is invalid")
    ...
    for registration in registrations:
        ...
        if not isinstance(raw, Mapping) or not isinstance(synchronized, Mapping):
            raise ValueError("sleeve loss-skip inference p-values are absent")
        ...
        if type(raw_pvalue) is not int or type(adjusted) is not int or adjusted != metric:
            raise ValueError("sleeve loss-skip inference metric binding drifted")
        rows.append(
            {
                **registration,
...
@dataclass(frozen=True, slots=True)
class SleeveLossSkipRiskJobPacket:
