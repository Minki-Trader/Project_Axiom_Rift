"""Discovery runner for registered US100-US30 relative strength Study."""
...
CALLABLE_IDENTITY = "axiom_rift.research.us30_sector_rotation_study.execute_us30_sector_rotation_job.v1"
SURFACE_SCHEMA = "us30_sector_rotation_surface.v1"
EVALUATION_SCHEMA = "us30_sector_rotation_evaluation.v1"
...
def build_us30_sector_rotation_validation_plan(
    executable_id: str,
    *,
    mission_id: str,
) -> dict[str, object]:
...
def _claim_metrics(evaluation: Mapping[str, Any]) -> dict[str, dict[str, int | None]]:
    ...
    if not isinstance(raw, Mapping):
        raise ValueError("us30_sector_rotation evaluation has no metrics")
    ...
    if any(type(name) is not str or type(value) is not int for name, value in metrics.items()):
        raise ValueError("us30_sector_rotation evaluation metrics must be integer scalars")
    ...
    def values(*names: str, null_when_not_evaluable: bool = False) -> dict[str, int | None]:
        ...
        for name in names:
            if name not in metrics:
                raise ValueError(f"us30_sector_rotation evaluation metric is absent: {name}")
            result[name] = (
...
def output_names(executable_id: str) -> dict[str, str]:
    ...
    prefix = f"scientific/us30-sector-rotation/{short}"
    return {
...
def surface_cache_output_name(repository_root: str | Path) -> str:
    ...
    return (
        "local/cache/us30-sector-rotation/us30_sector_rotation-surface-"
        f"{us30_sector_rotation_implementation_sha256()[:12]}-"
        f"{raw_sha256[:12]}.json"
    )


def surface_manifest_output_name() -> str:
    return "scientific/us30-sector-rotation/surface-cache-manifest.json"
...
def _registered_executable_ids(repository_root: str | Path) -> tuple[str, ...]:
    raw_sha256 = us30_raw_sha256(Path(repository_root).resolve())
    mapping = us30_sector_rotation_study_configuration_map(raw_sha256)
    if not isinstance(mapping, Mapping) or len(mapping) != 12:
        raise ValueError("us30_sector_rotation Study requires exactly twelve Executables")
    ...
    if any(
        ...
        raise ValueError("us30_sector_rotation Study Executable identities are invalid")
    return identities


def build_environment_manifest(repository_root: str | Path) -> dict[str, object]:
    if SELECTION_TOTAL_EXPOSURES != 562:
        raise ValueError("us30_sector_rotation Study requires global exposure total 562")
    source = _source_provenance(repository_root)
    value: dict[str, object] = {
        "dataset_sha256": DATASET_SHA256,
        "us30_sector_rotation_implementation_sha256": us30_sector_rotation_implementation_sha256(),
...
class US30SectorRotationJobPacket:
    ...
    def artifact(self, name: str) -> dict[str, Any]:
        ...
        if not isinstance(value, dict):
            raise ValueError("us30_sector_rotation Job artifact is not an object")
        return value
...
def _validate_surface_manifest(
    ...
    *,
    ...
    if (
        ...
        or value.get("schema") != "us30_sector_rotation_surface_cache_manifest.v1"
        ...
        or value.get("us30_sector_rotation_implementation_sha256")
        != us30_sector_rotation_implementation_sha256()
        ...
        or value.get("selection_total_exposures") != 562
        ...
        raise ValueError("us30_sector_rotation surface cache manifest differs from Job inputs")
    ...
    if not isinstance(producer, dict) or set(producer) != {
        ...
        raise ValueError("us30_sector_rotation surface producer execution is invalid")
    ...
    if producer["identity"] != execution.identity:
        raise ValueError("us30_sector_rotation surface producer identity is invalid")
    return value
...
def execute_us30_sector_rotation_job(
    *,
    ...
    if (
        ...
        raise ValueError("running Job is not bound to the registered us30_sector_rotation Study")
    executable_id = evidence_subject["id"]
    scientific_binding = spec.get("scientific_binding")
    if not isinstance(scientific_binding, dict):
        raise ValueError("us30_sector_rotation Job lacks a scientific binding")
    plan = build_us30_sector_rotation_validation_plan(
        executable_id,
    ...
    if not required_inputs.issubset(inputs):
        raise ValueError("us30_sector_rotation Job omits a required content-bound input")
    if scientific_binding.get("validation_plan_hash") != plan_hash:
        raise ValueError("us30_sector_rotation validation plan differs from Job input")
    ...
    if (
        ...
        raise ValueError("us30_sector_rotation scientific binding differs from preregistration")
    ...
    if not isinstance(output_classes, dict):
        raise ValueError("us30_sector_rotation Job output classes are invalid")
    ...
    if expected_outputs != expected_set or output_classes != expected_classes:
        raise ValueError("us30_sector_rotation Job outputs differ from the producer/projector protocol")

    manifest_artifact_hash: str
    manifest_bytes: bytes | None = None
    if produces_surface:
        surface = _validated_surface(
            _compute_registered_us30_sector_rotation_surface(root)
        )
        ...
        if not cache_target.is_file():
            raise ValueError("us30_sector_rotation surface cache is unavailable")
        surface_bytes = cache_target.read_bytes()
        surface_hash = sha256(surface_bytes).hexdigest()
        if surface_hash not in inputs:
            raise ValueError("us30_sector_rotation surface cache hash is not a Job input")
    ...
    context = project_registered_us30_sector_rotation_evaluation(
        surface,
    ...
    if set(output_manifest) != expected_outputs:
        raise ValueError("materialized us30_sector_rotation outputs differ from Job declaration")
    return US30SectorRotationJobPacket(
...
__all__ = [
    ...
    "build_us30_sector_rotation_validation_plan",
    "output_names",
    "planned_verdict",
    "execute_us30_sector_rotation_job",
