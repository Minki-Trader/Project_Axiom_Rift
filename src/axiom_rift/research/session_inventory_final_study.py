"""Final corrected runner for the registered session-inventory Study."""
...
from axiom_rift.research.session_inventory_discovery import (
    DATASET_SHA256,
    OBSERVED_MATERIAL_ID,
    ROLLING_SPLIT_SHA256,
    SELECTION_TOTAL_EXPOSURES,
    _compute_registered_session_inventory_surface,
    session_inventory_implementation_sha256,
    session_inventory_executable_configuration_map,
    loader_implementation_sha256,
    project_session_inventory_evaluation,
...
MISSION_ID = "MIS-0001"
STUDY_ID = "STU-0008"
CALLABLE_IDENTITY = "axiom_rift.research.session_inventory_final_study.execute_session_inventory_job.v1"
...
def build_session_inventory_validation_plan(executable_id: str) -> dict[str, object]:
...
def _claim_metrics(evaluation: Mapping[str, Any]) -> dict[str, dict[str, int | None]]:
    ...
    if not isinstance(raw, Mapping):
        raise ValueError("session_inventory evaluation has no metrics")
    ...
    if any(type(name) is not str or type(value) is not int for name, value in metrics.items()):
        raise ValueError("session_inventory evaluation metrics must be integer scalars")
    ...
    def values(*names: str, null_when_not_evaluable: bool = False) -> dict[str, int | None]:
        ...
        for name in names:
            if name not in metrics:
                raise ValueError(f"session_inventory evaluation metric is absent: {name}")
            result[name] = (
...
def surface_cache_output_name() -> str:
    return (
        f"local/cache/{STUDY_ID}/session_inventory-surface-"
        f"{session_inventory_implementation_sha256()[:16]}.json"
    )


def surface_manifest_output_name() -> str:
    return f"scientific/{STUDY_ID}/session_inventory-surface-cache-manifest.json"
...
def _registered_executable_ids() -> tuple[str, ...]:
    mapping = session_inventory_executable_configuration_map()
    if not isinstance(mapping, Mapping) or len(mapping) != 12:
        raise ValueError("session_inventory Study requires exactly twelve Executables")
    ...
    if any(
        ...
        raise ValueError("session_inventory Study Executable identities are invalid")
    return identities


def build_environment_manifest() -> dict[str, object]:
    if SELECTION_TOTAL_EXPOSURES != 114:
        raise ValueError("session_inventory Study requires global exposure total 114")
    value: dict[str, object] = {
        "dataset_sha256": DATASET_SHA256,
        "session_inventory_implementation_sha256": session_inventory_implementation_sha256(),
...
class SessionInventoryJobPacket:
    ...
    def artifact(self, name: str) -> dict[str, Any]:
        ...
        if not isinstance(value, dict):
            raise ValueError("session_inventory Job artifact is not an object")
        return value
...
def _materialize_cache(
    ...
    *,
    ...
    if cache_root not in target.parents:
        raise ValueError("session_inventory surface cache path escapes local/cache")
    expected_hash = sha256(content).hexdigest()
    if target.exists():
        if not target.is_file() or sha256(target.read_bytes()).hexdigest() != (
            expected_hash
        ):
            raise ValueError("existing session_inventory surface cache has different bytes")
        return
    target.parent.mkdir(parents=True, exist_ok=True)
    descriptor, temporary_name = tempfile.mkstemp(
        prefix=".session_inventory-surface-",
        suffix=".tmp",
        dir=target.parent,
    )
...
def _build_surface_manifest(
    *,
    ...
    value: dict[str, Any] = {
        ...
        "schema": "session_inventory_surface_cache_manifest.v1",
        "selection_total_exposures": SELECTION_TOTAL_EXPOSURES,
        "split_artifact_sha256": ROLLING_SPLIT_SHA256,
        "trend_dependency_sha256": trend_dependency_sha256(),
    }
...
def _validate_surface_manifest(
    ...
    *,
    ...
    if (
        ...
        or value.get("schema") != "session_inventory_surface_cache_manifest.v1"
        ...
        or value.get("session_inventory_implementation_sha256")
        != session_inventory_implementation_sha256()
        ...
        or value.get("selection_total_exposures") != 114
        or value.get("split_artifact_sha256") != ROLLING_SPLIT_SHA256
        or value.get("trend_dependency_sha256") != trend_dependency_sha256()
    ):
        raise ValueError("session_inventory surface cache manifest differs from Job inputs")
    ...
    if not isinstance(producer, dict) or set(producer) != {
        ...
        raise ValueError("session_inventory surface producer execution is invalid")
    ...
    if producer["identity"] != execution.identity:
        raise ValueError("session_inventory surface producer identity is invalid")
    return value
    ...
    *,
    ...
    for input_hash in input_hashes:
        ...
        if isinstance(value, dict) and value.get("schema") == (
            "session_inventory_surface_cache_manifest.v1"
        ):
            matches.append(
                (
                    input_hash,
    ...
    if len(matches) != 1:
        raise ValueError("Job inputs require one exact session_inventory surface manifest")
    return matches[0]


def execute_session_inventory_job(
    *,
    ...
    if (
        ...
        raise ValueError("running Job is not bound to the registered session_inventory Study")
    executable_id = evidence_subject["id"]
    scientific_binding = spec.get("scientific_binding")
    if not isinstance(scientific_binding, dict):
        raise ValueError("session_inventory Job lacks a scientific binding")
    plan = build_session_inventory_validation_plan(executable_id)
    ...
    if not required_inputs.issubset(inputs):
        raise ValueError("session_inventory Job omits a required content-bound input")
    if scientific_binding.get("validation_plan_hash") != plan_hash:
        raise ValueError("session_inventory validation plan differs from Job input")
    ...
    if (
        ...
        raise ValueError("session_inventory scientific binding differs from preregistration")
    ...
    if not isinstance(output_classes, dict):
        raise ValueError("session_inventory Job output classes are invalid")
    ...
    if expected_outputs != expected_set or output_classes != expected_classes:
        raise ValueError("session_inventory Job outputs differ from the producer/projector protocol")

    manifest_artifact_hash: str
    manifest_bytes: bytes | None = None
    if produces_surface:
        surface = _compute_registered_session_inventory_surface(root)
        ...
        if not cache_target.is_file():
            raise ValueError("session_inventory surface cache is unavailable")
        surface_bytes = cache_target.read_bytes()
        surface_hash = sha256(surface_bytes).hexdigest()
        if surface_hash not in inputs:
            raise ValueError("session_inventory surface cache hash is not a Job input")
        ...
        surface = parse_canonical(surface_bytes)
        if not isinstance(surface, dict):
            raise ValueError("session_inventory surface cache is not a canonical object")

    context = project_session_inventory_evaluation(
        surface,
    ...
    if set(output_manifest) != expected_outputs:
        raise ValueError("materialized session_inventory outputs differ from Job declaration")
    return SessionInventoryJobPacket(
...
__all__ = [
    ...
    "build_session_inventory_validation_plan",
    "output_names",
    "planned_verdict",
    "execute_session_inventory_job",
