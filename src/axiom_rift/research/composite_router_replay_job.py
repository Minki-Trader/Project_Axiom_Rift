"""Thin Writer-gated Job entry point for the STU-0016 replay adapter."""

from __future__ import annotations

from pathlib import Path

import axiom_rift.research.composite_router_discovery as source_module
import axiom_rift.research.composite_router_replay as replay_module
import axiom_rift.research.composite_router_replay_parity as parity_module
import axiom_rift.research.historical_family_stu0016 as historical_family_binding_module
...
CALLABLE_IDENTITY = (
    ...
    "execute_composite_router_replay_job.v1"
)
JOB_IMPLEMENTATION_PROTOCOL = "python.source.composite_router_replay.v1"
ARTIFACT_NAMESPACE = "stu0016-composite-router-replay-v1"
...
RUNTIME_ADAPTER = build_routed_sleeve_runtime_adapter(
    ...
    protocol_definition=composite_router_replay_protocol_definition,
    feature_builder=compute_composite_sleeve_scores,
    router_calibrator=calibrate_router,
    score_router=route_composite_score,
    spread_builder=causal_effective_spread,
    raw_parity_validator=assert_composite_router_historical_raw_parity,
)


def composite_router_replay_job_implementation_artifact() -> bytes:
    return fixed_hold_replay_job_implementation_artifact(RUNTIME_ADAPTER)


def composite_router_replay_job_implementation_sha256() -> str:
    return fixed_hold_replay_job_implementation_sha256(RUNTIME_ADAPTER)


def materialize_composite_router_replay_job_implementation(
    writer: StateWriter,
) -> str:
...
def materialize_composite_router_running_job_repair_proof(
    writer: StateWriter,
) -> str:
    return materialize_running_job_implementation_repair_proof(
        ...
        explanation=(
            ...
            "registered STU-0016 scientific family"
        ),
    )


def build_composite_router_replay_job_plan(
    *,
    mission_id: str,
    study_id: str,
    executable_id: str,
...
def execute_composite_router_replay_job(
    *,
...
__all__ = [
    ...
    "materialize_composite_router_running_job_repair_proof",
]
