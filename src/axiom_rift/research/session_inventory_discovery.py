...
"""Causal session-inventory discovery on Foundation-safe US100 M5 data.
...
_SESSION_INVENTORY_FILE = Path(__file__).resolve()


class SessionInventoryBoundaryError(DiscoveryBoundaryError):
...
def session_inventory_implementation_sha256() -> str:
    return _file_sha256(_SESSION_INVENTORY_FILE)
...
class SessionInventoryConfiguration:
    profile: str
    signal_sign: int
    holding_bars: int
...
def session_inventory_configurations() -> tuple[SessionInventoryConfiguration, ...]:
    return tuple(
...
def _local_implementation(function_name: str) -> str:
    return (
        f"axiom_rift.research.session_inventory_discovery.{function_name}@sha256:"
        f"{session_inventory_implementation_sha256()}"
    )
...
def session_inventory_components() -> tuple[ComponentSpec, ...]:
    return (
        ComponentSpec(
            display_name="causal completed-bar session-inventory score",
            protocol="feature.fixed_broker_clock_session_inventory.v1",
            implementation=_local_implementation(
                "compute_session_inventory_score"
            ),
            spec={
...
def session_inventory_executable(configuration: SessionInventoryConfiguration) -> ExecutableSpec:
    return ExecutableSpec(
        display_name=f"session-inventory contrast {configuration.configuration_id}",
        components=session_inventory_components(),
        ...
        engine_contract=(
            "engine:session_inventory_discovery_v1:python3_13_9:numpy2_3_4:"
            "pandas2_3_3:scipy1_16_3:"
            f"session_inventory_sha256_{session_inventory_implementation_sha256()}:"
...
def session_inventory_executable_configuration_map() -> dict[str, SessionInventoryConfiguration]:
    return {
        session_inventory_executable(configuration).identity: configuration
        for configuration in session_inventory_configurations()
    }


def executable_configuration_map() -> dict[str, SessionInventoryConfiguration]:
    return session_inventory_executable_configuration_map()


def compute_session_inventory_score(
    frame: pd.DataFrame,
    profile: str,
...
class _ConfigurationResult:
    configuration: SessionInventoryConfiguration
    executable_id: str
...
def _evaluate_configuration(
    *,
    ...
    score, volatility, run = compute_session_inventory_score(
    ...
    for fold in folds:
        ...
        prefix_score, prefix_volatility, prefix_run = (
            compute_session_inventory_score(prefix_frame, configuration.profile)
        )
    ...
    return _ConfigurationResult(
        configuration,
        session_inventory_executable(configuration).identity,
        metrics,
        fold_metrics,
        regime_metrics,
        session_metrics,
...
def _adjusted_bootstrap_upper_pvalue(values: np.ndarray, *, seed_label: str) -> int:
    ...
    if len(sample) < 30 or np.any(~np.isfinite(sample)):
        raise SessionInventoryBoundaryError("bootstrap series is invalid or too short")
...
def _matched_result(
    ...
    *,
    ...
    if len(matches) != 1:
        raise SessionInventoryBoundaryError("registered control match is not unique")
    return matches[0]
...
def _paired_pvalue(subject: _ConfigurationResult, control: _ConfigurationResult, role: str) -> int:
    if not subject.daily_pnl.index.equals(control.daily_pnl.index):
        raise SessionInventoryBoundaryError("paired controls have different eligible days")
...
def _populate_pvalues_and_controls(results: Sequence[_ConfigurationResult]) -> None:
    for subject in results:
        ...
        if any(type(value) is not int for value in subject.metrics.values()):
            raise SessionInventoryBoundaryError("scientific metrics are not fixed-point integers")
...
def _claim_limits() -> list[str]:
    return [
        ...
        "monthly_drawdown_share_gate_is_for_sparse_session_inventory_surfaces_only",
...
def _compute_registered_session_inventory_surface(repository_root: str | Path) -> dict[str, Any]:
    if not isinstance(repository_root, (str, Path)):
        raise SessionInventoryBoundaryError(
    ...
    surface: dict[str, Any] = {
        ...
        "discovery_implementation_sha256": session_inventory_implementation_sha256(),
        "engine_environment": {
        ...
        "schema": "session_inventory_discovery_surface.v1",
        "selection_context": [
            {
...
def project_session_inventory_evaluation(
    ...
    *,
    ...
    for name, digest in (
        ...
        if type(digest) is not str or len(digest) != 64 or any(c not in "0123456789abcdef" for c in digest):
            raise SessionInventoryBoundaryError(f"{name} hash is invalid")
    if not isinstance(job_execution, Mapping) or set(job_execution) != {
        "identity", "job_hash", "job_id", "job_permit_id", "start_record_id"
    }:
        raise SessionInventoryBoundaryError("Job execution binding is invalid")
    ...
    if job_execution["identity"] != canonical_digest(domain="running-job-execution", payload=payload):
        raise SessionInventoryBoundaryError("Job execution identity is invalid")
    value = dict(surface)
    if sha256(canonical_bytes(value)).hexdigest() != surface_artifact_hash:
        raise SessionInventoryBoundaryError("surface bytes differ from their artifact hash")
    expected_fields = {
    ...
    if set(value) != expected_fields or value.get("schema") != "session_inventory_discovery_surface.v1":
        raise SessionInventoryBoundaryError("session-inventory surface schema is invalid")
    expected = session_inventory_executable_configuration_map()
    ...
    evaluation = {
        **dict(by_identity[subject_executable_id]),
        ...
        "schema": "session_inventory_discovery_evaluation.v1",
...
__all__ = [
    ...
    "SessionInventoryBoundaryError", "SessionInventoryConfiguration", "SELECTOR_QUANTILE_BP",
    "SELECTION_BLOCK_LENGTHS", "SELECTION_BOOTSTRAP_SAMPLES",
    "SELECTION_MONTE_CARLO_CONFIDENCE_PPM", "SELECTION_SEED",
    "SELECTION_TOTAL_EXPOSURES", "_compute_registered_session_inventory_surface",
    "calibrate_selector", "compute_session_inventory_score",
    "executable_configuration_map",
    "loader_implementation_sha256", "project_session_inventory_evaluation",
    "session_inventory_components", "session_inventory_configurations", "session_inventory_executable",
    "session_inventory_executable_configuration_map", "session_inventory_implementation_sha256",
