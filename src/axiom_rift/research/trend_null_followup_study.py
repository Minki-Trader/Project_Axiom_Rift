"""Follow-up runner for the registered trend-null contrast Study."""
...
from axiom_rift.research.trend_null_followup_discovery import (
    DATASET_SHA256,
    OBSERVED_MATERIAL_ID,
    ROLLING_SPLIT_SHA256,
    SELECTION_TOTAL_EXPOSURES,
    _compute_registered_trend_null_surface as _compute_registered_trend_null_followup_surface,
    trend_null_implementation_sha256 as trend_null_followup_implementation_sha256,
    trend_null_executable_configuration_map as trend_null_followup_executable_configuration_map,
    loader_implementation_sha256,
    project_trend_null_evaluation as project_trend_null_followup_evaluation,
...
MISSION_ID = "MIS-0001"
STUDY_ID = "STU-0013"
CALLABLE_IDENTITY = "axiom_rift.research.trend_null_followup_study.execute_trend_null_followup_job.v1"
SURFACE_SCHEMA = "trend_null_followup_surface.v1"
...
def build_trend_null_followup_validation_plan(executable_id: str) -> dict[str, object]:
...
def _claim_metrics(evaluation: Mapping[str, Any]) -> dict[str, dict[str, int | None]]:
    ...
    if not isinstance(raw, Mapping):
        raise ValueError("trend_null_followup evaluation has no metrics")
    ...
    if any(type(name) is not str or type(value) is not int for name, value in metrics.items()):
        raise ValueError("trend_null_followup evaluation metrics must be integer scalars")
    ...
    def values(*names: str, null_when_not_evaluable: bool = False) -> dict[str, int | None]:
        ...
        for name in names:
            if name not in metrics:
                raise ValueError(f"trend_null_followup evaluation metric is absent: {name}")
            result[name] = (
...
def surface_cache_output_name() -> str:
    return (
        f"local/cache/{STUDY_ID}/trend_null_followup-surface-"
        f"{trend_null_followup_implementation_sha256()[:16]}.json"
    )


def surface_manifest_output_name() -> str:
    return f"scientific/{STUDY_ID}/trend_null_followup-surface-cache-manifest.json"
...
def _registered_executable_ids() -> tuple[str, ...]:
    mapping = trend_null_followup_executable_configuration_map()
    if not isinstance(mapping, Mapping) or len(mapping) != 12:
        raise ValueError("trend_null_followup Study requires exactly twelve Executables")
    ...
    if any(
        ...
        raise ValueError("trend_null_followup Study Executable identities are invalid")
    return identities


def build_environment_manifest() -> dict[str, object]:
    if SELECTION_TOTAL_EXPOSURES != 174:
        raise ValueError("trend_null_followup Study requires global exposure total 174")
    value: dict[str, object] = {
        "dataset_sha256": DATASET_SHA256,
        "trend_null_followup_implementation_sha256": trend_null_followup_implementation_sha256(),
...
class TrendNullFollowupJobPacket:
    ...
    def artifact(self, name: str) -> dict[str, Any]:
        ...
        if not isinstance(value, dict):
            raise ValueError("trend_null_followup Job artifact is not an object")
        return value
...
def _materialize_cache(
    ...
    *,
    ...
    if cache_root not in target.parents:
        raise ValueError("trend_null_followup surface cache path escapes local/cache")
    expected_hash = sha256(content).hexdigest()
    if target.exists():
        if not target.is_file() or sha256(target.read_bytes()).hexdigest() != (
            expected_hash
        ):
            raise ValueError("existing trend_null_followup surface cache has different bytes")
        return
    target.parent.mkdir(parents=True, exist_ok=True)
    descriptor, temporary_name = tempfile.mkstemp(
        prefix=".trend_null_followup-surface-",
        suffix=".tmp",
        dir=target.parent,
    )
...
def _build_surface_manifest(
    *,
    ...
    value: dict[str, Any] = {
        ...
        "schema": "trend_null_followup_surface_cache_manifest.v1",
        "selection_total_exposures": SELECTION_TOTAL_EXPOSURES,
        "split_artifact_sha256": ROLLING_SPLIT_SHA256,
        "trend_dependency_sha256": trend_dependency_sha256(),
    }
...
def _validate_surface_manifest(
    ...
    *,
    ...
    if (
        ...
        or value.get("schema") != "trend_null_followup_surface_cache_manifest.v1"
        ...
        or value.get("trend_null_followup_implementation_sha256")
        != trend_null_followup_implementation_sha256()
        ...
        or value.get("selection_total_exposures") != 174
        or value.get("split_artifact_sha256") != ROLLING_SPLIT_SHA256
        or value.get("trend_dependency_sha256") != trend_dependency_sha256()
    ):
        raise ValueError("trend_null_followup surface cache manifest differs from Job inputs")
    ...
    if not isinstance(producer, dict) or set(producer) != {
        ...
        raise ValueError("trend_null_followup surface producer execution is invalid")
    ...
    if producer["identity"] != execution.identity:
        raise ValueError("trend_null_followup surface producer identity is invalid")
    return value
    ...
    *,
    ...
    for input_hash in input_hashes:
        ...
        if isinstance(value, dict) and value.get("schema") == (
            "trend_null_followup_surface_cache_manifest.v1"
        ):
            matches.append(
                (
                    input_hash,
    ...
    if len(matches) != 1:
        raise ValueError("Job inputs require one exact trend_null_followup surface manifest")
    return matches[0]


def execute_trend_null_followup_job(
    *,
    repository_root: str | Path,
    execution: RunningJobExecution,
) -> TrendNullFollowupJobPacket:
    ...
    if (
        ...
        raise ValueError("running Job is not bound to the registered trend_null_followup Study")
    executable_id = evidence_subject["id"]
    scientific_binding = spec.get("scientific_binding")
    if not isinstance(scientific_binding, dict):
        raise ValueError("trend_null_followup Job lacks a scientific binding")
    plan = build_trend_null_followup_validation_plan(executable_id)
    ...
    if not required_inputs.issubset(inputs):
        raise ValueError("trend_null_followup Job omits a required content-bound input")
    if scientific_binding.get("validation_plan_hash") != plan_hash:
        raise ValueError("trend_null_followup validation plan differs from Job input")
    ...
    if (
        ...
        raise ValueError("trend_null_followup scientific binding differs from preregistration")
    ...
    if not isinstance(output_classes, dict):
        raise ValueError("trend_null_followup Job output classes are invalid")
    ...
    if expected_outputs != expected_set or output_classes != expected_classes:
        raise ValueError("trend_null_followup Job outputs differ from the producer/projector protocol")

    manifest_artifact_hash: str
    manifest_bytes: bytes | None = None
    if produces_surface:
        surface = _validated_surface(
            _compute_registered_trend_null_followup_surface(root)
        )
        ...
        if not cache_target.is_file():
            raise ValueError("trend_null_followup surface cache is unavailable")
        surface_bytes = cache_target.read_bytes()
        surface_hash = sha256(surface_bytes).hexdigest()
        if surface_hash not in inputs:
            raise ValueError("trend_null_followup surface cache hash is not a Job input")
    ...
    context = project_trend_null_followup_evaluation(
        surface,
    ...
    if set(output_manifest) != expected_outputs:
        raise ValueError("materialized trend_null_followup outputs differ from Job declaration")
    return TrendNullFollowupJobPacket(
...
__all__ = [
    ...
    "build_trend_null_followup_validation_plan",
    "output_names",
    "planned_verdict",
    "execute_trend_null_followup_job",
