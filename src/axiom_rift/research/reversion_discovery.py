"""Causal overextension-reversion discovery on Foundation-safe US100 M5 data.
...
_REVERSION_FILE = Path(__file__).resolve()


class ReversionBoundaryError(DiscoveryBoundaryError):
    """Raised before unregistered reversion semantics can enter evaluation."""
...
def reversion_implementation_sha256() -> str:
    return _file_sha256(_REVERSION_FILE)
...
class ReversionConfiguration:
    profile: str
    signal_sign: int
    holding_bars: int

    def __post_init__(self) -> None:
        if self.profile not in _PROFILE_WINDOWS:
    ...
    @property
    def window(self) -> int:
        return _PROFILE_WINDOWS[self.profile]
...
def reversion_configurations() -> tuple[ReversionConfiguration, ...]:
    return tuple(
        ReversionConfiguration(profile, signal_sign, holding_bars)
        for profile in ("fast_24", "balanced_48", "slow_96")
...
def _local_implementation(function_name: str) -> str:
    return (
        f"axiom_rift.research.reversion_discovery.{function_name}@sha256:"
...
def reversion_components() -> tuple[ComponentSpec, ...]:
    return (
        ComponentSpec(
            display_name="causal prior-close overextension z score",
            protocol="feature.prior_close_overextension_zscore.v1",
            ...
            spec={
                ...
                "profiles": dict(_PROFILE_WINDOWS),
            },
        ),
        ComponentSpec(
...
def reversion_executable(configuration: ReversionConfiguration) -> ExecutableSpec:
    return ExecutableSpec(
        display_name=f"overextension contrast {configuration.configuration_id}",
        components=reversion_components(),
        ...
        engine_contract=(
            "engine:reversion_discovery_v1:python3_13_9:numpy2_3_4:"
            "pandas2_3_3:scipy1_16_3:"
            f"reversion_sha256_{reversion_implementation_sha256()}:"
...
def reversion_executable_configuration_map() -> dict[str, ReversionConfiguration]:
    return {
        reversion_executable(configuration).identity: configuration
        for configuration in reversion_configurations()
    }


def executable_configuration_map() -> dict[str, ReversionConfiguration]:
    return reversion_executable_configuration_map()
...
def compute_overextension_score(
    ...
    if type(window) is not int or window not in set(_PROFILE_WINDOWS.values()):
...
class _ConfigurationResult:
    configuration: ReversionConfiguration
    executable_id: str
    ...
    *,
    ...
    for fold in folds:
        ...
        if len(train_volatility) < 1000:
            raise ReversionBoundaryError("regime calibration is too small")
        cutoffs = (
    ...
    return _ConfigurationResult(
        configuration,
        reversion_executable(configuration).identity,
        metrics,
        fold_metrics,
        regime_metrics,
        session_metrics,
...
def _adjusted_bootstrap_upper_pvalue(values: np.ndarray, *, seed_label: str) -> int:
    ...
    if len(sample) < 30 or np.any(~np.isfinite(sample)):
        raise ReversionBoundaryError("bootstrap series is invalid or too short")
...
def _matched_result(
    ...
    *,
    ...
    if len(matches) != 1:
        raise ReversionBoundaryError("registered control match is not unique")
    return matches[0]
...
def _paired_pvalue(subject: _ConfigurationResult, control: _ConfigurationResult, role: str) -> int:
    if not subject.daily_pnl.index.equals(control.daily_pnl.index):
        raise ReversionBoundaryError("paired controls have different eligible days")
...
def _populate_pvalues_and_controls(results: Sequence[_ConfigurationResult]) -> None:
    for subject in results:
        ...
        profile_controls = [
            ...
            for profile in _PROFILE_WINDOWS
        ...
        if any(type(value) is not int for value in subject.metrics.values()):
            raise ReversionBoundaryError("scientific metrics are not fixed-point integers")
...
def _compute_registered_reversion_surface(repository_root: str | Path) -> dict[str, Any]:
    if not isinstance(repository_root, (str, Path)):
        raise ReversionBoundaryError("reversion surface requires a repository path")
    _validate_engine_environment()
    ...
    surface: dict[str, Any] = {
        ...
        "discovery_implementation_sha256": reversion_implementation_sha256(),
        "engine_environment": {
        ...
        "schema": "reversion_discovery_surface.v1",
        "selection_context": [
            {
...
def project_reversion_evaluation(
    ...
    *,
    ...
    if job_execution["identity"] != canonical_digest(domain="running-job-execution", payload=payload):
        raise ReversionBoundaryError("Job execution identity is invalid")
    value = dict(surface)
    if sha256(canonical_bytes(value)).hexdigest() != surface_artifact_hash:
        raise ReversionBoundaryError("surface bytes differ from their artifact hash")
    expected_fields = {
    ...
    if set(value) != expected_fields or value.get("schema") != "reversion_discovery_surface.v1":
        raise ReversionBoundaryError("reversion surface schema is invalid")
    expected = reversion_executable_configuration_map()
    evaluations = value.get("evaluations")
    if not isinstance(evaluations, list) or len(evaluations) != len(expected):
        raise ReversionBoundaryError("reversion surface evaluation count is invalid")
    ...
    if len(by_identity) != len(evaluations) or set(by_identity) != set(expected) or subject_executable_id not in expected:
        raise ReversionBoundaryError("reversion surface subjects differ from registration")
    for identity, configuration in expected.items():
        if by_identity[identity].get("subject_configuration_id") != configuration.configuration_id:
            raise ReversionBoundaryError("reversion surface configuration binding differs")
    evaluation = {
        **dict(by_identity[subject_executable_id]),
        "claim_limits": value["claim_limits"],
        "job_execution": dict(job_execution),
        "schema": "reversion_discovery_evaluation.v1",
...
__all__ = [
    ...
    "ReversionBoundaryError", "ReversionConfiguration", "SELECTOR_QUANTILE_BP",
    "SELECTION_BLOCK_LENGTHS", "SELECTION_BOOTSTRAP_SAMPLES",
    "SELECTION_MONTE_CARLO_CONFIDENCE_PPM", "SELECTION_SEED",
    "SELECTION_TOTAL_EXPOSURES", "_compute_registered_reversion_surface",
    "calibrate_selector", "compute_overextension_score", "executable_configuration_map",
    "loader_implementation_sha256", "project_reversion_evaluation",
    "reversion_components", "reversion_configurations", "reversion_executable",
    "reversion_executable_configuration_map", "reversion_implementation_sha256",
