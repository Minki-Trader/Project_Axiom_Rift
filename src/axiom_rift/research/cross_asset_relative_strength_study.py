"""Discovery runner for registered US100-US500 relative strength Study."""
...
from axiom_rift.research.cross_asset_relative_strength_discovery import (
    DATASET_SHA256,
...
MISSION_ID = "MIS-0001"
STUDY_ID = "STU-0020"
CALLABLE_IDENTITY = "axiom_rift.research.cross_asset_relative_strength_study.execute_cross_asset_relative_strength_job.v1"
SURFACE_SCHEMA = "cross_asset_relative_strength_surface.v1"
...
def build_cross_asset_relative_strength_validation_plan(executable_id: str) -> dict[str, object]:
...
def _claim_metrics(evaluation: Mapping[str, Any]) -> dict[str, dict[str, int | None]]:
    ...
    if not isinstance(raw, Mapping):
        raise ValueError("cross_asset_relative_strength evaluation has no metrics")
    ...
    if any(type(name) is not str or type(value) is not int for name, value in metrics.items()):
        raise ValueError("cross_asset_relative_strength evaluation metrics must be integer scalars")
    ...
    def values(*names: str, null_when_not_evaluable: bool = False) -> dict[str, int | None]:
        ...
        for name in names:
            if name not in metrics:
                raise ValueError(f"cross_asset_relative_strength evaluation metric is absent: {name}")
            result[name] = (
...
def surface_cache_output_name(repository_root: str | Path) -> str:
    ...
    return (
        f"local/cache/{STUDY_ID}/cross_asset_relative_strength-surface-"
        f"{cross_asset_relative_strength_implementation_sha256()[:12]}-"
        f"{raw_sha256[:12]}.json"
    )


def surface_manifest_output_name() -> str:
    return f"scientific/{STUDY_ID}/cross_asset_relative_strength-surface-cache-manifest.json"
...
def _validated_surface(value: object) -> dict[str, Any]:
    if not isinstance(value, dict) or value.get("schema") != SURFACE_SCHEMA:
        raise ValueError("cross-asset relative-strength surface schema is invalid")
    return value
...
def _registered_executable_ids(repository_root: str | Path) -> tuple[str, ...]:
    mapping = cross_asset_relative_strength_executable_configuration_map(
        Path(repository_root).resolve()
    )
    if not isinstance(mapping, Mapping) or len(mapping) != 12:
        raise ValueError("cross_asset_relative_strength Study requires exactly twelve Executables")
    ...
    if any(
        ...
        raise ValueError("cross_asset_relative_strength Study Executable identities are invalid")
    return identities


def build_environment_manifest(repository_root: str | Path) -> dict[str, object]:
    if SELECTION_TOTAL_EXPOSURES != 234:
        raise ValueError("cross_asset_relative_strength Study requires global exposure total 234")
    source = _source_provenance(repository_root)
    value: dict[str, object] = {
        "dataset_sha256": DATASET_SHA256,
        "cross_asset_relative_strength_implementation_sha256": cross_asset_relative_strength_implementation_sha256(),
...
class CrossAssetRelativeStrengthJobPacket:
    ...
    def artifact(self, name: str) -> dict[str, Any]:
        ...
        if not isinstance(value, dict):
            raise ValueError("cross_asset_relative_strength Job artifact is not an object")
        return value
...
def _materialize_cache(
    ...
    *,
    ...
    if cache_root not in target.parents:
        raise ValueError("cross_asset_relative_strength surface cache path escapes local/cache")
    expected_hash = sha256(content).hexdigest()
    if target.exists():
        if not target.is_file() or sha256(target.read_bytes()).hexdigest() != (
            expected_hash
        ):
            raise ValueError("existing cross_asset_relative_strength surface cache has different bytes")
        return
    target.parent.mkdir(parents=True, exist_ok=True)
    descriptor, temporary_name = tempfile.mkstemp(
        prefix=".cross_asset_relative_strength-surface-",
        suffix=".tmp",
        dir=target.parent,
    )
...
def _build_surface_manifest(
    *,
    ...
    value: dict[str, Any] = {
        ...
        "schema": "cross_asset_relative_strength_surface_cache_manifest.v1",
        "selection_total_exposures": SELECTION_TOTAL_EXPOSURES,
        "split_artifact_sha256": ROLLING_SPLIT_SHA256,
        "trend_dependency_sha256": trend_dependency_sha256(),
        **source,
...
def _validate_surface_manifest(
    ...
    *,
    ...
    if (
        ...
        or value.get("schema") != "cross_asset_relative_strength_surface_cache_manifest.v1"
        ...
        or value.get("cross_asset_relative_strength_implementation_sha256")
        != cross_asset_relative_strength_implementation_sha256()
        ...
        or value.get("selection_total_exposures") != 234
        ...
        raise ValueError("cross_asset_relative_strength surface cache manifest differs from Job inputs")
    ...
    if not isinstance(producer, dict) or set(producer) != {
        ...
        raise ValueError("cross_asset_relative_strength surface producer execution is invalid")
    ...
    if producer["identity"] != execution.identity:
        raise ValueError("cross_asset_relative_strength surface producer identity is invalid")
    return value
    ...
    *,
    ...
    for input_hash in input_hashes:
        ...
        if isinstance(value, dict) and value.get("schema") == (
            "cross_asset_relative_strength_surface_cache_manifest.v1"
        ):
            matches.append(
                (
                    input_hash,
    ...
    if len(matches) != 1:
        raise ValueError("Job inputs require one exact cross_asset_relative_strength surface manifest")
    return matches[0]


def execute_cross_asset_relative_strength_job(
    *,
    repository_root: str | Path,
    execution: RunningJobExecution,
) -> CrossAssetRelativeStrengthJobPacket:
    ...
    if (
        ...
        raise ValueError("running Job is not bound to the registered cross_asset_relative_strength Study")
    executable_id = evidence_subject["id"]
    scientific_binding = spec.get("scientific_binding")
    if not isinstance(scientific_binding, dict):
        raise ValueError("cross_asset_relative_strength Job lacks a scientific binding")
    ...
    if not required_inputs.issubset(inputs):
        raise ValueError("cross_asset_relative_strength Job omits a required content-bound input")
    if scientific_binding.get("validation_plan_hash") != plan_hash:
        raise ValueError("cross_asset_relative_strength validation plan differs from Job input")
    ...
    if (
        ...
        raise ValueError("cross_asset_relative_strength scientific binding differs from preregistration")
    ...
    if not isinstance(output_classes, dict):
        raise ValueError("cross_asset_relative_strength Job output classes are invalid")
    ...
    if expected_outputs != expected_set or output_classes != expected_classes:
        raise ValueError("cross_asset_relative_strength Job outputs differ from the producer/projector protocol")

    manifest_artifact_hash: str
    manifest_bytes: bytes | None = None
    if produces_surface:
        surface = _validated_surface(
            _compute_registered_cross_asset_relative_strength_surface(root)
        )
        ...
        if not cache_target.is_file():
            raise ValueError("cross_asset_relative_strength surface cache is unavailable")
        surface_bytes = cache_target.read_bytes()
        surface_hash = sha256(surface_bytes).hexdigest()
        if surface_hash not in inputs:
            raise ValueError("cross_asset_relative_strength surface cache hash is not a Job input")
    ...
    context = project_cross_asset_relative_strength_evaluation(
        surface,
    ...
    if not isinstance(context, dict) or context.get("schema") != EVALUATION_SCHEMA:
        raise ValueError("cross-asset relative-strength evaluation schema is invalid")
    ...
    if set(output_manifest) != expected_outputs:
        raise ValueError("materialized cross_asset_relative_strength outputs differ from Job declaration")
    return CrossAssetRelativeStrengthJobPacket(
...
__all__ = [
    ...
    "output_names",
    "planned_verdict",
    "execute_cross_asset_relative_strength_job",
