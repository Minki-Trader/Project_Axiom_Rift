"""Independent validator for FPMarkets USDJPY source eligibility evidence."""
...
SOURCE_ELIGIBILITY_VALIDATOR_ID = validator_identity(
    protocol="fpmarkets_usdjpy_source_eligibility.v3",
...
class SourceEligibilityValidator:
    ...
    def validate(self, request: EvidenceValidationRequest) -> ValidatedEvidence:
        if request.domain != "source" or request.validator_id != self.validator_id:
            raise EvidenceValidationError("USDJPY validator received another domain")
        ...
        source_id = usdjpy_source_contract().source_contract_id
        if (
        ...
        measurement_hashes = tuple(
            sorted(artifacts[name].sha256 for name in measurement_names)
        )
        observed_hashes = result.get("measurement_artifact_hashes")
        if (
            not isinstance(observed_hashes, (list, tuple))
            or tuple(sorted(observed_hashes)) != measurement_hashes
        ):
        ...
        if transition == "historical_audit":
            ...
            expected = audit_usdjpy_historical_bytes(
            ...
            except USDJPYSourceError as exc:
                raise EvidenceValidationError(
                    "USDJPY runtime probe schema is invalid"
                ) from exc
