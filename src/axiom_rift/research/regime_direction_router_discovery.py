"""Registered high-vol momentum versus calm-regime reversal router surface."""
...
from axiom_rift.research.regime_direction_router_chassis import SELECTION_TOTAL_EXPOSURES, executable_configuration_map, loader_implementation_sha256, regime_direction_router_chassis_implementation_sha256, regime_direction_router_configurations, regime_direction_router_executable, simulate_regime_direction_router
...
def regime_direction_router_discovery_implementation_sha256() -> str:
...
def _matched(results: list[Any], policy: str) -> Any:
    ...
    if len(found) != 1:
        raise DiscoveryBoundaryError("regime direction router control is not unique")
    return found[0]


def _populate_controls(results: list[Any]) -> None:
    control = _matched(results, "high_vol_long_abstain_control")
    for subject in results:
        ...
        subject.metrics["router_control_pvalue_upper_ppm"] = 1_000_000 if subject is control else _paired_control_pvalue(subject, control, role="matched_high_vol_long_abstain_control", total_exposures=SELECTION_TOTAL_EXPOSURES)


def compute_registered_regime_direction_router_surface(repository_root: str | Path) -> dict[str, Any]:
    ...
    results = []
    for configuration in regime_direction_router_configurations():
        ...
        results.append(_evaluate_configuration(calibrations=calibrations, frame=frame, features=first, fold_features=fold_scores, folds=folds, configuration=configuration, effective_spread=spread, prefix_features=prefix_scores, prefix_spreads=prefix_spreads, time=time, executable_id=regime_direction_router_executable(configuration).identity, simulation_fn=simulate_regime_direction_router))
    ...
    surface = {
        "claim_limits": _claim_limits() + ["regime_trade_and_synthesis_are_the_primary_changed_research_layers", "dense12_feature_label_model_selector_lifecycle_risk_and_execution_are_fixed", "volatility_cutoffs_are_train_only_terciles", "two_trial_surface"],
        ...
        "regime_direction_router_chassis_implementation_sha256": regime_direction_router_chassis_implementation_sha256(),
        "regime_direction_router_discovery_implementation_sha256": regime_direction_router_discovery_implementation_sha256(),
        "schema": "regime_direction_router_surface.v1",
...
def project_regime_direction_router_evaluation(surface: Mapping[str, Any], *, job_execution: Mapping[str, str], subject_executable_id: str, surface_artifact_hash: str, surface_manifest_hash: str) -> dict[str, Any]:
    value = dict(surface)
    if sha256(canonical_bytes(value)).hexdigest() != surface_artifact_hash or value.get("schema") != "regime_direction_router_surface.v1":
        raise DiscoveryBoundaryError("regime direction router surface invalid")
    ...
    if set(by_subject) != set(expected) or subject_executable_id not in expected:
        raise DiscoveryBoundaryError("regime direction router subjects differ")
    ...
    if job_execution.get("identity") != canonical_digest(domain="running-job-execution", payload=payload):
        raise DiscoveryBoundaryError("regime direction router Job invalid")
    result = {**dict(by_subject[subject_executable_id]), "claim_limits": value["claim_limits"], "job_execution": dict(job_execution), "schema": "regime_direction_router_evaluation.v1", "selection_context": value["selection_context"], "selection_method": value["selection_method"], "session_semantics": value["session_semantics"], "surface_artifact_hash": surface_artifact_hash, "surface_manifest_hash": surface_manifest_hash}
    canonical_bytes(result)
    return result


__all__ = ["compute_registered_regime_direction_router_surface", "project_regime_direction_router_evaluation", "regime_direction_router_discovery_implementation_sha256"]
