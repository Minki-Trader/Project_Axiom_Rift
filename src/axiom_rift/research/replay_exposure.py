...
def derive_frozen_registered_family_exposure_context(
    *,
    family_trials: Sequence[IndexRecord],
    prior_global_exposure_floor: int,
    prior_registered_trial_count: int,
    study_id: str,
    expected_family_size: int,
    parameter_name: str | None,
    require_complete: bool = True,
) -> FrozenFamilyExposureContext:
    """Validate one keyed family using an independently projected ordinal.

    ``prior_registered_trial_count`` is the SQL-projected count of all trial
    records whose immutable Journal authority sequence precedes the first
    family member.  Keeping the count separate lets the operational reader
    avoid materializing all historical trial payloads while preserving the
    exact ordinal used by :func:`derive_frozen_family_exposure_context`.
    A crash-resume reader may set ``require_complete`` false to validate the
    non-empty registered prefix; scientific execution keeps it true.
    """
    ...
    if (
        type(prior_global_exposure_floor) is not int
        or prior_global_exposure_floor < 0
        or type(prior_registered_trial_count) is not int
        or prior_registered_trial_count < 0
        or type(expected_family_size) is not int
        or expected_family_size < 1
        or type(require_complete) is not bool
    ):
        raise ReplayExposureError("replay exposure bounds are invalid")
    family = tuple(family_trials)
    if any(not isinstance(record, IndexRecord) for record in family):
        raise ReplayExposureError("replay trial projection is not typed")
    if not family or len(family) > expected_family_size or (
        require_complete and len(family) != expected_family_size
    ):
        raise ReplayExposureError("registered replay family is incomplete")
    ...
    if len(sequences) != len(set(sequences)):
        raise ReplayExposureError("replay trial authority order is ambiguous")
    if any(record.payload.get("study_id") != study_id for record in family):
        raise ReplayExposureError("registered replay family Study binding drifted")
    family_ids = tuple(record.record_id for record in family)
    if len(family_ids) != len(set(family_ids)):
        raise ReplayExposureError("registered replay family is duplicated")
    first_sequence = min(
    ...
    frozen_count = prior_global_exposure_floor + prior_registered_trial_count
    if parameter_name is not None:
        contexts: set[int] = set()
        for record in family:
            executable = record.payload.get("executable")
