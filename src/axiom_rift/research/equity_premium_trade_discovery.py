"""Registered discovery surface for symmetric versus long-only entry policy."""
...
from axiom_rift.research.equity_premium_trade_chassis import (
    SELECTION_TOTAL_EXPOSURES, equity_premium_trade_chassis_implementation_sha256,
...
def equity_premium_trade_discovery_implementation_sha256() -> str:
    return sha256(_THIS_FILE.read_bytes()).hexdigest()


def _matched(results: list[Any], policy: str) -> Any:
    found = [value for value in results if value.configuration.trade_policy == policy]
    if len(found) != 1:
        raise DiscoveryBoundaryError("equity-premium trade control is not unique")
    return found[0]


def _populate_controls(results: list[Any]) -> None:
    control = _matched(results, "symmetric_direction_control")
    for subject in results:
        subject.metrics["trade_control_delta_net_profit_micropoints"] = (
            subject.metrics["net_profit_micropoints"]
            - control.metrics["net_profit_micropoints"]
        )
        subject.metrics["trade_control_pvalue_upper_ppm"] = (
            1_000_000 if subject is control else _paired_control_pvalue(
                subject, control, role="matched_symmetric_direction_control",
                total_exposures=SELECTION_TOTAL_EXPOSURES,
            )
        )


def compute_registered_equity_premium_trade_surface(
    ...
    results = [
        _evaluate_configuration(
            ...
            executable_id=equity_premium_trade_executable(configuration).identity,
            simulation_fn=simulate_equity_premium_trade,
        )
        for configuration in equity_premium_trade_configurations()
    ]
    ...
    surface = {
        "claim_limits": _claim_limits() + [
            "trade_is_the_only_primary_changed_research_layer",
            "signal_label_model_lifecycle_risk_lot_and_execution_are_fixed",
            "long_only_is_a_structural_direction_gate_not_a_threshold_tune",
            "two_trial_surface",
        ],
        ...
        "schema": "equity_premium_trade_surface.v1",
        "selection_context": [{
...
def project_equity_premium_trade_evaluation(
    ...
    if (
        ...
        or value.get("schema") != "equity_premium_trade_surface.v1"
    ):
        raise DiscoveryBoundaryError("equity-premium trade surface invalid")
    ...
    if set(by_id) != set(expected) or subject_executable_id not in expected:
        raise DiscoveryBoundaryError("equity-premium trade subjects differ")
    payload = {
    ...
    if job_execution.get("identity") != canonical_digest(
        ...
        raise DiscoveryBoundaryError("equity-premium trade Job invalid")
    result = {
        **dict(by_id[subject_executable_id]), "claim_limits": value["claim_limits"],
        "job_execution": dict(job_execution),
        "schema": "equity_premium_trade_evaluation.v1",
