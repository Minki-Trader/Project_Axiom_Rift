"""Writer-gated evidence Job for the path-event label contrast."""
...
MISSION_ID = "MIS-0006"
STUDY_ID = "STU-0065"
CALLABLE_IDENTITY = "axiom_rift.research.event_label_study.execute_event_label_job.v1"
EVIDENCE_DEPTH = "discovery"


def build_event_label_validation_plan(executable_id: str) -> dict[str, object]:
...
def surface_output_name() -> str:
    return f"scientific/{STUDY_ID}/event-label-surface.json"


def surface_manifest_output_name() -> str:
    return f"scientific/{STUDY_ID}/event-label-surface-manifest.json"
...
def _load_surface(
    ...
    for artifact_hash in input_hashes:
        try:
            ...
            continue
        if isinstance(value, dict) and value.get("schema") == "event_label_surface.v1":
            surface = (value, artifact_hash)
        if (
            isinstance(value, dict)
            and value.get("schema") == "event_label_surface_manifest.v1"
        ):
    ...
    if (
        ...
        raise ValueError("event-label surface is missing")
    return surface[0], surface[1], manifest[1]


def execute_event_label_job(
    *,
    ...
    if (
        ...
        raise ValueError("event-label binding is invalid")
    executable_id = subject["id"]
    plan = build_event_label_validation_plan(executable_id)
    ...
    if not required.issubset(inputs):
        raise ValueError("event-label inputs are missing")
    expected = set(spec["expected_outputs"])
    produces_surface = surface_output_name() in expected
    if produces_surface:
        surface = compute_registered_event_label_surface(root)
        surface_hash = writer.evidence.finalize(canonical_bytes(surface)).sha256
        manifest_value = {
            "schema": "event_label_surface_manifest.v1",
            "surface_artifact_hash": surface_hash,
            "surface_implementation_sha256": event_label_implementation_sha256(),
        }
    ...
    evaluation = project_event_label_evaluation(
        surface,
...
__all__ = [
    ...
    "build_event_label_validation_plan",
    "execute_event_label_job",
    "output_names",
