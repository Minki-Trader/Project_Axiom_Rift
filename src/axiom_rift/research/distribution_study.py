"""Writer-gated evidence Job for causal distribution asymmetry."""
...
from axiom_rift.research.distribution_discovery import compute_registered_distribution_surface,distribution_implementation_sha256,executable_configuration_map,loader_implementation_sha256,project_distribution_evaluation
...
MISSION_ID="MIS-0003";STUDY_ID="STU-0032";CALLABLE_IDENTITY="axiom_rift.research.distribution_study.execute_distribution_job.v1";EVIDENCE_DEPTH="discovery"
def build_distribution_validation_plan(eid:str)->dict[str,object]:return build_validation_plan(mission_id=MISSION_ID,executable_id=eid,evidence_depth=EVIDENCE_DEPTH,planned_claims=PLANNED_CLAIMS,evidence_modes=EVIDENCE_MODES,criteria=CRITERIA,candidate_eligible_on_pass=False)
...
def _load(w:StateWriter,hashes:tuple[str,...])->tuple[dict[str,Any],str,str]:
    ...
    for h in hashes:
        ...
        if isinstance(v,dict) and v.get("schema")=="distribution_asymmetry_surface.v1":s=(v,h)
        if isinstance(v,dict) and v.get("schema")=="distribution_surface_manifest.v1":m=(v,h)
    if s is None or m is None or m[0].get("surface_artifact_hash")!=s[1]:raise ValueError("surface missing")
    return s[0],s[1],m[1]
def execute_distribution_job(*,repository_root:str|Path,execution:RunningJobExecution)->DistributionJobPacket:
    ...
    eid=subject["id"];plan=build_distribution_validation_plan(eid);env=build_environment_manifest();ph=sha256(canonical_bytes(plan)).hexdigest();names=output_names(eid);inputs=tuple(spec["input_hashes"]);required={DATASET_SHA256,OBSERVED_MATERIAL_ID,ROLLING_SPLIT_SHA256,ph,distribution_implementation_sha256(),loader_implementation_sha256(),discovery_implementation_sha256()}
