"""Durable producer-bound family cache for the sleeve exposure-cap pair."""
...
SLEEVE_EXPOSURE_CAP_RISK_CACHE_PROVENANCE_SCHEMA = (
    "sleeve_exposure_cap_risk_cache_provenance.v1"
)
SLEEVE_EXPOSURE_CAP_RISK_CACHE_NAMESPACE = "sleeve-exposure-cap-risk-v1"
...
class SleeveExposureCapRiskCacheAuthority(Protocol):
    evidence: Any
...
def sleeve_exposure_cap_risk_cache_implementation_sha256() -> str:
    return sha256(
...
def sleeve_exposure_cap_risk_cache_output_name(
    ...
    return (
        ...
        f"{SLEEVE_EXPOSURE_CAP_RISK_CACHE_NAMESPACE}-{suffix}.json"
    )


def sleeve_exposure_cap_risk_cache_provenance_output_name(
    study_id: str,
) -> str:
    return (
        f"scientific/{_ascii('sleeve exposure-cap study_id', study_id)}/"
        f"{SLEEVE_EXPOSURE_CAP_RISK_CACHE_NAMESPACE}-family-cache-provenance.json"
    )


@dataclass(frozen=True, slots=True)
class SleeveExposureCapRiskFamilyCache:
    content: bytes
    produced: bool
    sha256: str
    ...
    def __post_init__(self) -> None:
        if type(self.content) is not bytes or type(self.produced) is not bool:
            raise ValueError("sleeve exposure-cap cache value is invalid")
        if _digest("sleeve exposure-cap cache hash", self.sha256) != sha256(
            self.content
        ).hexdigest():
            raise ValueError("sleeve exposure-cap cache content hash drifted")
    ...
    def trace(
        ...
        trace = self._trace
        if trace is None:
            trace = validate_sleeve_exposure_cap_risk_family_trace(
                self.content,
        ...
        if canonical_bytes(trace) != self.content:
            raise ValueError("sleeve exposure-cap cache trace differs from its bytes")
        return trace


def sleeve_exposure_cap_risk_family_cache(
    *,
    definition: ProspectivePairProtocolDefinition,
    family_trace: bytes | Mapping[str, Any],
    produced: bool,
) -> SleeveExposureCapRiskFamilyCache:
    trace = validate_sleeve_exposure_cap_risk_family_trace(
        family_trace,
        definition=definition,
    )
    content = canonical_bytes(trace)
    return SleeveExposureCapRiskFamilyCache(
        content=content,
        produced=produced,
...
def materialize_sleeve_exposure_cap_risk_cache(
    ...
    *,
    ...
    observed = publish_reproducible_cache(
        ...
        relative_path=sleeve_exposure_cap_risk_cache_output_name(definition),
        content=content,
    )
    if observed != sha256(content).hexdigest():
        raise ValueError("sleeve exposure-cap cache publication drifted")


def build_sleeve_exposure_cap_risk_cache_provenance(
    *,
    definition: ProspectivePairProtocolDefinition,
    mission_id: str,
    study_id: str,
    ...
    if producer_executable_id != definition.prospective_executable_ids[0]:
        raise ValueError("sleeve exposure-cap cache producer is not the first member")
    value = {
        "cache_output_name": sleeve_exposure_cap_risk_cache_output_name(definition),
        "cache_sha256": _digest(
            "sleeve exposure-cap cache hash",
            cache_sha256,
        ),
        "definition_identity": definition.identity,
        "family_id": definition.family_id,
        "mission_id": _ascii("sleeve exposure-cap mission_id", mission_id),
        ...
        "producer_trace_output_name": _ascii(
            "sleeve exposure-cap producer trace output",
            producer_trace_output_name,
        ),
        "producer_trace_sha256": _digest(
            "sleeve exposure-cap producer trace hash",
            producer_trace_sha256,
        ),
        "protocol_id": definition.protocol_id,
        "schema": SLEEVE_EXPOSURE_CAP_RISK_CACHE_PROVENANCE_SCHEMA,
        "study_id": _ascii("sleeve exposure-cap study_id", study_id),
    }
    return validate_sleeve_exposure_cap_risk_cache_provenance(
        value,
        definition=definition,
...
def validate_sleeve_exposure_cap_risk_cache_provenance(
    ...
    *,
    ...
    if not isinstance(value, Mapping) or set(value) != _PROVENANCE_FIELDS:
        raise ValueError("sleeve exposure-cap cache provenance schema is invalid")
    content = canonical_bytes(value)
    if len(content) > _MAX_PROVENANCE_BYTES:
        raise ValueError("sleeve exposure-cap cache provenance exceeds its bound")
    normalized = parse_canonical(content)
    if not isinstance(normalized, dict):
        raise ValueError("sleeve exposure-cap cache provenance is not an object")
    producer_id = definition.prospective_executable_ids[0]
    if (
        normalized.get("schema")
        != SLEEVE_EXPOSURE_CAP_RISK_CACHE_PROVENANCE_SCHEMA
        or normalized.get("cache_output_name")
        != sleeve_exposure_cap_risk_cache_output_name(definition)
        ...
        raise ValueError("sleeve exposure-cap cache provenance is out of scope")
    _digest(
        "sleeve exposure-cap cache hash",
    ...
    if not isinstance(producer, dict) or set(producer) != (
        ...
        raise ValueError("sleeve exposure-cap cache producer execution is invalid")
    ...
    if producer.get("identity") != execution.identity:
        raise ValueError("sleeve exposure-cap cache producer identity drifted")
    return normalized


def verify_sleeve_exposure_cap_risk_cache_producer(
    writer: SleeveExposureCapRiskCacheAuthority,
    *,
    repository_root: str | Path,
...
) -> tuple[SleeveExposureCapRiskFamilyCache, str, str, dict[str, object]]:
    if producer_executable_id != definition.prospective_executable_ids[0]:
        raise ValueError("sleeve exposure-cap cache producer identity is invalid")
    ...
    for input_hash in dict.fromkeys(inputs):
        ...
        if (
            ...
            matches.append(
                (
                    input_hash,
                    validate_sleeve_exposure_cap_risk_cache_provenance(
                        value,
                        definition=definition,
                        mission_id=mission_id,
                        study_id=study_id,
    ...
    if len(matches) != 1:
        raise ValueError(
            "sleeve exposure-cap consumer requires one cache provenance input"
        )
    ...
    for name, value in (
        ...
        if inputs.count(value) != 1:
            raise ValueError(
                f"sleeve exposure-cap {name} must be exactly one Job input"
            )
    ...
    if cache_content is None or producer_trace_content is None:
        raise ValueError("sleeve exposure-cap producer evidence is unavailable")
    ...
    if cache.sha256 != cache_hash:
        raise ValueError("sleeve exposure-cap cache hash differs from its bytes")
    ...
    if not isinstance(producer_trace, Mapping):
        try:
            ...
            raise ValueError(
                "sleeve exposure-cap producer trace is not canonical"
            ) from exc
    if not isinstance(producer_trace, Mapping):
        raise ValueError("sleeve exposure-cap producer trace is not an object")
    family = extract_sleeve_exposure_cap_risk_family_trace(
        producer_trace,
        definition=definition,
    )
    if canonical_bytes(family) != cache.content:
        raise ValueError("sleeve exposure-cap producer trace differs from its cache")
    ...
    if (
        ...
        raise ValueError("sleeve exposure-cap producer trace execution drifted")
    writer.verify_reproducible_cache_producer(
        execution,
        cache_output_name=sleeve_exposure_cap_risk_cache_output_name(definition),
        cache_hash=cache.sha256,
        expected_callable_identity=_ascii(
            "sleeve exposure-cap callable identity",
        ...
        manifest_output_name=(
            sleeve_exposure_cap_risk_cache_provenance_output_name(study_id)
        ),
...
__all__ = [
    "SLEEVE_EXPOSURE_CAP_RISK_CACHE_NAMESPACE",
    "SLEEVE_EXPOSURE_CAP_RISK_CACHE_PROVENANCE_SCHEMA",
    "SleeveExposureCapRiskFamilyCache",
