"""Follow-up runner for the registered reversion regime/session Study."""
...
from axiom_rift.research.reversion_regime_followup_discovery import (
    DATASET_SHA256,
    OBSERVED_MATERIAL_ID,
    ROLLING_SPLIT_SHA256,
    SELECTION_TOTAL_EXPOSURES,
    _compute_registered_reversion_regime_followup_surface,
    reversion_regime_followup_implementation_sha256,
    reversion_regime_followup_executable_configuration_map,
    loader_implementation_sha256,
    project_reversion_regime_followup_evaluation,
...
MISSION_ID = "MIS-0001"
STUDY_ID = "STU-0014"
CALLABLE_IDENTITY = "axiom_rift.research.reversion_regime_followup_study.execute_reversion_regime_followup_job.v1"
SURFACE_SCHEMA = "reversion_regime_followup_surface.v1"
EVALUATION_SCHEMA = "reversion_regime_followup_evaluation.v1"
...
def build_reversion_regime_followup_validation_plan(executable_id: str) -> dict[str, object]:
...
def _claim_metrics(evaluation: Mapping[str, Any]) -> dict[str, dict[str, int | None]]:
    ...
    if not isinstance(raw, Mapping):
        raise ValueError("reversion_regime_followup evaluation has no metrics")
    ...
    if any(type(name) is not str or type(value) is not int for name, value in metrics.items()):
        raise ValueError("reversion_regime_followup evaluation metrics must be integer scalars")
    ...
    def values(*names: str, null_when_not_evaluable: bool = False) -> dict[str, int | None]:
        ...
        for name in names:
            if name not in metrics:
                raise ValueError(f"reversion_regime_followup evaluation metric is absent: {name}")
            result[name] = (
...
def surface_cache_output_name() -> str:
    return (
        f"local/cache/{STUDY_ID}/reversion_regime_followup-surface-"
        f"{reversion_regime_followup_implementation_sha256()[:16]}.json"
    )


def surface_manifest_output_name() -> str:
    return f"scientific/{STUDY_ID}/reversion_regime_followup-surface-cache-manifest.json"
...
def _registered_executable_ids() -> tuple[str, ...]:
    mapping = reversion_regime_followup_executable_configuration_map()
    if not isinstance(mapping, Mapping) or len(mapping) != 12:
        raise ValueError("reversion_regime_followup Study requires exactly twelve Executables")
    ...
    if any(
        ...
        raise ValueError("reversion_regime_followup Study Executable identities are invalid")
    return identities


def build_environment_manifest() -> dict[str, object]:
    if SELECTION_TOTAL_EXPOSURES != 186:
        raise ValueError("reversion_regime_followup Study requires global exposure total 186")
    value: dict[str, object] = {
        "dataset_sha256": DATASET_SHA256,
        "reversion_regime_followup_implementation_sha256": reversion_regime_followup_implementation_sha256(),
...
class ReversionRegimeFollowupJobPacket:
    ...
    def artifact(self, name: str) -> dict[str, Any]:
        ...
        if not isinstance(value, dict):
            raise ValueError("reversion_regime_followup Job artifact is not an object")
        return value
...
def _materialize_cache(
    ...
    *,
    ...
    if cache_root not in target.parents:
        raise ValueError("reversion_regime_followup surface cache path escapes local/cache")
    expected_hash = sha256(content).hexdigest()
    if target.exists():
        if not target.is_file() or sha256(target.read_bytes()).hexdigest() != (
            expected_hash
        ):
            raise ValueError("existing reversion_regime_followup surface cache has different bytes")
        return
    target.parent.mkdir(parents=True, exist_ok=True)
    descriptor, temporary_name = tempfile.mkstemp(
        prefix=".reversion_regime_followup-surface-",
        suffix=".tmp",
        dir=target.parent,
    )
...
def _build_surface_manifest(
    *,
    ...
    value: dict[str, Any] = {
        ...
        "schema": "reversion_regime_followup_surface_cache_manifest.v1",
        "selection_total_exposures": SELECTION_TOTAL_EXPOSURES,
        "split_artifact_sha256": ROLLING_SPLIT_SHA256,
        "trend_dependency_sha256": trend_dependency_sha256(),
    }
...
def _validate_surface_manifest(
    ...
    *,
    ...
    if (
        ...
        or value.get("schema") != "reversion_regime_followup_surface_cache_manifest.v1"
        ...
        or value.get("reversion_regime_followup_implementation_sha256")
        != reversion_regime_followup_implementation_sha256()
        ...
        or value.get("selection_total_exposures") != 186
        or value.get("split_artifact_sha256") != ROLLING_SPLIT_SHA256
        or value.get("trend_dependency_sha256") != trend_dependency_sha256()
    ):
        raise ValueError("reversion_regime_followup surface cache manifest differs from Job inputs")
    ...
    if not isinstance(producer, dict) or set(producer) != {
        ...
        raise ValueError("reversion_regime_followup surface producer execution is invalid")
    ...
    if producer["identity"] != execution.identity:
        raise ValueError("reversion_regime_followup surface producer identity is invalid")
    return value
    ...
    *,
    ...
    for input_hash in input_hashes:
        ...
        if isinstance(value, dict) and value.get("schema") == (
            "reversion_regime_followup_surface_cache_manifest.v1"
        ):
            matches.append(
                (
                    input_hash,
    ...
    if len(matches) != 1:
        raise ValueError("Job inputs require one exact reversion_regime_followup surface manifest")
    return matches[0]


def execute_reversion_regime_followup_job(
    *,
    repository_root: str | Path,
    execution: RunningJobExecution,
) -> ReversionRegimeFollowupJobPacket:
    ...
    if (
        ...
        raise ValueError("running Job is not bound to the registered reversion_regime_followup Study")
    executable_id = evidence_subject["id"]
    scientific_binding = spec.get("scientific_binding")
    if not isinstance(scientific_binding, dict):
        raise ValueError("reversion_regime_followup Job lacks a scientific binding")
    plan = build_reversion_regime_followup_validation_plan(executable_id)
    ...
    if not required_inputs.issubset(inputs):
        raise ValueError("reversion_regime_followup Job omits a required content-bound input")
    if scientific_binding.get("validation_plan_hash") != plan_hash:
        raise ValueError("reversion_regime_followup validation plan differs from Job input")
    ...
    if (
        ...
        raise ValueError("reversion_regime_followup scientific binding differs from preregistration")
    ...
    if not isinstance(output_classes, dict):
        raise ValueError("reversion_regime_followup Job output classes are invalid")
    ...
    if expected_outputs != expected_set or output_classes != expected_classes:
        raise ValueError("reversion_regime_followup Job outputs differ from the producer/projector protocol")

    manifest_artifact_hash: str
    manifest_bytes: bytes | None = None
    if produces_surface:
        surface = _validated_surface(
            _compute_registered_reversion_regime_followup_surface(root)
        )
        ...
        if not cache_target.is_file():
            raise ValueError("reversion_regime_followup surface cache is unavailable")
        surface_bytes = cache_target.read_bytes()
        surface_hash = sha256(surface_bytes).hexdigest()
        if surface_hash not in inputs:
            raise ValueError("reversion_regime_followup surface cache hash is not a Job input")
    ...
    context = project_reversion_regime_followup_evaluation(
        surface,
    ...
    if set(output_manifest) != expected_outputs:
        raise ValueError("materialized reversion_regime_followup outputs differ from Job declaration")
    return ReversionRegimeFollowupJobPacket(
...
__all__ = [
    ...
    "build_reversion_regime_followup_validation_plan",
    "output_names",
    "planned_verdict",
    "execute_reversion_regime_followup_job",
