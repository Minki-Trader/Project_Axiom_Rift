...
from axiom_rift.research.dense_short_synthesis_chassis import SELECTION_TOTAL_EXPOSURES,calibrate_synthesis_selector,dense_short_synthesis_chassis_implementation_sha256,dense_short_synthesis_configurations,dense_short_synthesis_executable,executable_configuration_map,loader_implementation_sha256,simulate_dense_short_synthesis,terminal_return_sign_12
...
def dense_short_synthesis_discovery_implementation_sha256()->str:return sha256(_THIS_FILE.read_bytes()).hexdigest()
def _matched(results:list[Any],profile:str)->Any:
    found=[v for v in results if v.configuration.profile==profile]
    if len(found)!=1:raise DiscoveryBoundaryError("dense synthesis control is not unique")
    return found[0]
def _populate_controls(results:list[Any])->None:
    control=_matched(results,"sparse_volatility_clock_48_control")
    for subject in results:
        subject.metrics["synthesis_control_delta_net_profit_micropoints"]=subject.metrics["net_profit_micropoints"]-control.metrics["net_profit_micropoints"]
        subject.metrics["synthesis_control_pvalue_upper_ppm"]=1_000_000 if subject is control else _paired_control_pvalue(subject,control,role="matched_sparse48_control",total_exposures=SELECTION_TOTAL_EXPOSURES)
def compute_registered_dense_short_synthesis_surface(repository_root:str|Path)->dict[str,Any]:
    ...
    results=[]
    for configuration in dense_short_synthesis_configurations():
        ...
        results.append(_evaluate_configuration(calibrations=calibrations,frame=frame,features=first,fold_features=fold_scores,folds=folds,configuration=configuration,effective_spread=spread,prefix_features=prefix_scores,prefix_spreads=prefix_spreads,time=time,executable_id=dense_short_synthesis_executable(configuration).identity,simulation_fn=simulate_dense_short_synthesis))
    ...
    surface={"claim_limits":_claim_limits()+["typed_synthesis_changes_label_selector_and_lifecycle","feature_model_trade_risk_and_execution_are_fixed","dense_subject_targets_at_least_one_entry_per_eligible_day","two_trial_surface"],"dataset_sha256":DATASET_SHA256,"dense_short_synthesis_chassis_implementation_sha256":dense_short_synthesis_chassis_implementation_sha256(),"dense_short_synthesis_discovery_implementation_sha256":dense_short_synthesis_discovery_implementation_sha256(),"engine_environment":{"numpy":np.__version__,"pandas":pd.__version__,"python":".".join(str(v) for v in sys.version_info[:3]),"scipy":scipy.__version__},"evaluations":[{"direction_metrics":r.direction_metrics,"evaluable":all(r.metrics[n]==0 for n in ("unknown_cost_unresolved_signal_count","causality_violation_count","nonfinite_metric_count","prefix_invariance_mismatch_count","append_invariance_mismatch_count")),"fold_metrics":r.fold_metrics,"metrics":dict(sorted(r.metrics.items())),"regime_metrics":r.regime_metrics,"session_metrics":r.session_metrics,"subject_configuration_id":r.configuration.configuration_id,"subject_executable_id":r.executable_id} for r in results],"loader_implementation_sha256":loader_implementation_sha256(),"material_identity":OBSERVED_MATERIAL_ID,"schema":"dense_short_synthesis_surface.v1","selection_context":[{"configuration_id":r.configuration.configuration_id,"executable_id":r.executable_id,"net_profit_micropoints":r.metrics["net_profit_micropoints"],"selection_aware_pvalue_ppm":r.metrics["selection_aware_pvalue_ppm"]} for r in results],"selection_method":_selection_method(SELECTION_TOTAL_EXPOSURES),"session_semantics":"broker_clock_fixed_bins_no_dst_or_cash_session_claim","split_artifact_sha256":ROLLING_SPLIT_SHA256};canonical_bytes(surface);return surface
def project_dense_short_synthesis_evaluation(surface:Mapping[str,Any],*,job_execution:Mapping[str,str],subject_executable_id:str,surface_artifact_hash:str,surface_manifest_hash:str)->dict[str,Any]:
    value=dict(surface)
    if sha256(canonical_bytes(value)).hexdigest()!=surface_artifact_hash or value.get("schema")!="dense_short_synthesis_surface.v1":raise DiscoveryBoundaryError("dense synthesis surface invalid")
    expected=executable_configuration_map();by={v.get("subject_executable_id"):v for v in value["evaluations"]}
    if set(by)!=set(expected) or subject_executable_id not in expected:raise DiscoveryBoundaryError("dense synthesis subjects differ")
    payload={n:job_execution[n] for n in ("job_hash","job_id","job_permit_id","start_record_id")}
    if job_execution.get("identity")!=canonical_digest(domain="running-job-execution",payload=payload):raise DiscoveryBoundaryError("dense synthesis Job invalid")
    result={**dict(by[subject_executable_id]),"claim_limits":value["claim_limits"],"job_execution":dict(job_execution),"schema":"dense_short_synthesis_evaluation.v1","selection_context":value["selection_context"],"selection_method":value["selection_method"],"session_semantics":value["session_semantics"],"surface_artifact_hash":surface_artifact_hash,"surface_manifest_hash":surface_manifest_hash};canonical_bytes(result);return result
__all__=["compute_registered_dense_short_synthesis_surface","dense_short_synthesis_discovery_implementation_sha256","project_dense_short_synthesis_evaluation"]
