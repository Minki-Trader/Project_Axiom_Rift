...
from axiom_rift.research.high_vol_target_reversal_discovery import compute_registered_high_vol_target_reversal_surface, project_high_vol_target_reversal_evaluation, high_vol_target_reversal_discovery_implementation_sha256
...
CALLABLE_IDENTITY = "axiom_rift.research.high_vol_target_reversal_study.execute_high_vol_target_reversal_job.v1"
...
def build_high_vol_target_reversal_validation_plan(executable_id: str, *, mission_id: str) -> dict[str, object]:
...
def surface_output_name(*, study_id: str) -> str:
    return f"scientific/{study_id}/high-vol-target-reversal-surface.json"


def surface_manifest_output_name(*, study_id: str) -> str:
    return f"scientific/{study_id}/high-vol-target-reversal-surface-manifest.json"
...
class HighVolTargetReversalJobPacket:
...
def execute_high_vol_target_reversal_job(*, repository_root: str | Path, execution: RunningJobExecution) -> HighVolTargetReversalJobPacket:
    ...
    if not isinstance(mission_id, str) or not isinstance(study_id, str) or not isinstance(subject, dict) or subject.get("id") not in executable_configuration_map():
        raise ValueError("high volatility target reversal binding invalid")
    executable_id = subject["id"]
    plan = build_high_vol_target_reversal_validation_plan(executable_id, mission_id=mission_id)
    ...
    required = {DATASET_SHA256, OBSERVED_MATERIAL_ID, ROLLING_SPLIT_SHA256, plan_hash, high_vol_target_reversal_chassis_implementation_sha256(), high_vol_target_reversal_discovery_implementation_sha256(), loader_implementation_sha256(), discovery_implementation_sha256()}
    if not required.issubset(inputs):
        raise ValueError("high volatility target reversal inputs missing")
    produces = surface_output_name(study_id=study_id) in set(spec["expected_outputs"])
    if produces:
        surface = compute_registered_high_vol_target_reversal_surface(root)
    ...
    return HighVolTargetReversalJobPacket(tuple(sorted(outputs.items())), planned_verdict(plan, measurement))
