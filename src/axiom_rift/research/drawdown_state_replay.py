...
def _drawdown_causal_surface_digest(
    ...
    """Hash every causal input consumed by the drawdown replay simulation."""

    if type(surfaces) is not tuple or not surfaces:
        raise ValueError("drawdown causal replay surfaces are absent")
    digest = sha256()
    digest.update(b"drawdown-causal-input-surfaces.v1\0")
    ...
    for name, values in surfaces:
        if (
            ...
            raise ValueError("drawdown causal replay surface name is invalid")
        names.add(name)
        encoded_name = name.encode("ascii")
        array = np.asarray(values, dtype="<f8").copy(order="C")
        if array.ndim == 0:
            raise ValueError("drawdown causal replay surface is not an array")
            ...
            comparisons.append(
                {
                    ...
                    "full_feature_values_sha256": (
                        _drawdown_causal_surface_digest(full_causal_surfaces)
                    ),
                    "invariance_key": profile,
                    "prefix_feature_values_sha256": (
                        _drawdown_causal_surface_digest(prefix_causal_surfaces)
                    ),
                }
            )
    comparisons.sort(
