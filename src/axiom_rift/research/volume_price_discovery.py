"""Causal volume-price pressure discovery on Foundation-safe US100 M5 data.
...
_PROFILE_SPECS: dict[str, dict[str, int | str]] = {
    ...
    "range_direction_48_192": {
        "measure": "volume_weighted_range_direction_strength",
        "recent_window_bars": 48,
        "baseline_window_bars": 192,
    },
}
_VOLUME_PRICE_FILE = Path(__file__).resolve()


class VolumePriceBoundaryError(DiscoveryBoundaryError):
...
def volume_price_implementation_sha256() -> str:
    return _file_sha256(_VOLUME_PRICE_FILE)
...
class VolumePriceConfiguration:
    profile: str
    signal_sign: int
    holding_bars: int
...
def volume_price_configurations() -> tuple[VolumePriceConfiguration, ...]:
    return tuple(
        VolumePriceConfiguration(profile, signal_sign, holding_bars)
        for profile in (
            "body_pressure_12_96",
            "close_location_24_120",
            "range_direction_48_192",
...
def _local_implementation(function_name: str) -> str:
    return (
        f"axiom_rift.research.volume_price_discovery.{function_name}@sha256:"
...
def volume_price_components() -> tuple[ComponentSpec, ...]:
    return (
        ComponentSpec(
            display_name="causal completed-bar volume-price pressure score",
            protocol="feature.completed_bar_tick_volume_price_pressure.v1",
        ...
        ComponentSpec(
            display_name="fold isolated sparse volume-price pressure selector",
        ...
        ComponentSpec(
            ...
            protocol="trade.completed_bar_next_open_volume_price_pressure.v1",
...
def volume_price_executable(configuration: VolumePriceConfiguration) -> ExecutableSpec:
    return ExecutableSpec(
        display_name=f"volume-price pressure contrast {configuration.configuration_id}",
        components=volume_price_components(),
        ...
        engine_contract=(
            "engine:volume_price_discovery_v1:python3_13_9:numpy2_3_4:"
            "pandas2_3_3:scipy1_16_3:"
            f"volume_price_sha256_{volume_price_implementation_sha256()}:"
...
def volume_price_executable_configuration_map() -> dict[str, VolumePriceConfiguration]:
    return {
        volume_price_executable(configuration).identity: configuration
        for configuration in volume_price_configurations()
    }


def executable_configuration_map() -> dict[str, VolumePriceConfiguration]:
    return volume_price_executable_configuration_map()


def compute_volume_price_pressure_score(
    frame: pd.DataFrame,
    profile: str,
    ...
    if measure == "volume_weighted_candle_body_fraction":
        ...
        raise VolumePriceBoundaryError("volume-price measure is not registered")
...
class _ConfigurationResult:
    configuration: VolumePriceConfiguration
    executable_id: str
...
def _evaluate_configuration(
    *,
    ...
    score, volatility, run = compute_volume_price_pressure_score(
    ...
    for fold in folds:
        ...
        if len(train_volatility) < 1000:
            raise VolumePriceBoundaryError("regime calibration is too small")
        cutoffs = (
        ...
        prefix_score, prefix_volatility, prefix_run = (
            compute_volume_price_pressure_score(prefix_frame, configuration.profile)
        )
    ...
    return _ConfigurationResult(
        configuration,
        volume_price_executable(configuration).identity,
        metrics,
        fold_metrics,
        regime_metrics,
        session_metrics,
...
def _adjusted_bootstrap_upper_pvalue(values: np.ndarray, *, seed_label: str) -> int:
    ...
    if len(sample) < 30 or np.any(~np.isfinite(sample)):
        raise VolumePriceBoundaryError("bootstrap series is invalid or too short")
...
def _matched_result(
    ...
    *,
    ...
    if len(matches) != 1:
        raise VolumePriceBoundaryError("registered control match is not unique")
    return matches[0]
...
def _paired_pvalue(subject: _ConfigurationResult, control: _ConfigurationResult, role: str) -> int:
    if not subject.daily_pnl.index.equals(control.daily_pnl.index):
        raise VolumePriceBoundaryError("paired controls have different eligible days")
...
def _populate_pvalues_and_controls(results: Sequence[_ConfigurationResult]) -> None:
    for subject in results:
        ...
        if any(type(value) is not int for value in subject.metrics.values()):
            raise VolumePriceBoundaryError("scientific metrics are not fixed-point integers")
...
def _claim_limits() -> list[str]:
    return [
        ...
        "monthly_drawdown_share_gate_is_for_sparse_volume_price_surfaces_only",
...
def _compute_registered_volume_price_surface(repository_root: str | Path) -> dict[str, Any]:
    if not isinstance(repository_root, (str, Path)):
        raise VolumePriceBoundaryError(
    ...
    surface: dict[str, Any] = {
        ...
        "discovery_implementation_sha256": volume_price_implementation_sha256(),
        "engine_environment": {
        ...
        "schema": "volume_price_discovery_surface.v1",
        "selection_context": [
            {
...
def project_volume_price_evaluation(
    ...
    *,
    ...
    for name, digest in (
        ...
        if type(digest) is not str or len(digest) != 64 or any(c not in "0123456789abcdef" for c in digest):
            raise VolumePriceBoundaryError(f"{name} hash is invalid")
    if not isinstance(job_execution, Mapping) or set(job_execution) != {
        "identity", "job_hash", "job_id", "job_permit_id", "start_record_id"
    }:
        raise VolumePriceBoundaryError("Job execution binding is invalid")
    ...
    if job_execution["identity"] != canonical_digest(domain="running-job-execution", payload=payload):
        raise VolumePriceBoundaryError("Job execution identity is invalid")
    value = dict(surface)
    if sha256(canonical_bytes(value)).hexdigest() != surface_artifact_hash:
        raise VolumePriceBoundaryError("surface bytes differ from their artifact hash")
    expected_fields = {
    ...
    if set(value) != expected_fields or value.get("schema") != "volume_price_discovery_surface.v1":
        raise VolumePriceBoundaryError("volume-price surface schema is invalid")
    expected = volume_price_executable_configuration_map()
    ...
    evaluation = {
        **dict(by_identity[subject_executable_id]),
        ...
        "schema": "volume_price_discovery_evaluation.v1",
...
__all__ = [
    ...
    "VolumePriceBoundaryError", "VolumePriceConfiguration", "SELECTOR_QUANTILE_BP",
    "SELECTION_BLOCK_LENGTHS", "SELECTION_BOOTSTRAP_SAMPLES",
    "SELECTION_MONTE_CARLO_CONFIDENCE_PPM", "SELECTION_SEED",
    "SELECTION_TOTAL_EXPOSURES", "_compute_registered_volume_price_surface",
    "calibrate_selector", "compute_volume_price_pressure_score",
    "executable_configuration_map",
    "loader_implementation_sha256", "project_volume_price_evaluation",
    "volume_price_components", "volume_price_configurations", "volume_price_executable",
    "volume_price_executable_configuration_map", "volume_price_implementation_sha256",
