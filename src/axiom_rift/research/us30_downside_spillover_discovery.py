"""Causal US30 downside-spillover discovery on Foundation-safe US100 M5 data.
...
from axiom_rift.research import us30_source as us30_source_module
...
class US30DownsideSpilloverBoundaryError(DiscoveryBoundaryError):
...
def _require_digest(value: str, name: str) -> str:
    if (
        ...
        raise US30DownsideSpilloverBoundaryError(
            f"{name} must be a lowercase SHA256 digest"
        )
    return value


def us30_downside_spillover_implementation_sha256() -> str:
...
def us30_source_implementation_sha256() -> str:
    return _file_sha256(Path(us30_source_module.__file__).resolve())


def _raw_path(repository_root: str | Path) -> Path:
    root = Path(repository_root).resolve()
    target = (root / US30_RAW_RELATIVE_PATH).resolve()
    if root not in target.parents or not target.is_file():
        raise US30DownsideSpilloverBoundaryError(
            "US30 raw snapshot is absent or outside the repository"
        )
    return target


def us30_raw_sha256(repository_root: str | Path) -> str:
    return _file_sha256(_raw_path(repository_root))


@dataclass(frozen=True, slots=True)
class US30DevelopmentMetadata:
    raw_sha256: str
...
@dataclass(frozen=True, slots=True)
class US30ObservedDevelopment:
    frame: pd.DataFrame
    metadata: US30DevelopmentMetadata
...
def _record_body(raw_line: bytes, name: str) -> bytes:
    ...
    if not body:
        raise US30DownsideSpilloverBoundaryError(f"{name} contains an empty record")
    return body
...
def _timestamp_bytes(record: bytes) -> bytes:
    ...
    if not separator or len(stamp) != 19:
        raise US30DownsideSpilloverBoundaryError(
            "US30 source timestamp field boundary is invalid"
        )
    ...
    if any(stamp[index] != expected for index, expected in punctuation.items()) or any(
        ...
        raise US30DownsideSpilloverBoundaryError(
            "US30 source timestamp is invalid"
        )
    minute = 10 * (stamp[14] - 48) + stamp[15] - 48
    if stamp[17:19] != b"00" or minute >= 60 or minute % 5 != 0:
        raise US30DownsideSpilloverBoundaryError(
            "US30 source timestamp is off the M5 grid"
        )
    return stamp


def _scan_us30_source(path: Path, *, expected_raw_sha256: str) -> _SourceScan:
    expected = _require_digest(expected_raw_sha256, "expected US30 raw hash")
    expected_header = b",".join(field.encode("ascii") for field in US30_COLUMNS)
    ...
    try:
        with path.open("rb") as handle:
            ...
            if not header or _record_body(header, "US30 header") != expected_header:
                raise US30DownsideSpilloverBoundaryError(
                    "US30 raw schema or field order differs"
                )
            ...
            for raw_line in handle:
                ...
                record = _record_body(raw_line, "US30 source")
                stamp = _timestamp_bytes(record)
                if previous is not None and stamp <= previous:
                    raise US30DownsideSpilloverBoundaryError(
                        "US30 source timestamps are not strictly increasing"
                    )
                previous = stamp
                if stamp <= boundary:
                    if saw_later:
                        raise US30DownsideSpilloverBoundaryError(
                            "US30 development row follows the quarantined tail"
                        )
        ...
        prefix.close()
        raise US30DownsideSpilloverBoundaryError(
            "US30 raw snapshot cannot be scanned"
        ) from exc
    observed = full_hash.hexdigest()
    if observed != expected:
        prefix.close()
        raise US30DownsideSpilloverBoundaryError("US30 raw SHA256 changed")
    if copied_rows == 0 or not saw_later:
        prefix.close()
        raise US30DownsideSpilloverBoundaryError(
            "US30 source does not expose the exact development/tail boundary"
        )
    prefix.seek(0)
    return _SourceScan(
        parser_input=prefix,
...
def load_us30_observed_development(
    repository_root: str | Path,
    *,
    expected_raw_sha256: str | None = None,
) -> US30ObservedDevelopment:
    """Parse only US30 rows at or before the registered development boundary."""

    path = _raw_path(repository_root)
    expected = us30_raw_sha256(repository_root) if expected_raw_sha256 is None else expected_raw_sha256
    scan = _scan_us30_source(path, expected_raw_sha256=expected)
    try:
        frame = pd.read_csv(
            scan.parser_input,
            usecols=["time", "close"],
    ...
    if (
        ...
        raise US30DownsideSpilloverBoundaryError(
            "US30 development prefix violates its data boundary"
        )
    return US30ObservedDevelopment(
        frame=frame,
        metadata=US30DevelopmentMetadata(
...
@dataclass(frozen=True, slots=True)
class US30DownsideSpilloverConfiguration:
    profile: str
    route_sign: int
    holding_bars: int
    ...
    def semantic_parameters(self) -> dict[str, Any]:
        return {
            ...
            "long_realized_volatility_bars": 96,
            "return_horizon_bars": 6,
            "route_sign": self.route_sign,
            "score_profile": self.profile,
            "selector_quantile_bp": SELECTOR_QUANTILE_BP,
            "short_realized_volatility_bars": 12,
            "volatility_expansion_floor_ppm": 1_000_000,
        }


def us30_downside_spillover_configurations() -> tuple[US30DownsideSpilloverConfiguration, ...]:
    return tuple(
        US30DownsideSpilloverConfiguration(profile, route_sign, holding_bars)
...
def _local_implementation(function_name: str) -> str:
    return (
        "axiom_rift.research.us30_downside_spillover_discovery."
        f"{function_name}@sha256:{us30_downside_spillover_implementation_sha256()}"
    )
...
def us30_downside_spillover_components(raw_sha256: str) -> tuple[ComponentSpec, ...]:
    raw = _require_digest(raw_sha256, "US30 raw hash")
    source = _source_identity_payload()
    source_id = source["source_contract_id"]
    feature = ComponentSpec(
            display_name="exact causal US30 downside-volatility spillover",
            protocol=(
                "feature.cross_asset_downside_volatility_"
                "r6_rv12_rv96_z6.v1"
            ),
            implementation=_local_implementation(
                "compute_downside_spillover_features"
            ),
            ...
            spec={
                ...
                "expansion": (
                    "max(rv12_div_rv96_scaled_sqrt_12_div_96_minus_1,0)"
                ),
                "join": "exact_timestamp_inner_no_fill_no_asof_no_zero",
                "nonconsecutive_action": (
                    "source_or_target_gap_reset_and_rewarm_97_bars"
                ),
                "profiles": list(_PROFILES),
                "r6": "log_close_t_minus_log_close_t_minus_6",
                "raw_sha256": raw,
                "rv12": "root_sum_squared_one_bar_log_returns_last_12",
                "rv96": "root_sum_squared_one_bar_log_returns_last_96",
                "score_source_downside_expansion": (
                    "min(source_z6,0)_times_source_expansion"
                ),
                "score_source_symmetric_expansion": (
                    "source_z6_times_source_expansion"
                ),
                "score_target_only_downside": (
                    "min(target_z6,0)_times_target_expansion"
                ),
                "source_gap_action": (
                    "block_new_entries_until_97_consecutive_joint_bars"
                ),
                "source_identities": source,
                "z6": "r6_div_rv96_scaled_sqrt_6_div_96",
            },
        )
    label = ComponentSpec(
            display_name="US100 fixed-horizon terminal return direction",
            protocol="label.us100_terminal_return_sign_variable_hold.v1",
            implementation=_local_implementation("simulate_cross_asset_fixed_hold"),
            semantic_dependencies=(feature.identity,),
            spec={
                "entry_surface": "next_exact_US100_bar_open",
                "exit_surface": "original_US100_entry_index_plus_holding_bars_open",
                "parameter_fields": ["holding_bars"],
                "train_future_must_be_inside_fold": True,
            },
        )
    selector = ComponentSpec(
            display_name="fold isolated absolute 95th percentile selector",
    ...
    trade = ComponentSpec(
            ...
            spec={
                ...
                "source_gap_blocks_new_entry": True,
    ...
    lifecycle = ComponentSpec(
            ...
            spec={
                ...
                "existing_trade_ignores_later_source_gap": True,
    ...
    return feature, label, selector, trade, lifecycle, risk, execution


def us30_downside_spillover_executable(
    configuration: US30DownsideSpilloverConfiguration,
    raw_sha256: str,
) -> ExecutableSpec:
    if not isinstance(configuration, US30DownsideSpilloverConfiguration):
        raise TypeError("configuration must be US30DownsideSpilloverConfiguration")
    raw = _require_digest(raw_sha256, "US30 raw hash")
    source = _source_identity_payload()
    return ExecutableSpec(
        display_name=f"cross asset downside spillover {configuration.configuration_id}",
        components=us30_downside_spillover_components(raw),
        parameters={
        ...
        clock_contract=(
            "clock:US100_and_US30_completed_M5_event_time_plus_5m_"
        ...
        engine_contract=(
            "engine:us30_downside_spillover_discovery_v1:python3_13_9:"
            "numpy2_3_4:pandas2_3_3:scipy1_16_3:"
            f"implementation_{us30_downside_spillover_implementation_sha256()}:"
            f"trend_{trend_dependency_sha256()}:loader_{loader_implementation_sha256()}:"
            f"source_module_{us30_source_implementation_sha256()}:raw_{raw}:"
...
def _configuration_map_for_raw(raw_sha256: str) -> dict[str, US30DownsideSpilloverConfiguration]:
    return {
        us30_downside_spillover_executable(configuration, raw_sha256).identity: configuration
        for configuration in us30_downside_spillover_configurations()
    }


def us30_downside_spillover_executable_configuration_map(
    repository_root: str | Path,
) -> dict[str, US30DownsideSpilloverConfiguration]:
    return _configuration_map_for_raw(us30_raw_sha256(repository_root))


def executable_configuration_map(
    repository_root: str | Path,
) -> dict[str, US30DownsideSpilloverConfiguration]:
    return us30_downside_spillover_executable_configuration_map(repository_root)


@dataclass(frozen=True, slots=True)
class DownsideSpilloverFeatures:
    source_downside_expansion: np.ndarray
    source_symmetric_expansion: np.ndarray
    target_only_downside: np.ndarray
    source_r6: np.ndarray
    source_rv12: np.ndarray
    source_rv96: np.ndarray
    source_z6: np.ndarray
    target_r6: np.ndarray
    target_rv12: np.ndarray
    target_rv96: np.ndarray
    target_z6: np.ndarray
...
def _join_exact(target_frame: pd.DataFrame, source_frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if not required_target.issubset(target_frame.columns) or set(source_frame.columns) != {"time", "close"}:
        raise US30DownsideSpilloverBoundaryError("cross-asset frame schema is invalid")
    ...
    source = source_frame.rename(columns={"close": "us30_close"}).copy()
    for frame, name in ((target, "US100"), (source, "US30")):
        frame["time"] = pd.to_datetime(frame["time"], errors="raise")
        if frame["time"].duplicated().any() or not frame["time"].is_monotonic_increasing:
            raise US30DownsideSpilloverBoundaryError(f"{name} timestamps are invalid")
    ...
    if joined.empty or joined["time"].duplicated().any() or not joined["time"].is_monotonic_increasing:
        raise US30DownsideSpilloverBoundaryError("exact cross-asset join is empty or invalid")
    return joined.reset_index(drop=True)


def compute_downside_spillover_features(
    joined_frame: pd.DataFrame,
    target_frame: pd.DataFrame | None = None,
) -> DownsideSpilloverFeatures:
    required = {"time", "us100_close", "us30_close", "target_index"}
    if not required.issubset(joined_frame.columns):
        raise ValueError("joined cross-asset frame schema is invalid")
    source_run = _consecutive_run(_time_ns(joined_frame))
    ...
    def feature_set(
        ...
        z6_denominator = rv96 * sqrt(6 / 96)
        z6 = np.divide(
            r6,
            z6_denominator,
            out=np.full(len(close), np.nan),
            where=np.isfinite(z6_denominator) & (z6_denominator > 0),
        )
        expansion_denominator = rv96 * sqrt(12 / 96)
        expansion_ratio = np.divide(
            rv12,
            expansion_denominator,
            out=np.full(len(close), np.nan),
            where=(
                np.isfinite(expansion_denominator)
                & (expansion_denominator > 0)
            ),
        )
        expansion = np.maximum(expansion_ratio - 1.0, 0.0)
        for feature in (r6, rv12, rv96, z6, expansion):
            feature[run < 97] = np.nan
        return r6, rv12, rv96, z6, expansion

    source_close = pd.to_numeric(
        joined_frame["us30_close"], errors="raise"
    ).to_numpy(dtype=float)
    source_r6, source_rv12, source_rv96, source_z6, source_expansion = (
        feature_set(source_close, source_run, "us30")
    )
    if target_frame is None:
        target_close = pd.to_numeric(
            joined_frame["us100_close"], errors="raise"
        ).to_numpy(dtype=float)
        target_values = feature_set(target_close, source_run, "us100")
    else:
        target = target_frame.reset_index(drop=True)
        if not {"time", "close"}.issubset(target.columns):
            raise ValueError("US100 target frame schema is invalid")
        target_indices = pd.to_numeric(
            joined_frame["target_index"], errors="raise"
        ).to_numpy(dtype=np.int64)
        if (
            len(target) == 0
            or np.any(target_indices < 0)
            or np.any(target_indices >= len(target))
        ):
            raise ValueError("joined target indices are invalid")
        target_time = pd.to_datetime(target["time"], errors="raise")
        joined_time = pd.to_datetime(joined_frame["time"], errors="raise")
        if not np.array_equal(
            target_time.iloc[target_indices].to_numpy(),
            joined_time.to_numpy(),
        ):
            raise ValueError("joined rows differ from the US100 target timeline")
        target_close = pd.to_numeric(target["close"], errors="raise").to_numpy(
            dtype=float
        )
        target_run = _consecutive_run(_time_ns(target))
        target_values = tuple(
            value[target_indices]
            for value in feature_set(target_close, target_run, "us100")
        )
    target_r6, target_rv12, target_rv96, target_z6, target_expansion = (
        target_values
    )
    source_downside = np.minimum(source_z6, 0.0) * source_expansion
    source_symmetric = source_z6 * source_expansion
    target_downside = np.minimum(target_z6, 0.0) * target_expansion
    target_downside[source_run < 97] = np.nan
    return DownsideSpilloverFeatures(
        source_downside_expansion=source_downside,
        source_symmetric_expansion=source_symmetric,
        target_only_downside=target_downside,
        source_r6=source_r6,
        ...
        target_z6=target_z6,
        us100_volatility=target_rv96,
        joint_run=source_run,
    )
...
class _EvaluationContext:
    target_frame: pd.DataFrame
    joined_frame: pd.DataFrame
    features: DownsideSpilloverFeatures
...
def _evaluate_configuration(
    *,
    ...
    executable_id = us30_downside_spillover_executable(configuration, raw_sha256).identity
...
def _context(target_frame: pd.DataFrame, source_frame: pd.DataFrame) -> _EvaluationContext:
    ...
    return _EvaluationContext(
        target_frame=target,
        joined_frame=joined,
        features=compute_downside_spillover_features(joined, target),
...
def _claim_limits() -> list[str]:
    return [
        ...
        "US30_source_eligibility_is_not_scientific_performance_evidence",
        "US30_spread_is_not_US100_execution_cost",
        "exact_timestamp_inner_join_no_fill_no_asof_no_zero",
        "source_gap_blocks_new_entries_and_rewarms_97_joint_bars",
        "source_required_at_decision_not_after_entry",
        "existing_US100_trade_stays_on_target_timeline_after_source_gap",
...
def _surface_from_frames(
    *,
    ...
    raw = _require_digest(raw_sha256, "US30 raw hash")
    prefix_hash = _require_digest(source_prefix_sha256, "US30 prefix hash")
    ...
    surface: dict[str, Any] = {
        ...
        "discovery_implementation_sha256": us30_downside_spillover_implementation_sha256(),
        "engine_environment": {
        ...
        "schema": "us30_downside_spillover_surface.v1",
        "selection_context": [
            {
        ...
        "source_implementation_sha256": us30_source_implementation_sha256(),
        "source_raw_sha256": raw,
...
def _compute_registered_us30_downside_spillover_surface(
    repository_root: str | Path,
) -> dict[str, Any]:
    ...
    raw = us30_raw_sha256(root)
    source = load_us30_observed_development(root, expected_raw_sha256=raw)
    if target.metadata.last_development_time != DEVELOPMENT_END:
        raise US30DownsideSpilloverBoundaryError(
            "US100 and US30 development boundary differs"
        )
...
def project_us30_downside_spillover_evaluation(
    ...
    *,
    ...
    if set(value) != expected_fields or value.get("schema") != "us30_downside_spillover_surface.v1":
        raise US30DownsideSpilloverBoundaryError(
    ...
    if (
        ...
        != us30_downside_spillover_implementation_sha256()
        or value["loader_implementation_sha256"] != loader_implementation_sha256()
        or value["source_implementation_sha256"] != us30_source_implementation_sha256()
    ...
    raw = _require_digest(value["source_raw_sha256"], "surface US30 raw hash")
    _require_digest(
        value["source_development_prefix_sha256"], "surface US30 prefix hash"
    )
    ...
    evaluation = {
        **dict(by_identity[subject_executable_id]),
        ...
        "schema": "us30_downside_spillover_evaluation.v1",
...
__all__ = [
    ...
    "OBSERVED_MATERIAL_ID",
    "ROLLING_SPLIT_SHA256",
    "US30DownsideSpilloverBoundaryError",
    "US30DownsideSpilloverConfiguration",
    "DownsideSpilloverFeatures",
    ...
    "US30DevelopmentMetadata",
    "US30ObservedDevelopment",
    "_compute_registered_us30_downside_spillover_surface",
    "_surface_from_frames",
    "calibrate_selector",
    "compute_downside_spillover_features",
    "us30_downside_spillover_components",
    "us30_downside_spillover_configurations",
    "us30_downside_spillover_executable",
    "us30_downside_spillover_executable_configuration_map",
    "us30_downside_spillover_implementation_sha256",
    "executable_configuration_map",
    "load_us30_observed_development",
    "loader_implementation_sha256",
    "project_us30_downside_spillover_evaluation",
