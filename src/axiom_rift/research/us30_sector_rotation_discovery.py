"""Causal US30 relative-strength discovery on Foundation-safe US100 M5 data.
...
class US30SectorRotationBoundaryError(DiscoveryBoundaryError):
...
def _require_digest(value: str, name: str) -> str:
    if (
        ...
        raise US30SectorRotationBoundaryError(
            f"{name} must be a lowercase SHA256 digest"
        )
    return value


def us30_sector_rotation_implementation_sha256() -> str:
...
def _record_body(raw_line: bytes, name: str) -> bytes:
    ...
    if not body:
        raise US30SectorRotationBoundaryError(f"{name} contains an empty record")
    return body
...
def _scan_us30_source(path: Path, *, expected_raw_sha256: str) -> _SourceScan:
    ...
    if observed != expected:
        prefix.close()
        raise US30SectorRotationBoundaryError("US30 raw SHA256 changed")
...
@dataclass(frozen=True, slots=True)
class US30SectorRotationConfiguration:
    profile: str
    route_sign: int
    holding_bars: int
...
def us30_sector_rotation_configurations() -> tuple[US30SectorRotationConfiguration, ...]:
    return tuple(
        US30SectorRotationConfiguration(profile, route_sign, holding_bars)
...
def _local_implementation(function_name: str) -> str:
    return (
        "axiom_rift.research.us30_sector_rotation_discovery."
        f"{function_name}@sha256:{us30_sector_rotation_implementation_sha256()}"
    )
...
def us30_sector_rotation_components(raw_sha256: str) -> tuple[ComponentSpec, ...]:
    ...
    feature = ComponentSpec(
            display_name="exact causal US30 and US100 relative strength",
            protocol="feature.us30_sector_rotation_12_sigma48.v1",
...
def us30_sector_rotation_executable(
    configuration: US30SectorRotationConfiguration,
    raw_sha256: str,
) -> ExecutableSpec:
    if not isinstance(configuration, US30SectorRotationConfiguration):
        raise TypeError("configuration must be US30SectorRotationConfiguration")
    raw = _require_digest(raw_sha256, "US30 raw hash")
    source = _source_identity_payload()
    return ExecutableSpec(
        display_name=f"US30 sector rotation {configuration.configuration_id}",
        components=us30_sector_rotation_components(raw),
        parameters={
        ...
        engine_contract=(
            "engine:us30_sector_rotation_discovery_v1:python3_13_9:"
            "numpy2_3_4:pandas2_3_3:scipy1_16_3:"
            f"implementation_{us30_sector_rotation_implementation_sha256()}:"
...
def _configuration_map_for_raw(raw_sha256: str) -> dict[str, US30SectorRotationConfiguration]:
    return {
        us30_sector_rotation_executable(configuration, raw_sha256).identity: configuration
        for configuration in us30_sector_rotation_configurations()
    }


def us30_sector_rotation_executable_configuration_map(
    repository_root: str | Path,
...
def executable_configuration_map(
    ...
    return us30_sector_rotation_executable_configuration_map(repository_root)


@dataclass(frozen=True, slots=True)
class RelativeStrengthFeatures:
    relative_strength_12_joint: np.ndarray
    us30_direction_12_source_only: np.ndarray
...
def _join_exact(target_frame: pd.DataFrame, source_frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if not required_target.issubset(target_frame.columns) or set(source_frame.columns) != {"time", "close"}:
        raise US30SectorRotationBoundaryError("cross-asset frame schema is invalid")
    ...
    for frame, name in ((target, "US100"), (source, "US30")):
        ...
        if frame["time"].duplicated().any() or not frame["time"].is_monotonic_increasing:
            raise US30SectorRotationBoundaryError(f"{name} timestamps are invalid")
    ...
    if joined.empty or joined["time"].duplicated().any() or not joined["time"].is_monotonic_increasing:
        raise US30SectorRotationBoundaryError("exact cross-asset join is empty or invalid")
...
def compute_relative_strength_features(joined_frame: pd.DataFrame) -> RelativeStrengthFeatures:
    ...
    us30_score, _ = values["us30"]
    relative = us30_score - us100_score
    relative[~(np.isfinite(us30_score) & np.isfinite(us100_score))] = np.nan
    return RelativeStrengthFeatures(
        relative_strength_12_joint=relative,
        us30_direction_12_source_only=us30_score,
...
def _evaluate_configuration(
    *,
    ...
    executable_id = us30_sector_rotation_executable(configuration, raw_sha256).identity
...
def _surface_from_frames(
    *,
    ...
    surface: dict[str, Any] = {
        ...
        "discovery_implementation_sha256": us30_sector_rotation_implementation_sha256(),
        "engine_environment": {
        ...
        "schema": "us30_sector_rotation_surface.v1",
        "selection_context": [
            {
...
def _compute_registered_us30_sector_rotation_surface(
    repository_root: str | Path,
) -> dict[str, Any]:
...
def project_us30_sector_rotation_evaluation(
    ...
    *,
    ...
    if set(value) != expected_fields or value.get("schema") != "us30_sector_rotation_surface.v1":
        raise US30SectorRotationBoundaryError(
    ...
    evaluation = {
        **dict(by_identity[subject_executable_id]),
        ...
        "schema": "us30_sector_rotation_evaluation.v1",
...
__all__ = [
    ...
    "OBSERVED_MATERIAL_ID",
    "ROLLING_SPLIT_SHA256",
    "US30SectorRotationBoundaryError",
    ...
    "US30ObservedDevelopment",
    "_compute_registered_us30_sector_rotation_surface",
    "_surface_from_frames",
    "calibrate_selector",
    "compute_relative_strength_features",
    "us30_sector_rotation_components",
    "us30_sector_rotation_configurations",
    "us30_sector_rotation_executable",
    "us30_sector_rotation_executable_configuration_map",
    "us30_sector_rotation_implementation_sha256",
