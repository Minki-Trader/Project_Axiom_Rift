"""Discovery runner for registered US100-US30 downside spillover Study."""
...
CALLABLE_IDENTITY = "axiom_rift.research.us30_downside_spillover_study.execute_us30_downside_spillover_job.v1"
SURFACE_SCHEMA = "us30_downside_spillover_surface.v1"
EVALUATION_SCHEMA = "us30_downside_spillover_evaluation.v1"
...
def build_us30_downside_spillover_validation_plan(
    executable_id: str,
    *,
    mission_id: str,
) -> dict[str, object]:
...
def _claim_metrics(evaluation: Mapping[str, Any]) -> dict[str, dict[str, int | None]]:
    ...
    if not isinstance(raw, Mapping):
        raise ValueError("us30_downside_spillover evaluation has no metrics")
    ...
    if any(type(name) is not str or type(value) is not int for name, value in metrics.items()):
        raise ValueError("us30_downside_spillover evaluation metrics must be integer scalars")
    ...
    def values(*names: str, null_when_not_evaluable: bool = False) -> dict[str, int | None]:
        ...
        for name in names:
            if name not in metrics:
                raise ValueError(f"us30_downside_spillover evaluation metric is absent: {name}")
            result[name] = (
...
def output_names(executable_id: str) -> dict[str, str]:
    ...
    prefix = f"scientific/us30-downside-spillover/{short}"
    return {
...
def surface_cache_output_name(repository_root: str | Path) -> str:
    raw_sha256 = us30_raw_sha256(Path(repository_root).resolve())
    return (
        "local/cache/us30-downside-spillover/us30_downside_spillover-surface-"
        f"{us30_downside_spillover_implementation_sha256()[:12]}-"
        f"{raw_sha256[:12]}.json"
    )


def surface_manifest_output_name() -> str:
    return "scientific/us30-downside-spillover/surface-cache-manifest.json"
...
def _source_provenance(repository_root: str | Path) -> dict[str, str]:
    ...
    source_loader_hash = us30_source_implementation_sha256()
    if source_loader_hash != source_dependency_sha256():
        raise ValueError("US30 source loader hash differs from source qualification")
    value = {
        "source_contract_id": source_contract_id,
        "source_contract_sha256": source_contract_id.removeprefix("source:"),
        "source_raw_sha256": us30_raw_sha256(root),
...
def _registered_executable_ids(repository_root: str | Path) -> tuple[str, ...]:
    mapping = us30_downside_spillover_executable_configuration_map(
        Path(repository_root).resolve()
    )
    if not isinstance(mapping, Mapping) or len(mapping) != 12:
        raise ValueError("us30_downside_spillover Study requires exactly twelve Executables")
    ...
    if any(
        ...
        raise ValueError("us30_downside_spillover Study Executable identities are invalid")
    return identities


def build_environment_manifest(repository_root: str | Path) -> dict[str, object]:
    if SELECTION_TOTAL_EXPOSURES != 574:
        raise ValueError("us30_downside_spillover Study requires global exposure total 574")
    source = _source_provenance(repository_root)
    value: dict[str, object] = {
        "dataset_sha256": DATASET_SHA256,
        "us30_downside_spillover_implementation_sha256": us30_downside_spillover_implementation_sha256(),
...
class US30DownsideSpilloverJobPacket:
    ...
    def artifact(self, name: str) -> dict[str, Any]:
        ...
        if not isinstance(value, dict):
            raise ValueError("us30_downside_spillover Job artifact is not an object")
        return value
...
def _materialize_cache(
    ...
    *,
    ...
    if cache_root not in target.parents:
        raise ValueError("us30_downside_spillover surface cache path escapes local/cache")
    expected_hash = sha256(content).hexdigest()
    if target.exists():
        if not target.is_file() or sha256(target.read_bytes()).hexdigest() != (
            expected_hash
        ):
            raise ValueError("existing us30_downside_spillover surface cache has different bytes")
        return
    target.parent.mkdir(parents=True, exist_ok=True)
    descriptor, temporary_name = tempfile.mkstemp(
        prefix=".us30_downside_spillover-surface-",
        suffix=".tmp",
        dir=target.parent,
    )
...
def _build_surface_manifest(
    *,
    ...
    value: dict[str, Any] = {
        ...
        "schema": "us30_downside_spillover_surface_cache_manifest.v1",
        "selection_total_exposures": SELECTION_TOTAL_EXPOSURES,
        "split_artifact_sha256": ROLLING_SPLIT_SHA256,
        "trend_dependency_sha256": trend_dependency_sha256(),
        **source,
...
def _validate_surface_manifest(
    ...
    *,
    ...
    if (
        ...
        or value.get("schema") != "us30_downside_spillover_surface_cache_manifest.v1"
        ...
        or value.get("us30_downside_spillover_implementation_sha256")
        != us30_downside_spillover_implementation_sha256()
        ...
        or value.get("selection_total_exposures") != 574
        ...
        raise ValueError("us30_downside_spillover surface cache manifest differs from Job inputs")
    ...
    if not isinstance(producer, dict) or set(producer) != {
        ...
        raise ValueError("us30_downside_spillover surface producer execution is invalid")
    ...
    if producer["identity"] != execution.identity:
        raise ValueError("us30_downside_spillover surface producer identity is invalid")
    return value
...
def _load_surface_manifest(
    ...
    *,
    ...
    if len(matches) != 1:
        raise ValueError("Job inputs require one exact us30_downside_spillover surface manifest")
    return matches[0]


def execute_us30_downside_spillover_job(
    *,
    repository_root: str | Path,
    execution: RunningJobExecution,
) -> US30DownsideSpilloverJobPacket:
    ...
    if (
        ...
        raise ValueError("running Job is not bound to the registered us30_downside_spillover Study")
    executable_id = evidence_subject["id"]
    scientific_binding = spec.get("scientific_binding")
    if not isinstance(scientific_binding, dict):
        raise ValueError("us30_downside_spillover Job lacks a scientific binding")
    plan = build_us30_downside_spillover_validation_plan(
        executable_id,
    ...
    if not required_inputs.issubset(inputs):
        raise ValueError("us30_downside_spillover Job omits a required content-bound input")
    if scientific_binding.get("validation_plan_hash") != plan_hash:
        raise ValueError("us30_downside_spillover validation plan differs from Job input")
    ...
    if (
        ...
        raise ValueError("us30_downside_spillover scientific binding differs from preregistration")
    ...
    if not isinstance(output_classes, dict):
        raise ValueError("us30_downside_spillover Job output classes are invalid")
    ...
    if expected_outputs != expected_set or output_classes != expected_classes:
        raise ValueError("us30_downside_spillover Job outputs differ from the producer/projector protocol")

    manifest_artifact_hash: str
    manifest_bytes: bytes | None = None
    if produces_surface:
        surface = _validated_surface(
            _compute_registered_us30_downside_spillover_surface(root)
        )
        ...
        if not cache_target.is_file():
            raise ValueError("us30_downside_spillover surface cache is unavailable")
        surface_bytes = cache_target.read_bytes()
        surface_hash = sha256(surface_bytes).hexdigest()
        if surface_hash not in inputs:
            raise ValueError("us30_downside_spillover surface cache hash is not a Job input")
        ...
        writer.verify_reproducible_cache_producer(
            ...
            expected_study_id=binding["study_id"],
    ...
    context = project_us30_downside_spillover_evaluation(
        surface,
    ...
    if set(output_manifest) != expected_outputs:
        raise ValueError("materialized us30_downside_spillover outputs differ from Job declaration")
    return US30DownsideSpilloverJobPacket(
...
__all__ = [
    ...
    "build_us30_downside_spillover_validation_plan",
    "output_names",
    "planned_verdict",
    "execute_us30_downside_spillover_job",
