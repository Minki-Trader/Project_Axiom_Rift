"""Writer-gated evidence Job for causal gap recovery."""
...
MISSION_ID="MIS-0004";STUDY_ID="STU-0047";CALLABLE_IDENTITY="axiom_rift.research.gap_recovery_study.execute_gap_job.v4";EVIDENCE_DEPTH="discovery"
...
def _load(w:StateWriter,hashes:tuple[str,...])->tuple[dict[str,Any],str,str]:
    ...
    for h in hashes:
        ...
        if isinstance(v,dict) and v.get("schema")=="gap_recovery_surface.v4":s=(v,h)
    ...
    return s[0],s[1],m[1]
def execute_gap_job(*,repository_root:str|Path,execution:RunningJobExecution)->GapJobPacket:
    ...
    eid=subject["id"];plan=build_gap_validation_plan(eid);env=build_environment_manifest();ph=sha256(canonical_bytes(plan)).hexdigest();names=output_names(eid);inputs=tuple(spec["input_hashes"]);required={DATASET_SHA256,OBSERVED_MATERIAL_ID,ROLLING_SPLIT_SHA256,ph,gap_implementation_sha256(),loader_implementation_sha256(),discovery_implementation_sha256()}
