"""Writer-gated evidence Job for complementary sleeve netting."""
...
STUDY_ID = "STU-0066"
CALLABLE_IDENTITY = (
    "axiom_rift.research.complementary_sleeve_study."
    "execute_complementary_sleeve_job.v1"
)
EVIDENCE_DEPTH = "discovery"


def build_complementary_sleeve_validation_plan(
    executable_id: str,
) -> dict[str, object]:
...
def surface_output_name() -> str:
    return f"scientific/{STUDY_ID}/complementary-sleeve-surface.json"


def surface_manifest_output_name() -> str:
    return f"scientific/{STUDY_ID}/complementary-sleeve-surface-manifest.json"
...
class ComplementarySleeveJobPacket:
...
def _load_surface(
    ...
    for artifact_hash in input_hashes:
        ...
        if (
            ...
            and value.get("schema") == "complementary_sleeve_surface.v1"
        ):
            surface = (value, artifact_hash)
        if (
            isinstance(value, dict)
            and value.get("schema") == "complementary_sleeve_surface_manifest.v1"
        ):
    ...
    if (
        ...
        raise ValueError("complementary-sleeve surface is missing")
    return surface[0], surface[1], manifest[1]


def execute_complementary_sleeve_job(
    *,
    repository_root: str | Path,
    execution: RunningJobExecution,
) -> ComplementarySleeveJobPacket:
    ...
    if (
        ...
        raise ValueError("complementary-sleeve binding is invalid")
    executable_id = subject["id"]
    plan = build_complementary_sleeve_validation_plan(executable_id)
    ...
    if not required.issubset(inputs):
        raise ValueError("complementary-sleeve inputs are missing")
    expected = set(spec["expected_outputs"])
    produces_surface = surface_output_name() in expected
    if produces_surface:
        surface = compute_registered_complementary_sleeve_surface(root)
        surface_hash = writer.evidence.finalize(canonical_bytes(surface)).sha256
        manifest_value = {
            "schema": "complementary_sleeve_surface_manifest.v1",
    ...
    evaluation = project_complementary_sleeve_evaluation(
        surface,
    ...
    return ComplementarySleeveJobPacket(
...
__all__ = [
    ...
    "PLANNED_CLAIMS",
    "STUDY_ID",
    "build_complementary_sleeve_validation_plan",
    "build_environment_manifest",
    "execute_complementary_sleeve_job",
    "output_names",
