"""Registered router versus broker-session dense positive sleeve surface."""
...
from axiom_rift.research.session_dense_positive_sleeve_chassis import SELECTION_TOTAL_EXPOSURES, executable_configuration_map, loader_implementation_sha256, session_dense_positive_sleeve_chassis_implementation_sha256, session_dense_positive_sleeve_configurations, session_dense_positive_sleeve_executable, simulate_session_dense_positive_sleeves
...
def session_dense_positive_sleeve_discovery_implementation_sha256() -> str:
...
def _threshold(score: np.ndarray, mask: np.ndarray, quantile_bp: int) -> float:
    ...
    if len(values) < 1000:
        raise DiscoveryBoundaryError("session dense positive selector is too small")
...
def _matched(results: list[Any], profile: str) -> Any:
    ...
    if len(found) != 1:
        raise DiscoveryBoundaryError("session dense positive control is not unique")
    return found[0]
...
def compute_registered_session_dense_positive_sleeve_surface(repository_root: str | Path) -> dict[str, Any]:
    ...
    results: list[Any] = []
    for configuration in session_dense_positive_sleeve_configurations():
        ...
        results.append(_evaluate_configuration(calibrations=calibrations, frame=frame, features=first, fold_features=fold_scores, folds=folds, configuration=configuration, effective_spread=spread, prefix_features=prefix_scores, prefix_spreads=prefix_spreads, time=time, executable_id=session_dense_positive_sleeve_executable(configuration).identity, simulation_fn=simulate_session_dense_positive_sleeves))
    ...
    _populate(results)
    surface = {
        "claim_limits": _claim_limits() + ["portfolio_is_the_primary_changed_layer", "subject_selector_and_session_are_one_fixed_structural_extreme", "both_sleeves_are_US100_completed_bar_only", "each_sleeve_uses_one_fixed_lot", "no_adjacent_quantile_or_hour_window_tuning", "two_trial_surface"],
        ...
        "schema": "session_dense_positive_sleeve_surface.v1",
        ...
        "session_dense_positive_sleeve_chassis_implementation_sha256": session_dense_positive_sleeve_chassis_implementation_sha256(),
...
def project_session_dense_positive_sleeve_evaluation(surface: Mapping[str, Any], *, job_execution: Mapping[str, str], subject_executable_id: str, surface_artifact_hash: str, surface_manifest_hash: str) -> dict[str, Any]:
    value = dict(surface)
    if sha256(canonical_bytes(value)).hexdigest() != surface_artifact_hash or value.get("schema") != "session_dense_positive_sleeve_surface.v1":
        raise DiscoveryBoundaryError("session dense positive surface invalid")
    ...
    if set(by_executable) != set(expected) or subject_executable_id not in expected:
        raise DiscoveryBoundaryError("session dense positive subjects differ")
    ...
    if job_execution.get("identity") != canonical_digest(domain="running-job-execution", payload=payload):
        raise DiscoveryBoundaryError("session dense positive Job invalid")
    result = {**dict(by_executable[subject_executable_id]), "claim_limits": value["claim_limits"], "job_execution": dict(job_execution), "schema": "session_dense_positive_sleeve_evaluation.v1", "selection_context": value["selection_context"], "selection_method": value["selection_method"], "session_semantics": value["session_semantics"], "surface_artifact_hash": surface_artifact_hash, "surface_manifest_hash": surface_manifest_hash}
    canonical_bytes(result)
    return result


__all__ = ["compute_registered_session_dense_positive_sleeve_surface", "project_session_dense_positive_sleeve_evaluation", "session_dense_positive_sleeve_discovery_implementation_sha256"]
