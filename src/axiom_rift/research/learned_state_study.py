"""Writer-gated evidence Job for fold-trained learned state."""
...
from axiom_rift.research.learned_state_discovery import compute_registered_learned_surface,executable_configuration_map,learned_implementation_sha256,loader_implementation_sha256,project_learned_evaluation
...
MISSION_ID="MIS-0003";STUDY_ID="STU-0040";CALLABLE_IDENTITY="axiom_rift.research.learned_state_study.execute_learned_job.v1";EVIDENCE_DEPTH="discovery"
def build_learned_validation_plan(eid:str)->dict[str,object]:return build_validation_plan(mission_id=MISSION_ID,executable_id=eid,evidence_depth=EVIDENCE_DEPTH,planned_claims=PLANNED_CLAIMS,evidence_modes=EVIDENCE_MODES,criteria=CRITERIA,candidate_eligible_on_pass=False)
...
def _load(w:StateWriter,hashes:tuple[str,...])->tuple[dict[str,Any],str,str]:
    ...
    for h in hashes:
        ...
        if isinstance(v,dict) and v.get("schema")=="learned_state_surface.v1":s=(v,h)
        if isinstance(v,dict) and v.get("schema")=="learned_surface_manifest.v1":m=(v,h)
    if s is None or m is None or m[0].get("surface_artifact_hash")!=s[1]:raise ValueError("surface missing")
    return s[0],s[1],m[1]
def execute_learned_job(*,repository_root:str|Path,execution:RunningJobExecution)->LearnedJobPacket:
    ...
    eid=subject["id"];plan=build_learned_validation_plan(eid);env=build_environment_manifest();ph=sha256(canonical_bytes(plan)).hexdigest();names=output_names(eid);inputs=tuple(spec["input_hashes"]);required={DATASET_SHA256,OBSERVED_MATERIAL_ID,ROLLING_SPLIT_SHA256,ph,learned_implementation_sha256(),loader_implementation_sha256(),discovery_implementation_sha256()}
