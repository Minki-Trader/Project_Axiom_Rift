"""Writer-gated evidence Job for the causal long-trend regime contrast."""
...
from axiom_rift.research.trend_regime_chassis import executable_configuration_map,loader_implementation_sha256,trend_regime_chassis_implementation_sha256
from axiom_rift.research.trend_regime_discovery import compute_registered_trend_regime_surface,project_trend_regime_evaluation,trend_regime_discovery_implementation_sha256
...
CALLABLE_IDENTITY="axiom_rift.research.trend_regime_study.execute_trend_regime_job.v1"
EVIDENCE_DEPTH="discovery"
_CONTROL_DELTA_METRIC="regime_control_delta_net_profit_micropoints"
_CONTROL_PVALUE_METRIC="regime_control_pvalue_upper_ppm"
...
def build_trend_regime_validation_plan(executable_id:str,*,mission_id:str)->dict[str,object]:
...
def _load_surface(writer:StateWriter,input_hashes:tuple[str,...])->tuple[dict[str,Any],str,str]:
    ...
    for artifact_hash in input_hashes:
        ...
        if isinstance(value,dict) and value.get("schema")=="trend_regime_evaluation_surface.v1":surface=(value,artifact_hash)
        if isinstance(value,dict) and value.get("schema")=="trend_regime_surface_manifest.v1":manifest=(value,artifact_hash)
    if surface is None or manifest is None or manifest[0].get("surface_artifact_hash")!=surface[1]:raise ValueError("trend regime surface is missing")
    return surface[0],surface[1],manifest[1]
def execute_trend_regime_job(*,repository_root:str|Path,execution:RunningJobExecution)->TrendRegimeJobPacket:
    ...
    if not isinstance(mission_id,str) or not isinstance(study_id,str) or not isinstance(subject,dict) or subject.get("id") not in executable_configuration_map():raise ValueError("trend regime binding is invalid")
    executable_id=subject["id"];plan=build_trend_regime_validation_plan(executable_id,mission_id=mission_id);environment=build_environment_manifest();plan_hash=sha256(canonical_bytes(plan)).hexdigest();names=output_names(executable_id,study_id=study_id);inputs=tuple(spec["input_hashes"])
    required={DATASET_SHA256,OBSERVED_MATERIAL_ID,ROLLING_SPLIT_SHA256,plan_hash,trend_regime_chassis_implementation_sha256(),trend_regime_discovery_implementation_sha256(),volatility_clock_label_chassis_implementation_sha256(),event_label_implementation_sha256(),loader_implementation_sha256(),discovery_implementation_sha256()}
    if not required.issubset(inputs):raise ValueError("trend regime inputs are missing")
    expected=set(spec["expected_outputs"]);produces_surface=surface_output_name(study_id=study_id) in expected
    if produces_surface:
        surface=compute_registered_trend_regime_surface(root);surface_hash=writer.evidence.finalize(canonical_bytes(surface)).sha256;manifest_value={"schema":"trend_regime_surface_manifest.v1","surface_artifact_hash":surface_hash,"surface_implementation_sha256":trend_regime_discovery_implementation_sha256()};manifest_hash=writer.evidence.finalize(canonical_bytes(manifest_value)).sha256
    else:surface,surface_hash,manifest_hash=_load_surface(writer,inputs)
    evaluation=project_trend_regime_evaluation(surface,job_execution={**execution.payload(),"identity":execution.identity},subject_executable_id=executable_id,surface_artifact_hash=surface_hash,surface_manifest_hash=manifest_hash);evaluation_hash=writer.evidence.finalize(canonical_bytes(evaluation)).sha256
    ...
    return TrendRegimeJobPacket(output_manifest=tuple(sorted(outputs.items())),verdict=planned_verdict(plan,measurement))
__all__=["CALLABLE_IDENTITY","CRITERIA","EVIDENCE_DEPTH","EVIDENCE_MODES","PLANNED_CLAIMS","build_environment_manifest","build_trend_regime_validation_plan","execute_trend_regime_job","output_names","surface_manifest_output_name","surface_output_name"]
