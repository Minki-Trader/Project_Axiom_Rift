"""Evidence plan and writer-gated Job for bounded nonlinear discovery."""
...
from axiom_rift.research.nonlinear_interaction_discovery import compute_registered_nonlinear_surface,executable_configuration_map,loader_implementation_sha256,nonlinear_implementation_sha256,project_nonlinear_evaluation
...
MISSION_ID="MIS-0002";STUDY_ID="STU-0028";CALLABLE_IDENTITY="axiom_rift.research.nonlinear_interaction_study.execute_nonlinear_job.v1";EVIDENCE_DEPTH="discovery"
def build_nonlinear_validation_plan(eid:str)->dict[str,object]:return build_validation_plan(mission_id=MISSION_ID,executable_id=eid,evidence_depth=EVIDENCE_DEPTH,planned_claims=PLANNED_CLAIMS,evidence_modes=EVIDENCE_MODES,criteria=CRITERIA,candidate_eligible_on_pass=False)
...
def _load_surface(w:StateWriter,hashes:tuple[str,...])->tuple[dict[str,Any],str,str]:
    ...
    for h in hashes:
        ...
        if isinstance(v,dict) and v.get("schema")=="nonlinear_interaction_surface.v1":surface=(v,h)
        if isinstance(v,dict) and v.get("schema")=="nonlinear_surface_manifest.v1":manifest=(v,h)
    if surface is None or manifest is None or manifest[0].get("surface_artifact_hash")!=surface[1]:raise ValueError("nonlinear inputs lack surface")
    return surface[0],surface[1],manifest[1]
def execute_nonlinear_job(*,repository_root:str|Path,execution:RunningJobExecution)->NonlinearJobPacket:
    ...
    if binding.get("mission_id")!=MISSION_ID or binding.get("study_id")!=STUDY_ID or not isinstance(subject,dict) or subject.get("kind")!="Executable" or subject.get("id") not in executable_configuration_map():raise ValueError("running Job is not nonlinear bound")
    eid=subject["id"];plan=build_nonlinear_validation_plan(eid);env=build_environment_manifest();ph=sha256(canonical_bytes(plan)).hexdigest();names=output_names(eid);sb=spec.get("scientific_binding")
    if not isinstance(sb,dict) or sb.get("validator_id")!=SCIENTIFIC_DISCOVERY_VALIDATOR_ID or sb.get("validation_plan_hash")!=ph or sb.get("result_manifest_output")!=names["result"]:raise ValueError("nonlinear binding differs")
    inputs=tuple(spec.get("input_hashes",[]));required={DATASET_SHA256,OBSERVED_MATERIAL_ID,ROLLING_SPLIT_SHA256,ph,nonlinear_implementation_sha256(),loader_implementation_sha256(),discovery_implementation_sha256()}
    if not required.issubset(inputs):raise ValueError("nonlinear Job omits input")
