"""Independent validator for FPMarkets US30 source eligibility evidence."""
...
SOURCE_ELIGIBILITY_VALIDATOR_ID = validator_identity(
    protocol="fpmarkets_us30_source_eligibility.v3",
...
class SourceEligibilityValidator:
    ...
    def validate(self, request: EvidenceValidationRequest) -> ValidatedEvidence:
        if request.domain != "source" or request.validator_id != self.validator_id:
            raise EvidenceValidationError("US30 validator received another domain")
        ...
        source_id = us30_source_contract().source_contract_id
        if (
        ...
        if transition == "historical_audit":
            ...
            except US30SourceError as exc:
                raise EvidenceValidationError(
                    "US30 runtime probe schema is invalid"
                ) from exc
