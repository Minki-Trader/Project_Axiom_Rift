...
from axiom_rift.research.fold_train_target_role_chassis import executable_configuration_map, loader_implementation_sha256, fold_train_target_role_chassis_implementation_sha256
from axiom_rift.research.fold_train_target_role_discovery import compute_registered_fold_train_target_role_surface, project_fold_train_target_role_evaluation, fold_train_target_role_discovery_implementation_sha256
...
CALLABLE_IDENTITY = "axiom_rift.research.fold_train_target_role_study.execute_fold_train_target_role_job.v1"
EVIDENCE_DEPTH = "discovery"
_DELTA = "fixed_high_reverse_control_delta_net_profit_micropoints"
_PVALUE = "fixed_high_reverse_control_pvalue_upper_ppm"
CRITERIA = discovery_criteria(control_delta_metric=_DELTA, control_pvalue_metric=_PVALUE, include_opposite_sign=False)


def build_fold_train_target_role_validation_plan(executable_id: str, *, mission_id: str) -> dict[str, object]:
...
def surface_output_name(*, study_id: str) -> str:
    return f"scientific/{study_id}/fold-train-target-role-surface.json"


def surface_manifest_output_name(*, study_id: str) -> str:
    return f"scientific/{study_id}/fold-train-target-role-surface-manifest.json"


def build_environment_manifest() -> dict[str, object]:
    value = {"dataset_sha256": DATASET_SHA256, "loader_implementation_sha256": loader_implementation_sha256(), "material_identity": OBSERVED_MATERIAL_ID, "numpy_version": np.__version__, "pandas_version": pd.__version__, "python_version": ".".join(str(value) for value in sys.version_info[:3]), "runner_implementation_sha256": sha256(Path(__file__).resolve().read_bytes()).hexdigest(), "schema": "scientific_engine_environment.v1", "scipy_version": scipy.__version__, "fold_train_target_role_chassis_implementation_sha256": fold_train_target_role_chassis_implementation_sha256(), "fold_train_target_role_discovery_implementation_sha256": fold_train_target_role_discovery_implementation_sha256(), "shared_discovery_implementation_sha256": discovery_implementation_sha256(), "split_artifact_sha256": ROLLING_SPLIT_SHA256, "validator_id": SCIENTIFIC_DISCOVERY_VALIDATOR_ID, "writer_implementation_sha256": sha256(Path(writer_module.__file__).resolve().read_bytes()).hexdigest()}
...
class FoldTrainTargetRoleJobPacket:
...
def _load(writer: StateWriter, inputs: tuple[str, ...]) -> tuple[dict[str, Any], str, str]:
    ...
    for artifact_hash in inputs:
        try:
            ...
            continue
        if isinstance(value, dict) and value.get("schema") == "fold_train_target_role_surface.v1":
            surface = (value, artifact_hash)
        if isinstance(value, dict) and value.get("schema") == "fold_train_target_role_surface_manifest.v1":
            manifest = (value, artifact_hash)
    if surface is None or manifest is None or manifest[0].get("surface_artifact_hash") != surface[1]:
        raise ValueError("fold train target role surface missing")
    return surface[0], surface[1], manifest[1]


def execute_fold_train_target_role_job(*, repository_root: str | Path, execution: RunningJobExecution) -> FoldTrainTargetRoleJobPacket:
    ...
    if not isinstance(mission_id, str) or not isinstance(study_id, str) or not isinstance(subject, dict) or subject.get("id") not in executable_configuration_map():
        raise ValueError("fold train target role binding invalid")
    executable_id = subject["id"]
    plan = build_fold_train_target_role_validation_plan(executable_id, mission_id=mission_id)
    ...
    required = {DATASET_SHA256, OBSERVED_MATERIAL_ID, ROLLING_SPLIT_SHA256, plan_hash, fold_train_target_role_chassis_implementation_sha256(), fold_train_target_role_discovery_implementation_sha256(), loader_implementation_sha256(), discovery_implementation_sha256()}
    if not required.issubset(inputs):
        raise ValueError("fold train target role inputs missing")
    produces = surface_output_name(study_id=study_id) in set(spec["expected_outputs"])
    if produces:
        surface = compute_registered_fold_train_target_role_surface(root)
        surface_hash = writer.evidence.finalize(canonical_bytes(surface)).sha256
        manifest_value = {"schema": "fold_train_target_role_surface_manifest.v1", "surface_artifact_hash": surface_hash, "surface_implementation_sha256": fold_train_target_role_discovery_implementation_sha256()}
    ...
    evaluation = project_fold_train_target_role_evaluation(surface, job_execution={**execution.payload(), "identity": execution.identity}, subject_executable_id=executable_id, surface_artifact_hash=surface_hash, surface_manifest_hash=manifest_hash)
    ...
    return FoldTrainTargetRoleJobPacket(tuple(sorted(outputs.items())), planned_verdict(plan, measurement))


__all__ = ["CALLABLE_IDENTITY", "CRITERIA", "EVIDENCE_DEPTH", "EVIDENCE_MODES", "PLANNED_CLAIMS", "build_environment_manifest", "build_fold_train_target_role_validation_plan", "execute_fold_train_target_role_job", "output_names", "surface_manifest_output_name", "surface_output_name"]
