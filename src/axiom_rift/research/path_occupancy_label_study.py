"""Writer-gated evidence Job for the path-occupancy label contrast."""
...
CALLABLE_IDENTITY = (
    "axiom_rift.research.path_occupancy_label_study."
    "execute_path_occupancy_label_job.v1"
)
...
def build_path_occupancy_label_validation_plan(
    executable_id: str,
    *,
    mission_id: str,
) -> dict[str, object]:
...
def surface_output_name(*, study_id: str) -> str:
    return f"scientific/{study_id}/path-occupancy-label-surface.json"


def surface_manifest_output_name(*, study_id: str) -> str:
    return f"scientific/{study_id}/path-occupancy-label-surface-manifest.json"
...
class PathOccupancyLabelJobPacket:
...
def _load_surface(
    ...
    for artifact_hash in input_hashes:
        ...
        if (
            ...
            and value.get("schema") == "path_occupancy_label_surface.v1"
        ):
            surface = (value, artifact_hash)
        if (
            isinstance(value, dict)
            and value.get("schema")
            == "path_occupancy_label_surface_manifest.v1"
        ):
    ...
    if (
        ...
        raise ValueError("path-occupancy label surface is missing")
    return surface[0], surface[1], manifest[1]


def execute_path_occupancy_label_job(
    *,
    repository_root: str | Path,
    execution: RunningJobExecution,
) -> PathOccupancyLabelJobPacket:
    ...
    if (
        ...
        raise ValueError("path-occupancy label binding is invalid")
    executable_id = subject["id"]
    plan = build_path_occupancy_label_validation_plan(
    ...
    if not required.issubset(inputs):
        raise ValueError("path-occupancy label inputs are missing")
    expected = set(spec["expected_outputs"])
    produces_surface = surface_output_name(study_id=study_id) in expected
    if produces_surface:
        surface = compute_registered_path_occupancy_label_surface(root)
        surface_hash = writer.evidence.finalize(canonical_bytes(surface)).sha256
        manifest_value = {
            "schema": "path_occupancy_label_surface_manifest.v1",
            "surface_artifact_hash": surface_hash,
            "surface_implementation_sha256": path_occupancy_label_implementation_sha256(),
        }
    ...
    evaluation = project_path_occupancy_label_evaluation(
        surface,
    ...
    return PathOccupancyLabelJobPacket(
...
__all__ = [
    ...
    "build_path_occupancy_label_validation_plan",
    "execute_path_occupancy_label_job",
    "output_names",
