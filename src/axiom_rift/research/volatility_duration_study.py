"""Writer-gated evidence Job for causal volatility duration."""
...
from axiom_rift.research.volatility_duration_discovery import compute_registered_volatility_duration_surface,executable_configuration_map,loader_implementation_sha256,project_volatility_duration_evaluation,volatility_duration_implementation_sha256
MISSION_ID="MIS-0004";STUDY_ID="STU-0051";CALLABLE_IDENTITY="axiom_rift.research.volatility_duration_study.execute_volatility_duration_job.v2";EVIDENCE_DEPTH="discovery"
def build_volatility_duration_validation_plan(eid:str)->dict[str,object]:return build_validation_plan(mission_id=MISSION_ID,executable_id=eid,evidence_depth=EVIDENCE_DEPTH,planned_claims=PLANNED_CLAIMS,evidence_modes=EVIDENCE_MODES,criteria=CRITERIA,candidate_eligible_on_pass=False)
...
def surface_output_name()->str:return f"scientific/{STUDY_ID}/volatility-duration-surface.json"
def surface_manifest_output_name()->str:return f"scientific/{STUDY_ID}/volatility-duration-surface-manifest.json"
def build_environment_manifest()->dict[str,object]:
    v={"dataset_sha256":DATASET_SHA256,"loader_implementation_sha256":loader_implementation_sha256(),"material_identity":OBSERVED_MATERIAL_ID,"numpy_version":np.__version__,"pandas_version":pd.__version__,"python_version":".".join(str(x) for x in sys.version_info[:3]),"runner_implementation_sha256":sha256(Path(__file__).resolve().read_bytes()).hexdigest(),"schema":"scientific_engine_environment.v1","scipy_version":scipy.__version__,"shared_discovery_implementation_sha256":discovery_implementation_sha256(),"split_artifact_sha256":ROLLING_SPLIT_SHA256,"validator_id":SCIENTIFIC_DISCOVERY_VALIDATOR_ID,"volatility_duration_implementation_sha256":volatility_duration_implementation_sha256(),"writer_implementation_sha256":sha256(Path(writer_module.__file__).resolve().read_bytes()).hexdigest()};canonical_bytes(v);return v
...
class VolatilityDurationJobPacket:
...
def _load(w:StateWriter,hashes:tuple[str,...])->tuple[dict[str,Any],str,str]:
    ...
    for h in hashes:
        ...
        if isinstance(v,dict) and v.get("schema")=="volatility_duration_surface.v2":s=(v,h)
        if isinstance(v,dict) and v.get("schema")=="volatility_duration_surface_manifest.v1":m=(v,h)
    if s is None or m is None or m[0].get("surface_artifact_hash")!=s[1]:raise ValueError("volatility-duration surface missing")
    return s[0],s[1],m[1]
def execute_volatility_duration_job(*,repository_root:str|Path,execution:RunningJobExecution)->VolatilityDurationJobPacket:
    ...
    if binding.get("mission_id")!=MISSION_ID or binding.get("study_id")!=STUDY_ID or not isinstance(subject,dict) or subject.get("id") not in executable_configuration_map():raise ValueError("volatility-duration binding invalid")
    eid=subject["id"];plan=build_volatility_duration_validation_plan(eid);env=build_environment_manifest();ph=sha256(canonical_bytes(plan)).hexdigest();names=output_names(eid);inputs=tuple(spec["input_hashes"]);required={DATASET_SHA256,OBSERVED_MATERIAL_ID,ROLLING_SPLIT_SHA256,ph,volatility_duration_implementation_sha256(),loader_implementation_sha256(),discovery_implementation_sha256()}
    if not required.issubset(inputs):raise ValueError("volatility-duration inputs missing")
    expected=set(spec["expected_outputs"]);produces=surface_output_name() in expected
    if produces:
        surface=compute_registered_volatility_duration_surface(root);sh=w.evidence.finalize(canonical_bytes(surface)).sha256;mh=w.evidence.finalize(canonical_bytes({"schema":"volatility_duration_surface_manifest.v1","surface_artifact_hash":sh,"surface_implementation_sha256":volatility_duration_implementation_sha256()})).sha256
    else:surface,sh,mh=_load(w,inputs)
    evaluation=project_volatility_duration_evaluation(surface,job_execution={**execution.payload(),"identity":execution.identity},subject_executable_id=eid,surface_artifact_hash=sh,surface_manifest_hash=mh);eh=w.evidence.finalize(canonical_bytes(evaluation)).sha256;measurement=build_measurement(executable_id=eid,job_id=execution.job_id,job_hash=execution.job_hash,evaluation_artifact_hash=eh,evaluation=evaluation);meash=w.evidence.finalize(canonical_bytes(measurement)).sha256;result=build_result_manifest(executable_id=eid,job_id=execution.job_id,job_hash=execution.job_hash,measurement_artifact_hash=meash);outputs={names["context"]:eh,names["environment"]:w.evidence.finalize(canonical_bytes(env)).sha256,names["measurement"]:meash,names["plan"]:w.evidence.finalize(canonical_bytes(plan)).sha256,names["result"]:w.evidence.finalize(canonical_bytes(result)).sha256}
    if produces:outputs[surface_output_name()]=sh;outputs[surface_manifest_output_name()]=mh
    return VolatilityDurationJobPacket(output_manifest=tuple(sorted(outputs.items())),verdict=planned_verdict(plan,measurement))
__all__=["CALLABLE_IDENTITY","EVIDENCE_DEPTH","EVIDENCE_MODES","PLANNED_CLAIMS","STUDY_ID","build_environment_manifest","build_volatility_duration_validation_plan","execute_volatility_duration_job","output_names","surface_manifest_output_name","surface_output_name"]
