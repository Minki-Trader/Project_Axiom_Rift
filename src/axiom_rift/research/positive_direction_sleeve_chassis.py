...
def positive_direction_sleeve_components() -> tuple[ComponentSpec, ...]:
    ...
    execution = ComponentSpec(display_name="fixed FPMarkets target-direction completed-period spread proxy execution", protocol="execution.fpmarkets_completed_bar_spread_proxy.v3", implementation=_local("simulate_positive_direction_sleeves"), spec={"entry_proxy": "entry_index_minus_1", "exit_proxy": "exit_index_minus_1", "point": "0.01", "stress": "half_effective_spread_each_side"}, semantic_dependencies=(risk.identity,))
