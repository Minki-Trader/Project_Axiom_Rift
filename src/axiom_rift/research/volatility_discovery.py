"""Causal volatility-transition discovery on Foundation-safe US100 M5 data.
...
SELECTION_TOTAL_EXPOSURES = 66
SELECTION_SEED = 612_337_279
SELECTION_MONTE_CARLO_CONFIDENCE_PPM = 990_000

_PROFILE_SPECS: dict[str, dict[str, int | str]] = {
    "range_ratio_12_96": {
        "measure": "normalized_true_range_mean_ratio",
        "recent_window_bars": 12,
        "baseline_window_bars": 96,
    },
    "rv_ratio_24_120": {
        "measure": "log_return_realized_volatility_ratio",
        "recent_window_bars": 24,
        "baseline_window_bars": 120,
    },
    "jump_ratio_6_72": {
        "measure": "normalized_true_range_max_to_median_ratio",
        "recent_window_bars": 6,
        "baseline_window_bars": 72,
    },
}
_VOLATILITY_FILE = Path(__file__).resolve()


class VolatilityBoundaryError(DiscoveryBoundaryError):
    """Raised before unregistered volatility semantics can enter evaluation."""
...
def volatility_implementation_sha256() -> str:
    return _file_sha256(_VOLATILITY_FILE)
...
class VolatilityConfiguration:
    ...
    def __post_init__(self) -> None:
        if self.profile not in _PROFILE_SPECS:
            raise ValueError("volatility transition profile is not registered")
    ...
    def configuration_id(self) -> str:
        sign = "fade" if self.signal_sign == -1 else "breakout"
...
def volatility_configurations() -> tuple[VolatilityConfiguration, ...]:
    return tuple(
        VolatilityConfiguration(profile, signal_sign, holding_bars)
        for profile in (
            "range_ratio_12_96",
            "rv_ratio_24_120",
            "jump_ratio_6_72",
...
def _local_implementation(function_name: str) -> str:
    return (
        f"axiom_rift.research.volatility_discovery.{function_name}@sha256:"
...
def volatility_components() -> tuple[ComponentSpec, ...]:
    return (
        ComponentSpec(
            display_name="causal completed-bar volatility-transition score",
            protocol="feature.completed_bar_volatility_transition.v1",
            implementation=_local_implementation("compute_volatility_transition_score"),
            spec={
                "availability": "bar_open_plus_5m_after_completed_bar",
                "baseline": "disjoint_completed_bar_history_preceding_recent_window",
                "direction": "sign_of_recent_completed_log_return",
                "expansion_only": True,
        ...
        ComponentSpec(
            display_name="fold isolated sparse volatility-transition selector",
        ...
        ComponentSpec(
            display_name="completed-bar next-open volatility breakout-fade entry",
            protocol="trade.completed_bar_next_open_volatility_transition.v1",
            ...
            spec={
                ...
                "direction": "breakout_or_fade_times_recent_move_sign",
...
def volatility_executable(configuration: VolatilityConfiguration) -> ExecutableSpec:
    return ExecutableSpec(
        display_name=f"volatility-transition contrast {configuration.configuration_id}",
        ...
        engine_contract=(
            "engine:volatility_discovery_v1:python3_13_9:numpy2_3_4:"
            "pandas2_3_3:scipy1_16_3:"
            f"volatility_sha256_{volatility_implementation_sha256()}:"
...
def volatility_executable_configuration_map() -> dict[str, VolatilityConfiguration]:
    return {
        volatility_executable(configuration).identity: configuration
        for configuration in volatility_configurations()
    }


def executable_configuration_map() -> dict[str, VolatilityConfiguration]:
    return volatility_executable_configuration_map()


def compute_volatility_transition_score(
    frame: pd.DataFrame,
    profile: str,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Return signed expansion intensity using completed bars only.

    Each profile compares a recent realized-volatility statistic with a
    disjoint older baseline.  Compression observations are zero, while an
    expansion is signed by the completed recent price move.  The paired
    breakout/fade Executables therefore differ only in trade direction.
    """
    ...
    if (
        np.any(~np.isfinite(close))
        or np.any(~np.isfinite(high))
        or np.any(~np.isfinite(low))
        or np.any(close <= 0)
        or np.any(high < low)
    ):
        raise ValueError("OHLC values must be finite and valid")
    ...
    if measure == "normalized_true_range_mean_ratio":
        series = pd.Series(normalized_range)
        recent = series.rolling(recent_window, min_periods=recent_window).mean()
        baseline = series.shift(recent_window).rolling(
            baseline_window, min_periods=baseline_window
        ).mean()
    elif measure == "log_return_realized_volatility_ratio":
        series = pd.Series(log_return)
        recent = series.rolling(recent_window, min_periods=recent_window).std(ddof=1)
        baseline = series.shift(recent_window).rolling(
            baseline_window, min_periods=baseline_window
        ).std(ddof=1)
    elif measure == "normalized_true_range_max_to_median_ratio":
        series = pd.Series(normalized_range)
        recent = series.rolling(recent_window, min_periods=recent_window).max()
        baseline = series.shift(recent_window).rolling(
            baseline_window, min_periods=baseline_window
        ).median()
    else:  # pragma: no cover - closed by registered profile construction
        raise VolatilityBoundaryError("volatility measure is not registered")
    recent_values = recent.to_numpy(dtype=float)
    baseline_values = baseline.to_numpy(dtype=float)
    ratio = np.divide(
        recent_values,
        baseline_values,
        out=np.full(len(close), np.nan),
        where=np.isfinite(baseline_values) & (baseline_values > 0),
    )
    intensity = np.maximum(0.0, np.log(ratio))
    recent_move = np.full(len(close), np.nan)
    recent_move[recent_window:] = (
        log_close[recent_window:] - log_close[:-recent_window]
    )
    score = np.sign(recent_move) * intensity
    volatility = (
...
class _ConfigurationResult:
    configuration: VolatilityConfiguration
    executable_id: str
...
def _evaluate_configuration(
    *,
    ...
    score, volatility, run = compute_volatility_transition_score(
    ...
    for fold in folds:
        ...
        if len(train_volatility) < 1000:
            raise VolatilityBoundaryError("regime calibration is too small")
        cutoffs = (
        ...
        prefix_score, prefix_volatility, prefix_run = compute_volatility_transition_score(
            prefix_frame, configuration.profile
        )
    ...
    return _ConfigurationResult(
        configuration,
        volatility_executable(configuration).identity,
        metrics,
        fold_metrics,
        regime_metrics,
        session_metrics,
...
def _adjusted_bootstrap_upper_pvalue(values: np.ndarray, *, seed_label: str) -> int:
    ...
    if len(sample) < 30 or np.any(~np.isfinite(sample)):
        raise VolatilityBoundaryError("bootstrap series is invalid or too short")
...
def _matched_result(
    ...
    *,
    ...
    if len(matches) != 1:
        raise VolatilityBoundaryError("registered control match is not unique")
    return matches[0]
...
def _paired_pvalue(subject: _ConfigurationResult, control: _ConfigurationResult, role: str) -> int:
    if not subject.daily_pnl.index.equals(control.daily_pnl.index):
        raise VolatilityBoundaryError("paired controls have different eligible days")
...
def _populate_pvalues_and_controls(results: Sequence[_ConfigurationResult]) -> None:
    for subject in results:
        ...
        if any(type(value) is not int for value in subject.metrics.values()):
            raise VolatilityBoundaryError("scientific metrics are not fixed-point integers")
...
def _claim_limits() -> list[str]:
    return [
        ...
        "monthly_drawdown_share_gate_is_for_sparse_volatility_transition_surfaces_only",
...
def _compute_registered_volatility_surface(repository_root: str | Path) -> dict[str, Any]:
    if not isinstance(repository_root, (str, Path)):
        raise VolatilityBoundaryError("volatility surface requires a repository path")
    _validate_engine_environment()
    ...
    surface: dict[str, Any] = {
        ...
        "discovery_implementation_sha256": volatility_implementation_sha256(),
        "engine_environment": {
        ...
        "schema": "volatility_discovery_surface.v1",
        "selection_context": [
            {
...
def project_volatility_evaluation(
    ...
    *,
    ...
    for name, digest in (
        ...
        if type(digest) is not str or len(digest) != 64 or any(c not in "0123456789abcdef" for c in digest):
            raise VolatilityBoundaryError(f"{name} hash is invalid")
    if not isinstance(job_execution, Mapping) or set(job_execution) != {
        "identity", "job_hash", "job_id", "job_permit_id", "start_record_id"
    }:
        raise VolatilityBoundaryError("Job execution binding is invalid")
    ...
    if job_execution["identity"] != canonical_digest(domain="running-job-execution", payload=payload):
        raise VolatilityBoundaryError("Job execution identity is invalid")
    value = dict(surface)
    if sha256(canonical_bytes(value)).hexdigest() != surface_artifact_hash:
        raise VolatilityBoundaryError("surface bytes differ from their artifact hash")
    expected_fields = {
    ...
    if set(value) != expected_fields or value.get("schema") != "volatility_discovery_surface.v1":
        raise VolatilityBoundaryError("volatility surface schema is invalid")
    expected = volatility_executable_configuration_map()
    evaluations = value.get("evaluations")
    if not isinstance(evaluations, list) or len(evaluations) != len(expected):
        raise VolatilityBoundaryError("volatility surface evaluation count is invalid")
    ...
    if len(by_identity) != len(evaluations) or set(by_identity) != set(expected) or subject_executable_id not in expected:
        raise VolatilityBoundaryError("volatility surface subjects differ from registration")
    for identity, configuration in expected.items():
        if by_identity[identity].get("subject_configuration_id") != configuration.configuration_id:
            raise VolatilityBoundaryError("volatility surface configuration binding differs")
    evaluation = {
        **dict(by_identity[subject_executable_id]),
        "claim_limits": value["claim_limits"],
        "job_execution": dict(job_execution),
        "schema": "volatility_discovery_evaluation.v1",
...
__all__ = [
    ...
    "VolatilityBoundaryError", "VolatilityConfiguration", "SELECTOR_QUANTILE_BP",
    "SELECTION_BLOCK_LENGTHS", "SELECTION_BOOTSTRAP_SAMPLES",
    "SELECTION_MONTE_CARLO_CONFIDENCE_PPM", "SELECTION_SEED",
    "SELECTION_TOTAL_EXPOSURES", "_compute_registered_volatility_surface",
    "calibrate_selector", "compute_volatility_transition_score", "executable_configuration_map",
    "loader_implementation_sha256", "project_volatility_evaluation",
    "volatility_components", "volatility_configurations", "volatility_executable",
    "volatility_executable_configuration_map", "volatility_implementation_sha256",
