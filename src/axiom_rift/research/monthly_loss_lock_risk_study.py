"""Writer-gated evidence Job for the causal monthly loss-lock contrast."""
...
from axiom_rift.research.monthly_loss_lock_risk_chassis import executable_configuration_map, loader_implementation_sha256, monthly_loss_lock_risk_chassis_implementation_sha256
from axiom_rift.research.monthly_loss_lock_risk_discovery import compute_registered_monthly_loss_lock_risk_surface, monthly_loss_lock_risk_discovery_implementation_sha256, project_monthly_loss_lock_risk_evaluation
...
CALLABLE_IDENTITY = "axiom_rift.research.monthly_loss_lock_risk_study.execute_monthly_loss_lock_risk_job.v1"
...
def build_monthly_loss_lock_risk_validation_plan(executable_id: str, *, mission_id: str) -> dict[str, object]:
...
def build_measurement(*, executable_id: str, job_id: str, job_hash: str, evaluation_artifact_hash: str, evaluation: Mapping[str, Any], mission_id: str) -> dict[str, object]:
    value = {"claims": list(PLANNED_CLAIMS), "evidence_depth": EVIDENCE_DEPTH, "evidence_modes": list(EVIDENCE_MODES), "evaluation_artifact_hash": evaluation_artifact_hash, "executable_id": executable_id, "job_hash": job_hash, "job_id": job_id, "metrics": _claim_metrics(evaluation), "mission_id": mission_id, "schema": SCIENTIFIC_MEASUREMENT_SCHEMA}
    canonical_bytes(value)
    return value
...
class MonthlyLossLockRiskJobPacket:
...
def _load_surface(writer: StateWriter, input_hashes: tuple[str, ...]) -> tuple[dict[str, Any], str, str]:
    ...
    for artifact_hash in input_hashes:
        try:
            ...
            continue
        if isinstance(value, dict) and value.get("schema") == "monthly_loss_lock_risk_surface.v1":
            surface = (value, artifact_hash)
        if isinstance(value, dict) and value.get("schema") == "monthly_loss_lock_risk_surface_manifest.v1":
            manifest = (value, artifact_hash)
    if surface is None or manifest is None or manifest[0].get("surface_artifact_hash") != surface[1]:
        raise ValueError("monthly loss lock risk surface is missing")
    return surface[0], surface[1], manifest[1]


def execute_monthly_loss_lock_risk_job(*, repository_root: str | Path, execution: RunningJobExecution) -> MonthlyLossLockRiskJobPacket:
    ...
    if not isinstance(mission_id, str) or not isinstance(study_id, str) or not isinstance(subject, dict) or subject.get("id") not in executable_configuration_map():
        raise ValueError("monthly loss lock risk binding is invalid")
    executable_id = subject["id"]
    plan = build_monthly_loss_lock_risk_validation_plan(executable_id, mission_id=mission_id)
    ...
    required = {DATASET_SHA256, OBSERVED_MATERIAL_ID, ROLLING_SPLIT_SHA256, plan_hash, monthly_loss_lock_risk_chassis_implementation_sha256(), monthly_loss_lock_risk_discovery_implementation_sha256(), volatility_clock_label_chassis_implementation_sha256(), event_label_implementation_sha256(), loader_implementation_sha256(), discovery_implementation_sha256()}
    if not required.issubset(inputs):
        raise ValueError("monthly loss lock risk inputs are missing")
    expected = set(spec["expected_outputs"])
    produces_surface = surface_output_name(study_id=study_id) in expected
    if produces_surface:
        surface = compute_registered_monthly_loss_lock_risk_surface(root)
        surface_hash = writer.evidence.finalize(canonical_bytes(surface)).sha256
        manifest_value = {"schema": "monthly_loss_lock_risk_surface_manifest.v1", "surface_artifact_hash": surface_hash, "surface_implementation_sha256": monthly_loss_lock_risk_discovery_implementation_sha256()}
    ...
    evaluation = project_monthly_loss_lock_risk_evaluation(surface, job_execution={**execution.payload(), "identity": execution.identity}, subject_executable_id=executable_id, surface_artifact_hash=surface_hash, surface_manifest_hash=manifest_hash)
    ...
    return MonthlyLossLockRiskJobPacket(output_manifest=tuple(sorted(outputs.items())), verdict=planned_verdict(plan, measurement))


__all__ = ["CALLABLE_IDENTITY", "CRITERIA", "EVIDENCE_DEPTH", "EVIDENCE_MODES", "PLANNED_CLAIMS", "build_environment_manifest", "build_monthly_loss_lock_risk_validation_plan", "execute_monthly_loss_lock_risk_job", "output_names", "surface_manifest_output_name", "surface_output_name"]
