"""Writer-gated evidence Job for causal spread-aware entry abstention."""
...
CALLABLE_IDENTITY = (
    "axiom_rift.research.cost_aware_execution_study."
    "execute_cost_aware_execution_job.v1"
)
EVIDENCE_DEPTH = "discovery"
_CONTROL_DELTA_METRIC = "execution_control_delta_net_profit_micropoints"
_CONTROL_PVALUE_METRIC = "execution_control_pvalue_upper_ppm"
...
def surface_output_name(*, study_id: str) -> str:
    return f"scientific/{study_id}/cost-aware-execution-surface.json"


def surface_manifest_output_name(*, study_id: str) -> str:
    return f"scientific/{study_id}/cost-aware-execution-surface-manifest.json"
...
class CostAwareExecutionJobPacket:
...
def execute_cost_aware_execution_job(
    *,
    ...
    if (
        ...
        raise ValueError("cost-aware execution binding is invalid")
    ...
    if not required.issubset(inputs):
        raise ValueError("cost-aware execution inputs are missing")
    expected = set(spec["expected_outputs"])
    produces_surface = surface_output_name(study_id=study_id) in expected
    if produces_surface:
        surface = compute_registered_cost_aware_execution_surface(root)
        surface_hash = writer.evidence.finalize(canonical_bytes(surface)).sha256
        manifest_value = {
            "schema": "cost_aware_execution_surface_manifest.v1",
            "surface_artifact_hash": surface_hash,
            "surface_implementation_sha256": cost_aware_execution_implementation_sha256(),
        }
    ...
    return CostAwareExecutionJobPacket(
...
__all__ = [
    ...
    "execute_cost_aware_execution_job",
    "output_names",
