"""Writer-gated evidence Job for the cost-aware utility objective."""
...
from axiom_rift.research.cost_utility_objective_discovery import (
    compute_registered_cost_utility_objective_surface,
    cost_utility_objective_implementation_sha256,
    executable_configuration_map,
    loader_implementation_sha256,
    project_cost_utility_objective_evaluation,
)
...
MISSION_ID = "MIS-0006"
STUDY_ID = "STU-0069"
CALLABLE_IDENTITY = "axiom_rift.research.cost_utility_objective_study.execute_cost_utility_objective_job.v1"
EVIDENCE_DEPTH = "discovery"


def build_cost_utility_objective_validation_plan(executable_id: str) -> dict[str, object]:
    return build_validation_plan(
        mission_id=MISSION_ID, executable_id=executable_id, evidence_depth=EVIDENCE_DEPTH,
        planned_claims=PLANNED_CLAIMS, evidence_modes=EVIDENCE_MODES, criteria=CRITERIA,
...
def surface_output_name() -> str:
    return f"scientific/{STUDY_ID}/cost-utility-objective-surface.json"


def surface_manifest_output_name() -> str:
    return f"scientific/{STUDY_ID}/cost-utility-objective-surface-manifest.json"
...
def build_environment_manifest() -> dict[str, object]:
    value = {
        ...
        "dataset_sha256": DATASET_SHA256, "event_label_implementation_sha256": event_label_implementation_sha256(),
        ...
        "schema": "scientific_engine_environment.v1", "scipy_version": scipy.__version__,
        "shared_discovery_implementation_sha256": discovery_implementation_sha256(),
        "split_artifact_sha256": ROLLING_SPLIT_SHA256, "validator_id": SCIENTIFIC_DISCOVERY_VALIDATOR_ID,
...
class CostUtilityObjectiveJobPacket:
...
def _load_surface(writer: StateWriter, input_hashes: tuple[str, ...]) -> tuple[dict[str, Any], str, str]:
    ...
    for artifact_hash in input_hashes:
        try:
            ...
            continue
        if isinstance(value, dict) and value.get("schema") == "cost_utility_objective_surface.v1":
            surface = (value, artifact_hash)
        if isinstance(value, dict) and value.get("schema") == "cost_utility_objective_surface_manifest.v1":
            manifest = (value, artifact_hash)
    if surface is None or manifest is None or manifest[0].get("surface_artifact_hash") != surface[1]:
        raise ValueError("cost-utility objective surface is missing")
    return surface[0], surface[1], manifest[1]


def execute_cost_utility_objective_job(*, repository_root: str | Path, execution: RunningJobExecution) -> CostUtilityObjectiveJobPacket:
    ...
    if binding.get("mission_id") != MISSION_ID or binding.get("study_id") != STUDY_ID or not isinstance(subject, dict) or subject.get("id") not in executable_configuration_map():
        raise ValueError("cost-utility objective binding is invalid")
    executable_id = subject["id"]
    plan = build_cost_utility_objective_validation_plan(executable_id)
    ...
    required = {DATASET_SHA256, OBSERVED_MATERIAL_ID, ROLLING_SPLIT_SHA256, plan_hash, cost_utility_objective_implementation_sha256(), event_label_implementation_sha256(), loader_implementation_sha256(), discovery_implementation_sha256()}
    if not required.issubset(inputs):
        raise ValueError("cost-utility objective inputs are missing")
    produces_surface = surface_output_name() in set(spec["expected_outputs"])
    if produces_surface:
        surface = compute_registered_cost_utility_objective_surface(root)
        surface_hash = writer.evidence.finalize(canonical_bytes(surface)).sha256
        manifest_hash = writer.evidence.finalize(canonical_bytes({"schema": "cost_utility_objective_surface_manifest.v1", "surface_artifact_hash": surface_hash, "surface_implementation_sha256": cost_utility_objective_implementation_sha256()})).sha256
    else:
        surface, surface_hash, manifest_hash = _load_surface(writer, inputs)
    evaluation = project_cost_utility_objective_evaluation(surface, job_execution={**execution.payload(), "identity": execution.identity}, subject_executable_id=executable_id, surface_artifact_hash=surface_hash, surface_manifest_hash=manifest_hash)
    ...
    return CostUtilityObjectiveJobPacket(tuple(sorted(outputs.items())), planned_verdict(plan, measurement))


__all__ = ["CALLABLE_IDENTITY", "EVIDENCE_DEPTH", "EVIDENCE_MODES", "PLANNED_CLAIMS", "STUDY_ID", "build_cost_utility_objective_validation_plan", "build_environment_manifest", "execute_cost_utility_objective_job", "output_names", "surface_manifest_output_name", "surface_output_name"]
