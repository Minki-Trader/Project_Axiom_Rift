...
"""Causal fold-trained residual-sleeve consensus for US100 M5 discovery.
...
SELECTION_TOTAL_EXPOSURES = 222
SELECTION_SEED = 612_337_279
SELECTION_MONTE_CARLO_CONFIDENCE_PPM = 990_000

_CONSENSUS_PROFILES: dict[str, dict[str, str]] = {
    "full_regime_consensus": {
        "low": "volume",
        "middle": "volume_reversion_consensus",
        "high": "volume_volatility_consensus",
    },
...
_COMPOSITE_CONSENSUS_FILE = Path(__file__).resolve()


class CompositeConsensusBoundaryError(DiscoveryBoundaryError):
    """Raised before unregistered composite-consensus semantics enter evaluation."""
...
def composite_consensus_implementation_sha256() -> str:
    return _file_sha256(_COMPOSITE_CONSENSUS_FILE)
...
class CompositeConsensusConfiguration:
    profile: str
    route_sign: int
    holding_bars: int

    def __post_init__(self) -> None:
        if self.profile not in _CONSENSUS_PROFILES:
            raise ValueError("composite consensus profile is not registered")
        if self.route_sign not in {-1, 1}:
            raise ValueError("route_sign must be -1 or 1")
        if self.holding_bars not in {24, 48}:
    ...
    def routes(self) -> dict[str, str]:
        return dict(_CONSENSUS_PROFILES[self.profile])
...
def composite_consensus_configurations() -> tuple[CompositeConsensusConfiguration, ...]:
    return tuple(
        CompositeConsensusConfiguration(profile, route_sign, holding_bars)
        for profile in (
            "full_regime_consensus",
            "volume_primary_all_regimes",
            "middle_consensus_no_high",
        ...
        for holding_bars in (24, 48)
    )


def _local_implementation(function_name: str) -> str:
    return (
        f"axiom_rift.research.composite_consensus_discovery.{function_name}@sha256:"
        f"{composite_consensus_implementation_sha256()}"
    )
...
def composite_consensus_components() -> tuple[ComponentSpec, ...]:
    return (
        ComponentSpec(
        ...
        ComponentSpec(
            display_name="fold-train volatility-tertile sleeve consensus",
            protocol="synthesis.fold_train_tertile_residual_consensus.v1",
            implementation=_local_implementation("route_consensus_score"),
            spec={
                "consensus_strength": "same_sign_minimum_absolute_normalized_score",
                "entry_rule": (
                    "every_required_sleeve_absolute_normalized_score_at_least_one"
                ),
                "evidence_execution": "new_raw_data_load_no_prior_result_cache",
                "candidate_eligible": False,
                "profiles": {
                    name: dict(routes)
                    for name, routes in _CONSENSUS_PROFILES.items()
                },
                "regime_cutoffs": "fold_train_realized_volatility_tertiles",
                "sleeve_normalization": "fold_train_absolute_97_5th_percentile",
                "test_action": "freeze_cutoffs_and_three_sleeve_thresholds",
                "pass_next_depth": "independent_confirmation_required",
            },
        ),
        ComponentSpec(
            display_name="completed-bar next-open routed or inverted entry",
            protocol="trade.completed_bar_next_open_composite_consensus.v1",
...
def composite_consensus_executable(configuration: CompositeConsensusConfiguration) -> ExecutableSpec:
    return ExecutableSpec(
        display_name=f"composite residual consensus {configuration.configuration_id}",
        components=composite_consensus_components(),
        ...
        engine_contract=(
            "engine:composite_consensus_discovery_v1:python3_13_9:numpy2_3_4:"
            "pandas2_3_3:scipy1_16_3:"
            f"composite_consensus_sha256_{composite_consensus_implementation_sha256()}:"
...
def composite_consensus_executable_configuration_map() -> dict[str, CompositeConsensusConfiguration]:
    return {
        composite_consensus_executable(configuration).identity: configuration
        for configuration in composite_consensus_configurations()
    }


def executable_configuration_map() -> dict[str, CompositeConsensusConfiguration]:
    return composite_consensus_executable_configuration_map()
...
def compute_composite_sleeve_scores(frame: pd.DataFrame) -> _RouterFeatures:
    ...
    if not (
        ...
        raise CompositeConsensusBoundaryError("sleeve gap runs differ")
    ...
    for name, candidate in (
        ...
        if not np.allclose(
            ...
            raise CompositeConsensusBoundaryError(
...
def calibrate_router(
    ...
    if len(train_volatility) < 1000:
        raise CompositeConsensusBoundaryError("consensus regime calibration is too small")
    cutoffs = (
    ...
    if any(not np.isfinite(value) or value <= 0 for value in thresholds.values()):
        raise CompositeConsensusBoundaryError("consensus sleeve threshold is not positive")
    return cutoffs, thresholds


def route_consensus_score(
    features: _RouterFeatures,
    *,
    profile: str,
    ...
    """Apply exact same-sign consensus with frozen fold-train normalization."""

    if profile not in _CONSENSUS_PROFILES:
        raise ValueError("composite consensus profile is not registered")
    if set(sleeve_thresholds) != {"volume", "reversion", "volatility"}:
        raise ValueError("consensus sleeve thresholds are incomplete")
    low, high = volatility_cutoffs
    if not (np.isfinite(low) and np.isfinite(high) and low <= high):
        raise ValueError("consensus volatility cutoffs are invalid")
    normalized = {
    ...
    for regime, action in _CONSENSUS_PROFILES[profile].items():
        if action == "no_trade":
            continue
        if action == "volume":
            candidate = normalized["volume"]
            selected = (
                regimes[regime]
            ...
            continue
        left = normalized["volume"]
        right = normalized[
            "reversion"
            if action == "volume_reversion_consensus"
            else "volatility"
        ]
        selected = (
            regimes[regime]
        ...
        routed[selected] = np.sign(left[selected]) * np.minimum(
            np.abs(left[selected]), np.abs(right[selected])
        )
...
class _ConfigurationResult:
    configuration: CompositeConsensusConfiguration
    executable_id: str
...
def _evaluate_configuration(
    *,
    ...
    for fold in folds:
        ...
        routed_score = route_consensus_score(
            features,
        ...
        prefix_routed_score = route_consensus_score(
            prefix_features,
            profile=configuration.profile,
            volatility_cutoffs=cutoffs,
            sleeve_thresholds=sleeve_thresholds,
    ...
    top5_share = (
        ...
        if gross_positive <= 0
            ...
            1_000_000,
            int(round(1_000_000 * positive_daily.head(5).sum() / gross_positive)),
        )
    )
    regime_metrics: list[dict[str, int | str]] = []
    ...
    return _ConfigurationResult(
        configuration,
        composite_consensus_executable(configuration).identity,
        metrics,
        fold_metrics,
        regime_metrics,
        session_metrics,
...
def _adjusted_bootstrap_upper_pvalue(values: np.ndarray, *, seed_label: str) -> int:
    ...
    if len(sample) < 30 or np.any(~np.isfinite(sample)):
        raise CompositeConsensusBoundaryError("bootstrap series is invalid or too short")
...
def _matched_result(
    ...
    *,
    ...
    if len(matches) != 1:
        raise CompositeConsensusBoundaryError("registered control match is not unique")
    return matches[0]
...
def _paired_pvalue(subject: _ConfigurationResult, control: _ConfigurationResult, role: str) -> int:
    if not subject.daily_pnl.index.equals(control.daily_pnl.index):
        raise CompositeConsensusBoundaryError("paired controls have different eligible days")
...
def _populate_pvalues_and_controls(results: Sequence[_ConfigurationResult]) -> None:
    for subject in results:
        ...
        profile_controls = [
            ...
            for profile in _CONSENSUS_PROFILES
        ...
        if any(type(value) is not int for value in subject.metrics.values()):
            raise CompositeConsensusBoundaryError("scientific metrics are not fixed-point integers")
...
def _claim_limits() -> list[str]:
    return [
        ...
        "monthly_drawdown_share_gate_is_for_sparse_composite_consensus_surfaces_only",
        "consensus_has_no_broker_session_cash_session_or_runtime_authority_claim",
        "shared_in_memory_feature_arrays_are_not_prior_result_cache_reuse",
        "discovery_pass_is_not_candidate_eligible_and_requires_independent_confirmation",
...
def _compute_registered_composite_consensus_surface(repository_root: str | Path) -> dict[str, Any]:
    if not isinstance(repository_root, (str, Path)):
        raise CompositeConsensusBoundaryError("composite_consensus surface requires a repository path")
    _validate_engine_environment()
    ...
    surface: dict[str, Any] = {
        ...
        "discovery_implementation_sha256": composite_consensus_implementation_sha256(),
        "engine_environment": {
        ...
        "schema": "composite_consensus_surface.v1",
        "selection_context": [
            {
...
def project_composite_consensus_evaluation(
    ...
    *,
    ...
    for name, digest in (
        ...
        if type(digest) is not str or len(digest) != 64 or any(c not in "0123456789abcdef" for c in digest):
            raise CompositeConsensusBoundaryError(f"{name} hash is invalid")
    if not isinstance(job_execution, Mapping) or set(job_execution) != {
        "identity", "job_hash", "job_id", "job_permit_id", "start_record_id"
    }:
        raise CompositeConsensusBoundaryError("Job execution binding is invalid")
    ...
    if job_execution["identity"] != canonical_digest(domain="running-job-execution", payload=payload):
        raise CompositeConsensusBoundaryError("Job execution identity is invalid")
    value = dict(surface)
    if sha256(canonical_bytes(value)).hexdigest() != surface_artifact_hash:
        raise CompositeConsensusBoundaryError("surface bytes differ from their artifact hash")
    expected_fields = {
    ...
    if set(value) != expected_fields or value.get("schema") != "composite_consensus_surface.v1":
        raise CompositeConsensusBoundaryError("composite_consensus surface schema is invalid")
    expected = composite_consensus_executable_configuration_map()
    evaluations = value.get("evaluations")
    if not isinstance(evaluations, list) or len(evaluations) != len(expected):
        raise CompositeConsensusBoundaryError("composite_consensus surface evaluation count is invalid")
    ...
    if len(by_identity) != len(evaluations) or set(by_identity) != set(expected) or subject_executable_id not in expected:
        raise CompositeConsensusBoundaryError("composite_consensus surface subjects differ from registration")
    for identity, configuration in expected.items():
        if by_identity[identity].get("subject_configuration_id") != configuration.configuration_id:
            raise CompositeConsensusBoundaryError("composite_consensus surface configuration binding differs")
    evaluation = {
        **dict(by_identity[subject_executable_id]),
        "claim_limits": value["claim_limits"],
        "job_execution": dict(job_execution),
        "schema": "composite_consensus_evaluation.v1",
...
__all__ = [
    ...
    "CompositeConsensusBoundaryError", "CompositeConsensusConfiguration", "SELECTOR_QUANTILE_BP",
    "SELECTION_BLOCK_LENGTHS", "SELECTION_BOOTSTRAP_SAMPLES",
    "SELECTION_MONTE_CARLO_CONFIDENCE_PPM", "SELECTION_SEED",
    "SELECTION_TOTAL_EXPOSURES", "_compute_registered_composite_consensus_surface",
    "calibrate_router", "calibrate_selector", "compute_composite_sleeve_scores",
    "executable_configuration_map",
    "loader_implementation_sha256", "project_composite_consensus_evaluation",
    "composite_consensus_components", "composite_consensus_configurations", "composite_consensus_executable",
    "composite_consensus_executable_configuration_map", "composite_consensus_implementation_sha256",
