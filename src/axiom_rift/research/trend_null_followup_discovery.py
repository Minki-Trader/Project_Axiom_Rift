...
"""Causal trend and deterministic null follow-up for US100 M5 discovery.
...
_PROFILE_SPECS: dict[str, dict[str, int | str]] = {
    ...
    "prior_day_lag_null_24": {
        "lag_bars": 288,
        "lookback_bars": 24,
        "mode": "prior_day_row_lag_null",
    },
    "deterministic_day_phase_null_24": {
        "lag_bars": 0,
        "lookback_bars": 24,
        "mode": "deterministic_day_six_hour_phase_sign",
    },
}
_TREND_NULL_FILE = Path(__file__).resolve()


class TrendNullBoundaryError(DiscoveryBoundaryError):
    """Raised before unregistered trend-null semantics can enter evaluation."""
...
def trend_null_implementation_sha256() -> str:
    return _file_sha256(_TREND_NULL_FILE)
...
class TrendNullConfiguration:
    ...
    def __post_init__(self) -> None:
        if self.profile not in _PROFILE_SPECS:
            raise ValueError("trend-null profile is not registered")
    ...
    def lag_bars(self) -> int:
        return int(_PROFILE_SPECS[self.profile]["lag_bars"])

    @property
    def mode(self) -> str:
        return str(_PROFILE_SPECS[self.profile]["mode"])
    ...
    def configuration_id(self) -> str:
        sign = "reversal" if self.signal_sign == -1 else "continuation"
    ...
    def semantic_parameters(self) -> dict[str, Any]:
        return {
            ...
            "mode": self.mode,
            "null_randomness": "none",
            "signal_sign": self.signal_sign,
            "threshold_quantile_bp": SELECTOR_QUANTILE_BP,
        }


def trend_null_configurations() -> tuple[TrendNullConfiguration, ...]:
    return tuple(
        TrendNullConfiguration(profile, signal_sign, holding_bars)
        for profile in (
            "observed_trend_24",
            "prior_day_lag_null_24",
            "deterministic_day_phase_null_24",
...
def _local_implementation(function_name: str) -> str:
    return (
        f"axiom_rift.research.trend_null_followup_discovery.{function_name}@sha256:"
        f"{trend_null_implementation_sha256()}"
    )
...
def trend_null_components() -> tuple[ComponentSpec, ...]:
    return (
        ComponentSpec(
            display_name="causal completed-bar trend and deterministic null score",
            protocol="feature.completed_bar_trend_null_control.v1",
            implementation=_local_implementation(
                "compute_trend_null_score"
            ),
            spec={
                "availability": "bar_open_plus_5m_after_completed_bar",
                "day_phase_formula": (
                    "minus_one_power_epoch_day_index_plus_six_hour_broker_slot_plus_one"
                ),
                "future_ranking": False,
                "lag_null": "fixed_288_row_shift_of_causal_observed_score",
                "nonconsecutive_action": "source_and_current_lookback_must_rewarm",
                "parameter_fields": [
                    "lag_bars",
                    "lookback_bars",
                    "mode",
                ...
                "python_hash_or_randomness": False,
            },
        ),
        ComponentSpec(
            display_name="fold isolated sparse trend-null selector",
        ...
        ComponentSpec(
            display_name="completed-bar next-open continuation-reversal entry",
            protocol="trade.completed_bar_next_open_trend_null_control.v1",
            ...
            spec={
                ...
                "direction": "continuation_or_reversal_times_score_sign",
...
def trend_null_executable(configuration: TrendNullConfiguration) -> ExecutableSpec:
    return ExecutableSpec(
        display_name=f"trend-null contrast {configuration.configuration_id}",
        components=trend_null_components(),
        ...
        engine_contract=(
            "engine:trend_null_followup_discovery_v1:python3_13_9:numpy2_3_4:"
            "pandas2_3_3:scipy1_16_3:"
            f"trend_null_sha256_{trend_null_implementation_sha256()}:"
...
def trend_null_executable_configuration_map() -> dict[str, TrendNullConfiguration]:
    return {
        trend_null_executable(configuration).identity: configuration
        for configuration in trend_null_configurations()
    }


def executable_configuration_map() -> dict[str, TrendNullConfiguration]:
    return trend_null_executable_configuration_map()


def compute_trend_null_score(
    frame: pd.DataFrame,
    profile: str,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Return the observed 24-bar trend or one exact causal null control."""
    ...
    observed[run < lookback + 1] = np.nan
    if mode == "observed":
        score = observed.copy()
    elif mode == "prior_day_row_lag_null":
        if lag_bars != 288:
            raise TrendNullBoundaryError("prior-day null lag is not registered")
        score = np.full(len(close), np.nan)
        if len(close) > lag_bars:
            source = observed[:-lag_bars]
            valid = (
                np.isfinite(source)
                & (run[:-lag_bars] >= lookback + 1)
                & (run[lag_bars:] >= lookback + 1)
                & (time_ns[lag_bars:] > time_ns[:-lag_bars])
            )
            target = score[lag_bars:]
            target[valid] = source[valid]
    elif mode == "deterministic_day_six_hour_phase_sign":
        day_index = time_ns // (24 * 60 * 60 * 1_000_000_000)
        timestamp = pd.to_datetime(frame["time"], errors="raise")
        minute_of_day = (
            timestamp.dt.hour.to_numpy(dtype=np.int64) * 60
            + timestamp.dt.minute.to_numpy(dtype=np.int64)
        )
        six_hour_slot = minute_of_day // 360
        parity = (day_index + six_hour_slot + 1) % 2
        phase_sign = np.where(parity == 0, 1.0, -1.0)
        score = observed * phase_sign
    else:  # pragma: no cover - closed by registered profile construction
        raise TrendNullBoundaryError("trend-null mode is not registered")
    volatility = (
...
class _ConfigurationResult:
    configuration: TrendNullConfiguration
    executable_id: str
...
def _evaluate_configuration(
    *,
    ...
    score, volatility, run = compute_trend_null_score(
    ...
    for fold in folds:
        ...
        if len(train_volatility) < 1000:
            raise TrendNullBoundaryError("regime calibration is too small")
        cutoffs = (
        ...
        prefix_score, prefix_volatility, prefix_run = (
            compute_trend_null_score(prefix_frame, configuration.profile)
        )
    ...
    return _ConfigurationResult(
        configuration,
        trend_null_executable(configuration).identity,
        metrics,
        fold_metrics,
        regime_metrics,
        session_metrics,
...
def _adjusted_bootstrap_upper_pvalue(values: np.ndarray, *, seed_label: str) -> int:
    ...
    if len(sample) < 30 or np.any(~np.isfinite(sample)):
        raise TrendNullBoundaryError("bootstrap series is invalid or too short")
...
def _matched_result(
    ...
    *,
    ...
    if len(matches) != 1:
        raise TrendNullBoundaryError("registered control match is not unique")
    return matches[0]
...
def _paired_pvalue(subject: _ConfigurationResult, control: _ConfigurationResult, role: str) -> int:
    if not subject.daily_pnl.index.equals(control.daily_pnl.index):
        raise TrendNullBoundaryError("paired controls have different eligible days")
...
def _populate_pvalues_and_controls(results: Sequence[_ConfigurationResult]) -> None:
    for subject in results:
        ...
        if any(type(value) is not int for value in subject.metrics.values()):
            raise TrendNullBoundaryError("scientific metrics are not fixed-point integers")
...
def _claim_limits() -> list[str]:
    return [
        ...
        "monthly_drawdown_share_gate_is_for_sparse_trend_null_surfaces_only",
        "prior_day_null_is_a_fixed_288_row_lag_not_a_wall_clock_day_claim",
        "day_phase_null_is_deterministic_with_no_hash_or_random_state",
...
def _compute_registered_trend_null_surface(repository_root: str | Path) -> dict[str, Any]:
    if not isinstance(repository_root, (str, Path)):
        raise TrendNullBoundaryError(
            "trend-null surface requires a repository path"
        )
    _validate_engine_environment()
    ...
    surface: dict[str, Any] = {
        ...
        "discovery_implementation_sha256": trend_null_implementation_sha256(),
        "engine_environment": {
        ...
        "schema": "trend_null_followup_surface.v1",
        "selection_context": [
            {
...
def project_trend_null_evaluation(
    ...
    *,
    ...
    if job_execution["identity"] != canonical_digest(domain="running-job-execution", payload=payload):
        raise TrendNullBoundaryError("Job execution identity is invalid")
    value = dict(surface)
    if sha256(canonical_bytes(value)).hexdigest() != surface_artifact_hash:
        raise TrendNullBoundaryError("surface bytes differ from their artifact hash")
    expected_fields = {
    ...
    if set(value) != expected_fields or value.get("schema") != "trend_null_followup_surface.v1":
        raise TrendNullBoundaryError("trend-null surface schema is invalid")
    expected = trend_null_executable_configuration_map()
    evaluations = value.get("evaluations")
    if not isinstance(evaluations, list) or len(evaluations) != len(expected):
        raise TrendNullBoundaryError(
            "trend-null surface evaluation count is invalid"
        )
    ...
    if len(by_identity) != len(evaluations) or set(by_identity) != set(expected) or subject_executable_id not in expected:
        raise TrendNullBoundaryError(
            "trend-null surface subjects differ from registration"
        )
...
# Stable follow-up aliases used by the Study runner.  The shorter names above
# remain the domain API specified for this trend-null surface.
trend_null_followup_implementation_sha256 = trend_null_implementation_sha256
trend_null_followup_configurations = trend_null_configurations
trend_null_followup_components = trend_null_components
trend_null_followup_executable = trend_null_executable
trend_null_followup_executable_configuration_map = (
    trend_null_executable_configuration_map
)
_compute_registered_trend_null_followup_surface = (
    _compute_registered_trend_null_surface
)
project_trend_null_followup_evaluation = project_trend_null_evaluation


__all__ = [
    "DATASET_SHA256", "OBSERVED_MATERIAL_ID", "ROLLING_SPLIT_SHA256",
    "TrendNullBoundaryError", "TrendNullConfiguration", "SELECTOR_QUANTILE_BP",
    "SELECTION_BLOCK_LENGTHS", "SELECTION_BOOTSTRAP_SAMPLES",
    "SELECTION_MONTE_CARLO_CONFIDENCE_PPM", "SELECTION_SEED",
    "SELECTION_TOTAL_EXPOSURES", "_compute_registered_trend_null_surface",
    "calibrate_selector", "compute_trend_null_score",
    "executable_configuration_map",
    "loader_implementation_sha256", "project_trend_null_evaluation",
    "trend_null_components", "trend_null_configurations", "trend_null_executable",
    "trend_null_executable_configuration_map", "trend_null_implementation_sha256",
    "trend_null_followup_components", "trend_null_followup_configurations",
    "trend_null_followup_executable",
    "trend_null_followup_executable_configuration_map",
    "trend_null_followup_implementation_sha256",
    "_compute_registered_trend_null_followup_surface",
    "project_trend_null_followup_evaluation",
