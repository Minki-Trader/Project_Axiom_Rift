...
SELECTION_TOTAL_EXPOSURES=500;SELECTOR_QUANTILE_BP=8_500;ENTRY_WINDOW=48;MAX_HOLD=48;STOP_MULTIPLE_MILLI=1000;TAKE_MULTIPLE_MILLI=1500;_PROFILES=("volatility_barrier_48","fixed_hold_control_48");_THIS_FILE=Path(__file__).resolve();_FIVE_MINUTES_NS=300_000_000_000
...
class AdaptiveLifecycleConfiguration:
    ...
    def semantic_parameters(self)->dict[str,Any]:return {"entry_window":ENTRY_WINDOW,"holding_bars":MAX_HOLD,"profile":self.profile,"selector_quantile_bp":SELECTOR_QUANTILE_BP,"signal_sign":self.signal_sign,"stop_multiple_milli":STOP_MULTIPLE_MILLI,"take_multiple_milli":TAKE_MULTIPLE_MILLI}
...
def compute_entry_score(frame:pd.DataFrame,profile:str)->tuple[np.ndarray,np.ndarray,np.ndarray]:
    ...
    close=frame["close"].to_numpy(float);log=np.log(close);ret=np.full(len(close),np.nan);ret[1:]=np.diff(log);vol=pd.Series(ret).rolling(48,min_periods=48).std(ddof=1).to_numpy(float);change=np.full(len(close),np.nan);change[ENTRY_WINDOW:]=log[ENTRY_WINDOW:]-log[:-ENTRY_WINDOW];score=np.divide(change,vol*np.sqrt(ENTRY_WINDOW),out=np.full(len(close),np.nan),where=np.isfinite(vol)&(vol>0));run=_consecutive_run(_time_ns(frame));score[run<49]=np.nan;return score,vol,run
...
def simulate_adaptive_lifecycle(*,frame:pd.DataFrame,score:np.ndarray,volatility:np.ndarray,run:np.ndarray,threshold:float,configuration:AdaptiveLifecycleConfiguration,test_start:pd.Timestamp,test_end:pd.Timestamp,fold_id:str,regime_cutoffs:tuple[float,float],effective_spread:np.ndarray|None=None)->SimulationResult:
    ...
    for decision_index in candidates:
        ...
        if configuration.profile=="volatility_barrier_48":
            expected_move=opens[entry]*float(volatility[decision_index])*sqrt(MAX_HOLD);stop=expected_move*STOP_MULTIPLE_MILLI/1000;take=expected_move*TAKE_MULTIPLE_MILLI/1000
            for candidate in range(entry+1,maximum+1):
                if time_ns[candidate]-time_ns[candidate-1]!=_FIVE_MINUTES_NS:gap=True;break
                move=direction*(opens[candidate]-opens[entry])
                if move>=take or move<=-stop:exit_index=candidate;break
...
def project_adaptive_lifecycle_evaluation(surface:Mapping[str,Any],*,job_execution:Mapping[str,str],subject_executable_id:str,surface_artifact_hash:str,surface_manifest_hash:str)->dict[str,Any]:
    value=dict(surface)
    if sha256(canonical_bytes(value)).hexdigest()!=surface_artifact_hash or value.get("schema")!="adaptive_lifecycle_surface.v1":raise DiscoveryBoundaryError("adaptive-lifecycle surface invalid")
