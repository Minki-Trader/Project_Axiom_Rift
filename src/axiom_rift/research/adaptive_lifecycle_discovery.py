...
def simulate_adaptive_lifecycle(*,frame:pd.DataFrame,score:np.ndarray,volatility:np.ndarray,run:np.ndarray,threshold:float,configuration:AdaptiveLifecycleConfiguration,test_start:pd.Timestamp,test_end:pd.Timestamp,fold_id:str,regime_cutoffs:tuple[float,float],effective_spread:np.ndarray|None=None)->SimulationResult:
    ...
    for decision_index in candidates:
        ...
        execution_spreads=completed_bar_execution_spreads(spreads,entry_index=entry,exit_index=exit_index)
        if not execution_spreads.costs_known:unresolved+=1;intents.append((decision_time,entry_time,time.iloc[exit_index],direction,"unknown_cost"));continue
        native,stress=execution_pnl(direction=direction,entry_bid=float(opens[entry]),exit_bid=float(opens[exit_index]),entry_spread_points=execution_spreads.entry_spread_points,exit_spread_points=execution_spreads.exit_spread_points);entry_vol=float(volatility[decision_index]);regime="low" if entry_vol<=regime_cutoffs[0] else "high" if entry_vol>=regime_cutoffs[1] else "middle";records.append({"decision_bar_open_time":time.iloc[decision_index],"decision_time":decision_time,"entry_time":entry_time,"exit_time":time.iloc[exit_index],"direction":direction,"pnl":native,"stress_pnl":stress,"fold_id":fold_id,"regime":regime});intents.append((decision_time,entry_time,time.iloc[exit_index],direction,"executed"))
