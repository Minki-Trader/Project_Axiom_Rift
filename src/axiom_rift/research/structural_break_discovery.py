"""Causal online mean-shift structural-break discovery."""
...
SELECTION_TOTAL_EXPOSURES=468;SELECTOR_QUANTILE_BP=8_500;BASELINE_WINDOW=576;CONTROL_WINDOW=48;HORIZON=12;_PROFILES=("mean_shift_cusum_576","return_z_control_48");_THIS_FILE=Path(__file__).resolve()
...
def compute_structural_break_score(frame:pd.DataFrame,profile:str)->tuple[np.ndarray,np.ndarray,np.ndarray]:
    ...
    close=frame["close"].to_numpy(float);ret=np.full(len(close),np.nan);ret[1:]=np.diff(np.log(close));series=pd.Series(ret);baseline_mean=series.shift(1).rolling(BASELINE_WINDOW,min_periods=BASELINE_WINDOW).mean().to_numpy(float);baseline_std=series.shift(1).rolling(BASELINE_WINDOW,min_periods=BASELINE_WINDOW).std(ddof=1).to_numpy(float);z=np.divide(ret-baseline_mean,baseline_std,out=np.full(len(close),np.nan),where=np.isfinite(baseline_std)&(baseline_std>0));run=_consecutive_run(_time_ns(frame));score=np.full(len(close),np.nan)
    if profile=="mean_shift_cusum_576":
        pos=0.0;neg=0.0
        for i,value in enumerate(z):
            if not np.isfinite(value) or run[i]<2:pos=0.0;neg=0.0;continue
            pos=max(0.0,pos+value-0.25);neg=min(0.0,neg+value+0.25);score[i]=pos if pos>=abs(neg) else neg
    else:
        total=series.rolling(CONTROL_WINDOW,min_periods=CONTROL_WINDOW).sum().to_numpy(float);score=np.divide(total,baseline_std*np.sqrt(CONTROL_WINDOW),out=np.full(len(close),np.nan),where=np.isfinite(baseline_std)&(baseline_std>0))
...
def project_structural_break_evaluation(surface:Mapping[str,Any],*,job_execution:Mapping[str,str],subject_executable_id:str,surface_artifact_hash:str,surface_manifest_hash:str)->dict[str,Any]:
    value=dict(surface)
    if sha256(canonical_bytes(value)).hexdigest()!=surface_artifact_hash or value.get("schema")!="structural_break_surface.v1":raise DiscoveryBoundaryError("structural-break surface invalid")
    ...
    result={**dict(by[subject_executable_id]),"claim_limits":value["claim_limits"],"job_execution":dict(job_execution),"schema":"structural_break_evaluation.v1","selection_context":value["selection_context"],"selection_method":value["selection_method"],"session_semantics":value["session_semantics"],"surface_artifact_hash":surface_artifact_hash,"surface_manifest_hash":surface_manifest_hash};canonical_bytes(result);return result
