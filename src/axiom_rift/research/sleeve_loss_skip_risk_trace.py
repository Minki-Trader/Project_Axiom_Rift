"""Prospective atomic trace producer for the sleeve loss-skip risk pair."""
...
def _iso(value: object) -> str:
    ...
    if timestamp.tzinfo is not None:
        raise ValueError("sleeve loss-skip timestamps must be naive")
...
def build_sleeve_loss_skip_risk_protocol_definition(
    ...
    members = tuple(
        ProspectivePairMember(
            ...
            executable_id=sleeve_loss_skip_risk_executable(configuration).identity,
            ordinal=index,
        )
        ...
        sorted(
            {
                ...
                "sleeve_loss_skip_chassis_sha256": sleeve_loss_skip_risk_chassis_implementation_sha256(),
                "sleeve_loss_skip_trace_sha256": sleeve_loss_skip_risk_trace_implementation_sha256(),
            }.items()
        )
    )
    ...
    *,
    ...
    for fold in folds:
        ...
        if (
            ...
            raise ValueError("sleeve loss-skip calibration changed under append")
...
def _array_bundle_sha256(
    ...
    for name, raw in values:
        ...
        if np.issubdtype(array.dtype, np.floating):
            ...
            raise ValueError("sleeve loss-skip invariance array type is invalid")
...
def _trade_observation(
    ...
    *,
    ...
    if (entry_spread + exit_spread) % 2:
        raise ValueError("sleeve loss-skip stress cost is not integral")
    ...
    if (
        ...
        raise ValueError("sleeve loss-skip trade rounding is inconsistent")
    return {
...
def compute_sleeve_loss_skip_risk_trace(
    ...
    *,
    ...
    for configuration in sleeve_loss_skip_risk_configurations():
        executable_id = sleeve_loss_skip_risk_executable(configuration).identity
        for fold in folds:
            fold_id = str(fold["fold_id"])
            test = fold["test_oos"]
            ...
            simulation = simulate_sleeve_loss_skip_risk(
                frame=frame,
                score=full_score,
            ...
            prefix_simulation = simulate_sleeve_loss_skip_risk(
                frame=prefix_frame,
                score=prefix_score,
    ...
    trace = {
        "adapter_implementation_sha256": sleeve_loss_skip_risk_trace_implementation_sha256(),
        "attribution": {
