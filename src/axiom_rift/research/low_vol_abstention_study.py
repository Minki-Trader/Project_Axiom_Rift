...
from axiom_rift.research.low_vol_abstention_chassis import executable_configuration_map, loader_implementation_sha256, low_vol_abstention_chassis_implementation_sha256
from axiom_rift.research.low_vol_abstention_discovery import compute_registered_low_vol_abstention_surface, project_low_vol_abstention_evaluation, low_vol_abstention_discovery_implementation_sha256
...
CALLABLE_IDENTITY = "axiom_rift.research.low_vol_abstention_study.execute_low_vol_abstention_job.v1"
EVIDENCE_DEPTH = "discovery"
_DELTA = "session_dense_control_delta_net_profit_micropoints"
_PVALUE = "session_dense_control_pvalue_upper_ppm"
CRITERIA = discovery_criteria(control_delta_metric=_DELTA, control_pvalue_metric=_PVALUE, include_opposite_sign=False)


def build_low_vol_abstention_validation_plan(executable_id: str, *, mission_id: str) -> dict[str, object]:
...
class LowVolAbstentionJobPacket:
...
def _load(writer: StateWriter, inputs: tuple[str, ...]) -> tuple[dict[str, Any], str, str]:
    ...
    for artifact_hash in inputs:
        try:
            ...
            continue
        if isinstance(value, dict) and value.get("schema") == "low_vol_abstention_surface.v1":
            surface = (value, artifact_hash)
        if isinstance(value, dict) and value.get("schema") == "low_vol_abstention_surface_manifest.v1":
            manifest = (value, artifact_hash)
    if surface is None or manifest is None or manifest[0].get("surface_artifact_hash") != surface[1]:
        raise ValueError("low volatility abstention surface missing")
    return surface[0], surface[1], manifest[1]


def execute_low_vol_abstention_job(*, repository_root: str | Path, execution: RunningJobExecution) -> LowVolAbstentionJobPacket:
    ...
    if not isinstance(mission_id, str) or not isinstance(study_id, str) or not isinstance(subject, dict) or subject.get("id") not in executable_configuration_map():
        raise ValueError("low volatility abstention binding invalid")
    executable_id = subject["id"]
    plan = build_low_vol_abstention_validation_plan(executable_id, mission_id=mission_id)
    ...
    required = {DATASET_SHA256, OBSERVED_MATERIAL_ID, ROLLING_SPLIT_SHA256, plan_hash, low_vol_abstention_chassis_implementation_sha256(), low_vol_abstention_discovery_implementation_sha256(), loader_implementation_sha256(), discovery_implementation_sha256()}
    if not required.issubset(inputs):
        raise ValueError("low volatility abstention inputs missing")
    produces = surface_output_name(study_id=study_id) in set(spec["expected_outputs"])
    if produces:
        surface = compute_registered_low_vol_abstention_surface(root)
        surface_hash = writer.evidence.finalize(canonical_bytes(surface)).sha256
        manifest_value = {"schema": "low_vol_abstention_surface_manifest.v1", "surface_artifact_hash": surface_hash, "surface_implementation_sha256": low_vol_abstention_discovery_implementation_sha256()}
    ...
    evaluation = project_low_vol_abstention_evaluation(surface, job_execution={**execution.payload(), "identity": execution.identity}, subject_executable_id=executable_id, surface_artifact_hash=surface_hash, surface_manifest_hash=manifest_hash)
    ...
    return LowVolAbstentionJobPacket(tuple(sorted(outputs.items())), planned_verdict(plan, measurement))


__all__ = ["CALLABLE_IDENTITY", "CRITERIA", "EVIDENCE_DEPTH", "EVIDENCE_MODES", "PLANNED_CLAIMS", "build_environment_manifest", "build_low_vol_abstention_validation_plan", "execute_low_vol_abstention_job", "output_names", "surface_manifest_output_name", "surface_output_name"]
