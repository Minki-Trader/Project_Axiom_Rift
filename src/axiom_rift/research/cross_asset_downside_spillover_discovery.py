"""Causal US500 downside-spillover discovery on Foundation-safe US100 M5 data.
...
class CrossAssetDownsideSpilloverBoundaryError(DiscoveryBoundaryError):
...
def _require_digest(value: str, name: str) -> str:
    if (
        ...
        raise CrossAssetDownsideSpilloverBoundaryError(
            f"{name} must be a lowercase SHA256 digest"
        )
    return value


def cross_asset_downside_spillover_implementation_sha256() -> str:
...
def _record_body(raw_line: bytes, name: str) -> bytes:
    ...
    if not body:
        raise CrossAssetDownsideSpilloverBoundaryError(f"{name} contains an empty record")
    return body
...
def _scan_us500_source(path: Path, *, expected_raw_sha256: str) -> _SourceScan:
    ...
    if observed != expected:
        prefix.close()
        raise CrossAssetDownsideSpilloverBoundaryError("US500 raw SHA256 changed")
...
@dataclass(frozen=True, slots=True)
class CrossAssetDownsideSpilloverConfiguration:
    profile: str
    route_sign: int
    holding_bars: int
...
def cross_asset_downside_spillover_configurations() -> tuple[CrossAssetDownsideSpilloverConfiguration, ...]:
    return tuple(
        CrossAssetDownsideSpilloverConfiguration(profile, route_sign, holding_bars)
...
def _local_implementation(function_name: str) -> str:
    return (
        "axiom_rift.research.cross_asset_downside_spillover_discovery."
        f"{function_name}@sha256:{cross_asset_downside_spillover_implementation_sha256()}"
    )
...
def cross_asset_downside_spillover_components(raw_sha256: str) -> tuple[ComponentSpec, ...]:
    raw = _require_digest(raw_sha256, "US500 raw hash")
    source = _source_identity_payload()
    source_id = source["source_contract_id"]
    return (
        ComponentSpec(
            display_name="exact causal US500 downside-volatility spillover",
            protocol=(
...
def cross_asset_downside_spillover_executable(
    configuration: CrossAssetDownsideSpilloverConfiguration,
    raw_sha256: str,
) -> ExecutableSpec:
    if not isinstance(configuration, CrossAssetDownsideSpilloverConfiguration):
        raise TypeError("configuration must be CrossAssetDownsideSpilloverConfiguration")
    raw = _require_digest(raw_sha256, "US500 raw hash")
    source = _source_identity_payload()
    return ExecutableSpec(
        display_name=f"cross asset downside spillover {configuration.configuration_id}",
        components=cross_asset_downside_spillover_components(raw),
        parameters={
        ...
        engine_contract=(
            "engine:cross_asset_downside_spillover_discovery_v1:python3_13_9:"
            "numpy2_3_4:pandas2_3_3:scipy1_16_3:"
            f"implementation_{cross_asset_downside_spillover_implementation_sha256()}:"
...
def _configuration_map_for_raw(raw_sha256: str) -> dict[str, CrossAssetDownsideSpilloverConfiguration]:
    return {
        cross_asset_downside_spillover_executable(configuration, raw_sha256).identity: configuration
        for configuration in cross_asset_downside_spillover_configurations()
    }


def cross_asset_downside_spillover_executable_configuration_map(
    repository_root: str | Path,
) -> dict[str, CrossAssetDownsideSpilloverConfiguration]:
...
def executable_configuration_map(
    ...
    return cross_asset_downside_spillover_executable_configuration_map(repository_root)
...
def _join_exact(target_frame: pd.DataFrame, source_frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if not required_target.issubset(target_frame.columns) or set(source_frame.columns) != {"time", "close"}:
        raise CrossAssetDownsideSpilloverBoundaryError("cross-asset frame schema is invalid")
    ...
    for frame, name in ((target, "US100"), (source, "US500")):
        ...
        if frame["time"].duplicated().any() or not frame["time"].is_monotonic_increasing:
            raise CrossAssetDownsideSpilloverBoundaryError(f"{name} timestamps are invalid")
    ...
    if joined.empty or joined["time"].duplicated().any() or not joined["time"].is_monotonic_increasing:
        raise CrossAssetDownsideSpilloverBoundaryError("exact cross-asset join is empty or invalid")
...
def compute_downside_spillover_features(
    ...
    source_close = pd.to_numeric(
        joined_frame["us500_close"], errors="raise"
    ).to_numpy(dtype=float)
    source_r6, source_rv12, source_rv96, source_z6, source_expansion = (
        feature_set(source_close, source_run, "us500")
    )
...
def _evaluate_configuration(
    *,
    ...
    executable_id = cross_asset_downside_spillover_executable(configuration, raw_sha256).identity
...
def _surface_from_frames(
    *,
    ...
    surface: dict[str, Any] = {
        ...
        "discovery_implementation_sha256": cross_asset_downside_spillover_implementation_sha256(),
        "engine_environment": {
        ...
        "schema": "cross_asset_downside_spillover_surface.v1",
        "selection_context": [
            {
...
def _compute_registered_cross_asset_downside_spillover_surface(
    repository_root: str | Path,
) -> dict[str, Any]:
...
def project_cross_asset_downside_spillover_evaluation(
    ...
    *,
    ...
    if set(value) != expected_fields or value.get("schema") != "cross_asset_downside_spillover_surface.v1":
        raise CrossAssetDownsideSpilloverBoundaryError(
    ...
    evaluation = {
        **dict(by_identity[subject_executable_id]),
        ...
        "schema": "cross_asset_downside_spillover_evaluation.v1",
...
__all__ = [
    ...
    "OBSERVED_MATERIAL_ID",
    "ROLLING_SPLIT_SHA256",
    "CrossAssetDownsideSpilloverBoundaryError",
    "CrossAssetDownsideSpilloverConfiguration",
    "DownsideSpilloverFeatures",
    ...
    "calibrate_selector",
    "compute_downside_spillover_features",
    "cross_asset_downside_spillover_components",
    "cross_asset_downside_spillover_configurations",
    "cross_asset_downside_spillover_executable",
