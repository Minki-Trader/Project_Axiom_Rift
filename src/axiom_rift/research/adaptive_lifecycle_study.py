...
MISSION_ID="MIS-0005";STUDY_ID="STU-0063";CALLABLE_IDENTITY="axiom_rift.research.adaptive_lifecycle_study.execute_adaptive_lifecycle_job.v1";EVIDENCE_DEPTH="discovery"
...
def _load(w:StateWriter,hashes:tuple[str,...])->tuple[dict[str,Any],str,str]:
    ...
    for h in hashes:
        ...
        if isinstance(v,dict) and v.get("schema")=="adaptive_lifecycle_surface.v1":surface=(v,h)
        if isinstance(v,dict) and v.get("schema")=="adaptive_lifecycle_surface_manifest.v1":manifest=(v,h)
...
def execute_adaptive_lifecycle_job(*,repository_root:str|Path,execution:RunningJobExecution)->AdaptiveLifecycleJobPacket:
    ...
    if produces:
        surface=compute_registered_adaptive_lifecycle_surface(root);sh=w.evidence.finalize(canonical_bytes(surface)).sha256;mh=w.evidence.finalize(canonical_bytes({"schema":"adaptive_lifecycle_surface_manifest.v1","surface_artifact_hash":sh,"surface_implementation_sha256":adaptive_lifecycle_implementation_sha256()})).sha256
