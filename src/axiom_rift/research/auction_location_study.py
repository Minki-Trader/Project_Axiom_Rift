...
MISSION_ID="MIS-0005";STUDY_ID="STU-0058";CALLABLE_IDENTITY="axiom_rift.research.auction_location_study.execute_auction_job.v2";EVIDENCE_DEPTH="discovery"
...
def _load(w:StateWriter,hashes:tuple[str,...])->tuple[dict[str,Any],str,str]:
    ...
    for h in hashes:
        ...
        if isinstance(v,dict) and v.get("schema")=="auction_location_surface.v2":surface=(v,h)
        if isinstance(v,dict) and v.get("schema")=="auction_location_surface_manifest.v2":manifest=(v,h)
...
def execute_auction_job(*,repository_root:str|Path,execution:RunningJobExecution)->AuctionJobPacket:
    ...
    if produces:
        surface=compute_registered_auction_surface(root);sh=w.evidence.finalize(canonical_bytes(surface)).sha256;mh=w.evidence.finalize(canonical_bytes({"schema":"auction_location_surface_manifest.v2","surface_artifact_hash":sh,"surface_implementation_sha256":auction_implementation_sha256()})).sha256
