...
"""Causal realized-volatility residual follow-up for US100 M5 discovery.
...
SELECTION_TOTAL_EXPOSURES = 198
SELECTION_SEED = 612_337_279
SELECTION_MONTE_CARLO_CONFIDENCE_PPM = 990_000

_PROFILE_SPECS = {
    "rv24_120_unfiltered": "unfiltered",
    "rv24_120_middle_volatility_train_tertile": (
        "middle_volatility_train_tertile"
    ),
    "rv24_120_broker15_22": "broker_decision_hour_15_through_22",
}
_VOLATILITY_REGIME_FOLLOWUP_FILE = Path(__file__).resolve()


class VolatilityRegimeFollowupBoundaryError(DiscoveryBoundaryError):
    """Raised before unregistered volatility-regime semantics enter evaluation."""
...
def volatility_regime_followup_implementation_sha256() -> str:
    return _file_sha256(_VOLATILITY_REGIME_FOLLOWUP_FILE)
...
class VolatilityRegimeFollowupConfiguration:
    profile: str
    signal_sign: int
    holding_bars: int
    ...
    def __post_init__(self) -> None:
        if self.profile not in _PROFILE_SPECS:
            raise ValueError("volatility residual profile is not registered")
...
def volatility_regime_followup_configurations() -> tuple[VolatilityRegimeFollowupConfiguration, ...]:
    return tuple(
        VolatilityRegimeFollowupConfiguration(profile, signal_sign, holding_bars)
        for profile in (
            "rv24_120_unfiltered",
            "rv24_120_middle_volatility_train_tertile",
            "rv24_120_broker15_22",
        )
...
def _local_implementation(function_name: str) -> str:
    return (
        f"axiom_rift.research.volatility_regime_followup_discovery.{function_name}@sha256:"
        f"{volatility_regime_followup_implementation_sha256()}"
    )
...
def volatility_regime_followup_components() -> tuple[ComponentSpec, ...]:
    return (
        ComponentSpec(
            display_name="causal rv24-to-120 volatility-transition pressure",
            protocol="feature.completed_bar_rv24_120_transition_residual.v1",
            implementation=_local_implementation(
                "compute_volatility_residual_score"
            ),
            spec={
                "availability": "bar_open_plus_5m_after_completed_bar",
                "base_profile": "rv_ratio_24_120",
                "base_dependency_sha256": volatility_dependency_sha256(),
                "nonconsecutive_action": "invalid_until_full_rewarm",
                "parameter_fields": [
                    "baseline_window_bars",
                    "recent_window_bars",
                ],
                "profiles": {name: "rv_ratio_24_120" for name in _PROFILE_SPECS},
            },
        ),
        ComponentSpec(
        ...
        ComponentSpec(
            display_name="fold isolated sparse volatility-residual selector",
        ...
        ComponentSpec(
            display_name="completed-bar next-open volatility fade-breakout entry",
            protocol="trade.completed_bar_next_open_volatility_residual.v1",
...
def volatility_regime_followup_executable(configuration: VolatilityRegimeFollowupConfiguration) -> ExecutableSpec:
    return ExecutableSpec(
        display_name=f"rv24-120 regime follow-up {configuration.configuration_id}",
        components=volatility_regime_followup_components(),
        ...
        engine_contract=(
            "engine:volatility_regime_followup_discovery_v1:python3_13_9:numpy2_3_4:"
            "pandas2_3_3:scipy1_16_3:"
            f"volatility_regime_followup_sha256_{volatility_regime_followup_implementation_sha256()}:"
...
def volatility_regime_followup_executable_configuration_map() -> dict[str, VolatilityRegimeFollowupConfiguration]:
    return {
        volatility_regime_followup_executable(configuration).identity: configuration
        for configuration in volatility_regime_followup_configurations()
    }


def executable_configuration_map() -> dict[str, VolatilityRegimeFollowupConfiguration]:
    return volatility_regime_followup_executable_configuration_map()


def compute_volatility_residual_score(
    frame: pd.DataFrame,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Return the exact registered rv24-to-120 completed-bar base score."""

    return compute_volatility_transition_score(frame, "rv_ratio_24_120")
...
def apply_profile_gate(
    ...
    *,
    ...
    if gate == "unfiltered":
        ...
        raise VolatilityRegimeFollowupBoundaryError("profile gate is not registered")
    gated = values.copy()
...
class _ConfigurationResult:
    configuration: VolatilityRegimeFollowupConfiguration
    executable_id: str
...
def _evaluate_configuration(
    *,
    ...
    score, volatility, run = compute_volatility_residual_score(frame)
    ...
    for fold in folds:
        ...
        if len(train_volatility) < 1000:
            raise VolatilityRegimeFollowupBoundaryError("regime calibration is too small")
        cutoffs = (
    ...
    return _ConfigurationResult(
        configuration,
        volatility_regime_followup_executable(configuration).identity,
        metrics,
        fold_metrics,
        regime_metrics,
        session_metrics,
...
def _adjusted_bootstrap_upper_pvalue(values: np.ndarray, *, seed_label: str) -> int:
    ...
    if len(sample) < 30 or np.any(~np.isfinite(sample)):
        raise VolatilityRegimeFollowupBoundaryError("bootstrap series is invalid or too short")
...
def _matched_result(
    ...
    *,
    ...
    if len(matches) != 1:
        raise VolatilityRegimeFollowupBoundaryError("registered control match is not unique")
    return matches[0]
...
def _paired_pvalue(subject: _ConfigurationResult, control: _ConfigurationResult, role: str) -> int:
    if not subject.daily_pnl.index.equals(control.daily_pnl.index):
        raise VolatilityRegimeFollowupBoundaryError("paired controls have different eligible days")
...
def _populate_pvalues_and_controls(results: Sequence[_ConfigurationResult]) -> None:
    for subject in results:
        ...
        if any(type(value) is not int for value in subject.metrics.values()):
            raise VolatilityRegimeFollowupBoundaryError("scientific metrics are not fixed-point integers")
...
def _claim_limits() -> list[str]:
    return [
        ...
        "monthly_drawdown_share_gate_is_for_sparse_volatility_residual_surfaces_only",
...
def _compute_registered_volatility_regime_followup_surface(repository_root: str | Path) -> dict[str, Any]:
    if not isinstance(repository_root, (str, Path)):
        raise VolatilityRegimeFollowupBoundaryError("volatility_regime_followup surface requires a repository path")
    _validate_engine_environment()
    ...
    surface: dict[str, Any] = {
        ...
        "discovery_implementation_sha256": volatility_regime_followup_implementation_sha256(),
        "engine_environment": {
        ...
        "schema": "volatility_regime_followup_surface.v1",
        "selection_context": [
            {
...
def project_volatility_regime_followup_evaluation(
    ...
    *,
    ...
    for name, digest in (
        ...
        if type(digest) is not str or len(digest) != 64 or any(c not in "0123456789abcdef" for c in digest):
            raise VolatilityRegimeFollowupBoundaryError(f"{name} hash is invalid")
    if not isinstance(job_execution, Mapping) or set(job_execution) != {
        "identity", "job_hash", "job_id", "job_permit_id", "start_record_id"
    }:
        raise VolatilityRegimeFollowupBoundaryError("Job execution binding is invalid")
    ...
    if job_execution["identity"] != canonical_digest(domain="running-job-execution", payload=payload):
        raise VolatilityRegimeFollowupBoundaryError("Job execution identity is invalid")
    value = dict(surface)
    if sha256(canonical_bytes(value)).hexdigest() != surface_artifact_hash:
        raise VolatilityRegimeFollowupBoundaryError("surface bytes differ from their artifact hash")
    expected_fields = {
    ...
    if set(value) != expected_fields or value.get("schema") != "volatility_regime_followup_surface.v1":
        raise VolatilityRegimeFollowupBoundaryError("volatility_regime_followup surface schema is invalid")
    expected = volatility_regime_followup_executable_configuration_map()
    evaluations = value.get("evaluations")
    if not isinstance(evaluations, list) or len(evaluations) != len(expected):
        raise VolatilityRegimeFollowupBoundaryError("volatility_regime_followup surface evaluation count is invalid")
    ...
    if len(by_identity) != len(evaluations) or set(by_identity) != set(expected) or subject_executable_id not in expected:
        raise VolatilityRegimeFollowupBoundaryError("volatility_regime_followup surface subjects differ from registration")
    for identity, configuration in expected.items():
        if by_identity[identity].get("subject_configuration_id") != configuration.configuration_id:
            raise VolatilityRegimeFollowupBoundaryError("volatility_regime_followup surface configuration binding differs")
    evaluation = {
        **dict(by_identity[subject_executable_id]),
        "claim_limits": value["claim_limits"],
        "job_execution": dict(job_execution),
        "schema": "volatility_regime_followup_evaluation.v1",
...
__all__ = [
    ...
    "VolatilityRegimeFollowupBoundaryError", "VolatilityRegimeFollowupConfiguration", "SELECTOR_QUANTILE_BP",
    "SELECTION_BLOCK_LENGTHS", "SELECTION_BOOTSTRAP_SAMPLES",
    "SELECTION_MONTE_CARLO_CONFIDENCE_PPM", "SELECTION_SEED",
    "SELECTION_TOTAL_EXPOSURES", "_compute_registered_volatility_regime_followup_surface",
    "calibrate_selector", "compute_volatility_residual_score",
    "executable_configuration_map",
    "loader_implementation_sha256", "project_volatility_regime_followup_evaluation",
    "volatility_regime_followup_components", "volatility_regime_followup_configurations", "volatility_regime_followup_executable",
    "volatility_regime_followup_executable_configuration_map", "volatility_regime_followup_implementation_sha256",
    "trend_dependency_sha256", "volatility_dependency_sha256",
]
