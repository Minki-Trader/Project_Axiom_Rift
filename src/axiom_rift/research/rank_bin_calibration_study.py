"""Writer-gated evidence Job for validation rank-bin calibration."""
...
from axiom_rift.research.rank_bin_calibration_discovery import (
    compute_registered_rank_bin_calibration_surface,
    executable_configuration_map,
    loader_implementation_sha256,
    project_rank_bin_calibration_evaluation,
    rank_bin_calibration_implementation_sha256,
)
...
MISSION_ID = "MIS-0006"
STUDY_ID = "STU-0068"
CALLABLE_IDENTITY = "axiom_rift.research.rank_bin_calibration_study.execute_rank_bin_calibration_job.v1"
EVIDENCE_DEPTH = "discovery"


def build_rank_bin_calibration_validation_plan(executable_id: str) -> dict[str, object]:
...
def output_names(executable_id: str) -> dict[str, str]:
    ...
    return {name: f"{prefix}/{filename}" for name, filename in {
        "context": "evaluation.json", "environment": "environment.json",
        "measurement": "measurement.json", "plan": "validation-plan.json", "result": "result.json",
    }.items()}


def surface_output_name() -> str:
    return f"scientific/{STUDY_ID}/rank-bin-calibration-surface.json"


def surface_manifest_output_name() -> str:
    return f"scientific/{STUDY_ID}/rank-bin-calibration-surface-manifest.json"
...
def build_measurement(*, executable_id: str, job_id: str, job_hash: str, evaluation_artifact_hash: str, evaluation: Mapping[str, Any]) -> dict[str, object]:
    value = {
        ...
        "evidence_modes": list(EVIDENCE_MODES), "evaluation_artifact_hash": evaluation_artifact_hash,
...
def build_result_manifest(*, executable_id: str, job_id: str, job_hash: str, measurement_artifact_hash: str) -> dict[str, object]:
    value = {
        ...
        "observations": [{"claim_id": claim, "measurement_artifact_hash": measurement_artifact_hash} for claim in PLANNED_CLAIMS],
...
class RankBinCalibrationJobPacket:
...
def _load_surface(writer: StateWriter, input_hashes: tuple[str, ...]) -> tuple[dict[str, Any], str, str]:
    ...
    for artifact_hash in input_hashes:
        try:
            ...
            continue
        if isinstance(value, dict) and value.get("schema") == "rank_bin_calibration_surface.v1":
            surface = (value, artifact_hash)
        if isinstance(value, dict) and value.get("schema") == "rank_bin_calibration_surface_manifest.v1":
            manifest = (value, artifact_hash)
    if surface is None or manifest is None or manifest[0].get("surface_artifact_hash") != surface[1]:
        raise ValueError("rank-bin calibration surface is missing")
    return surface[0], surface[1], manifest[1]


def execute_rank_bin_calibration_job(*, repository_root: str | Path, execution: RunningJobExecution) -> RankBinCalibrationJobPacket:
    ...
    if binding.get("mission_id") != MISSION_ID or binding.get("study_id") != STUDY_ID or not isinstance(subject, dict) or subject.get("id") not in executable_configuration_map():
        raise ValueError("rank-bin calibration binding is invalid")
    executable_id = subject["id"]
    plan = build_rank_bin_calibration_validation_plan(executable_id)
    ...
    required = {DATASET_SHA256, OBSERVED_MATERIAL_ID, ROLLING_SPLIT_SHA256, plan_hash, rank_bin_calibration_implementation_sha256(), event_label_implementation_sha256(), loader_implementation_sha256(), discovery_implementation_sha256()}
    if not required.issubset(inputs):
        raise ValueError("rank-bin calibration inputs are missing")
    produces_surface = surface_output_name() in set(spec["expected_outputs"])
    if produces_surface:
        surface = compute_registered_rank_bin_calibration_surface(root)
        surface_hash = writer.evidence.finalize(canonical_bytes(surface)).sha256
        manifest_hash = writer.evidence.finalize(canonical_bytes({
            "schema": "rank_bin_calibration_surface_manifest.v1",
            "surface_artifact_hash": surface_hash,
            "surface_implementation_sha256": rank_bin_calibration_implementation_sha256(),
        })).sha256
    else:
        surface, surface_hash, manifest_hash = _load_surface(writer, inputs)
    evaluation = project_rank_bin_calibration_evaluation(
    ...
    measurement = build_measurement(
        executable_id=executable_id, job_id=execution.job_id, job_hash=execution.job_hash,
        evaluation_artifact_hash=evaluation_hash, evaluation=evaluation,
    )
    ...
    return RankBinCalibrationJobPacket(tuple(sorted(outputs.items())), planned_verdict(plan, measurement))


__all__ = [
    "CALLABLE_IDENTITY", "EVIDENCE_DEPTH", "EVIDENCE_MODES", "PLANNED_CLAIMS", "STUDY_ID",
    "build_environment_manifest", "build_rank_bin_calibration_validation_plan",
    "execute_rank_bin_calibration_job", "output_names", "surface_manifest_output_name", "surface_output_name",
]
