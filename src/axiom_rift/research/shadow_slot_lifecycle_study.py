"""Writer-gated evidence Job for the shadow-slot lifecycle contrast."""
...
from axiom_rift.research.shadow_slot_lifecycle_discovery import (
...
CALLABLE_IDENTITY = (
    "axiom_rift.research.shadow_slot_lifecycle_study."
    "execute_shadow_slot_lifecycle_job.v1"
)
EVIDENCE_DEPTH = "discovery"
_CONTROL_DELTA_METRIC = "lifecycle_control_delta_net_profit_micropoints"
_CONTROL_PVALUE_METRIC = "lifecycle_control_pvalue_upper_ppm"
...
def build_shadow_slot_lifecycle_validation_plan(
...
def surface_output_name(*, study_id: str) -> str:
    return f"scientific/{study_id}/shadow-slot-lifecycle-surface.json"


def surface_manifest_output_name(*, study_id: str) -> str:
    return f"scientific/{study_id}/shadow-slot-lifecycle-surface-manifest.json"
...
class ShadowSlotLifecycleJobPacket:
...
def _load_surface(
    ...
    for artifact_hash in input_hashes:
        try:
            ...
            continue
        if isinstance(value, dict) and value.get("schema") == "shadow_slot_lifecycle_surface.v1":
            surface = (value, artifact_hash)
        if isinstance(value, dict) and value.get("schema") == "shadow_slot_lifecycle_surface_manifest.v1":
            manifest = (value, artifact_hash)
    if (
        surface is None or manifest is None
        or manifest[0].get("surface_artifact_hash") != surface[1]
    ):
        raise ValueError("shadow-slot lifecycle surface is missing")
    return surface[0], surface[1], manifest[1]


def execute_shadow_slot_lifecycle_job(
    *, repository_root: str | Path, execution: RunningJobExecution,
) -> ShadowSlotLifecycleJobPacket:
    ...
    if (
        ...
        raise ValueError("shadow-slot lifecycle binding is invalid")
    executable_id = subject["id"]
    plan = build_shadow_slot_lifecycle_validation_plan(
    ...
    evaluation = project_shadow_slot_lifecycle_evaluation(
    ...
    return ShadowSlotLifecycleJobPacket(
...
__all__ = [
    ...
    "build_shadow_slot_lifecycle_validation_plan",
    "execute_shadow_slot_lifecycle_job", "output_names",
