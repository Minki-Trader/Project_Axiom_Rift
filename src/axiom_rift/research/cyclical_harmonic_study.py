"""Writer-gated evidence Job for causal harmonic agreement."""
...
from axiom_rift.research.cyclical_harmonic_discovery import compute_registered_harmonic_surface,harmonic_implementation_sha256,executable_configuration_map,loader_implementation_sha256,project_harmonic_evaluation
from axiom_rift.research.cyclical_phase_discovery import cyclical_implementation_sha256
...
MISSION_ID="MIS-0002";STUDY_ID="STU-0031";CALLABLE_IDENTITY="axiom_rift.research.cyclical_harmonic_study.execute_harmonic_job.v1";EVIDENCE_DEPTH="discovery"
def build_harmonic_validation_plan(eid:str)->dict[str,object]:return build_validation_plan(mission_id=MISSION_ID,executable_id=eid,evidence_depth=EVIDENCE_DEPTH,planned_claims=PLANNED_CLAIMS,evidence_modes=EVIDENCE_MODES,criteria=CRITERIA,candidate_eligible_on_pass=False)
...
def _load(w:StateWriter,hashes:tuple[str,...])->tuple[dict[str,Any],str,str]:
    ...
    for h in hashes:
        ...
        if isinstance(v,dict) and v.get("schema")=="cyclical_harmonic_surface.v1":s=(v,h)
        if isinstance(v,dict) and v.get("schema")=="harmonic_surface_manifest.v1":m=(v,h)
    if s is None or m is None or m[0].get("surface_artifact_hash")!=s[1]:raise ValueError("surface missing")
    return s[0],s[1],m[1]
def execute_harmonic_job(*,repository_root:str|Path,execution:RunningJobExecution)->HarmonicJobPacket:
    ...
    eid=subject["id"];plan=build_harmonic_validation_plan(eid);env=build_environment_manifest();ph=sha256(canonical_bytes(plan)).hexdigest();names=output_names(eid);inputs=tuple(spec["input_hashes"]);required={DATASET_SHA256,OBSERVED_MATERIAL_ID,ROLLING_SPLIT_SHA256,ph,harmonic_implementation_sha256(),cyclical_implementation_sha256(),loader_implementation_sha256(),discovery_implementation_sha256()}
