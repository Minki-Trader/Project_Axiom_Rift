"""Causal broker-spread liquidity-supply discovery."""
...
SELECTION_TOTAL_EXPOSURES=384;SELECTOR_QUANTILE_BP=9_000;_PROFILES=("spread_z_96","spread_delta_12");_THIS_FILE=Path(__file__).resolve()
def liquidity_implementation_sha256()->str:return sha256(_THIS_FILE.read_bytes()).hexdigest()
...
class LiquidityConfiguration:
    profile:str;signal_sign:int;holding_bars:int=24
    def __post_init__(self)->None:
        if self.profile not in _PROFILES or self.signal_sign not in {-1,1} or self.holding_bars!=24:raise ValueError("liquidity configuration invalid")
    @property
    def configuration_id(self)->str:return f"{self.profile}-{'aligned' if self.signal_sign==1 else 'inverse'}-h24"
    def semantic_parameters(self)->dict[str,Any]:return {"holding_bars":24,"profile":self.profile,"selector_quantile_bp":SELECTOR_QUANTILE_BP,"signal_sign":self.signal_sign}
def liquidity_configurations()->tuple[LiquidityConfiguration,...]:return tuple(LiquidityConfiguration(profile=p,signal_sign=s) for p in _PROFILES for s in (1,-1))
def _local(n:str)->str:return f"axiom_rift.research.liquidity_supply_discovery.{n}@sha256:{liquidity_implementation_sha256()}"
def _shared(n:str)->str:return f"axiom_rift.research.discovery.{n}@sha256:{discovery_implementation_sha256()}"
def liquidity_components()->tuple[ComponentSpec,...]:return (ComponentSpec(display_name="causal broker-spread liquidity state",protocol="feature.causal_liquidity_supply.v1",implementation=_local("compute_liquidity_score"),spec={"availability":"completed_bar_only","profiles":list(_PROFILES),"zero_spread_policy":"causal_prior_positive_median","parameter_fields":["profile"]}),ComponentSpec(display_name="fold isolated liquidity selector",protocol="selector.fold_train_abs_quantile.v2",implementation=_local("calibrate_selector"),spec={"calibration_role":"train_is_only","minimum_train_observations":1000,"quantile_basis_points":SELECTOR_QUANTILE_BP,"quantile_method":"higher"}),ComponentSpec(display_name="completed-bar next-open directional entry",protocol="trade.completed_bar_next_open_direction.v2",implementation=_shared("simulate_fixed_hold"),spec={"decision_time":"bar_open_plus_5m","entry_time":"next_exact_bar_open","direction":"signal_sign_times_score_sign","parameter_fields":["signal_sign"]}),ComponentSpec(display_name="fixed-hold nonoverlap lifecycle",protocol="lifecycle.fixed_hold_no_overlap.v2",implementation=_shared("simulate_fixed_hold"),spec={"entry_overlap":"reject_while_position_slot_is_occupied","exit_surface":"exact_bar_open_after_24_bars","gap_action":"exclude_path"}),ComponentSpec(display_name="FPMarkets bid-bar spread execution",protocol="execution.fpmarkets_bid_bar_spread.v2",implementation=_shared("execution_pnl"),spec={"point":"0.01","stress":"half_effective_spread_each_side"}),ComponentSpec(display_name="fixed one-lot risk",protocol="risk.fixed_one_lot.v1",implementation=_shared("simulate_fixed_hold"),spec={"dynamic_sizing":False,"lot":1,"positions_per_sleeve":1}))
def liquidity_executable(c:LiquidityConfiguration)->ExecutableSpec:return ExecutableSpec(display_name=f"liquidity supply {c.configuration_id}",components=liquidity_components(),parameters=c.semantic_parameters(),data_contract=f"data:{OBSERVED_MATERIAL_ID}",split_contract=f"split:{ROLLING_SPLIT_SHA256}:rolling_windows_9_observed_development",clock_contract="clock:fpmarkets_m5_bar_open_completed_plus_5m_v2",cost_contract="cost:bid_bar_spread_point_0_01_causal_zero_repair_half_spread_stress_v2",engine_contract=f"engine:liquidity_supply_v1:python{'.'.join(str(v) for v in sys.version_info[:3])}:numpy{np.__version__}:pandas{pd.__version__}:scipy{scipy.__version__}:implementation_{liquidity_implementation_sha256()}:loader_{loader_implementation_sha256()}:shared_{discovery_implementation_sha256()}:bootstrap_{SELECTION_BOOTSTRAP_SAMPLES}:blocks_5_10_20:bonferroni_{SELECTION_TOTAL_EXPOSURES}:seed_{SELECTION_SEED}")
def executable_configuration_map()->dict[str,LiquidityConfiguration]:return {liquidity_executable(c).identity:c for c in liquidity_configurations()}
def compute_liquidity_score(frame:pd.DataFrame,profile:str)->tuple[np.ndarray,np.ndarray,np.ndarray]:
    if profile not in _PROFILES:raise ValueError("profile invalid")
    time_ns=_time_ns(frame);spread=causal_effective_spread(frame["spread"].to_numpy(float),time_ns);series=pd.Series(spread);mean=series.rolling(96,min_periods=96).mean();std=series.rolling(96,min_periods=96).std(ddof=1);score=((series-mean)/std.where(std>0)).to_numpy(float) if profile=="spread_z_96" else ((series-series.shift(12))/std.where(std>0)).to_numpy(float);close=frame["close"].to_numpy(float);returns=np.full(len(close),np.nan);returns[1:]=np.diff(np.log(close));vol=pd.Series(returns).rolling(96,min_periods=96).std(ddof=1).to_numpy(float);run=_consecutive_run(time_ns);score[run<97]=np.nan;return score,vol,run
...
def compute_registered_liquidity_surface(repository_root:str|Path)->dict[str,Any]:
    ...
    for profile in _PROFILES:
        value=compute_liquidity_score(frame,profile);features[profile]=value;prefixes[profile]={};calibrations[profile]={}
        for f in folds:
            fid=str(f["fold_id"]);train=f["train_is"];mask=((time>=pd.Timestamp(train["start"]))&(time<=pd.Timestamp(train["end"]))).to_numpy();threshold=calibrate_selector(value[0],mask);vv=value[1][mask&np.isfinite(value[1])];cutoffs=(float(np.quantile(vv,1/3,method="higher")),float(np.quantile(vv,2/3,method="higher")));pv=compute_liquidity_score(prefix_frames[fid],profile);prefixes[profile][fid]=pv;pt=pd.to_datetime(prefix_frames[fid]["time"],errors="raise");pm=((pt>=pd.Timestamp(train["start"]))&(pt<=pd.Timestamp(train["end"]))).to_numpy();calibrations[profile][fid]=(threshold,cutoffs,calibrate_selector(pv[0],pm))
    results=[_evaluate_configuration(calibrations=calibrations[c.profile],frame=frame,features=features[c.profile],folds=folds,configuration=c,effective_spread=spread,prefix_features=prefixes[c.profile],prefix_spreads=prefix_spreads,time=time,executable_id=liquidity_executable(c).identity) for c in liquidity_configurations()];pv=_selection_adjusted_pvalues(results,total_exposures=SELECTION_TOTAL_EXPOSURES)
    for r in results:r.metrics["selection_aware_pvalue_ppm"]=pv[r.executable_id]
    _populate(results);surface={"claim_limits":_claim_limits()+["spread_state_is_past_only","four_trial_surface"],"dataset_sha256":DATASET_SHA256,"engine_environment":{"numpy":np.__version__,"pandas":pd.__version__,"python":".".join(str(v) for v in sys.version_info[:3]),"scipy":scipy.__version__},"evaluations":[{"direction_metrics":r.direction_metrics,"evaluable":all(r.metrics[n]==0 for n in ("unknown_cost_unresolved_signal_count","causality_violation_count","nonfinite_metric_count","prefix_invariance_mismatch_count","append_invariance_mismatch_count")),"fold_metrics":r.fold_metrics,"metrics":dict(sorted(r.metrics.items())),"regime_metrics":r.regime_metrics,"session_metrics":r.session_metrics,"subject_configuration_id":r.configuration.configuration_id,"subject_executable_id":r.executable_id} for r in results],"loader_implementation_sha256":loader_implementation_sha256(),"material_identity":OBSERVED_MATERIAL_ID,"schema":"liquidity_supply_surface.v1","selection_context":[{"configuration_id":r.configuration.configuration_id,"executable_id":r.executable_id,"net_profit_micropoints":r.metrics["net_profit_micropoints"],"selection_aware_pvalue_ppm":r.metrics["selection_aware_pvalue_ppm"]} for r in results],"selection_method":_selection_method(SELECTION_TOTAL_EXPOSURES),"session_semantics":"broker_clock_fixed_bins_no_dst_or_cash_session_claim","liquidity_implementation_sha256":liquidity_implementation_sha256(),"split_artifact_sha256":ROLLING_SPLIT_SHA256};canonical_bytes(surface);return surface
def project_liquidity_evaluation(surface:Mapping[str,Any],*,job_execution:Mapping[str,str],subject_executable_id:str,surface_artifact_hash:str,surface_manifest_hash:str)->dict[str,Any]:
    value=dict(surface)
    if sha256(canonical_bytes(value)).hexdigest()!=surface_artifact_hash or value.get("schema")!="liquidity_supply_surface.v1":raise DiscoveryBoundaryError("surface invalid")
