...
SELECTION_TOTAL_EXPOSURES=388;SELECTOR_QUANTILE_BP=9_000;_PROFILES=("spread_recovery_24","spread_vol_ratio_24");_THIS_FILE=Path(__file__).resolve()
...
class LiquidityConfiguration:
    ...
    def __post_init__(self)->None:
        if self.profile not in _PROFILES or self.signal_sign not in {-1,1} or self.holding_bars!=48:raise ValueError("liquidity configuration invalid")
    @property
...
def compute_liquidity_score(frame:pd.DataFrame,profile:str)->tuple[np.ndarray,np.ndarray,np.ndarray]:
    ...
    time_ns=_time_ns(frame);spread=causal_effective_spread(frame["spread"].to_numpy(float),time_ns);series=pd.Series(spread);mean=series.rolling(96,min_periods=96).mean();std=series.rolling(96,min_periods=96).std(ddof=1);score=((series.rolling(24,min_periods=24).max()-series)/std.where(std>0)).to_numpy(float) if profile=="spread_recovery_24" else (series.rolling(24,min_periods=24).std(ddof=1)/mean.where(mean>0)).to_numpy(float);close=frame["close"].to_numpy(float);returns=np.full(len(close),np.nan);returns[1:]=np.diff(np.log(close));vol=pd.Series(returns).rolling(96,min_periods=96).std(ddof=1).to_numpy(float);run=_consecutive_run(time_ns);score[run<97]=np.nan;return score,vol,run
...
def project_liquidity_evaluation(surface:Mapping[str,Any],*,job_execution:Mapping[str,str],subject_executable_id:str,surface_artifact_hash:str,surface_manifest_hash:str)->dict[str,Any]:
    value=dict(surface)
    if sha256(canonical_bytes(value)).hexdigest()!=surface_artifact_hash or value.get("schema")!="liquidity_supply_surface.v2":raise DiscoveryBoundaryError("surface invalid")
