"""Writer-gated evidence Job for the volatility-clock label contrast."""
...
from axiom_rift.research.volatility_clock_label_chassis import executable_configuration_map, loader_implementation_sha256, volatility_clock_label_chassis_implementation_sha256
from axiom_rift.research.volatility_clock_label_discovery import compute_registered_volatility_clock_label_surface, project_volatility_clock_label_evaluation, volatility_clock_label_discovery_implementation_sha256


CALLABLE_IDENTITY = "axiom_rift.research.volatility_clock_label_study.execute_volatility_clock_label_job.v1"
EVIDENCE_DEPTH = "discovery"
_CONTROL_DELTA_METRIC = "label_control_delta_net_profit_micropoints"
_CONTROL_PVALUE_METRIC = "label_control_pvalue_upper_ppm"
...
def build_volatility_clock_label_validation_plan(
...
def surface_output_name(*, study_id: str) -> str:
    return f"scientific/{study_id}/volatility-clock-label-surface.json"


def surface_manifest_output_name(*, study_id: str) -> str:
    return f"scientific/{study_id}/volatility-clock-label-surface-manifest.json"
...
def build_environment_manifest() -> dict[str, object]:
    value = {
        ...
        "volatility_clock_label_chassis_implementation_sha256": volatility_clock_label_chassis_implementation_sha256(),
        "volatility_clock_label_discovery_implementation_sha256": volatility_clock_label_discovery_implementation_sha256(),
...
class VolatilityClockLabelJobPacket:
...
def _load_surface(
    ...
    for artifact_hash in input_hashes:
        try:
            ...
            continue
        if isinstance(value, dict) and value.get("schema") == "volatility_clock_label_surface.v1":
            surface = (value, artifact_hash)
        if isinstance(value, dict) and value.get("schema") == "volatility_clock_label_surface_manifest.v1":
            manifest = (value, artifact_hash)
    if surface is None or manifest is None or manifest[0].get("surface_artifact_hash") != surface[1]:
        raise ValueError("volatility-clock label surface is missing")
    return surface[0], surface[1], manifest[1]


def execute_volatility_clock_label_job(
    *, repository_root: str | Path, execution: RunningJobExecution
) -> VolatilityClockLabelJobPacket:
    ...
    if (
        ...
        raise ValueError("volatility-clock label binding is invalid")
    executable_id = subject["id"]
    plan = build_volatility_clock_label_validation_plan(executable_id, mission_id=mission_id)
    ...
    if not required.issubset(inputs):
        raise ValueError("volatility-clock label inputs are missing")
    expected = set(spec["expected_outputs"])
    produces_surface = surface_output_name(study_id=study_id) in expected
    if produces_surface:
        surface = compute_registered_volatility_clock_label_surface(root)
        surface_hash = writer.evidence.finalize(canonical_bytes(surface)).sha256
        manifest_value = {
            "schema": "volatility_clock_label_surface_manifest.v1",
            "surface_artifact_hash": surface_hash,
            "surface_implementation_sha256": volatility_clock_label_discovery_implementation_sha256(),
        }
    ...
    evaluation = project_volatility_clock_label_evaluation(
        surface,
    ...
    return VolatilityClockLabelJobPacket(
...
__all__ = [
    ...
    "build_volatility_clock_label_validation_plan",
    "execute_volatility_clock_label_job", "output_names",
