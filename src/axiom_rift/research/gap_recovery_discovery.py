...
SELECTION_TOTAL_EXPOSURES=424;SELECTOR_QUANTILE_BP=7_000;_PROFILES=("open_gap_30m","first_bar_response_30m");_THIS_FILE=Path(__file__).resolve();_MIN_GAP_MINUTES=30
...
class GapConfiguration:
    ...
    def __post_init__(self)->None:
        if self.profile not in _PROFILES or self.signal_sign not in {-1,1} or self.holding_bars!=12:raise ValueError("gap configuration invalid")
    @property
    def configuration_id(self)->str:return f"{self.profile}-{'continuation' if self.signal_sign==1 else 'recovery'}-h12"
    def semantic_parameters(self)->dict[str,Any]:return {"holding_bars":12,"minimum_gap_minutes":30,"profile":self.profile,"selector_quantile_bp":SELECTOR_QUANTILE_BP,"signal_sign":self.signal_sign}
...
def calibrate_selector(score:np.ndarray,mask:np.ndarray)->float:
    ...
    if len(v)<500:raise DiscoveryBoundaryError("gap selector too small")
...
def project_gap_evaluation(surface:Mapping[str,Any],*,job_execution:Mapping[str,str],subject_executable_id:str,surface_artifact_hash:str,surface_manifest_hash:str)->dict[str,Any]:
    value=dict(surface)
    if sha256(canonical_bytes(value)).hexdigest()!=surface_artifact_hash or value.get("schema")!="gap_recovery_surface.v1":raise DiscoveryBoundaryError("surface invalid")
    ...
    result={**dict(by[subject_executable_id]),"claim_limits":value["claim_limits"],"job_execution":dict(job_execution),"schema":"gap_recovery_evaluation.v1","selection_context":value["selection_context"],"selection_method":value["selection_method"],"session_semantics":value["session_semantics"],"surface_artifact_hash":surface_artifact_hash,"surface_manifest_hash":surface_manifest_hash};canonical_bytes(result);return result
__all__=["compute_gap_score","compute_registered_gap_surface","executable_configuration_map","gap_configurations","gap_executable","gap_implementation_sha256","loader_implementation_sha256","project_gap_evaluation"]
