...
SELECTION_TOTAL_EXPOSURES=436;SELECTOR_QUANTILE_BP=7_000;_PROFILES=("residual_gap_after_first_bar","gap_fill_fraction_after_first_bar");_THIS_FILE=Path(__file__).resolve();_MIN_GAP_MINUTES=30
...
class GapConfiguration:
    profile:str;signal_sign:int;holding_bars:int=6;unknown_entry_action:str="cancel_before_open"
    def __post_init__(self)->None:
        if self.profile not in _PROFILES or self.signal_sign not in {-1,1} or self.holding_bars!=6 or self.unknown_entry_action!="cancel_before_open":raise ValueError("gap configuration invalid")
    @property
    def configuration_id(self)->str:return f"{self.profile}-{'continuation' if self.signal_sign==1 else 'recovery'}-h6"
    def semantic_parameters(self)->dict[str,Any]:return {"holding_bars":6,"minimum_gap_minutes":30,"profile":self.profile,"selector_quantile_bp":SELECTOR_QUANTILE_BP,"signal_sign":self.signal_sign,"unknown_entry_action":self.unknown_entry_action}
...
def gap_components()->tuple[ComponentSpec,...]:
    execution_spec=completed_period_proxy_execution_spec(
        repair_policy="same_contiguous_gap_segment_strict_prior_positive_288_bar_median_min_1_else_unknown"
    )
    return (
        ComponentSpec(display_name="causal post-gap path shape",protocol="feature.causal_gap_path_shape.v1",implementation=_local("compute_gap_score"),spec={"availability":"first_completed_bar_after_gap","minimum_gap_minutes":30,"profiles":list(_PROFILES),"non_event_value":"missing","parameter_fields":["profile"]}),
        ComponentSpec(display_name="fold isolated event selector",protocol="selector.fold_train_abs_quantile.v3",implementation=_local("calibrate_selector"),spec={"calibration_role":"train_is_only","minimum_train_observations":350,"quantile_basis_points":SELECTOR_QUANTILE_BP,"quantile_method":"higher"}),
        ...
        ComponentSpec(display_name="fixed-hold nonoverlap lifecycle",protocol="lifecycle.fixed_hold_no_overlap.v4",implementation=_shared("simulate_fixed_hold"),spec={"entry_overlap":"reject_while_position_slot_is_occupied","exit_surface":"exact_bar_open_after_6_bars","gap_action":"exclude_path","unknown_entry_reservation":False}),
        ComponentSpec(display_name="completed-period gap-segment spread-proxy execution",protocol="execution.fpmarkets_completed_period_spread_proxy.v2",implementation=_local("causal_gap_effective_spread"),spec=execution_spec),
...
def gap_executable(c:GapConfiguration)->ExecutableSpec:return ExecutableSpec(display_name=f"post-gap path {c.configuration_id}",components=gap_components(),parameters=c.semantic_parameters(),data_contract=f"data:{OBSERVED_MATERIAL_ID}",split_contract=f"split:{ROLLING_SPLIT_SHA256}:rolling_windows_9_observed_development",clock_contract="clock:fpmarkets_m5_bar_open_completed_plus_5m_v2",cost_contract="cost:fpmarkets_completed_bar_spread_proxy_gap_segment_positive_median_min_1_unknown_entry_cancel_half_spread_stress_v1",engine_contract=f"engine:gap_path_v1:python{'.'.join(str(v) for v in sys.version_info[:3])}:numpy{np.__version__}:pandas{pd.__version__}:scipy{scipy.__version__}:implementation_{gap_implementation_sha256()}:loader_{loader_implementation_sha256()}:shared_{discovery_implementation_sha256()}:bootstrap_{SELECTION_BOOTSTRAP_SAMPLES}:blocks_5_10_20:bonferroni_{SELECTION_TOTAL_EXPOSURES}:seed_{SELECTION_SEED}")
...
def compute_gap_score(frame:pd.DataFrame,profile:str)->tuple[np.ndarray,np.ndarray,np.ndarray]:
    ...
    time_ns=_time_ns(frame);delta=np.full(len(frame),np.nan);delta[1:]=np.diff(time_ns)/60_000_000_000;event=delta>=_MIN_GAP_MINUTES;o=frame["open"].to_numpy(float);c=frame["close"].to_numpy(float);previous=np.roll(c,1);opening=o-previous;raw=np.full(len(frame),np.nan)
    if profile=="residual_gap_after_first_bar":raw[event]=c[event]-previous[event]
    else:raw[event]=np.divide(np.sign(opening[event])*(c[event]-o[event]),np.abs(opening[event]),out=np.full(int(event.sum()),np.nan),where=np.abs(opening[event])>0)
    ret=np.full(len(c),np.nan);ret[1:]=np.diff(np.log(c));vol=pd.Series(ret).rolling(96,min_periods=96).std(ddof=1).to_numpy(float);score=raw if profile=="gap_fill_fraction_after_first_bar" else np.divide(raw,c*vol,out=np.full(len(c),np.nan),where=np.isfinite(vol)&(vol>0));run=_consecutive_run(time_ns);return score,vol,run
...
def project_gap_evaluation(surface:Mapping[str,Any],*,job_execution:Mapping[str,str],subject_executable_id:str,surface_artifact_hash:str,surface_manifest_hash:str)->dict[str,Any]:
    value=dict(surface)
    if sha256(canonical_bytes(value)).hexdigest()!=surface_artifact_hash or value.get("schema")!="gap_recovery_surface.v4":raise DiscoveryBoundaryError("surface invalid")
    ...
    result={**dict(by[subject_executable_id]),"claim_limits":value["claim_limits"],"job_execution":dict(job_execution),"schema":"gap_recovery_evaluation.v4","selection_context":value["selection_context"],"selection_method":value["selection_method"],"session_semantics":value["session_semantics"],"surface_artifact_hash":surface_artifact_hash,"surface_manifest_hash":surface_manifest_hash};canonical_bytes(result);return result
