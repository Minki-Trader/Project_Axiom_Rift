"""Volatility-clock event-label discovery surface."""
...
def volatility_clock_label_discovery_implementation_sha256() -> str:
...
def deterministic_score(
    ...
    result[valid] = np.sum(
        standardized * beta.reshape(1, -1), axis=1, dtype=np.float64
    ) + intercept
    return result
...
def _matched(results: list[Any], profile: str) -> Any:
    ...
    if len(found) != 1:
        raise DiscoveryBoundaryError("volatility-clock label control is not unique")
    return found[0]


def _populate_controls(results: list[Any]) -> None:
    profiles = tuple(value.profile for value in volatility_clock_label_configurations())
    for subject in results:
        control = _matched(
            results, next(value for value in profiles if value != subject.configuration.profile)
        )
        ...
        subject.metrics["label_control_pvalue_upper_ppm"] = _paired_control_pvalue(
            subject,
            control,
            role="matched_fixed_clock_label_control",
            total_exposures=SELECTION_TOTAL_EXPOSURES,
        )


def compute_registered_volatility_clock_label_surface(
    ...
    labels = build_labels(frame, full_volatility, full_run)
    ...
    results = []
    for configuration in volatility_clock_label_configurations():
        ...
        results.append(
            _evaluate_configuration(
                ...
                time=time,
                executable_id=volatility_clock_label_executable(configuration).identity,
            )
        )
    ...
    surface = {
        ...
        + [
            ...
            "activity_time_uses_a_pre_entry_volatility_budget",
            "activity_time_is_capped_at_48_bars",
            "feature_model_selector_and_trading_are_fixed",
            "two_trial_surface",
        ],
        ...
        "schema": "volatility_clock_label_surface.v1",
        "selection_context": [
            {
...
def project_volatility_clock_label_evaluation(
    ...
    *,
    ...
    if (
        ...
        or value.get("schema") != "volatility_clock_label_surface.v1"
    ):
        raise DiscoveryBoundaryError("volatility-clock label surface invalid")
    ...
    if set(by_executable) != set(expected) or subject_executable_id not in expected:
        raise DiscoveryBoundaryError("volatility-clock label subjects differ")
    payload = {
    ...
    if job_execution.get("identity") != canonical_digest(
        ...
        raise DiscoveryBoundaryError("volatility-clock label Job invalid")
    result = {
        **dict(by_executable[subject_executable_id]),
        "claim_limits": value["claim_limits"],
        "job_execution": dict(job_execution),
        "schema": "volatility_clock_label_evaluation.v1",
...
__all__ = [
    "compute_registered_volatility_clock_label_surface",
    "deterministic_score",
    "project_volatility_clock_label_evaluation",
