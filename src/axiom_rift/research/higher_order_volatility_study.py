...
MISSION_ID="MIS-0005";STUDY_ID="STU-0060";CALLABLE_IDENTITY="axiom_rift.research.higher_order_volatility_study.execute_higher_volatility_job.v2";EVIDENCE_DEPTH="discovery"
...
def _load(w:StateWriter,hashes:tuple[str,...])->tuple[dict[str,Any],str,str]:
    ...
    for h in hashes:
        ...
        if isinstance(v,dict) and v.get("schema")=="higher_order_volatility_surface.v2":surface=(v,h)
        if isinstance(v,dict) and v.get("schema")=="higher_order_volatility_surface_manifest.v2":manifest=(v,h)
