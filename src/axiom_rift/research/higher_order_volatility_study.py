"""Writer-gated evidence Job for higher-order volatility."""
...
from axiom_rift.research.higher_order_volatility_discovery import compute_registered_higher_volatility_surface,executable_configuration_map,higher_volatility_implementation_sha256,loader_implementation_sha256,project_higher_volatility_evaluation
...
MISSION_ID="MIS-0005";STUDY_ID="STU-0059";CALLABLE_IDENTITY="axiom_rift.research.higher_order_volatility_study.execute_higher_volatility_job.v1";EVIDENCE_DEPTH="discovery"
def build_higher_volatility_validation_plan(eid:str)->dict[str,object]:return build_validation_plan(mission_id=MISSION_ID,executable_id=eid,evidence_depth=EVIDENCE_DEPTH,planned_claims=PLANNED_CLAIMS,evidence_modes=EVIDENCE_MODES,criteria=CRITERIA,candidate_eligible_on_pass=False)
...
class HigherVolatilityJobPacket:
...
def _load(w:StateWriter,hashes:tuple[str,...])->tuple[dict[str,Any],str,str]:
    ...
    for h in hashes:
        ...
        if isinstance(v,dict) and v.get("schema")=="higher_order_volatility_surface.v1":surface=(v,h)
        if isinstance(v,dict) and v.get("schema")=="higher_order_volatility_surface_manifest.v1":manifest=(v,h)
    if surface is None or manifest is None or manifest[0].get("surface_artifact_hash")!=surface[1]:raise ValueError("higher-volatility surface missing")
    return surface[0],surface[1],manifest[1]
def execute_higher_volatility_job(*,repository_root:str|Path,execution:RunningJobExecution)->HigherVolatilityJobPacket:
    ...
    eid=subject["id"];plan=build_higher_volatility_validation_plan(eid);env=build_environment_manifest();ph=sha256(canonical_bytes(plan)).hexdigest();names=output_names(eid);inputs=tuple(spec["input_hashes"]);required={DATASET_SHA256,OBSERVED_MATERIAL_ID,ROLLING_SPLIT_SHA256,ph,higher_volatility_implementation_sha256(),loader_implementation_sha256(),discovery_implementation_sha256()}
