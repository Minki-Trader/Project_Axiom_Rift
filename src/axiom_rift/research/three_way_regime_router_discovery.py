"""Registered two-way versus low-abstain three-way router surface."""
...
from axiom_rift.research.three_way_regime_router_chassis import SELECTION_TOTAL_EXPOSURES, executable_configuration_map, loader_implementation_sha256, simulate_three_way_regime_router, three_way_regime_router_chassis_implementation_sha256, three_way_regime_router_configurations, three_way_regime_router_executable
...
def three_way_regime_router_discovery_implementation_sha256() -> str:
    return sha256(_THIS_FILE.read_bytes()).hexdigest()


def _matched(results: list[Any], policy: str) -> Any:
    found = [value for value in results if value.configuration.route_policy == policy]
    if len(found) != 1:
        raise DiscoveryBoundaryError("three-way regime router control is not unique")
    return found[0]


def _populate_controls(results: list[Any]) -> None:
    control = _matched(results, "two_way_calm_inverse_control")
    for subject in results:
        ...
        subject.metrics["router_control_pvalue_upper_ppm"] = 1_000_000 if subject is control else _paired_control_pvalue(subject, control, role="matched_two_way_calm_inverse_control", total_exposures=SELECTION_TOTAL_EXPOSURES)


def compute_registered_three_way_regime_router_surface(repository_root: str | Path) -> dict[str, Any]:
    ...
    results = []
    for configuration in three_way_regime_router_configurations():
        ...
        results.append(_evaluate_configuration(calibrations=calibrations, frame=frame, features=first, fold_features=fold_scores, folds=folds, configuration=configuration, effective_spread=spread, prefix_features=prefix_scores, prefix_spreads=prefix_spreads, time=time, executable_id=three_way_regime_router_executable(configuration).identity, simulation_fn=simulate_three_way_regime_router))
    ...
    surface = {
        "claim_limits": _claim_limits() + ["regime_trade_and_synthesis_are_the_primary_changed_research_layers", "dense12_feature_label_model_selector_lifecycle_risk_and_execution_are_fixed", "low_volatility_abstention_uses_the_train_only_lower_tercile", "two_trial_surface"],
        ...
        "loader_implementation_sha256": loader_implementation_sha256(), "material_identity": OBSERVED_MATERIAL_ID, "schema": "three_way_regime_router_surface.v1",
        ...
        "three_way_regime_router_chassis_implementation_sha256": three_way_regime_router_chassis_implementation_sha256(), "three_way_regime_router_discovery_implementation_sha256": three_way_regime_router_discovery_implementation_sha256(),
    }
    canonical_bytes(surface)
    return surface


def project_three_way_regime_router_evaluation(surface: Mapping[str, Any], *, job_execution: Mapping[str, str], subject_executable_id: str, surface_artifact_hash: str, surface_manifest_hash: str) -> dict[str, Any]:
    value = dict(surface)
    if sha256(canonical_bytes(value)).hexdigest() != surface_artifact_hash or value.get("schema") != "three_way_regime_router_surface.v1":
        raise DiscoveryBoundaryError("three-way regime router surface invalid")
    ...
    if set(by_subject) != set(expected) or subject_executable_id not in expected:
        raise DiscoveryBoundaryError("three-way regime router subjects differ")
    ...
    if job_execution.get("identity") != canonical_digest(domain="running-job-execution", payload=payload):
        raise DiscoveryBoundaryError("three-way regime router Job invalid")
    result = {**dict(by_subject[subject_executable_id]), "claim_limits": value["claim_limits"], "job_execution": dict(job_execution), "schema": "three_way_regime_router_evaluation.v1", "selection_context": value["selection_context"], "selection_method": value["selection_method"], "session_semantics": value["session_semantics"], "surface_artifact_hash": surface_artifact_hash, "surface_manifest_hash": surface_manifest_hash}
    canonical_bytes(result)
    return result


__all__ = ["compute_registered_three_way_regime_router_surface", "project_three_way_regime_router_evaluation", "three_way_regime_router_discovery_implementation_sha256"]
