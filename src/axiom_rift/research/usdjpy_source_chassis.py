...
from axiom_rift.research import (
    usdjpy_source_eligibility_validation as validator_module,
)
...
def usdjpy_source_implementation_sha256() -> str:
    return _file_sha256(Path(source_module.__file__).resolve())


def usdjpy_source_validator_implementation_sha256() -> str:
    return _file_sha256(Path(validator_module.__file__).resolve())


def usdjpy_source_baseline() -> ExecutableSpec:
    chassis_digest = usdjpy_source_chassis_implementation_sha256()
    source_digest = usdjpy_source_implementation_sha256()
    validator_digest = usdjpy_source_validator_implementation_sha256()
    contract = usdjpy_source_contract()
    source = ComponentSpec(
        display_name="FPMarkets USDJPY reconstruction-only source eligibility",
        protocol="external_source.fpmarkets_usdjpy_m5.v2",
        implementation=(
    ...
    return ExecutableSpec(
        display_name="USDJPY reconstruction-only source eligibility baseline",
        ...
        engine_contract=(
            "engine:usdjpy_source_eligibility_v2:"
