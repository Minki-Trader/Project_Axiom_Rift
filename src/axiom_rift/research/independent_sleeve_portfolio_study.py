"""Writer-gated evidence Job for the independent sleeve portfolio contrast."""
...
CALLABLE_IDENTITY = "axiom_rift.research.independent_sleeve_portfolio_study.execute_independent_sleeve_portfolio_job.v1"
...
def build_independent_sleeve_portfolio_validation_plan(executable_id: str, *, mission_id: str) -> dict[str, object]:
...
def surface_output_name(*, study_id: str) -> str:
    return f"scientific/{study_id}/independent-sleeve-portfolio-surface.json"


def surface_manifest_output_name(*, study_id: str) -> str:
    return f"scientific/{study_id}/independent-sleeve-portfolio-surface-manifest.json"
...
def build_environment_manifest() -> dict[str, object]:
    value = {
        ...
        "independent_sleeve_portfolio_followup_implementation_sha256": independent_sleeve_portfolio_followup_implementation_sha256(),
...
class IndependentSleevePortfolioJobPacket:
...
def execute_independent_sleeve_portfolio_job(*, repository_root: str | Path, execution: RunningJobExecution) -> IndependentSleevePortfolioJobPacket:
    ...
    if not isinstance(mission_id, str) or not isinstance(study_id, str) or not isinstance(subject, dict) or subject.get("id") not in executable_configuration_map():
        raise ValueError("independent sleeve portfolio binding is invalid")
    executable_id = subject["id"]
    plan = build_independent_sleeve_portfolio_validation_plan(executable_id, mission_id=mission_id)
    ...
    required = {DATASET_SHA256, OBSERVED_MATERIAL_ID, ROLLING_SPLIT_SHA256, plan_hash, independent_sleeve_portfolio_chassis_implementation_sha256(), independent_sleeve_portfolio_followup_implementation_sha256(), independent_sleeve_portfolio_discovery_implementation_sha256(), loader_implementation_sha256(), discovery_implementation_sha256()}
    if not required.issubset(inputs):
        raise ValueError("independent sleeve portfolio inputs are missing")
    expected = set(spec["expected_outputs"])
    produces_surface = surface_output_name(study_id=study_id) in expected
    if produces_surface:
        surface = compute_registered_independent_sleeve_portfolio_surface(root)
    ...
    return IndependentSleevePortfolioJobPacket(output_manifest=tuple(sorted(outputs.items())), verdict=planned_verdict(plan, measurement))


__all__ = ["CALLABLE_IDENTITY", "CRITERIA", "EVIDENCE_DEPTH", "EVIDENCE_MODES", "PLANNED_CLAIMS", "build_environment_manifest", "build_independent_sleeve_portfolio_validation_plan", "execute_independent_sleeve_portfolio_job", "output_names", "surface_manifest_output_name", "surface_output_name"]
