...
SELECTION_TOTAL_EXPOSURES=414;SELECTOR_QUANTILE_BP=7_000;_PROFILES=("run_sign_3","alternation_4","ordinal_slope_5");_THIS_FILE=Path(__file__).resolve()
...
class OrdinalConfiguration:
    ...
    def __post_init__(self)->None:
        if self.profile not in _PROFILES or self.signal_sign not in {-1,1} or self.holding_bars!=24:raise ValueError("ordinal configuration invalid")
    @property
    def configuration_id(self)->str:return f"{self.profile}-{'continuation' if self.signal_sign==1 else 'reversal'}-h24"
...
def compute_ordinal_score(frame:pd.DataFrame,profile:str)->tuple[np.ndarray,np.ndarray,np.ndarray]:
    ...
    if profile=="run_sign_3":score[3:]=(sign[1:-2]+sign[2:-1]+sign[3:])/3
    elif profile=="alternation_4":
        for i in range(4,len(close)):
            v=sign[i-3:i+1];score[i]=sign[i] if np.all(v[1:]*v[:-1]<0) else 0.0
    else:
        rank=np.arange(5,dtype=float);den=float(((rank-rank.mean())**2).sum())
        for i in range(4,len(close)):
            order=np.argsort(np.argsort(close[i-4:i+1])).astype(float);score[i]=float(((rank-rank.mean())*(order-order.mean())).sum()/den)
...
def project_ordinal_evaluation(surface:Mapping[str,Any],*,job_execution:Mapping[str,str],subject_executable_id:str,surface_artifact_hash:str,surface_manifest_hash:str)->dict[str,Any]:
    value=dict(surface)
    if sha256(canonical_bytes(value)).hexdigest()!=surface_artifact_hash or value.get("schema")!="ordinal_transition_surface.v1":raise DiscoveryBoundaryError("surface invalid")
    ...
    result={**dict(by[subject_executable_id]),"claim_limits":value["claim_limits"],"job_execution":dict(job_execution),"schema":"ordinal_transition_evaluation.v1","selection_context":value["selection_context"],"selection_method":value["selection_method"],"session_semantics":value["session_semantics"],"surface_artifact_hash":surface_artifact_hash,"surface_manifest_hash":surface_manifest_hash};canonical_bytes(result);return result
