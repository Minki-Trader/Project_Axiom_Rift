...
def execute_sleeve_loss_skip_risk_job(
    *,
    ...
    if type(plan_hash) is not str or plan_hash not in spec.get("input_hashes", ()):
        raise ValueError("sleeve loss-skip validation plan input is absent")
    try:
        plan_value = parse_canonical(writer.evidence.read_verified(plan_hash))
    except (FileNotFoundError, OSError, RuntimeError, TypeError, ValueError) as exc:
        raise ValueError("sleeve loss-skip validation plan is unavailable") from exc
    if not isinstance(plan_value, Mapping):
        raise ValueError("sleeve loss-skip validation plan is not an object")
    ...
    if (
        ...
        raise ValueError("sleeve loss-skip Job registration drifted")
    input_hashes = tuple(spec.get("input_hashes", ()))
    if len(input_hashes) != len(set(input_hashes)):
        raise ValueError("sleeve loss-skip Job inputs are duplicated")
    if scoped.produces_family_cache:
        if tuple(sorted(input_hashes)) != scoped.job_input_hashes():
            raise ValueError("sleeve loss-skip producer inputs drifted")
        family_trace = compute_sleeve_loss_skip_risk_family_trace(
            root,
        ...
        if cache_artifact.sha256 != cache.sha256:
            raise ValueError("sleeve loss-skip cache evidence identity drifted")
        materialize_sleeve_loss_skip_risk_cache(
            root,
            definition=scoped.definition,
        ...
        producer_plan = build_sleeve_loss_skip_risk_job_plan(
            repository_root=root,
            mission_id=mission_id,
            study_id=study_id,
        ...
        if tuple(sorted(input_hashes)) != expected_inputs:
            raise ValueError("sleeve loss-skip consumer inputs drifted")
        if provenance.get("cache_sha256") != cache.sha256:
            raise ValueError("sleeve loss-skip cache provenance drifted")
    trace = bind_sleeve_loss_skip_risk_family_trace(
    ...
    if set(outputs) != set(scoped.expected_outputs()):
        raise ValueError("sleeve loss-skip Job emitted undeclared outputs")
