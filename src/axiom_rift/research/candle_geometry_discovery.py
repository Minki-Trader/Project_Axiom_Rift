...
SELECTION_TOTAL_EXPOSURES=374;SELECTOR_QUANTILE_BP=8_500;_PROFILES=("wick_balance_12","close_location_12","body_impulse_12");_THIS_FILE=Path(__file__).resolve()
...
class CandleConfiguration:
    ...
    def __post_init__(self)->None:
        if self.profile not in _PROFILES or self.signal_sign not in {-1,1} or self.holding_bars!=48:raise ValueError("candle configuration invalid")
    @property
    def configuration_id(self)->str:return f"{self.profile}-{'aligned' if self.signal_sign==1 else 'inverse'}-h48"
...
def compute_candle_score(frame:pd.DataFrame,profile:str)->tuple[np.ndarray,np.ndarray,np.ndarray]:
    ...
    raw={"wick_balance_12":(lower-upper)/safe,"close_location_12":((c-l)-(h-c))/safe,"body_impulse_12":(c-o)/safe}[profile];score=pd.Series(raw).rolling(12,min_periods=12).mean().to_numpy(float);returns=np.full(len(c),np.nan);returns[1:]=np.diff(np.log(c));vol=pd.Series(returns).rolling(48,min_periods=48).std(ddof=1).to_numpy(float);run=_consecutive_run(_time_ns(frame));score[run<49]=np.nan;return score,vol,run
...
def project_candle_evaluation(surface:Mapping[str,Any],*,job_execution:Mapping[str,str],subject_executable_id:str,surface_artifact_hash:str,surface_manifest_hash:str)->dict[str,Any]:
    value=dict(surface)
    if sha256(canonical_bytes(value)).hexdigest()!=surface_artifact_hash or value.get("schema")!="candle_geometry_surface.v1":raise DiscoveryBoundaryError("surface invalid")
    ...
    result={**dict(by[subject_executable_id]),"claim_limits":value["claim_limits"],"job_execution":dict(job_execution),"schema":"candle_geometry_evaluation.v1","selection_context":value["selection_context"],"selection_method":value["selection_method"],"session_semantics":value["session_semantics"],"surface_artifact_hash":surface_artifact_hash,"surface_manifest_hash":surface_manifest_hash};canonical_bytes(result);return result
