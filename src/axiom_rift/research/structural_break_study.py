"""Writer-gated evidence Job for causal structural breaks."""
...
def surface_output_name()->str:return f"scientific/{STUDY_ID}/structural-break-surface.json"
def surface_manifest_output_name()->str:return f"scientific/{STUDY_ID}/structural-break-surface-manifest.json"
def build_environment_manifest()->dict[str,object]:
    v={"dataset_sha256":DATASET_SHA256,"loader_implementation_sha256":loader_implementation_sha256(),"material_identity":OBSERVED_MATERIAL_ID,"numpy_version":np.__version__,"pandas_version":pd.__version__,"python_version":".".join(str(x) for x in sys.version_info[:3]),"runner_implementation_sha256":sha256(Path(__file__).resolve().read_bytes()).hexdigest(),"schema":"scientific_engine_environment.v1","scipy_version":scipy.__version__,"shared_discovery_implementation_sha256":discovery_implementation_sha256(),"split_artifact_sha256":ROLLING_SPLIT_SHA256,"structural_break_implementation_sha256":structural_break_implementation_sha256(),"validator_id":SCIENTIFIC_DISCOVERY_VALIDATOR_ID,"writer_implementation_sha256":sha256(Path(writer_module.__file__).resolve().read_bytes()).hexdigest()};canonical_bytes(v);return v
...
class StructuralBreakJobPacket:
...
def execute_structural_break_job(*,repository_root:str|Path,execution:RunningJobExecution)->StructuralBreakJobPacket:
    ...
    return StructuralBreakJobPacket(output_manifest=tuple(sorted(outputs.items())),verdict=planned_verdict(plan,measurement))
__all__=["CALLABLE_IDENTITY","EVIDENCE_DEPTH","EVIDENCE_MODES","PLANNED_CLAIMS","STUDY_ID","build_environment_manifest","build_structural_break_validation_plan","execute_structural_break_job","output_names","surface_manifest_output_name","surface_output_name"]
