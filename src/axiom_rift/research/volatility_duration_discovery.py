...
SELECTION_TOTAL_EXPOSURES=448;SELECTOR_QUANTILE_BP=7_000;VOLATILITY_WINDOW=96;STATE_WINDOW=576;_PROFILES=("volatility_level_96_576","volatility_duration_96_576");_THIS_FILE=Path(__file__).resolve()
...
class VolatilityDurationConfiguration:
    ...
    def __post_init__(self)->None:
        if self.profile not in _PROFILES or self.signal_sign not in {-1,1} or self.holding_bars!=12 or self.unknown_entry_action!="cancel_before_open":raise ValueError("volatility-duration configuration invalid")
    @property
    def configuration_id(self)->str:return f"{self.profile}-{'follow' if self.signal_sign==1 else 'reverse'}-h12"
    def semantic_parameters(self)->dict[str,Any]:return {"holding_bars":12,"profile":self.profile,"selector_quantile_bp":SELECTOR_QUANTILE_BP,"signal_sign":self.signal_sign,"state_window":STATE_WINDOW,"unknown_entry_action":self.unknown_entry_action,"volatility_window":VOLATILITY_WINDOW}
...
def compute_volatility_duration_score(frame:pd.DataFrame,profile:str)->tuple[np.ndarray,np.ndarray,np.ndarray]:
    ...
    if profile=="volatility_level_96_576":score=level
    else:
        score=np.full(len(c),np.nan);previous=0;duration=0
        for i,value in enumerate(level):
            if not np.isfinite(value):previous=0;duration=0;continue
            state=1 if value>=0 else -1;duration=duration+1 if state==previous else 1;previous=state;score[i]=state*duration
...
def calibrate_selector(score:np.ndarray,mask:np.ndarray)->float:
    ...
    if len(v)<1000:raise DiscoveryBoundaryError("volatility-duration selector too small")
...
def project_volatility_duration_evaluation(surface:Mapping[str,Any],*,job_execution:Mapping[str,str],subject_executable_id:str,surface_artifact_hash:str,surface_manifest_hash:str)->dict[str,Any]:
    value=dict(surface)
    if sha256(canonical_bytes(value)).hexdigest()!=surface_artifact_hash or value.get("schema")!="volatility_duration_surface.v1":raise DiscoveryBoundaryError("volatility-duration surface invalid")
    ...
    result={**dict(by[subject_executable_id]),"claim_limits":value["claim_limits"],"job_execution":dict(job_execution),"schema":"volatility_duration_evaluation.v1","selection_context":value["selection_context"],"selection_method":value["selection_method"],"session_semantics":value["session_semantics"],"surface_artifact_hash":surface_artifact_hash,"surface_manifest_hash":surface_manifest_hash};canonical_bytes(result);return result
