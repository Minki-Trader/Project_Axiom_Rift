...
"""Causal session-inventory follow-up on Foundation-safe US100 M5 data.
...
_SESSION_INVENTORY_FOLLOWUP_FILE = Path(__file__).resolve()


class SessionInventoryFollowupBoundaryError(DiscoveryBoundaryError):
    """Raised before unregistered session-inventory semantics can enter evaluation."""
...
def session_inventory_followup_implementation_sha256() -> str:
    return _file_sha256(_SESSION_INVENTORY_FOLLOWUP_FILE)
...
class SessionInventoryFollowupConfiguration:
    profile: str
    signal_sign: int
    holding_bars: int
    ...
    def __post_init__(self) -> None:
        if self.profile not in _PROFILE_SPECS:
            raise ValueError("session-inventory profile is not registered")
        if self.signal_sign not in {-1, 1}:
            raise ValueError("signal_sign must be -1 or 1")
        if self.holding_bars not in {24, 72}:
            raise ValueError("holding_bars is not registered")

    @property
    def lookback_bars(self) -> int:
        return int(_PROFILE_SPECS[self.profile]["lookback_bars"])

    @property
    def decision_hour(self) -> int:
        return int(_PROFILE_SPECS[self.profile]["decision_hour"])

    @property
    def decision_minute(self) -> int:
        return int(_PROFILE_SPECS[self.profile]["decision_minute"])

    @property
    def target_open_hour(self) -> int:
        return int(_PROFILE_SPECS[self.profile]["target_open_hour"])
    ...
    def semantic_parameters(self) -> dict[str, Any]:
        return {
            "broker_clock_semantics": "fixed_no_dst_or_cash_session_claim",
            "clock_surface_basis": "verified_registered_opportunities",
            "decision_hour": self.decision_hour,
            "decision_minute": self.decision_minute,
            "direction_policy": "both",
            "holding_bars": self.holding_bars,
            "inventory_measure": "signed_return_over_realized_path_energy",
            "lookback_bars": self.lookback_bars,
            "signal_sign": self.signal_sign,
            "target_next_open_hour": self.target_open_hour,
            "threshold_quantile_bp": SELECTOR_QUANTILE_BP,
        }


def session_inventory_followup_configurations() -> tuple[SessionInventoryFollowupConfiguration, ...]:
    return tuple(
        ...
        for holding_bars in (24, 72)
    )


def _local_implementation(function_name: str) -> str:
    return (
        f"axiom_rift.research.session_inventory_followup_discovery.{function_name}@sha256:"
        f"{session_inventory_followup_implementation_sha256()}"
    )
...
def session_inventory_followup_components() -> tuple[ComponentSpec, ...]:
    return (
        ComponentSpec(
            display_name="causal completed-bar session-inventory follow-up score",
            protocol="feature.fixed_broker_clock_session_inventory_followup.v1",
            implementation=_local_implementation(
                "compute_session_inventory_followup_score"
            ),
            spec={
                "availability": "bar_open_plus_5m_after_completed_bar",
                "broker_clock": "fixed_recorded_clock",
                "clock_surface_basis": "verified_registered_opportunities",
                "cash_session_claim": False,
                "dst_claim": False,
                "economic_label_claim": False,
                "inventory_measure": "signed_return_over_realized_path_energy",
                "nonconsecutive_action": "invalid_until_lookback_rewarm",
                "opportunity_rule": "exact_registered_bar_ending_at_target_open",
                "parameter_fields": [
                    "decision_hour",
                    "decision_minute",
                    "lookback_bars",
                    "target_next_open_hour",
                ],
                "profiles": {name: dict(spec) for name, spec in _PROFILE_SPECS.items()},
            },
        ),
        ComponentSpec(
            display_name="fold isolated sparse session-inventory selector",
        ...
        ComponentSpec(
            display_name="completed-bar next-open session-inventory follow-fade entry",
            protocol="trade.fixed_broker_transition_next_open_inventory.v1",
            ...
            spec={
                ...
                "target_opens": ["02:00", "08:00", "15:00"],
...
def session_inventory_followup_executable(configuration: SessionInventoryFollowupConfiguration) -> ExecutableSpec:
    return ExecutableSpec(
        display_name=f"session-inventory follow-up {configuration.configuration_id}",
        components=session_inventory_followup_components(),
        ...
        clock_contract=(
            "clock:fpmarkets_m5_fixed_broker_transition_completed_plus_5m_"
        ...
        engine_contract=(
            "engine:session_inventory_followup_discovery_v1:python3_13_9:numpy2_3_4:"
            "pandas2_3_3:scipy1_16_3:"
            f"session_inventory_followup_sha256_{session_inventory_followup_implementation_sha256()}:"
...
def session_inventory_followup_executable_configuration_map() -> dict[str, SessionInventoryFollowupConfiguration]:
    return {
        session_inventory_followup_executable(configuration).identity: configuration
        for configuration in session_inventory_followup_configurations()
    }


def executable_configuration_map() -> dict[str, SessionInventoryFollowupConfiguration]:
    return session_inventory_followup_executable_configuration_map()


def compute_session_inventory_followup_score(
    frame: pd.DataFrame,
    profile: str,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Return a completed-return inventory score at one exact broker transition."""

    if type(profile) is not str or profile not in _PROFILE_SPECS:
        raise ValueError("session-inventory profile is not registered")
    spec = _PROFILE_SPECS[profile]
    lookback = int(spec["lookback_bars"])
    decision_hour = int(spec["decision_hour"])
    decision_minute = int(spec["decision_minute"])
    time = pd.to_datetime(frame["time"], errors="raise")
    if time.dt.tz is not None:
        raise ValueError("session-inventory requires timezone-naive broker clock")
    ...
    return_series = pd.Series(log_return)
    cumulative_return = return_series.rolling(
    ...
    score = np.divide(
        ...
        where=np.isfinite(path_energy) & (path_energy > 0),
    )
    opportunity = (
        (time.dt.hour.to_numpy() == decision_hour)
        & (time.dt.minute.to_numpy() == decision_minute)
        & (time.dt.second.to_numpy() == 0)
        & (time.dt.microsecond.to_numpy() == 0)
        & (time.dt.nanosecond.to_numpy() == 0)
    )
    score[~opportunity] = np.nan
    volatility = (
        pd.Series(log_return).rolling(48, min_periods=48).std(ddof=1).to_numpy(dtype=float)
    )
    score[run < lookback + 1] = np.nan
...
class _ConfigurationResult:
    configuration: SessionInventoryFollowupConfiguration
    executable_id: str
...
def _evaluate_configuration(
    *,
    ...
    score, volatility, run = compute_session_inventory_followup_score(
    ...
    for fold in folds:
        ...
        if len(train_volatility) < 100:
            raise SessionInventoryFollowupBoundaryError("regime calibration is too small")
        cutoffs = (
        ...
        prefix_score, prefix_volatility, prefix_run = (
            compute_session_inventory_followup_score(prefix_frame, configuration.profile)
        )
    ...
    return _ConfigurationResult(
        configuration,
        session_inventory_followup_executable(configuration).identity,
        metrics,
        fold_metrics,
        regime_metrics,
        session_metrics,
...
def _adjusted_bootstrap_upper_pvalue(values: np.ndarray, *, seed_label: str) -> int:
    ...
    if len(sample) < 30 or np.any(~np.isfinite(sample)):
        raise SessionInventoryFollowupBoundaryError("bootstrap series is invalid or too short")
...
def _matched_result(
    ...
    *,
    ...
    if len(matches) != 1:
        raise SessionInventoryFollowupBoundaryError("registered control match is not unique")
    return matches[0]
...
def _paired_pvalue(subject: _ConfigurationResult, control: _ConfigurationResult, role: str) -> int:
    if not subject.daily_pnl.index.equals(control.daily_pnl.index):
        raise SessionInventoryFollowupBoundaryError("paired controls have different eligible days")
...
def _populate_pvalues_and_controls(results: Sequence[_ConfigurationResult]) -> None:
    for subject in results:
        ...
        if any(type(value) is not int for value in subject.metrics.values()):
            raise SessionInventoryFollowupBoundaryError("scientific metrics are not fixed-point integers")
...
def _claim_limits() -> list[str]:
    return [
        ...
        "monthly_drawdown_share_gate_is_for_sparse_session_inventory_followup_surfaces_only",
        "opportunities_are_fixed_broker_clock_descriptions_not_economic_sessions",
        "no_dst_cash_session_or_economic_label_claim",
...
def _compute_registered_session_inventory_followup_surface(repository_root: str | Path) -> dict[str, Any]:
    if not isinstance(repository_root, (str, Path)):
        raise SessionInventoryFollowupBoundaryError(
            "session-inventory surface requires a repository path"
        )
    _validate_engine_environment()
    ...
    surface: dict[str, Any] = {
        ...
        "discovery_implementation_sha256": session_inventory_followup_implementation_sha256(),
        "engine_environment": {
        ...
        "schema": "session_inventory_followup_surface.v1",
        "selection_context": [
            {
...
def project_session_inventory_followup_evaluation(
    ...
    *,
    ...
    for name, digest in (
        ...
        if type(digest) is not str or len(digest) != 64 or any(c not in "0123456789abcdef" for c in digest):
            raise SessionInventoryFollowupBoundaryError(f"{name} hash is invalid")
    if not isinstance(job_execution, Mapping) or set(job_execution) != {
        "identity", "job_hash", "job_id", "job_permit_id", "start_record_id"
    }:
        raise SessionInventoryFollowupBoundaryError("Job execution binding is invalid")
    ...
    if job_execution["identity"] != canonical_digest(domain="running-job-execution", payload=payload):
        raise SessionInventoryFollowupBoundaryError("Job execution identity is invalid")
    value = dict(surface)
    if sha256(canonical_bytes(value)).hexdigest() != surface_artifact_hash:
        raise SessionInventoryFollowupBoundaryError("surface bytes differ from their artifact hash")
    expected_fields = {
    ...
    if set(value) != expected_fields or value.get("schema") != "session_inventory_followup_surface.v1":
        raise SessionInventoryFollowupBoundaryError("session-inventory surface schema is invalid")
    expected = session_inventory_followup_executable_configuration_map()
    evaluations = value.get("evaluations")
    if not isinstance(evaluations, list) or len(evaluations) != len(expected):
        raise SessionInventoryFollowupBoundaryError(
            "session-inventory surface evaluation count is invalid"
        )
    ...
    if len(by_identity) != len(evaluations) or set(by_identity) != set(expected) or subject_executable_id not in expected:
        raise SessionInventoryFollowupBoundaryError(
            "session-inventory surface subjects differ from registration"
        )
    for identity, configuration in expected.items():
        if by_identity[identity].get("subject_configuration_id") != configuration.configuration_id:
            raise SessionInventoryFollowupBoundaryError(
                "session-inventory surface configuration binding differs"
            )
    evaluation = {
        **dict(by_identity[subject_executable_id]),
        "claim_limits": value["claim_limits"],
        "job_execution": dict(job_execution),
        "schema": "session_inventory_followup_evaluation.v1",
...
__all__ = [
    ...
    "SessionInventoryFollowupBoundaryError", "SessionInventoryFollowupConfiguration", "SELECTOR_QUANTILE_BP",
    "SELECTION_BLOCK_LENGTHS", "SELECTION_BOOTSTRAP_SAMPLES",
    "SELECTION_MONTE_CARLO_CONFIDENCE_PPM", "SELECTION_SEED",
    "SELECTION_TOTAL_EXPOSURES", "_compute_registered_session_inventory_followup_surface",
    "calibrate_selector", "compute_session_inventory_followup_score",
    "executable_configuration_map",
    "loader_implementation_sha256", "project_session_inventory_followup_evaluation",
    "session_inventory_followup_components", "session_inventory_followup_configurations", "session_inventory_followup_executable",
    "session_inventory_followup_executable_configuration_map", "session_inventory_followup_implementation_sha256",
