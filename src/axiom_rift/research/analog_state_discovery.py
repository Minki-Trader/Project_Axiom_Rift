"""Fold-trained nearest historical analog-state discovery."""
...
SELECTION_TOTAL_EXPOSURES=492;SELECTOR_QUANTILE_BP=8_500;HORIZON=24;NEIGHBORS=25;LIBRARY_STRIDE=12;_PROFILES=("knn_multiscale_state_25","knn_return_control_25");_THIS_FILE=Path(__file__).resolve()
...
def _raw_features(frame:pd.DataFrame,profile:str)->tuple[np.ndarray,np.ndarray,np.ndarray]:
    ...
    close=frame["close"].to_numpy(float);log=np.log(close);ret=np.full(len(close),np.nan);ret[1:]=np.diff(log);series=pd.Series(ret);vol192=series.rolling(192,min_periods=192).std(ddof=1).to_numpy(float);columns=[]
    ...
    if profile=="knn_multiscale_state_25":
        vol48=series.rolling(48,min_periods=48).std(ddof=1).to_numpy(float);columns.append(np.divide(vol48,vol192,out=np.full(len(close),np.nan),where=np.isfinite(vol192)&(vol192>0))-1.0);peak=pd.Series(close).rolling(576,min_periods=576).max().to_numpy(float);columns.append(np.divide(close,peak,out=np.full(len(close),np.nan),where=np.isfinite(peak)&(peak>0))-1.0)
...
def compute_registered_analog_surface(repository_root:str|Path)->dict[str,Any]:
    ...
    feature_sets={};prefix_sets={};calibration_sets={}
    for profile in _PROFILES:
        feature_sets[profile]={};prefix_sets[profile]={};calibration_sets[profile]={}
        for f in folds:
            fid=str(f["fold_id"]);train=f["train_is"];start=pd.Timestamp(train["start"]);end=pd.Timestamp(train["end"]);value=fit_fold_analog(frame,profile,start,end);prefix=fit_fold_analog(prefix_frames[fid],profile,start,end);feature_sets[profile][fid]=value;prefix_sets[profile][fid]=prefix;mask=((time>=start)&(time<=end)).to_numpy();pt=pd.to_datetime(prefix_frames[fid]["time"],errors="raise");pm=((pt>=start)&(pt<=end)).to_numpy();vv=value[1][mask&np.isfinite(value[1])];cutoffs=(float(np.quantile(vv,1/3,method="higher")),float(np.quantile(vv,2/3,method="higher")));calibration_sets[profile][fid]=(calibrate_selector(value[0],mask),cutoffs,calibrate_selector(prefix[0],pm))
    results=[]
    for c in analog_configurations():
        first=feature_sets[c.profile][str(folds[0]["fold_id"])];results.append(_evaluate_configuration(calibrations=calibration_sets[c.profile],frame=frame,features=first,fold_features=feature_sets[c.profile],folds=folds,configuration=c,effective_spread=spread,prefix_features=prefix_sets[c.profile],prefix_spreads=prefix_spreads,time=time,executable_id=analog_executable(c).identity))
...
def project_analog_evaluation(surface:Mapping[str,Any],*,job_execution:Mapping[str,str],subject_executable_id:str,surface_artifact_hash:str,surface_manifest_hash:str)->dict[str,Any]:
    ...
    result={**dict(by[subject_executable_id]),"claim_limits":value["claim_limits"],"job_execution":dict(job_execution),"schema":"analog_state_evaluation.v1","selection_context":value["selection_context"],"selection_method":value["selection_method"],"session_semantics":value["session_semantics"],"surface_artifact_hash":surface_artifact_hash,"surface_manifest_hash":surface_manifest_hash};canonical_bytes(result);return result
__all__=["analog_configurations","analog_executable","analog_implementation_sha256","compute_registered_analog_surface","executable_configuration_map","fit_fold_analog","loader_implementation_sha256","project_analog_evaluation"]
