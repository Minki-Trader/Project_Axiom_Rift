...
def _load(writer:RunningJobExecutionContext,inputs:tuple[str,...])->tuple[dict[str,Any],str,str]:
    binding=read_surface_manifest_evidence_inputs(writer.evidence,inputs,surface_schema="positive_direction_sleeve_surface.v1",manifest_schema="positive_direction_sleeve_surface_manifest.v1",expected_surface_implementation_sha256=positive_direction_sleeve_discovery_implementation_sha256())
...
def execute_positive_direction_sleeve_job(*,repository_root:str|Path,execution:RunningJobExecution)->PositiveDirectionSleeveJobPacket:
    root=Path(repository_root).resolve();writer=RunningJobExecutionContext(root);binding=writer.verify_running_job_execution(execution,expected_callable_identity=CALLABLE_IDENTITY);spec=binding["spec"];subject=spec.get("evidence_subject");mission_id,study_id=binding.get("mission_id"),binding.get("study_id")
    ...
    else:surface,sh,mh=_load(writer,tuple(identity for identity in inputs if identity not in required))
