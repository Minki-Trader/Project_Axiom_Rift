"""Writer-gated evidence Job for fold-trained analog states."""
...
from axiom_rift.research.analog_state_discovery import analog_implementation_sha256,compute_registered_analog_surface,executable_configuration_map,loader_implementation_sha256,project_analog_evaluation
...
MISSION_ID="MIS-0005";STUDY_ID="STU-0061";CALLABLE_IDENTITY="axiom_rift.research.analog_state_study.execute_analog_job.v1";EVIDENCE_DEPTH="discovery"
def build_analog_validation_plan(eid:str)->dict[str,object]:return build_validation_plan(mission_id=MISSION_ID,executable_id=eid,evidence_depth=EVIDENCE_DEPTH,planned_claims=PLANNED_CLAIMS,evidence_modes=EVIDENCE_MODES,criteria=CRITERIA,candidate_eligible_on_pass=False)
...
def _load(w:StateWriter,hashes:tuple[str,...])->tuple[dict[str,Any],str,str]:
    ...
    for h in hashes:
        ...
        if isinstance(v,dict) and v.get("schema")=="analog_state_surface.v1":surface=(v,h)
        if isinstance(v,dict) and v.get("schema")=="analog_state_surface_manifest.v1":manifest=(v,h)
    if surface is None or manifest is None or manifest[0].get("surface_artifact_hash")!=surface[1]:raise ValueError("analog surface missing")
    return surface[0],surface[1],manifest[1]
def execute_analog_job(*,repository_root:str|Path,execution:RunningJobExecution)->AnalogJobPacket:
    ...
    eid=subject["id"];plan=build_analog_validation_plan(eid);env=build_environment_manifest();ph=sha256(canonical_bytes(plan)).hexdigest();names=output_names(eid);inputs=tuple(spec["input_hashes"]);required={DATASET_SHA256,OBSERVED_MATERIAL_ID,ROLLING_SPLIT_SHA256,ph,analog_implementation_sha256(),loader_implementation_sha256(),discovery_implementation_sha256()}
