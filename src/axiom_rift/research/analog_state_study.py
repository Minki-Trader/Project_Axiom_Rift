...
MISSION_ID="MIS-0005";STUDY_ID="STU-0062";CALLABLE_IDENTITY="axiom_rift.research.analog_state_study.execute_analog_job.v2";EVIDENCE_DEPTH="discovery"
...
def _load(w:StateWriter,hashes:tuple[str,...])->tuple[dict[str,Any],str,str]:
    ...
    for h in hashes:
        ...
        if isinstance(v,dict) and v.get("schema")=="analog_state_surface.v2":surface=(v,h)
        if isinstance(v,dict) and v.get("schema")=="analog_state_surface_manifest.v2":manifest=(v,h)
...
def execute_analog_job(*,repository_root:str|Path,execution:RunningJobExecution)->AnalogJobPacket:
    ...
    if produces:
        surface=compute_registered_analog_surface(root);sh=w.evidence.finalize(canonical_bytes(surface)).sha256;mh=w.evidence.finalize(canonical_bytes({"schema":"analog_state_surface_manifest.v2","surface_artifact_hash":sh,"surface_implementation_sha256":analog_implementation_sha256()})).sha256
