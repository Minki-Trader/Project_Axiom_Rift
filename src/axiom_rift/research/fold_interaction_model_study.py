"""Writer-gated evidence Job for the fold-interaction model contrast."""
...
from axiom_rift.research.fold_interaction_model_discovery import (
...
CALLABLE_IDENTITY = (
    "axiom_rift.research.fold_interaction_model_study."
    "execute_fold_interaction_model_job.v1"
)
EVIDENCE_DEPTH = "discovery"
_CONTROL_DELTA_METRIC = "model_control_delta_net_profit_micropoints"
_CONTROL_PVALUE_METRIC = "model_control_pvalue_upper_ppm"
...
def build_fold_interaction_model_validation_plan(
    executable_id: str,
    *,
    mission_id: str,
) -> dict[str, object]:
...
def surface_output_name(*, study_id: str) -> str:
    return f"scientific/{study_id}/fold-interaction-model-surface.json"


def surface_manifest_output_name(*, study_id: str) -> str:
    return f"scientific/{study_id}/fold-interaction-model-surface-manifest.json"
...
class FoldInteractionModelJobPacket:
...
def _load_surface(
    ...
    for artifact_hash in input_hashes:
        try:
            ...
            continue
        if isinstance(value, dict) and value.get("schema") == "fold_interaction_model_surface.v1":
            surface = (value, artifact_hash)
        if isinstance(value, dict) and value.get("schema") == "fold_interaction_model_surface_manifest.v1":
            manifest = (value, artifact_hash)
    if surface is None or manifest is None or manifest[0].get("surface_artifact_hash") != surface[1]:
        raise ValueError("fold-interaction model surface is missing")
    return surface[0], surface[1], manifest[1]


def execute_fold_interaction_model_job(
    *,
    repository_root: str | Path,
    execution: RunningJobExecution,
) -> FoldInteractionModelJobPacket:
    ...
    if (
        ...
        raise ValueError("fold-interaction model binding is invalid")
    executable_id = subject["id"]
    plan = build_fold_interaction_model_validation_plan(
    ...
    if not required.issubset(inputs):
        raise ValueError("fold-interaction model inputs are missing")
    expected = set(spec["expected_outputs"])
    produces_surface = surface_output_name(study_id=study_id) in expected
    if produces_surface:
        surface = compute_registered_fold_interaction_model_surface(root)
        surface_hash = writer.evidence.finalize(canonical_bytes(surface)).sha256
        manifest_value = {
            "schema": "fold_interaction_model_surface_manifest.v1",
            "surface_artifact_hash": surface_hash,
            "surface_implementation_sha256": fold_interaction_model_discovery_implementation_sha256(),
        }
    ...
    evaluation = project_fold_interaction_model_evaluation(
        surface,
    ...
    return FoldInteractionModelJobPacket(
...
__all__ = [
    ...
    "build_fold_interaction_model_validation_plan",
    "execute_fold_interaction_model_job",
    "output_names",
