"""Declarative evidence plan for the registered trend-contrast Study."""
...
MISSION_ID = "MIS-0001"
STUDY_ID = "STU-0002"
CALLABLE_IDENTITY = "axiom_rift.research.trend_study.execute_trend_job.v3"
...
def build_trend_validation_plan(executable_id: str) -> dict[str, object]:
...
def _claim_metrics(evaluation: Mapping[str, Any]) -> dict[str, dict[str, int | None]]:
    ...
    if not isinstance(raw, Mapping):
        raise ValueError("trend evaluation has no metrics")
    ...
    if any(type(name) is not str or type(value) is not int for name, value in metrics.items()):
        raise ValueError("trend evaluation metrics must be integer scalars")
    ...
    def values(*names: str, null_when_not_evaluable: bool = False) -> dict[str, int | None]:
        ...
        for name in names:
            if name not in metrics:
                raise ValueError(f"trend evaluation metric is absent: {name}")
            result[name] = (
...
def output_names(executable_id: str) -> dict[str, str]:
    ...
    prefix = f"scientific/STU-0002/{short}"
    return {
...
def surface_cache_output_name() -> str:
    return (
        f"local/cache/{STUDY_ID}/trend-surface-"
        f"{discovery_implementation_sha256()[:16]}.json"
    )


def surface_manifest_output_name() -> str:
    return f"scientific/{STUDY_ID}/trend-surface-cache-manifest.json"
...
class TrendJobPacket:
    ...
    def artifact(self, name: str) -> dict[str, Any]:
        ...
        if not isinstance(value, dict):
            raise ValueError("trend Job artifact is not an object")
        return value
...
def _materialize_cache(
    ...
    *,
    ...
    if cache_root not in target.parents:
        raise ValueError("trend surface cache path escapes local/cache")
    expected_hash = sha256(content).hexdigest()
    if target.exists():
        if not target.is_file() or sha256(target.read_bytes()).hexdigest() != (
            expected_hash
        ):
            raise ValueError("existing trend surface cache has different bytes")
        return
    target.parent.mkdir(parents=True, exist_ok=True)
    descriptor, temporary_name = tempfile.mkstemp(
        prefix=".trend-surface-",
        suffix=".tmp",
        dir=target.parent,
    )
...
def _build_surface_manifest(
    *,
    ...
    value: dict[str, Any] = {
        ...
        "executable_ids": sorted(executable_configuration_map()),
        "loader_implementation_sha256": loader_implementation_sha256(),
        "material_identity": OBSERVED_MATERIAL_ID,
        "producer_execution": {
            **execution.payload(),
        ...
        "schema": "trend_surface_cache_manifest.v1",
        "split_artifact_sha256": ROLLING_SPLIT_SHA256,
    }
    canonical_bytes(value)
    return value
...
def _validate_surface_manifest(
    ...
    *,
    ...
    if (
        ...
        or value.get("schema") != "trend_surface_cache_manifest.v1"
        ...
        or value.get("discovery_implementation_sha256")
        != discovery_implementation_sha256()
        or value.get("environment_sha256") != environment_hash
        or value.get("executable_ids") != sorted(executable_configuration_map())
        ...
        not in executable_configuration_map()
        or value.get("split_artifact_sha256") != ROLLING_SPLIT_SHA256
    ):
        raise ValueError("trend surface cache manifest differs from Job inputs")
    ...
    if not isinstance(producer, dict) or set(producer) != {
        ...
        raise ValueError("trend surface producer execution is invalid")
    ...
    if producer["identity"] != execution.identity:
        raise ValueError("trend surface producer identity is invalid")
    return value
    ...
    *,
    ...
    for input_hash in input_hashes:
        ...
        if isinstance(value, dict) and value.get("schema") == (
            "trend_surface_cache_manifest.v1"
        ):
            matches.append(
                (
                    input_hash,
    ...
    if len(matches) != 1:
        raise ValueError("Job inputs require one exact trend surface manifest")
    return matches[0]


def execute_trend_job(
    *,
    ...
    if (
        ...
        or evidence_subject.get("id") not in executable_configuration_map()
    ):
        raise ValueError("running Job is not bound to the registered trend Study")
    executable_id = evidence_subject["id"]
    scientific_binding = spec.get("scientific_binding")
    if not isinstance(scientific_binding, dict):
        raise ValueError("trend Job lacks a scientific binding")
    plan = build_trend_validation_plan(executable_id)
    ...
    if not required_inputs.issubset(inputs):
        raise ValueError("trend Job omits a required content-bound input")
    if scientific_binding.get("validation_plan_hash") != plan_hash:
        raise ValueError("trend validation plan differs from Job input")
    ...
    if (
        ...
        raise ValueError("trend scientific binding differs from preregistration")
    ...
    if not isinstance(output_classes, dict):
        raise ValueError("trend Job output classes are invalid")
    ...
    if expected_outputs != expected_set or output_classes != expected_classes:
        raise ValueError("trend Job outputs differ from the producer/projector protocol")

    manifest_artifact_hash: str
    manifest_bytes: bytes | None = None
    if produces_surface:
        surface = _compute_registered_trend_surface(root)
        ...
        if not cache_target.is_file():
            raise ValueError("trend surface cache is unavailable")
        surface_bytes = cache_target.read_bytes()
        surface_hash = sha256(surface_bytes).hexdigest()
        if surface_hash not in inputs:
            raise ValueError("trend surface cache hash is not a Job input")
        ...
        if not isinstance(surface, dict):
            raise ValueError("trend surface cache is not a canonical object")

    context = project_trend_evaluation(
        surface,
    ...
    if set(output_manifest) != expected_outputs:
        raise ValueError("materialized trend outputs differ from Job declaration")
    return TrendJobPacket(
