"""Writer-gated evidence Job for causal drawdown state."""
...
from axiom_rift.research.drawdown_state_discovery import compute_registered_drawdown_surface,drawdown_implementation_sha256,executable_configuration_map,loader_implementation_sha256,project_drawdown_evaluation
...
MISSION_ID="MIS-0004";STUDY_ID="STU-0048";CALLABLE_IDENTITY="axiom_rift.research.drawdown_state_study.execute_drawdown_job.v1";EVIDENCE_DEPTH="discovery"
def build_drawdown_validation_plan(eid:str)->dict[str,object]:return build_validation_plan(mission_id=MISSION_ID,executable_id=eid,evidence_depth=EVIDENCE_DEPTH,planned_claims=PLANNED_CLAIMS,evidence_modes=EVIDENCE_MODES,criteria=CRITERIA,candidate_eligible_on_pass=False)
...
def _load(w:StateWriter,hashes:tuple[str,...])->tuple[dict[str,Any],str,str]:
    ...
    for h in hashes:
        ...
        if isinstance(v,dict) and v.get("schema")=="drawdown_state_surface.v1":s=(v,h)
        if isinstance(v,dict) and v.get("schema")=="drawdown_surface_manifest.v1":m=(v,h)
    if s is None or m is None or m[0].get("surface_artifact_hash")!=s[1]:raise ValueError("drawdown surface missing")
    return s[0],s[1],m[1]
def execute_drawdown_job(*,repository_root:str|Path,execution:RunningJobExecution)->DrawdownJobPacket:
    ...
    if binding.get("mission_id")!=MISSION_ID or binding.get("study_id")!=STUDY_ID or not isinstance(subject,dict) or subject.get("id") not in executable_configuration_map():raise ValueError("drawdown binding invalid")
    eid=subject["id"];plan=build_drawdown_validation_plan(eid);env=build_environment_manifest();ph=sha256(canonical_bytes(plan)).hexdigest();names=output_names(eid);inputs=tuple(spec["input_hashes"]);required={DATASET_SHA256,OBSERVED_MATERIAL_ID,ROLLING_SPLIT_SHA256,ph,drawdown_implementation_sha256(),loader_implementation_sha256(),discovery_implementation_sha256()}
    if not required.issubset(inputs):raise ValueError("drawdown inputs missing")
    expected=set(spec["expected_outputs"]);produces=surface_output_name() in expected
    if produces:
        surface=compute_registered_drawdown_surface(root);sh=w.evidence.finalize(canonical_bytes(surface)).sha256;mh=w.evidence.finalize(canonical_bytes({"schema":"drawdown_surface_manifest.v1","surface_artifact_hash":sh,"surface_implementation_sha256":drawdown_implementation_sha256()})).sha256
    else:surface,sh,mh=_load(w,inputs)
    evaluation=project_drawdown_evaluation(surface,job_execution={**execution.payload(),"identity":execution.identity},subject_executable_id=eid,surface_artifact_hash=sh,surface_manifest_hash=mh);eh=w.evidence.finalize(canonical_bytes(evaluation)).sha256;measurement=build_measurement(executable_id=eid,job_id=execution.job_id,job_hash=execution.job_hash,evaluation_artifact_hash=eh,evaluation=evaluation);meash=w.evidence.finalize(canonical_bytes(measurement)).sha256;result=build_result_manifest(executable_id=eid,job_id=execution.job_id,job_hash=execution.job_hash,measurement_artifact_hash=meash);outputs={names["context"]:eh,names["environment"]:w.evidence.finalize(canonical_bytes(env)).sha256,names["measurement"]:meash,names["plan"]:w.evidence.finalize(canonical_bytes(plan)).sha256,names["result"]:w.evidence.finalize(canonical_bytes(result)).sha256}
    if produces:outputs[surface_output_name()]=sh;outputs[surface_manifest_output_name()]=mh
    return DrawdownJobPacket(output_manifest=tuple(sorted(outputs.items())),verdict=planned_verdict(plan,measurement))
__all__=["CALLABLE_IDENTITY","EVIDENCE_DEPTH","EVIDENCE_MODES","PLANNED_CLAIMS","STUDY_ID","build_drawdown_validation_plan","build_environment_manifest","execute_drawdown_job","output_names","surface_manifest_output_name","surface_output_name"]
