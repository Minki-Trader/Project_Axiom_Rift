"""Journal-bound source eligibility Jobs for FPMarkets USDJPY M5."""
...
from axiom_rift.research.usdjpy_source_eligibility_validation import (
    SOURCE_ELIGIBILITY_VALIDATOR_ID,
)


HISTORICAL_CALLABLE_IDENTITY = (
    "axiom_rift.research.usdjpy_source_study."
    "execute_usdjpy_historical_audit_job.v1"
)
RUNTIME_CALLABLE_IDENTITY = (
    "axiom_rift.research.usdjpy_source_study."
    "execute_usdjpy_runtime_availability_job.v1"
)
...
def output_names(transition_evidence: str) -> dict[str, str]:
    if transition_evidence == "historical_audit":
        return {
            "raw": "source/usdjpy/historical.csv",
            "measurement": "source/usdjpy/historical-audit.json",
            "result": "source/usdjpy/historical-result.json",
        }
    if transition_evidence == "runtime_availability_proof":
        return {
            "measurement": "source/usdjpy/runtime-probe.json",
            "result": "source/usdjpy/runtime-result.json",
        }
...
def _execute(
    *,
    ...
    contract_id = usdjpy_source_contract().source_contract_id
    ...
    if (
        ...
        raise ValueError(
            "running Job is not bound to the registered USDJPY source transition"
        )
    spec = binding["spec"]
    required_inputs = {
    ...
    if not required_inputs.issubset(spec.get("input_hashes", [])):
        raise ValueError("USDJPY source Job omits a required content-bound input")
    ...
    if (
        ...
        raise ValueError("USDJPY source Job outputs differ from registration")

    artifact_bytes: dict[str, bytes]
    if transition_evidence == "historical_audit":
        raw, measurement = build_usdjpy_historical_audit(root)
        artifact_bytes = {
            "raw": raw,
        ...
        measurement = probe_usdjpy_runtime(root)
    ...
    if set(output_manifest) != expected_outputs:
        raise ValueError("USDJPY source materialization differs from declaration")
...
def execute_usdjpy_historical_audit_job(
...
def execute_usdjpy_runtime_availability_job(
    *, repository_root: str | Path, execution: RunningJobExecution
) -> SourceJobPacket:
    return _execute(
        repository_root=repository_root,
...
__all__ = [
    ...
    "SOURCE_ELIGIBILITY_VALIDATOR_ID",
    "SourceJobPacket",
    "execute_usdjpy_historical_audit_job",
    "execute_usdjpy_runtime_availability_job",
    "output_names",
    "source_dependency_sha256",
    "source_study_implementation_sha256",
    "source_validator_implementation_sha256",
