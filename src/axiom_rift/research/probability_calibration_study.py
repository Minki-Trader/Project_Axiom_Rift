"""Writer-gated evidence Job for validation-only probability calibration."""
...
STUDY_ID = "STU-0067"
CALLABLE_IDENTITY = (
    "axiom_rift.research.probability_calibration_study."
    "execute_probability_calibration_job.v1"
)
EVIDENCE_DEPTH = "discovery"


def build_probability_calibration_validation_plan(
    executable_id: str,
) -> dict[str, object]:
...
def surface_output_name() -> str:
    return f"scientific/{STUDY_ID}/probability-calibration-surface.json"


def surface_manifest_output_name() -> str:
    return f"scientific/{STUDY_ID}/probability-calibration-surface-manifest.json"
...
class ProbabilityCalibrationJobPacket:
...
def _load_surface(
    ...
    for artifact_hash in input_hashes:
        ...
        if (
            ...
            and value.get("schema") == "probability_calibration_surface.v1"
        ):
            surface = (value, artifact_hash)
        if (
            isinstance(value, dict)
            and value.get("schema") == "probability_calibration_surface_manifest.v1"
        ):
    ...
    if (
        ...
        raise ValueError("probability-calibration surface is missing")
    return surface[0], surface[1], manifest[1]


def execute_probability_calibration_job(
    *,
    repository_root: str | Path,
    execution: RunningJobExecution,
) -> ProbabilityCalibrationJobPacket:
    ...
    if (
        ...
        raise ValueError("probability-calibration binding is invalid")
    executable_id = subject["id"]
    plan = build_probability_calibration_validation_plan(executable_id)
    ...
    if not required.issubset(inputs):
        raise ValueError("probability-calibration inputs are missing")
    expected = set(spec["expected_outputs"])
    produces_surface = surface_output_name() in expected
    if produces_surface:
        surface = compute_registered_probability_calibration_surface(root)
        surface_hash = writer.evidence.finalize(canonical_bytes(surface)).sha256
        manifest_value = {
            "schema": "probability_calibration_surface_manifest.v1",
    ...
    evaluation = project_probability_calibration_evaluation(
        surface,
    ...
    return ProbabilityCalibrationJobPacket(
...
__all__ = [
    ...
    "build_probability_calibration_validation_plan",
    "execute_probability_calibration_job",
    "output_names",
