...
def drawdown_components()->tuple[ComponentSpec,...]:
    ...
    return (
        ComponentSpec(display_name="causal drawdown phase interaction",protocol="feature.causal_drawdown_phase.v1",implementation=_local("compute_drawdown_score"),spec={"availability":"completed_bar_close","lookback_bars":LOOKBACK,"recovery_velocity_bars":12,"profiles":list(_PROFILES),"parameter_fields":["profile"]}),
        ComponentSpec(display_name="fold isolated extreme selector",protocol="selector.fold_train_abs_quantile.v5",implementation=_local("calibrate_selector"),spec={"calibration_role":"train_is_only","minimum_train_observations":1000,"quantile_basis_points":SELECTOR_QUANTILE_BP,"quantile_method":"higher"}),
        ...
        ComponentSpec(display_name="fixed-hold nonoverlap lifecycle",protocol="lifecycle.fixed_hold_no_overlap.v6",implementation=_shared("simulate_fixed_hold"),spec={"entry_overlap":"reject_while_position_slot_is_occupied","exit_surface":"exact_bar_open_after_12_bars","gap_action":"exclude_path","unknown_entry_reservation":False}),
