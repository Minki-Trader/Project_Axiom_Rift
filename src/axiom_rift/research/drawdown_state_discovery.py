...
SELECTION_TOTAL_EXPOSURES=440;SELECTOR_QUANTILE_BP=7_000;LOOKBACK=288;_PROFILES=("drawdown_depth_288","drawdown_duration_288");_THIS_FILE=Path(__file__).resolve()
...
class DrawdownConfiguration:
    ...
    def __post_init__(self)->None:
        if self.profile not in _PROFILES or self.signal_sign not in {-1,1} or self.holding_bars!=24 or self.unknown_entry_action!="cancel_before_open":raise ValueError("drawdown configuration invalid")
    @property
    def configuration_id(self)->str:return f"{self.profile}-{'deterioration' if self.signal_sign==1 else 'recovery'}-h24"
    def semantic_parameters(self)->dict[str,Any]:return {"holding_bars":24,"lookback_bars":LOOKBACK,"profile":self.profile,"selector_quantile_bp":SELECTOR_QUANTILE_BP,"signal_sign":self.signal_sign,"unknown_entry_action":self.unknown_entry_action}
...
def compute_drawdown_score(frame:pd.DataFrame,profile:str)->tuple[np.ndarray,np.ndarray,np.ndarray]:
    ...
    c=frame["close"].to_numpy(float);peak,age=_rolling_peak_age(c);raw=np.divide(c,peak,out=np.full(len(c),np.nan),where=np.isfinite(peak)&(peak>0))-1 if profile=="drawdown_depth_288" else -age
...
def project_drawdown_evaluation(surface:Mapping[str,Any],*,job_execution:Mapping[str,str],subject_executable_id:str,surface_artifact_hash:str,surface_manifest_hash:str)->dict[str,Any]:
    value=dict(surface)
    if sha256(canonical_bytes(value)).hexdigest()!=surface_artifact_hash or value.get("schema")!="drawdown_state_surface.v1":raise DiscoveryBoundaryError("drawdown surface invalid")
    ...
    result={**dict(by[subject_executable_id]),"claim_limits":value["claim_limits"],"job_execution":dict(job_execution),"schema":"drawdown_state_evaluation.v1","selection_context":value["selection_context"],"selection_method":value["selection_method"],"session_semantics":value["session_semantics"],"surface_artifact_hash":surface_artifact_hash,"surface_manifest_hash":surface_manifest_hash};canonical_bytes(result);return result
