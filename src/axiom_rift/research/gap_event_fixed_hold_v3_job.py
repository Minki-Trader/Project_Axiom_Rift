"""Runtime adapter for the distinct STU-0046 feasible-floor protocol."""
...
import axiom_rift.research.gap_event_fixed_hold_v3 as replay_module
import axiom_rift.research.gap_fixed_hold as legacy_module
...
from axiom_rift.research.gap_fixed_hold import GAP_FIXED_HOLD_CONTEXT_PARAMETER
...
CALLABLE_IDENTITY = (
    ...
    "execute_gap_event_fixed_hold_v3_job.v1"
)
JOB_IMPLEMENTATION_PROTOCOL = "python.source.gap_event_fixed_hold_v3.v1"
ARTIFACT_NAMESPACE = "gap-event-fixed-hold-v3"
...
def _definition(
    ...
    return gap_event_fixed_hold_v3_protocol_definition(context)
...
def _trace(
    ...
    return compute_gap_event_fixed_hold_v3_family_trace(
        repository_root,
        definition,
    )
...
RUNTIME_ADAPTER = FixedHoldReplayRuntimeAdapter(
        ...
        sorted(
            {
                ...
                Path(legacy_module.__file__).resolve(),
    ...
    context_parameter_name=GAP_FIXED_HOLD_CONTEXT_PARAMETER,
    bound_definition_builder=_definition,
    bound_trace_builder=_trace,
)


def gap_event_fixed_hold_v3_job_implementation_artifact() -> bytes:
    return fixed_hold_replay_job_implementation_artifact(RUNTIME_ADAPTER)


def gap_event_fixed_hold_v3_job_implementation_sha256() -> str:
    return fixed_hold_replay_job_implementation_sha256(RUNTIME_ADAPTER)


def materialize_gap_event_fixed_hold_v3_job_implementation(
...
def materialize_gap_event_fixed_hold_v3_running_job_repair_proof(
    writer: object,
    *,
    ...
    return materialize_running_job_implementation_repair_proof(
        ...
        adapter=RUNTIME_ADAPTER,
        explanation=(
            "repair the v3 implementation without changing the registered "
            "feasible-floor scientific protocol"
        ),
        verification_evidence_hashes=verification_evidence_hashes,
    )


def build_gap_event_fixed_hold_v3_job_plan(
    *,
    mission_id: str,
    study_id: str,
    executable_id: str,
...
def execute_gap_event_fixed_hold_v3_job(
    *,
