...
SELECTION_TOTAL_EXPOSURES=404;SELECTOR_QUANTILE_BP=9_000;_PROFILES=("ridge_multistate","ridge_price_control");_THIS_FILE=Path(__file__).resolve();_RIDGE=10.0;_HORIZON=48
...
class LearnedConfiguration:
    ...
    def __post_init__(self)->None:
        if self.profile not in _PROFILES or self.signal_sign not in {-1,1} or self.holding_bars!=48:raise ValueError("learned configuration invalid")
    @property
    def configuration_id(self)->str:return f"{self.profile}-{'model' if self.signal_sign==1 else 'inverse'}-h48"
    def semantic_parameters(self)->dict[str,Any]:return {"holding_bars":48,"profile":self.profile,"ridge_lambda":10,"selector_quantile_bp":SELECTOR_QUANTILE_BP,"signal_sign":self.signal_sign}
...
def project_learned_evaluation(surface:Mapping[str,Any],*,job_execution:Mapping[str,str],subject_executable_id:str,surface_artifact_hash:str,surface_manifest_hash:str)->dict[str,Any]:
    value=dict(surface)
    if sha256(canonical_bytes(value)).hexdigest()!=surface_artifact_hash or value.get("schema")!="learned_state_surface.v1":raise DiscoveryBoundaryError("surface invalid")
